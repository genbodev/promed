    ...
    return {
    	checkColumn: function(v, p, record)
		{
			...
			if ( v == 'gray' )
				...
				var style = 'x-grid3-check-col-non-border'+((String(v)=='true' || String(v)=='1')?'-on':'');
		...
		dirNumColumn: function(v, p, record)
		{
			...
			if(v && record.get('EvnDirection_Num') && record.get('EvnDirection_setDate')){
		...
		recordColumn: function(v, p, record)
		{
			...
			if(!v || !record.get('TimetableStac_insDT')){
				if(record.get('TimetableStac_setDate'))
					return '<div class="x-grid3-cc-'+this.id+'"><span style="color: #000079;">'+record.get('TimetableStac_setDate')+'</span></div>';
