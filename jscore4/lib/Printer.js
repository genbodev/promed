            ...
            var htmlMarkup = [
                ...
                '<html class="' + Ext.baseCSSPrefix + 'ux-grid-printer">',
                  ...
                  '<div class="' + Ext.baseCSSPrefix + 'ux-grid-printer-noprint ' + Ext.baseCSSPrefix + 'ux-grid-printer-links">',
                      '<button class="' + Ext.baseCSSPrefix + 'ux-grid-printer-linkprint" href="javascript:void(0);" onclick="window.print();">' + this.printLinkText + '</button>',
                      '<button class="' + Ext.baseCSSPrefix + 'ux-grid-printer-linkclose" href="javascript:void(0);" onclick="window.close();">' + this.closeLinkText + '</button>',
                  '</div>',
...
//        },
        ...
        generateBody : function( grid, columns, feature ) 
        {
			...
			var groupingFeature = this.getFeature(grid, 'grouping');
                ...
                var bodyTpl = [
                    '<tpl for=".">',
                        '<tr class="group-header">',
                            '<td style="padding: 10px 3px 4px 5px;" colspan="{[this.colSpan]}"><b>',
                    ...
                    '</tpl>',
                    {
                        ...
                        {							
							...
							//console.log('rcd', rcd, rcd.groupField)
							...
							//дико извиняюсь перед заморскими коллегами, но в нашем деле без костылей никуда
							//return this.groupTpl.apply(rcd); 
							return Ext.get(grid.getView().id+'-hd-'+rcd.name).child('div').getHTML()
                        },
                        getSummaryObject: function(align)
                        {      
                        	var summaryValues = {};
