...
Ext.define('sw.CmpCallsList', {
	alias: 'widget.CmpCallsList',
	...
	initComponent: function() {
		...
		me.items = [{
						...
						items: [
							{
								...
								items: [
									...
									{
										xtype: 'combo',
										fieldLabel: 'Форма поиска',
										store: Ext.create('Ext.data.Store', {
											fields: ['id', 'name', 'mode'],
											data : [
												{"id":1, "name":"Талон вызова"},
												{"id":2, "name":"Карта вызова"}
											]
										}),
										queryMode: 'local',
										name: 'searchType',
										...
										menu: {
											...
											items: [
												...
												},
												{
													...
													handler: function () {
														var params = me.down('[refId=CmpCallsListGrid]').bigGridStore.proxy.extraParams,
															strParams = '';
														if(!params)
															return;
			...
			items: [
				{
					xtype: 'gridPanelTwoStore',
					keepState: true,
					refId: 'CmpCallsListGrid',
					initStore:{
						fields: [
							{
								name: 'CmpCallCard_id',
						...
						*/
						{
							...
							renderer: function (value, cmp) {
								return Ext.Date.format(value, cmp.column.format)
							},
							filter: {
								xtype: 'swdatefield',
								format: 'd.m.Y',
						...
						{
							dataIndex: 'CmpCallCard_IsQuarantineText',
							text: 'Карантин',
							width: 100,
						...
						afterrender: function(){
							...
							if(tabpanel){
								tabpanel.on('tabchange', function( tabPanel, newCard, oldCard, eOpts){
									var journalCalls = newCard.down('panel[refId=CmpCallsList]');
	...
	searchCmpCalls: function(){
		var me = this,
			grid = me.down('grid'),
			armtype = me.initialConfig.armtype || 'default',
			baseForm = me.down('BaseForm'),
			dateTo = baseForm.down('[name=endDate]').getValue(),
			dateFrom = baseForm.down('[name=begDate]').getValue(),
			params = baseForm.getValues(),
			maxDays = (armtype == 'smpheaddoctor') ? 7 : 3,
			nameDays = (armtype == 'smpheaddoctor') ? 'дней' : 'дня';


		var dateDiffInDays = parseInt((dateTo - dateFrom)/(24*3600*1000));

		if(dateDiffInDays > maxDays && (getRegionNick() != 'astra')){
			Ext.Msg.alert('Ошибка','Заданный период не может превышать ' + maxDays +' '+ nameDays);
			return false;
		}
		...
		grid.bigGridStore.proxy.extraParams = params;
		if(getRegionNick() !== 'astra'){
			grid.clearHeaderFilters();
		}else{
			grid.applyHeaderFilters();
		}
		grid.bigGridStore.reload();
		grid.store.currentPage = 1;

		/*me.gridStore.proxy.extraParams = params;
		me.gridStore.reload();*/
		/*grid.store.proxy.extraParams = params;

		grid.store.currentPage = 1;
		grid.store.reload({ page: 1, start: 0, limit: 100 });*/
	...
	showSubMenu: function(x,y){
		...
		var subMenu = Ext.create('Ext.menu.Menu', {
			...
			items: [
				...
				{
					text: 'В первичные',
					hidden: me.isNmpArm && !(me.armtype.inlist(['smpdispatchstation'])) ,
					disabled: !( (recCard.get('CmpCallType_Code') == 14)), //Активно для поступивших дублей с которыми еще не ознакомились(в группе 1)
					handler: function(){

						var setCallToFirstCategoryWin = Ext.create('sw.standartToolsWindow', {
							title: 'Перевод вызова в первичный',
							height: 200,
							width: 400,
							layout: 'fit',
							modal: true,
							...
							configComponents: {
											...
											listeners: {
												render: function (cmp) {
													cmp.store.on('load', function () {
														cmp.setValue(cmp.store.findRecord('CmpCallType_Code', 1).get('CmpCallType_id'));
								...
								leftButtons: {
									...
									disabled: false,
									handler: function(){
										var frm = setCallToFirstCategoryWin.down('form').getForm();

										var params = {
											CmpCallCardStatusType_id: 18, //решение старшего врача
											CmpCallCard_id:	recCard.get('CmpCallCard_id'),
											CmpCallType_id: frm.findField('CmpCallType_id').getValue(),
											CmpCallCardStatus_Comment: frm.findField('CmpCallCardStatus_Comment').getValue(),
										...
										Ext.Ajax.request({
											...
											success: function(response, opts){
												...
												if ( obj.success ) {
													Ext.Msg.alert('Сообщение', 'Изменения сохранены', function(){
														this.close();
														setCallToFirstCategoryWin.close();
													});
												}
											}
										});
				...
				{
					text: 'Распечатать КТ',
					hidden: !getRegionNick().inlist(['astra','perm','krym','khak']) || !(recCard.get('EmergencyTeam_id') && recCard.get('CmpCallCard_id')) || me.isNmpArm,
					handler: function(){
						if (getRegionNick().inlist(['ufa','krym'])) {
							...
							Ext.Ajax.request({
								...
								params: {
									teamId: recCard.get('EmergencyTeam_id'),
									callId: recCard.get('CmpCallCard_id')
								},
								callback: function(opt, success, response){
									if (success){
										var win = window.open();
