...
Ext.extend(Ext.db.indexedDBDriver, Ext.util.Observable, {
	...
	open : function(db, version, config, first_run) {
		if (Ext.isIndexedDb) {
			...
			request.onsuccess = function(e) {
				...
				this.openState = true;
				this.setVersion(this.conn.version, config);
			}.bind(this);
	...
	getStore: function(name, mode){
		if(this.conn.objectStoreNames.contains(name)) {
			...
			if (Ext.db.IDBTransaction && Ext.db.IDBTransaction['readwrite']) {
				if (modeTrans == 'readwrite') {
					modeTrans = Ext.db.IDBTransaction['readwrite'];
				} else {
