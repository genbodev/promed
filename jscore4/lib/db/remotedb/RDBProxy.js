 ...
 * @param {Ext.data.JsonStore} store The datastore to bind to
...
Ext.rdb.Proxy = function(conn, table, keyName, store, readonly){
...
Ext.rdb.Proxy.DATE_FORMAT = 'Y-m-d H:i:s';

Ext.extend(Ext.rdb.Proxy, Ext.data.DataProxy, {
	...
	processData : function(o) {
		...
		if(f){
			if(f.type == 'date'){
				r[key] = v ? v.format(Ext.rdb.Proxy.DATE_FORMAT,10) : '';
