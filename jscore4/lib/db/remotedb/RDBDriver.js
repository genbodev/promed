...
Ext.db.remoteDBDriver = function(config){
	Ext.apply(this, config);
	Ext.db.remoteDBDriver.superclass.constructor.call(this);
	this.addEvents({
		open : true,
		close: true
	});
...
Ext.extend(Ext.db.remoteDBDriver, Ext.util.Observable, {
	openState : false,
...
Ext.db.remoteDBDriver.getInstance = function(db, config){
	if(Ext.isRemoteDB){ // remoteDB
		if (!Ext.rdb.RDBDriver) {
			Ext.rdb.RDBDriver = new Ext.db.remoteDBDriver(config);
		} 
		return Ext.rdb.RDBDriver;
	}
};