/* 
 * Форма АРМ Эксперта службы "Cудебно-медицинской экспертизы потерпевших, обвиняемых и других лиц"
 */

Ext.define('common.BSME.ForenPers.ExpertWP.swExpertWorkPlace', {
	extend: 'common.BSME.DefaultWP.DefaultExpertWP.swDefaultExpertWorkPlace',
	refId: 'forenmedexppersdprtbsmeexpert',
	id: 'ForenPersExpertWorkPlace',
	...
	getRequestListDataviewItemClick: function(self, rec, html_element, node_index, event){
		...
		if ( rec.get('EvnXml_id') || !rec.get('XmlType_id') || !rec.get('EvnForensic_id') ) {
			log(rec);
	...
	createRequestButtonHandler: function(){
		...
		Ext.create('common.BSME.ForenPers.SecretaryWP.tools.swCreateRequestWindow', {
			...
			MedPersonal_eid: parseInt(getGlobalOptions().CurMedPersonal_id),
