...
//        context.lineTo(prevx + shadowOffset / 2, prevy);
			alert('steps functionality is under construction')
      } else {
//        context.lineTo(prevx, prevy);
		context.dashedLine(x1,y1,prevx,prevy,[ options.lineLength, options.lineStep ]);
      }
    }
 ...
 * Рисуем график
 */
function drawGraph( container, data, params ){
	var graph;
	...
	graph = Flotr.draw(container, data, {
		...
		yaxis: {
			min: 0,
			max: 7,
			ticks: [[1,60], [2,70], [3,80], [4,90], [5,100], [6,120], [7,140]],
			tickDecimals: 0,
			showLabels: false,
			margin: true
		},
 ...
 * Рисуем сетку
 */
function drawGrid( parentObj, params ){
	...
	for ( var x=0; x<params.cells_x; x++ ){
		for ( var y=0; y<params.cells_y; y++ ){
			ctx.strokeRect( (params.cell_w+1)*x+0.5, (params.cell_h+1)*y+0.5, (params.cell_w+1), (params.cell_h+1) );
		}
	}
	ctx.closePath();
			
	parentObj.appendChild(grid);
}
