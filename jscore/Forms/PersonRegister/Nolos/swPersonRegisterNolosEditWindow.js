/**
 * swPersonRegisterNolosEditWindow - Форма просмотра/редактирования записи регистра по ВЗН (7 нозологиям)
 *
...
sw.Promed.swPersonRegisterNolosEditWindow = Ext.extend(sw.Promed.BaseForm, 
{
	winTitle: lang['zapis_registra_po_vzn'],
	...
	createInputArea: function(name, el_data) {
        ...
        var record = this.rightPanel.viewFormDataStore.getById('PersonRegisterNolos' +'_'+ el_data.object_id),
            params = null;
		...
		eloutputid = 'PersonRegisterNolos_'+ el_data.object_id +'_input'+name;
		elinputid = 'PersonRegisterNolos_'+ el_data.object_id +'_inputarea'+name;
		...
		switch(name){
			case 'Diag':
				if (ct && !me.input_cmp_list[eloutputid]) {
                    ...
                    config.PersonRegisterType_SysNick = 'nolos';
			...
			case 'Resist':
				if(ct && !me.input_cmp_list[eloutputid]) {
					...
					config = getBaseConfig({
						...
						,elInputId: elinputid
						,elOutput: eloutput
						,outputValue: ((params['PersonRegister_IsResist'])==1?2:1) || empty_value
						,elInputWrap: ct
						,elInput: null
                        ,requestCallback: function(result) {
                            if ( result.success ) {
                            	var fVal = 0;
                            	if(result.field_value == 2){
                            		fVal = 1;
                            	} else {
                            		fVal = 0;
                            	}
                                record.set('PersonRegister_IsResist', fVal);
                                record.commit(true);
                                me.rightPanel.viewFormDataStore.commitChanges();
                            }
                        }
					...
					cmp.setValue(((params['PersonRegister_IsResist'])==1?2:1) || null);
					cmp.focus(true, 500);
					me.input_cmp_list[eloutputid] = cmp;
				}
				break;
	...
	show: function() 
	{
		sw.Promed.swPersonRegisterNolosEditWindow.superclass.show.apply(this, arguments);
		...
		this.viewObject = {
			id: 'PersonRegisterNolos_'+ this.PersonRegister_id,
			attributes: {
				accessType: (this.allowSpecificEdit)?'edit':'view',
				text: 'test',
				object: 'PersonRegisterNolos',
	...
	initComponent: function() 
	{
		...
		this.rightPanel.configActions = {
			PersonRegisterNolos: {
				print: {
					actionType: 'view',
					sectionCode: 'PersonRegisterNolos',
				...
				editPers: {
					...
					sectionCode: 'PersonRegisterNolos',
					handler: function(e, c, d) {
						var data = win.rightPanel.getObjectData('PersonRegisterNolos',d.object_id),
				...
				inputPersonDecreedGroup: {
					...
					dblClick: false,
					handler: function(e, c, d) {
						win.createInputArea('PersonDecreedGroup', d);
					}
				}
			},
		...
		sw.Promed.swPersonRegisterNolosEditWindow.superclass.initComponent.apply(this, arguments);
	}
});