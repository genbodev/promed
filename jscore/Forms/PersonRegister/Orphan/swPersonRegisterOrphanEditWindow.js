/**
 * swPersonRegisterOrphanEditWindow - Форма просмотра/редактирования записи регистра по орфанным заболеваниям
 *
...
sw.Promed.swPersonRegisterOrphanEditWindow = Ext.extend(sw.Promed.BaseForm, 
{
	winTitle: lang['zapis_registra_po_orfannyim_zabolevaniyam'],
	...
	createInputArea: function(name, el_data) {
        ...
        var record = this.rightPanel.viewFormDataStore.getById('PersonRegisterOrphan' +'_'+ el_data.object_id),
            params = null;
			...
			if(conf.elOutput) {
				if (false && conf.field_name == 'Diag_id' && !conf.disableMsg) {
					sw.swMsg.show(
					{
						...
						msg: lang['izmenen_diagnoz_zapisi_registra_po_orfannyim_zabolevaniyam_izmenit_diagnoz_i_sozdat_�napravlenie_na_vnesenie_izmeneniy_v_registr'],
						title: lang['vopros']
					});
					return false;
				}
		...
		eloutputid = 'PersonRegisterOrphan_'+ el_data.object_id +'_input'+name;
		elinputid = 'PersonRegisterOrphan_'+ el_data.object_id +'_inputarea'+name;
		...
		switch(name){
			case 'Diag':
				if (ct && !me.input_cmp_list[eloutputid]) {
                    ...
                    config.MorbusType_SysNick = me.MorbusType_SysNick;
				...
				if(ct && !me.input_cmp_list[eloutputid]) {
					...
					config = getBaseConfig({
                        ...
                        ,requestCallback: function(result) {
                            if ( result.success ) {
                                if (result.field_rec) {
                                    record.set('Lpu_oid', result.field_value);
                                    record.set('LpuO_Name', result.field_raw_value);
                                } else {
                                    record.set('Lpu_oid', null);
                                    record.set('LpuO_Name', null);
                                }
                                record.commit(true);
                                me.rightPanel.viewFormDataStore.commitChanges();
                            }
					...
					config.hiddenName = 'Lpu_oid';
					cmp = new sw.Promed.SwLpuLocalCombo(config);
					cmp.getStore().load({
						callback: function(){
							if (this.getStore().getCount() > 0) {
								this.setValue(params['Lpu_oid'] || null);
							}
						},
						scope: cmp
					});
				...
				if(ct && !me.input_cmp_list[eloutputid]) {
					...
					config = getBaseConfig({
                        ...
                        ,requestCallback: function(result) {
                            if ( result.success ) {
                                if (result.field_rec) {
                                    record.set('Lpu_did', result.field_value);
                                    record.set('LpuD_Name', result.field_raw_value);
                                } else {
                                    record.set('Lpu_did', null);
                                    record.set('LpuD_Name', null);
                                }
                                record.commit(true);
                                me.rightPanel.viewFormDataStore.commitChanges();
                            }
					...
					config.width = 150;
					config.listWidth = 300;
					config.hiddenName = 'Lpu_did';
					cmp = new sw.Promed.SwLpuLocalCombo(config);
					cmp.getStore().load({
						callback: function(){
							if (this.getStore().getCount() > 0) {
								this.setValue(params['Lpu_did'] || null);
							}
						},
						scope: cmp
					});
	...
	show: function() 
	{
		sw.Promed.swPersonRegisterOrphanEditWindow.superclass.show.apply(this, arguments);
		...
		this.setTitle(this.winTitle + ': ' + ((this.allowSpecificEdit && (arguments[0].action!='view')) ? lang['_redaktirovanie'] : lang['_prosmotr']));
		this.viewObject = {
			id: 'PersonRegisterOrphan_'+ this.PersonRegister_id,
			attributes: {
				accessType: (this.allowSpecificEdit)?'edit':'view',
				text: 'test',
				object: 'PersonRegisterOrphan',
	...
	initComponent: function() 
	{
		...
		this.rightPanel.configActions = {
			PersonRegisterOrphan: {
				print: {
					actionType: 'view',
					sectionCode: 'PersonRegisterOrphan',
				...
				editPers: {
					...
					sectionCode: 'PersonRegisterOrphan',
					handler: function(e, c, d) {
						var data = win.rightPanel.getObjectData('PersonRegisterOrphan',d.object_id),
			...
			PersonPrivilegeInvAll: {
				toggleDisplay: {
					actionType: 'view',
					sectionCode: 'PersonPrivilegeInvAllList',
					handler: function(e, c, d) {
						var id = 'PersonPrivilegeInvAllTable_'+ d.object_id;
		...
		sw.Promed.swPersonRegisterOrphanEditWindow.superclass.initComponent.apply(this, arguments);
	}
});