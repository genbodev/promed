/**
* swEvnPrescrProcEditWindow - окно добавления/редактирования назначения.
*
...
* @version      0.001-12.11.2011
* @comment      Префикс для id компонентов EPRPREF (EvnPrescrProcEditForm)
*/
/*NO PARSE JSON*/

sw.Promed.swEvnPrescrProcEditWindow = Ext.extend(sw.Promed.BaseForm, {
	codeRefresh: true,
	objectName: 'swEvnPrescrProcEditWindow',
	objectSrc: '/jscore/Forms/Prescription/swEvnPrescrProcEditWindow.js',
	...
	doSave: function(options) {
		...
		if ( base_form.findField('EvnPrescrProc_setDate').disabled ) {
			params.EvnPrescrProc_setDate = Ext.util.Format.date(base_form.findField('EvnPrescrProc_setDate').getValue(), 'd.m.Y');
		}
		...
		if ( base_form.findField('Usluga_id').disabled ) {
		...
		base_form.submit({
			...
			success: function(result_form, action) {
				...
				if ( action.result ) {
					var data = new Object();
					var evnPrescrProcData = new Object();

					evnPrescrProcData.EvnPrescrProc_id = action.result.EvnPrescrProc_id;

					data.evnPrescrProcData = evnPrescrProcData;
	...
	initComponent: function() {
		this.FormPanel = new Ext.form.FormPanel({
			...
			reader: new Ext.data.JsonReader({
				...
				{ name: 'EvnPrescrProc_pid' },
				{ name: 'EvnPrescrProc_setDate' },
				{ name: 'EvnPrescrProc_setTime' },
				{ name: 'PersonEvn_id' },
			...
			region: 'center',
			url: '/?c=EvnPrescr&m=saveEvnPrescrProc',
			...
			items: [{
				...
				name: 'EvnPrescrProc_id', // Идентификатор бирки
				value: -1,
				xtype: 'hidden'
			}, {
				name: 'EvnPrescrProc_pid', // Идентификатор назначения с типом "Манипуляции и процедуры"
				value: -1,
				xtype: 'hidden'
			}, {
		...
		sw.Promed.swEvnPrescrProcEditWindow.superclass.initComponent.apply(this, arguments);
	},
	keys: [{
		alt: true,
		fn: function(inp, e) {
			var current_window = Ext.getCmp('EvnPrescrProcEditWindow');
	...
	resizable: false,
	show: function() {
		sw.Promed.swEvnPrescrProcEditWindow.superclass.show.apply(this, arguments);
		...
		this.disableUslugaChange = false;
		this.formStatus = 'edit';
		...
		if ( arguments[0].disableUslugaChange == true ) {
			this.disableUslugaChange = arguments[0].disableUslugaChange;
		}
		...
		switch ( this.action ) {
			case 'add':
				...
				if ( this.disableUslugaChange == true ) {
				...
				base_form.findField('EvnPrescrProc_setTime').focus(true, 250);
			break;
			...
			case 'view':
				base_form.load({
					...
					params: {
						'EvnPrescrProc_id': base_form.findField('EvnPrescrProc_id').getValue()
					},
					...
					url: '/?c=EvnPrescr&m=loadEvnPrescrProcEditForm'
				});
			break;
