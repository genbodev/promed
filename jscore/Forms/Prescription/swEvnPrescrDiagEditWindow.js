/**
* swEvnPrescrDiagEditWindow - окно добавления/редактирования назначения.
*
...
* @comment      Префикс для id компонентов EPRDGEF (EvnPrescrDiagEditForm)
*/
/*NO PARSE JSON*/
...
sw.Promed.swEvnPrescrDiagEditWindow = Ext.extend(sw.Promed.BaseForm, {
	codeRefresh: true,
	objectName: 'swEvnPrescrDiagEditWindow',
	objectSrc: '/jscore/Forms/Prescription/swEvnPrescrDiagEditWindow.js',
	...
	doSave: function(options) {
		...
		if ( base_form.findField('EvnPrescrDiag_setDate').disabled ) {
			params.EvnPrescrDiag_setDate = Ext.util.Format.date(base_form.findField('EvnPrescrDiag_setDate').getValue(), 'd.m.Y');
		}
		...
		base_form.submit({
			...
			success: function(result_form, action) {
				...
				if ( action.result ) {
					var data = new Object();
					var evnPrescrDiagData = new Object();

					evnPrescrDiagData.EvnPrescrDiag_id = action.result.EvnPrescrDiag_id;

					data.evnPrescrDiagData = evnPrescrDiagData;
	...
	initComponent: function() {
		this.FormPanel = new Ext.form.FormPanel({
			...
			reader: new Ext.data.JsonReader({
				...
				{ name: 'accessType' },
				{ name: 'EvnPrescrDiag_Descr' },
				{ name: 'EvnPrescrDiag_id' },
				{ name: 'EvnPrescrDiag_pid' },
				{ name: 'EvnPrescrDiag_setDate' },
				{ name: 'EvnPrescrDiag_setTime' },
				{ name: 'PersonEvn_id' },
			...
			region: 'center',
			url: '/?c=EvnPrescr&m=saveEvnPrescrDiag',
			...
			items: [{
				...
				name: 'EvnPrescrDiag_id', // Идентификатор бирки
				value: -1,
				xtype: 'hidden'
			}, {
				name: 'EvnPrescrDiag_pid', // Идентификатор назначения с типом "Диагностика"
				value: -1,
				xtype: 'hidden'
			}, {
		...
		sw.Promed.swEvnPrescrDiagEditWindow.superclass.initComponent.apply(this, arguments);
	},
	keys: [{
		alt: true,
		fn: function(inp, e) {
			var current_window = Ext.getCmp('EvnPrescrDiagEditWindow');
	...
	resizable: false,
	show: function() {
		sw.Promed.swEvnPrescrDiagEditWindow.superclass.show.apply(this, arguments);
		...
		switch ( this.action ) {
			case 'add':
				...
				base_form.findField('EvnPrescrDiag_setTime').focus(true, 250);
			break;
			...
			case 'view':
				base_form.load({
					...
					params: {
						'EvnPrescrDiag_id': base_form.findField('EvnPrescrDiag_id').getValue()
					},
					...
					url: '/?c=EvnPrescr&m=loadEvnPrescrDiagEditForm'
				});
			break;
