...
//        rec = this.EvnPrescrListDataView.getStore().getById(key);
        ...
        */
        ...
        var allowEditCourse = false;
        var allowViewCourse = false;
        var allowDeleteCourse = false;
        this.ViewActions['action_edit'].setHidden(false);
        this.ViewActions['action_exec'].setHidden(false);
        this.ViewActions['action_execWithUseDrug'].setHidden(true);
        ...
        this.ViewActions['action_editCourse'].setHidden(false);
        this.ViewActions['action_viewCourse'].setHidden(false);
        this.ViewActions['action_deleteCourse'].setHidden(false);
        var day_dt;
        var cur_dt = Date.parseDate(getGlobalOptions().date, 'd.m.Y');
        if (data && data.set_date) {
            day_dt = Date.parseDate(data.set_date, 'd.m.Y');
        }
        if (clickData.cellType=='EvnPrescrCell') {
            //Клик в заголовок
            allowEditCourse = rec.get('EvnCourse_id');
            allowViewCourse = rec.get('EvnCourse_id');
            allowDeleteCourse = rec.get('EvnCourse_id');
        ...
        this.ViewActions['action_editCourse'].setHidden(!allowEditCourse);
        this.ViewActions['action_viewCourse'].setHidden(!allowViewCourse);
        this.ViewActions['action_deleteCourse'].setHidden(!allowDeleteCourse);
        if (clickData.cellType=='EvnPrescrDayCell') {
            //Клик в ячейку строки назначения/курса
            allowActions = (rec.get('EvnPrescr_id') && day_dt);
            allowEdit =(allowActions && rec.get('EvnPrescr_IsExec') != 2);
            allowView = (allowActions);
            allowDelete = (allowEdit);
            allowExec = (allowActions && sw.Promed.EvnPrescr.isExecutable(this._createExecParams(rec, data)));
            allowUnExec = (allowActions && sw.Promed.EvnPrescr.isUnExecutable(this._createUnExecParams(rec, data)));
        }

        if (clickData.cellType == 'EvnPrescrDayRow') {
            //Клик в строку назначения в курсе
            allowActions = (rec.get('EvnCourse_id') && clickData.EvnPrescr_id && data);
            allowEdit =(allowActions && data.EvnPrescr_IsExec != 2);
            allowView = (allowActions);
            allowDelete = (allowEdit);
            allowExec = (allowActions && sw.Promed.EvnPrescr.isExecutable(this._createExecParams(rec, data)));
            allowUnExec = (allowActions && sw.Promed.EvnPrescr.isUnExecutable(this._createUnExecParams(rec, data)));
			...
			if (clickData.cellType=='EvnPrescrCell') {
                ...
                if (clickData.cellType=='EvnPrescrDayCell') {
                    ...
                    if (data.empty) {
                        ...
                        //log(['dd',rec]);
                        this.addPrescrByType({
                            EvnPrescr_key:key,
                            set_date:data.set_date,
                            PrescriptionType_id :rec.json.PrescriptionRegimeType_id
                        });
                        return true;
                    }
                }
                this.ViewActions['action_exec'].setHidden(true);
                this.ViewActions['action_unexec'].setHidden(true);
			break;
            case '2':
			    if (clickData.cellType=='EvnPrescrCell') {
                    //Клик в заголовок
                ...
                if (clickData.cellType=='EvnPrescrDayCell') {
                    ...
                    if (data.empty) {
                        ...
                        this.addPrescrByType({
                            EvnPrescr_key:key,
                            set_date:data.set_date,
                            PrescriptionType_id :rec.json.PrescriptionDietType_id
                        });
                        return true;
                    }
                }
		   ...
		   case '10':
                ...
                if (clickData.cellType=='EvnPrescrDayCell') {
                    ...
                    if (data.empty) {
                        ...
                        this.addPrescrByType({
							EvnPrescr_key:key,
							set_date:data.set_date,
							newEvnPrescr_id:rec.data.EvnPrescr_id
						});
                        return true;
                    }
                }
                ...
                if (clickData.cellType=='EvnPrescrDayCell') {
                    ...
                    if (data.empty) {
                        ...
                        if (false && rec.get('EvnPrescr_IsDir') == 2) {
                            this.copyEvnPrescrUsluga(rec, data);
                            return true;
                        } else {
            ...
            case '6':
                if (clickData.cellType=='EvnPrescrCell') {
                    ...
                    this.ViewActions['action_exec'].setHidden(true);
                    this.ViewActions['action_unexec'].setHidden(true);
                    this.ViewActions['action_view'].setHidden(true);
                ...
                if (clickData.cellType=='EvnPrescrDayCell') {
                    ...
                    if (data.empty) {
                        ...
                        if (rec.get('days') && data.set_date) {
                            //day_dt = Date.parseDate(data.set_date, 'd.m.Y');
                            var copy_day_key = '';
                            for (day_key in rec.get('days')) {
                                if ( !rec.get('days').hasOwnProperty(day_key) ) continue;
                                cur_dt = Date.parseDate(day_key.split('-')[1], 'd.m.Y');
                                if (day_dt > cur_dt && rec.get('days')[day_key].EvnPrescrDataList) {
                                    copy_day_key = day_key;
                                }
                                if (day_dt < cur_dt && rec.get('days')[day_key].EvnPrescrDataList) {
                                    copy_day_key = day_key;
                                    break;
                                }
                                //log([day_dt, cur_dt, copy_day_key]);
                            }
                            // Вызов формы для добавления назначения-направления в курс
                            this.addEvnPrescrProc(rec.get('EvnPrescr_key'), copy_day_key, data.set_date);
                            return true;
                        }
                        return false;
                    }
                ...
                if (clickData.cellType=='EvnPrescrDayCell') {
                    ...
                    if (data.empty) {
                        ...
                        if (rec.get('days') && data.set_date) {
                            ...
                            this.openEvnPrescrEditWindow('copy');
                            return true;
                        }
                    }
                    if (!data.empty) {
                        ...
                        allowActions = (data.EvnPrescr_id && day_dt);
                        allowEdit =(allowActions && !data.Day_IsExec);
                        allowView = (allowActions);
                        allowDelete = (allowEdit);
                        allowExec = (allowActions && sw.Promed.EvnPrescr.isExecutable(this._createExecParams(rec, data)));
                        allowUnExec = (allowActions && sw.Promed.EvnPrescr.isUnExecutable(this._createUnExecParams(rec, data)));
        ...
        this.ViewActions['action_execWithUseDrug'].setDisabled(!allowExec);
        ...
        this.ViewActions['action_editCourse'].setDisabled(!allowEditCourse);
        this.ViewActions['action_viewCourse'].setDisabled(!allowViewCourse);
        this.ViewActions['action_deleteCourse'].setDisabled(!allowDeleteCourse);
        this.ViewContextMenu.params = {
            EvnPrescrListRecord: rec,
            DayData: data
        };
     ...
     * Отмеченные назначения добавляем в лист назначений.
     */
	...
	createCourseTreatPrescription:function(){
	var base_form = this.FormPanel.getForm();
		var exceptionTypes = [];
		var thas = this, personFrame = this.PersonInfo;
	var params={};
	params.PrescriptionType_id = 5;
	params.PrescriptionType_Code = 5;
	params.action = 'add';
	params.userMedStaffFact = this.userMedStaffFact
	params.callbackEditWindow = function(){
		thas.isChange = true;
		thas.loadPrescrPlanView();
	}
	params.data = {
	...
	sw.Promed.EvnPrescr.openEvnCourseEditWindow(params);
	},
    ...
    unExecEvnPrescr: function() {
        ...
        var conf = this._createUnExecParams(rec, dayData);
    ...
    _createUnExecParams: function(rec, dayData) {
        if ( !rec ) {
            return false;
        }
        if ( !dayData ) {
        ...
        return {
            ...
            ,EvnPrescr_IsExec: ((dayData.set_date && dayData.Day_IsExec) || 2==rec.get('EvnPrescr_IsExec'))?2:1
            ,EvnPrescr_IsHasEvn: ((dayData.set_date && dayData.EvnPrescr_IsHasEvn) || 2==rec.get('EvnPrescr_IsHasEvn'))?2:1
            //,PrescriptionStatusType_id: rec.get('PrescriptionStatusType_id') || dayData.PrescriptionStatusType_id
        };
    },
    _createExecParams: function(rec, dayData) {
        if ( !rec ) {
            return false;
        }
        if ( !dayData ) {
        ...
        var conf = {
            ownerWindow: this
            ,allowChangeTime: !dayData.set_date
            ,EvnPrescr_setDate: (dayData.set_date)||null
            ,Person_id: base_form.findField('Person_id').getValue()
            ,PersonEvn_id: base_form.findField('PersonEvn_id').getValue()
            ,Server_id: base_form.findField('Server_id').getValue()
        };
        ...
        conf.EvnPrescr_IsExec = (dayData.set_date)?(dayData.Day_IsExec?2:1):rec.get('EvnPrescr_IsExec');
    ...
    execEvnPrescr: function(mode) {
		...
		var scrollTop = $('.EvnPrescrList_right').scrollTop();
		var scrollLeft = $('.EvnPrescrList_right').scrollLeft();
        var conf = this._createExecParams(rec, dayData);
        conf.mode = mode;
        conf.btnId = (dayData?('EvnPrescrDayCell_'+dayData.index):('EvnPrescrCell_'+rec.get('EvnPrescr_key')));
    ...
    openEvnCourseEditWindow: function(action) {
        //log([action, this.ViewContextMenu.params, this.ViewContextMenu.params.EvnPrescrListRecord]);
        var thas = this,
            base_form = this.FormPanel.getForm(),
            personFrame = this.PersonInfo;
    ...
    initComponent: function() {
        ...
        var actions = [
            ...
            },
            {
                ...
                text: lang['vyipolnit_s_ispolzovaniem_medikamentov'],
                tooltip: lang['otkryit_formu_spisaniya_medikamentov'],
                iconCls: 'exec16',
                handler: function() {
            ...
            {name:'action_editCourse', text:BTN_GRIDEDIT, tooltip: BTN_GRIDEDIT_TIP, icon: 'img/icons/edit16.png', handler: function() {thas.openEvnCourseEditWindow('edit');}},
            {name:'action_viewCourse', text:BTN_GRIDVIEW, tooltip: BTN_GRIDVIEW_TIP, icon: 'img/icons/view16.png', handler: function() {thas.openEvnCourseEditWindow('view');}},
            ...
            },
            {
				...
				iconCls: 'save16',
                hidden: true, // !isSuperAdmin(),//до восстановления работоспособности
        ...
        this.ViewContextMenu.add(this.ViewActions['action_editCourse']);
        this.ViewContextMenu.add(this.ViewActions['action_viewCourse']);
