/**
* swPersonDispOrpPredSearchWindow - окно поиска в регистре диспанцеризации несовершеннолетних
*
...
sw.Promed.swPersonDispOrpPredSearchWindow = Ext.extend(sw.Promed.BaseForm, {
	title: lang['napravleniya_na_predvaritelnyie_osmotryi_nesovershennoletnih_poisk'],
	id: 'PersonDispOrpPredSearchWindow',
	height: 550,
	width: 900,
	closable: true,
	closeAction: 'hide',
	...
	[{
		...
		fn: function(e) {Ext.getCmp("PersonDispOrpPredSearchWindow").addPersonDispOrp();},
		stopEvent: true
	}, 
	{
		key: "0123456789",
		alt: true,
		fn: function(e) {Ext.getCmp("PersonDispOrpPredFilterTabPanel").setActiveTab(Ext.getCmp("PersonDispOrpPredFilterTabPanel").items.items[ e - 49 ]);},
		stopEvent: true
	}, 
	{
		alt: true,
		fn: function(inp, e) 
		{
			var frm = Ext.getCmp('PersonDispOrpPredSearchWindow');
			switch (e.getKey())
			{
	...
	{
		frm = this;
		var loadMask = new Ext.LoadMask(Ext.get('PersonDispOrpPredSearchWindow'), { msg: "Подождите, идет сохранение..." });
		loadMask.show();
		var params = 
		{
			...
			PersonDispOrp_Year: Ext.getCmp('pdopreswYearCombo').getValue()
		};
					...
					if ( response.responseText.length > 0 )
					{
						...
						else
						{
							Ext.Msg.alert(
								...
								function () 
								{
									var years_combo = frm.findById('pdopreswYearCombo');
	...
	addPersonDispOrp: function() 
	{
		...
		getWnd('swPersonSearchWindow').show(
		{
			...
			onSelect: function(person_data) 
			{
				...
				getWnd('swPersonDispOrpPredEditWindow').show({
					action: 'add',
			...
			searchMode: (getRegionNick()=='perm'?'attbefore3':(getRegionNick().inlist(['astra','buryatiya','kareliya'])?'all':'att'))
		});
	},
	doResetAll: function() 
	{
  	var form = this.findById('PersonDispOrpPredFilterForm');
		var year = Ext.getCmp('pdopreswYearCombo').getValue();
		form.getForm().reset();
		Ext.getCmp('pdopreswYearCombo').setValue(year);
	...
	doEvnPLDOAdd: function() {
		...
		if ( !Ext.isEmpty(record.get('EducationInstitutionType_id')) ) {
			params.EducationInstitutionType_id = record.get('EducationInstitutionType_id');
		}
	...
	doSearch: function(params) 
	{
		...
		params.start = 0;
		params.limit = 100;
		params.PersonDispOrp_Year = Ext.getCmp('pdopreswYearCombo').getValue();
		grid.getStore().removeAll();
	...
	doStreamInputSearch: function() 
	{
		...
		params.limit = 100;
		params.PersonDispOrp_Year = Ext.getCmp('pdopreswYearCombo').getValue();
		params.SearchFormType = 'PersonDispOrpPred';
		this.GridPanel.removeAll();
		...
		Ext.Ajax.request(
		{
			...
			callback: function(opt, success, response) 
			{
				if (success && response.responseText != '')
				{
					...
					frm.findById('pdopreswStream_begDateTime').setValue(response_obj.begDate + ' ' + response_obj.begTime);
				}
			}
		});
	},
	refreshPersonDispOrpViewGrid: function() 
	{
		...
		var panel = this.findById('PersonDispOrpPredFilterTabPanel').getActiveTab();
	...
	setStreamInputMode: function()
	{
		...
		var grid = this.findById('PDOPRE_PersonDispOrpViewGrid').ViewGridPanel;
		this.setTitle(this.wintitle + lang['potochnyiy_vvod']);
		this.findById('PersonDispOrpPredFilterForm').hide();
		this.doLayout();
		this.is_potok = true;
		if ( this.findById('pdopreswYearCombo').getStore().getCount() > 0 )
			this.findById('pdopreswYearCombo').focus(true, 100);
	},
	setSearchMode: function()
	{
		...
		this.findById('PersonDispOrpPredFilterForm').show();
		this.doLayout();
		this.is_potok = false;
		if ( this.findById('pdopreswYearCombo').getStore().getCount() > 0 )
			this.findById('pdopreswYearCombo').focus(true, 100);
	...
	show: function() 
	{
		sw.Promed.swPersonDispOrpPredSearchWindow.superclass.show.apply(this, arguments);
		...
		this.wintitle = lang['napravleniya_na_predvaritelnyie_osmotryi_nesovershennoletnih'];
		...
		var years_combo = this.findById('pdopreswYearCombo');
		years_combo.getStore().baseParams.CategoryChildType_SysNick = 'orppred';
		...
		var tabPanel = this.findById('PersonDispOrpPredFilterTabPanel');
		tabPanel.setActiveTab('pdopreswFirstTab');
	...
	getButtonSearch: function() {
		...
		return Ext.getCmp('pdopreswSearchButton');
	},
	initComponent: function() 
	{
		...
		this.EditPanel = new Ext.Panel(
		{
				...
				[{
					...
					listeners: {
						'keydown': function (inp, e) {
							...
							{
								e.stopEvent();
								var panel = inp.ownerCt.ownerCt.findById('PersonDispOrpPredFilterTabPanel').getActiveTab();
								var els=panel.findByType('textfield', false);
								if (els==undefined)
									els=panel.findByType('combo', false);
								var el=els[0];
			...
			{
				id: 'PersonDispOrpPredFilterForm',
				ownerWindow: this,
				searchFormType: 'PersonDispOrpPred',
				tabIndexBase: TABINDEX_PDOPRESF,
				tabPanelId: 'PersonDispOrpPredFilterTabPanel',
				tabGridId: 'PDOPRE_PersonDispOrpViewGrid',
				tabs: [{
					autoHeight: true,
		...
		{
			dataUrl: C_SEARCH,
			id: 'PDOPRE_PersonDispOrpViewGrid',
			...
			focusOn: {name:'pdopreswSearchButton', type:'other'},
			stringfields:
			[
				...
				{name: 'ExistsDOPL',  type: 'checkbox', header: lang['karta_predvaritelnogo_osmotra']},
				{name: 'EvnPLDispTeenInspection_id',  type: 'int', hidden: true, hideable: false}
			],
			actions:
			[
				{name: 'action_add', handler: function() {Ext.getCmp('PersonDispOrpPredSearchWindow').addPersonDispOrp(); }},
				{name: 'action_edit', handler: function() {Ext.getCmp('PersonDispOrpPredSearchWindow').editPersonDispOrp('edit'); }},
				{name: 'action_view', handler: function() {Ext.getCmp('PersonDispOrpPredSearchWindow').editPersonDispOrp('view'); }},
				{name: 'action_delete', handler: function() {Ext.getCmp('PersonDispOrpPredSearchWindow').deletePersonDispOrp(); }},
		...
		Ext.apply(this, 
		{
			...
			[{
				...
				iconCls: 'search16',
				id: 'pdopreswSearchButton',
				tabIndex: TABINDEX_PDOPRESF + 50,
				text: BTN_FRMSEARCH
			}, 
			{
				handler: function() 
			...
			}, 
			{
				...
				//iconCls: 'resetsearch16',
				tabIndex: TABINDEX_PDOPRESF + 52,
				text: lang['karta_osmotra']
			},
			'-',
			HelpButton(this, TABINDEX_PDOPRESF + 53),
			{
				handler: function() 
				{
					this.ownerCt.hide();
				...
				onTabAction: function() {
					Ext.getCmp('pdopreswYearCombo').focus(true, 200);
				},
				iconCls: 'cancel16',
				tabIndex: TABINDEX_PDOPRESF + 54,
				text: BTN_FRMCANCEL
			}
		],
		items: 
		...
		sw.Promed.swPersonDispOrpPredSearchWindow.superclass.initComponent.apply(this, arguments);
	}
});