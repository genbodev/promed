...
sw.Promed.swEvnPLDispDopEditWindow = Ext.extend(sw.Promed.BaseForm, {
	...
	deleteEvnVizitDispDop: function() {
		...
		sw.swMsg.show({
			...
			msg: 'Удалить осмотр врача-специалиста?',
			title: 'Вопрос'
		});
	},
	deleteEvnUslugaDispDop: function() {
		sw.swMsg.show({
			...
			icon: Ext.MessageBox.QUESTION,
			msg: 'Удалить лабораторное исследование?',
			title: 'Вопрос'
		})
	},
	draggable: true,
	doSave: function(print, check_finish, print_blank, print_other) {
		...
		var soc_status_id = current_window.findById('EPLDDEF_PersonInformationFrame').getFieldValue('SocStatus_id');					

		var ter_date = null;
		current_window.findById('EPLDDEF_EvnVizitDispDopGrid').getStore().each(function(rec) {
			if ( rec.get('DopDispSpec_id') == 1 || rec.get('DopDispSpec_id') == 9 || rec.get('DopDispSpec_id') == 10 )
		...
		var usluga_is_full = true;
		if (
			( UsedDopDispUslugaType['1'] != 1 && ((soc_status_id == 1 || soc_status_id == 8) && age > 17) )
			|| ( UsedDopDispUslugaType['2'] != 1 && ((soc_status_id == 1 || soc_status_id == 8) && age > 17) )
			|| ( UsedDopDispUslugaType['3'] != 1 && (((soc_status_id == 1 || soc_status_id == 8) && age > 17)) )
			|| ( UsedDopDispUslugaType['4'] != 1 && (((soc_status_id == 1 || soc_status_id == 8) && age > 17)) )
			|| ( UsedDopDispUslugaType['5'] != 1 && ((sex_id == 2 && age >= 40 && (soc_status_id == 1 || soc_status_id == 8))) && Ext.getCmp('EPLDDEF_EvnPLDispDop_IsNotMammograf').getValue() != true )
			|| ( UsedDopDispUslugaType['6'] != 1 && ((soc_status_id == 1 || soc_status_id == 8) && age > 17) )
			|| ( UsedDopDispUslugaType['7'] != 1 && (((soc_status_id == 1 || soc_status_id == 8) && age > 17)) )
			|| ( UsedDopDispUslugaType['9'] != 1 && ((soc_status_id == 1 || soc_status_id == 8) && age > 17) )
			|| ( UsedDopDispUslugaType['10'] != 1 && ((soc_status_id == 1 || soc_status_id == 8) && age > 17) )
			|| ( UsedDopDispUslugaType['11'] != 1 && (sex_id == 2 && age >= 45 && (soc_status_id == 1 || soc_status_id == 8)) )
			|| ( UsedDopDispUslugaType['12'] != 1 && (sex_id == 1 && age >= 45 && (soc_status_id == 1 || soc_status_id == 8)) )
			|| ( UsedDopDispUslugaType['13'] != 1 && ((soc_status_id == 1 || soc_status_id == 8) && age > 17) )
			|| ( UsedDopDispUslugaType['14'] != 1 && ((soc_status_id == 1 || soc_status_id == 8) && age > 17) )
			|| ( UsedDopDispUslugaType['15'] != 1 && ((soc_status_id == 1 || soc_status_id == 8) && age > 17) )
			|| ( UsedDopDispUslugaType['16'] != 1 && ((soc_status_id == 1 || soc_status_id == 8) && age > 17) )
			|| ( UsedDopDispUslugaType['17'] != 1 && ((soc_status_id == 1 || soc_status_id == 8) && age > 17) )
			|| ( UsedDopDispUslugaType['18'] != 1 && ((sex_id == 2 && (soc_status_id == 1 || soc_status_id == 8))) && Ext.getCmp('EPLDDEF_EvnPLDispDop_IsNotCito').getValue() != true )
		)
			usluga_is_full = false;
		
		var spec_is_full = true;
		if (
			( UsedDopDispSpec['1'] != 1 && UsedDopDispSpec['10'] != 1 && (age > 17) )
			|| ( UsedDopDispSpec['2'] != 1 && (sex_id == 2) )
			|| ( UsedDopDispSpec['3'] != 1 )
			|| ( UsedDopDispSpec['5'] != 1 && (age > 17) )
			|| ( UsedDopDispSpec['6'] != 1 )			
		)
			spec_is_full = false;
		...
		else
		{
			if ( spec_is_full && usluga_is_full && check_finish != 2  )
			{
				sw.swMsg.show({
					...
					icon: Ext.MessageBox.QUESTION,
					msg: 'Заполнены все осмотры и исследования, хотите чтобы случай был закончен?',
					title: 'Вопрос'
				});
				return;
			}
		...
		evnpldispdop_form.getForm().submit({
			failure: function(result_form, action) {
				...
				if (action.result)
				{
					...
					{
						Ext.Msg.alert('Ошибка', 'При сохранении произошли ошибки [Тип ошибки: 1]');
					}
				}
			},
			params: {
					...
					new Ext.form.FormPanel({
						...
						items: [{
							...
							xtype: 'hidden'
								...
								fieldLabel: 'Код специальности по ОКВЭД',
								...
								allowBlank: true,
								editable: false,
								fieldLabel: 'ЛПУ постоянного прикрепления',						
								hiddenName: 'Lpu_aid',
								...
								items: [{
									...
									items: [{
										...
										editable: false,
										enableKeyEvents: true,
										fieldLabel: 'Паспорт здоровья выдан пациенту',
								...
								items: [{
									...
									items: [{
										...
										boxLabel: 'Невозможность проведения маммографии',
									...
									items: [{
										...
										boxLabel: 'Невозможность проведения цитологического исследования мазка из цервикального канала',
										hideLabel: true,
										xtype: 'checkbox'
									}]
								}]
...
/*											}
							...
							title: 'Осмотр врача-специалиста'
						}),
						new Ext.grid.GridPanel({
							autoExpandColumn: 'autoexpand',
							autoExpandMin: 100,
							columns: [{
								...
								header: 'Данные по показателям',
								hidden: true
							}, {
								dataIndex: 'RateGrid_DataNumber',
								header: 'Номер данных по показателям',
								hidden: true
							}],
							frame: false,
							height: 200,
...
/*											}
	...
	openEvnVizitDispDopEditWindow: function(action) {
		...
		{
			sw.swMsg.alert('Сообщение', 'Окно редактирования осмотра врача-специалиста уже открыто');
			return false;
		}
		...
		var soc_status_id = this.findById('EPLDDEF_PersonInformationFrame').getFieldValue('SocStatus_id');
	...
	openEvnUslugaDispDopEditWindow: function(action) {
		...
		{
			Ext.Msg.alert('Сообщение', 'Окно редактирования лабораторного исследования уже открыто');
			return false;
		}
	...
	openPersonCureHistoryWindow: function() {
		...
		{
			current_window.showMessage('Сообщение', 'Окно просмотра истории лечения человека уже открыто');
			return false;
		}
	...
	show: function() {
		...
		var loadMask = new Ext.LoadMask(Ext.get('EvnPLDispDopEditWindow'), { msg: LOAD_WAIT });
		loadMask.show();
		...
		this.findById('EPLDDEF_EvnVizitDispDopGrid').getTopToolbar().items.items[1].disable();
		this.findById('EPLDDEF_EvnVizitDispDopGrid').getTopToolbar().items.items[2].disable();
		this.findById('EPLDDEF_EvnVizitDispDopGrid').getTopToolbar().items.items[3].disable();

		this.findById('EPLDDEF_EvnUslugaDispDopGrid').getStore().removeAll();
		LoadEmptyRow(this.findById('EPLDDEF_EvnUslugaDispDopGrid'));
		this.findById('EPLDDEF_EvnUslugaDispDopGrid').getTopToolbar().items.items[1].disable();
		this.findById('EPLDDEF_EvnUslugaDispDopGrid').getTopToolbar().items.items[2].disable();
		this.findById('EPLDDEF_EvnUslugaDispDopGrid').getTopToolbar().items.items[3].disable();		
			...
			case 'view':
				...
				form.getForm().load({
					failure: function() {
						if ( inf_frame_is_loaded )
							loadMask.hide();
						Ext.Msg.alert('Ошибка', 'Ошибка при загрузке данных формы', function() { current_window.hide(); } );
					},
					params: {
						EvnPLDispDop_id: evnpldispdop_id,
						archiveRecord: current_window.archiveRecord
