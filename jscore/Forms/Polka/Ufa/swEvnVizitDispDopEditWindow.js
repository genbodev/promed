...
sw.Promed.swEvnVizitDispDopEditWindow = Ext.extend(sw.Promed.BaseForm, {
	...
	doSave: function() {
		...
		if ( (dopdisp_spec_id == 1 || dopdisp_spec_id == 10) && (this.findById('EVDDEW_EvnVizitDispDop_setDate').getValue() < this.max_date) )
		{
			Ext.MessageBox.show({
				buttons: Ext.Msg.OK,
				fn: function() {
		...
		if ( set_date < Date.parseDate('01.01.' + set_date.getFullYear(), 'd.m.Y') )
		{
			Ext.MessageBox.show({
				...
				icon: Ext.Msg.WARNING,
				msg: 'Дата начала не может быть меньше 01.01.' + set_date.getFullYear() + '.',
				title: 'Ошибка'
			});
			return false;
		}
		...
		if (index >= 0)
		{
			if ( dopdisp_spec_id == 1 )
				dopdisp_spec_name = 'Терапевт';
			else
	...
	enableEdit: function(enable) {
		if (enable)
		{
			...
			Ext.getCmp('EVDDEW_DopDispAlien_idCombo').enable();
		...
		else
		{
			...
			Ext.getCmp('EVDDEW_DopDispAlien_idCombo').disable();
	...
	initComponent: function() {
			...
			items: [
				...
				new Ext.form.FormPanel({
					...
					items: [{
						...
						allowBlank: false,
						fieldLabel: 'Сторонний специалист',						
		...
		this.findById('EVDDEW_DiagCombo').onChange = function(combo, newValue, oldValue) {
			...
			if ( diag_code.inlist(['Z00', 'Z01', 'Z02', 'Z04', 'Z10']) ) {
				if ( dop_disp_spec_id != '1' && dop_disp_spec_id != '9' && dop_disp_spec_id != '10' ) {
					health_kind_combo.disable();
			...
			else if ( dop_disp_spec_id != '1' && dop_disp_spec_id != '9' && dop_disp_spec_id != '10' ) {
	...
	setDopDispSpecFilter: function() {
		...
		var soc_status_id = this.SocStatus_id;
		...
		if (
			...
			|| ( UsedDopDispUslugaType['3'] != 1 && (((soc_status_id == 1 || soc_status_id == 8) && age > 17) || age < 17) )				
			|| ( UsedDopDispUslugaType['4'] != 1 && (((soc_status_id == 1 || soc_status_id == 8) && age > 17) || age < 17) )				
			|| ( UsedDopDispUslugaType['5'] != 1 && ((sex_id == 2 && age >= 40 && (soc_status_id == 1 || soc_status_id == 8))) )
			|| ( UsedDopDispUslugaType['6'] != 1 && ((soc_status_id == 1 || soc_status_id == 8) && age > 17) )
			|| ( UsedDopDispUslugaType['7'] != 1 && (((soc_status_id == 1 || soc_status_id == 8) && age > 17) || age < 17) )				
			...
			|| ( UsedDopDispUslugaType['18'] != 1 && ((sex_id == 2 && (soc_status_id == 1 || soc_status_id == 8))) )
			|| ( UsedDopDispUslugaType['19'] != 1 && age < 18 )
			|| ( UsedDopDispUslugaType['20'] != 1 && age < 18 )
			|| ( UsedDopDispUslugaType['21'] != 1 && age < 1 )
			|| ( UsedDopDispUslugaType['22'] != 1 && age < 18 )
		)
		...
		if (
			...
			|| ( UsedDopDispSpec['6'] != 1 )
			|| ( UsedDopDispSpec['11'] != 1 && age < 18 )
			|| ( UsedDopDispSpec['12'] != 1 && age < 18 )
			|| ( UsedDopDispSpec['13'] != 1 && age < 18 )
			|| ( UsedDopDispSpec['14'] != 1 && age < 18 )
			|| ( UsedDopDispSpec['15'] != 1 && !(age > 17 || age < 3) )
			|| ( UsedDopDispSpec['16'] != 1 && !((age > 17 || age < 5) || sex_id == 2) )
			|| ( UsedDopDispSpec['17'] != 1 && !(age > 17 || age < 5) )
		)
			spec_is_full = false;
		
		var spec_combo = base_form.findField('DopDispSpec_id');

		spec_combo.getStore().filterBy(function(record) {
			/* запрещенные специальности */
			if ( record.get('DopDispSpec_id') == 4 )
				if ( spec_combo.getValue() == 4 )
					return true;
				else
					return false;
			if ( record.get('DopDispSpec_id') == 7 )
				if ( spec_combo.getValue() == 7 )
					return true;
				else
					return false;
			if ( record.get('DopDispSpec_id') == 8 )
				if ( spec_combo.getValue() == 8 )
					return true;
				else
					return false;
				
			if (
				( record.get('DopDispSpec_id') == 1 && (!spec_is_full || UsedDopDispSpec['10'] == 1 || age < 18) )
				|| ( record.get('DopDispSpec_id') == 2 && !(sex_id == 2) )
				|| ( record.get('DopDispSpec_id') == 5 && !(age > 17) )
				|| ( record.get('DopDispSpec_id') == 9 && (!spec_is_full || age > 17) )
				|| ( record.get('DopDispSpec_id') == 10 && (!spec_is_full || UsedDopDispSpec['1'] == 1 || age < 18) )
				|| ( record.get('DopDispSpec_id') == 11 && age > 17 )
				|| ( record.get('DopDispSpec_id') == 12 && age > 17 )
				|| ( record.get('DopDispSpec_id') == 13 && age > 17 )
				|| ( record.get('DopDispSpec_id') == 14 && age > 17 )
				|| ( record.get('DopDispSpec_id') == 15 && age > 17 || age < 3 )
				|| ( record.get('DopDispSpec_id') == 16 && ((age > 17 || age < 5) || sex_id == 2) )
				|| ( record.get('DopDispSpec_id') == 17 && (age > 17 || age < 5) )				
			)
				return false;
			...
			/*if ( record.get('DopDispSpec_Code').inlist([4, 7, 8]) ) {
				...
				if ( record.get('DopDispSpec_Code') == 1 && (!spec_is_full) ) {
					return false;
				}
	...
	show: function() {
		...
		this.SocStatus_id = arguments[0].SocStatus_id;
		var soc_status_id = this.SocStatus_id;
