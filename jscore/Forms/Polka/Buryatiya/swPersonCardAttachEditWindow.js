...
sw.Promed.swPersonCardAttachEditWindow = Ext.extend(sw.Promed.BaseForm, {
	...
	title: langs('Заявление о выборе МО'),
	maximizable: false,
	modal: true,
	resizable: true,
	...
	loadLpuRegionFapList: function(callback) {
		callback = callback || Ext.emptyFn;
		var base_form = this.FormPanel.getForm();
		...
		base_form.submit({
			...
			failure: function(result_form, action) {
				...
				if (response_obj.Error_Code == 101) {
					buttons = {yes: langs('Соглесен'), no: langs('Отмена')};
				}
	...
	show: function() {
		...
		switch(this.action) {
			case 'add':
				this.setTitle(langs('Заявление о выборе МО: Добавление'));
				this.enableEdit(true);
				Ext.getCmp('PCAEW_SaveButton').setText(langs('Согласен'));
				Ext.getCmp('PCAEW_CancelButton').setText(langs('Отказ'));
					...
					callback: function(result) {
						if (result.date) {
							...
							sw.swMsg.show({
								...
								icon: Ext.Msg.ERROR,
								msg: langs('Не удалось получить текущуюю дату с сервера.'),
								title: langs('Ошибка')
							});
						}
					}.createDelegate(this)
			...
			case 'view':
				if (this.action=='edit') {
					this.enableEdit(true);
					this.setTitle(langs('Заявление о выборе МО: Редактирование'));
				} else {
					this.enableEdit(false);
					this.setTitle(langs('Заявление о выборе МО: Просмотр'));
				}
				Ext.getCmp('PCAEW_SaveButton').setText(BTN_FRMSAVE);
				Ext.getCmp('PCAEW_CancelButton').setText(BTN_FRMCANCEL);
	...
	initComponent: function()
	{
		...
		this.FilesPanel = new Ext.Panel({
			layout: 'form',
			title: langs('Список приложенных документов: <span style="color: gray;">нет приложенных документов</span>'),
			autoHeight: true,
			buttons: [
				{
					handler: function() {
		...
		this.FormPanel = new Ext.FormPanel({
			...
			items: [{
				...
				name: 'PersonCardAttach_ExpNameFile',
				xtype: 'hidden'
			}, {
				name: 'PersonCardAttach_ExpNumRow',
				xtype: 'hidden'
			}, {
				layout: 'column',
				border: false,
				items: [{
					...
					items: [{
						...
						fieldLabel: langs('Дата заявления'),
						width: 120
					}]
				}]
			}, {
				...
				fieldLabel: langs('Участковый врач'),
