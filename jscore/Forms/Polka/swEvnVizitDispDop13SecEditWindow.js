/**
 * swEvnVizitDispDop13SecEditWindow - окно редактирования/добавления осмотра по доп. диспансеризации
 *
 ...
 */
/*NO PARSE JSON*/
sw.Promed.swEvnVizitDispDop13SecEditWindow = Ext.extend(sw.Promed.BaseForm,
	{
		/* */
		codeRefresh: true,
		objectName: 'swEvnVizitDispDop13SecEditWindow',
		objectSrc: '/jscore/Forms/Polka/swEvnVizitDispDop13SecEditWindow.js',
		action: null,
		buttonAlign: 'left',
		...
		doSave: function(callback) {
			...
			var base_form = this.findById('EvnVizitDispDop13SecEditForm').getForm();
			var index = -1;

			if (!current_window.findById('EvnVizitDispDop13SecEditForm').getForm().isValid()){
				Ext.MessageBox.show({
					buttons: Ext.Msg.OK,
					fn: function() {
						current_window.findById('EVDO13SEFEvnVizitDispDop_setDate').focus(false);
					},
			...
			base_form.findField('Diag_id').fireEvent('blur', base_form.findField('Diag_id'));

			var
				Diag_Code = base_form.findField('Diag_id').getFieldValue('Diag_Code'),
				Diag_id = base_form.findField('Diag_id').getValue(),
				Diag_Name = Diag_Code + '. ' + base_form.findField('Diag_id').getFieldValue('Diag_Name'),
				DopDispAlien_id = base_form.findField('DopDispAlien_id').getValue(),
				DopDispAlien_Name = base_form.findField('DopDispAlien_id').getFieldValue('DopDispAlien_Name'),
				DopDispDiagType_id = base_form.findField('DopDispDiagType_id').getValue(),
			...
			if (getRegionNick().inlist([ 'kareliya', 'penza' ]) && Ext.isEmpty(MedSpecOms_id)) {
			...
			if (
				UslugaComplex_Code && UslugaComplex_Code.inlist(pedcodes)
				&& base_form.findField('EvnVizitDispDop_setDate').getValue() < current_window.max_date
			) {
				Ext.MessageBox.show({
					buttons: Ext.Msg.OK,
					fn: function() {
						base_form.findField('EvnVizitDispDop_setDate').focus(false);
					},
					icon: Ext.Msg.WARNING,
			...
			var set_time = base_form.findField('EvnVizitDispDop_setTime').getValue();
			var dis_date = base_form.findField('EvnVizitDispDop_disDate').getValue();
			var dis_time = base_form.findField('EvnVizitDispDop_disTime').getValue();
			...
			var data = {
				'EvnVizitDispDop_id': base_form.findField('EvnVizitDispDop_id').getValue(),
				...
				'EvnVizitDispDop_setTime': base_form.findField('EvnVizitDispDop_setTime').getValue(),
				'EvnVizitDispDop_disTime': base_form.findField('EvnVizitDispDop_disTime').getValue(),
			...
			current_window.EvnDiagDopDispGrid.getGrid().getStore().filterBy(function(record) {
				return (record.get('Record_Status') != 3);
			});
			
			if (this.DispClass_id == 12) {
				current_window.getLoadMask("Подождите, идет сохранение...").show();

				data.EvnVizitDispDop_setDate = set_date.format('d.m.Y');
				data.EvnVizitDispDop_disDate = (!Ext.isEmpty(dis_date) ? dis_date.format('d.m.Y') : null);

				Ext.Ajax.request({
					url: '/?c=EvnPLDispTeenInspection&m=saveEvnVizitDispDop',
					params: data,
					...
					success: function(result) {
						...
						if ( !Ext.isEmpty(resp_obj.Error_Msg) ) {
							sw.swMsg.alert(langs('Ошибка'), resp_obj.Error_Msg);
							return false;
						}
						else if ( Ext.isEmpty(resp_obj.EvnVizitDispDop_id) ) {
							sw.swMsg.alert(langs('Ошибка'), langs('При сохранении произошли ошибки'));
							return false;
						}

						base_form.findField('EvnVizitDispDop_id').setValue(resp_obj.EvnVizitDispDop_id);
						current_window.EvnDirectionGrid.getGrid().getStore().baseParams.EvnDirection_pid = base_form.findField('EvnVizitDispDop_id').getValue();

						data.EvnVizitDispDop_id = resp_obj.EvnVizitDispDop_id;
						data.EvnVizitDispDop_setDate = set_date;
						data.EvnVizitDispDop_disDate = dis_date;
		...
		draggable: true,
		height: 490,
		id: 'EvnVizitDispDop13SecEditWindow',
		...
		openEvnDiagDopDispEditWindow: function(action) {
			...
			var base_form = win.findById('EvnVizitDispDop13SecEditForm').getForm();
			var grid = this.EvnDiagDopDispGrid.getGrid();

			var params = new Object();
			params.action = action;
			params.formParams = new Object();
			params.formParams.EvnDiagDopDisp_pid = base_form.findField('EvnVizitDispDop_id').getValue();
		...
		initComponent: function() {
			...
			Ext.apply(this, {
				buttons: [{
					...
					tabIndex: TABINDEX_EVDD13SEF + 15,
					text: BTN_FRMSAVE
				}, {
					text: '-'
				},
					HelpButton(this, -1),
					{
						...
						onTabAction: function() {
							Ext.getCmp('EVDO13SEFEvnVizitDispDop_setDate').focus(true, 200);
						},
						onShiftTabAction: function() {
							Ext.getCmp('EVDO13SEFSaveButton').focus(true, 200);
						},
						tabIndex: TABINDEX_EVDD13SEF + 16,
						text: BTN_FRMCANCEL
					}],
				items: [
					...
					new Ext.form.FormPanel({
						...
						border: false,
						frame: false,
						id: 'EvnVizitDispDop13SecEditForm',
						labelAlign: 'right',
						labelWidth: 150,
						items: [{
							id: 'EVDO13SEFEvnVizitDispDop_id',
							...
							items: [{
								...
								items: [{
									...
									format: 'd.m.Y',
									id: 'EVDO13SEFEvnVizitDispDop_setDate',
									...
									listeners: {
										...
										'change': function(field, newValue, oldValue) {
											...
											this.findById('EVDO13SEFDiagCombo').setFilterByDate(newValue);
								...
								items: [{
									...
									id: 'EVDO13SEFEvnVizitDispDop_setTime',
									name: 'EvnVizitDispDop_setTime',
									onTriggerClick: function() {
										var base_form = this.findById('EvnVizitDispDop13SecEditForm').getForm();

										var time_field = base_form.findField('EvnVizitDispDop_setTime');
										...
										setCurrentDateTime({
											...
											dateField: base_form.findField('EvnVizitDispDop_setDate'),
											loadMask: true,
											setDate: true,
							...
							items: [{
								...
								items: [{
									...
									format: 'd.m.Y',
									id: 'EVDO13SEFEvnVizitDispDop_disDate',
									name: 'EvnVizitDispDop_disDate',
									plugins: [ new Ext.ux.InputTextMask('99.99.9999', false) ],
									tabIndex: TABINDEX_EVDD13SEF + 3,
									width: 100,
									xtype: 'swdatefield'
								}]
							}, {
								...
								items: [{
									...
									id: 'EVDO13SEFEvnVizitDispDop_disTime',
									name: 'EvnVizitDispDop_disTime',
									onTriggerClick: function() {
										var base_form = this.findById('EvnVizitDispDop13SecEditForm').getForm();

										var time_field = base_form.findField('EvnVizitDispDop_disTime');
										...
										setCurrentDateTime({
											dateField: base_form.findField('EvnVizitDispDop_disDate'),
											loadMask: true,
											setDate: true,
											setDateMaxValue: true,
											setDateMinValue: false,
								...
								items: [{
									...
									handler: function() {
										...
										base_form.findField('EvnVizitDispDop_disDate').setValue(base_form.findField('EvnVizitDispDop_setDate').getValue());
										base_form.findField('EvnVizitDispDop_disTime').setValue(base_form.findField('EvnVizitDispDop_setTime').getValue());
									}.createDelegate(this)
								}]
							}]
						}, {
							...
							tabIndex: TABINDEX_EVDD13SEF + 10,
							width: 450,
							...
							tabIndex: TABINDEX_EVDD13SEF + 12,
							width: 450,
							...
							listWidth: 580,
							tabIndex: TABINDEX_EVDD13SEF + 13,
							width: 450,
							xtype: 'swdiagcombo'
						}, {
							allowBlank: false,
							...
							tabIndex: TABINDEX_EVDD13SEF + 14,
							validateOnBlur: false,
							width: 350,
			...
			sw.Promed.swEvnVizitDispDop13SecEditWindow.superclass.initComponent.apply(this, arguments);
		...
		filterProfileAndMedSpec: function() {
			...
			if ( !Ext.isEmpty(base_form.findField('EvnVizitDispDop_setDate').getValue()) ) {
				curDate = Ext.util.Format.date(base_form.findField('EvnVizitDispDop_setDate').getValue(), 'd.m.Y');
			}
		...
		keys: [{
			...
			fn: function(inp, e) {
				...
				var current_window = Ext.getCmp('EvnVizitDispDop13SecEditWindow');
		...
		filterLpuCombo: function() {
			...
			if ( !Ext.isEmpty(base_form.findField('EvnVizitDispDop_setDate').getValue()) ) {
				curDate = base_form.findField('EvnVizitDispDop_setDate').getValue();
			}
		...
		setLpuSectionAndMedStaffFactFilter: function ()
		{
			...
			var EvnVizitDispDop_setDate = base_form.findField('EvnVizitDispDop_setDate').getValue();
			var DopDispAlien_id = base_form.findField('DopDispAlien_id').getValue();
			...
			if ( !Ext.isEmpty(DopDispAlien_id) && DopDispAlien_id == 2 ) {
				...
				var set_date = Ext.getCmp('EVDO13SEFEvnVizitDispDop_setDate').getValue();

				var params = {};

				if ( !getRegionNick().inlist(['ekb']) ) {
					params.isAliens = (DopDispAlien_id == 2);
					params.isPolkaAndStom = true;
				}
				...
				if (getRegionNick() === 'pskov')
				{
					...
					if (UslugaComplex_id && EvnVizitDispDop_setDate)
					{
						params.UslugaComplex_MedSpecOms = {
							UslugaComplex_id: UslugaComplex_id,
							didDate: Ext.util.Format.date(EvnVizitDispDop_setDate, 'd.m.Y')
						};
					}
				}
		...
		show: function() {
			sw.Promed.swEvnVizitDispDop13SecEditWindow.superclass.show.apply(this, arguments);
			...
			var form = current_window.findById('EvnVizitDispDop13SecEditForm');
			...
			if (this.DispClass_id && this.DispClass_id == 12) {
				this.EvnDirectionGrid.show();
			} else {
				this.EvnDirectionGrid.hide();
			}
			...
			var loadMask = new Ext.LoadMask(Ext.get('EvnVizitDispDop13SecEditWindow'), { msg: LOAD_WAIT });
			loadMask.show();
			...
			uslugacomplex_combo.getStore().baseParams['DispFilter'] = "DispDop13SecVizit";
			...
			current_window.findById('EVDO13SEFEvnVizitDispDop_setDate').setValue(arguments[0].formParams['EvnVizitDispDop_setDate']);
			...
			switch (current_window.action)
			{
				case 'add':
					...
					loadMask.hide();
					current_window.findById('EVDO13SEFEvnVizitDispDop_setDate').focus(false, 250);
						...
						callback: function() {
							diag_combo.getStore().each(function(record) {
								if (record.data.Diag_Code == 'Z00.0')
								{
									...
									diag_combo.setFilterByDate(base_form.findField('EvnVizitDispDop_setDate').getValue());
								}
							});
						},
					...
					if (getRegionNick() == 'perm') {
						...
						if (!Ext.isEmpty(base_form.findField('EvnVizitDispDop_setDate').getValue())) {
							date1 = base_form.findField('EvnVizitDispDop_setDate').getValue();
						}
						if (date1 < date2) {
							uslugacategory_combo.setFieldValue('UslugaCategory_SysNick', 'tfoms');
						} else {
				...
				case 'view':
					...
					var setDate = base_form.findField('EvnVizitDispDop_setDate').getValue();
					var setTime = base_form.findField('EvnVizitDispDop_setTime').getValue();
					var disDate = base_form.findField('EvnVizitDispDop_disDate').getValue();
					var disTime = base_form.findField('EvnVizitDispDop_disTime').getValue();
					...
					this.EvnDirectionGrid.loadData({
						params: {EvnDirection_pid: base_form.findField('EvnVizitDispDop_id').getValue(), includeDeleted: 1},
						globalFilters: {EvnDirection_pid: base_form.findField('EvnVizitDispDop_id').getValue(), includeDeleted: 1}
					});

					this.setMinDate(current_window.action);
					loadMask.hide();
					current_window.findById('EVDO13SEFEvnVizitDispDop_setDate').fireEvent('change', current_window.findById('EVDO13SEFEvnVizitDispDop_setDate'), current_window.findById('EVDO13SEFEvnVizitDispDop_setDate').getValue());
					current_window.findById('EVDO13SEFEvnVizitDispDop_setDate').focus(false, 250);
					break;
			}
		...
		setMinDate: function(action)
		{
			if (action=='add')
			{
				...
				{
					this.findById('EVDO13SEFEvnVizitDispDop_setDate').setMinValue(Date.parseDate('01.01.' + this.Year, 'd.m.Y'));
				}
				else
				{
					this.findById('EVDO13SEFEvnVizitDispDop_setDate').setMinValue(Date.parseDate('01.01.' + (Date.parseDate(getGlobalOptions().date, 'd.m.Y').getFullYear()-1), 'd.m.Y'));
				}
			}
			else
			{
				var year = this.findById('EVDO13SEFEvnVizitDispDop_setDate').getValue().getFullYear();
				if (year)
				{
					this.findById('EVDO13SEFEvnVizitDispDop_setDate').setMinValue(Date.parseDate('01.01.' + year, 'd.m.Y'));
				}
			}
		},
		setDisDT: function() {
		...
		toggleVisibleDisDTPanel: function(action)
		{
			...
			if (this.isVisibleDisDTPanel) {
				...
				base_form.findField('EvnVizitDispDop_disDate').setAllowBlank(true);
				base_form.findField('EvnVizitDispDop_disTime').setAllowBlank(true);
				base_form.findField('EvnVizitDispDop_disDate').setValue(null);
				base_form.findField('EvnVizitDispDop_disTime').setValue(null);
				base_form.findField('EvnVizitDispDop_disDate').setMaxValue(undefined);
				this.isVisibleDisDTPanel = false;
			} else {
				this.findById('EVDO13SEF_EvnVizitDisDTPanel').show();
				this.findById('EVDO13SEF_ToggleVisibleDisDTBtn').setText(lang['skryit_polya']);
				base_form.findField('EvnVizitDispDop_disDate').setAllowBlank(false);
				base_form.findField('EvnVizitDispDop_disTime').setAllowBlank(false);
				base_form.findField('EvnVizitDispDop_disDate').setMaxValue(getGlobalOptions().date);
				this.isVisibleDisDTPanel = true;
			}
		},
		addDirectionIssled: function() {
			...
			if (Ext.isEmpty(base_form.findField('EvnVizitDispDop_id').getValue())) {
			...
			var EvnDirection_pid = base_form.findField('EvnVizitDispDop_id').getValue();
