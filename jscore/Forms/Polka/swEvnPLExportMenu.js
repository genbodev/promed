/**
* swEvnPLExportMenu - окно настроек экспорта ТАП в DBF
*
...
sw.Promed.swEvnPLExportMenu = Ext.extend(sw.Promed.BaseForm, {
	codeRefresh: true,
	objectName: 'swEvnPLExportMenu',
	objectSrc: '/jscore/Forms/Polka/swEvnPLExportMenu.js',
	closable: false,
	width : 500,
	height : 400,
	modal: true,
	 ...
	 */
	show: function() {
		sw.Promed.swEvnPLExportMenu.superclass.show.apply(this, arguments);		
		...
		this.ChGroup.reset();
		this.RdGroup.reset();
		Ext.getCmp('eplChSelector').reset();
		Ext.getCmp('eplDateType').reset();
		Ext.getCmp('EPL_UseFilterGroup').items.items[0].setValue(true);
		Ext.getCmp('EPL_UseFilterGroup').items.items[1].setValue(false);
	...
	initComponent: function() {
		...
		this.UseFilterGroup = new Object({
			xtype: 'radiogroup',
			hideLabel: true,
			id: 'EPL_UseFilterGroup',
			name: 'EPL_UseFilterGroup',
			columns: 1,
			items: [								
    	...
    	Ext.apply(this, {
			...
			buttons : [{
						...
						handler : function(button, event) {							
							...
							data.date_type = Ext.getCmp('eplDateType').getValue();
							data.filter_type = Ext.getCmp('EPL_UseFilterGroup').items.items[0].checked?1:2;
		...
		sw.Promed.swEvnPLExportMenu.superclass.initComponent.apply(this, arguments);
	} //end initComponent()
});