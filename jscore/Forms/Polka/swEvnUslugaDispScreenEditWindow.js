/**
* swEvnUslugaDispScreenEditWindow - окно редактирования/добавления выполнения осмотра/исследования по скрининговому исследованию
*
...
* @comment			Префикс для id компонентов EUDSEW (swEvnUslugaDispScreenEditWindow)
*
*
* Использует: окно редактирования талона по доп. диспансеризации (swEvnPLDispScreenEditWindow)
*/

sw.Promed.swEvnUslugaDispScreenEditWindow = Ext.extend(sw.Promed.BaseForm, {
	action: null,
	buttonAlign: 'left',
	...
	doSave: function(callback, nothide, options) {
		...
		var base_form = this.findById('EUDSEW_EvnUslugaDispDopEditForm').getForm();
		...
		if ( !base_form.isValid() )
		{
			sw.swMsg.show({
				...
				fn: function() {
					win.formStatus = 'edit';
					win.findById('EUDSEW_EvnUslugaDispDopEditForm').getFirstInvalidEl().focus(false);
				},
		...
		if (getRegionNick() == 'pskov' && !options.ignoreCheckPersonAge && this.SurveyType_Code == 27) {
			...
			if ( !Ext.isEmpty(newSetDate) && win.AgeGroupDispRecord ) {
				age_start = swGetPersonAge(win.findById('EUDSEW_PersonInformationFrame').getFieldValue('Person_Birthday'), newSetDate);
				var year = newSetDate.getFullYear();
				var endYearDate = new Date(year, 11, 31);
				age_end = swGetPersonAge(win.findById('EUDSEW_PersonInformationFrame').getFieldValue('Person_Birthday'), endYearDate);
				month_start = swGetPersonAgeMonth(win.findById('EUDSEW_PersonInformationFrame').getFieldValue('Person_Birthday'), newSetDate);
			...
			success: function(result_form, action) {
				...
				if ( action.result ) {
					if ( action.result.EvnUslugaDispDop_id ) {
						...
						var items = win.findById('EUDSEW_ResultsPanel').items.items;
	...
	draggable: true,
    height: 400,
	id: 'EvnUslugaDispScreenEditWindow',
    initComponent: function() {
        ...
        Ext.apply(this, {
			buttons: [{
				...
				iconCls: 'save16',
				id: 'EUDSEW_SaveButton',
				tabIndex: TABINDEX_EUDSEW + 80,
				text: BTN_FRMSAVE
			},
			'-',
			HelpButton(win, TABINDEX_EUDSEW + 81),
			{
				handler: function() {
					win.hide();
				},
				iconCls: 'cancel16',
				id: 'EUDSEW_CancelButton',
				onTabAction: function() {
					Ext.getCmp('EUDSEW_EvnUslugaDispDop_setDate').focus(true, 200);
				},
				onShiftTabAction: function() {
					Ext.getCmp('EUDSEW_SaveButton').focus(true, 200);
				},
				tabIndex: TABINDEX_EUDSEW + 82,
				text: BTN_FRMCANCEL
			}],
			items: [
				new	sw.Promed.PersonInformationPanelShort({
					id: 'EUDSEW_PersonInformationFrame',
					region: 'north'
				}),
				new Ext.form.FormPanel({
					...
					border: false,
					frame: false,
					id: 'EUDSEW_EvnUslugaDispDopEditForm',
					labelAlign: 'right',
					labelWidth: 150,
					items: [{
						id: 'EUDSEW_EvnUslugaDispDop_id',
						...
						items: [
							{
								allowBlank: false,
								id: 'EUDSEWUslugaComplexCombo',
								fieldLabel: lang['usluga'],
								hiddenName: 'UslugaComplex_id',
								listWidth: 590,
								showUslugaComplexEndDate: true,
								tabIndex: TABINDEX_EUDSEW + 1,
								width: 400,
								nonDispOnly: false,
								xtype: 'swuslugacomplexnewcombo',
								listeners: {
									'change': function (combo, newValue, oldValue) {
										var base_form = win.findById('EUDSEW_EvnUslugaDispDopEditForm').getForm();
										...
										Ext.Ajax.request({
											...
											success: function (response, action) {
												...
												win.findById('EUDSEW_ResultsPanel').add(
													{
														layout: 'form',
														border: false,
														labelWidth: 200,
												...
												win.findById('EUDSEW_ResultsPanel').doLayout();
											}
										});
									}
								}
							},
							{
								...
								items: [{
									...
									items: [{
										...
										format: 'd.m.Y',
										allowBlank: false,
										id: 'EUDSEW_EvnUslugaDispDop_setDate',
										listeners: {
											'change': function(field, newValue, oldValue) {
												if ( blockedDateAfterPersonDeath('personpanelid', 'EUDSEW_PersonInformationFrame', field, newValue, oldValue) ) {
													return false;
												}
												...
												if (getRegionNick() == 'kz' && win.DispClass_id && win.DispClass_id.inlist([13]))
												{
													win.UslugaComplex_Date = newValue;
													base_form.findField('UslugaComplex_id').getStore().baseParams.UslugaComplex_Date = (typeof win.UslugaComplex_Date == 'object' ? Ext.util.Format.date(win.UslugaComplex_Date, 'd.m.Y') : win.UslugaComplex_Date);
													win.loadUslugaComplexCombo();
								...
								id: 'EUDSEW_LpuSectionCombo',
								lastQuery: '',
								listWidth: 650,
								linkedElements: [
									'EUDSEW_MedPersonalCombo'
								],
								listeners: {
								...
								tabIndex: TABINDEX_EUDSEW + 4,
								width: 450,
								xtype: 'swlpusectionglobalcombo'
							}, {
								hiddenName: 'MedStaffFact_id',
								id: 'EUDSEW_MedPersonalCombo',
								lastQuery: '',
								listWidth: 650,
								parentElementId: 'EUDSEW_LpuSectionCombo',
                                listeners: {
									'select': function(combo, record, index) {
								...
								lastQuery: '',
								tabIndex: TABINDEX_EUDSEW + 7,
								width: 450,
								xtype: 'swcommonsprcombo',
								loadParams: {params: {where: (getRegionNick() == 'kz')?' where Region_id = 101':''}},
							}, {
								name: 'ScreenType_id',
								xtype: 'hidden'
							}
					...
					reader: new Ext.data.JsonReader({
						...
						{ name: 'terapevt_vop' },
						{ name: 'accoucheur_gynecologist' },
						{ name: 'colposcopy' },
						{ name: 'biopsy' },
						{ name: 'rectoromanoscopy' },
						{ name: 'in_prostate_cancer' },
						{ name: 'cancer_stomach' },
						{ name: 'el_cardiography'},
						{ name: 'fec_occult_blood'},
						{ name: 'fec_occult_blood_conducted'},
						{ name: 'blood_cholest_lvl'},
						{ name: 'blood_sugar_lvl'},
						{ name: 'uteri_carvix_scrning'},
						{ name: 'mammography_scrning'},
						{ name: 'mammography_scrning_conducted'},
						{ name: 'intraocular_tens'},
						{ name: 'colposcopy_res'},
						{ name: 'biopsy_res'},
						{ name: 'coloscopy_res'}
					]),
		...
		sw.Promed.swEvnUslugaDispScreenEditWindow.superclass.initComponent.apply(this, arguments);
    },
    ...
    keys: [{
        ...
        fn: function(inp, e) {
        	...
        	var current_window = Ext.getCmp('EvnUslugaDispScreenEditWindow');
							...
							index = base_form.findField('UslugaComplex_id').getStore().findBy(function (rec) {
								switch (win.SurveyType_Code) {
									...
									case 111:
										...
										break;
									case 8:
										return (rec.get('UslugaComplex_Code') == 'D89.113.338');
										break;
									case 19:
										return (rec.get('UslugaComplex_Code') == 'A01.001.000');
										break;
									case 34:
								}
								return false;
	...
	show: function() {
		sw.Promed.swEvnUslugaDispScreenEditWindow.superclass.show.apply(this, arguments);
        ...
        var form = current_window.findById('EUDSEW_EvnUslugaDispDopEditForm');
		...
		current_window.object = 'EvnPLDispScreen';
		...
		this.findById('EUDSEW_ResultsPanel').removeAll();
		...
		current_window.findById('EUDSEW_PersonInformationFrame').load({
			...
			callback: function() {
				...
				clearDateAfterPersonDeath('personpanelid', 'EUDSEW_PersonInformationFrame', field);
			}
		});

  		var loadMask = new Ext.LoadMask(Ext.get('EvnUslugaDispScreenEditWindow'), { msg: LOAD_WAIT });
		loadMask.show();
		...
		switch (current_window.action)
		{
			...
			case 'view':
				...
				// устанавливаем врача
				current_window.findById('EUDSEW_MedPersonalCombo').getStore().findBy(function(record) {
					if ( record.get('MedPersonal_id') == med_personal_id )
					{
						current_window.findById('EUDSEW_MedPersonalCombo').setValue(record.get('MedStaffFact_id'));
						return true;
					}
				});
				...
				if (!Ext.isEmpty(base_form.findField('EvnUslugaDispDop_id').getValue())) {
					...
					base_form.load({
						...
						url: '/?c=EvnPLDispScreen&m=loadEvnUslugaDispDop'
					});
				}
				else {
...
/*
		...
		var items = this.findById('EUDSEW_ResultsPanel').items.items;
		for (var key in items) {
			var obj = items[key];
