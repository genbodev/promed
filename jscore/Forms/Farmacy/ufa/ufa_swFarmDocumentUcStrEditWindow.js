/**
* swFarmDocumentUcStrEditWindow  - произвольное окно редактирования для аптек
* переделанное от swNewDocumentUcStrEditWindow
*
...
* @comment      
*/
sw.Promed.swFarmDocumentUcStrEditWindow = Ext.extend(sw.Promed.BaseForm, {
	autoHeight: false,
	title: 'Медикамент',
	layout: 'border',
	...
	checkDrugShipmentName: function(params) { //проверка уникальности номера партии, если партия уникальна, выполняется callback, иначе возвращается сообщение об ошибке
		...
		callback(); // т.к. Убираем проверку на уникальность партии
		
		/*
		 * Убираем проверку на уникальность партии
		 * 
	...
	setDrugDocumentType: function(type_code) { //настройка внешнего вида формы в зависимости от типа документа
		...
		switch(type_code) {
			...
			case 6: //Приходная накладная
				...
				this.form.findField('PrepSeries_GodnDate').allowBlank = false;
				this.form.findField('DrugShipment_Name').allowBlank = false;
				if (this.action == 'edit' && this.params.DrugShipment_Name.indexOf('БФ.') != -1) {
				    this.form.findField('DrugShipment_Name').ownerCt.show(); 
				    this.form.findField('DrugShipment_Name').enable_blocked = true;
				} else {
				    this.form.findField('DrugShipment_Name').ownerCt.hide(); 
	...
	setDisabled: function(disable) {
		...
		/*
		 * вроде дублирует предыдущий блок
	...
	doSave:  function() {
		...
		if (this.checkGodnDate() || confirm('Внимание! Указан срок годности меньше текущей даты. Продолжить сохранение?')) {
			...
			if (data.DrugShipment_Name == '1' && data.DocumentUcStr_Ser != '')
			    data.DrugShipment_Name = 'ЛС';  //data.DocumentUcStr_Ser;
...
//                            data.DocumentUcStr_oid = undefined;
	...
	show: function() {
        var wnd = this;
		sw.Promed.swFarmDocumentUcStrEditWindow.superclass.show.apply(this, arguments);		
		this.action = '';
...
//                }
		...
		if(wnd.action == 'edit' && wnd.params.DocumentUc_IsImport == 2) {
			wnd.setDisabled(true);
			wnd.buttons[0].enable();
			 Ext.getCmp('SEW_Lpu4FarmStorage').enable();
		}
		else 
	...
	initComponent: function() {
		...
		wnd.drug_combo = new sw.Promed.SwDrugComplexMnnCombo({
			...
			onTrigger2Click: function() {
				...
				getWnd(searchWindow).show({
					...
					searchUrl: '/?c=DocumentUc&m=farm_loadDrugComboForDocumentUcStr',
			...
			initComponent: function() {
				...
				this.store = new Ext.data.Store({
					...
					url: '/?c=DocumentUc&m=farm_loadDrugComboForDocumentUcStr'
				});
			}
		});
					...
					wnd.drug_combo,
					{
						...
						items: [{
							...
							store: new Ext.data.Store({
								...
								url: '/?c=DocumentUc&m=farm_loadDocumentUcStrOidCombo'
							}),
                                                         tpl: new Ext.XTemplate(
		...
		sw.Promed.swFarmDocumentUcStrEditWindow.superclass.initComponent.apply(this, arguments);
