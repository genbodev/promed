/**
* swFarmDocumentUcEditWindow  - окно редактирования документа учета аптеки
* переделано от swNewDocumentUcEditWindow
*
...
sw.Promed.swFarmDocumentUcEditWindow = Ext.extend(sw.Promed.BaseForm, {
	autoHeight: false,
	...
	doSave:  function(options) {
		...
		else if ( wnd.form.findField('DocumentUc_Num').value == "" ) {
			sw.swMsg.show({
					buttons: Ext.Msg.OK,
					/*
					fn: function() {
						wnd.form.findField('DocumentUc_Num').focus(true);
					},
					*/
					icon: Ext.Msg.WARNING,
					msg: 'Не заполнен номер документа',
					title: ERR_INVFIELDS_TIT
			});
			return false;
		}
	...
	submit: function(options) {
		...
		if (!this.form.findField('Contragent_sid').enable_blocked)
		    params.Org_id = wnd.form.findField('Contragent_sid').store.data.items [0].data.Org_id; 
		else
		    params.Org_id = this.form.findField('Org_id');
		...
		params.SubAccountType_sid = wnd.SubAccountType_sid;
		params.SubAccountType_tid = wnd.SubAccountType_tid;
		...
		switch(type_code) {
			...
			case 6: //Приходная накладная
				...
				this.print_report_data = [{
					...
					report_file: 'Posstelag.rptdesign',
					//report_format: 'doc'
					report_format: 'pdf' // Заменен по просьбе заказчика #94273
				}];
			...
			case 10: //Расходная накладная
				...
				this.form.findField('Contragent_tid').getStore().baseParams.ContragentType_CodeList = '3';  //  3 - Аптека
					//'1,3,5,6'; //1 - Организация; 3 - Аптека; 5 - Аптека МУ; 6 - Региональный склад;
			...
			case 17: //Возвратная накладная (расходная)
				this.form.findField('Contragent_tid').getStore().baseParams.ContragentType_CodeList = '6';  //  6 - Региональный склад
...
//				this.form.findField('Storage_tid').ownerCt.hide();
			...
			break;
                ...
                if (getGlobalOptions().region.nick == 'ufa') {
                    ...
                    this.form.findField('DocumentUc_InvoiceDate').ownerCt.hide(); 
                    this.form.findField('DocumentUc_pid').ownerCt.hide();
                    //this.form.findField('DocumentUc_didDate').enable_blocked = true;  //!!!
					if (this.action != 'add')
						this.form.findField('DocumentUc_setDate').enable_blocked = true;
					...
					this.form.findField('DocumentUc_Num').disable();
                     /*
					 this.form.findField('Storage_tid').enable_blocked = true;
                     this.form.findField('Storage_tid').ownerCt.hide();
					 */
		...
		if (getGlobalOptions().region.nick == 'ufa') {
			//Ext.getCmp('combo_Contragent_tid').getStore().sort('Contragent_Name, Contragent_tid');
			this.form.findField('combo_Contragent_tid').store.sort('Contragent_Name');
			this.form.findField('Contragent_sid').store.sort('Contragent_Name');
		}
		this.doLayout();
	...
	DrugPeriodCloseDt: function(fun) {
				...
				callback: function(options, success, response) {
					if (success) {
						...
						if(response_obj[0] && response_obj[0].DrugPeriodClose_DT) {
							wnd.DrugPeriodClose_DT = new Date(response_obj[0].DrugPeriodClose_DT);
	...
	loadForm: function(callback) {
		var wnd = this;
		wnd.SubAccountType_tid = null;
		wnd.SubAccountType_sid = null;  
		wnd.DocumentUc_IsImport = null;
		Ext.Ajax.request({
			failure:function () {
			...
			success: function (response) { 
				...
				if (result[0].SubAccountType_tid > 0) wnd.SubAccountType_tid = result[0].SubAccountType_tid;
				if (result[0].SubAccountType_sid > 0) wnd.SubAccountType_sid = result[0].SubAccountType_sid;
				if (result[0].DocumentUc_IsImport > 0) wnd.DocumentUc_IsImport = result[0].DocumentUc_IsImport;
				...
				var dt = wnd.form.findField('DocumentUc_setDate');
				// дата закрытия периода и дата создания документа: берем более позднюю дату
				wnd.form.findField('DocumentUc_didDate').setMinValue((dt.minValue > dt.getValue()) ? dt.minValue : dt.getValue());
				...
				if (wnd.action == 'edit' && wnd.DocumentUc_IsImport == '2') {
					wnd.setDisabled(true);
					wnd.buttons[0].enable();
					wnd.buttons[1].enable();
				//	wnd.StrGrid.getAction('action_edit').setDisabled(false);
				}
				else {
	...
	show: function() {  
        var wnd = this;
		sw.Promed.swFarmDocumentUcEditWindow.superclass.show.apply(this, arguments);		
		this.action = '';
		...
		switch (this.action) {
			case 'add':
				...
				//loadMask.hide();
				this.DrugPeriodCloseDt(function(){loadMask.hide()});
			break;
			case 'edit':
			case 'view':
		...
		//  this.form.findField('DocumentUc_InvoiceDate').hidden = true;
		    ...
		    // Создаем события для отслеживания изменений
		    Ext.getCmp('NewDocumentUcEditWindow').onchange = false;
		    var $items = Ext.getCmp('NewDocumentUcEditWindow').form.items.items;
		    for(var k in $items){
			if (typeof $items[k] == 'object') {
			    $items[k].on(
				'change', function(){
				    Ext.getCmp('NewDocumentUcEditWindow').onchange = true;
				}
			    );
			}
		    } 
	...
	initComponent: function() {
				...
				items: [{
						...
						items: [{
							...
							listeners: {
								change: function (combo, newValue) {
									...
									//wnd.form.findField('DocumentUc_didDate').setValue(this.getValue());
					...
					items: [{
						...
						items: [{
							...
							id: 'DUEW_WhsDocumentCostItemTypeCombo',
							width: 200,
							allowBlank: false,
							listeners: {
								select: function(combo, record, index ) { log('record = ', record);
											if(record.get('WhsDocumentCostItemType_Name').indexOf('БСК') != 0) { //  Если статья расхода не БСК
											var DrugFinance_id = record.get('DrugFinance_id');
											if (DrugFinance_id)
												wnd.form.findField('DrugFinance_id').setValue(DrugFinance_id);	
										}
									}
								}
						}]
						...
						setValueById: function(id) {
							...
							combo.store.load({
								callback: function(){
                                                                       ...
                                                                       combo.store.baseParams.Contragent_id = null; 
								       if (Ext.getCmp('NewDocumentUcEditWindow').DrugDocumentType_Code == 10) {
									   // Для расходной накладной отбираем только аптеки
									   //Ext.getCmp('combo_Contragent_tid').store.filter('ContragentType_id', '3');
								       }
		...
		this.StrGrid = new sw.Promed.ViewFrame({
			...
			autoLoadData: false,
			border: true,
			dataUrl: '/?c=DocumentUc&m=farm_loadDocumentUcStrList',
			height: 180,
			region: 'center',
			object: 'DocumentUcStr',
			editformclassname: 'swFarmDocumentUcStrEditWindow',
			...
			onRowSelect: function(sm,rowIdx,record) { 
				console.log('wnd.DocumentUc_IsImport = ' + wnd.DocumentUc_IsImport + '; this.action = ' + wnd.action);
				if (record.get('DocumentUcStr_id') > 0 && wnd.action == 'edit' && wnd.DocumentUc_IsImport == '2') {
					this.ViewActions.action_edit.setDisabled(false);
					this.ViewActions.action_delete.setDisabled(true);
				} else if (record.get('DocumentUcStr_id') > 0 && !this.readOnly) {
					this.ViewActions.action_edit.setDisabled(false);
			...
			editGrid: function (action, options) {
				...
				if (action == 'edit' || action == 'view') {
					...
					if (selected_record && selected_record.get('DocumentUcStr_id') > 0) {
						...
						params.DrugFinance_id = wnd.form.findField('DrugFinance_id').getValue();
						params.WhsDocumentCostItemType_id = wnd.form.findField('WhsDocumentCostItemType_id').getValue();
						params.DocumentUc_IsImport = wnd.DocumentUc_IsImport;
//						if (action == 'edit' && params.DocumentUc_IsImport == '2')
//							action = 
						getWnd(view_frame.editformclassname).show({
							owner: view_frame,
							action: action,
...
//                                console.log(this.ownerCt);
                                if (this.ownerCt.DocW != undefined) {
                                    ...
                                    if ($isRegionUfa && wnd.DrugDocumentType_Code == 6 && !this.ownerCt.checkEmptyLpu()) {
                                                    if (confirm('Не для всех медикаментов указана мед. организация. Продолжить?')) {
                                                        b.setDisabled(true);//деактивируем кнопку (исключение повторных нажатий)    
														this.ownerCt.doExecute(b);
                                                    }
                                    } else {
                                        b.setDisabled(true);//деактивируем кнопку (исключение повторных нажатий)            
										this.ownerCt.doExecute(b);
		...
		Ext.apply(this, {
			...
			HelpButton(this, 0),
			{
				handler: function() {	// Если не было изменений
				    if (Ext.getCmp('NewDocumentUcEditWindow').onchange == false) {
					this.ownerCt.hide();
				    }
				    else {	// Если внесены изменения
					sw.swMsg.show({
		...
		sw.Promed.swFarmDocumentUcEditWindow.superclass.initComponent.apply(this, arguments);
