/**
* swMorbusHIVVacEditWindow - Вакцинация.
*
...
sw.Promed.swMorbusHIVVacEditWindow = Ext.extend(sw.Promed.BaseForm, 
{
	action: null,
	autoHeight: true,
	buttonAlign: 'left',
	...
	doSave: function() 
	{
		...
		if(this.formMode == 'local')
		{
			...
			data.MorbusHIVVac_setDT = (data.MorbusHIVVac_setDT)?Date.parseDate(data.MorbusHIVVac_setDT,'d.m.Y'):null;
			win.hide();
			win.callback(data);
			return true;
		}
	...
	show: function() 
	{
		sw.Promed.swMorbusHIVVacEditWindow.superclass.show.apply(this, arguments);
		...
		this.MorbusHIVVac_id = arguments[0].MorbusHIVVac_id || null;
		this.callback = arguments[0].callback || Ext.emptyFn;
		this.onHide = arguments[0].onHide || Ext.emptyFn;

		if (!this.action) 
		{
			if ( this.MorbusHIVVac_id && this.MorbusHIVVac_id > 0 )
				this.action = "edit";
			else 
				this.action = "add";
		}
		...
		if (this.action != 'add' && this.formMode != 'local') {
			Ext.Ajax.request({
				...
				params:{
					MorbusHIVVac_id: this.MorbusHIVVac_id
				},
				...
				url:'/?c=MorbusHIV&m=loadMorbusHIVVac'
			});			
		} else {
			loadMask.hide();
		}
	...
	initComponent: function() 
	{
		...
		{	
			...
			labelAlign: 'right',
			labelWidth: 120,
			url:'/?c=MorbusHIV&m=saveMorbusHIVVac',
			items: 
			[{
				name: 'MorbusHIVVac_id',
				xtype: 'hidden'
			}, {
				name: 'MorbusHIV_id',
				xtype: 'hidden'
		...
		sw.Promed.swMorbusHIVVacEditWindow.superclass.initComponent.apply(this, arguments);
	}
});