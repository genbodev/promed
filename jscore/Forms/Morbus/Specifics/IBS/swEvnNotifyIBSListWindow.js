/**
 * swEvnNotifyIBSListWindow - Журнал Извещений по ИБС
 *
 ...
 * @version      12.2014
 * @comment      Префикс для id компонентов ENIBSLW
 */
sw.Promed.swEvnNotifyIBSListWindow = Ext.extend(sw.Promed.BaseForm, {codeRefresh: true,
	buttonAlign: 'left',
	closable: true,
	closeAction: 'hide',
	collapsible: true,
	getButtonSearch: function() {
		return Ext.getCmp('ENIBSLW_SearchButton');
	},
	doReset: function() {
	...
	doSearch: function(params) {
		...
		if ( !params.firstLoad && this.findById('EvnNotifyIBSListFilterForm').isEmpty() ) {
		...
		var post = getAllFormFieldValues(this.findById('EvnNotifyIBSListFilterForm'));
	...
	initComponent: function() {
		...
		this.RootViewFrame = new sw.Promed.ViewFrame({
			actions: [
				...
				{name: 'action_print', handler: function() {
					...
					var EvnNotifyIBS_id = selected_record.data.EvnNotifyIBS_id;
					if (EvnNotifyIBS_id) {
                        var s = '/?c=MorbusIBS&m=doPrintEvnNotifyIBS'
                            + '&EvnNotifyIBS_id=' + EvnNotifyIBS_id;
			...
			autoLoadData: false,
			dataUrl: C_SEARCH,
			id: 'ENIBSLW_EvnNotifyIBSListSearchGrid',
			object: 'EvnNotifyIBS',
			pageSize: 100,
			paging: true,
			region: 'center',
			root: 'data',
			stringfields: [
				{name: 'EvnNotifyIBS_id', type: 'int', header: 'ID', key: true},
				{name: 'EvnNotifyIBS_pid', type: 'int', hidden: true},
				{name: 'EvnNotifyIBS_setDT', type: 'date', format: 'd.m.Y', header: lang['data_sozdaniya'], width: 120},	
			...
			onRowSelect: function(sm,index,record) {
				...
				var disable = (Ext.isEmpty(record.get('Person_id')) || !Ext.isEmpty(record.get('PersonRegister_setDate')) || false == me.isIBSRegistryUser());
		...
		Ext.apply(this, {
			...
			HelpButton(this, -1),
			{
				...
				onTabAction: function() {
                    me.findById('ENIBSLW_SearchFilterTabbar').getActiveTab().fireEvent('activate', me.findById('ENIBSLW_SearchFilterTabbar').getActiveTab());
				},
				text: BTN_FRMCLOSE
			}],
			getFilterForm: function() {
				if ( me.filterForm == undefined ) {
                    me.filterForm = me.findById('EvnNotifyIBSListFilterForm');
				}
				return me.filterForm;
			},
			items: [ getBaseSearchFiltersFrame({
				allowPersonPeriodicSelect: true,
				id: 'EvnNotifyIBSListFilterForm',
				labelWidth: 130,
				ownerWindow: this,
				searchFormType: 'EvnNotifyIBS',
				tabPanelHeight: 235,
				tabPanelId: 'ENIBSLW_SearchFilterTabbar',
				tabs: [{
					autoHeight: true,
		...
		sw.Promed.swEvnNotifyIBSListWindow.superclass.initComponent.apply(this, arguments);
	...
	listeners: {
        ...
        'resize': function (win, nW, nH, oW, oH) {
			win.findById('ENIBSLW_SearchFilterTabbar').setWidth(nW - 5);
	...
	resizable: true,
	show: function() {
		sw.Promed.swEvnNotifyIBSListWindow.superclass.show.apply(this, arguments);
		...
		this.RootViewFrame.addActions({
			...
			iconCls: 'reset16',
			disabled: true,
			menu: new Ext.menu.Menu({id:'ENIBSLW_personRegisterNotIncludeMenu'})
		});
        ...
        if (false == this.isIBSRegistryUser()) {
            attach_lpu_combo.setValue(getGlobalOptions().lpu_id);
        } else {
            attach_lpu_combo.setValue(null);
        }
        attach_lpu_combo.setDisabled(false == this.isIBSRegistryUser());
    ...
    isIBSRegistryUser: function() {
        var isNotGroup = (String(getGlobalOptions().groups).indexOf('IBSRegistry', 0) < 0);
	...
	createPersonRegisterFailIncludeCauseMenu: function() {
		sw.Promed.personRegister.createPersonRegisterFailIncludeCauseMenu({
			id: 'ENIBSLW_personRegisterNotIncludeMenu',
			ownerWindow: this,
			getParams: function(){
				...
				return {
					EvnNotifyBase_id: record.get('EvnNotifyIBS_id')
				};
			}.createDelegate(this),
	...
	openViewWindow: function()
	{
		...
		if ( getWnd('swEvnNotifyIBSEditWindow').isVisible() ) {
			getWnd('swEvnNotifyIBSEditWindow').hide();
		}
		getWnd('swEvnNotifyIBSEditWindow').show({
            action: 'view',
            EvnNotifyIBS_id: record.get('EvnNotifyIBS_id'),
            formParams: {
                EvnNotifyIBS_id: record.get('EvnNotifyIBS_id')
            }
        });
	...
	title: lang['jurnal_izvescheniy_po_ibs'],
	width: 800
});
