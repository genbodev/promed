/**
* swMorbusHIVChemPregEditWindow - «Проведение перинатальной профилактики ВИЧ».
*
...
sw.Promed.swMorbusHIVChemPregEditWindow = Ext.extend(sw.Promed.BaseForm, 
{
	action: null,
	autoHeight: true,
	buttonAlign: 'left',
	...
	doSave: function() 
	{
		...
		if(this.formMode == 'local')
		{
			...
			data.HIVPregnancyTermType_Name = base_form.findField('HIVPregnancyTermType_id').getRawValue();
			win.hide();
			win.callback(data);
			return true;
		}
	...
	show: function() 
	{
		sw.Promed.swMorbusHIVChemPregEditWindow.superclass.show.apply(this, arguments);
		...
		this.MorbusHIVChemPreg_id = arguments[0].MorbusHIVChemPreg_id || null;
		this.callback = arguments[0].callback || Ext.emptyFn;
		this.onHide = arguments[0].onHide || Ext.emptyFn;

		if (!this.action) 
		{
			if ( this.MorbusHIVChemPreg_id && this.MorbusHIVChemPreg_id > 0 )
				this.action = "edit";
			else 
				this.action = "add";
		}
		...
		{
			case 'add':
				this.setTitle(lang['provedenie_perinatalnoy_profilaktiki_vich_dobavlenie']);
				this.setFieldsDisabled(false);
				break;
			case 'edit':
				this.setTitle(lang['provedenie_perinatalnoy_profilaktiki_vich_redaktirovanie']);
				this.setFieldsDisabled(false);
				break;
			case 'view':
				this.setTitle(lang['provedenie_perinatalnoy_profilaktiki_vich_prosmotr']);
		...
		if (this.action != 'add' && this.formMode == 'remote') {
			...
			Ext.Ajax.request({
				...
				params:{
					MorbusHIVChemPreg_id: this.MorbusHIVChemPreg_id
				},
				...
				url:'/?c=MorbusHIV&m=loadMorbusHIVChemPreg'
			});			
		}
	...
	initComponent: function() 
	{
		...
		{	
			...
			labelAlign: 'right',
			labelWidth: 120,
			url:'/?c=MorbusHIV&m=saveMorbusHIVChemPreg',
			items: 
			[{
				name: 'MorbusHIVChemPreg_id',
				xtype: 'hidden'
			}, {
				name: 'MorbusHIV_id',
				xtype: 'hidden'
				...
				name: 'MorbusHIVChemPreg_Dose',
				allowBlank: false,
				xtype: 'textfield',
				maxLength: 20
			}, {
				fieldLabel: lang['period'],
				comboSubject: 'HIVPregnancyTermType',
				allowBlank: false,
				width: 300,
				listWidth: 500,
				typeCode: 'int',
		...
		sw.Promed.swMorbusHIVChemPregEditWindow.superclass.initComponent.apply(this, arguments);
	}
});