/**
 * swTTRScheduleAddDopWindow - окно добавления дополнительной бирки для ресурса
 *
...
/*NO PARSE JSON*/
sw.Promed.swTTRScheduleAddDopWindow = Ext.extend(sw.Promed.BaseForm, {
	title: lang['dobavlenie_dopolnitelnoy_birki'],
	id: 'TTRScheduleAddDopWindow',
	layout: 'border',
	maximizable: false,
	width: 450,
	height: 270,
	...
	resizable: false,
	codeRefresh: true,
	objectName: 'swTTRScheduleAddDopWindow',
	objectSrc: '/jscore/Forms/Reg/swTTRScheduleAddDopWindow.js',
	...
	show: function()
	{
		sw.Promed.swTTRScheduleAddDopWindow.superclass.show.apply(this, arguments);
		
		var form = this.findById('TTRScheduleAddDopForm');
		base_form = form.getForm();
		this.findById('ttraCreateAnnotationPanel').collapse();
		this.findById('ttraCreateAnnotationPanel').checkbox.dom.checked = false;
	...
	doSave: function()
	{
		...
		var loadMask = new Ext.LoadMask(Ext.get('TTRScheduleAddDopForm'), { msg: "Подождите, идет создание новых бирок..." });
		loadMask.show();
	...
	initComponent: function()
	{
				...
				buttons:
					[{
						...
						},
						{
							...
							{
								this.findById('TTRadScheduleCreationType').focus();
							}.createDelegate(this),
							onShiftTabAction: function()
							{
								Ext.getCmp('TTRadCreate').focus();
							}.createDelegate(this),
							handler: function()
							{
								this.hide();
		...
		sw.Promed.swTTRScheduleAddDopWindow.superclass.initComponent.apply(this, arguments);
	}
});