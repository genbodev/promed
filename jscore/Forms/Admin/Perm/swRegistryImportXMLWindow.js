...
sw.Promed.swRegistryImportXMLWindow = Ext.extend(sw.Promed.BaseForm, {
	...
	title: 'Импорт ошибок МЭК от СМО',
	width: 400,
	//layout: 'form',
	resizable: false,
	onHide: Ext.emptyFn,
	...
	doSave: function() 
	{
		...
		sw.swMsg.show({
			...
			msg: 'При импорте все ошибки от СМО из протокола будут удалены, продолжить?',
			title: 'Вопрос'
		});
	...
	submit: function() 
	{
		var form = this.TextPanel;
		var win = this;
		win.getLoadMask('Загрузка протокола МЭК. Подождите...').show();
			...
			success: function(result_form, action) 
			{
				...
				if (answer) 
				{
					if (answer.Registry_id)
					{
						...
						var errorText = "";
						if (!Ext.isEmpty(answer.errorFile)) {
							errorText = "<br>В процессе импорта возникли <a href='" + answer.errorFile + "' target='blank'>ошибки.</a>";
						}
						win.RegistryImportTpl.overwrite(win.RegistryImportPanel.body, 
						{
							recAll: "Всего записей с ошибками:  <b>"+answer.recAll+"</b>" + errorText
						});
