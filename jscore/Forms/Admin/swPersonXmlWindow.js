...
sw.Promed.swPersonXmlWindow = Ext.extend(sw.Promed.BaseForm, {
	...
	initComponent: function() {
		...
		win.exportTypeGroup = new sw.Promed.Panel({
			items: [{
				xtype: 'radio',
				hideLabel: true,
				boxLabel: lang['vse_dannyie'],
				inputValue: 0,
				id: 'rxw_radio_alldata',
				listeners: {
					'check': function(radio, checked) {
						var base_form = win.Panel.getForm();

						if ( checked == true ) {
							base_form.findField('Date_upload').disable();
							base_form.findField('Date_upload').setAllowBlank(true);
						}
					}
				},
				name: 'exporttype'
				...
				items: [{
					...
					items: [{
						xtype: 'radio',
						hideLabel: true,
						boxLabel: lang['izmeneniya_posle_datyi'],
						inputValue: 1,
						id: 'rxw_radio_changes',
						listeners: {
							'check': function(radio, checked) {
								var base_form = win.Panel.getForm();

								if ( checked == true ) {
									base_form.findField('Date_upload').setAllowBlank(false);
									base_form.findField('Date_upload').enable();
								}
							}
						},
						name: 'exporttype',
	...
	createXML: function(addParams) 
	{
		...
		if ( form.Panel.findById('rxw_radio_changes').getValue() && !Ext.isEmpty(base_form.findField('Date_upload').getValue()) ) {
	...
	show: function() 
	{
		...
		if ( getRegionNick().inlist([ 'astra' ]) ) {
			...
			form.Panel.findById('rxw_radio_alldata').setValue(false);
			form.Panel.findById('rxw_radio_changes').setValue(true);

			base_form.findField('Date_upload').setValue(getGlobalOptions().date);

			form.Panel.findById('rxw_radio_changes').fireEvent('check', form.Panel.findById('rxw_radio_changes'), true);
		}
		else {
