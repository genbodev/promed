/**
* swMesComboSearchWindow - окно поиска МЭС по диагнозу
*
...
sw.Promed.swMesComboSearchWindow = Ext.extend(sw.Promed.BaseForm, {
	buttonAlign: 'left',
	closeAction : 'hide',
	doReset: function() {
		this.findById('MCSW_MesComboSearchGrid').getStore().removeAll();
		this.findById('MesComboSearchForm').getForm().reset();
	...
	doSearch: function() {
		...
		var thisWindow = this;
		var grid = this.findById('MCSW_MesComboSearchGrid');
		var Mask = new Ext.LoadMask(Ext.get('swMesComboSearchWindow'), { msg: SEARCH_WAIT });
		var params = this.findById('MesComboSearchForm').getForm().getValues();
	...
	initComponent: function() {
		Ext.apply(this, {
			...
			items: [ new Ext.form.FormPanel({
				...
				keys: [{
					fn: function(e) {
						Ext.getCmp('swMesComboSearchWindow').doSearch();
					},
			...
			new Ext.grid.GridPanel({
				...
				keys: {
					...
					fn: function(inp, e) {
                    	...
                    	var grid = Ext.getCmp('MCSW_MesComboSearchGrid');
				...
				store: new Ext.data.JsonStore({
					...
					url: '/?c=Mes&m=loadMesComboSearchList'
				}),
		       	stripeRows: true
			})]
		});
		sw.Promed.swMesComboSearchWindow.superclass.initComponent.apply(this, arguments);
	},
	layout: 'border',
	listeners: {
		'hide': function() {
	...
	onOkButtonClick: function() {
		if (!this.findById('MCSW_MesComboSearchGrid').getSelectionModel().getSelected())
        {
        	this.hide();
        	return false;
        }

		var selected_record = this.findById('MCSW_MesComboSearchGrid').getSelectionModel().getSelected();
	...
	resizable: false,
	show: function() {
		sw.Promed.swMesComboSearchWindow.superclass.show.apply(this, arguments);
