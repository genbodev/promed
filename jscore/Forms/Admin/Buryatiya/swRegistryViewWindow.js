...
sw.Promed.swRegistryViewWindow = Ext.extend(sw.Promed.BaseForm,
{
	...
	onTreeClick: function(node,e)
	{
		...
		switch (level)
		{
			...
			case 4:
				...
				owner.ErrorGrid.setColumnHidden('NumCard', (RegistryType_id==15));
				owner.TFOMSErrorGrid.setColumnHidden('NumCard', (RegistryType_id==15));
				...
				if (RegistryType_id==1 || RegistryType_id==14)
				{
					...
					owner.DataGrid.setColumnHidden('Mes_Code', RegistryType_id!=1);
				}
				else 
				{
				...
				if (RegistryType_id==6) {
					...
					owner.DataGrid.auditOptions.field = 'CmpCloseCard_id';
				} else {
	...
	*/
	...
	/**
	 * Удаляем реестр 
	 * options.ignoreDeleteRemind @Boolean Не запрашивать подтверждение на удаление, если true
	 * options.ignoreDeleteLinkRemind @Boolean Не запрашивать подтверждение на удаление связей с объединенным реестром, если true
	*/
	deleteRegistry: function(options)
	{
		var form = this;
		var current_window = Ext.getCmp('RegistryViewWindow');
		var grid = current_window.AccountGrid.getGrid();
		if (Ext.isEmpty(options.Registry_id)) {
			var record = grid.getSelectionModel().getSelected();
			if (!record) {
				Ext.Msg.alert('Ошибка', 'Не выбран ни один счет/регистр.<br/>');
				return false;
			}
			options.id = record.get('Registry_id');
			if (!options.id) {
				return false;
			}
		}
		...
		params.ignoreDeleteRemind = (!Ext.isEmpty(options.ignoreDeleteRemind) && options.ignoreDeleteRemind === 1) ? 1 : 0;
		params.ignoreDeleteLinkRemind = (!Ext.isEmpty(options.ignoreDeleteLinkRemind) && options.ignoreDeleteLinkRemind === 1) ? 1 : 0;

		if (Ext.isEmpty(options.ignoreDeleteRemind)) {
			msg = '<b style="color:maroon;">В большинстве случаев гораздо удобнее переформировать реестр, <br/>предварительно исправив ошибки, чем удалить и создать новый.</b> <br/>Вы действительно хотите удалить реестр?';
			sw.swMsg.show(
			{
				...
				{
					if ( buttonId == 'yes' ) {
						options.ignoreDeleteRemind = 1;
						form.deleteRegistry(options);
						return true;
					}
				},
			...
			Ext.Ajax.request(
			{
				url: '/?c=Registry&m=deleteRegistry',
				params: params,
				callback: function(options, success, response) 
				{
					var result = Ext.util.JSON.decode(response.responseText);
						...
						if (!Ext.isEmpty(result.Alert_Msg)) {
							...
							sw.swMsg.show(
							{
								...
								fn: function(buttonId) 
								{
									if ( buttonId == 'yes' ) {
										options.ignoreDeleteRemind = 1;
										options.ignoreDeleteLinkRemind = 1;
										form.deleteRegistry(options);
										return true;
									}
								},
	...
	*/
	...
	setRegistryStatus: function(RegistryStatus_id)
	{
		...
		if ( RegistryStatus_id == 4 && (isSuperAdmin() || isRegistryUser()) ) {
	...
	exportRegistryToXml: function()
	{
		...
		if (record.get('Registry_IsNeedReform') == 2) {
			Ext.Msg.alert('Ошибка', 'Часть реестров нуждается в переформировании, экспорт невозможен.');
			return false;
		}
		...
		var params = {
			callback: function() {
				if ( record.get('RegistryCheckStatus_Code') != 1 ) {
					sw.swMsg.show({
						buttons: Ext.Msg.YESNO,
						fn: function ( buttonId ) {
							if ( buttonId == 'yes' ) {
								this.setRegistryCheckStatus(this.UnionRegistryGrid, 1);
							}
						}.createDelegate(this),
			...
			callback: function()
			{
				if ( record.get('RegistryCheckStatus_Code') == 1 ) {
					sw.swMsg.show({
						...
						fn: function ( buttonId ) {
							if ( buttonId == 'yes' ) {
								this.setRegistryCheckStatus(this.UnionRegistryGrid, null);
							}
						}.createDelegate(this),
	...
	setUnionRegistryMenuActions: function(object)
	{
		...
		object.addActions({
			...
			hidden: isUserGroup('RegistryUserReadOnly')
		});
		...
		},
		{
			...
			handler: function() 
			{
				form.setRegistryCheckStatus(form.UnionRegistryGrid, 1);
			}
		},
		{
			text: 'Разблокировать',
			...
			handler: function() 
			{
				form.setRegistryCheckStatus(form.UnionRegistryGrid, null);
			}
		}];
	...
	setRegistryCheckStatus: function(grid, RegistryCheckStatus_Code) {
		...
		if ( !Ext.isEmpty(RegistryCheckStatus_Code) ) {
			params.RegistryCheckStatus_Code = RegistryCheckStatus_Code;
		}
	...
	openForm: function (object, oparams, frm)
	{
		...
		var RegistryType_id = this.AccountGrid.getGrid().getSelectionModel().getSelected().get('RegistryType_id'),
			RegistryStatus_id = this.AccountGrid.getGrid().getSelectionModel().getSelected().get('RegistryStatus_id');
		...
		switch (type)
		{
			...
			case 15:
				...
				}else if(record.get('EvnClass_SysNick') == 'EvnUslugaCommon'){
					open_form = 'swEvnUslugaEditWindow';
					key = 'EvnUslugaCommon_id';
					parentClass = "EvnPL";
				}
		...
		var params = {action: 'edit', Person_id: Person_id, Server_id: Server_id, PersonEvn_id: PersonEvn_id, usePersonEvn: usePersonEvn, Evn_setDT: Evn_setDT}; //, Person_id: this.Person_id, Server_id: this.Server_id
		...
		if (open_form.inlist(['swCmpCallCardNewCloseCardWindow','swEvnUslugaEditWindow'])) { // карты вызова
	...
	initComponent: function() 
	{
		...
		this.AccountGrid = new sw.Promed.ViewFrame(
		{
			...
			onRowSelect: function(sm,index,record)
			{
					...
					if (!deletedRegistriesSelected){
						if (record.get('RegistryType_id').inlist(['1','2','6','14'])) {
							...
							if (record.get('RegistryStatus_id')=='2' && Ext.menu.MenuMgr.get('RegistryMenu').items.itemAt(1)) {
								Ext.menu.MenuMgr.get('RegistryMenu').items.itemAt(0).setVisible(record.get('RegistryCheckStatus_Code') != 1); // если не заблокирован (refs #70754)
								Ext.menu.MenuMgr.get('RegistryMenu').items.itemAt(1).setVisible(record.get('RegistryCheckStatus_Code') != 1); // если не заблокирован (refs #70754)
								Ext.menu.MenuMgr.get('RegistryMenu').items.itemAt(2).setVisible(record.get('RegistryCheckStatus_Code') != 1); // если не заблокирован (refs #70754)
								Ext.menu.MenuMgr.get('RegistryMenu').items.itemAt(3).setVisible(isSuperAdmin() || isLpuAdmin(record.get('Lpu_id')) || isRegistryUser() || record.get('KatNasel_SysNick') == 'inog'); // суперадмин или иногородние (refs #12116)
							}
							// Если оплачен
							if (record.get('RegistryStatus_id')=='4' && Ext.menu.MenuMgr.get('RegistryMenu').items.itemAt(1)) {
								Ext.menu.MenuMgr.get('RegistryMenu').items.itemAt(1).setVisible(isSuperAdmin() || isLpuAdmin(record.get('Lpu_id')) || record.get('KatNasel_SysNick') == 'inog'); // суперадмин или иногородние (refs #12116)
							}
							// Если в работе
				...
				if(isUserGroup('RegistryUserReadOnly')) {
		...
		this.DataGrid = new sw.Promed.ViewFrame(
		{
			...
			[
				...
				{name: 'Diag_Code', header: 'Код диагноза', width: 70},
			...
			onRowSelect: function(sm,rowIdx,record)
			{
				...
				if(isUserGroup('RegistryUserReadOnly')) {
					this.setReadOnly(true);
					this.setActionDisabled('action_delete_all_records',true);
					this.setActionDisabled('action_undelete_all_records',true);
				}
			},
		...
		this.ErrorGrid = new sw.Promed.ViewFrame(
		{
			...
			onRowSelect: function(sm,rowIdx,record)
			{
				...
				if(isUserGroup('RegistryUserReadOnly')) {
					this.setReadOnly(true);
					this.setActionDisabled('action_deleteall',true);
				}
			},
			onLoadData: function()
			{
		...
		this.ErrorGrid.ViewGridPanel.view = new Ext.grid.GridView(
		{
			getRowClass : function (row, index)
			{
				...
				if (row.get('RegistryData_deleted') == 2)
					cls = cls+'x-grid-rowdeleted ';
				if (row.get('RegistryErrorClass_id') == 2 && row.get('RegistryData_notexist') == 2)
					cls = cls+'x-grid-rowdeleted ';
				if (cls.length == 0)
					cls = 'x-grid-panel'; 
				return cls;
		...
		this.NoPolisGrid = new sw.Promed.ViewFrame(
		{
			...
			[
				...
				{name: 'Evn_setDate', type:'datetime', header: 'Начало', width: 70},
				{name: 'Evn_disDate', type:'datetime', header: 'Окончание', width: 70}
			],
			actions:
			[
				{name:'action_add', disabled: true, hidden: true },
		...
		this.TFOMSErrorGrid = new sw.Promed.ViewFrame(
		{
			...
			[
				...
				{name: 'RegistryErrorTFOMS_Comment', header: 'Комментарий', width: 200, hidden:true},
		...
		this.DoubleVizitGrid = new sw.Promed.ViewFrame({
			...
			onLoadData: function()
			{
				if(isUserGroup('RegistryUserReadOnly')) {
					this.setReadOnly(true);
					this.setActionDisabled('action_delete_all',true);
				}
			}
		});
		...
		this.DataTab = new Ext.TabPanel(
		{
			...
			[{
				...
				{
					title: '6. Итоги проверки ФЛК/ПТК',
					layout: 'fit',
					iconCls: 'good',
					id: 'tab_datatfomserr',
					border:false,
		...
		this.UnionRegistryGrid = new sw.Promed.ViewFrame(
		{
			...
			onRowSelect: function(sm,rowIdx,record)
			{
				...
				if (!Ext.isEmpty(record.get('Registry_id')))
				{
					...
					if (record.get('KatNasel_SysNick').inlist(['inog','oblast'])) {
				...
				if(isUserGroup('RegistryUserReadOnly')) {
					form.UnionRegistryGrid.setReadOnly(true);
				}
			},
			stringfields:
			[
