...
sw.Promed.swRegistryPaidEditWindow = Ext.extend(sw.Promed.BaseForm, {
	...
	doSave: function()
	{
		...
		if ( win.data.length > 0 ) {
			...
			for(var key in win.data) {
				...
				if (record.Evn_id) {
					var is_paid = 1;
					if ((Ext.isEmpty(record.RegistryData_IsPaid) || record.RegistryData_IsPaid == 2) && (Ext.isEmpty(record.RegistryErrorType_Code) || record.RegistryErrorType_Code == '1016')) {
						is_paid = 2;
					}
	...
	doSearch: function(clear)
	{
		...
		params.RegistryErrorType_Code = base_form.findField('RegistryErrorType_id').getCode();
	...
	checkRenderer: function(v, p, record) {
		...
		var value = 'value="'+id+'"';
		var checked = record.get('RegistryData_IsPaid')==1 ? ' checked="checked"' : '';
		var onclick = 'onClick="getWnd(\'swRegistryPaidEditWindow\').checkOne(this.value);"';
		var disabled = '';
		//disabled = (Ext.isEmpty(record.get('RegistryErrorType_Code')) || record.get('RegistryErrorType_Code') == '1016'?'':'disabled="disabled"');
	...
	checkOne: function(id) {
		...
		if (record) {
			var newVal = (record.get('RegistryData_IsPaid')==2 || Ext.isEmpty(record.get('RegistryData_IsPaid')))?1:2;
	...
	checkAllPaidCheckbox: function()
	{
		...
		win.filtredData.forEach(function(rec) {
			rec.RegistryData_IsPaid = 1;
		});
	},
	...
	show: function()
	{
		...
		var win = this,
			filtersForm = this.FiltersPanel.getForm();
		...
		filtersForm.findField('RegistryErrorType_id').getStore().load();
		Ext.Ajax.request({
	...
	loadRecords: function(params) {
		...
		win.filtredData = win.data.filter(function(el){
			resp = true;
			...
			if (!Ext.isEmpty(params.Evn_id)) {
				resp = (resp && String(el.Evn_id) == params.Evn_id);
			}
			...
			if (!Ext.isEmpty(params.filterRecords)) {
				switch(params.filterRecords) {
					...
					break;
					case 2:
						resp = resp && (el.RegistryData_IsPaid == 2 || Ext.isEmpty(el.RegistryData_IsPaid));
					break;
					case 3:
			...
			if (!Ext.isEmpty(params.RegistryErrorType_Code)) {
				resp = (resp && String(el.RegistryErrorType_Code) === params.RegistryErrorType_Code);
			}
			
			return resp;
		});

		response_obj.totalCount = win.filtredData.length;
		response_obj.data = win.filtredData.slice(params.start, params.start+params.limit);
		win.GridPanel.getGrid().getStore().loadData(response_obj);
	},
		...
		this.GridPanel = new sw.Promed.ViewFrame(
			{
				...
				onLoadData: function() {
					...
					this.getGrid().getStore().each(function(record) {
						if ( !Ext.isEmpty(record.get('RegistryErrorType_Code')) && record.get('RegistryErrorType_Code') != '1016' ) {
							record.set('RegistryData_IsPaid', 1);
							record.commit();
