...
sw.Promed.swRegistryViewWindow = Ext.extend(sw.Promed.BaseForm,
{
	...
	onTreeClick: function(node,e)
	{
		...
		switch (level)
		{
			...
			case 4:
				...
				owner.AccountGrid.setActionDisabled('action_print', (RegistryStatus_id!=4));
				...
				owner.AccountGrid.getAction('action_yearfilter').setHidden(RegistryStatus_id && !RegistryStatus_id.inlist([4,6]));
				if( RegistryStatus_id && RegistryStatus_id.inlist([4,6]) ) {
	...
	exportRegistryToDBF: function()
	{
		...
		getWnd('swUnionRegistryDBFWindow').show({
			onHide: function() {
				this.UnionRegistryGrid.loadData();
			}.createDelegate(this),
			Registry_id: record.get('Registry_id'),
			...
			RegistryType_id: record.get('RegistryType_id'),
			url:'/?c=Registry&m=exportUnionRegistryToDBF'
		});
	},
	...
	importRegistryFromSMO: function()
	{
		var record = this.UnionRegistryGrid.getGrid().getSelectionModel().getSelected();
		if (!record) {
			swsw.Msg.alert('Ошибка', 'Не выбран ни один счет/регистр.');
			return false;
		}
	...
	initComponent: function()
	{
		...
		this.AccountGrid = new sw.Promed.ViewFrame({
			...
			autoLoadData: false,
			stringfields: [
				{name: 'Registry_id', type: 'int', header: 'ИД реестра', key: true, hidden:!isSuperAdmin()},
				{name: 'Lpu_id', type: 'int', header: 'ИД МО', hidden:!isSuperAdmin()},
				...
				{name: 'RegistryNoPaid_IsData', type: 'int', hidden: true},
			...
			onRowSelect: function(sm,index,record)
			{
				...
				if (this.getCount()>0)
				{
					...
					this.setActionDisabled('action_delete',(record.get('RegistryStatus_id')!=3));
					this.setActionDisabled('action_edit',(record.get('RegistryStatus_id')!=3));
				...
				form.DataTab.getItem('tab_datanopay').setIconClass((record.get('RegistryNoPaid_IsData')==1)?'usluga-notok16':'good');
		...
		this.DataGrid = new sw.Promed.ViewFrame({
			...
			stringfields: [
				...
				{name: 'Mes_Code', header: 'КСГ', width: 80},
		...
		this.UnionRegistryGrid = new sw.Promed.ViewFrame({
			...
			onRowSelect: function(sm,rowIdx,record)
			{
				...
				if (record.get('KatNasel_SysNick') == 'all') {
					form.UnionActionsMenu.items.items[0].show();
					form.UnionActionsMenu.items.items[1].hide();
					form.UnionActionsMenu.items.items[2].show();
					form.UnionActionsMenu.items.items[3].hide();
				} else {
					form.UnionActionsMenu.items.items[0].hide();
					form.UnionActionsMenu.items.items[1].show();
			...
			stringfields: [
				...
				{name: 'RegistryCheckStatus_Name', type:'string', hidden: true},
				{name: 'Registry_xmlExportPath', header: 'Имя файла', type:'string'}
			],
			actions: [
				{name:'action_add' },
				{name:'action_edit' },
		...
		this.UnionActionsMenu = new Ext.menu.Menu({
			items: [{
				...
				text: 'Импорт результата ФЛК (в XML)',
				tooltip: 'Импорт результата ФЛК (в XML)',
				handler: function()
				{
				
				}
			}, {
				text: 'Импорт результата ФЛК (в DBF).',
				tooltip: 'Импорт результата ФЛК (в DBF).',
				handler: function()
				{
