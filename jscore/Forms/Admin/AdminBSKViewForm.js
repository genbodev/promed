...
sw.Promed.AdminBSKViewForm = Ext.extend(sw.Promed.BaseForm,
{
	...
	shim: false,
	buttonAlign: "right",
	objectName: 'AdminBSKViewForm',
	closeAction: 'hide',
	id: 'AdminBSKViewForm',
	objectSrc: '/jscore/Forms/Admin/AdminBSKViewForm.js',
    layout : 'fit',
	...
	{   
		...
		this.GridObjects = new sw.Promed.ViewFrame({
            ...
            object: 'GridObjects',
			dataUrl: '/?c=BSK_Register&m=getListObjects',
			autoLoadData: true,
 			...
 			actions: [
				{
                 ...
                 disabled: true,
                 handler: function() {       
                        var wnd = Ext.getCmp('AdminBSKViewForm');
                 ...
                 handler: function(){
                     ...
                     Ext.Ajax.request({
        		 		url: '/?c=BSK_Register&m=deleteObject',
        				params: {
        				    object_id : record_id
        				},
        				callback: function(options, success, response) {
                            if (success === true) {
                              Ext.getCmp('AdminBSKViewForm').showMsg('Предмет наблюдения удалён');
                              Ext.getCmp('GridObjects').getGrid().getStore().load();
                              //Ext.getCmp('AdminBSKViewForm').getTreeObjects(); 
        					}
            ...
            buttons : [
                {
                  ...
                  handler: function() {
                              ...
                              if(newObject.getValue().length< 2){
                     			  Ext.getCmp('AdminBSKViewForm').showMsg('Необходимо ввести наименование предмета наблюдения (не менее 3х символов)');
                    			  return false;                               
                              }
                              else{
                                  Ext.Ajax.request({
                        		 		url: '/?c=BSK_Register&m=addObject',
                        				params: {
                        				    BSKObject_name : newObject.getValue()
                        				},
                        				callback: function(options, success, response) {
                                            if (success === true) {
                                              Ext.getCmp('AdminBSKViewForm').showMsg('Предмет наблюдения успешно добавлен');
             ...
             buttons : [
                {
                  ...
                  handler: function() {
                              ...
                              if(editObject.getValue().length< 2){
                                  ...
                                  Ext.Ajax.request({
                        		 		url: '/?c=BSK_Register&m=editObject',
                        				params: {
                        				...
                        				callback: function(options, success, response) {
                                            if (success === true) {
                                              Ext.getCmp('AdminBSKViewForm').showMsg('Предмет наблюдения успешно изменён');
        ...
        this.GridObjects.getGrid().on(
            'rowclick', 
                function(){
					...
					Ext.getCmp('GridFormTemplate').getGrid().getStore().load({
						params: {
								BSKObject_id: Ext.getCmp('GridObjects').getGrid().selected.data.BSKObject_id
								}    
					});
					
					Ext.getCmp('GridGroupTypes').getGrid().getStore().removeAll();
		...
		/** Шаблоны  */
		this.GridFormTemplate = new sw.Promed.ViewFrame({
			id: 'GridFormTemplate',
			region: 'center',
			contextmenu: false,
			border: false,
			height: Ext.getBody().getHeight()*0.7,
			object: 'GridFormTemplate',
			dataUrl: '/?c=BSK_Register&m=getListFormTemplate',
			autoLoadData: false,
			focusOnFirstLoad: false,
			stringfields: [
				{name: 'BSKRegistryFormTemplate_id', type: 'int', header: 'ID', hidden: true},
				{name: 'BSKRegistryFormTemplate_id', type: 'int', header: 'id', width:20},
				{name: 'BSKRegistryFormTemplate_name', header: 'Период действия', id: 'autoexpand'},
			],
			actions: [
		...
		this.GridFormTemplate.getGrid().on(
			'rowclick', 
				function(){
					...
					Ext.getCmp('GridGroupTypes').getGrid().getStore().load({
						params: {
							BSKObject_id: Ext.getCmp('GridObjects').getGrid().selected.data.BSKObject_id,
							BSKRegistryFormTemplate_id: Ext.getCmp('GridFormTemplate').getGrid().selected.data.BSKRegistryFormTemplate_id
						}
					});
		...
		this.GridGroupTypes = new sw.Promed.ViewFrame({
			...
			dataUrl: '/?c=BSK_Register&m=getListGroupTypes',
			autoLoadData: false,
            focusOnFirstLoad: false,
			stringfields: [
        ...
        this.GridGroupTypes.getGrid().on(
            'rowclick', 
                function(){
                    Ext.getCmp('AdminBSKViewForm').addGroupTypePanel.hide();
                    Ext.getCmp('AdminBSKViewForm').editGroupTypePanel.hide(); 
                    
                    //Ext.getCmp('AdminBSKViewForm').GridTypes.getGrid().getSelectionModel().deselectRange(0,20);
                     
                    var targetGroupType = this.getSelectionModel().getSelected(); 
                    Ext.getCmp('AdminBSKViewForm').checkSymbolTest = 'undefined';
                        ...
                        if(typeof(targetGroupType.json) == 'object'){
                            ...
                            Ext.getCmp('GridTypes').getGrid().getStore().load({
                                params: {
										 ...
										 BSKRegistryFormTemplate_id : Ext.getCmp('GridFormTemplate').getGrid().getSelectionModel().getSelected().data.BSKRegistryFormTemplate_id
                                        }    
                            });
            ...
            buttons : [
                {
                  ...
                  handler: function() {
                              ...
                              if(newGroupType.getValue().length< 2){
                                  Ext.getCmp('AdminBSKViewForm').showMsg('Необходимо ввести наименование группы сведений (не менее 3х символов)');  
                    			  return false;                               
                              }                              
                              else{

                                  Ext.Ajax.request({
                        		 		url: '/?c=BSK_Register&m=addGroupType',
                        				params: {
                        				    BSKObject_id : Ext.getCmp('GridObjects').getGrid().selected.data.BSKObject_id,
                        				...
                        				callback: function(options, success, response) {
                                            ...
                                            if (success === true) {
                                              Ext.getCmp('AdminBSKViewForm').showMsg('Группа сведений успешно добавлена');    
             ...
             buttons : [
                {
                  ...
                  handler: function() {
                              ...
                              if(editGroupType.getValue().length< 2){
                                  ...
                                  Ext.Ajax.request({
                        		 		url: '/?c=BSK_Register&m=editGroupType',
                        				params: {
                        				...
                        				callback: function(options, success, response) {
                                            if (success === true) {
                                              ...
                                              Ext.getCmp('AdminBSKViewForm').showMsg('Группа сведений успешно изменёна');   
		...
		this.GridTypes = new sw.Promed.ViewFrame({
			...
			stringfields: [
                ...
                {name: 'BSKObservElement_symbol', header: 'обозначение', width:60, align:'center', hidden: true},
				{name: 'BSKObservDict_name', header: 'Справочник', width:70, align:'center'},
				{name: 'Unit_Name', header: 'Ед.Изм.', width:70, align:'center'},
        ...
        var TypesFormatStore = new Ext.data.JsonStore({
            url: '/?c=BSK_Register&m=getTypesFormat',
                  ...
                  handler: function() {
                              ...
                              if(minAge.getValue().length != 0 && maxAge.getValue().length != 0){
                                  if(parseInt(minAge.getValue()) > parseInt(maxAge.getValue())){
                                      Ext.getCmp('AdminBSKViewForm').showMsg('Некорректно ввёден возрастной интервал');  
                    			      return false;  
                                  } 
                              }                 
                
                              if(newType.getValue().length< 2){
                                  Ext.getCmp('AdminBSKViewForm').showMsg('Необходимо ввести наименование типа сведений');  
                    			  return false;                               
                              }               
                              else if(format.getValue() == ''){
                                  Ext.getCmp('AdminBSKViewForm').showMsg('Необходимо указать формат типа сведений');  
                    			  return false;                                  
                              }
                              else if(format.getValue() == 'formula (автоматический расчёт)' && formula.getValue().length< 3){
                                  Ext.getCmp('AdminBSKViewForm').showMsg('Необходимо указать формулу');
                    			  return false;                                  
                              }   
                              //Только по скринингу
                              else if(Ext.getCmp('GridObjects').getGrid().selected.data.BSKObject_id == 2  && stage.getValue() == ''){
                                  Ext.getCmp('AdminBSKViewForm').showMsg('Необходимо указать этап типа сведений');  
                    			  return false;                               
                              }                           
                              else{
                                  ...
                                  if(symbol.getValue().replace(/\s/g,'') != ''){
                                        Ext.getCmp('AdminBSKViewForm').checkSymbol(symbol.getValue(), null);
                                        
                                        //console.log('CBHF2:', typeof(Ext.getCmp('AdminBSKViewForm').checkSymbolTest));
                                                        
                                        if(Ext.getCmp('AdminBSKViewForm').checkSymbolTest != 'undefined'){
                                              Ext.getCmp('AdminBSKViewForm').showMsg('Данное бкувенное обозначение уже используется для другого типа сведений');
                                              return false;
                                        }
                                  }
                                                                             
                                  Ext.Ajax.request({
                        		 		url: '/?c=BSK_Register&m=addType',
                        				params: {
                        				...
                        				callback: function(options, success, response) {
                                            if (success === true) {
                                                Ext.getCmp('AdminBSKViewForm').showMsg('Тип сведений успешно добавлен');  
                  ...
                  handler: function() {
                              ...
                              //Только по скринингу
                                  ...
                                  if(editsymbol.getValue().replace(/\s/g,'') != ''){
                                        Ext.getCmp('AdminBSKViewForm').checkSymbol(editsymbol.getValue(), Type_id);
                                  ...
                                  Ext.Ajax.request({
                        				...
                        				callback: function(options, success, response) {
                                            if (success === true) {
                                                Ext.getCmp('AdminBSKViewForm').showMsg('Тип сведений успешно изменён');   
                                                Ext.getCmp('GridTypes').getGrid().getStore().load();
                                                //Ext.getCmp('AdminBSKViewForm').getTreeObjects(); 
                                			    return false;
                        					}
        ...
        this.GridTypes.getGrid().on(
            'rowclick', 
                function(){
                    ...
                    Ext.getCmp('AdminBSKViewForm').addTypePanel.hide();
                    Ext.getCmp('AdminBSKViewForm').editTypePanel.hide();
		...
		this.GridValues = new sw.Promed.ViewFrame({
			...
			height: Ext.getBody().getHeight()*0.7,
            object: 'GridTypes',
			dataUrl: '/?c=BSK_Register&m=getListValues',
			autoLoadData: false,
			stringfields: [
				{name: 'BSKObservElementValues_id', type: 'int', header: 'ID'},
                {name: 'BSKObservElementValues_id', type: 'int', header: 'id', width:20},
				{name: 'BSKObservElementValues_data', header: 'Значение', id: 'autoexpand'},
                {name: 'BSKObservElementValues_points', header: 'Баллы',width: 50, hidden: true},
                {name: 'BSKObservElementValues_sign', header: 'Расчет риска',width: 80},
                {name: 'BSKObservElementValues_min', header: 'Min',width: 50, hidden: true},
                {name: 'BSKObservElementValues_max', header: 'Max',width: 50, hidden: true},
			],
 			actions: [
				{name:'action_add', hidden: true},
				{name:'action_edit', hidden: true},
            ...
            items : [
                ...
                /*
                {
                 ...
                 handler : function(){
                    ...
                    getWnd('AdminBSKRecomendationWindow').show(params);                  
                 }     
                },
                {
                 xtype: 'button',   
                 ...
                 handler : function(){
                    //Ext.getCmp('AdminBSKViewForm').gridRecomendation.getGrid().getStore().load({params:{BSKObservRecomendationType_id:1, BSKObservElementValues:null}})
                    
                    //Ext.getCmp('AdminBSKViewForm').RecomendationWindow.setTitle('Рекомендации для врача');
                    //Ext.getCmp('AdminBSKViewForm').RecomendationWindow.Type = 1;
                    //Ext.getCmp('AdminBSKViewForm').RecomendationWindow.show();
                 }                        
                },
                */                                                                                                    
            ],
                  ...
                  handler: function() {
                              ...
                              if(Ext.getCmp('nameValue').isVisible() === true){
                                  if(nameValue.getValue().length< 2){
                                      Ext.getCmp('AdminBSKViewForm').showMsg('Необходимо ввести значение типа сведений (не менее 2х символов)');
                        			  return false;                               
                                  }    
                              }
                              
                              if(min.getValue().replace(/\s/g, '') != '' && min.getValue().replace(/,/,'.') >= max.getValue().replace(/,/,'.')){
                                  Ext.getCmp('AdminBSKViewForm').showMsg('Минимально допустимое значение не может быть больше или равно максималоно допустимого');
                    			  return false;                                  
                              }                         
                              else{
                                  Ext.Ajax.request({
                        		 		url: '/?c=BSK_Register&m=addValue',
                        				params: {
                        				...
                        				callback: function(options, success, response) {
                                            ...
                                            if (success === true) {
                                                ...
                                                Ext.getCmp('AdminBSKViewForm').showMsg('Значение успешно добавлено');                                           
                  ...
                  handler: function() {
                              ...
                              if(Ext.getCmp('editNameValue').isVisible() === true){
                                  if(editNameValue.getValue().length< 2){
                                      Ext.getCmp('AdminBSKViewForm').showMsg('Необходимо ввести значение (не менее 2х символов)');  
                        			  return false;                               
                                  }                                  
                              }
                              ...
                              Ext.Ajax.request({
                    		 		url: '/?c=BSK_Register&m=editValue',
                    				params: {
                    				...
                    				callback: function(options, success, response) {
                                        ...
                                        if (success === true) {
                                            ...
                                            Ext.getCmp('AdminBSKViewForm').showMsg('Значение успешно изменено');                                           
                            			    return false;
                    					}
        ...
        this.GridValues.getGrid().on(
            'rowclick', 
                function(){
                    ...
                    Ext.getCmp('AdminBSKViewForm').addValuePanel.hide();
                    Ext.getCmp('AdminBSKViewForm').editValuePanel.hide();
        ...
        this.listRecomendation = new sw.Promed.ViewFrame({
            			...
            			dataUrl: '/?c=BSK_Register&m=listRecomendation',
            			autoLoadData: false,
                        focusOnFirstLoad: false,
                        toolbar: false,
                        autoScroll: true,
            			stringfields: [
             			...
             			actions: [
            				...
            				{name:'action_refresh', hidden: true},
            				{name:'action_print', hidden: true},
                            {name: 'action_save', url: '/?c=BSK_Register&m=saveAfterEditRecomendation', hidden:true}
            			],
                        onLoadData: function(){
...
;
                                    		...
                                    		sw.swMsg.show({
                                    			...
                                    			fn: function(buttonId, text, obj) {
                                    				if ( buttonId == 'yes' ) {
                                                        ...
                                                        Ext.Ajax.request({
                                            		 		url: '/?c=BSK_Register&m=deleteEditRecomendation',
                                            				params: { 
                            ...
                            items:[
                            {
                                ...
                                listeners: {
                                    'active' : function(p){
                                        ...
                                        //Ext.getCmp('AdminBSKRecomendationWindow').gridRecomendation.getGrid().getStore().load();
                                    }
                                }
                                
                        },
                         {
                            title: 'Просмотр рекомендаций',
                            autoScroll: true,
                            bodyStyle: '',
                            frame: true,
                            height:900,
                            items:[
                                {
                						...
                						listeners: {
                							change: function(box, value) {
                							    ...
                							    Ext.getCmp('AdminBSKViewForm').listRecomendation.getGrid().getStore().load({
                							       params:{ 
                                              ...
                                              items : [                
                                                   ...
                                                   },
                                                   {
                                                     ...
                                                     handler: function(){
                                                         ...
                                                         Ext.Ajax.request({
                                            		 		url: '/?c=BSK_Register&m=addRecomendation',
                                            				params: { 
                                            				...
                                            				callback: function(options, success, response) {
                                            				     if(success === true){
                                            				        //Ext.getCmp('AdminBSKRecomendationWindow').hide();
                                                                    ...
                                                                    Ext.getCmp('AdminBSKViewForm').listRecomendation.getGrid().getStore().load({params:{ BSKObservRecomendationType_id: BSKObservRecomendationType_id }});
        ...
        sw.Promed.AdminBSKViewForm.superclass.initComponent.apply(this, arguments);
    },
    ...
    show: function(){
        ...
        sw.Promed.AdminBSKViewForm.superclass.show.apply(this, arguments);
    },  
    getTreeObjects: function(){
          
          Ext.Ajax.request({
		 		url: '/?c=BSK_Register&m=getTreeObjects',
				callback: function(options, success, response) {
                    if (success === true) {
          ...
          Ext.getCmp('AdminBSKViewForm').doLayout();
    },
    checkSymbol : function(symbol, BSKObservElement_id){
        //console.log('symbol: ', symbol);
        Ext.getCmp('AdminBSKViewForm').checkSymbolTest = 'undefined';
        Ext.Ajax.request({
     		url: '/?c=BSK_Register&m=checkSymbol',
    		params: {
    		    symbol : symbol,
                BSKObservElement_id: BSKObservElement_id
    		},
    		callback: function(options, success, response) {
                //Ext.getCmp('AdminBSKViewForm').checkSymbolTest = '';
                
                var rt = Ext.util.JSON.decode(response.responseText);
                
                Ext.getCmp('AdminBSKViewForm').checkSymbolTest = typeof(rt[0]); 
