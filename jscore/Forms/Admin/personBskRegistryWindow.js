...
sw.Promed.personBskRegistryWindow = Ext.extend(sw.Promed.BaseForm,
{
    ...
    buttonAlign: "left",
	objectName: 'personBskRegistryWindow',
	id: 'personBskRegistryWindow',
	objectSrc: '/jscore/Forms/Admin/Ufa/personBskRegistryWindow.js',
	switchFieldOnTab: function(id, isFirst) {
		//console.log(id, isFirst)
		var form = Ext.getCmp('personBskRegistryWindow');
		...
		if(isFirst){
			...
			setTimeout(function(){
				if(form.isButtonAdd === true  || typeof Periods == 'undefined' 
					...
					if (document.getElementById('i'+form.listIDSfocus[num_elem]) != null) {
						...
						if(Ext.getCmp('Answer_'+form.listIDSfocus[num_elem]) 
							&& Ext.getCmp('Answer_'+form.listIDSfocus[num_elem]).focus
							&& typeof Ext.getCmp('Answer_'+form.listIDSfocus[num_elem]).focus == 'function'){
							Ext.getCmp('Answer_'+form.listIDSfocus[num_elem]).focus();
						}
					}
			...
			if  (typeof(field) != 'undefined') {
				...
				if (typeof Ext.getCmp('Answer_'+ form.listIDSfocus[num_elem]) == 'undefined') {
					Ext.getCmp('personBskRegistryWindow').combExp = '';
					return false;
				}
				setTimeout(function() {
    ...
    hideShowButtons: function(tabid){
		...
		var is_disdate = (v = Ext.getCmp('GridObjectsUser').getGrid().getSelectionModel().getSelected()) && !!v.get('PersonRegister_disDate') && v.get('PersonRegister_disDate') < getValidDT(getGlobalOptions().date, '');    
		if(Ext.getCmp('GridObjectsUser').getGrid().store.data.length > 0 && (Ext.getCmp('GridObjectsUser').getGrid().store.data.items[0].json.Person_deadDT != null || is_disdate))
        {
        ...
        {
			//для ОКС, ХСН и пороков сердца кнопки недоступны
			if (!Ext.getCmp('personBskRegistryWindow').MorbusType_id.inlist([19,110,111,112])) {
	...
	{                
        ...
        this.GridObjects = new sw.Promed.ViewFrame({
        	...
        	dataUrl: '/?c=BSK_Register_User&m=getListObjectsCurrentUser',
        	autoLoadData: false,
            focusOnFirstLoad: false,
        	stringfields: [
        		{name: 'BSKObject_id', type: 'int', header: 'ID'},
                        {name: 'Person_deadDT', type: 'date', format: 'd.m.Y', header: langs('Дата смерти'), width: 90, hidden: true},
                        {name: 'PersonRegister_disDate', type: 'date', format: 'd.m.Y', width: 90, hidden: true},
        		...
        		{name: 'MorbusType_Name', header: 'Наименование', /*id: 'autoexpand',*/ width: '250px', renderer: function(value){
                  ...
                  //Скрининг
                  if(Ext.getCmp('personBskRegistryWindow').MorbusType_id == 84){
                     return '<div title="'+value+'" style="padding:5px;font-size:12px; float:left;display:block">' + t_val[0] + '</div>' + textGroupRisk;        
                  }
                  //Лёгочная гипертензия
                  else if(Ext.getCmp('personBskRegistryWindow').MorbusType_id == 88){
                     return '<div title="'+value+'" style="padding:5px;font-size:12px; float:left;display:block">' + t_val[0] + '</div>' + textGroupRisk;   
                  }
                  //Артериальная гипертензия
                  else if(Ext.getCmp('personBskRegistryWindow').MorbusType_id == 89){
                     return '<div title="'+value+'" style="padding:5px;font-size:12px; float:left;display:block">' + t_val[0] + '</div>' + textGroupRisk;   
                  }  
                  else{
        	...
        	onLoadData: function() {
                    ...
                    //открываем анкету из ЭМК
                    if(typeof Ext.getCmp('personBskRegistryWindow').EMKBSKObject_id != 'undefined') {
                        var BSKObject_id = Ext.getCmp('personBskRegistryWindow').EMKBSKObject_id;
                        var index = Ext.getCmp('GridObjectsUser').getGrid().store.data.findIndex('id', BSKObject_id);
                        Ext.getCmp('GridObjectsUser').getGrid().getSelectionModel().selectRow(index);
                        Ext.getCmp('GridObjectsUser').clickToRow();
                        Ext.getCmp('personBskRegistryWindow').getLastVizitRegistry(Ext.getCmp('personBskRegistryWindow').EMKBSKRegistry_id);
                    }                    
            ...
            clickToRow: function(){
					...
					if(form.MorbusType_id == 19) {
						form.getLastVizitRegistryOks(null);
					} else if (form.MorbusType_id.inlist([110,111,112])) {
						form.getBSKObjectWithoutAnket();
					} else
		...
		this.GridObjects.ViewGridPanel.view = new Ext.grid.GridView({
			getRowClass: function (row, index) {
				var cls = '';
				var currentdate = getValidDT(getGlobalOptions().date, '');
				if(row.get('MorbusType_id') == 19) {
         ...
         this.GridObjects.getGrid().on(
            'rowclick',
            function(grid, row){
                   Ext.getCmp('personBskRegistryWindow').isButtonAdd = false; 
				   Ext.getCmp('personBskRegistryWindow').listIDSfocus = [];
				   Ext.getCmp('personBskRegistryWindow').Reverse = false;
				   Ext.getCmp('personBskRegistryWindow').postfixQuestions = [];
                  ...
                  else
                  {
                   ...
                   Ext.getCmp('editBskDataButton').setDisabled(Ext.getCmp('personBskRegistryWindow').editableForm); 
                   ...
                   if( Ext.getCmp('personBskRegistryWindow').clickToPN == 0){					   
                        Ext.getCmp('GridObjectsUser').clickToRow();
                   }
                   else{
                        sw.swMsg.show({
                        ...
                        return Ext.getCmp('personBskRegistryWindow').Question;
                   } 
                    ...
                    if(direction == '<'){
                        ...
                        for(var k in Periods){
                            ...
                            if(t_period < this.datetime){
									...
									if(form.MorbusType_id == 19) {
										form.getLastVizitRegistryOks(Periods[k].BSKRegistry_id);
									} else
        ...
        this.EventsGrid = new sw.Promed.ViewFrame({
        	...
        	height: Ext.getBody().getHeight()*0.897,
            object: 'EventsGrid',
        	dataUrl: '/?c=BSK_Register_User&m=getListEvents',
        	autoLoadData: false,
            focusOnFirstLoad: false,
        	stringfields: [
        		{name: 'Events_id', type: 'int', header: 'ID'},
                {name: 'Events_setDate', header: 'Дата', type: 'date', width:120, format: 'd.m.Y'},
            ...
            deleteData : function(){
				...
				sw.swMsg.show({
					...
					fn: function( buttonId ) {
                        ...
                        Ext.Ajax.request({
            		 		url: '/?c=BSK_Register_User&m=deleteEvent',
            				params: {
            				...
            				callback: function(options, success, response) {
            				     ...
            				     if(success === true){
                                     ...
                                     //Ext.getCmp('BSKEventsEditWindow').showMsg('Событие успешно удалено!');
                                    ...
                                    Ext.getCmp('BSKEventsWindow').showMsg('Произошла ошибка при попытки удаления события!');
                                 }
                            }
                        });    
            ...
            editData : function(){
                ...
                getWnd('BSKEventsWindow').show({params : {
					Action: 'edit',
    	...
    	Ext.apply(this,
		{
                    ...
                    items : [ 
                    
                      {
        				...
        				items: [{
                            ...
                            width:'130px',
        					handler: function() {
        						Ext.getCmp('personBskRegistryWindow').panelButtonClick(1);
        					},
        					text: BTN_PERSCARD,
        					iconCls: 'pers-card16',
                            ...
                            width:'130px',
        					handler: function() {
        						Ext.getCmp('personBskRegistryWindow').panelButtonClick(2);
        					},
        					text: BTN_PERSEDIT,
        					iconCls: 'edit16',
                            ...
                            width:'130px',
        					handler: function() {
        						Ext.getCmp('personBskRegistryWindow').panelButtonClick(3);
        					},
                            ...
                            width:'130px',
        					handler: function() {
        						Ext.getCmp('personBskRegistryWindow').panelButtonClick(4);
        					},
        					text: BTN_PERSPRIV,
        					iconCls: 'pers-priv16',
                            ...
                            width:'130px',
        					handler: function() {
        						Ext.getCmp('personBskRegistryWindow').panelButtonClick(5);
        					},
        					text: BTN_PERSDISP,
        					iconCls: 'pers-disp16',
                     ...
                     listeners: {
                        ...
                        'expand':function(p) {
                                  Ext.getCmp('personBskRegistryWindow').resizePanel();
                                },
                        'collapse' :function(p) {
                                   Ext.getCmp('personBskRegistryWindow').resizePanel();
                                }
                    ...
                    items : [
                        {
                            ...
                            items : [
                            {
                                ...
                                tbar: [
                                       {
                                            ...
                                            iconCls: 'add16',
                                            handler: function(){
												Ext.getCmp('personBskRegistryWindow').clickToRow = true;
                                ...
                                items: [ Ext.getCmp('personBskRegistryWindow').GridObjects]
                            }],
                            listeners:{
                                /*'render': function() {
                                    Ext.getCmp('personBskRegistryWindow').resizePanel();
                            ...
                            tbar: [
                               {
                                    ...
                                    disabled: true,
                                    handler: function(){
										Ext.getCmp('personBskRegistryWindow').isButtonAdd = true; 
                                        var form = Ext.getCmp('personBskRegistryWindow');
                                        form.PrefixQuest = 0;
                                        Ext.getCmp('addBskDataButton').setDisabled(true);
                                        //Ext.getCmp('personBskRegistryWindow').pmuser_id = getGlobalOptions().pmuser_id;
                                        ...
                                        //24122014
                                        //Ext.getCmp('personBskRegistryWindow').BSKRegistry_id = false;
								...
								},
								{
										xtype: 'button',
										text: 'Сохранить',
										id: 'saveBskPrognosDiseases',
										iconCls: 'save16',
										hidden: true,
										disabled: true,
										handler: function(){
											wnd.getLoadMask('Сохранение прогнозируемых осложнений основного заболевания').show();
											Ext.Ajax.request({
												url: '/?c=BSK_Register_User&m=savePrognosDiseases',
												params: {
													PrognosOslDiagList : Ext.util.JSON.encode(Ext.getCmp('PrognosDiseases').getValues()),
													Person_id : Ext.getCmp('personBskRegistryWindow').personInfo.Person_id
												},
												callback: function(options, success, response) {
													wnd.getLoadMask().hide();
													if (success === true) {
														Ext.getCmp('personBskRegistryWindow').showMsg('Данные прогнозируемых осложнений основного заболевания успешно сохранены!');
													}
												}
											});
										}
								},
								{
                                    ...
                                    handler: function(){
                                        ...
                                        switch(activeTabIndex){
                                            //Сведения
                                            case 0:
                                                    if(Ext.getCmp('personBskRegistryWindow').Periods[Ext.getCmp('personBskRegistryWindow').MorbusType_id].length <=1){
                                                         Ext.getCmp('prevButton').setDisabled(true);                    
                                                    }
                                                    else{
                                                         Ext.getCmp('prevButton').setDisabled(false);     
                                                    ...
                                                    Ext.getCmp('personBskRegistryWindow').findRegisterData();
											break;
                               ...
                               },
                               {
                                    ...
                                    handler: function(){
                                            ...
                                            switch(Ext.getCmp('personBskRegistryWindow').MorbusType_id){
                               ...
                               },  
                               {
                                    ...
                                    handler: function(){
                                            ...
                                            window.open('/?c=BSK_Register_User&m=getDrugs&Person_id=' + Ext.getCmp('personBskRegistryWindow').Person_id, win_id);
                                            /*
                                            //На рабочем или тестовом
                                            var object_value = null;
                                            //var url = ((getGlobalOptions().birtpath != '/birt-viewer/') ? getGlobalOptions().birtpath : 'http://192.168.200.58:91:91/birt-viewer')  +'/run?__report=Report/';
                                ...
                                },
                				{
                					...
                					handler: function () {
                                        ...
                                        Ext.getCmp('personBskRegistryWindow').calendar.getPeriod('<');

                					}.createDelegate(this)
                				},
                				Ext.getCmp('personBskRegistryWindow').calendar,
                				{
                					text: 'Следующий',
                                    id: 'nextButton',
                                    disabled: true,
                            ...
                            items : [
                                {
                                  ...
                                  listeners : {
                                    'activate' : function(p){
                                        Ext.getCmp('personBskRegistryWindow').hideShowButtons(p.id);
                                    }
                                  }
                                },
                                {
                                  ...
                                  items : [
                                        {
                                            ...
                                            items : [
                                                ...
                                                new Ext.FormPanel({
                                                    title: null,
                                                    width: 1250,
                                                    id: 'panelBSKRegistryUsluga',
                                                    layout: 'form',
                                                    items: [ 
                                                        new sw.Promed.ViewFrame({
                                                            autoExpandColumn: 'autoexpand',
                                                            autoExpandMin: 110,
                                                            dataUrl: '/?c=BSK_Register_User&m=getListUslugforEvents',
                                                            autoLoadData: false,
                                                            id: 'gridBSKRegistryUsluga',
                                                            height: 150, 
                                                            autoWidth: true,
                                                            pageSize: 100,
                                                            contextmenu: false,
                                                            ...
                                                            stringfields: [ 
                                                                ...
                                                                {name: 'Usluga_Code', header: 'Код', width: 250, type:'string'},
                                                                {name: 'Usluga_Name', header: 'Наименование', width: 570, type:'string'},
                                                                {name: 'Lpu_Nick', header: 'МО', width: 85, type:'string', },
                                                                {name: 'EvnPS_id', header: 'Просмотр услуги', id: 'autoexpand', renderer: function (value, cellEl, rec) {
                                                ...
                                                }),
                                                {
                                                    xtype: 'label',
                                                    style: "font-size:11px;font-weight:bold;",
                                                    text: 'Случаи оказания амбулаторно-поликлинической медицинской помощи',
                                                    margin: '0 0 0 10'
                                                },
                                                new Ext.FormPanel({
                                                    title: null,
                                                    width: 1250,
                                                    id: 'panelBSKRegistryTAP',
                                                    layout: 'form',
                                                    items: [ 
                                                        new sw.Promed.ViewFrame({
                                                            autoExpandColumn: 'autoexpand',
                                                            autoExpandMin: 110,
                                                            dataUrl: '/?c=BSK_Register_User&m=getListPersonCureHistoryPL',
                                                            autoLoadData: false,
                                                            id: 'gridBSKRegistryTAP',
                                                            height: 150, 
                                                            autoWidth: true,
                                                            pageSize: 100,
                                                            contextmenu: false,
                                                            ...
                                                            stringfields: [ 
                                                                ...
                                                                {name: 'Diag_Name', header: 'Диагноз', width: 300, type:'string'},
                                                                {name: 'Lpu_Nick', header: 'МО', width: 80, type:'string', align:'left'},
                                                                {name: 'LpuSection_Name', header: 'Отделение', width: 270, type:'string'},
                                                                {name: 'Person_Fio', header: 'Врач', width: 220, type:'string'},
                                                                {name: 'EvnPL_NumCard', header: 'ТАП №', width: 90, type:'string'},
                                                                {name: 'EvnPL_id', header: 'Просмотр ТАП', id: 'autoexpand', renderer: function (value, cellEl, rec) {
                                                ...
                                                }),
                                                {
                                                    xtype: 'label',
                                                    style: "font-size:11px;font-weight:bold;",
                                                    text: 'Случаи оказания стационарной медицинской помощи',
                                                    margin: '0 0 0 10'
                                                },
                                                new Ext.FormPanel({
                                                    title: null,
                                                    width: 1250,
                                                    id: 'panelBSKRegistryKVS',
                                                    layout: 'form',
                                                    items: [ 
                                                        new sw.Promed.ViewFrame({
                                                            autoExpandColumn: 'autoexpand',
                                                            autoExpandMin: 110,
                                                            dataUrl: '/?c=BSK_Register_User&m=getListPersonCureHistoryPS',
                                                            autoLoadData: false,
                                                            id: 'gridBSKRegistryKVS',
                                                            height: 150, 
                                                            autoWidth: true,
                                                            pageSize: 100,
                                                            contextmenu: false,
                                                ...
                                                }),
                                                {
                                                    xtype: 'label',
                                                    style: "font-size:11px;font-weight:bold;",
                                                    text: 'Сопутствующие диагнозы',
                                                    margin: '0 0 0 10'
                                                },
                                                new Ext.FormPanel({
                                                    title: null,
                                                    width: 1250,
                                                    id: 'panelBSKRegistryDiag',
                                                    layout: 'form',
                                                    items: [ 
                                                        new sw.Promed.ViewFrame({
                                                            autoExpandColumn: 'autoexpand',
                                                            autoExpandMin: 110,
                                                            dataUrl: '/?c=BSK_Register_User&m=getListPersonCureHistoryDiagSop',
                                                            autoLoadData: false,
                                                            id: 'gridBSKRegistryDiag',
                                                            height: 150, 
                                                            autoWidth: true,
                                                            pageSize: 100,
                                                            contextmenu: false,
                                                            ...
                                                            stringfields: [ 
                                                                ...
                                                                {name: 'Diag_FullName', header: 'Диагноз', width: 300, type:'string'},
                                                                {name: 'Lpu_Nick', header: 'МО', width: 80, type:'string', align:'left'},
                                                                {name: 'LpuSection_Name', header: 'Отделение', width: 270, type:'string'},
                                                                {name: 'Person_Fio', header: 'Врач', width: 220, type:'string'},
                                                                {name: 'NumCard', header: 'КВС/ТАП №', width: 90, type:'string'},
                                                                {name: 'Evn_id', header: 'Просмотр', id: 'autoexpand', renderer: function (value, cellEl, rec) {
                                                ...
                                                new Ext.FormPanel({
                                                    title: null,
                                                    width: 1250,
                                                    id: 'panelBSKRegistryPostCardio',
                                                    layout: 'form',
                                                    items: [ 
                                                        new sw.Promed.ViewFrame({
                                                            autoExpandColumn: 'autoexpand',
                                                            autoExpandMin: 110,
                                                            dataUrl: '/?c=BSK_Register_User&m=getListPersonCureHistoryDiagKardio',
                                                            autoLoadData: false,
                                                            id: 'gridBSKRegistryPostCardio',
                                                            height: 150, 
                                                            autoWidth: true,
                                                            pageSize: 100,
                                                            contextmenu: false,
                                  ...
                                  listeners : {
                                      'activate' : function(p){
                                           ...
                                           // Ext.getCmp('eventsSubPanel').add(Ext.getCmp('EventsGrid'));
                                            ...
                                            Ext.getCmp('gridBSKRegistryUsluga').getGrid().getStore().load({
                                                params : {
                                                    Person_id :  Ext.getCmp('personBskRegistryWindow').Person_id 
                                                }
                                            });
                                            Ext.getCmp('gridBSKRegistryTAP').getGrid().getStore().load({
                                                params : {
                                                    Person_id :  Ext.getCmp('personBskRegistryWindow').Person_id 
                                                }
                                            });
                                            
                                            Ext.getCmp('gridBSKRegistryKVS').getGrid().getStore().load({
                                                params : {
                                                    Person_id :  Ext.getCmp('personBskRegistryWindow').Person_id 
                                                }
                                            });
                                            Ext.getCmp('gridBSKRegistryDiag').getGrid().getStore().load({
                                                params : {
                                                    Person_id :  Ext.getCmp('personBskRegistryWindow').Person_id,
                                                    type: 0
                                                }
                                            });
                                            Ext.getCmp('gridBSKRegistryPostCardio').getGrid().getStore().load({
                                                params : {
                                                    Person_id :  Ext.getCmp('personBskRegistryWindow').Person_id,
                                                    type: 1
                                                }
                                    ...
                                    listeners : {
                                        'activate' : function(p){
                                            ...
                                            var gridBSKRegistryResearch = new sw.Promed.ViewFrame({
                                                dataUrl: '/?c=BSK_Register_User&m=getLabResearch',
                                                autoLoadData: false,
                                                id: 'gridBSKRegistryResearch',
                                                object: 'gridBSKRegistryResearch',
                                               ...
                                               // toolbar: false, 
                                                ...
                                                stringfields: [ 
                                                    ...
                                                    {name: 'UslugaComplex_Code', header: 'Код', width: 180, type:'string'},
                                                    {name: 'UslugaComplex_Name', header: 'Наименование', width: 400, type:'string'},
                                    ...
                                    listeners : {
                                        'activate' : function(p){
                                            ...
                                            var gridBSKRegistrySurveys = new sw.Promed.ViewFrame({
                                                dataUrl: '/?c=BSK_Register_User&m=getLabSurveys',
                                                autoLoadData: false,
                                                id: 'gridBSKRegistrySurveys',
                                                object: 'gridBSKRegistrySurveys',
                                                height: Ext.getBody().getHeight()*0.58,
                                                width: 1250,
                                                autoScroll: true,
                                                border: true, 
                                                ...
                                                stringfields: [ 
                                                    ...
                                                    {name: 'UslugaComplex_Code', header: 'Код', width: 180, type:'string'},
                                                    {name: 'UslugaComplex_Name', header: 'Наименование', width: 400, type:'string'},
                                                    {name: 'Lpu_Nick', header: 'МО', width: 100, type:'string'},
                                                    {name: 'prosmotr', header: 'Просмотр результата', width: 150,id: 'autoexpand', renderer: function (value, cellEl, rec) {
                                                        if (Ext.isEmpty(rec.get('prosmotr'))) {
                                                            return value;
                                                        } else {
                                                            return "<a href='#' onClick='getWnd(\"swEvnXmlViewWindow\").show({ EvnXml_id: " + rec.get('prosmotr') + " });'>" + "Просмотреть" + "</a>";
                                ...
                                },
                                {
                                    ...
                                    disabled: false,
                                    autoScroll : true,
                                    html: 'Вкладка находится в разработке' ,
                                    style: 'padding:10px',
                                    listeners : {
                                        'activate' : function(p){
                                        }                                      
                                ...
                                */
                                {
                                  ...
                                  items : [
                                        {
                                            ...
                                            items : [
                                                /*
                                                Ext.getCmp('personBskRegistryWindow').RecomendationsDatesTreePanel,
                                                {
                                                    xtype: 'panel',
                                                    columnWidth : 90,
                                                    items : []
                                  ...
                                  listeners : {
                                      'activate' : function(p){
                                           ...
                                           //}
                                     	   ...
                                     	   form.RecomendationsDatesTreePanel = new Ext.tree.TreePanel({
                                    			...
                                    			loader: new Ext.tree.TreeLoader({
                                    				dataUrl: '/?c=BSK_Register_User&m=getTreeDatesRecomendations'
                                    			}),
                                    			rootVisible: false,
                                    			lastSelectedId: 0,
                                    			listeners: {
                                    			   ...
                                    			   'load' : function(){
                                                    ...
                                                    'click' : function(node, e ) {
                                                           ...
                                                           var DoctorRecomendations = new sw.Promed.ViewFrame({
                                                            	...
                                                            	dataUrl: '/?c=BSK_Register_User&m=getRecomendationByDate',
                                                            	autoLoadData: false,
                                                                focusOnFirstLoad: false,
                                                            	stringfields: [
                                           ...
                                           var tree =  Ext.getCmp('personBskRegistryWindow').RecomendationsDatesTreePanel;
                                ...
                                }, 
                                {
                                  ...
                                  listeners : {
                                    'activate' : function(p){
                                        ...
                                        Ext.Ajax.request({
                                        	url: '/?c=BSK_Register_User&m=getDrugs',
                                        	params: {
                                                Person_id   : Ext.getCmp('personBskRegistryWindow').personInfo.Person_id
                                        	},
                                        	callback: function(options, success, response) {
                                    ...
                                    'activate' : function(p){
                                        ...
                                        var DatesGrid = new sw.Promed.ViewFrame({
                                            ...
                                            */
                                        	...
                                        	height: Ext.getBody().getHeight()*0.58,
                                            object: 'DatesGrid',
                                        	dataUrl: '/?c=BSK_Register_User&m=getCompare',
                                        	autoLoadData: false,
                                            focusOnFirstLoad: false,
                                        	stringfields: [
                                        ...
                                        DatesGrid.getGrid().on(
                                            ...
                                            function(){
                                                ...
                                                if(sel.length>9){
                                                    Ext.getCmp('personBskRegistryWindow').showMsg('Указано максимальное количество дат для сравнения !'); 
								...
								{
									title: 'Прогнозируемые осложнения заболевания',
									id: 'PrognosDiseasesTab',
									style: 'padding:10px',
									autoScroll: true,
									html: '',
									listeners : {
									'activate' : function(p){
										Ext.getCmp('personBskRegistryWindow').hideShowButtons(p.id);
										Ext.getCmp('saveBskPrognosDiseases').show();
										Ext.getCmp('saveBskPrognosDiseases').setDisabled(false);
										var PrognosDiseases = new sw.Promed.DiagListPanelWithDescr({
											...
											fieldLabel: 'Прогнозируемые осложнения заболевания по МКБ',
											fieldDescLabel: 'Прогнозируемые осложнения заболевания'
										})
										this.removeAll();
										this.add(PrognosDiseases);
										this.doLayout();
										Ext.Ajax.request({
											url: '/?c=BSK_Register_User&m=loadPrognosDiseases',
											params: {
												Person_id : Ext.getCmp('personBskRegistryWindow').personInfo.Person_id
											},
											callback: function(options, success, response) {
												if (success === true) {
													...
													Ext.getCmp('PrognosDiseases').setValues(obj);
												}
											}
										});
									...
									'deactivate': function(p){
										Ext.getCmp('saveBskPrognosDiseases').hide();
									}
									} 
								}
							...
							[
								{
									 ...
									 iconCls: 'close16',
									 handler: function(){
										Ext.getCmp('personBskRegistryWindow').refresh();
									 }
								}							
							]
                }
            ...
            elemDisabled : function(){
                ...
                //console.log(Ext.getCmp('personBskRegistryWindow').pmuser_id,getGlobalOptions().pmuser_id)
                
                //return true;
				//console.log('Ext.getCmp(personBskRegistryWindow).Lpu_id', Ext.getCmp('personBskRegistryWindow').Lpu_id);
                //return (Ext.getCmp('personBskRegistryWindow').pmuser_id == getGlobalOptions().pmuser_id) || (typeof Ext.getCmp('personBskRegistryWindow').pmuser_id == 'undefined') ? false : true;
				
		return (Ext.getCmp('personBskRegistryWindow').Lpu_id == getGlobalOptions().lpu_id) || (typeof Ext.getCmp('personBskRegistryWindow').Lpu_id == 'undefined') ? false : true;
            },
            //Управление группой вопросов Лекарственного лечения (кроме скрининга)
            manageLLquestions : function(id_question){
                //Легочная гипертензия
                //if(Ext.getCmp('personBskRegistryWindow').MorbusType_id == 88){
            ...
            getLastVizitRegistry : function(BSKRegistry_id){
                var form = Ext.getCmp('personBskRegistryWindow');
                //Сравнение скрининга
                //console.log(typeof Ext.getCmp('personBskRegistryWindow').MorbusType_id)
                //console.log(Ext.getCmp('personBskRegistryWindow').MorbusType_id)
                
                if(!form.MorbusType_id.inlist(['19','110','111','112','113'])){
                ...
                Ext.Ajax.request({
                	url: '/?c=BSK_Register_User&m=getLastVizitRegistry',
                	params: {
                        MorbusType_id:form.MorbusType_id,
                        Person_id : form.personInfo.Person_id,
                	...
                	callback: function(options, success, response) {
                        ...
                        if (success === true) {  
                            ...
                            setTimeout(function(){
                                        ...
                                        if(typeof result[k] == 'object'){
                                            ...
                                            if(typeof  Ext.getCmp('addAnswer_'+result[k].BSKObservElement_id) != 'undefined' || typeof  Ext.getCmp('Answer_'+result[k].BSKObservElement_id) != 'undefined'){
											   ...
											   if (typeof Ext.getCmp('addAnswer_'+result[k].BSKObservElement_id) != 'undefined' && typeof answerPanel.params !='undefined' ) {
												...
												if (Ext.getCmp('Answer_'+result[k].BSKObservElement_id).getValue() == 'Да' || Ext.getCmp('personBskRegistryWindow').elemDisabled()) {//Если из базы загружен ответ 'да' или это не заполняющая организация, то залочиваем
                                                ...
                                                if(result[k].BSKUnits_name.length>0){
                                                    if(typeof  Ext.getCmp('UnitCombo_'+result[k].BSKObservElement_id) != 'undefined'){
														...
														if (Ext.getCmp('Answer_'+result[k].BSKObservElement_id).getValue() == 'Да' || Ext.getCmp('personBskRegistryWindow').elemDisabled()) {//Если из базы загружен ответ 'да' или это не заполняющая организация, то залочиваем размерность
                                ...
                                */
                                ...
                                if(typeof result[0] != 'undefined'){
                                    ...
                                    if(typeof Ext.getCmp('Answer_'+result[0].BSKObservElement_id) != 'undefined')
                                        Ext.getCmp('Answer_'+result[0].BSKObservElement_id).focus(false);
                                    // Поставить дату обследования                         
                                    //Ext.getCmp('calday').setValue(new Date(result[3].BSKRegistry_setDate.date));
                                    var date = result[0].BSKRegistry_setDate.date.split(" ");
                                    Ext.getCmp('calday').setValue(date[0]);
                                    Ext.getCmp('calday').datetime = new Date(result[0].BSKRegistry_setDate.date).format('Y-m-d H:i');
                                    Ext.getCmp('TextFieldDate').setValue(date[0]);
                                    // Поставим группу риска на текущий регистр
                                    
                                    //Только для скрининга
                                    if(Ext.getCmp('personBskRegistryWindow').MorbusType_id == 84){
                                        Ext.getCmp('personBskRegistryWindow').setGroupRisk(result[0].BSKRegistry_riskGroup);
                                    }
                                    ...
                                    form.BSKRegistry_id = result[0].BSKRegistry_id;
                                    Ext.Ajax.request({
                                        url: '/?c=BSK_Register_User&m=setIsBrowsed',
                                        params: {
                                            'BSKRegistry_id': result[0].BSKRegistry_id
                                        },
                                        callback: function(options, success, response) {
                                            if (success) {	
                                                var obj = Ext.util.JSON.decode(response.responseText);
                               ...
                               if(Ext.getCmp('personBskRegistryWindow').BSKRegistry_id){
                               ...
                               if(Ext.getCmp('personBskRegistryWindow').Periods[Ext.getCmp('personBskRegistryWindow').MorbusType_id] != false){
                                   
                                   //console.log('MorbusType_id', Ext.getCmp('personBskRegistryWindow').MorbusType_id);
                                   //console.log('=>',Ext.getCmp('personBskRegistryWindow').MorbusType_id, Ext.getCmp('personBskRegistryWindow').MorbusType_id.inlist([84,88]));
                                   //Блокировка для 84 Скрининг
                                   if(Ext.getCmp('personBskRegistryWindow').MorbusType_id.inlist([84,88,89,19,50])){
                                   ...
                                   if(Ext.getCmp('personBskRegistryWindow').MorbusType_id == 89){
                            ...
                            if(typeof result[3] != 'undefined' && options.params.BSKRegistry_id == null){
                                var currentdate = getValidDT(getGlobalOptions().date, '');
                                var BSKRegistrySetDate = new Date(result[3].BSKRegistry_setDate.date.split(' ')[0]);
                                var notice = false;
                                if(Ext.getCmp('personBskRegistryWindow').MorbusType_id == 84){
                                    var riskGroup = result[3].BSKRegistry_riskGroup
                                    switch(riskGroup){
                                        case 1 : BSKRegistrySetDate.addMonths(18); break;
                                        case 2 : BSKRegistrySetDate.addMonths(12); break;
                                        case 3 : BSKRegistrySetDate.addMonths(6); break;
                                    }
                                    if(currentdate > BSKRegistrySetDate){
                                        notice = true;
                                    }
                                }

                                if(Ext.getCmp('personBskRegistryWindow').MorbusType_id.inlist([50,89,88]) && (currentdate > BSKRegistrySetDate.addMonths(12))){
                                    notice = true;
                                }
                                if (notice == true){
                                    sw.swMsg.show({
            ...
            getRegistryDates : function(){
               var wnd = this;
                Ext.Ajax.request({
                	url: '/?c=BSK_Register_User&m=getRegistryDates',
                	params: {
                        MorbusType_id:Ext.getCmp('personBskRegistryWindow').MorbusType_id,
                        Person_id : Ext.getCmp('personBskRegistryWindow').personInfo.Person_id,
                        setDate : Ext.getCmp('calday').getValue()
                	...
                	callback: function(options, success, response) {
                        ...
                        if (success === true) {    
                            ...
                            //if(RegistryDates.length > 0){
                                ...
                                for(var k in RegistryDates){
                                    if(typeof RegistryDates[k].BSKRegistry_id != 'undefined'){
                                        dates.push({
											...
											disdate: !!RegistryDates[k].PersonRegister_disDate ? Date.parseDate(RegistryDates[0].PersonRegister_disDate, 'd.m.Y') : null
                                        })
                                ...
                                Ext.getCmp('personBskRegistryWindow').Periods[Ext.getCmp('personBskRegistryWindow').MorbusType_id] = {};
                                
                                Ext.getCmp('personBskRegistryWindow').Periods[Ext.getCmp('personBskRegistryWindow').MorbusType_id] = dates;
                                ...
                                //console.log(Ext.getCmp('personBskRegistryWindow').MorbusType_id, Ext.getCmp('personBskRegistryWindow').Periods[Ext.getCmp('personBskRegistryWindow').MorbusType_id]);                        
                            } 
                        //}
                    }
                });                  
            ...
            findRegisterData: function(){
                ...
                var listIDS = Ext.getCmp('personBskRegistryWindow').listIDS;
                var resultData = []; 
                var questions_ids = [];
                ...
                //console.log("Есть регистр?", Ext.getCmp('personBskRegistryWindow').BSKRegistry_id);
                
                //return;
                
                if(Ext.getCmp('personBskRegistryWindow').BSKRegistry_id === false){
            ...
            checkQuestionsAnswer : function(){
					...
					var postfixQuestions = Ext.getCmp('personBskRegistryWindow').postfixQuestions; 
                    ...
                    if(test === false && qstns.length > 0){
                        ...
                        Ext.getCmp('addBskDataButton').setDisabled(true);
                        Ext.getCmp('personBskRegistryWindow').showMsg('<b>Необходимо ответить на обязательные вопросы:</b><p>&nbsp;</p><br/>'+contentError+'</ol>');    
                        return false;
                    }   
            ...
            saveRegistryData: function(ListAnswers,questions_ids){
                ...
                Ext.Ajax.request({
                	url: '/?c=BSK_Register_User&m=checkRegisterDate',
                	params: {
                        MorbusType_id:Ext.getCmp('personBskRegistryWindow').MorbusType_id,
                        Person_id : Ext.getCmp('personBskRegistryWindow').personInfo.Person_id,
                	...
                	callback: function(options, success, response) {
                        ...
                        if (success === true) {   
                            ...
                            if(responseText.length>0){
                                //Ext.getCmp('personBskRegistryWindow').checkRegisterDateFlag = false;
                                Ext.getCmp('personBskRegistryWindow').showMsg('Регистр по данному предмету наблюдения, для данного пациента на указанную дату уже существует!'); 
                                return false;
                            }
                            else{   
                                    ...
                                    Ext.Ajax.request({
                                    	url: '/?c=BSK_Register_User&m=preSaveRegistryData',
                                    	params: {
                                    	    BSKObject_id : form.GridObjects.getGrid().getSelectionModel().getSelected().data.BSKObject_id,
                                            PersonData   : Ext.util.JSON.encode(Ext.getCmp('personBskRegistryWindow').personInfo),
                                    	...
                                    	callback: function(options, success, response) {                                            
                                            if (success === true) {   
                                                ...
                                                if(responseText.Error_Code == null && Ext.getCmp('personBskRegistryWindow').MorbusType_id == 84){
                                                    
                                                    Ext.getCmp('personBskRegistryWindow').setGroupRisk(responseText[0].riskGroupStatus);

                                                }
                                                Ext.getCmp('personBskRegistryWindow').BSKRegistry_id = responseText[0].BSKRegistry_id;
                                                Ext.getCmp('personBskRegistryWindow').showMsg('Данные регистра успешно сохранены !'); 
                                                Ext.getCmp('personBskRegistryWindow').getRegistryDates();
                                                Ext.getCmp('personBskRegistryWindow').getLastVizitRegistry(Ext.getCmp('personBskRegistryWindow').BSKRegistry_id);                                                   
                                            }
                                        }
                                    }); 
                            }   
            ...
            updateRegistryData: function(ListAnswers,questions_ids){
                ...
                Ext.Ajax.request({
                	url: '/?c=BSK_Register_User&m=preSaveRegistryData',
                	params: {
                	    BSKRegistry_id : Ext.getCmp('personBskRegistryWindow').BSKRegistry_id,
                        PersonData   : Ext.util.JSON.encode(Ext.getCmp('personBskRegistryWindow').personInfo),
                        ListAnswers  : Ext.util.JSON.encode(ListAnswers),
                        questions_ids : Ext.util.JSON.encode(questions_ids),
                	},
            ...
            setGroupRisk : function(numberGroupRisk){
                                ...
                                var record = Ext.getCmp('personBskRegistryWindow').GridObjects.getGrid().getSelectionModel().getSelected(); 
            ...
            addRegistryData: function(){
				...
				Ext.getCmp('personBskRegistryWindow').listIDSfocus = [];
				Ext.getCmp('personBskRegistryWindow').PrefixQuest = 0;
                Ext.getCmp('personBskRegistryWindow').BSKRegistry_id = false;
                Ext.getCmp('information').removeAll();
                
                /** По инициативе Влада - поле с датой проведения опроса */
                
                //console.log('textfield', Ext.getCmp('personBskRegistryWindow').elemDisabled());
                
         		var TextFieldDate = new sw.Promed.SwDateField({
        			id : 'TextFieldDate',
                    labelField: 'Дата проведения',
                    labelSeparator : ':',
                    disabled : Ext.getCmp('personBskRegistryWindow').elemDisabled(),
                    labelWidth : '50px',
                    //disabled: true,
                    width: '100px',
                	plugins: [
                            ...
                            if( (parseInt(params.data.id) == 107) || (parseInt(params.data.id) == 108) ){
                                ...
                                if (e.getKey() == e.ENTER) {
                                    //Ext.getCmp('searchRecomendationButton').handler();
                                      Ext.getCmp('personBskRegistryWindow').getIndex(parseInt(params.data.id));
                                }
                            }
                        },
                        */
                        'change' : function(){
                            if(Ext.getCmp('personBskRegistryWindow').MorbusType_id == 19){
                                Ext.getCmp('saveBskDataButton').setDisabled(true);
                                return;
                            } 
                            ...
                            var Person_Birthday = Ext.getCmp('personBskRegistryWindow').personInfo.Person_Birthday;
				...
				Ext.getCmp('calday').setValue(new Date());
                                ...
                                var age =  Ext.getCmp('personBskRegistryWindow').getAge(Person_Birthday, new Date());
				...
				Ext.getCmp('calday').setValue(new Date());
                                ...
                                var age =  Ext.getCmp('personBskRegistryWindow').getAge(Person_Birthday, this.getValue());
                                
								if (Ext.getCmp('Answer_'+answerAge)) {
									Ext.getCmp('Answer_'+answerAge).setValue(age);
								}
                            }
                        },
                ...
                var Person_id = Ext.getCmp('personBskRegistryWindow').Person_id;

                
                Ext.Ajax.request({
                	url: '/?c=BSK_Register_User&m=addRegistryData',
                	params: {
                            ...
                            var numberAnketQuestion = {
                             ...
                             for(var k in allQuestions.questions){
                                    if(typeof allQuestions.questions[k] == 'object'){
                                        ...
                                        var PersonSex_id = parseInt(Ext.getCmp('personBskRegistryWindow').personInfo.Sex_id);
                                        ...
                                        //}
                                            ...
                                            var PersonAge = Ext.getCmp('personBskRegistryWindow').personInfo.age;
                                            ...
                                            if(PersonAge>=minAge && PersonAge<maxAge){
                                                ...
                                                var index = ++Ext.getCmp('personBskRegistryWindow').PrefixQuest; 
                             ...
                             Ext.getCmp('personBskRegistryWindow').listIDS = listIDS;
				...
				var Periods = Ext.getCmp('personBskRegistryWindow').Periods[Ext.getCmp('personBskRegistryWindow').MorbusType_id]
				//console.log('Periods',Periods, 'typeof Periods', typeof Periods, 'Periods.length', Periods.length);
				if (Ext.getCmp('personBskRegistryWindow').isButtonAdd || typeof Periods == 'undefined' 
						|| (typeof Periods == 'object' && Periods.length == 0) 
						|| (typeof Periods == 'array' && Periods.length == 0)) {
					setTimeout(function() {
						Ext.getCmp('personBskRegistryWindow').getBodyMassIndexParams();	
					},200);
				}
                //Ext.getCmp('addBskDataButton').setDisabled(false);                       
            },
            ...
            getComboUnits: function(id){
                var store= new Ext.data.JsonStore({
                		url:'/?c=BSK_Register_User&m=getComboUnits',
                        autoLoad: true,
                		baseParams: {
            ...
            createField : function(format, params){
                ...
                switch(parseInt(format)){
                    ...
                    case 100: 
                          ...
                          var bgcolor = Ext.getCmp('personBskRegistryWindow').getBgColor(params.data.id);   

                          var textColor = parseInt(params.data.id).inlist(Ext.getCmp('personBskRegistryWindow').idInDb) ? '#1B769C' : 'black';  
                    ...
                    case 1: 
                            ...
                            //console.log('combo: ', Ext.getCmp('personBskRegistryWindow').elemDisabled())
                    		var ComboBox = new Ext.form.ComboBox({
                                ...
                                listeners : {
									'specialkey': function( field, e ) {
										if (e.getKey() == e.TAB /*&& Ext.getCmp('personBskRegistryWindow').collapsedCombo*/) {											
											Ext.getCmp('personBskRegistryWindow').switchFieldOnTab(params.data.id, false);
										}
									...
									'collapse' : function() {
										//Ext.getCmp('personBskRegistryWindow').collapsedCombo = true;
										/*Ext.getCmp('personBskRegistryWindow').switchFieldOnTab(params.data.id, false);*/
										
									},
									'expand' : function() {
										//Ext.getCmp('personBskRegistryWindow').collapsedCombo = false;				
										Ext.getCmp('personBskRegistryWindow').combExp = params.data.id;
									},
                                    ...
                                    'change': function(){
                                        ...
                                        //this.getEl().setStyle('color', id.inlist(Ext.getCmp('personBskRegistryWindow').idInDb) ? '#0B7EAE' : '#7A7A7A');
                                        ...
                                        */ 
                                        ...
                                        Ext.getCmp('personBskRegistryWindow').manageDrugSelect(paramsData, this);
                                        
                                        //управление лекарственным лечением других ПН (не скрининг !=84)
                                        Ext.getCmp('personBskRegistryWindow').manageLLquestions(id);
                                    ...
                                    'render': function(){
                                        ...
                                        {
                                            Ext.getCmp('saveBskDataButton').setDisabled(Ext.getCmp('personBskRegistryWindow').elemDisabled());
                                        }
                                        ...
                                        //this.getEl().setStyle('color', id.inlist(Ext.getCmp('personBskRegistryWindow').idInDb) ? '#0B7EAE' : '#7A7A7A');
                                        this.getEl().setStyle('padding','2px');
                                        this.setDisabled(Ext.getCmp('personBskRegistryWindow').elemDisabled());
                            ...
                            ComboBox.on('expand',
                                function(){
                                     ...
                                     if (id == 269) {
                                         ...
                                         var paramsWindow = {
                                            age: Ext.getCmp('personBskRegistryWindow').personInfo.age,
                                            sis: sisHand
                                         }
                    		...
                    		id : 'Answer_'+params.data.id,
            					...
            					listeners: {
                                    'specialkey':  function(field, e){
                                        ...
                                        if( (parseInt(params.data.id) == 107) || (parseInt(params.data.id) == 108) ){
                                            if (e.getKey() == e.ENTER) {
                                                  Ext.getCmp('personBskRegistryWindow').getIndex(parseInt(params.data.id), 110);
                                            }
                                        }
                                        //Лёгочная гипертензия
                                        else if( (parseInt(params.data.id) == 142) || (parseInt(params.data.id) == 143) ){
                                            if (e.getKey() == e.ENTER) {
                                                  Ext.getCmp('personBskRegistryWindow').getIndex(parseInt(params.data.id), 172);
                                            }
                                        }     
                                        //Артеириальная гипертензия
                                        else if( (parseInt(params.data.id) == 208) || (parseInt(params.data.id) == 209) ){
                                            if (e.getKey() == e.ENTER) {
                                                  Ext.getCmp('personBskRegistryWindow').getIndex(parseInt(params.data.id), 210);
                                            }
                                        }      
                                        //Ишемическая болезнь сердца
                                        else if( (parseInt(params.data.id) == 318) || (parseInt(params.data.id) == 319) ){
                                            if (e.getKey() == e.ENTER) {
                                                  Ext.getCmp('personBskRegistryWindow').getIndex(parseInt(params.data.id), 320);
                                            }
                                        } 
                                                ...
                                                if(typeof Ext.getCmp('Answer_'+imtField_id)  == 'undefined'){                                                
                                                    for(var k in form.listIDS){
                                                        ...
                                                        if(form.listIDS[k].BSKObservElement_id == imtField_id){
                                                            ...
                                                            Ext.getCmp('personBskRegistryWindow').getIMT(this,fields);                
                                                            field.setDisabled(true);
                                                        }
                                                    }   
                                                }
                                    ...
                                    'render': function(){
                                        ...
                                        //if(Ext.getCmp('personBskRegistryWindow').robot === false){
                                            //this.focus(true);
                                        //}
                ...
                */
                ...
                if(params.data.BSKObservElementFormat_id == 7){
                    ...
                    switch(parseInt(params.data.id)){
                        ...
                        case 317 : //ИБС        
                            ...
                            this.getValueOnDb('/?c=BSK_Register_User&m=havePrivilege', field);
                            field.disabled = true;
                            //return true;
                        break;    
                    ...
                    if(parseInt(params.data.id).inlist(Ext.getCmp('personBskRegistryWindow').idInDb)){
                        params.data.answer = [[0,'Да'], [1,'Нет']];
                    ...
                    switch(parseInt(params.data.id)){
                        ...
                        case 329:       
                            ...
                            this.getValueOnDb('/?c=BSK_Register_User&m=checkDiabetes', field);
                        ...
                        case 332:     
                            ...
                            this.getValueOnDb('/?c=BSK_Register_User&m=checkDisease', field);
                        ...
                        case 36:
                            ...
                            this.getValueOnDb('/?c=BSK_Register_User&m=checkGypofunction', field);
                        ...
                        case 37:
                            ...
                            this.getValueOnDb('/?c=BSK_Register_User&m=checkAutoimmune', field);
                        ...
                        case 43:
                            ...
                            this.getValueOnDb('/?c=BSK_Register_User&m=checkFattyLiver', field);
                        ...
                        case 44:
                            ...
                            this.getValueOnDb('/?c=BSK_Register_User&m=checkStonesInBubble', field);
                        ...
                        case 58:
                            ...
                            this.getValueOnDb('/?c=BSK_Register_User&m=checkSnoring', field);
                        ...
                        case 59:
                            ...
                            this.getValueOnDb('/?c=BSK_Register_User&m=checkBadHear', field);
                        ...
                        case 60:
                            ...
                            this.getValueOnDb('/?c=BSK_Register_User&m=checkDysfunction', field);
                        ...
                        case 61:
                            ...
                            this.getValueOnDb('/?c=BSK_Register_User&m=checkPolycystic', field);
                        ...
                        case 67:
                            ...
                            this.getValueOnDb('/?c=BSK_Register_User&m=checkGout', field);
                        ...
                        case 78:
                            ...
                            this.getValueOnDb('/?c=BSK_Register_User&m=checkLipodystrophy', field);
                        ...
                        case 79:
                            ...
                            this.getValueOnDb('/?c=BSK_Register_User&m=checkGlycogen', field);
                        ...
                        case 176:
                            ...
                            this.getValueOnDb('/?c=BSK_Register_User&m=checkHIV', field);
                            field.setDisabled(true);
                        ...
                        case 177:
                            ...
                            this.getValueOnDb('/?c=BSK_Register_User&m=checkPortalHypertension', field);
                            field.setDisabled(true);
                        ...
                        case 178:
                            ...
                            this.getValueOnDb('/?c=BSK_Register_User&m=checkLungPathology', field);
                            field.setDisabled(true);
                        ...
                        case 179:
                            ...
                            this.getValueOnDb('/?c=BSK_Register_User&m=checkHeartDefects', field);
                            field.setDisabled(true);
                        ...
                        case 180:
                            ...
                            this.getValueOnDb('/?c=BSK_Register_User&m=checkTissueDiseases', field);
                            field.setDisabled(true);
                        ...
                        case 181:
                            ...
                            this.getValueOnDb('/?c=BSK_Register_User&m=checkSnoringDiag', field);
                            field.setDisabled(true);
                        break; 
                        // Саркоидоз
                        case 182:
                            ...
                            this.getValueOnDb('/?c=BSK_Register_User&m=checkSarcoidosis', field);
                            field.setDisabled(true);
                        break;            
                        // Гистиоцитоз
                        case 183:
                            ...
                            this.getValueOnDb('/?c=BSK_Register_User&m=checkHistiocytosis', field);
                            field.setDisabled(true);
                        break;                                                                                                                     
                        // Шистосомоз
                        case 184:
                            ...
                            this.getValueOnDb('/?c=BSK_Register_User&m=checkSchistosomiasis', field);
                            field.setDisabled(true);
                        break;        
                        ...
                        case 220:
                            ...
                            this.getValueOnDb('/?c=BSK_Register_User&m=checkDiabetesDiag', field);
                            field.setDisabled(true);
                        break; 
                        // ИБС
                        case 330:      
                            ...
                            this.getValueOnDb('/?c=BSK_Register_User&m=checkIBS', field);
                            field.setDisabled(true);
                        break;     
                        case 221:      
                            params.data.width = 400;
                        ...
                        case 331:     
                            ...
                            this.getValueOnDb('/?c=BSK_Register_User&m=checkCerebrovascular', field);
                            field.setDisabled(true);
                        break;   
                        ...
                        case 223:
                            ...
                            this.getValueOnDb('/?c=BSK_Register_User&m=checkDiseaseDiag', field);
                            field.setDisabled(true);
                        break;                                                                                                                                                                                                                          
                        default: ''; break;                                
                    }
				...
				if (params.data.stage == 1) {
					/*if (params.data.min != null && age >= params.data.min && params.data.max != null && age < params.data.max ||
						...
						color = 'red';
						Ext.getCmp('personBskRegistryWindow').postfixQuestions.push(params.data.id);
					}
					else {
						color = 'gray';
					}*/
					color = 'red';
					var idQuestion = params.data.id;
					if (!idQuestion.inlist(Ext.getCmp('personBskRegistryWindow').postfixQuestions)) {
						Ext.getCmp('personBskRegistryWindow').postfixQuestions.push(params.data.id);
					}
				} else {
					color = 'gray';
                ...
                if(typeof field == 'undefined'){
                               ...
                               this.body.on('click', function() {
                                   ...
                                   if(typeof questionID != 'undefined'){
                                       ...
                                       if((typeof Ext.getCmp('Answer_'+questionID) == 'undefined' || Ext.getCmp('Answer_'+questionID).getValue() == '') 
											   && !Ext.getCmp('personBskRegistryWindow').elemDisabled()){
                                           Ext.Msg.alert('Ошибка ввода', 'Необходимо ввести препарат');
                                           return;
                                       }
                                   }                                   
                                   //конец защиты
                                   Ext.getCmp('personBskRegistryWindow').renderAnswer(params); 
								   
                                   if(params.data.BSKObservElementFormat_id == 1){
										if (Ext.getCmp('personBskRegistryWindow').isUserClick) {
					...
					Ext.getCmp('personBskRegistryWindow').switchFieldOnTab(params.data.id, true);
			...
			getBodyMassIndexParams: function() {	
				...
				Ext.Ajax.request({
					url: '/?c=BSK_Register_User&m=getLastAnketData',
					params: {
						Person_id   : Ext.getCmp('personBskRegistryWindow').personInfo.Person_id
					},
					callback: function(options, success, response) {						
            ...
            renderAnswer: function(params){
                ...
                if(typeof heightId != 'undefined' && typeof weightId != 'undefined'){
                    if(parseInt(Ext.getCmp('personBskRegistryWindow').MorbusType_id).inlist([84,88,89,19,50])){
                ...
                if(!params.data.id.inlist([
										   ...
										   /*Нарушения ритма и проводимости сердца*/417,420,421,422,423,424,425
                                           ]) 
            ...
            getValueOnDb: function(url, answer_field){
				//return;
                Ext.getCmp('personBskRegistryWindow').field = answer_field;
                ...
                Ext.Ajax.request({
                	...
                	callback: function(options, success, response) {
                        ...
                        /* OLD
                           ...
                           console.log('callback', Ext.getCmp('personBskRegistryWindow').field); 
                       ...
                       */
                       ...
                       if(typeof answer_field != 'undefined'){
                           ...
                           if( Ext.getCmp('personBskRegistryWindow').MorbusType == 84){ 
                           ...
                           if(!answer_field.id.inlist(['Answer_26','Answer_175','Answer_207','Answer_317'])){   
								...
								if (answer_field.getValue() == 'Да' /*|| Ext.getCmp('personBskRegistryWindow').elemDisabled()*/){ //если ответ выбран да  или анкету открывает не автор анкеты
								   ...
								   //answer_field.setDisabled(Ext.getCmp('personBskRegistryWindow').elemDisabled());
								}
                           }
						   else{
							   answer_field.setDisabled(true);
                    ...
                    scope : Ext.getCmp('personBskRegistryWindow').field
                });
            ...
            */
			...
			getLastVizitRegistryOks : function(BSKRegistry_id){
				var form = Ext.getCmp('personBskRegistryWindow');
				Ext.getCmp('compare').setDisabled(true);
				Ext.getCmp('addBskDataButton').setDisabled(true);
				Ext.getCmp('saveBskDataButton').setDisabled(true);
				Ext.getCmp('personBskRegistryWindow').clickToPN = 0; //это проверка на несохраненные данные, здесь не нужна
				Ext.getCmp('information').removeAll();
					...
					callback: function(options, success, response) {
						...
						if (success === true) {
							...
							var TextFieldDate = new sw.Promed.SwDateField({
								...
								xtype: 'swdatefield',
								format: 'd.m.Y',
								value : result.BSKRegistry_setDateFormat
							});
							...
							Ext.getCmp('calday').setValue(result.BSKRegistry_setDateFormat);
							Ext.getCmp('calday').datetime = new Date(result.BSKRegistry_setDate).format('Y-m-d H:i');
							//Ловим регистр с которым работаем - установление признака просмотра анкеты
							if (result.BSKRegistry_isBrowsed == '1') {
								form.BSKRegistry_id = result.BSKRegistry_id;
								Ext.Ajax.request({
									url: '/?c=BSK_Register_User&m=setIsBrowsed',
									params: {
							...
							for(var k in result.groups){
								...
								switch ( result.groups[k].id ) {
									case '32':
									case '44':
										var itemsColumn = new Ext.FormPanel({
											id : 'groupPanel_'+result.groups[k].id,
											border: false,
											layout: 'column', 
											bodyStyle: 'margin: 10px;',
											items : [{
												...
												layout: 'form',
												items: [{
													id : 'QuestionPanel_'+result.groups[k].id,
													title: result.groups[k].group,
													xtype: 'fieldset',
													autoHeight: true,
													width: 1200,
													labelWidth: 130,
														...
														items: [
															{
																...
																id : 'itemsQuestionPanel_'+result.groups[k].id,
																layout: 'form',
																border: false,
																labelAlign: 'right',
																labelWidth: 250,
								...
								Ext.getCmp('information').add(itemsColumn);
							}
							Ext.getCmp('information').doLayout();
							
							var dataQuestions = result.questions;

							dataQuestions.forEach(function(item, dQ, dataQuestions) {
								if (dataQuestions[dQ].BSKObservElementGroup_id.inlist(groups)) {
									//попытка отформатировать текст в дату, на будущее - формат поля из базы можно будет взять BSKObservElementFormat_id
									if (dataQuestions[dQ].BSKObservElement_id.inlist(['276','271','272','274','308','270','311','312'])) {
										var BSKRegistryData_data = new Date(dataQuestions[dQ].BSKRegistryData_data).format('d.m.Y H:i');
										BSKRegistryData_data = BSKRegistryData_data!='NaN.NaN.NaN NaN:NaN'?BSKRegistryData_data:null;
									}
									if (dataQuestions[dQ].BSKObservElementGroup_id.inlist(['30','31'])) {
										var TextField = new Ext.form.Checkbox({
											id : 'Answer_'+dataQuestions[dQ].BSKObservElement_id,
											boxLabel:  (dataQuestions[dQ].BSKRegistryData_data == 'Да'||dataQuestions[dQ].BSKRegistryData_data == 'Да.')?'<span style="color:red!important">'+dataQuestions[dQ].BSKObservElement_name+'</span>':dataQuestions[dQ].BSKObservElement_name,
											fieldLabel: '',
											labelSeparator: '',
											readOnly: true,
											tabIndex : -1,
											style: 'padding: 5px;',
											checked: (dataQuestions[dQ].BSKRegistryData_data == 'Да'||dataQuestions[dQ].BSKRegistryData_data == 'Да.')?true:false
										});
									} else
										var TextField = new Ext.form.TextField({
											id : 'Answer_'+dataQuestions[dQ].BSKObservElement_id,
											fieldLabel: dataQuestions[dQ].BSKObservElement_name,
											disabled : true,
											tabIndex : -1,
											width : 300,
											value : BSKRegistryData_data?BSKRegistryData_data:dataQuestions[dQ].BSKRegistryData_data
										});
									var groupCMP = '44';
									if (dataQuestions[dQ].BSKObservElement_id == '302' && groupCMP.inlist(groups)) {
										...
										Ext.getCmp('itemsQuestionPanel_'+dataQuestions[dQ].BSKObservElementGroup_id).add(TextField);
								}
							});
							Ext.getCmp('information').doLayout();
			...
			getBSKObjectWithoutAnket : function(){
				var form = Ext.getCmp('personBskRegistryWindow');
				Ext.getCmp('compare').setDisabled(true);
				Ext.getCmp('addBskDataButton').setDisabled(true);
				Ext.getCmp('saveBskDataButton').setDisabled(true);
				Ext.getCmp('personBskRegistryWindow').clickToPN = 0; //это проверка на несохраненные данные, здесь не нужна
				Ext.getCmp('information').removeAll();
				var PersonSex = form.personInfo.Sex_Name;
				var PersonAge = form.personInfo.Person_Age;
				//81,82,83,84,85 Инвалиды
				var PrivilegeType_id = (form.personInfo.PrivilegeType_id)? form.personInfo.PrivilegeType_id:0;
				var PrivilegeType_Name = PrivilegeType_id.inlist([81,82,83,84,85])? form.personInfo.PrivilegeType_Name:'Нет';
				var HSNhidden = form.MorbusType_id===110? false:true;
		
				Ext.Ajax.request({
					url: '/?c=BSK_Register_User&m=getBSKObjectWithoutAnket',
					params: {
						MorbusType_id : form.MorbusType_id,
						Person_id : form.personInfo.Person_id
					},
					callback: function(options, success, response) {
						...
						if (success === true) {
							...
							var TextFieldDate = new sw.Promed.SwDateField({
								id : 'TextFieldDate',
								fieldLabel: 'Дата включения в предмет наблюдения',
								labelSeparator : ':',
								labelWidth : '270px',
								disabled: true,
								width: '100px',
							...
							var BSKEvnGrid = new sw.Promed.ViewFrame({
								...
								border: true,
								object: 'BSKEvnGrid',
								dataUrl: '/?c=BSK_Register_User&m=loadBSKEvnGrid',
								autoLoadData: false,
								focusOnFirstLoad: false,
								autoExpandColumn: 'autoexpand',
								autoExpandMin: 200,
								...
								stringfields: [
									...
									{name: 'Diag_FullName', header: 'Диагноз', type: 'string', width:140},
									...
									{name: 'Lpu_Nick', header: 'Медицинская организация', id: 'autoexpand'},
									...
									items:[
										{
											...
											width: 800,
											autoHeight: true,
											title: 'Дата включения в предмет наблюдения',
											collapsible: false,
											layout: 'column',
											items:{
												layout: 'form',
											...
											items:{
												...
												items:[
													{
														fieldLabel: 'Пол пациента',
														disabled: true,
														width: 300,
														xtype: 'textfield',
														value: PersonSex
													},{
														fieldLabel: 'Возраст пациента',
														disabled: true,
														width: 300,
														xtype: 'textfield',
														value: PersonAge
							...
							Ext.getCmp('calday').setValue(result[0].PersonRegister_setDate);
							Ext.getCmp('calday').datetime = new Date(result[0].PersonRegister_setDate).format('Y-m-d H:i');

							Ext.getCmp('information').doLayout();
							Ext.getCmp('BSKEvnGrid').getGrid().getStore().baseParams = {
								MorbusType_id : form.MorbusType_id,
								Person_id : form.personInfo.Person_id
							}
							Ext.getCmp('BSKEvnGrid').getGrid().getStore().load({
								MorbusType_id : form.MorbusType_id,
								Person_id : form.personInfo.Person_id
							});
						}
        ...
        sw.Promed.personBskRegistryWindow.superclass.initComponent.apply(this, arguments);
    },
	...
	panelButtonClick: function(winType) {
        ...
        var personInfo = Ext.getCmp('personBskRegistryWindow').personInfo;
    ...
    getInfoPacient : function(Person_id){
              ...
              Ext.Ajax.request({
    		 		url: '/?c=BSK_Register_User&m=loadPersonData',
    				params: {
    				    Person_id : Person_id
    				},
    				callback: function(options, success, response) {
                        ...
                        if (success === true) {
                             ...
                             Ext.getCmp('personBskRegistryWindow').personInfo = personData.personInfo;
                             Ext.getCmp('personBskRegistryWindow').personInfo.age = Ext.getCmp('personBskRegistryWindow').getAge(personData.personInfo.Person_Birthday, new Date());
    ...
    show : function(params){
        ...
        if (typeof params.BSKRegistry_id != 'undefined') {
            this.EMKBSKRegistry_id = params.BSKRegistry_id;
            this.EMKBSKObject_id = params.BSKObject_id;
        }
        sw.Promed.personBskRegistryWindow.superclass.show.apply(this, arguments);
    ...
    listeners : {
		...
		'hide': function() {
		    Ext.getCmp('personBskRegistryWindow').clickToPN = 0;
			if (this.refresh)
				this.onHide();
