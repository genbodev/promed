			...
			stringfields: [
 			...
 			actions: [
                ...
                handler : function()
                {
                     ...
                     Ext.Ajax.request({
        		 		url: '/?c=BSK_Register&m=deleteGroupType',
        				params: {
        				    BSKObservElementGroup_id : BSKObservElementGroup_id
        				},
        				callback: function(options, success, response) {
                            if (success === true) {
                              Ext.getCmp('AdminBSKViewForm').showMsg('Группа сведений удалён');  
                              ...
                              if(newGroupType.getValue().length< 2){
                                  ...
                                  Ext.Ajax.request({
                        				...
                        				callback: function(options, success, response) {
                                            ...
                                            if (success === true) {
                                              Ext.getCmp('AdminBSKViewForm').showMsg('Группа сведений успешно добавлен');    
 			...
 			actions: [
				{name:'action_add', text: 'Создать', disabled: true,
                 handler: function() { 
                    ...
                    if(typeof(GroupType_parent) == 'undefined'){
                          Ext.getCmp('AdminBSKViewForm').showMsg('Необходимо создать группы типов сведений');                       
                    }
                    else
                 ...
                 },
                {
                 ...
                 handler: function(){
                     ...
                     Ext.Ajax.request({
        		 		url: '/?c=BSK_Register&m=deleteType',
        				params: {
        				    Type_id : record_id,
                            pmUser_updID : null
        				},
        				callback: function(options, success, response) {
                            if (success === true) {
                              Ext.getCmp('AdminBSKViewForm').showMsg('Тип сведений удалён');   
                  ...
                  handler: function() {
                              ...
                              if(newType.getValue().length< 2){
                                  ...
                                  if(symbol.getValue().replace(/\s/g,'') != ''){
                                        Ext.getCmp('AdminBSKViewForm').checkSymbol(symbol.getValue());

                                        if(typeof(Ext.getCmp('AdminBSKViewForm').checkSymbolTest) == 'undefined'){
 			...
 			actions: [
				{name:'action_add', text: 'Создать', 
                 handler: function() { 
                    ...
                    if(Types.data.BSK_TypesInfo_Format_id == 3 || Types.data.BSK_TypesInfo_Format_id == 4 || Types.data.BSK_TypesInfo_Format_id == 7 || Types.data.BSK_TypesInfo_Format_id == 8){
                            Ext.getCmp('AdminBSKViewForm').showMsg('Данный тип сведений не может иметь значение');   
                			return false;                                               
                    }                    
                        
                    Ext.getCmp('AdminBSKViewForm').manageCellsAddValuePanel('add');
                ...
                {name:'action_edit', text: 'Изменить', 
                 handler: function() {
                    ...
                    Ext.getCmp('AdminBSKViewForm').manageCellsAddValuePanel('edit');
                ...
                {name:'action_delete', text: 'Удалить',
                 handler : function(){
                     ...
                     Ext.Ajax.request({
        		 		url: '/?c=BSK_Register&m=deleteValue',
        				params: {
        				    Value_id: Value_id.id
        				},
        				callback: function(options, success, response) {
                            if (success === true) {
                              Ext.getCmp('AdminBSKViewForm').showMsg('Значение удалёно');   
