...
sw.Promed.swRegistryViewWindow = Ext.extend(sw.Promed.BaseForm,
{
	...
	onTreeClick: function(node,e)
	{
		...
		switch (level)
		{
			...
			case 4:
				...
				owner.AccountGrid.setActionDisabled('action_print', ((RegistryStatus_id!=4) || !(RegistryType_id.inlist([1,2,4,5,6,15])))); // !!! Пока только для полки, потом поправить обратно 
	...
	*/
	...
	exportRegistryToDBF: function()
	{
		...
		var fd = 'swUnionRegistryDBFWindow';
		var params = {onHide: function() { this.UnionRegistryGrid.loadData(); }.createDelegate(this), Registry_id: record.get('Registry_id'), KatNasel_id: record.get('KatNasel_id'), RegistryType_id: record.get('RegistryType_id'), url:'/?c=Registry&m=exportUnionRegistryToDBF'};
	...
	importRegistryFromTFOMS: function()
	{
		...
		var params =
		{
			...
			callback: function()
			{
				this.UnionRegistryGrid.getGrid().getStore().reload();
			}.createDelegate(this),
	...
	initComponent: function() 
	{
		...
		this.AccountGrid = new sw.Promed.ViewFrame(
		{
			...
			onRowSelect: function(sm,index,record)
			{
				...
				if (this.getCount()>0)
				{
					...
					// и параклинике (#54769)
					if (RegistryType_id.inlist([1,2,15]) && record.get('KatNasel_id') == 2) {
						this.setActionDisabled('action_print', true);
					} else {
						this.setActionDisabled('action_print', ((RegistryStatus_id!=4) || !(RegistryType_id.inlist([1,2,4,5,6,7,9,11,12,15]))));
					}
					...
					if ( record.get('RegistryStatus_id') != 5 ) {
						if (record.get('RegistryCheckStatus_Code') == 2) {
							Ext.menu.MenuMgr.get('RegistryMenu').items.itemAt(0).disable();
							Ext.menu.MenuMgr.get('RegistryMenu').items.itemAt(2).disable();

							// Плюс заблочить действия для дочерних гридов
						}
						else {
							Ext.menu.MenuMgr.get('RegistryMenu').items.itemAt(0).enable();
							Ext.menu.MenuMgr.get('RegistryMenu').items.itemAt(2).enable();
						}
					}
					...
					// Дисаблим акшены по статусу
					// Если полка или стац
					// Если не в папке удаленных
					if (!deletedRegistriesSelected){
						if (record.get('RegistryType_id').inlist(['1','2','6','15'])) {
							// Если к оплате 
							if (record.get('RegistryStatus_id')=='2' && Ext.menu.MenuMgr.get('RegistryMenu').items.itemAt(3)) {
								Ext.menu.MenuMgr.get('RegistryMenu').items.itemAt(3).setVisible(record.get('KatNasel_SysNick') != 'all' && (isUserGroup('RegistryUser') || isSuperAdmin() || isLpuAdmin(record.get('Lpu_id')) || record.get('KatNasel_SysNick') == 'inog')); // суперадмин или иногородние (refs #12116)
							}
							// Если оплачен
							...
							// Если к оплате 
							if (record.get('RegistryStatus_id')=='2' && Ext.menu.MenuMgr.get('RegistryMenu').items.itemAt(3)) {
								Ext.menu.MenuMgr.get('RegistryMenu').items.itemAt(3).setVisible(record.get('KatNasel_SysNick') != 'all');
							}
							// Если оплачен
							if (record.get('RegistryStatus_id')=='4' && Ext.menu.MenuMgr.get('RegistryMenu').items.itemAt(1)) {
		...
		this.AccountGrid.getGrid().view = new Ext.grid.GridView(
		{
			getRowClass : function (row, index)
			{
				...
				if ( row.get('Registry_IsProgress') == 1 || row.get('RegistryCheckStatus_Code') == 2 ) {
		...
		this.DataGrid = new sw.Promed.ViewFrame(
		{
			...
			[
				...
				{name: 'Mes_Code', type: 'string', header: 'КСГ/КПГ', width:80},
			...
			{
				var
					RegistryCheckStatus_Code;
					RegistryStatus_id = form.Tree.selModel.selNode.attributes.object_value;

				var reestr = form.AccountGrid.getGrid().getSelectionModel().getSelected();

				if ( typeof reestr == 'object' && !Ext.isEmpty(reestr.get('RegistryCheckStatus_Code')) ) {
					RegistryCheckStatus_Code = reestr.get('RegistryCheckStatus_Code');
				}

				form.DataGrid.setActionDisabled('action_edit', RegistryCheckStatus_Code == 2);
				form.DataGrid.setActionDisabled('action_delete',(RegistryStatus_id!=3 || RegistryCheckStatus_Code == 2));
				form.DataGrid.setActionDisabled('action_openperson', RegistryCheckStatus_Code == 2);
			},
			onDblClick: function()
			{
		...
		this.DataGrid.getGrid().view = new Ext.grid.GridView(
		{
			getRowClass : function (row, index)
			{
				...
				if ((row.get('IsRDL')>0) && (isAdmin))
					cls = cls+'x-grid-rowblue ';
				if (row.get('Err_Count') > 0 || (RegistryStatus_id == 4 && row.get('RegistryData_IsPaid') == 1))
		...
		this.TFOMSErrorGrid = new sw.Promed.ViewFrame(
		{
			...
			onRowSelect: function(sm,rowIdx,record)
			{
				...
				if (this.getCount()>0 && RegistryCheckStatus_Code != 2)
				{
			...
			onLoadData: function()
			{
				...
				this.setActionDisabled('action_edit',RegistryCheckStatus_Code == 2);
				this.setActionDisabled('action_openperson',RegistryCheckStatus_Code == 2);
				this.setActionDisabled('action_openevn',RegistryCheckStatus_Code == 2);
				this.setActionDisabled('action_delete',(RegistryStatus_id!=3 || RegistryCheckStatus_Code == 2));
				this.setActionDisabled('action_deleteall',(RegistryStatus_id!=3 || RegistryCheckStatus_Code == 2));
			}
		});

		columnsFilter = ['Evn_ident','RegistryErrorType_Code','Person_FIO','LpuSection_Name'];
		...
		this.UnionRegistryGrid = new sw.Promed.ViewFrame(
		{
			...
			onRowSelect: function(sm,rowIdx,record)
			{
				...
				if (record && record.get('KatNasel_SysNick') == 'all' && record.get('RegistryCheckStatus_Code') != 2) {
		...
		this.UnionRegistryGrid.getGrid().view = new Ext.grid.GridView({
			getRowClass: function (row, index) {
				var cls = '';

				if ( row.get('RegistryCheckStatus_Code') == 2 ) {
					cls = cls + 'x-grid-rowgray ';
				}
		...
		this.UnionActionsMenu = new Ext.menu.Menu({
			items: [{
				name: 'action_exportdbf',
				text: 'Экспорт в DBF',
				tooltip: 'Экспорт в DBF',
				handler: function () {
					form.exportRegistryToDBF();
				}
			}, {
				name: 'action_importfromtfoms',
