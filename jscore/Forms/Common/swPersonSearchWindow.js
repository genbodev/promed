...
sw.Promed.swPersonSearchWindow = Ext.extend(sw.Promed.BaseForm, {
	...
	onOkButtonClick: function (callback_data) {
		...
		if (callback_data && callback_data.Person_id > 0) {
			...
			data_to_return.onHide = function () {
				var index = grid.getStore().findBy(function(rec) { return rec.get('Person_id') == callback_data.Person_id; });
				grid.focus();
			...
			var person_index = grid.getStore().findBy(function(rec) { return rec.get('Person_id') == callback_data.Person_id; });
		...
		log({
			swPersonSearchWindow: 'onOkButtonClick',
	...
	show: function () {
		...
		if (isUserGroup('PM')) {
			win.hide();
			sw.swMsg.alert('Ошибка', 'Не хватает прав доступа к форме "Человек: Поиск"' );
		...
		if ( !this.PersonSearchViewFrame.getAction('action_revive') ) {
			this.PersonSearchViewFrame.addActions({
				...
				name: 'action_revive',
				text: lang['udalit_priznak_smerti'],
				iconCls: 'actions16',
				disabled: true,
				handler: function () {
		...
		if (arguments[0]) {
			...
			switch (this.searchMode) {
				case 'wow':
					this.setTitle(WND_PERS_SEARCH + lang['tolko_registr_vov']);
					break;

				case 'ddorpperiod':
					this.setTitle(WND_PERS_SEARCH + lang['tolko_registr_periodicheskih_osmotrov_nesovershennoletnih']);
					break;

				case 'ddorp':
				case 'ddorpsec':
					this.setTitle(WND_PERS_SEARCH + lang['tolko_registr_detey-sirot']);
					break;
			...
			if (arguments[0].Person_ids) {
				...
				if (this.Person_ids)  this.Person_ids = null;
			}
			//BOB - 21.03.2017
						
		}
		if(this.viewOnly || getWnd('swWorkPlaceMZSpecWindow').isVisible() || getWnd('swWorkPlacePolkaLLOWindow').isVisible() || arguments[0].Person_ids){
		...
		//form.findField('Person_country').setValue('');
	...
	initComponent: function () {
						...
						items: [{
							...
							items: [{
								...
								items: [{
									...
									labelWidth: 50,
									layout: 'form',
									hidden: !(isAdmin || (getRegionNick() == 'ufa' && isLpuAdmin()) || (getRegionNick() == 'ekb' && (isUserGroup('LpuUser') || isLpuAdmin())) || (getRegionNick() == 'perm')),
									items: [{
		...
		win.PersonSearchViewFrame = new sw.Promed.ViewFrame({
			id: /*win.id +*/ 'PersonSearchViewFrame',
			paging: true,
			pageSize: 100,
			uniqueId: true,
			region: 'center',
			...
			onRowSelect: function(sm, index, record) {
				...
				if((win.viewOnly == true) || getWnd('swWorkPlaceMZSpecWindow').isVisible() || getWnd('swWorkPlacePolkaLLOWindow').isVisible()) {
					this.setActionDisabled('action_view', false);
				}
				if (record && record.get('Person_id') && win.viewOnly == false && !getWnd('swWorkPlaceMZSpecWindow').isVisible() && !getWnd('swWorkPlacePolkaLLOWindow').isVisible()) {
			...
			stringfields: [
				...
				{name: 'Person_Inn', hidden: true, hideable: false},
				...
				{name: 'PersonQuarantine_IsOn', hidden: true, hideable: false},
				...
				{name: 'CmpLpu_id', hidden: true, hideable: false},
				{
					//id: 'autoexpand',
					...
					hidden: !(
						...
						(isAdmin || (getRegionNick() == 'ufa' && isLpuAdmin()) || (getRegionNick() == 'perm'))
					),
					sortable: true,
					width: 90
				},
				...
				new Ext.grid.CheckColumn({
					header: langs('Застрахован'),
					name: 'Person_IsInFOMS',
					width: 65,
					sortable: true,
					disabled: true,
					hidden: getRegionNick() != 'kz'
				...
				new Ext.grid.CheckColumn({
					header: (getRegionNick().inlist(['kz']) ? "Льгота" : "Фед. льг"),
