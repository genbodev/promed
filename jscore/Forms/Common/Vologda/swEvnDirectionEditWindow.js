...
sw.Promed.swEvnDirectionEditWindow = Ext.extend(sw.Promed.BaseForm,
{
	...
	returnData: function(params) {
		...
		if (getRegionNick() == 'kz' && params.DirType_id && params.DirType_id.inlist(['1', '5'])) {
	...
	refreshFilesPanel: function(EvnDirection_id, dir_type_id){
		...
		if (getRegionNick() == 'kz' && dir_type_id && dir_type_id.inlist(['1', '5'])) {
	...
	getFinanceSource: function() {
		...
		if (!DirType_id.inlist([1, 2, 3, 4, 5, 10])) return false;
	...
	show: function() {
		...
		if ( this.profileUslugaLinkStore.getCount() == 0 && getRegionNick() == 'kz' ) {
		...
		if (params.action != 'add')
		{
			bform.load(
			{
				...
				success: function(record)
				{
					...
					win.setPrintItemVisible(this.buttons[1].menu.items.items, bform.getForm().findField('UslugaComplex_did').getValue('UslugaComplex_id'));
		...
		base_form.items.each(function(field){
			...
			switch(field.getName()) {
					...
					visible = !Ext.isEmpty(value)
						|| (
							...
							&& (
								...
								|| (DirType_Code.inlist([1, 4, 5]) && Region_Nick.inlist(['kz']))
							)
						);
					allowBlank = !(
						visible
				...
				case 'Diag_cid':
					...
					break;
				case 'ScreenType_id':
					visible = Region_Nick.inlist(['kz']) && Lpu_IsNotForSystem == 2 &&
	...
	setPrintItemVisible: function(aMenuItems, sUslugaId) {
    	var form = this;
    	...
    	if (!! sUslugaId) {
			Ext.Ajax.request({
				...
				callback: function(opt, success, response) {
					...
					var oLab = aAttributes.find(function(oAttr) {
						return oAttr.UslugaComplexAttributeType_SysNick == 'lab';
					});
					
					if (!! oLab) {

						form.showAllPrintItems(aMenuItems);						
						
						aMenuItems.forEach(function(oItem) {
							if (!!oItem.tag && oItem.tag != '') {
								var oAttr = aAttributes.find(function (oAttr) {
									return oItem.tag == oAttr.UslugaComplexAttributeType_SysNick 
								});
						...
						form.hideAllPrintItems(aMenuItems);
					}
	...
	hideAllPrintItems: function(aMenuItems) {
    	var form = this;
		aMenuItems.forEach(function(oItem, index) {
			oItem.hide();
		});

		this.buttons[1].handler = function() {
			form.printEvnDirection();
		};
	},

	showAllPrintItems: function(aMenuItems) {
		var form = this;
		aMenuItems.forEach(function(oItem, index) {
			oItem.show();
		});
