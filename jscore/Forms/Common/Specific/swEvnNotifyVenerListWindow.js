/**
* swEvnNotifyVenerListWindow - Журнал Извещений о больных венерическими заболеваниями
*
...
* @version      2012/10
* @comment      Префикс для id компонентов ENVLW (EvnNotifyVenerListWindow)
*
*/
sw.Promed.swEvnNotifyVenerListWindow = Ext.extend(sw.Promed.BaseForm, {codeRefresh: true,
	buttonAlign: 'left',
	closable: true,
	closeAction: 'hide',
	collapsible: true,
	getButtonSearch: function() {
		return Ext.getCmp('ENVLW_SearchButton');
	},
	doReset: function() {
	...
	doSearch: function(params) {
		...
		if ( !params.firstLoad && this.findById('EvnNotifyVenerListFilterForm').isEmpty() ) {
		...
		var post = getAllFormFieldValues(this.findById('EvnNotifyVenerListFilterForm'));
	...
	initComponent: function() {
		...
		this.RootViewFrame = new sw.Promed.ViewFrame({
			actions: [
				...
				{name: 'action_print', handler: function() {
					...
					var EvnNotifyVener_id = selected_record.data.EvnNotifyVener_id;
			...
			autoLoadData: false,
			dataUrl: C_SEARCH,
			id: 'ENVLW_EvnNotifyVenerListSearchGrid',
			object: 'EvnNotifyVenerList',
			pageSize: 100,
			paging: true,
			region: 'center',
			root: 'data',
			stringfields: [
				{name: 'EvnNotifyVener_id', type: 'int', header: 'ID', key: true},
				{name: 'EvnNotifyVener_pid', type: 'int', hidden: true},
				{name: 'EvnNotifyVener_setDT', type: 'date', format: 'd.m.Y', header: lang['data_sozdaniya'], width: 120},	
			...
			onRowSelect: function(sm,index,record) {
				...
				var disable = (Ext.isEmpty(record.get('Person_id')) || !Ext.isEmpty(record.get('PersonRegister_setDate')) || isRegisterAutoInclude('vener'));
				this.getAction('person_register_include').setDisabled(disable);
				this.getAction('person_register_not_include').setDisabled(disable);
			},
			onDblClick: function(sm,index,e) {
				if ( index >= 0 && !Ext.isEmpty(this.getGrid().getStore().getAt(index).get('Person_id')) ) {
		...
		Ext.apply(this, {
			buttons: [{
				...
				iconCls: 'search16',
				tabIndex: TABINDEX_ENVLW + 120,
				id: 'ENVLW_SearchButton',
				text: BTN_FRMSEARCH
			}, {
				handler: function() {
					this.doReset();
			...
			HelpButton(this, -1),
			{
				...
				onTabAction: function() {
					this.findById('ENVLW_SearchFilterTabbar').getActiveTab().fireEvent('activate', this.findById('ENVLW_SearchFilterTabbar').getActiveTab());
				}.createDelegate(this),
				tabIndex: TABINDEX_ENVLW + 124,
				text: BTN_FRMCLOSE
			}],
			getFilterForm: function() {
				if ( this.filterForm == undefined ) {
					this.filterForm = this.findById('EvnNotifyVenerListFilterForm');
				}
				return this.filterForm;
			},
			items: [ getBaseSearchFiltersFrame({
				allowPersonPeriodicSelect: true,
				id: 'EvnNotifyVenerListFilterForm',
				labelWidth: 130,
				ownerWindow: this,
				searchFormType: 'EvnNotifyVener',
				tabIndexBase: TABINDEX_ENVLW,
				tabPanelHeight: 215,
				tabPanelId: 'ENVLW_SearchFilterTabbar',
				tabs: [{
					autoHeight: true,
					...
					listeners: {
						'activate': function(panel) {
							...
							form.findField('isNotifyProcessed').setContainerVisible(!isRegisterAutoInclude('vener'));
						}.createDelegate(this)
					},
		...
		sw.Promed.swEvnNotifyVenerListWindow.superclass.initComponent.apply(this, arguments);
	...
	listeners: {
		...
		'beforeShow': function(win) {
			if (String(getGlobalOptions().groups).indexOf('Vener', 0) < 0)
			{
				...
				{
					sw.swMsg.alert('Сообщение', 'Форма "'+ win.title +'" доступна только для пользователей, с указанной группой «Регистр по Венерологии»');
					return false;
				}
			}
		},
        ...
        'resize': function (win, nW, nH, oW, oH) {
			win.findById('ENVLW_SearchFilterTabbar').setWidth(nW - 5);
	...
	resizable: true,
	show: function() {
		sw.Promed.swEvnNotifyVenerListWindow.superclass.show.apply(this, arguments);
		...
		this.RootViewFrame.addActions({
			...
			iconCls: 'reset16',
			disabled: true,
			hidden: (String(getGlobalOptions().groups).indexOf('Vener', 0) < 0),
			menu: new Ext.menu.Menu({id:'ENVLW_personRegisterNotIncludeMenu'})
		});
	...
	createPersonRegisterFailIncludeCauseMenu: function() {
		sw.Promed.personRegister.createPersonRegisterFailIncludeCauseMenu({
			id: 'ENVLW_personRegisterNotIncludeMenu',
			ownerWindow: this,
			getParams: function(){
				...
				return {
					EvnNotifyBase_id: record.get('EvnNotifyVener_id')
				};
			}.createDelegate(this),
	...
	openViewWindow: function()
	{
		...
		if ( getWnd('swEvnNotifyVenerEditWindow').isVisible() ) {
			getWnd('swEvnNotifyVenerEditWindow').hide();
		}
		getWnd('swEvnNotifyVenerEditWindow').show({action: 'view', EvnNotifyVener_id: record.get('EvnNotifyVener_id')});
	},
	emkOpen: function()
	{
		var grid = this.RootViewFrame.getGrid();
	...
	title: lang['jurnal_izvescheniy_o_bolnyih_venericheskimi_zabolevaniyami'],
	width: 800
});
