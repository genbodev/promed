/**
* swLargeFamilyRegistryWindow - Регистр по детям из многодетных семей
*
...
* @version      2012/11
* @comment      Префикс для id компонентов LFRW (LargeFamilyRegistryWindow)
*
*/
sw.Promed.swLargeFamilyRegistryWindow = Ext.extend(sw.Promed.BaseForm, {codeRefresh: true,
	buttonAlign: 'left',
	closable: true,
	closeAction: 'hide',
	collapsible: true,
	getButtonSearch: function() {
		return Ext.getCmp('LFRW_SearchButton');
	},
	doReset: function() {
		
		var base_form = this.findById('LargeFamilyRegistryFilterForm').getForm();
		base_form.reset();
		this.LargeFamilyRegistrySearchFrame.ViewActions.person_register_dis.setDisabled(true);
		this.LargeFamilyRegistrySearchFrame.ViewActions.action_view.setDisabled(true);
		this.LargeFamilyRegistrySearchFrame.ViewActions.action_delete.setDisabled(true);
		this.LargeFamilyRegistrySearchFrame.ViewActions.action_refresh.setDisabled(true);
		this.LargeFamilyRegistrySearchFrame.getGrid().getStore().removeAll();
	},
	doSearch: function(params) {
		...
		if ( !params.firstLoad && this.findById('LargeFamilyRegistryFilterForm').isEmpty() ) {
			sw.swMsg.alert(lang['oshibka'], lang['ne_zapolneno_ni_odno_pole'], function() {
			});
			return false;
		}
		
		var grid = this.LargeFamilyRegistrySearchFrame.getGrid();
		...
		var post = getAllFormFieldValues(this.findById('LargeFamilyRegistryFilterForm'));
		...
		if ( base_form.isValid() ) {
			this.LargeFamilyRegistrySearchFrame.ViewActions.action_refresh.setDisabled(false);
		...
		switch(action) {
			case 'person_register_dis':
				sw.Promed.personRegister.out({
					...
					,MorbusType_SysNick: 'large family'
			...
			case 'add':
				sw.Promed.personRegister.add({
                    MorbusType_SysNick: 'large family' // Ребенок из многодетной семьи
					,Diag_id: 11259
					,searchMode: 'dt6'
					,callback: function(data) {
	...
	initComponent: function() {
		...
		var LargeFamilyRegistry = ( String(getGlobalOptions().groups).indexOf('LargeFamilyRegistry', 0) >= 0 );
		
		this.LargeFamilyRegistrySearchFrame = new sw.Promed.ViewFrame({
			actions: [
				{name: 'action_add', disabled: !LargeFamilyRegistry, handler: function() { this.openWindow('add') }.createDelegate(this)},
			...
			autoLoadData: false,
			dataUrl: C_SEARCH,
			id: 'LFRW_LargeFamilyRegistrySearchGrid',
			object: 'LargeFamilyRegistry',
			pageSize: 100,
			paging: true,
			region: 'center',
			root: 'data',
			stringfields: [
				...
				{name: 'Diag_Name', type: 'string', hidden: true},
			...
			onRowSelect: function(sm,index,record) {
				this.getAction('action_delete').setDisabled( record.get('DrugRequest') > 0 || !LargeFamilyRegistry || record.get('Lpu_id') != getGlobalOptions().lpu_id );
				this.getAction('person_register_dis').setDisabled( Ext.isEmpty(record.get('PersonRegister_disDate')) == false || !LargeFamilyRegistry || record.get('Lpu_id') != getGlobalOptions().lpu_id );
			}
		});

		this.SearchFilters = getBaseSearchFiltersFrame({
			allowPersonPeriodicSelect: true,
			id: 'LargeFamilyRegistryFilterForm',
			labelWidth: 130,
			ownerWindow: this,
			searchFormType: 'LargeFamilyRegistry',
			tabIndexBase: TABINDEX_LFRW,
			tabPanelHeight: 215,
			tabPanelId: 'LFRW_SearchFilterTabbar',
			tabs: [{
				autoHeight: true,
		...
		Ext.apply(this, {
			buttons: [{
				...
				iconCls: 'search16',
				tabIndex: TABINDEX_LFRW + 120,
				id: 'LFRW_SearchButton',
				text: BTN_FRMSEARCH
			}, {
				handler: function() {
					this.doReset();
			...
			HelpButton(this, -1),
			{
				...
				onTabAction: function() {
					this.findById('LFRW_SearchFilterTabbar').getActiveTab().fireEvent('activate', this.findById('LFRW_SearchFilterTabbar').getActiveTab());
				}.createDelegate(this),
				tabIndex: TABINDEX_LFRW + 124,
				text: BTN_FRMCLOSE
			}],
			getFilterForm: function() {
				if ( this.filterForm == undefined ) {
					this.filterForm = this.findById('LargeFamilyRegistryFilterForm');
				}
				return this.filterForm;
			},
			items: [ this.SearchFilters, this.LargeFamilyRegistrySearchFrame]
		});

		sw.Promed.swLargeFamilyRegistryWindow.superclass.initComponent.apply(this, arguments);
	...
	listeners: {
		...
		'maximize': function(win) {
			win.findById('LargeFamilyRegistryFilterForm').doLayout();
		},
		'restore': function(win) {
			win.findById('LargeFamilyRegistryFilterForm').doLayout();
		},
        'resize': function (win, nW, nH, oW, oH) {
			win.findById('LFRW_SearchFilterTabbar').setWidth(nW - 5);
			win.findById('LargeFamilyRegistryFilterForm').setWidth(nW - 5);
		}
	},
	maximizable: true,
	minHeight: 550,
	...
	resizable: true,
	show: function() {
		sw.Promed.swLargeFamilyRegistryWindow.superclass.show.apply(this, arguments);
		this.LargeFamilyRegistrySearchFrame.addActions({
			name:'action_export', 
		...
		this.maximize();
		this.doReset();
		//this.findById('LFRW_SearchFilterTabbar').setActiveTab(0);
		...
		if ( isSaratovOuzUser || String(getGlobalOptions().groups).indexOf('LargeFamilyRegistry', 0) >= 0 ) {
		...
		{
			this.LargeFamilyRegistrySearchFrame.setActionHidden('action_add', true);
			this.LargeFamilyRegistrySearchFrame.setActionHidden('action_delete', true);
		}
		else {
			this.LargeFamilyRegistrySearchFrame.setActionHidden('action_add', false);
			this.LargeFamilyRegistrySearchFrame.setActionHidden('action_delete', false);
		}
		this.doLayout();
	...
	title: lang['registr_po_detyam_iz_mnogodetnyih_semey'],
	width: 800
});
