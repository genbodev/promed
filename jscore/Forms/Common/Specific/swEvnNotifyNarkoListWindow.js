/**
* swEvnNotifyNarkoListWindow - Журнал Извещений по наркологии
*
...
* @version      2012/10
* @comment      Префикс для id компонентов ENNLW (EvnNotifyNarkoListWindow)
*
*/
sw.Promed.swEvnNotifyNarkoListWindow = Ext.extend(sw.Promed.BaseForm, {codeRefresh: true,
	buttonAlign: 'left',
	closable: true,
	closeAction: 'hide',
	collapsible: true,
	getButtonSearch: function() {
		return Ext.getCmp('ENNLW_SearchButton');
	},
	doReset: function() {
	...
	doSearch: function(params) {
		...
		if ( !params.firstLoad && this.findById('EvnNotifyNarkoListFilterForm').isEmpty() ) {
		...
		var post = getAllFormFieldValues(this.findById('EvnNotifyNarkoListFilterForm'));
	...
	initComponent: function() {
		...
		this.RootViewFrame = new sw.Promed.ViewFrame({
			...
			autoLoadData: false,
			dataUrl: C_SEARCH,
			id: 'ENNLW_EvnNotifyNarkoListSearchGrid',
			object: 'EvnNotifyNarcoList',
			pageSize: 100,
			paging: true,
			region: 'center',
			root: 'data',
			stringfields: [
				{name: 'EvnNotifyNarco_id', type: 'int', header: 'ID', key: true},
				{name: 'EvnNotifyNarco_pid', type: 'int', hidden: true},
				{name: 'EvnNotifyNarco_setDT', type: 'date', format: 'd.m.Y', header: lang['data_sozdaniya'], width: 120},	
			...
			onRowSelect: function(sm,index,record) {
				...
				var disable = (Ext.isEmpty(record.get('Person_id')) || !Ext.isEmpty(record.get('PersonRegister_setDate')) || isRegisterAutoInclude('narko'));
		...
		Ext.apply(this, {
			buttons: [{
                ...
                handler: function() {
                    ...
                    var paramLpu = 0;
                    var paramEvnNotifyNarco = 0;
                    if (record && record.get('EvnNotifyNarco_id') ){
                        if(!Ext.isEmpty(record.get('Lpu_id'))){
                            paramLpu = record.get('Lpu_id')
                        }
                        else{
                            paramLpu = getGlobalOptions().lpu_id;
                        }
                        paramEvnNotifyNarco = record.get('EvnNotifyNarco_id');
						printBirt({
							'Report_FileName': 'f091u.rptdesign',
							'Report_Params': '&paramLpu=' + paramLpu + '&paramEvnNotifyNarco=' + paramEvnNotifyNarco,
							'Report_Format': 'pdf'
						});
                    }
                }.createDelegate(this),
                tabIndex: TABINDEX_ENCLW + 124,
                text: lang['pechat_izvescheniya']
            },{
				text: '-'
			},
			HelpButton(this, -1),
			{
				...
				onTabAction: function() {
					this.findById('ENNLW_SearchFilterTabbar').getActiveTab().fireEvent('activate', this.findById('ENNLW_SearchFilterTabbar').getActiveTab());
				}.createDelegate(this),
				tabIndex: TABINDEX_ENCLW + 124,
				text: BTN_FRMCLOSE
			}],
			getFilterForm: function() {
				if ( this.filterForm == undefined ) {
					this.filterForm = this.findById('EvnNotifyNarkoListFilterForm');
				}
				return this.filterForm;
			},
			items: [ getBaseSearchFiltersFrame({
				allowPersonPeriodicSelect: true,
				id: 'EvnNotifyNarkoListFilterForm',
				labelWidth: 130,
				ownerWindow: this,
				searchFormType: 'EvnNotifyNarko',
				tabIndexBase: TABINDEX_ENCLW,
				tabPanelHeight: 215,
				tabPanelId: 'ENNLW_SearchFilterTabbar',
				tabs: [{
					autoHeight: true,
					...
					listeners: {
						'activate': function(panel) {
							...
							form.findField('isNotifyProcessed').setContainerVisible(!isRegisterAutoInclude('narko'));
						}.createDelegate(this)
					},
		...
		sw.Promed.swEvnNotifyNarkoListWindow.superclass.initComponent.apply(this, arguments);
	...
	listeners: {
        ...
        'resize': function (win, nW, nH, oW, oH) {
			win.findById('ENNLW_SearchFilterTabbar').setWidth(nW - 5);
	...
	resizable: true,
	show: function() {
		sw.Promed.swEvnNotifyNarkoListWindow.superclass.show.apply(this, arguments);
		...
		this.RootViewFrame.addActions({
			...
			iconCls: 'reset16',
			disabled: true,
			hidden: (String(getGlobalOptions().groups).indexOf('Narko', 0) < 0),
			menu: new Ext.menu.Menu({id:'ENNLW_personRegisterNotIncludeMenu'})
		});
	...
	createPersonRegisterFailIncludeCauseMenu: function() {
		sw.Promed.personRegister.createPersonRegisterFailIncludeCauseMenu({
			id: 'ENNLW_personRegisterNotIncludeMenu',
			ownerWindow: this,
			getParams: function(){
				...
				return {
					EvnNotifyBase_id: record.get('EvnNotifyNarco_id')
				};
			}.createDelegate(this),
	...
	openViewWindow: function()
	{
		...
		if ( getWnd('swEvnNotifyNarcoEditWindow').isVisible() ) {
			getWnd('swEvnNotifyNarcoEditWindow').hide();
		}
		getWnd('swEvnNotifyNarcoEditWindow').show({type:'narco', action: 'view', EvnNotifyNarco_id: record.get('EvnNotifyNarco_id')});
	},
	emkOpen: function()
	{
		var grid = this.RootViewFrame.getGrid();
	...
	title: lang['jurnal_izvescheniy_po_narkologii'],
	width: 800
});
