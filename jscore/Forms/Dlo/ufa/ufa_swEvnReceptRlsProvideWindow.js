	...
	show: function() {
		...
		Ext.getCmp('DrugNomen_OtpDate').setValue(new Date);
                ...
                if (arguments[0].selection == 1) {  //  Если передан параметр
                    ...
                    Ext.getCmp('DrugNomen_WhsDocumentCostItemType').ownerCt.hide();
                                ...
                                items: [
                                            ...
                                            ]},
                                            {
                                                ...
                                                items: [
                                                    ...
                                                    id: 'DrugNomen_WhsDocumentCostItemType',
		...
		this.GridPanel = new sw.Promed.ViewFrame({
			...
			editGrid: function (action) {
				...
				if (action == 'add') {
					...
					params.MedService_id = wnd.MedService_id;
                                        //console.log ('editGrid = ' + Ext.getCmp('DrugNomen_WhsDocumentCostItemType').value);
                                        if (!Ext.getCmp('DrugNomen_WhsDocumentCostItemType').ownerCt.hidden &&  Ext.getCmp('DrugNomen_WhsDocumentCostItemType').value != '')
                                            //  Если передана статья расхода
                                            params.WhsDocumentCostItemType_id = Ext.getCmp('DrugNomen_WhsDocumentCostItemType').value;
					...
					getWnd(view_frame.editformclassname).show({
						...
						callback: function(data) {
							...
							wnd.setSum();
                                                        ...
                                                        if  (!Ext.getCmp('DrugNomen_WhsDocumentCostItemType').ownerCt.hidden) 
...
//						}
						...
						view_frame.getGrid().store.insert(0, [new Ext.data.Record({
							    Grid_id: Grid_id,
...
//							    EndOst_Sum: rec['EndOst_Sum'].toFixed(2),
			...
			deleteRecord: function() {
				...
				wnd.setSum();
                                //console.log('getStore'); console.log(view_frame.getGrid().getStore().getCount());
                                if  (!Ext.getCmp('DrugNomen_WhsDocumentCostItemType').ownerCt.hidden) 
                                    if (view_frame.getGrid().getStore().getCount() == 0)
                                            Ext.getCmp('DrugNomen_WhsDocumentCostItemType').enable();
                                        else
