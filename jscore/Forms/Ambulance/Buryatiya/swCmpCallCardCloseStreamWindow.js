...
sw.Promed.swCmpCallCardCloseStreamWindow = Ext.extend(sw.Promed.BaseForm, {
	...
	objectSrc: '/jscore/Forms/Ambulance/Buryatiya/swCmpCallCardCloseStreamWindow.js',
	allfields: null,
	action: null,
	buttonAlign: 'left',
...
//		)
	...
	initComponent: function() {			
		...
		$.ajax({
			...
			console.log('baseaj.allfields', baseaj.allfields);
		});
						...
						items: [
							{
								...
								items: [
									{											
										...
										fieldLabel: '1. Номер фельдшера по приему вызова',
										name: 'Feldsher_id',
										xtype: 'textfield',
										maskRe: /\d/
									}, {
									...
									{
										fieldLabel: '3. Номер бригады скорой медицинской помощи',
						...
						items : [
							{
								...
								hiddenName: 'FeldsherAcceptPskov',
								allowBlank:true,
								width: 250,
	...
	show: function() {
		...
		*/
		...
		if (getGlobalOptions().region.nick == 'pskov') {			
			...
			SpecField.hide();
			SpecField.getEl().up('.x-form-item').setDisplayed(false);
			var SpecField = this.FormPanel.getForm().findField('FeldsherAcceptName');	
			SpecField.hide();
			SpecField.getEl().up('.x-form-item').setDisplayed(false);
			var SpecField = this.FormPanel.getForm().findField('FeldsherTransName');	
			SpecField.hide();
			SpecField.getEl().up('.x-form-item').setDisplayed(false);	
		}
		
		if (getGlobalOptions().region.nick != 'pskov') {
			var SpecField = this.FormPanel.getForm().findField('FeldsherAcceptPskov');
			SpecField.hide();
			SpecField.getEl().up('.x-form-item').setDisplayed(false);
			var SpecField = this.FormPanel.getForm().findField('FeldsherTransPskov');
			SpecField.hide();
			SpecField.getEl().up('.x-form-item').setDisplayed(false);
