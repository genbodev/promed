...
/**
* swEvnCytologicProtocolsViewWindow - Журнал протоколов цитологических диагностических исследований
* swCytologicalDiagnosticTestProtocolsViewWindow
...
sw.Promed.swEvnCytologicProtocolsViewWindow = Ext.extend(sw.Promed.BaseForm, {
	border: false,
	buttonAlign: 'left',
	closeAction: 'hide',
	deleteEvnHistologicProto: function() {
		...
		if ( !grid || !grid.getGrid() ) {
			sw.swMsg.alert(langs('Ошибка'), langs('При удалении протокола патологогистологического исследования возникли ошибки [Тип ошибки: 1]'));
			return false;
		}
		else if ( !grid.getGrid().getSelectionModel().getSelected() ) {
			sw.swMsg.alert(langs('Ошибка'), langs('Не выбран протокол патологогистологического исследования из списка'));
			return false;
		}
		...
		sw.swMsg.show({
				...
				if ( 'yes' == buttonId ) {
					Ext.Ajax.request({
						failure: function(response, options) {
							sw.swMsg.alert(langs('Ошибка'), langs('При удалении протокола патологогистологического исследования возникли ошибки [Тип ошибки: 2]'));
						},
						params: {
						...
						success: function(response, options) {
							...
							if ( response_obj.success == false ) {
								sw.swMsg.alert(langs('Ошибка'), response_obj.Error_Msg ? response_obj.Error_Msg : langs('При удалении протокола патологогистологического исследования возникли ошибки [Тип ошибки: 3]'));
							}
							else {
			...
			msg: langs('Удалить протокол патологогистологического исследования?'),
			title: langs('Вопрос')
		});
	},
	...
	initComponent: function() {
		...
		this.SearchGrid = new sw.Promed.ViewFrame({
			...
			onRowSelect: function(sm, index, record) {
				...
				else
				{
					...
					this.SearchGrid.ViewActions.action_isnotbad.setDisabled(disallowIsNotBad || disallowEdit);
					this.SearchGrid.ViewActions.action_isbad.setDisabled(disallowIsBad || disallowEdit);
				}
		...
		sw.Promed.swEvnCytologicProtocolsViewWindow.superclass.initComponent.apply(this, arguments);
	},
	keys: [{
		fn: function(inp, e) {
	...
	setIsBad: function(flag) {
		...
		Ext.Ajax.request({
			failure: function(response, options) {
				...
				sw.swMsg.alert(langs('Ошибка'), langs('Ошибка при ') + (flag == true ? langs('аннулировании') : langs('отмене аннулирования')) + langs(' протокола патологогистологического исследования [Тип ошибки: 2]'));
			}.createDelegate(this),
			params: {
				EvnHistologicProto_id: id,
			...
			success: function(response, options) {
				...
				if ( response_obj.success == false ) {
					sw.swMsg.alert(langs('Ошибка'), response_obj.Error_Msg ? response_obj.Error_Msg : langs('Ошибка при ') + (flag == true ? langs('аннулировании') : langs('отмене аннулирования')) + langs(' протокола патологогистологического исследования [Тип ошибки: 3]'));
				}
				else {
					var base_form = this.FilterPanel.getForm();
	...
	show: function() {
		sw.Promed.swEvnCytologicProtocolsViewWindow.superclass.show.apply(this, arguments);
		this.getLoadMask().show();
