/**
* swEvnDirectionCrioEditWindow - направление на перенос эмбрионов
*
...
sw.Promed.swEvnDirectionCrioEditWindow = Ext.extend(sw.Promed.BaseForm, {
	/* свойства */
	action: null,
	buttonAlign: 'left',
	closable: true,
	...
	formStatus: 'edit',
	height: 550,
	id: 'EvnDirectionCrioEditWindow',
	keys: [{
		alt: true,
		fn: function(inp, e) {
			var win = Ext.getCmp('EvnDirectionCrioEditWindow');
	...
	doSave: function(options) {
		var win = this;
		// options @Object
		// options.print @Boolean Вызывать печать направления на перенос эмбрионов, если true
		...
		base_form.submit({
			...
			success: function(result_form, action) {
				...
				if ( action.result ) {
					if ( action.result.EvnDirectionCrio_id ) {
						base_form.findField('EvnDirectionCrio_id').setValue(action.result.EvnDirectionCrio_id);

						this.callback();

						if ( options && options.print ) {
							this.printEvnDirectionCrio(true);
						}
						else {
							this.hide();
						}
	...
	onHide: Ext.emptyFn,
	printEvnDirectionCrio: function(print) {
		...
		window.open('/?c=EvnDirectionCrio&m=printEvnDirectionCrio&EvnDirectionCrio_id=' + base_form.findField('EvnDirectionCrio_id').getValue(), '_blank');
	},
	setEvnDirectionCrioNumber: function() {
		var base_form = this.FormPanel.getForm();
		...
		Ext.Ajax.request({
			callback: function(options, success, response) {
				if ( success ) {
					...
					base_form.findField('EvnDirectionCrio_Num').setValue(response_obj.EvnDirectionCrio_Num);
				}
				else {
					sw.swMsg.alert(langs('Ошибка'), langs('Ошибка при определении номера направления'), function() { base_form.findField('EvnDirectionCrio_Num').focus(true); }.createDelegate(this) );
				}
			}.createDelegate(this),
			url: '/?c=EvnDirectionCrio&m=getEvnDirectionCrioNumber'
		});
	},
	show: function() {
		sw.Promed.swEvnDirectionCrioEditWindow.superclass.show.apply(this, arguments);
		...
		switch ( this.action ) {
			case 'add':
				this.setTitle(langs('Направление на перенос эмбрионов: Добавление'));
				this.enableEdit(true);

				this.setEvnDirectionCrioNumber();
				...
				setCurrentDateTime({
					callback: function() {
						base_form.findField('EvnDirectionCrio_setDate').fireEvent('change', base_form.findField('EvnDirectionCrio_setDate'), base_form.findField('EvnDirectionCrio_setDate').getValue());

						loadMask.hide();
					
						base_form.findField('EvnDirectionCrio_Num').focus(true, 250);
					}.createDelegate(this),
					dateField: base_form.findField('EvnDirectionCrio_setDate'),
					loadMask: false,
					setDate: true,
			...
			case 'edit':
			case 'view':
				var EvnDirectionCrio_id = base_form.findField('EvnDirectionCrio_id').getValue();

				if ( !EvnDirectionCrio_id ) {
					loadMask.hide();
					this.hide();
					return false;
				}
				...
				base_form.load({
					...
					success: function(form, act) {
						...
						if ( this.action == 'edit' ) {
							this.setTitle(langs('Направление на перенос эмбрионов: Редактирование'));
							this.enableEdit(true);
						}
						else {
							this.setTitle(langs('Направление на перенос эмбрионов: Просмотр'));
					...
					url: '/?c=EvnDirectionCrio&m=loadEvnDirectionCrioEditForm'
				});
				break;
	...
	initComponent: function() {
		...
		win.FormPanel = new Ext.form.FormPanel({
			...
			reader: new Ext.data.JsonReader({
				...
				{ name: 'accessType' },
				{ name: 'Diag_id' },
				{ name: 'EvnDirectionCrio_Comment' },
				{ name: 'EvnDirectionCrio_CommentVKMZ' },
				{ name: 'EvnDirectionCrio_GiveDate' },
				{ name: 'EvnDirectionCrio_id' },
				{ name: 'EvnDirectionCrio_Num' },
				{ name: 'EvnDirectionCrio_NumVKMZ' },
				{ name: 'EvnDirectionCrio_setDate' },
				{ name: 'EvnDirectionCrio_VKMZDate' },
				{ name: 'KLRgnRF_id' },
				{ name: 'Org_id' },
				{ name: 'MedPersonal_id' },
			...
			region: 'center',
			url: '/?c=EvnDirectionCrio&m=saveEvnDirectionCrio',
		...
		this.PersonInfo = new sw.Promed.PersonInfoPanel({
			button1OnHide: function() {
				if ( this.action == 'view' ) {
					...
					this.FormPanel.getForm().findField('EvnDirectionCrio_Num').focus(true);
				}
			}.createDelegate(this),
			...
			buttons: [{
				...
				onShiftTabAction: function() {
					...
					if ( this.action == 'view' ) {
						...
						base_form.findField('EvnDirectionCrio_Comment').focus();
					}
				}.createDelegate(this),
		...
		sw.Promed.swEvnDirectionCrioEditWindow.superclass.initComponent.apply(this, arguments);
	}
});