...
function include_async_code()
{
	...
	fileref.setAttribute("src", "jscore/cryptopro_async.js");
...
function Common_RawSign(id, text, callback)
{
	var canAsync = !!cadesplugin.CreateObjectAsync;
	if(canAsync)
	{
		include_async_code().then(function(){
			return RawSign_Async(id, text, callback);
		});
	}else
	{
		return RawSign_NPAPI(id, text, callback);
	}
}

function Common_CheckForPlugIn() {
	cadesplugin.set_log_level(cadesplugin.LOG_LEVEL_ERROR); // cadesplugin.LOG_LEVEL_DEBUG
	var canAsync = !!cadesplugin.CreateObjectAsync;
	if(canAsync)
	{
		include_async_code().then(function(){
...
function Common_FillCertList(lstId) {
	var canAsync = !!cadesplugin.CreateObjectAsync;
	if(canAsync)
	{
		include_async_code().then(function(){
			return FillCertList_Async(lstId);
		});
	}else
	{
		return FillCertList_NPAPI(lstId);
	}
}
...
function MakeRawSign_NPAPI(dataToSign, certObject, setDisplayData, isBase64) {
	var errormes = "";
	...
	try {
		...
		alert('Failed to create CAdESCOM.HashedData: ' + err.number);
		return;
	}
	...
	if (dataToSign) {
		...
		try {
			Signature = oRawSignature.SignHash(oHashedData, certObject);
		}
		catch (err) {
...
function RawSign_NPAPI(certListBoxId, data, callback) {
	var certificate = GetCertificate_NPAPI(certListBoxId);
	try
	{
		var signature = MakeRawSign_NPAPI(data, certificate);
		callback({
			success: true,
			signature: signature
		});
...
function CheckForPlugIn_NPAPI() {
	...
	try {
		...
		document.getElementById('PlugInEnabledTxt').innerHTML += " Версия плагина: " + MakeVersionString(CurrentPluginVersion);
