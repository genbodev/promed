//Ext4.define("Ext4.locale.en.ux.picker.DateTimePicker", {
//	  override: "Ext4.ux.DateTimePicker",
//	  todayText: "Now",
//	  timeLabel: 'Time'
//  });

Ext4.define('Ext4.ux.DateTimePicker', {
	  extend: 'Ext4.picker.Date',
	  alias: 'widget.datetimepicker',
	  todayText: 'Текущее время',
	  timeLabel: 'Время',
	  timeFormat: 'H:i:s',
	  requires: ['Ext4.ux.form.TimePickerField'],
	  ...
	  onRender: function(container, position) {
		  if(!this.timefield) {
			  this.timefield = Ext4.create('Ext4.ux.form.TimePickerField', {
				    fieldLabel: this.timeLabel,
				    labelWidth: 36,
				    value: Ext4.Date.format(this.value, this.timeFormat),
		  ...
		  var table = Ext4.get(Ext4.DomQuery.selectNode('table', this.el.dom));
		  var tfEl = Ext4.core.DomHelper.insertAfter(table, {
			    tag: 'div',
			    style: 'border:0px;',
			    children: [{
				      tag: 'div',
	  ...
	  // listener изменение времени, timefield change
	  ...
	  handleDateClick: function(e, t) {
		  ...
		  e.stopEvent();
		  if(!me.disabled && t.dateValue && !Ext4.fly(t.parentNode).hasCls(me.disabledCellCls)) {
	  ...
	  selectToday: function() {
		  ...
		  if(btn && !btn.disabled) {
			  // me.setValue(Ext4.Date.clearTime(new Date())); //src
