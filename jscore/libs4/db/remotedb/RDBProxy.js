...
/**
 * @class Ext6.rdb.Proxy
 * @extends Ext6.data.DataProxy
 * An implementation of {@link Ext6.data.DataProxy} that reads from a SQLLite
 * database.
 *
 * @constructor
 * @param {Object} conn an {@link Ext6.rdb.Connection} object
 * @param {String} table The name of the database table
 * @param {String} keyName The primary key of the table
 * @param {Ext6.data.JsonStore} store The datastore to bind to
 ...
 */
Ext6.rdb.Proxy = function(conn, table, keyName, store, readonly){
	Ext6.rdb.Proxy.superclass.constructor.call(this);
	this.conn = conn;
...
Ext6.rdb.Proxy.DATE_FORMAT = 'Y-m-d H:i:s';

Ext6.extend(Ext6.rdb.Proxy, Ext6.data.DataProxy, {
	...
	processData : function(o) {
		...
		if(f){
			if(f.type == 'date'){
				r[key] = v ? v.format(Ext6.rdb.Proxy.DATE_FORMAT,10) : '';
