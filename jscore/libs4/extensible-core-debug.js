...
Ext6.define('Extensible', {
    ...
    // private
    hasBorderRadius : Ext6.supports.CSS3BorderRadius,
    ...
    getScrollWidth: function() {
        return Ext6.getScrollbarSize ? Ext6.getScrollbarSize().width : Ext6.getScrollBarWidth();
    },
    ...
    constructor: function() {
        Ext6.onReady(function() {
            if (Extensible.getScrollWidth() < 3) {
                ...
                Ext6.getBody().addCls('x-no-scrollbar');
            }
            if (Ext6.isWindows) {
                Ext6.getBody().addCls('x-win');
            }
        });
    },
    ...
    */
    Date : {
        ...
        compare : function(dt1, dt2, precise){
            var d1 = dt1, d2 = dt2;
            if(precise !== true){
                d1 = Ext6.Date.clone(dt1);
                d1.setMilliseconds(0);
                d2 = Ext6.Date.clone(dt2);
         ...
         */
        today : function() {
            return Ext6.Date.clearTime(new Date());
        },
...
Extensible.applyOverrides = function() {
    ...
    var extVersion = Ext6.getVersion();
    
    // This was fixed in Ext 4.0.5:
    if (Ext6.layout.container.AbstractCard) {
        Ext6.layout.container.AbstractCard.override({
    ...
    if (Ext6.picker && Ext6.picker.Color) {
        Ext6.picker.Color.override({
            constructor: function() {
                // use an existing renderTpl if specified
                this.renderTpl = this.renderTpl || Ext6.create('Ext6.XTemplate', '<tpl for="colors"><a href="#" ' +
    ...
    if (extVersion.isLessThan('4.1')) {
        if (Ext6.data && Ext6.data.reader && Ext6.data.reader.Reader) {
            Ext6.data.reader.Reader.override({
                extractData : function(root) {
                    ...
                    if (!root.length && Ext6.isObject(root)) {
                        root = [root];
                        length = 1;
                    }
    ...
    if (Ext6.form && Ext6.form.Basic) {
        Ext6.form.Basic.override({
            reset: function() {
                var me = this;
    ...
    if (Ext6.data && Ext6.data.proxy && Ext6.data.proxy.Memory) {
        Ext6.data.proxy.Memory.override({
            updateOperation: function(operation, callback, scope) {
                if (operation) {
                    Ext6.each(operation.records, function(rec) {
                        rec.commit();
                    });
                    operation.setCompleted();
                    operation.setSuccessful();
                    Ext6.callback(callback, scope || this, [operation]);
                }
            },
            create: function() {
...
Ext6.onReady(Extensible.applyOverrides);