Ext6.define('common.XmlTemplate.ShareWindow', {
	extend: 'base.BaseForm',
	alias: 'widget.swXmlTemplateShareWindow',
	...
	apply: function() {
		...
		if (shareTo.length == 0) {
			Ext6.Msg.alert(langs('Ошибка'), langs('Не выбраны пользователи для отправки шаблона'));
			return;
		}
		...
		Ext6.Ajax.request({
			url: '/?c=XmlTemplate6E&m=shareXmlTemplate',
			params: params,
			callback: function(options, success, response) {
				...
				if (responseObj.success) {
					...
					sw4.showInfoMsg({
						type: 'error',
						text: 'Ошибка при отправке шаблона'
					});
				}
			}
		});
	...
	show: function() {
		...
		if (!arguments[0] || !arguments[0].XmlTemplate_id) {
			me.hide();
			Ext6.Msg.alert(langs('Ошибка'), langs('Не переданы все необходимые параметры'));
			return;
		}

		me.XmlTemplate_id = arguments[0].XmlTemplate_id;
	},
	...
	initComponent: function() {
		...
		me.pmUserSearchField = Ext6.create('swBaseCombobox', {
			...
			store: Ext6.create('Ext6.data.Store', {
				...
				proxy: {
					type: 'ajax',
					url: '/?c=XmlTemplate6E&m=loadPMUserForShareList',
					reader: {type: 'json'}
				},
				listeners: {
					beforeload: function(store, operation) {
						var params = operation.getParams();
						params.XmlTemplate_id = me.XmlTemplate_id;
					},
					load: function(store) {
		...
		me.pmUserGrid = Ext6.create('Ext6.grid.Panel', {
			...
			columns: [
				...
				{xtype: 'actioncolumn', width: 30, align: 'end',
					items: [{
						getClass: () => 'sw-clear-trigger-big',
						getTip: () => 'Убрать',
						handler: (grid, rowIndex, colIndex, row, e, record) => {
