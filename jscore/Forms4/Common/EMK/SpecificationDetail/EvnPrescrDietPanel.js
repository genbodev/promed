/**
 * EvnPrescrDietPanel - Добавление диеты
 *
 ...
 */
Ext6.define('common.EMK.SpecificationDetail.EvnPrescrDietPanel', {
	/* свойства */
	alias: 'widget.EvnPrescrDietPanel',
	autoShow: false,
	cls: 'EvnPrescrDietPanel',
	constrain: true,
	...
	deletePrescr: function (){
		...
		if (cntr) {
			var grid = cntr.getGridByObject('EvnPrescrDiet');
			if (grid) {
				...
				if (rec){
					var cbFn = function(){
						...
						if(grid.getStore().getCount() > 0){
							...
							cntr.openSpecification('EvnPrescrDiet', grid, grid.getSelectionModel().getSelectedRecord(), true);
						}
						else {
	...
	doSave: function() {
		...
		base_form.url = '/?c=EvnPrescr&m=saveEvnPrescrDiet';
		if(me.PacketPrescr_id){
			base_form.url = '/?c=PacketPrescr&m=createPacketPrescrDiet';
