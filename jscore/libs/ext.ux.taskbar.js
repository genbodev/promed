...
Ext.ux.TaskBar.TaskButton = function(win, el){
    ...
    Ext.ux.TaskBar.TaskButton.superclass.constructor.call(this, {
        iconCls: win.iconCls,
        text: Ext.util.Format.ellipsis(win.title.replace(/<\/?[^>]+>/gi, ''), textLength),
        tooltip: win.title,
        ...
        handler : function(){
            if(win.minimized || win.hidden){
				markActive(win);
				win.toFront();
            }else if(win == sw.WindowMgr.getActive()){
                win.minimize();
            }else{
                win.toFront();
            }
...
Ext.extend(Ext.ux.TaskBar.TaskButton, Ext.Button, {
    onRender: function(){
        ...
        this.cmenu = new Ext.menu.Menu({
            items: [{
                ...
                scope: this.win,
				disabled: !this.win.minimizable
            },'-', {
                text: langs('Закрыть'),
                handler: this.closeWin.createDelegate(this, this.win, true),
                scope: this.win,
				disabled: !this.win.closable
            }]
        });
        ...
        this.cmenu.on('beforeshow', function(){
            ...
            var w = this.win;
            items[0].setDisabled(!w.minimizable || w.minimized);
        }, this);
	...
	setButtonText: function(text) {
        ...
        this.el.child(this.buttonSelector).dom['qtip'] = title;
		this.text = Ext.util.Format.ellipsis(title, this.textLength);
		if (this.el) {
			this.el.child("td.ux-taskbutton-center " + this.buttonSelector).update(this.text);
		}
	},
