...
Ext.extend(Ext.ux.UploadDialog.Dialog, Ext.Window, {
  ...
  rus2translit: function(text) {
		var arrru = new Array ('а','б','в','г','д','е','ё','ж','з','и','й','к','л','м','н','о','п','р','с','т','у','ф','х','ц','ч','ш','щ','ь','ы','ъ','э','ю','я','А','Б','В','Г','Д','Е','Ё','Ж','З','И','Й','К','Л','М','Н','О','П','Р','С','Т','У','Ф','Х','Ц','Ч','Ш','Щ','Ь','Ы','Ъ','Э','Ю','Я',' ');
		var arren = new Array ('a','b','v','g','d','e','jo','zh','z','i','y','k','l','m','n','o','p','r','s','t','u','f','kh','c','ch','sh','sch',"'",'y',"'",'e','ju','ja','A','B','V','G','D','E','Jo','Zh','Z','I','Y','K','L','M','N','O','P','R','S','T','U','F','Kh','C','Ch','Sh','Sch',"'",'Y',"'",'E','Ju','Ja','_');
  ...
  addFileToUploadQueue : function(browse_btn)
  {
    ...
    store.add(
      new Ext.ux.UploadDialog.FileRecord({
          ...
          filename: (getRegionNick() != 'kz' && this.url == '/?c=Mse&m=uploadFiles') ? this.rus2translit(input_file.dom.value) : input_file.dom.value,
...
p.i18n = {
    title: langs('Загрузка файлов на сервер.'),
    state_col_title: langs('Статус'),
    state_col_width: 70,
    filename_col_title: langs('Имя файла'),
    filename_col_width: 230,
    note_col_title: langs('Пояснение'),
    note_col_width: 150,
    add_btn_text: langs('Добавить'),
    add_btn_tip: langs('Добавить файл в очередь загрузки.'),
    remove_btn_text: langs('Удалить'),
    remove_btn_tip: langs('Удалить файл из очереди загрузки.'),
    reset_btn_text: langs('Очистить'),
    reset_btn_tip: langs('Очистить очередь загрузки.'),
    upload_btn_start_text: langs('Загрузить'),
    upload_btn_stop_text: langs('Отменить'),
    upload_btn_start_tip: langs('Загрузить файлы на сервер.'),
    upload_btn_stop_tip: langs('Отменить загрузку.'),
    close_btn_text: langs('Закрыть'),
    close_btn_tip: langs('Закрыть диалог загрузки.'),
    progress_waiting_text: langs('Жду...'),
    progress_uploading_text: langs('Загружаю: {0} из {1} файлов загружено.'),
    error_msgbox_title: langs('Ошибка'),
    permitted_extensions_join_str: ',',
    err_file_type_not_permitted: langs('Файлы с таким расширением запрещено загружать.<br/>Пожалуйста выбирайте файлы с одним из следующий расширений: {1}'),
    note_queued_to_upload: langs('Поставлен в очередь.'),
    note_processing: langs('Загружается...'),
    note_upload_failed: langs('Сервер недоступен, либо на сервере произошла ошибка.'),
    note_upload_success: langs('Загружен.'),
    note_upload_error: langs('Ошибка загрузки.'),
    note_aborted: langs('Загрузка прервана пользователем.'),
    note_canceled: langs('Загрузка отменена.')
}