...
Ext.db.webSqlDBDriver = function(config){
	Ext.apply(this, config);
	Ext.db.webSqlDBDriver.superclass.constructor.call(this);
	this.addEvents({
		open : true,
		close: true
	});
...
Ext.extend(Ext.db.webSqlDBDriver, Ext.util.Observable, {
	openState : false,
	/** Open database 
	 * Открывает базу данных 
	 */ 
	open : function(db, version, config) {
		if (Ext.isWebSqlDB) {
			version = (version)?version:'';
			try {
				this.conn = openDatabase(db, version, db, null); // , desc, size
			} 
			catch(e) {
				// например открываем версию которая меньше текущей 
				throw e;
			}
			if (!this.conn) {
	...
	setVersion : function (version, config) {
		...
		if(this.version!= this.conn.version && this.openState) {
			var requestVer = this.conn.changeVersion(this.conn.version, this.version, 
				// Callback 
				function () {
		...
		} else if (config && typeof config.success == 'function') {
			config.success();
		}
	},
	...
	createStore : function(o, config) {
		...
		var json = o.fields;
		if(!Ext.isArray(json)){ // Ext fields collection
			json = json.items;
		}
		if(!Ext.isArray(json)){ 
	...
	// Сохранение данных 
	save: function(storeName, json, options, clause) {
		if (clause.length>0) {
			var t = this;
			this.load(storeName, {success: function(tx, r) {
				if (r.rows.length>0) {
					t.update(storeName, json, options, clause);
				} else {
					t.add(storeName, json, options);
				}
			}, 
			failure:  function(tx, e) {
				//
...
Ext.db.webSqlDBDriver.getInstance = function(db, config){
	if(Ext.isWebSqlDB){ // webSqlDB
		if (!Ext.wsdb.WSDBDriver) {
			Ext.wsdb.WSDBDriver = new Ext.db.webSqlDBDriver(config);
		} 
		return Ext.wsdb.WSDBDriver;
	}
};