	...
	Ext.loadOptions = function(firstLoad) {
		controlStoreRequest = Ext.Ajax.request({
			...
			success: function(result){
				...
				if (Ext.globalOptions && Ext.globalOptions.globals && Ext.globalOptions.globals.openLLOFromEMIASData) {
					sw.openLLOFromEMIASData = Ext.globalOptions.globals.openLLOFromEMIASData;
				}
				...
				if (firstLoad) {
					...
					if (Ext.isRemoteDB) {
						loadName = 'remotedb|comboboxes';
						if (Ext.isIndexedDb) {
							loadName = loadName+'|indexeddb';
						}
						if (Ext.isWebSqlDB) {
							loadName = loadName+'|websqldb';
						}
						if (Ext.isGears) {
							loadName = loadName+'|gears';
						}
						...
						getWnd('gears|comboboxes').load({callback: function() {
							var localVerGears = new Ext.sql.Version();
						...
						getWnd('indexeddb|comboboxes').load({callback: function() {
							var IDBDriver = Ext.db.indexedDBDriver.getInstance();
							// Открываем без изменения версии 
						...
						getWnd('websqldb|comboboxes').load({callback: function() {
							var WSDBDriver = Ext.db.webSqlDBDriver.getInstance();
							// Открываем без изменения версии 
							WSDBDriver.open("Promed",'', {
								failure: function(e, change_version) {
