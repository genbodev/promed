 ...
 * @version 0.2.0
 */
Ext.define('Ext.ux.GridHeaderFilters',{
        ...
        for(var c=0; c < columns.length; c++)
        {
            ...
            if(column.filter)
            {
                var filterContainerConfig = {
                    ...
                    cls: column.filter.altCls || this.filterContainerCls,
                    layout: 'anchor',
    ...
    adjustFilterWidth: function()
    {
        ...
        for(var c=0; c < columns.length; c++)
        {
            ...
            }else if(this.containers[column.id] && !this.containers[column.id].isHidden()){
    ...
    parseFilters: function()
    {
        ...
        for(var fn in this.fields)
        {
            ...
            if(!field.isDisabled() && field.isValid()){
				//filterMap - подмена целевого поля фильтрации
				if(field.filterMap)
					filters[field.filterMap] = field.getRawValue();
				else
        ...
        if(this.grid.fireEvent('beforeheaderfiltersapply', this.grid, filters, this.grid.getStore()) !== false)
        {
            ...
            for(var fn in filters)
            {
                ...
                else
                {
                    ...
                    {
                        (function (fn, filters, plugin, field) {
                            var newSf = new Ext.util.Filter({
                                filterFn: function(item) {
                                    //для пустых групп

                                    if(plugin.idProperty && !item.get(plugin.idProperty)){
                                        return true;
                                    };

                                    if(item.get(fn)){
                                        if(field.completeMatch === true){
                                            return item.get(fn).toString().toLowerCase() == filters[fn].toLowerCase()
                                        }
										return item.get(fn).toString().toLowerCase().indexOf(filters[fn].toLowerCase()) != -1;
									}
									return false;
                                },
                                property: fn,
            ...
            if(change)
            {
                if(storeFilters.items.length == 0){
              ...
              //  this.grid.store.fireEvent('filterchange');
            }
        }
    },
    ...
    reloadStore: function()
    {
        ...
        if(this.grid.bigGridStore){
            this.grid.bigGridStore.filters = gs.filters;
            gs = this.grid.bigGridStore;
        }
