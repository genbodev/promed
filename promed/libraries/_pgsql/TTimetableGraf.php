...
class TTimetableGraf extends TTimetable {
	...
	function __construct( $arFields = NULL, $arReserved = NULL, $data = NULL ) {
		...
		if ( isset($arFields) ) {
			...
			$this->TimetableGraf_updDT = DateTime::createFromFormat('d.m.Y H:i:s', $arFields['TimetableGraf_updDT']);
			$this->TimetableGraf_begTime = DateTime::createFromFormat('d.m.Y H:i:s', $arFields['TimetableGraf_begTime']);
			...
			if ( isset($arFields['annotation']) && count($arFields['annotation']) ) {
				...
				foreach ($arFields['annotation'] as $annotation) {				
					$this->annotation[] = "<div class='ttcomments'>" . nl2br(htmlspecialchars($annotation['Annotation_Comment'])) . "<br/><i>" . $annotation['pmUser_Name'] . ", " . DateTime::createFromFormat('d.m.Y H:i', $annotation['Annotation_updDT']) . "</i></div>";
			...
			$this->curDT = new DateTime($data['curDT']);
	...
	function GetPersonTip() {
		...
		If ( $this->Direction['QpmUser_Name'] != '' ) { // записан из очереди
			$Tip .="<br/><b>Помещен в очередь:</b> " . Fmt($this->Direction['QpmUser_Name']) . "<br><b>Дата:</b> " . (gettype($this->Direction['EvnQueue_insDT']) == "object" ? $this->Direction['EvnQueue_insDT']->format("H:i d.m.y") : "");
		}
