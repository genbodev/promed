...
abstract class REST_Controller extends CI_Controller {
    ...
    public function __construct($config = 'rest')
    {
        ...
        if ($this->request->method != 'get') {
        	// параметры из строки запроса берём, даже если метод не get (refs #100788)
			$this->_parse_get();
		}
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        $GLOBALS['swClassName'] = get_class($this);
        $GLOBALS['swMethodName'] = $controller_method;
    ...
    /**
     * определяем что это метод для мобильного приложения с префиксом m___
     */
    public function isMobileMethod() {

        $controllerAction = $this->uri->segment(3);
        if (!empty($controllerAction) && !empty($controllerAction[0])) {
            if ($controllerAction[0] === 'm') return true;
        }
        return false;
    }
	 ...
	 * определяем
	 */
	public function getSwaggerSchema($key){
		$swagger_schemas = array(
			'id' => '@OA\Schema(type="integer", format="int64")',
			'int' => '@OA\Schema(type="integer", format="int64")',
			'string' => '@OA\Schema(type="string")',
			'date' => '@OA\Schema(type="string", format="date")',
			'datetime' => '@OA\Schema(type="string", format="date")',
			'bool' => '@OA\Schema(type="boolean")',
			'boolean' => '@OA\Schema(type="boolean")',
			'api_flag' => '@OA\Schema(type="boolean")'
		);

		return isset($swagger_schemas[$key]) ? $swagger_schemas[$key] : '@OA\Schema(type="string")';
	}

	/**
	 * определяем
	 */
	public function getSwaggerTemplate($key){

		$swagger_metadata = array(
			'main' => '
				@OA\%method_type(
					path="/%method_path",
					tags={"%ctrl_name"},
		...
		return isset($swagger_metadata[$key]) ? $swagger_metadata[$key] : '';
	}

	/**
	 * перелопатим входные параметры под свангер
	 */
	public function prepareInputParams($fields){

		$new_fields = array();

		foreach ($fields as $field) {
			$new_field['param_schema'] = $this->getSwaggerSchema($field['type']);
			$new_field['param_name'] = $field['field'];
			$new_field['param_description'] = $string = str_replace('"','\'',$field['label']);;

			if (isset($field['rules']) && (strpos($field['rules'], 'required') !== false) && empty($GLOBALS['isSwanApiKey'])) {
				$new_field['param_required'] = 'true';
			} else {
				$new_field['param_required'] = 'false';
			}
	...
	/**
	 * Генерация описания для сваггера
	 */
 	public function genSwaggerDescription($data){

		// определим описание метода
 		$output = array(
			'method_path' => $this->uri->uri_string,
 			'ctrl_name' => $this->uri->segment(2),
			'method_name' => $this->uri->segment(3)
		);

		if (!empty($this->_args['inputRule'])) {
			$rule = $this->_args['inputRule'];
		} else {
			$rule = $output['method_name'];
		}

		if (!isset($this->inputRules[$rule])) {
			echo 'Не удалось определить входные параметры, укажите правильный inputRule!';
			die();
		}

		// определим тип метода
		$method_info = explode('_',$data['fn']);
		if (!empty($method_info[1]) && in_array($method_info[1], $this->allowed_http_methods)) {
			$output['method_type'] = $method_info[1];
		} else {
			$output['method_type'] = 'undefined';
		}

		// подготовим входные параметры
		$input_fields = $this->prepareInputParams($this->inputRules[$rule]);

		// опишем входные параметры
		foreach ($input_fields as $field_params) {
			$output['input_params'][] = $this->assign($this->getSwaggerTemplate('input_params'), $field_params);
		}

		// преобразуем входные и выходные параметры в строку
		$output['input_params'] = implode(',', $output['input_params']);

		//echo '<pre>',print_r($data['responseData']),'</pre>'; die();

		// формируем выходные параметры
		$resp['response_content'] = $this->genSwaggerProperties($data['responseData']);
		$resp['response_content'] = implode(',', $resp['response_content']);

		$output['response'] = $this->assign($this->getSwaggerTemplate('response'), $resp);

		// формируем конечный результат
		$result = $this->assign($this->getSwaggerTemplate('main'), $output);
		echo $result;
	}
	 ...
	 */
	function showResultParamDescription($resp, $excluded = array())
	{
		$result_fields = array_keys($resp);
		$tables = array();
		foreach ($result_fields as $field) {
			$table = explode('_',$field)[0];
			if (!in_array($table,$excluded))	$tables[$table][] = $field;
		}

		if (!empty($tables)) {

			// кэш-аналоги таблиц тоже возьмем
			foreach ($tables as $tableName => $t_fields) {
				$tables[$tableName.'Cache'] = $t_fields;
			}

			$this->load->database();
			$this->load->model('Utils_model');

			$query_fields = $this->Utils_model->showResultParamDescriptions($tables);
	 ...
	 * осторожно, рекурсия!
	 */
	public function genSwaggerProperties($properties){

		$output = array();

		// не будем описывать сам нулевой ключ как параметр
		if (isset($properties[0])) {
			$properties = $properties[0];
		}

		$db_description = $this->showResultParamDescription($properties);

		foreach ($properties as $field => $value) {

			if (is_numeric($field) && $field > 0) {
				// выходим чтобы не перебирать весь массив
				break;
			}

			$param_data['prop_name'] = $field;

			if (strpos($field, '_id') !== false) {
				$param_data['prop_type'] = 'integer';
			} else if (strpos($field, '_Is') !== false) {
				$param_data['prop_type'] = 'boolean';
			} else {
				$param_data['prop_type'] = 'string';
			}

			if (isset($db_description[$field]) && !empty($db_description[$field]['descr'])) {
				$string = str_replace('"','\'',$db_description[$field]['descr']);
				$param_data['prop_desc'] = $db_description[$field]['tab_descr'].', '.$string;
			} else {
				$param_data['prop_desc'] = 'Описание';
			}

			if (is_array($value)) {

				$items['response_props'] = $this->genSwaggerProperties($value);
				$items['response_props'] = implode(',',$items['response_props']);

				$param_data['prop_type'] = 'array';
				$param_data['prop_items'] = $this->assign($this->getSwaggerTemplate('prop_items'), $items);
			} else {
				$param_data['prop_items'] = '';
			}

			$output[] = $this->assign($this->getSwaggerTemplate('response_props'), $param_data);
		}
    ...
    /**
     * реформатируем вывод Error_Msg = error_msg, Error_Code = error_code
     * а так же ищем их в дате
     */
    public function reformatMobileResponseParams($data) {

        // переназываем параметры
        $mod_args = array('error_msg', 'error_code');

        // если ошибка вложена в данные распакуем
		if (!empty($data['data']['Error_Msg']) && empty($data['data']['Error_Code'])) $data['data']['Error_Code'] = 6;
        if (!empty($data['data']['Error_Msg'])) $data = $data['data'];

        if (!empty($data)) {
            foreach ($data as $arg => $val) {
                if (in_array(strtolower($arg), $mod_args) && !in_array($arg, $mod_args)) {
                    $data[strtolower($arg)] = $val;
                    unset($data[$arg]);
                }
            }
        }

        // если дата вложена в дата

		if (
			isset($data['data']['data'])
			&& is_array($data['data']['data'])
		) {
			// если пришли пустые данные вложенные в параметр $data, убираем их
			if (count($data['data']['data']) == 0) {
				unset($data['data']['data']);
				if (isset($data['data']['totalCount'])) unset($data['data']['totalCount']);
			} else {
				// копируем дату в дате куда-то далеко
				$temp_data = $data['data']['data'];
				// выводим параметры из даты на уровень выше
				$data = array_merge($data, $data['data']);
				// возвращаем дату в нормальную дату
				$data['data'] = $temp_data;
			}
		}
	 ...
	 */
	function convertEmptyToNull(&$var) {
		if ($var === "") {
			$var = null;
		}
	}
    ...
    {
    	if (!empty($GLOBALS['isMobileApiKey']) && !empty($data) && is_array($data)) {
    		array_walk_recursive($data, array($this, 'convertEmptyToNull'));
		}
        ...
        elseif ($data !== NULL)
        {
            ...
            if (method_exists($this->format, 'to_' . $this->response->format))
            {
                ...
                // если метод с префиксом m___
                if ($this->isMobileMethod()) {

                    $data = $this->reformatMobileResponseParams($data);

					if (isset($this->_args['genswaggerdoc'])) {
						$this->genSwaggerDescription(array(
							'fn' => $this->getCalledFunction(3),
							'responseData' => $data
						));
						die();
					}
                    ...
                    $output = $this->format->factory($data)->{'to_' . $this->response->format}($data, JSON_UNESCAPED_UNICODE);

                    // An array must be parsed as a string, so as not to cause an array to string error
                    // Json is the most appropriate form for such a datatype
                    if ($this->response->format === 'array')
                    {
                        $output = $this->format->factory($output)->{'to_json'}($data, JSON_UNESCAPED_UNICODE);
                    }
            ...
            else
            {
                ...
                {
                    $data = $this->format->factory($data)->{'to_json'}(null, JSON_UNESCAPED_UNICODE);
                }
        ...
        if ($continue === FALSE)
        {
            ...
            // Вызываем post_system, чтобы записать в лог PHPLog данные о выполнении скрипта
