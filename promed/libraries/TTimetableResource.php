...
class TTimetableResource extends TTimetable {
	...
	function __construct( $arFields = NULL, $arReserved = NULL, $data = NULL ) {
		...
		if ( isset($arFields) ) {
			...
			$this->TimetableResource_updDT = $arFields['TimetableResource_updDT'];
			$this->TimetableResource_begTime = $arFields['TimetableResource_begTime'];
	...
	function PrintCell( $IsForRow = false ) {
		...
		$sText .= $this->TimetableResource_begTime->format('H:i');
		...
		if ( isset($this->Person) ) {
			...
			if ($this->isExt6 && !empty($this->EvnPrescr_id) && $this->Direction['EvnPrescr_id'] == $this->EvnPrescr_id) {
				...
				if (
					$this->TimetableResource_begTime->format("Y-m-d") >= date("Y-m-d") && (
			...
			if ( in_array($this->TimetableResource_id, $this->arReserved) ) {
				...
				if ( $this->canRecord() ) {
					...
					if ( $this->TimetableResource_begTime->format("Y-m-d H:i") < date("Y-m-d H:i") ) { //  прошедшее время
						If ( IsCZUser() || IsLpuRegAdmin($this->msData['Org_id']) ) {
							if (!$this->readOnly) {
								$sEvents .= " onclick=\"if (window.confirm('Вы записываете на прошедшее время, продолжить?')) {$this->ExtJSFunc}.getCmp('{$this->PanelID}').recordPerson({$this->TimetableResource_id}, '{$this->TimetableResource_begTime->format("d.m.Y")}', '{$this->TimetableResource_begTime->format("H:i")}');\"";
							}
						} else {
							$sClass .= " locked";
							if (!$this->readOnly) {
						...
						if (!$this->readOnly) {
							$sEvents .= " onclick=\"if (window.confirm('Вы записываете на платную бирку, продолжить?')) {$this->ExtJSFunc}.getCmp('{$this->PanelID}').recordPerson({$this->TimetableResource_id}, '{$this->TimetableResource_begTime->format("d.m.Y")}', '{$this->TimetableResource_begTime->format("H:i")}');\"";
						}
					} else {
						if (!$this->readOnly) {
							$sEvents .= " onclick=\"{$this->ExtJSFunc}.getCmp('{$this->PanelID}').recordPerson({$this->TimetableResource_id}, '{$this->TimetableResource_begTime->format("d.m.Y")}', '{$this->TimetableResource_begTime->format("H:i")}');\" href='#' ";
						}
					}
				} else {
					$sClass .= " locked";
	...
	function PrintCellForEdit() {
		...
		// Прошедшие дни
		if ( $this->TimetableResource_begTime->format("Y-m-d") < date("Y-m-d") ) {
			$sClass .= " old";			
		}
	...
	function PrintDayRowForPrint() {
		...
		if ( in_array($this->Person['PrivilegeType_id'], array(11, 20, 40, 50, 140, 150)) )
			$sText = "<img src=\"/images/star.gif\"> " . $this->TimetableResource_begTime->format('H:i');
		else
			$sText = $this->TimetableResource_begTime->format('H:i');
	...
	function GetPersonTip() {
		...
		$Tip = "";
		$Tip .=" onmouseover=\"this.style.backgroundColor = '#aaaaff';\" ext:qtip=\"<b>Занято:</b> {$sPerson}<br><b>Оператор:</b> " . Fmt($this->pmUser['PMUser_Name']) . "<br><b>Изменено:</b> " . $this->TimetableResource_updDT->format("H:i d.m.y");
