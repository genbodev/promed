 ...
 * Класс для работы с идентификацией людей (Бурятия)
 *
 * @package		Library
 * @access		public
...
class swPersonIdentBur {
	...
	public function doPersonIdentRequest($requestData = array()) {
		...
		*/
				...
				if (isset($xml->UlPatient) && isset($xml->UlPatient->PolicyList)) {
					...
					if (isset($PolicyList['UlPolicy'])) {
						...
						if (count($polises)>0) {
							...
							if (!empty($requestData['date'])) {		//Идентификация на дату
								$searchOnDate = date_format(date_create($requestData['date']), 'd.m.Y');
								$polisLast = array('IssueDate'=>'1900-01-01', 'Number'=>'0');
								foreach ($polises as $i=>$polis) {
									//Последний на указанную дату
									if (date_create($polis['IssueDate']) <= date_create($requestData['date']) &&
										date_create($polis['IssueDate']) > date_create($polisLast['IssueDate'])
									) {
										$resultPolis = $polisLast = $polis;
									}
								}
							} else {
								foreach ($polises as $i=>$polis) {
								...
								if (!$resultPolis) { // Если нет незакрытых полисов (т.е все закрытые)
									$polisLast = array('IssueDate'=>'1900-01-01', 'Number'=>'0');
									foreach ($polises as $i=>$polis) {
		...
		if (!$resultPolis && $searchOnDate) {
			$result['errorMsg'] = "Полис на дату {$searchOnDate} не найден";
			$result['errorCode'] = 1;
			$result['success'] = false;
		} else if (!$resultPolis) {
