...
class PzmSqlData extends SqlData {
	...
	static $from = array(
		...
		'pzm'=>'inner join v_MedService pzm with (nolock) on pzm.MedServiceType_id = 7 and msl.MedService_id = pzm.MedService_id and (pzm.MedService_endDT is null OR cast(pzm.MedService_endDT as date) > @cur_date)',
...
{
	static $from = array(
		'ttms'=>'v_TimetableMedService_lite ttms with (nolock)'
	);
	static $filters = array(
...
class UslugaComplex2011SqlData extends SqlData  {
	...
	static $select = array(
		...
		'UslugaComplex_id'=>'isnull(uc.UslugaComplex_id, uc11.UslugaComplex_id) as UslugaComplex_id',
		'UslugaComplex_Code'=>'isnull(uc.UslugaComplex_Code, uc11.UslugaComplex_Code) as UslugaComplex_Code',
		'UslugaComplex_Name'=>'coalesce(ucms.UslugaComplex_Name, uc.UslugaComplex_Name, uc11.UslugaComplex_Name) as UslugaComplex_Name',
		'UslugaComplex_AttributeList'=>'replace(substring(uca.UslugaComplexAttributeType_SysNick, 1, len(uca.UslugaComplexAttributeType_SysNick)-1), \' \', \'\') as UslugaComplex_AttributeList',
		'UslugaCategory_id'=>'uc.UslugaCategory_id',
		...
		'MedService_Name'=>'mss.MedService_Name',
		'MedService_Nick'=>'mss.MedService_Nick',
		'MedServiceType_id'=>'mst.MedServiceType_id',
		'MedServiceType_SysNick'=>'mst.MedServiceType_SysNick',
		//PROMEDWEB-9811
		//PROMEDWEB-9821
		//Иначе для нового интерфейса все превращается в тыкву
		'Lpu_id'=>'case when l.Lpu_id is null then mss.Lpu_id else l.Lpu_id end as Lpu_id',
		'Lpu_Nick'=>'l.Lpu_Nick',
		'LpuBuilding_id'=>'lu.LpuBuilding_id',
		'LpuBuilding_Name'=>'lu.LpuBuilding_Name',
		'LpuUnit_id'=>'lu.LpuUnit_id',
		'LpuUnit_Name'=>'lu.LpuUnit_Name',
		'LpuUnit_Address'=>'lua.Address_Address as LpuUnit_Address',
		'LpuUnitType_id'=>'lu.LpuUnitType_id',
		'LpuUnitType_SysNick'=>'lu.LpuUnitType_SysNick',
		'LpuSection_id'=>'ls.LpuSection_id',
		'LpuSection_Name'=>'ls.LpuSection_Name',
		'LpuSectionProfile_id'=>'ls.LpuSectionProfile_id',
		'LpuSectionProfile_Name'=>'ls.LpuSectionProfile_Name',
		...
		'ElectronicQueueInfo_id' => 'eq.ElectronicQueueInfo_id',
		'ElectronicService_id' => 'eq.ElectronicService_id',
	...
	static $from = array(
		'uc'=>'v_UslugaComplex uc with (nolock)',
		'ucms'=>'left join v_UslugaComplexMedService ucms with (nolock) on ucms.UslugaComplex_id = uc.UslugaComplex_id',
		'uc11'=>'outer apply ( Select top 1 * from v_UslugaComplex uc11 with (nolock) where uc.UslugaComplex_2011id = uc11.UslugaComplex_id ) uc11',
		'mss'=>'left join v_MedService mss with (nolock) on mss.MedService_id = ucms.MedService_id',
		'msl'=>'left join v_MedServiceLink msl (nolock) on msl.MedService_lid = mss.MedService_id and msl.MedServiceLinkType_id = 1',
		'pzm'=>'left join v_MedService pzm with (nolock) on pzm.MedServiceType_id = 7
		...
		'ucpzm'=>'left join v_UslugaComplexMedService ucpzm with (nolock) on ucpzm.MedService_id = pzm.MedService_id and ucpzm.UslugaComplex_id = uc.UslugaComplex_id',
		'ucres'=>'left join v_UslugaComplexResource ucres with (nolock) on ucres.UslugaComplexMedService_id = ucms.UslugaComplexMedService_id',
		'res'=>'left join v_Resource res with (nolock) on res.Resource_id = ucres.Resource_id and (res.Resource_endDT is null OR cast(res.Resource_endDT as date) > @cur_date)',
		'mst'=>'left join v_MedServiceType mst with (nolock) on mss.MedServiceType_id = mst.MedServiceType_id',
		'l'=>'left join v_Lpu l with (nolock) on mss.Lpu_id = l.Lpu_id',
		'lu'=>'left join v_LpuUnit lu with (nolock) on lu.LpuUnit_id = mss.LpuUnit_id',
		'lua'=>'left join v_Address lua with (nolock) on lua.Address_id = isnull(lu.Address_id,l.UAddress_id)',
		'ls'=>'left join v_LpuSection ls with (nolock) on mss.LpuSection_id = ls.LpuSection_id',
		'eq'=>'outer apply (
			select top 1
			...
			from
				...
				inner join v_MedServiceElectronicQueue mseq with(nolock) on mseq.UslugaComplexMedService_id = ucms.UslugaComplexMedService_id
	...
	static $filters = array(
		...
		'filterByBegDate'=>'uc.UslugaComplex_begDT <= @cur_date',
		'filterByEndDate'=>'isnull(uc.UslugaComplex_endDT, @cur_date) >= @cur_date',
	...
	where t1.UslugaComplex_id = isnull(uc11.UslugaComplex_id, uc.UslugaComplex_id)
		...
		'filterIsThisLPU'=>'not exists( select top 1 mslo.MedService_id from v_medservice mslo with(nolock) where mslo.MedService_id = mss.MedService_id and mslo.MedService_IsThisLPU=2 and mslo.lpu_id!=:Lpu_id)',
	);
}
...
class UslugaComplexSelectListSqlBuilder {
	...
	private static function setSqlDataSpecific() {
		if (self::$isLab) {
			...
			/*$whereHasAnalyzerTestTpl = "exists(
				...
				from
					lis.v_AnalyzerTest AnalyzerTest with (nolock)
			...
			$fromHasAnalyzerTestTpl = "
				cross apply (
					select top 1
						Analyzer.Analyzer_id,
						AnalyzerTest.AnalyzerTest_Name
					from
						lis.v_AnalyzerTest AnalyzerTest with (nolock)
						inner join lis.v_Analyzer Analyzer (nolock) on Analyzer.Analyzer_id = AnalyzerTest.Analyzer_id
					where
			...
			UslugaComplex2011SqlData::$MedServiceDisplayField = 'isnull(pzm_MedService_Nick,dd.MedService_Nick)';
					...
					case
						...
						when mss.MedService_id is null and exists(
							select top 1
								UslugaComplex_id
							from
								v_UslugaComplexComposition with (nolock)
							where
			...
			//PzmSqlData::$select['UslugaComplexMedService_id'] = 'ucpzm.UslugaComplexMedService_id';
		}
	...
	private static function setBaseFilters($data) {
		...
		if (2 == self::$_options['prescription']['service_name_show_type']) {
			...
			// вместо 'uc11'=>'left join v_UslugaComplex uc11 with (nolock) on uc.UslugaComplex_2011id = uc11.UslugaComplex_id',
			UslugaComplex2011SqlData::$from['uc11']='inner join v_UslugaComplex uc11 with (nolock) on uc.UslugaComplex_2011id = uc11.UslugaComplex_id';
			// Отображение наименований услуг из Справочник ГОСТ-2011
			// вместо isnull(uc.UslugaComplex_Code, uc11.UslugaComplex_Code) as UslugaComplex_Code
			UslugaComplex2011SqlData::$select['UslugaComplex_Code']='uc11.UslugaComplex_Code';
			// вместо coalesce(ucms.UslugaComplex_Name, uc.UslugaComplex_Name, uc11.UslugaComplex_Name) as UslugaComplex_Name
			UslugaComplex2011SqlData::$select['UslugaComplex_Name']='uc11.UslugaComplex_Name';
			//Фильтры по датам
			UslugaComplex2011SqlData::$filters['filterByBegDate']='uc11.UslugaComplex_begDT <= @cur_date';
			UslugaComplex2011SqlData::$filters['filterByEndDate']='isnull(uc11.UslugaComplex_endDT, @cur_date) >= @cur_date';
		}
		...
		if (!empty($data['filterByLpu_id']) || !empty($data['filterByLpu_str']) || !empty($data['filterByMedService_id'])) {
			...
			// вместо 'ucms'=>'left join v_UslugaComplexMedService ucms with (nolock) on ucms.UslugaComplex_id = uc.UslugaComplex_id',
			UslugaComplex2011SqlData::$from['ucms']='inner join v_UslugaComplexMedService ucms with (nolock) on ucms.UslugaComplex_id = uc.UslugaComplex_id';
			// вместо 'mss'=>'left join v_MedService mss with (nolock) on mss.MedService_id = ucms.MedService_id',
			UslugaComplex2011SqlData::$from['mss']='inner join v_MedService mss with (nolock) on mss.MedService_id = ucms.MedService_id';
		...
		if (in_array('func',$allowedUslugaComplexAttributeList)) {
			...
			UslugaComplex2011SqlData::$filters['filterMedServiceWithResources'] = "(exists(
					select top 1
						t2.Resource_id
					from
						v_UslugaComplexResource t1 with (nolock)
			...
			/*UslugaComplex2011SqlData::$filters['filterIsHasOnResource'] = 'exists (
				...
				from v_UslugaComplexResource ucres with (nolock)
		...
		UslugaComplex2011SqlData::$from['ucms'] .= ' AND cast(ucms.UslugaComplexMedService_begDT as date) <= @cur_date AND ISNULL(cast(ucms.UslugaComplexMedService_endDT as date), @cur_date) >= @cur_date';
		...
		switch (true) {
			...
			case (in_array('lab',$allowedUslugaComplexAttributeList)):
				self::$isLab = true;
				UslugaComplex2011SqlData::$from['mss'] .= ' AND (mss.MedServiceType_id = 6 or mss.MedServiceType_id = 71)';
				break;
		...
		if (!empty($data['filterByLpu_str'])) {
			...
			if (self::$isLab) {
				UslugaComplex2011SqlData::$filters['filterByLpu'] = "exists (
						...
						where (l.Lpu_id = mss.Lpu_id OR l.Lpu_id = pzm.Lpu_id) and l.Lpu_Nick like :filterByLpu_str
					)";
			} else {
				UslugaComplex2011SqlData::$from['mss'] .= ' AND exists (
					select top 1 l.Lpu_id
					from v_Lpu l with (nolock)
					where l.Lpu_id = mss.Lpu_id and l.Lpu_Nick like :filterByLpu_str
				)';
			}
		}

		if (self::$isLab) {
			UslugaComplex2011SqlData::$filters['filterIsThisLPU'] = 'not exists( select top 1 mslo.MedService_id from v_medservice mslo with(nolock) where mslo.MedService_id = ISNULL(pzm.MedService_id, mss.MedService_id) and mslo.MedService_IsThisLPU = 2 and mslo.lpu_id != :Lpu_id)';
		}
		...
		UslugaComplex2011SqlData::$from['mss'] .= ' AND cast(mss.MedService_begDT as date) <= @cur_date AND ISNULL(cast(mss.MedService_endDT as date), @cur_date) >= @cur_date';

		if (!empty($data['onlyByContract'])) {
			UslugaComplex2011SqlData::$filters['filterByContract'] = " exists(select top 1 LDC.LpuDispContract_id from v_LpuDispContract LDC (nolock) where mss.Lpu_id = ldc.Lpu_oid and ldc.Lpu_id = :Lpu_id and ls.LpuSectionProfile_id = LDC.LpuSectionProfile_id and ISNULL(LDC.LpuDispContract_setDate, @cur_date) <= @cur_date and ISNULL(LDC.LpuDispContract_disDate, @cur_date) >= @cur_date)";
		}
		...
		if(!empty($data['composition_type']) && $data['composition_type']=='composition_tests') {
			...
			UslugaComplex2011SqlData::$from['composition'] = "outer apply (
				...
				from v_UslugaComplexMedService ucmsoa with (nolock)
					cross apply(
						...
						where
							...
							and (at_child.AnalyzerTest_endDT >= @cur_date or at_child.AnalyzerTest_endDT is null)
							and (uc.UslugaComplex_endDT >= @cur_date or uc.UslugaComplex_endDT is null)
					) ATEST
				where ucmsoa.UslugaComplexMedService_pid = ucms.UslugaComplexMedService_id
			) composition";
		}
	...
	private static function setAdditionalFilters($data) {
		if (!empty($data['filterByUslugaComplex_str'])) {
			...
			UslugaComplex2011SqlData::$filters['filterByUslugaComplex'] = "(uc.UslugaComplex_Code + '. ' + uc.UslugaComplex_Name) like :filterByUslugaComplex_str";
			if (2 == self::$_options['prescription']['service_name_show_type']) {
				UslugaComplex2011SqlData::$filters['filterByUslugaComplex'] = "(uc11.UslugaComplex_Code + '. ' + uc11.UslugaComplex_Name) like :filterByUslugaComplex_str";
			}
		}
		...
		if (!empty($data['filterByAnalyzerTestName'])) {
			UslugaComplex2011SqlData::$filters['filterByAnalyzerTestName'] = "coalesce(att.AnalyzerTest_Name, uc.UslugaComplex_Name) like :filterByAnalyzerTestName";
	...
	static function getSql($data = array()) {
			...
			$orderbyfields = "
				...
				case when exists(
					...
					where
						...
						AND TimetableMedService_begTime < @upper_dt
				...
				if (self::$formMode == 'ExtJS6') {
					...
					$orderbyTimetable = "
						case when exists(
							...
							from
								v_TimetableResource_lite ttr (nolock)
								inner join UslugaComplexResource ucr (nolock) on ucr.Resource_id = ttr.Resource_id 
							where
		...
		if (self::$needTimetable) {
			if(self::$formMode == 'ExtJS6' && self::$isFunc === true){
				$tplParams['{select}'] .= "
					...
					,ttmsx.TimetableResource_begTime
		...
		$tplParams['{from}'] = "DD";
		/**
		 * UslugaComplexMedService_id - undefined
		 * MedService_id - 192
		 * is_pzm - undefined
		 * pzm_MedService_id - 6490
		 * lab_MedService_id - undefined
		 * UslugaComplex_id - undefined
		 */
	...
	static function getTimetableQueryFrom() {
		...
		';
			if(self::$formMode == 'ExtJS6'){
				...
				// @TODO Поправить запрос, чтобы не взрывался при открытии списка инструментальной диагностики в старом интерфейсе
				return strtr('
			outer apply
			(
				select top 1
				...
				from (
					...
					FROM
						...
						inner join v_UslugaComplexResource ucr with (nolock) on ucr.Resource_id = r.Resource_id
					where
						ttr.Person_id is null
						-- Пока закомментировал строку, чтобы убрать баг
						--and ((DD.Resource_id is not null and r.Resource_id = DD.Resource_id) or DD.Resource_id is null)
			...
			(
				...
				order by
					--сначало расписание услуги у ПЗ, потом ПЗ, потом расписание услуги у службы, затем расписание службы
					case when ttms2.is_pzm = 1 and ttms2.UslugaComplexMedService_id is not null then 0 else 1 end,
					ttms2.is_pzm desc,
					case when ttms2.UslugaComplexMedService_id is not null then 0 else 1 end
			) ttmsx
			', $all_ttms_params);
	...
	private static function getAddFields() {
		...
		$Unique_id = ",cast(ucdouble.UslugaComplex_id as varchar) + ISNULL('_' + cast(ucdouble.MedService_id as varchar), '') as Unique_id";
		...
		if (self::$isLab) {
			$addFields .= "
				...
				";
			$Group_id = ",cast(ucdouble.MedService_id as varchar) + ISNULL('_' + cast(ucdouble.pzm_MedService_id as varchar), '') as Group_id";
			$Unique_id = ",cast(ucdouble.UslugaComplex_id as varchar) + ISNULL('_' + cast(ucdouble.MedService_id as varchar), '') + ISNULL('_' + cast(ucdouble.pzm_MedService_id as varchar), '') as Unique_id";
		}
		if (self::$isFunc && self::$formMode == 'ExtJS6') {
			$addFields .= "
				...
				";
			$Group_id = ",cast(ucdouble.MedService_id as varchar) + ISNULL('_' + cast(ucdouble.Resource_id as varchar), '') as Group_id";
			$Unique_id = ",cast(ucdouble.UslugaComplex_id as varchar) + ISNULL('_' + cast(ucdouble.MedService_id as varchar), '') + ISNULL('_' + cast(ucdouble.Resource_id as varchar), '') as Unique_id";
			if(self::$needTimetable){
				$addFields .= "
