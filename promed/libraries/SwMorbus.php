...
class SwMorbus
{
	...
	public static function isAllow($morbus_type)
	{
		switch ($morbus_type) {
			...
			case 'ibs':
				...
				$isAllow = in_array($regionNick, array('buryatiya', 'perm', 'ufa'));
				break;
	...
	public static function onBeforeChangeDiag(EvnAbstract_model $evn)
	{
		...
		if (empty($evn->ignoreCheckMorbusOnko) && in_array($evn->evnClassId, array(11, 14, 32))) {
			$tmp = self::getStaticMorbusCommon()->getFirstResultFromQuery('
				select top 1 MorbusOnkoVizitPLDop_id as id from v_MorbusOnkoVizitPLDop with (nolock) where EvnVizit_id = :Evn_id
				union all
				select top 1 MorbusOnkoLeave_id as id from v_MorbusOnkoLeave with (nolock) where EvnSection_id = :Evn_id
			', array('Evn_id' => $evn->id));
			if ($tmp > 0) { 
	...
	public static function onAfterSaveEvn(EvnAbstract_model $evn)
	{
		...
		if (in_array($evn->evnClassId, array(11, 14, 32))) {
			...
			$diag_sop = self::getStaticMorbusCommon()->queryResult("
				select ed.Diag_id, ed.EvnDiag_id 
				from v_EvnDiag ed (nolock) 
				inner join v_Diag d (nolock) on ed.Diag_id = d.Diag_id
				where 
					ed.EvnDiag_pid = ? and 
			...
			foreach ( $arr as $row ) {
				...
				if (self::isAllow($MorbusType_SysNick) && self::isMorbus($MorbusType_SysNick)) {
					...
					if (isset($tmp['Morbus_id'])) {
						...
						if($MorbusType_SysNick == 'nephro' && in_array(self::getRegionNick(), array('buryatiya', 'perm', 'ufa'))){
					...
					if($MorbusType_SysNick == 'onko') {
						...
						$tmp = self::getStaticMorbusCommon()->getFirstRowFromQuery("
							select top 1 MorbusOnkoVizitPLDop_id, Diag_id
						...
						if(!empty($tmp)) {
							foreach($subsect_list as $subsect) {
								$mol_list = self::getStaticMorbusCommon()->queryList("select {$subsect}_id from {$subsect} (nolock) where MorbusOnkoVizitPLDop_id = :MorbusOnkoVizitPLDop_id", 
						...
						$tmp = self::getStaticMorbusCommon()->getFirstRowFromQuery("
							select top 1 MorbusOnkoLeave_id, Diag_id
						...
						if(!empty($tmp)) {
							foreach($subsect_list as $subsect) {
								$mol_list = self::getStaticMorbusCommon()->queryList("select {$subsect}_id from {$subsect} (nolock) where MorbusOnkoLeave_id = :MorbusOnkoLeave_id", 
							...
							if (self::getRegionNick() == 'perm') {
								self::getStaticMorbusCommon()->db->query("update dbo.RegistryDataEvnPS with (rowlock) set MorbusOnkoLeave_id = null where MorbusOnkoLeave_id = :MorbusOnkoLeave_id",
									array('MorbusOnkoLeave_id' => $tmp['MorbusOnkoLeave_id'])
								);
							}
							// И удаляем саму специфику
	...
	public static function removeEvnUslugaOnko($data) {
		$usluga_list = self::getStaticMorbusCommon()->queryResult("
			select EvnUsluga_id, EvnClass_SysNick
		...
		foreach($usluga_list as $row) {
			$query = "
				...
				exec p_" . $row['EvnClass_SysNick'] . "_del
			...
			if (isset($evn['listMorbus']['onko'])) {
				...
				foreach ($evn['listMorbus']['onko'] as &$onko) {
					$onko['MorbusOnkoVizitPLDop_id'] = self::getStaticMorbusCommon()->getFirstResultFromQuery('
						select MorbusOnkoVizitPLDop_id from MorbusOnkoVizitPLDop (nolock) where EvnVizit_id = ? and Diag_id = ?
					', array($evn[$idKey], $onko['Diag_id']));
					$onko['MorbusOnkoLeave_id'] = self::getStaticMorbusCommon()->getFirstResultFromQuery('
						select MorbusOnkoLeave_id from MorbusOnkoLeave (nolock) where EvnSection_id = ? and Diag_id = ?
					', array($evn[$idKey], $onko['Diag_id']));
				}
			}
	...
	public static function checkByEvn($morbus_type, $data, $mode)
	{
		...
		$tmp = self::getStaticMorbusCommon()->checkExistsExtended($morbus_type, $response['Person_id'], $response['Diag_id'], $add_select = "
			...
			,Morbus.Evn_pid
			,Morbus.Diag_id
			,convert(varchar(10), Morbus.Morbus_setDT, 120) as Morbus_setDT
			,v_{$specificTable}.{$specificTable}_id
			,v_EvnNotifyBase.EvnNotifyBase_id
			,v_PersonRegister.PersonRegister_id
		", $add_join = "
			inner join v_{$specificTable} with (nolock) on v_{$specificTable}.Morbus_id = Morbus.Morbus_id
			left join v_EvnNotifyBase with (nolock) on v_EvnNotifyBase.Morbus_id = Morbus.Morbus_id
			left join v_PersonRegister with (nolock) on v_PersonRegister.Morbus_id = Morbus.Morbus_id
		", false);
		...
		if ( empty($tmp) ) {
			...
			if (in_array($mode, array('onBeforeViewData', 'onBeforeSaveEvnNotify', 'onBeforeSaveEvnNotifyFromJrn')) 
				|| ($mode == 'onAfterSaveEvn' && $morbus_type == 'nephro' && in_array(self::getRegionNick(), array('buryatiya', 'perm', 'ufa'))))
			{
	...
	public static function checkByPersonRegister($morbus_type, $data, $mode)
	{
		...
		$tmp = self::getStaticMorbusCommon()->checkExistsExtended($morbus_type, $response['Person_id'], $response['Diag_id'], $add_select = "
			...
			,convert(varchar(10), Morbus.Morbus_setDT, 120) as Morbus_setDT
		", $add_join = "
			left join v_{$specificTable} with (nolock) on v_{$specificTable}.Morbus_id = Morbus.Morbus_id
		", true);
		if ( false == is_array($tmp) ) {
			throw new Exception('Неправильный ответ модели', 500);
		}
		...
		return "case 
					...
					AND (
						{$morbus_base_alias}.Person_id = :{$pid_parameter} or exists(
							select top 1 EvnEdit.Evn_id from v_Evn EvnEdit with (nolock)
							inner join v_Evn EvnRoot with (nolock) on EvnRoot.Evn_id = EvnEdit.Evn_rid
								AND ISNULL(EvnRoot.Evn_IsSigned,1) = 1 /* EvnEdit.Evn_rid не подписан */
							where EvnEdit.Evn_id = :{$pid_parameter}
								AND ISNULL(EvnEdit.Evn_IsSigned,1) = 1 /* {$pid_parameter} не подписан */
								{$specific_cause}
						)
			...
			if ($PersonRegister_id > 0) {
				...
				$query = '
					select
						...
						,Person_id
						,Diag_id
						,substring(convert(varchar, PersonRegister_setDate, 120),1,10) as Morbus_setDT
						,null as Evn_pid
				...
				$query = '
					select
						...
						,substring(convert(varchar, Evn.Evn_setDT, 120),1,10) as Morbus_setDT
					...
					from
						...
						left join v_Morbus M with(nolock) on M.Morbus_id = Evn.Morbus_id and M.MorbusType_id = :MorbusType_id
						left join EvnVizitPL PL with (nolock) on PL.EvnVizitPL_id = Evn.Evn_id
						left join v_EvnPLDispScreenOnko EPLDSO with (nolock) on EPLDSO.EvnPLDispScreenOnko_pid = Evn.Evn_id
						left join v_EvnVizitDispDop DD with (nolock) on DD.EvnVizitDispDop_id = Evn.Evn_id
						left join v_EvnPLDispDop13 EPLDD13 with (nolock) on EPLDD13.EvnPLDispDop13_id = Evn.Evn_pid
						left join EvnSection ES with (nolock) on ES.EvnSection_id = Evn.Evn_id
						left join EvnPS EPS with (nolock) on EPS.EvnPS_id = Evn.Evn_id
			...
			if (!empty($tmp) && $tmp['EvnClass_SysNick'] == 'EvnDiagPLStom') {
				...
				$query = '
					select
						M.Morbus_id
						,Evn.Person_id
						,isnull(EDPLS.Diag_id,EDPS.Diag_id) as Diag_id
						,M.Diag_id as MorbusDiag_id
						,substring(convert(varchar, Evn.Evn_setDT, 120),1,10) as Morbus_setDT
						,Evn.Evn_pid
						,EvnClass.EvnClass_SysNick
					from
						v_Evn Evn with (nolock)
						inner join v_Evn EvnPid with (nolock) on EvnPid.Evn_id = Evn.Evn_pid
						inner join EvnClass with (nolock) on EvnClass.EvnClass_id = EvnPid.EvnClass_id
						left join v_Morbus M with(nolock) on M.Morbus_id = Evn.Morbus_id and M.MorbusType_id = :MorbusType_id
						left join v_EvnDiagPLStom EDPS with (nolock) on EDPS.EvnDiagPLStom_id = Evn.Evn_id
