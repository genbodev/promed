<?php
include_once('FrameOld.php');
include_once('MessageOld.php');
include_once('ExceptionInterfaceOld.php');
include_once('Exception/StompExceptionOld.php');
include_once('MessageOld/MapOld.php');
include_once('MessageOld/BytesOld.php');
    ...
    */
    ...
    protected function _init ()
    {
        ...
        if (preg_match($pattern, $this->_brokerUri, $regs)) {
            ...
            throw new StompExceptionOld("Bad Broker URL {$this->_brokerUri}");
        }
    }
    /**
     * Process broker URL
    ...
    protected function _processUrl ($url)
    {
        ...
        if ($parsed) {
            ...
            throw new StompExceptionOld("Bad Broker URL $url");
        }
    }
    /**
    ...
    protected function _makeConnection ()
    {
        if (count($this->_hosts) == 0) {
            throw new StompExceptionOld("No broker defined");
        }
        ...
        while (! $connected && $att ++ < $this->_attempts) {
            ...
            if (!is_resource($this->_socket) && $att >= $this->_attempts && !array_key_exists($i + 1, $this->_hosts)) {
                throw new StompExceptionOld("Could not connect to $host:$port ($att/{$this->_attempts})");
        ...
        if (! $connected) {
            throw new StompExceptionOld("Could not connect to a broker");
        }
    }
	
	public function connectAndSendOld($destination, $message, $object)
	{
		$this->connect();
		$this->send($destination, json_encode($message), array('object'=>$object));
		$this->disconnect();
	}
    ...
    public function connect ($username = '', $password = '')
    {
		...
		$frame = new FrameOld("CONNECT", $headers);
        $this->_writeFrame($frame);
        $frame = $this->readFrame();

        if ($frame instanceof FrameOld && $frame->command == 'CONNECTED') {
            ...
            if ($frame instanceof FrameOld) {
                throw new StompExceptionOld("Unexpected command: {$frame->command}", 0, $frame->body);
            } else {
                throw new StompExceptionOld("Connection not acknowledged");
            }
        }
    }
    ...
    public function send ($destination, $msg, $properties = array(), $sync = null)
    {
        if ($msg instanceof FrameOld) {
            ...
            $frame = new FrameOld('SEND', $headers, $msg);
        }
     ...
     * @param boolean $sync
     */
    protected function _prepareReceipt (FrameOld $frame, $sync)
    {
        $receive = $this->sync;
        if ($sync !== null) {
            $receive = $sync;
     ...
     * @throws StompExceptionOld
     */
    protected function _waitForReceipt (FrameOld $frame, $sync)
    {
        ...
        if ($receive == true) {
            ...
            while(true) {
                ...
                if ($frame instanceof FrameOld && $frame->command == 'RECEIPT') {
                    if ($frame->headers['receipt-id'] == $id) {
                        $this->_waitbuf = $buf;
                        return true;
                    } else {
                        throw new StompExceptionOld("Unexpected receipt id {$frame->headers['receipt-id']}", 0, $frame->body);
                    }
                } else {
                    $buf[] = $frame;
                }
    ...
    public function subscribe ($destination, $properties = null, $sync = null)
    {
        ...
        $headers['destination'] = $destination;
        $frame = new FrameOld('SUBSCRIBE', $headers);
        $this->_prepareReceipt($frame, $sync);
        $this->_writeFrame($frame);
    ...
    public function unsubscribe ($destination, $properties = null, $sync = null)
    {
        ...
        $headers['destination'] = $destination;
        $frame = new FrameOld('UNSUBSCRIBE', $headers);
        $this->_prepareReceipt($frame, $sync);
        $this->_writeFrame($frame);
        if ($this->_waitForReceipt($frame, $sync) == true) {
    ...
    public function begin ($transactionId = null, $sync = null)
    {
        ...
        $frame = new FrameOld('BEGIN', $headers);
        $this->_prepareReceipt($frame, $sync);
        $this->_writeFrame($frame);
        return $this->_waitForReceipt($frame, $sync);
    }
    ...
    public function commit ($transactionId = null, $sync = null)
    {
        ...
        $frame = new FrameOld('COMMIT', $headers);
        $this->_prepareReceipt($frame, $sync);
        $this->_writeFrame($frame);
        return $this->_waitForReceipt($frame, $sync);
    }
    ...
    public function abort ($transactionId = null, $sync = null)
    {
        ...
        $frame = new FrameOld('ABORT', $headers);
        $this->_prepareReceipt($frame, $sync);
        $this->_writeFrame($frame);
        return $this->_waitForReceipt($frame, $sync);
    }
    ...
    {
        if ($message instanceof FrameOld) {
            ...
            $frame = new FrameOld('ACK', $headers);
            $this->_writeFrame($frame);
            return true;
        } else {
            $headers = array();
    ...
    public function disconnect ()
    {
        ...
        if (is_resource($this->_socket)) {
            $this->_writeFrame(new FrameOld('DISCONNECT', $headers));
            fclose($this->_socket);
        }
        $this->_socket = null;
     ...
     */
    protected function _writeFrame (FrameOld $stompFrame)
    {
        if (!is_resource($this->_socket)) {
            throw new StompExceptionOld('Socket connection hasn\'t been established');
        }
    ...
    public function readFrame ()
    {
        ...
        do {
			...
			if (strpos($read, "HTTP/1.1 400 No URI") !== false) {
				throw new StompExceptionOld('HTTP/1.1 400 No URI');
			}
            if ($read === false || $read === "") {
                $this->_reconnect();
    ...
    public function hasFrameToRead()
    {
        ...
        if ($has_frame_to_read === false) {
            throw new StompExceptionOld('Check failed to determine if the socket is readable');
