...
class SwMongodb {
	...
	*/
	...
	public function list_collections($db = "")
	{
		...
		else
		{
			try
			{
				$collections = $this->connection->{$db}->listCollections();
				return $collections;
			}
			catch (Exception $e)
			{
	...
	*/
	 ...
	 public function get($collection = "", $keys = array()) // 
	 {
		if (empty($collection))
	 	...
	 	$documents = $this->db->{$collection}->find($this->wheres, $this->selects)->timeout($this->timeout)->limit((int) $this->limit)->skip((int) $this->offset)->sort($this->sorts);
		...
		if ($documents->count()>0) {
			while (@$documents->hasNext())
			{
				...
				{
					if (count($keys)>0) {
						$returns[] = $this->array_keys_uncase((array) $documents->getNext(), $keys);
					} else {
	...
	*/
	...
	public function count($collection = "") {
		...
		$count = $this->db->{$collection}->find($this->wheres)->timeout($this->timeout)->limit((int) $this->limit)->skip((int) $this->offset)->count();
		$this->_clear();
		return ($count);
	}
	...
	*/
	...
	public function insert($collection = "", $insert = array())
	{
		...
		try
		{
			$this->db->{$collection}->insert($insert, array($this->config['mongo_query_safety'] => TRUE));
			if (isset($insert['_id']))
			{
				return ($insert['_id']);
			}
    ...
    */
    public function batch_insert($collection = "", $insert = array())
    {
            ...
            try
            {
                    $this->db->{$collection}->batchInsert($insert, array($this->config['mongo_query_safety'] => TRUE));
                    if (isset($insert['_id']))
                    {
                            return ($insert['_id']);
                    }
	...
	*/
	...
	public function delete($collection = "")
	{
		...
		try
		{
			$this->db->{$collection}->remove($this->wheres, array($this->config['mongo_query_safety'] => TRUE, 'justOne' => TRUE));
			$this->_clear();
			return (TRUE);
		}
		catch (MongoCursorException $e)
	...
	*/
	 ...
	 public function delete_all($collection = "")
	 {
		...
		try
		{
			$this->db->{$collection}->remove($this->wheres, array($this->config['mongo_query_safety'] => TRUE, 'justOne' => FALSE));
			$this->_clear();
			return (TRUE);
		}
		catch (MongoCursorException $e)
	...
	*/
	...
	private function connect()
	{
		...
		try
		{
			$this->connection = $this->getMongoClient($this->connection_string, $options);
			$this->db = $this->connection->{$this->config['mongo_db']};
			return ($this);	
		} 
	 ...
	 */
	private function getMongoClient($seeds = "", $options = array(), $retry = 3) {
		try {
