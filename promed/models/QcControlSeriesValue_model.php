...
class QcControlSeriesValue_model extends Scenario_model {
	...
	static function defAttributes() {
		return array(
			...
			'qccontrolseries_id' => array(
				...
				'save' => '',
				'type' => 'id',
				'join' => 'left join lis.v_QcControlSeries QCS with(nolock) on QCS.QcControlSeries_id = v_QcControlSeriesValue.QcControlSeries_id'
			),
			'setdt' => array(
				'properties' => array(
					self::PROPERTY_IS_SP_PARAM,
			...
			'setdate' => array(
				...
				'save' => 'trim',
				'type' => 'date',
				'select' => 'CONVERT( varchar, CAST( QcControlSeriesValue_setDT as datetime ), 120 ) as setDate'
			),
			'settime' => array(
				'properties' => array(
					self::PROPERTY_NOT_SAFE
				...
				'save' => 'trim',
				'type' => 'time',
				'select' => 'convert(char(5), QcControlSeriesValue_setDT, 108) as setTime'
			),
			'qcrule_id' => array(
				'properties' => array(
					self::PROPERTY_IS_SP_PARAM
	...
	function doLoadData($data = array()) {
		...
		if(isset($data['isCull'])) {
			...
			$where .= ' and isnull(QCSV.QcControlSeriesValue_isDisabled, 1) = :QcControlSeriesValue_isDisabled';
		}
		...
		if(!empty($data['QcControlSeriesValue_IsControlPassed'])) {
			...
			$where .= ' and isnull(QCSV.QcControlSeriesValue_IsControlPassed, 1) = :QcControlSeriesValue_IsControlPassed';
		}

		$query = "
			SELECT *,
				ROW_NUMBER() OVER(ORDER BY db.QcControlSeriesValue_setDT ASC, db.QcControlSeriesValue_id ASC) as num
			FROM
				(SELECT TOP 30
					QCSV.QcControlSeriesValue_id,
					QCSV.QcControlSeries_id,
					CONVERT( varchar, CAST( QCSV.QcControlSeriesValue_setDT as datetime ), 104 ) as setDate,
					QCSV.QcControlSeriesValue_setDT,
					convert(char(5), QCSV.QcControlSeriesValue_setDT, 108) as setTime,
					QCSV.QcControlSeriesValue_IsControlPassed,
					--isnullQCSV.QcControlSeriesValue_IsControlPassed, 1) as QcControlSeriesValue_IsControlPassed,
					QCSV.QcRule_id,
					QR.QcRule_Name,
				...
				LEFT JOIN lis.v_QcControlSeries QCSpid WITH(NOLOCK) on QCSpid.QcControlSeries_id = QCS.QcControlSeries_id
				INNER JOIN lis.v_QcControlSeriesValue QCSV WITH(NOLOCK) on QCSV.QcControlSeries_id = QCS.QcControlSeries_id
				LEFT JOIN lis.v_QcRule QR WITH(NOLOCK) on QR.QcRule_id = QCSV.QcRule_id
				LEFT JOIN lis.v_QcControlStage QCStage WITH(NOLOCK) on QCStage.QcControlStage_id = QCS.QcControlStage_id
				LEFT JOIN v_UslugaTest UT WITH(NOLOCK) on UT.UslugaTest_id = QCSV.UslugaTest_id
				WHERE (1=1) {$where}
	...
	function loadDataForCalculate($data) {
		...
		$where = ' and isnull(QCSV.QcControlSeriesValue_isDisabled,1) = 1';
		...
		if(!empty($data['_QcControlStage_id'])) {
			...
			$where .= ' and isnull(QCS.QcControlStage_id,0) != :_QcControlStage_id';
		}
		...
		$query = "
			...
			FROM
				(SELECT {$count}
					...
					convert( char(10), CAST( QCSV.QcControlSeriesValue_setDT as date ) ) as setDate,
					convert(char(5), QCSV.QcControlSeriesValue_setDT, 108) as setTime,
	...
	function getLastDate() {
		...
		$query = "
			select top 1
				cSeriesValue.QcControlSeriesValue_setDT
			from lis.v_QcControlSeries parentSeries with(nolock)
			left join lis.v_QcControlSeries childSeries with(nolock) on childSeries.QcControlSeries_pid = parentSeries.QcControlSeries_id
			left join lis.v_QcControlSeriesValue cSeriesValue with(nolock) on cSeriesValue.QcControlSeries_id = childSeries.QcControlSeries_id
			where isnull(cSeriesValue.QcControlSeriesValue_isDisabled,1) = 1
	...
	function _beforeValidate() {
		...
		switch($scenario) {
			...
			case self::SCENARIO_DO_SAVE_GRID:
				...
				$params = [
					'EvnLabSample_setDT' => $date ? $date->format('Y-m-d H:i:s') : null,
					'Lpu_id' => $this->Lpu_id,
