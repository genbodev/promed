...
class Treatment_model extends swModel {
	...
	*/
	function saveTreatment($data) {
		...
		$query = "
			...
			exec p_Treatment_" . $procedure_action . "
				@Server_id = :Server_id,
				@Treatment_id = @Res {$out},
	...
	function setStatusTreatment($data){
		$query = "
			declare
				...
				@ErrCode bigint,
				@ErrMsg varchar(4000);
			set @Res = :Treatment_id;

			exec p_Treatment_setStatus
				@Treatment_id = @Res,
				@TreatmentReview_id = :TreatmentReview_id,
				@Treatment_DateReview = @date,
				@pmUser_id = :pmUser_id,
				@Error_Code = @ErrCode output,
				@Error_Message = @ErrMsg output;
	...
	function saveTreatmentFeedback($data){
		...
		$query = "
			...
			exec p_TreatmentFeedback_ins
				@TreatmentFeedback_id = @Res output,
	...
	function getTreatmentList($data) {
		...
		$query = "
			select
				...
				CONVERT(varchar(10), t.Treatment_DateReg, 104) as Treatment_DateReg,
			...
			from
				-- from
				v_Treatment t with(nolock)
				left join v_TreatmentType tt with(nolock) on tt.TreatmentType_id = t.TreatmentType_id
				left join v_TreatmentSenderType tst with(nolock) on tst.TreatmentSenderType_id = t.TreatmentSenderType_id
				left join v_TreatmentRecipientType trt with(nolock) on trt.TreatmentRecipientType_id = t.TreatmentRecipientType_id
				left join pmUserCache u with(nolock) on u.pmUser_id = t.pmUser_insID
				-- end from
			where
				-- where
				(1=1) ".$where."
	...
	function getTreatment($data) {
		$query = "
			SELECT TOP 1
				t.Treatment_id,
				t.Treatment_Reg,
				convert(varchar,t.Treatment_DateReg,104) as Treatment_DateReg,
				t.TreatmentUrgency_id,
				t.TreatmentMultiplicity_id,
				t.TreatmentSenderType_id,
				RTRIM(t.Treatment_SenderDetails) as Treatment_SenderDetails,
				t.TreatmentType_id,
				t.TreatmentCat_id,
				t.TreatmentRecipientType_id,
				t.Lpu_rid,
				t.TreatmentSubjectType_id,
				t.Org_sid,
				t.MedPersonal_Lpu_sid,
				t.MedPersonal_sid,
				t.Lpu_sid,
				--as SubjectName
				RTRIM(t.Treatment_Text) as Treatment_Text,
				RTRIM(t.Treatment_Document) as Treatment_Document,
				t.TreatmentMethodDispatch_id,
				RTRIM(t.Treatment_Comment) as Treatment_Comment,
				t.TreatmentReview_id,
				t.Treatment_SenderPhone,
				tf.TreatmentFeedback_Message,
				tf.TreatmentFeedback_Note,
				tf.TreatmentFeedback_Document,
				...
				convert(varchar,t.Treatment_DateReview,104) as Treatment_DateReview
			FROM v_Treatment t with(nolock)
			left join v_TreatmentFeedback tf (nolock) on tf.Treatment_id = t.Treatment_id
			left join v_PersonState ps (nolock) on ps.Person_id = t.Person_id
	...
	*/
	function printTreatment($id) {
		$query = "
			...
			FROM 
				v_Treatment t with(nolock)
				left join v_TreatmentUrgency tu with(nolock) on tu.TreatmentUrgency_id = t.TreatmentUrgency_id
				left join v_TreatmentMultiplicity tm with(nolock) on tm.TreatmentMultiplicity_id = t.TreatmentMultiplicity_id
				left join v_TreatmentSenderType tst with(nolock) on tst.TreatmentSenderType_id = t.TreatmentSenderType_id
				left join v_TreatmentType tt with(nolock) on tt.TreatmentType_id = t.TreatmentType_id
				left join v_TreatmentCat tc with(nolock) on tc.TreatmentCat_id = t.TreatmentCat_id
				left join v_TreatmentRecipientType trt with(nolock) on trt.TreatmentRecipientType_id = t.TreatmentRecipientType_id
				left join v_Lpu lr with(nolock) on lr.Lpu_id = t.Lpu_rid
				left join v_TreatmentSubjectType tsubt with(nolock) on tsubt.TreatmentSubjectType_id = t.TreatmentSubjectType_id
				left join v_Org o with(nolock) on o.Org_id = t.Org_sid
				left join v_Lpu lm with(nolock) on lm.Lpu_id = t.MedPersonal_Lpu_sid
				left join v_MedPersonal m with(nolock) on m.MedPersonal_id = t.MedPersonal_sid
				left join v_Lpu ls with(nolock) on ls.Lpu_id = t.Lpu_sid
				left join v_TreatmentMethodDispatch tmd with(nolock) on tmd.TreatmentMethodDispatch_id = t.TreatmentMethodDispatch_id
				left join v_TreatmentReview tr with(nolock) on tr.TreatmentReview_id = t.TreatmentReview_id
			WHERE
				t.Treatment_id = :id
		";
	...
	function getTreatmentSearchList($data) {
		...
		$where = "";
			...
			if (isset($data['Treatment_Reg']) && strlen($data['Treatment_Reg']) > 0)
				$where .= " AND t.Treatment_Reg LIKE '%".$data['Treatment_Reg']."%'";
			...
			if (isset($data['Treatment_SenderDetails']) && strlen($data['Treatment_SenderDetails']) > 0)
				$where .= " AND t.Treatment_SenderDetails LIKE '%".$data['Treatment_SenderDetails']."%'";
			...
			$query = "
				select
					...
					t.Treatment_SenderDetails as Treatment_SenderDetails,
					trt.TreatmentRecipientType_Name as TreatmentRecipientType,
					CONVERT(varchar(10), t.Treatment_DateReview, 104) as Treatment_DateReview,
					tr.TreatmentReview_Name as TreatmentReview,
					tr.TreatmentReview_id
					-- end select
				from
					-- from
					v_Treatment t with(nolock)
	...
	*/
	function getTreatmentReport($data) {
		...
		if (isset($data['TreatmentMethodDispatch_id']))
		{
			...
			$left_join[]='left join v_TreatmentMethodDispatch tmd with(nolock) on tmd.TreatmentMethodDispatch_id = ' . $data['TreatmentMethodDispatch_id'];
		...
		if (isset($data['TreatmentMultiplicity_id']))
		{
			...
			$left_join[]='left join v_TreatmentMultiplicity tm with(nolock) on tm.TreatmentMultiplicity_id = ' . $data['TreatmentMultiplicity_id'];
		...
		if (isset($data['TreatmentType_id']))
		{
			...
			$left_join[]='left join v_TreatmentType tt with(nolock) on tt.TreatmentType_id = ' . $data['TreatmentType_id'];
		...
		if (isset($data['TreatmentCat_id']))
		{
			...
			$left_join[]='left join v_TreatmentCat tc with(nolock) on tc.TreatmentCat_id = ' . $data['TreatmentCat_id'];
		...
		if (isset($data['TreatmentRecipientType_id']))
		{
			...
			$left_join[]='left join v_TreatmentRecipientType trt with(nolock) on trt.TreatmentRecipientType_id = ' . $data['TreatmentRecipientType_id'];
		...
		if (isset($data['TreatmentReview_id']))
		{
			...
			$left_join[]='left join v_TreatmentReview tr with(nolock) on tr.TreatmentReview_id = ' . $data['TreatmentReview_id'];
		...
		switch ($data['node']) {
			case 'TRW_number':
				$query = "
					SELECT
						COUNT( * ) as all_item,
						(SELECT COUNT( * ) FROM v_Treatment with(nolock) WHERE TreatmentType_id = 1 ".$where.") as number_1,
						(SELECT COUNT( * ) FROM v_Treatment with(nolock) WHERE TreatmentType_id = 2 ".$where.") as number_2,
						(SELECT COUNT( * ) FROM v_Treatment with(nolock) WHERE TreatmentType_id = 3 ".$where.") as number_3,
						(SELECT COUNT( * ) FROM v_Treatment with(nolock) WHERE TreatmentType_id = 4 ".$where.") as number_4     
			...
			case 'TRW_cat':
				$query = "
					SELECT
						...
						(SELECT COUNT( * ) FROM v_Treatment t with(nolock) WHERE t.TreatmentCat_id = tc.TreatmentCat_id ".$where." ) as Number
					FROM v_TreatmentCat tc with(nolock)
				";
				$response = array();
				...
				for ($i=1; $i < 5; $i++)
				{
					$query = "
						SELECT
							tc.TreatmentCat_Name as Name,
							(SELECT COUNT( * ) FROM v_Treatment t with(nolock) WHERE t.TreatmentType_id = ". $i ." AND TreatmentCat_id = tc.TreatmentCat_id ".$where." ) as Number
						FROM v_TreatmentCat tc with(nolock)
					";
					$res = $this->db->query($query, array());
					if ( is_object($res) )
			...
			case 'TRW_sender':
				$query = "
					SELECT
						COUNT( * ) as all_item,
						(SELECT COUNT( * ) FROM v_Treatment with(nolock) WHERE TreatmentSenderType_id = 1 ".$where.") as patientes,
						(SELECT COUNT( * ) FROM v_Treatment with(nolock) WHERE TreatmentSenderType_id = 2 ".$where.") as org,
						(SELECT COUNT( * ) FROM v_Treatment with(nolock) WHERE TreatmentSenderType_id = 3 ".$where.") as com,
						(SELECT COUNT( * ) FROM v_Treatment with(nolock) WHERE TreatmentSenderType_id = 4 ".$where.") as glav_vrach,
						(SELECT COUNT( * ) FROM v_Treatment with(nolock) WHERE TreatmentSenderType_id = 5 ".$where.") as zav_otd,
						(SELECT COUNT( * ) FROM v_Treatment with(nolock) WHERE TreatmentSenderType_id = 6 ".$where.") as vrach,
						(SELECT COUNT( * ) FROM v_Treatment with(nolock) WHERE TreatmentSenderType_id = 7 ".$where.") as sister,
						(SELECT COUNT( * ) FROM v_Treatment with(nolock) WHERE TreatmentSenderType_id = 8 ".$where.") as other
					FROM v_Treatment with(nolock)
					WHERE (1=1) ".$where."
				";
				//echo $query;
			...
			case 'TRW_review':
				$query = "
					SELECT
						COUNT( * ) as all_item,
						(SELECT COUNT( * ) FROM v_Treatment with(nolock) WHERE TreatmentReview_id = 2 ".$where.") as review,
						(SELECT COUNT( * ) FROM v_Treatment with(nolock) WHERE TreatmentReview_id = 1 ".$where.") as notreview
					FROM v_Treatment with(nolock)
					WHERE (1=1) ".$where."
				";
				break;
			case 'TRW_multiplicity':
				$query = "
					SELECT
						COUNT( * ) as all_item,
						(SELECT COUNT( * ) FROM v_Treatment with(nolock) WHERE TreatmentMultiplicity_id = 1 ".$where.") as first,
						(SELECT COUNT( * ) FROM v_Treatment with(nolock) WHERE TreatmentMultiplicity_id = 2 ".$where.") as doubl
					FROM v_Treatment with(nolock)
					WHERE (1=1) ".$where."
				";
				break;
			case 'TRW_subjectLpu':
				...
				$query = "
					SELECT
						...
						(SELECT COUNT( * ) FROM v_Treatment with(nolock) WHERE Lpu_sid = l.Lpu_id ".$where." ) as number     
					FROM 
						v_Lpu l with(nolock)
					WHERE 
						(SELECT COUNT( * ) FROM v_Treatment with(nolock) WHERE Lpu_sid = l.Lpu_id ".$where." ) > 0
				";
				$res = $this->db->query($query, array());
				if ( is_object($res) )
			...
			case 'TRW_subjectMedpersonal':
				if ( isset($data['Lpu_sid']) )
				{
					...
					$left_join[]='left join v_Lpu l with(nolock) on l.Lpu_id = ' . $data['Lpu_sid'];
					$query = "
							SELECT TOP 1
								". implode(",\n						", $select) ."
							FROM
								v_Treatment with(nolock)
								". implode("\n						", $left_join) ."
					";
					$res = $this->db->query($query, array());
					if ( is_object($res) )
				...
				$query = "
					SELECT
						...
						RTRIM(m.Person_Fio) as name,
						(SELECT COUNT( * ) FROM v_Treatment with(nolock) WHERE MedPersonal_sid = m.MedPersonal_id AND MedPersonal_Lpu_sid = m.Lpu_id ".$where." ) as number
					FROM
						v_MedPersonal m with(nolock)
						left join v_Lpu l with(nolock) on l.Lpu_id = m.Lpu_id
					WHERE 
						".$lpu." (SELECT COUNT( * ) FROM v_Treatment with(nolock) WHERE MedPersonal_sid = m.MedPersonal_id AND MedPersonal_Lpu_sid = m.Lpu_id ".$where." ) > 0
					ORDER BY
