	...
	function loadAttachedList($data) {
		...
		$query = "
			select
				...
				convert(varchar(10), PC.PersonCard_begDate, 120) as DATA_REG,
			...
			from
				...
				inner join v_LpuSection LS with(nolock) on LS.LpuSection_id = LR.LpuSection_id
				...
				inner join v_OMSSprTerr OST with (nolock) on OST.OMSSprTerr_id = PLS.OMSSprTerr_id
				outer apply (
					select top 1
						PS.Person_Snils,
					...
					from
						...
						inner join v_MedStaffFact MSF with(nolock) on MSF.MedPersonal_id = MSR.MedPersonal_id
						inner join v_PersonState PS with(nolock) on PS.Person_id = MSF.Person_id
					where
					...
					order by
						...
						isnull(MSF.MedPersonal_TabCode,0) desc
				) MP
			where
