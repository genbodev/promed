...
class Rmis_model extends swModel {
	...
	function getSyncSpr($table, $id, $advancedKey = '', $useLinkTable = false, $allowBlank = false) {
		...
		if ($useLinkTable) {
			$query = "
				...
				from
					r66.RMIS{$table}Link with (nolock)
				where
	...
	function getPersonInfo($data) {
		...
		$query = "
			...
			left join PersonBirthPlace PDP with (nolock) on PS.Person_id = PDP.Person_id
	...
	function getAddressInfo($Address_id) {
		...
		$query = "
			select top 1
				...
				A.Address_Address,
				--Level 1 Страна
				Country.KLCountry_id,
				isnull(Country.KLCountry_Nick, Country.KLCountry_Name) as KLCountry_Name,
				--Level 2 Регион
				...
				A.Address_House+isnull(A.Address_Corpus, '') as Address_House,
				--Level 8 Квартира
				A.Address_Flat
			from
				v_Address A with(nolock)
				left join v_KLCountry Country with(nolock) on Country.KLCountry_id = A.KLCountry_id
				...
				left join v_KLSocr SubRgnSocr with(nolock) on SubRgnSocr.KLSocr_id = SubRgnSocr.KLSocr_id
				...
				left join v_KLSocr HouseSocr with(nolock) on HouseSocr.KLSocr_Name = 'ДОМ'
			where
	...
	function getMedStaffFactInfo($data) {
		...
		$query = "
			select top 1
				...
				convert(varchar(10), msf.WorkData_begDate, 120) as WorkData_begDate,
				convert(varchar(10), msf.WorkData_endDate, 120) as WorkData_endDate
			from
				v_MedStaffFact msf with (nolock)
				left join v_LpuSectionProfile lsp with(nolock) on lsp.LpuSectionProfile_id = msf.LpuSectionProfile_id
			where
		...
		if (!empty($resp[0])) {
			if (in_array($resp[0]['LpuSectionProfile_Code'], array('0','0000'))) {	//Если "0. Не определен", то берем профиль с отделения
				$query = "
					...
					and lslsp.LpuSectionLpuSectionProfile_begDate <= @date
					and (lslsp.LpuSectionLpuSectionProfile_endDate is null or lslsp.LpuSectionLpuSectionProfile_endDate > @date)
					order by NumType
				";
	...
	function getEvnDirectionInfo($data) {
		...
		$query = "
			...
			from
				...
				left join v_Evn PE with(nolock) on PE.Evn_id = ED.EvnDirection_pid
				left join v_pmUserCache FailUser with(nolock) on FailUser.pmUser_id = ED.pmUser_failID
	...
	function getEvnPSDiags($data) {
		...
		$query = "
			select top 1
				...
				D.Diag_Name,
				DSC.DiagSetClass_id,
				DSC.DiagSetClass_SysNick,
				DST.DiagSetType_id,
				DST.DiagSetType_SysNick,
				convert(varchar(10), EPS.EvnPS_setDate, 120) as EvnDiagPS_setDate,
				EPS.MedStaffFact_pid as MedStaffFact_id,
				1 as main
			from
				v_EvnPS EPS with(nolock)
				inner join v_Diag D with(nolock) on D.Diag_id = EPS.Diag_pid
				left join v_DiagSetClass DSC with(nolock) on DSC.DiagSetClass_SysNick = 'osn'
				left join v_DiagSetType DST with(nolock) on DST.DiagSetType_SysNick = 'priem'
			where
			...
			select
				...
				DST.DiagSetType_id,
				DST.DiagSetType_SysNick,
				convert(varchar(10), EDPS.EvnDiagPS_setDate, 120) as EvnDiagPS_setDate,
				isnull(EPS.MedStaffFact_pid, ES.MedStaffFact_id) as MedStaffFact_id,
				case when DSC.DiagSetClass_SysNick = 'osn' then 1 else 0 end as main
			from
				v_EvnDiagPS EDPS with(nolock)
				inner join v_Diag D with(nolock) on D.Diag_id = EDPS.Diag_id
				left join v_EvnPS EPS with(nolock) on EPS.EvnPS_id = EDPS.EvnDiagPS_pid
				left join v_EvnSection ES with(nolock) on ES.EvnSection_id = EDPS.EvnDiagPS_pid
				left join v_DiagSetClass DSC with(nolock) on DSC.DiagSetClass_id = EDPS.DiagSetClass_id
				left join v_DiagSetType DST with(nolock) on DST.DiagSetType_id = EDPS.DiagSetType_id
			where
	...
	function getEvnSectionDiags($data) {
		...
		$query = "
			select top 1
				...
				DSС.DiagSetClass_id,
				DST.DiagSetType_id,
				convert(varchar(10), ES.EvnSection_setDate, 120) as EvnDiagPS_setDate,
				1 as main
			from
				v_EvnSection ES with(nolock)
				left join v_Diag D with(nolock) on D.Diag_id = ES.Diag_id
				left join v_DiagSetClass DSС with(nolock) on DSС.DiagSetClass_SysNick = 'osn'
				left join v_DiagSetType DST with(nolock) on DST.DiagSetType_SysNick = 'klin'
			where
			...
			from
				v_EvnDiagPS EDPS with(nolock)
				left join v_Diag D with(nolock) on D.Diag_id = EDPS.Diag_id
				left join v_DiagSetClass DSC with(nolock) on DSC.DiagSetClass_id = EDPS.DiagSetClass_id
				left join v_DiagSetType DST with(nolock) on DST.DiagSetType_id = EDPS.DiagSetType_id
			where
	...
	function getEvnPSInfo($data) {
		...
		$query = "
			select top 1
				...
				convert(varchar(10), EPS.EvnPS_setDate, 120) as EvnPS_setDate,
				isnull(ESFIRST.PayType_id, EPS.PayType_id) as PayType_id,
			...
			from
				...
				outer apply(
					...
					from
						v_EvnSection ES with(nolock)
						left join v_Diag ESD with(nolock) on ESD.Diag_id = ES.Diag_id
				...
				outer apply (
					...
					from
						...
						inner join UslugaComplex with(nolock) on UslugaComplex.UslugaComplex_id = EvnUsluga.UslugaComplex_id
						inner join r66.UslugaComplexPartitionLink UCPL with (nolock) on UCPL.UslugaComplex_id = UslugaComplex.UslugaComplex_id
				...
				outer apply(
					select top 1
						 ...
						 D.Diag_Name
					from
						v_DeathSvid t with(nolock)
						left join v_Diag DSD with(nolock) on DSD.Diag_id = coalesce(Diag_iid,Diag_tid,Diag_mid,Diag_eid,Diag_oid)
					where
				...
				outer apply (
					...
					from r66.LpuSectionLink with (nolock)
					where LpuSection_id = ESLAST.LpuSection_id
					order by LpuSectionLink_insdt desc
				) LSL
				left join nsi.MedicalCareKind MCK with (nolock) on MCK.MedicalCareKind_id = LSL.MedicalCareKind_id
				left join v_PersonState ps (nolock) on ps.Person_id = EPS.Person_id
				left join v_PayMedType PMT with(nolock) on PMT.PayMedType_Code = ESLAST.PayMedType_Code
				left join v_Okei O with(nolock) on O.Okei_id = EPS.Okei_id
			where
				EPS.EvnPS_id = :EvnPS_id
		";
	...
	function getEvnSectionInfo($data) {
		...
		$query = "
			select top 1
				...
				convert(varchar(5), ES.EvnSection_setTime, 120) as EvnSection_setTime,
				convert(varchar(10), ES.EvnSection_disDate, 120) as EvnSection_disDate,
				CONVERT(varchar(5), ES.EvnSection_disTime, 108) as EvnSection_disTime,
				...
				ES.Lpu_id,
				PS.Person_id,
				convert(varchar(10), PS.Person_deadDT, 120) as Person_deadDate,
				convert(varchar(5), PS.Person_deadDT, 108) as Person_deadTime,
			...
			from
				...
				left join persis.WorkPlace WP (nolock) on WP.id = ES.MedStaffFact_id
				outer apply (
					select top 1
						EvnSection_id
					from
				...
				outer apply (
					...
					from
						v_EvnSection NES with(nolock)
						left join v_LpuSection NLS with(nolock) on NLS.LpuSection_id = NES.LpuSection_id
						left join persis.WorkPlace NWP (nolock) on NWP.id = NES.MedStaffFact_id
					where
						EvnSection_pid = ES.EvnSection_pid
						and EvnSection_Index = ES.EvnSection_Index+1
				) NextES
	...
	function getStaffInfo($data) {
		...
		$query = "
			select top 1
				...
				p.name as Post_Name,
				s.Post_id,
				convert(varchar(10), s.BeginDate, 120) as BeginDate,
				convert(varchar(10), s.EndDate, 120) as EndDate,
				MSO.MedSpecOms_id,
				MSO.MedSpecOms_Code
			from
				persis.v_Staff S with (nolock)
				left join persis.v_Post p with (nolock) on p.id = s.Post_id
				outer apply(
					select top 1 MedSpecOms_id
					from v_MedStaffFactCache with(nolock)
					where Staff_id = S.id
					group by MedSpecOms_id
					order by count(MedStaffFact_id) desc
				) MSF --Получение наиболее распространенной специальности
	...
	function getRMISVizitTypeId($data) {
		...
		if (!empty($data['VizitType_id'])) {
			$VizitType_Code = $this->getFirstResultFromQuery("
				select top 1 VizitType_Code from v_VizitType with(nolock) where VizitType_id = :VizitType_id
			...
			switch($DispClass_Code) {
				case 1: $vizitTypeId = 10;break;
				case 2: $vizitTypeId = 11;break;
				case 3: $vizitTypeId = 12;break;
				case 4: $vizitTypeId = 13;break;
				case 5: $vizitTypeId = 16;break;
				case 6:
					switch($EducationInstitutionType_SysNick) {
						case 'PreSchool': $vizitTypeId = 25;break;
						case 'School': $vizitTypeId = 26;break;
						case 'SpecSchool': $vizitTypeId = 27;break;
					}
					break;
				case 7: $vizitTypeId = 14;break;
				case 8: $vizitTypeId = 15;break;
				case 9:
					switch($EducationInstitutionType_SysNick) {
						case 'PreSchool': $vizitTypeId = 19;break;
						case 'School': $vizitTypeId = 20;break;
						case 'SpecSchool': $vizitTypeId = 21;break;
					}
					break;
				case 10:
				...
				case 10:
					switch($EducationInstitutionType_SysNick) {
						case 'PreSchool': $vizitTypeId = 22;break;
						case 'School': $vizitTypeId = 23;break;
						case 'SpecSchool': $vizitTypeId = 24;break;
					}
					break;
				case 12: $vizitTypeId = 18;break;
			}
		}
		return $vizitTypeId;
	}
	...
	function createPosition($clinicId, $departmentId, $StaffInfo) {
		...
		if (!empty($StaffInfo['MedSpecOms_Code'])) {
			$res = $this->getFirstResultFromQuery("
				select top 1 RMISSertificateSpeciality_id
				from r66.v_RMISSertificateSpeciality with(nolock)
	...
	function getLpuSectionInfo($data) {
		...
		$query = "
			select top 1
				...
				LpuSection_Code,
				LpuSection_Name,
				convert(varchar(10), LpuSection_setDate, 120) as LpuSection_setDate,
				convert(varchar(10), LpuSection_disDate, 120) as LpuSection_disDate
			from v_LpuSection_all with(nolock)
		...
		if (!empty($data['DocumentType_id']) && !empty($data['Document_Num'])) {
			...
			if (empty($typeId)) {
				$typeId = $this->getFirstResultFromQuery("
					select top 1 RMISDocumentType_id
					from r66.v_RMISDocumentType with(nolock)
	...
	function syncEmployeeEducations($MedPersonal_id) {
		...
		$query = "
			select
				id,
				YearOfGraduation,
				DiplomaSpeciality_id,
				EducationType_id
			from
				persis.SpecialityDiploma with(nolock)
			where
	...
	function syncEmployeeSpecialities($Lpu_id, $MedPersonal_id) {
		...
		$query = "
			...
			from
				persis.Certificate C with(nolock)
				inner join persis.Speciality S with(nolock) on S.id = C.Speciality_id
				inner join persis.FRMPSertificateSpeciality SS with(nolock) on SS.id = S.frmpEntry_id
	...
	function syncDocuments($Person_id) {
		...
		$query = "
			select
				...
				convert(varchar(10), d.Document_endDate, 120) as Document_endDate,
				ps.Polis_id,
				ps.PolisType_id,
				os.OrgSMO_Nick,
	...
	function getEvnPLDispInfo($data) {
		...
		$query = "
			select top 1
				...
				PS.SocStatus_id,
				DC.DispClass_Code,
				EvnVizit.EvnVizitDisp_id,
				D.Diag_id,
				D.Diag_Code,
				D.Diag_Name,
				EPLD.PayType_id,
				EIT.EducationInstitutionType_SysNick,
				EPLDTT.AgeGroupDisp_id,
			...
			from
				...
				left join v_EvnPLDispTeenInspection EPLDTT with(nolock) on EPLDTT.EvnPLDispTeenInspection_id = EPLD.EvnPLDisp_id
				...
				left join v_EducationInstitutionClass EIC with(nolock) on EIC.EducationInstitutionClass_id = EPLDTT.EducationInstitutionClass_id
				left join v_EducationInstitutionType  EIT with(nolock) on EIT.EducationInstitutionType_id = EIC.EducationInstitutionType_id
				left join v_AssessmentHealth AH with(nolock) on AH.EvnPLDisp_id = EPLD.EvnPLDisp_id
				outer apply(
					select top 1
						EVD.EvnVizitDisp_id,
						EVD.Diag_id
					from
						...
						inner join v_EvnVizitDisp EVD with(nolock) on EVD.DopDispInfoConsent_id = DDIC.DopDispInfoConsent_id
					where
						DDIC.EvnPLDisp_id = EPLD.EvnPLDisp_id
						and ST.SurveyType_Code in (19,27) -- Осмотр педиатром или терапевтом
				) EvnVizit
				left join v_Diag D with(nolock) on D.Diag_id = EvnVizit.Diag_id
			where
	...
	function getEvnPLDispDiags($data) {
		...
		$query = "
			select
				...
				convert(varchar(10), EVD.EvnVizitDisp_setDT, 120) as EvnDiag_setDate,
				EVD.MedStaffFact_id,
				1 as main
			from
			...
			select
				...
				convert(varchar(10), EDDD.EvnDiagDopDisp_setDT, 120) as EvnDiag_setDate,
				EVD.MedStaffFact_id,
				0 as main
			from
				v_EvnDiagDopDisp EDDD with(nolock)
				inner join v_Diag D with(nolock) on D.Diag_id = EDDD.Diag_id
				left join v_EvnUsluga EU with(nolock) on EU.EvnUsluga_id = EDDD.EvnDiagDopDisp_pid
				left join v_EvnVizitDisp EVD with(nolock) on EVD.EvnVizitDisp_id in (EDDD.EvnDiagDopDisp_pid, EU.EvnUsluga_pid)
				...
				left join v_DeseaseType DT with(nolock) on DT.DeseaseType_Code = (case
	...
	function getEvnVizitDispInfo($data) {
		...
		$query = "
			select top 1
				EVD.EvnVizitDisp_id,
				EVD.EvnVizitDisp_pid,
				EVD.EvnDirection_id,
				EVD.MedPersonal_id,
				EVD.MedStaffFact_id,
				convert(varchar(10), EVD.EvnVizitDisp_setDT, 120) as EvnVizitDisp_setDate,
				convert(varchar(8), EVD.EvnVizitDisp_setDT, 108) as EvnVizitDisp_setTime,
				LS.LpuSection_id,
				isnull(WP.LpuSectionProfile_id,LS.LpuSectionProfile_id) as LpuSectionProfile_id,
				DC.DispClass_Code,
				EIT.EducationInstitutionType_SysNick,
				EPLDTT.AgeGroupDisp_id,
				D.Diag_id,
			...
			from
				v_EvnVizitDisp EVD with(nolock)
				left join v_EvnPLDisp EPLD with(nolock) on EPLD.EvnPLDisp_id = EVD.EvnVizitDisp_pid
				left join v_DispClass DC with(nolock) on DC.DispClass_id = EPLD.DispClass_id
				left join v_EvnPLDispTeenInspection EPLDTT with(nolock) on EPLDTT.EvnPLDispTeenInspection_id = EPLD.EvnPLDisp_id
				left join v_EducationInstitutionClass  EIC with(nolock) on EIC.EducationInstitutionClass_id = EPLDTT.EducationInstitutionClass_id
				left join v_EducationInstitutionType  EIT with(nolock) on EIT.EducationInstitutionType_id = EIC.EducationInstitutionType_id
				left join v_LpuSection LS with(nolock) on LS.LpuSection_id = EVD.LpuSection_id
				left join persis.WorkPlace WP (nolock) on WP.id = EVD.MedStaffFact_id
				left join v_Diag D with (nolock) on D.Diag_id = EVD.Diag_id
				left join v_DiagSetClass DSC with(nolock) on DSC.DiagSetClass_SysNick = 'osn'
				left join v_DiagSetType DST with(nolock) on DST.DiagSetType_SysNick = 'klin'
			where
	...
	function getEvnVizitPLInfo($data) {
		...
		$query = "
			select top 1
				...
				convert(varchar(10), EVPL.EvnVizitPL_setDT, 120) as EvnVizitPL_setDate,
				convert(varchar(8), EVPL.EvnVizitPL_setDT, 108) as EvnVizitPL_setTime,
			...
			from
				...
				left join v_LeaveType LT with (nolock) on LT.LeaveType_fedid = RC.LeaveType_fedid
				...
				left join persis.WorkPlace WP (nolock) on WP.id = EVPL.MedStaffFact_id
			where
	...
	function syncEvnUslugas($EvnUsluga_pid) {
		...
		$query = "
			select
				...
				d.Diag_Code,
				d.Diag_Name,
				convert(varchar(10), isnull(eu.EvnUsluga_setDT, evd.EvnVizitDisp_setDT), 120) as EvnUsluga_setDate,
				convert(varchar(9), isnull(eu.EvnUsluga_setDT, evd.EvnVizitDisp_setDT), 108) as EvnUsluga_setTime,
				convert(varchar(10), isnull(eu.EvnUsluga_disDT, evd.EvnVizitDisp_disDT), 120) as EvnUsluga_disDate,
				eu.EvnUsluga_Kolvo,
				eu.EvnUsluga_isCito,
				eu.Person_id,
				eu.Lpu_id,
			...
			from
				...
				inner join v_Evn ParentEvn (nolock) on ParentEvn.Evn_id = eu.EvnUsluga_pid
				inner join v_Evn RootEvn (nolock) on RootEvn.Evn_id = eu.EvnUsluga_rid
				left join v_EvnSection es with(nolock) on es.EvnSection_id = ParentEvn.Evn_id
				left join v_EvnVizitPL evpl with(nolock) on evpl.EvnVizitPL_id = ParentEvn.Evn_id
				left join v_EvnVizitDisp evd (nolock) on evd.EvnVizitDisp_id = ParentEvn.Evn_id
				left join v_EvnPLDisp epld (nolock) on epld.EvnPLDisp_id = RootEvn.Evn_id
				left join v_Diag d with(nolock) on d.Diag_id = isnull(eu.Diag_id, evd.Diag_id)
				...
				outer apply(
					...
					inner join r66.UslugaComplexPartition UCP with(nolock) on UCPL.UslugaComplexPartition_id=UCP.UslugaComplexPartition_id
	...
	function syncServiceIdByMes($Mes_id, $Lpu_id, $LpuUnitType_SysNick) {
		$Mes = $this->getFirstRowFromQuery("
			select top 1
				Mes_Code,
				Mes_Name,
				convert(varchar(10), Mes_begDT, 120) as Mes_begDate,
				convert(varchar(10), Mes_endDT, 120) as Mes_endDate
			from v_MesOld with(nolock) where Mes_id = :Mes_id
		", array('Mes_id' => $Mes_id));
		if (!is_array($Mes)) {
		...
		if (true || empty($referralId)) {
			...
			if (!empty($EvnDirectionInfo['ParentEvn_id']) && in_array($EvnDirectionInfo['ParentEvnClass_SysNick'], array('EvnVizitPL','EvnSection'))) {
				...
				switch($EvnDirectionInfo['ParentEvnClass_SysNick']) {
					...
					case 'EvnVizitPL': $this->getVisit($stepId); break;
					case 'EvnSection': $this->getHspRecord($stepId); break;
				}
			}
		...
		try {
			$query = "
				...
				from
					...
					left join v_MedStaffFactCache MSF with (nolock) on MSF.MedStaffFact_id = EVPL.MedStaffFact_id
					outer apply(
						select top 1
							PersonCard_id
						from
						...
						where
							...
							and ISNULL(PersonCard_endDate, EVPL.EvnVizitPL_setDate) >= EVPL.EvnVizitPL_setDate
			...
			if (true || empty($caseId)) {
				...
				switch(2) {
					case 'EvnPS_IsImperHosp':
						$prehosStageDefId = 1; break;
					case 'EvnPS_IsShortVolume':
						$prehosStageDefId = 2; break;
					case 'EvnPS_IsWrongCure':
						$prehosStageDefId = 3; break;
					case 'EvnPS_IsDiagMismatch':
						$prehosStageDefId = 4; break;
				}
		...
		try {
			$query = "
				...
				from
					v_EvnSection ES with (nolock)
					inner join v_EvnPS EPS with (nolock) on EPS.EvnPS_id = ES.EvnSection_pid
					left join v_MedStaffFactCache MSF with (nolock) on MSF.MedStaffFact_id = ES.MedStaffFact_id
					outer apply(
						select top 1
							PersonCard_id
						from
						...
						where
							...
							and ISNULL(PersonCard_endDate, ES.EvnSection_setDate) >= ES.EvnSection_setDate
				...
				where
					...
					and (isnull(ES.EvnSection_isPriem, 1) = 1 or ES.LpuSection_id is not null)
			";
		...
		try {
			$query = "
				...
				from
					...
					inner join v_EvnVizitDisp EVD with(nolock) on EVD.DopDispInfoConsent_id = DDIC.DopDispInfoConsent_id
					left join v_MedStaffFactCache MSF with (nolock) on MSF.MedStaffFact_id = EVD.MedStaffFact_id
					outer apply(
						select top 1
							PersonCard_id
						from
						...
						where
							...
							and ISNULL(PersonCard_endDate, EPLD.EvnPLDisp_setDate) >= EPLD.EvnPLDisp_setDate
