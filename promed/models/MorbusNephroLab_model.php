 ...
 */
class MorbusNephroLab_model extends swModel
{
	/**
	...
	static function defAttributes()
	{
		return array(
			...
			'ratetype_id' => array(
				...
				'join' => 'inner join v_Rate r with (nolock) on r.Rate_id = {ViewName}.Rate_id',
			),
			'unit_id' => array(
				'properties' => array(),
				'alias' => 'Unit_id',
	...
	function doLoadGrid($data)
	{
		...
		if (!empty($data['isOnlyLast'])) {
			$add_join .= '
			outer apply (
					select top 1 t2.MorbusNephroRate_id
					from v_MorbusNephroRate t2 (nolock)
					inner join v_Rate r2 (nolock) on r2.Rate_id = t2.Rate_id
		...
		$sql = "
			select
				...
				rt.RateType_Name,
				r.Rate_ValueStr,
				convert(varchar(10), t.MorbusNephroRate_rateDT, 104) as {$dateAlias},
				:Evn_id as MorbusNephro_pid
			from v_MorbusNephroRate t (nolock)
