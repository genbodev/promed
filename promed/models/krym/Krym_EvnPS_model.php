...
class Krym_EvnPS_model extends EvnPS_model {
	...
	function exportHospDataForTfomsToXml($data) {
		...
		$query = "
			...
			SET @S1 = isnull((
			Select
			cast(lp.Lpu_f003mcod as varchar)+cast(year(EvnDirection_setDT) as varchar)+cast(EvnDirection_Num as varchar) as NOM_NAP,		--T(16) Номер направления (MCOD+NP)
			...
			isnull(pe.PersonPhone_Phone, 'не указан') as TLF, --Varchar(100) - Контактная информация
			...
			inner join dbo.v_LpuSection ls1 (nolock) on ls1.LpuSection_id = ed.LpuSection_id
			inner join dbo.v_LpuUnit lu1 (nolock) on ls1.LpuUnit_id=lu1.LpuUnit_id
			inner join dbo.v_LpuBuilding lb1 (nolock) on lb1.LpuBuilding_id = lu1.LpuBuilding_id
			...
			outer apply(
				select top 1 lses.LpuSection_Code, lses.LpuSectionProfile_Code 
			...
			from (
			select
			cast(lp.Lpu_f003mcod as varchar)+cast(year(ed.EvnDirection_setDT) as varchar)+cast(ed.EvnDirection_Num as varchar)	as	NOM_NAP, -- bigint , Идентификатор направления
			...
			inner join dbo.v_LpuBuilding lb (nolock) on lb.LpuBuilding_id = lu.LpuBuilding_id
			left join dbo.v_LpuSectionBedProfile lsb (nolock) on lsb.LpuSectionBedProfile_id = ls.LpuSectionBedProfile_id
			inner join dbo.v_Person_all pe (nolock) on pe.PersonEvn_id = es.PersonEvn_id and pe.Server_id = es.Server_id
	...
	protected function _printEvnPS($data = [], $response = []) {
		$template = 'evn_ps_template_list_a4_krym';
