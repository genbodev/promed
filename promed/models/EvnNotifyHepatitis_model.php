<?php
/**
* EvnNotifyHepatitis_model - модель для работы с таблицей EvnNotifyHepatitis
*
...
class EvnNotifyHepatitis_model extends swModel
{
	...
	/**
	 * контроль на наличие в системе Объекта «Извещение» с типом «Гепатит» и Объекта «Запись регистра» с типом «Гепатит».
	 */
	function checkEvnNotifyHepatitis($data)
	{
		$tableName = 'EvnNotifyHepatitis';
		$this->load->library('swMorbus');
		return swMorbus::getStaticMorbusCommon()->checkExistsExtended('hepa', $data['Person_id'], null,"
	...
	public function doSave($data = array(), $isAllowTransaction = true)
	{
		try {
			...
			if (empty($data['EvnNotifyHepatitis_setDT'])) {
			...
			$queryParams = array(
				...
				'EvnNotifyHepatitis_setDT' => $data['EvnNotifyHepatitis_setDT'],
			...
			$tmp = swMorbus::onAfterSaveEvnNotify($this->morbusTypeSysNick, array(
				...
				'EvnNotifyBase_setDate' => $data['EvnNotifyHepatitis_setDT'],
				'Server_id' => $data['Server_id'],
				'PersonEvn_id' => $data['PersonEvn_id'],
