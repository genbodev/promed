...
class PregnancyEvnPS_model extends swModel {
	/**
	 * Конструктор
	 */
	...
	function getPregnancyEvnPS($EvnPS_id) {
		...
		$query = "
			select top 1 * from v_PregnancyEvnPS with(nolock) where EvnPS_id = :EvnPS_id
		";
	...
	function savePregnancyEvnPS($data) {
		...
		$query = "
			declare
				...
				@Error_Code bigint,
				@Res bigint = :PregnancyEvnPS_id;
			exec {$procedure}
				@PregnancyEvnPS_id = @Res output,
				@PregnancyEvnPS_Period = :PregnancyEvnPS_Period,
				@EvnPS_id = :EvnPS_id,
				@pmUser_id = :pmUser_id,
				@Error_Code = @Error_Code output,
				@Error_Message = @Error_Message output
			select @Res as PregnancyEvnPS_id, @Error_Code as Error_Code, @Error_Message as Error_Msg;
		";
