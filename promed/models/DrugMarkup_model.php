...
class DrugMarkup_model extends swModel {
	...
	function load() {
		$q = "
			select
				DrugMarkup_id, DrugMarkup_begDT, DrugMarkup_endDT, DrugMarkup_MinPrice, DrugMarkup_MaxPrice, DrugMarkup_Wholesale, DrugMarkup_Retail, DrugMarkup_IsNarkoDrug, Drugmarkup_Delivery
			from
				dbo.v_DrugMarkup with (nolock)
			where
	...
	function loadList($filter) {
		...
		$q = "
			select
				v_DrugMarkup.DrugMarkup_id,
				convert(varchar(10), v_DrugMarkup.DrugMarkup_begDT, 104) as DrugMarkup_begDT,
				convert(varchar(10), v_DrugMarkup.DrugMarkup_endDT, 104) as DrugMarkup_endDT,
				v_DrugMarkup.DrugMarkup_MinPrice, v_DrugMarkup.DrugMarkup_MaxPrice, v_DrugMarkup.DrugMarkup_Wholesale, v_DrugMarkup.DrugMarkup_Retail, v_DrugMarkup.DrugMarkup_IsNarkoDrug, v_DrugMarkup.Drugmarkup_Delivery
				,DrugMarkup_IsNarkoDrug_ref.YesNo_Name DrugMarkup_IsNarkoDrug_Name,
				pmMediaData.pmMediaData_Comment as File_Name
			from
				dbo.v_DrugMarkup with (nolock)
				LEFT JOIN dbo.v_YesNo DrugMarkup_IsNarkoDrug_ref WITH (NOLOCK) ON DrugMarkup_IsNarkoDrug_ref.YesNo_id = v_DrugMarkup.DrugMarkup_IsNarkoDrug
				outer apply (
					select top 1
						pmMediaData_Comment
					from
	...
	function save() {
		...
		$q = "
			declare
				@DrugMarkup_id bigint,
				@ErrCode int,
				@ErrMessage varchar(4000);
			set @DrugMarkup_id = :DrugMarkup_id;
			exec dbo." . $procedure . "
				@DrugMarkup_id = @DrugMarkup_id output,
			...
			select @DrugMarkup_id as DrugMarkup_id, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
		";
		$p = array(
