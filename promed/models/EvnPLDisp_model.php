 ...
 * @property EvnPLDispTeenInspection_model $EvnPLDispTeenInspection_model
 * @property EvnPLDispDop13_model $EvnPLDispDop13_model
 * @property EvnPLDispProf_model $EvnPLDispProf_model
 * @property EvnPLDispOrp13_model $EvnPLDispOrp13_model
 * @property EvnPLDispDriver_model $EvnPLDispDriver_model
 * @property EvnPLDispMigrant_model $EvnPLDispMigrant_model
*/

class EvnPLDisp_model extends swModel
{
	...
	function updateDopDispInfoConsent($data) {
		...
		$resp_dd = $this->queryResult("
			select
				...
				epld.EvnPLDisp_fid,
				epld.Lpu_mid,
				epld.EvnPLDisp_IsMobile,
				epld.EvnPLDisp_IsOutLpu,
				epld.EvnPLDisp_IsNewOrder,
				epld.PayType_id,
				convert(varchar(10), epld.EvnPLDisp_consDT, 120) as EvnPLDisp_consDate,
				convert(varchar(10), epld.EvnPLDisp_setDT, 120) as EvnPLDisp_setDate,
				epld.PersonEvn_id,
				epld.Person_id,
				epld.Server_id,
			...
			from
				...
				left join v_EvnPLDispDriver epldd (nolock) on epldd.EvnPLDispDriver_id = epld.EvnPLDisp_id
			where
		...
		switch($data['DispClass_id']) {
			...
			case 2: // ДВН 2 этап
				...
				$resp_first = $this->queryResult("
					select top 1 epldd.EvnPLDispDop13_id
					from v_EvnPLDispDop13 epldd (nolock)
					where
						YEAR(epldd.EvnPLDispDop13_consDT) = YEAR(dbo.tzGetDate())
						and epldd.Person_id = :Person_id
						and epldd.DispClass_id = 1
						and epldd.EvnPLDispDop13_IsTwoStage = 2
						and epldd.Lpu_id = :Lpu_id
						and not exists(
							select top 1 EvnPLDispDop13_id from v_EvnPLDispDop13 (nolock) where EvnPLDispDop13_fid = epldd.EvnPLDispDop13_id -- и нет карты 2 этапа
						)
				", array(
					'Lpu_id' => $data['Lpu_id'],
					'Person_id' => $data['Person_id']
			...
			case 7: // ДДС 1 этап
				...
				$resp_persdo = $this->queryResult("
					...
					from
						v_PersonDispOrp persdo (nolock)
					where
						persdo.Person_id = :Person_id
						and persdo.CategoryChildType_id < 8
						and persdo.PersonDispOrp_Year = year(dbo.tzGetDate())
				", array(
					'Person_id' => $data['Person_id']
				));
				...
				$params['PayType_id'] = $this->getFirstResultFromQuery("select top 1 PayType_id from v_PayType (nolock) where PayType_SysNick = :PayType_SysNick", array(
					'PayType_SysNick' => getPayTypeSysNickOMS()
				));
			...
			case 8: // ДДС 2 этап
				...
				$resp_first = $this->queryResult("
					select top 1
						...
						epldd.DispClass_id
					from v_EvnPLDispOrp epldd (nolock)
					where
						YEAR(epldd.EvnPLDispOrp_consDT) = YEAR(dbo.tzGetDate())
						and epldd.Person_id = :Person_id
						and epldd.DispClass_id IN (3,7)
						and epldd.EvnPLDispOrp_IsTwoStage = 2
						and epldd.Lpu_id = :Lpu_id
						and not exists(
							select top 1 EvnPLDispOrp_id from v_EvnPLDispOrp (nolock) where EvnPLDispOrp_fid = epldd.EvnPLDispOrp_id -- и нет карты 2 этапа
						)
				", array(
					'Lpu_id' => $data['Lpu_id'],
					'Person_id' => $data['Person_id']
			...
			case 11: // Пред. МОН 2 этап
				...
				// ищем карту 1-го этапа в текущем году
				$resp_first = $this->queryResult("
					select top 1 epldd.EvnPLDispTeenInspection_id
					from v_EvnPLDispTeenInspection epldd (nolock)
					where
						YEAR(epldd.EvnPLDispTeenInspection_consDT) = YEAR(dbo.tzGetDate())
						and epldd.Person_id = :Person_id
						and epldd.DispClass_id = :DispClass_id
						and epldd.EvnPLDispTeenInspection_IsTwoStage = 2
						and epldd.Lpu_id = :Lpu_id
						and not exists(
							select top 1 EvnPLDispTeenInspection_id from v_EvnPLDispTeenInspection (nolock) where EvnPLDispTeenInspection_fid = epldd.EvnPLDispTeenInspection_id -- и нет карты 2 этапа
						)
				", array(
					'Lpu_id' => $data['Lpu_id'],
					'Person_id' => $data['Person_id'],
	...
	function getDispClassListAvailable($data) {
		...
		$resp = $this->getEvnPLDispInfo($data);
		...
		if($data['getAllDispInfo']) {
			return array(0=>array('avail'=>$avail, 'info'=>$pers));
		}
	...
	/**
	 * Получить информацию по диспансеризации пациента за текущий год
	 * Используется:	this->getDispClassListAvailable
	 * 					меню Диспансеризации в верхней панели ЭМК (ext6)
	 */
	function getEvnPLDispInfo($data) {
		...
		$sql = "
			...
			select
				ps.Person_id,
				MSR.MedStaffRegion_id, -- наличие у врача учатска основного прикрепления пациента
				dbo.Age2(ps.Person_BirthDay, @curDate) as Person_Age, -- возраст пациента
				dbo.Age2(ps.Person_BirthDay, @YearLastDay) as Person_AgeEndYear, -- возраст пациента на конец текущего года
				EPLDD13_2year.EvnPLDispDop13_id as EvnPLDispDop13_2year_id, --карта ДВН за прошедшие два года
				EPLDD13_2year.EvnPLDispDop13_IsFinish as EvnPLDispDop13_2year_IsFinish,
				EPLDD13.EvnPLDispDop13_id, -- карта ДВН в текущем году
				EPLDD13.EvnPLDispDop13_IsFinish, -- признак завершения карты ДВН в текущем году
				EPLDP.EvnPLDispProf_id, -- карта ПОВН в текущем году
				...
				PPW.PersonPrivilegeWOW_id, -- в регистре ВОВ
				...
				PDO.PersonDispOrp_id, -- в регистре детей сирот или усыновлённых
				EPLDO.EvnPLDispOrp_id, -- карта ДДС в текущем году
			...
			from
				...
				outer apply (
					...
					where
						YEAR(EvnPLDispDop13_consDT) = @Year
				...
				outer apply (
					...
					from
						v_EvnPLDispDop13 (nolock)
					where
						YEAR(EvnPLDispDop13_consDT) > @Year-3 and YEAR(EvnPLDispDop13_consDT) < @Year
						and Person_id = PS.Person_id
						and DispClass_id = 1
				) EPLDD13_2year
				outer apply (
					...
					where
						YEAR(epldd.EvnPLDispDop13_consDT) = @Year
				...
				outer apply (
					...
					where
						YEAR(epldd.EvnPLDispOrp_consDT) = @Year
						and epldd.Person_id = PS.Person_id
						and epldd.DispClass_id IN (3,7)
						and epldd.EvnPLDispOrp_IsTwoStage = 2
						and not exists(
				...
				outer apply (
					...
					from
						v_EvnPLDispTeenInspection epldd (nolock)
					where
						YEAR(epldd.EvnPLDispTeenInspection_consDT) = @Year
						and epldd.Person_id = PS.Person_id
				...
				outer apply (
					...
					where
						YEAR(EvnPLDispTeenInspection_consDT) = @Year
						and Person_id = PS.Person_id
						and DispClass_id = 6
				) EPDTIPER
				outer apply (
				...
				outer apply (
					...
					from
						v_EvnPLDispProf (nolock)
					where
						YEAR(EvnPLDispProf_consDT) = @Year - 1
						and Person_id = PS.Person_id
				) EPLDPP
				" . (count($personPrivilegeCodeList) > 0 ? "outer apply (
					select top 1
					...
					where
						...
						and pp.PersonPrivilege_begDate <= @YearLastDay
						and (pp.PersonPrivilege_endDate > @YearLastDay or pp.PersonPrivilege_endDate is null)
				) PP" : "") . "
				outer apply (
					select top 1
	...
	function getEvnPLDispYears($data) {
		...
		if ( in_array($data['DispClass_id'], array(2 /*, 4, 8, 11, 12*/)) ) {
			$joinList[] = "inner join v_{$object} EPLDF with (nolock) on EPLDF.{$object}_id = EPLD.{$object}_fid";
			$filterAlias = "EPLDF";
		}
		else {
			$filterAlias = "EPLD";
	...
	function loadEvnPLDispDataForXml($data) {
		...
		switch ( $data['DispClass_id'] ) {
			...
			case 7:
				...
				$fieldsList[] = "case when ISNULL(ostac.OrgStac_Code,0)=0 then null else ostac.OrgStac_Code end as idStacOrg";
				...
				elseif($data['DispClass_id'] == 7)
				{
                    ...
                    $fieldsList[] = "
						case when Org.OrgType_id in (7,8,9) 
							then
								case when ISNULL(ostac.OrgStac_Code,0)=0 
					...
					";
                    $fieldsList[] = "
						case when (ISNULL(ostac.OrgStac_Code,0)=0 and Klar.KLAdr_Code is not null) 
							then
								case 
				...
				else
				{
                    ...
                    $fieldsList[] = "null as educOrgName";
					/*$fieldsList[] = "
								case when Org.OrgType_id in (7,8,9) then
									case when ISNULL(ostac.OrgStac_Code,0)=0 then null else ostac.OrgStac_Code end
								else null end as idEducationOrg
									";

					$fieldsList[] = "
						case when Org.OrgType_id in (7,8,9) then
							case when ISNULL(ostac.OrgStac_Code,0)=0 then Klar.KLAdr_Code else null end
						else null end as kladrDistr
					";
					$fieldsList[] = "
					case when (ISNULL(ostac.OrgStac_Code,0)=0 and Klar.KLAdr_Code is not null) then
							case
								when Org.OrgType_id = 7 then 1
								when Org.OrgType_id = 8 then 2
								when Org.OrgType_id = 9 then 3
							else null end
						else null end as idEducType
					";
					$fieldsList[] = "
						case when Org.OrgType_id in (7,8,9) then
							case when ISNULL(ostac.OrgStac_Code,0)=0 then Org.Org_Name else null end
						else null end as educOrgName
					";*/
				}

				//$fieldsList[] = "null as educOrgName";	//https://redmine.swan.perm.ru/issues/68524

				$joinList[] = 'left join v_ChildStatusType cst with (nolock) on cst.ChildStatusType_id = epld.ChildStatusType_id';
				$joinList[] = "left join v_PersonDispOrp PDOrp with (nolock) on (PDOrp.Person_id = p.Person_id and PDOrp.PersonDispOrp_Year = YEAR(epld.{$object}_setDT) and PDOrp.CategoryChildType_id in ({$cat_ch_type}))";
				$joinList[] = 'left join fed.v_OrgStac ostac with (nolock) on ostac.Org_id = PDOrp.Org_id';
				$joinList[] = 'left join v_Org Org with (nolock) on Org.Org_id = PDOrp.Org_id'; //https://redmine.swan.perm.ru/issues/68524
				if($data['DispClass_id'] == 7){
					$joinList[] = 'left join v_Address Addr with (nolock) on Addr.Address_id = ISNULL(Org.PAddress_id, Org.UAddress_id)'; //https://redmine.swan.perm.ru/issues/149322
				}else{
					$joinList[] = 'left join v_Address Addr with (nolock) on Addr.Address_id = Org.UAddress_id'; //https://redmine.swan.perm.ru/issues/68524
				}
				$joinList[] = 'left join v_KLArea Klar with (nolock) on Klar.KLArea_id = Addr.KLSubRgn_id'; //https://redmine.swan.perm.ru/issues/68524
				
				$ageGroupJoin = '';
				//when epld.DispClass_id in (3, 7, 10) and epld.{$object}_setDT >= p.Person_Birthday then datediff(MONTH, p.Person_Birthday, epld.{$object}_setDT)
				/*$ageObsledSelect = 'case
						when epld.EvnPLDispOrp_setDT >= p.Person_Birthday then datediff(MONTH, p.Person_Birthday, epld.EvnPLDispOrp_setDT)
						else null
					 end as ageObsled';*/
			...
			case 10:
				...
				$ageGroupJoin = '
				left join v_PersonDispOrp POrp (nolock) on POrp.PersonDispOrp_id = epld.PersonDispOrp_id
				left join v_AgeGroupDisp agd with(nolock) on agd.AgeGroupDisp_id = ISNULL(epld.AgeGroupDisp_id, POrp.AgeGroupDisp_id)
				';
				$ageObsledSelect = "case
						when epld.DispClass_id in (10) and epld.EvnPLDispTeenInspection_setDT >= p.Person_Birthday then (ISNULL(agd.AgeGroupDisp_From,0)*12 + ISNULL(agd.AgeGroupDisp_monthFrom,0))
						else null
					 end as ageObsled,
					 ISNULL(agd.AgeGroupDisp_From,0)*12 + ISNULL(agd.AgeGroupDisp_monthFrom,0) as MonthsObsled,
					 case when agd.AgeGroupDisp_From >= 10 then 1 else 0 end as Older10
					 ";
				//$fieldsList[] = "case when ISNULL(ostac.OrgStac_Code,0)=0 then null else ostac.OrgStac_Code end as idEducationOrg";
				...
				if($data['DispClass_id'] == 10)
				{
					...
					$fieldsList[] = "case when ISNULL(ostac.OrgStac_Code,0)=0 then Klar.KLAdr_Code else null end as kladrDistr"; //https://redmine.swan.perm.ru/issues/68524
                ...
                {
                    $fieldsList[] = "case when (ISNULL(ostac.OrgStac_Code,0)=0 and Klar.KLAdr_Code is not null) then EIT.EducationInstitutionType_Code else null end as idEducType";
                }
				else
				{
                    $fieldsList[] = "case when (ISNULL(ostac.OrgStac_Code,0)=0 and Klar.KLAdr_Code is not null) then EIT.EducationInstitutionType_Code else null end as idEducType";
				...
				if($data['DispClass_id'] == 10)
				{
					...
					$fieldsList[] = "case when ISNULL(ostac.OrgStac_Code,0)=0 then Org.Org_Name else null end as educOrgName"; //https://redmine.swan.perm.ru/issues/68524
				/*$cat_ch_type = $data['DispClass_id'];
				if($data['DispClass_id'] == 6)
					$cat_ch_type = 8;*/

				//https://redmine.swan.perm.ru/issues/69036:
                $joinList[] = "left join v_PersonDispOrp PDOrp with (nolock) on PDOrp.PersonDispOrp_id = epld.PersonDispOrp_id";
				$joinList[] = 'left join fed.v_OrgStac ostac with (nolock) on ostac.Org_id = ISNULL(epld.Org_id,PDOrp.Org_id)'; //https://redmine.swan.perm.ru/issues/69036
				$joinList[] = 'left join v_Org Org with (nolock) on Org.Org_id = ISNULL(epld.Org_id,PDOrp.Org_id)'; //https://redmine.swan.perm.ru/issues/68524
				$joinList[] = 'left join v_Address Addr with (nolock) on Addr.Address_id = Org.UAddress_id'; //https://redmine.swan.perm.ru/issues/68524
				//$joinList[] = 'left join v_KLArea Klar with (nolock) on Klar.KLArea_id = ISNULL(Addr.KLSubRgn_id, Addr.KLRgn_id)';
                $joinList[] = 'left join v_KLArea Klar with (nolock) on Klar.KLArea_id = COALESCE(Addr.KLSubRgn_id, Addr.KLTown_id, Addr.KLCity_id, Addr.KLRgn_id)';
                if($data['DispClass_id'] == 9)
                {
                    $joinList[] = 'left join v_EducationInstitutionClass EIC with (nolock) on EIC.EducationInstitutionClass_id = epld.EducationInstitutionClass_id';
                    $joinList[] = 'left join v_EducationInstitutionType EIT with (nolock) on EIT.EducationInstitutionType_id = EIC.EducationInstitutionType_id';
                }
                else
                {
                    $joinList[] = 'left join v_EducationInstitutionType EIT with (nolock) on EIT.EducationInstitutionType_id = PDOrp.EducationInstitutionType_id';
                }
			...
			case 6:
				$joinList[] = "outer apply (
					...
					where Person_id = p.Person_id
						...
						and isnull(PersonCard_endDate, '2030-01-01') >= dbo.tzGetdate()
				) pc";
				$joinList[] = 'left join v_Lpu l with (nolock) on l.Lpu_id = pc.Lpu_id';
			break;

			case 7:
			case 9:
			case 10:
				$joinList[] = 'left join v_Lpu l with (nolock) on l.Lpu_id = epld.Lpu_id';
			break;
		}
		...
		if ( !empty($data['EvnPLDisp_IsPaid']) ) {
			$filterList[] = "ISNULL(epld.{$object}_IsPaid, 1) = :EvnPLDisp_IsPaid";
		...
		$query = "
			select
				...
				,p.Sex_id as idSex
				,CONVERT(varchar(10), p.Person_BirthDay, 120) as dateOfBirth
				,case
				...
				--,dt.DocumentType_Code as idDocument
				,case
				...
				,case
					when p.Person_Snils is not null and len(p.Person_Snils) = 11
					then LEFT(p.Person_Snils, 3) + '-' + SUBSTRING(p.Person_Snils, 4, 3) + '-' + SUBSTRING(p.Person_Snils, 7, 3) + '-' + RIGHT(p.Person_Snils, 2)
					else null
				 end as snils
				,CASE
					when p.Person_Snils is not null and len(p.Person_Snils) = 11 THEN null
				...
				,LOWER(ISNULL(pklc.KLCity_AOID, uklc.KLCity_AOID)) as fiasAoid -- Код города проживания по ФИАС (# 160177 использовать строчные буквы)
				--,LOWER(ISNULL(pklc.KLCity_AOGUID, uklc.KLCity_AOGUID)) as fiasAoid -- Код города проживания по ФИАС (# 163794 теперь снова AOID)
				...
				--,'' as dateOrphHabitation
				,convert(varchar(10), cast(PDODate.PersonDispOrp_setDate as datetime),126) as dateOrphHabitation
			...
			from v_{$object} epld with (nolock)
				inner join {$tmpTableName} tmp with (nolock) on tmp.id = epld.{$object}_id
				inner join v_Person_all p with (nolock) on p.PersonEvn_id = epld.PersonEvn_id and p.Server_id = epld.Server_id
				OUTER APPLY (
					SELECT TOP 1 KLCountry.KLCountry_Code AS country_code
					FROM v_PersonState vper with (nolock)
						...
						LEFT join KLCountry KLCountry with (nolock) ON KLCountry.KLCountry_id = ns.KLCountry_id
					WHERE vper.Person_id = p.Person_id
				) country_code
				outer apply (
					select top 1 CategoryChildType_id
					from v_PersonDispOrp with (nolock)
					where PersonDispOrp_Year = YEAR(epld.{$object}_setDT)
						and Person_id = p.Person_id
				) pdo
				outer apply (
					select top 1 PersonDispOrp_setDate
					from v_PersonDispOrp with (nolock)
					where Person_id = p.Person_id
					order by PersonDispOrp_id DESC
				) PDODate
				" . (count($joinList) > 0 ? implode(' ', $joinList) : "") . "
				left join v_CategoryChildType cct with (nolock) on cct.CategoryChildType_id = pdo.CategoryChildType_id
				...
				LEFT JOIN v_PersonState vper with (nolock) ON vper.Person_id = epld.Person_id
				left join v_Polis pls with (nolock) on pls.Polis_id = ISNULL(p.Polis_id, vper.Polis_id)
				left join v_OrgSMO os with (nolock) on os.OrgSmo_id = pls.OrgSmo_id
				left join v_OrgSMO OSN with (nolock) on OSN.Org_id = os.Org_nid
				left join v_Address a with (nolock) on a.Address_id = ISNULL(p.PAddress_id, p.UAddress_id)
				left join v_KLArea klnp with (nolock) on klnp.KLArea_id = COALESCE(a.KLTown_id, a.KLCity_id, a.KLRgn_id)
				left join v_KLStreet kls with (nolock) on kls.KLStreet_id = a.KLStreet_id
				left join v_Address la with (nolock) on la.Address_id = l.UAddress_id
				--left join v_PersonState PState with (nolock) on PState.Person_id = p.Person_id
				left join v_Address pa (nolock) on pa.Address_id = p.PAddress_id
				left join v_Address ua with (nolock) on ua.Address_id = p.UAddress_id
		...
		foreach ( $response as $card ) {
			...
			/*
			 * перенес в запрос
			if(in_array(getRegionNick(), array('buryatiya')) && !empty($card['fiasAoid'])){
				// # 160177 использовать строчные буквы
				$card['fiasAoid'] = mb_strtolower($card['fiasAoid']);
			}
			*/
		...
		if ( count($cardIds) > 0 ) {
			...
			$query = "
				SET NOCOUNT ON;
				IF OBJECT_ID(N'tempdb..#solberg', N'U') IS NOT NULL
					DROP TABLE #solberg;
				select
					...
					,evdd.Diag_id
				into #solberg
				from v_{$vizitObject} evdd with (nolock)
					inner join {$tmpTableName} tmp with (nolock) on tmp.id = evdd.{$vizitObject}_pid
				...
				select
					...
					,CONVERT(varchar(10), epld.{$object}_setDT, 120) as dateOfObsled
					,{$ageObsledSelect}
					--,datediff(MONTH, p.Person_Birthday, epld.{$object}_setDT) as personAge
					...
					,FLOOR(ISNULL(ah.AssessmentHealth_Head, 1)) as headSize
					,case
					...
					,NULLIF(isnull(ah.AssessmentHealth_Years, 0) * 12 + isnull(ah.AssessmentHealth_Month, 0), 0) as menarhe
					,case
						when ISNULL(ah.AssessmentHealth_IsRegular, 1) = 2 then 1
						when ISNULL(ah.AssessmentHealth_IsIrregular, 1) = 2 then 2
					 end as char1
					,case
						when ISNULL(ah.AssessmentHealth_IsAbundant, 1) = 2 then 3
						when ISNULL(ah.AssessmentHealth_IsScanty, 1) = 2 then 4
						when ISNULL(ah.AssessmentHealth_IsModerate, 1) = 2 then 5
					 end as char2
					,case
						when ISNULL(ah.AssessmentHealth_IsPainful, 1) = 2 then 6
						when ISNULL(ah.AssessmentHealth_IsPainless, 1) = 2 then 7
					 end as char3
					,ISNULL(hk_prev.HealthKind_Code,hk.HealthKind_Code) as healthGroupBefore
					,case
						...
						/*else HGTBEFORE.HealthGroupType_Code*/
							...
							when HGTBEFORE.HealthGroupType_id is not null then HGTBEFORE.HealthGroupType_Code
								...
								case 
									...
									else isnull(hg_prev.HealthGroupType_Code,1)
								end
						end
					 end as fizkultGroupBefore
					,case when d.Diag_Code is not null and left(d.Diag_Code, 1) = 'Z' and (SUBSTRING(Diag_Code,2,2) between '00' and '10') then d.Diag_Code else null end as healthyMKB
					,case when it.InvalidType_Code in (2, 3) then it.InvalidType_Code - 1 end as typeInvalid
					,convert(varchar(10), ah.AssessmentHealth_setDT, 120) as dateFirstDetected
					,convert(varchar(10), ah.AssessmentHealth_reExamDT, 120) as dateLastConfirmed
					,case
					...
					,convert(varchar(10), ev.{$vizitObject}_setDT, 120) as zakluchDate
					,mp.Person_SurName as lastNameMP
					,mp.Person_FirName as firstNameMP
					,mp.Person_SecName as middleNameMP
					,case when LEN(ISNULL(ah.AssessmentHealth_HealthRecom, '')) > 0 then ah.AssessmentHealth_HealthRecom else '0' end as recommendZOZH
					,convert(varchar(10), ah.AssessmentHealth_ReabDT, 120) as dateReab
					,ret.RehabilitEndType_Code as stateReab
					,pvt.ProfVaccinType_Code as statePriv
					,ISNULL(2 - oms.YesNo_Code, 0) as oms
				from v_{$object} epld with (nolock)
					inner join {$tmpTableName} tmp with (nolock) on tmp.id = epld.{$object}_id
					inner join v_PersonState p with (nolock) on p.Person_id = epld.Person_id
					outer apply(
						select top 1 * from v_AssessmentHealth (nolock) where EvnPLDisp_id = epld.{$object}_id
					) ah
					{$ageGroupJoin}
					left join v_HeightAbnormType hct with(nolock) on hct.HeightAbnormType_id = ah.HeightAbnormType_id
					left join v_WeightAbnormType wct with(nolock) on wct.WeightAbnormType_id = ah.WeightAbnormType_id
					left join v_NormaDisturbanceType psihmot with(nolock) on psihmot.NormaDisturbanceType_id = ah.NormaDisturbanceType_id
					left join v_NormaDisturbanceType intel with(nolock) on intel.NormaDisturbanceType_id = ah.NormaDisturbanceType_uid
					left join v_NormaDisturbanceType emotveg with(nolock) on emotveg.NormaDisturbanceType_id = ah.NormaDisturbanceType_eid
					left join v_InvalidType it with(nolock) on it.InvalidType_id = ah.InvalidType_id
					left join v_InvalidDiagType idt with(nolock) on idt.InvalidDiagType_id = ah.InvalidDiagType_id
					...
					left join v_HealthGroupType HGTBEFORE with(nolock) on HGTBEFORE.HealthGroupType_id = AH.HealthGroupType_oid

					outer apply(
					    select top 1 epld_t.{$object}_id as prev_id
					    from v_{$object} epld_t (nolock)
					    left join v_AssessmentHealth ah_t (nolock) on ah_t.EvnPLDisp_id = epld_t.{$object}_id
					    where
                    ...
                    outer apply(
                        select top 1 * from v_AssessmentHealth (nolock) where EvnPLDisp_id = epld_prev.prev_id
                    ) ah_prev
                    left join v_HealthKind hk_prev with(nolock) on hk_prev.HealthKind_id = ah_prev.HealthKind_id
                    left join v_HealthGroupType hg_prev (nolock) on hg_prev.HealthGroupType_id = ah_prev.HealthGroupType_id
					outer apply(
						select top 1
							 {$vizitObject}_setDT
							,MedPersonal_id
					...
					left join v_RehabilitEndType ret with (nolock) on ret.RehabilitEndType_id = ah.RehabilitEndType_id
					left join v_ProfVaccinType pvt with(nolock) on pvt.ProfVaccinType_id = ah.ProfVaccinType_id
					left join v_YesNo oms with(nolock) on oms.YesNo_id = epld.{$object}_IsPaid
			...
			$query = "
				select
					...
					/*,case when dst.DispSurveilType_Code in (1, 2) then dst.DispSurveilType_Code else 0 end as diagBeforeDispNablud*/
				...
				from v_EvnDiagDopDisp eddd with (nolock)
					inner join {$tmpTableName} tmp with (nolock) on tmp.id = eddd.EvnDiagDopDisp_pid
					left join v_Diag d with (nolock) on d.Diag_id = eddd.Diag_id
					left join v_DispSurveilType dst with (nolock) on dst.DispSurveilType_id = eddd.DispSurveilType_id
					-- лечение
					outer apply(
						select top 1 *
						from v_MedCare (nolock)
					...
					left join v_ConditMedCareType cmctn2 with (nolock) on cmctn2.ConditMedCareType_id = mc2.ConditMedCareType_nid
					left join v_PlaceMedCareType pmctn2 with (nolock) on pmctn2.PlaceMedCareType_id = mc2.PlaceMedCareType_nid
					left join v_ConditMedCareType cmct2 with (nolock) on cmct2.ConditMedCareType_id = mc2.ConditMedCareType_id
					left join v_PlaceMedCareType pmct2 with (nolock) on pmct2.PlaceMedCareType_id = mc2.PlaceMedCareType_id
					left join v_LackMedCareType lmct2 with (nolock) on lmct2.LackMedCareType_id = mc2.LackMedCareType_id
					left join v_ConditMedCareType cmctn3 with (nolock) on cmctn3.ConditMedCareType_id = mc3.ConditMedCareType_nid
					left join v_PlaceMedCareType pmctn3 with (nolock) on pmctn3.PlaceMedCareType_id = mc3.PlaceMedCareType_nid
					left join v_ConditMedCareType cmct3 with (nolock) on cmct3.ConditMedCareType_id = mc3.ConditMedCareType_id
					left join v_PlaceMedCareType pmct3 with (nolock) on pmct3.PlaceMedCareType_id = mc3.PlaceMedCareType_id
					left join v_LackMedCareType lmct3 with (nolock) on lmct3.LackMedCareType_id = mc3.LackMedCareType_id
					left join v_HTMRecomType vmp with (nolock) on vmp.HTMRecomType_id = eddd.HTMRecomType_id
			";
			$result = $this->db->query($query, $params);
			...
			if ( !in_array($data['DispClass_id'], array(6, 9)) ) {
				...
				$query = "
					...
					from v_{$vizitObject} evdo with (nolock)
						inner join {$tmpTableName} tmp with (nolock) on tmp.id = evdo.{$vizitObject}_pid
						left join v_Diag d with (nolock) on d.Diag_id = evdo.Diag_id
						left join v_DispSurveilType dst with (nolock) on dst.DispSurveilType_id = evdo.DispSurveilType_id
						-- консультация
						outer apply(
							select top 1 *
							from v_MedCare (nolock)
						...
						left join v_ConditMedCareType cmctn1 with (nolock) on cmctn1.ConditMedCareType_id = mc1.ConditMedCareType_nid
						left join v_PlaceMedCareType pmctn1 with (nolock) on pmctn1.PlaceMedCareType_id = mc1.PlaceMedCareType_nid
						left join v_ConditMedCareType cmctn2 with (nolock) on cmctn2.ConditMedCareType_id = mc2.ConditMedCareType_nid
						left join v_PlaceMedCareType pmctn2 with (nolock) on pmctn2.PlaceMedCareType_id = mc2.PlaceMedCareType_nid
						left join v_ConditMedCareType cmctn3 with (nolock) on cmctn3.ConditMedCareType_id = mc3.ConditMedCareType_nid
						left join v_PlaceMedCareType pmctn3 with (nolock) on pmctn3.PlaceMedCareType_id = mc3.PlaceMedCareType_nid
						left join v_YesNo vmp with (nolock) on vmp.YesNo_id = evdo.{$vizitObject}_IsVMP
						left join v_YesNo ift with (nolock) on ift.YesNo_id = evdo.{$vizitObject}_IsFirstTime
			...
			$query = "
				select
					...
					,convert(varchar(10), evdo.{$vizitObject}_setDT, 120) as osmotrDate
				from v_{$vizitObject} evdo with (nolock)
					inner join {$tmpTableName} tmp with (nolock) on tmp.id = evdo.{$vizitObject}_pid
					inner join v_DopDispInfoConsent ddic with (nolock) on ddic.DopDispInfoConsent_id = evdo.DopDispInfoConsent_id
					inner join v_SurveyTypeLink stl with (nolock) on stl.SurveyTypeLink_id = ddic.SurveyTypeLink_id
					inner join v_SurveyType st with (nolock) on st.SurveyType_id = stl.SurveyType_id
					inner join v_OrpDispSpec ods with (nolock) on ods.OrpDispSpec_id = st.OrpDispSpec_id
			";
			$result = $this->db->query($query, $params);
			//echo getDebugSQL($query, $params);
				...
				case 7:
					$query = "
						select
							...
							,convert(varchar(10), eu.EvnUslugaDispOrp_setDT, 120) as basicIssledDate
							,eu.EvnUslugaDispOrp_Result as basicIssledResult
						from v_DopDispInfoConsent ddic with (nolock)
							inner join {$tmpTableName} tmp with (nolock) on tmp.id = ddic.EvnPLDisp_id
							inner join v_SurveyTypeLink stl with (nolock) on stl.SurveyTypeLink_id = ddic.SurveyTypeLink_id
							inner join v_SurveyTypeLink stl2 with (nolock) on stl2.SurveyType_id = stl.SurveyType_id
								...
								and (stl2.Sex_id is null or ISNULL(stl2.Sex_id, 0) = ISNULL(stl.Sex_id, 0))
								and ISNULL(stl2.SurveyTypeLink_From, 0) = ISNULL(stl.SurveyTypeLink_From, 0)
								and ISNULL(stl2.SurveyTypeLink_To, 0) = ISNULL(stl.SurveyTypeLink_To, 0)
							...
							inner join v_SurveyType st with (nolock) on st.SurveyType_id = stl.SurveyType_id
							inner join v_EvnUslugaDispOrp eu with (nolock) on eu.UslugaComplex_id = stl2.UslugaComplex_id
						...
						where eu.EvnUslugaDispOrp_rid = tmp.id
							...
							and (ISNULL(ddic.DopDispInfoConsent_IsAgree, 1) = 2 or ISNULL(ddic.DopDispInfoConsent_IsEarlier, 1) = 2)
							and ISNULL(stl.SurveyTypeLink_IsDel, 1) = 1
					";
				break;
				...
				case 10:
					$query = "
						select
							...
							,convert(varchar(10), eu.EvnUslugaDispDop_setDT, 120) as basicIssledDate
							,eu.EvnUslugaDispDop_Result as basicIssledResult
						from v_EvnUslugaDispDop eu with (nolock)
							inner join {$tmpTableName} tmp with (nolock) on tmp.id = eu.EvnUslugaDispDop_rid
							inner join v_EvnVizitDispDop ev with (nolock) on ev.EvnVizitDispDop_id = eu.EvnUslugaDispDop_pid
							inner join v_DopDispInfoConsent ddic with (nolock) on ddic.DopDispInfoConsent_id = ev.DopDispInfoConsent_id
							inner join v_UslugaComplex uc with (nolock) on uc.UslugaComplex_id = eu.UslugaComplex_id
							left join v_SurveyTypeLink stl with (nolock) on stl.SurveyTypeLink_id = ddic.SurveyTypeLink_id
							left join v_SurveyType st with (nolock) on st.SurveyType_id = stl.SurveyType_id
			...
			// Результаты базовых исследований
			/* if ( count($issledIdList) > 0 ) {
				$query = "
					...
					from v_EvnUslugaRate eur with (nolock)
						inner join v_Rate r with (nolock) on r.Rate_id = eur.Rate_id
						inner join v_RateType rt with (nolock) on rt.RateType_id = r.RateType_id
				...
				foreach ( $cards as $cardId => $cardData ) {
					foreach ( $cardData['issledBasic'] as $key => $array ) {
						if ( array_key_exists($array['EvnUsluga_id'], $issledResults) && !empty($issledResults[$array['EvnUsluga_id']]) ) {
							$cards[$cardId]['issledBasic'][$key]['basicIssledResult'] = $issledResults[$array['EvnUsluga_id']];
						}
					}
				}
				...
				case 7:
					$query = "
						select
							 eu.EvnUslugaDispOrp_rid as EvnUsluga_rid
							,eu.EvnUslugaDispOrp_id as EvnUsluga_id
							,convert(varchar(10), eu.EvnUslugaDispOrp_setDT, 120) as otherIssledDate
							,st.SurveyType_Name as otherIssledName
							,case when uc.UslugaComplex_Code is not null then uc.UslugaComplex_Code + ' ' else '' end
								+ ISNULL(uc.UslugaComplex_Name, '') as otherIssledResult
						from v_EvnUslugaDispOrp eu with (nolock)
							inner join {$tmpTableName} tmp with (nolock) on tmp.id = eu.EvnUslugaDispOrp_rid
							inner join v_DopDispInfoConsent ddic with (nolock) on ddic.DopDispInfoConsent_id = eu.DopDispInfoConsent_id
							inner join v_SurveyTypeLink stl with (nolock) on stl.SurveyTypeLink_id = ddic.SurveyTypeLink_id
							inner join v_SurveyType st with (nolock) on st.SurveyType_id = stl.SurveyType_id
							inner join v_UslugaComplex uc with (nolock) on uc.UslugaComplex_id = eu.UslugaComplex_id
				...
				case 10:
					$query = "
						select
							 eu.EvnUslugaDispDop_rid as EvnUsluga_rid
							,eu.EvnUslugaDispDop_id as EvnUsluga_id
							,convert(varchar(10), eu.EvnUslugaDispDop_setDT, 120) as otherIssledDate
							,st.SurveyType_Name as otherIssledName
							,case when uc.UslugaComplex_Code is not null then uc.UslugaComplex_Code + ' ' else '' end
								+ ISNULL(uc.UslugaComplex_Name, '') as otherIssledResult
						from v_EvnUslugaDispDop eu with (nolock)
		...
		//var_dump(array('child' => $children));
		/*
		function addTagsToArrayValues(&$array) {
			$fieldsMapping = array(
				 'idInternalChild' => 'idInternal'
				,'idTypeChild' => 'idType'
				,'lastName' => 'last'
				,'firstName' => 'first'
				,'middleName' => 'middle'
				,'idInternalCard' => 'idInternal'
				,'idTypeCard' => 'idType'
				...
				,'charValue' => 'char'
				,'diagBeforeMKB' => 'mkb'
				,'diagBeforeDispNablud' => 'dispNablud'
				,'diagBeforeConditionLechen' => 'condition'
				,'diagBeforeOrganLechen' => 'organ'
				,'diagBeforeReasonLechen' => 'reason'
				,'diagBeforeReasonOtherLechen' => 'reasonOther'
				,'diagBeforeConditionReab' => 'condition'
				,'diagBeforeOrganReab' => 'organ'
				,'diagBeforeReasonReab' => 'reason'
				,'diagBeforeReasonOtherReab' => 'reasonOther'
				,'diagBeforeVMP' => 'vmp'
				,'diagAfterMKB' => 'mkb'
				,'conditionLechen' => 'condition'
				,'organLechen' => 'organ'
				,'conditionReab' => 'condition'
				,'organReab' => 'organ'
				,'conditionConsul' => 'condition'
				,'organConsul' => 'organ'
				,'stateConsul' => 'state'
				,'typeInvalid' => 'type'
				,'illnessValue' => 'illness'
				,'defectValue' => 'defect'
				,'basicIssledId' => 'id'
				,'basicIssledDate' => 'date'
				,'basicIssledResult' => 'result'
				,'otherIssledDate' => 'date'
				,'otherIssledName' => 'name'
				,'otherIssledResult' => 'result'
				,'lastNameMP' => 'last'
				,'firstNameMP' => 'first'
				,'middleNameMP' => 'middle'
				,'osmotrId' => 'id'
			...
			if ( is_array($array) ) {
				foreach ( $array as $key => $val ) {
					if ( is_array($val) ) {
						addTagsToArrayValues($array[$key]);
					}
					else {
						if ( !empty($val) || $val === 0  || $val === '0' ) {
							if ( array_key_exists($key, $fieldsMapping) ) {
								$array[$key] = "<{$fieldsMapping[$key]}>" . $val . "</{$fieldsMapping[$key]}>";
							}
							else {
	...
	function getEvnPLDispInfoViewData($data) {
		$queryDispDop = "
			...
			from
				v_EvnPLDispDop EPLDD with(nolock)
			where
		...
		$queryDispDop13 = "
			...
			from
				v_EvnPLDispDop13 EPLDD13 with(nolock)
			where
		...
		$queryDispOrp = "
			...
			from
				v_EvnPLDispOrp EPLDO with(nolock)
			where
		...
		$queryDispTeen14 = "
			...
			from
				v_EvnPLDispTeen14 EPLDT14 with(nolock)
			where
		...
		$queryDispTeenInspection = "
			select
				...
				EPLDTI.EvnPLDispTeenInspection_IsFinish,
		...
		$query = "
			select
				...
				EPLD.Lpu_id,
				L.Lpu_Nick,
				convert(varchar(10), EPLD.EvnPLDisp_setDate, 104) as EvnPLDisp_setDate,
				case when EPLD.EvnPLDisp_IsFinish = 2 then convert(varchar(10), EPLD.EvnPLDisp_disDate, 104) else null end as EvnPLDisp_disDate,
				HK.HealthKind_Name,
				EPLD.Object,
				EVNU.Diag_FullName
			from
				...
				select diag.Diag_FullName from v_EvnUslugaDispDop EVNU with(nolock)
				...
				left join v_SurveyTypeLink STL with(nolock) on STL.SurveyTypeLink_id=DDIC.SurveyTypeLink_id
				where EVNU.EvnUslugaDispDop_rid = EPLD.EvnPLDisp_id and STL.SurveyType_id=19 and EVDD.DopDispDiagType_id=2 and diag.Diag_Code not like 'Z%'
				union all
				select 
				D.Diag_FullName
			from
			...
			where
				...
				left join v_DispClass DC with(nolock) on DC.DispClass_id = EPLD.DispClass_id
				left join v_Lpu L with(nolock) on L.Lpu_id = EPLD.Lpu_id
				left join v_HealthKind HK with(nolock) on HK.HealthKind_id = EPLD.HealthKind_id
			order by
	...
	function getEvnPLDispAdultViewData($data) {
		...
		if (in_array($disp_class_code,array(1,2))) {
			$object = 'EvnPLDispDop13';
			$fields .= "isnull(IsBrain.YesNo_Name,'Нет') as IsBrain,";
			$join .= "left join v_YesNo IsBrain with(nolock) on IsBrain.YesNo_id = EPLD.{$object}_IsBrain";
		} else
		...
		$query = "
			select top 1
				...
				convert(varchar(10), EPLD.{$object}_setDate, 104) as EvnPLDisp_setDate,
				case when EPLD.{$object}_IsFinish = 2 then convert(varchar(10), EPLD.{$object}_disDate, 104) else '' end as EvnPLDisp_disDate,
				EPLD.{$object}_IsDisp as EvnPLDisp_IsDisp,
				isnull(IsDisp.YesNo_Name,'Нет') as IsDisp,
				{$fields}
				EPLD.{$object}_IsSanator as EvnPLDisp_IsSanator,
				isnull(IsSanator.YesNo_Name,'Нет') as IsSanator,
				EPLD.Lpu_id,
				L.Lpu_Nick,
				EPLD.DispClass_id,
				DC.DispClass_Code,
			...
			from
				...
				left join v_Lpu L with(nolock) on L.Lpu_id = EPLD.Lpu_id
				left join v_HealthKind HK with(nolock) on HK.HealthKind_id = EPLD.HealthKind_id
				left join v_YesNo IsDisp with(nolock) on IsDisp.YesNo_id = EPLD.{$object}_IsDisp
				left join v_YesNo IsSanator with(nolock) on IsSanator.YesNo_id = EPLD.{$object}_IsSanator
				{$join}
				left join v_NeedDopCure NDC with(nolock) on NDC.NeedDopCure_id = EPLD.NeedDopCure_id
				outer apply(
					select top 1 Post.name as Dolgnost_Name, MedP.Person_Fio as MedPerson_Fio
					from v_EvnVizitDisp EVD with(nolock)
						left join v_DopDispInfoConsent DDIC with (nolock) on DDIC.DopDispInfoConsent_id = EVD.DopDispInfoConsent_id
						left join v_SurveyTypeLink STL with (nolock) on STL.SurveyTypeLink_id = DDIC.SurveyTypeLink_id
						left join v_SurveyType ST with (nolock) on ST.SurveyType_id = STL.SurveyType_id
						left join v_MedStaffFact MedP with (nolock) on MedP.MedStaffFact_id = EVD.MedStaffFact_id
						left join persis.Post Post with (nolock) on Post.id = MedP.Post_id
	...
	function getEvnPLDispChildViewData($data) {
		...
		$query = "
			...
			from
				v_{$object} EPLD with(nolock)
				inner join v_Person_all P with(nolock) on P.PersonEvn_id = EPLD.PersonEvn_id
					and P.Server_id = EPLD.Server_id
				inner join v_DispClass DC with(nolock) on DC.DispClass_id = EPLD.DispClass_id
				left join v_Lpu L with(nolock) on L.Lpu_id = EPLD.Lpu_id
				left join v_Sex S with(nolock) on S.Sex_id = P.Sex_id
				left join v_AssessmentHealth AH with(nolock) on AH.EvnPLDisp_id = EPLD.{$object}_id
				left join v_WeightAbnormType WCT with(nolock) on WCT.WeightAbnormType_id = AH.WeightAbnormType_id
				left join v_HeightAbnormType HCT with(nolock) on HCT.HeightAbnormType_id = AH.HeightAbnormType_id
				left join v_NormaDisturbanceType Psych with(nolock) on Psych.NormaDisturbanceType_id = AH.NormaDisturbanceType_id
				left join v_NormaDisturbanceType Intelligence with(nolock) on Intelligence.NormaDisturbanceType_id = AH.NormaDisturbanceType_uid
				left join v_NormaDisturbanceType Emotion with(nolock) on Emotion.NormaDisturbanceType_id = AH.NormaDisturbanceType_eid
				left join v_HealthKind HK with(nolock) on HK.HealthKind_id = AH.HealthKind_id
				left join v_HealthGroupType HGT with(nolock) on HGT.HealthGroupType_id = AH.HealthGroupType_id
				outer apply(
					select top 1 Post.name as Dolgnost_Name, MedP.Person_Fio as MedPerson_Fio
	...
	function getEvnVizitDispViewData($data) {
		$query = "
			select
				EVD.EvnVizitDisp_id,
				convert(varchar(10), EVD.EvnVizitDisp_setDate, 104) as EvnVizitDisp_setDate,
				D.Diag_id,
				D.Diag_Code,
				D.Diag_Name,
			...
			from
				v_EvnVizitDisp EVD with(nolock)
				inner join v_Diag D with(nolock) on D.Diag_id = EVD.Diag_id
				left join v_YesNo IsFirstTime with(nolock) on IsFirstTime.YesNo_id = EVD.EvnVizitDisp_IsFirstTime
				left join v_DispSurveilType DST with(nolock) on DST.DispSurveilType_id = EVD.DispSurveilType_id
				left join v_YesNo IsVmp with(nolock) on IsVmp.YesNo_id = EVD.EvnVizitDisp_IsVmp
			where
				EVD.EvnVizitDisp_pid = :EvnPLDisp_id
				and Diag_Code not like 'Z%'
		";
	...
	function getDispDopInfoConsentPrescrData($data) {
		...
		if (!empty($data['DopDispInfoConsent_id'])) {
			...
			$join = "
				inner join v_EvnPrescrDirection epd (nolock) on epd.EvnPrescr_id = EP.EvnPrescr_id
				inner join v_EvnDirection_all ED (nolock) on ED.EvnDirection_id = epd.EvnDirection_id and ED.EvnStatus_id not in (12,13)
			";
		}
		...
		$result = $this->queryResult("
			select
				...
				case
					...
					then isnull(LS.LpuSection_Name,'') +' / '+ isnull(Lpu.Lpu_Nick,'') +' / '+ isnull(convert(varchar(10), TTG.TimetableGraf_begTime, 104),'')+' '+isnull(convert(varchar(5), TTG.TimetableGraf_begTime, 108),'')
					when EQ.EvnQueue_id is not null then
						case
							when LS.LpuSection_id is not null
							then isnull(LS.LpuSection_Name,'')
							else isnull(LSPD.LpuSectionProfile_Name,'')
						end +' / '+ isnull(Lpu.Lpu_Nick,'')
				else '' end as RecTo,
				case
					when TTG.TimetableGraf_id is not null then 'Записано '+ isnull(convert(varchar(10), TTG.TimeTableGraf_updDT, 104),'')+' '+isnull(convert(varchar(5), TTG.TimeTableGraf_updDT, 108),'')
					when EQ.EvnQueue_id is not null then 'В очереди с '+ isnull(convert(varchar(10), EQ.EvnQueue_setDate, 104),'')
				else '' end as RecDate,
				case
				...
				ED.DopDispInfoConsent_id,
				ED.EvnDirection_id as EvnPrescrDispDop_id,
				ED.EvnDirection_id
			from
				v_EvnDirection ED with (nolock)
				outer apply (
					Select top 1 TimetableGraf_id, TimetableGraf_begTime, TimeTableGraf_updDT from v_TimetableGraf_lite TTG with (nolock) where TTG.EvnDirection_id = ED.EvnDirection_id
				) TTG
				outer apply (
					Select top 1 EQ.EvnQueue_id, EQ.LpuUnit_did, EQ.LpuSectionProfile_did, Lpu_id, EQ.EvnQueue_setDate, EQ.EvnQueue_failDT, EQ.QueueFailCause_id
					from v_EvnQueue EQ with (nolock)
				...
				left join v_LpuUnit LU with (nolock) on coalesce(ED.LpuUnit_did,EQ.LpuUnit_did) = LU.LpuUnit_id
				left join v_LpuSectionProfile LSPD with (nolock) on coalesce(ED.LpuSectionProfile_id,EQ.LpuSectionProfile_did,LS.LpuSectionProfile_id) = LSPD.LpuSectionProfile_id
				left join v_Lpu Lpu with (nolock) on Lpu.Lpu_id = coalesce(ED.Lpu_did, LS.Lpu_id, EQ.Lpu_id)
			where {$filter}", $params);
		...
		$query = "
			...
			from
				...
				outer apply(
					...
					from v_MedCare MC (nolock)
						left join v_ConditMedCareType nCMCT with(nolock) on nCMCT.ConditMedCareType_id = MC.ConditMedCareType_nid
						left join v_PlaceMedCareType nPMCT with(nolock) on nPMCT.PlaceMedCareType_id = MC.PlaceMedCareType_nid
	...
	function checkXmlDataOnErrors(&$children, $dispClass) {
		...
		$childRules = array(
			...
			,'snils' => array('type' => 'string', 'allowEmpty' => true, 'name' => 'Номер СНИЛС (snils)') //dz //Добавлено 'allowEmpty' => true - https://redmine.swan-it.ru/issues/182612 
	...
	function loadEvnPLDispList($data) {
		$query = "
			select
				...
				,convert(varchar(10), epld.EvnPLDisp_setDT, 104) + '    ' + ISNULL(l.Lpu_Nick, '') as EvnPLDisp_Name
			from v_EvnPLDisp epld with (nolock)
				inner join v_Lpu l with (nolock) on l.Lpu_id = epld.Lpu_id
			where epld.DispClass_id = :DispClass_id
	...
	*/
	...
	public function getSurveyTypesByDispClass($data) {
		...
		$query = "
			...
			select distinct ST.SurveyType_id
			from SurveyTypeLink stl with (nolock)
				inner join v_SurveyType st with (nolock) on st.SurveyType_id = stl.SurveyType_id
			where ISNULL(stl.Region_id, @Region_id) = @Region_id
	...
	function getEvnPLDispPrintData($data) {
		...
		$query = "
			select top 1
				...
				left(ah.AssessmentHealth_Weight, 4) as AssessmentHealth_Weight,
				ah.AssessmentHealth_Height,
				ua.Address_Nick as UAddress,
				pa.Address_Nick as PAddress
			from v_EvnPLDisp epld (nolock)
			left join v_DispClass dc (nolock) on dc.DispClass_id = epld.DispClass_id
			left join v_PersonState_all ps (nolock) on ps.Person_id = epld.Person_id
			left join v_AssessmentHealth ah (nolock) on ah.EvnPLDisp_id = epld.EvnPLDisp_id
			left join v_PersonUAddress ua (nolock) on ua.Address_id = ps.UAddress_id
			left join v_PersonPAddress pa (nolock) on pa.Address_id = ps.PAddress_id
		...
		if (!empty($output) && !empty($output[0])) {
			...
			$query = "
				select
					...
					xmlData.EvnXml_Name,
					xmlData.EvnXml_Data,
					xmlData.XmlTemplate_Settings,
					xmlData.XmlTemplate_HtmlTemplate,
					xmlData.XmlTemplate_Data,
					xmlData.XmlTemplate_id,
					xmlData.XmlTemplateType_id,
					xmlData.Evn_id,
					xmlData.Evn_pid,
					xmlData.Evn_rid,
					xmlData.EvnClass_id
				...
				left join v_EvnVizitDispDop evdd (nolock) on evdd.DopDispInfoConsent_id = ddic.DopDispInfoConsent_id
				outer apply(
					select
						Evn.Evn_id,
						Evn.Evn_pid,
	...
	function Refuse($data) {
		$sql = "
			...
			EXEC	p_DispRefuse_ins
					@DispRefuse_id = @DispRefuse_id OUTPUT,
					@Person_id = :Person_id,
					@DispRefuse_setDT = @curdate,
			...
			SELECT	@DispRefuse_id as DispRefuse_id,
					convert(varchar(10), @curdate, 104) as DispRefuse_setDT,
        ...
        if ( !is_object($result) ) {
			return array('Error_Msg' => 'Ошибка при выполнении запроса к базе данных (сохранение отказа)');
		} else {
			$result = $result->result('array');
			return array('Error_Msg' => '', 'data' => $result[0]);
		}
	}
