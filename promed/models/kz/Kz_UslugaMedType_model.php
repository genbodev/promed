...
require_once(APPPATH . 'models/UslugaMedType_model.php');
...
class Kz_UslugaMedType_model extends UslugaMedType_model
{
    ...
    {
        $query = $this->queryResult("SELECT UslugaMedTypeLink_id FROM r101.UslugaMedTypeLink with(nolock) WHERE Evn_id=:Evn_id", [
            'Evn_id' => $data['Evn_id'],
        ]);
        ...
        foreach ($query as $v) {
            $query_delete = $this->queryResult("
                ...
                EXEC r101.p_UslugaMedTypeLink_del
        ...
        $sql = "
            ...
            EXEC r101.p_UslugaMedTypeLink_ins
                @UslugaMedTypeLink_id = @Res output,
            ...
            SELECT @Res as UslugaMedTypeLink_id, @Error_Code as Error_Code, @Error_Message as Error_Msg;
        ";
    ...
    {
        $sql = "SELECT UslugaMedType_id FROM r101.v_UslugaMedTypeLink with(nolock) WHERE Evn_id=:Evn_id";
    ...
    private function getUslugaMedTypeIdValue($data, $default=1400)
    {
        if (empty($data['UslugaMedType_id'])) {
            $sql = "SELECT UslugaMedType_id FROM r101.v_UslugaMedType with(nolock) WHERE UslugaMedType_Code=:UslugaMedType_Code";
