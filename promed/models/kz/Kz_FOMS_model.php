 ...
 *
 */
require_once(APPPATH.'models/FOMS_model.php');
	...
	function doRequestAuto() {
		...
		$response = $this->queryResult("
			declare @tomorrow int = (
				select day_id from v_Day (nolock) 
				where day_date = dateadd(day, 1, cast(dbo.tzGetdate() as date))
			) + 1;
			select top {$limit} 
				P.Person_IsInFOMS,
				PS.Person_Inn,
			...
			from v_Person P with (nolock)
				inner join v_PersonState PS with (nolock) on P.Person_id = PS.Person_id
				left join v_TimeTableGraf_lite ttg (nolock) on P.Person_id = ttg.Person_id and ttg.TimeTableGraf_Day = @tomorrow
			where P.Person_IsInFOMS is null and PS.Person_Inn is not null
