...
require_once(APPPATH.'models/ServiceRPN_model.php');
...
class Kz_ServiceRPN_model extends ServiceRPN_model
{
	...
	function generateAddressData($addressText, $needAddressText = true) {
			...
			switch($partName) {
				case 'РЕСПУБЛИКА':
					$addressParams['KLCountry_id'] = $this->getFirstResultFromQuery("
						select top 1 KLCountry_id
						from v_KLCountry with(nolock)
				...
				default:
					$query = "
						select top 1
							Area.KLArea_id,
							Area.KLSocr_id,
							Level.KLAreaLevel_Code,
							Level.KLAreaLevel_SysNick
						from
							v_KLArea Area with(nolock)
							inner join {$this->scheme}.v_KZKLSocrLink SocrLink with(nolock) on SocrLink.KLSocr_id = Area.KLSocr_id
							inner join {$this->scheme}.v_KZKLSocr kzSocr with(nolock) on kzSocr.KZKLSocr_id = SocrLink.KZKLSocr_id
							inner join v_KLAreaLevel Level with(nolock) on Level.KLAreaLevel_id = Area.KLAreaLevel_id
						where
							Area.KLCountry_id = :KLCountry_id
							and (:KLArea_pid is null or Area.KLArea_pid = :KLArea_pid)
							and Area.KLArea_Name like :KLArea_Name
							and kzSocr.KZKLSocr_Name like :KLSocr_Name
					";
					$queryParams = array(
					...
					if ($KLAreaInfo) {
						...
						$query = "
							select top 1
								...
								Street.KLSocr_id
							from
								v_KLStreet Street with(nolock)
								inner join v_KLSocr Socr with(nolock) on Socr.KLSocr_id = Street.KLSocr_id
							where
								(:KLArea_id is null or Street.KLArea_id = :KLArea_id)
								and Street.KLStreet_Name like :KLArea_Name
								and Socr.KLSocr_Name like :KLSocr_Name
						";
						$queryParams = array(
		...
		if ($needAddressText) {
			$address_address = $this->getFirstResultFromQuery("
				select top 1 dbo.Address_Compose(
					:KLCountry_id,
					:KLRgn_id,
					:KLSubRgn_id,
					:KLCity_id,
		...
		if ($socStatus) {
			...
			if ($socStatusLink) {
				...
				$SocStatus_id = $this->getFirstResultFromQuery("
					...
					and :onDate between isnull(SocStatus_begDT,:onDate) and  isnull(SocStatus_endDT,:onDate)
				", array(
		...
		if ($personlink) {
			...
			$query = "
				...
				from
					...
					left join v_PersonNationalityStatus PNS with (nolock) on P.Person_id = PNS.Person_id
			...
			$query = "
				select top 1 PE.Person_id
				from v_Person_all PE with(nolock)
				inner join Person P with(nolock) on P.Person_id = PE.Person_id
				where P.BDZ_id = :BDZ_id or (
					(
						:Person_SurName is null and Person_SurName is null
						or Person_SurName like :Person_SurName
					)
					and (
						:Person_FirName is null and Person_FirName is null
						or Person_FirName like :Person_FirName
					)
					and (
						:Person_SecName is null and Person_SecName is null
						or Person_SecName like :Person_SecName
					)
					and (
			...
			if (!$Person_id) {
				...
				if (!empty($person->national)) {
					...
					$Ethnos_id = $this->getFirstResultFromQuery("
						select top 1 Ethnos_id
						from v_Ethnos with(nolock)
					...
					if ($Ethnos_id) {
						...
						$query = "
						   ...
						   exec p_PersonInfo_ins
							   ...
							   @Error_Code = @ErrCode output,
							   @Error_Message = @ErrMsg output;
						   select @Res as PersonInfo_id, @ErrMsg as Error_Msg, @ErrCode as Error_Code
						";
						$resp = $this->queryResult($query, $queryParams);
	...
	/*function importSocialStatusRPN($data, $record) {
		...
		$query = "
			...
			exec {$proc}
				...
				@Error_Code = @ErrCode output,
				@Error_Message = @ErrMsg output;
			select @Res as PersonSocStatus_id, @ErrCode as Error_Code, @ErrMsg as Error_Msg;
		";
	}*/
	...
	function importPersonCard($data, $rpnPerson_id, $personCard, &$stat) {
		...
		$LpuRegionType_id = $this->getFirstResultFromQuery(
			'select top 1 LpuRegionType_id from v_LpuRegion with (nolock) where LpuRegion_id = :LpuRegion_id',
		...
		if (!isset($mo['Object_id'])) {
			...
			if (!empty($r->orgHealthCare->name)) {
				$Lpu_id = $this->getFirstResultFromQuery(
					'select top 1 Lpu_id from v_Lpu with (nolock) where Lpu_Name = :Lpu_Name',
				...
				if (($PersonCard['LpuRegion_id']!=$rpnData['LpuRegion_id']) ||
					...
					if ($rpnData['PersonCard_begDate']!=$PersonCard['PersonCard_begDate'] &&
						...
						$query = "
							...
							update PersonCard with(rowlock)
							...
							update PersonCardState with(rowlock)
							set PersonCardState_begDate = :PersonCard_begDate,
								Server_id = :Server_id,
								pmUser_updID = :pmUser_id,
								PersonCardState_updDT = @dt
							where PersonCard_id = :PersonCard_id
						";
		...
		if ($insert) { // Заносим новое прикрепление
			...
			$resp = $this->getFirstRowFromQuery("
				select top 1
					...
					L.Lpu_Nick,
					isnull(PS.Person_SurName,'')+isnull(' '+PS.Person_FirName,'')+isnull(' '+PS.Person_SecName,'')+isnull(' '+PS.PersonInn_Inn,'') as Person
				from
				...
				from
					...
					cross apply(
						select top 1
							Object_sid
						from
							v_ObjectSynchronLog with(nolock)
						where
			...
			foreach($elementSetList as $elementSet) {
				...
				if ((empty($ad['KLSubRgn_id']) || empty($ad['KLCity_id'])) && !empty($elementSet['rgnChildName'])) {
					$rgnChild = $this->getFirstRowFromQuery("
						...
						from
							v_KLArea Area with(nolock)
							inner join v_KZKLSocrLink SocrLink with(nolock) on SocrLink.KLSocr_id = Area.KLSocr_id
							inner join v_KLAreaLevel Level with(nolock) on Level.KLAreaLevel_id = Area.KLAreaLevel_id
						where
				...
				if (empty($ad['KLTown_id']) && !empty($elementSet['townName'])) {
					$town = $this->getFirstRowFromQuery("
						...
						from
							v_KLTown Town with(nolock)
							inner join v_KZKLSocrLink SocrLink with(nolock) on SocrLink.KLSocr_id = Town.KLSocr_id
						where
				...
				if (empty($ad['KLStreet_id']) && !empty($elementSet['streetName'])) {
					$street = $this->getFirstRowFromQuery("
						...
						from
							v_KLStreet Street with(nolock)
							inner join v_KZKLSocrLink SocrLink with(nolock) on SocrLink.KLSocr_id = Street.KLSocr_id
						where
		...
		foreach($parsedAddresses as $item) {
			...
			$LpuRegionStreet = $this->getFirstRowFromQuery("
				...
				where
					...
					and isnull(LRS.KLTown_id,0) = isnull(:KLTown_id,0)
					and isnull(LRS.KLStreet_id,0) = isnull(:KLStreet_id,0)
			", array(
	...
	function getLpuRegionList($data) {
		...
		$query = "
			select
				...
				LpuRegion.LpuRegion_Name, 
				LpuRegionType_Name 
			from v_LpuRegion LpuRegion with (nolock)
			where
				Lpu_id = :Lpu_id and 
				not exists(Select top 1 1 from ObjectSynchronLog  with (nolock) where Object_Name = 'LpuRegion' and ObjectSynchronLogService_id=2 and Object_id = LpuRegion.LpuRegion_id)";
	...
	function createPersonResponseData($person) {
		...
		if (!empty($person->citizen)) {
			$KLCountry_id = $this->getFirstResultFromQuery("
				select top 1 KLCountry_id
				from r101.CitizenshipLink with(nolock)
				where p_ID = :p_ID
			", array(
				'p_ID' => $person->citizen
			));
		...
		try {
			$query = "
				...
				where
					...
					/*and exists(
						select * from ObjectSynchronLog with(nolock)
	...
	function saveObject($objectData, $objectName, $idField = null, $allowEmptyId = false) {
		...
		if (empty($objectData[$objectName.'_id'])) {
			$objectData[$objectName.'_id'] = $this->getFirstResultFromQuery("
				declare @id bigint = (select top 1 max({$objectName}_id) from {$this->scheme}.{$objectName} with(nolock))
				select isnull(@id,0)+1
			");
		}
		...
		$query = "
			declare
				...
				@Error_Message varchar(4000);
			set @{$idField} = :{$idField}
			set @id = isnull(:{$objectName}_id, (
				select top 1 {$objectName}_id as id
				from {$this->scheme}.{$objectName} with(nolock)
				where {$idField} = @{$idField}
			))
			if @id is null or not exists(select * from {$this->scheme}.{$objectName} where {$objectName}_id = @id)
			exec {$this->scheme}.p_{$objectName}_ins
				@{$objectName}_id = @id output,
				{$execPartParamsStr},
				@Error_Code = @Error_Code output,
				@Error_Message = @Error_Message output
			else
			exec {$this->scheme}.p_{$objectName}_upd
				...
				@Error_Code = @Error_Code output,
				@Error_Message = @Error_Message output
			select @id as {$objectName}_id, @Error_Code as Error_Code, @Error_Message as Error_Msg
		";
	...
	function saveGetBuildingGetTerrService($data) {
		...
		$query = "
			declare
				...
				@Res bigint;
			set @Res = (
				select top 1 GetBuildingGetTerrService_id
				from {$this->scheme}.v_GetBuildingGetTerrService with(nolock)
				where GetBuilding_id = :GetBuilding_id and GetTerrService_id = :GetTerrService_id
			);
			if @Res is null
			exec {$this->scheme}.p_GetBuildingGetTerrService_ins
				@GetBuildingGetTerrService_id = @Res output,
				@GetBuilding_id = :GetBuilding_id,
				@GetTerrService_id = :GetTerrService_id,
				@pmUser_id = :pmUser_id,
				@Error_Code = @Error_Code output,
				@Error_Message = @Error_Message output;
			select @Res as GetBuildingGetTerrService_id, @Error_Code as Error_Code, @Error_Message as Error_Msg
		";
	...
	function saveGetAddressTerr($AddressTerr, $pmUser_id, $GetTerrService_id = null) {
		foreach ($AddressTerr as $addressElement) {
			$socr = $this->getFirstResultFromQuery("
				select top 1 KZSocr.KZKLSocr_Name
	...
	function importMissedGetAddressTerr($data) {
		$query = "
			select distinct t.GetAddressTerr_pid
			from {$this->scheme}.v_GetAddressTerr t with(nolock)
			where t.GetAddressTerr_pid is not null
			and not exists(
				select r.GetAddressTerr_id
				from {$this->scheme}.v_GetAddressTerr r with(nolock)
		...
		if (count($addresses) > 0) {
			...
			$resp = $this->queryResult("
				select GetAddressTerr_id
				from {$this->scheme}.v_GetAddressTerr with(nolock)
	...
	function saveAttachmentRequest($data) {
		...
		$rpnPerson_id = $this->getFirstResultFromQuery("
			select top 1 BDZ_id from v_Person with(nolock) where Person_IsInErz = 2 and Person_id = :Person_id
		...
		$addressData = $this->getFirstRowFromQuery("
			select top 1
				...
				Address_Flat,
				Address_Address
			from v_Address with(nolock) where Address_id = :Address_id
		", array(
		...
		if (!empty($attachlink)) {
			...
			$GetAttachment_id = $this->getFirstResultFromQuery("
				declare @id bigint = (select top 1 max(GetAttachment_id) from {$this->scheme}.GetAttachment with(nolock))
				select isnull(@id,0)+1
			");
		}
	...
	function sendAttachmentRequestToRpn($data) {
		...
		$attachment = $this->getFirstRowFromQuery("
			select top 1
				...
				convert(varchar(10), GA.GetAttachment_begDate, 120) as beginDate,
				isnull(GA.GetAttachment_IsCareHome, 0) as careAtHome,
			...
			from
				{$this->scheme}.v_GetAttachment GA with(nolock)
	...
	function getAttachmentRequestStatus($data) {
		...
		$request = $this->getFirstRowFromQuery("
			select top 1
				GA.Person_id,
				GA.GetTerrService_id,
				GA.GetAttachment_RPNid,
				GA.GetAttachment_Number,
				convert(varchar(10), GA.GetAttachment_begDate, 120) as GetAttachment_begDate,
				convert(varchar(10), GA.GetAttachment_endDate, 120) as GetAttachment_endDate,
				GA.GetAttachmentCase_id,
				GAS.GetAttachmentStatus_id,
				GAS.GetAttachmentStatus_Code
			from
				{$this->scheme}.v_GetAttachment GA with(nolock)
				left join {$this->scheme}.v_GetAttachmentStatus GAS with(nolock) on GAS.GetAttachmentStatus_id = GA.GetAttachmentStatus_id
			where GA.GetAttachment_id = :GetAttachment_id
		...
		foreach($attachments as $attachment) {
			if ($attachment->ParentID == $request['GetAttachment_RPNid']) {
				...
				$GetAttachmentStatus_Code = $this->getFirstResultFromQuery("
					select top 1 GetAttachmentStatus_Code
					from {$this->scheme}.v_GetAttachmentStatus with(nolock)
	...
	function syncLpuRegions($data) {
		...
		$query = "
			with LpuList as (
				...
				from v_Lpu L with(nolock)
				inner join v_ObjectSynchronLog OSL with(nolock) on OSL.Object_id = L.Lpu_id
					and OSL.ObjectSynchronLogService_id = :Service_id and OSL.Object_Name = 'Lpu'
			)
			select
				LL.Lpu_id,
				GTS.GetTerrService_id,
				GTS.GetTerrService_Number,
				GTS.GetTerrServiceProfile_ID,
				convert(varchar(10), GTS.GetTerrService_begDate, 112) as GetTerrService_begDate,
				convert(varchar(10), GTS.GetTerrService_endDate, 112) as GetTerrService_endDate
			from {$this->scheme}.v_GetTerrService GTS with(nolock)
			...
			if ($link) {
				$LpuRegion = $this->getFirstRowFromQuery("
					select top 1
						...
						LR.LpuRegion_Name,
						LR.LpuRegion_Descr,
						convert(varchar(10), LR.LpuRegion_begDate, 112) as LpuRegion_begDate,
						convert(varchar(10), LR.LpuRegion_endDate, 112) as LpuRegion_endDate,
						LR.LpuRegionType_id,
						LR.Lpu_id,
						LR.LpuRegion_Guid,
						LR.LpuSection_id
				...
				$query = "
					declare @bigdate datetime = dateadd(year, 50, dbo.tzGetDate());
					...
					from
						...
						inner join v_LpuRegionType LRT with(nolock) on LRT.LpuRegionType_id = LR.LpuRegionType_id and Region_id = :Region_id
						outer apply(
							select top 1 ObjectSynchronLog_id
							from v_ObjectSynchronLog with(nolock)
							where ObjectSynchronLogService_id = :Service_id and [Object_Name] = 'LpuRegion' and [Object_id] = LR.LpuRegion_id
					...
					where
						...
						and isnull(LR.LpuRegion_endDate, @bigdate) = isnull(cast(:LpuRegion_endDate as datetime), @bigdate)
					order by
	...
	function syncLpuRegionStreet($data) {
		...
		$query = "
			with LeafAddressList as(
				select distinct GB.GetAddressTerr_id
				from {$this->scheme}.v_GetBuildingGetTerrService BTS with(nolock)
				inner join {$this->scheme}.v_GetBuilding GB with(nolock) on GB.GetBuilding_id = BTS.GetBuilding_id
			...
			select
				Rec.GetAddressTerr_id,
				Rec.GetAddressTerr_pid,
				Rec.GetAddressTerr_Name,
				Rec.GetAddressTerr_NameKZ,
				Rec.Leaf,
				Rec.Level,
				Socr.KLSocr_id,
				Socr.KLSocr_Name,
				Level.KLAreaLevel_SysNick
			from Rec
				left join {$this->scheme}.v_KZKLSocrLink SocrLink with(nolock) on SocrLink.KZKLSocr_id = Rec.GetAddressTerrType_id
				left join v_KLSocr Socr with(nolock) on Socr.KLSocr_id = SocrLink.KLSocr_id
				left join v_KLAreaLevel Level with(nolock) on Level.KLAreaLevel_id = Socr.KLAreaLevel_id
		...
		foreach($addresses as $leaf => $address) {
			...
			foreach($address as $element) {
				if ($element['KLSocr_Name'] == 'РЕСПУБЛИКА') {
					$query = "
						select top 1 KLCountry_id
						from v_KLCountry with(nolock)
						where KLCountry_Name like :KLCountry_Name
					";
					...
					$query = "
						select top 1 Street.KLStreet_id
						from v_KLStreet Street with(nolock)
						where
							Street.KLArea_id = :KLArea_id
							and Street.KLSocr_id = :KLSocr_id
						 	and (
								Street.KLStreet_Name like :KLStreet_Name
								or Street.KLStreet_LocalName like :KLStreet_LocalName
							)
					";
					$params = array(
					...
					$query = "
						select top 1 Area.KLArea_id
						from v_KLArea Area with(nolock)
						where
							Area.KLCountry_id = :KLCountry_id
							and Area.KLArea_Name like :KLArea_Name
			...
			if ($allowSave) {
				...
				$buildings = $this->queryResult("
					select GetBuilding_Number
					from {$this->scheme}.v_GetBuilding with(nolock)
	...
	function getPersonCardData($data) {
		...
		$query = "
			SELECT
				...
				PC.PersonCard_DmsPolisNum,
				convert(varchar,cast(PC.PersonCard_DmsBegDate as datetime),104) as PersonCard_DmsBegDate,
				convert(varchar,cast(PC.PersonCard_DmsEndDate as datetime),104) as PersonCard_DmsEndDate,
				PC.OrgSMO_id,
			...
			FROM
				...
				left join LpuRegion LR with(nolock) on LR.LpuRegion_id = PC.LpuRegion_id
	...
	function syncPersonCards($data) {
		...
		$query = "
			...
			select
				...
				GA.Org_ID,
				GA.GetTerrService_id,
				GA.GetTerrService_Number,
				GA.GetTerrServiceProfile_id,
				convert(varchar(10), GA.GetAttachment_begDate, 120) as GetAttachment_begDate,
				convert(varchar(10), GA.GetAttachment_endDate, 120) as GetAttachment_endDate,
				LL.Lpu_id,
			...
			from
				r101.v_GetAttachment GA with(nolock)
				inner join LpuList LL on LL.Org_ID = GA.Org_ID
				inner join r101.v_GetAttachmentStatus GAS with(nolock) on GAS.GetAttachmentStatus_id = GA.GetAttachmentStatus_id
				outer apply (
					select top 1 case
				...
				left join v_LpuRegionType LRT with(nolock) on LRT.LpuRegionType_SysNick = LRTL.LpuRegionType_SysNick
				outer apply (
					select top 1 GAS1.GetAttachmentStatus_Code
					from r101.v_GetAttachment GA1 with(nolock)
					inner join r101.v_GetAttachmentStatus GAS1 with(nolock) on GAS1.GetAttachmentStatus_id = GA1.GetAttachmentStatus_id
				...
				left join v_CardCloseCause CCC with(nolock) on CCC.CardCloseCause_Code = CardCloseCause.Code
			where
		...
		foreach($attachments as $item) {
			...
			$query = "
				...
				inner join v_ObjectSynchronLog OSL with(nolock) on OSL.Object_id = PC.PersonCard_id
			...
			if (!$PersonCard_id) {
				$query = "
					...
					where
						...
						and not exists(
							select * from v_ObjectSynchronLog with(nolock)
			...
			if (!empty($PersonCard_id)) {
				...
				if ($PersonCard['PersonCard_endDate'] != $item['GetAttachment_endDate']) {
	...
	function getPregnancyFactorsList($data) {
		...
		$query = "
			with list1 as (
				...
				where PQ.PersonPregnancy_id = :PersonPregnancy_id and isnull(PQ.PregnancyQuestion_IsTrue,1) = 2
			),
			QuestionTypeList as (
			...
			DiagList as (
				...
				from v_Diag t with(nolock)
				where t.Diag_id in ({$motherDiagList_str})
				union all
				select e.Diag_id, e.Diag_pid
				from v_Diag e with(nolock)
				inner join DiagList as d on d.Diag_pid = e.Diag_id
			)
			select
				PFT.PregnancyFactorType_id,
				PFT.PregnancyFactorType_fid,
				PFT.PregnancyFactorType_fpid,
				PFT.PregnancyFactorType_NameRu,
				Diags.Diag_ids
			from r101.v_PregnancyFactorType PFT with(nolock)
			outer apply (
				SELECT STUFF((
					select
						','+cast(PFUC.Diag_id as varchar)
				...
				FOR XML PATH ('')),1,1,'') as Diag_ids
			) as Diags
			where
				PFT.PregnancyFactorType_fpid in (1,2,3,8)
				and exists(
					select *
					from r101.v_PregnancyFactorUslugaComplex PFUC with(nolock)
					where
		...
		foreach($factors as $factor) {
			...
			$diagList = $this->queryList("
				...
				inner join DiagList DL with(nolock) on DL.Diag_id = D.Diag_id
		...
		foreach($map as $fpid => $other_fid) {
			if (isset($factorsByParent[$fpid]) && isset($factorsByParent[$fpid][$other_fid])) {
				...
				if (count(array_intersect($tmp_diag_list, $other_diag_list)) == 0) {
					...
					$list = $this->queryList("select Diag_Name from v_Diag with(nolock) where Diag_id in ({$tmp_diag_list_str})");
					if (!is_array($list)) {
	...
	function sendBirthSvidToRPN($data) {
		...
		$query = "
			select top 1
				...
				isnull(PNB.PersonNewborn_Weight, case
				...
				ISNULL(PNB.PersonNewborn_Height, BS.BirthSvid_Height) as PersonNewborn_Height,
				Apgar_1min.Value as Apgar_1min,
				Apgar_5min.Value as Apgar_5min,
				PNB.PersonNewborn_IsBreath,
				PNB.PersonNewborn_IsHeart,
				PNB.PersonNewborn_IsPulsation,
				PNB.PersonNewborn_IsMuscle,
				...
				BS.Lpu_id,
				BS.BirthSvid_id,
				BS.BirthSvid_GiveDate,
				BS.BirthSvid_BirthDT,
			...
			from
				...
				inner join v_PersonState Mother with(nolock) on Mother.Person_id = BS.Person_id
				inner join v_PersonState Child with(nolock) on Child.Person_id = BS.Person_cid
				inner join v_PersonNewBorn PNB with(nolock) on PNB.Person_id = Child.Person_id
				left join v_BirthSpecStac BSS with(nolock) on BSS.BirthSpecStac_id = PNB.BirthSpecStac_id
				left join Person mP with(nolock) on mP.Person_id = Mother.Person_id
				left join Person cP with(nolock) on cP.Person_id = Child.Person_id
				left join v_Address A with(nolock) on A.Address_id = ISNULL(Child.PAddress_id, Child.UAddress_id)
				left join v_BirthPlace BP with(nolock) on BP.BirthPlace_id = BS.BirthPlace_id
				left join v_ChildTermType CTT with(nolock) on CTT.ChildTermType_id = PNB.ChildTermType_id
				left join v_Okei WeightOkei with(nolock) on WeightOkei.Okei_id = BS.Okei_mid
				outer apply (
					select top 1 cast(NewbornApgarRate_Values as int) as Value
					from v_NewbornApgarRate t with(nolock)
				...
				left join v_BirthEducation BE with(nolock) on BE.BirthEducation_id = PP.BirthEducation_id
				left join PregnancyFamilyStatus PFS with(nolock) on PFS.PregnancyFamilyStatus_id = PP.PregnancyFamilyStatus_id
				left join v_EvnPS cEPS with(nolock) on cEPS.EvnPS_id = PNB.EvnPS_id
				left join v_Address BA with(nolock) on BA.Address_id = BS.Address_rid
			where
	...
	function getHistoryAttachByPeriod($data) {
		...
		$lpu_ids = $this->queryResult("
			select l.Lpu_id, l.Lpu_Nick
			from v_lpu l with(nolock)
			inner join v_org o with (nolock) on o.org_id = l.org_id
			/*inner join r101.GetMO MO with(nolock) on MO.Lpu_id = l.Lpu_id*/
			...
			foreach ($result as $k=>$row) {
				if ($row->AttachStatus == 'ATTACH') {
					$person = $this->getFirstRowFromQuery("
						...
						left join v_PersonCard_all pc with(nolock) on pc.Person_id = p.Person_id and pc.LpuAttachType_id = 1
						left join v_Lpu l with(nolock) on l.Lpu_id = pc.Lpu_id
						left join v_LpuRegion lr with(nolock) on lr.LpuRegion_id = pc.LpuRegion_id
