...
class MorbusBase_model extends Abstract_model{
    ...
    public function getMorbusTypeByDiag($diag_id){
        $result = 1;
        $MorbusType_id = $this->getFirstResultFromQuery('SELECT MorbusType_id FROM dbo.v_MorbusDiag with(nolock) WHERE Diag_id = :Diag_id',array('Diag_id'=>$diag_id));
        if ($MorbusType_id) {
    ...
    public function getMorbusTypesByDiag($diag_id){
        $response = array(1);
        $query = 'SELECT MorbusType_id FROM dbo.v_MorbusDiag with(nolock) WHERE Diag_id = :Diag_id';
    ...
    public function activeMorbusCount($date, $exceptThis_id = null){
        ...
        $result = (int)$this->getFirstResultFromQuery("SELECT COUNT(*) FROM v_Morbus with(nolock) WHERE (:date BETWEEN Morbus_SetDT and IsNull(Morbus_DisDT, :date)) AND MorbusBase_id = :MorbusBase_id $condition", $params);
        return $result;
    }
    ...
    public function morbusCount($exceptThis_id = null){
        ...
        $result = (int)$this->getFirstResultFromQuery("SELECT COUNT(*) FROM v_Morbus with(nolock) WHERE MorbusBase_id = :MorbusBase_id $condition", $params);
        return $result;
    }
    ...
    public function updateBoundaries(){
        $this->MorbusBase_setDT = $this->getFirstResultFromQuery(
            'SELECT MIN(m.Morbus_setDT) FROM v_Morbus m with(nolock) WHERE m.MorbusBase_id = :MorbusBase_id',
            array('MorbusBase_id' => $this->MorbusBase_id)
        );
        $this->MorbusBase_disDT = $this->getFirstResultFromQuery(
            'SELECT (
              CASE
                WHEN EXISTS (SELECT 1 FROM v_Morbus m with(nolock) WHERE m.MorbusBase_id = :MorbusBase_id AND m.Morbus_disDT IS null)
            		THEN NULL
                ELSE
            		(SELECT MAX(m.Morbus_disDT) FROM v_Morbus m with(nolock) WHERE m.MorbusBase_id = :MorbusBase_id)
              END)',
            array('MorbusBase_id' => $this->MorbusBase_id)
        );
        if ($this->MorbusBase_disDT) {
            $this->MorbusResult_id = $this->getFirstResultFromQuery(
                '
                SELECT TOP 1 MorbusResult_id FROM dbo.v_Morbus with(nolock) WHERE morbusBase_id = :MorbusBase_id AND Morbus_disDT =
                (SELECT MAX(Morbus_disDT) FROM v_Morbus with(nolock) WHERE morbusBase_id = :MorbusBase_id)
                ',
                array('MorbusBase_id' => $this->MorbusBase_id)
            );
        } else {
