		...
		$query = "
		    ...
		    (
				...
				from
					...
					inner join v_EvnDiag Evn with (nolock) on Evn.Morbus_id = MV.Morbus_id
				where
			...
			select
				...
				EvnDiagNephro.EvnDiagNephro_id,
				convert(varchar,EvnDiagNephro.EvnDiagNephro_setDT,104) as EvnDiagNephro_setDate,
				Diag.Diag_id,
				Diag.Diag_Name,
				Diag.Diag_Code,
				EvnDiagNephro.Person_id,
	            EvnDiagNephro.PersonEvn_id,
	            EvnDiagNephro.Server_id,
				:Evn_id as MorbusNephro_pid
			from
				EvnDiagNephro with(nolock)
				inner join v_Diag Diag with (nolock) on Diag.Diag_id = EvnDiagNephro.Diag_id
				outer apply (
					select top 1 EvnDiagNephro_id
					from EvnDiagNephro with(nolock)
