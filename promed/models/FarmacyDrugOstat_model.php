...
class FarmacyDrugOstat_model extends CI_Model {
	/**
	 * Конструктор
	 */
	...
	*/
	function loadDrugOatatByDate($data) {
		...
		$query = "
			...
			from 
			-- from
			v_DocumentUcStr as DocumentUcStr  with(nolock)
			Inner join Drug as d with(nolock) on (DocumentUcStr.Drug_id = d.Drug_id)
			inner join DrugUnit du with(nolock) on d.DrugUnit_id = du.DrugUnit_id
			Inner join v_DocumentUc as DocumentUc with(nolock) on (DocumentUcStr.DocumentUc_id = DocumentUc.DocumentUc_id) and DocumentUc.DrugDocumentType_id<>6
			left join 
			...
			Rtrim(du.DrugUnit_Name) as val2,
			DocumentUcSTR.DocumentUcSTR_Price as val3,
			isnull(DocumentUcStr.DocumentUcStr_Count,0) - isnull(sum(DocumentUcStrRash.DocumentUcStr_Count),0) as val4,
			isnull(DocumentUcStr.DocumentUcStr_Sum,0) - isnull(sum(DocumentUcStrRash.DocumentUcStr_Sum),0) as val5
			from v_DocumentUcStr DocumentUcStr with(nolock)
			inner join Drug d with(nolock) on d.Drug_id = DocumentUcStr.Drug_id
			inner join DrugUnit du with(nolock) on d.DrugUnit_id = du.DrugUnit_id
			Inner join v_DocumentUc as DocumentUc with(nolock) on (DocumentUcStr.DocumentUc_id = DocumentUc.DocumentUc_id) and DocumentUc.DrugDocumentType_id<>6
			left outer join v_DocumentUcStr DocumentUcStrRash with(nolock) on DocumentUcStrRash.DocumentUcStr_oid = DocumentUcStr.DocumentUcStr_id
			left join v_DocumentUc DocumentUcRash with(nolock) on DocumentUcStrRash.DocumentUc_id = DocumentUcRash.DocumentUc_id
			and ((DocumentUcRash.DocumentUc_didDate < :begDate ) or ( :begDate is null))
			where (1=1)
			--and((d.Drug_id = @Drug_id)or(@Drug_id is null))
			and((DocumentUc.DrugFinance_id = :DrugFinance_id )or( :DrugFinance_id is null))
			and(DocumentUc.Contragent_tid = :Contragent_id )
			--and((DocumentUc.Mol_tid=@Mol_id)or(@Mol_id is null))
			...
			having (isnull(DocumentUcStr.DocumentUcStr_Count,0) - isnull(sum(DocumentUcStrRash.DocumentUcStr_Count),0)) > 0
			) as p
			...
			from v_DocumentUcStr as DocumentUcStr with(nolock)
			Inner join Drug as d with(nolock) on (DocumentUcStr.Drug_id = d.Drug_id)
			inner join DrugUnit du with(nolock) on d.DrugUnit_id = du.DrugUnit_id
			Inner join v_DocumentUc as DocumentUc with(nolock) on (DocumentUcStr.DocumentUc_id = DocumentUc.DocumentUc_id) and DocumentUc.DrugDocumentType_id<>6
			where (1=1)
			...
			from v_DocumentUcStr as DocumentUcStr with(nolock)
			inner join DocumentUcStr DocumentUcStrRash with(nolock) on DocumentUcStr.DocumentUcStr_oid = DocumentUcStrRash.DocumentUcStr_id
			inner join DocumentUc DocumentUcRash with(nolock) on DocumentUcStrRash.DocumentUc_id = DocumentUcRash.DocumentUc_id and DocumentUcRash.DrugDocumentType_id<>6
			Inner join Drug as d with(nolock) on (DocumentUcStrRash.Drug_id = d.Drug_id)
			inner join DrugUnit du with(nolock) on d.DrugUnit_id = du.DrugUnit_id
			Inner join v_DocumentUc as DocumentUc with(nolock) on (DocumentUcStr.DocumentUc_id = DocumentUc.DocumentUc_id) and DocumentUc.DrugDocumentType_id<>6
			where (1=1)
			--and((d.Drug_id = @Drug_id)or(@Drug_id is null))
			...
			--and((DocumentUc.Mol_sid=@Mol_id)or(@Mol_id is null))
			and((DocumentUc.DrugFinance_id = :DrugFinance_id )or( :DrugFinance_id is null))
			...
			Rtrim(du.DrugUnit_Name) as val2,
			DocumentUcSTR.DocumentUcSTR_Price as val3,
			isnull(DocumentUcStr.DocumentUcStr_Count,0) - isnull(sum(DocumentUcStrRash.DocumentUcStr_RashCount),0) as val12,
			isnull(DocumentUcStr.DocumentUcStr_Sum,0) - isnull(sum(DocumentUcStrRash.DocumentUcStr_Sum),0) as val13
			from v_DocumentUcStr DocumentUcStr with(nolock)
			inner join Drug d with(nolock) on d.Drug_id = DocumentUcStr.Drug_id
			inner join DrugUnit du with(nolock) on d.DrugUnit_id = du.DrugUnit_id
			Inner join v_DocumentUc as DocumentUc with(nolock) on (DocumentUcStr.DocumentUc_id = DocumentUc.DocumentUc_id) and DocumentUc.DrugDocumentType_id<>6
		...
		$cnt_query = "
			...
			Inner join v_DocumentUc as DocumentUc with(nolock) on (DocumentUcStr.DocumentUc_id = DocumentUc.DocumentUc_id) and DocumentUc.DrugDocumentType_id<>6
			left outer join v_DocumentUcStr DocumentUcStrRash with(nolock) on DocumentUcStrRash.DocumentUcStr_oid = DocumentUcStr.DocumentUcStr_id
			left join v_DocumentUc DocumentUcRash with(nolock) on DocumentUcStrRash.DocumentUc_id = DocumentUcStr.DocumentUc_id
			 and ((DocumentUcRash.DocumentUc_didDate <= :endDate) or ( :endDate is null))
			where (1=1)
			and(DocumentUc.Contragent_tid = :Contragent_id )
			--and((d.Drug_id = @Drug_id )or( @Drug_id is null))
	...
	*/
	function loadDrugOstatByFilters($data) {
		...
		if (isset($data['WhsDocumentCostItemType_id']) && $data['WhsDocumentCostItemType_id'] != '' && $data['WhsDocumentCostItemType_id'] > 0) {
			$filters .= ' and do.WhsDocumentCostItemType_id = :WhsDocumentCostItemType_id';
		...
		if (!empty($data['CLS_MZ_PHGROUP_ID'])) {
			...
			if (is_object($result)) {
				...
				if (is_array($res_arr) && !empty($res_arr)) {
					...
					$join .= ' inner join rls.TRADENAMES_DRUGFORMS td with(nolock) on td.TRADENAMEID = p.TRADENAMEID and td.DRUGFORMID = dcm.CLSDRUGFORMS_ID';
		...
		if (empty($data['searchByDrug']) || !$data['searchByDrug']) {
			$select .= ",ps.PrepSeries_id,ps.PrepSeries_Ser,block.PrepBlockCause_id,block.PrepBlockCause_Name";
			$join .= " left join rls.v_PrepSeries ps with(nolock) on ps.PrepSeries_id = dus.PrepSeries_id";
			$join .= " outer apply (
				select top 1
					pb.PrepBlock_id,
		...
		$query = "
			select
				...
				do.DocumentUcStr_id,
				d.Drug_id,
				RTRIM(ISNULL(d.Drug_Name, '')) as Drug_Name,
				convert(varchar(10), do.DocumentUcStr_godnDate, 104) as godnDate,
				RTRIM(ISNULL(d.Drug_PackName, '')) as unit,
				do.DocumentUcStr_Price as Price,
				do.DocumentUcStr_PriceR as PriceR,
				ISNULL(do.DocumentUcStr_Ost,0) as ostat,
				(ISNULL(do.DocumentUcStr_Ost,0) * ISNULL(do.DocumentUcStr_Price,0)) as Sum,
				(ISNULL(do.DocumentUcStr_Ost,0) * ISNULL(do.DocumentUcStr_PriceR,0)) as SumR,
				do.DrugFinance_id,
				do.DrugFinance_Name,
				do.WhsDocumentCostItemType_id,
				do.WhsDocumentCostItemType_Name,
				'' as quantity,
				am.STRONGGROUPID
				{$select}
				-- end select
			from 
				-- from
				v_DocumentUcOst_Lite do (nolock)
				inner join rls.v_Drug d (nolock) on d.Drug_id = do.Drug_id
				left join DocumentUcStr dus (nolock) on dus.DocumentUcStr_id = do.DocumentUcStr_id
				left join DocumentUc du (nolock) on du.DocumentUc_id = do.DocumentUc_id
			...
			where
				-- where
				do.Contragent_tid = :Contragent_id and do.DocumentUcStr_Ost <> 0
				{$filters}
				-- end where
			order by 
				-- order by
