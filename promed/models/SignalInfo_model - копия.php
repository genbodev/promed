...
class SignalInfo_model extends swModel {
	...
	public $inputRules = array(
		...
		'loadPregnancyRouteSMP' => array(
			...
			array('default' => 0, 'field' => 'start', 'label' => 'Начальная запись', 'rules' => '', 'type' => 'int'),
            array('default' => 0, 'field' => 'Lpu_iid', 'label' => 'ЛПУ учета', 'rules' => '', 'type' => 'id'),
            array('default' => 0, 'field' => 'CmpLpu_id', 'label' => 'ЛПУ госпитализации', 'rules' => '', 'type' => 'id'),           
            array('default' => '', 'field' => 'MedPersonal_iid', 'label' => 'Врач', 'rules' => '', 'type' => 'string'),
            array('default' => 0, 'field' => 'CmpResult_id', 'label' => 'Результат оказания СМП', 'rules' => '', 'type' => 'id'),   
			array('default' => '', 'field' => 'CmpCallCard_prmDate_Range', 'label' => 'Дата вызова СМП (диапазон)', 'rules' => '', 'type' => 'daterange'),
            array('default' => '', 'field' => 'Yesterday', 'label' => 'За вчерашний день', 'rules' => '', 'type' => 'id'),
            array('default' => '', 'field' => 'Person_SurName', 'label' => 'Фамилия', 'rules' => 'trim', 'type' => 'string'),
            array('default' => '', 'field' => 'Person_FirName', 'label' => 'Имя', 'rules' => 'trim', 'type' => 'string'),
	...
	*/
	function loadPregnancyRouteSMP($data)
	{
        ...
        if(!empty($data['CmpLpu_id'])) {
            $filters .= " and case when (CCC.CmpCallCardInputType_id in (1,2)) then CCC.CmpLpu_id else CCC.Lpu_hid end  = :CmpLpu_id ";
        }

        // Врач
        if(!empty($data['MedPersonal_iid'])) {
            $filters .= " and PR.MedPersonal_iid in (:MedPersonal_iid) ";
        }
        ...
        //Результат оказания СМП
        if (!empty($data['CmpResult_id'])) {
            $filters .= " and CLC.CmpResult_id = :CmpResult_id ";
        }

        // дата вызова СМП
        if (!empty($data['CmpCallCard_prmDate_Range'][0])) {
            $filters .= " and cast(CCC.CmpCallCard_prmDT as date) >= cast(:CmpCallCard_prmDate_Range_0 as date) ";
        }
        if (!empty($data['CmpCallCard_prmDate_Range'][1])) {
            $filters .= " and cast(CCC.CmpCallCard_prmDT as date) <= cast(:CmpCallCard_prmDate_Range_1 as date) ";
        }
		...
		if (isset($data['Yesterday']) && $data['Yesterday'] == 1) {
			...
			$query = "
				...
				select
                    ...
                    CCC.Person_SurName+ISNULL(' '+CCC.Person_FirName,'')+ISNULL(' '+CCC.Person_SecName,'') as Person_Fio,
                    CRES.CmpResult_Code+' '+CRES.CmpResult_Name as CmpResult_Name,
				...
				from
                    ...
                    left join v_CmpResult CRES with (nolock) on CRES.CmpResult_id = CLC.CmpResult_id
