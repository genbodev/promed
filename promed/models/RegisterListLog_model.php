 ...
 * @version
 */
class RegisterListLog_model extends swModel {
	...
	function load() {
		$q = "
			select
				RegisterListLog_id, RegisterListLog_begDT, RegisterListLog_endDT, RegisterListRunType_id, RegisterListLog_AllCount, RegisterListLog_UploadCount, RegisterListResultType_id, Lpu_id, RegisterList_id
			from
				stg.v_RegisterListLog with(nolock)
			where
	...
	function loadList($filter) {
		...
		if ($this->getRegionNick() == 'kareliya' && isset($filter['Lpu_id']) && $filter['Lpu_id']) {
			$join[] = 'left join v_pmUserCache puc with (nolock) on puc.PMUser_id = v_RegisterListLog.pmUser_insID';
			$where[] = 'ISNULL(v_RegisterListLog.Server_id, puc.Lpu_id) = :Lpu_id';
			//echo $filter['Lpu_id'];
		...
		$q = "
			SELECT
					TOP 100
					v_RegisterListLog.RegisterListLog_id, v_RegisterListLog.RegisterListLog_begDT, v_RegisterListLog.RegisterListLog_endDT, v_RegisterListLog.RegisterListRunType_id, v_RegisterListLog.RegisterListLog_AllCount, v_RegisterListLog.RegisterListLog_UploadCount, v_RegisterListLog.RegisterListResultType_id, v_RegisterListLog.RegisterList_id
					,RegisterListRunType_id_ref.RegisterListRunType_Name RegisterListRunType_id_Name, RegisterListResultType_id_ref.RegisterListResultType_Name RegisterListResultType_id_Name, RegisterList_id_ref.RegisterList_Name RegisterList_id_Name,
					l.Lpu_Nick,
					v_RegisterListLog.RegisterListLog_NameFile
			FROM
				stg.v_RegisterListLog WITH (NOLOCK)
				LEFT JOIN stg.v_RegisterListRunType RegisterListRunType_id_ref WITH (NOLOCK) ON RegisterListRunType_id_ref.RegisterListRunType_id = v_RegisterListLog.RegisterListRunType_id
				LEFT JOIN stg.v_RegisterListResultType RegisterListResultType_id_ref WITH (NOLOCK) ON RegisterListResultType_id_ref.RegisterListResultType_id = v_RegisterListLog.RegisterListResultType_id
				LEFT JOIN stg.v_RegisterList RegisterList_id_ref WITH (NOLOCK) ON RegisterList_id_ref.RegisterList_id = v_RegisterListLog.RegisterList_id
				LEFT JOIN v_Lpu l WITH (NOLOCK) ON l.Lpu_id = v_RegisterListLog.Lpu_id
	...
	function save() {
		...
		$q = "
			declare
				@RegisterListLog_id bigint,
				@ErrCode int,
				@ErrMessage varchar(4000);
			set @RegisterListLog_id = :RegisterListLog_id;
			exec stg." . $procedure . "
				@RegisterListLog_id = @RegisterListLog_id output,
			...
			select @RegisterListLog_id as RegisterListLog_id, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
		";
		$p = array(
	...
	function delete() {
		$q = "
			declare
				@ErrCode int,
				@ErrMessage varchar(4000);
			exec stg.p_RegisterListLog_del
				@RegisterListLog_id = :RegisterListLog_id,
				@Error_Code = @ErrCode output,
				@Error_Message = @ErrMessage output;
