 ...
 onclick=\"javascript: var div=this.nextElementSibling; if(div.style.display == 'none'){div.style.display = 'block';this.innerHTML='-';}else{div.style.display = 'none';this.innerHTML='+';}\">+</span>";
	...
	function getAccessRightsLpuBuildingOrg($data){
		...
		$query  = "
			select
				...
				LBAR.AccessRightsOrg_id,
				LBAR.LpuBuilding_id,
				LB.LpuBuilding_Name
			from 
				v_LpuBuildingAccessRightsLink LBAR with(nolock)
	...
	function checkAccessRightsIntersection($data) {
		...
		$query = "
			...
			from v_AccessRightsOrg ARO with(nolock)
				inner join v_Lpu Lpu with(nolock) on Lpu.Org_id = ARO.Org_id
				left join v_AccessRightsName ARN with(nolock) on ARN.AccessRightsName_id = ARO.AccessRightsName_id
	...
	function saveAccessRightsLpu($data) {
		...
		$query = "
			...
			exec {$procedure}
				...
				@Error_Code = @Error_Code output,
				@Error_Message = @Error_Message output
			select @AccessRightsOrg_id as AccessRightsLpu_id, @Error_Code as Error_Code, @Error_Message as Error_Msg
		";
		//echo getDebugSQL($query, $params);exit;
		$result = $this->db->query($query, $params);
		if (!is_object($result)) {
	...
	function loadAccessRightsForm($data) {
		...
		$query = "
			select 
				LBR.LpuBuilding_id,
				LBR.AccessRightsOrg_id
			from  
				v_LpuBuildingAccessRightsLink LBR
				left join v_AccessRightsOrg ARO with(nolock) on LBR.AccessRightsOrg_id=ARO.AccessRightsOrg_id
			where ARO.AccessRightsName_id =  :AccessRightsName_id
		";
		$result = $this->db->query($query, $params);
		if (!is_object($result)) {
