...
class Post_model extends swModel {
	...
	function savePost($data) {
		...
		$query = "
			...
			select @Res as Post_id, @Error_Code as Error_Code, @Error_Message as Error_Msg;
		";
	...
	function loadPostGrid($data) {
		...
		if (!empty($data['Post_Name'])) {
			$filters .= " and P.Post_Name like '%'+:Post_Name+'%'";
	...
	function loadPostList($data) {
		...
		if (!empty($data['Post_id'])) {
			...
			if (!empty($data['query'])) {
				$filter .= " and P.Post_Name like :Post_Name+'%'";
	...
	function getPostKinds() {
		...
		$query = "
			select
				...
				PK.name,
				PK.code
			from persis.v_PostKind PK with(nolock)
			order by
				PK.name
		";
	...
	function createPostForAPI($data){
		...
		$count = $this->getFirstResultFromQuery("SELECT COUNT(id) FROM persis.v_Post WHERE code = :Code", $data);
		if(!empty($count)){
			return array(
				'success' => false,
		...
		try {
			...
			if($res){
				$query = $this->db->query('SELECT @@IDENTITY AS Post_id');
				$query = $query->row();
	...
	function updatePostForAPI($data){
		...
		if(!empty($data['Code'])){
			//проверка на уникальность кода
			$count = $this->getFirstResultFromQuery("SELECT COUNT(id) FROM persis.v_Post WHERE code = :Code and id <> :Post_id", $data);
			if(!empty($count)){
				return array(
					'success' => false,
					'Error_Msg' => 'Запись с указанным кодом уже существует'
