...
class Kareliya_EvnPS_model extends EvnPS_model {
	...
	function getEvnPSNumber($data) {
		$query = "
			...
			exec xp_GenpmID @ObjectName = 'EvnPS', @Lpu_id = :Lpu_id, @ObjectID = @EvnPS_NumCard output, @ObjectValue = :ObjectValue;
	...
	function exportHospDataForTfomsToXml($data) {
			...
			$T3_AdditionalFields .= "
				case
					...
					when ed.EvnDirection_id is not null then
						ed_ld.Lpu_f003mcod
						+ cast(YEAR(ed.EvnDirection_setDT) as varchar(4))
					...
					when eps.PrehospDirect_id in (1, 2) and LEN(RTRIM(ISNULL(eps.EvnDirection_Num, ''))) > 0 and eps.EvnDirection_setDT is not null then
						right('000000' + ISNULL(case when eps.PrehospDirect_id = 1 then lp.Lpu_f003mcod else eps_ld.Lpu_f003mcod end, ''), 6)
						+ cast(YEAR(eps.EvnDirection_setDT) as varchar(4))
						+ right('000000' + eps.Evndirection_Num, 6)
				end as NOM_NAP,
				cast(
					case
						when ed.EvnDirection_id is not null then ed.EvnDirection_setDT
						when eps.PrehospDirect_id in (1, 2) and LEN(RTRIM(ISNULL(eps.EvnDirection_Num, ''))) > 0 and eps.EvnDirection_setDT is not null then eps.EvnDirection_setDT
					end
				as date) as DTA_NAP,
			";
		}
		...
		$query = "
			...
			SET @S1 = isnull((
			Select
				cast(lp.Lpu_f003mcod as varchar)+cast(year(EvnDirection_setDT) as varchar)
				+isnull(replicate('0',6-len(cast(EvnDirection_Num as varchar))),'')
				+cast(EvnDirection_Num as varchar) as NOM_NAP,		--T(16) Номер направления (MCOD+NP)
				cast(EvnDirection_setDT as date) as DTA_NAP,
				case when prt.PrehospType_Code in (2,3) or ed.DirType_id = 5 then 3 else isnull(prt.PrehospType_Code,1) end as FRM_MP, -- Int - Форма оказания медицинской помощи
				...
				case when pt.PolisType_CodeF008 in (1,2) then case when len(po.Polis_Ser) > 0 then po.Polis_Ser else null end else Null end as SPOLIS, -- Varchar(10) - Серия документа, подтверждающего факт страхования по ОМС
				...
				isnull(smo.Orgsmo_f002smocod,'00000') as SMO_CODE, -- numeric(6, 0) - СМО (реестровый номер, F002)
				...
				isnull(pe.PersonPhone_Phone, '-') as TLF, --Varchar(100) - Контактная информация
				...
				isnull(ls1.LpuSectionProfile_Code, lsp.LpuSectionProfile_Code) as KOD_PFK, -- Профиль койки
			...
			from dbo.v_EvnDirection ed (nolock)
				...
				left join dbo.v_LpuSectionProfile lsp (nolock) on lsp.LpuSectionProfile_id = ed.LpuSectionProfile_id
			...
			+isnull(replicate('0',6-len(cast(isnull(ed.EvnDirection_Num,eps.EvnDirection_Num) as varchar))),'')
			+cast(isnull(ed.EvnDirection_Num,eps.EvnDirection_Num) as varchar) as NOM_NAP, --Int
			cast(isnull(ed.EvnDirection_setDT,eps.EvnDirection_setDT) as date) as DTA_NAP,
			...
			ISNULL(di.Diag_Code, EvnSec.Diag_Code) as DS --Char(4) - Диагноз приемного отделения
			...
			left join dbo.v_OrgSMO smo (nolock) on smo.OrgSMO_id = po.OrgSmo_id
			outer apply(
				select top 1 lses.LpuSection_Code, lses.LpuSectionProfile_Code, d.Diag_Code
			...
			left join dbo.v_OrgSMO smo (nolock) on smo.OrgSMO_id = po.OrgSmo_id
			left join dbo.v_Org org (nolock) on org.Org_id = smo.Org_id
			left join dbo.v_Lpu ed_ld (nolock) on ed_ld.Lpu_id = ed.Lpu_id -- направившая МО из электронного напрваления
			left join dbo.v_Lpu_all eps_ld (nolock) on eps_ld.Org_id = eps.Org_did -- направившая МО из КВС
			outer apply(
			...
			+isnull(replicate('0',6-len(cast(ed.EvnDirection_Num as varchar))),'')
			...
			from dbo.v_EvnDirection ed (nolock)
			left join dbo.DirFailType dr (nolock) on dr.DirFailType_id = ed.DirFailType_id
			inner join dbo.pmUserCache puc (nolock) on ed.pmUser_failID=puc.PMUser_id
			inner join v_lpu lp (nolock) on lp.Lpu_id = puc.Lpu_id
			inner join v_lpu lp1 (nolock) on lp1.Lpu_id = ed.Lpu_id
			outer apply (
				select top 1
			...
			from dbo.v_EvnPS eps (nolock)
			left join dbo.v_EvnDirection ed (nolock) on ed.EvnDirection_id = eps.EvnDirection_id
			inner join dbo.v_LpuSection ls1 (nolock) on ls1.LpuSection_id = eps.LpuSection_id
			inner join dbo.v_LpuUnit lu (nolock) on ls1.LpuUnit_id=lu.LpuUnit_id
			...
			+cast(year(ISNULL(ed.EvnDirection_setDT, eps.EvnDirection_setDT)) as varchar)
			+isnull(replicate('0',6-len(cast(ISNULL(ed.EvnDirection_Num, eps.EvnDirection_Num) as varchar))),'')
			+cast(ISNULL(ed.EvnDirection_Num, eps.EvnDirection_Num) as varchar)		as	NOM_NAP, -- bigint , Идентификатор направления
			...
			cast(pe.Person_BirthDay as date) as DR, --date – дата рождения
			pt.PolisType_CodeF008 as VPOLIS,-- numeric(1, 0) -- Тип документа, подтверждающего факт страхования по ОМС (F008)
			case when pt.PolisType_CodeF008 = 1 and len(po.Polis_Ser) > 0 then po.Polis_Ser else null end as SPOLIS, -- Varchar(10) - Серия документа, подтверждающего факт страхования по ОМС
			case when pt.PolisType_CodeF008 = 3 then pe.Person_EdNum else po.Polis_Num end as NPOLIS, -- Numeric(20) - Номер документа, подтверждающего факт страхования по ОМС
			case
				when lu.LpuUnitType_id = 1 then 1 -- круглосуточный
				when lu.LpuUnitType_id = 6 then 21 -- дневной стационар при стационаре
	...
	function getEvnPSFields($data) {
		$query = "
			select top 1
				...
				,convert(varchar(10), ESWR.EvnStickWorkRelease_endDT, 104) as EvnStick_disDate
				,EST.StickLeaveType_id
			...
			from v_EvnPS EPS with (nolock)
				...
				outer apply (
					...
					from
						v_EvnStickWorkRelease t1 with (nolock)
					where
