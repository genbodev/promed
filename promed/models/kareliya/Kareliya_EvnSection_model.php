		...
		if (!empty($UslugaComplexIds)) {
			$query = "
				...
				where
					...
					and (mu.Diag_nid IS NULL OR mu.Diag_nid IN (select Diag_id from v_EvnDiagPS (nolock) where DiagSetClass_id = 3 and EvnDiagPS_pid IN ('" . implode("','", $EvnSectionIds) . "')))
		...
		if (!empty($data['Diag_id'])) {
			...
			$query = "
				...
				where
					mu.Diag_id = :Diag_id
					{$uslugaComplexFilter}
					and (mu.Diag_nid IS NULL OR mu.Diag_nid IN (select Diag_id from v_EvnDiagPS (nolock) where DiagSetClass_id = 3 and EvnDiagPS_pid = :EvnSection_id))
					and (mu.Sex_id = :Sex_id OR Sex_id IS NULL)
					and (
						(:Person_Age >= 18 and mu.MesAgeGroup_id = 1)
						or (:Person_Age < 18 and mu.MesAgeGroup_id = 2)
	...
	protected function calcCoeffCTP2019($data) {
		...
		if (isset($KSLPCodes[5])) {
			if (!empty($data['EvnSectionIds'])) {
				$AttributeValue = $this->queryResult("
					declare
						@AttributeVision_TablePKey bigint = (select top 1 TariffClass_id from v_TariffClass (nolock) where TariffClass_SysNick = 'diagpat');
		...
		if (isset($KSLPCodes[6])) {
			...
			if (!empty($data['EvnSectionIds'])) {
				...
				if (!$needKSLP6) {
					...
					$checkKSLP6 = $this->queryResult("
						select distinct ucat.UslugaComplexAttributeType_SysNick
						from
							v_EvnUsluga eu (nolock)
							inner join v_PayType pt (nolock) on pt.PayType_id = eu.PayType_id and pt.PayType_SysNick = 'oms'
		...
		if (isset($KSLPCodes[7])) {
			if (!empty($data['EvnSectionIds'])) {
				$AttributeValue = $this->queryResult("
					declare
						@AttributeVision_TablePKey bigint = (select top 1 TariffClass_id from v_TariffClass (nolock) where TariffClass_SysNick = 'sochir');
					...
					FROM
						...
						cross apply (
							...
							where
								...
								and av2.AttributeValue_ValueIdent IN (select eu.UslugaComplex_id from v_EvnUsluga eu (nolock) where eu.EvnUsluga_pid IN ('" . implode("','", $data['EvnSectionIds']) . "') and eu.EvnClass_id in (43,22,29,47) and eu.PayType_id = :PayTypeOms_id)
						) UC1FILTER
						cross apply (
							select
								av2.AttributeValue_ValueIdent
			...
			if (!empty($data['EvnSectionIds'])) {
				...
				if (!$needKSLP8) {
					$EvnUsluga_setDate = $this->getFirstResultFromQuery("
						SELECT top 1 eu.EvnUsluga_setDate
						FROM v_EvnUsluga eu (nolock)
						WHERE
							eu.EvnUsluga_pid IN ('" . implode("','", $data['EvnSectionIds']) . "')
							and eu.PayType_id = :PayTypeOms_id
			...
			if (!empty($data['EvnSectionIds'])) {
				...
				$resp_am = $this->queryResult("
					...
					from
						...
						inner join v_EvnCourseTreatDrug ECTD with (nolock) on ECTD.EvnCourseTreat_id = EPT.EvnCourse_id
						inner join rls.v_DrugComplexMnn dcm with (nolock) on dcm.DrugComplexMnn_id = ECTD.DrugComplexMnn_id
						inner join rls.DrugComplexMnnName dcmnn with (nolock) on dcm.DrugComplexMnnName_id = dcmnn.DrugComplexMnnName_id
					where
				...
				if (!empty($diagCodes)) {
					$EvnUsluga = $this->getFirstResultFromQuery("
						...
						WHERE
							...
							and 
								(
									substring(d.Diag_Code, 1, 3) IN ('" . implode("','", $diagCodes) . "')
									or
									exists(
										select top 1
											edps.EvnDiagPS_id
										from
											...
											inner join v_Diag d (nolock) on d.Diag_id = edps.Diag_id
										where
											edps.EvnDiagPS_pid IN ('" . implode("','", $data['EvnSectionIds']) . "')
											and substring(d.Diag_Code, 1, 3) IN ('" . implode("','", $diagCodes) . "')
											and edps.DiagSetClass_id=2
											and edps.DiagSetClass_id IN (2,3)
									)
								)
		...
		$resp_mo = $this->queryResult("
			declare @AttributeVision_TablePKey bigint = (select top 1 VolumeType_id from v_VolumeType (nolock) where VolumeType_Code = '2018_КСГ_45');
			...
			FROM
				...
				inner join v_MesOld mo (nolock) on mo.Mes_id = MESOLD.AttributeValue_ValueIdent
			WHERE
	...
	protected function calcCoeffCTP2018($data) {
		...
		if (!empty($data['EvnSection_id']) && $data['EvnSection_disDate'] < '2018-06-01') {
			$resp_mouc = $this->queryResult("
				...
				from
					v_EvnSection es (nolock)
					inner join v_MesOldUslugaComplex mouc (nolock) on mouc.MesOldUslugaComplex_id = es.MesOldUslugaComplex_id
				where
					es.EvnSection_id = :EvnSection_id
				...
				if (!empty($data['MesTariff_id'])) {
					$LpuSectionProfile_id = $this->getFirstResultFromQuery("
						...
						from
							...
							inner join v_LpuSectionProfile lsp (nolock) on lsp.LpuSectionProfile_id = mokpg.LpuSectionProfile_id
						where
							MesTariff_id = :MesTariff_id
			...
			if (!empty($data['EvnSectionIds'])) {
				$checkKSLP7 = $this->queryResult("
					...
					from
						...
						inner join v_PayType pt1 (nolock) on pt1.PayType_id = eu1.PayType_id and pt1.PayType_SysNick = 'oms'
						inner join v_UslugaComplexAttribute uca1 (nolock) on uca1.UslugaComplex_id = eu1.UslugaComplex_id
						inner join v_UslugaComplexAttributeType ucat1 (nolock) on ucat1.UslugaComplexAttributeType_id = uca1.UslugaComplexAttributeType_id
						cross apply (
							select top 1
								eu2.EvnUsluga_id
							from
								v_EvnUsluga eu2 (nolock)
								inner join v_PayType pt2 (nolock) on pt2.PayType_id = eu2.PayType_id and pt2.PayType_SysNick = 'oms'
							...
							where
								...
								and ISNULL(uca2.UslugaComplexAttribute_begDate, :EvnSection_disDate) <= :EvnSection_disDate
								and ISNULL(uca2.UslugaComplexAttribute_endDate, :EvnSection_disDate) >= :EvnSection_disDate
					...
					where
						...
						and ISNULL(uca1.UslugaComplexAttribute_begDate, :EvnSection_disDate) <= :EvnSection_disDate
						and ISNULL(uca1.UslugaComplexAttribute_endDate, :EvnSection_disDate) >= :EvnSection_disDate
				", [
					'EvnSection_disDate' => $data['EvnSection_disDate']
				]);
					...
					FROM
						...
						cross apply (
							...
							where
								av2.AttributeValue_rid = av.AttributeValue_id
								and a2.Attribute_TableName = 'dbo.UslugaComplex'
								and av2.AttributeValue_ValueIdent IN (select eu.UslugaComplex_id from v_EvnUsluga eu (nolock) where eu.EvnUsluga_pid IN ('" . implode("','", $data['EvnSectionIds']) . "') and eu.EvnClass_id in (43,22,29) and eu.PayType_id = :PayTypeOms_id)
						) UC1FILTER
						cross apply (
							select
								av2.AttributeValue_ValueIdent
		...
		if (
			...
			if (strtotime($data['EvnSection_disDate']) >= strtotime('01.04.2017')) {
				...
				$resp = $this->queryResult("
					declare
						@AttributeVision_TablePKey bigint = (select top 1 TariffClass_id from v_TariffClass (nolock) where TariffClass_SysNick = 'svrgos');
							...
							$query = "
								...
								from
									...
									outer apply(
										...
										from
											v_MesOld mo2 (nolock)
											inner join v_MesLink ml (nolock) on ml.Mes_sid = mo2.Mes_id
										where
											ml.Mes_id = mo.Mes_id
											and mo2.Mes_begDT <= :EvnSection_disDate
											and (IsNull(mo2.Mes_endDT, :EvnSection_disDate)>= :EvnSection_disDate)
			...
			where
				...
				and exists (
					select top 1
						LpuSectionFinans_id
					from
						LpuSectionFinans with (NOLOCK) 
						inner join PayType with (nolock) on PayType.PayType_id = LpuSectionFinans.PayType_id
					where
