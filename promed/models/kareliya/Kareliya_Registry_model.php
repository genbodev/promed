...
class Kareliya_Registry_model extends Registry_model {
	...
	function loadRegistryNoPolis($data)
	{
		...
		if (!empty($data['ExtendedFilterField']) && !empty($data['ExtendedFilterFieldValue'])) {
			$filters .= " and " . $data['ExtendedFilterField'] . " like '%' + :ExtendedFilterFieldValue + '%'";
	...
	function SetXmlPackNum($data) {
		...
		if ($data['KatNasel_SysNick'] == 'oblast') {
			$where .= " and kn.KatNasel_SysNick like 'oblast'";
		}
		else {
		...
		if ($this->RegistryType_id == 13) {
			$query = "
				...
				select
					...
					ISNULL(kn.KatNasel_SysNick, 'all') as KatNasel_SysNick,
					pt.PayType_SysNick,
					UR.Registry_IsZNO,
					...
					IsNull(UR.Registry_IsLocked, 1) as Registry_IsLocked,
			...
			$query = "
				select
					...
					RDSum.RegistryData_Count as RegistryData_Count,
					IsNull(R.Registry_IsLocked, 1) as Registry_IsLocked,
					IsNull(R.RegistryCheckStatus_id,0) as RegistryCheckStatus_id,
					IsNull(rcs.RegistryCheckStatus_Code,-1) as RegistryCheckStatus_Code,
					rcs.RegistryCheckStatus_Name as RegistryCheckStatus_Name,
					SUBSTRING(CONVERT(varchar(10), R.Registry_endDate, 112), 3, 4) as Registry_endMonth -- для использования в имени формируемых файлов https://redmine.swan.perm.ru/issues/6547
					,convert(varchar(10), Registry_begDate, 112) as Registry_begDate
				from {$this->scheme}.v_Registry R with (nolock)
	...
	function loadRegistryData($data)
	{
		...
		if ($data['RegistryType_id'] == 14) {
			$join .= " left join v_EvnSection es with (nolock) on es.EvnSection_id = RD.Evn_sid ";
		...
		if ( in_array($data['RegistryType_id'], $this->getAllowedRegistryTypes()) )
		{
            ...
            if ($data['RegistryType_id'] == 2) {
				...
				";
				$join .= "
					left join v_Diag DG with (NOLOCK) on DG.Diag_id = EPL.Diag_id
					outer apply (
						select
						...
						and exists (
							select 
								EvnVizit_id
							from
								v_EvnVizit with (nolock)
							where
								EvnVizit_id = RD.Evn_id
						)
					)vVC
					outer apply (
						select top 1
            ...
            if ($data['RegistryType_id'] == 1 || $data['RegistryType_id'] == 14) {
            	...
            	$fields .= "LTRIM(DG.Diag_Code + ' ' + DG.Diag_Name) as Diag_Name,";
				$join .= " left join Diag DG with (NOLOCK) on DG.Diag_id = RD.Diag_id ";
            }

			if ($data['RegistryType_id'] == 6) {
				$fields .= "
					case
						when ISNULL(CLC.Lpu_CodeSMO, CCC.Lpu_CodeSMO) is null then ''
						when ISNULL(CLC.Lpu_CodeSMO, CCC.Lpu_CodeSMO) = Lpu.Lpu_f003mcod then 'Да'
				...
				";
				$join .= "
					left join v_CmpCallCard CCC with (nolock) on CCC.CmpCallCard_id = RD.Evn_id
					left join v_CmpCloseCard CLC with (nolock) on CLC.CmpCallCard_id = CCC.CmpCallCard_id
					outer apply (
						select top 1 Lcmp.Lpu_Nick
						from v_Lpu Lcmp with (NOLOCK)
						where ISNULL(CLC.Lpu_CodeSMO, CCC.Lpu_CodeSMO) = Lcmp.Lpu_f003mcod
					) CMPAttachLpu
				";
			}
	...
	function setErrorFromTFOMSImportRegistry($data) {
		...
		$query = "
			...
			where RegistryErrorType_Name like :COMMENT
		";
		...
		if (count($resp) > 0) {
			...
			$query = "
				declare
					...
					@RegistryErrorType_Code int,
					@Error_Code bigint,
					@Error_Message varchar(4000);

				set	@RegistryErrorType_Code = isnull((
					select top 1 max(RegistryErrorType_Code)
					from {$this->scheme}.RegistryErrorType with(nolock)
	...
	function setErrorFromImportRegistry($data)
	{
		if ($data['Registry_id']>0)
		{
			$query = "SELECT TOP 1 RegistryErrorType_id, RegistryErrorType_Descr FROM {$this->scheme}.RegistryErrorType with (nolock) WHERE RegistryErrorType_Code = :OSHIB";
	 ...
	 *	Добавить ошибку для реестра из импортируемого файла
	 */
	function setFLKErrorFromImportRegistry($data)
	{
		if ($data['Registry_id']>0)
		{
			$resp_ret = $this->queryResult("SELECT TOP 1 RegistryErrorType_id, RegistryErrorType_Descr FROM {$this->scheme}.RegistryErrorType with (nolock) WHERE RegistryErrorType_Code = :OSHIB", array(
				'OSHIB' => $data['OSHIB']
			));
	...
	function addNewPolisToPerson($data)
	{
		if (!empty($data['oldPolis'])) {
			/*$query = "
				...
				select @ErrMsg as ErrMsg
			";*/
			$query = "update Polis with (rowlock) set Polis_endDate = :Polis_endDate where Polis_id = :Polis_id";
	 ...
	 *	Какая-то проверка
	 */
	function checkErrorDataInRegistry($data, $isLite = true)
	{
		$this->setRegistryParamsByType($data, true);
		...
		if (!empty($data['Evn_sid'])) {
			...
			if (!empty($data['PODR'])) {
				...
				$additionalFilter .= " and ISNULL(substring(rtrim(rd.LpuSection_Code),3,len(rtrim(rd.LpuSection_Code))), :PODR) = :PODR";
			}

			if (!empty($data['NOVOR']) && mb_strlen($data['NOVOR']) > 1) {
				$params['NOVOR'] = mb_substr($data['NOVOR'], -1);
				$additionalFilter .= " and CAST(isnull(NewBorn.PersonNewBorn_CountChild,isnull(BirthSvid.BirthSvid_ChildCount,1)) AS CHAR(1)) = :NOVOR";

				// жестокий кусок из реестров, необходимый для определения правильного случая ребенка.
				$oapply = "
					outer apply (
						select top 1 rd1.Evn_id,BirthSpecStac_OutcomDT,BirthSpecStac_id
						from v_BirthSpecStac BBS with (nolock)
						inner join r10.RegistryDataEvnPS rd1 with (nolock) on rd1.Evn_id = BBS.EvnSection_id and rd1.Registry_id = rd.Registry_id
						where rd1.Evn_rid = rd.Evn_rid
						and rd1.EvnSection_NumGroup1 = rd.EvnSection_NumGroup1
						and rd1.Evn_sid = rd.Evn_sid
					)BBS
					outer apply (
						select EvnPSC.EvnPS_id, Person_BirthDay,sex_id,EvnPSC.person_id,Diag_Code,PersonState.Person_SurName,PersonState.Person_FirName,PersonState.Person_SecName
						from  v_PersonNewBorn PersonNewBorn with (nolock)  
						inner join v_EvnPS EvnPSC with (nolock) on  EvnPSC.EvnPS_id=PersonNewBorn.EvnPS_id
						inner join v_EvnSection ES_Child with (nolock) on rd.Evn_sid = ES_Child.EvnSection_id and ES_Child.EvnSection_pid = EvnPSC.EvnPS_id
						inner join v_personstate PersonState  with (nolock) on EvnPSC.person_id=PersonState.person_id
						inner join v_diag diagchild with (nolock) on EvnPSC.diag_id=diagchild.diag_id
							where  PersonNewBorn.BirthSpecStac_id = BBS.BirthSpecStac_id
					)EvnPS_Child
					outer apply (
						select
						top 1 PersonNewBorn.PersonNewBorn_CountChild
						from PersonNewBorn with (nolock) 
						where  PersonNewBorn.Person_id=isnull(EvnPS_Child.person_id,rd.Person_id)
						order by PersonNewBorn_id desc
					) NewBorn
					outer apply (
						select
						top 1 BS.BirthSvid_ChildCount
						from v_BirthSvid BS with (nolock) 
						where  BS.Person_id=isnull(EvnPS_Child.person_id,rd.Person_id)
		...
		foreach ( $registryListByType as $RegistryType_id => $RegistryArray ) {
			...
			if ( $this->RegistryType_id == 6 ) {
				...
				$EvnClassCodeField = 'ec.EvnClass_Code';
				$join = "
					inner join Evn e (nolock) on e.Evn_id = rd.Evn_id
					inner join EvnClass ec (nolock) on ec.EvnClass_id = e.EvnClass_id
				";
			}
		...
		if ( !is_array($EvnData) || count($EvnData) == 0 ) {
			$EvnData = $this->getFirstRowFromQuery("
				...
				from
					{$this->scheme}.v_RegistryGroupLink RGL (nolock)
					inner join {$this->scheme}.v_Registry r (nolock) on r.Registry_id = rgl.Registry_id
					inner join {$this->scheme}.RegistryDataEvnPS rd (nolock) on rd.Registry_id = r.Registry_id
						and Number is not null
					inner join v_EvnSection es (nolock) on  es.EvnSection_id = rd.Evn_sid
					inner join v_EvnPS eps (nolock) on eps.EvnPS_id = es.EvnSection_pid
					inner join RegistryType rt (nolock) on rt.RegistryType_id = r.RegistryType_id
					inner join Evn e (nolock) on e.Evn_id = rd.Evn_id
					inner join EvnClass ec (nolock) on ec.EvnClass_id = e.EvnClass_id
					{$oapply}
				where
			...
			$personQuery1 = "
				cross apply (
					select top 1
						...
						pbdz.Person_EdNum
					from
						PersonDeputy PDEP with (nolock)
						inner join v_Person_bdz pbdz with (nolock) on pbdz.Person_id = PDEP.Person_pid
					where
			...
			default:
				...
				$query = "
					...
					select top 1
						...
						ps.Person_Secname,
						convert(varchar(10),ps.Person_Birthday, 120) as Person_Birthday,
						convert(varchar(10), case when ec.EvnClass_SysNick = 'EvnVizitPL' then rd2.Evn_disDate else rd2.Evn_disDate end, 120) as Evn_disDate,
						case when pol.PolisType_id = 4 then ps.Person_EdNum else pol.Polis_Num end as Polis_Num,
						pol.OmsSprTerr_id,
						pol.PolisType_id,
						pol.OrgSMO_id,
					...
					from
						...
						inner join EvnClass ec (nolock) on ec.EvnClass_id = e.EvnClass_id
						{$personQuery2}
						left join {$this->scheme}.{$this->RegistryDataObject} rd2 (nolock) on rd2.Registry_id = rd.Registry_id and rd2.Evn_id = rd.Evn_id -- дата конца случая по поликлинике берётся отсюда (а именно конец ТАП)
						left join v_Polis pol (nolock) on pol.Polis_id = ps.Polis_id
					where
						r.Registry_id = :Registry_id
	 ...
	 */
	function checkErrorDataInRegistryBySLID($data)
	{
		$params = array();
		$params['Registry_id'] = $data['Registry_id'];
		...
		foreach ( $registryListByType as $RegistryType_id => $RegistryArray ) {
			...
			if ( $this->RegistryType_id == 6 ) {
				$join = "
					left join v_CmpCloseCard clc with (nolock) on clc.CmpCallCard_id = rd.Evn_id
				";
				$select = '111 as EvnClass_Code, clc.CmpCloseCard_id';
				...
				$join = "
					...
					inner join EvnClass ec (nolock) on ec.EvnClass_id = e.EvnClass_id
				";
				$select = 'ec.EvnClass_Code, null as CmpCloseCard_id';
	...
	public function loadRegistryErrorTFOMS($data)
	{
		...
		if ($this->RegistryType_id == 14) {
			$joinList[] = "left join v_EvnSection ES2 with (nolock) on es2.EvnSection_id = RE.Evn_sid";
			$Evn_ident = "ES2.EvnSection_pid as Evn_ident,";

			if (!empty($data['Evn_id'])) {
				$filterList[] = "ES2.EvnSection_pid = :Evn_id";
				$params['Evn_id'] = $data['Evn_id'];
			}
			...
			case 6:
				$query = "
					...
					from 
						...
						left join v_CmpCallCard ccc (nolock) on ccc.CmpCallCard_id = RE.Evn_id
						left join v_CmpCloseCard clc (nolock) on clc.CmpCallCard_id = ccc.CmpCallCard_id
						left join v_LpuSection LS (nolock) on LS.LpuSection_id = ISNULL(clc.LpuSection_id, ccc.LpuSection_id)
						...
						outer apply(
							select top 1 Person_Fio from v_MedPersonal with (nolock) where MedPersonal_id = ISNULL(clc.MedPersonal_id, ccc.MedPersonal_id)
						) as MP
						outer apply (
							select top 1
								PersonEvn_id,
	...
	function checkCmpCallCardInRegistry( $data ){
		...
		$sql = "
			...
			from 
				...
				inner join {$this->scheme}.RegistryDataCmp rd with (nolock) on rd.CmpCallCard_id = ccc.CmpCallCard_id
	...
	function saveUnionRegistry($data)
	{
		...
		$KatNasel_Code = $this->getFirstResultFromQuery("select top 1 KatNasel_Code from v_KatNasel (nolock) where KatNasel_id = :KatNasel_id", $data);
		...
		$query = "
			...
			where
				...
				and year(R.Registry_accDate) = year(:Registry_accDate)
				and R.Registry_id <> ISNULL(:Registry_id, 0)
		...
		$query = "
			declare
				@Error_Code bigint,
				@Error_Message varchar(4000),
				@pt bigint = (select top 1 PayType_id from v_PayType with (nolock) where PayType_SysNick = 'oms'),
			...
			if (count($resp) > 0 && !empty($resp[0]['Registry_id'])) {
					...
					switch ($data['RegistryGroupType_id']) {
						case 1:
							if ( $data['Registry_begDate'] < '2018-11-01' ) {
								...
								$registrytypefilter = " and R.RegistryType_id in (1, 2, 6) and ISNULL(R.Registry_IsZNO, 1) = ISNULL(:Registry_IsZNO, 1)";
							}
							break;
						case 2:
				...
				$query = "
					...
					where
						...
						and not exists (
							select top 1 rgl.RegistryGroupLink_id
	...
	function getLoadRegistryQueueAdditionalFields() {
		return ', R.DispClass_id, ISNULL(R.Registry_IsLocked, 1) as Registry_IsLocked, R.Registry_IsOnceInTwoYears, R.Registry_IsZNO';
	}
	 ...
	 */
	function loadRegistryDataForXmlUsing($type, $data, &$number, &$Registry_EvnNum)
	{
	...
	public function loadRegistryDataForXmlUsing2018($type, $data, &$number, &$Registry_EvnNum) {
		...
		// Схемы (SHEMA)
		if ( !empty($p_shema) ) {
			$query = "exec {$p_shema} @Registry_id = :Registry_id";
			$result = $this->db->query($query, array('Registry_id' => $data['Registry_id']));
		...
		if ( !empty($p_regkr) ) {
			$query = "exec {$p_regkr} @Registry_id = :Registry_id";
			$result = $this->db->query($query, array('Registry_id' => $data['Registry_id']));
	...
	function setRegistryIsLocked($data) {
		...
		$query = "
			...
			begin try
				...
				if ( (select RegistryType_id from r10.Registry with (nolock) where Registry_id = :Registry_id) = 13 )
					begin
						update r10.Registry with (rowlock)
						set
							Registry_IsLocked = :Registry_IsLocked,
							Registry_updDT = dbo.tzGetDate(),
							pmUser_updID = :pmUser_id
						where Registry_id in (select Registry_id from r10.RegistryGroupLink (nolock) where Registry_pid = :Registry_id)
					end
			end try
	 ...
	 */
	function cancelRegistryIdentification($data) {
	...
	function checkRegistryIsBlocked($data) {
		$query = "
			select top 1 ISNULL(Registry_IsLocked, 1) as Registry_IsLocked
			from {$this->scheme}.v_Registry r with (nolock)
	...
	function setRegistryStatus($data) {
		...
		else if ($RegistryStatus_id == 3 && $data['RegistryStatus_id'] == 2){// если переводим из "В работе" в "К оплате" 
			$errCnt = $this->getFirstResultFromQuery("
				select top 1 Evn_id from {$this->scheme}.v_RegistryDouble with(nolock) where Registry_id = :Registry_id
				union all
				select top 1 Evn_id from {$this->scheme}.v_RegistryCmpDouble with(nolock) where Registry_id = :Registry_id
			", ['Registry_id' => $data['Registry_id']]);

			if (empty($data['CheckingDuplicates']) && $errCnt !== false && !empty($errCnt)) {
				return array(
					'Error_Msg' => 'YesNo',
					'Alert_Msg' => 'При формировании данного реестра были выявлены случаи-дубли. Рекомендуется исправить данные случаи, иначе они не будут включены в выгрузку. Вы уверены, что хотите перевести реестр «К оплате»?',
					'success' => true
				);
			}
		} 
	 ...
	 */
	function setLpuCodeCMOForEvn($data) {
			...
			case 6:
				$query = "
					select
						...
						,RTrim(IsNull(convert(varchar,cast(RD.Person_BirthDay as datetime),104),'')) as Person_BirthDay
						,ISNULL(CLC.Year_num, CCC.CmpCallCard_Ngod) as Evn_Num
						,ETS.EmergencyTeamSpec_Name as LpuSection_FullName
						,MP.Person_Fio as MedPersonal_Fio
						,convert(varchar(10), ISNULL(CCC.CmpCallCard_prmDT, CLC.AcceptTime), 104) as Evn_setDate
						,CLC.CmpCloseCard_id
						-- end select
					from
						-- from
						...
						left join v_CmpCallCard CCC with (nolock) on CCC.CmpCallCard_id = RD.Evn_id
						left join v_CmpCloseCard CLC with (nolock) on CLC.CmpCallCard_id = CCC.CmpCallCard_id
						left join v_EmergencyTeamSpec ETS with (nolock) on ETS.EmergencyTeamSpec_id = CLC.EmergencyTeamSpec_id
						outer apply(
							select top 1 Person_Fio, MedPersonal_id from v_MedPersonal with(nolock) where MedPersonal_id = ISNULL(CLC.MedPersonal_id, CCC.MedPersonal_id)
						) as MP
						-- end from
					where
						-- where
	...
	function checkEvnInRegistry($data, $action = 'delete') {
			...
			if ( is_array($resp) && count($resp) > 0 && in_array($resp[0]['DispClass_Code'], array(4, 8, 11, 12)) ) {
				...
				$query = "
					SELECT 
						...
						convert(varchar(10), Registry.Registry_accDate, 104) as Registry_accDate
					FROM {$this->scheme}.{$this->RegistryDataObject} RegistryData with (nolock)
						INNER JOIN {$this->scheme}.Registry Registry ON RegistryData.Registry_id = Registry.Registry_id
							...
							AND isnull(Registry.Registry_deleted,1) = 1
						INNER JOIN KatNasel on KatNasel.KatNasel_id = Registry.KatNasel_id
							AND KatNasel_SysNick != 'all'
						inner join v_EvnVizitDisp EVD with (nolock) on EVD.EvnVizitDisp_id = RegistryData.Evn_id
					WHERE isnull(RegistryData.RegistryData_IsPaid, 1) = 2
					...
					SELECT 
						...
						convert(varchar(10), Registry.Registry_accDate, 104) as Registry_accDate
					FROM {$this->scheme}.{$this->RegistryDataObject} RegistryData with (nolock)
						INNER JOIN {$this->scheme}.Registry Registry with (nolock) ON RegistryData.Registry_id = Registry.Registry_id
					...
					WHERE RegistryData.number IS NOT NULL
						and isnull(RegistryData.RegistryData_IsPaid, 1) = 2
						and EVD.EvnVizitDisp_pid = :Evn_id
				";
		...
		$query = "
			...
			where
				" . implode(' and ', $filterList) . "
				and (
					ISNULL(RD.RegistryData_IsPaid, 1) = 2
	 ...
	 * Идентификация людей
	 */
	function importRegistryErrorTfomsForIdent($data, $xmlfilepath, $xmlfilepath_pers) {
		$recall = 0;
		...
		$this->db->query("
			delete from {$this->scheme}.RegistryImportTFOMS with (rowlock) WHERE Registry_id = :Registry_id
		", array(
			'Registry_id' => $data['Registry_id']
		));
		...
		$data['RegistryCheckStatus_id'] = $this->getFirstResultFromQuery("select top 1 RegistryCheckStatus_id from v_RegistryCheckStatus (nolock) where RegistryCheckStatus_Code = :RegistryCheckStatus_Code", array('RegistryCheckStatus_Code' => 2), true);
		$this->setRegistryCheckStatus(array(
			'Registry_id' => $data['Registry_id'],
	 ...
	 */
	function identifyRegistryErrorTFOMS($data) {
		...
		$resp_r = $this->queryResult("
			declare @RegistryCheckStatus_id bigint = (select top 1 RegistryCheckStatus_id from v_RegistryCheckStatus (nolock) where RegistryCheckStatus_Code = '2');
		...
		foreach($resp_r as $resp_rone) {
			...
			try {
				...
				// берём "в работу"
				$data['RegistryCheckStatus_id'] = $this->getFirstResultFromQuery("select top 1 RegistryCheckStatus_id from v_RegistryCheckStatus (nolock) where RegistryCheckStatus_Code = :RegistryCheckStatus_Code", array('RegistryCheckStatus_Code' => 3), true);
				$this->setRegistryCheckStatus(array(
					'Registry_id' => $data['Registry_id'],
					'RegistryCheckStatus_id' => $data['RegistryCheckStatus_id'],
					'pmUser_id' => $data['pmUser_id']
				...
				$resp = $this->queryResult("
					select
						*
					from
						{$this->scheme}.v_RegistryImportTFOMS (nolock)
					where
						Registry_id = :Registry_id
				", array(
					'Registry_id' => $data['Registry_id']
				...
				foreach ($resp as $respone) {
					...
					if (preg_match("/^(\d+)(_SMP){1}$/", $respone['ID_PAC'], $matches)) {
					...
					$evnData = $this->checkErrorDataInRegistry(array(
						...
						'setDT' => !empty($respone['DATE_1'])?$respone['DATE_1']->format('Y-m-d'):null,
						'disDT' => !empty($respone['DATE_2'])?$respone['DATE_2']->format('Y-m-d'):null,
						'PODR' => $respone['PODR'],
						'NOVOR' => $respone['NOVOR'],
					...
					if (
						...
						$params = array(
							...
							'RegistryErrorType_id' => $this->getFirstResultFromQuery("select top 1 RegistryErrorType_id from {$this->scheme}.v_RegistryErrorType (nolock) where RegistryErrorType_Code = '1020'"),
							'RegistryErrorType_Code' => '1020',
							'COMMENT' => $this->getFirstResultFromQuery("select top 1 RegistryErrorType_Name from {$this->scheme}.v_RegistryErrorType (nolock) where RegistryErrorType_Code = '1020'"),
					...
					if ($respone['OrgSMO_id'] === false) {
						...
						$params = array(
							...
							'Evn_sid' => $evnData['Evn_sid'],
							'Registry_id' => $evnData['Registry_id'],
							'RegistryErrorType_id' => $this->getFirstResultFromQuery("select top 1 RegistryErrorType_id from {$this->scheme}.v_RegistryErrorType (nolock) where RegistryErrorType_Code = '1029'"),
							'RegistryErrorType_Code' => '1029',
							'COMMENT' => $this->getFirstResultFromQuery("select top 1 RegistryErrorType_Name from {$this->scheme}.v_RegistryErrorType (nolock) where RegistryErrorType_Code = '1029'"),
							'RegistryType_id' => $evnData['RegistryType_id'],
							'pmUser_id' => $data['pmUser_id'],
							'RegistryErrorTFOMS_FieldName' => '',
							'RegistryErrorTFOMS_BaseElement' => ''
					...
					if ($respone['PolisType_id'] === false) {
						...
						$params = array(
							...
							'Evn_sid' => $evnData['Evn_sid'],
							'Registry_id' => $evnData['Registry_id'],
							'RegistryErrorType_id' => $this->getFirstResultFromQuery("select top 1 RegistryErrorType_id from {$this->scheme}.v_RegistryErrorType (nolock) where RegistryErrorType_Code = '1028'"),
							'RegistryErrorType_Code' => '1028',
							'COMMENT' => $this->getFirstResultFromQuery("select top 1 RegistryErrorType_Name from {$this->scheme}.v_RegistryErrorType (nolock) where RegistryErrorType_Code = '1028'"),
							'RegistryType_id' => $evnData['RegistryType_id'],
							'pmUser_id' => $data['pmUser_id'],
							'RegistryErrorTFOMS_FieldName' => '',
							'RegistryErrorTFOMS_BaseElement' => ''
						...
						switch ($resp) {
							case 1:
								...
								$params = array(
									...
									'Evn_sid' => $evnData['Evn_sid'],
									'Registry_id' => $evnData['Registry_id'],
									'RegistryErrorType_id' => $this->getFirstResultFromQuery("select top 1 RegistryErrorType_id from {$this->scheme}.v_RegistryErrorType (nolock) where RegistryErrorType_Code = '1025'"),
									'RegistryErrorType_Code' => '1025',
									'COMMENT' => $this->getFirstResultFromQuery("select top 1 RegistryErrorType_Name from {$this->scheme}.v_RegistryErrorType (nolock) where RegistryErrorType_Code = '1025'"),
									'RegistryType_id' => $evnData['RegistryType_id'],
									'pmUser_id' => $data['pmUser_id'],
									'RegistryErrorTFOMS_FieldName' => '',
									'RegistryErrorTFOMS_BaseElement' => ''
							...
							case 2:
								...
								$params = array(
									...
									'Evn_sid' => $evnData['Evn_sid'],
									'Registry_id' => $evnData['Registry_id'],
									'RegistryErrorType_id' => $this->getFirstResultFromQuery("select top 1 RegistryErrorType_id from {$this->scheme}.v_RegistryErrorType (nolock) where RegistryErrorType_Code = '1026'"),
									'RegistryErrorType_Code' => '1026',
									'COMMENT' => $this->getFirstResultFromQuery("select top 1 RegistryErrorType_Name from {$this->scheme}.v_RegistryErrorType (nolock) where RegistryErrorType_Code = '1026'"),
									'RegistryType_id' => $evnData['RegistryType_id'],
									'pmUser_id' => $data['pmUser_id'],
									'RegistryErrorTFOMS_FieldName' => '',
									'RegistryErrorTFOMS_BaseElement' => ''
							...
							case 3:
								...
								$params = array(
									...
									'Evn_sid' => $evnData['Evn_sid'],
									'Registry_id' => $evnData['Registry_id'],
									'RegistryErrorType_id' => $this->getFirstResultFromQuery("select top 1 RegistryErrorType_id from {$this->scheme}.v_RegistryErrorType (nolock) where RegistryErrorType_Code = '1027'"),
									'RegistryErrorType_Code' => '1027',
									'COMMENT' => $this->getFirstResultFromQuery("select top 1 RegistryErrorType_Name from {$this->scheme}.v_RegistryErrorType (nolock) where RegistryErrorType_Code = '1027'"),
									'RegistryType_id' => $evnData['RegistryType_id'],
									'pmUser_id' => $data['pmUser_id'],
									'RegistryErrorTFOMS_FieldName' => '',
									'RegistryErrorTFOMS_BaseElement' => ''
				...
				$data['RegistryCheckStatus_id'] = $this->getFirstResultFromQuery("select top 1 RegistryCheckStatus_id from v_RegistryCheckStatus (nolock) where RegistryCheckStatus_Code = :RegistryCheckStatus_Code", array('RegistryCheckStatus_Code' => 4), true);
				$this->setRegistryCheckStatus(array(
					'Registry_id' => $data['Registry_id'],
					'RegistryCheckStatus_id' => $data['RegistryCheckStatus_id'],
					'pmUser_id' => $data['pmUser_id']
				...
				$data['RegistryCheckStatus_id'] = $this->getFirstResultFromQuery("select top 1 RegistryCheckStatus_id from v_RegistryCheckStatus (nolock) where RegistryCheckStatus_Code = :RegistryCheckStatus_Code", array('RegistryCheckStatus_Code' => 5), true);
				$this->setRegistryCheckStatus(array(
					'Registry_id' => $data['Registry_id'],
					'RegistryCheckStatus_id' => $data['RegistryCheckStatus_id'],
					'pmUser_id' => $data['pmUser_id']
	...
	public function loadRegistryEntiesSettings($data)
	{
		...
		$query = "
			...
			WHERE 
				...
				AND FLKSettings_EvnData LIKE '%kareliya%'
		".$where;
		$result = $this->db->query($query, $params);
		if (is_object($result))
		{
	...
	*/
	...
	public function loadRegistry($data) {
		...
		if ( empty($data['Registry_id']) ) {
			if ( !empty($data['PayType_SysNick']) && $data['PayType_SysNick'] == 'bud' ) {
				...
				$filter .= " and ISNULL(pt.PayType_SysNick, '') not in ('bud', 'fbud', 'subrf')";
			}
		}
		...
		// Готовые реестры
			...
			$query = "
				Select
					...
					isnull(R.Registry_Sum, 0.00) as Registry_Sum,
					" . ($source_table == 'v_Registry' ? "isnull(R.Registry_SumPaid, 0.00) as Registry_SumPaid," : "") . "
					" . ($source_table == 'v_Registry' ? "isnull(RegistryStom.RegistryStom_KdPlan, 0) as RegistryStom_KdPlan," : "") . "
					...
					RegistryErrorMZ.RegistryErrorMZ_IsData,
				...
				from {$this->scheme}.{$source_table} R with (NOLOCK)
					...
					outer apply(select top 1 case when RE.Registry_id is not null then 1 else 0 end as RegistryErrorMZ_IsData, RegistryHealDepResErr_id from v_RegistryHealDepResErr RE with (NOLOCK) where RE.Registry_id = R.Registry_id) RegistryErrorMZ
				where
					{$filter}
				order by
					R.Registry_endDate DESC,
	...
	protected function _loadRegistrySCHETForXmlUsing($data) {
		...
		$header = $this->getFirstRowFromQuery(
			"select top 1 * from {$this->scheme}.p_Registry_expScet{$postfix}_f(:Registry_id)",
			[ 'Registry_id' => $data['Registry_id'] ]
		);
	...
	protected function _loadRegistryDataForXmlByFunc($type, $data) {
		...
		$result = $this->db->query("
			...
			from
				#zsl z (nolock)
				inner join #sl s (nolock) on s.Evn_id = z.Evn_id
				inner join #pers p (nolock) on p.Evn_id = z.Evn_id
			order by
