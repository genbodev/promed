...
class FRMO_model extends swModel {
	...
	public function parseFRMOData($data) {
		try {
			...
			foreach ( $this->_files_MO as $Lpu_id => $xmlfile ) {
				...
				$MO_Data['id'] = $this->getFirstResultFromQuery('select top 1 id from tmp.FRMOData_MO with (nolock) where Lpu_id = :Lpu_id and updDT is null', $MO_Data, true);

				if ( !empty($MO_Data['id']) ) {
					$query = "
						update tmp.FRMOData_MO with (rowlock)
						set
				...
				$resp = $this->getFirstRowFromQuery("
					...
					select @Res as id, @Error_Code as Error_Code, @Error_Message as Error_Msg;
				", $MO_Data);
			...
			foreach ( $this->_files_MO_Building as $Lpu_id => $xmlfile ) {
				...
				foreach ( $xml->building as $building ) {
					...
					$MO_Building_Data['id'] = $this->getFirstResultFromQuery('select top 1 id from tmp.FRMOData_MO_Building with (nolock) where Lpu_id = :Lpu_id and FRMO_id = :FRMO_id and updDT is null', $MO_Building_Data, true);

					if ( !empty($MO_Building_Data['id']) ) {
						$query = "
							update tmp.FRMOData_MO_Building with (rowlock)
							set
								" . implode(", ", $updateParamsList) . ",
								insDT = dbo.tzGetDate()
							where id = :id
			...
			foreach ( $this->_files_MO_Depart as $Lpu_id => $xmlfile ) {
				...
				foreach ( $xml->depart as $depart  ) {
					...
					if ( !empty($MO_Depart_Data['id']) ) {
						$query = "
							update tmp.FRMOData_MO_Depart with (rowlock)
							set
								" . implode(", ", $updateParamsList) . ",
								insDT = dbo.tzGetDate()
							where id = :id
					...
					if ( property_exists($depart, 'departHospital') && property_exists($depart->departHospital, 'hospitalSubdivisions') ) {
						foreach ( $depart->departHospital->hospitalSubdivisions->hospitalSubdivision as $hospitalSubdivision ) {
							...
							if ( !empty($MO_Subdivision_Data['id']) ) {
								$query = "
									update tmp.FRMOData_MO_Subdivision with (rowlock)
									set
										" . implode(", ", $updateParamsList) . ",
										insDT = dbo.tzGetDate()
									where id = @Res
							...
							$this->db->query("delete from tmp.FRMOData_MO_HospitalSubdivisionBed where Subdivision_id = :Subdivision_id and updDT is null", array('Subdivision_id' => $MO_Subdivision_Data['id']));
					...
					if ( property_exists($depart, 'departLabs') ) {
						foreach ( $depart->departLabs->departLab as $departLab ) {
							...
							$MO_DepartLab_Data['id'] = $this->getFirstResultFromQuery('select top 1 id from tmp.FRMOData_MO_DepartLab with (nolock) where Lpu_id = :Lpu_id and oid = :oid and updDT is null', $MO_DepartLab_Data, true);

							if ( !empty($MO_DepartLab_Data['id']) ) {
								$query = "
									update tmp.FRMOData_MO_DepartLab with (rowlock)
									set
										" . implode(", ", $updateParamsList) . ",
										insDT = dbo.tzGetDate()
									where id = @Res
