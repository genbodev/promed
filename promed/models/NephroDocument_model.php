...
class NephroDocument_model extends swModel
{
	...
	function loadViewData($data) {
		...
		$query = "
			select 
				...
				ND.NephroDocument_id,
				ND.NephroDocument_filename,
				ND.NephroDocument_description,
				:MorbusNephro_id as MorbusNephro_id,
				:MorbusNephro_pid as MorbusNephro_pid
			from r2.v_NephroDocument ND with(nolock)
			left join v_MorbusNephro MN with(nolock) on MN.MorbusNephro_id = ND.MorbusNephro_id
	...
	function NephroDocument_ins($params) {
		$query = "
			DECLARE
				...
				@Error_Code int,
				@NephroDocument_id bigint
			
			EXEC r2.p_NephroDocument_ins
				...
				@NephroDocument_id = @NephroDocument_id output;
	...
	function getDocument($data){
		...
		$query = "
			...
			from r2.NephroDocument with(nolock)
