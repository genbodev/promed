...
class PrivilegeAccessRights_model extends swModel {
	/**
	 * Конструктор
	 */
	...
	function savePrivilegeAccessRights($data)
	{
		...
		if ($data['RecordStatus_isNewRecord'] == 1) {
			$query = "
				...
				from v_PrivilegeAccessRights with (nolock)
	...
	function savePrivilegeAccessRightsUnit($data) {
		$query = "
			...
			set @Res = :PrivilegeAccessRights_id;
			exec p_PrivilegeAccessRights_" . (!empty($data['PrivilegeAccessRights_id']) && $data['PrivilegeAccessRights_id'] > 0 ? "upd" : "ins") . "
				@PrivilegeAccessRights_id = @Res output,
	...
	function deletePrivilegeAccessRights($data)
	{
		...
		$query = "
			select PrivilegeAccessRights_id
			from v_PrivilegeAccessRights with (nolock)
			where PrivilegeType_id = :PrivilegeType_id
		";
	...
	function deletePrivilegeAccessRightsUnit($data)
	{
		$query = "
			declare
				@ErrCode int,
				@ErrMessage varchar(4000);
			exec p_PrivilegeAccessRights_del
	...
	function loadPrivilegeAccessRightsGrid($data)
	{
		...
		$query = "
			select distinct
				PAR.PrivilegeType_id,
				PT.PrivilegeType_Name,
				PAR.PrivilegeAccessRights_UserGroups,
				PAR1.PrivilegeAccessRights_HasLpu
			from
				v_PrivilegeAccessRights PAR with(nolock)
				left join v_PrivilegeType PT with(nolock) on PT.PrivilegeType_id = PAR.PrivilegeType_id
				outer apply (
					select
						(CASE COUNT(Lpu_id) WHEN 0 THEN 0 ELSE 1 END) PrivilegeAccessRights_HasLpu
					from v_PrivilegeAccessRights with(nolock)
	...
	function loadPrivilegeAccessRightsLpuGrid($data)
	{
		...
		$query = "
			select
				PAR.PrivilegeAccessRights_id,
				PAR.Lpu_id,
				L.Lpu_Nick,
				1 as RecordStatus_Code
			from v_PrivilegeAccessRights PAR with(nolock)
				left join v_Lpu L with(nolock) on L.Lpu_id = PAR.Lpu_id
			where
