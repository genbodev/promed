 ...
 * @author yunitsky
 */
class ReportEndUser_model extends CI_Model {
    /**
	...
	const REPORTS_FOLDERS_SQL = '
		select
			...
			ReportCatalog_pid,
			ReportCatalog_Name,
			ReportCatalog_Status,
			ReportCatalog_Position
		from rpt.v_ReportCatalog (nolock)
	';
    const REPORTS_FOLDERS_SQL_HN = "
    select
			RC.ReportCatalog_id,
			RC.ReportCatalog_pid,
			RC.ReportCatalog_Name,
			RC.ReportCatalog_Status,
			RC.ReportCatalog_Position,
			case when RC_P.ReportCatalog_Name = 'Государственные отчёты' then 1 else 0 end as allow_folder
		from rpt.v_ReportCatalog RC (nolock)
		left join rpt.v_ReportCatalog RC_P (nolock) on RC_P.ReportCatalog_id = RC.ReportCatalog_rid
    ";
	const REPORT_MYFOLDER_SQL = "
		select distinct R.*
		from rpt.v_ReportUser RU (nolock)
		inner join rpt.v_Report R (nolock) on R.Report_id = RU.Report_id
		where RU.User_id = :pmUser_id
		order by R.Report_Caption
	";
    const REPORTS_SQL = 'select * from rpt.v_Report (nolock) where ReportCatalog_id = :nodeId order by Report_Position';
    const REPORTS_SQL_HN = "
        ...
        from rpt.v_Report R (nolock)
        left join rpt.v_ReportCatalog RC (nolock) on RC.ReportCatalog_id = R.ReportCatalog_id
        left join rpt.v_ReportCatalog RC_P (nolock) on RC_P.ReportCatalog_id = RC.ReportCatalog_rid
	...
	const REPORTS_SQL_WITH_ARMSORT = "
									...
									from rpt.v_Report R (nolock)
										left join rpt.v_ReportARM RA (nolock) on RA.Report_id = R.Report_id
										left join v_ARMType AT (nolock) on AT.ARMType_id = RA.ARMType_id
									where R.ReportCatalog_id = :nodeId
		...
		else
		{
			...
			else
			{
				...
				if (!empty($pmUserCacheGroup_SysNick)) {
					$join_reports .= "
						inner join rpt.ReportUserGroup RUG with (nolock) on RUG.Report_id = R.Report_id
						inner join v_pmUserCacheGroup pucg with (nolock) on pucg.pmUserCacheGroup_id = RUG.pmUserCacheGroup_id and pucg.pmUserCacheGroup_SysNick = :pmUserCacheGroup_SysNick
					";
				...
				$query_reports = "
					select 
						...
						R.Report_insDT as Report_insDT,
						R.Report_updDT as Report_updDT,
						R.Region_id as Region_id,
						R.ReportType_id as ReportType_id,
						R.DatabaseType as DatabaseType,
						substring(ReportArmStr.ReportArmStr_Items, 1, len(ReportArmStr.ReportArmStr_Items)-1) as ReportArm
					from 
						rpt.Report R (nolock)
						{$join_reports}
						outer apply (
							select (
								...
								inner join v_ARMType AT (nolock) on AT.ARMType_id = RA.ARMType_id
								where RA.Report_id = R.Report_id
								for xml path('')
							) as ReportArmStr_Items
						) ReportArmStr
					where
						R.Region_id = :Region_id
						or exists (	-- загружаем отчёты связанные с регионом через таблицу ReportsList
							select top 1 RLs.ReportList_id
							from
								rpt.ReportList RLs (nolock)
							where
			...
			else
			{
				$query_folders = "
					select
						RC.ReportCatalog_id,
						RC.ReportCatalog_pid,
						RC.ReportCatalog_rid,
						RC.ReportCatalog_Name,
						RC.ReportCatalog_Status,
						RC.ReportCatalog_Path,
						RC.ReportCatalog_Position,
						RC.Region_id
					from 
						rpt.ReportCatalog RC (nolock)
					where
						Region_id = :Region_id
						or exists (
							select top 1 RLs.ReportList_id
	...
	*/
    ...
    public function getTreeCC($params) {
			...
			//Если находимся в папке "Мои отчеты" - используем соответствующий запрос, запихивая отчеты не с #rr, а с #_rr, чтоб избежать дурблирования в дереве
		{
			...
			if ($node == 'root') {
				$where = " where ReportCatalog_Name like '%для Центра записи%'";
				$node = 0;
			} else {
				$node = preg_replace('/[^\d]*/', '', $node);
				$where = ' where ReportCatalog_pid = ' . $node . '  order by ReportCatalog_Position';
    ...
    public function checkFolderOnReport($folderData) {
		...
		$query = "
			select
				ReportCatalog_id
			from
				rpt.v_ReportCatalog with(nolock)
			where
    ...
    protected function getAvailableARMs($data) {
		$query = "
			...
			from
				...
				left join rpt.ReportARM RA (nolock) on RA.ARMType_id = AT.ARMType_id
			where
	...
	{
		$query = "
			select ReportUser_id
			from rpt.v_ReportUser with(nolock)
		...
		if(isset($data['pmUser_id']) && $data['pmUser_id'] > 0)
		{
			if($data['upd_mode'] == 'add')
			{
				$query_add = "
					...
					@ReportUser_id bigint;
				exec rpt.p_ReportUser_ins
					@ReportUser_id = @ReportUser_id output,
					@Report_id = :Report_id,
					@pmUser_id = :pmUser_id,
					@User_id = :pmUser_id,
					@Error_Code = @ErrCode output,
					@Error_Message = @ErrMessage output;
				select @ReportUser_id as ReportUser_id, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
				";
				$res = $this->db->query($query_add, array('Report_id'=>$data['Report_id'],'pmUser_id'=>$data['pmUser_id']));
			}
			else
			{
				$query = "
					select ReportUser_id
					from rpt.v_ReportUser (nolock)
					where Report_id = :Report_id
					and User_id = :pmUser_id
				";
				$result = $this->db->query($query,array('Report_id'=>$data['Report_id'],'pmUser_id'=>$data['pmUser_id']));
