...
class Pskov_EvnSection_model extends EvnSection_model {
	...
	protected function calcCoeffCTP2018($data) {
		...
		if (isset($KSLPCodes[1])) {
			if (!empty($data['EvnSectionIds'])) {
				$AttributeValue = $this->queryResult("
					declare
						@AttributeVision_TablePKey bigint = (select top 1 VolumeType_id from v_VolumeType (nolock) where VolumeType_Code = '2018-01КСЛП_ДСД');
		...
		if (isset($KSLPCodes[2])) {
			if (!empty($data['EvnSectionIds'])) {
				$AttributeValue = $this->queryResult("
					declare
						@AttributeVision_TablePKey bigint = (select top 1 VolumeType_id from v_VolumeType (nolock) where VolumeType_Code = '15');
		...
		if (isset($KSLPCodes[3])) {
			...
			if (!empty($data['EvnSectionIds'])) {
				$resp = $this->queryResult("
					select distinct esdts.DrugTherapyScheme_id from v_EvnSectionDrugTherapyScheme esdts (nolock) where esdts.EvnSection_id in ('" . implode("','", $data['EvnSectionIds']) . "') and esdts.DrugTherapyScheme_id is not null
				");
				...
				$EvnUsluga = $this->queryResult("
					...
					WHERE
						...
						and exists(
							...
							from
								UslugaComplexAttribute uca (nolock)
		...
		if (in_array($data['LpuUnitType_id'], array('6','7','9')) && isset($KSLPCodes[7])) {
			$resp_eu = $this->queryResult("
				...
				from
					v_EvnUsluga eu (nolock)
					inner join v_AttributeSignValue asv (nolock) on asv.AttributeSignValue_TablePKey = eu.EvnUsluga_id
				WHERE
					eu.EvnUsluga_pid IN ('" . implode("','", $data['EvnSectionIds']) . "')
					and eu.PayType_id = :PayTypeOms_id
					and eu.EvnClass_id in (43,22,29)
	...
	protected function calcCoeffCTP($data) {
		...
		if (!empty($data['EvnSection_id'])) {
			$evnsections = $this->queryResult("select EvnSection_id from v_EvnSection (nolock) where 
				EvnSection_IndexNum = (select EvnSection_IndexNum from v_EvnSection (nolock) where EvnSection_id = :EvnSection_id) and
				EvnSection_pid = (select EvnSection_pid from v_EvnSection (nolock) where EvnSection_id = :EvnSection_id)
			", $data);
		...
		if (!empty($data['EvnSection_id'])) {
			$query = "
				...
				inner join v_MesOld mo (nolock) on mo.Mes_id = coalesce(es.Mes_sid, es.Mes_tid, es.Mes_kid)
				where es.EvnSection_pid = (select EvnSection_pid from v_EvnSection (nolock) where EvnSection_id = :EvnSection_id)
				and es.EvnSection_IndexNum = (select EvnSection_IndexNum from v_EvnSection (nolock) where EvnSection_id = :EvnSection_id)
			";
		...
		if (!empty($data['EvnSection_id'])) {
			$query = "
				select top 100
					...
					mo.MesType_id,
					mul.UslugaComplex_id,
					mul.MesOldUslugaComplexLink_Number,
					mu.MesOldUslugaComplex_DurationTo
				from v_EvnUsluga eu (nolock)
					...
					left join v_MesTariff mt (nolock) on mt.Mes_id = mo.Mes_id -- Коэффициент КСГ
					outer apply(
						select top 1 UslugaComplex_id, MesOldUslugaComplexLink_Number
		...
		if ($data['MesType_id'] == 9  && !empty($data['Diag_id'])) { // КПГ считается для дневного стаца
			$query = "
				select top 1
					...
					mt.MesTariff_id,
					mul.UslugaComplex_id,
					isnull(mu.MesOldUslugaComplex_DurationTo,1) as MesOldUslugaComplex_DurationTo
				from v_MesOldUslugaComplex mu (nolock)
					inner join v_MesOld mo (nolock) on mo.Mes_id = mu.Mes_id
					left join v_Diag d (nolock) on d.Diag_id = mu.Diag_id
					left join v_MesTariff mt (nolock) on mt.Mes_id = mo.Mes_id -- Коэффициент КСГ
				...
				where
					ISNULL(mu.Diag_id, :Diag_id) = :Diag_id
					and (mu.UslugaComplex_id is null OR mu.UslugaComplex_id IN (select eu.UslugaComplex_id from v_EvnUsluga eu (nolock) where eu.EvnUsluga_pid = :EvnSection_id and eu.EvnClass_id in (43,22,29) and eu.PayType_id = :PayTypeOms_id))
					and (mu.Diag_nid IS NULL OR mu.Diag_nid IN (select Diag_id from v_EvnDiagPS (nolock) where DiagSetClass_id IN (2,3) and EvnDiagPS_pid = :EvnSection_id))
					and (mu.Sex_id = :Sex_id OR Sex_id IS NULL)
					and (
					...
					and mo.Mes_begDT <= :EvnSection_disDate
					and (IsNull(mo.Mes_endDT, :EvnSection_disDate)>= :EvnSection_disDate)
					and (IsNull(mt.MesTariff_begDT, :EvnSection_disDate) <= :EvnSection_disDate)
					and (IsNull(mt.MesTariff_endDT, :EvnSection_disDate)>= :EvnSection_disDate)
	...
	protected function _recalcKSKP()
	{
		...
		$resp_es = $this->queryResult("
			select
				...
				mt.MesTariff_Value,
				mt.MesTariff_id,
				mouc.MesOldUslugaComplex_DurationTo,
				mo.Mes_Code,
				es.LeaveType_id,
			...
			from
				...
				left join v_MesTariff mt (nolock) on mt.MesTariff_id = es.MesTariff_id
				left join v_MesOldUslugaComplex mouc (nolock) on mouc.MesOldUslugaComplex_id = es.MesOldUslugaComplex_id
				left join v_MesOld mo (nolock) on mo.Mes_id = mt.Mes_id
				left join v_LeaveType lt (nolock) on lt.LeaveType_id = es.LeaveType_id
			where
				es.EvnSection_pid = :EvnSection_pid
				and ISNULL(es.EvnSection_IsPriem, 1) = 1
				and pt.PayType_SysNick = 'oms'
