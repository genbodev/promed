...
class PersonPregnancy_model extends swModel {
	...
	function loadList($data) {
		...
		if (isset($data['Person_AgeFrom']) && $data['Person_AgeFrom'] >= 0) {
			$filters .= " and dbo.Age2(ps.Person_BirthDay, @date) >= :Person_AgeFrom";
		}
		if (isset($data['Person_AgeTo']) && $data['Person_AgeTo'] >= 0) {
			$filters .= " and dbo.Age2(ps.Person_BirthDay, @date) <= :Person_AgeTo";
		}
		...
		if (!empty($data['Diag_sCode_From']) || !empty($data['Diag_sCode_To'])) {
			...
			$filters .= " and exists(
				select *
				from v_PregnancyScreenSopDiag PSSD with(nolock)
				inner join v_PregnancyScreen Screen with(nolock) on Screen.PregnancyScreen_id = PSSD.PregnancyScreen_id
				inner join v_Diag D with(nolock) on D.Diag_id = PSSD.Diag_id
		...
		if (isset($data['Type']) && $data['Type'] == 'all') {
			...
			if (!empty($data['PersonRegister_setDateRange'][0]) && !empty($data['PersonRegister_setDateRange'][1])) {
				...
				$filters .= " and PR.PersonRegister_setDate <= @date";
				$filters .= " and (PR.PersonRegister_disDate is null or PR.PersonRegister_disDate >= @date)";
			}
		} else {
			if (!empty($data['PersonRegister_setDateRange'][0]) && !empty($data['PersonRegister_setDateRange'][1])) {
		...
		if (!empty($data['RiskType_fid'])) {
			$filters .= " and RTglobal.RiskType_id = :RiskType_fid";
		}
		...
		if (getRegionNick() == 'perm') {
			...
			$join .= "
				outer apply(
					...
					where EVPL.Person_id = PR.Person_id
		...
		$query = "
			...
			select
				...
				OutCause.PersonRegisterOutCause_SysNick,
				...
				RT.RiskType_id,
				(case when RT.RiskType_Name = 'умеренный' then 'средний' else RT.RiskType_Name end) As RiskType_Name,
				PR.Lpu_iid,
				L.Lpu_Nick,
				...
				D.Diag_id,
				D.Diag_FullName,
				Result.PregnancyResult_id,
				Result.PregnancyResult_Name,
				...
				RTglobal.RiskType_Name as RiskGlobal,
			...
			from
				...
				left join v_RiskType RT with(nolock) on RT.RiskType_id = PR.RiskType_id
				...
				left join v_BirthCertificate BC with(nolock) on BC.PersonRegister_id = PR.PersonRegister_id
				left join v_EvnVizitPL EVPL with(nolock) on EVPL.EvnVizitPL_id = PP.Evn_id
				...
				left join v_EvnVizitPL rEVPL with(nolock) on rEVPL.EvnVizitPL_id = BSS.Evn_id
				left join v_PregnancyResult Result with(nolock) on Result.PregnancyResult_id = isnull(PR.PregnancyResult_id,BSS.PregnancyResult_id)
				outer apply (
					select top 1 Screen.*
				...
				outer apply (
					select top 1 PQ.PregnancyQuestion_ValuesStr as Lpu_id
				...
				left join v_Lpu_all LpuHosp with(nolock) on LpuHosp.Lpu_id = coalesce(ES.Lpu_id, LastScreenDir.Lpu_id, BC.Lpu_id)
				left join v_Diag D with(nolock) on D.Diag_id = coalesce(ES.Diag_id,rEVPL.Diag_id,LastScreen.Diag_id,PP.Diag_id,EVPL.Diag_id)
				left join v_RiskType RTglobal (nolock) on RTglobal.RiskType_id = case 
					when PP.PersonPregnancy_IsInQuarantine = 2 then 3 
					else dbo.GetPregnancyRiskGlobal(RT.RiskType_id, PP.RiskType_did, PP.RiskType_bid) 
					end
				{$join}
				-- end from
			where
		...
		if (!empty($data['Type']) && $data['Type'] == 'out') {
			$query = "
				...
				select
					...
					(case when PP.RiskType_did = 1 then 'низкий' when PP.RiskType_did = 2 then 'высокий' else 'низкий' end) as RiskPR,
					(case when PP.RiskType_bid = 1 then 'низкий' when PP.RiskType_bid = 2 then 'средний' when PP.RiskType_bid = 3 then 'высокий' else 'низкий' end) as Risk572N,
					(case when [dbo].[GetPregnancyRiskGlobal](RT.RiskType_id, PP.RiskType_did, PP.RiskType_bid) = 1 then 'низкий' when [dbo].[GetPregnancyRiskGlobal](RT.RiskType_id, PP.RiskType_did, PP.RiskType_bid) = 2 then 'средний' when [dbo].[GetPregnancyRiskGlobal](RT.RiskType_id, PP.RiskType_did, PP.RiskType_bid) = 3 then 'высокий' else '' end) as RiskGlobal,
					[dbo].[GetPregnancy572NRisk](PR.PersonRegister_id,1) as RiskFactor572N
					{$fields}
					-- end select
				from
		...
		if (count($PersonRegister_ids) > 0) {
			for ($i = 0;;$i++) {
				...
				$query = "
					...
					select
						PS.PregnancyScreen_id,
						PS.PersonRegister_id,
						convert(varchar(10), PS.PregnancyScreen_setDT, 104) as PregnancyScreen_setDate,
						coalesce(SA.amenordate, SA.embriondate, SA.uzidate, SA.fmovedate) as PregnancyScreen_Period
					from
						v_PregnancyScreen PS with(nolock)
						outer apply(
							select
							(
								select top 1 PregnancyQuestion_AnswerInt
	...
	function loadListRecommRouter($data) {
		...
		if (!empty($data['RiskType_fid'])) {
			$filters .= " and [dbo].[GetPregnancyRiskGlobal](RT.RiskType_id, PP.RiskType_did, PP.RiskType_bid)=:RiskType_fid";
		}
	...
	function loadListMonitorCenter($data) {
		...
		";
			...
			$fields .= ",
				EvnVizitPL_Date.EvnVizitPL_setDate,
				DiffDate.DiffDate,
				(case when Period.Value between 1 and 12  then '1 Триместр'
				when Period.Value between 13 and 27 then '2 Триместр'
				when Period.Value between 28 and 43 then '3 Триместр' end) as Trimester";
		...
		$query = "
			...
			select
				...
				convert(varchar(10), PR.PersonRegister_HighRiskDT, 104) as HighRisk_setDT,
				ISNULL(PP.PersonPregnancy_RiskDPP, 0) + ISNULL(LastScreen.PregnancyScreen_RiskPerPat, 0) as PersonPregnancy_ObRisk,
				[dbo].[GetPregnancyRoute](PR.PersonRegister_id, 1, 0) as lstfactorrisk,
				(case when ML.MesLevel_Name = 'Второй уровень (без высокотехнологичной помощи)' then 'МПЦ' else ML.MesLevel_Name end) as MesLevel_Name,
				EPS_LPU.Lpu_Nick as NickHospital,
				convert(varchar(10), EPS.EvnPS_setDate, 104) as DateHospital,
				--EPS.EvnPS_setDate,
				--EPS.EvnPS_setTime,
				...
				(case when PR.RiskType_aid>2 then (case when PR.PersonRegister_HighRiskDT is not null then 0 else 1 end) else 0 end) notopen_highrisk,
				(case when RT.RiskType_Name = 'умеренный' then 'средний' else RT.RiskType_Name end) As RiskType_Name,
				dbo.GetPregnancyPRRisk(PR.PersonRegister_id,1) as PRRiskFactor,
				(case when PP.RiskType_did = 1 then 'низкий' when PP.RiskType_did = 2 then 'высокий' else 'низкий' end) as RiskPR,
				...
				PR.Lpu_iid,
				UserCache.pmUser_id,
				UserCache.pmUser_Login,
				UserCache.pmUser_Name,
				pucgl.GroupPregnancy
				-- end select
			from
				-- from
				v_PersonRegister PR with(nolock)
				...
				left join v_RiskType RKT with(nolock) on RKT.RiskType_id = PR.RiskType_aid
				left join v_MesLevel ML with(nolock) on ML.MesLevel_id = PR.MesLevel_id
				left join v_EvnPS EPS with (nolock) on EPS.Person_id=PR.Person_id
				outer apply(
					select top 1 * from v_EvnSection ES with (nolock) where ES.EvnSection_pid=EPS.EvnPS_id and ES.LeaveType_id is null
				) ESS
				left join v_Lpu_all EPS_LPU with(nolock) on EPS_LPU.Lpu_id = ESS.Lpu_id
				left join v_Diag EPS_DIAG with(nolock) on EPS_DIAG.Diag_id = ESS.Diag_id
				...
				outer apply(
					select top 1 1 as GroupPregnancy
					from pmUserCacheGroupLink pucgl0 with(nolock)
						inner join pmUserCacheGroup pucg with (nolock) on pucg.pmUserCacheGroup_id=pucgl0.pmUserCacheGroup_id
...
count ( case when Period.Value between 28 and 43 then '3 триместр' end ) as Trimester3
			...
			from
				...
				-- end from
			where
				(1=1) and PR.PersonRegisterOutCause_id is null
				and ((case when ((Period.Value between 1 and 12) and DiffDate.DiffDate>35) then 1 end=1)
				or (case when ((Period.Value between 13 and 27) and DiffDate.DiffDate>35) then 1 end=1)
				or (case when ((Period.Value between 28 and 43) and DiffDate.DiffDate>22) then 1 end=1))
				group by
				PR.Lpu_iid,
	...
	function loadNotIncludeList($data) {
		...
		if ( !empty($data['EvnType_id']) && in_array($data['EvnType_id'], array(1, 2)) ) {
			switch ( $data['EvnType_id'] ) {
				case 1:
					...
					$fieldsList[] = "LT.LeaveType_id";
					$fieldsList[] = "null as ResultClass_id";
					$fieldsList[] = "LT.LeaveType_Name as EvnResult";

					$joinList[] = "left join v_LeaveType LT with(nolock) on LT.LeaveType_id = EPS.LeaveType_id";
					$joinList[] = "left join v_Diag D with(nolock) on D.Diag_id = EPS.Diag_id";
					//$joinList[] = "left join v_EvnSection ES with (nolock) on ES.EvnSection_pid = EPS.EvnPS_id and ES.EvnSection_Index = EvnSection_Count - 1";
					$joinList[] = "outer apply (select top 1 * from v_EvnSection with (nolock) where EvnSection_pid = EPS.EvnPS_id and EvnSection_Index = EvnSection_Count - 1) ES";
					$joinList[] = "left join v_MedStaffFact MSF with(nolock) on MSF.MedStaffFact_id = ES.MedStaffFact_id";
				...
				case 2:
					...
					$fieldsList[] = "RC.ResultClass_id";
					$fieldsList[] = "RC.ResultClass_Name as EvnResult";

					$joinList[] = "left join v_ResultClass RC with(nolock) on RC.ResultClass_id = EPL.ResultClass_id";
					$joinList[] = "left join v_Diag D with(nolock) on D.Diag_id = EPL.Diag_id";
					//$joinList[] = "left join v_EvnVizitPL EVPL with (nolock) on EVPL.EvnVizitPL_pid = EPL.EvnPL_id and EVPL.EvnVizitPL_Index = EvnVizitPL_Count - 1";
					$joinList[] = "outer apply (select top 1 * from v_EvnVizitPL with (nolock) where EvnVizitPL_pid = EPL.EvnPL_id and EvnVizitPL_Index = EvnVizitPL_Count - 1) EVPL";
					$joinList[] = "left join v_MedStaffFact MSF with(nolock) on MSF.MedStaffFact_id = EVPL.MedStaffFact_id";
			...
			$fieldsList[] = "isnull(EPL.EvnPL_NumCard, EPS.EvnPS_NumCard) as Evn_NumCard";
			$fieldsList[] = "LT.LeaveType_id";
			$fieldsList[] = "RC.ResultClass_id";
			$fieldsList[] = "isnull(LT.LeaveType_Name, RC.ResultClass_Name) as EvnResult";

			$joinList[] = "left join v_ResultClass RC with(nolock) on RC.ResultClass_id = EPL.ResultClass_id";
			$joinList[] = "left join v_LeaveType LT with(nolock) on LT.LeaveType_id = EPS.LeaveType_id";
			$joinList[] = "left join v_Diag D with(nolock) on D.Diag_id = ISNULL(EPL.Diag_id, EPS.Diag_id)";
			//$joinList[] = "left join v_EvnVizitPL EVPL with (nolock) on EVPL.EvnVizitPL_pid = EPL.EvnPL_id and EVPL.EvnVizitPL_Index = EvnVizitPL_Count - 1";
			$joinList[] = "outer apply (select top 1 * from v_EvnVizitPL with (nolock) where EvnVizitPL_pid = EPL.EvnPL_id and EvnVizitPL_Index = EvnVizitPL_Count - 1) EVPL";
			//$joinList[] = "left join v_EvnSection ES with (nolock) on ES.EvnSection_pid = EPS.EvnPS_id and ES.EvnSection_Index = EvnSection_Count - 1";
			$joinList[] = "outer apply (select top 1 * from v_EvnSection with (nolock) where EvnSection_pid = EPS.EvnPS_id and EvnSection_Index = EvnSection_Count - 1) ES";
			$joinList[] = "left join v_MedStaffFact MSF with(nolock) on MSF.MedStaffFact_id = ISNULL(EVPL.MedStaffFact_id, ES.MedStaffFact_id)";
		...
		$query = "
			...
			select
				...
				convert(varchar(10), isnull(EPL.EvnPL_setDate, EPS.EvnPS_setDate), 104) as Evn_setDate,
				convert(varchar(10), isnull(EPL.EvnPL_disDate, EPS.EvnPS_disDate), 104) as Evn_disDate,
				L.Lpu_id,
				L.Lpu_Nick,
				D.Diag_id,
				D.Diag_FullName,
			...
			from
				...
				(select
					...
					from
						v_Evn E with(nolock)
						left join v_PersonPregnancy PP with(nolock) on PP.Evn_id = E.Evn_id
						outer apply (
							select top 1 PersonRegister_id
							from v_PersonRegister PR with(nolock)
							where PR.Person_id = E.Person_id
							and PR.PersonRegisterType_id = 2
							and isnull(PR.PersonRegister_disDate, @bigdate) >= E.Evn_setDate
						) PR
					...
					WHERE 
						...
						{$EvnDateRange}
						{$EvnFilter}
						and exists (select top 1 1 from v_MorbusDiag MD with(nolock) where MD.MorbusType_id = 2 and MD.Diag_id = D.Diag_id)
				...
				left join v_EvnPS EPS with(nolock) on EPS.EvnPS_id = E.Evn_id
				outer apply (
					select top 1 PersonRegister_id from v_PersonRegister PR with(nolock)
					where PR.Person_id = E.Person_id
					and PR.PersonRegisterType_id = 2 -- общерегиональный
					and isnull(PR.PersonRegister_disDate, @bigdate) >= isnull(EPL.EvnPL_setDate, EPS.EvnPS_setDate)
				) PR				
				" . implode(" ", $joinList) . "
				left join v_PersonRegister PRR with(nolock) on PRR.PersonRegister_id=PR.PersonRegister_id
				left join v_RiskType RT with(nolock) on RT.RiskType_id = PRR.RiskType_id
				-- end from
			where
				-- where
			...
			order by
				-- order by
				isnull(EPL.EvnPL_disDate, EPS.EvnPS_disDate) desc,
				isnull(EPL.EvnPL_setDate, EPS.EvnPS_setDate) desc
				-- end order by
		";
		//echo getDebugSQL($query, $data);exit;
	...
	function loadFinishedList($data) {
		...
		if (!empty($data['HospLpu_id'])) {
			$filters .= " and e.Lpu_id = :HospLpu_id";
		}
		...
		$query = "
			select
				...
				lt.LeaveType_Name,
				eps.EvnPS_id,
				convert(varchar(10), e.Evn_setDate, 104) as EvnPS_setDate,
				convert(varchar(10), e.Evn_disDate, 104) as EvnPS_disDate,
			...
			from
				...
				inner join v_Evn e (nolock) on eu.evnusluga_rid = e.evn_id and e.EvnClass_id = 30
				inner join v_PersonState ps (nolock) on ps.person_id = e.person_id
				inner join v_Lpu l (nolock) on l.lpu_id = e.lpu_id
				left join EvnPS eps (nolock) on e.evn_id = eps.evn_id
	...
	function loadInterruptedList($data) {
		...
		$query = "
			...
			from
				...
				v_EvnUsluga eu (nolock)
				inner join UslugaComplex uc (nolock) on uc.uslugacomplex_id = eu.uslugacomplex_id
				inner join v_EvnPS eps (nolock) on eu.evnusluga_rid = eps.evnps_id
	...
	function loadPersonPregnancyResultGrid($data) {
		...
		$query = "
			select
				PPR.PersonPregnancyResult_id,
				PPR.PersonPregnancy_id,
				1 as RecordStatus_Code,
				PPR.PersonPregnancyResult_Num,
				PPR.PersonPregnancyResult_Year,
				PPR.PersonPregnancyResult_OutcomPeriod,
				PPR.PersonPregnancyResult_WeigthChild,
				PPR.PersonPregnancyResult_AgeChild,
				PPR.PersonPregnancyResult_Descr,
				PR.PregnancyResult_id,
				PR.PregnancyResult_Name,
				BCR.BirthChildResult_id,
				BCR.BirthChildResult_Name,
				CSR.ChildStateResult_id,
				CSR.ChildStateResult_Name
			from
				v_PersonPregnancyResult PPR with(nolock)
				left join v_PregnancyResult PR with(nolock) on PR.PregnancyResult_id = PPR.PregnancyResult_id
				left join v_BirthChildResult BCR with(nolock) on BCR.BirthChildResult_id = PPR.BirthChildResult_id
				left join v_ChildStateResult CSR with(nolock) on CSR.ChildStateResult_id = PPR.ChildStateResult_id
			where PPR.PersonPregnancy_id = :PersonPregnancy_id
		";
	...
	function loadPersonPregnancyGravidogramData($data) {
		...
		$query = "
			select
				PS.PregnancyScreen_id,
				PS.PregnancyScreen_Comment,
				convert(varchar(10), PS.PregnancyScreen_setDT, 104) as PregnancyScreen_setDate,
				convert(varchar(10), PP.PersonPregnancy_setDT, 104) as PersonPregnancy_setDT,
				coalesce(weeks.number, datediff(ww, PP.PersonPregnancy_setDT, PS.PregnancyScreen_setDT)) as PersonPregnancy_Week,
				PS.GestationalAge_id,
				PP.PersonPregnancy_Period
			from
				v_PregnancyScreen PS with(nolock)
				left join v_PersonPregnancy PP with(nolock) on PP.PersonRegister_id=PS.PersonRegister_id
				outer apply(
					select top 1
						PQ.PregnancyQuestion_AnswerInt as number
					from
						v_PregnancyQuestion PQ with(nolock)
						left join v_QuestionType QT with(nolock) on QT.QuestionType_id = PQ.QuestionType_id
					where
		...
		if (!empty($data['PersonRegister_id']) && $data['PersonRegister_id'] > 0) {
			$query = "
				select top 1
					...
					DM.DeathMother_id,
					BC.BirthCertificate_id,
					CountScreen.Value as ScreenCount,
					LastScreen.PregnancyScreen_RiskPerPat as ScreenRisk,
					PP.PersonPregnancy_RiskDPP
				from
					v_PersonRegister PR with(nolock)
					left join v_PersonPregnancy PP with(nolock) on PP.PersonRegister_id = PR.PersonRegister_id
					left join v_BirthSpecStac BSS with(nolock) on BSS.PersonRegister_id = PR.PersonRegister_id
					left join v_DeathMother DM with(nolock) on DM.PersonRegister_id = PR.PersonRegister_id
					left join v_BirthCertificate BC with(nolock) on BC.PersonRegister_id = PR.PersonRegister_id
					left join v_Evn AnketaEvn with(nolock) on AnketaEvn.Evn_id = PP.Evn_id
					left join v_Evn ResultEvn with(nolock) on ResultEvn.Evn_id = isnull(BSS.EvnSection_id, BSS.Evn_id)
					outer apply(
						select top 1 count(*) as Value
						from v_PregnancyScreen PS with(nolock)
						where PS.PersonRegister_id = PR.PersonRegister_id
					) CountScreen
					outer apply(
						select top 1 PS.PregnancyScreen_RiskPerPat
						from v_PregnancyScreen PS with(nolock)
						where PS.PersonRegister_id = PR.PersonRegister_id
	...
	function loadPersonPregnancyTree($data) {
					...
					foreach($items as $name => &$item) {
						...
						switch($name) {
							...
							case 'Result':
								...
								$resp_risk = $this->queryResult("
									select 1 as hasEcoBirthSpecStac from v_PersonPregnancy PP
									...
									where PP.PersonRegister_id=:PersonRegister_id and PP.Person_id=PR.Person_id
									and Eco.BirthSpecStac_id is not null
								", array(
									'PersonRegister_id' => $data['PersonRegister_id']
								));
				...
				case 'Anketa':
					if (!empty($data['PersonRegister_id'])) {
						$info = $this->getFirstRowFromQuery("
							select top 1 E.Evn_id, E.Lpu_id, PP.PersonPregnancy_id
							from v_PersonPregnancy PP with(nolock)
							left join v_Evn E with(nolock) on E.Evn_id = PP.Evn_id
							where PP.PersonRegister_id = :PersonRegister_id
						", $data);
						if (is_array($info)) {
					...
					if (!empty($data['PersonRegister_id']) && $data['PersonRegister_id'] > 0) {
						$query = "
							select
								PS.PregnancyScreen_id,
								convert(varchar(10), PS.PregnancyScreen_setDT, 104) as PregnancyScreen_setDate,
								isnull(PS.PregnancyScreen_RiskPerPat, 0) as PregnancyScreen_RiskPerPat,
								isnull(cast(weeks.number as varchar), '*') as weeks,
								E.Evn_id,
								E.Lpu_id
							from
								v_PregnancyScreen PS with(nolock)
								left join v_Evn E with(nolock) on E.Evn_id = PS.Evn_id
								outer apply(
									select top 1
										PQ.PregnancyQuestion_AnswerInt as number
									from
		...
		$this->db->query("update PersonRegister with (rowlock) set PersonRegister_OpenDT = getdate() where PersonRegister_id = :PersonRegister_id", array(
			'PersonRegister_id' => $data['PersonRegister_id']
		));
		return $nodes;
	}
	...
	function loadPersonPregnancyForm($data) {
		...
		$query = "
			select top 1
				...
				convert(varchar(10), PR.PersonRegister_setDate, 104) as PersonRegister_setDate,
				PP.PersonPregnancy_Period,
				convert(varchar(10), PP.PersonPregnancy_begMensDate, 104) as PersonPregnancy_begMensDate,
				convert(varchar(10), PP.PersonPregnancy_endMensDate, 104) as PersonPregnancy_endMensDate,
				convert(varchar(10), PP.PersonPregnancy_birthDate, 104) as PersonPregnancy_birthDate,
				convert(varchar(10), PP.PersonPregnancy_birthDate, 104) as PersonPregnancy_birthDate,
				PR.PersonRegister_Code,
				PR.Lpu_iid,
				PR.MedPersonal_iid,
				PP.PersonPregnancy_Phone,
				PP.PersonPregnancy_PhoneWork,
				PP.PersonPregnancyEducation_id,
				PP.Post_id,
				PP.PregnancyFamilyStatus_id,
				PP.PersonPregnancy_Height,
				PP.PersonPregnancy_Weight,
				case when PP.PersonPregnancy_IsWeight25 = 2 then 1 else 0 end as PersonPregnancy_IsWeight25,
				PP.BloodGroupType_id,
				PP.RhFactorType_id,
				PP.Person_did,
				case when PP.Person_did is not null
					then rtrim(dad.Person_SurName)+' '+rtrim(dad.Person_FirName)+rtrim(isnull(' '+dad.Person_SecName,''))
					else PP.PersonPregnancy_dadFIO
				end as PersonPregnancy_dadFIO,
				PP.PersonPregnancy_dadAge,
				PP.PersonPregnancy_dadAddress,
				PP.PersonPregnancy_dadPhone,
				PP.BloodGroupType_dadid,
				PP.RhFactorType_dadid,
				PP.Org_did,
				PP.Diag_id,
				PP.PregnancyResult_id,
				PP.Post_aid,
				PP.PersonDisp_id
			from
				v_PersonPregnancy PP with(nolock)
				inner join v_PersonRegister PR with(nolock) on PR.PersonRegister_id = PP.PersonRegister_id
				left join v_PersonState dad with(nolock) on dad.Person_id = PP.Person_did
			where
	...
	function getPersonRegisterId($data) {
		...
		$query = "
			select top 1 PersonRegister_id
			from v_PersonPregnancy with(nolock)
			where PersonPregnancy_id = :PersonPregnancy_id
		";
	...
	function validatePersonRegister($data) {
		...
		if (empty($data['PersonRegister_id'])) {
			$PersonInfo = $this->getFirstRowFromQuery("
				select top 1 PS.Person_id, PS.Sex_id
		...
		$query = "
			declare @bigdate date = dateadd(year, 50, dbo.tzGetDate())
			declare @setdate datetime = :PersonRegister_setDate
			declare @disdate datetime = :PersonRegister_disDate
			select top 1
			...
			where
				...
				and PRT.PersonRegisterType_SysNick like 'pregnancy'
				and PR.PersonRegister_id <> isnull(:PersonRegister_id,0)
				and PR.PersonRegister_setDate >= dateadd(month, -11, @setdate)
				and PR.PersonRegister_setDate <= isnull(@disdate, @bigdate)
				and isnull(PR.PersonRegister_disDate, @bigdate) >= @setdate
		";
		$params = array(
		...
		if ($count === false) {
			return $this->createError('','Ошибка при проверке пересечений с другими записями регитстра беременных');
		}
		if ($count > 0) {
	...
	function savePersonRegister($data, $isAllowTransaction = true, $force = false) {
		...
		if (!$force && !empty($data['PersonRegister_id']) && !empty($data['PersonRegisterOutCause_SysNick']) &&
			...
			$query = "
				select top 1
					Cause.PersonRegisterOutCause_Name,
					Cause.PersonRegisterOutCause_SysNick
				from
					v_PersonRegister PR with(nolock)
					left join v_PersonRegisterOutCause Cause with(nolock) on Cause.PersonRegisterOutCause_id = PR.PersonRegisterOutCause_id
		...
		if (!$force && !empty($data['PersonRegister_id']) &&
			...
			$query = "
				select top 1
					Cause.PersonRegisterOutCause_Code
				from
					v_PersonRegister PR with(nolock)
					left join v_PersonRegisterOutCause Cause with(nolock) on Cause.PersonRegisterOutCause_id = PR.PersonRegisterOutCause_id
				where PR.PersonRegister_id = :PersonRegister_id
	...
	function savePersonPregnancy($data, $isAllowTransaction = true) {
		...
		$query = "
			declare
				...
				@Error_Code bigint,
				@Res bigint = :PersonPregnancy_id;
			exec {$procedure}
				@PersonPregnancy_id = @Res output,
			...
			select @Res as PersonPregnancy_id, @Error_Code as Error_Code, @Error_Message as Error_Msg;
		";
		$params = array(
		...
		$query = "
			declare
				@Error_Message varchar(4000),
				@Error_Code bigint;
			exec p_PersonRegisterObstetricPathologyType_upd
				@PersonRegister_id = :PersonRegister_id,
				@pmUser_id = :pmUser_id,
				@Error_Code = @Error_Code output,
				@Error_Message = @Error_Message output
		...
		$query = "
			select ObstetricPathologyType_text from [dbo].[GetObstetricPathologyType](:PersonRegister_id)
		";
		$params = array(
			'PersonRegister_id' => $data['PersonRegister_id']
		);
	...
	function savePregnancyQuestionAnswers($data) {
		...
		$query = "
			...
			from v_QuestionType QT with(nolock)
			outer apply(
				select top 1
			...
			where QT.DispClass_id = :DispClass_id and QT.AnswerType_id is not null
		";
		$resp = $this->queryResult($query, $params);
		$questions = array();
		foreach($resp as $item) {
		...
		if (!empty($data['DifferentLpu']) && strlen($data['DifferentLpu'])>3){
			...
			set nocount on
			begin try
				SELECT @id=LpuDifferent_id from [Dbo].[LpuDifferent] where LpuDifferent_Name=:DifferentLpu;
				if (@id is null)
				begin
					INSERT INTO [Dbo].[LpuDifferent]([LpuDifferent_Code],[LpuDifferent_Name],[pmUser_insID],[pmUser_updID],[LpuDifferent_insDT],[LpuDifferent_updDT]) Values( '1',:DifferentLpu,'1','1',getdate(),getdate());
					UPDATE [Dbo].[LpuDifferent] set LpuDifferent_Code=scope_identity () where LpuDifferent_Id=scope_identity ();
			...
			set nocount off
			select @Error_Code as Error_Code, @Error_Message as Error_Msg, @id as id
			";
			$paramss = array(
				'DifferentLpu' => $data['DifferentLpu']
			);

			$respp = $this->queryResult($queryy, $paramss);
	...
	function recalculatePregnancyQuestionBirthSpecStacRisk($data)
	{
		...
		$query = "
			select
				pq.QuestionType_id,
				case
					when qt.AnswerType_id = 1 then cast(pq.PregnancyQuestion_IsTrue as varchar)
					when qt.AnswerType_id = 2 then pq.PregnancyQuestion_AnswerText
					when qt.AnswerType_id = 3 then cast(pq.PregnancyQuestion_ValuesStr as varchar)
					when qt.AnswerType_id = 5 then cast(pq.PregnancyQuestion_AnswerInt as varchar)
					else cast(pq.PregnancyQuestion_ValuesStr as varchar)
				end as value
			from
				v_BirthSpecStac bss (nolock)
				inner join v_PregnancyQuestion pq (nolock) on pq.PersonRegister_id = bss.PersonRegister_id
				left join v_QuestionType QT (nolock) on QT.QuestionType_id = pq.QuestionType_id
			where
				bss.BirthSpecStac_id = :BirthSpecStac_id
		";
	...
	function recalculatePregnancyQuestionScreenRisk($data)
	{
		...
		$this->db->query("update PregnancyScreen with (rowlock) set PregnancyScreen_RiskPerPat = :PregnancyScreen_RiskPerPat where PregnancyScreen_id = :PregnancyScreen_id", array(
			'PregnancyScreen_id' => $data['PregnancyScreen_id'],
	...
	function recalculateRiskType($data) {
		...
		$resp_risk = $this->queryResult("
			select
				ISNULL(PP.PersonPregnancy_RiskDPP, 0) + ISNULL(LastScreen.PregnancyScreen_RiskPerPat, 0) as PersonRegister_ObRisk
			from
				v_PersonRegister PR with(nolock)
				left join v_PersonPregnancy PP with(nolock) on PP.PersonRegister_id = PR.PersonRegister_id
				outer apply(
		...
		// обновляем в анкете
		$this->db->query("update PersonRegister with (rowlock) set
			RiskType_id = :RiskType_id,
			EvnPS_id=[dbo].[GetPregnancyEvnPS](PersonRegister_id),
			PersonRegister_HighRiskDT = (case when [dbo].[GetPregnancyRoute](PersonRegister_id,2,:RiskType_id)>2 then getdate() else null end),
			PersonRegister_ModerateRiskDT = (case when [dbo].[GetPregnancyRoute](PersonRegister_id,2,:RiskType_id)=2 then getdate() else null end),
			RiskType_aid = [dbo].[GetPregnancyRoute](PersonRegister_id,2,:RiskType_id),
			MesLevel_id = [dbo].[GetPregnancyRoute](PersonRegister_id,3,:RiskType_id)
			where PersonRegister_id = :PersonRegister_id", array(
			'PersonRegister_id' => $data['PersonRegister_id'],
			'RiskType_id' => $RiskType_id
		));
	}
	...
	function recalculatePregnancyQuestionRisk($data)
	{
		...
		$query = "
			select
				...
				pp.PersonPregnancyEducation_id,
				pp.PersonPregnancy_Height,
				pp.PersonPregnancy_IsWeight25,
				pp.RhFactorType_id
			from
				v_PersonPregnancy pp (nolock)
				left join v_PersonState psm (nolock) on psm.Person_id = pp.Person_id
			where
		...
		// обновляем в анкете
		$this->db->query("update PersonPregnancy with (rowlock) set PersonPregnancy_RiskDPP = :PersonPregnancy_RiskDPP where PersonRegister_id = :PersonRegister_id", array(
			'PersonRegister_id' => $data['PersonRegister_id'],
			'PersonPregnancy_RiskDPP' => $PersonPregnancy_RiskDPP
		));
	...
	function deletePregnancyQuestionAnswers($data) {
		...
		$query = "
			...
			set nocount on
			begin try
				delete from PregnancyQuestion with(rowlock)
				where PregnancyQuestion_id in (
	...
	function savePersonPregnancyResult($data) {
		...
		$query = "
			declare
				@Error_Message varchar(4000),
				@Error_Code bigint,
				@Res bigint = :PersonPregnancyResult_id;
			exec {$procedure}
				@PersonPregnancyResult_id = @Res output,
				...
				@Error_Code = @Error_Code output,
				@Error_Message = @Error_Message output
			select @Res as PersonPregnancyResult_id, @Error_Code as Error_Code, @Error_Message as Error_Msg;
		";
		//echo getDebugSQL($query, $params);exit;
		$resp = $this->queryResult($query, $params);
		if (!is_array($resp)) {
	...
	function deletePersonPregnancyResult($data) {
		...
		$query = "
			declare
				@Error_Message varchar(4000),
				@Error_Code bigint;
			exec p_PersonPregnancyResult_del
	...
	function savePregnancyQuestion($data) {
		...
		$query = "
			declare
				@Error_Message varchar(4000),
				@Error_Code bigint,
				@Res bigint = :PregnancyQuestion_id;
			exec {$procedure}
				@PregnancyQuestion_id = @Res output,
				@PersonRegister_id = :PersonRegister_id,
				...
				@Error_Code = @Error_Code output,
				@Error_Message = @Error_Message output
			select @Res as PregnancyQuestion_id, @Error_Code as Error_Code, @Error_Message as Error_Msg;
		";
		$resp = $this->queryResult($query, $params);
		if (!is_array($resp)) {
	...
	function checkDeletePersonRegister($data) {
		...
		foreach($linkObjectList as $object) {
			$checkExistsObjectList[] = "
				when exists(select * from {$object} with(nolock) where PersonRegister_id = PR.PersonRegister_id) then 1
			";
		}
	...
	function deletePersonRegister($data) {
		...
		$query = "
			...
			declare @PersonRegisterOutCause_id bigint = (
				select top 1 PersonRegisterOutCause_id from v_PersonRegisterOutCause with (nolock) where PersonRegisterOutCause_SysNick = 'other'
			)
			set nocount on
			begin try
				...
				set
					...
					PersonRegister_disDate = @date,
					PersonRegister_updDT = @date,
	...
	function deletePersonPregnancy($data, $isAllowTransaction = true) {
		...
		$PersonPregnancyResultList = $this->queryResult("
			select PersonPregnancyResult_id
			from v_PersonPregnancyResult with(nolock)
			where PersonPregnancy_id = :PersonPregnancy_id
		", $params);
	...
	function getPersonRegisterByEvnSection($data) {
		...
		$query = "
			...
			declare @EvnSection_id bigint = :EvnSection_id
			declare @bigDate date = dateadd(year, 50, dbo.tzGetDate())
			declare @begDate date = :EvnSection_setDate
			declare @endDate date = :EvnSection_disDate
			...
			where
				...
				and (
					...
					or (
						not exists(select * from v_BirthSpecStac with (nolock) where EvnSection_id = @EvnSection_id)
						and PR.PersonRegister_setDate <= @begDate
						and PR.PersonRegister_setDate <= isnull(@endDate, @bigDate)
						and PR.PersonRegister_setDate >= dateadd(month, -11, @begDate)
						and (PR.PersonRegister_disDate is null or PR.PersonRegister_disDate >= @begDate)
	...
	function getPersonRegisterByEvnVizitPL($data) {
		...
		$query = "
			...
			where
				...
				and (
					(PR.PersonRegister_setDate <= @date and isnull(PR.PersonRegister_disDate, @date) >= @date)
					or PP.Evn_id = @Evn_id
					or BSS.Evn_id = @Evn_id
					or exists(
						select * from v_PregnancyScreen t with(nolock)
						where t.PersonRegister_id = PR.PersonRegister_id and t.Evn_id = @Evn_id
					)
				)
			order by
				PR.PersonRegister_setDate desc
	...
	function updatePersonRegister($data) {
		...
		$query = "
			...
			declare @PersonRegister_id bigint = :PersonRegister_id;
			declare @Diag_id bigint;
			declare @PregnanyResult_id bigint;

			select top 1
				@Diag_id = ES.Diag_id,
				@PregnanyResult_id = BSS.PregnancyResult_id
			from
				v_BirthSpecStac BSS with(nolock)
				left join v_EvnSection ES with(nolock) on ES.EvnSection_id = BSS.EvnSection_id
			where BSS.PersonRegister_id = @PersonRegister_id

			if @Diag_id is null
			set @Diag_id = (
				select top 1 PS.Diag_id
				from v_PregnancyScreen PS with(nolock)
			...
			begin try
				...
				set
					Diag_id = @Diag_id,
					PregnancyResult_id = @PregnanyResult_id
				where PersonRegister_id = @PersonRegister_id
			end try
			begin catch
				set @Error_Code = error_number()
	...
	function savePregnancyScreen($data, $isAllowTransaction = true) {
		...
		$query = "
			declare
				@Error_Message varchar(4000),
				@Error_Code bigint,
				@Res bigint = :PregnancyScreen_id;
			exec {$procedure}
				@PregnancyScreen_id = @Res output,
				@PersonRegister_id = :PersonRegister_id,
			...
			select @Res as PregnancyScreen_id, @Error_Code as Error_Code, @Error_Message as Error_Msg;
		";
		$params = array(
			'PregnancyScreen_id' => !empty($data['PregnancyScreen_id'])?$data['PregnancyScreen_id']:null,
			'PersonRegister_id' => $data['PersonRegister_id'],
	...
	function savePregnancyScreenSopDiag($data) {
		...
		$query = "
			declare
				@Error_Message varchar(4000),
				@Error_Code bigint,
				@Res bigint = :PregnancyScreenSopDiag_id;
			exec {$procedure}
				@PregnancyScreenSopDiag_id = @Res output,
				@PregnancyScreen_id = :PregnancyScreen_id,
				@Diag_id = :Diag_id,
				...
				@Error_Code = @Error_Code output,
				@Error_Message = @Error_Message output
			select @Res as PregnancyScreenSopDiag_id, @Error_Code as Error_Code, @Error_Message as Error_Msg;
		";
		$resp = $this->queryResult($query, $params);
		if (!is_array($resp)) {
	...
	function deletePregnancyScreenSopDiag($data) {
		...
		$query = "
			declare
				@Error_Message varchar(4000),
				@Error_Code bigint;
			exec p_PregnancyScreenSopDiag_del
	...
	function deletePregnancyScreen($data, $isAllowTransaction = true) {
		...
		$PersonRegister_id = $this->getFirstResultFromQuery("
			select top 1 PersonRegister_id
			from v_PregnancyScreen with(nolock)
			where PregnancyScreen_id = :PregnancyScreen_id
		", $params);
		if (!$PersonRegister_id) {
			$this->rollbackTransaction();
		...
		$SopDiagList = $this->queryResult("
			select PregnancyScreenSopDiag_id
			from v_PregnancyScreenSopDiag with(nolock)
			where PregnancyScreen_id = :PregnancyScreen_id
		", $params);
	...
	function loadPregnancyScreenForm($data) {
		...
		$query = "
			select top 1
				...
				PS.PregnancyScreen_Comment,
				convert(varchar(10), PS.PregnancyScreen_setDT, 120) as PregnancyScreen_setDate,
				PS.Evn_id,
				...
				PS.GestationalAge_id,
				PP.PersonPregnancy_Period,
				convert(varchar(10), PP.PersonPregnancy_setDT, 120) as PersonPregnancy_setDT
			from
				v_PregnancyScreen PS with(nolock)
				left join v_PersonPregnancy PP with(nolock) on PP.PersonRegister_id=PS.PersonRegister_id
			where
	...
	function loadPregnancyScreenSopDiagGrid($data) {
		...
		$query = "
			select
				PSSD.PregnancyScreenSopDiag_id,
				PSSD.PregnancyScreen_id,
				D.Diag_id,
				D.Diag_FullName,
				DSC.DiagSetClass_id,
				...
				1 as RecordStatus_Code
			from
				v_PregnancyScreenSopDiag PSSD with(nolock)
				left join v_Diag D with(nolock) on D.Diag_id = PSSD.Diag_id
				left join v_DiagSetClass DSC with(nolock) on DSC.DiagSetClass_id = PSSD.DiagSetClass_id
			where
	...
	function loadPersonPregnancyEvnGrid($data) {
		...
		$query = "
			...
			select top 1
				@begDate = PR.PersonRegister_setDate,
				@endDate = isnull(PR.PersonRegister_disDate,dateadd(year, 50, dbo.tzGetDate())),
			...
			from
				...
				left join v_Diag D with(nolock) on D.Diag_id = coalesce(EPL.Diag_id, EPS.Diag_id)
				-- end from
			where
				-- where
				E.EvnClass_SysNick in ('EvnPL','EvnPS')
				and E.Person_id = @Person_id
				and E.Evn_setDate between @begDate and @endDate
				-- end where
			order by
				-- order by
				E.Evn_setDate
		...
		if (count($evn_ids) > 0) {
			...
			$query = "
				select
					E.Evn_rid as Evn_id,
					PP.PersonPregnancy_setDT as setDT,
					'Anketa' as Category
				from v_PersonPregnancy PP with(nolock)
				left join v_Evn E with(nolock) on E.Evn_id = PP.Evn_id
				where E.Evn_rid in ({$evn_ids_str})
				union all
				select
					E.Evn_rid as Evn_id,
					PS.PregnancyScreen_setDT as setDT,
					'Screen' as Category
				from v_PregnancyScreen PS with(nolock)
				left join v_Evn E with(nolock) on E.Evn_id = PS.Evn_id
				where E.Evn_rid in ({$evn_ids_str})
				...
				select
					E.Evn_rid as  Evn_id,
					BC.BirthCertificate_setDT as setDT,
					'Certificate' as Category
				from v_BirthCertificate BC with(nolock)
				left join v_Evn E with(nolock) on E.Evn_id = BC.Evn_id
				where E.Evn_rid in ({$evn_ids_str})
				union all
				select
					E.Evn_rid as Evn_id,
					BSS.BirthSpecStac_OutcomDT as setDT,
					'Result' as Category
				from v_BirthSpecStac BSS with(nolock)
				left join v_Evn E with(nolock) on E.Evn_id = isnull(BSS.EvnSection_id, BSS.Evn_id)
				where E.Evn_rid in ({$evn_ids_str})
				order by setDT
			";
			...
			foreach($resp as $item) {
				...
				switch($item['Category']){
					case 'Anketa':
						...
						break;
					case 'Screen':
						$str = 'Создан скрининг от '.$item['setDT']->format('d.m.Y');
						break;
					case 'Certificate':
	...
	function loadPersonPregnancyEvnUslugaGrid($data) {
		...
		if (!empty($data['AttributeList']) && is_array($data['AttributeList']) && count($data['AttributeList']) > 0) {
			...
			$filter .= "
				and exists(
					...
					where UCA.UslugaComplex_id = UC.UslugaComplex_id and UCAT.UslugaComplexAttributeType_SysNick in ({$attribute_list_str})
				)
			";
		}
		...
		$query = "
			...
			from
				...
				left join v_Evn ParentEvn with(nolock) on ParentEvn.Evn_id = EU.EvnUsluga_pid
				...
				left join v_EvnXml EX with(nolock) on EX.Evn_id = EU.EvnUsluga_id
				-- end from
			where
				-- where
		...
		$query = "
			...
			select
				...
				--(case when EU.Org_uid is NULL then NULL else EU.pmUser_insID end) as pmUser
				-- end select
			from
				-- from
				v_EvnUsluga EU with(nolock)
	...
	function loadBirthSpecStacForm($data) {
		...
		if (getRegionNick() == 'khak'){
			$pregnancysrok = "
				,(select
				(case when LastScreen.PregnancyScreen_setDT is null then '' when PP.PersonPregnancy_setDT is null then '' when PP.PersonPregnancy_begMensDate is null  then ''
					else (datediff(day, PP.PersonPregnancy_begMensDate, PP.PersonPregnancy_setDT) + datediff(day, PP.PersonPregnancy_setDT, LastScreen.PregnancyScreen_setDT))  end) as PregnancySrokInDay
				from v_PersonPregnancy PP with(nolock)
					outer apply (
						select top 1 Screen.*
						from v_PregnancyScreen Screen with(nolock)
				...
				where PP.PersonRegister_id = BSS.PersonRegister_id)	as PregnancySrokInDay
			";
		}
		...
		$query = "
			select top 1
				...
				isnull(BSS.BirthSpecStac_IsHIVtest,1) as BirthSpecStac_IsHIVtest,
				isnull(BSS.BirthSpecStac_IsHIV,1) as BirthSpecStac_IsHIV,
				isnull(BSS.BirthSpecStac_IsRWtest,1) as BirthSpecStac_IsRWtest,
				isnull(BSS.BirthSpecStac_IsRW,1) as BirthSpecStac_IsRW,
				isnull(BSS.BirthSpecStac_IsHBtest,1) as BirthSpecStac_IsHBtest,
				isnull(BSS.BirthSpecStac_IsHB,1) as BirthSpecStac_IsHB,
				isnull(BSS.BirthSpecStac_IsHCtest,1) as BirthSpecStac_IsHCtest,
				isnull(BSS.BirthSpecStac_IsHC,1) as BirthSpecStac_IsHC,
				isnull(BSS.BirthSpecStac_IsContrac,1) as BirthSpecStac_IsContrac,
				BSS.BirthSpecStac_ContracDesc,
				convert(varchar(10), BSS.BirthSpecStac_OutcomDT, 104) as BirthSpecStac_OutcomDate,
				convert(varchar(5), BSS.BirthSpecStac_OutcomDT, 108) as BirthSpecStac_OutcomTime,
				(select PregnancyQuestion_AnswerInt from dbo.PregnancyQuestion with (nolock) where PersonRegister_id=BSS.PersonRegister_id and QuestionType_id=231) as AnketaParitet,
				(select
					top 1 PQ.PregnancyQuestion_ValuesStr
				from dbo.v_PregnancyQuestion PQ with (nolock)
					inner join dbo.v_PregnancyScreen PS with (nolock) on PS.PregnancyScreen_id=PQ.PregnancyScreen_id
				where PQ.PersonRegister_id=BSS.PersonRegister_id and PQ.QuestionType_id=382 order by PS.PregnancyScreen_setDT desc) as ScrinnPredleg,
                (select PregnancyQuestion_AnswerInt from dbo.PregnancyQuestion with (nolock) where PersonRegister_id=BSS.PersonRegister_id and QuestionType_id=770) as AnketaCaesarian,
				(select
					top 1 PQ.PregnancyQuestion_ValuesStr
				from dbo.v_PregnancyQuestion PQ with (nolock)
					inner join dbo.v_PregnancyScreen PS with (nolock) on PS.PregnancyScreen_id=PQ.PregnancyScreen_id
				where PQ.PersonRegister_id=BSS.PersonRegister_id and PQ.QuestionType_id=381 order by PS.PregnancyScreen_setDT desc) as ScrinnPolog,
				BSS.PregnancyType_id as PregnancyType_id,
				OperationCaesarian.IsOperationCaesarian,
				BSS.LaborActivity_id,
				BSS.FetalHeartbeat_id,
				BSS.FetalHead_id,
				isnull(ES.MedPersonal_id,EVPL.MedPersonal_id) as MedPersonal_oid{$pregnancysrok}
			from
				v_BirthSpecStac BSS with(nolock)
				left join v_EvnSection ES with(nolock) on ES.EvnSection_id = BSS.EvnSection_id
				left join v_EvnVizitPL EVPL with(nolock) on EVPL.EvnVizitPL_id = BSS.Evn_id
				outer apply (
					select 1 as IsOperationCaesarian
					from v_EvnUsluga EU with (nolock)
						inner join v_PersonRegister PR with(nolock) on PR.PersonRegister_id = BSS.PersonRegister_id
						inner join v_EvnSection ES with(nolock) on ES.Person_id = PR.Person_id
					...
					where
						ISNULL(EU.EvnUsluga_IsVizitCode, 1) = 1
						and UC.UslugaComplex_Code = 'A16.20.005'
						and EU.EvnUsluga_setDate between BSS.BirthSpecStac_OutcomDT-30 and BSS.BirthSpecStac_OutcomDT
	...
	function loadBirthSpecStacDefaults($data) {
		...
		$queryAnswers = "
			...
			create table {$tableName} (QuestionType_SysNick varchar(100), DispClass_id bigint, Answer varchar(1024));
		";
		...
		if ($params['PersonRegister_id'] > 0 && !empty($data['PregnancyScreenDates'])) {
			...
			$screens = $this->queryResult("
				select
					PS.PregnancyScreen_id,
					cast(PS.PregnancyScreen_setDT as date) as PregnancyScreen_setDate,
					0 as changed
				from v_PregnancyScreen PS with(nolock)
				where PS.PersonRegister_id = :PersonRegister_id
		...
		if (!empty($data['AnketaAnswers']) || !empty($data['LastScreenAnswers'])) {
			...
			$query = "
				select QuestionType_Code, QuestionType_SysNick, DispClass_id
				from v_QuestionType with(nolock)
				where QuestionType_Code in ({$codeList_str}) and QuestionType_SysNick is not null
			";
		...
		if (empty($AnketaAnswers) && $data['PersonRegister_id'] > 0) {
			$queryAnswers .= "
				...
				from
					...
					inner join v_QuestionType QT with(nolock) on QT.QuestionType_id = PQ.QuestionType_id
				where
					QT.DispClass_id = 14 and PQ.PersonRegister_id = @PersonRegister_id;
			";
		}
			...
			$queryAnswers .= "
				...
				where
					...
					and PQ.PregnancyScreen_id = (
						select top 1 PS.PregnancyScreen_id
						from v_PregnancyScreen PS with(nolock)
						where PS.PersonRegister_id = @PersonRegister_id
						order by PS.PregnancyScreen_setDT desc
					);
		...
		$query = "
			...
			declare @PersonRegister_id bigint = :PersonRegister_id;
			declare @Evn_id bigint = :Evn_id;
			declare @currDT datetime = dbo.tzGetDate();
			...
			select top 1
				...
				isnull(ES.EvnSection_pid, E.Evn_id) as Evn_id,
				ES.EvnSection_id,
				E.Lpu_id as Lpu_oid,
				isnull(ES.MedPersonal_id, EVPL.MedPersonal_id) as MedPersonal_oid,
				isnull((
					select top 1 Answer from {$tableName} where QuestionType_SysNick = 'Pregnancy_Count'
				), 1) as BirthSpecStac_CountPregnancy,
				isnull((
					select top 1 Answer from {$tableName} where QuestionType_SysNick = 'Pregnancy_CountBirth'
				), 0) + 1 as BirthSpecStac_CountBirth,
				isnull((
					select top 1 Answer from {$tableName} where QuestionType_SysNick = 'PregChildCount'
				), 0) as BirthSpecStac_CountChild,
				(
					select top 1 BirthPlace_id from v_BirthPlace with(nolock) where BirthPlace_Code = 1
				) as BirthPlace_id,
				...
				--convert(varchar(10), @currDT, 104) as BirthSpecStac_OutcomDate,
				--convert(varchar(5), @currDT, 108) as BirthSpecStac_OutcomTime,
				isnull((
					select top 1 Answer from {$tableName} where QuestionType_SysNick = 'IsHIVtest'
				), 1) as BirthSpecStac_IsHIVtest,
				isnull((
					select top 1 Answer from {$tableName} where QuestionType_SysNick = 'IsHIV'
				), 1) as BirthSpecStac_IsHIV,
				isnull((
					select top 1 Answer from {$tableName} where QuestionType_SysNick = 'IsRWtest'
				), 1) as BirthSpecStac_IsRWtest,
				isnull((
					select top 1 Answer from {$tableName} where QuestionType_SysNick = 'IsRW'
				), 1) as BirthSpecStac_IsRW,
				isnull((
					select top 1 Answer from {$tableName} where QuestionType_SysNick = 'IsHBtest'
				), 1) as BirthSpecStac_IsHBtest,
				isnull((
					select top 1 Answer from {$tableName} where QuestionType_SysNick = 'IsHB'
				), 1) as BirthSpecStac_IsHB,
				isnull((
					select top 1 Answer from {$tableName} where QuestionType_SysNick = 'IsHCtest'
				), 1) as BirthSpecStac_IsHCtest,
				isnull((
					select top 1 Answer from {$tableName} where QuestionType_SysNick = 'IsHC'
				), 1) as BirthSpecStac_IsHC,
                ...
                OperationCaesarian.IsOperationCaesarian,
                (select PregnancyQuestion_AnswerInt from dbo.PregnancyQuestion with (nolock) where PersonRegister_id=@PersonRegister_id and QuestionType_id=231) as AnketaParitet,
				(select
					top 1 PQ.PregnancyQuestion_ValuesStr
				from dbo.v_PregnancyQuestion PQ with (nolock)
					inner join dbo.v_PregnancyScreen PS with (nolock) on PS.PregnancyScreen_id=PQ.PregnancyScreen_id
				where PQ.PersonRegister_id=@PersonRegister_id and PQ.QuestionType_id=382 order by PS.PregnancyScreen_setDT desc) as ScrinnPredleg,
                (select PregnancyQuestion_AnswerInt from dbo.PregnancyQuestion with (nolock) where PersonRegister_id=@PersonRegister_id and QuestionType_id=770) as AnketaCaesarian,
				(select
					top 1 PQ.PregnancyQuestion_ValuesStr
				from dbo.v_PregnancyQuestion PQ with (nolock)
					inner join dbo.v_PregnancyScreen PS with (nolock) on PS.PregnancyScreen_id=PQ.PregnancyScreen_id
				where PQ.PersonRegister_id=@PersonRegister_id and PQ.QuestionType_id=381 order by PS.PregnancyScreen_setDT desc) as ScrinnPolog
			from
				(select 1 as f) as t
				...
				outer apply (
					select 1 as IsOperationCaesarian
					from v_EvnSection ES0 with(nolock)
						left join v_EvnUsluga EU with (nolock) on EU.EvnUsluga_pid=ES0.EvnSection_id
						left join v_UslugaComplex UC with (nolock) on UC.UslugaComplex_id = EU.UslugaComplex_id
					where
						UC.UslugaComplex_Code='A16.20.005' and
						ES0.EvnSection_id=ES.EvnSection_id
				) OperationCaesarian
			SET NOCOUNT OFF
		";
		//echo getDebugSQL($query, $params);exit;
	...
	function loadBirthSpecStac($data) {
		...
		if (empty($BirthSpecStac_id)) {
			$BirthSpecStac_id = $this->getFirstResultFromQuery("
				...
				from v_BirthSpecStac BSS with(nolock)
				where (BSS.PersonRegister_id = :PersonRegister_id or BSS.EvnSection_id = :Evn_id)
			", $params, true);
	...
	function getChildrenCount($data) {
		...
		$query = "
			...
			select (
				select count(*) from v_PersonNewBorn with(nolock) where BirthSpecStac_id = @BirthSpecStac_id
			) + (
				select count(*) from v_ChildDeath with(nolock) where BirthSpecStac_id = @BirthSpecStac_id
			)
		";
		return $this->getFirstResultFromQuery($query, $params);
	}
	...
	function validateBirthSpecStac($data) {
		...
		if (empty($data['BirthSpecStac_id'])) {
			$query = "
				select top 1 count(BSS.BirthSpecStac_id) as cnt
				from v_BirthSpecStac BSS with(nolock)
				where BSS.PersonRegister_id = :PersonRegister_id
			";
			...
			if (empty($data['ignoreCheckBirthSpecStacDate']) || !$data['ignoreCheckBirthSpecStacDate']) {
				$query = "
					...
					from
						...
						inner join v_PersonRegister PR with(nolock) on PR.PersonRegister_id = BSS.PersonRegister_id
					where
						PR.Person_id = (select top 1 Person_id from v_PersonRegister where PersonRegister_id = :PersonRegister_id)
	...
	function saveBirthSpecStac($data, $isAllowTransaction = true) {
		...
		$query = "
			select top 1
				(select PregnancyQuestion_AnswerInt from dbo.PregnancyQuestion with (nolock) where PersonRegister_id = PR.PersonRegister_id and QuestionType_id = 231) as anketaParitet,
				(select top 1 PQ.PregnancyQuestion_ValuesStr from dbo.PregnancyQuestion PQ with (nolock) inner join v_PregnancyScreen PS with (nolock) on PS.PregnancyScreen_id = PQ.PregnancyScreen_id  where PQ.PersonRegister_id = PR.PersonRegister_id and PQ.QuestionType_id = 382 order by PS.PregnancyScreen_setDT desc) as scrinnPredleg,
				(select PregnancyQuestion_AnswerInt from dbo.PregnancyQuestion with (nolock) where PersonRegister_id = PR.PersonRegister_id and QuestionType_id = 770) as anketaCaesarian,
				(select top 1 PQ.PregnancyQuestion_ValuesStr from dbo.PregnancyQuestion PQ with (nolock) inner join v_PregnancyScreen PS with (nolock) on PS.PregnancyScreen_id = PQ.PregnancyScreen_id  where PQ.PersonRegister_id = PR.PersonRegister_id and PQ.QuestionType_id = 381 order by PS.PregnancyScreen_setDT desc) as scrinnPolog
			from
				v_PersonRegister PR with(nolock)
			where
				PR.PersonRegister_id = :PersonRegister_id
		...
		if ($data["BirthSpecStac_BloodLoss"] > 1000){
			...
			if(is_array($users_notice) && count($users_notice)>0){
				...
				@Error_Message varchar(4000),
				@Error_Code bigint;
					exec [dbo].[p_BirthSpecStac_Notice_Add]
				@PersonRegister_id = :PersonRegister_id,
				@UserIdS = :UserIdS,
				@Error_Code = @Error_Code output,
				@Error_Message = @Error_Message output
			...
			SELECT distinct
				...
				where
					(
						...
						or
						puc.lpu_id = (select top 1 lpu_id from dbo.v_BirthSpecStac where PersonRegister_id = :PersonRegister_id order by BirthSpecStac_insDT desc)
					)
					and ISNULL(puc.pmUser_deleted, 1) = 1
					and puc.pmUser_EvnClass like '%is_perinatal_haemorrhage%'
		";
			...
			SELECT distinct
				...
				from
					...
					cross apply (
						...
						union
						select msmp.MedPersonal_id from
		...
		$query = "
			...
			from
				...
				outer apply(
					...
					where E.Person_id in (select Person_id from v_PersonNewBorn with(nolock) where BirthSpecStac_id = @BirthSpecStac_id)
				) Evn
				outer apply(
					select top 1 count(*) as cnt
					from v_BirthSvid BS with(nolock)
					where BS.Person_cid in (select Person_id from v_PersonNewBorn with(nolock) where BirthSpecStac_id = @BirthSpecStac_id)
					and isnull(BS.BirthSvid_IsBad, 1) = 1
				) BirthSvid
				outer apply(
					select top 1 count(*) as cnt
					from v_PntDeathSvid PDS with(nolock)
					where (
						PDS.Person_cid in (select Person_id from v_PersonNewBorn with(nolock) where BirthSpecStac_id = @BirthSpecStac_id)
						or PDS.PntDeathSvid_id in (select PntDeathSvid_id from v_ChildDeath where BirthSpecStac_id = @BirthSpecStac_id)
					)
	...
	function generateDeathMother($data) {
		...
		$query = "
			select top 1
				...
				convert(varchar(10), isnull(DS.DeathSvid_DeathDate, ES.EvnSection_disDT), 104) as DeathMother_DeathDate,
				convert(varchar(5), isnull(DS.DeathSvid_DeathDate, ES.EvnSection_disDT), 108) as DeathMother_DeathTime,
				isnull(DMT.DeathMotherType_id, DM.DeathMotherType_id) as DeathMotherType_id,
				ES.Diag_id as Diag_cid,
				isnull(ED.Diag_aid, DM.Diag_aid) as Diag_aid,
				isnull(DP.DeathPlace_Name, DM.DeathMother_DeathPlace) as DeathMother_DeathPlace
			from
				v_EvnSection ES
				inner join v_EvnDie ED with(nolock) on ED.EvnDie_pid = ES.EvnSection_id
				inner join v_PersonRegister PR with(nolock) on PR.Person_id = ES.Person_id and PR.PersonRegister_id = :PersonRegister_id
				left join v_DeathMother DM with(nolock) on DM.PersonRegister_id = PR.PersonRegister_id
				left join v_BirthSpecStac BSS with(nolock) on BSS.PersonRegister_id = PR.PersonRegister_id
				left join v_PregnancyResult Result with(nolock) on Result.PregnancyResult_id = BSS.PregnancyResult_id
				outer apply(
					select top 1 DMT.DeathMotherType_id
					from v_DeathMotherType DMT with(nolock)
	...
	function loadDeathMotherForm($data) {
		...
		$query = "
			select top 1
				DM.DeathMother_id,
				DM.PersonRegister_id,
				DM.Evn_id,
				convert(varchar(10), DM.DeathMother_DeathDate, 104) as DeathMother_DeathDate,
				DM.DeathMother_DeathPlace,
				DM.DeathMotherType_id,
				DM.Diag_cid,
				DM.Diag_aid,
				PR.Lpu_did as Lpu_oid,
				PR.MedPersonal_did as MedPersonal_oid
			from
				v_DeathMother DM with(nolock)
				left join v_PersonRegister PR with(nolock) on PR.PersonRegister_id = DM.PersonRegister_id
			where DM.DeathMother_id = :DeathMother_id
		";
		return $this->queryResult($query, $params);
	}
	...
	function saveDeathMother($data, $isAllowTransaction = true) {
		...
		$query = "
			declare
				@Error_Message varchar(4000),
				@Error_Code bigint,
				@Res bigint = :DeathMother_id;
			exec {$procedure}
				@DeathMother_id = @Res output,
				@PersonRegister_id = :PersonRegister_id,
				@Evn_id = :Evn_id,
				...
				@Error_Code = @Error_Code output,
				@Error_Message = @Error_Message output
			select @Res as DeathMother_id, @Error_Code as Error_Code, @Error_Message as Error_Msg;
		";
		$resp = $this->queryResult($query, $params);
		if (!is_array($resp)) {
			$this->rollbackTransaction();
	...
	function deleteDeathMother($data) {
		...
		$info = $this->getFirstRowFromQuery("
			select top 1
				PR.PersonRegister_id,
				Result.PregnancyResult_Code,
				BSS.Lpu_id,
				isnull(ES.MedPersonal_id,EVPL.MedPersonal_id) as MedPersonal_id,
				convert(varchar(10), BSS.BirthSpecStac_OutcomDT, 108) as BirthSpecStac_OutcomDate
			from
				v_DeathMother DM with(nolock)
				inner join v_PersonPregnancy PR with(nolock) on PR.PersonRegister_id = DM.PersonRegister_id
				left join v_BirthSpecStac BSS with(nolock) on BSS.PersonRegister_id = PR.PersonRegister_id
				left join v_PregnancyResult Result with(nolock) on Result.PregnancyResult_id = BSS.PregnancyResult_id
				left join v_EvnSection ES with(nolock) on ES.EvnSection_id = BSS.EvnSection_id
				left join v_EvnVizitPL EVPL with(nolock) on EVPL.EvnVizitPL_id = BSS.Evn_id
	...
	function loadBirthCertificateForm($data) {
		...
		$query = "
			select top 1
				BC.BirthCertificate_id,
				BC.PersonRegister_id,
				BC.BirthCertificate_Num,
				BC.BirthCertificate_Ser,
				convert(varchar(10), BC.BirthCertificate_setDT, 104) as BirthCertificate_setDate,
				BC.Lpu_id as Lpu_oid
			from v_BirthCertificate BC with(nolock)
			where BC.BirthCertificate_id = :BirthCertificate_id
		";
		return $this->queryResult($query, $params);
	}
	...
	function saveBirthCertificate($data) {
		...
		$query = "
			declare
				@Error_Message varchar(4000),
				@Error_Code bigint,
				@Res bigint = :BirthCertificate_id;
			exec {$procedure}
				@BirthCertificate_id = @Res output,
				@PersonRegister_id = :PersonRegister_id,
				...
				@Error_Code = @Error_Code output,
				@Error_Message = @Error_Message output
			select @Res as BirthCertificate_id, @Error_Code as Error_Code, @Error_Message as Error_Msg;
		";
		$resp = $this->queryResult($query, $params);
		if (!is_array($resp)) {
	...
	function getPersonRegisterInfo($data) {
		...
		if (!empty($data['PersonRegister_id'])) {
			$query = "
				select top 1
					...
					ISNULL(PP.PersonPregnancy_RiskDPP, 0) + ISNULL(LastScreen.PregnancyScreen_RiskPerPat, 0) as PersonPregnancy_ObRisk,
					(case when RT.RiskType_Name = 'умеренный' then 'средний' else RT.RiskType_Name end) As RiskType_Name,
					(case when RKT.RiskType_Name is null then '&nbsp;&nbsp;&nbsp;&nbsp;' else RKT.RiskType_Name end) as RiskType_AName,
                    (case when ML.MesLevel_Name = 'Второй уровень (без высокотехнологичной помощи)' then 'МПЦ' when ML.MesLevel_Name is null then '    ' else ML.MesLevel_Name end) as MesLevel_Name,
				...
				from
					...
					left join v_RiskType RKT with(nolock) on RKT.RiskType_id = PR.RiskType_aid
					left join v_MesLevel ML with(nolock) on ML.MesLevel_id = PR.MesLevel_id
					left join v_PregnancyQuestion PQ with(nolock) on PQ.PersonRegister_id = PR.PersonRegister_id and PQ.QuestionType_id=772
					left join v_RobsonPopulation RP with(nolock) on RP.RobsonPopulation_id = PQ.PregnancyQuestion_ValuesStr
				where
					PR.PersonRegister_id = :PersonRegister_id
			";
		} else {
		...
		if (!empty($data['PersonRegister_id'])) {
			$this->db->query("update dbo.PersonPregnancy with (rowlock) set RiskType_bid=[dbo].[GetPregnancy572NRisk](PersonRegister_id,2), RiskType_did=dbo.GetPregnancyPRRisk(PersonRegister_id,2) where PersonRegister_id = :PersonRegister_id", array(
				'PersonRegister_id' => $data['PersonRegister_id']
			));
		}
	 ...
	 */
	function getAnketaForScreen($data) {
		$params = array('PersonRegister_id' => $data['PersonRegister_id']);
		$query = "
			select top 1 PersonPregnancy_Period, convert(varchar(10), PersonPregnancy_setDT, 120) as PersonPregnancy_setDT
			from v_PersonPregnancy with(nolock)
			where PersonRegister_id = :PersonRegister_id
		";
    ...
    function getEcoLpuId() {
        $query = "
            ...
            from v_LpuLicence LpuLicence (nolock)
                inner join fed.v_LpuLicenceProfile LLP with (nolock) on LLP.LpuLicence_id=LpuLicence.LpuLicence_id
                inner join fed.LpuLicenceProfileType LLPT with (nolock) on LLPT.LpuLicenceProfileType_id=LLP.LpuLicenceProfileType_id
            where
    ...
    function getDifferentLpu(){
        $params = array();
        $query = "select LpuDifferent_id, LpuDifferent_Code, LpuDifferent_Name from [Dbo].[LpuDifferent]";
     ...
     * Удаление Иное МО
     */
    function deleteDifferentLpu($data) {
	...
	function getFilterObstetricComplication($value)
	{
		...
		if ($value == 1)
			return array(
				'join' => '
					outer apply(
						select
							top 1 PC1.Preeclampsia_Code as Val
						from dbo.PregnancyQuestion PQ1 with (nolock)
							inner join v_PregnancyScreen PS1 with (nolock) on PS1.PregnancyScreen_id = PQ1.PregnancyScreen_id
							inner join v_Preeclampsia PC1 with (nolock) on PC1.Preeclampsia_id = PQ1.PregnancyQuestion_ValuesStr
		...
		if ($value == 2)
			return array(
				'join' => '
						outer apply(
							select
								top 1 PQ1.PregnancyQuestion_IsTrue as Val
							from dbo.PregnancyQuestion PQ1 with (nolock)
								inner join v_PregnancyScreen PS1 with (nolock) on PS1.PregnancyScreen_id = PQ1.PregnancyScreen_id
		...
		if ($value == 3)
			return array(
				'join' => '
					outer apply(
						select
							top 1 PQ2.PregnancyQuestion_ValuesStr as Val
						from v_PregnancyScreen PS1 with (nolock)
							inner join v_PregnancyQuestion PQ1 with (nolock) on PQ1.PregnancyScreen_id=PS1.PregnancyScreen_id
							inner join v_PregnancyQuestion PQ2 with (nolock) on PQ2.PregnancyScreen_id=PS1.PregnancyScreen_id
		...
		if ($value == 5)
			return array(
				'join' => '
					left join v_PregnancyQuestion PQ1 with(nolock) on PQ1.PersonRegister_id = PR.PersonRegister_id and PQ1.QuestionType_id = 599
					left join v_PregnancyQuestion PQ2 with(nolock) on PQ2.PersonRegister_id = PR.PersonRegister_id and PQ2.QuestionType_id = 600
					left join v_PregnancyQuestion PQ3 with(nolock) on PQ3.PersonRegister_id = PR.PersonRegister_id and PQ3.QuestionType_id = 601
				',
				'filters' => '
    ...
    function checkAndSaveQuarantine($data) {
		...
		$pp = $this->getFirstRowFromQuery("
			select top 1 PersonPregnancy_id, PersonPregnancy_IsInQuarantine
			from v_PersonPregnancy PP (nolock) 
			inner join v_PersonRegister PR (nolock) on PP.PersonRegister_id = PR.PersonRegister_id
			where 
				PP.Person_id = :Person_id
		...
		$chk = $this->getFirstResultFromQuery("
			with diags as (select Diag_id from Diag (nolock) where Diag_Code in ('U07.1', 'U07.2', 'Z03.8', 'Z11.5', 'Z20.8', 'Z22.8')) 
			
			select 
				case 
					when exists (select top 1 EvnVizitPL_id from v_EvnVizitPL (nolock) where Person_id = :Person_id and Diag_id in (select Diag_id from diags)) then 1 
					else 0 
				end +
				case 
					when exists (select top 1 EvnSection_id from v_EvnSection (nolock) where Person_id = :Person_id and Diag_id in (select Diag_id from diags)) then 1 
					else 0 
				end +
				case 
					when exists (select top 1 PersonQuarantine_id from v_PersonQuarantine (nolock) where Person_id = :Person_id) then 1 
					else 0 
				end 
			as dsign
		", $data);
	...
	function checkLinkEvn($data){
		$query="select count(*)
 			from PersonPregnancy pp WITH(NOLOCK)
 			where pp.Evn_id  in (select e.Evn_id from v_Evn e WITH(NOLOCK)  where  :Evn_id in (e.Evn_id, e.Evn_pid,e.Evn_rid))";
