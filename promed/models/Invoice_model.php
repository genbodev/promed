...
class Invoice_model extends swModel {
	/**
	 * Конструктор
	 */
	...
	function loadInvoiceGrid($data) {
		...
		if (!empty($data['InventoryItem_id'])) {
			$filters .= " and exists(
				select top 1 InvoicePosition_id
				from v_InvoicePosition IP with(nolock)
		...
		$query = "
			...
			select
				-- select
				I.Invoice_id,
				convert(varchar(10), I.Invoice_Date, 104) as Invoice_Date,
				I.Invoice_Num,
				IPSum.Invoice_Sum,
				IT.InvoiceType_id,
				IT.InvoiceType_Code,
				Subj.InvoiceSubject_id,
				Subj.InvoiceSubject_Name,
				S.Storage_id,
				S.Storage_Name
				-- end select
			from
				-- from
				v_Invoice I with(nolock)
				inner join v_InvoiceType IT with(nolock) on IT.InvoiceType_id = I.InvoiceType_id
				inner join v_InvoiceSubject Subj with(nolock) on Subj.InvoiceSubject_id = I.InvoiceSubject_id
				inner join storage_list S with(nolock) on S.Storage_id = I.Storage_id
				outer apply(
					select top 1
						sum(InvoicePosition_Count*InvoicePosition_Price) as Invoice_Sum
					from v_InvoicePosition with(nolock)
	...
	function loadShipmentGrid($data) {
		...
		$query = "
			...
			select
				-- select
				Sh.Shipment_id,
				convert(varchar(10), Sh.Shipment_setDate, 104) as Shipment_setDate,
				Sh.Shipment_Price,
				Sh.Shipment_Count - isnull(t.Summa, 0) as Shipment_Count,
				II.InventoryItem_id,
				...
				O.Okei_id,
				O.Okei_NationSymbol,
				convert(varchar(10), LastInvoiceOut.Invoice_Date, 104) as LastInvoiceOut_Date
				-- end select
			from
				-- from
				v_Shipment Sh with(nolock)
				inner join v_InventoryItem II with(nolock) on II.InventoryItem_id = Sh.InventoryItem_id
				inner join v_Okei O with(nolock) on O.Okei_id = II.Okei_id
				inner join storage_list S with(nolock) on S.Storage_id = Sh.Storage_id
				outer apply (
					select top 1
					...
					inner join v_InvoiceSubject ISubj with(nolock) on ISubj.InvoiceSubject_id = I.InvoiceSubject_id
	...
	function loadInvoicePositionGrid($data) {
		...
		$query = "
			select
				IP.InvoicePosition_id,
				IP.Invoice_id,
				1 as RecordStatus_Code,
				IP.InvoicePosition_PositionNum,
				IP.InvoicePosition_Coeff,
				IP.InvoicePosition_Comment,
				IP.InvoicePosition_Count,
				IP.InvoicePosition_Count as InvoicePosition_PrevCount,
				IP.InvoicePosition_Price,
				...
				II.InventoryItem_Name
			from v_InvoicePosition IP with(nolock)
				left join v_Okei O with(nolock) on O.Okei_id = IP.Okei_id
				left join v_Shipment Sh with(nolock) on Sh.Shipment_id = IP.Shipment_id
				left join v_InventoryItem II with(nolock) on II.InventoryItem_id = Sh.InventoryItem_id
	...
	function loadInvoiceSubjectList($data) {
		...
		if (!empty($data['InvoiceSubject_id'])) {
			...
			if (!empty($data['query'])) {
				$filters .= " and str(Subj.InvoiceSubject_Code)+' '+Subj.InvoiceSubject_Name like '%'+:query+'%'";
		...
		$query = "
			select
				Subj.InvoiceSubject_id,
				Subj.InvoiceSubject_pid,
				Subj.InvoiceSubject_Code,
				Subj.InvoiceSubject_Name,
				convert(varchar(10), Subj.InvoiceSubject_begDate, 104) as InvoiceSubject_begDate,
				convert(varchar(10), Subj.InvoiceSubject_endDate, 104) as InvoiceSubject_begDate,
				Subj.InvoiceSubject_IsLevel
			from v_InvoiceSubject Subj with(nolock)
			where
				{$filters}
		";
	...
	function saveInvoice($data) {
		...
		$query = "
			...
			exec {$procedure}
				...
				@Error_Code = @Error_Code output,
				@Error_Message = @Error_Message output
			select @Invoice_id as Invoice_id, @Error_Code as Error_Code, @Error_Message as Error_Msg
		";
	...
	function calculateInvoicePositions($data) {
		...
		$query = "
			select
				...
				IP.Shipment_id,
				IP.Okei_id,
				isnull(IP.InvoicePosition_Coeff, 1) as InvoicePosition_Coeff,
				IP.InvoicePosition_Price,
				IP.InvoicePosition_Count,
				IP.InvoicePosition_Comment,
		...
		$query = "
			...
			group by
				IP.Invoice_id,
				Sh.InventoryItem_id,
				isnull(IP.InvoicePosition_Coeff, 1),
				IP.Okei_id
		";
	...
	function saveInvoicePosition($data, $excludeReserved = array()) {
		...
		$query = "
			declare
				@InvoicePosition_id bigint = :InvoicePosition_id,
				@InvoicePosition_PositionNum int = :InvoicePosition_PositionNum,
				@Error_Code bigint,
				@Error_Message varchar(4000);

			if @InvoicePosition_id is null
			set @InvoicePosition_PositionNum = isnull((
				select top 1 max(IP.InvoicePosition_PositionNum)
				from v_InvoicePosition IP with(nolock)
				where IP.Invoice_id = :Invoice_id
			), 0)+1

			exec {$procedure}
				@InvoicePosition_id = @InvoicePosition_id output,
				@Invoice_id = :Invoice_id,
				@InvoicePosition_PositionNum = @InvoicePosition_PositionNum,
				...
				@Error_Code = @Error_Code output,
				@Error_Message = @Error_Message output
			select @InvoicePosition_id as InvoicePosition_id, @Error_Code as Error_Code, @Error_Message as Error_Msg
		";
	...
	function saveShipment($data) {
		...
		$query = "
			declare
				@Shipment_id bigint = :Shipment_id,
				@Error_Code bigint,
				@Error_Message varchar(4000);
			exec {$procedure}
				@Shipment_id = @Shipment_id output,
				@Storage_id = :Storage_id,
				...
				@Error_Code = @Error_Code output,
				@Error_Message = @Error_Message output
			select @Shipment_id as Shipment_id, @Error_Code as Error_Code, @Error_Message as Error_Msg
		";
	...
	function checkShipment($data, $excludeReserved = array()) {
		if ($data['InvoiceType_id'] == 1 && !empty($data['Shipment_id'])) {
			...
			$query = "
				select top 1 COUNT(InvoicePosition_id) as Count
				from v_InvoicePosition IP with(nolock)
				inner join v_Invoice I with(nolock) on I.Invoice_id = IP.Invoice_id
				inner join v_Shipment Sh with(nolock) on Sh.Shipment_id = IP.Shipment_id
			...
			$query = "
				select top 1 SRWT.StorageRecWriteType_Code
				from v_Shipment Sh with(nolock)
				inner join v_Storage S with(nolock) on S.Storage_id = Sh.Storage_id
				inner join v_StorageRecWriteType SRWT with(nolock) on SRWT.StorageRecWriteType_id = S.StorageRecWriteType_id
			...
			$query = "
				declare @reserved_count numeric(18,2) = isnull((
					select top 1
						SUM(IP.InvoicePosition_Count*isnull(IP.InvoicePosition_Coeff,1)) as Summa
					from
						v_InvoicePosition IP with(nolock)
	...
	function getShipmentData($data) {
		...
		$query = "
			...
			where
				...
				and Sh.Storage_id = :Storage_id
				and Sh.Shipment_Count > 0
				and Sh.Shipment_Count-isnull(t.Summa, 0) > 0
				{$shipmentFilter}
			order by
				{$order}
		";
