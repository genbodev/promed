...
* @version      01.07.2019
*/
abstract class Scenario_model extends SwModel {
	...
	public function getAttributesValues () {
		...
		foreach ($this->defAttribute as $key => $info) {
			...
			if($value instanceof DateTime) {
				// datetime конвертируется при возвращении данных в d.m.Y H:i, только при наличии свойства PROPERTY_DATE_TIME
				if(empty($info['properties']) || !in_array(self::PROPERTY_DATE_TIME, $info['properties'])) {
					//а вот это нигде не конвертируется, todo подумать над свойством date и time у аттрибутов
	...
	private function _SafelyUpdate($data=array()) {
		...
		$fields[] = "{$object}_updDT = dbo.tzGetDate()";
		...
		if(!$result) {
			$response['success'] = false;
			$response['Error_Msg'] = 'Ошибка при обновлении аттрибутов объекта';
			$response['Error_Code'] = '';
		} else {
			$response['success'] = empty($result[0]['Error_Message']) && empty($result[0]['Error_Code']);
			$response['Error_Msg'] = $result[0]['Error_Message'];
			$response['Error_Code'] = $result[0]['Error_Code'];
		}
