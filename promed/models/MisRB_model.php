...
class MisRB_model extends swModel {
		...
		if ($withoutLink) {
			...
			$query = "
				select top 1
					mt.MIS{$table}_Code as code
				from
					r3.MIS{$table}Link mtl with (nolock)
					inner join r3.MIS{$table} mt with (nolock) on mtl.MIS{$table}_id = mt.MIS{$table}_id 
				where
	...
	function getPersonInfo($data) {
		...
		$query = "
			...
			from
				...
				left join v_Org do with (nolock) on do.Org_id = od.Org_id
	...
	function getEvnPLInfo($data) {
		...
		$query = "
			select top 1
				EPL.EvnPL_id,
				EPL.Person_id,
				convert(varchar(23), EPL.EvnPL_setDate, 126) as EvnPL_setDate,
				case
					when EPL.EvnPL_IsFinish = 2 then convert(varchar(23), EPL.EvnPL_disDate, 126)
					else null
				end as EvnPL_disDate,
				EPL.EvnPL_NumCard,
				EVPLLAST.PayType_id,
				EPL.Lpu_id,
				rdt.ResultDeseaseType_fedid,
				EPL.ResultClass_id,
				EPL.EvnDirection_id,
				EVPLLAST.MPLpu_id,
				EVPLLAST.MPSex_Code,
				EVPLLAST.MPPerson_BirthDay,
				EVPLLAST.MPPerson_id,
				EVPLLAST.MPPerson_SurName,
				EVPLLAST.MPPerson_FirName,
				EVPLLAST.MPPerson_SecName,
				EVPLLAST.MPMedSpecOms_Code,
				EVPLLAST.MPPost_Code,
				EVPLLAST.MPPerson_SNILS,
				EVPLLAST.MPPolisType_id,
				EVPLLAST.MPPolis_Ser,
				EVPLLAST.MPPolis_Num,
				EVPLLAST.MPPolis_endDate,
				EVPLLAST.MPPolis_begDate,
				EVPLLAST.MPOrgSMO_f002smocod,
				EVPLLAST.MPOrgSMO_Name,
				EVPLLAST.MPOmsSprTerr_Code,
				EVPLLAST.MPDocumentType_Code,
				EVPLLAST.MPDocument_Ser,
				EVPLLAST.MPDocument_Num,
				EVPLLAST.MPDocument_endDate,
				EVPLLAST.MPDocument_begDate,
				EVPLLAST.MPDOrg_Name,
				EVPLLAST.Diag_Code,
				EVPLLAST.DeseaseType_id
			from
				...
				cross apply (
					select top 1
						...
						convert(varchar(10), PS.Person_BirthDay, 120) as MPPerson_BirthDay,
						...
						convert(varchar(10), d.Document_begDate, 120) as MPDocument_begDate,
						convert(varchar(10), d.Document_endDate, 120) as MPDocument_endDate,
						do.Org_Name as MPDOrg_Name,
						ps.PolisType_id as MPPolisType_id,
						ps.Polis_Ser as MPPolis_Ser,
						ps.Polis_Num as MPPolis_Num,
						convert(varchar(10), p.Polis_begDate, 120) as MPPolis_begDate,
						convert(varchar(10), p.Polis_endDate, 120) as MPPolis_endDate,
					...
					from
						...
						left join v_PersonState ps with (nolock) on ps.Person_id = msf.Person_id
						left join v_Document d with (nolock) on d.Document_id = ps.Document_id
						left join v_OrgDep od with (nolock) on od.OrgDep_id = d.OrgDep_id
						left join v_Org do with (nolock) on do.Org_id = od.Org_id
						left join v_DocumentType dt with (nolock) on dt.DocumentType_id = d.DocumentType_id
						...
						left join v_Diag di with (nolock) on di.Diag_id = evpl.Diag_id
					where
	...
	function getEvnPLDispDop13Info($data) {
		...
		$query = "
			select top 1
				...
				EPLDD13.Person_id,
				convert(varchar(23), EPLDD13.EvnPLDispDop13_setDate, 126) as EvnPLDispDop13_setDate,
				convert(varchar(23), EPLDD13.EvnPLDispDop13_disDate, 126) as EvnPLDispDop13_disDate,
				EPLDD13.PayType_id,
				EPLDD13.Lpu_id,
				EPLDD13.EvnPLDispDop13_IsMobile,
				EPLDD13.EvnPLDispDop13_IsDisp,
				EPLDD13.EvnPLDispDop13_IsSanator,
				EPLDD13.EvnPLDispDop13_IsTwoStage,
				EPLDD13.HealthKind_id,
				EVDDTER.EvnVizitDispDop_setDate,
				EVDDTER.MPLpu_id,
				EVDDTER.MPSex_Code,
				EVDDTER.MPPerson_BirthDay,
				EVDDTER.MPPerson_id,
				EVDDTER.MPPerson_SurName,
				EVDDTER.MPPerson_FirName,
				EVDDTER.MPPerson_SecName,
				EVDDTER.MPMedSpecOms_Code,
				EVDDTER.MPPost_Code,
				EVDDTER.MPPerson_SNILS,
				EVDDTER.MPPolisType_id,
				EVDDTER.MPPolis_Ser,
				EVDDTER.MPPolis_Num,
				EVDDTER.MPPolis_endDate,
				EVDDTER.MPPolis_begDate,
				EVDDTER.MPOrgSMO_f002smocod,
				EVDDTER.MPOrgSMO_Name,
				EVDDTER.MPOmsSprTerr_Code,
				EVDDTER.MPDocumentType_Code,
				EVDDTER.MPDocument_Ser,
				EVDDTER.MPDocument_Num,
				EVDDTER.MPDocument_endDate,
				EVDDTER.MPDocument_begDate,
				EVDDTER.MPDOrg_Name,
				EVDDTER.Diag_Code
			from
				...
				cross apply (
					select top 1
						...
						ost.OmsSprTerr_Code as MPOmsSprTerr_Code,
						di.Diag_Code,
						convert(varchar(23), evdd.EvnVizitDispDop_setDT, 126) as EvnVizitDispDop_setDate
					from
						v_EvnVizitDispDop evdd with (nolock)
						...
						left join v_MedStaffFact msf with (nolock) on msf.MedStaffFact_id = evdd.MedStaffFact_id
						left join v_PersonState ps with (nolock) on ps.Person_id = msf.Person_id
						left join v_Document d with (nolock) on d.Document_id = ps.Document_id
						left join v_OrgDep od with (nolock) on od.OrgDep_id = d.OrgDep_id
						left join v_Org do with (nolock) on do.Org_id = od.Org_id
						...
						left join v_MedSpecOms mso with (nolock) on mso.MedSpecOms_id = msf.MedSpecOms_id
						left join persis.Post post with (nolock) on post.id = msf.Post_id
						left join v_Diag di with (nolock) on di.Diag_id = evdd.Diag_id
					where
	...
	function getEvnPLDispProfInfo($data) {
		...
		$query = "
			select top 1
				...
				EPLDP.Person_id,
				convert(varchar(23), EPLDP.EvnPLDispProf_setDate, 126) as EvnPLDispProf_setDate,
				convert(varchar(23), EPLDP.EvnPLDispProf_disDate, 126) as EvnPLDispProf_disDate,
				EPLDP.PayType_id,
				EPLDP.Lpu_id,
				EPLDP.EvnPLDispProf_IsMobile,
	...
	function getEvnPSInfo($data) {
		...
		$query = "
			select top 1
				EPS.EvnPS_id,
				EPS.Person_id,
				convert(varchar(23), EPS.EvnPS_setDate, 126) as EvnPS_setDate,
				convert(varchar(23), EPS.EvnPS_disDate, 126) as EvnPS_disDate,
				EPS.EvnPS_NumCard,
				ESLAST.PayType_id,
				EPS.Lpu_id,
				...
				ESLAST.MPLpu_id,
				ESLAST.MPSex_Code,
				ESLAST.MPPerson_BirthDay,
				ESLAST.MPPerson_id,
				ESLAST.MPPerson_SurName,
				ESLAST.MPPerson_FirName,
				ESLAST.MPPerson_SecName,
				ESLAST.MPMedSpecOms_Code,
				ESLAST.MPPost_Code,
				ESLAST.MPPerson_SNILS,
				ESLAST.MPPolisType_id,
				ESLAST.MPPolis_Ser,
				ESLAST.MPPolis_Num,
				ESLAST.MPPolis_endDate,
				ESLAST.MPPolis_begDate,
				ESLAST.MPOrgSMO_f002smocod,
				ESLAST.MPOrgSMO_Name,
				ESLAST.MPOmsSprTerr_Code,
				ESLAST.MPDocumentType_Code,
				ESLAST.MPDocument_Ser,
				ESLAST.MPDocument_Num,
				ESLAST.MPDocument_endDate,
				ESLAST.MPDocument_begDate,
				ESLAST.MPDOrg_Name,
				ESLAST.Diag_Code,
				ESD.Diag_deathCode,
				EPS.LeaveType_id
			from
				...
				cross apply (
					...
					from
						...
						left join v_MedSpecOms mso with (nolock) on mso.MedSpecOms_id = msf.MedSpecOms_id
						left join persis.Post post with (nolock) on post.id = msf.Post_id
						left join v_Diag di with (nolock) on di.Diag_id = ES.Diag_id
				...
				outer apply (
					...
					from
						...
						inner join v_Diag diag (nolock) on diag.Diag_id = es.Diag_id
						inner join v_LeaveType lt (nolock) on lt.LeaveType_id = es.LeaveType_id
					where
						es.EvnSection_pid = eps.EvnPS_id
	...
	function getEvnVizitPLInfo($data) {
		...
		$query = "
			select
				...
				evpl.ServiceType_id,
				evpl.VizitType_id,
				convert(varchar(23), EVPL.EvnVizitPL_setDate, 126) as EvnVizitPL_setDate,
				convert(varchar(23), EVPL.EvnVizitPL_disDate, 126) as EvnVizitPL_disDate,
				evpl.PayType_id,
				msf.Lpu_id as MPLpu_id,
				s.Sex_Code as MPSex_Code,
				convert(varchar(10), PS.Person_BirthDay, 120) as MPPerson_BirthDay,
			...
			from
				...
				left join v_MedSpecOms mso with (nolock) on mso.MedSpecOms_id = msf.MedSpecOms_id
				left join persis.Post post with (nolock) on post.id = msf.Post_id
				left join v_Diag diag with (nolock) on diag.Diag_id = evpl.Diag_id
			where
	...
	function getEvnUslugaInfo($data) {
		...
		$query = "
			select
				eu.EvnUsluga_id,
				convert(varchar(23), eu.EvnUsluga_setDate, 126) as EvnUsluga_setDate,
				convert(varchar(23), eu.EvnUsluga_disDate, 126) as EvnUsluga_disDate,
				eu.PayType_id,
				msf.Lpu_id as MPLpu_id,
				s.Sex_Code as MPSex_Code,
				convert(varchar(10), PS.Person_BirthDay, 120) as MPPerson_BirthDay,
				...
				os.OrgSMO_Name as MPOrgSMO_Name,
				ost.OmsSprTerr_Code as MPOmsSprTerr_Code,
				ucgost.UslugaComplex_Code,
				ucgost.UslugaComplex_Name,
				eu.EvnUsluga_KolVo
			from
				...
				inner join v_MedStaffFact msf with (nolock) on msf.MedStaffFact_id = eu.MedStaffFact_id
				left join v_PersonState ps with (nolock) on ps.Person_id = msf.Person_id
				left join v_Document d with (nolock) on d.Document_id = ps.Document_id
				left join v_OrgDep od with (nolock) on od.OrgDep_id = d.OrgDep_id
				left join v_Org do with (nolock) on do.Org_id = od.Org_id
	...
	function getEvnDiagPLSopInfo($data) {
		...
		$query = "
			select
				...
				diag.Diag_Code,
				convert(varchar(10), edpls.EvnDiagPLSop_setDate, 120) as EvnDiagPLSop_setDate,
				edpls.DiagSetClass_id,
				msf.Lpu_id as MPLpu_id,
				s.Sex_Code as MPSex_Code,
				convert(varchar(10), PS.Person_BirthDay, 120) as MPPerson_BirthDay,
				ps.Person_id as MPPerson_id,
				...
				ost.OmsSprTerr_Code as MPOmsSprTerr_Code
			from
				v_EvnDiagPLSop edpls with (nolock)
				inner join v_EvnVizitPL evnpl with (nolock) on evnpl.EvnVizitPL_id = edpls.EvnDiagPLSop_pid
				inner join v_MedStaffFact msf with (nolock) on msf.MedStaffFact_id = evnpl.MedStaffFact_id
				left join v_PersonState ps with (nolock) on ps.Person_id = msf.Person_id
				left join v_Document d with (nolock) on d.Document_id = ps.Document_id
				left join v_OrgDep od with (nolock) on od.OrgDep_id = d.OrgDep_id
				left join v_Org do with (nolock) on do.Org_id = od.Org_id
	...
	function getEvnDiagPSInfo($data) {
		...
		$query = "
			...
			from
				...
				inner join v_EvnSection es with (nolock) on es.EvnSection_id = edps.EvnDiagPS_pid
				inner join v_MedStaffFact msf with (nolock) on msf.MedStaffFact_id = es.MedStaffFact_id
				left join v_PersonState ps with (nolock) on ps.Person_id = msf.Person_id
				left join v_Document d with (nolock) on d.Document_id = ps.Document_id
				left join v_OrgDep od with (nolock) on od.OrgDep_id = d.OrgDep_id
				left join v_Org do with (nolock) on do.Org_id = od.Org_id
				...
				left join v_MedSpecOms mso with (nolock) on mso.MedSpecOms_id = msf.MedSpecOms_id
				left join persis.Post post with (nolock) on post.id = msf.Post_id
				left join v_Diag diag with (nolock) on diag.Diag_id = edps.Diag_id
			where
	...
	function getEvnDirectionInfo($data) {
		...
		$query = "
			select
				ed.EvnDirection_id,
				convert(varchar(23), ed.EvnDirection_setDate, 126) as EvnDirection_setDate,
				convert(varchar(23), ed.EvnDirection_disDate, 126) as EvnDirection_disDate,
				ed.Lpu_id,
				ed.Lpu_did,
				ed.DirType_id,
				diag.Diag_Code,
	...
	function getEvnStickInfo($data) {
		...
		$query = "
			select
				es.EvnStick_id,
				convert(varchar(23), es.EvnStick_setDT, 126) as EvnStick_setDate,
				convert(varchar(23), eswr.EvnStickWorkRelease_begDT, 126) as EvnStickWorkRelease_begDate,
				convert(varchar(23), eswr.EvnStickWorkRelease_endDT, 126) as EvnStickWorkRelease_endDate,
				msf.Lpu_id as MPLpu_id,
				s.Sex_Code as MPSex_Code,
				convert(varchar(10), PS.Person_BirthDay, 120) as MPPerson_BirthDay,
				ps.Person_id as MPPerson_id,
			...
			from
				...
				outer apply (
					select top 1
						MedStaffFact_id
					from
						v_EvnStickWorkRelease (nolock)
					where
						EvnStickBase_id = es.EvnStick_id
					order by
						EvnStickWorkRelease_begDT asc
				) eswr_first
				inner join v_MedStaffFact msf with (nolock) on msf.MedStaffFact_id = ISNULL(es.MedStaffFact_id, eswr_first.MedStaffFact_id)
				left join v_PersonState ps with (nolock) on ps.Person_id = msf.Person_id
				left join v_Document d with (nolock) on d.Document_id = ps.Document_id
				left join v_OrgDep od with (nolock) on od.OrgDep_id = d.OrgDep_id
				left join v_Org do with (nolock) on do.Org_id = od.Org_id
	...
	function getEvnUslugaLabInfo($data) {
		...
		$query = "
			...
			from
				v_EvnUsluga eu with (nolock)
				inner join v_EvnLabRequest elr (nolock) on elr.EvnDirection_id = eu.EvnDirection_id -- лабораторные только
				inner join v_UslugaComplex uc with (nolock) on uc.UslugaComplex_id = eu.UslugaComplex_id
				inner join v_UslugaComplex ucgost with (nolock) on ucgost.UslugaComplex_id = uc.UslugaComplex_2011id
				inner join v_MedStaffFact msf with (nolock) on msf.MedStaffFact_id = eu.MedStaffFact_id
				left join v_PersonState ps with (nolock) on ps.Person_id = msf.Person_id
	...
	function getEvnUslugaConsultInfo($data) {
		...
		$query = "
			select
				...
				ost.OmsSprTerr_Code as MPOmsSprTerr_Code
			from
				v_EvnPrescrConsUsluga epcu with (nolock)
				inner join v_EvnPrescrDirection epd with (nolock) on epd.EvnPrescr_id = epcu.EvnPrescrConsUsluga_id 
				inner join v_EvnVizitPL evpl with (nolock) on evpl.EvnDirection_id = epd.EvnDirection_id -- консультативный приём
				left join v_MedStaffFact msf with (nolock) on msf.MedStaffFact_id = evpl.MedStaffFact_id
				left join v_PersonState ps with (nolock) on ps.Person_id = msf.Person_id
				left join v_Document d with (nolock) on d.Document_id = ps.Document_id
				left join v_OrgDep od with (nolock) on od.OrgDep_id = d.OrgDep_id
	...
	function getEvnXmlInfo($data) {
		...
		$query = "
			select
				ex.EvnXml_id,
				convert(varchar(23), ex.EvnXml_insDT, 126) as EvnXml_insDate,
				msf.Lpu_id as MPLpu_id,
				s.Sex_Code as MPSex_Code,
				convert(varchar(10), PS.Person_BirthDay, 120) as MPPerson_BirthDay,
				ps.Person_id as MPPerson_id,
			...
			from
				...
				left join v_EvnSection es with (nolock) on es.EvnSection_id = ex.Evn_id
				left join v_MedStaffFact msf with (nolock) on msf.MedStaffFact_id = es.MedStaffFact_id
				left join v_PersonState ps with (nolock) on ps.Person_id = msf.Person_id
				left join v_Document d with (nolock) on d.Document_id = ps.Document_id
				left join v_OrgDep od with (nolock) on od.OrgDep_id = d.OrgDep_id
				...
				left join v_MedSpecOms mso with (nolock) on mso.MedSpecOms_id = msf.MedSpecOms_id
				left join persis.Post post with (nolock) on post.id = msf.Post_id
				left join XmlTemplateData xtd with (NOLOCK) on xtd.XmlTemplateData_id = ex.XmlTemplateData_id
			where
	...
	function getMesInfo($data) {
		...
		$query = "
			...
			set @Person_Age = (select top 1 dbo.Age2(ps.Person_BirthDay, e.Evn_setDT) from v_Evn e with (nolock) inner join v_PersonState ps with (nolock) on ps.Person_id = e.Person_id where e.Evn_id = :Evn_pid)
			...
			from
				v_Evn e with (nolock)
				left join v_EvnVizitPL ev with (nolock) on ev.EvnVizitPL_id = e.Evn_id
				...
				cross apply (
					...
					from
						v_Mes m with (nolock)
					where
						m.Diag_id = ISNULL(ev.Diag_id, es.Diag_id)
						and (
							m.MesAgeGroup_id is null
							or (@Person_Age >= 18 and m.MesAgeGroup_id = 1)
							or (@Person_Age < 18 and m.MesAgeGroup_id = 2)
						)
				) m	
			where
				e.Evn_pid = :Evn_pid
	...
	function getEvnUslugaFuncInfo($data) {
		...
		$query = "
			...
			from
				v_EvnUsluga eu with (nolock)
				inner join v_EvnFuncRequest efr (nolock) on efr.EvnFuncRequest_pid = eu.EvnDirection_id -- лабораторные только
				inner join v_UslugaComplex uc with (nolock) on uc.UslugaComplex_id = eu.UslugaComplex_id
				inner join v_UslugaComplex ucgost with (nolock) on ucgost.UslugaComplex_id = uc.UslugaComplex_2011id
				inner join v_MedStaffFact msf with (nolock) on msf.MedStaffFact_id = eu.MedStaffFact_id
				left join v_PersonState ps with (nolock) on ps.Person_id = msf.Person_id
	...
	function getEvnReceptInfo($data) {
		...
		$query = "
			select
				er.EvnRecept_id,
				convert(varchar(23), er.EvnRecept_setDate, 126) as EvnRecept_setDate,
				convert(varchar(23), er.EvnRecept_disDate, 126) as EvnRecept_disDate,
				msf.Lpu_id as MPLpu_id,
				s.Sex_Code as MPSex_Code,
				convert(varchar(10), PS.Person_BirthDay, 120) as MPPerson_BirthDay,
				ps.Person_id as MPPerson_id,
			...
			from
				...
				outer apply (
					select top 1
						CLSATC.NAME
					from
						rls.v_Drug rd with (nolock)
						left join rls.PREP Prep with (nolock) on Prep.Prep_id = rd.DrugPrep_id
	...
	function getEvnVizitDispDopInfo($data) {
		...
		$query = "
			select
				...
				convert(varchar(23), EVDD.EvnVizitDispDop_setDate, 126) as EvnVizitDispDop_setDate,
				convert(varchar(23), EVDD.EvnVizitDispDop_disDate, 126) as EvnVizitDispDop_disDate,
				EPLD.PayType_id,
				msf.Lpu_id as MPLpu_id,
				s.Sex_Code as MPSex_Code,
				convert(varchar(10), PS.Person_BirthDay, 120) as MPPerson_BirthDay,
			...
			from
				v_EvnVizitDispDop EVDD with (nolock)
				left join v_EvnPLDisp epld with (nolock) on epld.EvnPLDisp_id = evdd.EvnVizitDispDop_pid
				left join v_MedStaffFact msf with (nolock) on msf.MedStaffFact_id = EVDD.MedStaffFact_id
				left join v_PersonState ps with (nolock) on ps.Person_id = msf.Person_id
				left join v_Document d with (nolock) on d.Document_id = ps.Document_id
				left join v_OrgDep od with (nolock) on od.OrgDep_id = d.OrgDep_id
				...
				left join v_MedSpecOms mso with (nolock) on mso.MedSpecOms_id = msf.MedSpecOms_id
				left join persis.Post post with (nolock) on post.id = msf.Post_id
				left join v_Diag diag with (nolock) on diag.Diag_id = evdd.Diag_id
			where
	...
	function getEvnHistologicProtoInfo($data) {
		...
		$query = "
			select
				...
				convert(varchar(23), EHP.EvnHistologicProto_setDate, 126) as EvnHistologicProto_setDate,
				convert(varchar(23), EHP.EvnHistologicProto_didDate, 126) as EvnHistologicProto_didDate,
				msf.Lpu_id as MPLpu_id,
				s.Sex_Code as MPSex_Code,
				convert(varchar(10), PS.Person_BirthDay, 120) as MPPerson_BirthDay,
				ps.Person_id as MPPerson_id,
			...
			from
				...
				left join v_MedSpecOms mso with (nolock) on mso.MedSpecOms_id = msf.MedSpecOms_id
				left join persis.Post post with (nolock) on post.id = msf.Post_id
				left join v_Diag diag with (nolock) on diag.Diag_id = EHP.Diag_id
			where
	...
	function getEvnSectionInfo($data) {
		...
		$query = "
			select
				ES.EvnSection_id,
				convert(varchar(23), ES.EvnSection_setDate, 126) as EvnSection_setDate,
				convert(varchar(23), ES.EvnSection_disDate, 126) as EvnSection_disDate,
				ES.PayType_id,
				msf.Lpu_id as MPLpu_id,
				s.Sex_Code as MPSex_Code,
				convert(varchar(10), PS.Person_BirthDay, 120) as MPPerson_BirthDay,
			...
			from
				...
				outer apply (
					...
					from
						v_LpuSectionBedProfileLink ss (nolock)
	...
	function getPersonPrivilege($data) {
		...
		$query = "
			...
			from
				...
				inner join r3.MISPrivilegeType mpt with (nolock) on mpt.PrivilegeType_id = pp.PrivilegeType_id
			where
		...
		foreach($resp as $respone) {
			...
			try {
				...
				$this->db->query("
					...
					exec r3.p_MISError_ins						
						@MISError_id = @Res output,
					...
					select @Res as MISError_id, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
				", array(
	...
	function loadMISErrorGrid($data) {
		...
		$query = "
			select
				-- select
				me.MISError_id,
				convert(varchar(10), me.MISError_setDT, 104) + ' ' + convert(varchar(5), me.MISError_setDT, 108) as MISError_setDT,
				l.Lpu_Nick,
				me.MISError_QueryName,
				me.MISError_ErrorCode,
				me.MISError_ErrorMessage,
				me.Evn_id
				-- end select
			from
				-- from
				r3.v_MISError me (nolock)
				left join v_Lpu l (nolock) on l.Lpu_id = me.Lpu_id
				-- end from
			where
				-- where
				1=1
