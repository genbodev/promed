...
class Samara_Person_model extends Person_model {
	...
	function getPostColoredList($data) {
        ...
        if( isset($data['Post_id'])){
            ...
            if( isset($data['query'])){
                $filter .= ' and Post_Name LIKE :query';
	...
	function savePersonDocument(&$person){
		...
		$sql = "select top 1 DocumentType_id
				from DocumentType
	...
	function getPersonSearchGrid($data)
	{
		...
		LEFT JOIN v_Lpu nlpu with (nolock) on ps.Lpu_id = nlpu.Lpu_id -- Petrov	Pavel
		LEFT JOIN v_PersonRefuse with (nolock) ON v_PersonRefuse.Person_id = ps.Person_id and PersonRefuse_IsRefuse = 2 and PersonRefuse_Year = YEAR(dbo.tzGetDate())
		outer apply (
		select top 1 Person_id
		from personprivilege pp with (nolock)
		left join PrivilegeType pt (nolock) on pt.PrivilegeType_id = pp.PrivilegeType_id
		where
		...
		LEFT JOIN v_OftenCallers OC WITH (nolock) on ps.Person_id = OC.Person_id
		-- end from
	...
	function savePersonEditWindowSamara($data)
	{
		...
		if ( empty($data['Person_SNILS']) ) {
		return array(array('Error_Msg' => 'Поле СНИЛС обязательно для заполнения.'));
		}
		else {
		$query = "
		...
		return array(array('Error_Msg' => 'Человек с введённым номером СНИЛС уже есть в базе.'));
		}
		}
		}
		}*/
	...
	|| array_key_exists('PersonSprTerrDop_id', $newFields) )
	{
			...
			if ( is_object($res) )
		{
			...
			if ( $is_pc_upd ) {
				...
				$row = $this->getFirstRowFromQuery('select * from v_PersonChild where personChild_id = :PersonChild_id', array('PersonChild_id' => $data['PersonChild_id']));
				if ($row) {
