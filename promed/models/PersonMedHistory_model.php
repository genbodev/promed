...
class PersonMedHistory_model extends swModel {
	var $scheme = 'dbo';
    /**
     * Doc
     */
	...
	private function getAccessType($data)
	{
		...
		$query = "
			...
			from
				...
				left join v_MedStaffRegion MSR with (nolock) on MSR.LpuRegion_id = PC.LpuRegion_id
			where
	...
	function getPersonMedHistoryViewData($data)
	{
		$query = "
			SELECT TOP 1 
				PersonMedHistory_id
				,Person_id
				,convert(varchar(10),PersonMedHistory_setDT,104) as PersonMedHistory_setDT
	...
	function loadPersonMedHistoryPanel($data) {
		return $this->queryResult("
			select
				...
				convert(varchar(10), pmh.PersonMedHistory_setDT, 104) as PersonMedHistory_setDate,
				pmh.PersonMedHistory_Descr,
				pmh.PersonMedHistory_Text,
				pu.pmUser_Name
			from
				v_PersonMedHistory pmh (nolock)
				left join v_pmUser pu (nolock) on pu.pmUser_id = pmh.pmUser_updID
			where
