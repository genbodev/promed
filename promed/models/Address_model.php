...
* @version      ?
*/
class Address_model extends swModel {
	...
	*/
	...
	function loadChildLists($data)
	{
		...
		else
		{
			...
			if(!empty($data['Form']) && $data['Form'] == 'swAddressEditWindow'){
				...
				";
				$from = "
					LEFT JOIN KLArea klapid with (nolock) ON klapid.KLArea_id = kla.KLArea_id
					left join KLSocr klspid with (nolock) on klapid.KLSocr_id = klspid.KLSocr_id
				";
			...
			if (getRegionNumber() == '10' && $data['KLArea_id'] == '10') {//#10288
				...
				SELECT
					...
					{$select1}
				FROM
					v_KLAreaStat kla with (nolock)
					LEFT JOIN KLArea klaid with (nolock) ON klaid.KLArea_id = kla.KLCity_id
				where 
	...
	public function getCitiesList( $data ) {
		if ( isset( $this->mongo_switch['kladr'] ) && $this->mongo_switch['kladr'] === true ) {
            ...
            if($_SESSION['region']['nick'] == 'kz'){
                $addictWhere = " (kla.KLAdr_Code LIKE :subregion_id + '%' AND kla.KLAreaLevel_id = 3 ) OR ";
            };
		...
		if (isset($data['showSocr']) && ($data['showSocr'] == 1)) {
			$sql = "
				select
					...
					{$select}
				from KLStreet kla with (nolock)
					inner join KLSocr kls WITH (nolock) on kls.KLSocr_id = kla.KLSocr_id
					{$join}
			...
			$sql = "
				SELECT
					...
					{$select}
				from KLStreet kla with (nolock)
					left join KLSocr kls WITH (nolock) on kls.KLSocr_id = kla.KLSocr_id
					{$join}
				where
					kla.KLAdr_Actual = 0
					{$where}
	...
	function loadAddress($data,$addressType = null,$extraData = false)
	{
		...
		if($extraData){
			$extraSelect = " 
				...
				,a.KLAreaStat_id
				,a.Address_Nick
				,a.PersonSprTerrDop_id
				,a.AddressSpecObject_id
				,a.Address_begDate
			";
		}
		$query = "
			select
				...
				a.Address_Address,
				convert(varchar(10), a.Address_insDT, 120) as Address_insDT,
	...
	public function getAddressParamsByFIASCode($AoidArea = null, $AoidStreet = null) {
		...
		if ( !empty($AoidStreet) ) {
			$streetData = $this->getFirstRowFromQuery("
				select top 1 kls.KLStreet_id, kla.KLArea_pid, kla.KLArea_id, kla.KLAreaLevel_id
				from v_KLStreet kls with (nolock)
			...
			while ( $KLAreaLevel_id > 1 && $i < 5 ) {
				...
				$areaData = $this->getFirstRowFromQuery("
					select top 1 KLArea_pid, KLArea_id, KLAreaLevel_id
					from v_KLArea with (nolock)
	...
	public function getAddressTextBrief($address_id) {
		$result = $this->getFirstRowFromQuery("select
    			rtrim(a.Address_House) as Address_House,
                rtrim(a.Address_Corpus) as Address_Corpus,
                rtrim(a.Address_Flat) as Address_Flat,
    		...
    		from
                ...
                left outer join KLArea ci with (nolock) on a.KLCity_id = ci.KLArea_id
                left outer join KLSocr cis with (nolock) on ci.KLSocr_id = cis.KLSocr_id
                left outer join KLArea t with (nolock) on a.KLTown_id = t.KLArea_id
                left outer join KLSocr ts with (nolock) on t.KLSocr_id = ts.KLSocr_id
                left outer join KLStreet s with (nolock) on a.KLStreet_id = s.KLStreet_id
                left outer join KLSocr ss with (nolock) on s.KLSocr_id = ss.KLSocr_id
