	...
	function loadRefValues($data)
	{
		...
		$query = "
			Select
				-- select
				rv.RefValues_id,
				rv.RefValues_OPMUCode,
				rv.RefValues_Name,
				rv.RefValues_Nick,
				rv.RefValuesType_id,
				RefValuesType.RefValuesType_Name,
				rv.RefCategory_id,
				RefCategory.RefCategory_Name,
				rv.RefValues_LowerLimit,
				rv.RefValues_UpperLimit,
				rv.RefValuesUnit_id,
				RefValuesUnit.RefValuesUnit_Name,
				rv.RefMaterial_id,
				RefMaterial.RefMaterial_Name
				-- end select
			from
				-- from
				v_RefValues rv with (NOLOCK)
				inner join v_RefValuesType RefValuesType with(nolock) on RefValuesType.RefValuesType_id = rv.RefValuesType_id
				left join v_RefCategory RefCategory with(nolock) on RefCategory.RefCategory_id = rv.RefCategory_id
				left join v_RefValuesUnit RefValuesUnit with(nolock) on RefValuesUnit.RefValuesUnit_id = rv.RefValuesUnit_id
				left join v_RefMaterial RefMaterial with(nolock) on RefMaterial.RefMaterial_id = rv.RefMaterial_id
			-- end from
			where
				-- where
	...
	function loadRefValuesList($data)
	{
		...
		$query = "
			...
			from
				...
				v_RefValues rv with (NOLOCK)
				left join v_RefValuesUnit RefValuesUnit with (NOLOCK) on RefValuesUnit.RefValuesUnit_id = rv.RefValuesUnit_id
				left join v_RefMaterial rm with (NOLOCK) on rm.RefMaterial_id = rv.RefMaterial_id
				left join v_RefCategory rc with (NOLOCK) on rc.RefCategory_id = rv.RefCategory_id
				/*inner join v_RefValuesType RefValuesType with(nolock) on RefValuesType.RefValuesType_id = rv.RefValuesType_id
				left join v_RefCategory RefCategory with(nolock) on RefCategory.RefCategory_id = rv.RefCategory_id
				*/
			-- end from
	...
	function saveRefValues($data)
	{
		...
		$query = "
			declare
				...
				@RefValues_id bigint = :RefValues_id;
			...
			exec " .$proc. "
				...
				select @RefValues_id as RefValues_id, @Error_Code as Error_Code, @Error_Message as Error_Msg;
		";
		//echo getDebugSQL($query, $params);exit;
		$result = $this->db->query($query, $params);
	...
	function loadEvnXmlList($data) {
		...
		switch ($data['filterDoc']) {
			case 'emk':
				...
				$filters .= 'doc.Person_id = @Person_id';	
				break;
			default:
		...
		{
			$filters .= ' and doc.EvnXml_id != @EvnXml_id';	
		}
		$query = '
			declare
			...
			select
				...
				/*
				,doc.XmlTemplate_id
				,XmlTemplate.XmlTemplateType_id
				,doc.Evn_rid
				,doc.Evn_id
				,doc.Person_id
				,EvnClass.EvnClass_id
				*/
				,CONVERT(varchar,doc.EvnXml_updDT,104) as EvnXml_updDT
				,XmlType.XmlType_Name
			...
			from
				(
					select
						doc.EvnXml_id
						,doc.XmlTemplate_id
						,isnull(doc.XmlType_id,1) as XmlType_id
						,doc.EvnXml_updDT
						,doc.EvnXml_Name
						,doc.Evn_id
						,Evn.EvnClass_id
						...
						,Evn.Person_id
						,doc.pmUser_insID
					from v_EvnXml doc with (nolock) inner join Evn with (nolock) on Evn.Evn_id = doc.Evn_id and isnull(Evn.Evn_deleted,1) = 1
				) doc
				...
				left join v_EvnUslugaPar UP with (nolock) on UP.EvnUslugaPar_id = doc.Evn_id and UP.EvnUslugaPar_setDT is not null
				left join v_UslugaComplex UC with (nolock) on UC.UslugaComplex_id = UP.UslugaComplex_id
				left join pmUserCache puc with (nolock) on doc.pmUser_insID = puc.pmUser_id
				inner join EvnClass with (nolock) on EvnClass.EvnClass_id = doc.EvnClass_id
				inner join XmlTemplate with (nolock) on XmlTemplate.XmlTemplate_id = doc.XmlTemplate_id
				inner join XmlType with (nolock) on XmlType.XmlType_id = doc.XmlType_id
			where
				'. $filters .'
			order by
							...
							else
							{
								...
								if ('EvnXml_Data' == $subsection['code']) {
									...
									if ($object_key == 'EvnXml_id') {
										...
										$res = $this->db->query("
											select top 1 EvnXml_id from v_EvnXml with(nolock) where Evn_id = :Evn_id order by EvnXml_Index desc
										", array('Evn_id' => $evn_id));
		...
		*/
		...
		switch($object)
		{
			...
			case 'PersonPPT':
				...
				$this->load->model('PersonWeight_model', 'PersonWeight_model');
				$heightData = $this->PersonWeight_model->getFirstRowFromQuery("select top 1 PersonHeight_Height, Okei_id from v_PersonHeight with (nolock) where Person_id = :Person_id order by PersonHeight_setDT desc", $data);

				if ( $heightData !== false && !empty($heightData['PersonHeight_Height']) ) {
					$weightData = $this->PersonWeight_model->getFirstRowFromQuery("select top 1 PersonWeight_Weight, Okei_id from v_PersonWeight with (nolock) where Person_id = :Person_id order by PersonWeight_setDT desc", $data);
			...
			case 'EvnPL':
				...
				if (!empty($object_data[0]['EvnPL_id'])) {
					$this->load->model('Registry' . ($regionNick == 'ufa' ? 'Ufa' : '') . '_model', 'Reg_model');
					...
					if (in_array($regionNick, array('buryatiya'/*, 'kareliya'*/))) {
			...
			case 'BactEvnUslugaPar':
				...
				if ($this->usePostgreLis) {
					$this->load->swapi('lis');
					$bactMicroList = $this->lis->GET('EvnUslugaPar/getBactMicroList', $data, 'list');
					$bactAntibioticList = $this->lis->GET('EvnUslugaPar/getBactAntibioticList', $data, 'list');
					$bactIsNotFind = $this->lis->GET('EvnUslugaPar/getBactMicroIsNotFind', $data, 'list');

					$bactIsNotFind = (!empty($bactIsNotFind[0]) && !empty($bactIsNotFind[0]['count'])) ? $bactIsNotFind[0]['count'] : 0;
				} else {
					...
					$bactIsNotFind = empty($this->EvnUslugaPar_model->getBactMicroIsNotFind($data)) ? false : true;
				}
			...
			case 'MantuReaction':
				$modelName = 'Vaccine_model';
				$templateName = 'eew_mantureaction_list';

				if ($regionNick == 'vologda')
				{
					$modelName = 'Vologda_'.$modelName;
					$templateName = 'vologda_'.$templateName;
				}

				$this->load->model($modelName, 'VacModel');
				$object_data = $this->VacModel->getMantuReaction($data);

				$list = array('MantuReactionList'=>$isMobileTemplate.$templateName.'_item');

				$template = $isMobileTemplate.$templateName;
				$parent_key = 'Person_id';
				$parent_value = $data['Person_id'];

				if ($regionNick == 'vologda')
					$object_key = 'JournalMantu_id';
				else
					$object_key = 'dateVac';
			break;
			...
			case 'Inoculation':
				$modelName = 'Vaccine_model';
				$templateName = 'eew_inoculation_list';

				// Вид вакцинации нужен только для региона 'ufa',
				// наименование ЛПУ нужно только для региона 'vologda':
				$data['needType'] = ($regionNick == 'ufa');
				$data['needLpu'] = ($regionNick == 'vologda');

				$this->load->model($modelName,'VacModel');
				$object_data = $this->VacModel->getInoculationData($data);

				$list = array('InoculationList'=>$isMobileTemplate.$templateName.'_item');
				$template = $isMobileTemplate.$templateName;
				$parent_key = 'Person_id';
				$parent_value = $data['Person_id'];
			...
			case 'InoculationPlan':
				$modelName = 'Vaccine_model';
				$templateName = 'eew_inoculationplan_list';

				$this->load->model($modelName,'VacModel');
				$object_data = $this->VacModel->getInoculationPlanData($data);

				$list = array('InoculationPlanList'=>$isMobileTemplate.$templateName.'_item');
				$template = $isMobileTemplate.$templateName;
				$parent_key = 'Person_id';
				$parent_value = $data['Person_id'];
