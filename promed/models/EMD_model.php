 ...
 */
class EMD_model extends swModel {
	/**
	 *	Конструктор
	 */
	...
	protected $documents = array(
		...
		'EvnDirectionHTM' => array(
			'docName' => 'Направление на ВМП', // наименование
			'objectName' => 'EvnDirectionHTM', // объект в БД
			'viewName' => 'v_EvnDirectionHTM', // вьюха для выборки даты и номера
			'numberField' => 'EvnDirectionHTM_id', // поле номера
			'dateField' => 'EvnDirectionHTM_setDT', // поле даты
			'idField' => 'EvnDirectionHTM_id', // поле идентификатора
			'signField' => 'Evn_IsSigned', // поле с признаком подписи
			'signDateField' => 'Evn_signDT', // поле с датой подписи
			'signUserField' => 'pmUser_signID', // поле с пользователем подписи
			'signObject' => 'Evn', // таблица для апдейта признака подписи
			'printParams' => array(
				array(
					'printModel' => 'EvnDirectionHTM_model', // модель в которой есть функция печати
					'printMethod' => 'getEvnDirectionHTMJSON' // метод печати в модели
				)
			)
		),
		'EvnUslugaPar' => array(
			...
			'printParams' => array(
				array(
					'printModel' => 'EvnUslugaPar_model', // модель в которой есть функция печати
					'printMethod' => 'printEvnUslugaParHL7' // метод печати в модели
				)
			)
			/*'printParams' => array(
				array(
...
/**
 * Статус регистрации документа ЕГИСЗ.
 */
	protected $EMDRegistrationStatus =
	[
		...
		// Отправлены на регистрацию:
		2 => [
			'EMDVersion_IsReady' => [2]
		],
	...
	function loadEMDQueryError($data) {
		$query = '
			SELECT
				...
				emdqe."EMDQueryError_Code",
				emdqe."EMDQueryError_Message"
			FROM
	...
	function loadEMDCertificateList($data) {
		...
		if (!empty($data['isMOSign']) && !empty($data['session']['lpu_id'])) {
			...
			$ogrn = $this->getFirstResultFromQuery('
					select top 1 Lpu_OGRN
					from v_Lpu (nolock) where Lpu_id = :Lpu_id
				', array(
	...
	function makeQuerySearchScope($data) {
		...
		-- ТАП
			SELECT TOP 100
				...
				null as EvnXml_id
			FROM v_EvnPL e (nolock)
			INNER JOIN v_Evn evn (nolock) on evn.Evn_id = e.EvnPL_id
			LEFT JOIN v_PersonState ps (nolock) on ps.Person_id = evn.Person_id
			LEFT JOIN v_MedStaffFact msf (nolock) on msf.MedStaffFact_id = e.MedStaffFact_did
			LEFT JOIN v_LpuSection ls (nolock) on ls.LpuSection_id = msf.LpuSection_id
			{$join}
			outer apply (
				SELECT TOP 1 cast(EvnPL_NumCard as varchar) as Doc_Num FROM v_EvnPL (nolock) WHERE EvnPL_id = evn.Evn_id
			) num
			WHERE (1=1)
				and e.EvnClass_id = 3
				{$filter}
		...
		-- Посещение поликлиники
			SELECT TOP 100
				...
				null as EvnXml_id
			FROM v_EvnVizitPL e (nolock)
			INNER JOIN v_Evn evn (nolock) on evn.Evn_id = e.EvnVizitPL_id
			LEFT JOIN v_PersonState ps (nolock) on ps.Person_id = evn.Person_id
			LEFT JOIN v_LpuSection ls (nolock) on ls.LpuSection_id = e.LpuSection_id
			{$join}
			outer apply (
				SELECT TOP 1 cast(EvnVizitPL_id as varchar) as Doc_Num FROM v_EvnVizitPL (nolock) WHERE EvnVizitPL_id = evn.Evn_id
			) num
			WHERE (1=1)
		...
		-- Выписка льготного рецепта
			SELECT TOP 100
				...
				null as EvnXml_id
			FROM v_EvnRecept e (nolock)
			INNER JOIN v_Evn evn (nolock) on evn.Evn_id = e.EvnRecept_id
			LEFT JOIN v_PersonState ps (nolock) on ps.Person_id = evn.Person_id
			LEFT JOIN v_LpuSection ls (nolock) on ls.LpuSection_id = e.LpuSection_id
			{$join}
			outer apply (
				SELECT TOP 1 (EvnRecept_Ser + EvnRecept_Num) as Doc_Num FROM v_EvnRecept (nolock) WHERE EvnRecept_id = evn.Evn_id
			) num
			WHERE (1=1)
				and e.EvnClass_id = 4
				{$filter}
		...
		-- Выписка рецепта
			SELECT TOP 100
				...
				null as EvnXml_id
			FROM v_EvnReceptGeneral e (nolock)
			INNER JOIN v_Evn evn (nolock) on evn.Evn_id = e.EvnReceptGeneral_id
			LEFT JOIN v_PersonState ps (nolock) on ps.Person_id = evn.Person_id
			LEFT JOIN v_LpuSection ls (nolock) on ls.LpuSection_id = e.LpuSection_id
			{$join}
			outer apply (
				SELECT TOP 1 (EvnReceptGeneral_Ser + EvnReceptGeneral_Num) as Doc_Num FROM v_EvnReceptGeneral (nolock) WHERE EvnReceptGeneral_id = evn.Evn_id
			) num
			WHERE (1=1)
		...
		-- Выписка направлений
			SELECT TOP 100
				...
				null as EvnXml_id
			FROM v_EvnDirection_all e (nolock)
			INNER JOIN v_Evn evn (nolock) on evn.Evn_id = e.EvnDirection_id
			LEFT JOIN v_PersonState ps (nolock) on ps.Person_id = evn.Person_id
			LEFT JOIN v_LpuSection ls (nolock) on ls.LpuSection_id = e.LpuSection_id
			{$join}
			outer apply (
				SELECT TOP 1 cast(EvnDirection_Num as varchar) as Doc_Num FROM v_EvnDirection_all (nolock) WHERE EvnDirection_id = evn.Evn_id
			) num
			WHERE (1=1)
				and e.EvnClass_id = 27
				and ISNULL(e.EvnStatus_id, 16) not in (12,13)
				{$filter}
		";
		...
		-- Клинико-экспертная работа и протокол ВК
			SELECT TOP 100
				...
				null as EvnXml_id
			FROM v_EvnVK e (nolock)
			INNER JOIN v_Evn evn (nolock) on evn.Evn_id = e.EvnVK_id
			LEFT JOIN v_MedService ms (nolock) on ms.MedService_id = e.MedService_id
			LEFT JOIN v_LpuSection ls (nolock) on ls.LpuSection_id = ms.LpuSection_id
			LEFT JOIN v_PersonState ps (nolock) on ps.Person_id = evn.Person_id
			{$join}
			outer apply (
				SELECT TOP 1 cast(EvnVK_NumProtocol as varchar) as Doc_Num FROM v_EvnVK (nolock) WHERE EvnVK_id = evn.Evn_id
			) num
			WHERE (1=1)
		...
		-- Специф. сведения о назначении на МСЭ
			SELECT TOP 100
				...
				null as EvnXml_id
			FROM v_EvnPrescrMse e (nolock)
			INNER JOIN v_Evn evn (nolock) on evn.Evn_id = e.EvnPrescrMse_id
			LEFT JOIN v_PersonState ps (nolock) on ps.Person_id = evn.Person_id
			LEFT JOIN v_LpuSection ls (nolock) on ls.LpuSection_id = e.LpuSection_sid
			{$join}
			outer apply (
				SELECT TOP 1 cast(EvnPrescrMse_id as varchar) as Doc_Num FROM v_EvnPrescrMse (nolock) WHERE EvnPrescrMse_id = evn.Evn_id
			) num
			WHERE (1=1)
		...
		-- Направление на ВМП
			SELECT TOP 100
				...
				0 as MedStaffFact_id,
				num.Doc_Num,
				e.EvnDirectionHTM_IsSigned as IsSigned,
				null as EvnXml_id
			FROM v_EvnDirectionHTM e (nolock)
			INNER JOIN v_Evn evn (nolock) on evn.Evn_id = e.EvnDirectionHTM_id
			LEFT JOIN v_PersonState ps (nolock) on ps.Person_id = evn.Person_id
			LEFT JOIN v_LpuSection ls (nolock) on ls.LpuSection_id = e.LpuSection_id
			{$join}
			outer apply (
				SELECT TOP 1 cast(EvnDirectionHTM_id as varchar) as Doc_Num FROM v_EvnDirectionHTM (nolock) WHERE EvnDirectionHTM_id = evn.Evn_id
			) num
			WHERE (1=1)
				and e.EvnClass_id = 117
				{$filter}
		";
		...
		-- Протокол лабораторной услуги
			SELECT TOP 100
				...
				null as EvnXml_id
			FROM v_EvnUslugaPar e (nolock)
			INNER JOIN v_Evn evn (nolock) on evn.Evn_id = e.EvnUslugaPar_id
			inner join v_EvnLabSample els (nolock) on els.EvnLabSample_id = e.EvnLabSample_id
			LEFT JOIN v_PersonState ps (nolock) on ps.Person_id = evn.Person_id
			LEFT JOIN v_EvnDirection_all ed (nolock) on ed.EvnDirection_id = e.EvnDirection_id
			LEFT JOIN v_LpuSection ls (nolock) on ls.LpuSection_id = ed.LpuSection_id
			{$join}
			outer apply (
				SELECT TOP 1 cast(EvnUslugaPar_id as varchar) as Doc_Num FROM v_EvnUslugaPar (nolock) WHERE EvnUslugaPar_id = evn.Evn_id
			) num
			WHERE (1=1)
		...
		-- Обратный талон
			SELECT TOP 100
				...
				null as EvnXml_id
			FROM v_EvnMse e (nolock)
			INNER JOIN v_Evn evn (nolock) on evn.Evn_id = e.EvnMse_id
			LEFT JOIN v_PersonState ps (nolock) on ps.Person_id = evn.Person_id
			LEFT JOIN v_MedService ms (nolock) on ms.MedService_id = e.MedService_id
			LEFT JOIN v_LpuSection ls (nolock) on ls.LpuSection_id = ms.LpuSection_id
			{$join}
			outer apply (
				SELECT TOP 1 cast(EvnMse_NumAct as varchar) as Doc_Num FROM v_EvnMse (nolock) WHERE EvnMse_id = evn.Evn_id
			) num
			WHERE (1=1)
		...
		-- Протокол консультации
			SELECT TOP 100
				...
				x.EvnXml_id
			FROM v_EvnXml x (nolock)
			INNER JOIN v_Evn evn (nolock) on evn.Evn_id = x.Evn_id
			INNER JOIN v_EvnPL e (nolock) on e.EvnPL_id = evn.Evn_id
			LEFT JOIN v_PersonState ps (nolock) on ps.Person_id = evn.Person_id
			LEFT JOIN v_MedStaffFact msf (nolock) on msf.MedStaffFact_id = e.MedStaffFact_did
			LEFT JOIN v_LpuSection ls (nolock) on ls.LpuSection_id = msf.LpuSection_id
			{$join}
			outer apply (
				SELECT TOP 1 cast(EvnXml_id as varchar) as Doc_Num FROM v_EvnXml (nolock) WHERE EvnXml_id = x.EvnXml_id
			) num
			WHERE (1=1)
				and e.EvnClass_id = 3
				and x.XmlType_id = 3
		...
		-- Осмотры
			...
			FROM v_EvnXml x (nolock)
			INNER JOIN v_Evn evn (nolock) on evn.Evn_id = x.Evn_id
			INNER JOIN v_EvnSection e (nolock) on e.EvnSection_id = evn.Evn_id
			LEFT JOIN v_PersonState ps (nolock) on ps.Person_id = evn.Person_id
		...
		-- Справка
			...
			FROM v_EvnXml x (nolock)
			INNER JOIN v_Evn evn (nolock) on evn.Evn_id = x.Evn_id
			INNER JOIN v_EvnVizitPL e (nolock) on e.EvnVizitPL_id = evn.Evn_id
			LEFT JOIN v_PersonState ps (nolock) on ps.Person_id = evn.Person_id
			LEFT JOIN v_LpuSection ls (nolock) on ls.LpuSection_id = e.LpuSection_id
			{$join}
			outer apply (
		...
		-- Справка студенту / учащемуся / ребенку о болезни/карантине и прочих причинах отсутствия
			...
			LEFT JOIN v_PersonState ps (nolock) on ps.Person_id = evn.Person_id
			LEFT JOIN v_LpuSection ls (nolock) on ls.LpuSection_id = e.LpuSection_id
			LEFT JOIN v_XmlTemplate tpl (nolock) on tpl.XmlTemplate_id = x.XmlTemplate_id
			{$join}
			outer apply (
				SELECT TOP 1 cast(EvnXml_id as varchar) as Doc_Num FROM v_EvnXml (nolock) WHERE EvnXml_id = x.EvnXml_id
			) num
		...
		-- Справка
			...
			FROM v_EvnXml x (nolock)
			INNER JOIN v_Evn evn (nolock) on evn.Evn_id = x.Evn_id
			INNER JOIN v_EvnVizitPLStom e (nolock) on e.EvnVizitPLStom_id = evn.Evn_id
			LEFT JOIN v_PersonState ps (nolock) on ps.Person_id = evn.Person_id
			LEFT JOIN v_LpuSection ls (nolock) on ls.LpuSection_id = e.LpuSection_id
			{$join}
			outer apply (
		...
		-- Протокол консультации
			...
			FROM v_EvnXml x (nolock)
			INNER JOIN v_Evn evn (nolock) on evn.Evn_id = x.Evn_id
			INNER JOIN v_EvnUslugaCommon e (nolock) on e.EvnUslugaCommon_id = evn.Evn_id
			LEFT JOIN v_PersonState ps (nolock) on ps.Person_id = evn.Person_id
			LEFT JOIN v_LpuSection ls (nolock) on ls.LpuSection_id = e.LpuSection_uid
			{$join}
			outer apply (
				SELECT TOP 1 cast(EvnXml_id as varchar) as Doc_Num FROM v_EvnXml (nolock) WHERE EvnXml_id = x.EvnXml_id
			) num
		...
		-- Протокол инструментальных исследований
			...
			FROM v_EvnXml x (nolock)
			INNER JOIN v_Evn evn (nolock) on evn.Evn_id = x.Evn_id
			INNER JOIN v_EvnUslugaPar e (nolock) on e.EvnUslugaPar_id = evn.Evn_id
			LEFT JOIN v_PersonState ps (nolock) on ps.Person_id = evn.Person_id
			LEFT JOIN v_LpuSection ls (nolock) on ls.LpuSection_id = e.LpuSection_uid
			{$join}
			outer apply (
		...
		-- Протокол консультации с применением телемедицинских технологий
			...
			FROM v_EvnXml x (nolock)
			INNER JOIN v_Evn evn (nolock) on evn.Evn_id = x.Evn_id
			INNER JOIN v_EvnUslugaTelemed e (nolock) on e.EvnUslugaTelemed_id = evn.Evn_id
			LEFT JOIN v_PersonState ps (nolock) on ps.Person_id = evn.Person_id
			LEFT JOIN v_LpuSection ls (nolock) on ls.LpuSection_id = e.LpuSection_uid
			{$join}
			outer apply (
		...
		-- КВС
			SELECT TOP 100
				...
				null as EvnXml_id
			FROM v_EvnPS e (nolock)
			INNER JOIN v_Evn evn (nolock) on evn.Evn_id = e.EvnPS_id
			LEFT JOIN v_PersonState ps (nolock) on ps.Person_id = evn.Person_id
			LEFT JOIN v_MedStaffFact msf (nolock) on msf.MedStaffFact_id = e.MedStaffFact_did
			LEFT JOIN v_LpuSection ls (nolock) on ls.LpuSection_id = msf.LpuSection_id
			{$join}
			outer apply (
				SELECT TOP 1 cast(EvnPS_NumCard as varchar) as Doc_Num FROM v_EvnPS (nolock) WHERE EvnPS_id = evn.Evn_id
			) num
			WHERE (1=1)
		...
		foreach ($docType as $scope) {
			...
			if (!empty($query)) {
				$query = str_replace("TOP 100", $merged_data, $query);
				$scopes[] = $query;
			}
		}
	...
	function searchDocs($data) {
		...
		if (!empty($data['MedPersonal_id'])) {
			$join = "
				LEFT JOIN v_pmUserCache pmuIns (nolock) on pmuIns.PMUser_id = evn.pmUser_insID
				LEFT JOIN v_pmUserCache pmuUpd (nolock) on pmuUpd.PMUser_id = evn.pmUser_updID
			";
		...
		if (!empty($data['Doc_Num'])) {
			$filter .= " and num.Doc_Num like :Doc_Num + '%'";
		}
		...
		if (!empty($scopes)) {
			...
			$query = "
				...
				SELECT TOP 100
					ISNULL(d.EvnXml_id, d.Evn_id) as EMDRegistry_ObjectID,
					case when d.EvnXml_id is not null then 'EvnXml' else cls.EvnClass_SysNick end as EMDRegistry_ObjectName,
					d.LpuSection_id,
					d.LpuBuilding_id,
					d.MedStaffFact_id,
					d.Doc_Num,
					d.EMDDocumentTypeLocal_id,
					d.EMDDocumentTypeLocal_Name,
					msf.Person_Fio as MedPersonal_Fio,
					evn.Lpu_id,
					evn.pmUser_insID,
					evn.pmUser_updID,
					(convert(varchar(10), evn.Evn_insDT, 104) + ' ' + convert(varchar(8), evn.Evn_insDT, 108)) as Evn_insDT,
					(convert(varchar(10), evn.Evn_updDT, 104) + ' ' + convert(varchar(8), evn.Evn_updDT, 108)) as Evn_updDT,
					d.IsSigned,
					evn.Person_id,
					ps.Person_id,
				...
				FROM doc d (nolock)
				INNER JOIN v_Evn evn (nolock) on evn.Evn_id = d.Evn_id
				LEFT JOIN v_EvnClass cls (nolock) on cls.EvnClass_id = evn.EvnClass_id
				LEFT JOIN v_PersonState ps (nolock) on ps.Person_id = evn.Person_id
				LEFT JOIN v_Evn parent (nolock) on parent.Evn_id = evn.Evn_pid
				LEFT JOIN v_MedStaffFact msf (nolock) on msf.MedStaffFact_id = d.MedStaffFact_id
				where (1=1)
					{$filter}
			";
	...
	function EMDSearch($data) {
		...
		$filter = ""; $join = ""; $joinLAteral = ""; $versions_join = "";
		...
		if (!empty($data['isWithoutRegistration'])) {

			$filter .= '
				AND version."EMDVersion_RegistrationDate" is null
			';
		}
		...
		if (!empty($data['Person_FIO'])) {
			...
			if (!empty($fullName[0])) {
				$p_filter .= " and ps.Person_SurName like '%' + :Person_Surname + '%' ";
				$search_params['Person_Surname'] = $fullName[0];
			}

			if (!empty($fullName[1])) {
				$p_filter .= " and ps.Person_FirName like '%' + :Person_Firname + '%' ";
				$search_params['Person_Firname'] = $fullName[1];
			}

			if (!empty($fullName[2])) {
				$p_filter .= " and ps.Person_SecName like '%' + :Person_Secname + '%' ";
				$search_params['Person_Secname'] = $fullName[2];
			}

			// ищем id по фио в БД промеда
			$query = "
				SELECT TOP 1000 ps.Person_id
		...
		if (!empty($data['EMDRegistry_Num'])) {
			$filter .= ' and emdr."EMDRegistry_Num" like :EMDRegistry_Num || \'%\' ';
		}
		...
		if (!empty($data['EMDRegistrationStatus_Code']) &&
			!empty($this->EMDRegistrationStatus[$data['EMDRegistrationStatus_Code']]))
			$filter .=
				' AND emdv."EMDVersion_IsReady" IN (' .
				implode(',', $this->EMDRegistrationStatus[$data['EMDRegistrationStatus_Code']]['EMDVersion_IsReady']) . ') ';

		$regStatusExpr = 'CASE';

		foreach ($this->EMDRegistrationStatus as $key => $value)
			$regStatusExpr .= ' WHEN version."EMDVersion_IsReady" IN (' . implode(',', $value['EMDVersion_IsReady']) . ') THEN ' . $key;

		$regStatusExpr .= ' END';


		if (empty($data['isGlobalAndActive']))
			$EMDDocumentTypeLocal_Name = 'dtl."EMDDocumentTypeLocal_Name",';
		else
			$EMDDocumentTypeLocal_Name = '
				CASE
						WHEN dtl."EMDDocumentTypeLocal_id" IN
							(SELECT dtl1."EMDDocumentTypeLocal_id"
								FROM "EMD"."EMDDocumentTypeLocal" dtl1
									INNER JOIN "EMD"."EMDDocumentType" dt1
										ON dt1."EMDDocumentType_id" = dtl1."EMDDocumentType_id"
								WHERE dt1."EMDDocumentType_endRegDT" IS NULL)
							THEN dtl."EMDDocumentTypeLocal_Name"
					END AS "EMDDocumentTypeLocal_Name",';
		...
		$query = '
			SELECT
				...
				emdr."pmUser_insID",
				case 
					when COALESCE(version."EMDVersion_IsReady", 0) > 1 then \'Да\'
					else \'Нет\'
				end as shipmentstatus,' .
				$regStatusExpr . ' AS "EMDRegistrationStatus_Code"
			...
			FROM
				...
				left join lateral (
					SELECT emdr."EMDRegistry_id",
               			emdv."EMDVersion_id",
               			emdv."EMDVersion_IsReady",
               			emdv."EMDVersion_RegistrationDate"
        			FROM "EMD"."EMDVersion" emdv
        			WHERE emdr."EMDRegistry_id" = emdv."EMDRegistry_id"
					ORDER BY emdv."EMDVersion_id" DESC
...
// 		echo(getDebugSQL($query, $data)); die();
		...
		if (!empty($result)) {
			...
			SELECT DISTINCT
			    -- select
			    count(emdr."EMDRegistry_id") total_count
				--emdr."EMDRegistry_id",
				--emdr."EMDRegistry_ObjectName",
				--emdr."EMDRegistry_ObjectID", 
				--emdr."EMDRegistry_Num",
				...
				--emdr."pmUser_insID",
				--case 
					--when emdjq."EMDJournalQuery_id" is not null then \'Да\'
					--else \'Нет\'
				--end as shipmentstatus,
				--case 
					--when emdjq."EMDJournalQuery_id" is not null and emdr."EMDRegistry_Num" is not null then \'Да\'
					--else \'Нет\'
			...
			FROM
				...
				--Смотрим журнал (EMDJournalQuery): если есть хотябы одна запись для данного документа (EMDRegistry_id) со временем больше, чем время создания последней версии - значить отправлялась.
				LEFT JOIN LATERAL (
					select 
						emdjq."EMDJournalQuery_id",
						emdjq."EMDJournalQuery_insDT"
					from "EMD"."EMDJournalQuery" emdjq 
					where emdjq."EMDRegistry_id" = emdr."EMDRegistry_id"
						and emdjq."EMDJournalQuery_insDT" > sign."EMDVersion_insDT"
					limit 1
				) emdjq on true
				-- end from
			WHERE
		...
		if (isset($resp_count)) {
//					$totalCount = count($resp_count);
					$totalCount = $resp_count[0]['total_count'];
				}
		}
			...
			if (count($signObjects) > 0) {
				foreach($signObjects as $oneSignObject) {
					...
					switch($docInfo['signObject']) {
						case 'Evn':
							$join .= " left join EvnRecept er (nolock) on er.Evn_id = s.Evn_id";
							$join .= " left join EvnPrescrMse epm (nolock) on epm.Evn_id = s.Evn_id";
							$join .= " left join EvnDirection ed (nolock) on ed.Evn_id = s.Evn_id";
							$join .= "
								left join v_MedStaffFact msf (nolock) on (
							...
							";
							$join .= " left join v_LpuSection ls (nolock) on ls.LpuSection_id = msf.LpuSection_id";
							$join .= " left join v_Evn r (nolock) on r.Evn_id = s.Evn_rid";
							$join .= " left join EvnPL epl (nolock) on epl.EvnPL_id = r.Evn_id";
							$join .= " left join EvnPS eps (nolock) on eps.EvnPS_id = r.Evn_id";
							$fields .= ", msf.MedPersonal_id";
							$fields .= ", msf.Person_Fio as MedPersonal_Fio";
							$fields .= ", ls.LpuSection_Name";
							$fields .= "
								, case
							...
							break;
						case 'EvnXml':
							$join .= " left join v_Evn e (nolock) on e.Evn_id = s.Evn_id";
							$join .= " left join EvnVizit ev (nolock) on ev.Evn_id = s.Evn_id";
							$join .= " left join EvnSection es (nolock) on es.Evn_id = s.Evn_id";
							$join .= " left join EvnUsluga eu (nolock) on eu.Evn_id = s.Evn_id";
							$join .= " left join v_MedStaffFact msf (nolock) on msf.MedStaffFact_id = coalesce(ev.MedStaffFact_id, es.MedStaffFact_id, eu.MedStaffFact_id)";
							$join .= " left join v_LpuSection ls (nolock) on ls.LpuSection_id = msf.LpuSection_id";
							$join .= " left join v_Evn r (nolock) on r.Evn_id = e.Evn_rid";
							$join .= " left join EvnPL epl (nolock) on epl.EvnPL_id = r.Evn_id";
							$join .= " left join EvnPS eps (nolock) on eps.EvnPS_id = r.Evn_id";
							$fields .= ", msf.MedPersonal_id";
							$fields .= ", msf.Person_Fio as MedPersonal_Fio";
					...
					$schemaName = (empty($docInfo['schemaName']) ? '' : $docInfo['schemaName'] . '.');
					...
					$query = "
						select
							...
							{$fields}
						from
							{$schemaName}{$docInfo['signObject']} s (nolock)
							{$join}
						where
...
// 					echo(getDebugSQL($query, $data));
						...
						foreach ($object_data as $o) {
							if (!empty($oneSignObject['ids'][$o['signObjectId']])) {
								foreach ($oneSignObject['ids'][$o['signObjectId']] as $emd_key) {
									...
									if(!empty($data['MedPersonal_id']) && !empty($o['MedPersonal_id']) && $data['MedPersonal_id'] == $o['MedPersonal_id']) $searchMedPersonal[] = $emd_key;
								}
							}
						}
					}
			...
			if(!empty($data['MedPersonal_id'])) {
				//отсавляем данные только по MedPersonal_id
				$docs = array_intersect_key($docs,  array_flip($searchMedPersonal));
			}
			if (count($persons) > 0) {
	...
	function generateEMDRegistry($data) {
		...
		if ($data['EMDRegistry_ObjectName'] == 'EvnXml') {
			...
			if ($data['EMDRegistry_ObjectName'] == 'EvnRecept') {
				$ReceptType_id = $this->getFirstResultFromQuery("select ReceptType_id from v_EvnRecept (nolock) where EvnRecept_id = :EvnRecept_id", ['EvnRecept_id' => $data['EMDRegistry_ObjectID']]);
				if ($ReceptType_id == 3) {
	...
	function saveEMDRegistry($data) {
		...
		if ($data['EMDRegistry_ObjectName'] == 'EvnXml') {
			...
			$query = "
					SELECT TOP 1
						...
						t.XmlTypeKind_id
					FROM v_EvnXml x (nolock)
					INNER JOIN v_Evn e (nolock) on e.Evn_id = x.Evn_id
					LEFT JOIN v_XmlTemplate t (nolock) on t.XmlTemplate_id = x.XmlTemplate_id
					WHERE (1=1)
			...
			$query = "
					SELECT TOP 1
						e.Evn_id,
						e.EvnClass_SysNick
					FROM v_EvnUslugaTelemed tele (nolock)
					INNER JOIN v_Evn e (nolock) on e.Evn_id = tele.EvnUslugaTelemed_pid
					WHERE (1=1)
		...
		if (empty($data['EvnClass_SysNick'])) {
			switch($data['EMDRegistry_ObjectName']) {
				...
				case 'BirthSvid':
					...
					$query = "
						...
						LEFT JOIN v_PersonState ps (nolock) on ps.Person_id = bs.Person_id
						LEFT JOIN v_LpuSection ls (nolock) on ls.LpuSection_id = bs.LpuSection_id
						LEFT JOIN fed.v_PassportToken lputid (nolock) on lputid.Lpu_id = ls.Lpu_id
						LEFT JOIN nsi.v_FRMOSection fs (nolock) on fs.FRMOSection_id = ls.FRMOSection_id
						LEFT JOIN v_LpuSectionProfile lsp (nolock) on lsp.LpuSectionProfile_id = ls.LpuSectionProfile_id
						LEFT JOIN v_LpuUnit lu (nolock) on lu.LpuUnit_id = ls.LpuUnit_id
						LEFT JOIN nsi.v_FRMOUnit fu (nolock) on fu.FRMOUnit_id = ISNULL(ls.FRMOUnit_id, lu.FRMOUnit_id)
				...
				case 'DeathSvid':
					...
					$query = "
						...
						LEFT JOIN v_PersonState ps (nolock) on ps.Person_id = ds.Person_id
						LEFT JOIN v_LpuSection ls (nolock) on ls.LpuSection_id = ds.LpuSection_id
						LEFT JOIN fed.v_PassportToken lputid (nolock) on lputid.Lpu_id = ls.Lpu_id
						LEFT JOIN nsi.v_FRMOSection fs (nolock) on fs.FRMOSection_id = ls.FRMOSection_id
						LEFT JOIN v_LpuSectionProfile lsp (nolock) on lsp.LpuSectionProfile_id = ls.LpuSectionProfile_id
						LEFT JOIN v_LpuUnit lu (nolock) on lu.LpuUnit_id = ls.LpuUnit_id
			...
			if ($data['EvnClass_SysNick'] === 'EvnPL') {
				...
				$join = "
					LEFT JOIN v_MedStaffFact msf (nolock) on msf.MedStaffFact_id = evn.MedStaffFact_did
					LEFT JOIN v_LpuSection ls (nolock) on ls.LpuSection_id = msf.LpuSection_id
					LEFT JOIN nsi.v_FRMOSection fs (nolock) on fs.FRMOSection_id = ls.FRMOSection_id
					LEFT JOIN v_LpuSectionProfile lsp (nolock) on lsp.LpuSectionProfile_id = ls.LpuSectionProfile_id
					LEFT JOIN v_LpuUnit lu (nolock) on lu.LpuUnit_id = ls.LpuUnit_id
					LEFT JOIN nsi.v_FRMOUnit fu (nolock) on fu.FRMOUnit_id = ISNULL(ls.FRMOUnit_id, lu.FRMOUnit_id)
					LEFT JOIN fed.v_PassportToken lpusid (nolock) on lpusid.Lpu_id = evn.Lpu_oid
					LEFT JOIN v_Diag d (nolock) on d.Diag_id = evn.Diag_id
					LEFT JOIN v_Diag zakd (nolock) on zakd.Diag_id = evn.Diag_lid
					LEFT JOIN v_ResultClass rc (nolock) on rc.ResultClass_id = evn.ResultClass_id
				";
				...
				$join = "
					LEFT JOIN v_LpuSection ls (nolock) on ls.LpuSection_id = evn.LpuSection_id
					LEFT JOIN nsi.v_FRMOSection fs (nolock) on fs.FRMOSection_id = ls.FRMOSection_id
					LEFT JOIN v_LpuSectionProfile lsp (nolock) on lsp.LpuSectionProfile_id = ls.LpuSectionProfile_id
					LEFT JOIN v_LpuUnit lu (nolock) on lu.LpuUnit_id = ls.LpuUnit_id
					LEFT JOIN nsi.v_FRMOUnit fu (nolock) on fu.FRMOUnit_id = ISNULL(ls.FRMOUnit_id, lu.FRMOUnit_id)
				...
				$join = "
					...
					LEFT JOIN v_LpuUnit lu (nolock) on lu.LpuUnit_id = ls.LpuUnit_id
					LEFT JOIN nsi.v_FRMOUnit fu (nolock) on fu.FRMOUnit_id = ISNULL(ls.FRMOUnit_id, lu.FRMOUnit_id)
					LEFT JOIN v_EvnPL parent (nolock) on parent.EvnPL_id = evn.EvnVizitPL_pid
					LEFT JOIN fed.v_PassportToken lpusid (nolock) on lpusid.Lpu_id = parent.Lpu_oid
					LEFT JOIN v_Diag d (nolock) on d.Diag_id = parent.Diag_id
					LEFT JOIN v_Diag zakd (nolock) on zakd.Diag_id = parent.Diag_lid
					LEFT JOIN v_ResultClass rc (nolock) on rc.ResultClass_id = parent.ResultClass_id
				";
				...
				$join = "
					...
					LEFT JOIN v_LpuUnit lu (nolock) on lu.LpuUnit_id = ls.LpuUnit_id
					LEFT JOIN nsi.v_FRMOUnit fu (nolock) on fu.FRMOUnit_id = ISNULL(ls.FRMOUnit_id, lu.FRMOUnit_id)
					LEFT JOIN v_EvnPLStom parent (nolock) on parent.EvnPLStom_id = evn.EvnVizitPLStom_pid
					LEFT JOIN fed.v_PassportToken lpusid (nolock) on lpusid.Lpu_id = parent.Lpu_oid
					LEFT JOIN v_Diag d (nolock) on d.Diag_id = parent.Diag_id
					LEFT JOIN v_Diag zakd (nolock) on zakd.Diag_id = parent.Diag_lid
					LEFT JOIN v_ResultClass rc (nolock) on rc.ResultClass_id = parent.ResultClass_id
				...
				$join = "
					...
					LEFT JOIN v_LpuUnit lu (nolock) on lu.LpuUnit_id = ls.LpuUnit_id
					LEFT JOIN nsi.v_FRMOUnit fu (nolock) on fu.FRMOUnit_id = ISNULL(ls.FRMOUnit_id, lu.FRMOUnit_id)
					LEFT JOIN fed.v_PassportToken lpusid (nolock) on lpusid.Lpu_id = evn.Lpu_id
					LEFT JOIN v_Diag d (nolock) on d.Diag_id = evn.Diag_id
					LEFT JOIN v_ResultClass rc (nolock) on rc.ResultClass_id = evn.ResultClass_id
				";
				...
				$join = "
					LEFT JOIN v_LpuSection ls (nolock) on ls.LpuSection_id = evn.LpuSection_uid
					LEFT JOIN nsi.v_FRMOSection fs (nolock) on fs.FRMOSection_id = ls.FRMOSection_id
					LEFT JOIN v_LpuSectionProfile lsp (nolock) on lsp.LpuSectionProfile_id = ls.LpuSectionProfile_id
					LEFT JOIN v_LpuUnit lu (nolock) on lu.LpuUnit_id = ls.LpuUnit_id
					LEFT JOIN nsi.v_FRMOUnit fu (nolock) on fu.FRMOUnit_id = ISNULL(ls.FRMOUnit_id, lu.FRMOUnit_id)
					LEFT JOIN fed.v_PassportToken lpusid (nolock) on lpusid.Lpu_id = evn.Lpu_uid
				";
			...
			elseif ($data['EvnClass_SysNick'] === 'EvnMse') {
				...
				$join = "
					LEFT JOIN v_MedService ms (nolock) on ms.MedService_id = evn.MedService_id
					LEFT JOIN v_LpuSection ls (nolock) on ls.LpuSection_id = ms.LpuSection_id
					LEFT JOIN nsi.v_FRMOSection fs (nolock) on fs.FRMOSection_id = ls.FRMOSection_id
					LEFT JOIN v_LpuSectionProfile lsp (nolock) on lsp.LpuSectionProfile_id = ls.LpuSectionProfile_id
					LEFT JOIN v_LpuUnit lu (nolock) on lu.LpuUnit_id = ls.LpuUnit_id
				...
				$select ="";
				$join="";
			} elseif ($data['EvnClass_SysNick'] === 'EvnDirectionHTM') {
				...
				$select = ",
					...
					{$LpuBuilding_frmo} as LpuBuilding_frmo,
					lu.LpuUnit_Name,
					evn.EvnDirectionHTM_id as docNum
				";
				...
				$join = "
					...
					LEFT JOIN v_LpuUnit lu (nolock) on lu.LpuUnit_id = ls.LpuUnit_id
					LEFT JOIN nsi.v_FRMOUnit fu (nolock) on fu.FRMOUnit_id = ISNULL(ls.FRMOUnit_id, lu.FRMOUnit_id)
					LEFT JOIN fed.v_PassportToken lpusid (nolock) on lpusid.Lpu_id = evn.Lpu_did
				";
			...
			} elseif ($data['EvnClass_SysNick'] === 'EvnUslugaTelemed') {
				...
				$select = ",
					...
					{$LpuBuilding_frmo} as LpuBuilding_frmo,
					lu.LpuUnit_Name,
					evn.EvnUslugaTelemed_id as docNum
				";

				$join = "
					LEFT JOIN v_MedStaffFact msf (nolock) on msf.MedStaffFact_id = evn.MedStaffFact_id
					LEFT JOIN v_LpuSection ls (nolock) on ls.LpuSection_id = msf.LpuSection_id
					LEFT JOIN nsi.v_FRMOSection fs (nolock) on fs.FRMOSection_id = ls.FRMOSection_id
					LEFT JOIN v_LpuSectionProfile lsp (nolock) on lsp.LpuSectionProfile_id = ls.LpuSectionProfile_id
					LEFT JOIN v_LpuUnit lu (nolock) on lu.LpuUnit_id = ls.LpuUnit_id
			...
			$query = "
				...
				SELECT TOP 1
					...
					{$select}
				FROM {$from}
				left join v_Evn e (nolock) on e.Evn_id = evn.{$data['EvnClass_SysNick']}_id
				left join v_PersonState ps (nolock) on ps.Person_id = evn.Person_id
		...
		if (!empty($doc_data)) {
			...
			if (empty($doc_data['PersonCard_Code']) && !empty($doc_data['LpuSectionProfile_fedid'])) {
				...
				$LpuAttachType_id = 1;
				$select = " coalesce(curr.PersonCard_Code,alt.PersonCard_Code) PersonCard_Code ";
				$alter = "
						left join v_PersonCard alt (nolock) on alt.Person_id = :Person_id and alt.LpuAttachType_id = 4
					";

				// посещение в гинекологии
				if (in_array($doc_data['LpuSectionProfile_fedid'], array(138, 139))) {
					$LpuAttachType_id = 2;
					$select = " coalesce(curr.PersonCard_Code,gen.PersonCard_Code,alt.PersonCard_Code) PersonCard_Code ";
					$alter = "
							left join v_PersonCard gen (nolock) on gen.Person_id = :Person_id and gen.LpuAttachType_id = 1
							left join v_PersonCard alt (nolock) on alt.Person_id = :Person_id and alt.LpuAttachType_id = 4
						";
				}
				...
				if (in_array($doc_data['LpuSectionProfile_fedid'], array(74, 75))) {
					$LpuAttachType_id = 4;
					$select = " coalesce(curr.PersonCard_Code,gen.PersonCard_Code) PersonCard_Code ";
					$alter = "
							left join v_PersonCard gen (nolock) on gen.Person_id = :Person_id and gen.LpuAttachType_id = 1
						";
				}
	...
	function exportSignature($data) {
		...
		if (file_exists($resp[0]['EMDVersion_FilePath'])) {
			$file_path = $resp[0]['EMDVersion_FilePath'];
			$basename = basename($file_path);
			$zip_file_path = mb_substr($file_path, 0, -4) . '.zip';

			if (!file_exists($zip_file_path)) {
				$signfile_path = $file_path . '.p7s';
				$certfile_path = $file_path . '.cer';
				file_put_contents($signfile_path, $resp[0]['Signatures_SignedData']);
				file_put_contents($certfile_path, $resp[0]['EMDCertificate_OpenKey']);
				$zip = new ZipArchive();
				$zip->open($zip_file_path, ZIPARCHIVE::CREATE);
				$zip->AddFile($file_path, $basename);
				$zip->AddFile($signfile_path, $basename . '.p7s');
				$zip->AddFile($certfile_path, $basename . '.cer');
				$zip->close();
				@unlink($signfile_path);
				@unlink($certfile_path);
			}
			...
			header("Content-Disposition: attachment; filename=\"sign.zip\"");
			header("Content-Transfer-Encoding: binary");


			@readfile($zip_file_path);
		} else {
			DieWithError('Не найден файл версии, экспорт невозможен');
		}
	}
		...
		if (!empty($data['isMOSign'])) {
			...
			if (!empty($resp_al[0]['ApprovalList_id'])) {
				...
				if (empty($role[0]['EMDPersonRole_id'])) {
					...
					$Person_Fin = $this->getFirstResultFromQuery("select top 1 Person_Fin from v_MedPersonal (nolock) where MedPersonal_id = :MedPersonal_id", [
						'MedPersonal_id' => $data['session']['medpersonal_id'] ?? null
					]);
					...
					$MedStaffFactIds = [];
					$resp_msf = $this->queryResult("select MedStaffFact_id from v_MedStaffFact (nolock) where MedPersonal_id = :MedPersonal_id", [
						'MedPersonal_id' => $data['session']['medpersonal_id'] ?? null
					]);
			...
			if (false && !empty($rules)) { // временно отключаем проверку должности
				$resp_msf = $this->queryResult("
					...
					from
						...
						inner join nsi.MedPost mp with (nolock) on mp.MedPost_id = p.MedPost_id
					where
		...
		if (!empty($resp[0]['Signatures_id'])) {
			...
			if (empty($data['isMOSign'])) {
				...
				$MedPersonal_data = $this->getFirstRowFromQuery("
					...
					from
						v_MedStaffFact msf (nolock)
						left join persis.Post Post with (nolock) on msf.Post_id = Post.id
						left join persis.FRMPPost frmpp with (nolock) on frmpp.id = Post.frmpEntry_id
						left join persis.Speciality spec with (nolock) on spec.id = Post.Speciality_id
						left join persis.FRMRNomenSpeciality frmns with (nolock) on frmns.FRMRNomenSpeciality_id = spec.FRMRNomenSpeciality_id
					where
						msf.MedStaffFact_id = :MedStaffFact_id
				", array(
					'MedStaffFact_id' => $data['MedStaffFact_id']
		...
		switch ($data['EMDRegistry_ObjectName']) {
			case 'EvnUslugaPar':
				$resp_elr = $this->queryResult("
					...
					from
						...
						inner join v_EvnLabRequest elr (nolock) on elr.EvnDirection_id = eup.EvnDirection_id
					where
			...
			case 'EvnXml':
				$resp_efr = $this->queryResult("
					...
					from
						v_EvnXml ex (nolock)
						inner join v_EvnUslugaPar eup (nolock) on eup.EvnUslugaPar_id = ex.Evn_id
		...
		if ($data['EMDRegistry_ObjectName'] == 'EvnXml') {
			$resp = $this->queryResult("
				...
				FROM
					...
					left join v_XmlTemplate xt (nolock) on xt.XmlTemplate_id = ex.XmlTemplate_id
				WHERE
	...
	function getEMDSignWindowGrid($data) {
		...
		switch($data['EMDRegistry_ObjectName']) {
			case 'ReportRun':
				$db = $this->load->database('reports', true); // БД для отчётов
				$fields .= ', r.Report_Caption';
				$fields .= ', t.Report_Params';
				$joins .= ' left join rpt.Report r (nolock) on r.Report_id = t.Report_id';
				break;
			case 'EvnRecept':
				$fields .= ', t.EvnRecept_Ser';
				$fields .= ', t.EvnRecept_IsKEK';
				$fields .= ', t.CauseVK_id';
				$fields .= ', t.PrescrSpecCause_id';
				break;
			case 'DeathSvid':
				$fields = ", 
					...
					as MissingDataList";
				$joins = " left join v_PersonState psr with (nolock) on psr.Person_id = t.Person_rid
		...
		$result = $db->query("
			select
				...
				convert(varchar(10), t.{$doc['dateField']}, 104) as docDate,
			...
			from
				{$view} t
				left join {$table} s (nolock) on s.{$doc['signObject']}_id = t.{$doc['idField']}
				...
				left join v_pmUser pu (nolock) on pu.pmUser_id = :pmUser_id
				{$joins}
			where
	...
	function loadEMDSignWindow($data) {
		...
		switch ($data['EMDRegistry_ObjectName']) {
			...
			case 'EvnVK':
				...
				break;
			case 'EvnPrescrMse':
				/* --#190847 перенос проверки в сохранение самой формы
	...
	*/
	public function getEMDSignDocNextVersion($data)
	{
		...
		select
			...
			COALESCE((select MAX("EMDRegistry_id") from "EMD"."EMDVersion" ) + 1, 1) as "EMDRegistry_id"
				from
					"EMD"."EMDVersion" e2
					inner join "EMD"."EMDRegistry" emdr on e2."EMDRegistry_id" = emdr."EMDRegistry_id"
						where
	...
	function loadEMDVersionList($data) {
		$resp = $this->queryResult('
			...
			where
				...
				and emdr."EMDRegistry_ObjectID" = :EMDRegistry_ObjectID
			order by
				emdv."EMDVersion_VersionNum" desc
		', array(
			'EMDRegistry_ObjectName' => $data['EMDRegistry_ObjectName'],
			'EMDRegistry_ObjectID' => $data['EMDRegistry_ObjectID']
		), $this->emddb);

		if (!empty($resp[0]['EMDVersion_id'])) {
			// проверяем актуальность подписи (поле IsSigned) и если она актуальна, то помечаем последнюю версию актуальной
			$doc = $this->documents[$data['EMDRegistry_ObjectName']];
			if (!empty($doc['signObject'])) {
				$scheme="";
				if($doc['signObject']=='ReportRun'){
					$scheme='rpt.';
				}
				$resp_check = $this->queryResult("
					select top 1
						{$doc['signField']} as IsSigned
					from
						{$scheme}{$doc['signObject']}
					where
						{$doc['signObject']}_id = :EMDRegistry_ObjectID
				", [
					'EMDRegistry_ObjectID' => $data['EMDRegistry_ObjectID']
				...
				if (!empty($resp_check[0]['IsSigned']) && $resp_check[0]['IsSigned'] == 2) {
					$resp[0]['EMDVersion_IsActual'] = 2;
				}
			}
		}
	...
	function getEMDDocumentListByEvn($data) {
		...
		if (in_array($data['EvnClass_SysNick'], array('EvnPL', 'EvnPLStom', 'EvnVizitPL', 'EvnVizitPLStom'))) {
			...
			$EvnList = $this->queryResult("
				...
				left join v_EvnDirection_all ed (nolock) on ed.EvnDirection_id = evn.Evn_id
				left join v_EvnRecept er (nolock) on er.EvnRecept_id = evn.Evn_id
				where
					{$evn_query_filter}
			...
			$XmlList = $this->queryResult("
				select
					...
					xml.EvnXml_Name,
					xml.XmlType_id
				from v_EvnXml xml (nolock)
				inner join v_Evn parent (nolock) on parent.Evn_id = xml.Evn_id
				where
					{$xml_query_filter}
	...
	function checkSignedEvnContent($data) {
		...
		$EvnList = $this->queryResult("
			select
				...
				evn.Evn_pid,
				(convert(varchar(10), evn.Evn_updDT, 104) + ' ' + convert(varchar(8), evn.Evn_updDT, 108)) as actualDate,
				parent.EvnClass_Name as parent_EvnClass_Name,
				(convert(varchar(10), parent.Evn_setDate, 104)) as parent_Evn_setDate,
				isnull(ed.EvnDirection_Num, er.EvnRecept_Num) as docNum
			from v_Evn evn (nolock)
			inner join v_Evn parent (nolock) on parent.Evn_id = evn.Evn_pid
			left join v_EvnDirection_all ed (nolock) on ed.EvnDirection_id = evn.Evn_id
			left join v_EvnRecept er (nolock) on er.EvnRecept_id = evn.Evn_id
		...
		$XmlList = $this->queryResult("
			select
				...
				(convert(varchar(10), xml.EvnXml_updDT, 104) + ' ' + convert(varchar(8), xml.EvnXml_updDT, 108)) as actualDate,
				parent.EvnClass_Name as parent_EvnClass_Name,
				(convert(varchar(10), parent.Evn_setDate, 104)) as parent_Evn_setDate
			from v_EvnXml xml (nolock)
			inner join v_Evn parent (nolock) on parent.Evn_id = xml.Evn_id
	 ...
	 * @param array $data
	 * @return array
	 * @description Запрос списка ЭМД пациента
	 */
	public function searchRegistryItem(array $data):array {
		$serviceUrl = $this->config->item('SWANRemdServiceUrl');
		if (empty($serviceUrl)) {
			return ['Error_Msg' => 'Не указан адрес API сервиса SWAN REMD (SWANRemdApiUrl)'];
		}

		$params = [
			'patientSnils' => $data['Person_Snils']
		];

		if (!empty($data['EMDDocumentType_id'])) {
			$resp_edt = $this->queryResult('
				select
					"EMDDocumentType_OID"
				from
					"EMD"."EMDDocumentType"
				where
					"EMDDocumentType_id" = :EMDDocumentType_id
			', [
				'EMDDocumentType_id' => $data['EMDDocumentType_id']
			], $this->emddb);

			if (!empty($resp_edt[0]['EMDDocumentType_OID'])) {
				$params['kind'] = $resp_edt[0]['EMDDocumentType_OID'];
			}
		}

		$url = $serviceUrl . '/api/searchRegistryItem?' . http_build_query($params);
		...
		return ['Error_Msg' => 'Ошибка взаимодействия с сервисом РЭМД: ' . var_export($result, true)];
	}

	/**
	 * @param array $data
	 * @return array
	 * @description Запрос файла ЭМД
	 */
	public function demandContent(array $data):array {
		$serviceUrl = $this->config->item('SWANRemdServiceUrl');
		if (empty($serviceUrl)) {
			return ['Error_Msg' => 'Не указан адрес API сервиса SWAN REMD (SWANRemdApiUrl)'];
		}
		...
		$params = [
			'emdrId' => $data['EMDOuterRegistry_emdrId']
		];

		$url = $serviceUrl . '/api/demandContent?' . http_build_query($params);
	 ...
	 */
	function getEmdSignatureRules($data) {
