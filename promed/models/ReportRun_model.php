...
class ReportRun_model extends swModel
{
	/**
	...
	private function getReportUrl($relative = false, $fromQueue = false) {
		...
		$query = "
			select
				...
				ReportCatalog_Path,
				Report_FileName
			from rpt.Report Report with(nolock)
			join rpt.ReportCatalog ReportCatalog with(nolock) on
	...
	function updateReportRun(){
		$query = "
			update rpt.ReportRun with (rowlock)
			set
	...
	function InsertNewReportRun()
	{
		...
		$query = "
			declare
				...
				@ReportRun_id bigint,
				@gdt datetime = ".(($this->isqueue)?"dbo.tzGetDate()":"null").";
			exec rpt.p_ReportRun_ins
				@ReportRun_id = @ReportRun_id output,
				@ReportRun_Code = :ReportRun_Code,
				@Report_id = :Report_id,
				@Report_Params = :Report_Params,
				@pmUser_id = :pmUser_id,
				@ReportRun_queueDT = @gdt,
			...
			select @ReportRun_id as ReportRun_id, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
		";
		$result = $this->db->query(
			$query,
			array(
	...
	function CheckIfUserCanRunReport($Report_id, $pmUser_id)
	{
		...
		$query = "
			select
				...
				ReportRun_begDT,
				ReportRun_Code
			from rpt.ReportRun with (nolock)
		...
		if ( is_object($result) ) {
			...
			if ( isset($res[0]) ) {
				...
				Формирование отчёта началось: ".$res[0]['ReportRun_begDT']->format("d.m.Y H:i")."<br/>
	...
	function CheckIfReportInCache($Report_id, $pmUser_id, $Report_Params)
	{
		...
		$query = "
			...
			where pmUser_insID = :pmUser_id
				...
				and dbo.tzGetDate() < dateadd(n, :ReportCacheTime, ReportRun_endDT)
	...
	function GetWaitingProcessCount($DatabaseType = 2)
	{
		...
		$query = "
			...
			where s.is_user_process = 1 and r.total_elapsed_time > 1000 and program_name like '%JDBC Driver%'";
	...
	function loadReportGrid($data) {
		...
		if (isset($data['filterReportStatus'])) {
			...
			$filter .= " and ISNULL(rr.ReportRun_Status, 0) = :ReportRun_Status";
		}
		if (isset($data['filterReportSign'])) {
			$params['ReportRun_IsSigned'] = $data['filterReportSign'];
			$filter .= " and ISNULL(rr.ReportRun_IsSigned, 1) = :ReportRun_IsSigned";
		}
		...
		if (isset($data['filterReportName'])) {
			...
			$filter .= " and r.Report_Title like :Report_Title";
		}
		...
		$query = "
			...
			-- select
				ReportRun_id,
					...
					rr.pmUser_insID,
					pmuser.pmUser_name,
					convert(varchar(10), cast(ReportRun_queueDT as datetime), 104) + ' ' + convert(varchar(5), cast(ReportRun_queueDT as datetime), 108) as ReportRun_queueDT,
					convert(varchar(10), cast(ReportRun_begDT as datetime), 104) + ' ' + convert(varchar(5), cast(ReportRun_begDT as datetime), 108) as ReportRun_begDT,
					convert(varchar(10), cast(ReportRun_endDT as datetime), 104) + ' ' + convert(varchar(5), cast(ReportRun_endDT as datetime), 108) as ReportRun_endDT,
					ReportRun_Status,
					case
						when ReportRun_Status is null then 'Не сформирован'
						when ReportRun_Status = '' then 'Не сформирован'
					...
					rr.ReportRun_IsSigned,
					convert(varchar(10), rr.ReportRun_signDT, 104) as ReportRun_signDT
				-- end select
			from
			-- from
				rpt.ReportRun rr with (nolock)
				left join rpt.Report r with (nolock) on rr.Report_id = r.Report_id
				left join v_pmUser pmuser with (nolock) on pmuser.pmUser_id = rr.pmUser_insID
				left join v_pmUser pmusers with (nolock) on pmusers.pmUser_id = rr.pmUser_signID
			--end from
			where
			-- where
				{$filter}
	...
	function getReportQueue($data = array()) {
		...
		$query = "
			select top 1
				...
				ReportRun_FilePath,
				ReportRun_Format
			from rpt.ReportRun rr with (nolock)
			left join rpt.Report r with (nolock) on rr.Report_id = r.Report_id
			left join v_pmUser pmuser with (nolock) on pmuser.pmUser_id = rr.pmUser_insID
	...
	function checkReportQueue($data = array()) {
		$query = "
			select top 1 convert(varchar(10), cast(ReportRun_queueDT as datetime), 104) + ' ' + convert(varchar(5), cast(ReportRun_queueDT as datetime), 108) as queueDT
			from rpt.ReportRun rr with (nolock)
			left join rpt.Report r with (nolock) on rr.Report_id = r.Report_id
			left join v_pmUser pmuser with (nolock) on pmuser.pmUser_id = rr.pmUser_insID
	...
	*/
	...
	function CheckReportForLong($Report_id) {
		...
		$query = "
			...
			from rpt.v_ReportLong with(nolock)
	...
	*/
	...
	public function getDatabaseTypeByReport($Report_id = null) {
		...
		$DatabaseType = $this->model->getFirstResultFromQuery('select top 1 DatabaseType from rpt.Report with (nolock) where Report_id = :Report_id', array('Report_id' => $Report_id));
	...
	function getReportForSign($data) {
		...
		$resp = $this->queryResult("select * from rpt.ReportRun where ReportRun_id = :ReportRun_id", array(
