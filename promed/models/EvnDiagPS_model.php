	...
	protected function _validate()
	{
		...
		if (in_array($this->scenario, array(self::SCENARIO_DO_SAVE, self::SCENARIO_AUTO_CREATE))) {
				...
				switch ($this->DiagSetType_id) {
					case 1:  // Направившего учреждения
						$osn_diag_id = $this->getFirstResultFromQuery("
							select Diag_did from v_EvnPS (nolock) where EvnPS_id = ?
						", [$this->pid]);
						break;
					case 2: // Предварительный (Приемного)
						$osn_diag_id = $this->getFirstResultFromQuery("
							select Diag_pid from v_EvnPS (nolock) where EvnPS_id = ?
						", [$this->pid]);
						break;
					case 3: // Клинический (основной в движении)
						$osn_diag_id = $this->getFirstResultFromQuery("
							select Diag_id from v_EvnSection (nolock) where EvnSection_id = ?
						", [$this->pid]);
						break;
				}
			...
			$result = $this->getFirstResultFromQuery('
				select count(ED.EvnDiagPS_id) as rec
				from v_EvnDiagPS ED with (nolock)
				where (1 = 1)
					and ED.Lpu_id = :Lpu_id -- в рамках ЛПУ
					and ED.EvnDiagPS_pid = :EvnDiagPS_pid -- в рамках КВС
					and ED.EvnDiagPS_id <> ISNULL(:EvnDiagPS_id, 0) -- исключая текущий диагноз
					and ED.Diag_id = ISNULL(:Diag_id, 0) -- там диагноз
