 ...
 *
 */
class MorbusOnkoTumorStatus_model extends swModel
{
	/**
	 * Пользователь
	 * @var integer
	...
	public function read()
	{
		...
		$sql = "
			SELECT
				...
				,MOTS.MorbusOnkoBasePersonState_id
				,MOTS.OnkoTumorStatus_Num as MorbusOnkoTumorStatus_NumTumor
				,MOTS.Diag_id
				,MOTS.OnkoTumorStatusType_id
			FROM
				v_OnkoTumorStatus MOTS with (nolock)
			WHERE
				OnkoTumorStatus_id = ?
		";
	...
	public function save($data = array())
	{
		...
		$sql = "
			declare
				...
				@ErrCode int,
				@ErrMsg varchar(4000);
			set @Res = :MorbusOnkoTumorStatus_id;
			exec {$procedure}
				@OnkoTumorStatus_id = @Res output,
			...
			select @Res as MorbusOnkoTumorStatus_id, @ErrCode as Error_Code, @ErrMsg as Error_Msg;
		";
		$params = array(
	...
	public function destroyList($MorbusOnkoBasePersonState_id)
	{
		$query = "
			...
			FROM v_OnkoTumorStatus with (nolock)
		...
		$query = '
			...
			exec dbo.p_OnkoTumorStatus_del
	...
	private function loadTumorList()
	{
		$query = "
			SELECT
				...
				ISNULL(MO.OnkoTumorStatusType_id, 10) as OnkoTumorStatusType_id
			FROM v_MorbusOnkoBase MOB with (nolock)
				inner join v_Morbus M with (nolock) on MOB.MorbusBase_id = M.MorbusBase_id
				inner join v_MorbusOnko MO with (nolock) on M.Morbus_id = MO.Morbus_id
			WHERE
				MOB.MorbusOnkoBase_id = :MorbusOnkoBase_id
				and MO.MorbusOnko_setDiagDT <= CAST (:MorbusOnkoBasePersonState_setDT as datetime)
		";
		$params = array(
	...
	function readList($data = array())
	{
		...
		$query = "
			...
			FROM v_OnkoTumorStatus MOTS with (nolock)
				inner join v_Diag Diag with (nolock) on MOTS.Diag_id = Diag.Diag_id
				left join v_OnkoTumorStatusType OTST with (nolock) on MOTS.OnkoTumorStatusType_id = OTST.OnkoTumorStatusType_id
