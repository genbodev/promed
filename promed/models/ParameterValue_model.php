 ...
 */
class ParameterValue_model extends swModel
{
	...
	static function defAttributes()
	{
		...
		$arr['parametervaluelisttype_name'] = array(
			...
			'select' => 't.ParameterValueListType_Name',
			'join' => 'left join v_ParameterValueListType t with (nolock) on t.ParameterValueListType_id = {ViewName}.ParameterValueListType_id',
		);
	...
	private function _getLoadQuery($add_select = '', $add_join = '', $filters = '', $params = array())
	{
		...
		if (!empty($this->alias)) {
			$filters .= "
				and v_ParameterValue.ParameterValue_Alias like :alias";
			$query['params']['alias'] = '%' . $this->alias . '%';
		}
		if (!empty($this->name)) {
			$filters .= "
				and v_ParameterValue.ParameterValue_Name like :name";
		...
		$query['sql'] = "
			Select
				-- select
				v_ParameterValue.ParameterValue_id {$add_select}
				-- end select
			from
				-- from
				v_ParameterValue with (NOLOCK) {$add_join}
				-- end from
			where
				-- where
	...
	function doLoadGrid($data)
	{
		...
		$query = $this->_getLoadQuery(",
				...
				v_ParameterValue.ParameterValue_Alias,
				v_ParameterValue.ParameterValue_Name,
				ListType.ParameterValueListType_Name,
				(select COUNT(val.ParameterValue_id) from v_ParameterValue val with (NOLOCK) where val.ParameterValue_pid = v_ParameterValue.ParameterValue_id) as ParameterValue_valueCnt,
				v_ParameterValue.ParameterValueListType_id,
				v_ParameterValue.ParameterValue_SysNick
		", '
				left join v_ParameterValueListType ListType with (NOLOCK) on ListType.ParameterValueListType_id = v_ParameterValue.ParameterValueListType_id');
		/*
	...
	public function getParameterFieldData($markers)
	{
		...
		$query = '
			select
				...
				,Parameter.ParameterValueListType_id
				,Value.ParameterValue_id as Value_id
				,Value.ParameterValue_Name as Value_Name
			from 
				v_ParameterValue Parameter with (NOLOCK)
				inner join v_ParameterValue Value with (NOLOCK) on Value.ParameterValue_pid = Parameter.ParameterValue_id
			where
	...
	function getTypeList() {
		$query = "
			select
				PVLT.ParameterValueListType_id,
				PVLT.ParameterValueListType_Code,
				PVLT.ParameterValueListType_Name,
				PVLT.ParameterValueListType_SysNick
			from v_ParameterValueListType PVLT with(nolock)
		";
