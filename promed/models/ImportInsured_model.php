 ...
 */
class ImportInsured_model extends swModel {
	...
	private function importNP($data) {
		...
		$query = 'select top 1 OrgSMO_Name from v_OrgSMO with(nolock) where Orgsmo_f002smocod =  :SMOCode';
		$params = array("SMOCode"=>$this->SMOCode);
		$res = $this->db->query($query, $params);
		$SMORes = $res->result('array');
		$query = 'select top 1  Org_Nick from v_Lpu_all with(nolock) where Lpu_f003mcod = :MOCode';
		...
		if (in_array($data['CASE_VID'], array(1, 2))) {
			...
			*/
			$query = "select
						...
						isnull(pol.Server_id,1) as Server_id,
						pt.PolisType_CodeF008 as VPOLIS,
						p.Person_id,
						CONVERT(varchar(10),p.Person_deadDT,120) as Person_DeadDT
						...
						and REPLACE(p.Person_Firname,'ё','е') = REPLACE(:Person_Firname,'ё','е')
						and isnull(REPLACE(p.Person_Secname,'ё','е'),'')=isnull(REPLACE(:Person_Secname,'ё','е'),'')
						and convert(date,p.Person_Birthday) = convert(date,:Person_Birthday)";
			$params = array(
				...
				if (count($res) == 1) {
					...
					if (
						...
						$sql = "
							...
							declare @SMO_id bigint
							declare @PersonPolis_insDT datetime

							set @PersonPolis_insDT = dateadd(s,1,:DATE_BEGIN)
							set @SMO_id = (
								select top 1 smo.OrgSMO_id
								from v_OrgSMO smo (nolock)
					...
					if ( count($AdressReg) > 0 ) {
						$sel = $this->getFirstRowFromQuery("
							select top 1
								p.PersonEvn_id,
								p.Server_id,
								ISNULL(Adr.KLRgn_id, 0) as KLRgn_id,
								ISNULL(Adr.KLSubRgn_id, 0) as KLSubRgn_id,
								ISNULL(Adr.KLCity_id, 0) as KLCity_id,
								ISNULL(Adr.KLTown_id, 0) as KLTown_id,
								ISNULL(Adr.KLStreet_id, 0) as KLStreet_id,
								ISNULL(Adr.Address_House, '') as Address_House,
								ISNULL(Adr.Address_Corpus, '') as Address_Corpus,
								ISNULL(Adr.Address_Flat, '') as Address_Flat
							from
								v_Person_all p with (nolock)
								left join v_Address Adr with(nolock) on Adr.Address_id = p.UAddress_id
							where
					...
					if ( count($AdressProg) > 0 ) {
						$sel = $this->getFirstRowFromQuery("
							...
							from
								v_Person_all p with (nolock)
								left join v_Address Adr with(nolock) on Adr.Address_id = p.PAddress_id
							where
...
;";
	...
	private function LpuAttach($data) {
		$Lpu_id = $this->getFirstResultFromQuery("
			select top 1 Lpu_id from v_Lpu with(nolock) where Lpu_f003mcod = :CODE_MO
		", array(
		...
		$sql = "
			declare @currTime datetime =  dbo.tzGetDate();
			SELECT
				count(*) as cnt
			FROM
			...
			WHERE
				Person_id = :ID_PAC
				and isnull(PersonCard_endDate,@currTime)>=@currTime and LpuAttachType_id = 1
		";
		//Получим участок по адресу человека
		$lpuRegion_id = null;
		$query_lpuregion = "
			declare @Lpu_id bigint = :Lpu_id;
			declare @PAddress_id bigint;
			declare @UAddress_id bigint;
			declare @LpuRegionType int;
			declare @Birthday datetime;
			set @Birthday = :Person_Birthday
			set @PAddress_id = (
							select PAddress_id
							from v_PersonState
			...
			set @LpuRegionType = (
							case when datediff(YEAR, @Birthday,dbo.tzgetDate()) < 18 then 2
							else 1 end
						)
			...
			cross apply (
				select top 1
					...
					a.Address_Corpus
				from dbo.Address a
				where a.Address_id in(@PAddress_id,@UAddress_id)
			) Addr
			inner join v_LpuRegion LR on (LR.LpuRegion_id = LRS.LpuRegion_id and LR.Lpu_id = @Lpu_id)
			where dbo.GetHouse(isnull(LRS.LpuRegionStreet_HouseSet,'  '),(LTRIM(RTRIM(Addr.Address_House))+(CASE WHEN Addr.Address_Corpus IS NOT NULL THEN '/'+RTRIM(LTRIM(Addr.Address_Corpus)) ELSE '' end))) = 1
			and LRS.KLStreet_id = Addr.KLStreet_id
			and LR.LpuRegionType_id = @LpuRegionType
		";
		...
		if ($result[0]['cnt'] == 0) {
			...
			do{
				...
				from v_PersonAmbulatCard with(nolock)
				where 
					ISNUMERIC(PersonAmbulatCard_Num) = 1
					and Lpu_id = :Lpu_id
