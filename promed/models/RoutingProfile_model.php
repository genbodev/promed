...
* @version      11.2019
*/
class RoutingProfile_model extends swModel {
	...
	public function loadProfileList($params) {
		...
		$query = "select
				rp.RoutingProfile_id,
				rp.RoutingProfile_name,
				rp.RoutingProfile_sysnick,
				rp.MorbusType_id,
				rp.Region_id
			from v_RoutingProfile (nolock) rp
			where
				(rp.RoutingProfile_endDate is null or FLOOR(CAST(rp.RoutingProfile_endDate as float)) > FLOOR(CAST(GETDATE() as float)))
				{$whereClause}";
		try {
