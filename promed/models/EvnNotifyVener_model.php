...
class EvnNotifyVener_model extends swModel
{
	...
	function checkEvnNotifyVener($data)
	{
		...
		return swMorbus::getStaticMorbusCommon()->checkExistsExtended('vener', $data['Person_id'], $data['Diag_id'],"
				,isnull(PR.EvnNotifyBase_id, EN.EvnNotifyVener_id) as EvnNotifyBase_id
	...
	function load($data)
	{
		$query = '
			select
				...
				convert(varchar,ENO.EvnNotifyVener_setDT,104) as EvnNotifyVener_setDT,
				ENO.MedPersonal_id,
				ENO.PersonCategoryType_id,
				ENO.EvnNotifyVener_OtherPersonCategory,
				convert(varchar,ENO.EvnNotifyVener_DiagDT,104) as EvnNotifyVener_DiagDT,
				ENO.EvnNotifyVener_IsReInfect,
				ENO.VenerPathTransType_id,
				ENO.VenerPregPeriodType_id,
				ENO.VenerLabConfirmType_id,
				ENO.EvnNotifyVener_OtherLabConfirm,
				ENO.VenerDetectionPlaceType_id,
				ENO.LpuSectionProfile_id,
				ENO.MedPersonal_pid,
				ENO.EvnNotifyVener_OtherDetectPlace,
				ENO.VenerDetectionFactType_id,
				ENO.MedPersonal_fid,
				ENO.EvnNotifyVener_OtherDetectFact,
				isnull(EvnNotifyVener_IsIFA, 0) as EvnNotifyVener_IsIFA,
				isnull(EvnNotifyVener_IsImmun, 0) as EvnNotifyVener_IsImmun,
				isnull(EvnNotifyVener_IsKSR, 0) as EvnNotifyVener_IsKSR,
				isnull(EvnNotifyVener_IsRIBT, 0) as EvnNotifyVener_IsRIBT,
				isnull(EvnNotifyVener_IsRIF, 0) as EvnNotifyVener_IsRIF,
				isnull(EvnNotifyVener_IsRMP, 0) as EvnNotifyVener_IsRMP,
				isnull(EvnNotifyVener_IsRPGA, 0) as EvnNotifyVener_IsRPGA,
				isnull(EvnNotifyVener_IsRPR, 0) as EvnNotifyVener_IsRPR,
				isnull(EvnNotifyVener_IsTPM, 0) as EvnNotifyVener_IsTPM,
				isnull(VenerSocGroup_id, 0) as VenerSocGroup_id,
				isnull(EvnNotifyVener_Pathogen, 0) as EvnNotifyVener_Pathogen,
				ENO.VenerSocGroup_id,
				ENO.EvnNotifyVener_Pathogen
			from
				v_EvnNotifyVener ENO with (nolock)
				left join v_EvnVizitPL PL with (nolock) on ENO.EvnNotifyVener_pid = PL.EvnVizitPL_id
				left join v_EvnSection ST with (nolock) on ENO.EvnNotifyVener_pid = ST.EvnSection_id
	...
	function del($data)
	{
		$query = '
			declare
				...
				@ErrCode bigint,
				@ErrMsg varchar(4000);
			set @Res = :EvnNotifyVener_id;
			exec p_EvnNotifyVener_del
				@EvnNotifyVener_id = @Res,
				@pmUser_id = :pmUser_id,
				@Error_Code = @ErrCode output,
				@Error_Message = @ErrMsg output;
			select @Res as EvnNotifyVener_id, @ErrCode as Error_Code, @ErrMsg as Error_Msg;
		';
