...
class Perm_EvnSection_model extends EvnSection_model {
	...
	function getEvnSectionGroup($data) {
		...
		return $this->queryResult("
			select
				...
				es.Mes_kid,
				es.MesTariff_id,
				es.MesOldUslugaComplex_id,
				es.EvnSection_TotalFract,
				mt.MesTariff_Value,
				mo.Mes_id,
				mo.Mes_Code,
				mo.MesOld_Num,
			...
			where
				...
				and (ISNULL(flsp.LpuSectionProfile_Code, '1') <> '158' OR ISNULL(es.EvnSection_disDate, es.EvnSection_setDate) >= '2016-01-01')
				...
				and (ISNULL(es.EvnSection_IsPaid, 1) = 1 OR ISNULL(es.EvnSection_disDate, es.EvnSection_setDate) >= '2015-01-01') -- оплаченные движения с датой окончания до 2015 года не участвуют в группировке
				{$filter}
		", $queryParams);
	}
	...
	function checkMultiKSG($data) {
		...
		if (!empty($data['KSGFullArray'])) {
			...
			if (!in_array($this->lpuUnitTypeSysNick, array('dstac', 'hstac', 'pstac'))) {
				...
				$ksg4_resuscitation = array(); // оказание спецпомощи в реанимации на фоне основного заболевания
				$ksg4_basic = array(); 
				foreach($data['KSGFullArray'] as $ksg) {
					...
					if (mb_strlen($ksg['MesOld_Num']) == 8 && in_array($ksg['MesOld_Num'],array('st36.009','st36.010','st36.011'))) {
						$ksg4_resuscitation = $ksg;
					}
					if (mb_strlen($ksg['MesOld_Num']) == 8 && !in_array($ksg['MesOld_Num'],array('st36.009','st36.010','st36.011'))) {
						$ksg4_basic = $ksg;
					}
				...
				if (!empty($ksg160) && (!empty($ksg160Dop1Array1) || !empty($ksg160Dop1Array2))) {
					...
					$resp_eu = $this->queryResult("
						...
						select
							...
							ISNULL(eu2.EvnUsluga_disDate, eu2.EvnUsluga_setDate) as EvnUsluga2_disDate,
							eu.EvnUsluga_id as EvnUsluga1_id,
							eu.EvnUsluga_setDate as EvnUsluga1_setDate,
							ISNULL(eu.EvnUsluga_disDate, eu.EvnUsluga_setDate) as EvnUsluga1_disDate,
						...
						from
							...
							cross apply (
								...
								from
									EvnUslugaDopList eu (nolock)
								where
									eu.EvnUsluga_setDate > ISNULL(eu2.EvnUsluga_disDate, eu2.EvnUsluga_setDate)
								order by
									{$orderby}
									eu.EvnUsluga_setDate
							) eu
						where
							...
							and exists (
								...
								from
									v_MesOldUslugaComplex mouc (nolock)
									inner join v_MesOld mo (nolock) on mo.Mes_id = mouc.Mes_id and mo.MesOld_Num = 'st19.038'
								where
									mouc.UslugaComplex_id = eu2.UslugaComplex_id
						...
						from
							...
							cross apply (
								...
								where
									eu2.EvnUsluga_pid = :EvnSection_id
									and eu2.EvnClass_SysNick in ('EvnUslugaCommon', 'EvnUslugaOper', 'EvnUslugaPar', 'EvnUslugaStom')
									and eu2.EvnUsluga_setDate > ISNULL(eu.EvnUsluga_disDate, eu.EvnUsluga_setDate)
									and exists (
										select top 1
											MesOldUslugaComplex_id
										from
							...
							cross apply (
								...
								from
									v_MesOldUslugaComplex mouc (nolock)
									inner join v_MesOld mo (nolock) on mo.Mes_id = mouc.Mes_id and LEN(mo.MesOld_Num) = 8 and mo.MesOld_Num >= 'st19.001' and mo.MesOld_Num <= 'st19.026'
								where
						...
						from
							...
							cross apply (
								...
								from
									v_MesOldUslugaComplex mouc (nolock)
									inner join v_MesOld mo (nolock) on mo.Mes_id = mouc.Mes_id and LEN(mo.MesOld_Num) = 8 and mo.MesOld_Num >= 'st19.001' and mo.MesOld_Num <= 'st19.026'
									left join v_MesTariff mt with (nolock) on mt.Mes_id = mo.Mes_id
								where
									mouc.UslugaComplex_id = eu.UslugaComplex_id
									and mouc.MesOldUslugaComplex_id in ('".implode("','", $MesOldUslugaComplexArray)."')
							) mouc
				...
				// 1.4 Оказание спецпомощи в реанимации на фоне основного заболевания:
				//КСГ, которая НЕ входит в перечень: st36.009;st36.010;st36.011. + КСГ из перечня:st36.009;st36.010;st36.011+ движение содержит услугу «B02.003.001».
				if (!empty($ksg4_resuscitation) && !empty($ksg4_basic) ) {
					$resp_usl_resuscitation = $this->queryResult("
						select top 1
							eu.EvnUsluga_id as EvnUsluga_id,
							eu.EvnUsluga_setDT as EvnUsluga_setDate,
							isnull(eu.EvnUsluga_disDT, eu.EvnUsluga_setDT) as EvnUsluga_disDate,
							es.EvnSection_setDT as EvnSection_setDate,
							isnull(es.EvnSection_disDT, es.EvnSection_setDT) as EvnSection_disDate
						from
							v_EvnUsluga eu (nolock)
							inner join v_UslugaComplex uc (nolock) on uc.UslugaComplex_id = eu.UslugaComplex_id
							inner join v_EvnSection es (nolock) on es.EvnSection_id = :EvnSection_id
						where
							eu.EvnUsluga_pid = :EvnSection_id  
							and eu.EvnClass_SysNick in ('EvnUslugaCommon', 'EvnUslugaOper', 'EvnUslugaPar', 'EvnUslugaStom')
							and uc.UslugaComplex_Code = 'B02.003.001'	
						order by
							isnull(datediff(second, eu.EvnUsluga_setDT, eu.EvnUsluga_disDT), 0) desc,
							eu.EvnUsluga_setDT
					", array(
						'EvnSection_id' => $this->id
					));

					if (count($resp_usl_resuscitation) > 0) {
						$ksg4_basic['EvnSectionKSG_begDate'] = $resp_usl_resuscitation[0]['EvnSection_setDate'];
						$ksg4_basic['EvnSectionKSG_endDate'] = $resp_usl_resuscitation[0]['EvnSection_disDate'];
						$ksgArray[] = $ksg4_basic;
						$ksg4_resuscitation['EvnSectionKSG_begDate'] = $resp_usl_resuscitation[0]['EvnUsluga_setDate'];
						$ksg4_resuscitation['EvnSectionKSG_endDate'] = $resp_usl_resuscitation[0]['EvnUsluga_disDate'];
						$ksgArray[] = $ksg4_resuscitation;
					}
				}
			} else {
				// 2. Дневной стационар
					...
					$resp_eu = $this->queryResult("
						...
						where
							...
							and exists (
								...
								from
									v_MesOldUslugaComplex mouc (nolock)
									inner join v_MesOld mo (nolock) on mo.Mes_id = mouc.Mes_id and mo.MesOld_Num = 'ds19.028'
								where
									mouc.UslugaComplex_id = eu2.UslugaComplex_id
									and mouc.MesOldUslugaComplex_id in ('".implode("','", $MesOldUslugaComplexArray)."')
							)
					...
					$resp_eu = $this->queryResult("
						...
						from
							...
							cross apply (
								...
								from
									v_MesOldUslugaComplex mouc (nolock)
									inner join v_MesOld mo (nolock) on mo.Mes_id = mouc.Mes_id and LEN(mo.MesOld_Num) = 8 and mo.MesOld_Num >= 'ds19.016' and mo.MesOld_Num <= 'ds19.017'
								where
									mouc.UslugaComplex_id = eu.UslugaComplex_id
									and mouc.MesOldUslugaComplex_id in ('".implode("','", $MesOldUslugaComplexArray)."')
							) mouc
	...
	function loadKSGKPGKOEFCombo2018($data) {
		...
		$Diag_pid = $this->getFirstResultFromQuery("
			select top 1
				isnull(d4.Diag_pid, d3.Diag_pid) as Diag_pid
			from
				v_Diag d (nolock)
				left join v_Diag d2 (nolock) on d2.Diag_id = d.Diag_pid
				left join v_Diag d3 (nolock) on d3.Diag_id = d2.Diag_pid
		...
		if (!empty($Diag_pid)) {
			...
			$query = "
				...
				WHERE
					...
					and ISNULL(es.EvnSection_IsPriem, 1) = 1
					and es.HTMedicalCareClass_id is null
					and isnull(d4.Diag_pid, d3.Diag_pid) = :Diag_pid
	...
	function loadKSGKPGKOEFCombo2015($data) {
							...
							$query = "
								...
								from
									...
									cross apply(
										...
										from
											r59.v_MesLpuUnitType mlut (nolock)
										where
		...
		if (!$isKruglStac) {
			$filter = "
				and exists(
					...
					from
						v_UslugaComplexAttribute uca (nolock)
						inner join v_UslugaComplexAttributeType ucatype (nolock) on ucatype.UslugaComplexAttributeType_id = uca.UslugaComplexAttributeType_id
					where
		...
		if (!empty($data['EvnSection_id'])) {
			$query = "
				...
				where
						...
						or (
							...
							and exists(
								...
								from
									v_EvnDiagPS edps (nolock)
									inner join v_Diag d (nolock) on d.Diag_id = edps.Diag_id
									inner join v_Diag dp (nolock) on dp.Diag_id = d.Diag_pid
								where
									edps.DiagSetClass_id IN (2,3)
									and edps.EvnDiagPS_pid = :EvnSection_id
					...
					and (IsNull(mt.MesTariff_endDT, :EvnSection_disDate)>= :EvnSection_disDate)
					and (
						mo.Mes_Code <> '4' or not exists (select top 1 EvnUsluga_id from v_EvnUsluga eu (nolock) inner join v_UslugaComplex uc (nolock) on uc.UslugaComplex_id = eu.UslugaComplex_id where uc.UslugaComplex_Code in ('B01.001.006', 'B01.001.009', 'B02.001.002') and eu.EvnUsluga_pid = :EvnSection_id and eu.PayType_id = {$this->getPayTypeIdBySysNick('oms')})
					)
					{$filter}
				order by
					eu.EvnUsluga_id,
		...
		if ($isKruglStac || strtotime($data['EvnSection_disDate']) >= strtotime('01.05.2015')) {
			...
			if (!empty($data['Diag_id'])) {
				...
				where
					mu.Diag_id = :Diag_id
					and (mu.UslugaComplex_id is null OR mu.UslugaComplex_id IN (select eu.UslugaComplex_id from v_EvnUsluga eu (nolock) where eu.EvnUsluga_pid = :EvnSection_id and eu.PayType_id = {$this->getPayTypeIdBySysNick('oms')}))
					and (mu.Sex_id = :Sex_id OR Sex_id IS NULL)
					and (
						(:Person_Age >= 18 and mu.MesAgeGroup_id = 1)
						or (:Person_Age < 18 and mu.MesAgeGroup_id = 2)
		...
		foreach($combovalues as &$combovalue) {
			...
			if (!empty($data['EvnSection_pid'])) {
				$query = "
					...
					from
						v_EvnPS eps (nolock)
						left join v_EvnSection es (nolock) on es.EvnSection_id = :EvnSection_id
					where
	...
	function loadKSGKPGKOEFCombo($data) {
		...
		if (!empty($data['Diag_id']) && !empty($data['DiagPriem_id'])) {
			$DiagSSZ_id = $this->getFirstResultFromQuery('select DiagSSZ_id from v_DiagSSZ (nolock) where Diag_id = :Diag_id', $data);
			if (!empty($DiagSSZ_id)) {
		...
		if (!empty($data['EvnSection_id'])) {
			$query = "
				...
				from v_EvnUsluga eu (nolock)
					...
					inner join v_UslugaCategory ucat (nolock) on ucat.UslugaCategory_id = uc.UslugaCategory_id and ucat.UslugaCategory_SysNick = 'gost2011'
					inner join v_MesOldUslugaComplex mu (nolock) on mu.UslugaComplex_id = uc.UslugaComplex_id
					inner join v_MesOld mo (nolock) on mo.Mes_id = mu.Mes_id and mo.MesType_id IN (2,3,5) -- КСГ
					left join v_MesTariff mt (nolock) on mt.Mes_id = mo.Mes_id -- Коэффициент КСГ
				where
					eu.EvnUsluga_pid = :EvnSection_id
					...
					and (
						...
						OR
						(
							(select Diag_Code from v_Diag (nolock) where Diag_id = :Diag_id) NOT IN ('N17.8', 'N17.9')
							and	exists(
								select top 1
									edps.EvnDiagPS_id
								from
		...
		if (!empty($data['Diag_id'])) {
			$query = "
				...
				from v_MesOldDiag mod (nolock)
					inner join v_MesOld mo (nolock) on mo.Mes_id = mod.Mes_id and mo.MesType_id IN (2,3,5) -- КСГ
				...
				where
					...
					and (
						...
						OR
						exists (select top 1 EvnUsluga_id from v_EvnUsluga eu (nolock) inner join v_UslugaComplex uc (nolock) on uc.UslugaComplex_id = eu.UslugaComplex_id where uc.UslugaComplex_Code in ('B01.001.006', 'B01.001.009', 'B02.001.002') and eu.EvnUsluga_pid = :EvnSection_id and eu.PayType_id in ({$this->getPayTypeIdBySysNick('oms')},{$this->getPayTypeIdBySysNick('mbudtrans')}))
					)
				order by 
					mt.MesTariff_Value desc
			";
			...
			if (is_object($result)) {
				...
				if (count($resp) > 0) {
					...
					if ( in_array($resp[0]['Diag_Code'], array('I20.1', 'I20.8', 'I20.9', 'I24.0', 'I24.1', 'I24.8', 'I24.9', 'I25.0', 'I25.1', 'I25.2', 'I25.3', 'I25.4', 'I25.5', 'I25.6', 'I25.8', 'I25.9')) ) {
						...
						$EvnUsluga_id = $this->getFirstResultFromQuery("select top 1 EvnUsluga_id from v_EvnUsluga eu (nolock) inner join v_UslugaComplex uc (nolock) on uc.UslugaComplex_id = eu.UslugaComplex_id where uc.UslugaComplex_Code in ('A06.10.006') and eu.EvnUsluga_pid = :EvnSection_id and eu.PayType_id in ({$this->getPayTypeIdBySysNick('oms')},{$this->getPayTypeIdBySysNick('mbudtrans')})", array('EvnSection_id' => $data['EvnSection_id']));
	...
	protected function calcCoeffCTP2019($data) {
		...
		$resp_codes = $this->queryResult("
			declare
				@AttributeVision_TablePKey bigint = (select top 1 TariffClass_id from v_TariffClass (nolock) where TariffClass_SysNick = '2018KSLP');
		...
		if ($data['LpuUnitType_id'] == '1' && isset($KSLPCodes[$codeKSLP])) {
			if (!empty($data['EvnSectionIds'])) {
				...
				$Mes_id = $this->getFirstResultFromQuery("select top 1 Mes_id from v_MesOld (nolock) where MesOld_Num = 'st38.001' and (Mes_endDT is null or Mes_endDT >= :EvnSection_disDate)", $data);
		...
		if ($data['LpuUnitType_id'] == '1' && isset($KSLPCodes[$codeKSLP])) {
			$Diags = $this->queryResult("
				declare
					...
					@AttributeVision_TablePKey bigint = (select TariffClass_id from v_TariffClass (nolock) where TariffClass_Code = '2016-01Диаг');
				...
				FROM
					v_AttributeVision avis (nolock)
					inner join v_AttributeValue av (nolock) on av.AttributeVision_id = avis.AttributeVision_id
					inner join v_Diag d (nolock) on d.Diag_id = av.AttributeValue_ValueIdent
				WHERE
					avis.AttributeVision_TableName = 'dbo.TariffClass'
					and avis.AttributeVision_TablePKey = @AttributeVision_TablePKey
					and avis.AttributeVision_IsKeyValue = 2
		...
		if ($data['LpuUnitType_id'] == '1' && isset($KSLPCodes[$codeKSLP])) {
			...
			if (count($data['KSGs']) > 0) {
				...
				$MesLinkType = $this->queryResult("
					...
					where
						...
						and ISNULL(MesLink_endDT, :EvnSection_disDate) >= :EvnSection_disDate
			...
			if (!$needKSLP5 && !empty($data['EvnSectionIds'])) {
				$DTSList = $this->queryResult("
					select distinct DrugTherapyScheme_id
		...
		if ($data['LpuUnitType_id'] == '1' && isset($KSLPCodes[$codeKSLP])) {
			if (!empty($data['EvnSectionIds'])) {
				$AttributeValue = $this->queryResult("
					declare
						@AttributeVision_TablePKey bigint = (select top 1 VolumeType_id from v_VolumeType (nolock) where VolumeType_Code = '2018-01СочетХирургВмеш');
		...
		if ($data['LpuUnitType_id'] == '1' && isset($KSLPCodes[$codeKSLP])) {
			...
			if (!empty($data['EvnSectionIds'])) {
				...
				if (!empty($EvnUslugaList23) && is_array($EvnUslugaList23) && count($EvnUslugaList23) == 2) {
					...
					// КПГ 12
					$kpg12 = $this->getFirstResultFromQuery("select Mes_id from v_MesOld with (nolock) where  Mes_Code = '12' and MesType_id = 4", [], true);
					...
					if (!empty($kpg12)) {
						...
						foreach ($diagFilters as $diagFilter) {
							$MesLink_id = $this->getFirstResultFromQuery($query . ' and ' . $diagFilter, [
								'EvnSection_disDate' => $data['EvnSection_disDate'],
								'Mes_sid' => $kpg12,
								'Diag_id' => $data['Diag_id'],
							]);
		...
		if (strtotime($data['EvnSection_disDate']) >= strtotime('01.07.2015')) {
			$EvnDiagPS_id = $this->getFirstResultFromQuery("
				select top 1
					edp.EvnDiagPS_id
				from
					v_EvnDiagPS edp (nolock)
					inner join v_Diag d (nolock) on d.Diag_id = edp.Diag_id
					inner join v_Diag d2 (nolock) on d2.Diag_id = d.Diag_pid
				where
		...
		foreach($groupped as $group) {
			...
			foreach($group['EvnSections'] as $es) {
				if ($es['EvnSection_IsManualIdxNum'] != 2) {
					$this->db->query("update EvnSection with (rowlock) set EvnSection_IndexNum = :EvnSection_IndexNum where EvnSection_id = :EvnSection_id", array(
