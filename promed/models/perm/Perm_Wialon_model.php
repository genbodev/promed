<?php
/**
* Perm_Wialon_model - модель для работы с Wialon
*
...
require_once(APPPATH.'models/Wialon_model.php');

class Perm_Wialon_model extends Wialon_model {
	/**
	...
	public function checkAuth(){
		...
		if ( isset( $_SESSION['wialon']['user'] ) && isset( $_SESSION['wialon']['password'] ) ) {
			...
			$this->_token =  $_SESSION['wialon']['WialonToken'];
			if ( $this->login() ) {
				$auth = true;
			} else {
	...
	public function getSummaryReportStdMileage($param) {
		...
		$name = 'СВОДНЫЙ ОТЧЕТ STD пробег'; // название отчета, который мы хотим получить из wialon
		//можно не создавать дополнительный запрос. ID шаблона не меняется
		/*
		$reportParam = array(
			'name' => $name ,
			'type' => 'avl_unit'
		 ...
		 */
		$template_id=9; // ID шаблона 'СВОДНЫЙ ОТЧЕТ STD пробег'.
		$resource_id = $this->getResourceId();
		$resource_id = 308; // Указано значения напрямую из для корректной работы отчетов виалона на рабочем смп перми (refs #113165)
		...
		if( !empty($report_result['error']) ){
			...
			if(!empty($report_result['reportResult']['stats']) && is_array($report_result['reportResult']['stats'])){
	...
	/**
	 * возвращает пройденое расстояние из отчета 'СВОДНЫЙ ОТЧЕТ STD пробег'
	 */
