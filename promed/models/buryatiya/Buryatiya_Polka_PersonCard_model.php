	...
	*/
	...
	function exportAttachmentsApplicationsDBF($data){
		...
		$query = "
			declare
				...
				@Lpu_f003mcod varchar(20),
				@Orgsmo_f002smocod varchar(20);

			select top 1
				@Lpu_f003mcod = l.Lpu_f003mcod,
				@Orgsmo_f002smocod = smo.Orgsmo_f002smocod
			from v_Lpu l with (nolock)
				outer apply (
					select top 1 Orgsmo_f002smocod
					from v_OrgSMO with (nolock)
			...
			SELECT DISTINCT
				...
				PC.Person_id AS ID,
				CASE
					WHEN PCAC.PersonCard_id is not null and PCAC.Lpu_id = PC.Lpu_id and ISNULL(PCAC.CardCloseCause_Code, 0) = 4 then 'И'
					WHEN PCAC.PersonCard_id is null THEN 'Р'
					WHEN PCAC.Lpu_id is not null and PS.Lpu_id != PCAC.Lpu_id THEN 'Р'
					ELSE 'И'
				END AS OP,
				ISNULL(PS.Person_SurName,'') AS FAM,
				ISNULL(PS.Person_FirName,'') AS IM,
				ISNULL(PS.Person_Secname,'') AS OT,
				CONVERT(VARCHAR, PS.Person_Birthday, 112) AS DR,
				PS.Sex_id AS W,
				PS.Polis_Ser AS SPOL,
				CASE WHEN PS.PolisType_id = 4 THEN PS.Person_edNum ELSE PS.Polis_Num END AS NPOL,
				@Orgsmo_f002smocod AS Q,
				@Lpu_f003mcod AS LPU,
				convert(varchar(8),PC.PersonCard_begDate, 112) AS LPUDZ,
				convert(varchar(8),PC.PersonCard_updDT, 112) AS LPUDU,
				CASE 
					WHEN PC.PersonCardAttach_id IS NULL THEN '1'
					WHEN PC.PersonCardAttach_id IS NOT NULL
						AND (
							ISNULL(PA.KLRgn_id, 0) != ISNULL(LastPA.KLRgn_id, 0)
							OR ISNULL(PA.KLSubRgn_id, 0) != ISNULL(LastPA.KLSubRgn_id, 0)
							OR ISNULL(PA.KLCity_id, 0) != ISNULL(LastPA.KLCity_id, 0)
							OR ISNULL(PA.KLTown_id, 0) != ISNULL(LastPA.KLTown_id, 0)
							OR ISNULL(PA.KLStreet_id, 0) != ISNULL(LastPA.KLStreet_id, 0)
							OR ISNULL(PA.Address_House, '') != ISNULL(LastPA.Address_House, '')
							OR ISNULL(PA.Address_Corpus, '') != ISNULL(LastPA.Address_Corpus, '')
							OR ISNULL(PA.Address_Flat, '') != ISNULL(LastPA.Address_Flat, '')
						)
						THEN '1'
					ELSE '2'
				END LPUTP,
				...
				left(L.Lpu_f003mcod, 6) + CASE
					when mrmp.LpuRegionType_SysNick in ('ter', 'ped', 'vop', 'feld') then
						...
						RIGHT('00'+ISNULL(mrmp.LpuRegion_Name,''), 2)
					else ''
				end as LPUKOD,
				ISNULL(mrmp.code, '') AS LPUKAT,
				CASE
					...
					ELSE ''
				END as OKATO,
				ISNULL(srgn_p.KLArea_Name, 'Бурятия') AS RNNAME,
				ISNULL(city_p.KLArea_Name, town_p.KLArea_Name) AS NPNAME,
				ISNULL(street_p.KLStreet_Name, '') AS UL,
				ISNULL(PA.Address_House, '') AS DOM,
				ISNULL(PA.Address_Corpus, '') AS KORP,
				ISNULL(PA.Address_Flat, '') AS KV,
				'' AS LPUDT,
				'' AS LPUDX,
				'' AS STATUS,
				'' AS ERR,
			...
			from
				...
				left join KLArea country_p with (nolock) on country_p.KLArea_id = PA.KLCountry_id
				left join KLArea rgn_p with (nolock) on rgn_p.KLArea_id = PA.KLRgn_id
				left join KLArea srgn_p with (nolock) on srgn_p.KLArea_id = PA.KLSubRgn_id
				left join KLArea city_p with (nolock) on city_p.KLArea_id = PA.KLCity_id
				left join KLArea town_p with (nolock) on town_p.KLArea_id = PA.KLTown_id
				left join KLStreet street_p with (nolock) on street_p.KLStreet_id = PA.KLStreet_id
				...
				OUTER APPLY (
					SELECT TOP 1 PCS.PersonCardAttachStatusType_id, PersonCardStatus_setDate
					FROM PersonCardStatus PCS with (nolock)
					WHERE PCS.PersonCard_id = PC.PersonCard_id
					ORDER BY PersonCardStatus_setDate DESC
				) PCS
				left join PersonCardAttachStatusType PCAST (nolock) on PCAST.PersonCardAttachStatusType_id = PCS.PersonCardAttachStatusType_id
				OUTER APPLY (
					SELECT PersonCard_IsAttachAuto FROM PersonCard with (nolock) WHERE PersonCard_id = PC.PersonCard_id
				) PCAUTO
				outer apply(
					...
					select top 1
						...
						PCA.Lpu_id
					FROM v_PersonCard_all PCA with(nolock)
						LEFT JOIN v_CardCloseCause ccc with (nolock) on ccc.CardCloseCause_id = PCA.CardCloseCause_id	
				...
				outer apply(
					...
					select top 1 PCA.PersonCard_id
					FROM v_PersonCard_all PCA with(nolock)
					where pc.Person_id = PCA.Person_id
						and PCA.PersonCard_id <> pc.PersonCard_id
						and PCA.LpuAttachType_id = 1
				...
				OUTER APPLY (
					...
					FROM v_MedStaffRegion msr with (nolock)
						...
						outer apply (
							select top 1 Person_Fio, Person_Snils, Person_SurName, Person_FirName, Person_SecName
							from v_MedPersonal with (nolock)
							where MedPersonal_id = msr.MedPersonal_id
						) mp
						inner join v_LpuRegion lr with(nolock) on lr.LpuRegion_id = msr.LpuRegion_id
						LEFT JOIN persis.v_PostKind pk WITH (nolock) ON pk.id = msf.PostKind_id
					WHERE (1=1)  
				...
				left join v_Address LastPA with (nolock) on LastPA.Address_id = LastAddrPer.Address_id
			where 
				PC.Lpu_id = @Lpu_id
				AND PC.PersonCard_endDate IS NULL
				AND PC.LpuAttachType_id = 1
			...
			SELECT DISTINCT
				...
				@Orgsmo_f002smocod AS Q,
				@Lpu_f003mcod AS LPU,
				convert(varchar(8),PCA.PersonCardAttach_setDate, 112) AS LPUDZ,
				convert(varchar(8),PCA.PersonCardAttach_setDate, 112) AS LPUDU,
				CASE
					WHEN
						ISNULL(PA.KLRgn_id, 0) != ISNULL(LastPA.KLRgn_id, 0)
						OR ISNULL(PA.KLSubRgn_id, 0) != ISNULL(LastPA.KLSubRgn_id, 0)
			...
			from
				...
				OUTER APPLY (
					SELECT TOP 1 Person_id, PersonCard_id, Lpu_id, PersonCard_begDate
				...
				outer apply (
					select top 1 PersonCardAttachStatus_id, PersonCardAttachStatusType_id, PersonCardAttachStatus_setDate
					from v_PersonCardAttachStatus (nolock)
					where PersonCardAttach_id = PCA.PersonCardAttach_id
					order by PersonCardAttachStatus_setDate desc
				) PCAS
				inner join PersonCardAttachStatusType PCAST (nolock) on PCAST.PersonCardAttachStatusType_id = PCAS.PersonCardAttachStatusType_id
				...
				SELECT
					...
					CASE
						when LR.LpuRegionType_SysNick in ('ter', 'ped', 'vop', 'feld') then
							...
							RIGHT('00'+ISNULL(LR.LpuRegion_Name,''), 2)
						else ''
					end as LpuRegionKOD
				FROM v_MedStaffRegion msr with (nolock)
					INNER join v_LpuRegion lr with(nolock) on lr.LpuRegion_id = msr.LpuRegion_id
	...
	function searchPersonAttachDBF($data){
		...
		$sql = "
			select
				...
				PCA.LpuRegion_fapid,
				PCA.MedStaffFact_id,
				mrmp.Person_SurName, mrmp.Person_FirName, mrmp.Person_SecName,
				mrmp.MedPersonal_id,
				mrmp.MedPersonal_Snils
			from v_PersonCardAttach PCA with(nolock)
				inner join v_PersonState PS (nolock) on PS.Person_id = PCA.Person_id
				left join v_Lpu L with (nolock) on L.Lpu_id = PCA.Lpu_id
				OUTER APPLY (
					SELECT
				...
				LEFT JOIN v_PersonCard_all PC WITH (nolock) ON PCA.PersonCardAttach_id = PC.PersonCardAttach_id
				/*
				outer apply (
	...
	function setPersonCardStatusDBF($data){
		...
		$query = "
			...
			exec p_PersonCardStatus_ins
				@PersonCardStatus_id = @Res output,
				@PersonCard_id = :PersonCard_id,
				@PersonCardAttachStatusType_id = :PersonCardAttachStatusType_id,
				@PersonCardStatus_setDate = @setDate,
				@pmUser_id = :pmUser_id,
				@Error_Code = @ErrCode output,
				@Error_Message = @ErrMessage output;

			select @Res as PersonCardStatus_id, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
		";
	...
	function getByCodePersonCardAttachStatusTypeID($code){
		...
		$query = "SELECT PersonCardAttachStatusType_id FROM v_PersonCardAttachStatusType WHERE PersonCardAttachStatusType_Code = :Code";
	...
	function getLpuIdByAppointment_f003mcod($data){
		...
		$query = "SELECT top 1 Lpu_id FROM v_Lpu L WHERE left(L.Lpu_f003mcod, 6) = :f003mcod";
	...
	function getLpuRegionID_LPUKOD($data){
		...
		if(!empty($Lpu_id) && !empty($numRegion) && !empty($typeRegion) && !empty($regionSysNick[$typeRegion])){
			$query = "
				SELECT LR.LpuRegion_id, LR.LpuRegionType_id
				FROM v_LpuRegion LR with(nolock)
				WHERE 
					RIGHT('00'+ISNULL(LR.LpuRegion_Name,''), 2) = :LpuRegion_Name
	...
	function closePersonCard_importDBF($data, $line){
		...
		$query = "
			...
			from
				v_PersonCard PC with(nolock)
				left join v_LpuAttachType LAT with(nolock) on LAT.LpuAttachType_id = PC.LpuAttachType_id
				left join v_CardCloseCause CCC with(nolock) on CCC.CardCloseCause_Code = :CardCloseCause_Code
			where
