<?php
class Wialon_model extends swModel {
	/**
	...
	public function mergeEmergencyTeam( $data ){
		...
		if ( $is_pg ) {
			...
			$sql = "SELECT EmergencyTeamWialonRel_id FROM v_EmergencyTeamWialonRel WHERE EmergencyTeam_id=:EmergencyTeam_id";
		}
		...
		if ( $is_pg ) {
			if ( !$sqlArr['EmergencyTeamWialonRel_id'] ) {
				$sql = "
					INSERT INTO dbo.\"EmergencyTeamWialonRel\" (
						...
						NOW(),
						NOW()
					) RETURNING \"EmergencyTeamWialonRel_id\", null as \"Error_Code\", null as \"Error_Msg\";
				";
			} else {
				$sql = "
					...
					RETURNING
						\"EmergencyTeamWialonRel_id\", null as \"Error_Code\", null as \"Error_Msg\";
				";
			}
		} else {
			if ( $sqlArr['EmergencyTeamWialonRel_id'] ) {
			...
			$sql = "
				...
				SET @Res = :EmergencyTeamWialonRel_id;

				EXEC ".$procedure."
					@EmergencyTeamWialonRel_id = @Res output,
					@EmergencyTeam_id = :EmergencyTeam_id,
					@WialonEmergencyTeamId = :WialonEmergencyTeamId,
					@pmUser_id = :pmUser_id,
					@Error_Code = @ErrCode output,
					@Error_Message = @ErrMessage output;
				SELECT @Res as EmergencyTeamWialonRel_id, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
			";
		}
	...
	public function deleteMergeEmergencyTeam( $data ){
		...
		if ( $this->db->dbdriver == 'postgre' ) {
			$sql = "DELETE FROM dbo.\"EmergencyTeamWialonRel\" WHERE \"EmergencyTeam_id\" = :EmergencyTeam_id";
		} else {
			$sql = "
				DECLARE
					@ErrCode int,
				...
				EXEC p_EmergencyTeamWialonRel_delByETId
					@EmergencyTeam_id = :EmergencyTeam_id,
					@Error_Code = @ErrCode output,
					@Error_Message = @ErrMessage output;
	...
	public function getWialonCredentialsByLpuDepartment($data){
		...
		$result = $this->db->query($query.$filters, $params);
		if (is_object($result))
		{
	...
	public function saveEmergencyTeamRel( $data ){
		...
		foreach( $data['data'] as $k => $v ) {
			$prequery = "DELETE FROM v_EmergencyTeamWialonRel with(rowlock) WHERE EmergencyTeam_id=:EmergencyTeam_id";
						//AND Lpu_id=:Lpu_id";
			$preparams = array(
				'Lpu_id'					=> $data['Lpu_id'],
			...
			if ( (int)$v->EmergencyTeam_id ) {
				$query = $this->db->query("
					...
					EXEC p_EmergencyTeamWialonRel_ins
						@EmergencyTeamWialonRel_id = @Res output,
						@Lpu_id = :Lpu_id,
						@EmergencyTeam_id = :EmergencyTeam_id,
						@WialonEmergencyTeamId = :WialonEmergencyTeamId,
	...
	public function getUnitIdByEmergencyTeamId( $data ){
		...
		$sql = "
			SELECT
				WialonEmergencyTeamId
			FROM
				v_EmergencyTeamWialonRel with(nolock)
			WHERE
				EmergencyTeam_id=:EmergencyTeam_id
		";
	...
	public function getHousesByCityId( $city_id, $page=1 ){
		...
		$sql = "
			SELECT
				...
				h.KLHouse_Name
			FROM
				KLStreet s with(nolock)
				LEFT JOIN KLHouse h with(nolock) ON( h.KLStreet_id=s.KLStreet_id )
			WHERE
				s.KLArea_id=3310
	...
	public function checkIfHouseLatLngExists( $house_num, $house_id, $street_id, $city_id ) {
		...
		$sql = "IF EXISTS( SELECT TOP 1 KLHouseCoords_id FROM KLHouseCoords khc with(nolock) WHERE khc.KLArea_id=:KLArea_id AND khc.KLStreet_id=:KLStreet_id AND khc.KLHouse_id=:KLHouse_id AND khc.KLHouseCoords_Name=:KLHouseCoords_Name ) SELECT 1 as cnt";
		$query = $this->db->query( $sql, array(
			'KLArea_id' => $city_id,
	...
	public function saveHouseLatLng( $lat, $lng, $house_num, $house_id, $street_id, $city_id ){
		...
		$sql = "
			...
			SELECT @Res as KLHouseCoords_id, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
		";
	...
	public function bindLpuBuildingHouse( $LpuBuilding_id, $KLHouseCoords_id ){
		// Проверим существование записи
		$sql = "SELECT TOP 1 LpuBuildingKLHouseCoordsRel_id FROM LpuBuildingKLHouseCoordsRel with(nolock) WHERE LpuBuilding_id=:LpuBuilding_id AND KLHouseCoords_id=:KLHouseCoords_id";
		$query = $this->db->query( $sql, array(
		...
		$sql = "
			...
			EXEC p_LpuBuildingKLHouseCoordsRel_ins
				@LpuBuildingKLHouseCoordsRel_id = @Res output,
				@LpuBuilding_id = :LpuBuilding_id,
				@KLHouseCoords_id = :KLHouseCoords_id,
				@pmUser_id = :pmUser_id,
				@Error_Code = @ErrCode output,
			...
			SELECT @Res as LpuBuildingKLHouseCoordsRel_id, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
		";
