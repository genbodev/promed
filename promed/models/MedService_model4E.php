...
class MedService_model4E extends swModel {
	...
	*/
		...
		$query = "
			...
			from 
				v_MedPersonal MP with(nolock)
				 left join v_MedServiceMedPersonal MSMP with(nolock) on MP.MedPersonal_id = MSMP.MedPersonal_id
				 left join v_MedService MS with(nolock) on MS.MedService_id = MSMP.MedService_id
			where
				MP.Lpu_id = :Lpu_id
				{$where}
	...
	function loadLpu($data)
	{
		...
		$filterPriem .= " and (IsNull(LpuSection_disDate, @getdate) >= @getdate)";
		$filterPriem .= " and (IsNull(L.Lpu_endDate, @getdate) >= @getdate)";

		$declare = "declare @getdate datetime = dbo.tzGetDate();
		";
		$querySection = "
					...
					SELECT
							...
							LS.Lpu_id as lpu_id,   
							cast(L.Lpu_Nick as varchar)+ '/' + cast(RTRIM(LS.LpuSection_Name) as varchar) + '/' + cast(ISNULL(A.Address_Nick, '') as varchar) as name,
							'LpuSection' as code	
		...
		$queryMO = "
				SELECT 
					...
					Ml.Lpu_Nick as name,
					'MO' as code
				FROM v_Lpu Ml with (nolock) 
				  WHERE 
					NOT EXISTS(
						SELECT top 1 Mls.LpuSection_id
						 FROM v_LpuSection Mls with (nolock) 
						 inner join v_LpuSectionProfile MLSP with (nolock) on MLSP.LpuSectionProfile_id = Mls.LpuSectionProfile_id
						 inner join v_Lpu L with (nolock) on L.Lpu_id = Mls.Lpu_id
						 WHERE
						 Ml.Lpu_id = mls.Lpu_id
						 AND M".$filterPriem."
					 )
					 AND COALESCE(Ml.Lpu_IsTest, 1) != 2
					 AND IsNull(Ml.Lpu_endDate, @getdate) >= @getdate
				";
		$queryVolMO = 'SELECT
						...
						INNER JOIN v_VolumeType vt (nolock) ON vt.VolumeType_Code = \'LpuHospitalizationSMP\' 
						WHERE
	...
	public function getVolumeTypeLpuHospitalizationSMPExist( $data ){
		...
		$sql = "SELECT
				...
				WHERE
					avis.AttributeVision_TableName = 'dbo.VolumeType'
				AND avis.AttributeVision_TablePKey = vt.VolumeType_id
				AND isnull(av.AttributeValue_endDate, getDate()) >= getDate()";
	...
	public function loadMedPersonalRolesExpertList( $data ){
		...
		$sql = "
			...
			FROM
				v_MedPersonal MP with(nolock)
				INNER JOIN v_pmUserCache PUC with(nolock) ON( PUC.MedPersonal_id = MP.MedPersonal_id )
			WHERE
				MP.Lpu_id=:Lpu_id
				AND PUC.pmUser_groups LIKE '%{\"name\":\"".$BSME_EXPERT_ROLE."\"}%'
		";
	...
	public function loadMedPersonalLpuBuildings( $data ){
		...
		$sql = 'declare @date datetime = cast(dbo.tzGetDate() as date); 
            SELECT	DISTINCT LB.LpuBuilding_id, -- ID подстанции
                    LB.LpuBuilding_Name, -- Полное название подстанции
                    LB.LpuBuilding_Nick  -- ник
            from v_MedServiceMedPersonal msmp with (nolock) -- службы врача
                    left join v_MedPersonal mp with (nolock) on msmp.MedPersonal_id = mp.MedPersonal_id and mp.Lpu_id = :Lpu_id -- врач в нашей МО
                    left join v_MedService MS with (NOLOCK) on MS.MedService_id = msmp.MedService_id
                    left join v_MedServiceType mst with (nolock) on mst.MedServiceType_Code = 19 -- Тип СМП
            ...
            and sut.SmpUnitType_Code = 4 -- опер отдел
            and (
                cast(msmp.MedServiceMedPersonal_begDT as date) <= @date
                and (cast(msmp.MedServiceMedPersonal_endDT as date) >= @date
		...
		if (!empty($lpuBuilding[0]['LpuBuilding_id'])) {
			if (!empty($operDpt[0]['LpuBuilding_id']) && !in_array($params['SmpUnitType_Code'], array('2','5')) ){ // Значит юзер под опер. отделом.
				$sql = "
                    SELECT DISTINCT
                        LB.LpuBuilding_id, -- ID подстанции
                        LB.LpuBuilding_Name, -- Полное название подстанции
                        LB.LpuBuilding_Nick  -- ник
                    FROM v_SmpUnitParam sup
                    left join LpuBuilding LB with (nolock) ON sup.LpuBuilding_id = LB.LpuBuilding_id
                    where sup.LpuBuilding_pid = :LpuBuilding_pid";
			}else{ // Тянем подчиненные или удаленные подстанции по рабочим местам юзера.
				$sql = 'declare @date datetime = cast(dbo.tzGetDate() as date); 
                    SELECT	DISTINCT
                            LB.LpuBuilding_id, -- ID подстанции
                            LB.LpuBuilding_Name, -- Полное название подстанции
                    ...
                    FROM v_MedServiceMedPersonal msmp with (nolock) -- службы врача
                            ...
                            left join v_SmpUnitType sut with(nolock) ON sup.SmpUnitType_id = sut.SmpUnitType_id
                    where msmp.MedPersonal_id = :medPersonal_id
                    and sut.SmpUnitType_Code = :SmpUnitType_Code
                    and lb.Lpu_id = :Lpu_id  -- Из этой МО
                    and (
                        cast(msmp.MedServiceMedPersonal_begDT as date) <= @date
                        and (cast(msmp.MedServiceMedPersonal_endDT as date) >= @date
                        or msmp.MedServiceMedPersonal_endDT is null)
	...
	*/
		...
		switch ($action)
		{
			...
			case "AllAddress":
				...
				if( in_array($CurArmType, array('dispcallnmp', 'dispdirnmp', 'dispnmp', 'nmpgranddoc')) ){
					...
					$nmpBuildings = " LEFT JOIN dbo.v_DataStorage ds with(nolock) on (ds.Lpu_id = lpu.Lpu_id and ds.DataStorage_Name = 'homevizit_isallowed' and ds.DataStorageGroup_SysNick = 'homevizit')";
					//$data["LpuBuildingType_id"] = 28;
					//$filter .= " AND LB.LpuBuildingType_id = :LpuBuildingType_id";
					$nmpBuildings .= " outer apply (select top 1 * from v_LpuBuilding LB with (nolock) where LB.LpuBuilding_id = MS.LpuBuilding_id) as LB
						inner join v_SmpUnitParam sup with (nolock) ON LB.LpuBuilding_id = sup.LpuBuilding_id
						inner join v_SmpUnitType sut with (nolock) ON sup.SmpUnitType_id = sut.SmpUnitType_id
					";
				...
				//только открытые МО
				$filter .= " and ((dbo.tzGetDate() BETWEEN LPU.Lpu_begDate AND LPU.Lpu_endDate) OR ISNULL(LPU.Lpu_endDate, 1) = 1 )";
				...
				//если нужно подставить конкретную МО
				if(!empty($data['ConcreteLpu_id'])) {
					$filter = "LPU.Lpu_id = :ConcreteLpu_id";
					$nmpFields = '';
					$nmpBuildings = '';
				}
	...
	function loadNmpMedServiceList($data) {
		$params = array();
		$filter = "MST.MedServiceType_SysNick like 'slneotl'";
	...
	function loadSectionProfileByMO($data)
	{
		$query = "
			...
			WHERE
			  LSP.LpuSectionProfile_id IN (SELECT DISTINCT LS.LpuSectionProfile_id FROM v_LpuSection LS WITH (nolock) WHERE LS.Lpu_id = :Lpu_id)
					";
	...
	//					case '6':
	//						$add_join = 'inner join v_EvnUslugaCommon EP with (nolock) on EP.EvnPrescrProc_id = :EvnPrescr_id
	...
	//				CONVERT(varchar(10), ttms.TimetableMedService_begTime, 104) as FirstFreeDate,
	//				CONVERT(varchar(5), ttms.TimetableMedService_begTime, 108) as FirstFreeTime
	...
	//				outer apply (
	//					select top 1 ttms.TimetableMedService_begTime from v_TimetableMedService_lite ttms with (nolock)
	...
	//			"select {$filterField} from v_UslugaComplex  with (nolock) where UslugaComplex_id = :UslugaComplex_id",
	...
	//			select
	//				-- select
	//				isnull(pzm.MedService_id, ms.MedService_id) as MedService_id,
	//				ucms.UslugaComplexMedService_id,
	//				lpu.Lpu_id,
	//				lpu.Lpu_Nick,
	//				lu.LpuBuilding_id,
	...
	//				left join v_MedServiceLink msl with (nolock) on ms.MedServiceType_id = 6
	//					and msl.MedService_lid = ms.MedService_id
	//					and msl.MedServiceLinkType_id = 1
	//				left join v_MedService pzm with (nolock) on ms.MedServiceType_id = 6
	...
	//				left join v_UslugaComplexMedService ucpzm with (nolock) on ms.MedServiceType_id = 6
	//					and ucpzm.MedService_id = pzm.MedService_id
	//					and ucms.UslugaComplex_id = ucpzm.UslugaComplex_id
	//				left join v_MedServiceType mst with (nolock) on mst.MedServiceType_id = ms.MedServiceType_id--isnull(pzm.MedServiceType_id, ms.MedServiceType_id) =
	//				left join v_Lpu lpu with (nolock) on lpu.Lpu_id = ms.Lpu_id --isnull(pzm.Lpu_id, ms.Lpu_id)
	//				left join v_LpuUnit lu with (nolock) on lu.LpuUnit_id = isnull(pzm.LpuUnit_id, ms.LpuUnit_id)
	//				left join v_LpuSection ls with (nolock) on ms.LpuSection_id = ls.LpuSection_id
	...
	//								and t2.UslugaComplexAttributeType_SysNick like 'lab'
	//						) then 6 else 0 end
	...
	//			$filter .= ' and (uc.UslugaComplex_Code like :ucQuery or uc.UslugaComplex_Name like :ucQuery)';
	...
	//				UC.UslugaComplex_Name
	//			from v_UslugaComplexMedService UCMS with (nolock)
	//			left join v_UslugaComplexMedService UCCMS with (nolock) on UCCMS.UslugaComplexMedService_pid = UCMS.UslugaComplexMedService_id
	//			left join v_UslugaComplexComposition UCC with (nolock)on UCCMS.UslugaComplex_id is null and UslugaComplex_pid = UCMS.UslugaComplex_id
	//			inner join v_UslugaComplex UC with (nolock) on isnull(UCCMS.UslugaComplex_id, UCC.UslugaComplex_id) = UC.UslugaComplex_id
