...
class EvnLabSample_model extends Collection_model
{
	...
	function takeLabSample($data)
	{
		...
		$this->db->query("
			update
				e with (rowlock)
			set
				e.Evn_setDT = dbo.tzGetDate()
			from
				Evn e
				inner join EvnLabSample els (nolock) on els.Evn_id = e.Evn_id
			where
		...
		$EvnLabSample_DelivDTField = "@currTime";
		...
		if (!empty($UslugaComplexTargetIds)) {
			$resp_rs = $this->queryResult("
				...
				from
					...
					left join v_UslugaComplexMedService ucms_child with(nolock) on ucms_child.UslugaComplexMedService_pid  = ucms.UslugaComplexMedService_id
				where
		...
		$query = "
			...
			set @EvnLabSample_id = :EvnLabSample_id;
			set @currTime = dbo.tzGetDate();

			-- служба выполнения при вязатии пробы должна остаться выбранная
			select
			...
			exec p_EvnLabSample_upd
				@EvnLabSample_id = @EvnLabSample_id output,
			...
			select @EvnLabSample_id as EvnLabSample_id, convert(varchar(20), @currTime, 120) as EvnLabSample_setDTForBD, convert(varchar(5), @currTime, 108) + ' ' + convert(varchar(10), @currTime, 104) as EvnLabSample_setDT, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
		";
		...
		if (is_object($result)) {
			...
			if (!empty($resp[0]['EvnLabSample_id'])) {
				...
				$EvnLabSampleNumCheck = $this->getFirstResultFromQuery("
					...
					from
						EvnLabSample els with(nolock)
						inner join v_Evn e with(nolock) on e.Evn_id = els.Evn_id
	...
	function saveLabSample($data)
	{
		...
		$query = "
			...
			set @EvnLabSample_id = null;
			set @currTime = dbo.tzGetDate();

			exec p_EvnLabSample_ins
				@EvnLabSample_id = @EvnLabSample_id output,
				@EvnLabRequest_id = :EvnLabRequest_id,
				@RefSample_id = :RefSample_id,
				@Lpu_id = :Lpu_id,
			...
			select @EvnLabSample_id as EvnLabSample_id, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
		";
	...
	function saveQcSampleTest($data) {
		...
		$query = "
			declare
				@UslugaTest_id bigint,
				@Error_Msg varchar(4000),
				@Error_Code bigint;
			exec p_UslugaTest_ins
		...
		foreach($EvnUslugaPars as $EvnUslugaPar) {
			...
			if (is_array($tests)) {
				...
				if (count($tests) > 0 && !empty($data['Analyzer_id'])) {
					...
					$res = $this->EvnPrescr_model->getPrescrByDirection($lrdata);
					if (!is_array($res) || empty($res)) {
						throw new Exception('Ошибка при поиске назначения');
					}
					...
					$person = $this->Person_model->getPersonDataForRefValues($dt);
		...
		if (!empty($data['EvnLabSamples'])) {
			...
			foreach($arrayId as $id) {
				...
				if ($data['MedService_IsQualityTestApprove'] == 1) {
					...
					$join = "inner join v_EvnUslugaPar eup (nolock) on eup.EvnUslugaPar_id = ut.UslugaTest_rid
						inner join v_EvnLabSample els (nolock) on els.EvnLabSample_id = eup.EvnLabSample_id
						inner join UslugaComplexMedService ucms (nolock) on ucms.UslugaComplex_id = ut.UslugaComplex_id and ucms.MedService_id = :MedService_id
						inner join lis.AnalyzerTest at (nolock) on at.UslugaComplexMedService_id = ucms.UslugaComplexMedService_id and at.Analyzer_id = els.Analyzer_id
					";
		...
		foreach($EvnUslugaPars as $EvnUslugaPar) {
			...
			if (!empty($test)) {
				if (!empty($EvnUslugaPar['EvnPrescr_id'])) {
					$this->load->model('EvnPrescr_model');
					$res = $this->EvnPrescr_model->execEvnPrescr([
						'EvnPrescr_id' => $EvnUslugaPar['EvnPrescr_id'],
						'pmUser_id' => $data['pmUser_id']
					]);
					if (!$this->isSuccessful($res)) {
						throw new Exception('Ошибка при выполнении назначения', 400);
					}
				}
				...
				if (!empty($EvnUslugaPar['EvnPrescr_id'])) {
					$this->load->model('EvnPrescr_model');
					$res = $this->EvnPrescr_model->rollbackEvnPrescrExecution([
						'EvnPrescr_id' => $EvnUslugaPar['EvnPrescr_id'],
						'pmUser_id' => $data['pmUser_id']
					]);
					if (!$this->isSuccessful($res)) {
				...
				$res = $this->EvnXmlBase_model->deleteByEvn([
	...
	function getRefValuesForQualitativeTest($AnalyzerTestRefValues_id)
	{
		...
		$query = "
			...
			from
				...
				inner join lis.v_QualitativeTestAnswerAnalyzerTest qtaat with(nolock) on qtaat.QualitativeTestAnswerAnalyzerTest_id = qtarv.QualitativeTestAnswerAnalyzerTest_id
			where
	...
	function loadRefValues($data)
	{
		...
		if (!array_key_exists('EvnPrescr_id', $data)) {
			$this->load->model('EvnPrescr_model');
			$res = $this->EvnPrescr_model->getPrescrByDirection($data);
			if (!is_array($res) || empty($res)) {
				return $this->createError('','Ошибка при поиске назначения');
			}
			$data['EvnPrescr_id'] = $res[0]['EvnPrescr_id'];
		}
		...
		if (!array_key_exists('PersonData', $data)) {
			$this->load->model('Person_model');
			$person = $this->Person_model->getPersonDataForRefValues($data);
			if (!is_array($person) || empty($person)) {
				return $this->createError('','Ошибка при поиске человека');
			}
		...
		if (is_array($UslugaComplex_ids) && count($UslugaComplex_ids) > 0) {
			...
			$query = "
				...
				from
					...
					left join lis.v_AnalyzerTestRefValues atrv (nolock) on atrv.AnalyzerTest_id = at.AnalyzerTest_id
					left join v_RefValues rv (nolock) on rv.RefValues_id = atrv.RefValues_id
	...
	function saveEvnLabSampleDefect($data)
	{
		...
		if ($this->regionNick == 'vologda' && $data['MedServiceType_SysNick'] == 'pzm') {
			$query = "
				...
				left join v_MedService ms with(nolock) on ms.MedService_id = els.MedService_did
				where
		...
		$query = "
			...
			FROM
				EvnLabSample els
				inner join v_EvnLabSample (nolock) vels on vels.EvnLabSample_id = els.EvnLabSample_id
			WHERE
				vels.EvnLabSample_id = :EvnLabSample_id and vels.Lpu_id = :Lpu_id
		";
	...
	function load($field = null, $value = null, $selectFields = '*', $addNameEntries = true)
	{
		...
		$query = "
			select
				...
				convert(varchar(10), t.EvnLabSample_setDT, 104)+' '+convert(varchar(5), t.EvnLabSample_setDT, 108) as \"EvnLabSample_setDT\",
				convert(varchar(10), t.EvnLabSample_disDT, 104)+' '+convert(varchar(5), t.EvnLabSample_disDT, 108) as \"EvnLabSample_disDT\",
				convert(varchar(10), t.EvnLabSample_insDT, 104)+' '+convert(varchar(5), t.EvnLabSample_insDT, 108) as \"EvnLabSample_insDT\",
				convert(varchar(10), t.EvnLabSample_updDT, 104)+' '+convert(varchar(5), t.EvnLabSample_updDT, 108) as \"EvnLabSample_updDT\",
				convert(varchar(10), t.EvnLabSample_signDT, 104)+' '+convert(varchar(5), t.EvnLabSample_signDT, 108) as \"EvnLabSample_signDT\",
				convert(varchar(10), t.EvnLabSample_DelivDT, 104)+' '+convert(varchar(5), t.EvnLabSample_DelivDT, 108) as \"EvnLabSample_DelivDT\",
				convert(varchar(10), t.EvnLabSample_StudyDT, 104)+' '+convert(varchar(5), t.EvnLabSample_StudyDT, 108) as \"EvnLabSample_StudyDT\",
			...
			from
				dbo.v_EvnLabSample t with(nolock)
				LEFT JOIN v_Lpu Lpu_id_ref with(nolock) on Lpu_id_ref.Lpu_id = t.Lpu_id
				LEFT JOIN v_Morbus Morbus_id_ref with(nolock) on Morbus_id_ref.Morbus_id = t.Morbus_id
				LEFT JOIN v_YesNo EvnLabSample_IsSigned_ref with(nolock) on EvnLabSample_IsSigned_ref.YesNo_id = t.EvnLabSample_IsSigned
				LEFT JOIN v_RefSample RefSample_id_ref with(nolock) on RefSample_id_ref.RefSample_id = t.RefSample_id
				LEFT JOIN v_Lpu Lpu_did_ref with(nolock) on Lpu_did_ref.Lpu_id = t.Lpu_did
				LEFT JOIN v_LpuSection LpuSection_did_ref with(nolock) on LpuSection_did_ref.LpuSection_id = t.LpuSection_did
				LEFT JOIN v_Lpu Lpu_aid_ref with(nolock) on Lpu_aid_ref.Lpu_id = t.Lpu_aid
				LEFT JOIN v_LpuSection LpuSection_aid_ref with(nolock) on LpuSection_aid_ref.LpuSection_id = t.LpuSection_aid
				LEFT JOIN v_LabSampleDefectiveType LabSampleDefectiveType_id_ref with(nolock) on LabSampleDefectiveType_id_ref.LabSampleDefectiveType_id = t.LabSampleDefectiveType_id
				LEFT JOIN  dbo.v_RefSample s with(nolock) on s.RefSample_id = t.RefSample_id
				LEFT JOIN  dbo.v_RefMaterial r with(nolock) on s.RefMaterial_id = r.RefMaterial_id
			where
				EvnLabSample_id = :EvnLabSample_id
		";
	...
	*/
	...
	function getRequest2Data($data){
		...
		$UslugaComplex_id = $this->getFirstResultFromQuery("
			...
			from
				v_EvnLabSample els with(nolock)
				inner join v_EvnLabRequestUslugaComplex elruc with(nolock) on elruc.EvnLabRequest_id = els.EvnLabRequest_id
			where
				els.EvnLabSample_id = :EvnLabSample_id
		", array(
			'EvnLabSample_id' => $data['EvnLabSample_id']
		...
		$query = "
			select
				...
				datepart(year,ps.Person_BirthDay) as \"BirthDay_Year\",
				datepart(month,ps.Person_BirthDay) as \"BirthDay_Month\",
				datepart(day,ps.Person_BirthDay) as \"BirthDay_Day\",
			...
			from
				v_EvnLabSample EvnLabSample with(nolock)
				left join v_PersonState ps with(nolock) on ps.Person_id = EvnLabSample.Person_id
				...
				left join v_EvnLabRequest lr with(nolock) on lr.EvnLabRequest_id = EvnLabSample.EvnLabRequest_id
				left join v_evnUslugaPar eup with(nolock) on eup.EvnDirection_id = lr.EvnDirection_id
				...
				-- подразделение
				--left join lis.DepartamentLink dl with(nolock) on dl.MedService_id = lr.MedService_id
				--left join lis.departament d with(nolock) on d.departament_id = dl.departament_id
				-- todo: таких услуг может быть несколько в одной лаборатории, может быть нужно уточнить по подразделению
				outer apply (
					Select top 1 id as target_id
					from lis._target target with(nolock)
					where target.code = uc2011.UslugaComplex_Code
				) target
				-- для получения биоматериала
				-- LEFT JOIN v_UslugaComplexMedService us with(nolock) on us.UslugaComplex_id = u.UslugaComplex_id and us.MedService_id = lr.MedService_id
				LEFT JOIN dbo.v_RefSample r with(nolock) on r.RefSample_id = EvnLabSample.RefSample_id
				-- todo: здесь тоже надо делать через стыковочную таблицу, но сейчас id в промед и в ЛИС совпадают
				LEFT JOIN dbo.v_RefMaterial rm with(nolock) on r.RefMaterial_id = rm.RefMaterial_id
				outer apply (
					select (
						select distinct
							t.id + ',' as 'data()'
						from
							v_UslugaTest UslugaTest (nolock)
							INNER JOIN v_UslugaComplex uc (nolock) ON UslugaTest.UslugaComplex_id = uc.UslugaComplex_id
							inner join v_UslugaComplexMedService ucms (nolock) on ucms.UslugaComplex_id = uc.UslugaComplex_id and ucms.MedService_id = lr.MedService_id -- услуга на службе
							inner join lis.v_AnalyzerTest at_child (nolock) on at_child.UslugaComplexMedService_id = ucms.UslugaComplexMedService_id -- тест
							inner join lis.v_AnalyzerTest at (nolock) on at.AnalyzerTest_id = at_child.AnalyzerTest_pid -- исследование
							inner join lis.v_Analyzer a (nolock) on a.Analyzer_id = at.Analyzer_id and (a.Analyzer_Code <> '000' OR a.pmUser_insID <> 1) -- анализатор, только не с ручных методик
							INNER JOIN dbo.v_UslugaComplex ucgost (nolock) ON ucgost.UslugaComplex_id = uc.UslugaComplex_2011id --and uc.UslugaComplex_id != uc.UslugaComplex_2011id
							outer apply (
								Select id
	...
	function getRequest2DataForAsMlo($data) {
		$query = "
			...
			from
				v_EvnLabSample els with(nolock)
				left join v_PersonState ps with(nolock) on ps.Person_id = els.Person_id
				inner join v_EvnLabRequest elr with(nolock) on elr.EvnLabRequest_id = els.EvnLabRequest_id
				...
				left join v_Lpu l with(nolock) on l.Lpu_id = elr.Lpu_id
				left join v_RefSample r with(nolock) on r.RefSample_id = els.RefSample_id
				left join v_RefMaterial rm with(nolock) on r.RefMaterial_id = rm.RefMaterial_id
			...
			if (!empty($resp[0]['EvnLabSample_id'])) {
				...
				$query = "
					...
					from v_UslugaTest ut with(nolock)
						...
						left join v_UslugaComplex uc with(nolock) ON ut.UslugaComplex_id = uc.UslugaComplex_id -- без вьюхи т.к. выбрать значение другого региона все равно не возможно
						left join v_UslugaComplex ucp with(nolock) ON eupp.UslugaComplex_id = ucp.UslugaComplex_id
						left join v_UslugaComplexMedService ucms with(nolock) on ucms.UslugaComplex_id = uc.UslugaComplex_id -- услуга на службе
						left join lis.v_AnalyzerTest at_child with(nolock) on at_child.UslugaComplexMedService_id = ucms.UslugaComplexMedService_id -- тест
						left join lis.v_AnalyzerTest at with(nolock) on at.AnalyzerTest_id = at_child.AnalyzerTest_pid -- исследование
						left join lis.v_Analyzer a with(nolock) on a.Analyzer_id = coalesce(at.Analyzer_id, at_child.Analyzer_id) -- анализатор, только не с ручных методик
						--left join v_UslugaComplex ucgost with(nolock) ON ucgost.UslugaComplex_id = uc.UslugaComplex_2011id
					  	outer apply (
							Select top 1 UslugaComplex_Code from v_UslugaComplex ucgost with(nolock) where ucgost.UslugaComplex_id = uc.UslugaComplex_2011id
						) ucgost
						--left join v_UslugaComplex ucpgost ON ucpgost.UslugaComplex_id = ucp.UslugaComplex_2011id
						outer apply(
							Select top 1 UslugaComplex_Code from v_UslugaComplex ucpgost with(nolock) where ucpgost.UslugaComplex_id = ucp.UslugaComplex_2011id
						) ucpgost
					where
						ut.EvnLabSample_id = :EvnLabSample_id
						and ucms.MedService_id = :MedService_id
	...
	function loadEvnLabSampleListForWorksheet($data) {
		$query = "
			...
			FROM
				-- from
				v_EvnLabSample els with(nolock)
				inner join lis.v_AnalyzerWorksheetEvnLabSample awels with(nolock) on els.EvnLabSample_id = awels.EvnLabSample_id
				left join v_PersonState ps with(nolock) on ps.Person_id = els.Person_id
				-- end from
			WHERE
				-- where
	...
	{
		$query = "
           declare @EvnLabSample_Test varchar(max);

			set @EvnLabSample_Test = (
				select top 1
					substring(Tests.EvnLabSample_Nums, 1, len(Tests.EvnLabSample_Nums)-1)
				from
					v_EvnLabSample els (nolock)
					outer apply (
						Select (select
							uc.UslugaComplex_Name + ', ' as 'data()'
						from v_UslugaComplex uc (nolock)
							inner join v_UslugaTest ut (nolock) on ut.UslugaComplex_id = uc.UslugaComplex_id
			...
			update
				EvnLabSample with (rowlock)
			set
				EvnLabSample_Test = @EvnLabSample_Test
			where
				EvnLabSample_id = :EvnLabSample_id
		";
	...
	function ReCacheLabSampleStatus($data)
	{
		...
		if (!empty($data['MedServiceType_SysNick']) && $data['MedServiceType_SysNick'] == 'microbiolab') {
			if (!empty($data['action']) && $data['action'] == 'approve') {
				$join .= 'left join v_BactMicroProbe (nolock) bmp on bmp.UslugaTest_id = ut.UslugaTest_id
							left join v_BactMicroProbeAntibiotic (nolock) bmpa on bmpa.UslugaTest_id = ut.UslugaTest_id';
				$whereClause .= ' and (bmp.BactMicroProbe_id is not null or bmpa.BactMicroProbeAntibiotic_id is not null)';
			} else {
				$join .= " inner join v_BactMicroProbe (nolock) bmp on bmp.UslugaTest_id = ut.UslugaTest_id";
			}
		}
		$query = "
			declare
				@LabSampleStatus_id bigint,
				@EvnLabSample_StudyDT datetime;

			set @LabSampleStatus_id = (
				select top 1
					CASE
						...
						WHEN a.Analyzer_Code = '000' and a.pmUser_insID = 1 and ISNULL(mstdid.MedServiceType_SysNick,'lab') <> 'pzm' then 7
						-- 1. Новые
						ELSE 1
					END as LabSampleStatus_id
				from
					v_EvnLabSample els (nolock)
					left join v_MedService msdid (nolock) on msdid.MedService_id = els.MedService_did
					left join v_MedServiceType mstdid (nolock) on mstdid.MedServiceType_id = msdid.MedServiceType_id
					left join lis.v_Analyzer a (nolock) on a.Analyzer_id = els.Analyzer_id
					outer apply(
						select top 1
							Link_id
						from
					...
					outer apply (
						select
							...
							max(case when ISNULL(ut.UslugaTest_ResultApproved, 1) = 1 then 1 else 0 end) as NOTAPP_EvnUslugaPar_id,
						...
						from v_UslugaTest ut (nolock)
						{$join}
						where ut.EvnLabSample_id = els.EvnLabSample_id and ut.EvnDirection_id is null -- только тесты
						{$whereClause}
					) EUP
				where
					els.EvnLabSample_id = :EvnLabSample_id
			...
			set @EvnLabSample_StudyDT = (select EvnLabSample_StudyDT from v_EvnLabSample (nolock) where EvnLabSample_id = :EvnLabSample_id);

			if (@LabSampleStatus_id IN (1,2,7))
			begin
				set @EvnLabSample_StudyDT = null;
			end

			if (@LabSampleStatus_id IN (3,4,6) and @EvnLabSample_StudyDT is null)
			begin
			...
			update
				EvnLabSample with (rowlock)
			set
				LabSampleStatus_id = @LabSampleStatus_id,
				EvnLabSample_StudyDT = @EvnLabSample_StudyDT
			where
				EvnLabSample_id = :EvnLabSample_id
		";
	...
	function getIFAEvnLabSamples($data) {
		...
		$query = "
			...
			inner join lis.v_AnalyzerTest AnT with(nolock) on AnT.UslugaComplexMedService_id = ucms.UslugaComplexMedService_id
			inner join lis.v_Analyzer A with(nolock) on A.Analyzer_id = AnT.Analyzer_id and A.MedService_id = ELR.MedService_id
			inner join v_MethodsIFAAnalyzerTest MIAT with(nolock) on MIAT.AnalyzerTest_id = AnT.AnalyzerTest_id
			inner join v_MethodsIFA MI with(nolock) on MI.MethodsIFA_id = MIAT.MethodsIFA_id
			where {$where}
		";
	...
	function loadWorkList($data){
		try {
			...
			if (getRegionNick() == 'kz') {
				...
				";
				$bdzJoin = "
					inner join v_Person pers with (nolock) on pers.Person_id = table1.Person_id
				";
			}
			...
			$query = "
				with tempTable as (
					...
					FROM
						dbo.v_EvnLabSample els with(nolock)
						inner join v_EvnLabRequest elr with(nolock) on els.EvnLabRequest_id = elr.EvnLabRequest_id
						...
						left join v_RefSample rs with(nolock) on rs.RefSample_id = els.RefSample_id
						left join v_RefMaterial rm with(nolock) on rm.RefMaterial_id = rs.RefMaterial_id
						--left join lis.v_Link link with(nolock) on link.object_id = els.EvnLabSample_id and link.link_object = 'EvnLabSample'
						outer apply (
							Select top 1
								link.lis_id
							from lis.v_Link link with(nolock)
							where link.object_id = els.EvnLabSample_id and link.link_object = 'EvnLabSample'
						) link
						left join lis.v_Analyzer a with(nolock) on a.Analyzer_id = els.Analyzer_id
						outer apply(
							select top 1
								AnalyzerWorksheetEvnLabSample_id
							from
								lis.v_AnalyzerWorksheetEvnLabSample with(nolock)
							where
	...
	function getSampleUsluga($data) {
		...
		foreach ($EvnLabSample_id as $key => $value) {
			if (empty($value)) {
				unset($EvnLabSample_id[$key]);
			}
		}
		...
		$query = "
			...
			From
				v_EvnLabSample els with(nolock)
				inner join v_UslugaTest ut with(nolock) on ut.EvnLabSample_id = els.EvnLabSample_id
				inner join v_EvnUslugaPar eup with(nolock) on ut.UslugaTest_pid = eup.EvnUslugaPar_id and eup.EvnDirection_id is not null
				left join v_UslugaComplex ucp with(nolock) on ucp.UslugaComplex_id = eup.UslugaComplex_id
				outer apply(
					select top 1
					...
					from
						...
						left join lis.v_AnalyzerTest at with(nolock) on at.AnalyzerTest_id = COALESCE(at_child.AnalyzerTest_pid, at.AnalyzerTest_id) -- родительское исследование, если есть
					...
					where
						...
						and (at.AnalyzerTest_endDT >= (select curdate from myvars with(nolock)) or at.AnalyzerTest_endDT is null)
						and (at_child.AnalyzerTest_endDT >= (select curdate from myvars with(nolock)) or at_child.AnalyzerTest_endDT is null)
						and (uctest.UslugaComplex_endDT >= (select curdate from myvars with(nolock)) or uctest.UslugaComplex_endDT is null)
					order by
		...
		try {
			...
			$query = "
				...
				FROM
					v_EvnLabSample els with(nolock)
					inner join v_EvnLabRequest elr with(nolock) on els.EvnLabRequest_id = elr.EvnLabRequest_id
					left join lis.v_DefectCauseType dct with(nolock) on dct.DefectCauseType_id = els.DefectCauseType_id
					left join v_EvnDirection_all ed with(nolock) on ed.EvnDirection_id = elr.EvnDirection_id
					left join v_LabSampleStatus lss with(nolock) on lss.LabSampleStatus_id = els.LabSampleStatus_id
					left join v_RefSample rs with(nolock) on rs.RefSample_id = els.RefSample_id
					left join v_RefMaterial rm with(nolock) on rm.RefMaterial_id = rs.RefMaterial_id
	...
	function checkTestCanBeCanceled($data) {
		$query = "
			...
			From
				...
				LEFT JOIN v_UslugaTest ut with(nolock) ON ut.EvnLabSample_id = ls.EvnLabSample_id
			where
	...
	function updateResult($data) {
		...
		if (count($resp) > 0) {
			...
			$query = "
				...
				set @updDT = {$updDate};
				set @chkDT = {$chkDate};
				EXEC dbo.p_UslugaTest_upd
					@UslugaTest_id = @UslugaTest_id  output, -- bigint
					@UslugaTest_pid = :UslugaTest_pid,       -- bigint
					@UslugaTest_rid = :UslugaTest_rid,       -- bigint
					@UslugaTest_setDT = @updDT ,   -- date
		...
		if (!empty($data['EvnUslugaParChanged'])) {
			foreach($data['EvnUslugaParChanged'] as $EvnUslugaPar_id) {
				$resp_eup = $this->queryResult("
					...
					from
						v_EvnUslugaPar eupp with(nolock)
						inner join v_EvnLabRequest elr with(nolock) on elr.EvnDirection_id = eupp.EvnDirection_id
						left join v_UslugaTest ut with(nolock) on ut.UslugaTest_pid = eupp.EvnUslugaPar_id and ut.UslugaTest_ResultApproved = 2
					 	left join v_Evn e with(nolock) on e.Evn_id = eupp.EvnUslugaPar_pid
					where
				...
				if (!empty($resp_eup[0]['EvnUslugaPar_pid'])) {
					...
					$res = $this->EvnMediaFiles_model->getEvnMediaByEvn([
						'Evn_id' => $EvnUslugaPar_id
					]);
					if (!is_array($res)) {
						return $this->createError('','Ошибка при получении файла события');
					}

					$resp_eup[0]['EvnMediaData_id'] = count($res) > 0 ? $res[0]['EvnMediaData_id'] : null;
					...
					if (!empty($resp_eup[0]['UslugaTest_id']) || !empty($resp_eup[0]['EvnMediaData_id'])) {
						...
						// оперделяем Evn_pid
						$this->load->model('EvnPrescr_model');
						$uslugaParams = $this->EvnPrescr_model->defineUslugaParams($dt);
					...
					if (!empty($data['session']['medpersonal_id'])) {
						...
						$resp_msf = $this->MedStaffFact_model->getMSFData($params);
						if (!is_array($resp_msf)) {
							return $this->createError('','Ошибка при получении данных рабочего места врача');
						}

						$data['MedStaffFact_id'] = null;
						if (count($resp_msf) > 0 && !empty($resp_msf[0]['MedStaffFact_id'])) {
			...
			foreach($resp_elruc as $one_elruc) {
				$query = "
					declare
						@ErrCode int,
						@ErrMessage varchar(4000);
					exec p_EvnLabRequestUslugaComplex_del
	...
	function getLabSampleResultGrid($data) {
		...
		if (empty($data['ingorePrescr'])) {
			...
			$EvnLabRequestUslugaComplex_id = $this->getFirstResultFromQuery("
				...
				from
					v_EvnLabRequestUslugaComplex elruc with(nolock)
					inner join v_EvnLabRequest elr with(nolock) on elr.EvnLabRequest_id = elruc.EvnLabRequest_id
				where
			...
			if (!empty($EvnLabRequestUslugaComplex_id)) {
				...
				$query = "
					...
					from
						v_EvnDirectionUslugaComplex educ with(nolock)
					where
		...
		$resp = $this->queryResult("
			...
			from
				v_UslugaTest ut with(nolock)
				left join v_BactMicroProbe (nolock) bmp on bmp.UslugaTest_id = ut.UslugaTest_id
				left join v_BactMicroProbeAntibiotic (nolock) bmpa on bmpa.UslugaTest_id = ut.UslugaTest_id
				{$firstJoin}
			where
				{$firstWhere}
		...
		$resp = $this->queryResult("
			...
			where
				...
				and not exists(
					select top 1 elruc.EvnUslugaPar_id
		...
		$query = "
			...
			Select
				...
				ut.UslugaTest_ResultApproved as \"UslugaTest_ResultApproved\",
				convert(varchar(5), ut.UslugaTest_CheckDT, 108)+' '+convert(varchar(10), ut.UslugaTest_CheckDT, 104) as \"UslugaTest_CheckDT\",
			...
			From
				v_EvnLabSample els with(nolock)
				inner join v_UslugaTest ut with(nolock) on ut.UslugaTest_id in (" . implode(',', $evnUslugaParBySampleList[1]) . ")
				inner join v_EvnUslugaPar eup with(nolock) on ut.UslugaTest_pid = eup.EvnUslugaPar_id and eup.EvnDirection_id is not null
				left join v_UslugaComplex ucp with(nolock) on ucp.UslugaComplex_id = eup.UslugaComplex_id
				left join v_UslugaComplex uc with(nolock) on ut.UslugaComplex_id = uc.UslugaComplex_id
				...
				outer apply(
					...
					from
						lis.v_AnalyzerTest at_child with(nolock)
						inner join lis.v_Analyzer a with(nolock) on a.Analyzer_id = at_child.Analyzer_id
						inner join v_UslugaComplexMedService ucms_at with(nolock) on at_child.UslugaComplexMedService_id = ucms_at.UslugaComplexMedService_id
						left join v_UslugaComplexMedService ucms_parent with(nolock) on ucms_parent.UslugaComplexMedService_id = coalesce(ucms_at.UslugaComplexMedService_pid, ucms_at.UslugaComplexMedService_id)
						left join v_UslugaComplex uctest with(nolock) on at_child.UslugaComplex_id = uctest.UslugaComplex_id
					where
				...
				outer apply(
					...
					from
						...
						inner join lis.v_AnalyzerTest at with(nolock) on at.Analyzer_id = a.Analyzer_id
			...
			From
				v_EvnLabSample els with(nolock)
				inner join v_EvnUslugaPar eup with(nolock) on eup.EvnUslugaPar_id in (" . implode(',', $evnUslugaParBySampleList[2]) . ") and eup.EvnDirection_id is not null
				left join v_UslugaComplex ucp with(nolock) on ucp.UslugaComplex_id = eup.UslugaComplex_id
				left join v_UslugaComplexMedService ucms_usluga_parent with(nolock) on eup.UslugaComplex_id = ucms_usluga_parent.UslugaComplex_id and ucms_usluga_parent.MedService_id = els.MedService_id and ucms_usluga_parent.UslugaComplexMedService_pid is null -- исследование
				left join v_UslugaComplexMedService ucms_usluga with(nolock) on ucms_usluga.UslugaComplexMedService_pid = ucms_usluga_parent.UslugaComplexMedService_id
				left join v_UslugaComplex uc with(nolock) on COALESCE(ucms_usluga.UslugaComplex_id, ucp.UslugaComplex_id, ucms_usluga_parent.UslugaComplex_id) = uc.UslugaComplex_id
				left join v_UslugaTest ut with(nolock) on ut.UslugaTest_pid = eup.EvnUslugaPar_id and ut.UslugaComplex_id = uc.UslugaComplex_id and ut.EvnLabSample_id = els.EvnLabSample_id -- Результат по пробе
				left join v_RefValues rv with(nolock) on rv.RefValues_id = ut.RefValues_id
				outer apply(
					select top 1
						at_child.AnalyzerTest_id,
					...
					from
						lis.v_AnalyzerTest at_child with(nolock)
						left join lis.v_AnalyzerTest at with(nolock) on at.AnalyzerTest_id = at_child.AnalyzerTest_pid
						inner join lis.v_Analyzer a with(nolock) on a.Analyzer_id = COALESCE(at.Analyzer_id, at_child.Analyzer_id)
						left join v_UslugaComplex uctest with(nolock) on uctest.UslugaComplex_id = at_child.UslugaComplex_id
					where
				...
				and (
					...
					or exists (
						select
							*
						from
							v_EvnLabRequestUslugaComplex elruc_child with(nolock)
						where
	...
	function loadList($data) {
		$sql = "
			SELECT
				...
				convert(varchar(10), EvnLabSample_setDT, 104) as \"EvnLabSample_setDT\",
				convert(varchar(10), EvnLabSample_disDT, 104) as \"EvnLabSample_disDT\",
				convert(varchar(10), EvnLabSample_didDT, 104) as \"EvnLabSample_didDT\",
				convert(varchar(10), EvnLabSample_insDT, 104) as \"EvnLabSample_insDT\",
				convert(varchar(10), EvnLabSample_updDT, 104) as \"EvnLabSample_updDT\",
				...
				convert(varchar(10), EvnLabSample_signDT, 104) as \"EvnLabSample_signDT\",
				EvnLabRequest_id as \"EvnLabRequest_id\",
				EvnLabSample_Num as \"EvnLabSample_Num\",
				...
				convert(varchar(10), EvnLabSample_DelivDT, 104) as \"EvnLabSample_DelivDT\",
				Lpu_aid as \"Lpu_aid\",
				LpuSection_aid as \"LpuSection_aid\",
				MedPersonal_aid as \"MedPersonal_aid\",
				MedPersonal_said as \"MedPersonal_said\",
				convert(varchar(10), EvnLabSample_StudyDT, 104) as \"EvnLabSample_StudyDT\",
	...
	function getEvnUslugasRoot($data) {
		...
		$query = "
			SELECT distinct
				...
				convert(varchar(10), eu.EvnUslugaPar_setDT, 120) as \"EvnUslugaPar_setDT\",
			...
			FROM
				...
				inner join v_EvnUslugaPar eu with(nolock) on eu.EvnDirection_id = elr.EvnDirection_id
			WHERE
				1=1
				and eu.EvnDirection_id is not null
				{$filter}
	...
	function getLabSampleTestsForChangeAnalyzerValues($data) {
		$query = "
			...
			from
				v_UslugaTest ut with(nolock)
				left join v_EvnUslugaPar eupp with(nolock) on eupp.EvnUslugaPar_id = ut.UslugaTest_pid
				left join lis.v_AnalyzerTestRefValues atrv with(nolock) on atrv.RefValues_id = ut.RefValues_id
				left join lis.v_AnalyzerTest at with(nolock) on at.AnalyzerTest_id = atrv.AnalyzerTest_id
			where
				ut.EvnLabSample_id = :EvnLabSample_id
				and ut.EvnDirection_id is null
		";
	...
	function saveLabSampleAnalyzer($data) {
		...
		if (count($tests) > 0 && !empty($data['Analyzer_id'])) {
			...
			$this->load->model('Person_model');
			$person = $this->Person_model->getPersonDataForRefValues($pars);
			$data['PersonData'] = $person;
		}
	...
	function approveResults($data) {
		...
		$query = "
			...
			where
				...
				and ISNULL(ut.UslugaTest_ResultApproved, 1) = 1
				{$filter}
		";
		...
		if ($data['MedServiceType_SysNick'] == 'microbiolab' && !empty($UslugaTest_ids)) {
			$query_uta = "with UT as (
					...
					from dbo.v_UslugaTest (nolock)
					inner join dbo.v_EvnLabSample (nolock) on v_EvnLabSample.EvnLabSample_id = v_UslugaTest.EvnLabSample_id
		...
		foreach($resp_eup as $respone) {
			...
			$query2 = "
				...
				from v_UslugaTest ut with(nolock)
					...
					left join v_EvnLabRequest t with(nolock) on t.EvnLabRequest_id = ls.EvnLabRequest_id
					left join v_EvnDirection_all d with(nolock) on d.EvnDirection_id = t.EvnDirection_id
...
/*		foreach($EvnUslugaParChanged as $UslugaTest_pid) {
	...
	function getDataFromEvnLabRequest($data) {
		...
		if (!empty($data['EvnLabRequest_id'])) {
			...
			$join .= "inner join v_EvnLabSample els with(nolock) on els.EvnLabRequest_id = elr.EvnLabRequest_id";
			$filter .= " and els.EvnLabSample_id = :EvnLabSample_id";
			$queryParams['EvnLabSample_id'] = $data['EvnLabSample_id'];
		...
		$query = "
			...
			FROM
				v_EvnLabRequest elr with(nolock)
				{$join}
				inner join v_MedService ms with(nolock) on {$MedServiceJoin}
			WHERE
				(1=1)
				{$filter}
		";
	...
	function saveResearch($data) {
		...
		$this->load->model('EvnPrescr_model');
		$EvnPrescr_Date = $this->EvnPrescr_model->getSetDateByUslugaPar([
			'EvnUslugaPar_id' => $data['EvnUslugaPar_id']
		]);
	...
	function saveEvnUslugaDone($data) {
		$EvnUslugaParData = $this->queryResult("
			...
			from
				v_EvnUslugaPar eup with (nolock)
				left join v_EvnLabRequest elr with (nolock) on eup.EvnDirection_id = elr.EvnDirection_id
	...
	function loadResearchEditForm($data) {
        ...
        if (getRegionNick() === 'kz') {
            $sql_fields .= "
		        ...
		        ";
            $sql_joins .= "
		            LEFT JOIN r101.v_UslugaMedTypeLink UMTL with(nolock) ON UMTL.Evn_id=eup.EvnUslugaPar_id
		        ";
        }
		...
		$query = "
			select
				...
				eup.EvnDirection_id as \"EvnDirection_id\",
				convert(varchar(10), dbo.tzGetDate(), 104) as \"EvnUslugaPar_maxDate\",
				eup.EvnUslugaPar_Comment as \"EvnUslugaPar_Comment\",
			...
			from
				v_EvnUslugaPar eup with(nolock)
				left join v_EvnLabRequest elr with(nolock) on elr.EvnDirection_id = eup.EvnDirection_id
		...
		if (empty($resp[0]['EvnUslugaPar_setDate']) && !empty($resp[0]['EvnLabSample_id'])) {
		    ...
		    if (getRegionNick() === 'kz') {
		        ...
		        ";
		        $sql_joins .= "
		            LEFT JOIN r101.v_UslugaMedTypeLink UMTL with(nolock) ON UMTL.Evn_id=ELS.EvnLabSample_id
		        ";
            }
		...
		if (!empty($resp[0])) {
			...
			if (!empty($resp[0]['EvnDirection_id'])) {
				...
				$this->load->model('EvnPrescr_model');
				$res = $this->EvnPrescr_model->getEvnPrescrInsDate([
					'EvnDirection_id' => $resp[0]['EvnDirection_id']
				]);
	...
	function saveEvnUslugaData($data) {
		...
		if (empty($data['MedStaffFact_id']) && !empty($data['MedPersonal_id']) && !empty($data['LpuSection_uid'])) {
			$this->load->model('MedStaffFact_model');
			$MedStaffFact = $this->MedStaffFact_model->getMedStaffFactByParams([
				'MedPersonal_id' => $data['MedPersonal_id'],
				'LpuSection_id' => $data['LpuSection_uid']
			]);
			if (!is_array($MedStaffFact)) {
				throw new Exception('Ошибка при получении идентификатора рабочего места врача');
			}

			$data['MedStaffFact_id'] = (count($MedStaffFact) > 0) ?
				$MedStaffFact[0]['MedStaffFact_id'] : null;
		}
		...
		$query = "
			DECLARE
				@EvnUslugaPar_id bigint = :EvnUslugaPar_id,
				@Error_Code bigint,
				@Error_Message varchar(4000),
				@dt datetime = :EvnUslugaPar_setDT;
			EXEC dbo.p_EvnUslugaPar_{$ins_upd}
				@EvnUslugaPar_id = @EvnUslugaPar_id  output, -- bigint
				@EvnUslugaPar_pid = :EvnUslugaPar_pid,       -- bigint
				@EvnUslugaPar_rid = :EvnUslugaPar_pid,       -- bigint
				@EvnUslugaPar_setDT = :EvnUslugaPar_setDT,                   -- date
				@Lpu_id = :Lpu_id,                           -- bigint
				@MedPersonal_id = :MedPersonal_id,           -- bigint
				@MedPersonal_sid = :MedPersonal_sid,           -- bigint
				...
				@EvnUslugaPar_Result = :EvnUslugaPar_Result, -- bigint
				@RefValues_id = :RefValues_id,
				@Unit_id = :Unit_id,
				...
				if (isset($result[0])) {
                    ...
                    if (!empty($result[0]['EvnUslugaPar_id'])) {
						...
						$this->ApprovalList_model->deleteApprovalList(array(
							...
							'ApprovalList_ObjectId' => $result[0]['EvnUslugaPar_id']
						));
                    }
	...
	function saveUslugaTest($data) {
		...
		$query = "
		    DECLARE
				...
				@Error_Code bigint,
				@Error_Message varchar(4000),
				@dt datetime = :UslugaTest_setDT;
			EXEC dbo.p_UslugaTest_{$ins_upd}
				@UslugaTest_id = @UslugaTest_id  output, -- bigint
				@UslugaTest_pid = :UslugaTest_pid,       -- bigint
	...
	function loadListForCandiPicker($data) {
		...
		$q = "
			SELECT
				...
				elr.EvnLabRequest_BarCode as \"EvnLabRequest_BarCode\",
				convert(varchar(10), ves.EvnLabSample_setDT, 104) as \"EvnLabSample_setDT\"
			FROM
				v_EvnLabSample ves with(nolock)
				INNER JOIN dbo.v_EvnLabRequest elr with(nolock) on elr.EvnLabRequest_id = ves.EvnLabRequest_id
				INNER JOIN dbo.v_UslugaComplex uc with(nolock) on uc.UslugaComplex_id = elr.UslugaComplex_id
				INNER JOIN dbo.v_UslugaCategory ucat with(nolock) on ucat.UslugaCategory_id = uc.UslugaCategory_id
				LEFT JOIN dbo.v_RefSample RefSample_id_ref with(nolock) on RefSample_id_ref.RefSample_id = ves.RefSample_id
				LEFT JOIN dbo.v_RefMaterial rm with(nolock) on rm.RefMaterial_id = RefSample_id_ref.RefMaterial_id
				outer apply(
					select top 1
						m_child.UslugaComplexMedService_id
					from
						v_UslugaComplexMedService m with(nolock) -- комплексная услуга
						inner join v_UslugaComplexMedService m_child with(nolock) on m.UslugaComplexMedService_id = m_child.UslugaComplexMedService_pid
					where
						m.MedService_id = elr.MedService_id
						and m.UslugaComplex_id = elr.UslugaComplex_id
						and m_child.RefSample_id = ves.RefSample_id
						and not exists(select * from lis.v_AnalyzerTest with(nolock) where UslugaComplex_id = m_child.UslugaComplex_id and Analyzer_id = :Analyzer_id)
                ) UC_SOST_NOTINANALYZER
			WHERE
				{$where}
				uc.UslugaComplex_id IN (
					SELECT
	...
	function loadBarCode($data) {
        ...
        WHERE {$filter} and ucc.uslugacomplex_id IN (
            ...
            WHERE
                ...
                FROM
                    lis.v_AnalyzerWorksheet w with(nolock)
                    LEFT JOIN lis.v_AnalyzerWorksheetType wt with(nolock) ON w.AnalyzerWorksheetType_id = wt.AnalyzerWorksheetType_id
                    LEFT JOIN lis.v_AnalyzerTestWorksheetType twt with(nolock) ON wt.AnalyzerWorksheetType_id = twt.AnalyzerWorksheetType_id
                    INNER JOIN lis.v_AnalyzerTestUslugaComplex tu with(nolock) ON twt.AnalyzerTest_id = tu.AnalyzerTest_id
                    INNER JOIN dbo.v_UslugaComplex vuc with(nolock) ON tu.UslugaComplex_id = vuc.UslugaComplex_id
                WHERE
	...
	function isLabSampleTestInHole($EvnLabSample_id) {
		...
		$query = "
			...
			from v_EvnLabSample ELS with(nolock)
			inner join v_UslugaTest UT with(nolock) on UT.EvnLabSample_id = ELS.EvnLabSample_id
			inner join v_Hole H with(nolock) on UT.UslugaTest_id = H.UslugaTest_id
			inner join v_Tablet T with(nolock) on T.Tablet_id = H.Tablet_id
    ...
    function getInWorkSamples($data) {
        ...
        $query = "
	        ...
	        where
		        ...
		        and len(ELS.EvnLabSample_Num) = 12
    ...
    function getOverdueSamples($data) {
        ...
        $query = "
	        ...
	        where
		        LabSampleStatus_id in (1,2,3,7)
		        and  EvnLabSample_insDT <= (dbo.tzGetDate() - 30)
		        and len(EvnLabSample_Num) = 12
	...
	function getPersonBySample($data) {
		...
		$query = "
			declare @Person_id bigint = (select top 1 Person_id from v_EvnLabSample where EvnLabSample_id = :EvnLabSample_id);

			select top 1
				(Person_SurName + ' ' + Person_FirName + ' ' + Person_SecName) as Person_Fio,
				Sex_id,
				Person_Age
			from v_PersonState_all (nolock)
	...
	function loadResearchHistory($data) {
		...
		$query = "
			...
			select
				...
				convert(varchar, ut.UslugaTest_CheckDT, 104) as UslugaTest_CheckDT,
			...
			from v_UslugaTest ut (nolock)
			inner join v_Lpu lpu (nolock) on lpu.Lpu_id = ut.Lpu_id
			inner join v_UslugaComplex (nolock) uc on uc.UslugaComplex_id = ut.UslugaComplex_id
			inner join v_EvnLabSample (nolock) els on els.EvnLabSample_id = ut.EvnLabSample_id
			where
				els.Person_id = @Person_id
	...
	function loadLabResearchResultHistory($data) {
		...
		$query = "
			select
				uth.UslugaTestHistory_id,
				(uc.UslugaComplex_Code + ' ' + uc.UslugaComplex_Name) as UslugaComplex_Name,
				uth.UslugaTest_id,
				uth.UslugaTestHistory_Result,
				uth.UslugaTestHistory_Comment,
				(convert(varchar, uth.UslugaTestHistory_CheckDT, 104) + ' ' + convert(varchar, uth.UslugaTestHistory_CheckDT, 108)) as UslugaTestHistory_CheckDT,
				mp.Person_Fio
			from v_UslugaTestHistory (nolock) uth
			left join v_pmUserCache (nolock) pmu on pmu.PMUser_id = uth.pmUser_insID
			left join v_MedPersonal (nolock) mp on mp.MedPersonal_id = pmu.MedPersonal_id and mp.Lpu_id = pmu.Lpu_id
			inner join v_UslugaTest (nolock) ut on ut.UslugaTest_id = uth.UslugaTest_id
			inner join UslugaComplex (nolock) uc on uc.UslugaComplex_id = ut.UslugaComplex_id
	...
	function loadPathologySamples($data) {
		$query = "
			select
				EvnLabSample_id,
				EvnLabSample_IsOutNorm
			from v_EvnLabSample (nolock)
    ...
    function checkEvnLabSampleBarCodeUnique($data) {
        ...
        if ( is_object($result) ) {
            $resp =  $result->result('array');
            if (!empty($resp[0]['EvnLabSample_id'])) {
                return array('Error_Msg' => 'Проба с штрих-кодом '.$data['EvnLabSample_BarCode'].' уже существует');
            } else {
                return true;
            }
        }
    ...
    function checkEvnLabSampleNumUnique($data) {
        ...
        if ( is_object($result) ) {
            $resp =  $result->result('array');
            if ($resp[0]['count'] > 0) {
                return array('Error_Msg' => 'Проба с номером '.mb_substr($data['EvnLabSample_Num'], -4).' уже существует');
            } else {
                return true;
            }
        }
        ...
        $resp = $this->queryResult("
			...
			from
				...
				outer apply (
					...
					from
						v_UslugaComplexMedService ucms with(nolock)
                        inner join v_RefSample rs with(nolock) on rs.RefSample_id = ucms.RefSample_id
					where
    ...
    function getBioResearch( $UslugaComplexMedService_id ) {
        ...
        $resp = $this->queryResult("
				...
				from
					v_UslugaComplexMedService ucms with(nolock)
					left join v_RefSample rs with(nolock) on rs.RefSample_id = ucms.RefSample_id
				where
    ...
    function getComplexResearchTests( $UslugaComplexMedService_id ) {
        ...
        $resp = $this->queryResult("
				...
				select distinct
					...
					rs.RefMaterial_id as \"RefMaterial_id\"
				from
					v_UslugaComplexMedService ucms_usluga_parent with(nolock)
					left join v_UslugaComplexMedService ucms_usluga with(nolock) on ucms_usluga.UslugaComplexMedService_pid = ucms_usluga_parent.UslugaComplexMedService_id
					inner join lis.v_AnalyzerTest at_child with(nolock) on at_child.UslugaComplexMedService_id = COALESCE(ucms_usluga.UslugaComplexMedService_id, ucms_usluga_parent.UslugaComplexMedService_id)
					left join lis.v_AnalyzerTest at with(nolock) on at.AnalyzerTest_id = at_child.AnalyzerTest_pid
					inner join lis.v_Analyzer a with(nolock) on a.Analyzer_id = COALESCE(at.Analyzer_id, at_child.Analyzer_id)
					left join v_UslugaComplex uctest with(nolock) on uctest.UslugaComplex_id = at_child.UslugaComplex_id
					left join v_RefSample rs with(nolock) on rs.RefSample_id = ucms_usluga.RefSample_id
				where
	...
	function getTestListForm250($data) {
		...
		$uslWhere = "els0.MedService_id IN (select MSL.MedService_lid from MedServiceLink MSL with(nolock) where msl.MedService_id = :MedService_id)";
		...
		$query = "
			DECLARE @dd varchar(20) = :EvnLabSample_DelivDT;
			DECLARE @paramDate date = CAST(@dd AS date);

			SELECT DISTINCT usl.UslugaComplex_id
				, at.AnalyzerTest_SysNick
				, at.AnalyzerTest_id
				, at.AnalyzerTestType_id
				, uc.UslugaComplex_Name
				, ucgost.UslugaComplex_Name UslugaComplex_Name_Gost
				, coalesce(at.AnalyzerTest_SysNick, uc.UslugaComplex_Name, ucgost.UslugaComplex_Name, 'NoName') as TestName
				, ISNULL(at.AnalyzerTest_SortCode, '777') AnalyzerTest_SortCode
				--, at.UslugaComplex_id
			...
			FROM (
				...
				FROM (
					...
					FROM dbo.v_EvnLabSample els0 (nolock)
					WHERE
						{$uslWhere}
						AND els0.EvnLabSample_setDT >= @paramDate
						AND els0.EvnLabSample_setDT < DATEADD(day, 1, @paramDate)
				) els
				LEFT JOIN dbo.v_UslugaTest ut (nolock) ON ut.EvnLabSample_id = els.EvnLabSample_id
			) usl
			LEFT JOIN dbo.MedServiceLink msl (nolock) ON msl.MedService_id = :MedService_id
			LEFT JOIN dbo.UslugaComplexMedService ucms (nolock) ON ucms.MedService_id = {$msParam} AND ucms.UslugaComplex_id = usl.UslugaComplex_id
			LEFT JOIN lis.v_AnalyzerTest at (nolock) ON at.UslugaComplexMedService_id = ucms.UslugaComplexMedService_id
			LEFT JOIN dbo.v_UslugaComplex uc (nolock) ON uc.UslugaComplex_id = at.UslugaComplex_id
			LEFT JOIN dbo.v_UslugaComplex ucgost (nolock) ON uc.UslugaComplex_2011id = ucgost.UslugaComplex_id
			LEFT JOIN lis.AnalyzerTest at2 (nolock) ON at2.AnalyzerTest_id = at.AnalyzerTest_pid and ucms.MedService_id = {$msParam}
			" . $filterUslugaTest . "
		...
		$query = "
			...
			WHERE
				...
				(
					...
					FROM v_UslugaComplexMedService ms with(nolock)
	...
	function loadSampleListForm250($data) {
		...
		$elsWhere = ( !empty($data['MedServiceType_SysNick']) && $data['MedServiceType_SysNick'] == 'lab' )
			...
			: "els0.MedService_id IN (select MSL.MedService_lid from MedServiceLink MSL with(nolock) where msl.MedService_id = :MedService_id)";
		...
		$query = "
			...
			SELECT
				els.*
				, elr.EvnLabRequest_Ward
				, ISNULL('Палата '+elr.EvnLabRequest_Ward+'; ', '') + ISNULL('Отделение '+sec.LpuSection_Name, '') LpuSection
				, ISNULL(elr.Diag_id, ev.Diag_id) as Diag_id
				, ISNULL(diag.Diag_Name, vizitdiag.Diag_Name) as DiagName
				, ISNULL(ps.Person_SurName+' ', '') + ISNULL(ps.Person_FirName+' ', '') + ISNULL(ps.Person_SecName, '') PatientName
				, ut.UslugaTest_id
				, CASE ut.UslugaTest_ResultApproved
					WHEN 2 THEN ut.UslugaTest_ResultValue + ISNULL(' '+ut.UslugaTest_ResultUnit, '')
					ELSE '+'
				  END UslugaTest_ResultValue
				, ut.UslugaTest_ResultApproved
				, ut.UslugaTest_CheckDT
				, ut.UslugaTest_ResultUnit
				, ut.UslugaComplex_id
				, (STUFF(( select ',' + CAST(uc.UslugaComplex_id as varchar(10))
					from EvnUslugaPar eup
					inner join EvnUsluga eu on eu.EvnUsluga_id = eup.EvnUsluga_id
			...
			LEFT JOIN dbo.v_EvnLabRequest elr (nolock) ON els.EvnLabRequest_id = elr.EvnLabRequest_id
			LEFT JOIN dbo.v_UslugaTest ut (nolock) ON ut.EvnLabSample_id = els.EvnLabSample_id
				AND ut.UslugaTest_pid IS NOT NULL
			LEFT JOIN dbo.v_PersonState ps (nolock) ON ps.Person_id = els.Person_id
			LEFT JOIN dbo.v_Diag diag (nolock) ON diag.Diag_id = elr.Diag_id
			LEFT JOIN dbo.v_EvnDirection_all da (nolock) ON da.EvnDirection_id = elr.EvnDirection_id
			LEFT JOIN dbo.v_LpuSection sec (nolock) ON sec.LpuSection_id = da.LpuSection_id
			LEFT JOIN dbo.v_EvnVizitPL ev (nolock) ON ev.EvnVizitPL_id = da.EvnDirection_pid
			LEFT JOIN dbo.v_Diag vizitdiag (nolock) ON vizitdiag.Diag_id = ev.Diag_id
			" . $filterUslugaTest;
	...
	function createEvnLabSampleAPI($data){
		...
		//$query = 'SELECT EvnLabRequest_id, MedService_id FROM EvnLabRequest WHERE EvnDirection_id = :EvnDirection_id';
		$query = "
			SELECT
		...
		if (!empty($res['EvnStatus_SysNick']) && in_array($res['EvnStatus_SysNick'], array('Declined', 'Canceled'))) {
			$query = "
				...
				FROM
					...
					left join v_EvnStatus es with(nolock) on es.EvnStatus_id = d.EvnStatus_id
					left join v_EvnLabRequest elr with(nolock) on elr.EvnDirection_id = d.EvnDirection_id
					left join v_Lpu LCID with(nolock) on d.Lpu_cid = LCID.Lpu_id
					left join v_EvnStatusHistory ESH with(nolock) on ESH.Evn_id = d.EvnDirection_id
		...
		$query = "
			declare
				@EvnLabRequestUslugaComplex_id bigint = :EvnLabRequestUslugaComplex_id,
				@Error_Code bigint,
				@Error_Msg varchar(4000);
			exec p_EvnLabRequestUslugaComplex_upd
	...
	function updateUslugaTestAPI($data){
		...
		if(!empty($params['PersonEvn_id'])){
			$this->load->model('Person_model');
			$res = $this->Person_model->serverByPersonEvn([
				'PersonEvn_id' => $params['PersonEvn_id']
			]);

			$params['Server_id'] = !empty($res) ? $res['Server_id'] : $params['Server_id'];
		}
	...
	function setMedPersonalAID($data) {
		...
		return $this->db->query(
			'UPDATE EvnLabSample with (rowlock) SET MedPersonal_aid = :MedPersonal_aid WHERE EvnLabSample_id = :EvnLabSample_id',
			$data
		);
	}
