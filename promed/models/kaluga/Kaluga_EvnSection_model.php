...
class Kaluga_EvnSection_model extends EvnSection_model {
	...
	function loadKSGKPGKOEF2018($data) {
		...
		$data['TariffClass_id'] = $this->getFirstResultFromQuery("select top 1 TariffClass_id from v_TariffClass (nolock) where TariffClass_SysNick = :TariffClass_SysNick", array(
		...
		if (!empty($data['LpuSectionBedProfile_id'])) {
			...
			$query = "
				select top 1
					...
					mtkpg.MesTariff_id
				from
					v_LpuSectionBedProfile lsbp (nolock)
					inner join MesOld mo (nolock) on mo.Mes_Code = '169'+right('0000'+cast(lsbp.LpuSectionBedProfile_Code as varchar),3)+'{$stacKod}' and mo.MesType_id IN (4) -- КПГ
					{$crossapplymesvol}
					left join v_MesTariff mtkpg (nolock) on mtkpg.Mes_id = mo.Mes_id and mtkpg.MesPayType_id = 5 -- Коэффициент КПГ
				where
	...
	function loadKSGKPGKOEF($data) {
		...
		if (!empty($data['LpuSectionBedProfile_id'])) {
			$query = "
				...
				from
					v_LpuSectionBedProfile lsbp (nolock)
					inner join MesOld mokpg (nolock) on mokpg.Mes_Code = '159'+right('0000'+cast(lsbp.LpuSectionBedProfile_Code as varchar),3) and mokpg.MesType_id IN (4) -- КПГ
