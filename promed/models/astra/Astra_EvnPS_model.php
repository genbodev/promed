...
class Astra_EvnPS_model extends EvnPS_model {
	...
	function exportHospDataForTfomsToXml($data) {
		...
		$query = "
			...
			cast(year(ed.EvnDirection_setDT) as varchar) +cast(lp.Lpu_f003mcod as varchar)+cast(ed.EvnDirection_Num as varchar) as N_NPR_LPU,		--T(16) Номер направления (MCOD+NP)
			cast(ed.EvnDirection_setDT as date) as D_NPR,
			case when dt.DirType_Code = 5 then 2 else isnull(dt.DirType_Code,1) end as FOR_POM, -- Int - Форма оказания медицинской помощи
			lp.Lpu_f003mcod as NCODE_MO, -- T(6)-- numeric(6, 0) - МО, направившая на госпитализацию	(реестровый номер, F003)
			lb.LpuBuilding_Code as NLPU_1, -- T(8)-- numeric(4, 0) - Код подразделения медицинской организации, направившей на госпитализацию
			ISNULL(lp1.Lpu_f003mcod, '0') as DCODE_MO, -- T(6)-- numeric(6, 0) - МО, куда направлен пациент	(реестровый номер, F003)
			lb1.LpuBuilding_Code as DLPU_1, -- T(8) Код подразделения медицинской организации, куда выписано направление
			pt.PolisType_CodeF008 as VPOLIS,-- numeric(1, 0) , -- Тип документа, подтверждающего факт страхования по ОМС (F008)
			case when pt.PolisType_CodeF008 in (1,2) then po.Polis_Ser else Null end as SPOLIS, -- Varchar(10) - Серия документа, подтверждающего факт страхования по ОМС
			case when pt.PolisType_CodeF008=3 then polisInfo.edNum else po.Polis_Num end as NPOLIS, -- Numeric(20) - Номер документа, подтверждающего факт страхования по ОМС
			smo.Orgsmo_f002smocod as SMO, -- numeric(6, 0) - СМО (реестровый номер, F002)
			org.Org_OGRN as SMO_OGRN, --Т(15) ОГРН СМО
			org.Org_OKATO as SMO_OK, --Т(5) ОКАТО территории страхования
			case when org.Org_OGRN is null and org.Org_OKATO is null then isnull(org.Org_Name, smo.OrgSMO_Name) else Null end as SMO_NAM, --Т(100) Наименование СМО
			...
			case when len(isnull(pe.PersonPhone_Phone, '')) = 0 then 'телефон не указан' else pe.PersonPhone_Phone end as CT, --Varchar(100) - Контактная информация
			isnull(di.Diag_Code, '') as DS1, --Char(4) - Код диагноза по МКБ
			case 
				when lut1.LpuUnitType_SysNick = 'stac' then 1
				when lut1.LpuUnitType_SysNick in ('dstac','hstac','pstac') then 2
			end as USL_OK,
			case when dbo.Age2(pe.Person_BirthDay, ed.EvnDirection_setDT) < 18 then 1 else 0 end as DET,
			lsp.LpuSectionProfile_Code as PROFIL, --numeric(4, 0) - Профиль койки
			ls1.LpuSection_Code as PODR, --numeric(4, 0) - Код отделения (профиль)
			substring(mp.Person_Snils,1,3)+'-'+substring(mp.Person_Snils,4,3)+'-'+substring(mp.Person_Snils,7,3)+' '+substring(mp.Person_Snils,10,2) as IDDOKT, --Varchar(16) - Снилс медицинского работника, направившего больного
			cast(isnull(tt.TimetableStac_setDate, ed.EvnDirection_desDT) as date) as DATE_1 -- date - Плановая дата госпитализации
			from dbo.v_EvnDirection ed (nolock)
			left join dbo.v_EvnQueue eq (nolock) on eq.EvnDirection_id = ed.EvnDirection_id
			left join dbo.v_EvnPS eps (nolock) on eps.EvnDirection_id = ed.EvnDirection_id
			left join v_EvnDirectionExt ext_npr (nolock) on ext_npr.EvnDirection_id = ed.EvnDirection_id
			left join r30.v_EvnNPR npr (nolock) on npr.Evn_id = ed.EvnDirection_id
			left join r30.v_EvnNPR npr_eps (nolock) on npr_eps.Evn_id = eps.EvnPS_id
			...
			inner join v_lpu lp (nolock) on lp.Lpu_id = isnull(ed.Lpu_sid, ed.Lpu_id)
			...
			left join dbo.v_LpuBuilding lb (nolock) on lb.LpuBuilding_id = lu.LpuBuilding_id
			inner join dbo.v_MedPersonal mp (nolock) on ed.MedPersonal_id=mp.MedPersonal_id and mp.Lpu_id = lp.Lpu_id
			...
			left join dbo.v_LpuUnitType lut1 (nolock) on lut1.LpuUnitType_id=isnull(ed.LpuUnitType_id,lu1.LpuUnitType_id)
			left join dbo.v_LpuBuilding lb1 (nolock) on lb1.LpuBuilding_id = lu1.LpuBuilding_id
			left join v_LpuSectionProfile lsp with(nolock) on lsp.LpuSectionProfile_id = isnull(ls1.LpuSectionProfile_id,ed.LpuSectionProfile_id)
			/*outer apply (
			select top 1 fedprof.LpuSectionProfile_Code
			from fed.LpuSectionProfileLink link (nolock)
			left join nsi.LpuSectionProfile fedprof (nolock) on fedprof.LpuSectionProfile_id = link.LpuSectionProfile_did
			where link.LpuSectionProfile_id = ls1.LpuSectionProfile_id
			and fedprof.LpuSectionProfile_begDate <= ed.EvnDirection_setDT
			and (fedprof.LpuSectionProfile_endDate is null or fedprof.LpuSectionProfile_endDate > ed.EvnDirection_setDT)
			order by fedprof.LpuSectionProfile_begDate desc
			) as lspf*/
			...
			left join dbo.v_PersonDeputy pd (nolock) on pd.Person_id = pe.Person_id
			outer apply (
				select top 1
					pe.Person_id,
					pe.Polis_id,
					pe.Person_EdNum
				from dbo.v_Person_all pe with(nolock)
				where pe.Person_id = pd.Person_pid and pe.PersonEvn_begDT <= ed.EvnDirection_insDT
			...
			--данные полиса
			left join dbo.v_Polis po (nolock) on po.Polis_id = polisInfo.Polis_id
			...
			and (@Lpu_id is null or isnull(ed.Lpu_sid, ed.Lpu_id)=@Lpu_id)
			...
			select
			coalesce(npr.EvnNPR_NPRID, npr_eps.EvnNPR_NPRID, ext_npr.EvnDirectionExt_NPRID, 0) as N_NPR, --Int
			cast(isnull(ed.EvnDirection_setDT, eps.EvnDirection_setDT) as date) as D_NPR,
			case when prt.PrehospType_Code in (2,3) then 2 else isnull(prt.PrehospType_Code,1) end as FOR_POM, -- Int - Форма оказания медицинской помощи
			lp.Lpu_f003mcod as DCODE_MO, -- T(6)-- numeric(6, 0) - МО госпитализации	(реестровый номер, F003)
			lb.LpuBuilding_Code as DLPU_1, -- T(8) Код подразделения МО госпитализации
			lp1.Lpu_f003mcod as NCODE_MO, -- T(6)-- numeric(6, 0) - Код подразделения МО, создавшей направление	(реестровый номер, F003)
			lb1.LpuBuilding_Code as NLPU_1, -- T(8)-- numeric(4, 0) - Код подразделения МО, создавшей направление
			cast(eps.EvnPS_setDate as date) as DATE_1, --Дата фактической госпитализации
			convert(varchar(5), eps.EvnPS_setTime, 108) as TIME_1, --Время фактической госпитализации
			pt.PolisType_CodeF008 as VPOLIS,-- numeric(1, 0) , -- Тип документа, подтверждающего факт страхования по ОМС (F008)
			case when pt.PolisType_CodeF008 in (1,2) then po.Polis_Ser else Null end as SPOLIS, -- Varchar(10) - Серия документа, подтверждающего факт страхования по ОМС
			case when pt.PolisType_CodeF008=3 then polisInfo.edNum else po.Polis_Num end as NPOLIS, -- Numeric(20) - Номер документа, подтверждающего факт страхования по ОМС
			pe.Person_SurName as FAM, --varchar(30) – Фамилия
			...
			case when dbo.Age2(pe.Person_BirthDay, eps.EvnPS_setDT) < 18 then 1 else 0 end as DET,
			lsp.LpuSectionProfile_Code as PROFIL, --numeric(4, 0) - Профиль койки
			ls.LpuSection_Code as PODR, --numeric(4, 0) - Код отделения (профиль)
			eps.EvnPS_NumCard as NHISTORY, --Номер карты стационарного больного
			coalesce(di.Diag_Code, dirdi.Diag_Code, '') as DS1 --Char(4) - Диагноз приемного отделения
			from dbo.v_EvnPS eps (nolock)
			left join dbo.v_EvnDirection ed (nolock) on ed.EvnDirection_id = eps.EvnDirection_id
			left join v_EvnDirectionExt ext_npr (nolock) on ext_npr.EvnDirection_id = eps.EvnDirection_id
			left join r30.v_EvnNPR npr (nolock) on npr.Evn_id = eps.EvnDirection_id
			left join r30.v_EvnNPR npr_eps (nolock) on npr_eps.Evn_id = eps.EvnPS_id
			...
			from fed.LpuSectionProfileLink link (nolock)
			left join nsi.LpuSectionProfile fedprof (nolock) on fedprof.LpuSectionProfile_id = link.LpuSectionProfile_did
			where link.LpuSectionProfile_id = ls.LpuSectionProfile_id
			and fedprof.LpuSectionProfile_begDate <= eps.EvnPS_setDT
			and (fedprof.LpuSectionProfile_endDate is null or fedprof.LpuSectionProfile_endDate > eps.EvnPS_setDT)
			order by fedprof.LpuSectionProfile_begDate desc
			) as lspf*/
			left join dbo.v_LpuSection ls1 (nolock) on ls1.LpuSection_id = isnull(ed.LpuSection_id, eps.LpuSection_did)
			left join dbo.v_LpuUnit lu1 (nolock) on ls1.LpuUnit_id=lu1.LpuUnit_id
			left join dbo.v_LpuBuilding lb1 (nolock) on lb1.LpuBuilding_id = lu1.LpuBuilding_id
			inner join dbo.v_lpu lp1 (nolock) on lp1.Lpu_id = coalesce(ed.Lpu_sid, ed.Lpu_id, eps.Lpu_did, ls1.Lpu_id)
			left join dbo.Diag di (nolock) on di.Diag_id = isnull(eps.Diag_pid,eps.Diag_id)
			left join dbo.Diag dirdi (nolock) on dirdi.Diag_id = ed.Diag_id
			--пациент
			...
			outer apply (
				select top 1
					...
					pe.Person_EdNum
				from dbo.v_Person_all pe with(nolock)
				where pe.Person_id = pd.Person_pid and pe.PersonEvn_begDT <= eps.EvnPS_insDT
				order by pe.PersonEvn_insDT desc
			) d_pe
			--идентификатор полиса пациента или его представителя
			outer apply(
			...
			((
			select
			coalesce(npr.EvnNPR_NPRID, npr_eps.EvnNPR_NPRID, 0) as N_NPR, --Int
			lp.Lpu_f003mcod as DCODE_MO, -- T(6)-- numeric(6, 0) - МО госпитализации	(реестровый номер, F003)
			lb.LpuBuilding_Code as DLPU_1, -- T(8) Код подразделения МО госпитализации
			cast(eps.EvnPS_setDate as date) as DATE_1, --Дата фактической госпитализации
			convert(varchar(5), eps.EvnPS_setTime, 108) as TIME_1, --Время фактической госпитализации
			...
			org.Org_OGRN as SMO_OGRN, --Т(15) ОГРН СМО
			org.Org_OKATO as SMO_OK, --Т(5) ОКАТО территории страхования
			case when org.Org_OGRN is null and smo.Orgsmo_f002smocod is null then isnull(org.Org_Name, smo.OrgSMO_Name) else Null end as SMO_NAM, --Т(100) Наименование СМО
			pe.Person_SurName as FAM, --varchar(30) – Фамилия
			pe.Person_FirName as IM, --varchar(30) – Имя
			pe.Person_SecName as OT, --varchar(30) – Отчество
			case when pe.Sex_id=3 then 1 else pe.Sex_id end as W, --numeric(1) – пол (1 - муж, 2 - жен)
			...
			from dbo.v_EvnPS eps (nolock)
			left join dbo.v_EvnDirection ed (nolock) on ed.EvnDirection_id = eps.EvnDirection_id
			--left join v_EvnDirectionExt ext_npr (nolock) on eps.EvnPS_id=npr.Evn_id
			left join r30.v_EvnNPR npr (nolock) on npr.Evn_id = eps.EvnDirection_id
			left join r30.v_EvnNPR npr_eps (nolock) on npr_eps.Evn_id = eps.EvnPS_id
			left join dbo.v_PrehospType prt (nolock) on eps.PrehospType_id=prt.PrehospType_id
			inner join v_lpu lp (nolock) on lp.Lpu_id = eps.Lpu_id
			...
			from (
				...
				from dbo.v_EvnDirection ed (nolock)
					left join v_EvnPS eps (nolock) on eps.EvnDirection_id = ed.EvnDirection_id
					left join v_EvnDirectionExt ext_npr (nolock) on ext_npr.EvnDirection_id = ed.EvnDirection_id
					left join r30.v_EvnNPR npr (nolock) on npr.Evn_id = ed.EvnDirection_id
					left join r30.v_EvnNPR npr_eps (nolock) on npr_eps.Evn_id = eps.EvnPS_id
				...
				where
					...
					and (@Lpu_id is null or ed.Lpu_id=@Lpu_id)
					and ed.EvnDirection_statusDate>=@startTime and ed.EvnDirection_statusDate<=@finalTime

				union all

				select
					coalesce(npr.EvnNPR_NPRID, npr_eps.EvnNPR_NPRID, ext_npr.EvnDirectionExt_NPRID, 0) as N_NPR, --Int
					cast(isnull(ed.EvnDirection_setDT,eps.EvnDirection_setDT) as date) as D_NPR, --Дата направления	= дата создания направления
				...
				from dbo.v_EvnPS eps (nolock)
					...
					left join r30.v_EvnNPR npr_eps (nolock) on npr_eps.Evn_id = eps.EvnPS_id
					left join dbo.v_LpuSection ls1 (nolock) on ls1.LpuSection_id = ed.LpuSection_did
					left join dbo.v_LpuUnit lu (nolock) on ls1.LpuUnit_id=lu.LpuUnit_id
				...
				where
					...
					and eps.PayType_id = @PayType_id
					and isnull(ed.EvnDirection_setDT, eps.EvnDirection_setDT) is not null
			...
			select
			COALESCE(npr.EvnNPR_NPRID, npr_eps.EvnNPR_NPRID, ext_npr.EvnDirectionExt_NPRID, 0) as N_NPR, --Int
			cast(coalesce(ed.EvnDirection_setDT, eps.EvnDirection_setDT, eps.EvnPS_setDT) as date) as D_NPR, --Дата направления	= дата создания направления
			case when prt.PrehospType_Code in (2,3) then 2 else isnull(prt.PrehospType_Code,1) end as FOR_POM, -- Int - Форма оказания медицинской помощи
			lp.Lpu_f003mcod as LPU, -- T(6)-- numeric(6, 0) - МО госпитализации	(реестровый номер, F003)
			lb.LpuBuilding_Code as LPU_1, -- T(8) Код подразделения МО госпитализации
			cast(eps.EvnPS_setDate as date) as DATE_1, --Дата госпитализации
			cast (eps.EvnPS_disDT as date)	as	DATE_2, -- datetime - Дата выбытия
			--pe.Person_SurName as FAM, --varchar(30) – Фамилия
			--pe.Person_FirName as IM, --varchar(30) – Имя
			--pe.Person_SecName as OT, --varchar(30) – Отчество
			case when pe.Sex_id=3 then 1 else pe.Sex_id end as W, --numeric(1) – пол (1 - муж, 2 - жен)
			...
			left join r30.v_EvnNPR npr (nolock) on npr.Evn_id = ed.EvnDirection_id
			left join r30.v_EvnNPR npr_eps (nolock) on npr_eps.Evn_id = eps.EvnPS_id
			left join dbo.v_PrehospType prt (nolock) on isnull(ed.PrehospType_did,eps.PrehospType_id)=prt.PrehospType_id
			inner join v_lpu lp (nolock) on lp.Lpu_id = eps.Lpu_id
			inner join dbo.v_LpuSection ls (nolock) on ls.LpuSection_id = eps.LpuSection_id
			inner join dbo.v_LpuUnit lu (nolock) on ls.LpuUnit_id=lu.LpuUnit_id
			inner join dbo.v_LpuBuilding lb (nolock) on lb.LpuBuilding_id = lu.LpuBuilding_id
			...
			and (@Lpu_id is null or eps.Lpu_id=@Lpu_id)
			and eps.PayType_id = @PayType_id
			and eps.EvnPS_disDT>=@startTime and eps.EvnPS_disDT<=@finalTime
			for xml path('NPR')
			), '')
			...
			((
			select
			DATA, LPU, LPU_1, USL_OK, DET, PROFIL, COUNTP, POSTP, VIBP, PLANP, FREEK, FREEM, FREEW, FREED,
			SMO as 'OBSMO/SMO',
			SMOSL as 'OBSMO/SMOSL',
			SMOKD as 'OBSMO/SMOKD'
	...
	function saveEvnNPR($data) {
		...
		$query = "
			...
			set @EvnNPR_id = :EvnNPR_id;
			exec " . $procedure . "
				@EvnNPR_id = @EvnNPR_id output,
				@Evn_id = :Evn_id,
			...
			select @EvnNPR_id as EvnNPR_id, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
		";
	...
	function identHospData($tmpTableName, $method, &$stat = array('ident' => 0), &$errors = array()) {
		...
		switch($method) {
			case 'N1':
				$object = 'EvnDirection';
				$join .= " left join Lpu nl with(nolock) on nl.Lpu_f003mcod = pl.NCODE_MO";
				$join .= " left join Lpu dl with(nolock) on dl.Lpu_f003mcod = pl.DCODE_MO";
				$where .= " and pe.Person_SurName = pl.FAM";
				$where .= " and ISNULL(pe.Person_FirName, '') = ISNULL(pl.IM, '')";
				$where .= " and ISNULL(pe.Person_SecName, '') = ISNULL(pl.OT, '')";
				$where .= " and pe.Person_BirthDay = pl.DR";
				$where .= " and e.EvnDirection_setDate = pl.D_NPR";
				$where .= " and coalesce(e.Lpu_sid, e.Lpu_id, 0) = isnull(nl.Lpu_id, 0)";
				$where .= " and isnull(e.Lpu_did, 0) = isnull(dl.Lpu_Id, 0)";
			...
			case 'N2':
				...
				$join .= " left join Lpu nl with(nolock) on nl.Lpu_f003mcod = pl.NCODE_MO";
				$join .= " left join Lpu dl with(nolock) on dl.Lpu_f003mcod = pl.DCODE_MO";
				$peJoin .= " left join v_EvnDirection ed with(nolock) on ed.EvnDirection_id = e.EvnDirection_id";
				$peJoin .= " left join v_LpuSection ls with(nolock) on ls.LpuSection_id = e.LpuSection_did";
				$where .= " and e.EvnPS_setDate = pl.DATE_1";
				$where .= " and coalesce(ed.EvnDirection_setDate, e.EvnDirection_setDT, '1900-01-01') = coalesce(pl.D_NPR, '1900-01-01')";
			...
			case 'N3':
				...
				$peJoin .= " left join v_LpuSection ls with(nolock) on ls.LpuSection_id = e.LpuSection_did";
				$where .= " and e.EvnPS_setDate = pl.DATE_1";
				$where .= " and coalesce(ed.EvnDirection_setDate, e.EvnDirection_setDT, '1900-01-01') = coalesce(pl.D_NPR, ed.EvnDirection_setDate, e.EvnDirection_setDT, '1900-01-01')";
			...
			case 'N5':
				$object = 'EvnPS';
				$where .= " and pe.Person_SurName like pl.FAM";
				$where .= " and ISNULL(pe.Person_FirName, '') like ISNULL(pl.IM, '')";
				$where .= " and ISNULL(pe.Person_SecName, '') like ISNULL(pl.OT, '')";
				$where .= " and e.EvnPS_setDate = pl.DATE_1";
				$where .= " and pe.Person_BirthDay = pl.DR";
		...
		$doubleFieldsOnFn = function($field){
			switch(true) {
				case in_array($field, array('FAM','IM','OT')):
					return "isnull(dl.$field,'') = isnull(pl.$field,'')";
				case in_array($field, array('D_NPR','DATE_1','DR')):
					return "isnull(dl.$field,@date) = isnull(pl.$field,@date)";
				case in_array($field, array('NCODE_MO','DCODE_MO')):
					return "isnull(dl.$field,0) = isnull(pl.$field,0)";
				default:
					return '1=1';
			}
		};
		...
		$query = "
			...
			declare @Error_Code bigint = null
			declare @Error_Message varchar(4000) = ''
			declare @Ident_Count int = 0
			set nocount on
			begin try
				with double_list as (
					select max(NPRID) as NPRID, {$doubleFieldsStr}
					from {$tmpTableName}
				...
				inner join double_list dl with(nolock) on {$doubleFieldsOnStr}
				--where dl.NPRID <> pl.NPRID
				...
				cross apply (
					select top 1 *
					from v_EvnDirectionExt ede with(nolock)
				...
				--inner join v_EvnDirectionExt ede with(nolock) on ede.EvnDirectionExt_NPRID = pl.NPRID
				...
				from
					...
					cross apply(
						...
						from
							...
							inner join v_{$object} e with(nolock) on e.PersonEvn_id = pe.PersonEvn_id and e.Server_id = pe.Server_id
				...
				set @Ident_Count = (select top 1 count(*) from {$tmpTableName} where Person_id is not null)
			end try
			begin catch
				set @Error_Code = error_number()
				set @Error_Message = error_message()
			...
			set nocount off
			select @Ident_Count as Ident_Count, @Error_Code as Error_Code, @Error_Message as Error_Msg
		";
		...
		$doublesList = $this->queryResult("select NPRID, doubleID from {$tmpTableName} where doubleID is not null");
		...
		$nonIdentList = $this->queryList("select NPRID from {$tmpTableName} where Person_id is null and doubleID is null");
	...
	function saveEvnDirectionExt($tmpTableName, $session, &$stat = array('ext' => 0, 'upd' => 0), &$errors = array()) {
		$query = "
			select
				...
				convert(varchar(10), pl.D_NPR, 120) as EvnDirectionExt_setDT,
				case when pl.FOR_POM = 1 then 2 else 1 end as PrehospType_id,
				dl.Lpu_id,		--МО куда направлен
				dl.Lpu_Name,	--МО куда направлен
				...
				convert(varchar(10), pl.DR, 120) as Person_BirthDay,
				...
				convert(varchar(10), pl.DATE_1, 120) as EvnDirectionExt_planDate
				-- end select
			from 
				-- from
				{$tmpTableName} pl
				--inner join v_Lpu nl with(nolock) on nl.Lpu_f003mcod = pl.NCODE_MO
				left join v_Lpu dl with(nolock) on dl.Lpu_f003mcod = pl.DCODE_MO
				left join v_Diag d with(nolock) on d.Diag_Code = pl.DS1 and DiagLevel_id = 4
				left join v_LpuSectionProfile lsp with(nolock) on lsp.LpuSectionProfile_Code = cast(pl.PROFIL as varchar)
				left join v_EvnPS eps with(nolock) on eps.EvnPS_id = pl.Evn_id
				outer apply(
					select top 1 *
					from v_EvnDirectionExt ede with(nolock)
	...
	function saveEvnDirectionInt($tmpTableName, $session, &$stat = array('int' => 0, 'upd' => 0), &$errors = array()) {
		$query = "
			...
			from
				-- from
				{$tmpTableName} pl
				left join r30.v_EvnNPR npr with(nolock) on npr.Evn_id = pl.Evn_id
				-- end from
			where
				-- where
				pl.Evn_id is not null
				and isnull(pl.isExternal, 0) = 0
		...
		$query = "
			select top 1
				...
				,case when LUTLast.LpuUnitType_SysNick = 'stac'
				  then
                    datediff(day, EPS.EvnPS_setDate, EPS.EvnPS_disDate) + abs(sign(datediff(day, EPS.EvnPS_setDate, EPS.EvnPS_disDate)) - 1) -- круглосуточные
				  else
				    (datediff(day, EPS.EvnPS_setDate, EPS.EvnPS_disDate) + 1) -- дневные
			...
			from v_EvnPS EPS with (nolock)
				...
				outer apply (
								select top 1 PP.PrivilegeType_id,
