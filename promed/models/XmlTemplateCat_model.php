 ...
 */
class XmlTemplateCat_model extends swModel
{
	/**
			...
			if ( isset($this->_params['MedStaffFact_id']) ) {
				$tmp = $this->getFirstRowFromQuery('
					select top 1
						...
						v_Lpu.Lpu_Nick
					from
						dbo.v_MedStaffFact msf with (NOLOCK)
						inner join dbo.v_Lpu with (NOLOCK) on v_Lpu.Lpu_id = msf.Lpu_id
					where
				...
				$tmp = $this->getFirstRowFromQuery('
					select top 1
						...
						v_Lpu.Lpu_Nick
					from
						dbo.v_MedServiceMedPersonal msmp with (NOLOCK)
						inner join dbo.v_MedService ms with (NOLOCK) on ms.MedService_id = msmp.MedService_id
							and ms.Lpu_id = :Lpu_id
						inner join dbo.v_MedPersonal mp with (NOLOCK) on mp.MedPersonal_id = msmp.MedPersonal_id
							and mp.Lpu_id = ms.Lpu_id
						inner join dbo.v_Lpu with (NOLOCK) on v_Lpu.Lpu_id = ms.Lpu_id
					where
		...
		if (self::SCENARIO_DELETE == $this->scenario) {
			...
			$tmp = $this->getFirstRowFromQuery("
				select
					...
					xtcd.Lpu_Nick,
					xtcd.Person_Fio,
					xtcd.MedService_Name,
					xtcd.LpuSection_Name,
					xtcd.XmlTemplateCatDefault_id
				from XmlTemplateCat xtc (NOLOCK)
				outer apply (
					select top 1
					xtcd.XmlTemplateCatDefault_id,
					Lpu.Lpu_Nick,
					...
					mp.Person_Fio
					from XmlTemplateCatDefault xtcd (NOLOCK)
					left join v_MedStaffFact msf (NOLOCK) on msf.MedStaffFact_id = xtcd.MedStaffFact_id
					left join Medservice (NOLOCK) on Medservice.Medservice_id = xtcd.Medservice_id
					left join v_Lpu Lpu (NOLOCK) on Lpu.Lpu_id = coalesce(xtcd.Lpu_id,Medservice.Lpu_id, msf.Lpu_id)
					left join LpuSection (NOLOCK) on LpuSection.LpuSection_id = coalesce(xtcd.LpuSection_id,Medservice.LpuSection_id, msf.LpuSection_id)
					left join v_pmUserCache puc (NOLOCK) on puc.PMUser_id = xtcd.pmUser_insID
					left join v_MedPersonal mp (NOLOCK) on mp.MedPersonal_id = coalesce(xtcd.MedPersonal_id,msf.MedPersonal_id,puc.MedPersonal_id)
				...
				and not exists (
					select top 1 ch.XmlTemplateCat_id
					from (
						select top 1
						ch.XmlTemplateCat_id
						from v_XmlTemplateCat ch (NOLOCK)
						where ch.XmlTemplateCat_pid = xtc.XmlTemplateCat_id
						union all
						select top 1
						ch.XmlTemplateCat_id
						from v_XmlTemplate ch (NOLOCK)
						where ch.XmlTemplateCat_id = xtc.XmlTemplateCat_id
							and ISNULL(ch.XmlTemplate_IsDeleted,1) = 1
					) ch
				)
			", $params);
			if (empty($tmp)) {
	...
	protected function _beforeDelete($data = array())
	{
		...
		$query = "
			select XmlTemplate_id
			from XmlTemplate (NOLOCK)
			where XmlTemplateCat_id = :XmlTemplateCat_id
				and ISNULL(XmlTemplate_IsDeleted,1) = 2
		";
	...
	function loadCombo($data)
	{
		...
		if (!empty($data['query'])) {
			...
			$queryFilter = ' and xtc.XmlTemplateCat_Name like :query';
		}
		$pidFilter = '';
		...
		if (!empty($data['needMaxLevelFilter'])) {
			$maxLevelFilter = ' and not exists (
				select top 1 p6.XmlTemplateCat_id
				from XmlTemplateCat p0 with (NOLOCK)
				left join dbo.XmlTemplateCat p1 with (nolock) on p0.XmlTemplateCat_pid = p1.XmlTemplateCat_id
				left join dbo.XmlTemplateCat p2 with (nolock) on p1.XmlTemplateCat_pid = p2.XmlTemplateCat_id
				left join dbo.XmlTemplateCat p3 with (nolock) on p2.XmlTemplateCat_pid = p3.XmlTemplateCat_id
				left join dbo.XmlTemplateCat p4 with (nolock) on p3.XmlTemplateCat_pid = p4.XmlTemplateCat_id
				left join dbo.XmlTemplateCat p5 with (nolock) on p4.XmlTemplateCat_pid = p5.XmlTemplateCat_id
				left join dbo.XmlTemplateCat p6 with (nolock) on p5.XmlTemplateCat_pid = p6.XmlTemplateCat_id
