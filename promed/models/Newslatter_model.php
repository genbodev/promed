...
class Newslatter_model extends swModel {
	...
	function loadList($data) {
		...
		if (!empty($data['Newslatter_Text'])) {
			$filter .= ' and N.Newslatter_Text LIKE :Newslatter_Text';
		...
		if (!empty($data['Person_Fio'])) {
			...
			$join .= '
				inner join v_PersonNewslatter PN with(nolock) ON PN.Newslatter_id = N.Newslatter_id
				inner join v_NewslatterAccept NA with(nolock) ON NA.NewslatterAccept_id = PN.NewslatterAccept_id
		...
		$query = "
			select
				-- select
				N.Newslatter_id
				,isnull(N.NewslatterGroupType_id,1) as NewslatterGroupType_id
				,convert(varchar(10), N.Newslatter_insDT, 104) as Newslatter_insDT
				,convert(varchar(10), N.Newslatter_begDate, 104) + isnull(' - ' + convert(varchar(10), N.Newslatter_endDate, 104), '') as Newslatter_Date
				,convert(varchar(5), N.Newslatter_begTime, 108) as Newslatter_Time
				,pm.PMUser_Name
				,N.Newslatter_Text
				,CASE WHEN ISNULL(N.Newslatter_IsActive, 1) = 1 THEN 'false' else 'true' END as Newslatter_IsActive
				-- end select
			from
				-- from
				v_Newslatter N with(nolock)
	...
	function load($data) {
		...
		$query = "
			select
				...
				,N.Lpu_id
				,CASE WHEN ISNULL(N.Newslatter_IsActive, 1) = 1 THEN 'false' else 'true' END as Newslatter_IsActive
				,CASE WHEN ISNULL(N.Newslatter_IsSMS, 1) = 1 THEN 'false' else 'true' END as Newslatter_IsSMS
				,CASE WHEN ISNULL(N.Newslatter_IsEmail, 1) = 1 THEN 'false' else 'true' END as Newslatter_IsEmail
				,convert(varchar(10), N.Newslatter_insDT, 104) as Newslatter_insDT
				,convert(varchar(10), N.Newslatter_begDate, 104) as Newslatter_begDate
				,convert(varchar(10), N.Newslatter_endDate, 104) as Newslatter_endDate
				,convert(varchar(5), N.Newslatter_begTime, 108) as Newslatter_begTime
				,N.Newslatter_Text
				,N.NewslatterType_id
				-- end select
			from
	...
	function loadPersonNewslatterList($data) {
		...
		switch ($groupType) {
			...
			case 3:
				if($groupType == 2){
					$select = " LSNL.LpuSectionNewslatter_id ";
					$prfix = 'LS';
					$join = "
						INNER JOIN v_LpuSectionNewslatter LSNL with (nolock) ON N.Newslatter_id = LSNL.Newslatter_id
						INNER JOIN dbo.v_LpuSection LS WITH(NOLOCK) ON LS.LpuSection_id = LSNL.LpuSection_id
						INNER JOIN v_LpuRegion LR WITH(nolock) ON LR.LpuSection_id = LS.LpuSection_id
					";
				}
				if($groupType == 3){
					$select = " LRNL.LpuRegionNewslatter_id ";
					$prfix = 'LR';
					$join = "
						INNER JOIN v_LpuRegionNewslatter LRNL with (nolock) ON N.Newslatter_id = LRNL.Newslatter_id
						INNER JOIN v_LpuRegion LR WITH(nolock) ON LR.LpuRegion_id = LRNL.LpuRegion_id
					";
				}
				$join .= "
					INNER JOIN v_PersonCard PC with (nolock) ON PC.LpuRegion_id = LR.LpuRegion_id and PC.LpuAttachType_id = 1
					INNER JOIN v_PersonCardAttach PCA with(nolock) on PCA.PersonCardAttach_id = pc.PersonCardAttach_id
					INNER JOIN v_PersonState PS with(nolock) ON PS.Person_id = PC.Person_id
					outer apply (
						select top 1 NA.NewslatterAccept_id
						from v_NewslatterAccept NA with(nolock)
						where NA.Person_id = ps.Person_id 
							AND NA.Lpu_id = {$prfix}.Lpu_id
							AND CAST(dbo.tzGetDate() AS DATE) BETWEEN cast(NA.NewslatterAccept_begDate as date) AND CAST(ISNULL(NA.NewslatterAccept_endDate, '2030-01-01') AS DATE)
				...
				$where = "
					...
					AND CAST(dbo.tzGetDate() AS DATE) BETWEEN cast(PC.PersonCard_begDate as date) AND CAST(ISNULL(PC.PersonCard_endDate, '2030-01-01') AS DATE)
				";
				break;
	...
	function save($data) {
		...
		$query = "
			declare
				@Newslatter_id bigint = :Newslatter_id,
				@Error_Code bigint,
				@Error_Message varchar(4000);
			exec {$procedure}
				@Newslatter_id = @Newslatter_id output,
				@Lpu_id = :Lpu_id,
			...
			select @Newslatter_id as Newslatter_id, @Error_Code as Error_Code, @Error_Message as Error_Msg;
		";
	...
	function savePersonNewslatter($data) {
		...
		$query = "
			declare
				@PersonNewslatter_id bigint = :PersonNewslatter_id,
				@Error_Code bigint,
				@Error_Message varchar(4000);
			exec {$procedure}
				@PersonNewslatter_id = @PersonNewslatter_id output,
				@Newslatter_id = :Newslatter_id,
				@NewslatterAccept_id = :NewslatterAccept_id,
				@pmUser_id = :pmUser_id,
				@Error_Code = @Error_Code output,
				@Error_Message = @Error_Message output;
			select @PersonNewslatter_id as PersonNewslatter_id, @Error_Code as Error_Code, @Error_Message as Error_Msg;
		";
	...
	function send() {
		...
		$query = "
			select
				...
				,N.Newslatter_Text
				,NA.NewslatterAccept_IsSMS
				,NA.NewslatterAccept_Phone
				,NA.NewslatterAccept_IsEmail
		...
		*/
		$query = "
			with NLTMP as (
				...
				from v_Newslatter N with(nolock)
				WHERE 1=1
					AND (
						(
							N.Newslatter_begDate <= CAST(dbo.tzGetDate() as DATE) and
					...
					AND N.Newslatter_begTime between CAST(DATEADD(minute, -5, dbo.tzGetDate()) as TIME) and CAST(dbo.tzGetDate() as TIME)
	...
	function loadLpuSectionNewslatterList($data) {
		...
		$query = "
			...
			from
				-- from
				v_Newslatter N with(nolock)
				inner join v_LpuSectionNewslatter LSN with(nolock) ON LSN.Newslatter_id = N.Newslatter_id
				inner join v_LpuSection LS with(nolock) ON LS.LpuSection_id = LSN.LpuSection_id
	...
	function saveLpuSectionNewslatter($data) {
		...
		$query = "
			declare
				@LpuSectionNewslatter_id bigint = :LpuSectionNewslatter_id,
				@Error_Code bigint,
				@Error_Message varchar(4000);
			exec {$procedure}
				@LpuSectionNewslatter_id = @LpuSectionNewslatter_id output,
				@Newslatter_id = :Newslatter_id,
				@LpuSection_id = :LpuSection_id,
				@pmUser_id = :pmUser_id,
				@Error_Code = @Error_Code output,
				@Error_Message = @Error_Message output;
			select @LpuSectionNewslatter_id as LpuSectionNewslatter_id, @Error_Code as Error_Code, @Error_Message as Error_Msg;
		";
	...
	function deleteLpuSectionNewslatter($data) {
		...
		$query = "
			...
			exec p_LpuSectionNewslatter_del
	...
	function loadLpuRegionNewslatterList($data) {
		...
		$query = "
			...
			from
				-- from
				v_Newslatter N with(nolock)
				inner join v_LpuRegionNewslatter LRN with(nolock) ON LRN.Newslatter_id = N.Newslatter_id
				inner join v_LpuRegion LR with(nolock) ON LR.LpuRegion_id = LRN.LpuRegion_id
	...
	function saveLpuRegionNewslatter($data) {
		...
		$query = "
			declare
				@LpuRegionNewslatter_id bigint = :LpuRegionNewslatter_id,
				@Error_Code bigint,
				@Error_Message varchar(4000);
			exec {$procedure}
				@LpuRegionNewslatter_id = @LpuRegionNewslatter_id output,
				@Newslatter_id = :Newslatter_id,
				@LpuRegion_id = :LpuRegion_id,
				@pmUser_id = :pmUser_id,
				@Error_Code = @Error_Code output,
				@Error_Message = @Error_Message output;
			select @LpuRegionNewslatter_id as LpuRegionNewslatter_id, @Error_Code as Error_Code, @Error_Message as Error_Msg;
		";
	...
	function deleteLpuRegionNewslatter($data) {
		...
		$query = "
			...
			exec p_LpuRegionNewslatter_del
	...
	function getNewslatterGroupTypeCode($data){
		...
		$query = "
			select
				N.Newslatter_id,
				NGT.NewslatterGroupType_id,
				NGT.NewslatterGroupType_Code
			from
				v_Newslatter N
