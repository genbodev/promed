...
class RegistryESStorage_model extends swModel {
	/**
	 *	Конструктор
	 */
	...
	public function loadRegistryESStorageGrid($data) {
		...
		$query = "
			select
				...
				RESS.RegistryESStorage_NumQuery,
				RESS.EvnStickBase_Num,
				convert(varchar(10), RESS.RegistryESStorage_updDT, 104) as RegistryESStorage_updDT,
				case when RESS.EvnStickBase_id is not null then 2 else 1 end as RegistryESStorage_IsUsed,
				ISNULL(puc.pmUser_Name, '') + ' (' + rtrim(puc.pmUser_Login) + ')' as pmUser_Name
			from
				v_RegistryESStorage RESS with (nolock)
				left join v_pmUserCache puc with (nolock) on puc.pmUser_id = RESS.pmUser_insID
			where
				{$filters}
				...
				and exists( -- рамках одного блока (Один номер запроса) есть хотя бы один не использованный номер
					select top 1
						RESS2.RegistryESStorage_id
					from
						v_RegistryESStorage RESS2 with (nolock)
					where
	...
	public function loadRegistryESStorageNumQuery($data) {
		...
		$query = "
			select
				ISNULL(MAX(cast(RESS.RegistryESStorage_NumQuery as int)), 0) + 1 as RegistryESStorage_NumQuery
			from
				v_RegistryESStorage RESS with (nolock)
			where
					...
					if ($status->nodeValue == '1') {
						...
						foreach($nums as $num) {
							if(!empty($num->nodeValue)) {
								...
								$this->db->query("
									declare
										@RegistryESStorage_id bigint,
										@Error_Code bigint,
										@Error_Message varchar(4000);
										
									exec p_RegistryESStorage_ins
										@RegistryESStorage_id = @RegistryESStorage_id output,
...
'.$EncryptedData.'
	...
	public function getEvnStickNum($data) {
		...
		$resp = $this->queryResult("
			...
			where
				...
				and DATEDIFF(mi, ISNULL(RegistryESStorage_bookDT, '2010-01-01'), @curDT) > 30
				and Lpu_id = :Lpu_id
		...
		if (!empty($resp[0]['RegistryESStorage_id'])) {
			$this->db->query("
				update RegistryESStorage with (rowlock) set RegistryESStorage_bookDT = dbo.tzGetDate() where RegistryESStorage_id = :RegistryESStorage_id
			", array(
	...
	public function unbookEvnStickNum($data) {
		...
		$result = $this->db->query("
			select 
				RES.RegistryESStorage_id
			from RegistryESStorage RES with (nolock) 
			left join v_EvnStickBase ESB with (nolock) on ESB.EvnStickBase_id = RES.EvnStickBase_id
			left join v_EvnStickWorkRelease ESWR with (nolock) on ESWR.EvnStickBase_id = RES.EvnStickBase_id
			where 
				RES.RegistryESStorage_id = :RegistryESStorage_id and (
					isnull(ESB.EvnStickBase_IsInReg,1) <> 2 and
					isnull(ESB.EvnStickBase_IsPaid,1) <> 2 and
					isnull(ESWR.EvnStickWorkRelease_IsInReg,1) <> 2 and
					isnull(ESWR.EvnStickWorkRelease_IsPaid,1) <> 2
				)
		", array(
		...
		if(isset($result[0])) {
			$this->db->query("
				update RegistryESStorage with (rowlock) 
				set 
					RegistryESStorage_bookDT = null 
				where 
