...
class OnkoConsult_model extends swModel {
	...
	function loadList($data) {
		...
		$query = "
			select
				...
				,'edit' as accessType
				,OC.MorbusOnko_id
				,OC.MorbusOnkoVizitPLDop_id
				,OC.MorbusOnkoLeave_id
				,OC.MorbusOnkoDiagPLStom_id
				,convert(varchar(10), OC.OnkoConsult_consDate, 104) as OnkoConsult_consDate
				,convert(varchar(10), OC.OnkoConsult_PlanDT, 104) as OnkoConsult_PlanDT
				,OHT.OnkoHealType_Name
				,OCR.OnkoConsultResult_Name
				,:Evn_id as MorbusOnko_pid
				,MO.Morbus_id
				-- end select
			from
				-- from
				v_OnkoConsult OC with(nolock)
				inner join v_MorbusOnko MO WITH (NOLOCK) on MO.MorbusOnko_id = OC.MorbusOnko_id
				left join v_OnkoHealType OHT with(nolock) on OHT.OnkoHealType_id = OC.OnkoHealType_id
				left join v_OnkoConsultResult OCR with(nolock) on OCR.OnkoConsultResult_id = OC.OnkoConsultResult_id
				-- end from
			where
				-- where
				{$filter}
	...
	function load($data) {
		...
		if(getRegionNick() == 'ufa') {
			$fields .= "
				,OC.UslugaComplex_id
				,OC.DrugTherapyScheme_id
				,OC.OnkoChemForm_id
				,OC.OnkoConsult_CouseCount
				,OC.OnkoConsult_Commentary
			...
			";
			$joins .= "left join v_MedStaffFact MSF with(nolock) on MSF.MedStaffFact_id = OC.MedStaffFact_id";
		}
	...
	function save($data) {
		...
		$query = "
			declare
				@OnkoConsult_id bigint = :OnkoConsult_id,
				@Error_Code bigint,
				@Error_Message varchar(4000);
			exec {$procedure}
				@OnkoConsult_id = @OnkoConsult_id output,
			...
			select @OnkoConsult_id as OnkoConsult_id, @Error_Code as Error_Code, @Error_Message as Error_Msg;
		";
	...
	function loadUslugaList($data) {
		$query = "
			select 
				oc.UslugaComplex_id 
			from  v_OnkoConsultUsluga  OC with(nolock)
	...
	function loadListDrugTherapySchemeList($data) {
		$query = "
			select 
				oc.DrugTherapyScheme_id 
			from  v_OnkoConsultDrugScheme  OC with(nolock)
			where OnkoConsult_id = :OnkoConsult_id
		";
		$result = $this->db->query($query, $data);
	...
	function checkDoubles($data) {
		...
		$query = "
			select
				OnkoConsult_id
			from
				v_OnkoConsult with(nolock)
			where
				{$filter} and
				OnkoConsult_id != isnull(:OnkoConsult_id, 0) and
