...
class EvnReceptGeneral_model extends EvnAbstract_model
{
	...
	{
		switch ($enum_name) {
			case 'ReceptValid': $store = $this->receptValidStore; break;
			case 'ReceptForm': $store = $this->receptFormStore; break;
			case 'WhsDocumentCostItemType': $store = $this->whsDocumentCostItemTypeStore; break;
			default: $store = null; break;
		}
		if (empty($store)) {
	...
	function getReceptValidStore()
	{
		if (empty(self::$_receptValidStore)) {
			...
			select ReceptValid_id, ReceptValid_Code, ReceptValid_Name from dbo.v_ReceptValid (nolock)'
			);
	...
	function getWhsDocumentCostItemTypeStore()
	{
		if (empty(self::$_whsDocumentCostItemTypeStore)) {
			$result = $this->db->query('
			select * from v_WhsDocumentCostItemType (nolock)'
			);
			if (false == is_object($result)) {
	...
	function getReceptFormStore()
	{
		if (empty(self::$_receptFormStore)) {
			...
			select
				...
				ReceptForm_Code,
				ReceptForm_Name,
				ReceptForm_IsPrivilege,
				ReceptForm_begDate,
				ReceptForm_endDate
				from v_ReceptForm (nolock)'
			);
			if (false == is_object($result)) {
	...
	function genSerNum()
	{
		$row = $this->getFirstRowFromQuery("
			select max(EvnReceptGeneral_Num) as num, max(EvnReceptGeneral_Ser) as ser from EvnReceptGeneral (nolock)
		");
		...
		switch (strlen($row['ser'])) {
			case 1: $row['ser'] = '0000' . $row['ser']; break;
			case 2: $row['ser'] = '000' . $row['ser']; break;
			case 3: $row['ser'] = '00' . $row['ser']; break;
			case 4: $row['ser'] = '0' . $row['ser']; break;
			case 5: $row['ser'] = '' . $row['ser']; break;
			default: $row['ser'] = '00001'; $row['num']++; break;
		}
		switch (strlen($row['num'])) {
			case 1: $row['ser'] = '000' . $row['ser']; break;
			case 2: $row['ser'] = '00' . $row['ser']; break;
			case 3: $row['ser'] = '0' . $row['ser']; break;
			case 4: $row['ser'] = '' . $row['ser']; break;
			default: $row['ser'] = '00001'; $row['num'] = '5901'; break;
		}
		return $row;
	}
}