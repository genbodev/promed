...
class ServiceYC_model extends swModel {
	...
	function saveCertificateCenter($data) {
		...
		$resp = $this->queryResult("
			select top 1
				...
				CC.CertificateCenter_Name,
				CC.CertificateCenter_Nick,
				CC.CertificateCenter_INN,
				CC.CertificateCenter_Ogrn,
				CCSH.CertificateCenterStatusHist_id,
				CCS.CertificateCenterStatus_id,
				CCS.CertificateCenterStatus_Name
			from
				v_CertificateCenter CC (nolock)
				left join CertificateCenterStatusHist CCSH (nolock) on CCSH.CertificateCenter_id = CC.CertificateCenter_id
				left join CertificateCenterStatus CCS (nolock) on CCS.CertificateCenterStatus_id = CCSH.CertificateCenterStatus_id
			where
		...
		$query = "	
			declare
				@CertificateCenter_id bigint,
				@Error_Code int,
				@Error_Message varchar(4000);
			set @CertificateCenter_id = :CertificateCenter_id;
			exec {$procCC}
				@CertificateCenter_id = @CertificateCenter_id output,
			...
			select @CertificateCenter_id as CertificateCenter_id, @Error_Code as Error_Code, @Error_Message as Error_Msg;
		";
		...
		if (isset($certificateCenter_id[0]['CertificateCenter_id'])) {
			...
			$query = "	
				declare
					@CertificateCenterStatusHist_id bigint,
					@Error_Code int,
					@Error_Message varchar(4000);
				set @CertificateCenterStatusHist_id = :CertificateCenterStatusHist_id;
				exec {$procCCS}
					@CertificateCenterStatusHist_id = @CertificateCenterStatusHist_id output,
				...
				select @CertificateCenterStatusHist_id as CertificateCenterStatusHist_id, @Error_Code as Error_Code, @Error_Message as Error_Msg;
			";
	...
	{
		$query = "
			SELECT CertificateCenterStatus_id
