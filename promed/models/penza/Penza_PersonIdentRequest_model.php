...
class Penza_PersonIdentRequest_model extends PersonIdentRequest_model {
	...
	function processPolisIdentData($person, $identData, $data) {
		$query = "
			...
			left join v_OmsSprTerr OST with(nolock) on OST.KLRgn_id = 58	--ПЕНЗЕНСКАЯ ОБЛАСТЬ
		";
	...
	function PersonIdentPackage() {
		...
		$query = "
			select top {$limit}
				...
				convert(varchar(10), PRD.Person_BirthDay, 104) as birthdate,
			...
			where 
				...
				and nullif(rtrim(PRD.Docum_Num), '') is not null
			order by
