 ...
 *
*/
class EvnFuncRequest_model extends swModel
{
	/**
	 * constructor
	 */
		...
		if ( !empty($data['EvnDirection_id']) ) {
			$queryList[] = "
				...
				from 
					...
					left join v_EvnXml ex with (nolock) on ex.Evn_id = eup.EvnUslugaPar_id
				where 
	...
	function cancelEvnUslugaPar($data) {
		$query = "
			...
			from
				...
				left join v_TimetableResource_lite ttr (nolock) on ttr.EvnDirection_id = eup.EvnDirection_id
				left join v_Evn e (nolock) on e.Evn_id = eup.EvnUslugaPar_pid
				outer apply (
	...
	function cancelDirection($data)
	{
		...
		$query = "
			...
			from
				...
				left join v_TimetableResource_lite ttr (nolock) on ttr.TimetableResource_id = d.TimetableResource_id
		...
		$query = "
			...
			begin try
				...
				if ( :TimetableResource_id is not null )
					begin
						exec p_TimetableResource_cancel
	...
	function ReCacheFuncRequestStatus($data) {
		...
		$query = "
			...
			from
				...
				left join v_TimetableResource_lite TTR (nolock) on ED.EvnDirection_id = TTR.EvnDirection_id
				outer apply(
					select top 1
						eup.EvnUslugaPar_id
					from
	...
	function checkUslugaComplexMedServiceTimeTable($data) {
		...
		$query = "
			...
			from
				...
				inner join v_TimetableResource_lite TTR (nolock) on TTR.UslugaComplexMedService_id = ucms.UslugaComplexMedService_id
	...
	function loadEvnFuncRequestList($data) {
		...
		if ( !empty($data['Search_SurName']) ) {
			if (allowPersonEncrypHIV($data['session']) && isSearchByPersonEncrypHIV($data['Search_SurName'])) {
				$personFilters[] = "PEH.PersonEncrypHIV_Encryp like :Search_SurName";
			} else {
				$personFilters[] = "PS.Person_SurName LIKE (:Search_SurName+'%')";
			}
			$queryParams['Search_SurName'] = rtrim($data['Search_SurName']);
		}
		
		if ( !empty($data['Search_FirName']) ) {
			$personFilters[] = "PS.Person_FirName LIKE (:Search_FirName+'%')";
			$queryParams['Search_FirName'] = rtrim($data['Search_FirName']);
		}
		
		if ( !empty($data['Search_SecName']) ) {
			$personFilters[] = "PS.Person_SecName LIKE (:Search_SecName+'%')";
		...
		if ( !empty($data['begDate']) && !empty($data['endDate']) ) {
			...
			$EFR_EQ_Filters[] = "(
				EFR.EvnStatus_id = @EvnStatus_id_NewFunc
				OR
		...
		if (allowPersonEncrypHIV($data['session'])) {
			...
			$selectPersonData = "case when PEH.PersonEncrypHIV_Encryp is null then [dbo].[getPersonPhones](ed.Person_id, '<br />') else null end as Person_Phone,
				case when PEH.PersonEncrypHIV_Encryp is null then (PS.Person_SurName + ' ' + IsNull(PS.Person_FirName, '') + ' ' + IsNull(PS.Person_SecName,'')) else PEH.PersonEncrypHIV_Encryp end as Person_FIO,
				case when PEH.PersonEncrypHIV_Encryp is null then convert(varchar(10), PS.Person_BirthDay, 104) else null end as Person_BirthDay,";
		}
		...
		if(empty($commonFilters) || (isset($data['wnd_id']) && !empty($data['wnd_id']) && ($data['wnd_id'] != 'swWorkPlaceFuncDiagWindow'))) {
			...
			$querys[] = "
				select
					...
					convert(varchar(10),TTR.TimetableResource_begTime,104) as group_name,
					case when TTR.TimetableResource_begTime is not null
						then convert(varchar,cast(TTR.TimetableResource_begTime as datetime),104)
						else null
					end as TimetableResource_begDate,
					null as TimetableResource_Date,
					r.Resource_id,
					r.Resource_Name,
					isnull(convert(varchar(5), TTR.TimetableResource_begTime, 108),'б/з') as TimetableResource_begTime,
					convert(varchar(10),TTR.TimetableResource_begTime,104) as TimetableResource_Type,
				...
				FROM v_TimetableResource_lite TTR with (nolock)
					...
					--LEFT JOIN v_pmUserCache PUC with(nolock) on PUC.PMUser_id = TTR.pmUser_insID
				WHERE 
		...
		$querys[] = "
			-- variables
			declare @EvnStatus_id_NewFunc bigint = (select top 1 EvnStatus_id from v_EvnStatus with (nolock) where EvnStatus_SysNick = 'FuncNew');
			-- end variables
			...
			select
				...
				case
					when TimetableResource_begTime is not null then convert(varchar,TimetableResource_begTime,104)
					else convert(varchar,TimetableResource_insDT,104)
			...
			FROM v_EvnFuncRequest efr (nolock)
				inner join v_EvnDirection_all ed (nolock) on ed.EvnDirection_id = efr.EvnFuncRequest_pid and ISNULL(ed.EvnStatus_id, 16) not in (12, 13)
				left join v_EvnPrescrDirection epd (nolock) on ed.EvnDirection_id = epd.evnDirection_id
				left join v_EvnPrescrFuncDiag epfd (nolock) on epfd.EvnPrescrFuncDiag_id = epd.EvnPrescr_id
				...
				left join v_Resource R (nolock) on R.Resource_id = TTR.Resource_id
				...
				left join v_Person pers (nolock) on pers.Person_id = ED.Person_id
				{$joinPersonEncrypHIV}
				...
				left join v_ElectronicTalonRedirect etr (nolock) on etr.EvnDirection_uid = TTR.EvnDirection_id
				left join v_ElectronicTalon et (nolock) on (et.EvnDirection_uid = TTR.EvnDirection_id or et.EvnDirection_id = TTR.EvnDirection_id)
				left join v_ElectronicTalonStatus ets (nolock) on ets.ElectronicTalonStatus_id = et.ElectronicTalonStatus_id
				left join v_Evn EPC with (nolock) on EPC.Evn_id = ED.EvnDirection_pid
				...
				--left join v_ElectronicTalonRedirect etr (nolock) on etr.EvnDirection_uid = et.EvnDirection_uid
			WHERE 
				efr.MedService_id = :MedService_id
				and exists(Select top 1 1 from v_EvnQueue eq (nolock) where eq.EvnDirection_id = ed.EvnDirection_id and eq.EvnQueue_failDT is null and eq.EvnQueue_recDT is null)
		...
		$querys[] = "
			declare @EvnStatus_id_FuncDonerec bigint = (select top 1 EvnStatus_id from v_EvnStatus with (nolock) where EvnStatus_SysNick = 'FuncDonerec');
			...
			WITH table1 as (
				...
				from 
					v_TimeTableResource_lite ttr with (nolock)
						left join v_Resource r with (nolock) on ttr.Resource_id = r.Resource_id
						left join v_MedService ms with (nolock) on r.MedService_id = ms.MedService_id
						left join Evn with (nolock) on ttr.EvnDirection_id = Evn.Evn_pid and Evn.Evn_deleted = 1
						left join v_EvnDirection_all ed with (nolock) on ed.EvnDirection_id = ttr.EvnDirection_id and isnull(ed.EvnStatus_id, 16) not in (12,13)
						left join v_EvnDirection_all ed2 with (nolock) on ed2.EvnDirection_id = ttr.EvnDirection_id
						left join v_EvnPrescrDirection epd (nolock) on ed.EvnDirection_id = epd.evnDirection_id
						left join v_EvnPrescrFuncDiag epfd (nolock) on epfd.EvnPrescrFuncDiag_id = epd.EvnPrescr_id
						left join EvnFuncRequest efr with (nolock) on efr.EvnFuncRequest_id = Evn.Evn_id
				WHERE
			...
			select
				...
				convert(varchar(10),efr.TimetableResource_begTime,104) as group_name,
				case
					when efr.TimetableResource_begTime is not null then convert(varchar,cast(efr.TimetableResource_begTime as datetime),104)
					when EFR.EvnStatus_id = @EvnStatus_id_FuncDonerec then convert(varchar,cast(efr.EvnFuncRequest_statusDate as datetime),104)
					else null
				end as TimetableResource_begDate,
				efr.TimetableResource_Date,
				efr.Resource_id,
				efr.Resource_Name,
				isnull(convert(varchar(5), efr.TimetableResource_begTime, 108),'б/з') as TimetableResource_begTime,
				convert(varchar(10),efr.TimetableResource_begTime,104) as TimetableResource_Type,
				case when 2 = ISNULL(EFR.EvnPrescrFuncDiag_IsCito, 1) then 'true' else 'false' end as EvnDirection_IsCito,
				convert(varchar(10), EFR.EvnDirection_setDT, 104) as EvnDirection_setDT,
				...
				RCCR.RemoteConsultCenterResearch_id,
				RCCR.RemoteConsultCenterResearch_status,
				case when EFR.EvnStatus_id = @EvnStatus_id_FuncDonerec then 'true' else 'false' end as FuncRequestState,
				case
					when PUC.PMUser_surName is null then ''
					else PUC.PMUser_surName+' '+isnull(left(PUC.PMUser_firName,1),'')+' '+isnull(left(PUC.PMUser_secName,1),'')
				end as Operator,
				...
				etr.ElectronicService_id as toElectronicService_id,
				etr.ElectronicService_uid as fromElectronicService_id
			FROM table1 efr (nolock)
				left join v_Lpu LpuFrom with (nolock) on LpuFrom.Lpu_id = efr.Lpu_sid
				left join v_LpuSection LpuSectionFrom with (nolock) on LpuSectionFrom.LpuSection_id = efr.LpuSection_id
				inner join v_PersonState PS (nolock) on PS.Person_id = efr.Person_id
				left join v_Person pers (nolock) on pers.Person_id = efr.Person_id
				--left join v_EvnStatus ES (nolock) on ES.EvnStatus_id = EFR.EvnStatus_id
				outer apply(
					SELECT top 1 EvnUslugaPar_id, EvnUslugaPar_setDate from v_EvnUslugaPar with (nolock) where EvnDirection_id = efr.EvnDirection_id
				) EvnUslugaPar
				...
				left join v_ElectronicTalonRedirect etr (nolock) on etr.EvnDirection_uid = efr.EvnDirection_id
				--left join v_ElectronicTalon et (nolock) on (et.EvnDirection_uid = efr.EvnDirection_id or et.EvnDirection_id = efr.EvnDirection_id)
				outer apply (
					Select top 1 * from  v_ElectronicTalon et (nolock) where (et.EvnDirection_uid = efr.EvnDirection_id or et.EvnDirection_id = efr.EvnDirection_id)
				) et
				left join v_ElectronicTalonStatus ets (nolock) on ets.ElectronicTalonStatus_id = et.ElectronicTalonStatus_id
				left join v_Evn EPC with (nolock) on EPC.Evn_id = efr.EvnDirection_pid
				{$joinPersonEncrypHIV}
			WHERE
				(1 = 1)
				{$filterByUslugaResultDT}
	...
	function mLoadEvnFuncRequestList($data) {
		...
		// 2a. запрос по заявкам (все из EvnFuncRequest) не записанные (из очереди и остальные)
		$queuedQuery = "
			declare @EvnStatus_id_NewFunc bigint = (
				select top 1 EvnStatus_id 
				from v_EvnStatus with (nolock) 
			...
			into #table1
				FROM
					EvnFuncRequest EFR  with (nolock)
					inner join Evn with (nolock) on Evn.Evn_id = EFR.Evn_id and ISNULL(Evn.Evn_deleted, 1) = 1
					inner JOIN v_EvnDirection_all ED with (nolock) ON ed.EvnDirection_id = Evn.Evn_pid and ISNULL(ed.EvnStatus_id, 16) not in (12, 13)
					left join v_Evn EPC with (nolock) on EPC.Evn_id = ED.EvnDirection_pid
				WHERE
					Evn.Lpu_id = :Lpu_id
			...
			into #table2
			FROM
				EvnFuncRequest efr (nolock)
				inner join Evn with (nolock) on Evn.Evn_id = EFR.Evn_id and ISNULL(Evn.Evn_deleted, 1) = 1
				inner join v_EvnDirection_all ed (nolock) on ed.EvnDirection_id = Evn.Evn_pid and ISNULL(ed.EvnStatus_id, 16) not in (12, 13)
				left join v_EvnStatus ES (nolock) on ES.EvnStatus_id = Evn.EvnStatus_id
				left join v_Evn EPC with (nolock) on EPC.Evn_id = ED.EvnDirection_pid
			WHERE 
				efr.MedService_id = :MedService_id
				-- бывший EFR_EQ_Filters
				and (
					Evn.EvnStatus_id = @EvnStatus_id_NewFunc OR
					(
			...
			from (
				select
					...
					null as group_name,
					case
						when table2.EvnStatus_SysNick = 'FuncDonerec' 
						then convert(varchar,cast(table2.EvnFuncRequest_statusDate as datetime),104)
						else null
					end as TimetableResource_begDate,
					r.Resource_id,
					r.Resource_Name,
				...
				FROM
					...
					left join v_TimetableResource_lite TTR (nolock) on TTR.EvnDirection_id = table2.EvnDirection_id
					left join v_Resource R (nolock) on R.Resource_id = TTR.Resource_id
					left join v_Lpu LpuFrom with (nolock) on LpuFrom.Lpu_id = table2.Lpu_sid
					left join v_LpuSection LpuSectionFrom with (nolock) on LpuSectionFrom.LpuSection_id = table2.LpuSection_id
					left join v_PersonState PS (nolock) on PS.Person_id = table2.Person_id
					...
					outer apply(
						select top 1 EvnUslugaPar_id from v_EvnUslugaPar with (nolock) where EvnDirection_id = table2.EvnDirection_id
					) EvnUslugaPar
					LEFT JOIN v_RemoteConsultCenterResearch RCCR with (nolock) on RCCR.EvnUslugaPar_id = EvnUslugaPar.EvnUslugaPar_id
					LEFT JOIN v_MedServiceLink MSL with (nolock) on MSL.MedService_lid = :MedService_lid AND MSL.MedServiceLinkType_id = (select top 1 MSLT.MedServiceLinkType_id from v_MedServiceLinkType MSLT (nolock) where MSLT.MedServiceLinkType_Code = :MedServiceLinkType_Code)
					LEFT JOIN v_pmUserCache PUC with(nolock) on PUC.PMUser_id = table2.pmUser_insID
					CROSS APPLY (SELECT TOP 1 1 AS Eq from v_EvnQueue eq (nolock) where eq.EvnDirection_id = table2.EvnDirection_id and eq.EvnQueue_failDT is null and eq.EvnQueue_recDT is NULL) AS EQ
				WHERE 
				...
				FROM
					...
					outer apply (
						Select top 1 EvnUslugaPar_id, EvnUslugaPar_setDT from v_EvnUslugaPar EvnUslugaPar with (nolock) where EvnUslugaPar.EvnDirection_id = table1.EvnDirection_id
					) EvnUslugaPar
					LEFT JOIN v_EvnStatus ES (nolock) on ES.EvnStatus_id = table1.EvnStatus_id
					LEFT JOIN v_PersonState PS with (nolock) on PS.Person_id = table1.Person_id
					left join v_PersonEncrypHIV peh with(nolock) on peh.Person_id = PS.Person_id
					LEFT JOIN v_RemoteConsultCenterResearch RCCR with (nolock) on RCCR.EvnUslugaPar_id = EvnUslugaPar.EvnUslugaPar_id
					LEFT JOIN v_MedServiceLink MSL with (nolock) on MSL.MedService_lid = :MedService_lid AND MSL.MedServiceLinkType_id = (select top 1 MSLT.MedServiceLinkType_id from v_MedServiceLinkType MSLT (nolock) where MSLT.MedServiceLinkType_Code = :MedServiceLinkType_Code)
					LEFT JOIN v_pmUserCache UC with(nolock) on UC.PMUser_id = table1.pmUser_insID
				WHERE
					1 = 1
		...
		// дублирующий 2a. запрос по заявкам (все из EvnFuncRequest) не записанные (из очереди и остальные)
		// пока убрал!
		//$querys[] = "
		//	-- variables
		//	declare @EvnStatus_id_NewFunc bigint = (
		...
		//		)
		//		-- бывший EFR_EQ_Filters
		//		and (EFR.EvnStatus_id = @EvnStatus_id_NewFunc OR
		//		(
		//			ES.EvnStatus_SysNick IN ('FuncDonerec', 'FuncDone')
		...
		// 2b. запрос по заявкам (все из EvnFuncRequest) только записанные
		$requestsQuery = "
			declare @EvnStatus_id_FuncDonerec bigint = (
				select top 1 
					EvnStatus_id 
				from v_EvnStatus with (nolock) 
	...
	function loadEvnFuncQueueRequestList($data) {
		...
		if ( !empty($data['begDate']) && !empty($data['endDate']) ) {
			...
			$EFR_EQ_Filters[] = "(
				Evn.EvnStatus_id = @EvnStatus_id_NewFunc
				OR
				(ES.EvnStatus_SysNick IN ('FuncDonerec', 'FuncDone') and CAST(Evn.Evn_statusDate as date) between :begDate and :endDate)
			)";
			$EFRFilters[] = "(CAST(Evn.Evn_setDT as date) between :begDate and :endDate)";
			$this->load->helper('Reg');
			$queryParams['begDay_id'] = TimeToDay(strtotime($data['begDate']));
			$queryParams['endDay_id'] = TimeToDay(strtotime($data['endDate']));
			$queryParams['begDate'] = $data['begDate'];
		...
		if ( !empty($data['Search_SurName']) ) {
			if (allowPersonEncrypHIV($data['session']) && isSearchByPersonEncrypHIV($data['Search_SurName'])) {
				$commonFilters[] = "PEH.PersonEncrypHIV_Encryp like :Search_SurName";
			} else {
		...
		if( !empty($data['Search_PersonInn']) || $data['Search_PersonInn'] == '0') {
			$commonFilters[] = "PS.Person_Inn LIKE (:Search_PersonInn+'%')";
			$queryParams['Search_PersonInn'] = $data['Search_PersonInn'];
		}
		...
		$query = "
			...
			from
				...
				select
					...
					null as group_name,
					case
						when table2.EvnStatus_SysNick = 'FuncDonerec' then convert(varchar,cast(table2.EvnFuncRequest_statusDate as datetime),104)
						else null
					end as TimetableResource_begDate,
					r.Resource_id,
					r.Resource_Name,
	...
	function loadEvnFuncRequestViewList($data) {
		...
		$TTRFilters[] = "(ISNULL(res.MedService_id, UCMS.MedService_id) = :MedService_id)"; // Направление в эту определенную службу
		...
		$querys[0] = "
			select
				...
				'false' as FuncRequestState,
				'' as Operator,
				isnull(UCMS.UslugaComplexMedService_id,1) as UslugaComplexMedService_id,
			...
			LEFT JOIN v_UslugaComplexMedService UCMS (nolock) on UCMS.UslugaComplexMedService_id = ucr.UslugaComplexMedService_id
				left join v_UslugaComplex UC (nolock) on UC.UslugaComplex_id = UCMS.UslugaComplex_id
				LEFT JOIN v_MedService MS with (nolock) ON MS.MedService_id = ISNULL(res.MedService_id, UCMS.MedService_id)
				LEFT JOIN v_MedServiceLink MSL with (nolock) on MSL.MedService_lid = :MedService_lid AND MSL.MedServiceLinkType_id = (select top 1 MSLT.MedServiceLinkType_id from v_MedServiceLinkType MSLT (nolock) where MSLT.MedServiceLinkType_Code = :MedServiceLinkType_Code)
				--LEFT JOIN v_pmUserCache PUC with(nolock) on PUC.PMUser_id = TTR.pmUser_insID
			WHERE 
				" . implode(' and ', $TTRFilters) . "
	...
	function getEvnProcRequest($data) {
		$query = "
			...
			FROM v_EvnDirection_all ED with (nolock)
				...
				LEFT JOIN v_TimetableResource_lite TTR with (nolock) ON ED.EvnDirection_id = TTR.EvnDirection_id
	...
	function getEvnFuncRequest($data) {
		...
		$query = "
			select
				...
				EFR.Diag_id,
				ISNULL(TTR.Resource_id, ED.Resource_id) as Resource_id, -- берём из бирки, если есть бирка
				EFR.EvnFuncRequest_Ward,
				ISNULL(ED.EvnDirection_IsReceive, 1) as EvnDirection_IsReceive,
				ED.EvnDirection_IsAuto,
				isnull(EFR.StudyTarget_id,ED.StudyTarget_id) as StudyTarget_id,
		...
		if ($EvnDirection_id) {
			...
			$sql = "
				DECLARE
					...
					@Error_Code bigint,
					@Error_Message varchar(4000);
				EXEC dbo.p_EvnFuncRequest_{$insupd}
					@EvnFuncRequest_id = @EvnFuncRequest_id output, 
					@EvnFuncRequest_pid = :EvnFuncRequest_pid,
					@EvnFuncRequest_setDT =  :EvnFuncRequest_setDT,
					@MedService_id = :MedService_id,
    ...
    private function saveEvnDirection($data)
    {
        ...
        switch ((int)$data['PrehospDirect_id']) {
            case 1: // 1 Отделение ЛПУ
                ...
                break;
            case 2: // 2 Другое ЛПУ
				$Lpu_id = $this->getFirstResultFromQuery('SELECT top 1 Lpu_id FROM v_Lpu (nolock) WHERE Org_id = :Org_sid',array('Org_sid' => $data['Org_sid']));
	...
	function loadEvnUslugaEditForm($data) {
		...
		if (getRegionNick() == 'perm') {
			...
			$join .= " left join v_UslugaComplex uc (nolock) on uc.UslugaComplex_id = eup.UslugaComplex_id";
			$join .= " outer apply (
				select top 1
					es.EvnSection_id
				from
				...
				where
					...
					and exists (
						...
						from
							...
							inner join v_EvnUsluga eu (nolock) on eu.EvnUsluga_pid = es2.EvnSection_id
							inner join v_UslugaComplex uc2 (nolock) on uc2.UslugaComplex_id = eu.UslugaComplex_id
						where
		...
		if (getRegionNick() === 'kz') {
			...
			";
            $join .= "
				left join r101.v_UslugaMedTypeLink UMTL (nolock) ON UMTL.Evn_id=EUP.EvnUslugaPar_id
			";
        }
		...
		$query = "
			select
				...
				EUP.EvnUslugaPar_IsPaid,
				EUP.EvnUslugaPar_IndexRep,
				EUP.EvnUslugaPar_IndexRepInReg,
				EUP.MedicalCareFormType_id as EUP_MedicalCareFormType_id,
				ED.Diag_id as DirectionDiag_id,
				EUP.EvnUslugaPar_NumUsluga,
				EUP.EvnUslugaPar_UslugaNum,
				EUP.StudyResult_id,
				EDlink.EvnDirection_id as link_EvnDirection_id,
				EDlink.EvnDirection_Num as link_EvnDirection_Num,
				convert(varchar(10), EDlink.EvnDirection_setDate, 104) as link_EvnDirection_setDate,
			...
			FROM v_EvnUslugaPar EUP with (nolock)
				...
				outer apply (
					select (
						...
						from v_ToothNumEvnUsluga tneu with (nolock)
	...
	function defineUslugaParams($data) {
		...
		$query = "
			...
			from
				v_EvnPrescrDirection epd (nolock)
				inner join v_EvnPrescr ep (nolock) on epd.EvnPrescr_id = ep.EvnPrescr_id
				left join v_Evn e (nolock) on e.Evn_id = ep.EvnPrescr_pid
			where
	...
	public function saveEvnUslugaEditForm( $data ){
		...
		$query = "
			...
			from
				v_EvnDirection_all ed (nolock)
				left join v_EvnQueue eq (nolock) on eq.EvnQueue_id = ed.EvnQueue_id
				left join v_TimetableResource_lite ttr (nolock) on ttr.TimetableResource_id = ed.TimetableResource_id
			where
				ed.EvnDirection_id = :EvnDirection_id
		";
		$resp = $this->queryResult($query, array(
		...
		if (getRegionNick() == 'penza' && !empty($data['EvnUslugaPar_UslugaNum']) && !empty($resp[0]['MedService_id'])) {
			$query = "
				...
				where
				 	...
				 	and year(EUP.EvnUslugaPar_setDT) = year(:EvnUslugaPar_setDate)
