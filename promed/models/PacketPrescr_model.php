 ...
 * @version			07.05.2018
 */
class PacketPrescr_model extends swModel {
	/**
	...
	function createPacketPrescr($data)
	{
		...
		$resp_packet = $this->queryResult("
			declare
				@PacketPrescr_id bigint,
				@ErrCode int,
				@ErrMessage varchar(4000);
			set @PacketPrescr_id = null;
			exec p_PacketPrescr_ins
				@PacketPrescr_id = @PacketPrescr_id output,
			...
			select @PacketPrescr_id as PacketPrescr_id, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
		", array(
		...
		$resp_lpu = $this->queryResult("
			declare
				@PacketPrescrLpu_id bigint,
				@ErrCode int,
				@ErrMessage varchar(4000);
			set @PacketPrescrLpu_id = null;
			exec p_PacketPrescrLpu_ins
				@PacketPrescrLpu_id = @PacketPrescrLpu_id output,
				...
				@Error_Code = @ErrCode output,
				@Error_Message = @ErrMessage output;
			select @PacketPrescrLpu_id as PacketPrescrLpu_id, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
		", array(
			...
			foreach ($diags as $diag) {
				if ( !empty($diag) && intval($diag)>0 ) { // Проверка на отсутствие диагноза в поле
					$resp_ppd = $this->queryResult("
						declare
							@PacketPrescrDiag_id bigint,
							@ErrCode int,
							@ErrMessage varchar(4000);
						set @PacketPrescrDiag_id = null;
						exec p_PacketPrescrDiag_ins
							@PacketPrescrDiag_id = @PacketPrescrDiag_id output,
							@PacketPrescr_id = :PacketPrescr_id,
							@Diag_id = :Diag_id,
							@pmUser_id = :pmUser_id,
							@Error_Code = @ErrCode output,
							@Error_Message = @ErrMessage output;
						select @PacketPrescrDiag_id as PacketPrescrDiag_id, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
						", array(
							'PacketPrescr_id' => $resp_packet[0]['PacketPrescr_id'],
							'Diag_id' => $diag,
							'pmUser_id' => $data['pmUser_id']
		...
		$resp_uslnaz = $this->queryResult("
			...
			select
				epfdu.UslugaComplex_id,
				epfd.StudyTarget_id,
				'FuncDiag' as PrescriptionType_Code,
				epfd.MedService_id 
			from
				v_EvnPrescrFuncDiag epfd (nolock)
				inner join v_EvnPrescrFuncDiagUsluga epfdu (nolock) on epfdu.EvnPrescrFuncDiag_id = epfd.EvnPrescrFuncDiag_id
			where
		...
		if(!empty($resp_uslnaz)) {
			foreach ($resp_uslnaz as $one_uslnaz) {
				...
				/*$resp_ppu = $this->queryResult("
							declare
								@PacketPrescrUsluga_id bigint = null,
								@ErrCode int,
								@ErrMessage varchar(4000);
							set @PacketPrescrUsluga_id = null;
							exec p_PacketPrescrUsluga_ins
								@PacketPrescrUsluga_id = @PacketPrescrUsluga_id output,
								...
								@Error_Code = @ErrCode output,
								@Error_Message = @ErrMessage output;
							select @PacketPrescrUsluga_id as PacketPrescrUsluga_id, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
						", array(
					'PacketPrescrList_id' => $arrPrescriptionTypeIDs[$typeCode],
					'UslugaComplex_id' => $one_uslnaz['UslugaComplex_id'],
					'StudyTarget_id' => $one_uslnaz['StudyTarget_id'],
					'PacketPrescrUsluga_Count' => 1,
					'PacketPrescrUsluga_DailyIterCount' => null,
					'PacketPrescrUsluga_Duration' => null,
					'PacketPrescrUsluga_RepeatNonstop' => null,
					'PacketPrescrUsluga_Break' => null,
					'DurationType_did' => null,
					'DurationType_rid' => null,
					'DurationType_bid' => null,
					'MedService_id' => $one_uslnaz['MedService_id'],
					'pmUser_id' => $data['pmUser_id']
				));*/
		...
		$resp_regime = $this->queryResult("
			select
				ISNULL(Regime.PrescriptionRegimeType_id, 0) PrescriptionRegimeType_id
		...
		if(!empty($resp_diet)) {
			...
			foreach ($resp_diet as $one_diet) {
				...
				/*$resp_oneDiet = $this->queryResult("
						declare
							@PacketPrescrDiet_id bigint = null,
							@ErrCode int,
							@ErrMessage varchar(4000);
						set @PacketPrescrDiet_id = null;
						exec p_PacketPrescrDiet_ins
							@PacketPrescrDiet_id = @PacketPrescrDiet_id output,
							@PacketPrescrList_id = :PacketPrescrList_id,
							...
							@Error_Code = @ErrCode output,
							@Error_Message = @ErrMessage output;
						select @PacketPrescrDiet_id as PacketPrescrDiet_id, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
					", array(
					'PacketPrescrList_id' => $arrPrescriptionTypeIDs['Diet'],
					'PrescriptionDietType_id' => $one_diet['PrescriptionDietType_id'],
					'PacketPrescrDiet_Duration' => $one_diet['PacketPrescrDiet_Duration'],
					'pmUser_id' => $data['pmUser_id']
				));*/
		...
		$resp_proc = $this->queryResult("
			select
				...
				,coalesce(ECPR.EvnCourseProc_MaxCountDay, ECPR.EvnCourseProc_MinCountDay, '')  as PacketPrescrUsluga_DailyIterCount
				,isnull(ECPR.EvnCourseProc_Duration, '') as PacketPrescrUsluga_Duration
				,isnull(ECPR.EvnCourseProc_ContReception, '') as PacketPrescrUsluga_RepeatNonstop
				,isnull(ECPR.EvnCourseProc_Interval, '') as PacketPrescrUsluga_Break
		...
		$resp_treat = $this->queryResult("
			with
			ep_distinct as ( --yl: 199635 без дублей в стационаре по одному из курса
				select
					EvnCourse_id,
			...
			select
                ...
                ,isnull(ec_drug.DrugComplexMnn_id,Drug.DrugComplexMnn_id) as DrugComplexMnn_id
                ,isnull(EPTD.EvnPrescrTreatDrug_Kolvo, ec_drug.EvnCourseTreatDrug_Kolvo) as PacketPrescrTreatDrug_Kolvo
                ,isnull(EPTD.EvnPrescrTreatDrug_KolvoEd, ec_drug.EvnCourseTreatDrug_KolvoEd) as PacketPrescrTreatDrug_KolvoEd
			...
			foreach ($resp_treat as $one_treat) {
				...
				if (!in_array($one_treat['EvnPrescr_id'], $evnTreatPrescrIds)) {
					...
					$resp_course = $this->queryResult("
							declare
								@PacketPrescrTreat_id bigint = null,
								@ErrCode int,
								@ErrMessage varchar(4000);
							set @PacketPrescrTreat_id = null;
							exec p_PacketPrescrTreat_ins
                            	@PacketPrescrTreat_id = @PacketPrescrTreat_id output,
                                @PacketPrescrList_id = :PacketPrescrList_id,
								...
								@Error_Code = @ErrCode output,
								@Error_Message = @ErrMessage output;
							select @PacketPrescrTreat_id as PacketPrescrTreat_id, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
						", array(
					...
					$resp_method = $this->queryResult("
							declare
								@PacketPrescrTreatMethod_id bigint = null,
								@ErrCode int,
								@ErrMessage varchar(4000);
							set @PacketPrescrTreatMethod_id = null;
							exec p_PacketPrescrTreatMethod_ins
								@PacketPrescrTreatMethod_id = @PacketPrescrTreatMethod_id output,
								...
								@Error_Code = @ErrCode output,
								@Error_Message = @ErrMessage output;
							select @PacketPrescrTreatMethod_id as PacketPrescrTreatMethod_id, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
						", array(
				...
				$resp_drug = $this->queryResult("
							declare
								@PacketPrescrTreatDrug_id bigint = null,
								@ErrCode int,
								@ErrMessage varchar(4000);
							set @PacketPrescrTreatDrug_id = null;
							exec p_PacketPrescrTreatDrug_ins
								@PacketPrescrTreatDrug_id = @PacketPrescrTreatDrug_id output,
								...
								@Error_Code = @ErrCode output,
								@Error_Message = @ErrMessage output;
							select @PacketPrescrTreatDrug_id as PacketPrescrTreatDrug_id, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
						", array(
	...
	function updatePacketPrescr($data)
	{
		...
		$resp_OldPacket = $this->queryResult("
			select
				...
				PacketPrescr_Name,
				PacketPrescr_Descr,
				PacketPrescrVision_id,
				MedPersonal_id
			from
				v_PacketPrescr (nolock)
			where
				PacketPrescr_id = :PacketPrescr_id
		", array(
		...
		$resp_packet = $this->queryResult("
			declare
				...
				@ErrCode int,
				@ErrMessage varchar(4000);
			set @Res = :PacketPrescr_id;
			exec p_PacketPrescr_upd
				@PacketPrescr_id = @Res,
				@PacketPrescr_Name = :PacketPrescr_Name,
				@PacketPrescr_Descr = :PacketPrescr_Descr,
				@PacketPrescrVision_id = :PacketPrescrVision_id,
				...
				@Error_Code = @ErrCode output,
				@Error_Message = @ErrMessage output;
			select @Res as PacketPrescr_id, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
		", array(
			'PacketPrescr_id' => $data['PacketPrescr_id'],
			'PacketPrescr_Name' => $data['PacketPrescr_Name'],
			'PacketPrescr_Descr' => $data['PacketPrescr_Descr'],
		...
		$resp_OldDiags = $this->queryResult("
			select
				PacketPrescrDiag_id
			from 
				v_PacketPrescrDiag sppd (nolock)
			WHERE
...
// эти 2 условия для МАРМ так как там передается параметр строкой, а не булевым значением
		...
		if (!empty($data['Diag_id'])) {
			...
			$filter .= " and exists(select top 1 ppd.PacketPrescrDiag_id from v_PacketPrescrDiag ppd (nolock) where ppd.PacketPrescr_id = pp.PacketPrescr_id and ppd.Diag_id = :Diag_id)";
		}
				...
				case 'my':
					...
					if($data['node']=='my'){
						$count = $this->getFirstResultFromQuery("
							...
							from v_PacketPrescrShare with(nolock)
		...
		if (!empty($data['Sex_Code'])) {
			$filter .= " and ISNULL(s.Sex_Code,'3') IN (:Sex_Code,'3')";
			$queryParams['Sex_Code'] = $data['Sex_Code'];
		}
		if (!empty($data['PersonAgeGroup_Code'])) {
			$filter .= " and ISNULL(pag.PersonAgeGroup_Code,'3') IN (:PersonAgeGroup_Code,'3')";
			$queryParams['PersonAgeGroup_Code'] = $data['PersonAgeGroup_Code'];
		}
		if (!empty($data['query'])) {
			$queryParams['query'] = $data['query'];
			$filter .= " and pp.PacketPrescr_Name LIKE '%' + :query + '%'";
		}
		if (!empty($data['node']) && $data['node']=='share'){

			$query = "
				select pp.PacketPrescr_id,
					pps.PacketPrescrShare_id,
					pp.PacketPrescr_Name,
					pp.PacketPrescr_Descr,
					substring(Diag_Codes.Codes_str, 1, len(Diag_Codes.Codes_str)-1) as Diag_Codes,
					CASE WHEN ppf.PacketPrescrFavour_id is null THEN 0 ELSE 2 END as Packet_IsFavorite,
					pp.PacketPrescrVision_id,
					ppv.PacketPrescrVision_Name,
					COALESCE(pag.PersonAgeGroup_Name,'Не определен') as PersonAgeGroup_Name,
					COALESCE(s.Sex_Name,'Не определен') as Sex_Name,
					convert(varchar(10), cast(pp.PacketPrescr_updDT as datetime), 104) as PacketPrescr_updDT,
					'true' as leaf
				from dbo.pmUserCache PUC WITH (NOLOCK)
					left join dbo.pmUserCache PUCQ WITH (NOLOCK) ON PUC.MedPersonal_id = PUCQ.MedPersonal_id
					left join v_PacketPrescrShare pps (nolock) on pps.pmUser_getID = pucq.PMUser_id
					left join v_PacketPrescr pp (nolock) on pps.PacketPrescr_id = pp.PacketPrescr_id
					left join v_PacketPrescrLpu ppl (nolock) on ppl.PacketPrescr_id = pp.PacketPrescr_id
					left join v_PacketPrescrFavour ppf (nolock) on (ppf.PacketPrescr_id = pp.PacketPrescr_id  and ppf.MedPersonal_id = PUCQ.MedPersonal_id)
					left join v_PacketPrescrVision ppv (nolock) on ppv.PacketPrescrVision_id = pp.PacketPrescrVision_id
					left join v_Sex s (nolock) on s.Sex_id = pp.Sex_id
					left join v_PersonAgeGroup pag (nolock) on pag.PersonAgeGroup_id = pp.PersonAgeGroup_id
					outer apply (
						Select 
						(
							select
								dc.Diag_Code + ', ' as 'data()'
							from 
								v_PacketPrescrDiag sppd (nolock)
								left join v_diag dc (nolock) on dc.diag_id = sppd.diag_id
							WHERE
				...
				where PUC.PMUser_id =  :pmUser_id AND pp.PacketPrescr_id IS NOT NULL
					{$filter}
			...
			$query = "
				...
				'true' as leaf
				from
					v_PacketPrescr pp (nolock)
					left join v_PacketPrescrLpu ppl (nolock) on ppl.PacketPrescr_id = pp.PacketPrescr_id
					left join v_PacketPrescrFavour ppf (nolock) on (ppf.PacketPrescr_id = pp.PacketPrescr_id {$MedPersFavFilter})
					left join v_PacketPrescrVision ppv (nolock) on ppv.PacketPrescrVision_id = pp.PacketPrescrVision_id
					left join v_Sex s (nolock) on s.Sex_id = pp.Sex_id
					left join v_PersonAgeGroup pag (nolock) on pag.PersonAgeGroup_id = pp.PersonAgeGroup_id
					outer apply (
		...
		return array_merge($templates,$folders);
	}
	...
	function loadPMUserForShareList($data) {
		...
		$query = "
			...
			and not exists(
				select * from v_PacketPrescrShare with(nolock)
	...
	function createPacketPrescrShare($data) {
		...
		$query = "
			declare
				...
				@ErrCode int,
				@ErrMessage varchar(4000);
			exec p_PacketPrescrShare_ins
				@PacketPrescrShare_id = @Res output,
				...
				@Error_Code = @ErrCode output,
				@Error_Message = @ErrMessage output;
			select @Res as PacketPrescrShare_id, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
		";
	...
	function loadCureStandartList($data){
		...
		if (!empty($data['Diag_id'])) {
			$queryParams['Diag_id'] = $data['Diag_id'];
			$filter .= " and exists(select top 1 csd.CureStandartDiag_id from v_CureStandartDiag csd (nolock) where csd.CureStandart_id = cs.CureStandart_id and csd.Diag_id = :Diag_id)";
		}
		else
			$top = 'top 100';
		...
		return $this->queryResult("
			select
				...
				CS.CureStandart_ClinRecDescr,
				CSAGT.CureStandartAgeGroupType_Name,
                CSPT.CureStandartPhaseType_Name,
                CSST.CureStandartStageType_Name,
                CSCT.CureStandartComplicationType_Name,
                MCK.MedicalCareKind_Name,
				null as CureStandart_Descr
			from
				v_CureStandart CS (nolock)
				left join v_CureStandartAgeGroupType as CSAGT with(nolock) on CS.CureStandartAgeGroupType_id = CSAGT.CureStandartAgeGroupType_id
                left join v_CureStandartPhaseType as CSPT with(nolock) on CS.CureStandartPhaseType_id = CSPT.CureStandartPhaseType_id
                left join v_CureStandartStageType as CSST with(nolock) on CS.CureStandartStageType_id = CSST.CureStandartStageType_id
                left join v_CureStandartComplicationType as CSCT with(nolock) on CS.CureStandartComplicationType_id = CSCT.CureStandartComplicationType_id
                left join v_MedicalCareKind as MCK with(nolock) on CS.MedicalCareKind_id = MCK.MedicalCareKind_id
			where
				1 = 1
				{$filter}
		", $queryParams);
				...
				if (!empty($data['CureStandart_id'])) {
					...
					return $this->queryResult("
						select distinct
							...
							ppu.MedService_id,
							l.Lpu_Nick,
							isnull(l.Lpu_id,ms.Lpu_id) as Lpu_id,
							ms.MedService_Nick,
							ppu.PacketPrescrUsluga_id
						from
                        	v_PacketPrescr pp (nolock)
                        	inner join v_PacketPrescrList ppl (nolock) on ppl.PacketPrescr_id = pp.PacketPrescr_id
                        	inner join v_PacketPrescrUsluga ppu (nolock) on ppu.PacketPrescrList_id = ppl.PacketPrescrList_id
							inner join v_UslugaComplex uc (nolock) on uc.UslugaComplex_id = ppu.UslugaComplex_id
							left join v_MedService ms (nolock) on ms.MedService_id = ppu.MedService_id
				...
				if (!empty($data['CureStandart_id'])) {
					return $this->queryResult("
						select distinct
							...
							1 as Replaseability
						from
							v_CureStandartTreatment cst (nolock)
							inner join v_CureStandartTreatmentDrug cstd (nolock) on cstd.CureStandartTreatment_id = cst.CureStandartTreatment_id
							...
							outer apply (
								...
								from rls.PREP_ACTMATTERS p_am with (nolock)
									left join rls.v_PREP_ATC p_at with (nolock) on p_at.PREPID = p_am.PREPID
									inner join rls.v_CLSATC c_at with (nolock) on c_at.CLSATC_ID = p_at.UNIQID
									inner join rls.v_CLSATC c_at2 with (nolock) on c_at2.CLSATC_ID = c_at.PARENTID
									inner join rls.v_CLSATC c_at3 with (nolock) on c_at3.CLSATC_ID = c_at2.PARENTID
								where
					...
					$arrDrugData = $this->queryResult("
						select
							...
							,ppt.PacketPrescrTreat_id
							,pptd.PacketPrescrTreatDrug_id
							,pptd.DrugComplexMnn_id
							,pptd.Drug_id
							,pptd.ACTMATTERS_ID
						from
							dbo.v_PacketPrescr pp (nolock)
							inner join dbo.v_PacketPrescrList ppl (nolock) on ppl.PacketPrescr_id = pp.PacketPrescr_id
							inner join dbo.v_PacketPrescrTreat ppt (nolock) on ppt.PacketPrescrList_id = ppl.PacketPrescrList_id
							left join dbo.v_PacketPrescrTreatDrug pptd (nolock) on pptd.PacketPrescrTreat_id = ppt.PacketPrescrTreat_id
							...
							left join rls.v_DrugComplexMnn dcm with (nolock) on dcm.DrugComplexMnn_id = isnull(pptd.DrugComplexMnn_id,Drug.DrugComplexMnn_id)
						where
							pp.PacketPrescr_id = :PacketPrescr_id
				...
				if (!empty($data['CureStandart_id'])) {
					return $this->queryResult("
						select
							...
							uc.UslugaComplex_Name
						from
							v_CureStandartDiagnosis csd (nolock)
							inner join v_UslugaComplex uc (nolock) on uc.UslugaComplex_id = csd.UslugaComplex_id
						where
							...
							and exists (
								...
								from
									v_UslugaComplexAttribute t1 with (nolock)
									inner join v_UslugaComplexAttributeType t2 with (nolock) on t2.UslugaComplexAttributeType_id = t1.UslugaComplexAttributeType_id
								where
				...
				if (!empty($data['CureStandart_id'])) {
					...
					return $this->queryResult("
						select
							...
							,ppr.PrescriptionRegimeType_id
							,ppr.PacketPrescrRegime_Duration
							,ppr.PacketPrescrRegime_id
						from
							dbo.v_PacketPrescr pp (nolock)
							inner join dbo.v_PacketPrescrList ppl (nolock) on ppl.PacketPrescr_id = pp.PacketPrescr_id
							inner join dbo.v_PacketPrescrRegime ppr (nolock) on ppr.PacketPrescrList_id = ppl.PacketPrescrList_id
							inner join PrescriptionRegimeType PRT with (nolock) on PRT.PrescriptionRegimeType_id = ppr.PrescriptionRegimeType_id
						where
							pp.PacketPrescr_id = :PacketPrescr_id
				...
				if (!empty($data['CureStandart_id'])) {
					...
					return $this->queryResult("
						select
							...
							,ppd.PrescriptionDietType_id
							,ppd.PacketPrescrDiet_Duration
							,ppd.PacketPrescrDiet_id
						from
							dbo.v_PacketPrescr pp (nolock)
							inner join dbo.v_PacketPrescrList ppl (nolock) on ppl.PacketPrescr_id = pp.PacketPrescr_id
							inner join dbo.v_PacketPrescrDiet ppd (nolock) on ppd.PacketPrescrList_id = ppl.PacketPrescrList_id
							inner join v_PrescriptionDietType pdt with (nolock) on pdt.PrescriptionDietType_id = ppd.PrescriptionDietType_id
						where
							pp.PacketPrescr_id = :PacketPrescr_id
	...
	function createPacketPrescrList($PacketPrescr_id, $PrescriptionType_Code, $pmUser_id){
		...
		$resp_ppl = $this->queryResult("
			declare
				@PacketPrescrList_id bigint,
				@ErrCode int,
				@ErrMessage varchar(4000);
			set @PacketPrescrList_id = null;
			exec p_PacketPrescrList_ins
				@PacketPrescrList_id = @PacketPrescrList_id output,
				@PacketPrescr_id = :PacketPrescr_id,
				@PrescriptionType_id = :PrescriptionType_id,
				@pmUser_id = :pmUser_id,
				@Error_Code = @ErrCode output,
				@Error_Message = @ErrMessage output;
			select @PacketPrescrList_id as PacketPrescrList_id, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
		", array(
			'PacketPrescr_id' => $PacketPrescr_id,
			'PrescriptionType_id' => $PrescriptionType_id,
			'pmUser_id' => $pmUser_id
		...
		switch($typePrescr){
			case 'drug':
				...
				$res_DrugData = $this->queryResult("
						select
							pptm.PrescriptionIntroType_id
							,ppt.PacketPrescrTreat_id
							,ppt.PacketPrescrTreat_Duration
							,ppt.PacketPrescrTreat_MinCountDay
							,ppt.PacketPrescrTreat_MaxCountDay
							,ppt.PacketPrescrTreat_ContReception
							,ppt.DurationType_id
							,ppt.DurationType_recid
							,ppt.DurationType_intid
							,ppt.PacketPrescrTreat_PrescrCount
							,pptd.PacketPrescrTreatDrug_id
							,pptm.PrescriptionTreatType_id
							,pptm.PerformanceType_id
							...
							,isnull(pptd.PacketPrescrTreatDrug_MinDoseDay, pptd.PacketPrescrTreatDrug_MaxDoseDay) as DoseDay
						...
						from
							...
							inner join dbo.v_PacketPrescrTreat ppt (nolock) on ppt.PacketPrescrList_id = ppl.PacketPrescrList_id
							left join dbo.v_PacketPrescrTreatDrug pptd (nolock) on pptd.PacketPrescrTreat_id = ppt.PacketPrescrTreat_id
							left join dbo.v_PacketPrescrTreatMethod pptm (nolock) on pptm.PacketPrescrTreatMethod_id = pptd.PacketPrescrTreatMethod_id
							left join rls.v_DrugComplexMnnName MnnName with (nolock) on MnnName.ACTMATTERS_id = pptd.ACTMATTERS_ID
							left join rls.v_Drug Drug with (nolock) on Drug.Drug_id = pptd.Drug_id
							left join rls.v_DrugComplexMnn dcm with (nolock) on dcm.DrugComplexMnn_id = isnull(pptd.DrugComplexMnn_id,Drug.DrugComplexMnn_id)
						where
	...
	function setPacketFavorite($data){
		if($data['Packet_IsFavorite']){
			$resp_ppl = $this->queryResult("
					declare
						@PacketPrescrFavour_id bigint,
						@ErrCode int,
						@ErrMessage varchar(4000);
					set @PacketPrescrFavour_id = null;
					exec p_PacketPrescrFavour_ins
						@PacketPrescrFavour_id = @PacketPrescrFavour_id output,
						@PacketPrescr_id = :PacketPrescr_id,
						@MedPersonal_id = :MedPersonal_id,
						@pmUser_id = :pmUser_id,
						@Error_Code = @ErrCode output,
						@Error_Message = @ErrMessage output;
					select @PacketPrescrFavour_id as PacketPrescrFavour_id, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
				", array(
				'PacketPrescr_id' => $data['PacketPrescr_id'],
				'MedPersonal_id' => $data['MedPersonal_id'],
				'pmUser_id' => $data['pmUser_id']
			...
			$resp_fav = $this->queryResult("
				select
					PacketPrescrFavour_id
				from
					v_PacketPrescrFavour (nolock)
				where
					PacketPrescr_id = :PacketPrescr_id
	 ...
	 * Установка/снятие избранности для пакета
	 */
	function setPacketReaded($data){
			$resp_OldPacket = $this->queryResult("
			select
				PPS.PacketPrescrShare_id,
				PPS.PacketPrescr_id,
				PPS.PacketPrescrShare_IsReaded,
				PPS.pmUser_sendID,
				PPS.Lpu_sid,
				PPS.pmUser_getID,
				PPS.Lpu_gid,
				PPS.pmUser_insID,
				PPS.pmUser_updID,
				convert(varchar(10), cast(pps.PacketPrescrShare_updDT as datetime), 104) as PacketPrescrShare_insDT,
				convert(varchar(10), cast(pps.PacketPrescrShare_updDT as datetime), 104) as PacketPrescrShare_updDT
			from
				v_PacketPrescrShare PPS (nolock)
			where
				PPS.PacketPrescrShare_id = :PacketPrescrShare_id
			", array('PacketPrescrShare_id' => $data['PacketPrescrShare_id']));
			if (empty($resp_OldPacket[0]['PacketPrescrShare_id'])) {
				return array('Error_Msg' => 'Ошибка получения данных о пакете');
			}
			...
			$resp_packet = $this->queryResult("
					declare @Res bigint, 
					@ErrCode int,	@ErrMessage varchar(4000);
					set @Res = :PacketPrescrShare_id;
					exec p_PacketPrescrShare_upd
						@PacketPrescrShare_id = @Res,
						@PacketPrescr_id = :PacketPrescr_id,
						@PacketPrescrShare_IsReaded = :PacketPrescrShare_IsReaded,
						@pmUser_sendID = :pmUser_sendID,
						@Lpu_sid = :Lpu_sid,
					...
					select @Res as PacketPrescrShare_id, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
				", array(
					'PacketPrescrShare_id' => $data['PacketPrescrShare_id'],
					'PacketPrescr_id' => $data['PacketPrescr_id'],
					'PacketPrescrShare_IsReaded' => $data['PacketPrescrShare_IsReaded'],
					'pmUser_sendID' => $data['pmUser_sendID'],
					'Lpu_sid' => $data['Lpu_sid'],
					'pmUser_getID' => $data['pmUser_getID'],
					'Lpu_gid' => $data['Lpu_gid'],
					'pmUser_id' => $data['pmUser_updID']

			)
		);
		if (empty($resp_packet[0]['PacketPrescrShare_id'])) {
			$this->rollbackTransaction();
			return array('Error_Msg' => 'Ошибка сохранения пакета назначений');
		}
		$this->commitTransaction();
	...
	function loadEditPacketForm($data) {
		...
		return $this->queryResult("
			select
				...
				pp.PacketPrescrVision_id,
				pp.Sex_id,
				pp.PersonAgeGroup_id,
				substring(Diag_Codes.Codes_str, 1, len(Diag_Codes.Codes_str)-1) as Diag_id
			from
				v_PacketPrescr pp (nolock)
				left join v_PacketPrescrLpu ppl (nolock) on ppl.PacketPrescr_id = pp.PacketPrescr_id
				outer apply (
					...
					(
						select
							CAST(sppd.diag_id as nvarchar) + ',' as 'data()'
						from 
							v_PacketPrescrDiag sppd (nolock)
						WHERE
							sppd.PacketPrescr_id = pp.PacketPrescr_id
	 ...
	 * @return array
	 */
	function deletePacketPrescrShare($data) {
		$queryParams = array('PacketPrescrShare_id' => $data['PacketPrescrShare_id']);
		return $this->queryResult("
					declare
						@ErrCode int,
						@ErrMessage varchar(4000);
					exec p_PacketPrescrShare_del
						@PacketPrescrShare_id = :PacketPrescrShare_id,
						@Error_Code = @ErrCode output,
						@Error_Message = @ErrMessage output;
					select @ErrCode as Error_Code, @ErrMessage as Error_Msg;
				", $queryParams);
	...
	function applyPacketPrescr($data) {
		...
		$evn_setdt = $this->getFirstResultFromQuery('
			select evn_setdt from v_evn (nolock) where evn_id = :id
		', array(
			'id' => $data['Evn_pid'],
		));
	...
	function applyAllPacketPrescr($data) {
		...
		$allPrescr = $this->queryResult("
			select
				ppu.UslugaComplex_id
				,ppu.MedService_id
				,ppl.PrescriptionType_id
				,ISNULL(ppd.PrescriptionDietType_id,ppr.PrescriptionRegimeType_id) as RegimeDietType_id
				,ISNULL(ppd.PacketPrescrDiet_Duration,ppr.PacketPrescrRegime_Duration) as RegimeDiet_Duration
				,ppt.PacketPrescrTreat_id
				,pptd.PacketPrescrTreatDrug_id
				,pptd.DrugComplexMnn_id
				,pptd.Drug_id
			...
			from
				v_PacketPrescr pp (nolock)
				inner join v_PacketPrescrList ppl (nolock) on ppl.PacketPrescr_id = pp.PacketPrescr_id
				left join v_PacketPrescrUsluga ppu (nolock) on ppu.PacketPrescrList_id = ppl.PacketPrescrList_id
				left join dbo.v_PacketPrescrDiet ppd (nolock) on ppd.PacketPrescrList_id = ppl.PacketPrescrList_id
				left join dbo.v_PacketPrescrRegime ppr (nolock) on ppr.PacketPrescrList_id = ppl.PacketPrescrList_id
				left join dbo.v_PacketPrescrTreat ppt (nolock) on ppt.PacketPrescrList_id = ppl.PacketPrescrList_id
				left join dbo.v_PacketPrescrTreatDrug pptd (nolock) on pptd.PacketPrescrTreat_id = ppt.PacketPrescrTreat_id
			where
	...
	function loadDrugTemplateList($data){
		$res = $this->queryResult("
				...
				top 5
					ppt.PacketPrescrTreat_id
					,ppt.PacketPrescrTreat_Name
				from
					dbo.v_PacketPrescrTreat ppt (nolock)
				where 
					MedPersonal_id = :MedPersonal_id
			  	order by
	...
	function loadLastSelectedDrugList($data)
	{
		...
		if ($resCache = $this->swcache->get($cacheObject)) {
			...
			from 
				v_EvnPrescr EP with (nolock)
				--left join pmUserCache UC (nolock) on UC.PMUser_id = EP.pmUser_insID
				inner join v_EvnPrescrTreat EPT with (nolock) on EPT.EvnPrescrTreat_id = EP.EvnPrescr_id
				inner join v_EvnCourseTreat ECT with (nolock) on ECT.EvnCourseTreat_id = EPT.EvnCourse_id
				left join v_EvnCourseTreatDrug ec_drug with (nolock) on ec_drug.EvnCourseTreat_id = EPT.EvnCourse_id
				left join rls.v_Drug Drug with (nolock) on Drug.Drug_id = ec_drug.Drug_id
			...
			where
				EP.PrescriptionType_id = 5
				and EP.PrescriptionStatusType_id != 3
				--and UC.MedPersonal_id = MedPersonal_id
				and EP.Lpu_id = :Lpu_id
				and EP.pmUser_insID = :pmUser_id
			--group by Drug.Drug_id,dcm.DrugComplexMnn_id,Drug.Drug_Name,dcm.DrugComplexMnn_RusName,ln.NAME,ACT.LATNAME,dcm.DrugComplexMnn_LatName
			order by
				--max(EP.EvnPrescr_setDate) DESC
	...
	function loadPacketCourseTreatEditForm($data) {
		...
		$query = "
			select
				...
				isnull(pptd.PacketPrescrTreatDrug_MaxDoseDay, pptd.PacketPrescrTreatDrug_MinDoseDay) as DoseDay,
				'' as DrugListData,
				convert(varchar(10), dbo.tzGetDate(), 104) as EvnCourseTreat_setDate,
				...
				,ppt.PacketPrescrTreat_id
				,pptd.PacketPrescrTreatDrug_id
				,pptd.PacketPrescrTreatMethod_id
				,'new' as status
			from
				v_PacketPrescrTreat ppt with (nolock)
				left join dbo.v_PacketPrescrTreatDrug pptd with (nolock) on pptd.PacketPrescrTreat_id = ppt.PacketPrescrTreat_id
				left join dbo.v_PacketPrescrTreatMethod pptm with (nolock) on pptm.PacketPrescrTreatMethod_id = pptd.PacketPrescrTreatMethod_id

				left join rls.Drug D with (nolock) on D.Drug_id = pptd.Drug_id
				left join rls.v_DrugComplexMnn dcm with (nolock) on dcm.DrugComplexMnn_id = isnull(pptd.DrugComplexMnn_id,D.DrugComplexMnn_id)
	...
	function saveDrugTemplate($data) {
		...
		if (!empty($TreatDrugArr)) {
			...
			", $param_drug);
				...
				// Оставим на всякий случай, пока все случаи употребления не проработаем
				$treatCourseData['TreatDrug'] = $resp_drug[0]['PacketPrescrTreatDrug_id'];
				$treatCourseData['TreatDrug'.$key] = $resp_drug[0]['PacketPrescrTreatDrug_id'];
			}
		}
	...
	function copyPacket($data) {
		...
		return $this->queryResult("
					declare
						@PacketPrescr_NewId BIGINT,
						@ErrCode int,
						@ErrMessage varchar(4000);
					set @PacketPrescr_NewId = null;
					exec p_PacketPrescr_Copy
						@PacketPrescr_NewId = @PacketPrescr_NewId OUTPUT,
						@PacketPrescr_id = :PacketPrescr_id,
						@PacketPrescr_Name = :PacketPrescr_Name,
						@pmUser_id = :pmUser_id,
						@Error_Code = @ErrCode output,
						@Error_Message = @ErrMessage output;
					select @PacketPrescr_NewId AS PacketPrescr_id, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
				", $queryParams);
	}
	/**
	...
	function createPacketPrescrRegime($data,$PacketPrescrList_id = false) {
		...
		return $this->queryResult("
						declare
							@PacketPrescrRegime_id bigint = null,
							@ErrCode int,
							@ErrMessage varchar(4000);
						set @PacketPrescrRegime_id = null;
						exec p_PacketPrescrRegime_ins
							@PacketPrescrRegime_id = @PacketPrescrRegime_id output,
							@PacketPrescrList_id = :PacketPrescrList_id,
							...
							@Error_Code = @ErrCode output,
							@Error_Message = @ErrMessage output;
						select @PacketPrescrRegime_id as PacketPrescrRegime_id, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
					", array(
			'PacketPrescrList_id' => $PacketPrescrList_id,
	...
	function getFuncDiagData($data) {
		if (!empty($data['save_data']['funcdiag'])) {
			...
			return $this->queryResult("
						select
							ISNULL(ms.MedService_id,0) as MedService_id,
							uc.UslugaComplex_id
						from
                        	v_PacketPrescr pp (nolock)
                        	inner join v_PacketPrescrList ppl (nolock) on ppl.PacketPrescr_id = pp.PacketPrescr_id
	...
	function getRegimeData($data) {
		if (!empty($data['save_data']['regime'])) {
			...
			return $this->queryResult("
						select
							 ppr.PrescriptionRegimeType_id
							,ISNULL(ppr.PacketPrescrRegime_Duration,0) as PacketPrescrRegime_Duration
						from
							dbo.v_PacketPrescr pp (nolock)
							inner join dbo.v_PacketPrescrList ppl (nolock) on ppl.PacketPrescr_id = pp.PacketPrescr_id
							inner join dbo.v_PacketPrescrRegime ppr (nolock) on ppr.PacketPrescrList_id = ppl.PacketPrescrList_id
	...
	function getDietData($data) {
		if (!empty($data['save_data']['diet'])) {
			...
			return $this->queryResult("
						select
							 ppd.PrescriptionDietType_id
							,ISNULL(ppd.PacketPrescrDiet_Duration,0) as PacketPrescrDiet_Duration
						from
							dbo.v_PacketPrescr pp (nolock)
							inner join dbo.v_PacketPrescrList ppl (nolock) on ppl.PacketPrescr_id = pp.PacketPrescr_id
							inner join dbo.v_PacketPrescrDiet ppd (nolock) on ppd.PacketPrescrList_id = ppl.PacketPrescrList_id
