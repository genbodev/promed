...
class Demand_model extends CI_Model {
	...
	function loadDemandListGrid($data, $demand_type) {
		...
		if ($demand_type == "attachment") {
			...
			if (isset($data['Person_Surname']) && strlen($data['Person_Surname']) > 0) $where .= " AND p.Person_Surname LIKE '".$data['Person_Surname']."%'";
			if (isset($data['Person_Firname']) && strlen($data['Person_Firname']) > 0) $where .= " AND p.Person_Firname LIKE '".$data['Person_Firname']."%'";
			if (isset($data['Person_Secname']) && strlen($data['Person_Secname']) > 0) $where .= " AND p.Person_Secname LIKE '".$data['Person_Secname']."%'";
			if (isset($data['Lpu_id']) && $data['Lpu_id'] > 0) $where .= " AND d.Lpu_id = ".$data['Lpu_id'];
			
			$query = "
				select
					-- select
					d.AttachmentDemand_id,
					d.DemandState_id,
					...
					l.Lpu_Name,
					ds.DemandState_Name,
					CONVERT(varchar(10), d.Insert_Date, 104) as Insert_Date
					-- end select
				from
					-- from
					AttachmentDemand d with (nolock)
					left join v_Person_ER p with (nolock) on p.Person_id = d.Person_Id
					left join v_Lpu l with (nolock) on l.Lpu_id = d.Lpu_Id
					left join DemandState ds with (nolock) on ds.DemandState_Id = d.DemandState_id
					-- end from
				where 
					-- where
					(1=1) ".$where." 
		...
		if ($demand_type == "changesmo") { 
			$query = "
				...
				from
					-- from
					ChangeSmoDemand d with (nolock)
					left join v_Person_ER p with (nolock) on p.Person_id = d.Person_Id
					left join OrgSmo l with (nolock) on l.OrgSmo_id = d.Smo_Id
		...
		if ($demand_type == "attachment") {
			$query = "
				SELECT TOP 1
					...
					ISNULL(l.Lpu_Name, 'информация отсутствует') as currentLpu_Name,
					ISNULL(dl.Lpu_Name, 'информация отсутствует') as Lpu_Name,
				...
				FROM
					AttachmentDemand d with (nolock)
					left outer join v_Person_ER p with (nolock) on d.Person_Id = p.Person_id
					left outer join v_Lpu l with (nolock) on p.Lpu_id = l.Lpu_id
					left outer join v_Lpu dl with (nolock) on d.Lpu_id = dl.Lpu_id
				WHERE (1 = 1)
	...
	function deleteDemand($data, $demand_type) {
		if ($demand_type == "attachment")
			$query = "delete from AttachmentDemand with(nolock) where DemandState_Id in (1,2,3,4,5,6) and AttachmentDemand_Id = ".$data['AttachmentDemand_id'];
		if ($demand_type == "changesmo")
			$query = "delete from ChangeSmoDemand with(nolock) where DemandState_Id in (1,2,3,4,5,6) and ChangeSmoDemand_Id = ".$data['ChangeSmoDemand_id'];
	...
	function getOrgSmoList() {		
		$query = "
			SELECT os.OrgSMO_id Smo_id, o.Org_Name Smo_Name
	...
	function getOrgSmoFilialList($data) {		
		...
		$query = "
			SELECT OrgSmoFilial_id ,osf.OrgSmoFilial_Name OrgSmoFilial_Name
			FROM OrgSMOFilial osf with (nolock)
			left join address a with (nolock) on osf.address_id= a.address_id
			left join KLStreet s with (nolock) on a.KLStreet_id = s.KLStreet_id
	...
	function getCountNewDemand($data) {		
		$query = "
			SELECT count(*) cnt
			FROM AttachmentDemand with (nolock)
