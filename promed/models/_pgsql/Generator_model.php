 ...
 */
class Generator_model extends SwPgModel {
	...
	function exec() {
		...
		$resp = $this->queryResult("
			select
				ObjectList_Name as \"ObjectList_Name\",
				ObjectList_IdCount as \"ObjectList_IdCount\",
				ObjectList_Table as \"ObjectList_Table\",
				ObjectList_Threshold as \"ObjectList_Threshold\"
			from
				generator.ObjectList
			where
			...
			foreach ($resp as $respone) {
				...
				$resp_count = $this->queryResult("
					select
						count(*) as \"cnt\"
					from
						{$respone['ObjectList_Table']}
					where
						COALESCE(Generator_IsUsed, 1) = 1 
				");
