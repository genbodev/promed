...
class LpuRegion_model extends swPgModel {
	/**
	 * Конструктор
	 */
    ...
    function getLpuRegionListFeld($data)
    {
        ...
        if(isset($data['Org_id'])){
            $join = "inner join v_Lpu L on L.Lpu_id = LR.Lpu_id";
        ...
        $sql = "
            ...
            from v_LpuRegionType LRT
                inner join v_LpuRegion LR on LR.LpuRegionType_id = LRT.LpuRegionType_id
                {$join}
            where 
                LRT.LpuRegionType_SysNick = 'feld'
            {$and}
        ";
        //echo getDebugSQL($sql,$params);die;
        $res = $this->db->query($sql,$params);
	...
	*/
    function getLpuRegionsByAddress($data)
    {
        ...
        $sql = "
			Select
				...
				coalesce(addr.Address_Nick, addr.Address_Address) as \"LpuBuilding_Address\",
			...
			from LpuRegionStreet lrs
				...
				left join v_LpuRegion lr on lrs.LpuRegion_id = lr.LpuRegion_id
				...
				left join v_LpuBuilding lb on ls.LpuBuilding_id = lb.LpuBuilding_id
        ...
        if (count($res) == 0) {
			...
			Select
				...
				lrs.LpuRegionStreet_HouseSet as \"LpuRegionStreet_HouseSet\"
			from LpuRegionStreet lrs
				left join KLArea c on c.Klarea_id = lrs.KLTown_id
				left join KLSocr cs on cs.KLSocr_id = c.KLSocr_id
				left join v_LpuRegion lr on lrs.LpuRegion_id = lr.LpuRegion_id
				left join v_LpuRegionType lrt on lrt.LpuRegionType_id = lr.LpuRegionType_id
				left join v_LpuSection ls on ls.LpuSection_id = lr.LpuSection_id
	...
	public function getLpuRegionByMedStaffFact($msf_id) {
		$result = $this->queryResult("select
            lr.LpuRegion_id,
            lr.LpuRegion_Name
        from v_LpuRegion lr 
        left join v_MedStaffRegion msr  on msr.LpuRegion_id = lr.LpuRegion_id
        left join v_MedStaffFact msf  on msr.MedStaffFact_id = msf.MedStaffFact_id and coalesce (msr.MedStaffRegion_endDate, '2030-01-01') > getdate()
