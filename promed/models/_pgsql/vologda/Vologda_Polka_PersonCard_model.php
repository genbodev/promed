	...
	*/
	...
	function exportPersonCardAttach($data) {
		$params = array(
			...
			'begDate' => "{$data['year']}-{$data['month']}-01",
			'endDate' => date('Y-m-t', strtotime("{$data['year']}-{$data['month']}-01")),
		);
		...
		$query = "
			select
				...
				date_part('year', CAST(:endDate as date)) as \"YEAR\",
				date_part('month', CAST(:endDate as date)) as \"MONTH\"
			from
				(select 1 as a) t
	...
	function closePersonCardByImport($data) {
		...
		$query = "
			...
			select 
				...
				to_char(dateadd('day', -1, CAST(:date as date)), 'YYYY-MM-DD') as \"PersonCard_endDate\",
	...
	function importPersonCardRegister($data) {
		...
		if ($fileext == 'xml') {
			...
			$Lpu_id = $this->getFirstResultFromQuery("
				select Lpu_id  as \"Lpu_id\" from v_Lpu  where Lpu_OGRN = :Lpu_OGRN limit 1
			", array(
				'Lpu_OGRN' => $Lpu_OGRN
			));
		...
		for ($i = 1; $i <= $count; $i++) {
			...
			try {
				...
				if ($PersonCard) {
					RegisterListDetailLog_model::createLogMessage(new DateTime(), 1, "Пациент уже прикреплен" .
						" N_PP={$item['N_PP']}, {$item['FAM']} {$item['IM']} {$item['OT']}, {$item['DR']}, {$item['N_POL']}", $RegisterListLog_id, $data['pmUser_id']);
					continue;
				}
				...
				$query = "
					select
						...
						to_char(CAST(:PersonCard_begDate as timestamp), 'YYYY-MM-DD') as \"PersonCard_begDate\",
				...
				if (!empty($PersonCard['PersonCardAttach_id'])) {
					...
					if (!is_array($PersonCardResp)) {
						$log[] = "{$item['FAM']} {$item['IM']} {$item['OT']}, {$item['DR']}, {$item['N_POL']} не был прикреплен: " .
							"Ошибка при создании прикрепления на основе заявления";
						$error_cnt++;
					}
				...
				if (!empty($PersonCardResp[0]['PersonCard_id'])) {
					if (!empty($PersonCard['MedPersonal_Fio'])) {
	...
	function getOrgDepId($Org_Name) {
		...
		return $this->getFirstResultFromQuery("
			select OrgDep_id  as \"OrgDep_id\" from v_OrgDep  where Org_Name = :Org_Name or Org_Nick = :Org_Name limit 1
		", array(
			'Org_Name' => $Org_Name
		), true);
	}
	...
	private function setDocument($person_id, $server_id, $data) {
		...
		$old = $this->getFirstRowFromQuery("
			...
			from v_PersonState p 
				inner join v_Document d  on p.Document_id = d.Document_id
	...
	private function setAddress($person_id, $server_id, $data) {
		...
		$address = $this->getFirstRowFromQuery("
			select 
				:Server_id as \"Server_id\",
				:Person_id as \"Person_id\",
				klc.KLCountry_id as \"KLCountry_id\",
				null as \"KLAreaType_id\",
				klc.KLRgn_id as \"KLRgn_id\",
				klc.KLSubRgn_id as \"KLSubRgn_id\",
				klc.KLCity_id as \"KLCity_id\",
				klc.KLTown_id as \"KLTown_id\",
				...
				1 as \"pmUser_id\"
			from v_KLStreet kls 
				inner join v_KladrCache klc  on klc.KLStreet_id = kls.KLStreet_id
			where 
