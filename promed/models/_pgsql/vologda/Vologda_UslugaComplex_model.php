	...
	function loadForSelect($data) {
		...
		if (!empty($data['Mes_id']) && isset($data['UslugaComplex_id'])) {
			...
			if (!empty($data['UslugaComplexTariff_Date'])) {
				...
				";
				$datefilter2 = "
					and COALESCE(muc.MesOldUslugaComplex_begDT, CAST(:UslugaComplexTariff_Date as date)) <= :UslugaComplexTariff_Date
					and COALESCE(muc.MesOldUslugaComplex_endDT, CAST(:UslugaComplexTariff_Date as date)) >= :UslugaComplexTariff_Date
				";
				$datefilter3 = "
					and COALESCE(UC.UslugaComplex_begDT, CAST(:UslugaComplexTariff_Date as date)) <= :UslugaComplexTariff_Date
					and COALESCE(UC.UslugaComplex_endDT, CAST(:UslugaComplexTariff_Date as date)) >= :UslugaComplexTariff_Date
				";
			}
			$joinMes = "LEFT JOIN LATERAL (
                select muc.Mes_id
                from v_MesOldUslugaComplex muc 
		...
		if (!empty($data['UslugaComplex_id'])) {
			...
			$joinuc = "inner join v_UslugaComplex UC  on UC.UslugaComplex_2011id = muc.UslugaComplex_id";
			if (!empty($data['EvnDiagPLStom_id'])) {
				// для КСГ надо напрямую джойнить
				$joinuc = "inner join v_UslugaComplex UC  on UC.UslugaComplex_id = muc.UslugaComplex_id";
			}
            ...
            from v_MesOldUslugaComplex muc 
            inner join v_MesOld m  on m.Mes_id = muc.Mes_id
            {$joinuc}
