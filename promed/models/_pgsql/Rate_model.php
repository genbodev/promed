...
class Rate_model extends SwPgModel {
   ...
   function loadRateValueList($data) {
		...
		if ($rate_type == "person") {
			$query = "
				...
				from
					...
					left outer join RateType rt  on rt.RateType_id = r.RateType_id
					left outer join RateValueType rvt  on rvt.RateValueType_id = rt.RateValueType_id
				where
	...
	function getPersonMeasure($data) {
		$query = "
			select
				PersonMeasure_id as \"PersonMeasure_id\",
				to_char(PersonMeasure_setDT,'dd.mm.yyyy') as \"PersonMeasure_setDT_Date\",
				substring(to_char(PersonMeasure_setDT,'hh:mm:ss'),1,5) as \"PersonMeasure_setDT_Time\",
				Lpu_id as \"Lpu_id\",
	...
	function loadPersonMeasureList($data) {
		$query = "
			select
				-- select
				PersonMeasure_id as \"PersonMeasure_id\",
				to_char(PersonMeasure_setDT,'dd.mm.yyyy') as \"date\",
				ls.LpuSection_Name as \"lpusection_name\",
				(Coalesce(cast(mp.MedPersonal_Code as varchar) || ' ','') || mp.Person_Fio) as \"medpersonal_fio\",
				pm.MedPersonal_id as \"MedPersonal_id\",
				pm.LpuSection_id as \"LpuSection_id\",
				to_char(PersonMeasure_setDT,'dd.mm.yyyy') as \"PersonMeasure_setDT_Date\",
				substring(to_char(PersonMeasure_setDT,'hh:mm:ss'),1,5) as \"PersonMeasure_setDT_Time\",
				1 as \"Record_Status\"
				-- end select
			from
				-- from
				PersonMeasure pm 
				left join LpuSection ls  on ls.LpuSection_id = pm.LpuSection_id
				left outer join v_MedPersonal mp  on mp.MedPersonal_id = pm.MedPersonal_id
				-- end from
			where
				-- where
   ...
   function deleteRate($data) {
		...
		" : "
			select
				ur.Rate_id as \"Rate_id\"
			from
				EvnUslugaRate ur 
				left join Rate r  on r.Rate_id = ur.Rate_id
			where
				EvnUsluga_id = :rate_subid
		";
