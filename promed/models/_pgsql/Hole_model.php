...
class Hole_model extends Scenario_model {
	...
	public function doLoadGrid($data = array())
	{
		...
		$query = "
			select
				H.Hole_id as \"Hole_id\",
				H.Tablet_id as \"Tablet_id\",
				H.Hole_Number as \"Hole_Number\",
				H.HoleState_id as \"HoleState_id\",
				H.Hole_IsDefect as \"Hole_IsDefect\",
				H.Hole_defectDT as \"Hole_defectDT\",
				H.DefectCauseType_id as \"DefectCauseType_id\",
				DCT.DefectCauseType_Name as \"DefectCauseType_Name\",
				H.Hole_Comment as \"Hole_Comment\",
				H.UslugaTest_id as \"UslugaTest_id\",
				HS.HoleState_Name as \"HoleState_Name\",
				...
				RF.RefValues_id as \"RefValues_id\",
				RF.RefValues_UpperLimit as \"RefValues_UpperLimit\",
				RF.RefValues_LowerLimit as \"RefValues_LowerLimit\",
			...
			from v_Hole H
			left join v_HoleState HS on HS.HoleState_id = H.HoleState_id
			left join v_Tablet T on T.Tablet_id = H.Tablet_id
			left join v_UslugaTest UT on UT.UslugaTest_id = H.UslugaTest_id
			left join v_RefValues RF on RF.RefValues_id = UT.RefValues_id 
			...
			left join v_PersonState PS on ELS.Person_id = PS.Person_id
			left join v_PersonEncrypHIV PEH on PEH.Person_id = PS.Person_id
			left join lis.v_DefectCauseType DCT on DCT.DefectCauseType_id = H.DefectCauseType_id
	...
	function getEmptyHoles($Tablet_id) {
		...
		$query = "
			select
				Hole_id as \"Hole_id\",
				Hole_Number as \"Hole_Number\",
				Hole_IsDefect as \"Hole_IsDefect\"
			from v_Hole
	...
	function getHoleIdForClear($Tablet_id) {
		...
		$query = "
			...
			from v_Hole H
			left join v_UslugaTest UT on UT.UslugaTest_id = H.UslugaTest_id
			where H.Tablet_id = :Tablet_id and coalesce(H.HoleState_id,1) <> 1
