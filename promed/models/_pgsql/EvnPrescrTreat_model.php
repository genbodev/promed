...
class EvnPrescrTreat_model extends EvnPrescrAbstract_model
{
	...
	protected function _saveEvnCourseTreat(&$data,$drug_list = null) {
		...
		$query = "
            select
               EvnCourseTreat_id as \"EvnCourseTreat_id\",
               Error_Code as \"Error_Code\",
               Error_Message as \"Error_Msg\"
			from p_EvnCourseTreat_{$action} (
	...
	protected function _save($data = array(), $isDoSave = false) {
		...
		$query = "
            select 
                EvnPrescrTreat_id as \"EvnPrescrTreat_id\",
                Error_Code as \"Error_Code\",
                Error_Message as \"Error_Msg\"
			from p_EvnPrescrTreat_" . $action . " (
			...
			if ($arr_data == array()) {
				$query = "
                            Select 
                                Error_Code as \"Error_Code\",
                                Error_Message as \"Error_Msg\"
							from {$prefix}.p_CourseTimeIntake_del (
								CourseTimeIntake_id := :Evn_id
								)
						 ";
				$params = array();
				...
				$query = "
                    Select 
                        Error_Code as \"Error_Code\",
                        Error_Message as \"Error_Msg\"
                    from {$prefix}.p_CourseTimeIntake_upd (
                        xml := :xml
                        )
						";
			}
			...
			if (empty($data['EvnCourseTreat_id'])) {
				...
				$query1 = "
					select
						...
						EPTD.DrugComplexMnn_id as \"DrugComplexMnn_id\",
					...
					from
						v_EvnPrescrTreatDrug EPTD
						left join v_EvnPrescrTreat EPT on EPT.EvnPrescrTreat_id = EPTD.EvnPrescrTreat_id
						LEFT JOIN LATERAL (
							select
							    ED.Evn_id as EvnDrug_id
							from EvnDrug ED
				...
				$query = "
					select
						...
						COALESCE(EDr.EvnDrug_id,ED.EvnDrug_id) as \"EvnDrug_id\",
						EPT.EvnPrescrTreat_IsExec as \"EvnPrescrTreat_IsExec\"
					from
						v_EvnPrescrTreat EPT
						LEFT JOIN LATERAL (
							select
								ED2.Evn_id as EvnDrug_id
							from
								EvnDrug ED2
							where
						...
						LEFT JOIN LATERAL (
							select ED1.Evn_id as EvnDrug_id from v_EvnPrescrTreatDrug EPTD
							inner join EvnDrug ED1 on ED1.EvnPrescrTreatDrug_id = EPTD.EvnPrescrTreatDrug_id
							where EPTD.EvnPrescrTreat_id = EPT.EvnPrescrTreat_id limit 1
						) EDr ON TRUE
					where
						EPT.EvnCourse_id = :EvnCourse_id
				";
				...
				foreach ($ep_list as $row) {
					$row['EvnPrescrTreat_setDT'] = ConvertDateFormat($row['EvnPrescrTreat_setDT'], 'Y-m-d');
					...
					if ($arr_data == array()) {
						$query = "
                            Select
				                Error_Code as \"Error_Code\",
				                Error_Message as \"Error_Msg\"
							from r2.p_EvnCourseTreatTimeEntry_del (
								Evn_id := :Evn_id
				                )
						 ";
						$params = array();
						...
						$query = "
                                 Select
                                    Error_Code as \"Error_Code\",
				                    Error_Message as \"Error_Msg\"
								 from r2.XP_evnCourseTreatTimeEntry_upd (
				                    xml := :xml
				                 )
						         ";
					}
	...
	private function _saveEvnPrescrTreatDrug($data, $drug) {
		...
		$query = "
            select 
                ...
                Error_Code as \"Error_Code\",
                Error_Message as \"Error_Msg\"
			from p_EvnPrescrTreatDrug_" . ( !empty($drug['id'])? "upd" : "ins" ) . " (
	...
	private function _saveEvnCourseTreatDrug($data, $drug) {
		if (!empty($drug['id']) && empty($drug['MinDoseDay']) && empty($drug['MaxDoseDay'])) {
			$query = "	
				...
				CUBICUNITS_id as \"CUBICUNITS_id\",
				MASSUNITS_id as \"MASSUNITS_id\",
				ACTUNITS_id as \"ACTUNITS_id\",
				EvnCourseTreatDrug_FactCount as \"EvnCourseTreatDrug_FactCount\",
				EvnCourseTreatDrug_FactDose as \"EvnCourseTreatDrug_FactDose\",
				EvnCourseTreatDrug_MaxDoseDay as \"EvnCourseTreatDrug_MaxDoseDay\",
				EvnCourseTreatDrug_MinDoseDay as \"EvnCourseTreatDrug_MinDoseDay\"
				from v_EvnCourseTreatDrug
				where EvnCourseTreatDrug_id = ?
			";
		...
		$query = "
            select 
                EvnCourseTreatDrug_id as \"EvnCourseTreatDrug_id\", 
                Error_Code as \"Error_Code\",
                Error_Message as \"Error_Msg\"
			from p_EvnCourseTreatDrug_" . ( !empty($drug['id'])? "upd" : "ins" ) . " (
				EvnCourseTreatDrug_id := :EvnCourseTreatDrug_id,
				EvnCourseTreat_id := :EvnCourseTreat_id,
				Drug_id := :Drug_id,
				DrugComplexMnn_id := :DrugComplexMnn_id,
		...
		if ( is_object($result) ) {
			...
			if (// только ПЕрмь, только ГП2
				...
				if (!empty($queryParams['Drug_rlsid'])) {
					$join = 'inner join rls.v_DrugComplexMnn dcm on dcm.DrugComplexMnn_id = drug.DrugComplexMnn_id';
				}
				if (!empty($queryParams['DrugComplexMnn_id'])) {
					$join = 'inner join rls.v_DrugComplexMnn dcm on dcm.DrugComplexMnn_id = :DrugComplexMnn_id';
				}
				$query = "
                    select
                    ...
                    mnndose.DrugComplexMnnDose_Name as \"DrugComplexMnnDose_Name\",
                    mnndose.DrugComplexMnnDose_Mass as \"DrugComplexMnnDose_Mass\",
                    mnndose.MASSUNITS_id as \"MASSUNITS_id\",
                    Drug.Drug_Fas as \"Drug_Fas\",
                    coalesce(nomen.DRUGSINPPACK,1) as \"DRUGSINPPACK\",
                    coalesce(nomen.PPACKINUPACK,1) as \"PPACKINUPACK\",
                    case when exists (
                        select actmatters.actmatters_id as actmatters_id
                        from rls.drugcomplexmnnname
                        inner join rls.actmatters on actmatters.actmatters_id = drugcomplexmnnname.actmatters_id
                    ...
                    from evnvizitpl e
                    left join rls.v_drug drug on drug.drug_id = :Drug_rlsid
                    {$join}
					left join rls.v_Nomen nomen on nomen.NOMEN_ID = drug.Drug_id
                    left join rls.v_DrugComplexMnnFas mnnfas on mnnfas.DrugComplexMnnFas_id = dcm.DrugComplexMnnFas_id
                    left join rls.v_DrugComplexMnnDose mnndose on mnndose.DrugComplexMnnDose_id = dcm.DrugComplexMnnDose_id
                    where e.evn_id = :EvnCourseTreat_pid
                    limit 1
                ";
				...
				*/
				...
				if(!empty($drug['DrugComplexMnn_id']) && empty($drug['Drug_Name']) && empty($drug['Drug_Name']))
				{
					$query_getNames = "
                        select
                          COALESCE(DrugForm_Name,'') as \"DrugForm_Name\",
                          COALESCE(Drug_Name,'') as \"Drug_Name\"
                        from rls.Drug where DrugComplexMnn_id = :DrugComplexMnn_id
                    ";
	...
	public function doLoad($data) {
		$query = "
			select
				case when EPT.PrescriptionStatusType_id = 1 and coalesce(EPT.EvnPrescrTreat_IsExec,1) = 1 and ECT.EvnCourseTreat_disDT is null then 'edit' else 'view' end as \"accessType\",
				...
				EPT.PrescriptionType_id as \"PrescriptionType_id\",
				EPT.PrescriptionStatusType_id as \"PrescriptionStatusType_id\",
				to_char (EPT.EvnPrescrTreat_setDT, 'dd.mm.yyyy') as \"EvnPrescrTreat_setDate\",
				EPT.EvnPrescrTreat_PrescrCount as \"EvnPrescrTreat_PrescrCount\",
				case when coalesce(EPT.EvnPrescrTreat_IsCito,1) = 1 then 'off' else 'on' end as \"EvnPrescrTreat_IsCito\",
				EPT.EvnPrescrTreat_IsExec as \"EvnPrescrTreat_IsExec\",
				EPT.EvnPrescrTreat_Descr as \"EvnPrescrTreat_Descr\",
				EPT.PersonEvn_id as \"PersonEvn_id\",
				EPT.Server_id as \"Server_id\"
			from
				v_EvnPrescrTreat EPT
				inner join v_EvnPrescrTreatDrug EPTD on EPT.EvnPrescrTreat_id = EPTD.EvnPrescrTreat_id
				inner join v_EvnCourseTreat ECT on EPT.EvnCourse_id = ECT.EvnCourseTreat_id
				left join rls.Drug D on D.Drug_id = EPTD.Drug_id
				left join rls.v_DrugComplexMnn dcm on dcm.DrugComplexMnn_id = coalesce(EPTD.DrugComplexMnn_id,D.DrugComplexMnn_id)
				left join rls.CLSDRUGFORMS df on dcm.CLSDRUGFORMS_ID = df.CLSDRUGFORMS_ID
				left join rls.MASSUNITS mu on EPTD.MASSUNITS_ID = mu.MASSUNITS_ID
				left join rls.CUBICUNITS cu on EPTD.CUBICUNITS_id = cu.CUBICUNITS_id
				left join rls.ACTUNITS au on EPTD.ACTUNITS_id = au.ACTUNITS_id
			where
	...
	public function doLoadEvnCourseTreatEditForm($data) {
		...
		if (in_array($this->getRegionNick(),array('ufa','vologda'))) {
            ...
            $selectTime = ", '[ ' || (
		                select
			                string_agg('{ idx:' || cast(CourseTimeIntake_idx as varchar) || ',' || ' time:{$ch}' || CourseTimeIntake_time || '{$ch} }', ',')
		                from
			                {$prefix}.CourseTimeIntake
		                where
			                EvnCourse_id = ECT.EvnCourseTreat_id
			                and coalesce(EvnDrug_id, 0) = 0
                ) || ']' as \"EvnPrescrTreat_Time\" ";
		};
		...
		$query = "
			select
				...
				ECT.CourseType_id as \"CourseType_id\",
				ECT.Lpu_id as \"Lpu_id\",
				ECT.MedPersonal_id as \"MedPersonal_id\",
				ECT.LpuSection_id as \"LpuSection_id\",
				ECT.Morbus_id as \"Morbus_id\",
				to_char (ECT.EvnCourseTreat_setDT, 'dd.mm.yyyy') as \"EvnCourseTreat_setDate\",
				ECT.EvnCourseTreat_MaxCountDay as \"EvnCourseTreat_MaxCountDay\",
				ECT.EvnCourseTreat_MinCountDay as \"EvnCourseTreat_MinCountDay\",
				--null as EvnCourseTreat_CountDay,
				ECT.EvnCourseTreat_Duration as \"EvnCourseTreat_Duration\",
				ECT.EvnCourseTreat_ContReception as \"EvnCourseTreat_ContReception\",
				ECT.EvnCourseTreat_Interval as \"EvnCourseTreat_Interval\",
				...
				ECT.EvnCourseTreat_PrescrCount as \"EvnCourseTreat_PrescrCount\",
				ECT.ResultDesease_id as \"ResultDesease_id\",
				...
				case when coalesce(EPTG.EvnPrescrTreat_IsCito,1) = 1 then 'off' else 'on' end as \"EvnPrescrTreat_IsCito\",
				EPTG.EvnPrescrTreat_Descr as \"EvnPrescrTreat_Descr\",
				EPTG.EvnPrescrTreatDrug_DoseDay as \"EvnPrescrTreatDrug_DoseDay\",
				ECT.PersonEvn_id as \"PersonEvn_id\",
				ECT.Server_id as \"Server_id\",
				COALESCE(ln.NAME,ACT.LATNAME,DrugComplexMnn_LatName,'') as \"LatName\",
				erg.EvnReceptGeneral_id as \"EvnReceptGeneral_id\",
				erg.EvnReceptGeneralDrugLink_id as \"EvnReceptGeneralDrugLink_id\"
				{$selectTime}
			from
				v_EvnCourseTreat ECT
				inner join v_EvnCourseTreatDrug ECTD on ECT.EvnCourseTreat_id = ECTD.EvnCourseTreat_id
				left join rls.Drug D on D.Drug_id = ECTD.Drug_id
				left join rls.v_DrugComplexMnn dcm on dcm.DrugComplexMnn_id = coalesce(ECTD.DrugComplexMnn_id,D.DrugComplexMnn_id)
				LEFT JOIN LATERAL (
					select
						EPT.EvnPrescrTreat_IsCito as EvnPrescrTreat_IsCito,
						EPT.EvnPrescrTreat_Descr as EvnPrescrTreat_Descr,
						EPTD.EvnPrescrTreatDrug_DoseDay as EvnPrescrTreatDrug_DoseDay
				...
				left join rls.CLSDRUGFORMS df on dcm.CLSDRUGFORMS_ID = df.CLSDRUGFORMS_ID
				left join rls.MASSUNITS mu on ECTD.MASSUNITS_ID = mu.MASSUNITS_ID
				left join rls.CUBICUNITS cu on ECTD.CUBICUNITS_id = cu.CUBICUNITS_id
				left join rls.ACTUNITS au on ECTD.ACTUNITS_id = au.ACTUNITS_id
	...
	function getOldDrugOnOstat($data) {
		...
		$query = "
			select
				DUO.Drug_id as \"Drug_id\"
			from
				rls.v_Drug Drug
				inner join v_DocumentUcOst_Lite DUO on Drug.Drug_id = DUO.Drug_id
				inner join Contragent C on C.Contragent_id = DUO.Contragent_tid
			where
	...
	function getDrugOnOstat($data) {
		...
		$query = "
			...
			from
				...
				inner join rls.v_Drug Drug on Drug.Drug_id = DOR.Drug_id
				LEFT JOIN LATERAL (
					select coalesce(sum(rED.EvnDrug_Kolvo),0) as EvnDrug_Kolvo
					from v_EvnDrug rED
					...
					where oDSL.DrugShipment_id = DOR.DrugShipment_id and coalesce(rDU.DrugDocumentStatus_id,1) = 1 --новый
					limit 1
				) rl ON TRUE
				LEFT JOIN LATERAL (
					select
						(DOR.DrugOstatRegistry_Kolvo-coalesce(rl.EvnDrug_Kolvo,0)) as DocumentUcStr_Ost
                    limit 1
				) ost ON TRUE
			where
				ost.DocumentUcStr_Ost > 0
				and DOR.Storage_id in (select Storage_id as Storage_id from v_StorageStructLevel where LpuSection_id = :LpuSection_id)
				{$filters}
		";
	...
	function doOldLoadEvnDrugGrid($data) {
		$query = "
			select distinct
				...
				,coalesce(EvnDrug.Drug_id, DD.Drug_id) as \"Drug_id\"
				,coalesce(Drug.Drug_Name, dcm.DrugComplexMnn_RusName, '') as \"Drug_Name\"
				,DD.DrugPrepFas_id as \"DrugPrepFas_id\"
				,DD.Drug_Fas as \"Drug_Fas\"
				,EPTD.EvnPrescrTreatDrug_id as \"EvnPrescrTreatDrug_id\"
				,EPTD.DrugComplexMnn_id as \"DrugComplexMnn_id\"
				-- для списания в полке берем общее число приемов в курсе
				,case when EVPL.Evn_id is not null
					then ECT.EvnCourseTreat_PrescrCount
					else EPT.EvnPrescrTreat_PrescrCount
				end as \"EvnPrescrTreat_PrescrCount\"
				,case when EVPL.Evn_id is not null
					then coalesce(ECTD.EvnCourseTreatDrug_FactCount,0)
					else coalesce(EPTD.EvnPrescrTreatDrug_FactCount,0)
				...
				,ECTD.EvnCourseTreatDrug_FactCount as \"EvnCourseTreatDrug_FactCount\"
				,coalesce(EvnDrug.EvnCourse_id, EPT.EvnCourse_id) as \"EvnCourse_id\"
				,coalesce(EvnDrug.EvnDrug_rid, EPT.EvnPrescrTreat_rid) as \"EvnDrug_rid\"
				,coalesce(EvnDrug.EvnDrug_pid, EPT.EvnPrescrTreat_pid) as \"EvnDrug_pid\"
				,coalesce(EvnDrug.EvnPrescr_id, EPT.EvnPrescrTreat_id) as \"EvnPrescr_id\"
				,coalesce(EvnDrug.Person_id, EPT.Person_id) as \"Person_id\"
				,coalesce(EvnDrug.PersonEvn_id, EPT.PersonEvn_id) as \"PersonEvn_id\"
				,coalesce(EvnDrug.Server_id, EPT.Server_id) as \"Server_id\"
				,to_char (coalesce(EvnDrug.EvnDrug_setDT, EPT.EvnPrescrTreat_setDT), 'dd.mm.yyyy') as \"EvnDrug_setDate\"
				,coalesce(EvnDrug.EvnDrug_setTime, EPT.EvnPrescrTreat_setTime) as \"EvnDrug_setTime\"
				,coalesce(cast(EvnDrug.EvnDrug_Kolvo as numeric(18,0)), (EPTD.EvnPrescrTreatDrug_Kolvo * COALESCE(EPT.EvnPrescrTreat_PrescrCount, 1))) as \"EvnDrug_Kolvo\"
				,coalesce(EvnDrug.EvnDrug_KolvoEd, (EPTD.EvnPrescrTreatDrug_KolvoEd * COALESCE(EPT.EvnPrescrTreat_PrescrCount, 1))) as \"EvnDrug_KolvoEd\"

				,case when EVPL.Evn_id is not null then 1 else 2 end as \"ParentEvn_IsStac\"
				,to_char (v_Evn.Evn_setDT, 'dd.mm.yyyy') as \"Evn_setDate\"
				...
				,DD.DocumentUcStr_Ost as \"DocumentUcStr_Ost\"
				,DD.Mol_tid as \"Mol_id\"
				,DD.Mol_Name as \"Mol_Name\"
				,DD.DrugFinance_Name as \"DrugFinance_Name\"
				,DD.WhsDocumentCostItemType_Name as \"WhsDocumentCostItemType_Name\"
				,COALESCE(DD.DocumentUcStr_Name, '') as \"DocumentUcStr_Name\"
			from
				v_EvnPrescrTreatDrug EPTD
				INNER JOIN LATERAL (
					Select
                        ...
                        evnprescrtreat_id as evnprescrtreat_id,
                        evnprescrtreat_setdate as evnprescrtreat_setdate,
                        evnprescrtreat_settime as evnprescrtreat_settime,
                        evnprescrtreat_disdate as evnprescrtreat_disdate,
                        evnprescrtreat_distime as evnprescrtreat_distime,
                        evnprescrtreat_diddate as evnprescrtreat_diddate,
                        evnprescrtreat_didtime as evnprescrtreat_didtime,
                        evnprescrtreat_pid as evnprescrtreat_pid,
                        evnprescrtreat_rid as evnprescrtreat_rid,
                        lpu_id as lpu_id,
                        server_id as server_id,
                        personevn_id as personevn_id,
                        evnprescrtreat_setdt as evnprescrtreat_setdt,
                        evnprescrtreat_disdt as evnprescrtreat_disdt,
                        evnprescrtreat_diddt as evnprescrtreat_diddt,
                        evnprescrtreat_insdt as evnprescrtreat_insdt,
                        evnprescrtreat_upddt as evnprescrtreat_upddt,
                        evnprescrtreat_index as evnprescrtreat_index,
                        evnprescrtreat_count as evnprescrtreat_count,
                        pmuser_insid as pmuser_insid,
                        pmuser_updid as pmuser_updid,
                        person_id as person_id,
                        morbus_id as morbus_id,
                        evnprescrtreat_issigned as evnprescrtreat_issigned,
                        pmuser_signid as pmuser_signid,
                        evnprescrtreat_signdt as evnprescrtreat_signdt,
                        evnprescrtreat_isarchive as evnprescrtreat_isarchive,
                        evnprescrtreat_guid as evnprescrtreat_guid,
                        evnprescrtreat_indexminusone as evnprescrtreat_indexminusone,
                        evnstatus_id as evnstatus_id,
                        evnprescrtreat_statusdate as evnprescrtreat_statusdate,
                        evnprescrtreat_istransit as evnprescrtreat_istransit,
                        prescriptiontype_id as prescriptiontype_id,
                        evnprescrtreat_iscito as evnprescrtreat_iscito,
                        prescriptionstatustype_id as prescriptionstatustype_id,
                        evnprescrtreat_descr as evnprescrtreat_descr,
                        medpersonal_sid as medpersonal_sid,
                        lpusection_sid as lpusection_sid,
                        medpersonal_cid as medpersonal_cid,
                        lpusection_cid as lpusection_cid,
                        evnprescrtreat_isexec as evnprescrtreat_isexec,
                        ...
                        evnprescrtreat_countcomposit as evnprescrtreat_countcomposit,
                        evnprescrtreat_prescrcount as evnprescrtreat_prescrcount,
                        durationtype_preid as durationtype_preid
					from v_EvnPrescrTreat EPT where EPT.EvnPrescrTreat_id = EPTD.EvnPrescrTreat_id limit 1
				) EPT ON TRUE
				LEFT JOIN LATERAL(select
				                    evnclass_id as evnclass_id,
                                    evnclass_name as evnclass_name,
                                    evndrug_id as evndrug_id,
                                    evndrug_setdate as evndrug_setdate,
                                    evndrug_settime as evndrug_settime,
                                    evndrug_disdate as evndrug_disdate,
                                    evndrug_distime as evndrug_distime,
                                    evndrug_diddate as evndrug_diddate,
                                    evndrug_didtime as evndrug_didtime,
                                    evndrug_pid as evndrug_pid,
                                    evndrug_rid as evndrug_rid,
                                    lpu_id as lpu_id,
                                    server_id as server_id,
                                    personevn_id as personevn_id,
                                    evndrug_setdt as evndrug_setdt,
                                    evndrug_disdt as evndrug_disdt,
                                    evndrug_diddt as evndrug_diddt,
                                    evndrug_insdt as evndrug_insdt,
                                    evndrug_upddt as evndrug_upddt,
                                    evndrug_index as evndrug_index,
                                    evndrug_count as evndrug_count,
                                    pmuser_insid as pmuser_insid,
                                    pmuser_updid as pmuser_updid,
                                    person_id as person_id,
                                    morbus_id as morbus_id,
                                    evndrug_issigned as evndrug_issigned,
                                    pmuser_signid as pmuser_signid,
                                    evndrug_signdt as evndrug_signdt,
                                    evndrug_isarchive as evndrug_isarchive,
                                    evndrug_guid as evndrug_guid,
                                    evndrug_indexminusone as evndrug_indexminusone,
                                    evnstatus_id as evnstatus_id,
                                    evndrug_statusdate as evndrug_statusdate,
                                    evndrug_istransit as evndrug_istransit,
                                    drug_id as drug_id,
                                    lpusection_id as lpusection_id,
                                    evndrug_price as evndrug_price,
                                    evndrug_sum as evndrug_sum,
                                    documentuc_id as documentuc_id,
                                    documentucstr_id as documentucstr_id,
                                    documentucstr_oid as documentucstr_oid,
                                    mol_id as mol_id,
                                    evndrug_kolvo as evndrug_kolvo,
                                    evndrug_kolvoed as evndrug_kolvoed,
                                    evndrug_realkolvo as evndrug_realkolvo,
                                    ...
                                    evndrug_useddt as evndrug_useddt,
                                    evndrug_comment as evndrug_comment,
                                    goodsunit_bid as goodsunit_bid
				                  from v_EvnDrug where EvnPrescrTreatDrug_id = EPTD.EvnPrescrTreatDrug_id limit 1) EvnDrug ON TRUE
				left join v_EvnCourseTreat ECT on ECT.EvnCourseTreat_id = EPT.EvnCourse_id
				left join v_EvnCourseTreatDrug ECTD on ECTD.EvnCourseTreatDrug_id = EPTD.EvnCourseTreatDrug_id
				...
				inner join v_Evn on v_Evn.Evn_id = EPT.EvnPrescrTreat_pid
				left join EvnPS EPS on EPS.Evn_id = v_Evn.Evn_id
				left join EvnSection ES on ES.Evn_id = v_Evn.Evn_id
				left join EvnVizit EVPL on EVPL.Evn_id = v_Evn.Evn_id
				inner join LpuSection LS on LS.LpuSection_id = COALESCE(ECT.LpuSection_id, ES.LpuSection_id, EVPL.LpuSection_id, EPS.LpuSection_pid)
				inner join v_MedPersonal MP on MP.MedPersonal_id = COALESCE(ECT.MedPersonal_id, ES.MedPersonal_id, EVPL.MedPersonal_id, EPS.MedPersonal_pid)
				LEFT JOIN LATERAL (
					select
						/*coalesce(DP.DrugPrep_Name, D.DrugTorg_Name) as DrugPrep_Name,*/
						...
						,DUS.DocumentUcStr_Ost as DocumentUcStr_Ost
						,DUS.DrugFinance_Name as DrugFinance_Name
						,DUS.WhsDocumentCostItemType_Name as WhsDocumentCostItemType_Name
						,DUS.DocumentUcStr_Ser as DocumentUcStr_Ser
						,RTRIM(LTRIM((COALESCE(M.Person_SurName, '') || ' ' || COALESCE(M.Person_FirName, '') || ' ' || COALESCE(M.Person_SecName, '')))) as Mol_Name
						,cast(cast(round(coalesce(DUS.DocumentUcStr_Ost, 0), 4) as numeric(16, 4)) as varchar(20)) || ', фин. '
							|| RTRIM(RTRIM(COALESCE(DUS.DrugFinance_Name, 'отсут.'))) || ', серия ' || RTRIM(COALESCE(DUS.DocumentUcStr_Ser, ''))
						as DocumentUcStr_Name
					from dbo.DocumentUcOst_Lite(null) DUS
						inner join rls.v_Drug D on DUS.Drug_id = D.Drug_id
						/*left join rls.DrugPrep DP on DP.DrugPrepFas_id = D.DrugPrepFas_id*/
						inner join v_DocumentUc DU on DU.DocumentUc_id = DUS.DocumentUc_id
						left join v_Mol M on M.Mol_id = DU.Mol_tid
						inner join Contragent C on C.Contragent_id = DUS.Contragent_tid and C.LpuSection_id = LS.LpuSection_id
					where (D.Drug_id = coalesce(EvnDrug.Drug_id, EPTD.Drug_id)
	...
	function doLoadEvnDrugGrid($data) {
		...
		if(!empty($data['forAPI'])){
			$fields .= "
				,DD.DocumentUc_id as \"DocumentUc_id\"
				,DD.DocumentUcStr_Price as \"DocumentUcStr_Price\"
				,DD.DocumentUcStr_Sum as \"DocumentUcStr_Sum\"
				,GU.GoodsUnit_id as \"GoodsUnit_id\"
				,DD.GoodsUnit_bid as \"GoodsUnit_bid\"
				,GU.GoodsPackCount_Count AS \"GoodsPackCount_bCount\"
			";
			$outer_apply .= "
				LEFT JOIN LATERAL (
					select 
						i_gu.GoodsUnit_id as GoodsUnit_id,
						coalesce(i_gpc.GoodsPackCount_Count, i_gpc_m.GoodsPackCount_Count, 1) as GoodsPackCount_Count
					from
						v_GoodsUnit i_gu 
						left join v_GoodsPackCount i_gpc  on i_gpc.GoodsUnit_id = i_gu.GoodsUnit_id and i_gpc.DrugComplexMnn_id = EPTD.DrugComplexMnn_id and i_gpc.TRADENAMES_ID = Drug.DrugTorg_id -- ищем по мнн+торговое
						left join v_GoodsPackCount i_gpc_m  on i_gpc_m.GoodsUnit_id = i_gu.GoodsUnit_id and i_gpc_m.DrugComplexMnn_id = EPTD.DrugComplexMnn_id and i_gpc_m.TRADENAMES_ID is null -- ищем по мнн среди записей без торгового
					where
						(
		...
		$query = "
			...
			from
				v_EvnPrescrTreat EPT
				inner join v_EvnPrescrTreatDrug EPTD on EPT.EvnPrescrTreat_id = EPTD.EvnPrescrTreat_id
				left join lateral (select * from v_EvnDrug where EvnPrescrTreatDrug_id = EPTD.EvnPrescrTreatDrug_id limit 1) EvnDrug ON TRUE
				left join v_EvnCourseTreat ECT on ECT.EvnCourseTreat_id = EPT.EvnCourse_id
				left join v_EvnCourseTreatDrug ECTD on ECTD.EvnCourseTreatDrug_id = EPTD.EvnCourseTreatDrug_id
				left join rls.v_Drug Drug on Drug.Drug_id = EPTD.Drug_id
				left join rls.v_DrugComplexMnn dcm on dcm.DrugComplexMnn_id = coalesce(EPTD.DrugComplexMnn_id,Drug.DrugComplexMnn_id)
				inner join v_Evn on v_Evn.Evn_id = EPT.EvnPrescrTreat_pid
				inner join v_Lpu on v_Evn.Lpu_id = v_Lpu.Lpu_id 
				left join EvnPS EPS on EPS.Evn_id = v_Evn.Evn_id
				left join EvnSection ES on ES.Evn_id = v_Evn.Evn_id
				left join EvnVizit EVPL on EVPL.Evn_id = v_Evn.Evn_id
				inner join v_LpuSection_all LS on LS.LpuSection_id = COALESCE(ECT.LpuSection_id, ES.LpuSection_id, EVPL.LpuSection_id, EPS.LpuSection_pid)
				inner join v_MedPersonal MP on MP.MedPersonal_id = COALESCE(ECT.MedPersonal_id, ES.MedPersonal_id, EVPL.MedPersonal_id, EPS.MedPersonal_pid)
				LEFT JOIN LATERAL (
					select
						D.DrugPrepFas_id as DrugPrepFas_id
						...
						,DUS.Drug_id as Drug_id
						,DUS.DocumentUcStr_id as DocumentUcStr_id
						,coalesce(ost.DocumentUcStr_Ost,0) as DocumentUcStr_Ost
						...
						,case
							...
							else RTRIM(LTRIM((COALESCE(M.Person_SurName, '') || ' ' || COALESCE(M.Person_FirName, '') || ' ' || COALESCE(M.Person_SecName, ''))))
						end as Mol_Name
						,cast(cast(round(coalesce(ost.DocumentUcStr_Ost, 0), 4) as numeric(16, 4)) as varchar(20)) || ', фин. '|| RTRIM(RTRIM(COALESCE(DF.DrugFinance_Name, 'отсут.'))) || ', серия ' || RTRIM(COALESCE(DUS.DocumentUcStr_Ser, ''))
						as DocumentUcStr_Name
					...
					from
						...
						left join v_Mol M on M.Mol_id = DU.Mol_tid and M.Storage_id = DU.Storage_tid
						LEFT JOIN LATERAL (
						...
						LEFT JOIN LATERAL (
							...
							inner join v_DocumentUc rDU on rDU.DocumentUc_id = rDUS.DocumentUc_id
							inner join v_DrugShipmentLink oDSL on oDSL.DocumentUcStr_id = rED.DocumentUcStr_oid
							where oDSL.DrugShipment_id = DSL.DrugShipment_id and coalesce(rDU.DrugDocumentStatus_id,1) = 1 --новый
							limit 1
						) rl ON TRUE
						LEFT JOIN LATERAL (
							select
					...
					where  1=1
						and (D.Drug_id = coalesce(EvnDrug.Drug_id, EPTD.Drug_id)
						OR D.DrugComplexMnn_id = EPTD.DrugComplexMnn_id) --не всегда работает
	...
	function getDrugComplexMnnByActMatters($data) {
		$query = '
			select mnn.DrugComplexMnn_id as "DrugComplexMnn_id"
			from rls.v_DrugComplexMnn mnn
			where exists (
				select MnnName.DrugComplexMnnName_id as DrugComplexMnnName_id from rls.v_DrugComplexMnnName MnnName
	...
	function getPrintData($data) {
		$query = "
			select distinct
				...
				,(select count(EvnPrescrTreatDrug_id) from v_EvnPrescrTreatDrug where EvnPrescrTreat_id = EP.EvnPrescrTreat_id) as \"cntDrug\"
			...
			from v_EvnPrescrTreat EP
				...
				left join rls.CUBICUNITS on EPTD.CUBICUNITS_ID = CUBICUNITS.CUBICUNITS_ID
				left join rls.MASSUNITS on EPTD.MASSUNITS_ID = MASSUNITS.MASSUNITS_ID
				left join rls.ACTUNITS on EPTD.ACTUNITS_ID = ACTUNITS.ACTUNITS_ID
				...
				left join DurationType DTN on ECT.DurationType_recid = DTN.DurationType_id
				left join DurationType DTI on ECT.DurationType_intid = DTI.DurationType_id
	...
	public function doOldLoadEvnPrescrTreatDrugCombo($data) {
		...
		$query = "
			...
			from
				...
				left join rls.v_DrugComplexMnn dcm on dcm.DrugComplexMnn_id = coalesce(EPTD.DrugComplexMnn_id,Drug.DrugComplexMnn_id)
				left join rls.CLSDRUGFORMS df  on dcm.CLSDRUGFORMS_ID = df.CLSDRUGFORMS_ID
				left join v_EvnPS EPS on EPS.EvnPS_id = EPT.EvnPrescrTreat_pid
				left join v_EvnSection ES on ES.EvnSection_id = EPT.EvnPrescrTreat_pid
				left join v_EvnVizitPL EVPL on EVPL.EvnVizitPL_id = EPT.EvnPrescrTreat_pid
				...
				LEFT JOIN LATERAL (
					...
					from dbo.DocumentUcOst_Lite(null) DUS
						...
						inner join v_DocumentUc DU on DU.DocumentUc_id = DUS.DocumentUc_id
						inner join v_Mol M on M.Mol_id = DU.Mol_tid
	...
	public function doLoadEvnPrescrTreatDrugCombo($data) {
		...
		if (in_array($this->getRegionNick(),array('ufa','vologda'))) {
			$join = "left join r2.fn_GoodsPackCount ( EPTD.DrugComplexMnn_id) i_gpc_m  on i_gpc_m.GoodsUnit_id = i_gu.GoodsUnit_id and i_gpc_m.DrugComplexMnn_id = EPTD.DrugComplexMnn_id  -- ищем по мнн среди записей без торгового
					";
            ...
            $fldTime = ", '[ ' || (
							select 
							    string_agg('{ idx:' || cast(CourseTimeIntake_idx as varchar) || ',' || ' time:{$ch}' || CourseTimeIntake_time || '{$ch} }', ',')
                            from
                                {$prefix}.v_CourseTimeIntake ect
                            where 
                                EvnCourse_id = EPT.EvnCourse_id 
                                and EvnDrug_id is null
                                and not exists(select 1 from r2.v_CourseTimeIntake ect2)
			...
			$join = "left join v_GoodsPackCount i_gpc_m on i_gpc_m.GoodsUnit_id = i_gu.GoodsUnit_id and i_gpc_m.DrugComplexMnn_id = EPTD.DrugComplexMnn_id and i_gpc_m.TRADENAMES_ID is null -- ищем по мнн среди записей без торгового
						";
		}
		...
		$query = "
			select
				...
				coalesce(Drug.Drug_Name, dcm.DrugComplexMnn_RusName, '') as \"Drug_Name\",
				DD.DrugPrepFas_id as \"DrugPrepFas_id\",
				coalesce(DD.Drug_Fas, 1) as \"Drug_Fas\",
				DD.DocumentUcStr_id as \"DocumentUcStr_oid\",
				DD.DocumentUcStr_Ost as \"DocumentUcStr_Ost\",
				DD.Storage_tid as \"Storage_id\",
				DD.Mol_id as \"Mol_id\",
				...
				coalesce(GU_S.GoodsPackCount_Count, 1) as \"GoodsPackCount_sCount\"
				{$fldTime}
			from
				v_EvnPrescrTreat EPT
				inner join v_EvnPrescrTreatDrug EPTD on EPT.EvnPrescrTreat_id = EPTD.EvnPrescrTreat_id
				...
				left join v_EvnSection ES on ES.EvnSection_id = EPT.EvnPrescrTreat_pid
				left join v_EvnVizitPL EVPL on EVPL.EvnVizitPL_id = EPT.EvnPrescrTreat_pid
				left join v_LpuSection LS on LS.LpuSection_id = coalesce(ES.LpuSection_id, EVPL.LpuSection_id, EPS.LpuSection_pid)
				...
				LEFT JOIN LATERAL (
					...
					from
						...
						left join v_Mol M on M.Mol_id = DU.Mol_tid and M.Storage_id = DOR.Storage_id
						LEFT JOIN LATERAL (
							select Person_Fio as Person_Fio
							from v_MedPersonal
							where MedPersonal_id = M.MedPersonal_id
						...
						LEFT JOIN LATERAL (
							select
								(
									(case when S.MedServiceType_SysNick ilike 'merch' then 10 else 0 end) +
					...
					where  1=1
						...
						and DOR.DrugOstatRegistry_Kolvo > coalesce(rl.EvnDrug_Kolvo,0)
					order by
						ord.idx desc
                    limit 1
				) DD ON TRUE
				...
				LEFT JOIN LATERAL (
					select
						i_gpc.GoodsPackCount_Count as GoodsPackCount_Count
					from
	...
	public function doLoadEvnPrescrTreatDrugDataView($data) {
		...
		$query = "
			select
				...
				,to_char (EPT.EvnPrescrTreat_setDT, 'dd.mm.yyyy') as \"EvnPrescr_setDate\"
			...
			from
				...
				LEFT JOIN LATERAL (
					select EvnDrug_id as EvnDrug_id, EvnDrug_setDT as EvnDrug_setDT from v_EvnDrug
		...
		if ($isNeedPrintEvnReceptGeneral) {
			...
			$addJoin .= '
				LEFT JOIN LATERAL (
					...
					from v_EvnReceptGeneralDrugLink ergdl
						inner join v_EvnReceptGeneral ERG on ERG.EvnReceptGeneral_id = ergdl.EvnReceptGeneral_id
						left join v_ReceptForm rf on rf.ReceptForm_id = erg.ReceptForm_id
						left join v_ReceptType RT on RT.ReceptType_id = ERG.ReceptType_id
			...
			$addSelect .= '
				,erg.EvnReceptGeneral_id as "EvnReceptGeneral_id"
				,erg.ReceptForm_Code as "ReceptForm_Code"
				,erg.EvnReceptGeneral_Ser as "EvnReceptGeneral_Ser"
				,erg.EvnReceptGeneral_Num as "EvnReceptGeneral_Num"
				,erg.MedPersonal_id as "MedPersonal_id"
				,erg.EvnReceptGeneralDrugLink_id as "EvnReceptGeneralDrugLink_id"
				,erg.ReceptType_Code as "ReceptType_Code"
				,erg.ReceptForm_Name as "ReceptForm_Name"
			';
		}
		$query = "
			with prescr as (
				select
					...
					,case when 2 = EP.EvnPrescr_IsExec then to_char(EP.EvnPrescr_updDT,'dd.mm.yyyy')||' '||to_char(EP.EvnPrescr_updDT,'hh:mm:ss') else null end as EvnPrescr_execDT
					...
					,EP.EvnPrescr_updDT as EvnPrescr_updDT
				from v_EvnPrescr EP
				where
			...
			select
				...
				,case when EDr.EvnDrug_id is null then 1 else 2 end as \"EvnPrescr_IsHasEvn\"
				,EP.EvnPrescr_execDT as \"EvnPrescr_execDT\"
				,EP.EvnPrescr_IsDir as \"EvnPrescr_IsDir\"
				,EP.PrescriptionStatusType_id as \"PrescriptionStatusType_id\"
				,EP.PrescriptionType_id as \"PrescriptionType_id\"
				,EP.PrescriptionType_Code as \"PrescriptionType_Code\"
				...
				,EPTD.EvnPrescrTreatDrug_FactCount as \"FactCntDay\"
				,EPT.EvnPrescrTreat_PrescrCount as \"PrescrCntDay\"
				,ec_drug.EvnCourseTreatDrug_id as \"EvnCourseTreatDrug_id\"
				...
				,LTRIM(cast(cast(ec_drug.EvnCourseTreatDrug_KolvoEd as decimal(10, 2)) as varchar(11))) as \"EvnCourseTreatDrug_KolvoEd\"
				,coalesce(CourseDf.CLSDRUGFORMS_NameLatinSocr,CourseDf.NAME,CourseDrug.DrugForm_Name,'') as \"CourseDrugForm_Name\"
				,LTRIM(cast(cast(ec_drug.EvnCourseTreatDrug_Kolvo as decimal(10, 2)) as varchar(11))) as \"EvnCourseTreatDrug_Kolvo\"
				...
				,ec_drug.EvnCourseTreatDrug_MaxDoseDay as \"EvnCourseTreatDrug_MaxDoseDay\"
				,ec_drug.EvnCourseTreatDrug_MinDoseDay as \"EvnCourseTreatDrug_MinDoseDay\"
				,ec_drug.EvnCourseTreatDrug_PrescrDose as \"EvnCourseTreatDrug_PrescrDose\"
				,ec_drug.EvnCourseTreatDrug_FactDose as \"EvnCourseTreatDrug_FactDose\"
				,COALESCE(PIT.PrescriptionIntroType_Name, '') as \"PrescriptionIntroType_Name\"
				,COALESCE(PFT.PerformanceType_Name, '') as \"PerformanceType_Name\"
				,EPT.EvnCourse_id as \"EvnCourse_id\"
				,COALESCE(ECT.EvnCourseTreat_MaxCountDay, 0) as \"MaxCountInDay\"
				,COALESCE(ECT.EvnCourseTreat_MinCountDay, 0) as \"MinCountInDay\"
				,COALESCE(ECT.EvnCourseTreat_Duration, 0) as \"Duration\"
				,COALESCE(ECT.EvnCourseTreat_ContReception, 0) as \"ContReception\"
				,COALESCE(ECT.EvnCourseTreat_Interval, 0) as \"Interval\"
				,ECT.EvnCourseTreat_PrescrCount as \"EvnCourseTreat_PrescrCount\"
			...
			from prescr EP
				...
				left join rls.CUBICUNITS ep_cu on EPTD.CUBICUNITS_id = ep_cu.CUBICUNITS_id
				left join rls.ACTUNITS ep_au on EPTD.ACTUNITS_id = ep_au.ACTUNITS_id
				left join rls.MASSUNITS ec_mu on ec_drug.MASSUNITS_ID = ec_mu.MASSUNITS_ID
				left join rls.CUBICUNITS ec_cu on ec_drug.CUBICUNITS_id = ec_cu.CUBICUNITS_id
				left join rls.ACTUNITS ec_au on ec_drug.ACTUNITS_id = ec_au.ACTUNITS_id
				...
				--left join rls.v_DrugComplexMnn dcm on dcm.DrugComplexMnn_id = coalesce(EPTD.DrugComplexMnn_id,Drug.DrugComplexMnn_id)
				left join rls.CLSDRUGFORMS df on dcm.CLSDRUGFORMS_ID = df.CLSDRUGFORMS_ID
				left join rls.v_Drug CourseDrug on CourseDrug.Drug_id = ec_drug.Drug_id
				left join rls.v_DrugComplexMnn CourseDcm on CourseDcm.DrugComplexMnn_id = coalesce(ec_drug.DrugComplexMnn_id,CourseDrug.DrugComplexMnn_id)
				left join rls.CLSDRUGFORMS CourseDf on CourseDcm.CLSDRUGFORMS_ID = CourseDf.CLSDRUGFORMS_ID
				left join DurationType DTP on ECT.DurationType_id = DTP.DurationType_id
				left join DurationType DTN on ECT.DurationType_recid = DTN.DurationType_id
				left join DurationType DTI on ECT.DurationType_intid = DTI.DurationType_id
				LEFT JOIN LATERAL (
	...
	function getDrugPackData($data) {
		...
		$join = "
			LEFT JOIN LATERAL (
                        select -- лекарственная форма, если не находим в справочнике, то считаем что это таблетка
                        ...
                        from
                            rls.CLSDRUGFORMS i_cdf
                            left join v_GoodsUnit i_gu on i_gu.GoodsUnit_Name = i_cdf.FULLNAME
							left join v_GoodsUnit i_gu_tab on i_gu_tab.GoodsUnit_Nick = 'таб.'
                        where
		...
		if (!empty($data['Drug_id'])) {
			$query = "
                select
                    ...
                    (
                       coalesce(dcmf.DrugComplexMnnFas_Kol, 1)*coalesce(dcmf.DrugComplexMnnFas_KolPrim, 1)*coalesce(dcmf.DrugComplexMnnFas_Tert, 1)
                    ) as \"Fas_NKolvo\",
                    (
                        case
                ...
                from
                    ...
                    left join rls.MASSUNITS f_mu on f_mu.MASSUNITS_ID = dcmf.MASSUNITS_ID
                    left join rls.CUBICUNITS f_cu on f_cu.CUBICUNITS_ID = dcmf.CUBICUNITS_ID
                    left join rls.v_DrugComplexMnnDose dcmd on dcmd.DrugComplexMnnDose_id = dcm.DrugComplexMnnDose_id
                    left join rls.MASSUNITS d_mu on d_mu.MASSUNITS_ID = dcmd.MASSUNITS_ID
                    left join rls.CONCENUNITS d_cu on d_cu.CONCENUNITS_ID = dcmd.CONCENUNITS_ID
                    left join rls.ACTUNITS d_au on d_au.ACTUNITS_ID = dcmd.ACTUNITS_ID
					{$join}
                    LEFT JOIN LATERAL (
                        select
                            i_gu.GoodsUnit_id as GoodsUnit_id,
                            i_gu.GoodsUnit_Nick as GoodsUnit_Nick
                        from
                            v_GoodsUnit i_gu
                        where
                            (
	...
	function getEvnPrescrTreatForAPI($data) {
		...
		if (!empty($data['DrugComplexMnn_id'])) {
			$filters[] = "coalesce(dcmf.DrugComplexMnnFas_Tert, 1) = :DrugComplexMnn_id";
			$params['DrugComplexMnn_id'] = $data['DrugComplexMnn_id'];
		}
		...
		$query = "
			select
				...
				EPT.EvnClass_id as \"EvnClass_id\",
				EPT.Lpu_id as \"Lpu_id\",
				to_char (EPT.EvnPrescrTreat_setDate, 'yyyy-mm-dd hh24:mm:ss') as \"Evn_setDT\",
				EPT.Person_id as \"Person_id\",
				EPT.PrescriptionType_id as \"PrescriptionType_id\",
				...
				ECTD.Drug_id as \"Drug_id\",
				EPTD.EvnPrescrTreatDrug_Kolvo as \"EvnPrescrTreatDrug_Kolvo\",
				ECTD.GoodsUnit_id as \"GoodsUnit_id\",
				EPTD.EvnPrescrTreatDrug_KolvoEd as \"EvnPrescrTreatDrug_KolvoEd\",
				ECTD.GoodsUnit_sid as \"GoodsUnit_sid\",
				EPTD.EvnPrescrTreatDrug_DoseDay as \"EvnPrescrTreatDrug_DoseDay\",
				ECTD.EvnCourseTreatDrug_Kolvo as \"EvnCourseTreatDrug_Kolvo\",
				ECTD.EvnCourseTreatDrug_KolvoEd as \"EvnCourseTreatDrug_KolvoEd\",
				ECTD.EvnCourseTreatDrug_PrescrDose as \"EvnCourseTreatDrug_PrescrDos\"
			from 
				v_EvnPrescrTreat EPT
				left join v_EvnPrescrTreatDrug EPTD on EPTD.EvnPrescrTreat_id = EPT.EvnPrescrTreat_id
				left join v_EvnCourseTreat ECT on ECT.EvnCourseTreat_id = EPT.EvnCourse_id
				left join v_EvnCourseTreatDrug ECTD on ECTD.EvnCourseTreat_id = ECT.EvnCourseTreat_id
				left join rls.v_Drug D on D.Drug_id = ECTD.Drug_id
				left join v_YesNo IsCito on IsCito.YesNo_id = EPT.EvnPrescrTreat_IsCito
				left join v_YesNo IsExec on IsExec.YesNo_id = EPT.EvnPrescrTreat_IsExec
			where
				{$filters_str}
		";
	...
	function loadEvnPrescrTreatInfoForAPI($data) {
		...
		$query = "
			select
				...
				EPT.Server_id as \"Server_id\",
				EPT.Lpu_id as \"Lpu_id\",
				coalesce(EPT.EvnPrescrTreat_isExec, 1) as \"EvnPrescr_isExec\",
				coalesce(EPT.EvnPrescrTreat_IsCito, 1) as \"EvnPrescr_IsCito\",
				EPT.EvnPrescrTreat_Descr as \"EvnPrescr_Descr\",
				EPT.EvnPrescrTreat_PrescrCount as \"EvnPrescrTreat_PrescrCount\",
				...
				ECT.LpuSection_id as \"LpuSection_id\",
				ECT.Morbus_id as \"Morbus_id\",
				to_char (ECT.EvnCourseTreat_setDate, 'yyyy-mm-dd hh24:mm:ss') as \"Evn_setDT\",
				ECT.EvnCourseTreat_Duration as \"EvnCourse_Duration\",
				ECT.DurationType_id as \"DurationType_id\",
				ECT.EvnCourseTreat_ContReception as \"EvnCourse_ContReception\",
				ECT.DurationType_recid as \"DurationType_recid\",
			...
			from
				...
				inner join v_EvnCourseTreat ECT on ECT.EvnCourseTreat_id = EPT.EvnCourse_id
				inner join v_EvnCourseTreatDrug ECTD on ECTD.EvnCourseTreat_id = ECT.EvnCourseTreat_id
				left join v_Evn E on E.Evn_id = EPT.EvnPrescrTreat_pid
			where
				EPT.EvnPrescrTreat_id = :EvnPrescrTreat_id
            limit 1
		";
	...
	function loadEvnPrescrTreatDataForUpdate($data) {
		...
		$query = "
			select
				...
				ECT.PrescriptionIntroType_id as \"PrescriptionIntroType_id\",
				to_char (ECT.EvnCourseTreat_setDate, 'yyyy-mm-dd hh24:mm:ss') as \"EvnCourseTreat_setDate\",
	...
	public function checkPersonPrescrTreat($data) {
		$sql = "
			SELECT
				coalesce(Drug.Drug_Name, dcm.DrugComplexMnn_RusName, '') as \"Drug_Name\"
                ,to_char(EP.EvnPrescr_setDT::date, 'DD.MM.YYYY') as \"date_start\"
                ,ECT.EvnCourseTreat_Duration||' '||DTP.DurationType_Nick AS \"duration\"
                ,to_char(duration.date_end::date, 'DD.MM.YYYY') as \"date_end\"
			...
			FROM v_EvnPrescr EP
				...
				LEFT JOIN LATERAL (
					select
						case
							when ECT.DurationType_id = 1 then EP.EvnPrescr_setDT + ECT.EvnCourseTreat_Duration * INTERVAL '1 day' --день
							when ECT.DurationType_id = 2 then EP.EvnPrescr_setDT + 7 * ECT.EvnCourseTreat_Duration * INTERVAL '1 day' --неделя
							when ECT.DurationType_id = 3 then EP.EvnPrescr_setDT + ECT.EvnCourseTreat_Duration * INTERVAL '1 month' --месяц
						end as date_end
				) duration ON TRUE -- дата окончания назанчения
                
			WHERE
				EP.Person_id=:Person_id
				and EP.PrescriptionType_id = 5 --лекарство
				and EP.PrescriptionStatusType_id != 3 --рабочее
				and duration.date_end > dbo.tzGetDate() --просрочка
			LIMIT 6
		";//exit($sql);
			...
			if($allergic){
				...
				$sql = "
					...
					FROM rls.v_DrugComplexMnn dcm
						left join rls.v_Drug Drug on Drug.Drug_id = :DrugComplexMnn_id
					WHERE 
						dcm.DrugComplexMnn_id = :DrugComplexMnn_id
					LIMIT 1
				";
