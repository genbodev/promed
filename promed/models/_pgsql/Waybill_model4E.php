<?php
class Waybill_model4E extends swPgModel {
	...
	public function saveWaybill( $data ){
		...
		$query = "
			SELECT Waybill_id as \"Waybill_id\", Error_Code as \"Error_Code\", Error_Message as \"Error_Msg\"
			FROM ".$procedure."(
		...
		if ( array_key_exists( 'WaybillRoute', $data ) && !empty( $data['WaybillRoute'] ) && $data['WaybillRoute'] != '[]' ) {
			...
			foreach( $waybillroute as $k => $route ) {
				...
				$query = "
					SELECT WaybillRoute_id as \"WaybillRoute_id\", Error_Code as \"Error_Code\", Error_Message as \"Error_Msg\"
					FROM ".$procedure."(
						WaybillRoute_id := :WaybillRoute_id,
						Waybill_id := :Waybill_id,
	...
	function printWaybill( $data ) {
		...
		$query = "
			SELECT 
				...
				Waybill_CalcMakePost as \"Waybill_CalcMakePost\",
				Waybill_CalcMakeName as \"Waybill_CalcMakeName\",
				wg.WaybillGas_Code as \"WaybillGas_Code\",
				wg.WaybillGas_Name as \"WaybillGas_Name\",
			...
			FROM
				...
				LEFT JOIN v_MedPersonal as MP2  ON( MP.MedPersonal_id=ET.EmergencyTeam_Driver )
			WHERE
				w.Waybill_id = :Waybill_id
            LIMIT 1
		";
