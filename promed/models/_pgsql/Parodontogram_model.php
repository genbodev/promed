 ...
 */
class Parodontogram_model extends swPgModel
{
	...
	public function doLoadToothStateValues($data)
	{
		...
		$selectString = "
				...
				t.Tooth_Code \"Tooth_Code\",
				s.ToothStateType_id as \"ToothStateType_id\",-- === ToothStateType_Code
				s.ToothStateType_Code as \"ToothStateType_Code\",
				s.ToothStateType_Name as \"ToothStateType_Name\",
				s.ToothStateType_Nick as \"ToothStateType_Nick\",
				v.ToothStateValues_id as \"ToothStateValues_id\",
		...
		$query = "
			select
				{$selectString}
			from v_Tooth t
			inner join v_ToothStateValues v on t.Tooth_id = v.Tooth_id
			inner join v_ToothStateType s on s.ToothStateType_id = v.ToothStateType_id
			order by
	...
	public function doLoadHistory($data)
	{
		...
		$query = "
            SELECT e.EvnUslugaStom_id as \"EvnUslugaStom_id\",
				   to_char(v.EvnVizitPLStom_setDate, 'dd.mm.yyyy')  as \"EvnUslugaStom_setDate\",
				   v.EvnVizitPLStom_setTime as \"EvnUslugaStom_setTime\",
				   e.Lpu_id as \"Lpu_id\",
				   e.MedPersonal_id as \"MedPersonal_id\",
				   lpu.Lpu_Nick as \"Lpu_Nick\",
				   mp.person_surname || ' ' || LEFT(mp.person_firname, 1) || '. ' ||
				   coalesce(LEFT(mp.person_secname, 1) || '. ', '') as \"MedPersonal_Fin\"
			FROM v_EvnUslugaStom e
				 inner join v_EvnVizitPLStom v on v.EvnVizitPLStom_id = e.EvnUslugaStom_pid
	...
	public function doLoadMarkerData($data)
	{
		...
		try {
			 ...
			 */
			$query = "
				select eu.EvnUslugaStom_id as \"EvnUslugaStom_id\"
				from v_EvnVizitPLStom ev
						 inner join v_EvnUslugaStom eu on eu.EvnUslugaStom_pid = ev.EvnVizitPLStom_id
						 inner join (select st.EvnVizitPLStom_setDT as max_dt, st.Person_id as persid
									 from v_EvnVizitPLStom st
									 where st.EvnVizitPLStom_id = :EvnVizitPLStom_id) tmp ON ev.Person_id = tmp.persid and exists(
						select p.Parodontogram_id
						from v_Parodontogram p
						where p.EvnUslugaStom_id = eu.EvnUslugaStom_id
					) and ev.EvnVizitPLStom_setDT <= tmp.max_dt
	...
	public function hasParodontogram($EvnUslugaStom_id = 0, $EvnVizitPLStom_id = 0)
	{
		if ($EvnUslugaStom_id > 0) {
			$query = "
				select p.EvnUslugaStom_id as \"EvnUslugaStom_id\"
				from v_Parodontogram p
			...
			$query = "
				select
					e.EvnUslugaStom_id as \"EvnUslugaStom_id\"
				from v_EvnUslugaStom e 
	...
	public function loadEvnUslugaStomData($EvnUslugaStom_id = 0, $Person_id = 0)
	{
		if ($Person_id > 0) {
			...
			$from_clause = "v_PersonState PS
			inner join v_UslugaComplex uc2011 on uc2011.UslugaCategory_id = 4 and
				uc2011.UslugaComplex_Code like 'A02.07.009'
			left join lateral (
				select
					e.EvnUslugaStom_id,
					e.EvnUslugaStom_pid,
					null::timestamp as EvnUslugaStom_setDT
				from v_EvnUslugaStom e
			...
			$where_clause = "e.EvnUslugaStom_id = :EvnUslugaStom_id
				and exists (
					select uca.UslugaComplex_id
					from v_UslugaComplexAttribute uca
					inner join v_UslugaComplexAttributeType ucat on ucat.UslugaComplexAttributeType_id = uca.UslugaComplexAttributeType_id
						and ucat.UslugaComplexAttributeType_SysNick like 'parondontogram'
					where uca.UslugaComplex_id = e.UslugaComplex_id
				)";
			$from_clause = "v_EvnUslugaStom e
			inner join v_PersonState PS on PS.Person_id = e.Person_id
			inner join v_UslugaComplex uc on uc.UslugaComplex_id = e.UslugaComplex_id
		...
		$selectString = "
			...
			age2(PS.Person_BirthDay, coalesce(e.EvnUslugaStom_setDT, dbo.tzGetDate())) as \"Person_Age\",
			e.EvnUslugaStom_id as \"EvnUslugaStom_id\",
			e.EvnUslugaStom_pid as \"EvnUslugaStom_pid\",
			uc2011.UslugaComplex_Code as \"UslugaComplex_Code\",
			uc2011.UslugaComplex_Name as \"UslugaComplex_Name\",
			to_char(coalesce(e.EvnUslugaStom_setDT, dbo.tzGetDate()), 'dd.mm.yyyy') as \"EvnUslugaStom_setDate\"
		";
		$query = "
			select
				{$selectString}
	...
	public function loadToothStateValues($data, $EvnUslugaStom_id)
	{
		...
		if ($EvnUslugaStom_id > 0 && $this->getOperation() != 'doLoadNewViewData') {
			...
			$from_clause = '
				v_Parodontogram p
				inner join ToothAll on ToothAll.Tooth_id = p.Tooth_id
				left join v_ToothStateValues v on v.ToothStateType_id = p.ToothStateType_id
					and v.Tooth_id = p.Tooth_id
				left join v_ToothStateType s on s.ToothStateType_id = v.ToothStateType_id';
		...
		if ($this->getOperation() == 'doLoadNewViewData' || empty($EvnUslugaStom_id)) {
            ...
            AS (
                SELECT
                    ...
                    substring(cast(v_Tooth.Tooth_Code as varchar(2)), 2, 1) as Tooth_Num
                FROM v_Tooth
                inner join v_PersonToothCard on v_PersonToothCard.Tooth_id = v_Tooth.Tooth_id
                WHERE v_PersonToothCard.Person_id = :Person_id
                    and v_PersonToothCard.PersonToothCard_begDate < now()
                    and v_PersonToothCard.PersonToothCard_endDate is null
            )';
			$from_clause = 'ToothAll
                left join ToothMap tm on tm.Tooth_id = ToothAll.Tooth_id
                    and tm.ToothStateClass_id in (12,13,14)';
			$tm_select = 'tm.PersonToothCard_id,';
			$where_clause = 'tm.PersonToothCard_id is not null or
			 (
                 not exists (
                    select PersonToothCard_id
                    from ToothMap where ToothMap.JawPartType_id = ToothAll.JawPartType_id
			...
			if ($EvnUslugaStom_id > 0) {
				...
				$from_clause .= '
				left join PrevParodontogram p on p.Tooth_id = ToothAll.Tooth_id
					and tm.PersonToothCard_id is null
                left join v_ToothStateValues v on v.Tooth_id = ToothAll.Tooth_id
                    and v.ToothStateType_id = coalesce(tm.ToothStateType_id, p.ToothStateType_id, 1)
				';
			} else {
				$from_clause .= '
				left join v_ToothStateValues v on v.Tooth_id = ToothAll.Tooth_id
					and v.ToothStateType_id = coalesce(tm.ToothStateType_id, 1)
				';
			}
			$from_clause .= '
				left join v_ToothStateType s on s.ToothStateType_id = v.ToothStateType_id';
		...
		$query = "
			...
			select
				...
				ToothAll.Tooth_id as \"Tooth_id\",
				ToothAll.Tooth_Code \"Tooth_Code\",
				coalesce(s.ToothStateType_id, 0) as \"ToothStateType_id\",-- === ToothStateType_Code
				coalesce(s.ToothStateType_Code::varchar, '') as \"ToothStateType_Code\",
				coalesce(s.ToothStateType_Nick,' ') as \"ToothStateType_Nick\",
				coalesce(v.ToothStateValues_Values,0) as \"ToothState_Value\"
			from {$from_clause}
			where {$where_clause}
			order by
	...
	protected function _setRemoveListByEvn($id, $sysnick)
	{
		...
		switch ($sysnick) {
			case 'EvnUslugaStom':
				$query = "
					select Parodontogram_id as \"Parodontogram_id\" from v_Parodontogram
					where EvnUslugaStom_id = :id
				";
				break;
			case 'EvnVizitPLStom':
				$query = "
					select p.Parodontogram_id as \"Parodontogram_id\" from v_EvnUslugaStom e
					inner join v_Parodontogram p  on p.EvnUslugaStom_id = e.EvnUslugaStom_id
	...
	protected function _destroy($data)
	{
        ...
        $queryParams = array(
            'Parodontogram_id' => $data['Parodontogram_id']
        );
	...
	protected function _save($data = array())
	{
		...
		$query = "
			select 
				parodontogram_id as \"Parodontogram_id\",
				error_code as \"Error_Code\", 
            			error_message as \"Error_Msg\"
            		from p_Parodontogram_ins(
            			parodontogram_id := :Parodontogram_id,
