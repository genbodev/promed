...
class Penza_Registry_model extends Registry_model {
	...
	function loadRegistryErrorTFOMS($data)
	{
		...
		if ($this->RegistryType_id == 6) {
			...
			from
				...
				left join {$this->scheme}.v_RegistryDataCmp RD  on RD.Registry_id = RE.Registry_id and RD.CmpCloseCard_id = RE.Evn_id
				left join v_CmpCloseCard CmpCloseCard  on CmpCloseCard.CmpCloseCard_id = RD.CmpCloseCard_id
			...
			from
				...
				LEFT JOIN LATERAL(
					select Person_Fio from v_MedPersonal  where MedPersonal_id = COALESCE(ES.MedPersonal_id, evpl.MedPersonal_id)
                    limit 1
				) as MP ON true
	...
	function loadRegistryErrorTFOMSFilter($data)
	{
		...
		if($filter_mode['type'] == 'unicFilter')
		{
			...
			$iLIKE = ($filter_mode['specific'] === false) ? "" : " and ".$orderBy." iLIKE  :Value";
		...
		$result = $this->db->query($this->_getLimitSQLPH($query, $distinct, $field, $data['start'], $data['limit'], $iLIKE, $orderBy), $params);
	...
	public function loadRegistryDataForXmlUsing2018($data, &$Registry_EvnNum, $xml_file, $file_re_data_name, $file_re_pers_data_name, $error_log = '') {
		...
		$query = "SELECT {$p_sl} (Registry_id := :Registry_id)";
		$this->textlog->add('Запуск ' . getDebugSQL($query, array('Registry_id' => $data['Registry_id'])));
	...
	function loadRegistry($data) 
	{
		...
		if ((isset($data['RegistryStatus_id'])) && ($data['RegistryStatus_id']==5))
		{
			...
			Select 
				...
				R.DispClass_id as \"DispClass_id\",
				R.Registry_IsAddAcc as \"Registry_IsAddAcc\",
	...
	function reformRegistry($data) 
	{
		$query = "
			select
				...
				Registry_IsAddAcc as \"Registry_IsAddAcc\",
	...
	function checkErrorDataInRegistry($data) {
		...
		$query = "Select rd.Registry_id  as \"Registry_id\" from {$this->scheme}.v_{$this->RegistryDataObject} rd	where rd.Registry_id = :Registry_id  and (rd.Evn_id = :IDCASE OR :IDCASE IS NULL) and (rd.Person_id = :ID_PERS OR :ID_PERS IS NULL)";
	...
	function getRegistryDataForImport($data) {
		$resp = $this->queryResult("
			select
				...
				to_char(rd.Evn_setDate, 'YYYY-MM-DD') as \"Evn_setDate\",
			...
			from
				{$this->scheme}.v_RegistryGroupLink rgl 
				inner join {$this->scheme}.v_RegistryDataPar rd  on rd.Registry_id = rgl.Registry_id
				left join v_Diag d  on d.Diag_id = rd.Diag_id
			where
				rgl.Registry_pid = :Registry_id
				and COALESCE(rd.RegistryData_deleted, 1) = 1
			...
			where
				rgl.Registry_pid = :Registry_id
				and COALESCE(rd.RegistryDataCmp_deleted, 1) = 1
		", array(
			'Registry_id' => $data['Registry_id']
		));
		...
		if (!empty($data['IDCASE'])) {
			...
			$resp = $this->queryResult("
				...
				from
					{$this->scheme}.v_RegistryGroupLink rgl 
					inner join {$this->scheme}.RegistryDataCmp rd  on rd.Registry_id = rgl.Registry_id
					inner join v_PersonEvn pe  on pe.Person_id = rd.Person_id
					left join v_Diag d  on d.Diag_id = rd.Diag_id
				where
					rgl.Registry_pid = :Registry_id
			...
			if ($data['Registry_id']>0)
			{
				...
				if (is_object($resp))
				{
					...
					if (is_array($ret) && (count($ret) > 0)) {
						...
						$query = "
							Insert into {$this->scheme}.v_{$this->RegistryErrorObject} (Registry_id, Evn_id, RegistryErrorType_id, LpuSection_id, pmUser_insID, pmUser_updID, RegistryError_insDT, RegistryError_updDT)
							Select 
	...
	public function setErrorImportRegistry($data) {
		...
		$query = "
			Insert into {$this->scheme}.RegistryErrorTFOMS (Registry_id, Evn_id, RegistryErrorType_id, RegistryErrorType_Code, RegistryErrorTFOMS_Comment, RegistryErrorClass_id, pmUser_insID, pmUser_updID, RegistryErrorTFOMS_insDT, RegistryErrorTFOMS_updDT)
			Select
	...
	public function setManualFLKError($data) {
		...
		$query = "
			insert into {$this->scheme}.RegistryErrorTFOMS (Registry_id, Evn_id, RegistryErrorType_id, RegistryErrorType_Code, RegistryErrorTFOMS_Comment,
	...
	public function saveUnionRegistry($data)
	{
		...
		$registryFileNumCheck = $this->getFirstResultFromQuery("
			...
			where
				...
				and date_part('year', Registry_endDate) = date_part('year', CAST(:Registry_endDate as date))
				and date_part('month', Registry_endDate) = date_part('month', CAST(:Registry_endDate as date))
				and Registry_id != COALESCE(CAST(:Registry_id as bigint), 0)
			limit 1
		", $data, true);
		...
		if (is_object($result)) 
		{
			...
			if (count($resp) > 0 && !empty($resp[0]['Registry_id'])) {
				...
				$paytypefilter = " and COALESCE(PT.PayType_SysNick, 'oms') = 'oms'";
				...
				switch ($data['RegistryGroupType_id']) {
					...
					case 4:
						if ( $data['Registry_begDate'] >= $dateX20180501 ) {
							...
							$registrytypefilter = " and R.RegistryType_id IN (7) and R.DispClass_id = 2 and COALESCE(R.Registry_IsOnceInTwoYears, 1) = 1";
						}
					break;
					case 5:
				...
				$query = "
					...
					where
						...
						and COALESCE(R.Registry_IsAddAcc, 1) = :Registry_IsAddAcc
	...
	function getRegistryDataGroupForDelete($data)
	{
		...
		$query = "
			select Evn_rid as \"Evn_rid\", EvnMax_id as \"EvnMax_id\"
			from {$this->scheme}.v_{$this->RegistryDataObject} RD 
			where RD.Registry_id = :Registry_id and RD.Evn_id = :Evn_id
		";
		$resp = $this->queryResult($query, $params);
	...
	public function loadRegistryEntiesSettings($data)
	{
		...
		$query = "
			...
			WHERE 
				...
				AND FLKSettings_EvnData iLIKE '%penza%'
			limit 1
		".$where;
		$result = $this->db->query($query, $params);
		if (is_object($result))
