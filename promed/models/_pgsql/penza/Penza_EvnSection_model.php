...
class Penza_EvnSection_model extends EvnSection_model {
	...
	function loadKSGKPGKOEFCombo2019($data) {
		...
		if (!empty($data['EvnSection_id'])) {
			...
			$query = "
				select pt.PolyTrauma_Code as \"PolyTrauma_Code\",
					   SOPUT.EvnDiagPS_id as \"EvnDiagPS_id\"
				from v_PolyTrauma pt
						 LEFT JOIN LATERAL
					(
					select edps2.EvnDiagPS_id
					from v_EvnDiagPS edps2
							 inner join v_PolyTrauma pt2 on pt2.Diag_id = edps2.Diag_id
					where edps2.DiagSetClass_id IN (2, 3)
					  and edps2.EvnDiagPS_pid = :EvnSection_id
					  and pt2.PolyTrauma_Code <> pt.PolyTrauma_Code
					  and pt2.PolyTrauma_begDT <= CAST(:EvnSection_disDate as date)
					  and (COALESCE(pt2.PolyTrauma_endDT, CAST(:EvnSection_disDate as date)) >= CAST(:EvnSection_disDate as date))
					limit 1
					) SOPUT ON true
				where pt.Diag_id = :Diag_id
				  and pt.PolyTrauma_begDT <= CAST(:EvnSection_disDate as date)
				  and (COALESCE(pt.PolyTrauma_endDT, CAST(:EvnSection_disDate as date)) >= CAST(:EvnSection_disDate as date))
				  and exists(
						select edps.EvnDiagPS_id
						...
						where edps.DiagSetClass_id IN (2, 3)
						  ...
						  and d.Diag_Code IN
							  ('J94.2', 'J94.8', 'J94.9', 'J93', 'J93.0', 'J93.1', 'J93.8', 'J93.9', 'J96.0', 'N17', 'N17.0', 'N17.1',
							   'N17.2', 'N17.8', 'N17.9', 'T79.4', 'R57.1', 'R57.8')
						limit 1
					)
				limit 1
			";
			...
			if (is_object($result)) {
				...
				if (count($resp) > 0) {
					if($resp[0]['PolyTrauma_Code'] == 7 || !empty($resp[0]['EvnDiagPS_id'])) {
						$query = "
							select mo.Mes_Code as \"Mes_Code\",
							...
							from v_MesOld mo
									 ...
									 LEFT JOIN LATERAL
								(
								select mo2.Mes_id as Mes_kid,
									   mo2.Mes_Code || COALESCE('. ' || mo2.Mes_Name, '') as KPG
								from v_MesOld mo2
										 inner join v_MesLink ml on ml.Mes_sid = mo2.Mes_id and ml.MesLinkType_id = 1
								where ml.Mes_id = mo.Mes_id
								  and mo2.Mes_begDT <= :EvnSection_disDate
								  and (COALESCE(mo2.Mes_endDT, CAST(:EvnSection_disDate as date)) >= CAST(:EvnSection_disDate as date))
								limit 1
								) mokpg ON true
							where mo.Mes_Code = '248'
							  and mo.Mes_begDT <= :EvnSection_disDate
							  and (COALESCE(mo.Mes_endDT, CAST(:EvnSection_disDate as date)) >= CAST(:EvnSection_disDate as date))
							  and mt.MesTariff_begDT <= :EvnSection_disDate
							  and (COALESCE(mt.MesTariff_endDT, CAST(:EvnSection_disDate as date)) >= CAST(:EvnSection_disDate as date))
							limit 1
						";
		...
		if (!$KSGFromPolyTrauma) {
			...
			if (!empty($data['EvnSectionIds']) && !empty($data['UslugaComplexIds'])) {
				...
				$fractTempTable="
					with penza_evnsection_model_fraction as (
						select
			...
			if (!empty($fractTempTable)) {
				$FractionJoin = "left join penza_evnsection_model_fraction fr on fr.UslugaComplex_id = mu.UslugaComplex_id";
				$FractionFilter = "
			...
			$resp_ur = $this->queryResult("
				select cast(av.AttributeValue_ValueFloat as int) as \"LpuLevel\"
				from v_AttributeVision avis
						 inner join v_AttributeValue av on av.AttributeVision_id = avis.AttributeVision_id
						 INNER JOIN LATERAL
					(
					select av2.AttributeValue_ValueString
					from v_AttributeValue av2
					...
					where av2.AttributeValue_rid = av.AttributeValue_id
					  ...
					  and COALESCE(av2.AttributeValue_ValueIdent, CAST(:Lpu_id as bigint)
							  ) = :Lpu_id
					limit 1
					) MOFILTER ON true
				where avis.AttributeVision_TableName = 'dbo.TariffClass'
				  and avis.AttributeVision_TablePKey =
					  (
						  select TariffClass_id
						  from v_TariffClass
						  where TariffClass_SysNick = 'level'
						  limit 1
					  )
				  and avis.AttributeVision_IsKeyValue = 2
				  and COALESCE(av.AttributeValue_begDate, CAST(:EvnSection_disDate as date)) <= CAST(:EvnSection_disDate as date)
				  and COALESCE(av.AttributeValue_endDate, CAST(:EvnSection_disDate as date)) >= CAST(:EvnSection_disDate as date)
				limit 1
			", array(
				'EvnSection_disDate' => $data['EvnSection_disDate'],
				'Lpu_id' => $data['Lpu_id']
				...
				$query = "
					...
					from v_UslugaComplex uc 
						...
						LEFT JOIN LATERAL(
							...
							where
								ml.Mes_id = mo.Mes_id
								and mo2.Mes_begDT <= CAST(:EvnSection_disDate as date) and (COALESCE(mo2.Mes_endDT, CAST(:EvnSection_disDate as date))>= CAST(:EvnSection_disDate as date)
                                )
                                limit 1
						) mokpg ON true
						left join v_MesTariff mt  on mt.Mes_id = mo.Mes_id -- Коэффициент КСГ
					...
					where
						...
						and (COALESCE(mu.MesOldUslugaComplex_endDT, CAST(:EvnSection_disDate as date)) >= CAST(:EvnSection_disDate as date))
						and mo.Mes_begDT <= :EvnSection_disDate
						and (COALESCE(mo.Mes_endDT, CAST(:EvnSection_disDate as date))>= CAST(:EvnSection_disDate as date))
						and mt.MesTariff_begDT <= :EvnSection_disDate
						and (COALESCE(mt.MesTariff_endDT, CAST(:EvnSection_disDate as date))>= CAST(:EvnSection_disDate as date))
						and COALESCE(mo.MesType_id, CAST(:MesType_id as bigint)) = :MesType_id
						and COALESCE(mt.MesPayType_id, CAST(:MesPayType_id as bigint)) = :MesPayType_id
						and COALESCE(mu.LpuSectionProfile_id, CAST(:LpuSectionProfile_id as bigint)) = :LpuSectionProfile_id
						and COALESCE(mu.MesOldUslugaComplex_LpuLevel, CAST(:LpuLevel as bigint)) = :LpuLevel
				";
			...
			if (!empty($KSGList)) {
				...
				$resp_ml = $this->queryResult("
					...
					where
						...
						COALESCE(MesLink_endDT, CAST(:EvnSection_disDate as date)) >= CAST(:EvnSection_disDate as date) and
						MesLinkType_id = 2
				", array(
					'EvnSection_disDate' => $data['EvnSection_disDate']
				));
	...
	function loadKSGKPGKOEFCombo2018($data) {
		...
		if (!empty($data['EvnSection_id'])) {
			...
			$query = "
				...
				where pt.Diag_id = :Diag_id
				  ...
				  and (COALESCE(pt.PolyTrauma_endDT, CAST(:EvnSection_disDate as date)) >= :EvnSection_disDate)
				  and exists(
						select edps.EvnDiagPS_id
						from v_EvnDiagPS edps
								 inner join v_Diag d on d.Diag_id = edps.Diag_id
					...
					if($resp[0]['PolyTrauma_Code'] == 7 || !empty($resp[0]['EvnDiagPS_id'])) {
						$query = "
							select mo.Mes_Code as \"Mes_Code\",
								   ...
								   mt.MesTariff_Value as \"MesTariff_Value\",
								   mt.MesTariff_id as \"MesTariff_id\",
								   mokpg.MesKpg_Code as \"MesKpg_Code\",
								   mokpg.Mes_kid as \"Mes_kid\"
							from v_MesOld mo
									 left join v_MesTariff mt on mt.Mes_id = mo.Mes_id -- Коэффициент КСГ
									 LEFT JOIN LATERAL
								...
								where ml.Mes_id = mo.Mes_id
								  and mo2.Mes_begDT <= :EvnSection_disDate
								  and (COALESCE(mo2.Mes_endDT, CAST(:EvnSection_disDate as date)) >= :EvnSection_disDate)
								limit 1
								) mokpg ON true
							where mo.Mes_Code = '233'
							  and mo.Mes_begDT <= :EvnSection_disDate
							  and (COALESCE(mo.Mes_endDT, CAST(:EvnSection_disDate as date)) >= :EvnSection_disDate)
							  and mt.MesTariff_begDT <= :EvnSection_disDate
							  and (COALESCE(mt.MesTariff_endDT, CAST(:EvnSection_disDate as date)) >= :EvnSection_disDate)
		...
		$resp_ur = $this->queryResult("
			...
			from v_AttributeVision avis
				...
				where av2.AttributeValue_rid = av.AttributeValue_id
				  and a2.Attribute_TableName = 'dbo.Lpu'
				  and COALESCE(av2.AttributeValue_ValueIdent, CAST(:Lpu_id as bigint)) = :Lpu_id
				limit 1
				) MOFILTER ON true
			where avis.AttributeVision_TableName = 'dbo.TariffClass'
			  and avis.AttributeVision_TablePKey =
		...
		if (!$KSGOper && !$KSGTerr && !$KSGFromPolyTrauma) {
			...
			if (!empty($data['EvnSection_id'])) {
				...
				$data['LpuSectionProfile_id'] = $this->getFirstResultFromQuery("
					SELECT case
							   when ESNBLSP.LpuSectionProfile_id IS NOT NULL and LSP.LpuSectionProfile_Code IN ('1035', '2035', '3035')
								   then ESNBLSP.LpuSectionProfile_id
							   else lsp.LpuSectionProfile_id
					...
					FROM v_EvnSection es
							 ...
							 LEFT JOIN LATERAL
						(
						select esnbls.LpuSectionProfile_id
						...
						order by esnb.EvnSectionNarrowBed_setDate desc
						limit 1
						) ESNBLSP ON true
			...
			if (!empty($data['LpuSectionProfile_id'])) {
				$query = "
					select mokpg.Mes_Code           as \"MesKpg_Code\",
					...
					where mokpg.LpuSectionProfile_id = :LpuSectionProfile_id
					  and mokpg.MesType_id IN (4) -- КПГ
					  and mokpg.Mes_begDT <= :EvnSection_disDate
					  and (COALESCE(mokpg.Mes_endDT, CAST(:EvnSection_disDate as date)) >= :EvnSection_disDate)
					  and mtkpg.MesTariff_begDT <= :EvnSection_disDate
					  and (COALESCE(mtkpg.MesTariff_endDT, CAST(:EvnSection_disDate as date)) >= :EvnSection_disDate)
					  and COALESCE(mtkpg.MesPayType_id, CAST(:MesPayType_id as bigint)) = :MesPayType_id
					order by mtkpg.MesTariff_Value desc
					limit 1
				";
		...
		if ($KSGOper && $KSGTerr) {
			...
			$data['MesLink_id'] = $this->getFirstResultFromQuery("
				...
				where
					...
					Mes_sid = :Mes_sid and
					MesLink_begDT <= :EvnSection_disDate and
					COALESCE(MesLink_endDT, CAST(:EvnSection_disDate as date)) >= :EvnSection_disDate and
					MesLinkType_id = 2
				limit 1
			", array(
			...
			if (in_array($data['LpuUnitType_id'], array('6','7','9'))) {
				...
				$resp = $this->queryResult("
                    ...
                    FROM v_AttributeVision avis
                         ...
                         (
                           ...
                           from v_AttributeValue av2
                                inner join v_Attribute a2 on a2.Attribute_id = av2.Attribute_id
                           where av2.AttributeValue_rid = av.AttributeValue_id and
                                 a2.Attribute_TableName = 'dbo.LpuSectionProfile' and
                                 av2.AttributeValue_ValueIdent = :LpuSectionProfile_id
                           limit 1
                         ...
                         (
                           ...
                           from v_AttributeValue av2
                                inner join v_Attribute a2 on a2.Attribute_id = av2.Attribute_id
                           where av2.AttributeValue_rid = av.AttributeValue_id and
                                 a2.Attribute_TableName = 'dbo.Diag' and
                                 av2.AttributeValue_ValueIdent = :Diag_id
                                 limit 1
                         ) DIAGFILTER ON true
                    WHERE avis.AttributeVision_TableName = 'dbo.VolumeType' and
                          av.AttributeValue_ValueIdent = :Mes_id and
                          avis.AttributeVision_TablePKey =
                          (
                            select VolumeType_id
                            from v_VolumeType
                            where VolumeType_Code = '2017КСГ-Запрет'
                            limit 1
                          ) and
                          avis.AttributeVision_IsKeyValue = 2 and
                          COALESCE(av.AttributeValue_begDate, CAST(:EvnSection_disDate as date)) <= :EvnSection_disDate and
                          COALESCE(av.AttributeValue_endDate, CAST(:EvnSection_disDate as date)) >= :EvnSection_disDate
                    LIMIT 1
				", array(
					'LpuSectionProfile_id' => $data['LpuSectionProfile_id'],
				...
				$resp = $this->queryResult("
					...
					WHERE avis.AttributeVision_TableName = 'dbo.VolumeType'
					  ...
					  and avis.AttributeVision_TablePKey =
						  (select VolumeType_id from v_VolumeType where VolumeType_Code = '2017КСГ-СтрогУсл' limit 1)
					  and avis.AttributeVision_IsKeyValue = 2
					  and COALESCE(av.AttributeValue_begDate, CAST(:EvnSection_disDate as date)) <= :EvnSection_disDate
					  and COALESCE(av.AttributeValue_endDate, CAST(:EvnSection_disDate as date)) >= :EvnSection_disDate
				", array(
			...
			$query = "
				...
				from v_PolyTrauma pt
					...
					where edps2.DiagSetClass_id IN (2, 3)
					  ...
					  and pt2.PolyTrauma_Code <> pt.PolyTrauma_Code
					  and pt2.PolyTrauma_begDT <= :EvnSection_disDate
					  and (COALESCE(pt2.PolyTrauma_endDT, CAST(:EvnSection_disDate as date)) >= :EvnSection_disDate)
					limit 1
					) SOPUT ON true
				where pt.Diag_id = :Diag_id
				  and pt.PolyTrauma_begDT <= :EvnSection_disDate
		...
		if (!empty($data['Diag_id'])) {
			$query = "
				select 
					d.Diag_Code as \"Diag_Code\",
					mu.MesOldUslugaComplex_id as \"MesOldUslugaComplex_id\",
					mo.Mes_Code || COALESCE('. ' || mo.Mes_Name, '') as \"Mes_Name\",
					mo.Mes_id as \"Mes_id\",
					mt.MesTariff_Value as \"MesTariff_Value\",
					mt.MesTariff_id as \"MesTariff_id\",
					mokpg.MesKpg_Code as \"MesKpg_Code\",
				...
				where
					mu.Diag_id = :Diag_id
					and (mu.UslugaComplex_id is null OR mu.UslugaComplex_id IN (select eu.UslugaComplex_id from v_EvnUsluga eu  where eu.EvnUsluga_pid = :EvnSection_id and eu.EvnClass_id in (43,22,29) and eu.PayType_id IN (".implode(',', $this->PayType_ids).")))
					and (mu.Diag_nid IS NULL OR mu.Diag_nid IN (select Diag_id from v_EvnDiagPS  where DiagSetClass_id IN (2,3) and EvnDiagPS_pid = :EvnSection_id))
					and (mu.Sex_id = :Sex_id OR Sex_id IS NULL)
					and (
						(:Person_Age >= 18 and mu.MesAgeGroup_id = 1)
		...
		if (!$KSGOper && !$KSGTerr && !$KSGFromPolyTrauma) {
			...
			if (!empty($data['LpuSectionProfile_id'])) {
				$query = "
				    ...
				    from MesOld mokpg
				           left join v_MesTariff mtkpg on mtkpg.Mes_id = mokpg.Mes_id -- Коэффициент КПГ
				    where mokpg.LpuSectionProfile_id = :LpuSectionProfile_id and
				            mokpg.MesType_id IN (4) -- КПГ
				            and mokpg.Mes_begDT <= :EvnSection_disDate and
				            (COALESCE(mokpg.Mes_endDT, CAST(:EvnSection_disDate as date)) >= :EvnSection_disDate) and
				            mtkpg.MesTariff_begDT <= :EvnSection_disDate and
				            (COALESCE(mtkpg.MesTariff_endDT, CAST(:EvnSection_disDate as date)) >= :EvnSection_disDate) and
				            COALESCE(mtkpg.MesPayType_id, CAST(:MesPayType_id as bigint)) = :MesPayType_id
				    order by mtkpg.MesTariff_Value desc
				    limit 1
				";
					...
					WHERE avis.AttributeVision_TableName = 'dbo.VolumeType'
					  ...
					  and avis.AttributeVision_TablePKey =
						  (
							  select VolumeType_id
							  from v_VolumeType
							  where VolumeType_Code = '2017КСГ-СтрогУсл'
						  )
					  and avis.AttributeVision_IsKeyValue = 2
					  and COALESCE(av.AttributeValue_begDate, CAST(:EvnSection_disDate as date)
							  ) <= :EvnSection_disDate
					  and COALESCE(av.AttributeValue_endDate, CAST(:EvnSection_disDate as date)
							  ) >= :EvnSection_disDate
				", array(
					'LpuSectionProfile_id' => $data['LpuSectionProfile_id'],
					'Mes_id' => $maxKSG['Mes_id'],
		...
		if (!empty($data['EvnSection_id'])) {
			...
			if (is_object($result)) {
				...
				if (count($resp) > 0) {
					if($resp[0]['PolyTrauma_Code'] == 7 || !empty($resp[0]['EvnDiagPS_id'])) {
						$query = "
							select mo.Mes_Code || COALESCE('. ' || mo.Mes_Name, '') as \"Mes_Name\",
								   mo.Mes_id as \"Mes_id\",
								   mt.MesTariff_Value as \"MesTariff_Value\",
								   mt.MesTariff_id as \"MesTariff_id\",
								   mokpg.MesKpg_Code as \"MesKpg_Code\",
							...
							from v_MesOld mo
								...
								where ml.Mes_id = mo.Mes_id
								  and mo2.Mes_begDT <= :EvnSection_disDate
								  and (COALESCE(mo2.Mes_endDT, CAST(:EvnSection_disDate as date)
										   ) >= :EvnSection_disDate
									)
								limit 1
								) mokpg ON true
							where mo.Mes_Code = '216'
							  and mo.Mes_begDT <= :EvnSection_disDate
							  and (COALESCE(mo.Mes_endDT, CAST(:EvnSection_disDate as date)
									   ) >= :EvnSection_disDate
								)
							  and mt.MesTariff_begDT <= :EvnSection_disDate
							  and (COALESCE(mt.MesTariff_endDT, CAST(:EvnSection_disDate as date)
									   ) >= :EvnSection_disDate
								)
							limit 1
						";
	...
	protected function _recalcIndexNum()
	{
		...
		if (!$isManualIdxNum) {
			...
			$query = "
				...
				where
					e.Evn_pid = :EvnSection_pid and EvnSection.evn_id = es.evn_id
			";
		...
		$this->db->query("
			update
				EvnSection
			set
				EvnSection_IsWillPaid = case when es.Evn_id = es2.EvnSection_id then 2 else 1 end
			from
				EvnSection es
				inner join Evn e  on e.Evn_id = es.Evn_id
				LEFT JOIN LATERAL (
					...
					from
						v_EvnSection es2 
						left join v_MesTariff mt  on mt.MesTariff_id = es2.MesTariff_id
					where
			...
			where
				...
				and es.EvnSection_IndexNum is not null
				and EvnSection.evn_id = es.evn_id				
		", array(
			'EvnSection_pid' => $this->pid
		));
	...
	protected function _recalcIndexNum2()
	{
		...
		$query = "
			update
				EvnSection
			set
				EvnSection_IndexNum2 = null,
				EvnSection_IsWillPaid2 = null
			from
				EvnSection es
				inner join Evn e  on e.Evn_id = es.evn_id
			where
	...
	function getEvnSectionGroup2() {
		$resp_es = $this->queryResult("
			select
				...
				COALESCE(EvnSection_disDate, EvnSection_setDate) as \"date\",
		...
		if (empty($data['EvnSection_pid'])) {
			...
			$union = "
				...
				select
					...
					COALESCE (CAST(:EvnSection_disDate as date), CAST(:EvnSection_setDate as date)) as \"date\",
				...
				from
					...
					inner join v_PayType pt  on pt.PayType_id = :PayType_id
					left join v_HTMedicalCareClass htmcc  on htmcc.HTMedicalCareClass_id = :HTMedicalCareClass_id
					left join v_LpuSectionProfile lsp  on lsp.LpuSectionProfile_id = COALESCE(htmcc.LpuSectionProfile_id, CAST(:LpuSectionProfile_id as bigint))
					left join v_Diag d2  on d2.Diag_id = d.Diag_pid
					left join v_Diag d3  on d3.Diag_id = d2.Diag_pid
					left join v_Diag d4  on d4.Diag_id = d3.Diag_pid
				where
	...
	protected function calcCoeffCTP2018($data) {
		...
		$resp_codes = $this->queryResult("
			...
			from
				...
				inner join v_AttributeValue av  on av.AttributeVision_id = avis.AttributeVision_id
				left join v_MesTariff mt  on mt.MesTariff_id = :MesTariff_id
				LEFT JOIN LATERAL (
					select
			...
			where
				...
				and (CODE.AttributeValue_ValueString = '9' or exists (
					...
					from
						r58.v_KslpMesOldLink kmol 
						inner join r58.v_Kslp k  on k.Kslp_id = kmol.Kslp_id
					where
						kmol.Mes_id = mt.Mes_id
						and k.Kslp_Code = CODE.AttributeValue_ValueString
						and COALESCE(kmol.KslpMesOldLink_begDate, CAST(:EvnSection_disDate as date)) <= :EvnSection_disDate
						and COALESCE(kmol.KslpMesOldLink_endDate, CAST(:EvnSection_disDate as date)) >= :EvnSection_disDate
						and COALESCE(k.Kslp_begDate, CAST(:EvnSection_disDate as date)) <= :EvnSection_disDate
						and COALESCE(k.Kslp_endDate, CAST(:EvnSection_disDate as date)) >= :EvnSection_disDate
				))
		", array(
			'EvnSection_disDate' => $data['EvnSection_disDate'],
		...
		if (isset($KSLPCodes[$codeCTP]) && !empty($data['EvnSection_id'])) {
			$AttributeValue = $this->queryResult("
				...
				WHERE
					...
					and avis.AttributeVision_TablePKey = (select VolumeType_id from v_VolumeType  where VolumeType_Code = '2017Диаг_Патол' limit 1)
					and avis.AttributeVision_IsKeyValue = 2
					and COALESCE(av.AttributeValue_begDate, CAST(:EvnSection_disDate as date)) <= :EvnSection_disDate
					and COALESCE(av.AttributeValue_endDate, CAST(:EvnSection_disDate as date)) >= :EvnSection_disDate
			...
			if (!empty($data['EvnSection_id'])) {
				$checkKSLP8 = $this->queryResult("
					select distinct
						...
						mo1.Mes_Code as \"Mes_Code\"
					from
						v_EvnUsluga eu1 
						inner join v_MesOldUslugaComplex mouc1  on mouc1.UslugaComplex_id = eu1.UslugaComplex_id
						inner join v_MesOld mo1  on mo1.Mes_id = mouc1.Mes_id
		...
		if (isset($KSLPCodes[$codeCTP]) && !empty($data['EvnSection_id'])) {
			$queryResult = $this->queryResult("
				...
				WHERE
					avis.AttributeVision_TableName = 'dbo.VolumeType'
					and avis.AttributeVision_TablePKey = (select VolumeType_id from v_VolumeType  where VolumeType_Code = '2017Сочет_Хирург_Вмеш' limit 1)
					and avis.AttributeVision_IsKeyValue = 2
			...
			from
				...
				INNER JOIN LATERAL (
					...
					where
						...
						and COALESCE(av2.AttributeValue_ValueIdent, :LpuUnitType_id) = :LpuUnitType_id
                    limit 1
				) LUT ON true
				LEFT JOIN LATERAL (
					select
			...
			if (isset($KSLPCodes[$codeCTP]) && $data['Duration'] > 4) {
				...
				$Diag_id = $this->getFirstResultFromQuery("
					...
					where es.EvnSection_id IN ('" . implode("','", $data['EvnSectionIds']) . "')
						and d.Diag_Code between 'A00' and 'B99'
					limit 1
				");
				...
				if(!empty($Diag_id) && !empty($EvnUsluga_id)){
					$coeffCTP = $KSLPCodes[$codeCTP];
					if ($EvnSection_CoeffCTP > 0) {
						$EvnSection_CoeffCTP = $EvnSection_CoeffCTP + $coeffCTP - 1;
					} else {
	...
	protected function calcCoeffCTP($data) {
		...
		$resp_codes = $this->queryResult("
			...
			from
				v_MesTariff mt 
				inner join r58.v_KslpMesOldLink kmol  on mt.Mes_id = kmol.Mes_id
				inner join r58.v_Kslp k  on k.Kslp_id = kmol.Kslp_id
			where
	...
	protected function _recalcKSKP()
	{
		...
		foreach($resp_es as $respone) {
			...
			if (!empty($respone['EvnSection_disDT'])) {
				$datediff = strtotime($respone['EvnSection_disDT']) - strtotime($respone['EvnSection_setDT']);
				$Duration = floor($datediff/(60*60*24));
			} else {
				$datediff = 0;
				$Duration = 0;
		...
		if (!empty($data['EvnSectionIds'])) {
			$query = "
				...
				from
					...
					inner join v_UslugaCategory ucat  on ucat.UslugaCategory_id = uc.UslugaCategory_id and ucat.UslugaCategory_SysNick = 'gost2011'
				where
					cast(eu.EvnUsluga_pid as varchar) IN ('" . implode("','", $data['EvnSectionIds']) . "')
					and eu.PayType_id IN (".implode(',', $this->PayType_ids).") 
					and eu.EvnUsluga_setDT is not null
