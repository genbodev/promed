 ...
 */
abstract class EvnAbstract_model extends swPgModel
{
	/**
	...
	function getParentEvnClassSysNick()
	{
		...
		if (empty($this->pid)) {
			...
			SELECT EvnClass_id as "EvnClass_id", EvnClass_SysNick  as "EvnClass_SysNick" FROM v_Evn
	...
	function loadPersonData($isByPersonEvn = false, $params = array())
	{
		...
		return $this->getFirstRowFromQuery("
			SELECT 
				...
				Person.Person_deadDT as \"Person_deadDT\",
				Person.Person_closeDT as \"Person_closeDT\",
				...
				OmsSprTerr.OmsSprTerr_id as \"OmsSprTerr_id\",
	...
	protected function _afterSave($result)
	{
		...
		if (!empty($this->pid)) {
			...
			$this->db->query("
				update Evn set Evn_IsSigned = 1 where Evn_IsSigned = 2 and Evn_id = :Evn_id and EvnClass_id in (3, 6, 30)
			", array(
				'Evn_id' => $this->pid
			));
		}
	...
	function loadPayTypeIdBySysNick($sysNick)
	{
		$res = $this->getFirstResultFromQuery("
			...
			from v_PayType
			where PayType_SysNick ilike :PayType_SysNick
			limit 1
	...
	function rollbackStatus($params)
	{
		...
		$join_current_status = 'inner join v_EvnStatus CurrentStatus on CurrentStatus.EvnStatus_id = :EvnStatus_id';
		if (empty($params['EvnStatus_id']) && isset($params['EvnStatus_SysNick'])) {
			$join_current_status = 'inner join v_EvnStatus CurrentStatus on CurrentStatus.EvnStatus_SysNick ilike :EvnStatus_SysNick';
		}
		$query = "
		select EvnStatusHistory.EvnStatus_id as \"EvnStatus_id\", CurrentStatus.EvnStatus_id as \"CurrentStatus_id\"
			from EvnStatusHistory 
			{$join_current_status}
			inner join Evn on Evn.Evn_id = EvnStatusHistory.Evn_id and Evn.EvnStatus_id = CurrentStatus.EvnStatus_id and Evn.EvnClass_id = :EvnClass_id
			where EvnStatusHistory.Evn_id = :Evn_id 
            and EvnStatusHistory.EvnStatusHistory_begDate < Evn.Evn_statusDate
	...
	function checkIntersectEvnSectionWithVizit($data) {
		...
		if (!empty($data['EvnSection_disDT'])
			...
			$query = "
				select 
					EV.EvnVizitPL_id as \"EvnVizitPL_id\",
					to_char(EV.EvnVizitPL_setDate, 'DD.MM.YYYY') as \"EvnVizitPL_setDate\",
					L.Lpu_Nick as \"Lpu_Nick\",
					D.Diag_FullName as \"Diag_FullName\",
					D.Diag_Code as \"Diag_Code\"
				from
					v_EvnVizitPL EV 
					inner join v_LpuSection LS  on :LpuSection_id = LS.LpuSection_id
					inner join v_LpuUnit LU  on LU.LpuUnit_id = LS.LpuUnit_id
					inner join LpuUnitType LUT  on LU.LpuUnitType_id = LUT.LpuUnitType_id
					left join v_Lpu L  on L.Lpu_id = EV.Lpu_id
	...
	public function CheckEvnUslugasDate($Evn_id, $ignoreParentEvnDateCheck, $fromEmk = '') {
		if (!$ignoreParentEvnDateCheck) {
			...
			$Evn_DT = $this->getFirstRowFromQuery(' select EvnClass_SysNick as "EvnClass_SysNick", Evn_setDT as "Evn_setDT", Evn_disDT  as "Evn_disDT" from v_Evn where Evn_id = :Evn_id', array('Evn_id' => $Evn_id));
			...
			switch($Evn_DT['EvnClass_SysNick']){
				...
				case 'EvnPL':
					...
					$lastEvnVizit = $this->queryResult('select Evn_setDT  as "Evn_setDT" from v_Evn where Evn_pid = :Evn_id order by Evn_setDT desc limit 1', array('Evn_id' => $Evn_id));
					if (!empty($lastEvnVizit[0]['Evn_setDT'])) {
						$Evn_DT['Evn_disDT'] = $lastEvnVizit[0]['Evn_setDT'];
					} else {
						$Evn_DT['Evn_disDT'] = date("Y-m-d");
					}
			...
			$query = "
				select
					...
					EvnUsluga_disDT as \"EvnUsluga_disDT\"
				from
					v_EvnUsluga EU 
