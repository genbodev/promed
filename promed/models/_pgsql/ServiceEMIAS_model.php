...
class ServiceEMIAS_model extends SwPgModel {
	...
	function getEvnReceptSerialNum($data) {
		...
		$query = "
			select
				...
				LP.LpuPeriodDLO_Code as \"LpuPeriodDLO_Code\",
				L.Lpu_Ouz as \"Lpu_Ouz\"
			from v_LpuSection LS
				left join v_LpuPeriodDLO LP on
					LP.LpuUnit_id = LS.LpuUnit_id
					and (cast(dbo.tzGetDate() as date) between LP.LpuPeriodDLO_begDate
						and COALESCE(LP.LpuPeriodDLO_endDate, cast(dbo.tzGetDate() as date))
					)
		...
		$query = "
			...
			from
				r50.MedPersonalDLOPeriod MPDP
				left join r50.MedstaffFactDLOPeriodLink MFDPL on
					MFDPL.MedPersonalDLOPeriod_id = MPDP.MedPersonalDLOPeriod_id
					and ( cast(dbo.tzGetDate() as date) between MFDPL.MedstaffFactDLOPeriodLink_begDate and COALESCE(MFDPL.MedstaffFactDLOPeriodLink_endDate,cast(dbo.tzGetDate() as date)) )
				left join v_MedStaffFact MSF on MSF.MedStaffFact_id = MFDPL.MedStaffFact_id
			where
				MSF.Lpu_id = :Lpu_id
		...
		$query = "
			select
				...
				ReceptFreeNum_insDT as \"ReceptFreeNum_insDT\",
				ReceptFreeNum_updDT as \"ReceptFreeNum_updDT\",
				MCOD as \"MCOD\"
			...
			order by
				ReceptFreeNum_updDT desc,
				ReceptFreeNum_insDT desc
			limit 1
		";
	...
	function getRemains($data) {
		...
		$PrivilegeCategoryId = $this->getFirstResultFromQuery("
			select PrivilegeType_Code as \"PrivilegeType_Code\"
		...
		$FundingSourceId = $this->getFirstResultFromQuery("
			select FundingSource_id as \"FundingSource_id\"
			from r50.FundingSourceLink
		...
		if (!empty($data['Drug_id'])) {
			$PrivilegedDrugId = $this->getFirstResultFromQuery("
				select DrugNomen_Code as \"DrugNomen_Code\"
		...
		foreach ($resp as $response) {
			...
			$org = $this->getFirstRowFromQuery("
				...
				from v_Org o 
				left join v_OrgFarmacy orf on o.Org_id = orf.Org_id
			...
			if ($OrgFarmacyIndex_id == false) {
				...
				$org_farmacy_index = $this->getFirstResultFromQuery("
					select
						coalesce(max(OrgFarmacyIndex_Index), 0) + 1 as \"max_index\"
			...
			$sql = "
				select
					...
					Error_Code as \"Error_Code\",
					Error_Message as \"Error_Msg\"
				from dbo.p_DrugOstatRegistry_{$proc}(
					...
					DrugOstatRegistry_ImportDate := dbo.tzGetDate(),
					pmUser_id := :pmUser_id
				)
			";
	...
	function syncPersonPrivilege($data, $log) {
		...
		$resp_pp = $this->queryResult("
			select
				...
				sppr.SPOULOPrivilege_SPOULOId as \"SPOULOPrivilege_SPOULOId\",
				spp.SPOULOPerson_SPOULOId as \"SPOULOPerson_SPOULOId\",
				...
				coalesce(ps.Person_EdNum, ps.Polis_Num) as \"Polis_Num\"
			from
				v_PersonPrivilege pp
				inner join r50.v_SPOULOPerson spp on pp.Person_id = spp.Person_id
				...
				left join r50.v_SPOULOPrivilege sppr on sppr.PersonPrivilege_id = pp.PersonPrivilege_id
			where
		...
		$resp_pp = $this->queryResult("
			...
			from
				...
				inner join r50.v_SPOULOPerson spp on pp.Person_id = spp.Person_id
				inner join v_PrivilegeType pt on pt.PrivilegeType_id = pp.PrivilegeType_id
				inner join r50.v_SPOULOPrivilege sppr on sppr.PersonPrivilege_id = pp.PersonPrivilege_id
				inner join v_PersonState ps on ps.Person_id = pp.Person_id
			where
	...
	function syncEvnRecept($data, $log) {
		...
		$resp_er = $this->queryResult("
			with erf as (
				...
				from
					...
					left join r50.v_SPOULORecept spr  on er.EvnRecept_id = spr.EvnRecept_id
				where
				...
				from
					v_EvnRecept er 
					inner join v_Lpu l  on l.Lpu_id = er.Lpu_id
					inner join r50.v_SPOULOPerson spp  on er.Person_id = spp.Person_id
					inner join r50.v_SPOULOPrivilege sppr  on sppr.PersonPrivilege_id = er.PersonPrivilege_id
					left join r50.v_SPOULORecept spr  on er.EvnRecept_id = spr.EvnRecept_id
				where
					er.EvnRecept_IsPrinted = 2
					and	spr.SPOULORecept_id is null -- новый
			...
			select
				erf.EvnRecept_id as \"EvnRecept_id\",
				erf.SPOULOPerson_SPOULOId as \"SPOULOPerson_SPOULOId\",
				erf.SPOULOPrivilege_SPOULOId as \"SPOULOPrivilege_SPOULOId\",
				...
				COALESCE(ua.KLRgn_id, pa.KLRgn_id) as \"KLRgn_id\",
				case
				...
				COALESCE(lpd.LpuPeriodDLO_Code, erf.Lpu_Ouz) as \"LpuPeriodDLO_Code\",
				erf.Lpu_OGRN as \"Lpu_OGRN\",
				...
				to_char(er.EvnRecept_VKProtocolDT, 'YYYY-MM-DD') as \"EvnRecept_VKProtocolDT\",
				...
				to_char(dp.DocumentPrivilege_begDate, 'YYYY-MM-DD') as \"DocumentPrivilege_begDate\",
				to_char(pp.PersonPrivilege_begDate, 'YYYY-MM-DD') as \"PersonPrivilege_begDate\",
				to_char(pp.PersonPrivilege_endDate, 'YYYY-MM-DD') as \"PersonPrivilege_endDate\"
			from
				erf
				inner join v_EvnRecept er  on er.EvnRecept_id = erf.EvnRecept_id
				...
				left join v_PersonAmbulatCard pac  on pac.PersonAmbulatCard_id = er.PersonAmbulatCard_id
				left join v_Address ua on ua.Address_id = ps.UAddress_id
				left join v_Address pa on pa.Address_id = ps.PAddress_id
				left join v_KLAreaStat ukas on (
				...
				left join v_KLAreaStat pkas on (
				...
				LEFT JOIN LATERAL (
					...
					where
						...
						and COALESCE(LpuPeriodDLO_begDate, er.EvnRecept_setDate) <= er.EvnRecept_setDate
						and COALESCE(LpuPeriodDLO_endDate, er.EvnRecept_setDate) >= er.EvnRecept_setDate
                    limit 1
				) lpd ON true
				LEFT JOIN LATERAL (
					select 
					...
					from
						...
						inner join r50.SPOULODrug sud with on
		...
		$resp_er = $this->queryResult("
			select
				...
				er.EvnRecept_Num as \"EvnRecept_Num\",
				rrct.ReceptRemoveCauseType_Code as \"ReceptRemoveCauseType_Code\",
				ps.Person_SurName as \"Person_SurName\",
				ps.Person_FirName as \"Person_FirName\",
				ps.Person_SecName as \"Person_SecName\",
				to_char(ps.Person_BirthDay, 'yyyy-mm-dd') as \"Person_BirthDay\",
			...
			from
				EvnRecept er
				inner join Evn e on e.Evn_id = er.Evn_id
				inner join r50.v_SPOULOPerson spp on e.Person_id = spp.Person_id
				inner join r50.v_SPOULORecept spr on er.EvnRecept_id = spr.EvnRecept_id
	...
	function syncPerson($data, $log) {
		...
		$resp_ps = $this->queryResult("
			select
				...
				coalesce(l.Lpu_Ouz, lpd.LpuPeriodDLO_Code) as \"Lpu_Ouz\",
			...
			from
				...
				inner join v_Address ua on ua.Address_id = coalesce(ps.UAddress_id, ps.PAddress_id)
				inner join r50.v_AreaLink al on al.KLArea_id = coalesce(ua.KLCity_id, ua.KLSubRgn_id)
				inner join r50.v_Area a on a.Area_id = al.Area_id
				left join v_Document d on d.Document_id = ps.Document_id
				left join v_OrgDep od on od.OrgDep_id = d.OrgDep_id
				left join r50.v_SPOULOPerson spp on ps.Person_id = spp.Person_id
				left join r50.v_DocumentTypeLinkMsk dtlm on dtlm.DocumentType_id = d.DocumentType_id
				left join r50.v_DocumentTypeMsk dtm on dtm.DocumentType_id = dtlm.DocumentTypeMsk_id
				left join lateral(
					select
						lpd.LpuPeriodDLO_Code
					from
						v_LpuPeriodDLO lpd
						inner join v_LpuUnit lu on lu.LpuUnit_id = lpd.LpuUnit_id
					where
						lu.Lpu_id = ps.Lpu_id
						and coalesce(LpuPeriodDLO_begDate, dbo.tzGetDate()) <= dbo.tzGetDate()
						and coalesce(LpuPeriodDLO_endDate, dbo.tzGetDate()) >= dbo.tzGetDate()
					limit 1
				) lpd on true
			where
		...
		$resp_ps = $this->queryResult("
			select
				...
				ps.PersonSnils_Snils as \"PersonSnils_Snils\",
				to_char(ps.PersonBirthDay_BirthDay, 'yyyy-mm-dd') as \"Person_BirthDay\",
				case when ps.PolisType_id = 4 then '' else ps.Polis_Ser end as \"Polis_Ser\",
				coalesce(ps.PersonPolisEdNum_EdNum, ps.Polis_Num) as \"Polis_Num\"
			from
				Person p
				inner join PersonState ps on ps.Person_id = p.Person_id
				inner join r50.v_SPOULOPerson spp on p.Person_id = spp.Person_id
			where
				p.Person_deleted = 2
				and spp.SPOULOPerson_SPOULOId is not null
	...
	function saveSPOULOPerson($data) {
		$resp = $this->queryResult("
			select
				SPOULOPerson_id as \"SPOULOPerson_id\"
			from
				r50.v_SPOULOPerson
			where
				Person_id = :Person_id
		...
		return $this->queryResult("
			select
				...
				Error_Message as \"Error_Msg\"
			from r50.{$proc}(
				SPOULOPerson_id := :SPOULOPerson_id,
				Person_id := :Person_id,
	...
	function saveSPOULORecept($data) {
		$resp = $this->queryResult("
			select
				SPOULORecept_id as \"SPOULORecept_id\"
			from
				r50.v_SPOULORecept
			where
				EvnRecept_id = :EvnRecept_id
		...
		return $this->queryResult("
			select
				...
				Error_Message as \"Error_Msg\"
			from r50.{$proc}(
				SPOULORecept_id := :SPOULORecept_id,
				EvnRecept_id := :EvnRecept_id,
				pmUser_id := :pmUser_id
			)
		", $queryParams);
	...
	function saveSPOULOPrivilege($data) {
		$resp = $this->queryResult("
			select
				SPOULOPrivilege_id as \"SPOULOPrivilege_id\"
			from
				r50.v_SPOULOPrivilege
			where
		...
		return $this->queryResult("
			select
				...
				Error_Message as \"Error_Msg\"
			from r50.{$proc}(
				SPOULOPrivilege_id := :SPOULOPrivilege_id,
				PersonPrivilege_id := :PersonPrivilege_id,
