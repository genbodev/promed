 ...
 */
class TFOMSAutoInteract_model extends SwPgModel {
	protected $host;
	protected $port;
	protected $user;
	protected $password;
	...
	function isNotSendPackage($packageType, $procDataType, $objectID, $packageBody, $log) {
		...
		$query = "
			...
			from
				...
				inner join stg.v_PackageRouteType PRT on PRT.PackageRouteType_id = SLPT.PackageRouteType_id
				inner join stg.v_PackageStatus PS on PS.PackageStatus_id = SLP.PackageStatus_id
				left join stg.v_ServicePackage SP on SP.ServiceListPackage_id = SLP.ServiceListPackage_id
			where 1=1
	...
	function createActualPackageTable() {
		...
		$query = "
			...
			CREATE UNIQUE INDEX Idx1 ON {$tmpTable}({$this->packagesIndexedFields});
		";
	...
	function fillActualPackagesTable($data, $tmpTable, $packageTypesCur, $packageTypes = null) {
		...
		if (!empty($data['exportId'])) {
			...
			if (!empty($data['packageType'])) {
				...
				if (in_array('PERSONATTACH', $allowedPackageTypes) || in_array('PERSONATTACHDISTRICT', $allowedPackageTypes)) {
					...
					$personCardIds = $this->queryList("select LpuRegion_id from v_PersonCard_all where PersonCard_id in ({$exportIds_str})");
		...
		if ($this->regionNick == 'kareliya') {
			$filter .= " and not exists(
				select * from stg.v_ServiceListPackage SLP2
				inner join stg.v_ServiceListDetailLog SLDL on SLDL.ServiceListPackage_id = SLP2.ServiceListPackage_id
				where SLP2.ServiceListPackage_GUID = SLP.ServiceListPackage_GUID and SLDL.ServiceListLogType_id = 2
			)";
		}
		...
		$query = "
			...
			where
				...
				and not exists(
					select ServiceListDetailLog_id
		...
		if ($procDataType == 'Delete') {
			$query = "
				select
					...
					cast(dbo.tzgetdate() as date) as \"DATE\",
					...
					to_char(MSF.MedStaffRegion_begDate, 'YYYY-MM-DD') as \"DOC_DATE\",
					to_char(LR.LpuRegion_begDate, 'YYYY-MM-DD') as \"LPUREGION_BEGDATE\",
					to_char(LR.LpuRegion_endDate, 'YYYY-MM-DD') as \"LPUREGION_ENDDATE\"
					-- end select
				from
					-- from
					{$tmpTable} PL
					inner join fed.v_PassportToken LPT on LPT.Lpu_id = PL.Lpu_id
					left join LpuRegion LR on LR.LpuRegion_id = PL.ObjectID
					...
					left join lateral (
						...
						and A.Attribute_SysNick ilike 'Section_Code'
						limit 1
					) SectionCode on true
					left join lateral (
						select AV.AttributeValue_ValueString as Value
					...
					left join lateral (
						...
						from v_MedStaffRegion MSFR
							inner join v_MedStaffFact MSF on MSF.MedStaffFact_id = MSFR.MedStaffFact_id
							left join MedStaffRegionMain MSFRM on MSFRM.MedStaffRegion_id = MSFR.MedStaffRegion_id
						where MSFR.LpuRegion_id = LR.LpuRegion_id
							and MSFR.MedStaffRegion_begDate <= coalesce(LR.LpuRegion_endDate, cast(dbo.tzgetdate() as date))
							and coalesce(MSFR.MedStaffRegion_endDate, cast(dbo.tzgetdate() as date)) >= LR.LpuRegion_begDate
						order by
				...
				where
					...
					and nullif(PL.LPU,0) is not null
					{$filters_del}
					-- end where
				order by
					-- order by
            ...
            if ($this->regionNick == 'kareliya') {
                $update_select = "
					when 
						...
						MSF.MedStaffFact_id != coalesce(MSFR_old.MedStaffFact_id, 0)
					then 'Update'
				";
            } else {
			...
			$query = "
				select
					...
					coalesce(DISTRICT.GUID, newid()) as \"GUID\",
				...
				from
					-- from
					v_LpuRegion LR
					inner join v_Lpu L on L.Lpu_id = LR.Lpu_id
					inner join v_LpuRegionType LRT on LRT.LpuRegionType_id = LR.LpuRegionType_id
					inner join fed.v_PassportToken LPT on LPT.Lpu_id = LR.Lpu_id
					left join v_LpuSection LS on LS.LpuSection_id = LR.LpuSection_id
					left join v_LpuBuilding LB on LB.LpuBuilding_id = LS.LpuBuilding_id
					left join lateral (
						select ASV.AttributeSignValue_id
					...
					left join lateral (
						...
						from v_MedStaffRegion MSFR
							...
							left join MedStaffRegionMain MSFRM on MSFRM.MedStaffRegion_id = MSFR.MedStaffRegion_id
						where 
						    MSFR.LpuRegion_id = LR.LpuRegion_id
						    and MSFR.MedStaffRegion_begDate <= coalesce(LR.LpuRegion_endDate, cast(dbo.tzgetdate() as date))
						    and coalesce(MSFR.MedStaffRegion_endDate, cast(dbo.tzgetdate() as date)) >= LR.LpuRegion_begDate
						order by
						    MSFR.MedStaffRegion_isMain desc,
					...
					left join lateral (
						select MSFR.MedStaffRegion_id, MSFR.MedStaffFact_id
						from MedStaffRegion MSFR 
						inner join MedStaffRegionMain MSFRM on MSFRM.MedStaffRegion_id = MSFR.MedStaffRegion_id
						where 
				...
				where
					-- where
					ProcDataType.Value ilike :ProcDataType
		...
		if ($procDataType == 'Delete') {
			$query = "
				...
				from
					-- from
					{$tmpTable} PL
					left join v_PersonCard_all PC on PC.PersonCard_id = PL.ObjectID
					-- end from
				where
					-- where
					PL.PACKAGE_TYPE in ({$packageTypesInSQL})
			...
			$query = "
				select
					...
					coalesce(PERSONATTACH.GUID, newid()) as \"GUID\",
					ProcDataType.Value as \"OPERATIONTYPE\",
					P.BDZ_id as \"BDZID\",
					PC.LpuRegion_id as \"LPUREGION_ID\",
					PC.LpuRegion_fapid as \"LPUREGIONFAP_ID\",
					to_char(PC.PersonCard_begDate, 'YYYY-MM-DD') as \"PERSONATTACHDATE\",
					to_char(PC.PersonCard_endDate, 'YYYY-MM-DD') as \"PERSONATTACHENDT\",
				...
				from
					...
					left join v_YesNo IsAttachCondit on IsAttachCondit.YesNo_id = coalesce(PC.PersonCard_IsAttachCondit, 1)
					...
					left join lateral (
						select case
							...
							when PERSONATTACH.ID is not null and PERSONATTACH.DATE <= coalesce(PC.PersonCardEnd_updDT, PC.PersonCardBeg_updDT) then 'Update'
						end as Value
					) ProcDataType on true
					-- end from
				where
		...
		if ($procDataType == 'Delete') {
			$query = "
				select
					...
					'Delete' as \"OPERATIONTYPE\",
					3 as \"OPER_TYPE\",
					to_char(cast(dbo.tzgetdate() as date), 'YYYY-MM-DD') as \"DATA\",
					P.BDZ_id as \"BDZID\",
					...
					case when DT.DocumentType_Code = 14 and length(Document.Document_Ser) = 4
					...
					to_char(coalesce(PC.PersonCard_endDate, PC.PersonCard_begDate), 'YYYY-MM-DD') as \"DATE\",
					case when IsAttachCondit.YesNo_Code = 1 then 1 else 2 end as \"ATTACH_TYPE\",
					0 as \"T_PRIK\",
					to_char(PC.PersonCard_begDate, 'YYYY-MM-DD') as \"ATTACH_DT_MO\",
					L.Lpu_f003mcod as \"ATTACH_CODE_MO\",
					to_char(PC.PersonCard_endDate, 'YYYY-MM-DD') as \"DETACH_DT_MO\",
					case 
						when PC.PersonCard_endDate is not null 
						then coalesce(CCC.CardCloseCause_Code, CCCByNextPC.CardCloseCause_Code)
					...
					to_char(PC.PersonCard_begDate, 'YYYY-MM-DD') as \"ATTACH_DT\",
					...
					case
						...
						then to_char(PC.PersonCard_begDate, 'YYYY-MM-DD')
					end as \"ATTACH_DT_F\",
					case 
						when fapLR.LpuRegion_id is not null 
						then to_char(PC.PersonCard_endDate, 'YYYY-MM-DD')
					end as \"DETACH_DT_F\",
					case 
				...
				from
					-- from
					{$tmpTable} PL
					left join PersonCard PC on PC.PersonCard_id = PL.ObjectID
					inner join fed.v_PassportToken LPT on LPT.Lpu_id = PL.Lpu_id
					inner join v_Lpu L on L.Lpu_id = PC.Lpu_id
					...
					left join v_CardCloseCause CCC on CCC.CardCloseCause_id = PC.CardCloseCause_id
					left join v_CardCloseCause CCCByNextPC on CCCByNextPC.CardCloseCause_id = case 
					...
					left join lateral (
						...
						from
							v_CardReasonDetachLink crdl
							inner join v_CardReasonDetach crd on crd.CardReasonDetach_id = crdl.CardReasonDetach_id
						where
							crdl.CardCloseCause_id = coalesce(CCC.CardCloseCause_id, CCCByNextPC.CardCloseCause_id)
						limit 1
					) CRD on true
					left join lateral (
						select ASV.AttributeSignValue_id
					...
					left join lateral (
						...
						inner join v_MedStaffFact MSF on MSF.MedStaffFact_id = MSFR.MedStaffFact_id
						where MSFR.LpuRegion_id = LR.LpuRegion_id
						and MSFR.MedStaffRegion_begDate <= coalesce(PC.PersonCard_endDate, cast(dbo.tzgetdate() as date))
						and coalesce(MSFR.MedStaffRegion_endDate, cast(dbo.tzgetdate() as date)) >= PC.PersonCard_begDate
						order by
						MSFR.MedStaffRegion_isMain desc,
						MSFR.MedStaffRegion_begDate desc
						limit 1
					...
					left join v_LpuRegionType fapLRT on fapLRT.LpuRegionType_id = LR.LpuRegionType_id
					left join v_LpuSection fapLS on fapLS.LpuSection_id = fapLR.LpuSection_id
					left join v_LpuBuilding fapLB on fapLB.LpuBuilding_id = fapLS.LpuBuilding_id
					left join lateral (
						select ASV.AttributeSignValue_id
						from v_AttributeSignValue ASV
						inner join v_AttributeSign ASign on ASign.AttributeSign_id = ASV.AttributeSign_id
			...
			if ($procDataType == 'Insert') {
				...
				$filters .= " and exists(
					select * from {$tmpTable} 
					where ObjectID = PC.PersonCard_id and PACKAGE_TYPE in ({$packageTypesInSQL})
					and DATE <= coalesce(PC.PersonCardEnd_updDT, PC.PersonCardBeg_updDT)
				)";
			}
			...
			$query = "
				select
					...
					PC.PersonCard_id as \"ObjectID\",
					PC.PersonCard_id as \"PERSONATTACHID\",
					coalesce(PERSONATTACHDISTRICT.GUID, newid()) as \"GUID\",
				...
				from
					...
					v_PersonCard_all PC
					inner join v_Lpu L on L.Lpu_id = PC.Lpu_id
					inner join fed.v_PassportToken LPT on LPT.Lpu_id = L.Lpu_id
					inner join v_LpuRegion LR on LR.LpuRegion_id = PC.LpuRegion_id
					inner join Person P on P.Person_id = PC.Person_id
					inner join v_PersonState PS on PS.Person_id = PC.Person_id
					left join v_Sex Sex on Sex.Sex_id = PS.Sex_id
					...
					left join lateral (
						select crd.CardReasonDetach_Code
						from v_CardReasonDetachLink crdl
						inner join v_CardReasonDetach crd on crd.CardReasonDetach_id = crdl.CardReasonDetach_id
						where crdl.CardCloseCause_id = coalesce(CCC.CardCloseCause_id, CCCByNextPC.CardCloseCause_id)
						limit 1
					) CRD on true
					inner join lateral (
						select ASV.AttributeSignValue_id
				...
				where
					...
					{$filters}
					and nullif(nullif(L.Lpu_f003mcod,'0'),'') is not null
					and (L.Lpu_endDate is null or L.Lpu_endDate > cast(dbo.tzgetdate() as date))
					and PC.LpuRegion_id is not null
					-- end where
				order by
					-- order by
			...
			if ($procDataType != 'Delete') {
				...
				$query = "
					select
						...
						to_char(PDV.PersonDispVizit_NextDate, 'YYYY-MM-DD') as \"PLAN_DATE\"
					from
		...
		if ($procDataType == 'Delete') {
			...
			$query = "
				select
					...
					PD.PersonDisp_id as \"ObjectID\",
					PD.PersonDisp_id as \"DISP_ID\",
					coalesce(DISPOUT.GUID, newid()) as \"GUID\",
					ProcDataType.Value as \"OPERATIONTYPE\",
					to_char(cast(dbo.tzgetdate() as date), 'YYYY-MM-DD') as \"DATA\",
					{$fields}
					P.Person_id as \"ID_PAC\",
				...
				from
					...
					left join v_PersonState MPPS on MPPS.Person_id = MP.Person_id
					left join {$tmpTable} DISP on DISP.PACKAGE_TYPE = 'DISP' and DISP.ObjectID = PD.PersonDisp_id
					left join {$tmpTable} DISPOUT on DISPOUT.PACKAGE_TYPE = 'DISPOUT' and DISPOUT.ObjectID = PD.PersonDisp_id
					left join lateral (
						select case
	...
	function package_FREE_BEDS_INFORMATION($tmpTable, $procDataType, $data, $returnType = 'data', $start = 0, $limit = 500) {
		...
		$query = "
			select
				...
				coalesce(FBI.GUID, newid()) as \"GUID\",
				'Insert' as \"OPERATIONTYPE\",
				to_char(cast(dbo.tzgetdate() as date), 'YYYY-MM-DD') as \"DATA\",
				to_char(cast(dbo.tzgetdate() as date), 'YYYY-MM-DD') as \"ACTUAL_DATE\",
				...
				case 
					...
					when LU.LpuUnitType_SysNick ilike 'stac' then 1
					when LU.LpuUnitType_SysNick ilike 'dstac' then 2
					when LU.LpuUnitType_SysNick ilike 'pstac' then 2
					when LU.LpuUnitType_SysNick ilike 'hstac' then 2
				end as \"CARETYPE\",
				CurrAmount.Value as \"BEDOCCUPIED\",
				PrevAmount.Value as \"BEDOCCUPIEDTODAY\",
				PrevLeaveAmount.Value as \"BEDCLEARTODAY\",
				LSBS.LpuSectionBedState_Plan as \"BEDPLANNED\",
				LSBS.LpuSectionBedState_Fact - CurrAmount.Value - CurrDirectionAmount.Value as \"BEDFREE\",
				case
					when coalesce(LS.LpuSectionAge_id, 1) <> 2 
					then LSBS.LpuSectionBedState_Fact - CurrAmount.Value - CurrDirectionAmount.Value else 0
				end as \"BEDFREEADULT\",
				case 
					when coalesce(LS.LpuSectionAge_id, 1) = 2
					then LSBS.LpuSectionBedState_Fact - CurrAmount.Value - CurrDirectionAmount.Value else 0
			...
			from
				...
				left join lateral (
					select
						coalesce(sum(LSBS.LpuSectionBedState_Plan), 0) as LpuSectionBedState_Plan,
						coalesce(sum(LSBS.LpuSectionBedState_Fact), 0) as LpuSectionBedState_Fact
					from v_LpuSectionBedState LSBS
					where LSBS.LpuSection_id = LS.LpuSection_id
					and LSBS.LpuSectionBedState_begDate <= cast(dbo.tzgetdate() as date)
					and (LSBS.LpuSectionBedState_endDate is null or LSBS.LpuSectionBedState_endDate > cast(dbo.tzgetdate() as date))
					limit 1
				) LSBS on true
				left join lateral (
					...
					and cast(dbo.tzgetdate() as date) between ES.EvnSection_setDate and coalesce(ES.EvnSection_disDate, cast(dbo.tzgetdate() as date))
					limit 1
				) CurrAmount on true
				left join lateral (
					select count(*) as Value
					from v_EvnSection ES
					where ES.LpuSection_id = LS.LpuSection_id
					and :prevdate between ES.EvnSection_setDate and coalesce(ES.EvnSection_disDate, :prevdate)
					limit 1
				) PrevAmount on true
				left join lateral (
					select count(*) as Value
		...
		if ($procDataType == 'Delete') {
			$query = "
				...
				from
					-- from
					{$tmpTable} PL
					inner join EvnDirection ED on ED.Evn_id = PL.ObjectID
			...
			$query = "
				select
					...
					case
						...
						when :region = 10 then left(dLS.LpuSection_Code, 2)
					...
					case
						...
						when dLU.LpuUnitType_SysNick ilike 'stac' then 1
						when dLU.LpuUnitType_SysNick ilike 'dstac' then 2
						when dLU.LpuUnitType_SysNick ilike 'pstac' then 2
						when dLU.LpuUnitType_SysNick ilike 'hstac' then 2
					end as \"CARETYPE\",
	...
	function package_CANCEL_HOSPITALISATION_REFERRAL($tmpTable, $procDataType, $data, $returnType = 'data', $start = 0, $limit = 500) {
		...
		if ($this->regionNick == 'kareliya') {
			$msf = " coalesce(ED.MedStaffFact_fid, ED.MedStaffFact_id) ";
		} else {
		...
		if ($procDataType == 'Delete') {
			$query = "
				...
				from
					-- from
					{$tmpTable} PL
					inner join v_EvnDirection ED on ED.EvnDirection_id = PL.ObjectID
					left join v_Lpu dL on dL.Lpu_id = ED.Lpu_did
					-- end from
				where
					-- where
			...
			$query = "
				select
					...
					coalesce(CANCEL_HOSPITALISATION_REFERRAL.GUID, newid()) as \"GUID\",
					ProcDataType.Value as \"OPERATIONTYPE\",
					to_char(cast(dbo.tzgetdate() as date), 'YYYY-MM-DD') as \"DATA\",
					ED.EvnDirection_Num as \"REFERRAL_NUMBER\",
					(
					...
					to_char(ED.EvnDirection_setDT, 'YYYY-MM-DD') as \"DATE\",
				...
				from
					...
					inner join v_Lpu L on L.Lpu_id = ED.Lpu_id
					inner join v_Lpu dL on dL.Lpu_id = ED.Lpu_did
					left join v_MedStaffFact fMSF on fMSF.MedStaffFact_id = {$msf}
					left join v_Lpu fL on fL.Lpu_id = coalesce(fMSF.Lpu_id, ED.Lpu_cid, ED.Lpu_id)
					left join v_LpuSection fLS on fLS.LpuSection_id = fMSF.LpuSection_id
					left join v_LpuBuilding fLB on fLB.LpuBuilding_id = fMSF.LpuBuilding_id
					left join lateral (
						select ASV.AttributeSignValue_id
						from v_AttributeSignValue ASV
						inner join v_AttributeSign ASign on ASign.AttributeSign_id = ASV.AttributeSign_id
					...
					left join lateral (
						select ESCL.EvnStatusCause_id
						from v_EvnStatusCauseLink ESCL
						left join v_EvnStatusCause ESC on ESC.EvnStatusCause_id = ESCL.EvnStatusCause_id
					...
					left join v_EvnStatusCause ESC on ESC.EvnStatusCause_id = coalesce(ESH.EvnStatusCause_id, ESCL.EvnStatusCause_id)
		...
		if ($procDataType == 'Delete') {
			...
			$query = "
				select
					...
					to_char(EPS.EvnPS_setDT, 'YYYY-MM-DD\"T\"HH24:MI:SS') as \"HOSPITALISATION_TIME\",
					(
				...
				from
					...
					left join {$tmpTable} HOSPITALISATION on HOSPITALISATION.PACKAGE_TYPE in ({$packageTypesInSQL}) 
		...
		if ($procDataType == 'Delete') {
			...
			$query = "
				select
					...
					EPS.EvnPS_id as \"ObjectID\",
					EPS.EvnPS_id as \"H_ID\",
					coalesce(EXTRHOSPITALISATION.GUID, newid()) as \"GUID\",
					ProcDataType.Value as \"OPERATIONTYPE\",
					to_char(cast(dbo.tzgetdate() as date), 'YYYY-MM-DD') as \"DATA\",
					coalesce(ED.EvnDirection_Num, EPS.EvnDirection_Num) as \"REFERRAL_NUMBER\",
					(
				...
				from
					...
					left join {$tmpTable} EXTRHOSPITALISATION on EXTRHOSPITALISATION.PACKAGE_TYPE in ({$packageTypesInSQL})
		...
		if ($procDataType == 'Delete') {
			...
			$query = "
				select
					...
					coalesce(CANCEL_HOSPITALISATION.GUID, newid()) as \"GUID\",
					ProcDataType.Value as \"OPERATIONTYPE\",
					...
					0 as \"REASON\",
					1 as \"CANCEL_SOURSE\",
					to_char(EPS.EvnPS_OutcomeDT, 'YYYY-MM-DD') as \"DATE_CANCEL\",
					1 as \"CANCEL_TYPE\",
				...
				from
					...
					v_EvnPS EPS
					inner join v_Lpu L on L.Lpu_id = EPS.Lpu_id
					inner join v_LpuSection pLS on pLS.LpuSection_id = EPS.LpuSection_pid
					inner join v_LpuBuilding pLB on pLB.LpuBuilding_id = pLS.LpuBuilding_id
					inner join v_LpuSection LS on LS.LpuSection_id = pLB.LpuBuilding_id
					inner join v_LpuBuilding LB on LB.LpuBuilding_id = LS.LpuSection_id
					left join {$tmpTable} CANCEL_HOSPITALISATION on CANCEL_HOSPITALISATION.PACKAGE_TYPE = 'CANCEL_HOSPITALISATION' 
		...
		if ($procDataType == 'Delete') {
			...
			$query = "
				select
					...
					ProcDataType.Value as \"OPERATIONTYPE\",
					EPS.EvnPS_id as \"HOSPITALISATION_ID\",
					coalesce(MOTION_IN_HOSPITAL.GUID, newid()) as \"GUID\",
					to_char(cast(dbo.tzgetdate() as date), 'YYYY-MM-DD') as \"DATA\",
					coalesce(ED.EvnDirection_Num, EPS.EvnDirection_Num) as \"REFERRAL_NUMBER\",
					(
						cast(sL.Lpu_f003mcod as varchar) ||
					...
					to_char(coalesce(ED.EvnDirection_setDT, EPS.EvnDirection_setDT), 'YYYY-MM-DD') as \"REFERRAL_DATE\",
					case 
						when :region = 'kareliya' and PT.PrehospType_Code = 1 then 1
						when :region = 'kareliya' then 2
						when PT.PrehospType_Code = 1 then 0 
						else 1
					end as \"HOSPITALISATION_TYPE\",
					--LB.LpuBuilding_Code as \"BRANCH\",
					case
						--  #195757 - по требованию Макаровой Елены (Карелия)
						when :region = 'kareliya' then left(LS.LpuSection_Code, 2)
						else LB.LpuBuilding_Code
					end as \"BRANCH\",
					LS.LpuSection_Code as \"DIVISION\",
					LSP.LpuSectionProfile_Code as \"STRUCTURE_BED\",
					...
					LSBS.LpuSectionBedState_id as \"DLSB\",
					case
						when :region <> 'kareliya' then null
						when LU.LpuUnitType_SysNick ilike 'stac' then 1
						when LU.LpuUnitType_SysNick ilike 'dstac' then 2
						when LU.LpuUnitType_SysNick ilike 'pstac' then 2
						when LU.LpuUnitType_SysNick ilike 'hstac' then 2
				...
				from
					-- from
					v_Lpu L
					inner join v_EvnSection ES on L.Lpu_id = ES.Lpu_id
					...
					left join {$tmpTable} MOTION_IN_HOSPITAL on MOTION_IN_HOSPITAL.PACKAGE_TYPE = 'MOTION_IN_HOSPITAL' 
		...
		if ($procDataType == 'Delete') {
			$query = "
				...
				from
					-- from
					{$tmpTable} PL
					left join v_EvnRecept ER on ER.EvnRecept_id = PL.ObjectID
					-- end from
				where
					-- where
			...
			$query = "
				select
					...
					ProcDataType.Value as \"OPERATIONTYPE\",
					coalesce(ONKOTRAMADOL.GUID, newid()) as \"GUID\",
					to_char(cast(dbo.tzgetdate() as date), 'YYYY-MM-DD') as \"DATA\",
					P.Person_id as \"ID_PAC\",
					P.BDZ_id as \"BDZ_ID\",
					Diag.Diag_Code as \"DS\",
					date_part('quarter', ER.EvnRecept_setDT) as \"KV\",
					date_part('year', ER.EvnRecept_setDT) as \"YEAR\",
					null as \"RECEPTCOMMENT\"
					-- end select
				from
					-- from
					v_EvnRecept ER
					inner join v_Lpu L on L.Lpu_id = ER.Lpu_id
					inner join Person P on P.Person_id = ER.Person_id
					inner join v_Diag Diag on Diag.Diag_id = ER.Diag_id
					left join rls.v_Drug D on D.Drug_id = ER.Drug_rlsid
					inner join rls.v_DrugComplexMnn DCM on DCM.DrugComplexMnn_id = coalesce(ER.DrugComplexMnn_id, D.DrugComplexMnn_id)
					inner join rls.v_DrugComplexMnnName DCMN on DCMN.DrugComplexMnnName_id = DCM.DrugComplexMnnName_id
					inner join rls.v_ACTMATTERS A on A.ACTMATTERS_ID = DCMN.ACTMATTERS_id
					left join {$tmpTable} ONKOTRAMADOL on ONKOTRAMADOL.PACKAGE_TYPE = 'ONKOTRAMADOL' 
		...
		if ($procDataType == 'Delete') {
			...
			$query = "
				select
					...
					ER.EvnRecept_id as \"REG_ID\",
					ProcDataType.Value as \"OPERATIONTYPE\",
					coalesce(ONKODRUG.GUID, newid()) as \"GUID\",
					to_char(cast(dbo.tzgetdate() as date), 'YYYY-MM-DD') as \"DATA\",
					P.Person_id as \"ID_PAC\",
					P.BDZ_id as \"BDZ_ID\",
					Diag.Diag_Code as \"DS\",
				...
				from
					...
					left join rls.v_Drug rlsD on rlsD.Drug_id = coalesce(ER.Drug_rlsid, D.RLS_id)
					inner join rls.v_DrugComplexMnn DCM  on DCM.DrugComplexMnn_id = coalesce(ER.DrugComplexMnn_id, rlsD.DrugComplexMnn_id)
					inner join rls.v_DrugComplexMnnName DCMN on DCMN.DrugComplexMnnName_id = DCM.DrugComplexMnnName_id
					inner join rls.v_ACTMATTERS A on A.ACTMATTERS_ID = DCMN.ACTMATTERS_id
					left join {$tmpTable} ONKODRUG on ONKODRUG.PACKAGE_TYPE = 'ONKODRUG' 
		...
		if ($procDataType == 'Delete') {
			$query = "
				...
				from
					-- from
					{$tmpTable} PL
					left join PersonRegister PR on PR.PersonRegister_id = PL.ObjectID
					-- end from
				where
					-- where
			...
			$query = "
				select
					...
					ProcDataType.Value as \"OPERATIONTYPE\",
					coalesce(PERSCANCER.GUID, newid()) as \"GUID\",
					to_char(cast(dbo.tzgetdate() as date), 'YYYY-MM-DD') as \"DATA\",
					P.BDZ_id as \"BDZ_ID\",
					Diag.Diag_Code as \"DS\",
					to_char(PR.PersonRegister_setDate, 'YYYY-MM-DD') as \"DATE_IN\",
					to_char(PR.PersonRegister_disDate, 'YYYY-MM-DD') as \"DATE_OUT\",
				...
				from
					...
					inner join v_Lpu L on L.Lpu_id = PR.Lpu_iid
					left join v_Diag Diag on Diag.Diag_id = PR.Diag_id
					left join v_PersonRegisterOutCause PROutCause on PROutCause.PersonRegisterOutCause_id = PR.PersonRegisterOutCause_id
					left join {$tmpTable} PERSCANCER on PERSCANCER.PACKAGE_TYPE = 'PERSCANCER'
		...
		if ($procDataType == 'Delete') {
			$query = "
				...
				from
					-- from
					{$tmpTable} PL
					left join v_EvnDirectionHTM EDH on EDH.EvnDirectionHTM_id = PL.ObjectID
					inner join EvnDirectionHTM EDHD on EDHD.Evn_id = PL.ObjectID
					inner join v_LpuHTM LH on LH.LpuHTM_id = EDHD.LpuHTM_id
					-- end from
				where
					-- where
			...
			$query = "
				select
					...
					EDH.EvnDirectionHTM_id as \"HTMC_ID\",
					ProcDataType.Value as \"OPERATIONTYPE\",
					coalesce(HTMC_REFERRAL.GUID, newid()) as \"GUID\",
					to_char(cast(dbo.tzgetdate() as date), 'YYYY-MM-DD') as \"DATA\",
					P.BDZ_id as \"BDZ_ID\",
					EDH.EvnDirectionHTM_Num as \"REFERRAL_NUMBER\",
					to_char(EDH.EvnDirectionHTM_directDate, 'YYYY-MM-DD') as \"REFERRAL_DATE\",
					EDH.EvnDirectionHTM_TalonNum as \"TALON_NUMBER\",
					to_char(EDH.EvnDirectionHTM_setDate, 'YYYY-MM-DD') as \"TALON_DATE\",
					...
					to_char(EDH.EvnDirectionHTM_VKProtocolDate, 'YYYY-MM-DD') as \"PROTDATE\",
				...
				from
					...
					inner join v_Lpu L on L.Lpu_id = EDH.Lpu_sid
					inner join v_LpuHTM LH on LH.LpuHTM_id = EDH.LpuHTM_id
					inner join v_PersonState PS on PS.Person_id = EDH.Person_id
					inner join Person P on P.Person_id = PS.Person_id
					inner join v_EvnPS EPS on EPS.EvnDirection_id = EDH.EvnDirectionHTM_id
					left join lateral (
						select ES.*
						from v_EvnSection ES
					...
					left join v_MedStaffFact MSF on MSF.MedStaffFact_id = EDH.MedStaffFact_id
					left join v_EvnVK EVK on EVK.EvnVK_id = EDH.EvnDirectionHTM_pid
					left join {$tmpTable} HTMC_REFERRAL on HTMC_REFERRAL.PACKAGE_TYPE = 'HTMC_REFERRAL'
	...
	function package_ECO_REFERRAL($tmpTable, $procDataType, $data, $returnType = 'data', $start = 0, $limit = 500) {
		$params = array(
			'ProcDataType' => $procDataType,
			'date' => $this->currentDT->format('YYYY-MM-DD'),
		);
		...
		if ($procDataType == 'Delete') {
			$query = "
				...
				from
					-- from
					{$tmpTable} PL
					inner join v_EvnDirectionEco EDE on EDE.EvnDirectionEco_id = PL.ObjectID
					left join v_Org dL on dL.Org_id = EDE.Org_id
					left join v_Evn_del EDel on EDel.Evn_id = EDE.EvnDirectionEco_id
					-- end from
				where
					-- where
					PL.PACKAGE_TYPE = 'ECO_REFERRAL'
					and coalesce(EDE.EvnDirectionEco_failDT, EDel.Evn_delDT) > PL.DATE
					and nullif(PL.LPU,0) is not null
					{$filters_del}
					-- end where
				order by
			...
			$query = "
				select
					...
					coalesce(ECO_REFERRAL.GUID, newid()) as \"GUID\",
					P.BDZ_id as \"BDZ_ID\",
					EDE.EvnDirectionEco_Num as \"REFERRAL_NUMBER\",
					to_char(EDE.EvnDirectionEco_setDT, 'YYYY-MM-DD') as \"REFERRAL_DATE\",
					MSF.Person_Snils as \"DOC_CODE\",
					EDE.EvnDirectionEco_NumVKMZ as \"PROTNUM\",
					to_char(EDE.EvnDirectionEco_VKMZDate, 'YYYY-MM-DD') as \"PROTDATE\",
					EDE.EvnDirectionEco_CommentVKMZ as \"PROTCOMMENT\",
					EDE.EvnDirectionEco_Comment as \"ECOCOMMENT\"
					-- end select
				from
					-- from
					v_EvnDirectionEco EDE
					inner join v_Lpu L on L.Lpu_id = EDE.Lpu_sid
					inner join v_Org LD on LD.Org_id = EDE.Org_id
					inner join v_PersonState PS on PS.Person_id = EDE.Person_id
					inner join Person P on P.Person_id = PS.Person_id
					left join v_MedStaffFact MSF on MSF.MedStaffFact_id = EDE.MedStaffFact_id
					left join {$tmpTable} ECO_REFERRAL on ECO_REFERRAL.PACKAGE_TYPE = 'ECO_REFERRAL'
		...
		if ($procDataType == 'Delete') {
			$query = "
				...
				from
					-- from
					{$tmpTable} PL
					inner join MorbusNephro MN on MN.MorbusNephro_id = PL.ObjectID
					-- end from
				where
					-- where
			...
			$query = "
				select
					...
					coalesce(HEMODIAL.GUID, newid()) as \"GUID\",
					P.BDZ_id as \"BDZ_ID\",
					to_char(MNDF.MorbusNephroDialysis_begDT, 'YYYY-MM-DD') as \"DATE_IN\",
					to_char(MNDL.MorbusNephroDialysis_endDT, 'YYYY-MM-DD') as \"DATE_OUT\",
					PROutCause.PersonRegisterOutCause_Code as \"CAUSEOUT\",
					'' as \"DIALCOMMENT\"
					-- end select
				from
					...
					inner join v_Lpu L on L.Lpu_id = MNDL.Lpu_id
					inner join v_PersonState PS on PS.Person_id = MB.Person_id
					inner join Person P on P.Person_id = PS.Person_id
					left join v_PersonRegisterOutCause PROutCause on PROutCause.PersonRegisterOutCause_id = MNDL.PersonRegisterOutCause_id
					left join {$tmpTable} HEMODIAL on HEMODIAL.PACKAGE_TYPE = 'HEMODIAL'
		...
		if ($procDataType == 'Delete') {
			$query = "
				...
				from
					-- from
					{$tmpTable} PL
					inner join MorbusHepatitisPlan MHP on MHP.MorbusHepatitisPlan_id = PL.ObjectID
					-- end from
				where
					-- where
			...
			$query = "
				select
					...
					coalesce(PERSHEPATITIS.GUID, newid()) as \"GUID\",
					P.BDZ_id as \"BDZ_ID\",
				...
				from
					-- from
					v_MorbusHepatitisPlan MHP
					inner join v_MorbusHepatitis MH on MH.MorbusHepatitis_id = MHP.MorbusHepatitis_id
					inner join v_MorbusBase MB on MB.MorbusBase_id = MH.MorbusBase_id
					inner join v_Lpu L on L.Lpu_id = MHP.Lpu_id
					inner join v_PersonState PS on PS.Person_id = MB.Person_id
					inner join Person P on P.Person_id = PS.Person_id
					inner join fed.v_MedicalCareType MCT on MCT.MedicalCareType_id = MHP.MedicalCareType_id
					left join {$tmpTable} PERSHEPATITIS on PERSHEPATITIS.PACKAGE_TYPE = 'PERSHEPATITIS'
	...
	function import_package_ANSWER($package, $body, $properties, $packageType, $data, $log) {
		...
		$query = "
			...
			and SLP.ServiceListPackage_ObjectName ilike :ServiceListPackage_ObjectName
	...
	function searchPerson($data) {
		...
		if ($has('FAM')) {
			$filters[] = "rtrim(PS.Person_SurName) ilike :FAM";
			$params['FAM'] = $data['FAM'];
		}
		if ($has('IM')) {
			$filters[] = "rtrim(PS.Person_FirName) ilike :IM";
			$params['IM'] = $data['IM'];
		}
		if ($has('OT')) {
			$filters[] = "coalesce(rtrim(PS.Person_SecName), '') ilike :OT";
			$params['OT'] = $data['OT'];
		}
		if ($has('DR')) {
		...
		if ($has('DOCSER')) {
			$filters[] = "coalesce(replace(replace(D.Document_Ser, ' ', ''), '-', ''), '') = coalesce(replace(replace(:DOCSER, ' ', ''), '-', ''), '')";
		...
		if ($has('SNILS')) {
			$filters[] = "coalesce(PS.Person_Snils, '') = coalesce(cast(:SNILS as varchar),'')";
		...
		$DTFormat = in_array($this->regionNick,['perm','kareliya'])?'YYYY-MM-DD\"T\"HH24:MI:SS':'YYYY-MM-DD';
		...
		$query = "
			select
				...
				PS.Person_Snils as \"SNILS\",
				DT.DocumentType_Code as \"DOCTYPE\",
				case when DT.DocumentType_Code = 14 and length(D.Document_Ser) = 4
					then substring(D.Document_Ser, 1, 2) || ' ' || substring(D.Document_Ser, 3, 2)
					else D.Document_Ser
				end as \"DOCSER\",
				D.Document_Num as \"DOCNUM\",
				to_char(PS.Person_BirthDay, 'YYYY-MM-DD') as \"DR\",
				to_char(PS.Person_deadDT, 'YYYY-MM-DD') as \"DS\",
				...
				to_char(Polis.Polis_begDate, '{$DTFormat}') as \"POLISBEGDT\",
				to_char(Polis.Polis_endDate, '{$DTFormat}') as \"POLISENDDT\",
			...
			from
				...
				left join v_KLCountry KLC on KLC.KLCountry_id = PS.KLCountry_id
				-- end from
			where
				-- where
				{$filters_str}
	...
	function getPolisForClose($data) {
		...
		$query = "
			select
				PS.Person_id as \"Person_id\",
				Polis.PersonPolis_id as \"PersonPolis_id\",
				Polis.Polis_id as \"Polis_id\",
				rtrim(PS.Person_SurName) as \"FAM\",
				rtrim(PS.Person_FirName) as \"IM\",
				rtrim(PS.Person_SecName) as \"OT\",
				to_char(PS.Person_BirthDay, 'YYYY-MM-DD') as \"DR\",
				to_char(dbo.tzGetDate(), 'YYYY-MM-DD') as \"POLISENDDT\"
			from
				v_PersonState PS
				inner join Person P on P.Person_id = PS.Person_id
				inner join v_PersonPolis Polis on Polis.Polis_id = PS.Polis_id
			where
				P.BDZ_id = :BDZID
				and Polis.Server_id in (0, 2)
				and (Polis.Polis_endDate is null or Polis.Polis_endDate > dbo.tzGetDate())
			limit 1
		";
	...
	function searchPolis($data) {
		$params = array(
			'bigdate' => $this->currentDT->modify('+50 year'),
			'begPeriod' => null,
			'endPeriod' => null,
		);
		$filters = array('1=1');
		...
		if (!empty($data['SMOCODE'])) {
			$filters[] = "(
				not exists(select * from v_OrgSmo where OrgSmo_f002smocod = :SMOCODE)
		...
		if (!empty($data['begPeriod'])) {
			$filters[] = "P.Polis_begDate <= coalesce(cast(:endPeriod as date), cast(:bigdate as date))";
			$filters[] = "coalesce(P.Polis_endDate, :bigdate) >= :begPeriod";
			$params['begPeriod'] = $data['begPeriod'];
		...
		$DTFormat = ($this->regionNick == 'perm')?'YYYY-MM-DD\"T\"HH24:MI:SS':'YYYY-MM-DD';

		$query = "
			select
				P.Polis_id as \"Polis_id\",
				P.PersonPolis_id as \"PersonPolis_id\",
				...
				to_char(P.Polis_begDate, '{$DTFormat}') as \"POLISBEGDT\",
				to_char(P.Polis_endDate, '{$DTFormat}') as \"POLISENDDT\",
			...
			from
				...
				left join v_PolisCloseCause PCC on PCC.PolisCloseCause_id = P.PolisCloseCause_id
			where
				{$filters_str}
			order by
				P.PersonPolis_insDT
	...
	function getPolisData($Person_id) {
		...
		$query = "
			select
				PT.PolisType_CodeF008 as \"POLISTYPE\",
				P.Polis_Ser as \"POLISSER\",
				coalesce(P.Polis_Num, ENP.PersonPolisEdNum_EdNum) as \"POLISNUM\",
				to_char(P.Polis_begDate, '{$DTFormat}') as \"POLISBEGDT\",
				to_char(P.Polis_endDate, '{$DTFormat}') as \"POLISENDDT\"
			from
				v_PersonPolis P
				inner join v_PolisType PT on PT.PolisType_id = P.PolisType_id
				left join lateral (
					select ENP.PersonPolisEdNum_EdNum
					from v_PersonPolisEdNum ENP
	...
	function setPerson($person, $data) {
		...
		if (array_key_exists('SOC_STATUS', $person) && $this->regionNick == 'khak') {
			...
			$params['SOC_STATUS'] = $this->getFirstResultFromQuery("
				select SocStatus_id from v_SocStatus
				where SocStatus_Code = :SocStatus_Code limit 1
			", array(
	...
	function setNationality($nationality, $data) {
		...
		$nationality['KLCountry_id'] = $this->getFirstResultFromQuery("
			select KLCountry_id from v_KLCountry where KLCountry_Code = :OKSM_ALFA3 limit 1
		", $nationality);
		...
		$prevNationality = $this->getFirstRowFromQuery("
			select
				...
				NS.NationalityStatus_IsTwoNation as \"NationalityStatus_IsTwoNation\",
				NS.LegalStatusVZN_id as \"LegalStatusVZN_id\"
			from 
				v_PersonState PS
				inner join v_NationalityStatus NS on NS.NationalityStatus_id = PS.NationalityStatus_id
			where 
	...
	function setDocument($document, $data) {
		...
		$query = "			
			...
			from p_PersonDocument_ins (
				DocumentType_id := (
					select DocumentType_id from v_DocumentType
					where DocumentType_Code = :DOCTYPE limit 1
				),
				Document_Ser := :DOCSER,
				Document_Num := :DOCNUM,
				Person_id := :Person_id,
	...
	function createPolis($polis, $data) {
		...
		$query = "
			...
			from p_PersonPolis_ins (
				OmsSprTerr_id := (
					select OmsSprTerr_id from v_OmsSprTerr
					where KLRgn_id = :Region_id limit 1
				),
				PolisType_id := (
					select PolisType_id from v_PolisType
					where PolisType_CodeF008 = :POLISTYPE limit 1
				),
				OrgSmo_id := (
					select OrgSmo_id from v_OrgSmo
					where OrgSmo_f002smocod = :SMOCODE limit 1
				),
				Polis_Ser := :POLISSER,
				Polis_Num := :POLISNUM,
				...
				PolisCloseCause_id := (
					select PolisCloseCause_id from v_PolisCloseCause
					where PolisCloseCause_Code = :POLISCLOSECAUSE limit 1
				),
				Person_id := :Person_id,
				Server_id := :Server_id,
				pmUser_id := :pmUser_id
	...
	function updatePolis($polis, $data) {
		...
		$query = "
			select
				...
				to_char(PP.Polis_begDate, 'YYYY-MM-DD') as \"Polis_begDate\",
				to_char(PP.Polis_endDate, 'YYYY-MM-DD') as \"Polis_endDate\",
				PP.PolisCloseCause_id as \"PolisCloseCause_id\"
			from
				v_PersonPolis PP
			where
	...
	function setPersonCard($personCard, $data) {
		...
		$query = "
			...
			from p_PersonCard_ins (
				...
				Person_id := :Person_id,
				Lpu_id := (
					select Lpu_id from v_Lpu
					where Lpu_f003mcod = :ATTACHMO limit 1
				),
				LpuAttachType_id := 1,
	...
	function _publicateDopDispPlan($data, $log) {
		...
		$query = "
			...
			from
				...
				inner join v_Lpu L on L.Lpu_id = PDDP.Lpu_id
				inner join fed.v_PassportToken LPT on LPT.Lpu_id = L.Lpu_id
		...
		$query = "
			select
				...
				{$fields}
				coalesce(PATIENT.GUID, newid()) as \"GUID\",
				rtrim(PS.Person_SurName) as \"FAM\",
				rtrim(PS.Person_FirName) as \"IM\",
				rtrim(PS.Person_SecName) as \"OT\",
				Sex.Sex_fedid as \"W\",
				...
				coalesce(SectionCode.Value, LS.LpuSection_Code) as \"LPUSECTION_CODETFOMS\",
				coalesce(BuildingCode.Value, LB.LpuBuilding_Code) as \"LPUBUILDING_CODETFOMS\",
				case when coalesce(EPLDD13.EvnPLDispDop13_IsRefusal, 1) = 2
					then to_char(EPLDD13.EvnPLDispDop13_setDT, 'YYYY-MM-DD')
				end as \"REJECT_DATE\",
				...
				nullif(rtrim(COALESCE(PS.Person_Phone, PInfo.PersonInfo_InternetPhone)), '') as \"TEL1\",
				null as \"TEL2\"
			from
				...
				left join v_PersonPrivilegeWOW PPW on PPW.Person_id = P.Person_id
				left join lateral (
					select PC.*
				...
				left join lateral (
					...
					inner join v_MedStaffFact MSF on MSF.MedStaffFact_id = MSFR.MedStaffFact_id
					where MSFR.LpuRegion_id = LR.LpuRegion_id
					and MSFR.MedStaffRegion_begDate <= coalesce(PC.PersonCard_endDate, :date)
					and coalesce(MSFR.MedStaffRegion_endDate, :date) >= PC.PersonCard_begDate
					order by
					MSFR.MedStaffRegion_isMain desc,
					MSFR.MedStaffRegion_begDate desc
					limit 1
				...
				left join lateral (
					...
					and :date between ASV.AttributeSignValue_begDate and coalesce(ASV.AttributeSignValue_endDate, :date)
					order by ASV.AttributeSignValue_begDate desc, ASV.AttributeSignValue_insDT desc
					limit 1
				) ASV on true
				left join lateral (
				...
				left join lateral (
					...
					and date_part('year', EPLDD13.EvnPLDispDop13_setDT) = PDDP.PersonDopDispPlan_Year
					and EPLDD13.DispClass_id = 1
					limit 1
				) EPLDD13 on true
				left join lateral (
					select OSL.Object_Guid::uuid as GUID
					from v_ObjectSynchronLog OSL
					inner join v_ObjectSynchronLogService OSLS on OSLS.ObjectSynchronLogService_id = OSL.ObjectSynchronLogService_id
			...
			where
				PDDP.PersonDopDispPlan_id in ({$PersonDopDispPlan_ids_str})
				and coalesce(PPLS.PlanPersonListStatusType_id, 1) in (1,5)
				and nullif(Smo.Orgsmo_f002smocod, '') is not null
				{$filters}
			order by
				Smo.OrgSmo_id
	...
	function import_package_ANSWER_DISPPLAN($package, $body, $properties, $data, $log) {
		...
		$query = "
			select
				...
				PPL.PersonDopDispPlanExport_id as \"PersonDopDispPlanExport_id\"
			from stg.v_ServiceListPackage SLP
			inner join stg.v_ServiceListLog SLL on SLL.ServiceListLog_id = SLP.ServiceListLog_id
			inner join v_PlanPersonList PPL on PPL.PlanPersonList_id = SLP.ServiceListPackage_ObjectID
			where SLL.ServiceList_id = :ServiceList_id
			and SLP.ServiceListPackage_ObjectName ilike :ServiceListPackage_ObjectName
			and SLP.ServiceListPackage_GUID = :ServiceListPackage_GUID
			limit 1
		...
		if ($procDataType == 'Delete') {
			$query = "
				...
				from
					-- from
					{$tmpTable} PL
					left join v_PersonRegister PR on PR.PersonRegister_id = PL.ObjectID
			...
			$query = "
				select
					...
					PR.PersonRegister_id as \"ObjectID\",
					PR.PersonRegister_id as \"REG_ID\",
					coalesce(GEBT.GUID, newid()) as \"GUID\",
					ProcDataType.Value as \"OPERATIONTYPE\",
					:date as \"DATA\",
					P.BDZ_id as \"BDZ_ID\",
					L.Lpu_id as \"Lpu_id\",
				...
				from
					...
					inner join v_MorbusGEBT MG on MG.Morbus_id = M.Morbus_id
					inner join lateral (
						select
							Lpu_id,
							MorbusGEBTPlan_Year,
					...
					inner join v_Lpu L on L.Lpu_id = mgp.Lpu_id
					inner join v_PersonState PS on PS.Person_id = PR.Person_id
					inner join Person P on P.Person_id = PS.Person_id
					inner join fed.v_MedicalCareType mct on mct.MedicalCareType_id = mgp.MedicalCareType_id
					left join {$tmpTable} GEBT on GEBT.PACKAGE_TYPE = 'GEBT' 
