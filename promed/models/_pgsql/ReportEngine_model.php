 ...
 * @author yunitsky
 */
class ReportEngine_model extends swPgModel {
	private $_regionList = [
	 ...
	 * @comment Прямой вызов функции библиотеки pg
	 * Коннект к серверу
	 */
	...
	// const REPORTS_FOLDERS_SQL = 'select * from rpt.v_ReportCatalog ReportCatalog  ';
	const REPORTS_FOLDERS_SQL = "
		select ReportCatalog.ReportCatalog_id as \"ReportCatalog_id\",
		   (
		   ...
		   ReportCatalog.ReportCatalog_Status as \"ReportCatalog_Status\",
		   ReportCatalog.ReportCatalog_Path as \"ReportCatalog_Path\",
		   ReportCatalog.ReportCatalog_Position as \"ReportCatalog_Position\",
		   ReportCatalog.pmUser_insID as \"pmUser_insID\",
		   ReportCatalog.pmUser_updID as \"pmUser_updID\",
		   ReportCatalog.ReportCatalog_insDT as \"ReportCatalog_insDT\",
		   ReportCatalog.ReportCatalog_updDT as \"ReportCatalog_updDT\",
		   ReportCatalog.Region_id as \"Region_id\"
		from rpt.ReportCatalog ReportCatalog
	";
	...
	const REPORTS_SQL = '
		select
			...
			Report_insDT as "Report_insDT",
			Report_updDT as "Report_updDT",
			Region_id as "Region_id",
			ReportType_id as "ReportType_id",
			DatabaseType as "DatabaseType",
			Report_Rowversion as "Report_Rowversion"
		from rpt.Report Report
		where ReportCatalog_id = ?
	...
	protected function getAvailableARMs($data) {
		$query = "
			...
			from
				v_ARMType AT
				left join rpt.v_ReportARM RA on RA.ARMType_id = AT.ARMType_id
			where
	...
	const PARAM_FOLDERS_SQL = '
    	...
    	from rpt.ReportParameterCatalog ReportParameterCatalog';
	const PARAM_SQL = '
		select
		...
		from rpt.ReportParameter ReportParameter
	...
	const REPORT_FIELDSET_SQL = '
		...
		from rpt.ReportContent ReportContent
		where ReportContent_Name is not null and
			  Report_id = ?
		order by ReportContent_Position
	';
	const REPORT_PARAM_SQL = '
		select a.ReportContentParameter_id as "ReportContentParameter_id",
			...
			COALESCE(a.ReportContentParameter_ReportLabel, c.ReportParameter_Label)
				|| \' - <span class="x-grid-gray-text">\'
				|| COALESCE(a.ReportContentParameter_ReportId, c.ReportParameter_Name)
			|| \'</span>\' as "Name"
		from rpt.ReportContentParameter a
			join rpt.v_ReportContent b on a.ReportContent_id = b.ReportContent_id
			join rpt.v_ReportParameter c on c.ReportParameter_id = a.ReportParameter_id
		where b.ReportContent_Name is null and
			b.Report_id = ?
		order by "position"
	';
	...
	const FIELDSET_SQL = '
		select
			...
			COALESCE(a.ReportContentParameter_ReportLabel, c.ReportParameter_Label) || \' - <span class="x-grid-gray-text">\' || c.ReportParameter_Name || \'</span>\' as "Name"
		from rpt.ReportContentParameter a
			join rpt.v_ReportContent b on a.ReportContent_id = b.ReportContent_id
			join rpt.v_ReportParameter c on c.ReportParameter_id = a.ReportParameter_id
		where b.ReportContent_Name is not null and
			b.ReportContent_id = ?
		order by "position"
	';
	...
	const REPORT_SQL = '
		select
			...
			Report_Rowversion as "Report_Rowversion"
		from rpt.Report Report';
	//const REPORTCATALOG_SQL = 'select * from rpt.v_ReportCatalog ReportCatalog  ';
	const REPORTCATALOG_SQL = "
		select
			ReportCatalog.ReportCatalog_id as \"ReportCatalog_id\",
			ReportCatalog.ReportCatalog_Name as \"ReportCatalog_Name\",
			ReportCatalog.ReportCatalog_Status as \"ReportCatalog_Status\",
			ReportCatalog.ReportCatalog_Path as \"ReportCatalog_Path\",
			ReportCatalog.ReportCatalog_Position as \"ReportCatalog_Position\",
			ReportCatalog.pmUser_insID as \"pmUser_insID\",
	...
	const REPORTCONTENTFIELDSET_SQL = "
		...
		from rpt.ReportContentParameter ReportContentFieldset";
	const REPORTCONTENTPARAMETER_SQL = '
		...
		from rpt.ReportContentParameter ReportContentParameter';
	// const REPORTPARAMETER_SQL = 'select * from rpt.ReportParameter ReportParameter  ';
	const REPORTPARAMETER_SQL = "
		select
			ReportParameter.ReportParameter_id as \"ReportParameter_id\",
			ReportParameter.ReportParameterCatalog_id as \"ReportParameterCatalog_id\",
			ReportParameter.ReportParameter_Name as \"ReportParameter_Name\",
			ReportParameter.ReportParameter_Type as \"ReportParameter_Type\",
			(
				ReportParameter.ReportParameter_Label || ' (' ||
				case when ReportParameter.Region_id is not null then dbo.GetRegionNamebyCode(ReportParameter.Region_id) else '' end
				|| ')'
			) as \"ReportParameter_Label\",
			ReportParameter.ReportParameter_Mask as \"ReportParameter_Mask\",
			ReportParameter.ReportParameter_Length as \"ReportParameter_Length\",
			ReportParameter.ReportParameter_MaxLength as \"ReportParameter_MaxLength\",
			ReportParameter.ReportParameter_Default as \"ReportParameter_Default\",
			ReportParameter.ReportParameter_Align as \"ReportParameter_Align\",
			ReportParameter.ReportParameter_CustomStyle as \"ReportParameter_CustomStyle\",
			ReportParameter.ReportParameter_SQL as \"ReportParameter_SQL\",
			ReportParameter.ReportParameter_SQL_IdField as \"ReportParameter_SQL_IdField\",
			ReportParameter.ReportParameter_SQL_TextField as \"ReportParameter_SQL_TextField\",
			ReportParameter.ReportParameter_SQL_XTemplate as \"ReportParameter_SQL_XTemplate\",
			ReportParameter.pmUser_insID as \"pmUser_insID\",
			ReportParameter.pmUser_updID as \"pmUser_updID\",
			ReportParameter.ReportParameter_insDT as \"ReportParameter_insDT\",
			ReportParameter.ReportParameter_updDT as \"ReportParameter_updDT\",
			ReportParameter.Region_id as \"Region_id\"
		from rpt.ReportParameter";
	...
	public function getReport($params){
		...
		if (!empty($params['ReportContent_id'])) {
			$query = "
				...
				from 
					rpt.ReportContent RC
					inner join rpt.Report R on R.Report_id = RC.Report_id
				where
		...
		if($params['ownerId'] && $params['ownerId'] != 'all'){
			$sql = '
				select
					...
					ReportContent_insDT as "ReportContent_insDT",
					ReportContent_updDT as "ReportContent_updDT"
				from rpt.v_ReportContent ReportContent
				where ReportContent_Name is not null and
					  Report_id = ?
				order by ReportContent_Position
			';
			...
			if($_REQUEST['isFieldset']){
				$sql = '
					select
						...
						COALESCE(cast (a.ReportContentParameter_ReportId as varchar (200)), c.ReportParameter_Name || \'<span class="x-grid-gray-text"> - наследуется </span>\') as "originalId",
						COALESCE(cast (a.ReportContentParameter_ReportLabel as varchar (200)), c.ReportParameter_Label || \'<span class="x-grid-gray-text"> - наследуется </span>\') as "originalLabel",
						COALESCE(cast (a.ReportContentParameter_Default as varchar (200)), c.ReportParameter_Default || \'<span class="x-grid-gray-text"> - наследуется </span>\') as "originalDefault"
					from rpt.ReportContentParameter a
						 join rpt.ReportContent b on a.ReportContent_id = b.ReportContent_id
						 join rpt.ReportParameter c on c.ReportParameter_id = a.ReportParameter_id
	...
	public function getParametersCombo($params){
		...
		if (!empty($_REQUEST['reportId'])) {
			...
			";
			$filter .= " 
				and coalesce(R.Region_id, 0) = coalesce(ReportParameter.Region_id, 0)
			";
	...
	protected function getDependences($sql,$params) {
		...
		if(preg_match_all('/\@([a-zA-Z_]+)/',$sql,$matches)) {
			...
			foreach($depends as $id){
				/*$result = sqlsrv_query(
						$conn,
						"select ReportParameter_Default from rpt.ReportParameter ReportParameter  where ReportParameter_Name = ?",
						array($id)
				);*/
				$result = @$this->db->query(
						"select ReportParameter_Default  as \"ReportParameter_Default\" from rpt.ReportParameter ReportParameter  where ReportParameter_Name = ?",
						array($id)
				);
				if(!$result){
	...
	protected function getFields($conn,$sql,$Region_id){
		$parameters = [];
		list($sql,$parameters) = $this->prepareSql($conn,$sql,$Region_id);
		$prep = pg_query($conn, $sql);
		if(!$prep) throw new Exception(self::getErrors());

		$count = pg_num_fields($prep);
		$temp = [];

		for($i = 0; $i < $count; $i++) {
			$temp[] = pg_field_name($prep, $i);
		}
	 ...
	 */
	const REPORT_ENGINE_SQL = '
		select
			Report.Report_id as "Report_id",
			Report.ReportCatalog_id as "ReportCatalog_id",
			Report.Report_Caption as "Report_Caption",
			Report.Report_Description as "Report_Description",
			Report.Report_Title as "Report_Title",
			Report.Report_FileName as "Report_FileName",
			Report.Report_Status as "Report_Status",
			Report.Report_Position as "Report_Position",
			Report.pmUser_insID as "pmUser_insID",
			Report.pmUser_updID as "pmUser_updID",
			Report.Report_insDT as "Report_insDT",
			Report.Report_updDT as "Report_updDT",
			Report.Region_id as "Region_id",
			Report.ReportType_id as "ReportType_id",
			Report.DatabaseType as "DatabaseType",
			Report.Report_Rowversion as "Report_Rowversion",
			ReportCatalog.ReportCatalog_id as "ReportCatalog_id",
			ReportCatalog.ReportCatalog_pid as "ReportCatalog_pid",
			ReportCatalog.ReportCatalog_Name as "ReportCatalog_Name",
			ReportCatalog.ReportCatalog_Status as "ReportCatalog_Status",
			ReportCatalog.ReportCatalog_Path as "ReportCatalog_Path",
			ReportCatalog.ReportCatalog_Position as "ReportCatalog_Position",
			...
			ReportCatalog.ReportCatalog_updDT as "ReportCatalog_updDT",
			ReportCatalog.Region_id as "Region_id",
			ReportCatalog.ReportCatalog_rid as "ReportCatalog_rid"
		from rpt.Report Report
			 join rpt.ReportCatalog ReportCatalog on Report.ReportCatalog_id = ReportCatalog.ReportCatalog_id
		where Report_id = ?
	';
	//const PARAMETER_ENGINE_SQL = 'select * from rpt.v_ReportContent  where reportid = ? order by position,inPosition';
	...
	const PARAMETER_ENGINE_SQL = "
		select
			...
			COALESCE(RCP.ReportContentParameter_Default, RP.ReportParameter_Default) AS \"default\",
			...
			COALESCE(RCP.ReportContentParameter_ReportLabel, RP.ReportParameter_Label) AS \"label\",
			COALESCE(RCP.ReportContentParameter_ReportId, RP.ReportParameter_Name) AS \"id\",
			...
			case
				when coalesce(RCP.ReportContentParameter_SQL, '') != '' then RCP.ReportContentParameter_SQL
				else RP.ReportParameter_SQL 
			end AS \"sql\",
			lower(case 
				when coalesce(RCP.ReportContentParameter_SQLIdField, '') != '' and coalesce(RCP.ReportContentParameter_SQL, '') != '' then RCP.ReportContentParameter_SQLIdField
				else RP.ReportParameter_SQL_IdField
			end) AS \"idField\",
			lower(case
				when coalesce(RCP.ReportContentParameter_SQLTextField, '') != '' and coalesce(RCP.ReportContentParameter_SQL, '') != '' then RCP.ReportContentParameter_SQLTextField
			...
			RCParamArmStr.RCParamArmStr_Items as \"RCParameterArm\",
			CASE 
				WHEN RC.ReportContent_Name IS NULL THEN RCP.ReportContentParameter_Position
				ELSE RC.ReportContent_Position 
			END AS \"position\"
		from 
			rpt.ReportContent RC 
			INNER JOIN rpt.ReportContentParameter RCP  ON RCP.ReportContent_id = RC.ReportContent_id
			INNER JOIN rpt.ReportParameter RP  ON RP.ReportParameter_id = RCP.ReportParameter_id
			left join lateral (				
				select string_agg(AT.ARMType_SysNick, ',') as RCParamArmStr_Items
				from rpt.v_ReportContentParameterLink RCPL
				inner join v_ARMType AT on AT.ARMType_id = RCPL.ARMType_id
				where RCPL.ReportContentParameter_id = RCP.ReportContentParameter_id
			) RCParamArmStr on true
		where
			RC.report_id = ?
			and (
				RP.Region_id = dbo.getRegion()
				or exists (
					select ReportList_id
					from 
						rpt.ReportList
					where 
	...
	const PARAMETER_LPU_DEFAULT_SQL = '
		select
			Lpu_id as "Lpu_id",
			Lpu_IsOblast as "Lpu_IsOblast",
			Lpu_Nick as "Lpu_Nick",
			COALESCE(case when cast (to_number(substring(ltrim(CAST(Lpu_Ouz as text)), 3, 2)) as int) <= 7 then 1
				else cast (to_number(substring(ltrim(CAST(Lpu_Ouz as text)), 3, 2)) as int)
			end, - 1) as "OMSSprTerrPrm"
		from v_Lpu
		where Lpu_id = ?';
	...
	public function getReportContentEngine($params) {
		...
		$sql_region = "select Region_id  as \"Region_id\" from rpt.Report  where Report_id = :Report_id";
	...
	const GET_PARAMETER_ENGINE_SQL = '
		select ReportParameter_id as "ReportParameter_id",
			   ReportParameterCatalog_id as "ReportParameterCatalog_id",
			   ReportParameter_SQL_XTemplate as "ReportParameter_SQL_XTemplate",
			   pmUser_insID as "pmUser_insID",
			   pmUser_updID as "pmUser_updID",
			   ...
			   ReportParameter_SQL_IdField as "ReportParameter_SQL_IdField",
			   ReportParameter_SQL_TextField as "ReportParameter_SQL_TextField"
		from rpt.ReportParameter RP
		where ReportParameter_Name = ?
			  and coalesce(Region_id, 0) = coalesce(cast(? as bigint), 0)
	';
	const CHECK_UNIQUE_REPORT_CAPTION = '
		select
			Report_id as "Report_id",
			ReportCatalog_id as "ReportCatalog_id",
			Report_Caption as "Report_Caption",
		...
		where
			Report_Caption = ? and
			coalesce(Report_id, 0) <> coalesce(cast(? as bigint), 0) 
			and coalesce(Region_id, 0) = (
				select coalesce(Region_id, 0) as Region_id 
				from rpt.ReportCatalog 
				where ReportCatalog_id = ?
			)
	';
	...
	public function getParameterContentEngine($params){
		...
		$sql_region = "select R.Region_id  as \"Region_id\" from rpt.ReportContent RC  left join rpt.Report R  on R.Report_id = RC.Report_id where RC.ReportContent_id = :ReportContent_id";
		$result_region = @$this->db->query($sql_region,$params_region);
		if(is_object($result_region)){
			$result_region = $result_region->result('array');
			if(is_array($result_region) && count($result_region) > 0){
		...
		$query = "
			...
			from 
				rpt.ReportContentParameter RCP
			where
	...
	public function copyArmAccess($Report_id, $ReportContentParameter_id) {
		$query = "
			...
			from
				rpt.ReportARM RA
			where
				RA.Report_id = CAST(NULLIF(:Report_id, '') as bigint)
		";
		...
		if (is_object($resp)) {
			...
			if (!empty($result)) {
				...
				$query = "
					select
						ReportContentParameterLink_id as \"ReportContentParameterLink_id\",
						Error_Code as \"Error_Code\",
						Error_Message as \"Error_Msg\"
					from rpt.p_ReportContentParameterLink_ins (
		...
		if(($mode == 'edit') && ($name == 'ReportCatalog')){
			if (!$_REQUEST['ReportCatalog_pid']){
				...
				$sql_reportcatalog_pid = "
									select ReportCatalog_pid as \"ReportCatalog_pid\"
									from rpt.ReportCatalog
			...
			case 'add' :
				if($name == 'Report'){ //Если добавляем или редактируем отчет
                    ...
                    if(isset($_REQUEST['ReportCatalog_id']) && ($_REQUEST['ReportCatalog_id'] <> 0)){
                        $and .= ' and coalesce(Region_id, 0) = (select coalesce(Region_id, 0) as Region_id from rpt.ReportCatalog where ReportCatalog_id = '.$_REQUEST['ReportCatalog_id'].')';
                    }
					$query = "
				...
				foreach ($_REQUEST as $key=>$value) {
					...
					switch ($key) {
						case 'ReportCatalog_pid':
							$params .= "{$key} := cast(:{$key} as bigint),\n";
							break;
						case 'Report_Position':
						case 'ReportCatalog_Position':
						case 'Report_id':
						case 'ReportContent_id':
							if (empty($value)) {
								$value = null;
							}
							$params .= "{$key} := :{$key},\n";
					...
					if (($key == 'ReportType_id' || $key == 'Report_id') && ($value == '')) {
						$values[$key] = null;
					} else {
						$values[$key] = $value === null ? null : toAnsi($value);
					}
				}
				//Получим регион:
				$region_id = '';
				...
				$id = ($sfx == 'ins') ? "{$name}_id := null," : "{$name}_id := CAST(NULLIF(?, '') as bigint),";
				$query = "
					SELECT
						{$name}_id as \"insertId\"
					from rpt.p_{$name}_{$sfx} (
						{$id}
						{$params}
						{$region_id}
						pmUser_id := {$pmUser_id}
		...
		try {
			...
			if(!$result) {
				...
				if (!empty($Object_id) && $mode != 'delete') {
					...
					//	'Object_id' => $Object_id
					//);
					$this->db->query($query, ['Object_id' => $Object_id]);
					...
					if ( !empty($Region_ids) ) {
						...
						foreach ($diffRegion_ids as $key => $value) {
							$query = "
								select
									ReportList_id as \"ReportList_id\",
									Error_Code as \"Error_Code\",
									Error_Message as \"Error_Msg\"
								from rpt.p_ReportList_ins (
									ReportCatalog_id := :ReportCatalog_id::bigint,
									Report_id := :Report_id::bigint,
									ReportContentParameter_id := :ReportContentParameter_id::bigint,
									Region_id := :Region_id,
									pmUser_id := :pmUser_id
								)
							";
	 ...
	 * @return string
	 */
	public function getRegionNumberByName($Region_Name){
		switch($Region_Name){
			case 'Perm':
				return '59';
				break;
	...
	public function moveReportParametersUp($ReportContentParameter_id){
		...
		$response = new JsonResponce();
		$sql = "SELECT ReportContent_id as \"ReportContent_id\", ReportContentParameter_Position  as \"ReportContentParameter_Position\" FROM rpt.ReportContentParameter ReportContentParameter  WHERE ReportContentParameter_id = ?";
		...
		if(is_array($rsp) && count($rsp) > 0){
			...
			$sql = "SELECT Report_id  as \"Report_id\" FROM rpt.ReportContent ReportContent  WHERE ReportContent_id = ?";
			...
			if ( is_array($rsp) && count($rsp) > 0 ) {
				...
				$sql = "UPDATE rpt.ReportContentParameter
						...
						WHERE ReportContentParameter_Position > ? AND ((SELECT COUNT(ReportContent_id) FROM rpt.v_ReportContent ReportContent  WHERE Report_Id = ? AND ReportContent_id = ReportContentParameter.ReportContent_id) > 0)";
	...
	public function moveReportParametersDown($position, $Report_id, $mode){
		...
		if ($mode=='edit')
		{
			...
			$sql = "SELECT ReportContentParameter_Position  as \"ReportContentParameter_Position\" FROM rpt.ReportContentParameter ReportContentParameter
					WHERE ReportContentParameter_id = ? AND ((SELECT COUNT(ReportContent_id) FROM rpt.ReportContent ReportContent  WHERE Report_Id = ? AND ReportContent_id = ReportContentParameter.ReportContent_id) > 0)";
		...
		$sql = "SELECT ReportContentParameter_id  as \"ReportContentParameter_id\" FROM rpt.ReportContentParameter ReportContentParameter
				WHERE ReportContentParameter_id <> ? AND ReportContentParameter_Position = ? AND ((SELECT COUNT(ReportContent_id) FROM rpt.ReportContent ReportContent  WHERE ReportContent_id = ReportContentParameter.ReportContent_id) > 0)";
		// Report_Id = ? AND 
		...
		if ( is_array($rsp) && count($rsp) > 0 ) {
			...
			$sql = "UPDATE rpt.ReportContentParameter
					SET ReportContentParameter_Position = ReportContentParameter_Position + 1
					WHERE ReportContentParameter_Position >= '{$position}' {$where} AND ((SELECT COUNT(ReportContent_id) FROM rpt.ReportContent ReportContent  WHERE Report_Id = ? AND ReportContent_id = ReportContentParameter.ReportContent_id) > 0)";
	...
	public function moveDatasetDown($position, $Report_id, $mode) {
		...
		if ($mode=='edit') 
		{
			...
			// получаем старую позицию, если она больше новой, то сдвигаем все остальные параметры на +1 только до этой позици
			$sql = "
				SELECT ReportContent_Position as \"ReportContent_Position\"
				FROM 
					rpt.ReportContent RC
				WHERE 
		...
		$sql = "
			...
			FROM 
				rpt.ReportContent ReportContent
			WHERE 
				ReportContent_id <> :ReportContent_id 
	...
	public function moveReportUp($Report_id){
		//$conn = $this->getConnection($_REQUEST['serverId']);
		$response = new JsonResponce();
		$sql = "SELECT ReportCatalog_id as \"ReportCatalog_id\", Report_Position  as \"Report_Position\" FROM rpt.Report Report  WHERE Report_id = ?";
	...
	public function moveReportDown($position, $ReportCatalog_id, $mode){
		...
		if ($mode=='edit')
		{
			...
			// получаем старую позицию, если она больше новой, то сдвигаем все остальные параметры на +1 только до этой позици
			$sql = "SELECT Report_Position  as \"Report_Position\" FROM rpt.Report Report  WHERE Report_id = ?";
			//$result = sqlsrv_query($conn,$sql, array($Report_id));
			//$res = sqlsrv_fetch_array($result,SQLSRV_FETCH_ASSOC);
			$result = @$this->db->query($sql, array($Report_id));
			if(!$result) return $response->error(self::getErrors())->json();
		...
		// смотрим есть ли такая позиция уже.
		$sql = "SELECT Report_id  as \"Report_id\" FROM rpt.Report Report  WHERE Report_id <> ? AND Report_Position = cast(? as bigint) AND ReportCatalog_id = ?";
		//$result = sqlsrv_query($conn,$sql, array($Report_id, $position, $ReportCatalog_id));
		$result = @$this->db->query($sql, array($Report_id, empty($position) ? null: $position, $ReportCatalog_id));
		if(!$result) return $response->error(self::getErrors())->json();
		$rsp = $result->result('array');
		...
		if ( is_array($rsp) && count($rsp) > 0 ) {
			...
			// прибавляем 1 всем
			$sql = "UPDATE rpt.Report SET Report_Position = Report_Position + 1
	...
	public function moveReportCatalogUp($ReportCatalog_id)
	{
		...
		$sql_RC_pid = "SELECT ReportCatalog_pid  as \"ReportCatalog_pid\" FROM rpt.ReportCatalog  WHERE ReportCatalog_id = ?";
		...
		if ( is_array($rsp) && count($rsp) > 0 ) {
			...
			$sql = "UPDATE rpt.ReportCatalog SET ReportCatalog_Position = ReportCatalog_Position - 1
					WHERE ReportCatalog_Position > (SELECT ReportCatalog_Position FROM rpt.v_ReportCatalog  WHERE ReportCatalog_id = ?)".$filter_RC_pid;
	...
	public function moveReportCatalogDown($ReportCatalog_id,$mode,$position,$ReportCatalog_pid)
	{
		...
		if ($mode == 'edit')
		{
			...
			$sql = "SELECT ReportCatalog_Position  as \"ReportCatalog_Position\" from rpt.ReportCatalog  where ReportCatalog_id = ?";
		...
		$sql = "SELECT ReportCatalog_id  as \"ReportCatalog_id\" FROM rpt.v_ReportCatalog  WHERE ReportCatalog_id <> ? AND ReportCatalog_Position = ?".$filter_RC_pid;
	...
	public function getFormats($params){
		...
		if($getall){
			$sql = "
				SELECT
					RF.ReportFormat_id as \"ReportFormat_id\",
					RF.ReportFormat_Name as \"ReportFormat_Name\",
					RF.ReportFormat_Ext as \"ReportFormat_Ext\",
					RF.ReportFormat_Icon as \"ReportFormat_Icon\",
					RFL.ReportFormat_Sort as \"ReportFormat_Sort\",
					YN.YesNo_id as \"YesNo_id\"
				FROM
					rpt.ReportFormatList RFL
					LEFT JOIN rpt.ReportFormat RF ON RF.ReportFormat_id = RFL.ReportFormat_id
					LEFT JOIN v_YesNo YN ON YN.YesNo_Code = CASE
	...
	public function checkReportFormatList($params){
		...
		$sql = "
			...
			FROM rpt.ReportFormat ReportFormat
			WHERE not exists (
				SELECT ReportFormatList_id
				FROM rpt.ReportFormatList
	...
	public function disableReportFormat($params){
		...
		$sql = "UPDATE rpt.ReportFormatList SET ReportFormat_Disabled = ? WHERE Report_id = ? AND ReportFormat_Id = ?";
	...
	public function changePositionFormat($params){
		...
		$sql = "SELECT ReportFormat_Sort as \"ReportFormat_Sort\" FROM rpt.ReportFormatList  WHERE Report_id = ? AND ReportFormat_Id = ? LIMIT 1";
		...
		if ($changePositionflag==0) {
			...
			$sql = "SELECT ReportFormat_Id as \"ReportFormat_Id\", ReportFormat_Sort  as \"ReportFormat_Sort\" FROM rpt.ReportFormatList  WHERE Report_id = ? AND ReportFormat_Sort < ? ORDER BY ReportFormat_Sort DESC LIMIT 1";
			...
			if ( is_array($rsp) && count($rsp) > 0 ) {
				...
				$sql = "UPDATE rpt.ReportFormatList SET ReportFormat_Sort = ? WHERE Report_id = ? AND ReportFormat_Id = ?";
			...
			$sql = "SELECT ReportFormat_Id as \"ReportFormat_Id\", ReportFormat_Sort  as \"ReportFormat_Sort\" FROM rpt.ReportFormatList  WHERE Report_id = ? AND ReportFormat_Sort > ? ORDER BY ReportFormat_Sort ASC LIMIT 1";
			//$result = sqlsrv_query($conn,$sql,array($reportId, $ReportFormat_Sort));
			$result = @$this->db->query($sql,array($reportId, $ReportFormat_Sort));
			if(!$result) return $response->error(self::getErrors())->json();
			$rsp = $result->result('array');
	...
	public function RunReportDBF($data)	{
		...
		$NumTabName = (($data['NumTab'] == '306') ? '5300' : $data['NumTab']);
	...
	public function ajaxcheckParamId($params){
		...
		if (empty($RegionId)) {
			$query = 'select * from rpt.ReportParameter ReportParameter where ReportParameter_Name = ? and Region_id is null';
	...
	public function setReportCatalog($data){
		...
		$query = "
			select Report_id as \"Report_id\", Error_Code as \"Error_Code\", Error_Message as \"Error_Msg\"
			from rpt.{$proc}(
				Report_id := :Report_id,
	...
	public function getAllReportARM($data){
		$ARMQuery = "
			...
			from rpt.ReportARM as ARM
			where ARM.Report_id =CAST(NULLIF(:Report_id, '') as bigint)";
	...
	public function createReportARM($data){
		$query = "
			select ReportARM_id as \"ReportARM_id\", Error_Code as \"Error_Code\", Error_Message as \"Error_Msg\"
			from rpt.p_ReportARM_ins(
				ReportARM_id := null,
				ARMType_id := :ARMType_id,
				Report_id := :Report_id,
				pmUser_id := :pmUser_id)
	...
	public function getAllReportContent($data){
		$ARMQuery = "
			select ReportContent_id as \"ReportContent_id\",
				   Report_id as \"Report_id\",
				   ReportContent_Name as \"ReportContent_Name\",
				   ReportContent_Position as \"ReportContent_Position\",
				   pmUser_insID as \"pmUser_insID\",
	...
	public function getAllReportContentParam($allRContentIDs){
		...
		$RCParamsQuery = "
			select
				...
				ReportContentParameter_insDT as \"ReportContentParameter_insDT\",
				ReportContentParameter_updDT as \"ReportContentParameter_updDT\"
			from rpt.ReportContentParameter as RCP
	...
	public function getAllReportParam($allRParamIDs){
		...
		$RParamsQuery = "
			select
				...
				ReportParameter_SQL_IdField as \"ReportParameter_SQL_IdField\",
				ReportParameter_SQL_TextField as \"ReportParameter_SQL_TextField\"
			from rpt.ReportParameter as RP
	...
	public function getRegionParamsByNames($data){
		...
		$RParamsNameQuery = "select
							rp.ReportParameter_id as \"ReportParameter_id\",
							rp.ReportParameter_Name as \"ReportParameter_Name\"
					  	from rpt.ReportParameter as rp
					  	where coalesce(rp.Region_id, 0) = coalesce(cast(:Region_id as bigint), 0)
	...
	public function copyReport($data){
		...
		if (empty($allReportContent)) {
			$this->commitTransaction();
			return array('NewReport_id' => $NewReport_id, 'success' => true);
		}
		...
		foreach($allReportContentParam as $rContentParam){
			...
			$rContentParam['ReportContent_id'] = (!empty($allNewRContentIDs[$rContentParam['ReportContent_id']])) ? $allNewRContentIDs[$rContentParam['ReportContent_id']] : null;
	...
	public function createReportContent($data){
		$query = "
			select
				null as \"ReportContent_id\",
				Error_Code as \"Error_Code\",
				Error_Message as \"Error_Msg\"
			from rpt.p_ReportContent_ins (
				ReportContent_id := null,
				Report_id := :Report_id,
	...
	public function createReportParameter($data){
		$query = "
			select
				null as \"ReportParameter_id\",
				Error_Code as \"Error_Code\",
				Error_Message as \"Error_Msg\"
			from rpt.p_ReportParameter_ins (
				ReportParameter_id := null,
	...
	public function createReportContentParameter($data){
		...
		$query = "
			select
				...
				Error_Code as \"Error_Code\",
				Error_Message as \"Error_Msg\"
			from rpt.p_ReportContentParameter_ins (
				ReportContentParameter_id := null::bigint,
				ReportParameter_id := :ReportParameter_id::bigint,
				ReportContent_id := :ReportContent_id::bigint,
				Report_id := :Report_id::bigint,
