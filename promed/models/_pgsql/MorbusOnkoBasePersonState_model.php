 ...
 */
class MorbusOnkoBasePersonState_model extends swPgModel
{
	/**
	 * Пользователь
	 * @var integer
	...
	public function read()
	{
		...
		$sql = "
			SELECT
				MorbusOnkoBasePersonState_id as \"MorbusOnkoBasePersonState_id\",
				MorbusOnkoBase_id as \"MorbusOnkoBase_id\",
				to_char(MorbusOnkoBasePersonState_setDT, 'dd.mm.yyyy') as \"MorbusOnkoBasePersonState_setDT\",
				OnkoPersonStateType_id as \"OnkoPersonStateType_id\"
			FROM
				dbo.v_MorbusOnkoBasePersonState
			WHERE
				MorbusOnkoBasePersonState_id = ?
		";
	...
	public function destroy($data = array())
	{
		...
		$sql = '
		    select
				Error_Code as "Error_Code",
				Error_Message as "Error_Msg"
			from dbo.p_MorbusOnkoBasePersonState_del (
				MorbusOnkoBasePersonState_id := :MorbusOnkoBasePersonState_id,
				pmUser_id := :pmUser_id
			)
		';
	...
	function getViewData($data = array())
	{
		...
		$query = "
			SELECT
				...
				to_char(MOBPS.MorbusOnkoBasePersonState_setDT, 'dd.mm.yyyy') as \"MorbusOnkoBasePersonState_setDT\",
				OPST.OnkoPersonStateType_id as \"OnkoPersonStateType_id\",
			...
			FROM
				...
				INNER JOIN dbo.v_MorbusOnkoBasePersonState MOBPS on MOB.MorbusOnkoBase_id = MOBPS.MorbusOnkoBase_id
				INNER JOIN dbo.v_OnkoPersonStateType OPST ON MOBPS.OnkoPersonStateType_id = OPST.OnkoPersonStateType_id
			where
