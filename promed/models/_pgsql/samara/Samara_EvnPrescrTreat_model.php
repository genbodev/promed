			...
			if (empty($data['EvnCourseTreat_id'])) {
				...
				$query = "
					select
						...
						EPT.EvnPrescrTreat_FactCount as \"EvnPrescrTreat_FactCount\",
					...
					from
						...
						left join lateral(
							...
							inner join EvnDrug ED on ED.EvnPrescrTreatDrug_id = EPTD.EvnPrescrTreatDrug_id
				...
				foreach ($ep_list as $row) {
					if (is_object($row['EvnPrescrTreat_setDT'])) {
						if ($row['EvnPrescrTreat_setDT'] instanceof DateTime) {
							...
							$row['EvnPrescrTreat_setDT'] = ConvertDateFormat($var, 'Y-m-d');
						}
					}
	...
	public function doLoadEvnCourseTreatEditForm($data) {
		$query = "
			select
				...
				coalesce(mu.SHORTNAME, cu.SHORTNAME) as \"EdUnits_Nick\",
				ECTD.EvnCourseTreatDrug_KolvoEd as \"KolvoEd\",
				coalesce(df.NAME,D.DrugForm_Name) as \"DrugForm_Name\",
				...
				ECT.EvnCourseTreat_FactCount as \"EvnCourseTreat_FactCount\",
                ...
                ECT.EvnCourseTreat_IsPrescrInfusion as \"EvnCourseTreat_IsPrescrInfusion\",
				ECT.PerformanceType_id as \"PerformanceType_id\",
				ECT.PrescriptionIntroType_id as \"PrescriptionIntroType_id\",
				case when coalesce(EPT.EvnPrescrTreat_IsCito, 1) = 1
					then 'off'
					else 'on'
				end as E\"vnPrescrTreat_IsCito\",
