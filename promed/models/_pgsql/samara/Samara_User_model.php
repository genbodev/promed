<?php
/**
* Samara_User_model - модель для работы с учетными записями пользователей (Самара)
*
...
class Samara_User_model extends User_model {
    /**
     * Конструктор
     */
		...
		if ($data['session']['orgtype']!='lpu') { // если это не ЛПУ 
			...
			if (havingGroup('orgadmin')) {
				...
				$sql = " 
					...
					where
						MS.Org_id = :Org_id 
						and MS.MedService_begDT <= cast(dbo.tzgetdate() as date)
						and (MS.MedService_endDT >= cast(dbo.tzgetdate() as date) or MS.MedService_endDT is null) 
						{$filter}
				";
			} else {
				$sql = " 
			...
			$use_date = true;
			if ($use_date) {
				$filter_medstafffact = 'and cast(msf.WorkData_begDate as date) <= cast(dbo.tzgetdate() as date)
				and (cast(msf.WorkData_endDate as date) >= cast(dbo.tzgetdate() as date) or msf.WorkData_endDate is null)';
				$filter_medservicemedpersonal = 'and (cast(msmp.MedServiceMedPersonal_begDT as date) <= cast(dbo.tzgetdate() as date)
				and (cast(msmp.MedServiceMedPersonal_endDT as date) >= cast(dbo.tzgetdate() as date) or msmp.MedServiceMedPersonal_endDT is null))';
				$filter_medservice = 'and cast(MS.MedService_begDT as date) <= cast(dbo.tzgetdate() as date)
				and (cast(MS.MedService_endDT as date) >= cast(dbo.tzgetdate() as date) or MS.MedService_endDT is null)';
			}
			$filter = '';
			...
			$sql_medservice = "
				...
				where
					...
					and msmp.MedPersonal_id = :MedPersonal_id
					and mst.MedServiceType_SysNick in ('HTM', 'vk', 'mse', 'lab', 'pzm', 'func', 'patb', 'mstat', 'prock', 'dpoint', 'merch', 'regpol', 'sprst', 'okadr', 'minzdravdlo', 'leadermo', 'mekllo', 'spesexpertllo', 'adminllo', 'touz', 'reglab', 'oper_block', 'smp', 'slneotl', 'konsult', 'foodserv', 'vac', 'lvn')
			";
				...
				FROM
					...
					inner join lateral(
						...
						where 
							...
							and WorkData_begDate <= cast(dbo.tzgetdate() as date)
							and (WorkData_endDate is null or WorkData_endDate >= cast(dbo.tzgetdate() as date))
						limit 1
					) msf on true
					left join v_PostMed ps on ps.PostMed_id = msf.Post_id
				WHERE
					lut.LpuUnitType_SysNick in ('stac','hstac','pstac','dstac')
					and lsp.LpuSectionProfile_SysNick = 'priem'
					and ls.Lpu_id = :Lpu_id
					and coalesce(ls.LpuSection_setDate, cast(dbo.tzgetdate() as date)) <= cast(dbo.tzgetdate() as date)
					and coalesce(ls.LpuSection_disDate, cast(dbo.tzgetdate() as date)) >= cast(dbo.tzgetdate() as date)
					{$filter}
			";
