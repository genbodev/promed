...
require_once(APPPATH.'models/_pgsql/Search_model.php');
...
class Samara_Search_model extends Search_model {
	...
	function searchData($data, $getCount = false, $print = false, $dbf = false) {
		...
		if (isset($data['and_kvsperson']) && $data['and_kvsperson']) {
			if ( $dbf === true ) {
				$query .= "
					...
					rtrim(coalesce(to_char({$PS_prefix}.Person_BirthDay, 'dd.mm.yyyy'),'')) as \"BIRTHDAY\",
					{$PS_prefix}.Person_Snils as \"SNILS\",
					PrsIsInv.YesNo_Code as \"INV_N\",
					PrsInvD.Diag_Code as \"INV_DZ\",
					rtrim(coalesce(to_char(PrsPCh.PersonChild_invDate, 'dd.mm.yyyy'),'')) as \"INV_DATA\",
					...
					rtrim(coalesce(to_char(PrsPol.Polis_begDate, 'dd.mm.yyyy'),'')) as \"P_DATA\",
					...
					rtrim(coalesce(to_char(PrsDoc.Document_begDate, 'dd.mm.yyyy'),'')) as \"D_DATA\",
				";
			}
		}
		...
		switch ( $data['SearchFormType'] ) {
			case 'CmpCallCard':
				...
				$query .= "
					...
					case when coalesce(CClC.CmpCloseCard_id,0)=0 then 0 else CClC.CmpCloseCard_id end as \"CmpCloseCard_id\",
					...
					coalesce(dbo.Age2(PS.Person_BirthDay, dbo.tzGetDate()), CCC.Person_Age) as \"Person_Age\",
					...
					COALESCE(PAddr.Address_Address, RTRIM(replace(CCC.CmpCallCard_PCity, '=', ''))
			...
			case 'PersonDopDisp':
				...
				$query .= "
					...
					PS.Person_id as \"Person_id\",
					coalesce(epldd.Server_id, PS.Server_id) as \"Server_id\", -- проблема при редактировании талона по ДД (#6670) - с регистра передается текущая, с поиска талона - та которая в талоне.
					coalesce(epldd.PersonEvn_id, PS.PersonEvn_id) as \"PersonEvn_id\", -- сделал так, что если есть талон, то бралась периодика с талона
			...
			case 'PersonDispOrpProf':
				...
				$query .= "
					...
					to_char(cast(DOr.PersonDispOrp_begDate as timestamp), 'dd.mm.yyyy') as \"PersonDispOrp_begDate\",
					EPLDTI.EvnPLDispTeenInspection_id as \"EvnPLDispTeenInspection_id\",
			...
			case 'EvnPLDispDop13':
				...
				$query .= "
					...
					case when exists (select EvnPLDispDop13_id from v_EvnPLDispDop13 where Person_id = PS.Person_id and date_part('year', EvnPLDispDop13_setDate limit 1) = :PersonDopDisp_Year and Lpu_id <> :Lpu_id) then 4 else 0 end as \"AccessType_Code\",
			...
			case 'EvnPLDispProf':
				...
				$query .= "
					...
					to_char(EPLDP.EvnPLDispProf_consDT, 'dd.mm.yyyy') as \"EvnPLDispProf_setDate\",
					...
					case when DDICData.DopDispInfoConsent_IsAgree = 1 then to_char(EPLDP.EvnPLDispProf_consDT, 'dd.mm.yyyy') else null end as \"EvnPLDispProf_rejDate\",
					case when exists (select EvnPLDispProf_id from v_EvnPLDispProf where Person_id = PS.Person_id and date_part('year', EvnPLDispProf_setDate) = :PersonDopDisp_Year and Lpu_id <> :Lpu_id limit 1) then 4 else 0 end as \"AccessType_Code\"
				";
			break;
			...
			case 'EvnPLDispTeen14':
				...
				$query .= "
					...
					to_char(EPLDT14.EvnPLDispTeen14_setDate, 'dd.mm.yyyy') as \"EvnPLDispTeen14_setDate\",
					to_char(EPLDT14.EvnPLDispTeen14_disDate, 'dd.mm.yyyy') as \"EvnPLDispTeen14_disDate\"
				";
			break;
			...
			case 'EvnPLDispOrp':
				...
				$query .= "
					...
					to_char(EPLDO.EvnPLDispOrp_setDate, 'dd.mm.yyyy') as \"EvnPLDispOrp_setDate\",
					to_char(EPLDO.EvnPLDispOrp_disDate, 'dd.mm.yyyy') as \"EvnPLDispOrp_disDate\"
				";
			break;
			...
			case 'EvnUsluga':
				...
				if ( $dbf === true ) {
					$query .= "
						...
						RTRIM(coalesce(to_char(EvnUsluga.EvnUsluga_setDT, 'dd.mm.yyyy'), '')) as \"SETDATE\",
			...
			case 'EvnSection':
				...
				if ( $dbf === true ) {
					$query .= "
						...
						RTRIM(coalesce(to_char(ESEC.EvnSection_setDate, 'dd.mm.yyyy'), '')) as \"SETDATE\",
						ESEC.EvnSection_setTime as \"SETTIME\",
						RTRIM(coalesce(to_char(ESEC.EvnSection_disDate, 'dd.mm.yyyy'), '')) as \"DISDATE\",
					...
					$query .= "
						...
						,to_char(ESEC.EvnSection_setDate, 'dd.mm.yyyy') as \"EvnSection_setDate\"
						,to_char(ESEC.EvnSection_disDate, 'dd.mm.yyyy') as \"EvnSection_disDate\"
						...
						,case when ESEC.EvnSection_disDate is not null
							then
								case
									when DATEDIFF('DAY', ESEC.EvnSection_setDate, ESEC.EvnSection_disDate) + interval '1 day' > 1
									then DATEDIFF('DAY', ESEC.EvnSection_setDate, ESEC.EvnSection_disDate)
									else DATEDIFF('DAY', ESEC.EvnSection_setDate, ESEC.EvnSection_disDate) + interval '1 day'
								end
							else null
			...
			case 'EvnDiag':
				...
				if ( $dbf === true ) {
					$query .= "
						...
						RTRIM(coalesce(to_char(EDPS.EvnDiagPS_setDate, 'dd.mm.yyyy'), '')) as \"SETDATE\",
			...
			case 'EvnLeave':
				...
				if ( $dbf === true ) {
					$query .= "
						coalesce(ELV.EvnLeave_id, coalesce(dbfeol.EvnOtherLpu_id, coalesce(dbfed.EvnDie_id, coalesce(dbfeost.EvnOtherStac_id, dbfeos.EvnOtherSection_id)))) as \"ELV_COD\",
						...
						coalesce(ELV.EvnLeave_UKL, coalesce(dbfeol.EvnOtherLpu_UKL, coalesce(dbfed.EvnDie_UKL, coalesce(dbfeost.EvnOtherStac_UKL, dbfeos.EvnOtherSection_UKL)))) as \"UKL\",
			...
			case 'EvnStick':
				...
				if ( $dbf === true ) {
					$query .= "
						...
						RTRIM(coalesce(to_char(EPS.EvnPS_setDate, 'dd.mm.yyyy'), '')) as \"SETDATE\",
						...
						RTRIM(coalesce(to_char(EST.EvnStick_begDate, 'dd.mm.yyyy'), '')) as \"BEGDATE\",
						RTRIM(coalesce(to_char(EST.EvnStick_endDate, 'dd.mm.yyyy'), '')) as \"ENDDATE\",
			...
			case 'KvsPerson':
				...
				if ( $dbf === true ) {
					$query .= "
						...
						rtrim(coalesce(to_char(PCh.PersonChild_invDate, 'dd.mm.yyyy'),'')) as \"INV_DATA\",
						Sex.Sex_Name as \"SEX\",
						...
						{$PS_prefix}.Person_EdNum as \"P_NUMED\",
						rtrim(coalesce(to_char(Pol.Polis_begDate, 'dd.mm.yyyy'),'')) as \"P_DATA\",
						...
						rtrim(coalesce(to_char(Doc.Document_begDate, 'dd.mm.yyyy'),'')) as \"D_DATA\"
					";
				}
			break;
			...
			case 'KvsPersonCard':
				...
				if ( $dbf === true ) {
					$query .= "
						...
						rtrim(coalesce(to_char(PC.PersonCard_begDate, 'dd.mm.yyyy'),'')) as \"PR_DATA\",
			...
			case 'KvsEvnDiag':
				...
				if ( $dbf === true ) {
					$query .= "
						...
						rtrim(coalesce(to_char(EDPS.EvnDiagPS_setDT, 'dd.mm.yyyy'),'')) as \"DZ_DATA\",
			...
			case 'KvsEvnPS':
				...
				if ( $dbf === true ) {
					$query .= "
						...
						rtrim(coalesce(to_char(EPS.EvnPS_setDate, 'dd.mm.yyyy'),'')) as \"DATAPOST\",
						...
						rtrim(coalesce(to_char(EPS.EvnDirection_setDT, 'dd.mm.yyyy'),'')) as \"KN_DATA\",
			...
			case 'KvsEvnSection':
				...
				if ( $dbf === true ) {
					$query .= "
						...
						".(isset($data['and_kvsperson']) && $data['and_kvsperson'] ? "" : "PS.PersonEvn_id as \"PCT_ID\",")."
						PS.Person_id as \"P_ID\",
						rtrim(coalesce(to_char(ESEC.EvnSection_setDate, 'dd.mm.yyyy'),'')) as \"DATAP\",
						ESEC.EvnSection_setTime as \"TIMEP\",
						rtrim(coalesce(to_char(ESEC.EvnSection_disDate, 'dd.mm.yyyy'),'')) as \"DATAW\",
						...
						case when ESEC.EvnSection_disDate is not null
							then
								case
									when LUT.LpuUnitType_Code = 2 and DATEDIFF('DAY', ESEC.EvnSection_setDate, ESEC.EvnSection_disDate) + interval '1 day' > 1
									then DATEDIFF('DAY', ESEC.EvnSection_setDate, ESEC.EvnSection_disDate)
									else DATEDIFF('DAY', ESEC.EvnSection_setDate, ESEC.EvnSection_disDate) + interval '1 day'
								end
							else null
			...
			case 'KvsNarrowBed':
				...
				if ( $dbf === true ) {
					$query .= "
						...
						EPS.EvnPS_id as \"GSP_ID\",
						ESEC.EvnSection_id as \"HSTRY_ID\",
						rtrim(coalesce(to_char(ESNB.EvnSectionNarrowBed_setDT, 'dd.mm.yyyy'),'')) as \"DATAP\",
						rtrim(coalesce(to_char(ESNB.EvnSectionNarrowBed_disDT, 'dd.mm.yyyy'),'')) as \"DATAW\",
						LS.LpuSection_Code as \"OTDLK\",
						LS.LpuSection_Name as \"OTDL\"
					";
				}
			...
			case 'KvsEvnUsluga':
				...
				if ( $dbf === true ) {
					$query .= "
						...
						rtrim(coalesce(to_char(EU.EvnUsluga_setDate, 'dd.mm.yyyy'),'')) as \"U_DATA\",
			...
			case 'KvsEvnUslugaOsl':
				...
				if ( $dbf === true ) {
					$query .= "
						...
						PS.Person_id as \"P_ID\",
						EPS.EvnPS_id as \"GSP_ID\",
						rtrim(coalesce(to_char(EA.EvnAgg_setDate, 'dd.mm.yyyy'),'')) as \"U_DATA\",
			...
			case 'KvsEvnDrug':
				...
				if ( $dbf === true ) {
					$query .= "
						...
						PS.Person_id as \"P_ID\",
						EPS.EvnPS_id as \"GSP_ID\",
						rtrim(coalesce(to_char(ED.EvnDrug_setDate, 'dd.mm.yyyy'),'')) as \"M_DATA\",
						...
						(
							'годн. ' || coalesce(to_char(Part.DocumentUcStr_godnDate, 'dd.mm.yyyy'),'отсут.') || 
							', цена ' || cast(ROUND(coalesce(Part.DocumentUcStr_PriceR,0), 2) as varchar(20)) || 
							', ост. ' || cast(Round(coalesce(Part.DocumentUcStr_Ost,0),4) as varchar(20)) || 
			...
			case 'KvsEvnStick':
				...
				if ( $dbf === true ) {
					$query .= "
						...
						coalesce(NULLIF(LTRIM(RTRIM(coalesce(EST.EvnStick_Ser, '') || ' ' || coalesce(EST.EvnStick_Num, '') || ', ' || rtrim(coalesce(to_char(EST.EvnStick_begDate, 'dd.mm.yyyy'),'')))), 'от'), '') as \"LWNOLD\",
						EST.EvnStick_Ser as \"LWN_S\",
						EST.EvnStick_Num as \"LWN_N\",
						rtrim(coalesce(to_char(EST.EvnStick_begDate, 'dd.mm.yyyy'),'')) as \"LWN_D\",
						...
						rtrim(coalesce(to_char(EST.EvnStick_sstBegDate, 'dd.mm.yyyy'),'')) as \"SKL_DN\",
						rtrim(coalesce(to_char(EST.EvnStick_sstEndDate, 'dd.mm.yyyy'),'')) as \"SKL_DK\",
						...
						rtrim(coalesce(to_char(EST.EvnStick_workDT, 'dd.mm.yyyy'),'')) as \"LWN_DR\",
			...
			case 'EvnAgg':
				...
				if ( $dbf === true ) {
					$query .= "
						...
						RTRIM(coalesce(to_char(EvnAgg.EvnAgg_setDT, 'dd.mm.yyyy'), '')) as \"SETDATE\",
			...
			case 'EvnVizitPL':
				...
				if ( $dbf === true ) {
					$query .= "
						...
						rtrim(coalesce(to_char( cast(EVizitPL.EvnVizitPL_setDate as timestamp), 'dd.mm.yyyy'),'')) as \"SETDATE\",
				...
				else
				{
					$query .= "
						...
						to_char(cast(EVizitPL.EvnVizitPL_setDate as timestamp), 'dd.mm.yyyy') as \"EvnVizitPL_setDate\",--Дата посещения,
			...
			case 'EvnVizitPLStom':
				...
				$query .= "
					...
					to_char(cast(EVPLS.EvnVizitPLStom_setDate as timestamp), 'dd.mm.yyyy') as \"EvnVizitPLStom_setDate\",
			...
			case 'EvnPL':
				...
				if ( $dbf === true )
				{
					$query .= "
						...
						as \"PDO_CODE\",
						rtrim(coalesce(to_char( cast(EPL.EvnPL_setDate as timestamp), 'dd.mm.yyyy'),'')) as \"SETDATE\",
						EPL.EvnPL_setTime as \"SETTIME\",
						rtrim(coalesce(to_char( cast(EPL.EvnPL_disDate as timestamp), 'dd.mm.yyyy'),'')) as \"DISDATE\",
						...
						rtrim(coalesce(to_char( cast(PS.Person_BirthDay as timestamp), 'dd.mm.yyyy'),'')) as \"BIRTHDAY\",
						...
						dbfdt.DeseaseType_code as \"DST_COD\",
						cast(EPL.EvnPL_UKL as decimal) as \"UKL\",
			...
			case 'EvnPS':
				...
				if ( $dbf === true )
				{
					$query .= "
						...
						RTRIM(coalesce(to_char(EPS.EvnPS_disDate, 'dd.mm.yyyy'), '')) as \"DISDATE\",
						...
						RTRIM(coalesce(to_char(EPS.EvnDirection_setDT, 'dd.mm.yyyy'), '')) as \"DATEORD\",
				...
				else
				{
					$query .= "
						...
						CASE WHEN EPS.PrehospWaifRefuseCause_id > 0 
							THEN to_char(EPS.EvnPS_setDate, 'dd.mm.yyyy') 
							ELSE to_char(EPS.EvnPS_disDate, 'dd.mm.yyyy') 
						...
						case when LpuUnitType.LpuUnitType_SysNick = 'stac' 
							then datediff('day', EPS.EvnPS_setDate, EPS.EvnPS_disDate) + abs(sign(datediff('day', EPS.EvnPS_setDate, EPS.EvnPS_disDate)) - interval '1 day') -- круглосуточные 
							else (datediff('day', EPS.EvnPS_setDate, EPS.EvnPS_disDate) + interval '1 day') -- дневные 
			...
			case 'EvnRecept':
				...
				$query .= "
					...
					(select ReceptDelayType_Name from ReceptDelayType ER_RDT where ER_RDT.ReceptDelayType_id = ER.ReceptDelayType_id limit 1) as \"ReceptDelayType_Name\",
					...
					(
						...
						then
							(select ReceptDelayType_Name from ReceptDelayType ER_RDT where ER_RDT.ReceptDelayType_id = ER.ReceptDelayType_id limit 1) || ' ' || (select Org_Name from v_OrgFarmacy ER_OF where ER_OF.OrgFarmacy_id = ER.OrgFarmacy_oid limit 1)
						else
							''
						end
					) as \"Delay_info\",
			...
			case 'EvnUslugaPar':
				...
				$query .= "
					...
					RTRIM(MP.Person_Fio) as \"MedPersonal_Fio\",
					CASE WHEN EUP.UslugaComplex_id IS NULL
						THEN RTRIM(Usluga.Usluga_Code)
						ELSE RTRIM(UslugaComplex.UslugaComplex_Code)
					END as \"Usluga_Code\",
					CASE WHEN EUP.UslugaComplex_id IS NULL
						THEN RTRIM(Usluga.Usluga_Name)
						ELSE RTRIM(UslugaComplex.UslugaComplex_Name)
					END as \"Usluga_Name\",
					RTRIM(PT.PayType_Name) as \"PayType_Name\",
					coalesce(EUP.EvnUslugaPar_Kolvo, '') as \"EvnUslugaPar_Kolvo\",
					coalesce(PostMed.PostMed_Name, '') as \"PostMed_Name\",
					(			-- это отделение, направившее пациента
						select
							LpuSection_Name
						from
							v_LpuSection
			...
			case 'PersonCard':
				...
				$query .= "
					...
					coalesce(paddr.Address_Address, '') as \"Person_PAddress\",
					coalesce(uaddr.Address_Address, '') as \"Person_UAddress\"
				";
			break;
			...
			case 'PersonCardStateDetail':
				...
				$query .= "
					...
					to_char(cast(pcc.Person_BirthDay as timestamp), 'dd.mm.yyyy') as \"Person_BirthDay\",
					to_char(cast(PCSD.PersonCard_begDate as timestamp), 'dd.mm.yyyy') as \"PersonCard_begDate\",
					to_char(cast(PCSD.PersonCard_endDate as timestamp), 'dd.mm.yyyy') as \"PersonCard_endDate\",
					...
					coalesce(rtrim(Address1.Address_Address), '') as \"UAddress_Address\",
			...
			case 'PersonDisp':
				...
				$query .= "
					...
					RTRIM(PS.Person_SecName) as \"Person_Secname\",
					to_char(cast(PS.Person_BirthDay as timestamp), 'dd.mm.yyyy') as \"Person_Birthday\",
					to_char(cast(PD.PersonDisp_begDate as timestamp), 'dd.mm.yyyy') as \"PersonDisp_begDate\",
					to_char(cast(PD.PersonDisp_endDate as timestamp), 'dd.mm.yyyy') as \"PersonDisp_endDate\",
					to_char(cast(PD.PersonDisp_NextDate as timestamp), 'dd.mm.yyyy') as \"PersonDisp_NextDate\",
			...
			case 'EvnDtpWound':
				...
				$query .= "
					...
					to_char(EDW.EvnDtpWound_setDate, 'dd.mm.yyyy') as \"EvnDtpWound_setDate\",
					RTRIM(PS.Person_SurName) as \"Person_Surname\",
					RTRIM(PS.Person_FirName) as \"Person_Firname\",
					RTRIM(PS.Person_SecName) as \"Person_Secname\",
					to_char(PS.Person_Birthday, 'dd.mm.yyyy') as \"Person_Birthday\"
			...
			case 'EvnInfectNotify':
				...
				$query .= "
					...
					to_char(EIN.EvnInfectNotify_insDT, 'dd.mm.yyyy') as \"EvnInfectNotify_insDT\",
					PS.Person_id as \"Person_id\",
					RTRIM(PS.Person_SurName) as \"Person_Surname\",
					RTRIM(PS.Person_FirName) as \"Person_Firname\",
					RTRIM(PS.Person_SecName) as \"Person_Secname\",
			...
			case 'EvnNotifyHepatitis':
				...
				$query .= "
					...
					ENH.MorbusType_id as \"MorbusType_id\",
					ENH.MedPersonal_id as \"MedPersonal_id\",
					ENH.pmUser_updId as \"pmUser_updId\",
					to_char(ENH.EvnNotifyHepatitis_setDT, 'dd.mm.yyyy') as \"EvnNotifyHepatitis_setDT\",
					PS.Person_id as \"Person_id\",
					PS.Server_id as \"Server_id\",
					PS.PersonEvn_id as \"PersonEvn_id\",
					RTRIM(PS.Person_SurName) as \"Person_Surname\",
					...
					Diag.Diag_id as \"Diag_id\",
					Diag.diag_FullName as \"Diag_Name\",
					to_char(coalesce(ENH.EvnNotifyHepatitis_niDate,PR.PersonRegister_setDate), 'dd.mm.yyyy') as \"PersonRegister_setDate\"
				";
			break;
			...
			case 'EvnOnkoNotify':
				...
				$query .= "
					...
					EON.MorbusType_id as \"MorbusType_id\",
					EON.MedPersonal_id as \"MedPersonal_id\",
					EON.pmUser_updId as \"pmUser_updId\",
					to_char(EON.EvnOnkoNotify_setDT, 'dd.mm.yyyy') as \"EvnOnkoNotify_setDT\",
					PS.Person_id as \"Person_id\",
					PS.Server_id as \"Server_id\",
					PS.PersonEvn_id as \"PersonEvn_id\",
					RTRIM(PS.Person_SurName) as \"Person_Surname\",
					...
					to_char(coalesce(EON.EvnOnkoNotify_niDate,PR.PersonRegister_setDate), 'dd.mm.yyyy') as \"PersonRegister_setDate\"
				";
			break;
			...
			case 'EvnNotifyOrphan':
				...
				$query .= "
					...
					ENO.EvnNotifyType_Name as \"vnNotifyType_Name\",
					ENO.EvnNotifyType_SysNick as \"vnNotifyType_SysNick\",
					PS.Person_id as \"Person_id\",
					PS.Server_id as \"Server_id\",
					PS.PersonEvn_id as \"PersonEvn_id\",
					ENO.Morbus_id as \"Morbus_id\",
					ENO.MorbusType_id as \"MorbusType_id\",
					RTRIM(PS.Person_SurName) as \"Person_Surname\",
					RTRIM(PS.Person_FirName) as \"Person_Firname\",
					RTRIM(PS.Person_SecName) as \"Person_Secname\",
					to_char(PS.Person_Birthday, 'dd.mm.yyyy') as \"Person_Birthday\",
					...
					Diag.Diag_id as \"Diag_id\",
					Diag.diag_FullName as \"Diag_Name\",
					to_char(coalesce(ENO.EvnNotifyOrphan_niDate,PR.PersonRegister_setDate), 'dd.mm.yyyy') as \"PersonRegister_setDate\",
			...
			case 'EvnNotifyCrazy': // Психиатрия
				...
				$query .= "
					...
					to_char(ENC.EvnNotifyCrazy_setDT, 'dd.mm.yyyy') as \"EvnNotifyCrazy_setDT\",
					PS.Person_id as \"Person_id\",
					PS.Server_id as \"Server_id\",
					PS.PersonEvn_id as \"PersonEvn_id\",
					ENC.Morbus_id as \"Morbus_id\",
					ENC.MorbusType_id as \"MorbusType_id\",
					RTRIM(PS.Person_SurName) as \"Person_Surname\",
					RTRIM(PS.Person_FirName) as \"Person_Firname\",
					RTRIM(PS.Person_SecName) as \"Person_Secname\",
					to_char(PS.Person_Birthday, 'dd.mm.yyyy') as \"Person_Birthday\",
					...
					Diag.Diag_id as \"Diag_id\",
					Diag.diag_FullName as \"Diag_Name\",
					to_char(coalesce(ENC.EvnNotifyCrazy_niDate,PR.PersonRegister_setDate), 'dd.mm.yyyy') as \"PersonRegister_setDate\",
			...
			case 'EvnNotifyTub': // Туберкулез
				...
				$query .= "
					...
					to_char(ENC.EvnNotifyTub_setDT, 'dd.mm.yyyy') as \"EvnNotifyTub_setDT\",
					PS.Person_id as \"Person_id\",
					PS.Server_id as \"Server_id\",
					PS.PersonEvn_id as \"PersonEvn_id\",
					ENC.Morbus_id as \"Morbus_id\",
					...
					Diag.Diag_id as \"Diag_id\",
					Diag.diag_FullName as \"Diag_Name\",
					to_char(coalesce(ENC.EvnNotifyTub_niDate,PR.PersonRegister_setDate), 'dd.mm.yyyy') as \"PersonRegister_setDate\",
					ENC.MedPersonal_id as \"MedPersonal_id\",
					ENC.pmUser_updId as \"pmUser_updId\"
				";
				break;
			case 'EvnNotifyHIV': // ВИЧ
				...
				$query .= "
					...
					to_char(ENB.EvnNotifyBase_setDT, 'dd.mm.yyyy') as \"EvnNotifyBase_setDT\",
					PS.Person_id as \"Person_id\",
					PS.Server_id as \"Server_id\",
					PS.PersonEvn_id as \"PersonEvn_id\",
					ENB.Morbus_id as \"Morbus_id\",
					ENB.MorbusType_id as \"MorbusType_id\",
					RTRIM(PS.Person_SurName) as \"Person_Surname\",
					RTRIM(PS.Person_FirName) as \"Person_Firname\",
					RTRIM(PS.Person_SecName) as \"Person_Secname\",
					to_char(PS.Person_Birthday, 'dd.mm.yyyy') as \"Person_Birthday\",
					...
					Diag.Diag_id as \"Diag_id\",
					Diag.diag_FullName as \"Diag_Name\",
					to_char(coalesce(ENB.EvnNotifyBase_niDate,PR.PersonRegister_setDate), 'dd.mm.yyyy') as \"PersonRegister_setDate\",
			...
			case 'EvnNotifyVener': // Туберкулез
				...
				$query .= "
					...
					to_char(ENC.EvnNotifyVener_setDT, 'dd.mm.yyyy') as \"EvnNotifyVener_setDT\",
					PS.Person_id as \"Person_id\",
					PS.Server_id as \"Server_id\",
					PS.PersonEvn_id as \"PersonEvn_id\",
					ENC.Morbus_id as \"Morbus_id\",
					...
					Diag.Diag_id as \"Diag_id\",
					Diag.diag_FullName as \"Diag_Name\",
					to_char(coalesce(ENC.EvnNotifyVener_niDate,PR.PersonRegister_setDate), 'dd.mm.yyyy') as \"PersonRegister_setDate\",
					ENC.MedPersonal_id as \"MedPersonal_id\",
					ENC.pmUser_updId as \"pmUser_updId\"
				";
				break;
		...
		switch ( $data['SearchFormType'] ) {
			case 'CmpCallCard':
				$query .= "
					...
					left join Diag UD on UD.Diag_id = CCC.Diag_uid
				...
				if ( isset($data['CmpCallCard_prmDate_Range'][0]) ) {
					$filter .= " and cast(CCC.CmpCallCard_prmDT as date) >= cast(:CmpCallCard_prmDate_Range_0 as timestamp)";
					$queryParams['CmpCallCard_prmDate_Range_0'] = $data['CmpCallCard_prmDate_Range'][0];
				}

				// Вызов
				if ( isset($data['CmpCallCard_prmDate_Range'][1]) ) {
					$filter .= " and cast(CCC.CmpCallCard_prmDT as date) <= cast(:CmpCallCard_prmDate_Range_1 as timestamp)";
				...
				if ( !empty($data['CmpCallCard_D201']) ) {
					$filter .= " and SUBSTRING(CCC.CmpCallCard_D201, 1, position(' ' in CCC.CmpCallCard_D201)) = :CmpCallCard_D201";
				...
				if ( !empty($data['CmpCallCard_Dsp1']) ) {
					$filter .= " and SUBSTRING(CCC.CmpCallCard_Dsp1, 1, position(' ' in CCC.CmpCallCard_Dsp1)) = :CmpCallCard_Dsp1";
					$queryParams['CmpCallCard_Dsp1'] = $data['CmpCallCard_Dsp1'];
				}

				// Управление вызовом
				if ( !empty($data['CmpCallCard_Dsp2']) ) {
					$filter .= " and SUBSTRING(CCC.CmpCallCard_Dsp2, 1, position(' ' in CCC.CmpCallCard_Dsp2)) = :CmpCallCard_Dsp2";
					$queryParams['CmpCallCard_Dsp2'] = $data['CmpCallCard_Dsp2'];
				}

				// Управление вызовом
				if ( !empty($data['CmpCallCard_Dsp3']) ) {
					$filter .= " and SUBSTRING(CCC.CmpCallCard_Dsp3, 1, position(' ' in CCC.CmpCallCard_Dsp3)) = :CmpCallCard_Dsp3";
					$queryParams['CmpCallCard_Dsp3'] = $data['CmpCallCard_Dsp3'];
				}

				// Управление вызовом
				if ( !empty($data['CmpCallCard_Dspp']) ) {
					$filter .= " and SUBSTRING(CCC.CmpCallCard_Dspp, 1, position(' ' in CCC.CmpCallCard_Dspp)) = :CmpCallCard_Dspp";
				...
				if ( !empty($data['CmpCallCard_Vr51']) ) {
					$filter .= " and SUBSTRING(CCC.CmpCallCard_Vr51, 1, position(' ' in CCC.CmpCallCard_Vr51)) = :CmpCallCard_Vr51";
			...
			case 'PersonPrivilegeWOW':
				...
				$query .= " inner join PersonPrivilegeWOW PPW on PPW.Person_id = PS.Person_id";
				$query .= " left join PrivilegeTypeWOW PTW on PTW.PrivilegeTypeWow_id = PPW.PrivilegeTypeWow_id";
				$query .= " left join Sex on Sex.Sex_id = PS.Sex_id";
				$query .= " left join v_Address UAdd on UAdd.Address_id = ps.UAddress_id";
				$query .= " left join v_Address PAdd on PAdd.Address_id = ps.PAddress_id";
			...
			case 'EvnPLWOW':
				if ( $data['PersonPeriodicType_id'] == 2 ) {
					$query .= " inner join v_EvnPLWOW EPW on EPW.Server_id = PS.Server_id and EPW.PersonEvn_id = PS.PersonEvn_id and EPW.Lpu_id = :Lpu_id";
				}
				else {
				...
				// $query .= " inner join v_EvnPLWOW EPW on EPW.Person_id = PS.Person_id and EPW.Lpu_id = :Lpu_id";
				//$query .= " left join PersonPrivilegeWOW PPW on PPW.Person_id = PS.Person_id";
				$query .= " left join lateral( select PrivilegeTypeWow_id from PersonPrivilegeWOW where Person_id = PS.Person_id limit 1) as PPW on true";
				$query .= " left join PrivilegeTypeWOW PTW on PTW.PrivilegeTypeWow_id = PPW.PrivilegeTypeWow_id";
				$query .= " left join YesNo on YesNo.YesNo_id = EPW.EvnPLWOW_IsFinish";
				if ( isset( $data['PrivilegeTypeWow_id'] ) )
				{
					$filter .= " and PTW.PrivilegeTypeWow_id = :PrivilegeTypeWow_id ";
					$queryParams['PrivilegeTypeWow_id'] = $data['PrivilegeTypeWow_id'];	
			...
			case 'EvnDtpWound':
				if ( $data['PersonPeriodicType_id'] == 2 ) {
					$query .= " inner join v_EvnDtpWound EDW on EDW.Server_id = PS.Server_id and EDW.PersonEvn_id = PS.PersonEvn_id and EDW.Lpu_id = :Lpu_id";
				}
				else {
			...
			case 'PersonDopDisp':
				$query .= " inner join PersonDopDisp DD on DD.Person_id = PS.Person_id and DD.Lpu_id = :Lpu_id";
				$query .= " left join Sex on PS.Sex_id = Sex.Sex_id";
				$query .= " 
				...
				$query .= "
					left join v_EvnPLDispDop epldd on
				...
				if ( isset($data['PersonDopDisp_Year']) )
				{
					...
					//and date_part('year', epldd.EvnPLDispDop_setDate) = 2010
			...
			case 'PersonDispOrpPeriod':
				$query .= " inner join v_PersonDispOrp DOr on DOr.Person_id = PS.Person_id and DOr.Lpu_id = :Lpu_id";
				$query .= " left join Sex on PS.Sex_id = Sex.Sex_id";
				$query .= " left join v_Address UAdd on UAdd.Address_id = ps.UAddress_id
							left join v_Address PAdd on PAdd.Address_id = ps.PAddress_id";
				$query .= " left join v_EvnPLDispTeenInspection EPLDTI on EPLDTI.PersonDispOrp_id = DOr.PersonDispOrp_id";
				$query .= "
					left join lateral(
						select
			...
			case 'PersonDispOrpProf':
				...
				$query .= " left join Sex on PS.Sex_id = Sex.Sex_id";
				$query .= " left join v_EvnPLDispTeenInspection EPLDTI on EPLDTI.PersonDispOrp_id = DOr.PersonDispOrp_id";
				$query .= " left join v_AgeGroupDisp AGD on AGD.AgeGroupDisp_id = DOr.AgeGroupDisp_id";
				...
				if ( !empty($data['OrgExist']) ) {
					if ($data['OrgExist'] == 2) {
						$filter .= " and DOr.Org_id IS NOT NULL";
					} else {
						$filter .= " and DOr.Org_id IS NULL";
					}
				}
			...
			case 'PersonDispOrpOld':
				...
				// до 2013 года
				$filter .= " and DOr.PersonDispOrp_Year <= 2012 ";
			...
			case 'EvnPLDispDop13':
				...
				if ( isset($data['EvnPLDispDop13_setDate']) ) {
					$filter .= " and EPLDD13.EvnPLDispDop13_setDate = cast(:EvnPLDispDop13_setDate as timestamp) ";
					$queryParams['EvnPLDispDop13_setDate'] = $data['EvnPLDispDop13_setDate'];
				}
				if ( isset($data['EvnPLDispDop13_setDate_Range'][0]) ) {
					$filter .= " and EPLDD13.EvnPLDispDop13_setDate >= cast(:EvnPLDispDop13_setDate_Range_0 as timestamp) ";
				...
				if ( isset($data['EvnPLDispDop13_setDate_Range'][1]) ) {
					$filter .= " and EPLDD13.EvnPLDispDop13_setDate <= cast(:EvnPLDispDop13_setDate_Range_1 as timestamp) ";
					$queryParams['EvnPLDispDop13_setDate_Range_1'] = $data['EvnPLDispDop13_setDate_Range'][1];
				}
				if ( isset($data['EvnPLDispDop13_disDate']) ) {
					$filter .= " and EPLDD13.EvnPLDispDop13_disDate = cast(:EvnPLDispDop13_disDate as timestamp) ";
					$queryParams['EvnPLDispDop13_disDate'] = $data['EvnPLDispDop13_disDate'];
				}
				if ( isset($data['EvnPLDispDop13_disDate_Range'][0]) ) {
					$filter .= " and EPLDD13.EvnPLDispDop13_disDate >= cast(:EvnPLDispDop13_disDate_Range_0 as timestamp) ";
				...
				if ( isset($data['EvnPLDispDop13_disDate_Range'][1]) ) {
					$filter .= " and EPLDD13.EvnPLDispDop13_disDate <= cast(:EvnPLDispDop13_disDate_Range_1 as timestamp) ";
					$queryParams['EvnPLDispDop13_disDate_Range_1'] = $data['EvnPLDispDop13_disDate_Range'][1];
				}
				if ( isset($data['EvnPLDispDop13_IsFinish']) ) {
					$filter .= " and coalesce(EPLDD13.EvnPLDispDop13_IsEndStage, 1) = :EvnPLDispDop13_IsFinish ";
				...
				if ( isset($data['EvnPLDispDop13_Cancel']) ) {
					...
					$filter .= " and coalesce(DDICData.DopDispInfoConsent_IsAgree,2) = :DopDispInfoConsent_IsAgree ";
					$queryParams['DopDispInfoConsent_IsAgree'] = $data['DopDispInfoConsent_IsAgree'];
				}
				if ( isset($data['EvnPLDispDop13_IsTwoStage']) ) {
					$filter .= " and coalesce(EPLDD13.EvnPLDispDop13_IsTwoStage, 1) = :EvnPLDispDop13_IsTwoStage ";
				...
				if ( isset($data['EvnPLDispDop13Second_setDate_Range'][0]) ) {
					$filter .= " and DopDispSecond.EvnPLDispDop13_setDate >= cast(:EvnPLDispDop13Second_setDate_Range_0 as timestamp) ";
				...
				if ( isset($data['EvnPLDispDop13Second_setDate_Range'][1]) ) {
					$filter .= " and DopDispSecond.EvnPLDispDop13_setDate <= cast(:EvnPLDispDop13Second_setDate_Range_1 as timestamp) ";
				...
				if ( isset($data['EvnPLDispDop13Second_disDate']) ) {
					$filter .= " and DopDispSecond.EvnPLDispDop13_disDate = cast(:EvnPLDispDop13Second_disDate as timestamp) ";
				...
				if ( isset($data['EvnPLDispDop13Second_disDate_Range'][0]) ) {
					$filter .= " and DopDispSecond.EvnPLDispDop13_disDate >= cast(:EvnPLDispDop13Second_disDate_Range_0 as timestamp) ";
				...
				if ( isset($data['EvnPLDispDop13Second_disDate_Range'][1]) ) {
					$filter .= " and DopDispSecond.EvnPLDispDop13_disDate <= cast(:EvnPLDispDop13Second_disDate_Range_1 as timestamp) ";
				...
				if ( isset($data['EvnPLDispDop13Second_IsFinish']) ) {
					$filter .= " and coalesce(DopDispSecond.EvnPLDispDop13_IsEndStage, 1) = :EvnPLDispDop13Second_IsFinish ";
				...
				$filter .= "
					and (
						(dbo.Age2(PS.Person_BirthDay, cast(:PersonDopDisp_Year || '-12-31' as date)) - 21 >= 0 and (dbo.Age2(PS.Person_BirthDay, cast(:PersonDopDisp_Year || '-12-31' as date)) - 21)%3 = 0)
						or
						((dbo.Age2(PS.Person_BirthDay, cast(:PersonDopDisp_Year || '-12-31' as date)) BETWEEN 18 AND 99) and exists (
							select
								pp.PersonPrivilege_id
							from v_PersonPrivilege pp
							inner join v_PrivilegeType pt on pt.PrivilegeType_id = pp.PrivilegeType_id
							where pt.PrivilegeType_Code IN ('" . implode("','", $this->EvnPLDispDop13_model->getPersonPrivilegeCodeList($queryParams['PersonDopDisp_Year'] . '-01-01')) . "')
								and pp.Person_id = PS.Person_id
								and pp.PersonPrivilege_begDate <= cast(:PersonDopDisp_Year || '-12-31' as date)
								and (pp.PersonPrivilege_endDate >= cast(:PersonDopDisp_Year || '-12-31' as date) or pp.PersonPrivilege_endDate is null)
							)
							limit 1
						) -- refs #23044
						{$add_filter}
					) and dbo.Age2(PS.Person_BirthDay, cast(:PersonDopDisp_Year || '-12-31' as date))<=99 
				";

				if ( $data['PersonPeriodicType_id'] == 2 ) {
					$query .= " 
						left join v_EvnPLDispDop13 EPLDD13 on EPLDD13.Server_id = PS.Server_id
							and EPLDD13.PersonEvn_id = PS.PersonEvn_id
							and EPLDD13.Lpu_id = :Lpu_id
					...
					$query .= " 
						left join v_EvnPLDispDop13 EPLDD13 on PS.Person_id = EPLDD13.Person_id
							and EPLDD13.Lpu_id = :Lpu_id
							and coalesce(DispClass_id,1) = 1
							and date_part('year', EvnPLDispDop13_setDate) = :PersonDopDisp_Year
					";
			...
			case 'EvnPLDispProf':
				...
				if ( $data['PersonPeriodicType_id'] == 2 ) {
					$query .= " 
						inner join v_EvnPLDispProf EPLDP on EPLDP.Server_id = PS.Server_id and EPLDP.PersonEvn_id = PS.PersonEvn_id and EPLDP.Lpu_id = :Lpu_id and date_part('year', EvnPLDispProf_setDate) = :PersonDopDisp_Year
					";
				}
				else {
					$query .= " 
						inner join v_EvnPLDispProf EPLDP on PS.Person_id = EPLDP.Person_id and EPLDP.Lpu_id = :Lpu_id and date_part('year', EvnPLDispProf_setDate) = :PersonDopDisp_Year
					";
				}
			...
			case 'EvnPLDispDop':
				...
				if ( $data['PersonPeriodicType_id'] == 2 ) {
					$query .= " 
						inner join v_EvnPLDispDop EPLDD on EPLDD.Server_id = PS.Server_id and EPLDD.PersonEvn_id = PS.PersonEvn_id and EPLDD.Lpu_id = :Lpu_id
					";
				}
				else {
					$query .= " 
				...
				if ( isset($data['EvnPLDispDop_HealthKind_id']) ) {
					...
					$query .= " 
						inner join v_EvnVizitDispDop EVPLDD on EVPLDD.EvnVizitDispDop_pid = EPLDD.EvnPLDispDop_id and coalesce(EPLDD.EvnPLDispDop_IsFinish, 1) = 2 and EVPLDD.DopDispSpec_id = 1 and EVPLDD.HealthKind_id = :EvnPLDispDop_HealthKind_id and EVPLDD.Lpu_id = :Lpu_id
					";
				}				
			...
			case 'EvnPLDispTeen14':
				if ( isset($data['EvnPLDispTeen14_setDate']) ) {
					$filter .= " and EPLDT14.EvnPLDispTeen14_setDate = cast(:EvnPLDispTeen14_setDate as timestamp) ";
					$queryParams['EvnPLDispTeen14_setDate'] = $data['EvnPLDispTeen14_setDate'];
				}
				if ( isset($data['EvnPLDispTeen14_setDate_Range'][0]) ) {
					$filter .= " and EPLDT14.EvnPLDispTeen14_setDate >= cast(:EvnPLDispTeen14_setDate_Range_0 as timestamp) ";
				...
				if ( isset($data['EvnPLDispTeen14_setDate_Range'][1]) ) {
					$filter .= " and EPLDT14.EvnPLDispTeen14_setDate <= cast(:EvnPLDispTeen14_setDate_Range_1 as timestamp) ";
					$queryParams['EvnPLDispTeen14_setDate_Range_1'] = $data['EvnPLDispTeen14_setDate_Range'][1];
				}
				if ( isset($data['EvnPLDispTeen14_disDate']) ) {
					$filter .= " and EPLDT14.EvnPLDispTeen14_disDate = cast(:EvnPLDispTeen14_disDate as timestamp) ";
					$queryParams['EvnPLDispTeen14_disDate'] = $data['EvnPLDispTeen14_disDate'];
				}
				if ( isset($data['EvnPLDispTeen14_disDate_Range'][0]) ) {
					$filter .= " and EPLDT14.EvnPLDispTeen14_disDate >= cast(:EvnPLDispTeen14_disDate_Range_0 as timestamp) ";
				...
				if ( isset($data['EvnPLDispTeen14_disDate_Range'][1]) ) {
					$filter .= " and EPLDT14.EvnPLDispTeen14_disDate <= cast(:EvnPLDispTeen14_disDate_Range_1 as timestamp) ";
				...
				if ( $data['PersonPeriodicType_id'] == 2 ) {
					$query .= " 
						inner join v_EvnPLDispTeen14 EPLDT14 on EPLDT14.Server_id = PS.Server_id and EPLDT14.PersonEvn_id = PS.PersonEvn_id and EPLDT14.Lpu_id = :Lpu_id
					";
				}
				else {
					$query .= " 
				...
				if ( isset($data['EvnPLDispTeen14_HealthKind_id']) ) {
					...
					$query .= " 
						inner join v_EvnVizitDispTeen14 EVPLDT14 on EVPLDT14.EvnVizitDispTeen14_pid = EPLDT14.EvnPLDispTeen14_id and coalesce(EPLDT14.EvnPLDispTeen14_IsFinish, 1) = 2 and EVPLDT14.Teen14DispSpecType_id = 1 and EVPLDT14.HealthKind_id = :EvnPLDispTeen14_HealthKind_id and EVPLDT14.Lpu_id = :Lpu_id
					";
				}				
			...
			case 'EvnPLDispOrp':
				...
				if ( $data['PersonPeriodicType_id'] == 2 ) {
					$query .= " 
						inner join v_EvnPLDispOrp EPLDO on EPLDO.Server_id = PS.Server_id and EPLDO.PersonEvn_id = PS.PersonEvn_id and EPLDO.Lpu_id = :Lpu_id and EPLDO.DispClass_id IN (3,7)
					";
				}
				else {
					$query .= " 
				...
				if ( isset($data['EvnPLDispOrp_HealthKind_id']) ) {
					...
					$query .= " 
						inner join v_EvnVizitDispDop EVPLDD on EVPLDD.EvnVizitDispDop_pid = EPLDO.EvnPLDispOrp_id and coalesce(EPLDO.EvnPLDispOrp_IsFinish, 1) = 2 and EVPLDD.DopDispSpec_id = 1 and EVPLDD.HealthKind_id = :EvnPLDispOrp_HealthKind_id and EVPLDD.Lpu_id = :Lpu_id
					";
				}				
			...
			case 'EvnPLDispOrpOld':
				...
				if ( $data['PersonPeriodicType_id'] == 2 ) {
					$query .= " 
						inner join v_EvnPLDispOrp EPLDO on EPLDO.Server_id = PS.Server_id and EPLDO.PersonEvn_id = PS.PersonEvn_id and EPLDO.Lpu_id = :Lpu_id
					";
				}
				else {
					$query .= " 
						inner join v_EvnPLDispOrp EPLDO on PS.Person_id = EPLDO.Person_id and EPLDO.Lpu_id = :Lpu_id
					";
				}
				// группа здоровья
				if ( isset($data['EvnPLDispOrp_HealthKind_id']) ) {
			...
			case 'EvnPLDispOrpSec':
				...
				if ( $data['PersonPeriodicType_id'] == 2 ) {
					$query .= " 
						inner join v_EvnPLDispOrp EPLDO on EPLDO.Server_id = PS.Server_id and EPLDO.PersonEvn_id = PS.PersonEvn_id and EPLDO.Lpu_id = :Lpu_id and EPLDO.DispClass_id IN (4,8)
					";
				}
				else {
					$query .= " 
			...
			case 'EvnPLDispTeenInspectionPred':
				if ( isset($data['EvnPLDispTeenInspection_setDate']) ) {
					$filter .= " and EPLDTI.EvnPLDispTeenInspection_setDate = cast(:EvnPLDispTeenInspection_setDate as timestamp) ";
				...
				if ( isset($data['EvnPLDispTeenInspection_setDate_Range'][0]) ) {
					$filter .= " and EPLDTI.EvnPLDispTeenInspection_setDate >= cast(:EvnPLDispTeenInspection_setDate_Range_0 as timestamp) ";
				...
				if ( isset($data['EvnPLDispTeenInspection_setDate_Range'][1]) ) {
					$filter .= " and EPLDTI.EvnPLDispTeenInspection_setDate <= cast(:EvnPLDispTeenInspection_setDate_Range_1 as timestamp) ";
				...
				if ( isset($data['EvnPLDispTeenInspection_disDate']) ) {
					$filter .= " and EPLDTI.EvnPLDispTeenInspection_disDate = cast(:EvnPLDispTeenInspection_disDate as timestamp) ";
				...
				if ( isset($data['EvnPLDispTeenInspection_disDate_Range'][0]) ) {
					$filter .= " and EPLDTI.EvnPLDispTeenInspection_disDate >= cast(:EvnPLDispTeenInspection_disDate_Range_0 as timestamp) ";
				...
				if ( isset($data['EvnPLDispTeenInspection_disDate_Range'][1]) ) {
					$filter .= " and EPLDTI.EvnPLDispTeenInspection_disDate <= cast(:EvnPLDispTeenInspection_disDate_Range_1 as timestamp) ";
				...
				if ( $data['PersonPeriodicType_id'] == 2 ) {
					$query .= " 
						inner join v_EvnPLDispTeenInspection EPLDTI on EPLDTI.Server_id = PS.Server_id and EPLDTI.PersonEvn_id = PS.PersonEvn_id and EPLDTI.Lpu_id = :Lpu_id
					";
				}
				else {
					$query .= " 
						inner join v_EvnPLDispTeenInspection EPLDTI on PS.Person_id = EPLDTI.Person_id and EPLDTI.Lpu_id = :Lpu_id
					";
				}
			...
			case 'EvnVizitPL':
				if ( $data['PersonPeriodicType_id'] == 2 ) {
					...
					{
						$query .= " inner join v_EvnVizitPL EVizitPL on EVizitPL.Server_id = PS.Server_id and EVizitPL.PersonEvn_id = PS.PersonEvn_id and EVizitPL.Lpu_id = :Lpu_id";
						$query .= " inner join v_EvnPL EPL on EPL.EvnPL_id=EVizitPL.EvnVizitPL_pid and EPL.Lpu_id = :Lpu_id and EPL.EvnClass_id = 3";
					}
					else
					{
						$query .= " inner join v_EvnPL EPL on EPL.Server_id = PS.Server_id and EPL.PersonEvn_id = PS.PersonEvn_id and EPL.Lpu_id = :Lpu_id and EPL.EvnClass_id = 3";
					}
				}
				else {
					if ( 'EvnVizitPL' == $data['SearchFormType'])
					{
						$query .= " inner join v_EvnVizitPL EVizitPL on EVizitPL.Person_id = PS.Person_id and EVizitPL.Lpu_id = :Lpu_id";
						$query .= " inner join v_EvnPL EPL on EPL.EvnPL_id=EVizitPL.EvnVizitPL_pid and EPL.Lpu_id = :Lpu_id and EPL.EvnClass_id = 3";
					}
					else
					{
				...
				// $query .= " inner join v_EvnPL EPL on EPL.Person_id = PS.Person_id and EPL.Lpu_id = :Lpu_id and EPL.EvnClass_id = 3";
				// $query .= " left join Diag EVPLD on EVPLD.Diag_id = EPL.Diag_id";
				if ( $dbf !== true ) {
					if ( 'EvnVizitPL' == $data['SearchFormType'] ) {
						$query .= "
							...
							left join v_MedPersonal as evplmp on evplmp.MedPersonal_id=EVizitPL.MedPersonal_id and evplmp.Lpu_id = EPL.Lpu_id
						...
						$query .= "
							left join Diag EVPLD on EVPLD.Diag_id = EPL.Diag_id
							left join v_MedPersonal MP on MP.MedPersonal_id = EPL.MedPersonal_id and MP.Lpu_id = :Lpu_id
					...
					switch ( $data['SearchFormType'] ) {
						case 'EvnAgg':
							$query .= "
								inner join v_EvnUsluga EvnUsluga on EvnUsluga.EvnUsluga_rid = EPL.EvnPL_id and EvnUsluga.Lpu_id = :Lpu_id
								inner join v_EvnAgg EvnAgg on EvnAgg.EvnAgg_pid = EvnUsluga.EvnUsluga_id and EvnAgg.Lpu_id = :Lpu_id
						...
						case 'EvnPL':
							$query .= "
								left join Diag as dbfdiag on dbfdiag.Diag_id = EPL.Diag_id
								left join v_Lpu as dbflpu on dbflpu.Lpu_id = EPL.Lpu_id
								...
								left join KLArea dbfkla on dbfkla.KLArea_id = COALESCE(dbfaddr.KLTown_id, dbfaddr.KLCity_id, dbfaddr.KLSubRgn_id, dbfaddr.KLRgn_id)
						...
						case 'EvnVizitPL':
							$query .= "
								left join Diag as dbfdiag on dbfdiag.Diag_id=EVizitPL.Diag_id
								left join LpuSection as dbfls on dbfls.LpuSection_id=EVizitPL.LpuSection_id
								left join v_MedPersonal as dbfmp on dbfmp.MedPersonal_id=EVizitPL.MedPersonal_id and dbfmp.Lpu_id = EPL.Lpu_id
				...
				{
					$query .= " inner join v_EvnVizitPL EVizitPL2 on EVizitPL2.EvnVizitPL_pid = EPL.EvnPL_id";
				...
				if ( isset($data['LpuSectionViz_id']) || isset($data['EvnVizitPL_isPaid']) || isset($data['LpuBuildingViz_id']) || isset($data['MedPersonalViz_id']) || isset($data['MedPersonalViz_sid']) ||
					...
					$filter .= " and exists (
						...
						left join Diag on Diag.Diag_id = EVPL2.Diag_id
						where
							EVPL2.Lpu_id = :Lpu_id
					";
					...
					if ( isset($data['EvnVizitPL_isPaid']) ) {
						$filter .= " and coalesce(EVPL2.EvnVizitPL_isPaid,1) = :EvnVizitPL_isPaid";
				...
				)
				{
					...
					if ( isset($data['EvnStick_endDate_Range'][0]) ) {
						$evn_stick_filter .= " and (
							...
							or (ESB.StickType_id = 2 and exists (select EvnStickWorkRelease_id from v_EvnStickWorkRelease where EvnStickBase_id = ESB.EvnStickBase_id and EvnStickWorkRelease_endDT >= :EvnStick_endDate_Range_0))
						)";
					...
					if ( isset($data['EvnStick_endDate_Range'][1]) ) {
						$evn_stick_filter .= " and (
							...
							or (ESB.StickType_id = 2 and exists (select EvnStickWorkRelease_id from v_EvnStickWorkRelease where EvnStickBase_id = ESB.EvnStickBase_id and EvnStickWorkRelease_endDT <= :EvnStick_endDate_Range_1))
						)";
			...
			case 'EvnVizitPLStom':
				if ( $data['PersonPeriodicType_id'] == 2 ) {
					...
					{
						$query .= " inner join v_EvnVizitPLStom as EVPLS on EVPLS.Server_id = PS.Server_id and EVPLS.PersonEvn_id = PS.PersonEvn_id and EVPLS.Lpu_id = :Lpu_id";
						$query .= " inner join v_EvnPLStom EPLS on EPLS.EvnPLStom_id = EVPLS.EvnVizitPLStom_pid and EPLS.Lpu_id = :Lpu_id and EPLS.EvnClass_id = 6";
					}
					else
					{
						$query .= " inner join v_EvnPLStom EPLS on EPLS.Server_id = PS.Server_id and EPLS.PersonEvn_id = PS.PersonEvn_id and EPLS.Lpu_id = :Lpu_id and EPLS.EvnClass_id = 6";
					}
				}
				else {
					if ( 'EvnVizitPLStom' == $data['SearchFormType'])
					{
						$query .= " inner join v_EvnVizitPLStom as EVPLS on EVPLS.Person_id = PS.Person_id and EVPLS.Lpu_id = :Lpu_id";
						$query .= " inner join v_EvnPLStom EPLS on EPLS.EvnPLStom_id = EVPLS.EvnVizitPLStom_pid and EPLS.Lpu_id = :Lpu_id and EPLS.EvnClass_id = 6";
					}
					else
					{
				...
				{
					$query .= " left join v_EvnVizitPLStom EVPLS on EVPLS.EvnVizitPLStom_pid = EPLS.EvnPLStom_id and EVPLS.EvnVizitPLStom_Index = EVPLS.EvnVizitPLStom_Count - 1 and EVPLS.Lpu_id = :Lpu_id";
					$query .= " left join YesNo IsFinish on IsFinish.YesNo_id = EPLS.EvnPLStom_IsFinish";
				...
				$query .= " left join v_MedPersonal MP on MP.MedPersonal_id = EVPLS.MedPersonal_id and MP.Lpu_id = :Lpu_id";
				...
				if ( isset($data['LpuSectionViz_id']) || isset($data['EvnVizitPLStom_isPaid']) || isset($data['LpuBuildingViz_id']) || isset($data['MedPersonalViz_id']) || isset($data['MedPersonalViz_sid']) ||
					...
					if ( isset($data['EvnVizitPLStom_isPaid']) ) {
						$filter .= " and coalesce(EVPLS2.EvnVizitPLStom_isPaid, 1) = :EvnVizitPLStom_isPaid";
			...
			case 'KvsEvnStick':
				if ( $data['PersonPeriodicType_id'] == 2 ) {
					$query .= " inner join v_EvnPS EPS on EPS.Server_id = PS.Server_id and EPS.PersonEvn_id = PS.PersonEvn_id and EPS.Lpu_id = :Lpu_id";
				}
				else {
				...
				if ( $dbf === true ) {
					switch ( $data['SearchFormType'] ) {
						...
						case 'EvnSection':
							$query .= "
								inner join v_EvnSection as ESEC on ESEC.EvnSection_pid = EPS.EvnPS_id and ESEC.Lpu_id = :Lpu_id
								left join LpuSection as dbfsec on dbfsec.LpuSection_id = ESEC.LpuSection_id
								left join PayType as dbfpay on dbfpay.PayType_id = ESEC.PayType_id
								left join TariffClass as dbftar on dbftar.TariffClass_id = ESEC.TariffClass_id
								left join v_MedPersonal dbfmp on dbfmp.MedPersonal_id = EPS.MedPersonal_pid and dbfmp.Lpu_id = EPS.Lpu_id
							";
						break;
						case 'EvnDiag':
							$query .= "
								left join v_EvnSection sect on sect.EvnSection_pid = EPS.EvnPS_id and sect.Lpu_id = :Lpu_id
								left join v_EvnLeave leav on leav.EvnLeave_pid = EPS.EvnPS_id and leav.Lpu_id = :Lpu_id
						...
						case 'EvnLeave':
							$query .= "
								left join v_EvnLeave ELV on ELV.EvnLeave_pid = EPS.EvnPS_id and EPS.LeaveType_id = 1 and ELV.Lpu_id = :Lpu_id
								left join v_EvnOtherLpu dbfeol on dbfeol.EvnOtherLpu_pid = EPS.EvnPS_id and EPS.LeaveType_id = 2 and dbfeol.Lpu_id = :Lpu_id
								left join v_EvnDie dbfed on dbfed.EvnDie_pid = EPS.EvnPS_id and EPS.LeaveType_id = 3 and dbfed.Lpu_id = :Lpu_id
								left join v_EvnOtherStac dbfeost on dbfeost.EvnOtherStac_pid = EPS.EvnPS_id and EPS.LeaveType_id = 4 and dbfeost.Lpu_id = :Lpu_id
								left join v_EvnOtherSection dbfeos on dbfeos.EvnOtherSection_pid = EPS.EvnPS_id and EPS.LeaveType_id = 5  and dbfeos.Lpu_id = :Lpu_id
								inner join v_LeaveType dbflt on dbflt.LeaveType_id = EPS.LeaveType_id and dbflt.Lpu_id = :Lpu_id
									and (
						...
						case 'EvnStick':
							$query .= "
								inner join v_EvnStick EST on EST.EvnStick_pid = EPS.EvnPS_id and EST.Lpu_id = :Lpu_id
							";
						break;
						case 'KvsPerson':
							...
							$query .= "
								left join Sex on Sex.Sex_id = {$PS_prefix}.Sex_id
								left join SocStatus Soc on Soc.SocStatus_id = {$PS_prefix}.SocStatus_id
								left join PersonChild PCh on PCh.Person_id = {$PS_prefix}.Person_id
								left join YesNo IsInv on IsInv.YesNo_id = PCh.PersonChild_IsInvalid
								left join Diag InvD on InvD.Diag_id = PCh.Diag_id
								left join Polis Pol on Pol.Polis_id = {$PS_prefix}.Polis_id
								...
								left join v_Address_all UA on UA.Address_id = {$PS_prefix}.UAddress_id 
								left join v_Address_all PA on PA.Address_id = {$PS_prefix}.PAddress_id
								left join Document Doc on Doc.Document_id = {$PS_prefix}.Document_id 
						...
						case 'KvsEvnDiag':
							$query .= "
								left join v_EvnSection ESEC on ESEC.EvnSection_pid = EPS.EvnPS_id and ESEC.Lpu_id = :Lpu_id
								inner join v_EvnDiagPS EDPS on EDPS.EvnDiagPS_pid = EPS.EvnPS_id 
						...
						case 'KvsEvnSection':
							$query .= "
								inner join v_EvnSection as ESEC on ESEC.EvnSection_pid = EPS.EvnPS_id and ESEC.Lpu_id = :Lpu_id
								inner join LpuSection LS on LS.LpuSection_id = ESEC.LpuSection_id
								...
								inner join v_MedPersonal MP on MP.MedPersonal_id = ESEC.MedPersonal_id
									and MP.Lpu_id = EPS.Lpu_id
						...
						case 'KvsNarrowBed':
							$query .= "
								inner join v_EvnSection as ESEC on ESEC.EvnSection_pid = EPS.EvnPS_id and ESEC.Lpu_id = :Lpu_id
								inner join v_EvnSectionNarrowBed as ESNB on ESNB.EvnSectionNarrowBed_pid = ESEC.EvnSection_id and ESNB.Lpu_id = :Lpu_id
								left join LpuSection LS on LS.LpuSection_id = ESNB.LpuSection_id
							";
						break;
						case 'KvsEvnUsluga':
							$query .= "
								inner join v_EvnUsluga as EU on EU.EvnUsluga_rid = EPS.EvnPS_id and EU.Lpu_id = :Lpu_id
								...
								left join lateral(
									select
										MedPersonal_TabCode, Person_Fio
									from v_MedPersonal
						...
						case 'KvsEvnUslugaOB':
							$query .= "
								left join v_EvnUsluga as EU on EU.EvnUsluga_rid = EPS.EvnPS_id and EU.Lpu_id = :Lpu_id
								left join v_EvnUslugaOper EUO on EUO.EvnUslugaOper_id = EU.EvnUsluga_id
						...
						case 'KvsEvnUslugaOsl':
							$query .= "
								left join v_EvnUsluga as EU on EU.EvnUsluga_rid = EPS.EvnPS_id and EU.Lpu_id = :Lpu_id
								inner join v_EvnAgg EA on EA.EvnAgg_pid = EU.EvnUsluga_id and EA.Lpu_id = :Lpu_id
						...
						case 'KvsEvnDrug':
							$query .= "
								inner join v_EvnDrug as ED on ED.EvnDrug_rid = EPS.EvnPS_id and ED.Lpu_id = :Lpu_id
						...
						case 'KvsEvnLeave':
							$query .= "
								inner join v_EvnSection as ESEC on ESEC.EvnSection_pid = EPS.EvnPS_id and ESEC.Lpu_id = :Lpu_id
								left join v_EvnLeave ELV on ELV.EvnLeave_pid = ESEC.EvnSection_id and ESEC.LeaveType_id = 1 and ELV.Lpu_id = :Lpu_id
								left join v_EvnOtherLpu EOLpu on EOLpu.EvnOtherLpu_pid = ESEC.EvnSection_id and ESEC.LeaveType_id = 2 and EOLpu.Lpu_id = :Lpu_id
								left join v_EvnDie EDie on EDie.EvnDie_pid = ESEC.EvnSection_id and ESEC.LeaveType_id = 3 and EDie.Lpu_id = :Lpu_id
								left join v_EvnOtherStac EOStac on EOStac.EvnOtherStac_pid = ESEC.EvnSection_id and ESEC.LeaveType_id = 4 and EOStac.Lpu_id = :Lpu_id
								left join v_EvnOtherSection EOSect on EOSect.EvnOtherSection_pid = ESEC.EvnSection_id and ESEC.LeaveType_id = 5 and EOSect.Lpu_id = :Lpu_id
								...
								left join v_MedPersonal MP on MP.MedPersonal_id = EDie.MedPersonal_aid
									and MP.Lpu_id = EPS.Lpu_id
						...
						case 'KvsEvnStick':
							$query .= "
								...
								-- left join Diag D2 on D2.Diag_id = EST.Diag_id
							";
						break;
					}
					...
					if (isset($data['and_kvsperson']) && $data['and_kvsperson']) { //при комбинировании с таблицой персон добавляем дополнительные поля
						...
						$query .= "
							left join Sex PrsSex on PrsSex.Sex_id = {$PS_prefix}.Sex_id
							left join SocStatus PrsSoc on PrsSoc.SocStatus_id = {$PS_prefix}.SocStatus_id
							left join PersonChild PrsPCh on PrsPCh.Person_id = {$PS_prefix}.Person_id
							...
							left join Polis PrsPol on PrsPol.Polis_id = {$PS_prefix}.Polis_id
							...
							left join v_Address_all PrsUA on PrsUA.Address_id = {$PS_prefix}.UAddress_id 
							left join v_Address_all PrsPA on PrsPA.Address_id = {$PS_prefix}.PAddress_id
							left join Document PrsDoc on PrsDoc.Document_id = {$PS_prefix}.Document_id 
					...
					switch ( $data['SearchFormType'] ) {
						case 'EvnPS':
							$query .= "
								...
								--left join v_Lpu LpuD on LpuD.Lpu_id = EPS.Lpu_did
								-- Тарас сказал что Index и Count - это почти всегда правильно (с) Night, 2011-06-22 
								left join v_EvnSection EPSLastES on EPSLastES.EvnSection_pid = EPS.EvnPS_id and EPSLastES.EvnSection_Index = EPSLastES.EvnSection_Count-1 and EPSLastES.Lpu_id = :Lpu_id
								...
								left join LeaveType LT on LT.LeaveType_id = EPSLastES.LeaveType_id
								left join v_MedPersonal MP on MP.MedPersonal_id = EPSLastES.MedPersonal_id
						...
						case 'EvnSection':
							$query .= "
								inner join v_EvnSection as ESEC on ESEC.EvnSection_pid = EPS.EvnPS_id
								...
								left join lateral(
									select
										v_MedPersonal.Person_Fio,MedStaffFact.MedStaffFact_id
									from v_MedPersonal
                                    inner join v_MedStaffFact MedStaffFact on MedStaffFact.MedPersonal_id = v_MedPersonal.MedPersonal_id and MedStaffFact.LpuSection_id = ESEC.LpuSection_id
				...
				if ( isset($data['EvnDirection_setDate_Range'][0]) ) {
					$filter .= " and EPS.EvnDirection_setDT >= cast(:EvnDirection_setDate_Range_0 as timestamp)";
					$queryParams['EvnDirection_setDate_Range_0'] = $data['EvnDirection_setDate_Range'][0];
				}

				// Госпитализация
				if ( isset($data['EvnDirection_setDate_Range'][1]) ) {
					$filter .= " and EPS.EvnDirection_setDT <= cast(:EvnDirection_setDate_Range_1 as timestamp)";
					...
					switch ( $data['SearchFormType'] ) {
						case 'EvnPS':
							...
							if ( !empty($data['EvnSection_isPaid']) ) {
								$filter .= " and coalesce(ES.EvnSection_isPaid,1) = :EvnSection_isPaid";
				...
				)
				{
					switch ( $data['SearchFormType'] ) {
						case 'EvnPS':
							$query .= " left join v_EvnLeave EL on EL.EvnLeave_pid = EPSLastES.EvnSection_id and EPS.LeaveType_id = 1 and EL.Lpu_id = :Lpu_id";
							$query .= " left join v_EvnOtherLpu EOL on EOL.EvnOtherLpu_pid = EPSLastES.EvnSection_id and EPS.LeaveType_id = 2 and EOL.Lpu_id = :Lpu_id";
							$query .= " left join v_EvnDie ED on ED.EvnDie_pid = EPSLastES.EvnSection_id and EPS.LeaveType_id = 3 and ED.Lpu_id = :Lpu_id";
							$query .= " left join v_EvnOtherStac EOST on EOST.EvnOtherStac_pid = EPSLastES.EvnSection_id and EPS.LeaveType_id = 4 and EOST.Lpu_id = :Lpu_id";
							$query .= " left join v_EvnOtherSection EOS on EOS.EvnOtherSection_pid = EPSLastES.EvnSection_id and EPS.LeaveType_id = 5 and EOS.Lpu_id = :Lpu_id";
						break;

						case 'EvnSection':
							$query .= " left join v_EvnLeave EL on EL.EvnLeave_pid = ESEC.EvnSection_id and ESEC.LeaveType_id = 1";
							$query .= " left join v_EvnOtherLpu EOL on EOL.EvnOtherLpu_pid = ESEC.EvnSection_id and ESEC.LeaveType_id = 2";
							$query .= " left join v_EvnDie ED on ED.EvnDie_pid = ESEC.EvnSection_id and ESEC.LeaveType_id = 3";
							$query .= " left join v_EvnOtherStac EOST on EOST.EvnOtherStac_pid = ESEC.EvnSection_id and ESEC.LeaveType_id = 4";
							$query .= " left join v_EvnOtherSection EOS on EOS.EvnOtherSection_pid = ESEC.EvnSection_id and ESEC.LeaveType_id = 5";
						break;
					}
					...
					if ( isset($data['ResultDesease_id']) ) {
						$filter .= " and coalesce(EL.ResultDesease_id, coalesce(EOL.ResultDesease_id, coalesce(EOST.ResultDesease_id, EOS.ResultDesease_id))) = :ResultDesease_id";
			...
			case 'EvnRecept':
				...
				$query .= " left join v_Drug ERDrug on ERDrug.Drug_id = ER.Drug_id";
				$query .= " left join rls.v_Drug ERDrugRls on ERDrugRls.Drug_id = ER.Drug_rlsid";
				$query .= " left join dbo.v_WhsDocumentCostItemType wdcit on wdcit.WhsDocumentCostItemType_id = ER.WhsDocumentCostItemType_id";
				$query .= " left join v_MedPersonal ERMP on ERMP.MedPersonal_id = ER.MedPersonal_id";
				if (!$isFarmacy && $data['Lpu_id'] > 0 )
					$query .= " and ERMP.Lpu_id = :Lpu_id";

				if ( (strlen($data['ER_Diag_Code_From']) > 0) || (strlen($data['ER_Diag_Code_To']) > 0) ) {
					$query .= " inner join Diag ERDiag on ERDiag.Diag_id = ER.Diag_id";
				...
				if ( isset($data['EvnRecept_setDate']) ) {
					$filter .= " and ER.EvnRecept_setDate = cast(:EvnRecept_setDate as timestamp)";
					$queryParams['EvnRecept_setDate'] = $data['EvnRecept_setDate'];
				}

				// Рецепт
				if ( isset($data['EvnRecept_setDate_Range'][0]) ) {
					$filter .= " and ER.EvnRecept_setDate >= cast(:EvnRecept_setDate_Range_0 as timestamp)";
					$queryParams['EvnRecept_setDate_Range_0'] = $data['EvnRecept_setDate_Range'][0];
				}

				// Рецепт
				if ( isset($data['EvnRecept_setDate_Range'][1]) ) {
					$filter .= " and ER.EvnRecept_setDate <= cast(:EvnRecept_setDate_Range_1 as timestamp)";
			...
			case 'EvnUslugaPar':
				if ( $data['PersonPeriodicType_id'] == 2 ) {
					$query .= " inner join v_EvnUslugaPar EUP on EUP.Server_id = PS.Server_id and EUP.PersonEvn_id = PS.PersonEvn_id and EUP.Lpu_id = :Lpu_id";
				}
				else {
					$query .= " inner join v_EvnUslugaPar EUP on EUP.Person_id = PS.Person_id and EUP.Lpu_id = :Lpu_id";
				}
				$query .= " inner join LpuSection LS on LS.LpuSection_id = EUP.LpuSection_uid";
				$query .= " inner join v_MedPersonal MP on MP.MedPersonal_id = EUP.MedPersonal_id and MP.Lpu_id = EUP.Lpu_id";
				$query .= " left join Usluga on Usluga.Usluga_id = EUP.Usluga_id";
				$query .= " left join UslugaComplex on UslugaComplex.UslugaComplex_id = EUP.UslugaComplex_id";
				$query .= " inner join PayType PT on PT.PayType_id = EUP.PayType_id";
				$query .= " inner join v_MedStaffFact MedStaffFact on MedStaffFact.MedPersonal_id = MP.MedPersonal_id and MedStaffFact.LpuSection_id = LS.LpuSection_id";
				$query .= " left join v_PostMed PostMed on PostMed.PostMed_id = MedStaffFact.Post_id";
				$query .= " left join v_EvnDirection_all ED on EUP.EvnDirection_id = ED.EvnDirection_id";
				...
				if ( isset($data['EvnUslugaPar_setDate_Range'][0]) ) {
					$filter .= " and EUP.EvnUslugaPar_setDate >= cast(:EvnUslugaPar_setDate_Range_0 as timestamp)";
					$queryParams['EvnUslugaPar_setDate_Range_0'] = $data['EvnUslugaPar_setDate_Range'][0];
				}

				// Услуга
				if ( isset($data['EvnUslugaPar_setDate_Range'][1]) ) {
					$filter .= " and EUP.EvnUslugaPar_setDate <= cast(:EvnUslugaPar_setDate_Range_1 as timestamp)";
			...
			case 'PersonCardStateDetail':
				...
				$query .= "
					...
					inner join lateral(
						...
						left join LpuRegion lr on lr.LpuRegion_id = pc.LpuRegion_id
					...
					inner join PersonCard PCSD on PCSD1.PersonCard_id = PCSD.PersonCard_id
					left join v_PersonState pcc on pcc.Person_id = PCSD.Person_id
					--участок по ОМС
					left join LpuRegion lr on lr.LpuRegion_id = PCSD.LpuRegion_id
					...
					--left join v_PersonCard pc1 on PCSD.Person_id=pc1.Person_id and PCSD.LpuAttachType_id=pc1.LpuAttachType_id
					--left join v_Lpu lp on pc1.Lpu_id=lp.Lpu_id
					--left join PersonState ps1 on ps1.Person_id = PCSD.Person_id
					left join Address on ps.PAddress_id = Address.Address_id
					left join Address Address1 on ps.UAddress_id = Address1.Address_id
					-- текущий полис
					left join v_Polis pols on pols.Polis_id = PS.Polis_id and cast(to_char(pols.Polis_begDate, 'yyyymmdd') as timestamp) < cast(to_char(dbo.tzGetDate(), 'yyyymmdd') as timestamp) and ( pols.Polis_EndDate is null or cast(to_char(Pols.Polis_endDate, 'yyyymmdd') as timestamp) > cast(to_char(dbo.tzGetDate(), 'yyyymmdd') as timestamp) )
					left join v_OrgSmo omsOrgSmo on pols.PolisType_id in (1,4) and pols.OrgSmo_id = omsOrgSmo.OrgSmo_id
					--для СМО по ДМС
					left join v_PersonCard dmspc on dmspc.Lpu_id = :Lpu_id 
						and dmspc.Person_id = PS.Person_id 
						and  dmspc.LpuAttachType_id = 5 
						and cast(to_char(dmspc.PersonCard_begDate, 'yyyymmdd') as timestamp) < cast(to_char(dbo.tzGetDate(), 'yyyymmdd') as timestamp) 
						and ( dmspc.PersonCard_endDate is null or cast(to_char(dmspc.PersonCard_endDate, 'yyyymmdd') as timestamp) > cast(to_char(dbo.tzGetDate(), 'yyyymmdd') as timestamp) )
					left join v_OrgSmo dmsOrgSmo on dmspc.OrgSmo_id = dmsOrgSmo.OrgSmo_id
					  left join lateral(
                            ...
                            where
								...
								and cast(to_char(Polis.Polis_begDate, 'yyyymmdd') as timestamp) < :PCSD_begDate
								and ( Polis.Polis_endDate is null or (cast(to_char(Polis.Polis_endDate, 'yyyymmdd') as timestamp) >= :PCSD_begDate) )
							limit 1
                        ) as PolisBeg on true
                        left join lateral(
                            Select
                            ...
                            where
                                Person.Person_id = PCSD.Person_id
								and Person.Server_pid = 0
								and cast(to_char(Polis.Polis_begDate, 'yyyymmdd') as timestamp) <= :PCSD_endDate
								and ( Polis.Polis_endDate is null or (cast(to_char(Polis.Polis_endDate, 'yyyymmdd') as timestamp) > :PCSD_endDate) )
							limit 1
                        ) as PolisEnd on true
					 left join lateral(
							select
				...
				switch ( $data['PCSD_mode'] )
				{
					case 'BegCount':
						$filter .= "
							and cast(to_char(PCSD.PersonCard_begDate, 'yyyymmdd') as timestamp) < :PCSD_begDate
					...
					case 'EndCount':
						$filter .= "
							and cast(to_char(PCSD.PersonCard_begDate, 'yyyymmdd') as timestamp) <= :PCSD_endDate
							and (PCSD.PersonCard_endDate is null or cast(to_char(PCSD.PersonCard_endDate, 'yyyymmdd') as timestamp) > :PCSD_endDate)
						";
						if ( isset($data['PCSD_ToLpu_id']) && (int)$data['PCSD_ToLpu_id'] > 0 )
						{
							$filter .= ' and 1 = 2 ';
					...
					case 'AttachCount':
						$filter .= "
							and cast(to_char(PCSD.PersonCard_begDate, 'yyyymmdd') as timestamp) between :PCSD_begDate and :PCSD_endDate
						";
						if ( isset($data['PCSD_ToLpu_id']) && (int)$data['PCSD_ToLpu_id'] > 0 )
						{
							$filter .= ' and 1 = 2 ';
					...
					case 'AttachIncomeBDZ':
						$filter .= "
							and cast(to_char(PCSD.PersonCard_begDate, 'yyyymmdd') as timestamp) between cast('1970-01-01' as date) and :PCSD_endDate
							and (PCSD.PersonCard_endDate is null or cast(to_char(PCSD.PersonCard_endDate, 'yyyymmdd') as timestamp) >= :PCSD_begDate)
							and PS.Server_pid = 0
					...
					case 'DettachCount':
						...
						{
							$filter .= "
								and cast(to_char(NextCard.PersonCard_begDate, 'yyyymmdd') as timestamp) between :PCSD_begDate and :PCSD_endDate and NextCard.Lpu_id = :ToLpu_id
							";
						}
						else
						{
							$filter .= "
								and cast(to_char(PCSD.PersonCard_endDate, 'yyyymmdd') as timestamp) between :PCSD_begDate and :PCSD_endDate
							";
						}
					break;
			...
			break;
			case 'PersonDisp':
				$query .= " inner join v_PersonDisp PD on PD.Person_id = PS.Person_id ";
				$query .= " left join Diag dg1 on PD.Diag_id = dg1.Diag_id ";
				$query .= " left join Diag dg2 on PD.Diag_pid = dg2.Diag_id ";
				$query .= " left join Diag dg3 on PD.Diag_nid = dg3.Diag_id ";
				$query .= " left join v_MedPersonal mp1 on PD.MedPersonal_id = mp1.MedPersonal_id and PD.Lpu_id = mp1.Lpu_id ";
				$query .= " left join LpuSection lpus1 on PD.LpuSection_id = lpus1.LpuSection_id ";
				$query .= " left join Sickness scks on PD.Sickness_id = scks.Sickness_id ";
				$query .= " left join v_Lpu lpu1 on PD.Lpu_id = lpu1.Lpu_id ";
				$query .= " left join lateral(
					select
						max(
							case when 
								to_char(PersonDispMedicament_begDate, 'yyyymmdd') <= to_char(dbo.tzGetDate(), 'yyyymmdd')
								and (
									to_char(PersonDispMedicament_endDate, 'yyyymmdd') >= to_char(dbo.tzGetDate(), 'yyyymmdd')
			...
			case 'EvnInfectNotify':
				$query .= " inner join v_EvnInfectNotify EIN on ". ( ($data['PersonPeriodicType_id'] == 2) ? 'EIN.PersonEvn_id = PS.PersonEvn_id and EIN.Server_id = PS.Server_id' : 'EIN.Person_id = PS.Person_id' ) ;
				$query .= " left join v_EvnVizitPL EVPL on EIN.EvnInfectNotify_pid = EVPL.EvnVizitPL_id ";
				$query .= " left join v_EvnSection ES on EIN.EvnInfectNotify_pid = ES.EvnSection_id ";
				$query .= " 					
					left join lateral(
						select
							Lpu_id
				...
				$query .= " left join v_Lpu Lpu on Lpu.Lpu_id = PC.Lpu_id ";
				$query .= " left join v_Diag Diag on Diag.Diag_id = EVPL.Diag_id ";
				$query .= " left join v_Diag Diag1 on Diag1.Diag_id = ES.Diag_id ";
				...
				if ( isset($data['EvnNotifyBase_setDT_Range'][0]) ) {
					$filter .= " and EIN.EvnInfectNotify_insDT >= cast(:EvnInfectNotify_insDT_Range_0 as timestamp) ";
				...
				if ( isset($data['EvnNotifyBase_setDT_Range'][1]) ) {
					$filter .= " and EIN.EvnInfectNotify_insDT <= cast(:EvnInfectNotify_insDT_Range_1 as timestamp) ";
			...
			case 'EvnNotifyHepatitis':				
				$query .= '
					inner join v_EvnNotifyHepatitis ENH on '. ( ($data['PersonPeriodicType_id'] == 2) ? 'ENH.PersonEvn_id = PS.PersonEvn_id and ENH.Server_id = PS.Server_id' : 'ENH.Person_id = PS.Person_id' ) .'
				...
				if ( isset($data['EvnNotifyBase_setDT_Range'][0]) ) {
					$filter .= " and ENH.EvnNotifyHepatitis_setDT >= cast(:EvnNotifyHepatitis_setDT_Range_0 as timestamp) ";
					$queryParams['EvnNotifyHepatitis_setDT_Range_0'] = $data['EvnNotifyBase_setDT_Range'][0];
				}
				
				if ( isset($data['EvnNotifyBase_setDT_Range'][1]) ) {
					$filter .= " and ENH.EvnNotifyHepatitis_setDT <= cast(:EvnNotifyHepatitis_setDT_Range_1 as timestamp) ";
			...
			case 'EvnOnkoNotify':				
				$query .= '
					inner join v_EvnOnkoNotify EON on '. ( ($data['PersonPeriodicType_id'] == 2) ? 'EON.PersonEvn_id = PS.PersonEvn_id and EON.Server_id = PS.Server_id' : 'EON.Person_id = PS.Person_id' ) .'
				...
				if ( isset($data['EvnNotifyBase_setDT_Range'][0]) ) {
					$filter .= " and EON.EvnOnkoNotify_setDT >= cast(:EvnOnkoNotify_setDT_Range_0 as timestamp) ";
					$queryParams['EvnOnkoNotify_setDT_Range_0'] = $data['EvnNotifyBase_setDT_Range'][0];
				}
				
				if ( isset($data['EvnNotifyBase_setDT_Range'][1]) ) {
					$filter .= " and EON.EvnOnkoNotify_setDT <= cast(:EvnOnkoNotify_setDT_Range_1 as timestamp) ";
			...
			case 'OnkoRegistry':
				$query .= '
					inner join v_PersonRegister PR on PR.Person_id = PS.Person_id and PR.MorbusType_id = 3
				...
				if ( isset($data['PersonRegister_setDate_Range'][0]) ) {
					$filter .= " and PR.PersonRegister_setDate >= cast(:PersonRegister_setDate_Range_0 as timestamp) ";
				...
				if ( isset($data['PersonRegister_setDate_Range'][1]) ) {
					$filter .= " and PR.PersonRegister_setDate <= cast(:PersonRegister_setDate_Range_1 as timestamp) ";
				...
				if ( isset($data['PersonRegister_disDate_Range'][0]) ) {
					$filter .= " and PR.PersonRegister_disDate >= cast(:PersonRegister_disDate_Range_0 as timestamp) ";
				...
				if ( isset($data['PersonRegister_disDate_Range'][1]) ) {
					$filter .= " and PR.PersonRegister_disDate <= cast(:PersonRegister_disDate_Range_1 as timestamp) ";
				...
				if ( !empty($data['MorbusOnko_setDiagDT_Range'][0]) ) {
					$filter .= " and MO.MorbusOnko_setDiagDT >= cast(:MorbusOnko_setDiagDT_Range_0 as timestamp) ";
					$queryParams['MorbusOnko_setDiagDT_Range_0'] = $data['MorbusOnko_setDiagDT_Range'][0];
				}
				if ( !empty($data['MorbusOnko_setDiagDT_Range'][1]) ) {
					$filter .= " and MO.MorbusOnko_setDiagDT <= cast(:MorbusOnko_setDiagDT_Range_1 as timestamp) ";
				...
				if ( isset($data['MorbusOnkoSpecTreat_begDate_Range'][0]) && isset($data['MorbusOnkoSpecTreat_begDate_Range'][1]) ) {
					$filter .= ' and MO.MorbusOnko_specSetDT between cast(:MorbusOnkoSpecTreat_begDate_Range_0 as timestamp) and cast(:MorbusOnkoSpecTreat_begDate_Range_1 as timestamp) ';
				...
				if ( isset($data['MorbusOnkoSpecTreat_endDate_Range'][0]) && isset($data['MorbusOnkoSpecTreat_endDate_Range'][1]) ) {
					$filter .= ' and MO.MorbusOnko_specDisDT between cast(:MorbusOnkoSpecTreat_endDate_Range_0 as timestamp) and cast(:MorbusOnkoSpecTreat_endDate_Range_1 as timestamp) ';
				...
				)
				{
					...
					$query .= '
						inner join v_MorbusOnkoBasePersonState MOBPS on MOBPS.MorbusOnkoBase_id = MOB.MorbusOnkoBase_id '. $filterDop ;
				}
				if ( !empty($data['OnkoStatusYearEndType_id']) )
				{
					...
					$query .= '
						inner join v_MorbusOnkoBaseStatusYearEnd MOBSYE on MOBSYE.MorbusOnkoBase_id = MOB.MorbusOnkoBase_id '. $filterDop ;
				}
			...
			case 'HepatitisRegistry':
				$query .= '
					inner join v_PersonRegister PR on PR.Person_id = PS.Person_id and PR.MorbusType_id = 5
					left join v_PersonRegisterOutCause PROUT on PROUT.PersonRegisterOutCause_id = PR.PersonRegisterOutCause_id
				...
				if ( !empty($data['MorbusHepatitisDiag_setDT_Range'][0]) ) {
					$filter .= " and MHD.MorbusHepatitisDiag_setDT >= cast(:MorbusHepatitisDiag_setDT_Range_0 as timestamp) ";
				...
				if ( !empty($data['MorbusHepatitisDiag_setDT_Range'][1]) ) {
					$filter .= " and MHD.MorbusHepatitisDiag_setDT <= cast(:MorbusHepatitisDiag_setDT_Range_1 as timestamp) ";
				...
				if( (isset($data['MorbusHepatitisLabConfirm_setDT_Range'][0]) && isset($data['MorbusHepatitisLabConfirm_setDT_Range'][1])) || !empty($data['HepatitisLabConfirmType_id']) || !empty($data['MorbusHepatitisLabConfirm_Result']) ) {
					...
					$query .= '
						inner join v_MorbusHepatitisLabConfirm MHLC on MH.MorbusHepatitis_id = MHLC.MorbusHepatitis_id '. $filterDop ;
				}
				...
				if( (isset($data['MorbusHepatitisFuncConfirm_setDT_Range'][0]) && isset($data['MorbusHepatitisFuncConfirm_setDT_Range'][1])) || !empty($data['HepatitisFuncConfirmType_id']) || !empty($data['MorbusHepatitisFuncConfirm_Result']) ) {
					...
					$query .= '
						inner join v_MorbusHepatitisFuncConfirm MHFC on MH.MorbusHepatitis_id = MHFC.MorbusHepatitis_id '. $filterDop ;
				}
				...
				if( isset($data['MorbusHepatitisCure_begDT']) || isset($data['MorbusHepatitisCure_endDT']) || !empty($data['HepatitisResultClass_id']) || !empty($data['HepatitisSideEffectType_id']) || !empty($data['MorbusHepatitisCure_Drug']) ) {
					...
					$query .= '
						inner join v_MorbusHepatitisCure MHC on MH.MorbusHepatitis_id = MHC.MorbusHepatitis_id '. $filterDop ;
				}
			...
			case 'EvnNotifyOrphan':
				$query .= "
					left join lateral(
						...
						from v_EvnNotifyOrphan EvnNotifyOrphan 
						...
						select
							...
							PersonRegister.PersonRegister_id
						from v_EvnNotifyOrphanOut EvnNotifyOrphanOut 
						...
						select
							...
							PersonRegister.PersonRegister_id
						from v_EvnDirectionOrphan EvnDirectionOrphan 
				...
				if ( isset($data['EvnNotifyBase_setDT_Range'][0]) ) {
					$filter .= ' and ENO.EvnNotifyOrphan_setDT >= cast(:EvnNotifyBase_setDT_Range_0 as timestamp) ';
					$queryParams['EvnNotifyBase_setDT_Range_0'] = $data['EvnNotifyBase_setDT_Range'][0];
				}
				
				if ( isset($data['EvnNotifyBase_setDT_Range'][1]) ) {
					$filter .= ' and ENO.EvnNotifyOrphan_setDT <= cast(:EvnNotifyBase_setDT_Range_1 as timestamp) ';
			...
			case 'EvnNotifyCrazy': // Психиатрия
				$query .= '
					inner join v_EvnNotifyCrazy ENC on '. ( ($data['PersonPeriodicType_id'] == 2) ? 'ENC.PersonEvn_id = PS.PersonEvn_id and ENC.Server_id = PS.Server_id' : 'ENC.Person_id = PS.Person_id' ) .'
				...
				if ( isset($data['EvnNotifyBase_setDT_Range'][0]) ) {
					$filter .= ' and ENC.EvnNotifyCrazy_setDT >= cast(:EvnNotifyBase_setDT_Range_0 as timestamp) ';
					$queryParams['EvnNotifyBase_setDT_Range_0'] = $data['EvnNotifyBase_setDT_Range'][0];
				}

				if ( isset($data['EvnNotifyBase_setDT_Range'][1]) ) {
					$filter .= ' and ENC.EvnNotifyCrazy_setDT <= cast(:EvnNotifyBase_setDT_Range_1 as timestamp) ';
					$queryParams['EvnNotifyBase_setDT_Range_1'] = $data['EvnNotifyBase_setDT_Range'][1];
				}
			...
			case 'EvnNotifyTub': // Туберкулез
				$query .= '
					inner join v_EvnNotifyTub ENC on '. ( ($data['PersonPeriodicType_id'] == 2) ? 'ENC.PersonEvn_id = PS.PersonEvn_id and ENC.Server_id = PS.Server_id' : 'ENC.Person_id = PS.Person_id' ) .'
					inner join v_Morbus MO on MO.Morbus_id = ENC.Morbus_id
					left join v_PersonCard PC on PC.Person_id = PS.Person_id and PC.LpuAttachType_id = 1
					left join v_Lpu Lpu on Lpu.Lpu_id = PC.Lpu_id
				...
				if ( isset($data['EvnNotifyBase_setDT_Range'][0]) ) {
					$filter .= ' and ENC.EvnNotifyTub_setDT >= cast(:EvnNotifyBase_setDT_Range_0 as timestamp) ';
					$queryParams['EvnNotifyBase_setDT_Range_0'] = $data['EvnNotifyBase_setDT_Range'][0];
				}

				if ( isset($data['EvnNotifyBase_setDT_Range'][1]) ) {
					$filter .= ' and ENC.EvnNotifyTub_setDT <= cast(:EvnNotifyBase_setDT_Range_1 as timestamp) ';
					$queryParams['EvnNotifyBase_setDT_Range_1'] = $data['EvnNotifyBase_setDT_Range'][1];
				}
			...
			case 'EvnNotifyHIV': // Извещения ВИЧ
				$query .= '
					inner join v_EvnNotifyBase ENB on ENB.MorbusType_id = 9 and '. ( ($data['PersonPeriodicType_id'] == 2) ? 'ENB.PersonEvn_id = PS.PersonEvn_id and ENB.Server_id = PS.Server_id' : 'ENB.Person_id = PS.Person_id' ) .'
				...
				if ( isset($data['EvnNotifyBase_setDT_Range'][0]) ) {
					$filter .= ' and ENB.EvnNotifyBase_setDT >= cast(:EvnNotifyBase_setDT_Range_0 as timestamp) ';
					$queryParams['EvnNotifyBase_setDT_Range_0'] = $data['EvnNotifyBase_setDT_Range'][0];
				}

				if ( isset($data['EvnNotifyBase_setDT_Range'][1]) ) {
					$filter .= ' and ENB.EvnNotifyBase_setDT <= cast(:EvnNotifyBase_setDT_Range_1 as timestamp) ';
					$queryParams['EvnNotifyBase_setDT_Range_1'] = $data['EvnNotifyBase_setDT_Range'][1];
				}
			...
			case 'EvnNotifyVener': // Венер
				$query .= '
					inner join v_EvnNotifyVener ENC on '. ( ($data['PersonPeriodicType_id'] == 2) ? 'ENC.PersonEvn_id = PS.PersonEvn_id and ENC.Server_id = PS.Server_id' : 'ENC.Person_id = PS.Person_id' ) .'
					inner join v_Morbus MO on MO.Morbus_id = ENC.Morbus_id
					left join v_PersonCard PC on PC.Person_id = PS.Person_id and PC.LpuAttachType_id = 1
					left join v_Lpu Lpu on Lpu.Lpu_id = PC.Lpu_id
				...
				if ( isset($data['EvnNotifyBase_setDT_Range'][0]) ) {
					$filter .= ' and ENC.EvnNotifyVener_setDT >= cast(:EvnNotifyBase_setDT_Range_0 as timestamp) ';
					$queryParams['EvnNotifyBase_setDT_Range_0'] = $data['EvnNotifyBase_setDT_Range'][0];
				}

				if ( isset($data['EvnNotifyBase_setDT_Range'][1]) ) {
					$filter .= ' and ENC.EvnNotifyVener_setDT <= cast(:EvnNotifyBase_setDT_Range_1 as timestamp) ';
					$queryParams['EvnNotifyBase_setDT_Range_1'] = $data['EvnNotifyBase_setDT_Range'][1];
				}
			...
			case 'OrphanRegistry':
				$query .= '
					inner join v_PersonRegister PR on PR.Person_id = PS.Person_id and PR.MorbusType_id = 6
			...
			case 'CrazyRegistry':
				$query .= '
					inner join v_PersonRegister PR on PR.Person_id = PS.Person_id and PR.MorbusType_id = 4
					left join v_PersonRegisterOutCause PROUT on PROUT.PersonRegisterOutCause_id = PR.PersonRegisterOutCause_id
			...
			case 'TubRegistry':
				$query .= '
					inner join v_PersonRegister PR on PR.Person_id = PS.Person_id and PR.MorbusType_id = 7
					left join v_PersonRegisterOutCause PROUT on PROUT.PersonRegisterOutCause_id = PR.PersonRegisterOutCause_id
			...
			case 'LargeFamilyRegistry':
				...
				$query .= '
					inner join v_PersonRegister PR on PR.Person_id = PS.Person_id and PR.MorbusType_id = '. $registers[$data['SearchFormType']] .'
					left join v_PersonRegisterOutCause PROUT on PROUT.PersonRegisterOutCause_id = PR.PersonRegisterOutCause_id
					left join v_PersonCard PC on PC.Person_id = PS.Person_id and PC.LpuAttachType_id = 1
					left join v_Lpu Lpu on Lpu.Lpu_id = PC.Lpu_id
					left join v_Diag Diag on Diag.Diag_id = PR.Diag_id
			...
			case 'HIVRegistry':
				$query .= '
					inner join v_PersonRegister PR on PR.Person_id = PS.Person_id and PR.MorbusType_id = 9
					left join v_PersonRegisterOutCause PROUT on PROUT.PersonRegisterOutCause_id = PR.PersonRegisterOutCause_id
			...
			case 'VenerRegistry':
				$query .= '
					inner join v_PersonRegister PR on PR.Person_id = PS.Person_id and PR.MorbusType_id = 8
					left join v_PersonRegisterOutCause PROUT on PROUT.PersonRegisterOutCause_id = PR.PersonRegisterOutCause_id
		...
		if ( ($data['SearchFormType'] == 'PersonPrivilege')) {
			$query .= " inner join v_PersonPrivilege PP on PP.Person_id = PS.Person_id";
			$query .= " inner join v_PrivilegeType PT on PT.PrivilegeType_id = PP.PrivilegeType_id";
			$query .= " left join v_PrivilegeAccessRights PAR on PAR.PrivilegeType_id = PP.PrivilegeType_id";
			//$query .= " inner join v_Lpu LPU on LPU.Lpu_id = PP.Lpu_id";
		...
		// Подключаем PersonPrivilege, если заданы фильтры на вкладке "Льгота"
			...
			$filter .= " and exists (select personprivilege_id from v_PersonPrivilege PP ";
			$filter .= " inner join v_PrivilegeType PT on PT.PrivilegeType_id = PP.PrivilegeType_id";
			$filter .= " left join v_PrivilegeAccessRights PAR on PAR.PrivilegeType_id = PP.PrivilegeType_id";
		...
		if ( ( isset($data['Refuse_id']) ) || ( isset($data['RefuseNextYear_id']) ) ) {
			...
			if ( isset($data['Refuse_id']) ) {
				$filter .= " and (exists (
					...
					where PP.Person_id = PS.Person_id
						and coalesce(PS.Person_IsRefuse2, 1) = :Refuse_id
				...
				if ($data['Refuse_id'] == 1 ) {
					$filter .= " or
						not exists ( select PersonRefuse_id from v_PersonRefuse PR where PR.Person_id = PS.Person_id and PR.PersonRefuse_Year = date_part('year', dbo.tzGetDate()) )
					";
				}
				*/
				$filter .= ")";
			...
			if ( isset($data['RefuseNextYear_id']) ) {
				$filter .= " and (exists (
					select 1
					from v_PersonPrivilege PPN
						inner join v_PrivilegeType PTN on PTN.PrivilegeType_id = PPN.PrivilegeType_id
							and PTN.ReceptFinance_id = 1
						left join lateral(
							select
								coalesce(PRN.PersonRefuse_IsRefuse, 1) as PersonRefuse_IsRefuse
							from v_PersonRefuse PRN
							where PRN.Person_id = PPN.Person_id
								and coalesce(PRN.PersonRefuse_Year, date_part('year', dbo.tzGetDate())) = date_part('year', dbo.tzGetDate()) + 1
							limit 1
						) PRefN on true
					where PPN.Person_id = PS.Person_id
						and coalesce(PRefN.PersonRefuse_IsRefuse, 1) = :RefuseNextYear_id
						and PPN.PersonPrivilege_begDate <= dbo.tzGetDate()
						and (PPN.PersonPrivilege_endDate is null or PPN.PersonPrivilege_endDate >= cast(to_char(dbo.tzGetDate(), 'yyyymmdd') as timestamp))
				";
		...
		if ( $data['SearchFormType'] == 'PersonCard' ) {
			...
			if ( $data['PersonCardStateType_id'] == 1 ) {
                if ($data['session']['region']['nick'] == 'khak') {
                    ...
                    $query .= " inner join v_PersonCard PC on PC.Person_id = PS.Person_id";
                }
                //$query .= " inner join v_PersonCard PC on PC.Person_id = PS.Person_id";
				$filter .= " and (PC.PersonCard_endDate is null or cast(PC.PersonCard_endDate as timestamp) > dbo.tzGetDate())";
			}
			else {
				$query .= " inner join v_PersonCard_all PC on PC.Person_id = PS.Person_id";
			}

			$query .= " left join LpuRegion LR on LR.LpuRegion_id = PC.LpuRegion_id";
			$query .= " left join v_Lpu AttachLpu on AttachLpu.Lpu_id = PC.Lpu_id";
			$query .= " 
				left join Address uaddr on uaddr.Address_id = ps.UAddress_id
				left join Address paddr on paddr.Address_id = ps.PAddress_id
			";
			$query .= " left join PersonRefuse PRef on (PRef.Person_id = ps.Person_id and PRef.PersonRefuse_Year=date_part('year', dbo.tzGetDate())+1)";
			...
			if ( $data['PersonCardStateType_id'] == 1 ) {
				...
				left join lateral(
					...
					from Person
					left join v_PersonCard PC_MAIN on PC_MAIN.Person_id = PS.Person_id and PC_MAIN.LpuAttachType_id = 1
				...
				left join lateral(
					...
					from Person
					left join v_PersonCard_all PC_MAIN on PC_MAIN.Person_id = PS.Person_id and PC_MAIN.LpuAttachType_id = 1
					-- Если у пациента нет основного прикрепления, то показывать гинекологическое, стоматологическое прикрепление
		...
		if (substr($data['SearchFormType'],0,3) == 'Kvs') { //для выгрузки квс в дбф
			...
			if ( isset($data['InsDate']) ) {
				if ($data['SearchFormType'] == 'KvsEvnLeave')
					$filter .= " and (
						(EPS.LeaveType_id = 1 and cast(to_char(ELV.EvnLeave_insDT, 'yyyymmdd') as timestamp) = cast(:InsDate as timestamp))
						or (EPS.LeaveType_id = 2 and cast(to_char(EOLpu.EvnOtherLpu_insDT, 'yyyymmdd') as timestamp) = cast(:InsDate as timestamp))
						or (EPS.LeaveType_id = 3 and cast(to_char(EDie.EvnDie_insDT, 'yyyymmdd') as timestamp) = cast(:InsDate as timestamp))
						or (EPS.LeaveType_id = 4 and cast(to_char(EOStac.EvnOtherStac_insDT, 'yyyymmdd') as timestamp) = cast(:InsDate as timestamp))
						or (EPS.LeaveType_id = 5 and cast(to_char(EOSect.EvnOtherSection_insDT, 'yyyymmdd') as timestamp) = cast(:InsDate as timestamp))
					)";
				else
					$filter .= " and cast(to_char(".$fld_name.($is_pc ? "Beg_insDT" : "_insDT").", 'yyyymmdd') as timestamp) = cast(:InsDate as timestamp)";
			...
			if ( isset($data['InsDate_Range'][0]) ) {
				if ($data['SearchFormType'] == 'KvsEvnLeave')
					$filter .= " and (
						(EPS.LeaveType_id = 1 and cast(to_char(ELV.EvnLeave_insDT, 'yyyymmdd') as timestamp) >= cast(:InsDate_Range_0 as timestamp))
						or (EPS.LeaveType_id = 2 and cast(to_char(EOLpu.EvnOtherLpu_insDT, 'yyyymmdd') as timestamp) >= cast(:InsDate_Range_0 as timestamp))
						or (EPS.LeaveType_id = 3 and cast(to_char(EDie.EvnDie_insDT, 'yyyymmdd') as timestamp) >= cast(:InsDate_Range_0 as timestamp))
						or (EPS.LeaveType_id = 4 and cast(to_char(EOStac.EvnOtherStac_insDT, 'yyyymmdd') as timestamp) >= cast(:InsDate_Range_0 as timestamp))
						or (EPS.LeaveType_id = 5 and cast(to_char(EOSect.EvnOtherSection_insDT, 'yyyymmdd') as timestamp) >= cast(:InsDate_Range_0 as timestamp))
					)";
				else
					$filter .= " and cast(to_char(".$fld_name.($is_pc ? "Beg_insDT" : "_insDT").", 'yyyymmdd') as timestamp) >= cast(:InsDate_Range_0 as timestamp)";
			...
			if ( isset($data['InsDate_Range'][1]) ) {
				if ($data['SearchFormType'] == 'KvsEvnLeave')
					$filter .= " and (
						(EPS.LeaveType_id = 1 and cast(to_char(ELV.EvnLeave_insDT, 'yyyymmdd') as timestamp) <= cast(:InsDate_Range_1 as timestamp))
						or (EPS.LeaveType_id = 2 and cast(to_char(EOLpu.EvnOtherLpu_insDT, 'yyyymmdd') as timestamp) <= cast(:InsDate_Range_1 as timestamp))
						or (EPS.LeaveType_id = 3 and cast(to_char(EDie.EvnDie_insDT, 'yyyymmdd') as timestamp) <= cast(:InsDate_Range_1 as timestamp))
						or (EPS.LeaveType_id = 4 and cast(to_char(EOStac.EvnOtherStac_insDT, 'yyyymmdd') as timestamp) <= cast(:InsDate_Range_1 as timestamp))
						or (EPS.LeaveType_id = 5 and cast(to_char(EOSect.EvnOtherSection_insDT, 'yyyymmdd') as timestamp) <= cast(:InsDate_Range_1 as timestamp))
					)";
				else
					$filter .= " and cast(to_char(".$fld_name.($is_pc ? "Beg_insDT" : "_insDT").", 'yyyymmdd') as timestamp) <= cast(:InsDate_Range_1 as timestamp)";
			...
			if ( isset($data['UpdDate']) ) {
				if ($data['SearchFormType'] == 'KvsEvnLeave')
					$filter .= " and (
						(EPS.LeaveType_id = 1 and cast(to_char(ELV.EvnLeave_updDT, 'yyyymmdd') as timestamp) = cast(:UpdDate as timestamp))
						or (EPS.LeaveType_id = 2 and cast(to_char(EOLpu.EvnOtherLpu_updDT, 'yyyymmdd') as timestamp) = cast(:UpdDate as timestamp))
						or (EPS.LeaveType_id = 3 and cast(to_char(EDie.EvnDie_updDT, 'yyyymmdd') as timestamp) = cast(:UpdDate as timestamp))
						or (EPS.LeaveType_id = 4 and cast(to_char(EOStac.EvnOtherStac_updDT, 'yyyymmdd') as timestamp) = cast(:UpdDate as timestamp))
						or (EPS.LeaveType_id = 5 and cast(to_char(EOSect.EvnOtherSection_updDT, 'yyyymmdd') as timestamp) = cast(:UpdDate as timestamp))
					)";
				else
					$filter .= " and cast(to_char(".$fld_name.($is_pc ? "Beg_insDT" : "_updDT").", 'yyyymmdd') as timestamp) = cast(:UpdDate as timestamp)";
			...
			if ( isset($data['UpdDate_Range'][0]) ) {
				if ($data['SearchFormType'] == 'KvsEvnLeave')
					$filter .= " and (
						(EPS.LeaveType_id = 1 and cast(to_char(ELV.EvnLeave_updDT, 'yyyymmdd') as timestamp) >= cast(:UpdDate_Range_0 as timestamp))
						or (EPS.LeaveType_id = 2 and cast(to_char(EOLpu.EvnOtherLpu_updDT, 'yyyymmdd') as timestamp) >= cast(:UpdDate_Range_0 as timestamp))
						or (EPS.LeaveType_id = 3 and cast(to_char(EDie.EvnDie_updDT, 'yyyymmdd') as timestamp) >= cast(:UpdDate_Range_0 as timestamp))
						or (EPS.LeaveType_id = 4 and cast(to_char(EOStac.EvnOtherStac_updDT, 'yyyymmdd') as timestamp) >= cast(:UpdDate_Range_0 as timestamp))
						or (EPS.LeaveType_id = 5 and cast(to_char(EOSect.EvnOtherSection_updDT, 'yyyymmdd') as timestamp) >= cast(:UpdDate_Range_0 as timestamp))
					)";
				else
					$filter .= " and cast(to_char(".$fld_name.($is_pc ? "Beg_insDT" : "_updDT").", 'yyyymmdd') as timestamp) >= cast(:UpdDate_Range_0 as timestamp)";
			...
			if ( isset($data['UpdDate_Range'][1]) ) {
				if ($data['SearchFormType'] == 'KvsEvnLeave')
					$filter .= " and (
						(EPS.LeaveType_id = 1 and cast(to_char(ELV.EvnLeave_updDT, 'yyyymmdd') as timestamp) <= cast(:UpdDate_Range_1 as timestamp))
						or (EPS.LeaveType_id = 2 and cast(to_char(EOLpu.EvnOtherLpu_updDT, 'yyyymmdd') as timestamp) <= cast(:UpdDate_Range_1 as timestamp))
						or (EPS.LeaveType_id = 3 and cast(to_char(EDie.EvnDie_updDT, 'yyyymmdd') as timestamp) <= cast(:UpdDate_Range_1 as timestamp))
						or (EPS.LeaveType_id = 4 and cast(to_char(EOStac.EvnOtherStac_updDT, 'yyyymmdd') as timestamp) <= cast(:UpdDate_Range_1 as timestamp))
						or (EPS.LeaveType_id = 5 and cast(to_char(EOSect.EvnOtherSection_updDT, 'yyyymmdd') as timestamp) <= cast(:UpdDate_Range_1 as timestamp))
					)";
				else
					$filter .= " and cast(to_char(".$fld_name.($is_pc ? "Beg_insDT" : "_updDT").", 'yyyymmdd') as timestamp) <= cast(:UpdDate_Range_1 as timestamp)";
			...
			if ( isset($data['InsDate']) ) {
				if ($data['SearchFormType'] == 'PersonCard' && $data['PersonCardStateType_id'] != 1) {
					$filter .= " and cast(to_char(" . $main_alias . "." . $data['SearchFormType'] . "Beg_insDT, 'yyyymmdd') as timestamp) = cast(:InsDate as timestamp)";
				} else {
					$filter .= " and cast(to_char(" . $main_alias . "." . $fld_name . "_insDT, 'yyyymmdd') as timestamp) = cast(:InsDate as timestamp)";
				}			
				$queryParams['InsDate'] = $data['InsDate'];
			}
			...
			if ( isset($data['InsDate_Range'][0]) ) {
				if ($data['SearchFormType'] == 'PersonCard' && $data['PersonCardStateType_id'] != 1) {
					$filter .= " and cast(to_char(" . $main_alias . "." . $data['SearchFormType'] . "Beg_insDT, 'yyyymmdd') as timestamp) >= cast(:InsDate_Range_0 as timestamp)";
				} else {
					$filter .= " and cast(to_char(" . $main_alias . "." . $fld_name . "_insDT, 'yyyymmdd') as timestamp) >= cast(:InsDate_Range_0 as timestamp)"; 
				}
				$queryParams['InsDate_Range_0'] = $data['InsDate_Range'][0];
			}
			...
			if ( isset($data['InsDate_Range'][1]) ) {
				if ($data['SearchFormType'] == 'PersonCard' && $data['PersonCardStateType_id'] != 1) {
					$filter .= " and cast(to_char(" . $main_alias . "." . $data['SearchFormType'] . "Beg_insDT, 'yyyymmdd') as timestamp) <= cast(:InsDate_Range_1 as timestamp)";
				} else {
					$filter .= " and cast(to_char(" . $main_alias . "." . $fld_name . "_insDT, 'yyyymmdd') as timestamp) <= cast(:InsDate_Range_1 as timestamp)";
				}
				$queryParams['InsDate_Range_1'] = $data['InsDate_Range'][1];
			}
			...
			if ( isset($data['UpdDate']) ) {
				if ($data['SearchFormType'] == 'PersonCard' && $data['PersonCardStateType_id'] != 1) {
					$filter .= " and cast(to_char(" . $main_alias . "." . $data['SearchFormType'] . "Beg_insDT, 'yyyymmdd') as timestamp) = cast(:UpdDate  as timestamp)";
				} else {
					$filter .= " and cast(to_char(" . $main_alias . "." . $fld_name . "_updDT, 'yyyymmdd') as timestamp) = cast(:UpdDate as timestamp)";
				}
				$queryParams['UpdDate'] = $data['UpdDate'];
			}
			...
			if ( isset($data['UpdDate_Range'][0]) ) {
				if ($data['SearchFormType'] == 'PersonCard' && $data['PersonCardStateType_id'] != 1) {
					$filter .= " and cast(to_char(" . $main_alias . "." . $data['SearchFormType'] . "Beg_insDT, 'yyyymmdd') as timestamp) >= cast(:UpdDate_Range_0 as timestamp)";
				} else {
					$filter .= " and cast(to_char(" . $main_alias . "." . $fld_name . "_updDT, 'yyyymmdd') as timestamp) >= cast(:UpdDate_Range_0 as timestamp)";
				}
				$queryParams['UpdDate_Range_0'] = $data['UpdDate_Range'][0];
			}
			...
			if ( isset($data['UpdDate_Range'][1]) ) {
				if ($data['SearchFormType'] == 'PersonCard' && $data['PersonCardStateType_id'] != 1) {
					$filter .= " and cast(to_char(" . $main_alias . "." . $data['SearchFormType'] . "Beg_insDT, 'yyyymmdd') as timestamp) <= cast(:UpdDate_Range_1 as timestamp)";
				} else {
					$filter .= " and cast(to_char(" . $main_alias . "." . $fld_name . "_updDT, 'yyyymmdd') as timestamp) <= cast(:UpdDate_Range_1 as timestamp)";
				}
				$queryParams['UpdDate_Range_1'] = $data['UpdDate_Range'][1];
			}
		}
