...
class EvnVizitPLStom_model extends EvnVizitPL_model
{
	...
	static function defAttributes()
	{
		...
		$arr['tooth_code'] = array(
			...
			'join' => 'left join v_Tooth on v_Tooth.Tooth_id = {ViewName}.Tooth_id',
			'label' => 'Код зуба',
			'save' => 'trim',
			'type' => 'int'
		);
	 ...
	 * @return bool
	 */
	public function _isRequiredDiag()
	{
	...
	protected function _beforeSave($data = array())
	{
        ...
        if (in_array($this->getRegionNick(), array('perm','vologda')) && $data && $data['EvnVizitPLStom_setDate'] && $data['EvnVizitPLStom_setDate'] > "2015-10-31") {
			...
			if (!empty($this->id)) {
				$query = "
					select
						edps.EvnDiagPLStom_id as \"EvnDiagPLStom_id\",
					...
					where
						edps.EvnDiagPLStom_id IN (select EvnDiagPLStom_id from v_EvnUslugaStom where EvnUslugaStom_pid = :EvnUslugaStom_pid)
				";
				...
				foreach($resp_edps as $resp_edpsone) {
					if (!empty($resp_edpsone['Mes_id'])) {
						...
						if (!empty($resp_edpsone['EvnUslugaStom_setDate']) && strtotime($resp_edpsone['EvnUslugaStom_setDate']) < $EvnUslugaStom_setDate) {
							$EvnUslugaStom_setDate = strtotime($resp_edpsone['EvnUslugaStom_setDate']);
						}

						$EvnUslugaStom_disDate = strtotime($this->setDate);
						if (!empty($resp_edpsone['EvnUslugaStom_disDate']) && strtotime($resp_edpsone['EvnUslugaStom_disDate']) > $EvnUslugaStom_disDate) {
							$EvnUslugaStom_disDate = strtotime($resp_edpsone['EvnUslugaStom_disDate']);
						}

						// получили даты, проверяем дату КСГ
						if (
							(!empty($resp_edpsone['Mes_begDT']) && strtotime($resp_edpsone['Mes_begDT']) > $EvnUslugaStom_setDate)
							|| (!empty($resp_edpsone['Mes_endDT']) && strtotime($resp_edpsone['Mes_endDT']) < $EvnUslugaStom_disDate)
						) {
		...
		if (!empty($resp_save[0]['EvnVizitPLStom_id'])) {
			...
			$usluga_resp = $this->queryResult("
				select
					...
					EvnUslugaCommon_Price as \"EvnUslugaCommon_Price\"
				from
					v_EvnUslugaCommon 
				where
			...
			if (!empty($this->UslugaComplex_id)) {
				...
				$query = "				
					...
					from dbo.p_EvnUslugaCommon_{$action}(
	...
	function updateUslugaComplexTariffId($id, $value = null)
	{
		$usluga = $this->getFirstRowFromQuery("
			select
				EvnUslugaCommon_id as \"EvnUslugaCommon_id\",
				EvnUslugaCommon_pid as \"EvnUslugaCommon_pid\",
				Lpu_id as \"Lpu_id\",
				Server_id as \"Server_id\",
				PersonEvn_id as \"PersonEvn_id\",
				to_char(EvnUslugaCommon_setDT, 'yyyy-mm-dd') as \"EvnUslugaCommon_setDT\",
				...
				EvnUslugaCommon_Price as \"EvnUslugaCommon_Price\",
				EvnUslugaCommon_Summa as \"EvnUslugaCommon_Summa\"
		...
		$price = $this->getFirstResultFromQuery("
			select
				UslugaComplexTariff_UED as \"UslugaComplexTariff_UED\"
		...
		if (count($usluga) && !empty($usluga['EvnUslugaCommon_id'])) {
			$query = "
				select
					...
					Error_Message as \"Error_Msg\",
					EvnUslugaCommon_id as \"EvnUslugaCommon_id\"
				from dbo.p_EvnUslugaCommon_upd(
					EvnUslugaCommon_id := :EvnUslugaCommon_id,
			...
			if($currentPD[0]['BitePersonType_id'] != $data['BitePersonType_id']){
				...
				$query = "
					select
						Error_Code as \"Error_Code\",
						Error_Message as \"Error_Msg\",
						BitePersonData_id as \"BitePersonData_id\"
					from dbo.p_BitePersonData_upd(
						BitePersonData_id := :BitePersonData_id,
						Person_id := :Person_id,
	...
	function getCurrentBitePersonData($data){
		...
		select
    		...
    		bitepersondata_setdate as \"BitePersonData_setDate\",
    		bitepersondata_disdate as \"BitePersonData_disDate\",
    		bitepersontype_id as \"BitePersonType_id\",
    		pmuser_insid as \"pmUser_insID\",
    		pmuser_updid as \"pmUser_updID\",
    		bitepersondata_insdt as \"BitePersonData_insDT\",
    		bitepersondata_upddt as \"BitePersonData_updDT\"
		...
		if (!empty($data['LpuSection_id'])) {
			...
			$Lpu_id = $this->getFirstResultFromQuery("
				select
					Lpu_id as \"Lpu_id\"
				from v_LpuSection
				where LpuSection_id = :LpuSection_id", array('LpuSection_id' => $data['LpuSection_id']));
			if (!empty($Lpu_id)) {
     ...
     * @param array $data
     */
    function set_IsPrimaryVizit($data = [])
    {
        if(!empty($this->setDate) && !empty($this->Person_id) && !empty($this->VizitType_id) && $this->scenario == self::SCENARIO_AUTO_CREATE /*&& in_array(getRegionNick(), array('kareliya'))*/){
            ...
            $query = "
				...
				from 
					v_EvnPLStom EPLS
					left join v_EvnVizitPLStom EVPLS on EPLS.EvnPLStom_id = EVPLS.EvnVizitPLStom_pid
				where 
				    (1 = 1)
                and EPLS.EvnClass_id = 6 
                and coalesce (EPLS.EvnPLStom_IsArchive, 1) = 1
                and EPLS.Person_id = :Person_id
                and date_part('year', EPLS.EvnPLStom_setDate) = :Year
