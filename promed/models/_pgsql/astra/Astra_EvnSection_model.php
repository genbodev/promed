...
class Astra_EvnSection_model extends EvnSection_model {
	...
	function loadKSGKPGKOEFCombo2019($data) {
		...
		$query = "
			select
				...
				PS.Sex_id as \"Sex_id\",
				case
					when date_part('month', PS.Person_BirthDay) = date_part('month', cast(:EvnSection_setDate as date))
						and date_part('day', PS.Person_BirthDay) = date_part('day', cast(:EvnSection_setDate as date))
							then 1
							else 0
				end as \"BirthToday\"
			from
		...
		if (isset($data['UslugaComplexIds']) && isset($data['UslugaComplexCodes'])) {
			...
			if (!empty($EvnSectionIds)) {
				$query = "
					...
					from
						v_EvnUsluga eu
						inner join v_PayType pt on pt.PayType_id = eu.PayType_id {$filter_paytype}
		...
		if (!empty($EvnSectionIds)) {
			$query = "
				select
					to_char(eu.EvnUsluga_setDT, 'dd.mm.yyyy HH24:MI:SS') as \"EvnUsluga_setDT\",
					to_char(eu.EvnUsluga_disDT, 'dd.mm.yyyy HH24:MI:SS') as \"EvnUsluga_disDT\"
				from
					v_EvnUsluga eu
					inner join v_PayType pt on pt.PayType_id = eu.PayType_id {$filter_paytype}
					inner join v_UslugaComplex uc on uc.UslugaComplex_id = eu.UslugaComplex_id
			...
			if (!empty($UslugaComplexIds)) {
				...
				if ( $step == 2 ) {
					...
					$query = "
						...
						select
							uc.UslugaComplex_idas as \"UslugaComplex_id\",
							mu.MesOldUslugaComplex_idas as \"MesOldUslugaComplex_id\",
							mo.Mes_Codeas as \"Mes_Code\",
							mo.Mes_Nameas as \"Mes_Name\",
							mo.MesOld_Numas as \"MesOld_Num\",
							mo.Mes_idas as \"Mes_id\",
							mt.MesTariff_Valueas as \"MesTariff_Value\",
							mt.MesTariff_idas as \"MesTariff_id\",
							mo.MesType_idas as \"MesType_id\",
							mu.Diag_nidas as \"Diag_nid\",
							mu.Diag_idas as \"Diag_id\",
							case when mu.DrugTherapyScheme_id is not null then 1 else 0 end
								+ case when mu.RehabScale_id is not null then 1 else 0 end
								+ case when mu.MesOldUslugaComplex_FracFrom is not null or mu.MesOldUslugaComplex_FracTo is not null then 1 else 0 end -- считаются как 1 критерий
								+ case when mu.MesOldUslugaComplex_SofaScalePoints is not null or mu.MesOldUslugaComplex_IVLHours is not null then 1 else 0 end
	...
	function loadKSGKPGKOEFCombo2018($data) {
		...
		$data['VolumeType_id'] = $this->getFirstResultFromQuery("select VolumeType_id as \"VolumeType_id\" from v_VolumeType where VolumeType_Code = :VolumeType_Code limit 1", array(
		...
		if (!empty($data['Diag_id'])) {
			...
			$ucin = array();
			if (!empty($data['EvnSection_id'])) {
				$uc_resp = $this->queryResult("select eu.UslugaComplex_id as \"UslugaComplex_id\" from v_EvnUsluga eu where eu.EvnUsluga_pid = :EvnSection_id and eu.PayType_id = :PayTypeOms_id", array(
					'EvnSection_id' => $data['EvnSection_id'],
	...
	function loadKSGKPGKOEFCombo2015($data) {
					...
					if($resp[0]['PolyTrauma_Code'] == 7 || !empty($resp[0]['EvnDiagPS_id'])) {
						$query = "
							select
								...
								mt.MesTariff_Value as \"MesTariff_Value\",
								mt.MesTariff_id as \"MesTariff_id\",
								MESV.MesAgeGroup_id as \"MesAgeGroup_id\"
							from
								v_MesOld mo
								{$crossapply}
								{$crossapplymesvol}
		...
		if (!empty($data['Diag_id'])) {
			...
			$diagin = array();
			if (!empty($data['EvnSection_id'])) {
				$diag_resp = $this->queryResult("select Diag_id as \"Diag_id\" from v_EvnDiagPS where DiagSetClass_id IN (2,3) and EvnDiagPS_pid = :EvnSection_id", array(
					'EvnSection_id' => $data['EvnSection_id']
				));
		...
		if (!empty($data['LpuSectionProfile_id'])) {
			$query = "
				select
					...
					mtkpg.MesTariff_Value as \"MesTariff_Value\",
					mtkpg.MesTariff_id as \"MesTariff_id\",
					mokpg.MesAgeGroup_id as \"MesAgeGroup_id\"
				from MesOld mokpg
					inner join v_MesTariff mtkpg on mtkpg.Mes_id = mokpg.Mes_id -- Коэффициент КПГ
				where
		...
		foreach($combovalues as &$onevalue) {
			...
			if ($onevalue['MesType_id'] != 4) {
				$from = "
					v_MesLink ml
					inner join v_MesOld mo on mo.Mes_id = ml.Mes_sid
				";
				$filter = "
					ml.Mes_id = :Mes_id
			...
			$query = "
				select
					MesVol_id as \"MesVol_id\"
				from
					{$from}
					inner join r30.v_MesVol mv on mv.Mes_id = mo.Mes_id
				where
					{$filter}
					...
					if($resp[0]['PolyTrauma_Code'] == 7 || !empty($resp[0]['EvnDiagPS_id'])) {
						$query = "
							...
							from
								...
								left join v_MesTariff mt on mt.Mes_id = mo.Mes_id and mt.MesPayType_id = 5 -- Коэффициент КСГ
							where
		...
		if (!empty($data['EvnSection_id'])) {
			$query = "
				...
				from v_EvnUsluga eu
					...
					inner join v_UslugaCategory ucat on ucat.UslugaCategory_id = uc.UslugaCategory_id and ucat.UslugaCategory_SysNick = 'gost2011'
					inner join v_MesOldUslugaComplex mu on mu.UslugaComplex_id = uc.UslugaComplex_id
					inner join v_MesOld mo on mo.Mes_id = mu.Mes_id and mo.MesType_id IN (2,3,5) -- КСГ терапевтическая
					inner join lateral(
						select
							MesVol_id
						from
						...
						where
							...
							and LpuUnitType_id = :LpuUnitType_id
							and MesVol_begDate <= :EvnSection_disDate
							and (coalesce(MesVol_endDate, :EvnSection_disDate) >= :EvnSection_disDate)
						limit 1
					) MESV on true
					left join v_MesTariff mt on mt.Mes_id = mo.Mes_id and mt.MesPayType_id = 5 -- Коэффициент КСГ
				where
		...
		if (!empty($data['Diag_id'])) {
			$query = "
				...
				from v_MesOldDiag mod
					inner join v_MesOld mo on mo.Mes_id = mod.Mes_id and mo.MesType_id IN (2,3,5) -- КСГ терапевтическая
					inner join lateral(
						select
							MesVol_id
						from
				...
				where
					...
					and (
						...
						OR
						exists (select EvnUsluga_id from v_EvnUsluga eu inner join v_UslugaComplex uc on uc.UslugaComplex_id = eu.UslugaComplex_id where uc.UslugaComplex_Code in ('A25.30.014', 'A25.30.032') and eu.EvnUsluga_pid = :EvnSection_id and eu.PayType_id = :PayTypeOms_id limit 1)
					)
					and (
						d.Diag_Code NOT IN ('O04.0', 'O04.1', 'O04.2', 'O04.3', 'O04.4', 'O04.5', 'O04.6', 'O04.7', 'O04.8', 'O04.9')
						OR
						exists (select EvnUsluga_id from v_EvnUsluga eu inner join v_UslugaComplex uc on uc.UslugaComplex_id = eu.UslugaComplex_id where uc.UslugaComplex_Code in ('A16.20.037') and eu.EvnUsluga_pid = :EvnSection_id and eu.PayType_id = :PayTypeOms_id limit 1)
					)
				order by
					mt.MesTariff_Value desc
				limit 1
			...
			if (is_object($result)) {
				...
				if (count($resp) > 0) {
					...
					if ( in_array($resp[0]['Diag_Code'], array('I20.1', 'I20.8', 'I20.9', 'I24.0', 'I24.1', 'I24.8', 'I24.9', 'I25.0', 'I25.1', 'I25.2', 'I25.3', 'I25.4', 'I25.5', 'I25.6', 'I25.8', 'I25.9')) ) {
						...
						$EvnUsluga_id = $this->getFirstResultFromQuery("select EvnUsluga_id as \"EvnUsluga_id\" from v_EvnUsluga eu inner join v_UslugaComplex uc on uc.UslugaComplex_id = eu.UslugaComplex_id where uc.UslugaComplex_Code in ('A06.10.006') and eu.EvnUsluga_pid = :EvnSection_id and eu.PayType_id = :PayTypeOms_id limit 1", array('EvnSection_id' => $data['EvnSection_id'], 'PayTypeOms_id' => $data['PayTypeOms_id']));
								...
								where
									...
									and (exists (
										select
											MesOldUslugaComplex_id
										from
											v_MesOldUslugaComplex muc
											inner join v_UslugaComplex uc on uc.UslugaComplex_id = muc.UslugaComplex_id
										where
					...
					if ( in_array($resp[0]['Diag_Code'], array('I20.0', 'I21.0', 'I21.1', 'I21.2', 'I21.3', 'I21.4', 'I21.9', 'I22.0', 'I22.1', 'I22.8', 'I22.9')) ) {
						...
						$EvnUsluga_id = $this->getFirstResultFromQuery("select EvnUsluga_id as \"EvnUsluga_id\" from v_EvnUsluga eu inner join v_UslugaComplex uc on uc.UslugaComplex_id = eu.UslugaComplex_id where uc.UslugaComplex_Code in ('A16.12.004.009') and eu.EvnUsluga_pid = :EvnSection_id and eu.PayType_id = :PayTypeOms_id limit 1", array('EvnSection_id' => $data['EvnSection_id'], 'PayTypeOms_id' => $data['PayTypeOms_id']));
		...
		if (empty($data['forGroup'])) {
			$query = "
				select
					...
					null as \"Mes_Name\",
					coalesce(cast(Mes.Mes_KoikoDni as varchar), '') as \"Mes_KoikoDni\",
					'' as \"MesOperType_Name\"
				from v_MesOld Mes
				where
					Mes.Diag_id = :Diag_id
			...
			$query = "
				...
				from v_MesOld Mes
				where
					Mes.Diag_id in (select Diag_pid from v_Diag where Diag_id = :Diag_id limit 1)
			";
		}
	...
	protected function calcCoeffCTP($data) {
		...
		$resp_codes = $this->queryResult("
			...
			where
				...
				and avis.AttributeVision_TablePKey in (select TariffClass_id from v_TariffClass where TariffClass_SysNick = 'kclp' limit 1)
				and avis.AttributeVision_IsKeyValue = 2
				and coalesce(av.AttributeValue_begDate, :EvnSection_disDate) <= :EvnSection_disDate
				and coalesce(av.AttributeValue_endDate, :EvnSection_disDate) >= :EvnSection_disDate
		", array(
		...
		if (isset($KSLPCodes[$code]) && $data['LpuUnitType_id'] == '1') {
			$AttributeValue = $this->queryResult("
				...
				WHERE
					avis.AttributeVision_TableName = 'dbo.TariffClass'
					and avis.AttributeVision_TablePKey in (select TariffClass_id from v_TariffClass where TariffClass_SysNick = 'diagp' limit 1)
					and avis.AttributeVision_IsKeyValue = 2
					and coalesce(av.AttributeValue_begDate, :EvnSection_disDate) <= :EvnSection_disDate
					and coalesce(av.AttributeValue_endDate, :EvnSection_disDate) >= :EvnSection_disDate
			...
			$EvnUsluga = $this->queryResult("
				with eus as (
					...
					FROM
						...
						inner join v_MesOld mo on mo.Mes_id = mouc.Mes_id {$mesTypeFilter}
					where
						eu.EvnUsluga_pid = :EvnSection_id
					...
					with dts as (
						...
						FROM
							...
							inner join v_DrugTherapyScheme dts on dts.DrugTherapyScheme_id = esdts.DrugTherapyScheme_id
								and dts.DrugTherapyScheme_Code ilike 'sh%'
							inner join v_MesOldUslugaComplex mouc on mouc.DrugTherapyScheme_id = esdts.DrugTherapyScheme_id
								and coalesce(mouc.MesOldUslugaComplex_begDT, :EvnSection_disDate) <= :EvnSection_disDate
								and coalesce(mouc.MesOldUslugaComplex_endDT, :EvnSection_disDate) >= :EvnSection_disDate
							inner join v_MesOld mo on mo.Mes_id = mouc.Mes_id {$mesTypeFilter}
						where
		...
		if (isset($KSLPCodes[$code]) && $data['LpuUnitType_id'] == '1') {
			$queryResult = $this->queryResult("
				...
				WHERE
					avis.AttributeVision_TableName = 'dbo.TariffClass'
					and avis.AttributeVision_TablePKey in (select TariffClass_id from v_TariffClass where TariffClass_SysNick = 'SochHirVmesh' limit 1)
					and avis.AttributeVision_IsKeyValue = 2
					and coalesce(av.AttributeValue_begDate, :EvnSection_disDate) <= :EvnSection_disDate
					and coalesce(av.AttributeValue_endDate, :EvnSection_disDate) >= :EvnSection_disDate
				limit 1
		...
		if (substr($data['EvnSection_disDate'], 0, 4) < '2018') {
			...
			// 12. ДС. Проведение первого этапа экстракорпорального оплодотворения (стимуляция суперовуляции) (#136745)
			$code = '12';
					...
					WHERE
						...
						and not exists(
							...
							FROM
								...
								inner join v_UslugaCategory ucat2 on ucat2.UslugaCategory_id = uc2.UslugaCategory_id
							WHERE
			...
			// 13. ДС. Полный цикл экстракорпорального оплодотворения с криоконсервацией эмбрионов (#136745)
			$code = '13';
			if (isset($KSLPCodes[$code]) && in_array($data['LpuUnitType_id'], array('6','7','9'))) {
				$EvnUsluga = $this->queryResult("
					SELECT
			...
			// 14. ДС. Размораживание криоконсервированных эмбрионов с последующим переносом эмбрионов в полость матки (неполный цикл) (#136745)
			$code = '14';
			if (isset($KSLPCodes[$code]) && in_array($data['LpuUnitType_id'], array('6','7','9'))) {
				$EvnUsluga = $this->queryResult("
					SELECT
		...
		if (empty($data['EvnSection_pid'])) {
			...
			$union = "
				...
				from
					...
					left join v_CureResult cr on cr.CureResult_id = :CureResult_id
				where
					d.Diag_id = :Diag_id
		...
		foreach($groupped as $group) {
			if (count($group['EvnSections']) == 1) {
				...
				foreach($ess as $key => $value) {
					...
					if (empty($esPregnKey) && in_array($value['MesOld_Num'], ['st02.003', 'st02.004'])) {
						$esPregnKey = $key;
						continue;
					}
					...
					if (
						...
						if (empty($resp_eu[0]['EvnUsluga_id'])) {
							...
							break; // выходим из цикла, врядли в рамках одной группы будет 2 кесаревых сечения/родоразрешения
						}
		...
		foreach($groupped['group1'] as $group) {
			...
			if (!empty($EvnSectionIds)) {
				$query = "
					...
					where
						Evn_id in (" . implode(',', $EvnSectionIds) . ")
				";
				$this->db->query($query, [
		...
		foreach($groupped['group2'] as $group) {
			...
			if (!empty($group['EvnSectionIds'])) {
				$query = "
					...
					where
						Evn_id in (" . implode(',', $group['EvnSectionIds']) . ")
				";
				$this->db->query($query, [
