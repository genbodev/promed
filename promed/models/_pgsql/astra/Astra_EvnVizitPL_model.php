...
require_once(APPPATH.'models/_pgsql/EvnVizitPL_model.php');
		...
		if (
			...
			$query = "
				select
					...
					to_char(EVPL.{$this->tableName()}_setDate, 'dd.mm.yyyy') as \"EvnVizitPL_setDate\",
				...
				where EVPL.Person_id = :Person_id
					...
					and EVPL.{$this->tableName()}_setDate = cast(:EvnVizitPL_setDate as timestamp)
			";
