<?php
class DbfImporter_model extends swPgModel {
	/**
	 * Конструктор
	 */
	...
	public function importFRL($files, $RegisterListLog_id, $pmUser_id, $ignoredelete = false) {
        ...
        if($ignoredelete == false) {
			$registerListLog_id_list = $this->queryList('SELECT distinct RegisterListLog_id as "RegisterListLog_id" FROM stg.frlPrivilege');
			if(is_array($registerListLog_id_list) && count($registerListLog_id_list)>0){
				if (
					TRUE !== $this->db->query("delete from stg.frlPrivilege WHERE RegisterListLog_id in (".implode(', ', $registerListLog_id_list).")")
					|| 
					TRUE !== $this->db->query("delete from  stg.frlPerson WHERE RegisterListLog_id in (".implode(', ', $registerListLog_id_list).")")
				){
		...
		try{
			...
			foreach ($files as $key => $value) {
				...
				foreach ($value["L"] as $n => $val) {
					...
					$q = "
						select
							frlPrivilege_id as \"frlPrivilege_id\",
							Error_Code as \"Error_Code\",
							Error_Message as \"Error_Msg\"
						from stg.p_frlPrivilege_ins(
	...
	public function import($RegisterListLog_id, $dbfFile, $tableName, $fields_mapping = array(), $dropTable = false, $clearTable = false, Closure $onEveryRecord = null) {
		...
		if ($clearTable) {
			...
			if (TRUE !== $this->db->query('delete from stg.' . $tableName)) {
				throw new Exception('Не удалось очистить таблицу перед импортом');
			}
		}
	...
	function linkBySNILS($RegisterListLog_id) {
		...
		$query = "
			...
			where Priv.frlPerson_id is null
				and Priv.RegisterListLog_id = :RegisterListLog_id
