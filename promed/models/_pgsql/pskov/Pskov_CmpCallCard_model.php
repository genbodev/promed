	...
	function printCmpCloseCard110($data) {
		$query = "
			select 
				...
				,CLC.EfAD as \"EfAD\"
				,CASE WHEN COALESCE(CLC.EfChss,0) = 0 THEN NULL ELSE CLC.EfChss END as \"EfChss\"
				,CASE WHEN COALESCE(CLC.EfPulse,0) = 0 THEN NULL ELSE CLC.EfPulse END as \"EfPulse\"
				,CLC.EfTemperature as \"EfTemperature\"
				,CASE WHEN COALESCE(CLC.EfChd,0) = 0 THEN NULL ELSE CLC.EfChd END as \"EfChd\"
				,CLC.EfPulsks as \"EfPulsks\"
				,CLC.EfGluck as \"EfGluck\"
				,CLC.Kilo as \"Kilo\"
				,CLC.DescText as \"DescText\"
				,CLC.MessageNum as \"MessageNum\"
				...
				,CASE WHEN COALESCE(CLC.isOtkazSign,1) = 2 THEN 'Да' ELSE 'Нет' END as \"isOtkazSign\"				
				,CLC.OtkazSignWhy as \"OtkazSignWhy\"
				,to_char(CLC.DisStart, 'DD.MM.YYYY') as \"DisStart\"
				,CLC.AcceptFio as \"AcceptFio\"
				,to_char(CLC.CmpCloseCardWhere_DT, 'HH24:MI') as \"CmpCloseCardWhere_DT\"
				,Cause.CmpCloseCardCause_name as \"CmpCloseCardCause_name\"
				,WhereReported.CmpCloseCardWhereReported_name as \"CmpCloseCardWhereReported_name\"
				-- ,CLC.pmUser_id
			from
				v_CmpCloseCard CLC 
				LEFT JOIN Sex SX  on SX.Sex_id = CLC.Sex_id
				LEFT JOIN v_MedPersonal as MPA  ON MPA.MedPersonal_id = to_number(CLC.FeldsherAccept)
				LEFT JOIN v_MedPersonal as MPT  ON MPT.MedPersonal_id = to_number(CLC.FeldsherTrans)
				LEFT join v_pmUserCache PMCA  on PMCA.PMUser_id = to_number(CLC.FeldsherAccept)
				LEFT join v_pmUserCache PMCT  on PMCT.PMUser_id = to_number(CLC.FeldsherTrans)
				LEFT JOIN v_EmergencyTeam ET  on ET.EmergencyTeam_id = CLC.EmergencyTeam_id
				...
				LEFT JOIN v_CmpReason RS  on RS.CmpReason_id = CC.CmpReason_id
				left join v_Lpu Lpu  on Lpu.Lpu_id = CC.Lpu_id
				LEFT JOIN v_LpuBuilding LB  on LB.LpuBuilding_id = CLC.LpuBuilding_id
				left join v_CmpCloseCardCause Cause  on Cause.CmpCloseCardCause_id = CLC.CmpCloseCardCause_id
				left join v_CmpCloseCardWhereReported WhereReported  on WhereReported.CmpCloseCardWhereReported_id = CLC.CmpCloseCardWhereReported_id
			where
	...
	function loadCmpCloseCardEditForm($data) {
		...
		$query = "
			select top 1
				...
				to_char(CCC.CmpCallCard_prmDT, 'DD.MM.YYYY')+' '+to_char(CCC.CmpCallCard_prmDT, 'HH24:MI:SS') as AcceptTime,


				to_char(CCCStatusData.TransTime, 'DD.MM.YYYY')+' '+to_char(CCCStatusData.TransTime, 'HH24:MI:SS') as TransTime,
				...
				,to_char(CCC.CmpCallCard_TEnd, 'DD.MM.YYYY') + ' ' + SUBSTRING( to_char(CCC.CmpCallCard_TEnd, 'HH24:MI:SS'), 0, 6 ) as EndTime
	...
	function loadCmpCloseCardViewForm($data) {
		...
		$query = "
			select top 1
				...
				,to_char(CClC.AcceptTime, 'DD.MM.YYYY')+' '+to_char(cast(CClC.AcceptTime as datetime), 'HH24:MI:SS') as AcceptTime


				,to_char(CClC.TransTime, 'DD.MM.YYYY')+' '+to_char(cast(CClC.TransTime as datetime), 'HH24:MI:SS') as TransTime


				,to_char(CClC.GoTime, 'DD.MM.YYYY')+' '+to_char(cast(CClC.GoTime as datetime), 'HH24:MI:SS') as GoTime


				
				,to_char(CClC.ArriveTime, 'DD.MM.YYYY')+' '+to_char(cast(CClC.ArriveTime as datetime), 'HH24:MI:SS') as ArriveTime


				,to_char(CClC.TransportTime, 'DD.MM.YYYY')+' '+to_char(cast(CClC.TransportTime as datetime), 'HH24:MI:SS') as TransportTime


				,to_char(CClC.ToHospitalTime, 'DD.MM.YYYY')+' '+to_char(cast(CClC.ToHospitalTime as datetime), 'HH24:MI:SS') as ToHospitalTime


				,to_char(CClC.EndTime, 'DD.MM.YYYY')+' '+to_char(cast(CClC.EndTime as datetime), 'HH24:MI:SS') as EndTime


				,to_char(CClC.BackTime, 'DD.MM.YYYY')+' '+to_char(cast(CClC.BackTime as datetime), 'HH24:MI:SS') as BackTime
				...
				,CClC.Gluck
				,CClC.LocalStatus
				,to_char(cast(CClC.Ekg1Time as datetime), 'HH24:MI:SS') as Ekg1Time

				,CClC.Ekg1
				,to_char(cast(CClC.Ekg2Time as datetime), 'HH24:MI:SS') as Ekg2Time
				...
				,CClC.OtkazSignWhy
				,to_char(CClC.DisStart, 'DD.MM.YYYY') as DisStart

				,CClC.AcceptFio
				,to_char(cast(CClC.CmpCloseCardWhere_DT as datetime), 'HH24:MI:SS') as CmpCloseCardWhere_DT				
			...
			from
				...
				--LEFT JOIN v_EmergencyTeam EMT  on EMT.EmergencyTeam_id = CClC.EmergencyTeam_id	
    ...
    */
	...
	function saveCmpStreamCard($data) {
		...
		SET @SQLString = 
			...
			THEN COALESCE(CmpCallCard_Numv,0) ELSE 0 END)+1,

			@UnicCmpCallCard_NgodOUT = MAX(CASE WHEN YEAR( CCC.CmpCallCard_prmDT ) = ".date('Y')." 
			THEN COALESCE(CmpCallCard_Ngod,0) ELSE 0 END)+1
		...
		if(
			...
			$query="
				...
				select @Pers_id as Pid, @Error_Code as Error_Code, @Error_Message as Error_Msg;
		...
		$query = "
			...
			select @Res as CmpCallCard_id, @Error_Code as Error_Code, @Error_Message as Error_Msg;
