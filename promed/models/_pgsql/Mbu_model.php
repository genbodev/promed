 ...
 * @version
 */
class Mbu_model extends swPgModel {
	/**
	 * Конструктор
	 */
	...
	function preSendMbu($data) {
		...
		$msset = $this->getFirstRowFromQuery("
			SELECT 
				MedService_IsSendMbu as \"MedService_IsSendMbu\"
		...
		if (isset($msset) && ($msset['MedService_IsSendMbu']==2)) { // Записываем только для служб с настройками
			...
			$query = "
				Select 
					...
					mst.MedServiceType_code as \"MedServiceType_code\",
					COALESCE(uc.UslugaComplex_oid,ucg.UslugaComplex_oid) as \"UslugaComplex_oid\",
				...
				FROM 
					...
					-- Бактериология
					left join v_BactMicroProbe bmp  on bmp.UslugaTest_id = ut.UslugaTest_id and bmp.EvnLabSample_id = ut.EvnLabSample_id
				where
	...
	function save($data) {
		...
		$query = "
			select MbuPerson_id as \"MbuPerson_id\", Error_Code as \"Error_Code\", Error_Message as \"Error_Msg\"            
			from {$procedure}(
	...
	function loadRecord($data) {
		...
		$query = "
			SELECT
				m.MbuPerson_id as \"MbuPerson_id\",
				-- Person
				m.Person_id as \"Person_id\",
				person.Person_guid as \"Person_guid\",
				...
				to_char(cast(Person_BirthDay as date), 'YYYY-MM-DD') as \"birthDate\",
				...
				m.UslugaTest_id as \"UslugaTest_id\",
				ut.UslugaTest_guid as \"UslugaTest_guid\",
				ut.UslugaTest_Comment as \"UslugaTest_Comment\",
				to_char(cast(ut.UslugaTest_CheckDT as date), 'YYYY-MM-DD') as \"UslugaTest_CheckDT\",
				uc.UslugaComplex_Name as \"UslugaComplex_Name\",
				uc.UslugaComplex_code as \"UslugaComplex_code\",
				ut.UslugaTest_ResultValue as \"UslugaTest_ResultValue\",
				ut.UslugaTest_ResultLower as \"UslugaTest_ResultLower\",
				ut.UslugaTest_ResultUpper as \"UslugaTest_ResultUpper\",
				m.MbuPerson_IsBact as \"MbuPerson_IsBact\",
				m.BactMicro_id as \"BactMicro_id\",
				m.MbuStatus_id as \"MbuStatus_id\",
				ms.MbuStatus_Name as \"MbuStatus_Name\",
				m.InterpretationResult_id as \"InterpretationResult_id\",
				to_char(cast(m.MbuPerson_sendDT as date), 'YYYY-MM-DD') as \"MbuPerson_sendDT\",
				m.MbuPerson_GUID as \"MbuPerson_GUID\",
				m.UslugaComplex_oid as \"UslugaComplex_oid\",
				to_char(cast(ls.EvnLabSample_DelivDT as date), 'YYYY-MM-DD') as \"EvnLabSample_DelivDT\",
				lr.EvnLabRequest_guid as \"EvnLabRequest_guid\",
				ls.EvnLabSample_guid as \"EvnLabSample_guid\",
				lr.EvnLabRequest_id as \"EvnLabRequest_id\",
				ltm.LabTestMaterial_code as \"LabTestMaterial_code\", --RefMaterial_NSI
				lt.LabTest_code as \"LabTest_code\",
				ltg.LabTestGroup_code as \"LabTestGroup_code\",
				ir.InterpretationResult_code as \"InterpretationResult_code\",
				bmp.BactMicroProbe_id as \"BactMicroProbe_id\",
				bmp.BactMicroProbe_IsNotShown as \"BactMicroProbe_IsNotShown\",
				bmas.BactMicroABPSens_ShortName as \"BactMicroABPSens_ShortName\",
				bmpa.BactMicroProbeAntibiotic_id as \"BactMicroProbeAntibiotic_id\",
				ba.BactAntibiotic_code as \"BactAntibiotic_code\",
				bm.BactMicro_code as \"BactMicro_code\",
				bmw.BactMicroWorld_code as \"BactMicroWorld_code\",
				mu.MeasureUnit_Code as \"MeasureUnit_Code\",
				m.MbuPerson_AnswerCode as \"MbuPerson_AnswerCode\"
			FROM
				MbuPerson m 
				inner join v_PersonState ps  on ps.Person_id = m.Person_id
				left join person  on person.Person_id = m.Person_id
				left join VIPPerson vip  on vip.Person_id = ps.Person_id -- возможно тут надо фильтр еще и по Lpu
				-- адрес регистрации
				left join v_Address uaddr  on ps.UAddress_id = uaddr.Address_id
				left join v_KLRgn URegion  on URegion.KLRgn_id = uaddr.KLRgn_id
				left join v_KLSubRgn USubRgn  on USubRgn.KLSubRgn_id = uaddr.KLSubRgn_id
				left join v_KLCity UCity  on UCity.KLCity_id = uaddr.KLCity_id
				left join v_KLTown UTown  on UTown.KLTown_id = uaddr.KLTown_id
				...
				left join v_KLRgn PRegion  on PRegion.KLRgn_id = paddr.KLRgn_id
				left join v_KLSubRgn PSubRgn  on PSubRgn.KLSubRgn_id = paddr.KLSubRgn_id
				left join v_KLCity PCity  on PCity.KLCity_id = paddr.KLCity_id
				left join v_KLTown PTown  on PTown.KLTown_id = paddr.KLTown_id
				left join v_KLStreet PStreet  on PStreet.KLStreet_id = paddr.KLStreet_id
				inner join v_UslugaTest ut  on ut.UslugaTest_id = m.UslugaTest_id
				left join v_EvnLabSample ls  on ut.EvnLabSample_id = ls.EvnLabSample_id
				left join v_EvnLabRequest lr  on lr.EvnLabRequest_id = ls.EvnLabRequest_id
				left join v_RefSample RefSample_id_ref  on RefSample_id_ref.RefSample_id = ls.RefSample_id
				...
				left join lis.v_Unit rvu  on rvu.Unit_id = rv.Unit_id
				left join nsi.MeasureUnit mu  on mu.MeasureUnit_id = rvu.MeasureUnit_id
				...
				left join nsi.LabTest lt  on lt.LabTest_id = ut.LabTest_id
				left join nsi.LabTestGroup ltg  on ltg.LabTestGroup_id = lt.LabTestGroup_id
				left join nsi.InterpretationResult ir  on m.InterpretationResult_id = ir.InterpretationResult_id
				left join v_MbuStatus ms  on ms.MbuStatus_id = m.MbuStatus_id
				-- и бактериология
				left join v_BactMicroProbe bmp  on bmp.UslugaTest_id = ut.UslugaTest_id and bmp.EvnLabSample_id = ut.EvnLabSample_id
				left join v_BactMicroProbeAntibiotic bmpa  on bmp.BactMicroProbe_id = bmpa.BactMicroProbe_id and ut.UslugaTest_id = bmpa.UslugaTest_id
				left join v_BactMicroABPSens bmas  on bmas.BactMicroABPSens_id = bmpa.BactMicroABPSens_id
				left join v_BactAntibiotic ba  on bmpa.BactAntibiotic_id = ba.BactAntibiotic_id 
				left join v_BactMicro bm  on bmp.BactMicro_id = bm.BactMicro_id 
				left join v_BactMicroWorld bmw  on bmw.BactMicroWorld_id = bm.BactMicroWorld_id 
			where
				m.MbuPerson_id = :MbuPerson_id 
                and m.MbuStatus_id!=3
		";
	...
	function getMbuLpu($data) {
		...
		$query = "
			SELECT 
				ml.MbuLpu_id as \"MbuLpu_id\",
				ml.Lpu_id as \"Lpu_id\",
				ml.MbuLpu_oid as \"MbuLpu_oid\",
				ml.MbuLpu_GUID as \"MbuLpu_GUID\",
				ml.MbuLpu_Token as \"MbuLpu_Token\",
				ml.MbuLpu_begDT as \"MbuLpu_begDT\",
				ml.MbuLpu_endDT as \"MbuLpu_endDT\",
				pt.PassportToken_tid as \"PassportToken_tid\"
			FROM
				MbuLpu ml 
				inner join v_Lpu Lpu  on Lpu.Lpu_id = ml.Lpu_id
				left join fed.PassportToken pt  on pt.Lpu_id = Lpu.lpu_id
			where
				ml.Lpu_id = :Lpu_id 
			and MbuLpu_begDT <= dbo.tzgetdate() and COALESCE(MbuLpu_endDT, dbo.tzgetdate()) >= dbo.tzgetdate()
	...
	function loadList($data) {
		...
		if (!empty($data['Search_SurName'])) {
			$where[] = "ps.Person_SurName iLIKE :Search_SurName||'%'";

		}
		if (!empty($data['Search_FirName'])) {
			$where[] = "ps.Person_FirName iLIKE :Search_FirName||'%'";

		}
		if (!empty($data['Search_SecName'])) {
			$where[] = "ps.Person_SecName iLIKE :Search_SecName||'%'";
		...
		$query = "
			SELECT
				...
				ms.MbuStatus_Name as \"MbuStatus_Name\",
				m.InterpretationResult_id as \"InterpretationResult_id\",
				to_char(cast(m.MbuPerson_sendDT as timestamp), 'DD.MM.YYYY') as \"MbuPerson_sendDT\",
				m.MbuPerson_GUID as \"MbuPerson_GUID\",
				m.UslugaComplex_oid as \"UslugaComplex_oid\",
				m.MbuPerson_AnswerCode as \"MbuPerson_AnswerCode\"
			FROM
