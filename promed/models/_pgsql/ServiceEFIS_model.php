...
class ServiceEFIS_model extends SwPgModel {
	public $scheme = 'r101';
	...
	function execSaveObjectQueries() {
		...
		$tpl = "
			CREATE OR REPLACE FUNCTION pg_temp.execQueries(
		...
		$declare_variables = function($v){return "$v;";};
	...
	function addSaveObjectQuery($objectData, $objectName, $keyField = null) {
		...
		$tpl = "
			id := (select {idField} from {scheme}.{objectName} where {keyField} = '{keyValue}' limit 1);
			if id is null then
	...
	function importDrugList($data) {
		$lastDateUpdate = $this->getFirstResultFromQuery("
			select
			to_char(max(KMD.DateUpdate), 'dd.mm.yyyy') as \"DateUpdate\"
			from {$this->scheme}.KM_Drug KMD
            limit 1
		", array(), true);
	...
	function addSyncCommercialOfferQuery($data) {
		$list = $this->queryResult("
			select
				...
				KMCO.Org_id as \"Org_id\",
				KMCO.CommercialOffer_Comment as \"CommercialOffer_Comment\",
				to_char(KMCO.CommercialOffer_begDT, 'yyyy-mm-dd hh:mi:ss') as \"CommercialOffer_begDT\",
				to_char(KMCO.CommercialOffer_endDT, 'yyyy-mm-dd hh:mi:ss') as \"CommercialOffer_endDT\",
				KMCO.Status as \"CommercialOffer_Status\"
			from
				{$this->scheme}.KM_CommercialOffer KMCO
				left join dbo.CommercialOffer CO  on CO.CommercialOffer_guid = KMCO.KM_CommercialOffer_guid
		");
		if (!is_array($list)) {
		...
		$tpl = "
			select t.Error_Code, t.Error_Message
			into Error_Code, Error_Message
			from p_CommercialOffer_{action} (
	...
	function importCommercialOfferList($data) {
		$Org_id = $this->getFirstResultFromQuery("
			select Org_id from v_Org  where Org_Nick ilike 'СК Фармация' limit 1
		", array(), true);
		if ($Org_id === false) {
		...
		if (!empty($activeCommercialOffer)) {
			$this->addSaveObjectQueryWithTpl("
				update {scheme}.KM_CommercialOffer
	...
	function addSaveCommecialOfferDrugQuery($item, $CommercialOffer_guid = null) {
		...
		if (empty($CommercialOffer_guid)) {
			$CommercialOffer_guid = $this->getFirstResultFromQuery("
				select KM_CommercialOffer_guid as \"KM_CommercialOffer_guid\"
				from {$this->scheme}.KM_CommercialOffer
				where Name ilike :priceId
                limit 1
			", $item, true);
		...
		$query = "
			...
			from rls.v_REGCERT R
			inner join rls.v_PREP P on P.REGCERTID = R.REGCERT_ID
	...
	function addSyncCommercialOfferDrugQuery($data) {
		$list = $this->queryResult("
			select
				...
				CO.CommercialOffer_id as \"CommercialOffer_id\",
				CO.CommercialOffer_guid as \"CommercialOffer_guid\",
				KMCOD.Drug_id as \"Drug_id\",
				KMCOD.DrugPrepFas_id as \"DrugPrepFas_id\",
				...
				KMCOD.Goods_guid as \"Goods_guid\",
			...
			from
				{$this->scheme}.KM_CommercialOfferDrug KMCOD
				inner join v_CommercialOffer CO on CO.CommercialOffer_guid = KMCOD.KM_CommercialOffer_guid
				left join v_CommercialOfferDrug COD on COD.CommercialOfferDrug_PriceDetail = KMCOD.Price_detail
			where not exists(
				select * from v_CommercialOfferDrug
		...
		//$this->addSaveObjectVariables('id bigint');	//в postgre не нужно
		...
		$tpl = "
			select t.Error_Code, t.Error_Message
			into Error_Code, Error_Message
			from p_CommercialOfferDrug_{action} (
	...
	function importCommercialOfferDrugList($data) {
		$ActiveCommercialOffer = $this->getFirstRowFromQuery("
			select
				KM_CommercialOffer_guid as \"guid\",
				Name as \"Name\"
			from {$this->scheme}.KM_CommercialOffer
			where Status = 1
			limit 1
	...
	function addSavePrepSeriesQuery($item) {
		...
		$query = "
			select P.Prep_id as \"Prep_id\"
			from v_CommercialOfferDrug COD
			left join rls.v_REGCERT R on R.REGNUM = COD.CommercialOfferDrug_RegCertName
			left join rls.v_PREP P on P.REGCERTID = R.REGCERT_ID
	...
	function importWhsDocumentUc($data)
	{
		...
		if($data['import_Type'] == 2)
		{
			...
			$query_check_Additional = "
				select WhsDocumentSupply_id as \"WhsDocumentSupply_id\"
		...
		foreach($res_whsuc as $res_whsuc_item){
			...
			$query_getCommercialOffer = "
				select CommercialOffer_id as \"CommercialOffer_id\"
			...
			$query_check_FinanceSource = "
				select FinanceSource_id as \"FinanceSource_id\"
			...
			$query_add_WhsDocumentSupply = "
				...
				from dbo.p_WhsDocumentSupply_ins (
			...
			$query_set_InsertDT = 'update WhsDocumentUc  set WhsDocumentUc_ImportDT = dbo.tzGetDate() where WhsDocumentUc_id = :WhsDocumentUc_id';
			...
			for($c=0; $c<count($ContractDetails); $c++)
			{
				...
				$query_get_CommercialOfferDrug = "
					select
						...
						COALESCE(COD.CommercialOfferDrug_Package,1) as \"CommercialOfferDrug_Package\",
						COALESCE(GU.GoodsUnit_id,0) as \"GoodsUnit_id\"
					from v_CommercialOfferDrug COD
					left join rls.v_Drug D on D.Drug_id = COD.Drug_id
					left join dbo.GoodsUnit GU on GU.GoodsUnit_Name = COD.CommercialOfferDrug_UnitName
						...
						if(is_object($result_add_WhsDocumentSupplySpec))
						{
							...
							if(is_array($result_add_WhsDocumentSupplySpec) && count($result_add_WhsDocumentSupplySpec) > 0)
							{
								...
								$query_add_WhsDocumentDelivery = "
									...
									from dbo.p_WhsDocumentDelivery_ins (
										WhsDocumentSupply_id := :WhsDocumentSupply_id,
										WhsDocumentSupplySpec_id := :WhsDocumentSupplySpec_id,
		...
		foreach($res as $result) {
			...
			$query_check_WhsDocumentUc = "
				select
					...
					--COALESCE(CS.Contragent_id,0) as Contragent_sid,
				...
				from v_WhsDocumentUc WDU
				inner join v_WhsDocumentSupply WDS on WDS.WhsDocumentUc_id = WDU.WhsDocumentUc_id
				inner join v_Org O on O.Org_id = WDS.Org_sid
				LEFT JOIN LATERAL(
					select C1.Contragent_id
					from v_Contragent C1
					where C1.Org_id = WDS.Org_sid
                    limit 1
				) as CS on true
				where WDU.WhsDocumentUc_Num = :WhsDocumentUc_Num
				and O.Org_Nick ilike '%Фармация%'
                limit 1
			";
				...
				if(count($result_check_WhsDocumentUc) > 0)
				{
					...
					$query_get_Contragent = "
						select
							...
							COALESCE(M.Mol_id, 0) as \"Mol_id\"
						from v_ContragentOrg CO
						inner join v_Contragent C on C.Contragent_id = CO.Contragent_id
					...
					if(is_object($result_add_DocumentUc))
					{
						...
						if(is_array($result_add_DocumentUc) && count($result_add_DocumentUc) > 0)
						{
							...
							foreach($res_det as $result)
							{
								...
								$query_get_CommercialOfferDrug = "
									select
										COALESCE(COD.Drug_id,-1) as \"Drug_id\",
										COALESCE(P.Prep_id,-1) as \"Prep_id\",
										COALESCE(GU.GoodsUnit_id,-1) as \"GoodsUnit_id\",
										COD.CommercialOfferDrug_UnitName as \"CommercialOfferDrug_UnitName\",
										COALESCE(COD.CommercialOfferDrug_Package,1) as \"CommercialOfferDrug_Package\",
										to_char(CO.CommercialOffer_begDT,'yyyy-mm-dd') as \"CommercialOffer_begDT\",
										COD.CommercialOfferDrug_Price as \"CommercialOfferDrug_Price\"
									from v_CommercialOfferDrug COD
									left join v_GoodsUnit GU  on GU.GoodsUnit_Name = COD.CommercialOfferDrug_UnitName
													...
													if($DrugShipment_id == -1)
													{
														...
														$query_add_DrugShipment = "
															...
															from p_DrugShipment_ins (
																DrugShipment_Name := COALESCE((
																	select max(cast(DrugShipment_Name as bigint))+1
																	from v_DrugShipment
																	where ISNUMERIC(DrugShipment_Name)=1 and
																	DrugShipment_Name not ilike '%.%' and DrugShipment_Name not ilike '%,%'
																), 1),
																WhsDocumentSupply_id := :WhsDocumentSupply_id,
