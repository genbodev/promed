...
class Adygeya_EvnSection_model extends EvnSection_model {
	...
	function loadKSGKPGKOEF($data) {
		...
		if (!empty($data['EvnSection_id'])) {
			...
			$resp_es = $this->recalcIndexNumByGroup(array(
				...
				'EvnSection_setDate' => !empty($data['EvnSection_setDate'])?$data['EvnSection_setDate']:null,
	...
	protected function recalcIndexNumByGroup($data = array())
	{
		...
		//Если данные движения передаются явным образом, то считать их актуальными
		if (!empty($data['EvnSection_id'])) {
			foreach ($resp_es as $key => $value) {
				if ($resp_es[$key]['EvnSection_id'] == $data['EvnSection_id']) {
					if (!empty($data['EvnSection_setDate'])) {
						if ($resp_es[$key]['EvnSection_disDate'] == $resp_es[$key]['EvnSection_setDate']) {
							$resp_es[$key]['EvnSection_disDate'] = $data['EvnSection_setDate'];
						}
						
						$resp_es[$key]['EvnSection_setDate'] = $data['EvnSection_setDate'];
					}
					
					if (!empty($data['EvnSection_disDate'])) {
						$resp_es[$key]['EvnSection_disDate'] = $data['EvnSection_disDate'];
					}
				}
			}
		}
