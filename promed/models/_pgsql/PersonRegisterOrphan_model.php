 ...
 */
class PersonRegisterOrphan_model extends PersonRegisterBase_model
{
	protected $_personRegisterTypeSysNick = 'orphan'; // всегда перекрывать
	protected $_userGroupCode = 'Orphan'; // можно не перекрывать, если задано стандартно, например "OrphanRegistry" для типа регистра "orphan"
	protected $_PersonRegisterType_id = 6; // если не для всех регионов, то нельзя перекрывать
	...
	static function defAttributes()
	{
		...
		$arr[self::ID_KEY]['label'] = 'Запись регистра по орфанным заболеваниям';
	...
	protected function _beforeSave($data = array())
	{
		...
		if ($this->isNewRecord && empty($this->EvnNotifyBase_id)) {
			...
			$res = $instance->doSave(array(
				...
				'EvnNotifyRegister_setDate' => (!empty($this->_params['Direction_setDate']) ? $this->_params['Direction_setDate'] : $this->setDate->format('Y-m-d')),
	...
	function updateLpuOid($id, $value, $isAllowTransaction = true)
	{
		...
		if (empty($value)) {
			throw new Exception('Не указано МО, в которой пациенту впервые установлен диагноз орфанного заболевания', 400);
		}
		$data = array();
		$data[$this->primaryKey(true)] = $id;
		$this->setAttributes($data);
	...
	function setParams($data)
	{
		...
		if (in_array($this->scenario, array('create'))) {
			$this->_params['Direction_setDate'] = isset($data['Direction_setDate']) ? $data['Direction_setDate'] : null ;
		}
	...
	protected function _createEvnNotifyRegisterExcept()
	{
		if ('except' == $this->scenario) {
			 ...
			 * @var EvnNotifyRegister_model $instance
			 */
			$instance = new $className('orphan', 3);
			if(
				!empty($_POST['PersonRegisterType_SysNick']) 
				&& in_array($_POST['PersonRegisterType_SysNick'],array('orphan'))
				&& empty($instance->sessionParams['lpu_id'])
   ...
   function getPersonPrivilegeInvAllViewData($data)
	{
		...
		$query = "
			...
			FROM
				...
				inner join v_PrivilegeType PT  on PT.PrivilegeType_id = PP.PrivilegeType_id and PT.PrivilegeType_Code in ('83','82','81','84')
				LEFT JOIN LATERAL (
	...
	function getViewData($data)
	{
		...
		$query = "
			SELECT
				...
				,RTRIM(DoOrg.Org_Name) as \"OrgDep_Name\"
				...
				,ENR.EvnNotifyRegister_id as \"EvnNotifyRegister_id\"
			...
			FROM
				...
				left join v_EvnNotifyRegister ENR  on ENR.EvnNotifyRegister_id = PR.EvnNotifyBase_id
					and ENR.NotifyType_id = 1
				left join v_Lpu LpuO  on LpuO.Lpu_id = ENR.Lpu_oid
				left join v_Lpu LpuD  on LpuD.Lpu_id = ENR.Lpu_id
			WHERE
		...
		$query = "
			...
			ER as (
				select  /* Рецепты не отоваренные */
					Coalesce(ER.Drug_id,ER.Drug_rlsid) as ID,
					to_char(ER.EvnRecept_setDT, 'dd.mm.yyyy') as DATE_ISSUE,
					to_char(ER.EvnRecept_otpDT, 'dd.mm.yyyy') as DATE_DISP,
					Coalesce(D.Drug_Name,DRls.Drug_Name) as DESCR,
					PR.Person_id,
					PR.MorbusType_id
				from v_EvnRecept ER
				...
				where
					not exists (select RO.ReceptOtov_id from ReceptOtov RO  where RO.EvnRecept_id = ER.EvnRecept_id limit 1)
				union all
				select  /* Рецепты отоваренные */
					coalesce(ER.Drug_cid,ReceptOtovDop.Drug_rlsid,ER.Drug_id) as ID,
					to_char(ER.EvnRecept_setDT, 'dd.mm.yyyy') as DATE_ISSUE,
					to_char(ER.EvnRecept_otpDate, 'dd.mm.yyyy') as DATE_DISP,
					Coalesce(DRls.Drug_Name,D.Drug_Name) as DESCR,
					PR.Person_id,
					PR.MorbusType_id
				from ReceptOtov ER
					inner join PersonRegisterDiag PRD  on PRD.Diag_id = ER.Diag_id
					...
					left join rls.v_Drug DRls  on DRls.Drug_id = Coalesce(ER.Drug_cid,ReceptOtovDop.Drug_rlsid)
					left join v_Drug D  on D.Drug_id = ER.Drug_id
				where
					1=1
			)
            ...
            select
                ...
                (SELECT xmlagg(item)
                 FROM (SELECT xmlelement (name ITEM,
					                        xmlelement(name ID,ER.ID),
							                xmlelement(name DATE_ISSUE,ER.DATE_ISSUE),
							                xmlelement(name DATE_DISP,ER.DATE_DISP),
							                xmlelement(name DESCR,ER.DESCR)
                                          ) as item
					    FROM ER
