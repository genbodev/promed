 ...
 */
class AnalyzerModel_model extends SwPgModel
{
	...
	function load() {
		$q = "
			select
				...
				AnalyzerModel_Name as \"AnalyzerModel_Name\",
				AnalyzerModel_SysNick as \"AnalyzerModel_SysNick\",
				FRMOEquipment_id as \"FRMOEquipment_id\",
				AnalyzerClass_id as \"AnalyzerClass_id\",
				AnalyzerInteractionType_id as \"AnalyzerInteractionType_id\",
				AnalyzerModel_IsScaner as \"AnalyzerModel_IsScaner\",
				AnalyzerWorksheetInteractionType_id as \"AnalyzerWorksheetInteractionType_id\"
			from
				lis.v_AnalyzerModel
			where
	...
	function loadList($filter) {
		...
		$q = "
			SELECT
				AM.AnalyzerModel_id as \"AnalyzerModel_id\",
				AM.AnalyzerModel_Name as \"AnalyzerModel_Name\",
				AM.AnalyzerModel_SysNick as \"AnalyzerModel_SysNick\",
				coalesce(FRMO.FRMOEquipment_Name, '') as \"FRMOEquipment_Name\",
				AM.AnalyzerClass_id as \"AnalyzerClass_id\",
				AM.AnalyzerInteractionType_id as \"AnalyzerInteractionType_id\",
				case when coalesce(AM.AnalyzerModel_IsScaner,1) = 2 then 'true' else 'false' end as \"AnalyzerModel_IsScaner\",
				AM.AnalyzerWorksheetInteractionType_id as \"AnalyzerWorksheetInteractionType_id\",
				AC.AnalyzerClass_Name as \"AnalyzerClass_Name\",
				AIT.AnalyzerInteractionType_Name as \"AnalyzerInteractionType_Name\",
				YN.YesNo_Name as \"YesNo_Name\",
				AWIT.AnalyzerWorksheetInteractionType_Name as \"AnalyzerWorksheetInteractionType_Name\"
			FROM
				lis.v_AnalyzerModel AM
				LEFT JOIN lis.v_AnalyzerClass AC ON AC.AnalyzerClass_id = AM.AnalyzerClass_id
				LEFT JOIN lis.v_AnalyzerInteractionType AIT ON AIT.AnalyzerInteractionType_id = AM.AnalyzerInteractionType_id
				LEFT JOIN dbo.v_YesNo YN ON YN.YesNo_id = AM.AnalyzerModel_IsScaner
				LEFT JOIN lis.v_AnalyzerWorksheetInteractionType AWIT ON AWIT.AnalyzerWorksheetInteractionType_id = AM.AnalyzerWorksheetInteractionType_id
				LEFT JOIN passport.v_FRMOEquipment FRMO ON FRMO.FRMOEquipment_id = AM.FRMOEquipment_id
			WHERE
				{$where_clause}
	...
	function delete() {
		$tests_count = $this->getFirstResultFromQuery("
			...
			WHERE
				AnalyzerModel_id = :AnalyzerModel_id
		", array('AnalyzerModel_id' => $this->AnalyzerModel_id));
		if ($tests_count > 0) {
			throw new Exception("Нельзя удалить данную модель анализатора, так как у нее есть тесты");
		}
		$analyzers_count = $this->getFirstResultFromQuery("
			SELECT
				COUNT(*)
			FROM
				lis.v_Analyzer
		...
		if ( is_object($racks) ) {
			...
			$this->load->model('AnalyzerRack_model');
