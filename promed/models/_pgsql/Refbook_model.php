...
class Refbook_model extends SwPgModel {
	...
	function getLeaveTypeByLpuUnitType($data) {
		...
		$resp = $this->queryResult("
			select
				rlt.RISHLeaveType_id as \"RISHLeaveType_id\",
				rlt.LpuUnitType_id as \"LpuUnitType_id\",
				rlt.LeaveType_id as \"LeaveType_id\",
				lt.LeaveType_fedid as \"LeaveType_fedid\"
			from
				v_RISHLeaveTypeLink rlt
				left join LeaveType lt on lt.LeaveType_id = rlt.LeaveType_id
			where
	...
	function loadOrgSMOListForAPI($data) {
		...
		select
			...
			coalesce(OrgSMO_IsTFOMS, 1) as \"OrgSMO_IsTFOMS\",
			Orgsmo_f002smocod as \"Orgsmo_f002smocod\"
			from
				v_OrgSMO
			where
	...
	function loadKLAreaList($data) {
		...
		$resp = $this->queryResult("
			select
				kla.KLCountry_id as \"KLCountry_id\",
			...
			from
				v_KLArea kla
				left join v_KLArea kla_town on case when kla.KLAreaLevel_id = 4 then kla.KLArea_id else kla.KLArea_pid end = kla_town.KLArea_id and kla_town.KLAreaLevel_id = 4
				left join v_KLArea kla_city on case when kla.KLAreaLevel_id = 3 then kla.KLArea_id else coalesce(kla_town.KLArea_pid, kla.KLArea_pid) end = kla_city.KLArea_id and kla_city.KLAreaLevel_id = 3
				left join v_KLArea kla_subrgn on case when kla.KLAreaLevel_id = 2 then kla.KLArea_id else coalesce(kla_city.KLArea_pid, kla_town.KLArea_pid, kla.KLArea_pid) end = kla_subrgn.KLArea_id and kla_subrgn.KLAreaLevel_id = 2
				left join v_KLArea kla_rgn on case when kla.KLAreaLevel_id = 1 then kla.KLArea_id else coalesce(kla_subrgn.KLArea_pid, kla_city.KLArea_pid, kla_town.KLArea_pid, kla.KLArea_pid) end = kla_rgn.KLArea_id and kla_rgn.KLAreaLevel_id = 1
			where
				(1=1)
				{$filter}
		", $queryParams);
	...
	function loadOmsSprTerrList($data) {
		...
		$resp = $this->queryResult("
			select {$type}
				...
				ost.OmsSprTerr_Name as \"OmsSprTerr_Name\"
				{$select}
			from
				v_KLArea kla
				inner join v_OmsSprTerr ost on kla.KLArea_id = ( case
	...
	*/
	function loadRefbookUslugaComplex($data) {
		...
		if (!empty($data['Lpu_id'])) {
			...
			$filter .= " and coalesce(uc.Lpu_id, :Lpu_id) = :Lpu_id";
     ...
     * @return array|false
     */
    public function mLoadRefbookUslugaComplex($data)
    {
	...
	function loadRefbookDiag($data) {
		...
		$resp = $this->queryResult("
			select
				...
				d.Diag_begDate as \"Diag_begDate\"
			from v_Diag d
	...
	function loadKLStreetList($data) {
		...
		$namefilter = " and lower(kst.KLStreet_Name) = lower(:KLStreet_Name)";
		...
		$resp = $this->queryResult("
			select
				kst.KLStreet_id as \"KLStreet_id\",
				kst.KLStreet_Name as \"KLStreet_Name\"
			from
				v_KLStreet kst
			where
				(1=1)
		...
		if (empty($resp)) {
			$namefilter = " and lower(kst.KLStreet_Name || coalesce(' ' || ksc.KLSocr_Nick,'')) = lower(:KLStreet_Name)";
			$resp = $this->queryResult("
				select
					kst.KLStreet_id as \"KLStreet_id\",
					kst.KLStreet_Name as \"KLStreet_Name\"
				from
					v_KLStreet kst
					left join v_KLSocr ksc on kst.KLSocr_id = ksc.KLSocr_id
				where
					(1=1)
					{$namefilter}
					{$filter}
	...
	function loadKLSubRgnList($data) {
		...
		$resp = $this->queryResult("
			with myvar as (select KLArea_id from v_KLArea where lower(KLAdr_Code) = lower(:KLAdr_Code) and KLAreaLevel_id = 1 limit 1)

			select
				ka.KLArea_id as \"KLArea_id\",
				ka.KLArea_Name as \"KLArea_id\",
				ka.KLSocr_id as \"KLArea_id\",
				ks.KLSocr_Name as \"KLArea_id\",
				ks.KLSocr_Nick as \"KLArea_id\"
			from
				v_KLArea ka
				inner join v_KLSocr ks on ks.KLSocr_id = ka.KLSocr_id
			where
				ka.KLArea_pid = (select KLArea_id from myvar)
	...
	function loadRefbook($data) {
		...
		if (!empty($data['Refbook_Code'])) {
			$filter .= " and lower(RISHRefbook_Code) = lower(:RISHRefbook_Code)";
			$queryParams['RISHRefbook_Code'] = $data['Refbook_Code'];
		} 
		if (!empty($data['Refbook_TableName'])) {
			$filter .= " and lower(Refbook_TableName) = lower(:Refbook_TableName)";
			$queryParams['Refbook_TableName'] = $data['Refbook_TableName'];
		} 

		$resp = $this->queryResult("
			select
				RISHRefbook_id as \"RISHRefbook_id\",
				Refbook_TableName as \"Refbook_TableName\"
			from
				v_RISHRefbook
			where
		...
		if (isset($data['Code'])) {
			$filter .= " and lower({$subject}_Code) = lower(:Code)";
			$queryParams['Code'] = $data['Code'];
		}
		if (isset($data['Name'])) {
			$filter .= " and lower({$subject}_Name) = lower(:Name)";
		...
		$resp = $this->queryResult("select column_name as \"column_name\" from dbo.v_columns where lower(schema_name) = lower(:schema_name) and lower(table_name) = lower(:table_name)", array(
			'schema_name' => $schema,
		...
		if (!empty($resp)) {
			...
			if (in_array(mb_strtolower($subject."_begDT"), $columns)) {
				$fields .= "to_char({$subject}_begDT, 'yyyy-mm-dd') as \"begDate\",";
			} elseif (in_array(mb_strtolower($subject."_begDate"), $columns)) {
				$fields .= "to_char({$subject}_begDate, 'yyyy-mm-dd') as \"begDate\",";
			} else {
				$fields .= "null as \"begDate\",";
			}

			if (in_array(mb_strtolower($subject."_endDT"), $columns)) {
				$fields .= "to_char({$subject}_endDT, 'yyyy-mm-dd') as \"endDate\"";
			} elseif (in_array(mb_strtolower($subject."_endDate"), $columns)) {
				$fields .= "to_char({$subject}_endDate, 'yyyy-mm-dd') as \"endDate\"";
			} else {
		...
		if($schema == 'persis'){
			...
			$fields = '';
			$fields .= (in_array('id', $arrColumns)) ? ' id as \"id\",' : ' null as \"id\",';
			...
			if (isset($data['Code']) && in_array('code', $arrColumns)) {
				$filter .= " and lower(code) = lower(:Code)";
				$queryParams['Code'] = $data['Code'];
			}
			if (isset($data['Name']) && in_array('name', $arrColumns)) {
				$filter .= " and lower(name) = lower(:Name)";
				$queryParams['Name'] = $data['Name'];
			}
		...
		if (isset($data['start']) && (!empty($data['limit'])) && isset($isIdBySubject)) {
			...
			$query_body = "
				...
				select id as \"id\", Name as \"Name\", Code as \"Code\", begDate as \"begDate\", endDate as \"endDate\" from rb where \"RowNumber\" between :start and :limit;
			";
		}
	...
	function loadRefbookbyColumnExt($data) {
		...
		if (!empty($resp)) {
			...
			if (!empty($data['Column_Name']) && in_array(mb_strtolower($data['Column_Name']), $columns) && isset($data['Column_Value'])) {
				$filter2 .= " and lower({$data['Column_Name']}) = lower(:Column_Value)";
	...
	function loadRefbookMap($data) {
		...
		$resp = $this->queryResult("
			select
				RISHRefbook_id as \"RISHRefbook_id\",
				RISHRefbook_Code as \"RISHRefbook_Code\",
				RISHRefbook_Name as \"RISHRefbook_Name\",
				RISHRefbookType_id as \"RISHRefbookType_id\",
				RISHRefbook_MapName as \"RISHRefbook_MapName\"
			from
				v_RISHRefbook
			where
				lower(RISHRefbook_Code) = lower(:RISHRefbook_Code)
		...
		$query = "
				...
				".$fields."	
				from
					{$schema}.{$view}{$subject}Link t1
					inner join {$schema}.{$view}{$subject2} t2 on t2.{$subject2}_id = t1.{$subject2}_id
					inner join {$schema}.{$view}{$subject} t3 on t3.{$subject}_id = t1.{$subject}_id
		";
		//echo getDebugSQL($query, $data);exit;
		$resp = $this->queryResult($query, $queryParams);
	...
	function loadRefbookGoodsUnit($data) {
		...
		if (!empty($data['GoodsUnit_Name'])) {
			$filters[] = "lower(GU.GoodsUnit_Name) like lower(:GoodsUnit_Name)";
			$params['GoodsUnit_Name'] = $data['GoodsUnit_Name'];
		}
		if (!empty($data['GoodsUnit_Nick'])) {
			$filters[] = "lower(GU.GoodsUnit_Nick) like lower(:GoodsUnit_Nick)";
	...
	function loadRefbookPrivilegeType($data) {
		...
		if (!empty($data['PrivilegeType_Code'])) {
			$filters[] = "lower(PT.PrivilegeType_Code) = lower(:PrivilegeType_Code)";
			$params['PrivilegeType_Code'] = $data['PrivilegeType_Code'];
		}
		if (!empty($data['PrivilegeType_Name'])) {
			$filters[] = "lower(PT.PrivilegeType_Name) like lower(:PrivilegeType_Name)";
			$params['PrivilegeType_Name'] = $data['PrivilegeType_Name'];
		}
		if (!empty($data['PrivilegeType_Descr'])) {
			$filters[] = "lower(PT.PrivilegeType_Descr) like lower(:PrivilegeType_Descr)";
		...
		$query = "
			select
				...
				PT.PrivilegeType_Descr as \"PrivilegeType_Descr\",
	...
	function loadRefbookDrugComplexMnn($data) {
		...
		if (!empty($data['CLSDRUGFORMS_ID'])) {
			...
			$filters[] = "DCM.CLSDRUGFORMS_ID in (
				select CLSDRUGFORMS_ID from v_RISHDrugFormsLink where RISHDrugForms_id = :FEDDRUGFORMS_ID
			)";
		...
		$query = "
			with SecondLevelDCM as (
				...
				from
					...
					left join rls.v_Drug D on D.DrugComplexMnn_id = DCM.DrugComplexMnn_id
					left join rls.v_PREP_ATC PA on PA.PREPID = D.DrugPrep_id
					left join rls.v_CLSATC CLSATC on CLSATC.CLSATC_ID = PA.UNIQID
				where
			...
			select
				...
				DCM.CLSDRUGFORMS_ID as \"CLSDRUGFORMS_ID\"
			from 
				rls.v_DrugComplexMnn DCM
			where
	...
	function getRefbookState($data) {
		...
		$state = $this->queryResult(
			"
				select
					( select
						max({$updateFieldName}) from {$schema}.{$view}{$tableName} limit 1
					) as \"lastUpdateDT\",
					( select
						count({$idFieldName}) from {$schema}.{$view}{$tableName} limit 1
					) as \"row_count\"
