...
abstract class EvnPLDispAbstract_model extends EvnPLAbstract_model
{
	...
	function checkZnoDirection($data, $sysNick)
	{
		if (
			...
			$DispAppoint_id = $this->getFirstResultFromQuery('select DispAppoint_id from DispAppoint da where da.DispAppointType_id = 2 and da.EvnPLDisp_id = ? limit 1', array($data["{$sysNick}_id"]));
			...
			$DispAppoint_id = $this->getFirstResultFromQuery('select DispAppoint_id from DispAppoint da where da.DispAppointType_id = 3 and da.EvnPLDisp_id = ? limit 1', array($data["{$sysNick}_id"]));
			$EvnDirection_id = $this->getFirstResultFromQuery('
				select EvnDirection_id 
				from v_EvnDirection_all ed
				inner join v_EvnPLDisp epd on ed.EvnDirection_rid = epd.EvnPLDisp_id
