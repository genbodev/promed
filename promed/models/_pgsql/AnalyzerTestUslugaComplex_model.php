 ...
 * @version
 */
class AnalyzerTestUslugaComplex_model extends SwPgModel {
	...
	function loadAlowedUslugaCategory_SysNicks($AnalyzerTest_id){
		...
		$q = <<<q
			...
			FROM
			    UslugaCategory
			WHERE
			    UslugaCategory_SysNick IN ('gost2011', 'tfoms', 'syslabprofile', 'promed')
...
q;
	...
	function load() {
		$q = "
			select
				AnalyzerTestUslugaComplex_id as \"AnalyzerTestUslugaComplex_id\", 
				AnalyzerTest_id as \"AnalyzerTest_id\", 
				uc.UslugaComplex_id as \"UslugaComplex_id\", 
				AnalyzerTestUslugaComplex_Deleted as \"AnalyzerTestUslugaComplex_Deleted\",
				c.UslugaCategory_SysNick as \"UslugaCategory_SysNick\"
			from
				lis.v_AnalyzerTestUslugaComplex atuc
				LEFT JOIN v_UslugaComplex uc ON uc.UslugaComplex_id = atuc.UslugaComplex_id
				LEFT JOIN v_UslugaCategory c ON c.UslugaCategory_id = uc.UslugaCategory_id
			where
	...
	function loadList($filter) {
		...
		$q = "
			SELECT
				ATUC.AnalyzerTestUslugaComplex_id as \"AnalyzerTestUslugaComplex_id\",
				ATUC.AnalyzerTest_id as \"AnalyzerTest_id\",
				ATUC.UslugaComplex_id as \"UslugaComplex_id\",
				ATUC.AnalyzerTestUslugaComplex_Deleted as \"AnalyzerTestUslugaComplex_Deleted\",
			...
			FROM
				lis.v_AnalyzerTestUslugaComplex ATUC
				LEFT JOIN lis.v_AnalyzerTest AnalyzerTest_id_ref ON AnalyzerTest_id_ref.AnalyzerTest_id = ATUC.AnalyzerTest_id
				LEFT JOIN dbo.v_UslugaComplex UslugaComplex_id_ref ON UslugaComplex_id_ref.UslugaComplex_id = ATUC.UslugaComplex_id
				LEFT JOIN dbo.v_UslugaCategory c ON c.UslugaCategory_id = UslugaComplex_id_ref.UslugaCategory_id
			$where_clause
		";
	...
	function delete() {
		$q = "
			select
				Error_Code as \"Error_Code\",
				Error_Message as \"Error_Msg\"
			from lis.p_AnalyzerTestUslugaComplex_del (
				AnalyzerTestUslugaComplex_id := :AnalyzerTestUslugaComplex_id
			)
		";
