...
class CmpWorkPlan_model extends SwPgModel {
	...
	function getWorkPlan($data) {
		...
		$query="SELECT
					CWP.CmpWorkPlanData_id as \"CmpWorkPlanData_id\",
					CWP.CmpPlanType_id as \"CmpPlanType_id\",
					CWP.CmpWorkTime_id as \"CmpWorkTime_id\",
					...
					LB.Lpu_id as \"Lpu_id\"
				FROM
					r2.v_CmpWorkPlan CWP
					left join v_LpuBuilding LB on LB.LpuBuilding_id = CWP.LpuBuilding_id
				WHERE
	...
	function getWorkPlans($data) {
		...
		if(!$filters)
			$filters .= " AND CWP.CmpWorkPlan_BegDT = (SELECT MAX(CmpWorkPlan_BegDT) FROM r2.v_CmpWorkPlan WHERE LpuBuilding_id = CWP.LpuBuilding_id)";

		$query="SELECT
					CWP.CmpWorkPlan_id as \"CmpWorkPlan_id\",
					CWP.LpuBuilding_id as \"LpuBuilding_id\",
					CWP.LpuBuilding_Name as \"LpuBuilding_Name\",
					to_char(CmpWorkPlan_BegDT,'dd.mm.yyyy') as \"CmpWorkPlan_BegDT\",
					to_char(CmpWorkPlan_EndDT,'dd.mm.yyyy') as \"CmpWorkPlan_EndDT\",
					LB.Lpu_id as \"Lpu_id\",
	...
	function getSubstationPlans($data){
		...
		$query = '	SELECT DISTINCT
						...
						to_char(CWP.CmpWorkPlan_BegDT,\'dd.mm.yyyy\') as "CmpWorkPlan_BegDT",
						to_char(CWP.CmpWorkPlan_EndDT,\'dd.mm.yyyy\') as "CmpWorkPlan_EndDT"
					FROM
						r2.v_CmpWorkPlan CWP
					WHERE
	 ...
	 */
	function getLpuList() {
		$query="SELECT L.Lpu_id as \"Lpu_id\",L.Lpu_Nick as \"Lpu_Nick\"
				FROM v_Lpu L
