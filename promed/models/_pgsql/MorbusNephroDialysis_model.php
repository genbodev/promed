 ...
 * @version      11.2014
 */
class MorbusNephroDialysis_model extends swPgModel
{
	/**
	 * Method description
	 */
	...
	protected function _validate()
	{
		...
		if (in_array($this->scenario, array(self::SCENARIO_DO_SAVE))) {
			...
			if (!empty($this->endDT)) {
				$checkFilter = "
					(MorbusNephroDialysis_begDT <= :MorbusNephroDialysis_begDT and coalesce(MorbusNephroDialysis_endDT, :MorbusNephroDialysis_begDT) >= :MorbusNephroDialysis_begDT)
					OR
					(MorbusNephroDialysis_begDT <= :MorbusNephroDialysis_endDT and coalesce(MorbusNephroDialysis_endDT, :MorbusNephroDialysis_endDT) >= :MorbusNephroDialysis_endDT)
				";
				$queryParams['MorbusNephroDialysis_endDT'] = $this->endDT;
			} else {
				$checkFilter = ":MorbusNephroDialysis_begDT <= coalesce(MorbusNephroDialysis_endDT, :MorbusNephroDialysis_begDT)";
			}
	...
	function doLoadGrid($data)
	{
		...
		$sql = "
			select
				...
				t.MorbusNephroDialysis_id as \"MorbusNephroDialysis_id\",
				t.MorbusNephro_id as \"MorbusNephro_id\",
				to_char(t.MorbusNephroDialysis_begDT, 'dd.mm.yyyy') as \"MorbusNephroDialysis_begDT\",
				to_char(t.MorbusNephroDialysis_endDT, 'dd.mm.yyyy') as \"MorbusNephroDialysis_endDT\",
				OutCause.PersonRegisterOutCause_Name as \"PersonRegisterOutCause_Name\",
				l.Lpu_Nick as \"Lpu_Nick\",
				:Evn_id as \"MorbusNephro_pid\"
			from
				v_MorbusNephroDialysis t
				inner join v_MorbusNephro MV on MV.MorbusNephro_id = t.MorbusNephro_id
				left join v_PersonRegisterOutCause OutCause on OutCause.PersonRegisterOutCause_id = t.PersonRegisterOutCause_id
				left join v_Lpu l on l.Lpu_id = t.Lpu_id
			where
				{$filters}
			order by
