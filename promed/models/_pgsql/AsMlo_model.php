...
class AsMlo_model extends SwPgModel {
	...
	function getMedServiceData() {
		...
		if (!empty($MedServiceId)) {
				...
				$resp = $this->queryResult("
					...
					from
						v_LpuSection ls
						inner join v_MedService ms on ms.LpuSection_id = ls.LpuSection_id and ms.MedServiceType_id = 7
					where
	...
	function isSend2AnalyzerEnabled($data)
	{
		...
		if (!empty($data['EvnLabSamples'])) {
			...
			with x1 as (
				...
				FROM
					v_EvnLabSample s
					LEFT JOIN lis.v_Analyzer a ON a.Analyzer_id = s.Analyzer_id
				WHERE
	...
	function get2wayAnalyzersForEvnLabRequests($data)
	{
		...
		if (!empty($innerData['EvnLabRequests'])) {
			...
			with m as (
				...
				FROM v_EvnLabRequest r 
					LEFT JOIN v_EvnLabSample s ON r.EvnLabRequest_id = s.EvnLabRequest_id
					LEFT JOIN lis.v_Analyzer a ON a.MedService_id = r.MedService_id OR a.MedService_id = s.MedService_id
				WHERE 
	...
	function doSetSuccessConfirmation() {
		...
		$query = "
			...
			from
				lis.v_Link l
				inner join v_EvnLabSample els on els.EvnLabSample_id = l.object_id
			where
				...
				if (!empty($result['Error_Msg'])) {
					$query ="
							...
							FROM v_EvnLabSample els
								inner join lis.v_Analyzer a on a.Analyzer_id = els.Analyzer_id
								left join lis.v_Link lnk on lnk.object_id = els.EvnLabSample_id and lnk.link_Object = 'EvnLabSample'
				...
				foreach ($sample['targets'] as $target) {
					if (!empty($target['protocol'])) {
						...
						if (!empty($resp_eup[0]['EvnUslugaPar_id'])) {
							...
							$resp = $this->common->POST('EvnMediaData', array(
	...
	function getFormulaSample($data, $pushed = false)
	{
		...
		$query = "
			...
			SELECT DISTINCT
				...
				(select EvnLabSample_id from myvars) as \"EvnLabSample_id\",
			...
			FROM v_UslugaComplex uc
				INNER JOIN lis.v_AnalyzerTestFormulaArguments atfa on atfa.Usluga_id = uc.UslugaComplex_id
				INNER JOIN lis.v_AnalyzerTestFormula atf on atf.AnalyzerTest_id = atfa.AnalyzerTest_id
					and atf.Analyzer_id = (select Analyzer_id from myvars)
			WHERE uc.UslugaComplex_2011id in (select UslugaComplex_id from UslugaList)
		";
	...
	function getSampleTests($data)
	{
		...
		$query = "
			Select
				...
				case when length(EvnLabSample_Num)=12 then substring(EvnLabSample_Num from (length(EvnLabSample_Num)-3) for 4) else EvnLabSample_Num end as \"EvnLabSample_Num\", --номер пробы для вывода сообщения
		...
		if (!empty($check['EvnLabSample_ids'])) {
			if (empty($data['changeNumber'])) {
				...
				foreach($check['EvnLabSample_ids'] as $EvnLabSample_id) {
					...
					$time = $this->getFirstResultFromQuery("select to_char(dbo.tzgetdate(), 'HH24:MI DD.MM.YYYY')");
					$this->db->query("
						UPDATE EvnLabSample
						SET
							EvnLabSample_Num = :EvnLabSample_Num,
							EvnLabSample_BarCode = :EvnLabSample_BarCode,
							Evn_setDT = to_timestamp(:Evn_setDT, 'HH24:MI DD.MM.YYYY')
