...
class Khak_Registry_model extends Registry_model
{
	...
	{
		return ",OrgSMO_id as \"OrgSMO_id\",DispClass_id as \"DispClass_id\",Registry_IsOnceInTwoYears as \"Registry_IsOnceInTwoYears\"";
	}
	...
	{
		return ", R.OrgSMO_id as \"OrgSMO_id\", R.DispClass_id as \"DispClass_id\", R.Registry_IsOnceInTwoYears as \"Registry_IsOnceInTwoYears\",R.Registry_IsZNO as \"Registry_IsZNO\"";
	}
	...
	{
		return ", R.DispClass_id as \"DispClass_id\", R.Registry_IsOnceInTwoYears as \"Registry_IsOnceInTwoYears\",R.Registry_IsZNO as \"Registry_IsZNO\"";
	}
	...
	public function getYearsList($data)
	{
		if (6 == (int)$data['RegistryStatus_id']) {
			...
			$query = "
				select distinct
					EXTRACT(YEAR FROM Registry_begDate) as \"reg_year\"
				from
	...
	protected function _loadRegistrySCHETForXmlUsing($type, $data)
	{
		...
		$query = "select * from {$p_schet} (Registry_id := :Registry_id)";
		$result = $this->db->query($query, $data);
	...
	protected function _loadRegistryDataForXml($type, $data, $file_re_data_name, $file_re_pers_data_name, $registry_data_template_body, $person_data_template_body)
	{
		...
		// посещения
		$query = "select * from {$p_sl} (Registry_id := :Registry_id)";
		...
		if ( !empty($p_lekpr) ) {
			$query = "select * from {$p_lekpr} (Registry_id := :Registry_id)";
			$this->textlog->add('Запуск ' . getDebugSQL($query, $queryParams));
	...
	protected function _setXmlPackNum($data)
	{
		$query = "
						...
						set Registry_pack = (COALESCE((
							select MAX(Registry_pack)
							FROM (
							(select max(Registry_pack) as rp
