 ...
 */
class EvnUslugaOnkoChem_model extends swPgModel
{
	...
	function load()
	{
		$q = "
			select
				EU.EvnUslugaOnkoChem_id as \"EvnUslugaOnkoChem_id\", 
				EU.EvnUslugaOnkoChem_pid as \"EvnUslugaOnkoChem_pid\", 
				EU.Server_id as \"Server_id\", 
				EU.PersonEvn_id as \"PersonEvn_id\", 
				EU.Person_id as \"Person_id\",
				to_char(EU.EvnUslugaOnkoChem_setDT, 'DD.MM.YYYY') as \"EvnUslugaOnkoChem_setDate\",
				to_char(EU.EvnUslugaOnkoChem_setDT, 'HH24:MI') as \"EvnUslugaOnkoChem_setTime\",
				to_char(EU.EvnUslugaOnkoChem_disDT, 'DD.MM.YYYY') as \"EvnUslugaOnkoChem_disDate\",
				to_char(EU.EvnUslugaOnkoChem_disDT, 'HH24:MI') as \"EvnUslugaOnkoChem_disTime\",
				MO.Morbus_id as \"Morbus_id\", 
				MO.MorbusOnko_id as \"MorbusOnko_id\", 
				EU.Lpu_uid as \"Lpu_uid\",
				EU.OnkoUslugaChemKindType_id as \"OnkoUslugaChemKindType_id\", 
				EU.OnkoUslugaChemFocusType_id as \"OnkoUslugaChemFocusType_id\", 
				...
				EU.OnkoUslugaChemStageType_id as \"OnkoUslugaChemStageType_id\",
				EU.EvnUslugaOnkoChem_Scheme as \"EvnUslugaOnkoChem_Scheme\",
				UC.UslugaCategory_id as \"UslugaCategory_id\",
				UC.UslugaComplex_id as \"UslugaComplex_id\"
			from
				dbo.v_EvnUslugaOnkoChem EU 
	...
	function save($data) {
		...
		$q = "
			WITH ct1 AS (
                SELECT CAST(:PayType_id as bigint) AS pt
            ),
            ct2 AS (
                SELECT CASE
                     WHEN COALESCE((SELECT pt FROM ct1), 0) = 0 AND :EvnUslugaOnkoChem_pid IS NOT NULL
                     THEN (SELECT PayType_id FROM v_EvnSection WHERE EvnSection_id =:EvnUslugaOnkoChem_pid LIMIT 1)
                     ELSE (SELECT pt FROM ct1) END 
                AS pt
            ),
            ct3 AS (
                SELECT CASE
                     WHEN COALESCE((SELECT pt FROM ct2), 0) = 0 AND :EvnUslugaOnkoChem_pid IS NOT NULL
                     THEN (SELECT PayType_id FROM v_EvnVizit WHERE EvnVizit_id = :EvnUslugaOnkoChem_pid LIMIT 1)
            ...
            select EvnUslugaOnkoChem_id as \"EvnUslugaOnkoChem_id\", Error_Code as \"Error_Code\", Error_Message as \"Error_Msg\"
			from dbo." . $procedure . "(
	...
	function delete($data)
	{
		...
		$q = "
			...
			from dbo.p_EvnUslugaOnkoChem_del(
				EvnUslugaOnkoChem_id := :EvnUslugaOnkoChem_id);
		";
	...
	function getEvnUslugaOnkoChemForAPI($data) {
		...
		return $this->queryResult("
			select
				mo.MorbusOnko_id as \"MorbusOnko_id\",
				euoc.EvnUslugaOnkoChem_id as \"EvnUslugaOnkoChem_id\",
				euoc.EvnUslugaOnkoChem_id as \"EvnUsluga_id\",
				to_char(euoc.EvnUslugaOnkoChem_setDT, 'YYYY-MM-DD') as \"Evn_setDT\",
				to_char(euoc.EvnUslugaOnkoChem_disDT, 'YYYY-MM-DD') as \"Evn_disDT\",
				euoc.OnkoUslugaChemKindType_id as \"OnkoUslugaChemKindType_id\",
				euoc.OnkoUslugaChemFocusType_id as \"OnkoUslugaChemFocusType_id\",
				euoc.TreatmentConditionsType_id as \"TreatmentConditionsType_id\",
				euoc.AggType_id as \"AggType_id\",
				euoc.OnkoTreatType_id as \"OnkoTreatType_id\",
				euoc.EvnUslugaOnkoChem_Scheme as \"EvnUslugaOnkoChem_Scheme\",
				euoc.OnkoUslugaChemStageType_id as \"OnkoUslugaChemStageType_id\"
			from
				v_MorbusOnko mo 
				inner join v_EvnUslugaOnkoChem euoc  on euoc.Morbus_id = mo.Morbus_id
			where
				1=1
				{$filter}
		", $queryParams);
