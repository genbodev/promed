...
class DocumentUc_model extends swPgModel {
	...
	function load($data) {
		$query = "
			select
				...
				du.DocumentUc_pid as \"DocumentUc_pid\",
				du.Contragent_id as \"Contragent_id\",
				du.DocumentUc_Num as \"DocumentUc_Num\",
				du.DocumentUc_setDate as \"DocumentUc_setDate\",
				du.DocumentUc_didDate as \"DocumentUc_didDate\",
				du.DocumentUc_InvoiceNum as \"DocumentUc_InvoiceNum\",
				du.DocumentUc_InvoiceDate as \"DocumentUc_InvoiceDate\",
				du.WhsDocumentUc_id as \"WhsDocumentUc_id\",
				du.DocumentUc_DogDate as \"DocumentUc_DogDate\",
				du.DocumentUc_DogNum as \"DocumentUc_DogNum\",
				du.Org_id as \"Org_id\",
				du.Contragent_sid as \"Contragent_sid\",
				du.Storage_sid as \"Storage_sid\",
				du.Mol_sid as \"Mol_sid\",
				smol.Person_Fio as \"Mol_sPerson\",
				du.Contragent_tid as \"Contragent_tid\",
				du.Storage_tid as \"Storage_tid\",
				du.Mol_tid as \"Mol_tid\",
				...
				du.EmergencyTeam_id as \"EmergencyTeam_id\",
				du.StorageZone_sid as \"StorageZone_sid\",
				du.StorageZone_tid as \"StorageZone_tid\",
				...
				invent.WhsDocumentUcInvent_id as \"WhsDocumentUcInvent_id\",
                dul.Lpu_id as \"Lpu_id\",
                dul.LpuBuilding_id as \"LpuBuilding_id\",
				--  Читаем дату закрытия отчетного периода
				/*
				(SElect COALESCE(max(DrugPeriodClose_DT), '2016-06-01') DrugPeriodClose_DT  from DrugPeriodClose 
				...
				*/
				(SElect COALESCE(
					...
					'2016-06-01') DrugPeriodClose_DT  from DrugPeriodClose 

						where Org_id = du.Org_id
                        limit 1
				) as \"DrugPeriodClose_DT\",
				(
				    COALESCE(wdc.WhsDocumentClass_Name, wdt.WhsDocumentType_Name) ||

				    ' № ' ||
                    cast(wdu.WhsDocumentUc_Num as varchar) ||
                    ' от ' ||
                    to_char(wdu.WhsDocumentUc_Date, 'DD.MM.YYYY')

				) as \"WhsDocumentUc_FullName\",
                (
                    select 
                        '№ ' ||
                        cast(du2.DocumentUc_Num as varchar) ||
                        ' от ' ||
                        to_char(du2.DocumentUc_setDate, 'DD.MM.YYYY')
					...
					where
                        ...
                        and  date_part('YEAR',du2.DocumentUc_setDate) = date_part('YEAR',du.DocumentUc_setDate)
                    limit 1
				) as \"DocW\", -- Поле инициализации дубля документа прихода (исполненного)
				acc_type.AccountType_id as \"AccountType_id\",
				du.SubAccountType_tid as \"SubAccountType_tid\",
				du.SubAccountType_sid as \"SubAccountType_sid\",
				case
					when du.DrugDocumentType_id = 27 or du.DrugDocumentType_id = 28
						then rtrim(COALESCE(p.Person_Fio,'')||' '||COALESCE(to_char(p.Person_BirthDay, 'DD.MM.YYYY'),''))
			...
			from
				...
				left join v_WhsDocumentUc wdu  on wdu.WhsDocumentUc_id = du.WhsDocumentUc_id
				...
				left join v_WhsDocumentUcInvent invent  on invent.WhsDocumentUc_id = du.WhsDocumentUc_id

				left join v_Person_all p  on p.Person_id = du.Person_id
				...
				LEFT JOIN LATERAL (
					...
					from
						...
						left join v_MedPersonal mp  on mp.MedPersonal_id = m.MedPersonal_id

						left join v_PersonState ps  on ps.Person_id = mp.Person_id or ps.Person_id = m.Person_id
				...
				LEFT JOIN LATERAL (
					...
					select
						(
							er.EvnRecept_Ser||
							' № '||er.EvnRecept_Num||
							COALESCE(' от '||to_char(er.EvnRecept_setDT, 'DD.MM.YYYY'), '')||
				...
				LEFT JOIN LATERAL (
                    ...
                    from
                        ...
                        inner join v_DrugShipmentLink i_dsl  on i_dsl.DocumentUcStr_id = i_dus.DocumentUcStr_id
				...
				LEFT JOIN LATERAL (
				    ...
				    select 
                        ltrim(rtrim( COALESCE(ETSC.EmergencyTeamSpec_Name||' ','')||'Бригада № '||COALESCE(et.EmergencyTeam_Num,'')||' / '||COALESCE(mp_hs.Person_Fio,'')||COALESCE((' '||duty.EmergencyTeamDuty_DTStartDate || ' ' || duty.EmergencyTeamDuty_DTStart ||' - '),'')||COALESCE((duty.EmergencyTeamDuty_DTFinishDate || ' ' || duty.EmergencyTeamDuty_DTFinish),'') )) as objectName

                    from
                        v_EmergencyTeam et 

                        left join v_MedPersonal mp_hs  on mp_hs.MedPersonal_id = et.EmergencyTeam_HeadShift
                        ...
                        LEFT JOIN LATERAL (
							...
							select 
								...
								to_char(etd.EmergencyTeamDuty_DTStart, 'DD.MM.YYYY') as EmergencyTeamDuty_DTStartDate,

								to_char(etd.EmergencyTeamDuty_DTFinish, 'DD.MM.YYYY') as EmergencyTeamDuty_DTFinishDate,

								to_char(etd.EmergencyTeamDuty_DTStart, 'HH24:MI:SS') as EmergencyTeamDuty_DTStart,

								to_char(etd.EmergencyTeamDuty_DTFinish, 'HH24:MI:SS') as EmergencyTeamDuty_DTFinish
	...
	function loadList($data) {
		...
		if(!empty($data['DocumentUc_id']) && $data['DocumentUc_id'] > 0) {
			...
			if (!empty($data['query'])) {
				$filter .= " and du.DocumentUc_Num iLIKE '%'||:DocumentUc_Num||'%'";
	...
	function deleteDrugShipLink($DrugShipmentLink_id)
	{
		$query = "
            select Error_Code as \"Error_Code\", Error_Message as \"Error_Message\"
			from dbo.p_DrugShipmentLink_del(
	...
	function loadStorageList($data) {
		...
		if(!empty($data['Storage_id']) && empty($data['query'])) {
			...
			if (
                ...
                if (!empty($data['StructFilterPreset']) && !empty($data['Lpu_oid'])) { //используем готовые персеты фильтров структуры
							...
							if (!empty($data['LpuSection_id']) || !empty($data['Lpu_oid'])) {
								...
								$with[] = "ls_storage_list as (
									...
									from
										...
										left join v_Storage i_s  on i_s.Storage_id = i_ssl.Storage_id or i_s.Storage_pid = i_ssl.Storage_id
                        	...
                        	if (!empty($data['LpuSection_id'])) { //форма swNewEvnDrugEditWindow поле склад
								if (!empty($data['LpuSection_id'])) {
									$storage_list = $this->queryResult("
										select distinct
											i_s.Storage_id as \"Storage_id\",
											i_ms.MedService_id as \"MedService_id\"
										from
											v_StorageStructLevel i_ssl 
											...
											LEFT JOIN LATERAL ( -- проверка связаны ли дочерние склады со службой с типом АРМ товароведа
							...
							if (!empty($data['MedService_id'])) {
								...
								$with[] = "storage_tree as (
									select
										i_s.Storage_id
									from
										ms_storage_list i_msl
										left join v_Storage i_s on i_s.Storage_pid = i_msl.Storage_id
									where
						...
						case 'DocumentUcEdit_Storage_tid_Lpu_UserOrg': //форма редактирования документа учета, поле "склад-получатель", тип организации - МО, организация является организацией пользователя
							if (!empty($data['Storage_sid'])) {
								...
								$with[] = "ls_s_ssl as (
										...
										from
											v_StorageStructLevel i_ssl
											left join v_StorageStructLevel i_ls_ssl on i_ls_ssl.LpuBuilding_id = i_ssl.LpuBuilding_id
										where
								...
								$with[] = "children_list as ( -- дочерние склады складов из storage_list (кроме тех, что являются складами отделения)
									...
									from
										...
										inner join v_Storage s on s.Storage_pid = storage_list.Storage_id
									where
                    ...
                    if (!empty($data['Org_id'])) {
                        $struct_filter .= " and COALESCE(SSL.Org_id, L.Org_id) = :Org_id";
                    ...
                    if (isset($data['session']) && !empty($data['filterByOrgUser']) && $data['filterByOrgUser']) {
                        ...
                        $struct_filter .= " and COALESCE(SSL.OrgStruct_id,0) in ({$ids_str})";
			...
			if (!empty($data['EvnPrescrTreatDrug_id'])) {
				$TreatDrugInfo = $this->getFirstRowFromQuery("
					select Drug_id as \"Drug_id\", DrugComplexMnn_id as \"DrugComplexMnn_id\"
					from v_EvnPrescrTreatDrug 
		...
		$query = "
			...
			from
				...
				left join v_MedService MS  on MS.MedService_id = StSL.MedService_id

				left join v_MedServiceType MST  on MST.MedServiceType_id = MS.MedServiceType_id

				left join v_Address adr  on adr.Address_id = Storage.Address_id
	...
	function loadStorage2LpuList($data) {
		...
		if(!empty($data['Storage_id']) && $data['Storage_id'] > 0) {
			...
			if (!empty($data['Org_id']) && $data['Org_id'] > 0) {
				$filter .= " and Storage.Storage_id in (select Storage_id from v_StorageStructLevel  where Org_id = :Org_id)";

				$params['Org_id'] = $data['Org_id'];
			}
			if (!empty($data['Lpu_oid']) && $data['Lpu_oid'] > 0) {
				$filter .= " and Storage.Storage_id in (select Storage_id from v_StorageStructLevel  where Lpu_id = :Lpu_oid)";

				$params['Lpu_oid'] = $data['Lpu_oid'];
			}
			if (!empty($data['MedService_id']) && $data['MedService_id'] > 0) {
				$filter .= " and Storage.Storage_id in (select Storage_id from v_StorageStructLevel  where MedService_id = :MedService_id)";

				$params['MedService_id'] = $data['MedService_id'];
			}
			if (!empty($data['LpuSection_id']) && $data['LpuSection_id'] > 0) {
				$filter .= " and Storage.Storage_id in (select Storage_id from v_StorageStructLevel  where LpuSection_id = :LpuSection_id)";
		...
		$query = "
			select
				...
				rtrim(Storage.Storage_Name) as \"Storage_Name\",
                                ...
                                farm.Lpu_Nick as \"Lpu_Nick\"
			from
				v_Storage Storage 

                                LEFT JOIN LATERAL (

					Select i.Lpu_id, lpu.Lpu_Nick from v_OrgFarmacyIndex i 

						left join v_lpu lpu  on lpu.Lpu_id = i.Lpu_id
	...
	function loadDocumentUcStrList($filter) {
        ...
        switch($doc_type_code) {
            case '6': //Приходная накладная
                $select .= "to_char(dus.DocumentUcStr_RegDate, 'DD.MM.YYYY') as \"DocumentUcStr_RegDate\", ";
            ...
            case '34': //Разукомплектация: списание
                ...
                $select .= "COALESCE(isnds.YesNo_Code, 0) as \"PostDocumentUcStr_IsNDS\", ";
                ...
                $select .= "
                    (
                        case
                            ...
                            then
                                COALESCE(post_dus.DocumentUcStr_Sum, 0)

                            else
                                COALESCE(post_dus.DocumentUcStr_Sum, 0) + COALESCE(post_dus.DocumentUcStr_SumNds, 0)
                ...
                $join .= "
                    LEFT JOIN LATERAL (
                        ...
                        from
                            ...
                            left join v_GoodsUnit i_gu  on i_gu.GoodsUnit_id = i_dus.GoodsUnit_id

                            left join v_GoodsUnit i_gu_b  on i_gu_b.GoodsUnit_id = i_dus.GoodsUnit_bid
		...
		$query = "
			select
				...
				(
				    ...
				    COALESCE(' '||o_ds.AccountType_Name, '')

				) as \"DocumentUcStr_oName\",
				dus.Drug_id as \"Drug_id\",
				d.Drug_Name as \"Drug_Name\",
				dnmn.DrugNomen_Code as \"DrugNomen_Code\",
				dus.DocumentUcStr_PlanKolvo as \"DocumentUcStr_PlanKolvo\",
				...
				COALESCE(ps.PrepSeries_Ser, dus.DocumentUcStr_Ser) as \"DocumentUcStr_Ser\",
				...
				COALESCE(okei.Okei_NationSymbol, 'упак') as \"Okei_NationSymbol\",

				sf.cnt as \"SavedFileCount\",
				ds.DrugShipment_Name as \"DrugShipment_Name\",
				ds.AccountType_Name as \"AccountType_Name\",
				dus.DocumentUcStr_Reason as \"DocumentUcStr_Reason\",
				pers.Person_id as \"Person_id\",
				COALESCE(pers.Person_SurName || ' ', '') || COALESCE(pers.Person_FirName || ' ', '') || COALESCE(pers.Person_SecName,'') as \"Person_Fio\",

				dus.DrugDocumentStatus_id as \"DrugDocumentStatus_id\",
				dds.DrugDocumentStatus_Code as \"DrugDocumentStatus_Code\",
				dds.DrugDocumentStatus_Name as \"DrugDocumentStatus_Name\",
				...
				(case
				    ...
				    then
				        rtrim(COALESCE(sz.StorageZone_Address,'') || ' ' || COALESCE(sut.StorageUnitType_Name,''))

				    else
				        'Без места хранения'
				end) as \"StorageZone_Name\",
				last_dusw.DocumentUcStorageWork_id as \"DocumentUcStorageWork_id\",
				last_dusw.DocumentUcStorageWork_FactQuantity as \"DocumentUcStorageWork_FactQuantity\",
				last_dusw.DocumentUcStorageWork_Comment as \"DocumentUcStorageWork_Comment\",
				...
				COALESCE(dpbc.cnt, 0) as \"SavedBarCode_Count\",

				COALESCE(dpbc.cnt, 0) as \"BarCode_Count\",
				...
				(
                    case
                        when COALESCE(am.NARCOGROUPID, 0) > 0 or COALESCE(am.STRONGGROUPID, 0) > 0
				...
				RTRIM(COALESCE(dp.DrugPrep_Name, '')) as \"hintTradeName\", --Торговое наименование, лекарственная форма, дозировка, фасовка

				COALESCE(d.Drug_RegNum || ',   ', '') || COALESCE(to_char(d.Drug_begDate, 'DD.MM.YYYY') || ', ', '') || COALESCE(to_char(d.Drug_endDate, 'DD.MM.YYYY') ||', ', '--, ') || COALESCE(to_char(REG.REGCERT_insDT, 'DD.MM.YYYY')||', ', '') || COALESCE(REGISTR.regNameCauntries, '') as \"hintRegistrationData\", --Данные о регистрации
			...
			from
				...
				left join v_PersonState pers  on pers.Person_id = dus.Person_id

				left join v_StorageZone sz  on sz.StorageZone_id = dus.StorageZone_id

				left join v_StorageUnitType sut  on sut.StorageUnitType_id = sz.StorageUnitType_id

				left join v_DrugDocumentStatus dds  on dds.DrugDocumentStatus_id = dus.DrugDocumentStatus_id

				left join v_GoodsUnit gu  on gu.GoodsUnit_id = dus.GoodsUnit_id

				left join v_GoodsUnit gu_b  on COALESCE(gu_b.GoodsUnit_id, :DefaultGoodsUnit_id) = COALESCE(dus.GoodsUnit_bid, :DefaultGoodsUnit_id)



				left join rls.drugprep dp  on d.DrugPrepFas_id = dp.DrugPrepFas_id

				left join rls.PREP P  on P.Prep_id = d.DrugPrep_id

				left join rls.REGCERT REG  on REG.REGCERT_ID =P.REGCERTID

				left join rls.NOMEN NM  on NM.Nomen_id = d.Drug_id
				...
				LEFT JOIN LATERAL (
					...
					from
						...
						left join v_DrugShipment i_dsh  on i_dsh.DrugShipment_id = i_dsl.DrugShipment_id

						left join v_AccountType i_at  on i_at.AccountType_id = i_dsh.AccountType_id
		...
		$query = "
			select
				...
				dusw.DocumentUcStr_id as \"DocumentUcStr_id\",
				to_char(dusw.DocumentUcStorageWork_insDT, 'DD.MM.YYYY') as \"DocumentUcStorageWork_insDT\",

				to_char(dusw.DocumentUcStorageWork_endDate, 'DD.MM.YYYY')||' '||to_char(dusw.DocumentUcStorageWork_endDate, 'HH24:MI:SS') as \"DocumentUcStorageWork_endDate\",


				dusw.DocumentUcTypeWork_id as \"DocumentUcTypeWork_id\",
				dusw.Person_eid as \"Person_eid\",
				dusw.Post_eid as \"Post_eid\",
				ePW.PersonWork_id as \"PersonWork_eid\",
				dusw.DocumentUcStorageWork_FactQuantity as \"DocumentUcStorageWork_FactQuantity\",
				dusw.DocumentUcStorageWork_Comment as \"DocumentUcStorageWork_Comment\",
				1 as \"RecordStatus_Code\"
			from
				v_DocumentUcStorageWork dusw 

				inner join v_DocumentUcStr dus  on dus.DocumentUcStr_id = dusw.DocumentUcStr_id

				left join v_PersonWork ePW  on ePW.Person_id = dusw.Person_eid and ePW.Post_id = dusw.Post_eid and ePW.Org_id = :Org_id
	...
	function getDocumentUcStrListByWhsDocumentSupply($data) {
        ...
        $query = "
            select
                ...
                COALESCE(o.Okei_NationSymbol, 'упак') as \"Okei_NationSymbol\"
        ...
        if ($suppliers_ostat_control) {
            $query = "
                select
                    ...
                    ps.PrepSeries_id as \"PrepSeries_id\",
                    ps.PrepSeries_Ser as \"PrepSeries_Ser\",
                    to_char(ps.PrepSeries_GodnDate, 'YYYY-MM-DD HH24:MI:SS') as \"PrepSeries_godnDate\",
            ...
            $query = "
                ...
                from
                    ...
                    left join v_DrugNds dn  on dn.DrugNds_Code = wdss.WhsDocumentSupplySpec_NDS
	...
	function getDocumentUcStrListByWhsDocumentUcOrder($data) {
		...
		$query = "
			select
                ...
                dn.DrugNds_Code as \"DrugNds_Code\"
            from
                v_WhsDocumentUcOrder wduo 

                left join v_WhsDocumentUcOrderMaterial wduom  on wduom.WhsDocumentUcOrder_id = wduo.WhsDocumentUcOrder_id

                left join v_DrugShipment ds  on ds.DrugShipment_id = wduom.DrugShipment_id

                left join v_DrugShipmentLink dsl  on dsl.DrugShipment_id = wduom.DrugShipment_id

                left join rls.v_Drug d  on d.Drug_id = wduom.Drug_id

                left join rls.v_PrepSeries ps  on ps.PrepSeries_id = wduom.PrepSeries_id

                left join v_YesNo isdef  on isdef.YesNo_id = ps.PrepSeries_isDefect

                left join v_DrugNds dn  on dn.DrugNds_id = wduo.DrugNds_id
                ...
                $query = "
                    select 
                        ...
                        (
                            ...
                            COALESCE(' '||ds.AccountType_Name, '')

                        ) as \"DocumentUcStr_oName\",
                        dus.Drug_id as \"Drug_id\",
                        d.Drug_Name as \"Drug_Name\",
                        dnmn.DrugNomen_Code as \"DrugNomen_Code\",
                        dus_cnt.DocumentUcStr_Count as \"DocumentUcStr_Count\",
                        dus_cnt.DocumentUcStr_EdCount as \"DocumentUcStr_EdCount\",
                        dus.DrugNds_id as \"DrugNds_id\",
                        COALESCE(dn.DrugNds_Code, 0) as \"DrugNds_Code\",

                        dus.DocumentUcStr_Price as \"DocumentUcStr_Price\",
                        dus_cnt.DocumentUcStr_Sum as \"DocumentUcStr_Sum\",
                        dus_cnt.DocumentUcStr_SumNds as \"DocumentUcStr_SumNds\",
                        (
                            case
                                when
                                    COALESCE(isnds.YesNo_Code, 0) = 1
                                ...
                                then
                                    COALESCE(dus_cnt.DocumentUcStr_Sum, 0)

                                else
                                    COALESCE(dus_cnt.DocumentUcStr_Sum, 0) + COALESCE(dus_cnt.DocumentUcStr_SumNds, 0)
                        ...
                        dpbc.DrugPackageBarCode_id as \"DrugPackageBarCode_id\",
                        dpbc.DrugPackageBarCode_BarCode as \"DrugPackageBarCode_BarCode\",
                        dpbc.DrugPackageBarCodeType_id as \"DrugPackageBarCodeType_id\",
                        dpbc.DrugPackageBarCode_GTIN as \"DrugPackageBarCode_GTIN\",
                        dpbc.DrugPackageBarCode_SeriesNum as \"DrugPackageBarCode_SeriesNum\",
                        to_char(dpbc.DrugPackageBarCode_expDT, 'DD.MM.YYYY') as \"DrugPackageBarCode_expDT\",

                        dpbc.DrugPackageBarCode_TNVED as \"DrugPackageBarCode_TNVED\",
                        dpbc.DrugPackageBarCode_FactNum as \"DrugPackageBarCode_FactNum\"
                    from
                        ...
                        LEFT JOIN LATERAL (
                            ...
                            select
                                ...
                                dus.DocumentUcStr_EdCount/COALESCE(dus.DocumentUcStr_Count, 1) as DocumentUcStr_EdCount,

                                dus.DocumentUcStr_Sum/COALESCE(dus.DocumentUcStr_Count, 1) as DocumentUcStr_Sum,

                                dus.DocumentUcStr_SumNds/COALESCE(dus.DocumentUcStr_Count, 1) as DocumentUcStr_SumNds
	...
	function deleteDocumentUcStr($data) {
        ...
        $query = "
            select
                dus.DocumentUcStr_id as \"DocumentUcStr_id\",
                ddt.DrugDocumentType_SysNick as \"DrugDocumentType_SysNick\",
                COALESCE(dds_s.DrugDocumentStatus_Code, 1) as \"StrStatus_Code\",

                COALESCE(dds_d.DrugDocumentStatus_Code, 1) as \"DocStatus_Code\"
            ...
            from
                ...
                left join v_DrugDocumentStatus dds_s  on dds_s.DrugDocumentStatus_id = dus.DrugDocumentStatus_id

                left join v_DrugDocumentStatus dds_d  on dds_d.DrugDocumentStatus_id = du.DrugDocumentStatus_id
		...
		if ($result && $result['cnt'] == 0) {
			...
			$query = "
				...
				where
					...
					and not exists(
						select * from v_WhsDocumentUcInventDrug 
            ...
            $query = "
                select
                    DrugPackageBarCode_id as \"DrugPackageBarCode_id\"
                from
                    v_DrugPackageBarCode 
	...
	public function loadDrugComboForDocumentUcStr($filter) {
		...
		if ($filter['Drug_id'] > 0) {
			...
			if (strlen($filter['query']) > 0) {
                ...
                $filter['query'] = preg_replace('/\'/', '', $filter['query']);
				$where_drug[] = 'lower(d.Drug_Name) LIKE lower(\''.$filter['query'].'\')'; //по неизвестной причине, если отдавать подстроку параметром, запрос перестает работать
			...
			if ((!empty($filter['Storage_id']) && $filter['Storage_id'] > 0) || (!empty($filter['Contragent_id']) && $filter['Contragent_id'] > 0)) {
                ...
                if (!empty($filter['DrugShipment_setDT_max'])) {
                    $ost_sub_join .= "left join v_DrugShipment ds  on ds.DrugShipment_id = dor.DrugShipment_id ";

                    $ost_sub_where .= "cast(ds.DrugShipment_setDT as date) <= CAST(:DrugShipment_setDT_max as date) and ";
                }
                    ...
                    $ost_sub_join .= "
                        LEFT JOIN LATERAL (
							...
							where
								...
								COALESCE(dor.PrepSeries_id,0) = COALESCE(dsz2.PrepSeries_id,0) and

								COALESCE(dor.DrugShipment_id,0) = COALESCE(dsz2.DrugShipment_id,0)
				...
				$join_drug[] = "left join ostat_cnt on ostat_cnt.Drug_id = d.Drug_id";
				$where_drug[] = 'ostat_cnt.cnt > 0';
			}
		}
		$where_drug_str = count($where_drug) > 0 ? " WHERE ".join($where_drug, ' and '): "";
		$with[] = " drugCTE AS (
						SELECT d.* 
						FROM 
							rls.v_Drug d
		...
		if (count($where) > 0 || count($where_drug) > 0 || count($with) > 0) {
			$where_str = count($where) > 0 ? "
			where
			-- where 
			".join($where, ' and ')
			."-- end where
			...
			$q = "
				select 
					...
					d.Drug_Nomen as \"hintPackagingData\",  --Данные об упаковке – это часть поля Упаковка (drug_nomen) от начала строки до данных о производителе.
					rtrim(COALESCE(d.Drug_RegNum || '.   ', '') || COALESCE(to_char(d.Drug_begDate, 'DD.MM.YYYY') || ', ', '') || COALESCE(to_char(d.Drug_endDate, 'DD.MM.YYYY') ||', ', '--, ') || COALESCE(to_char(REG.REGCERT_insDT, 'DD.MM.YYYY')||', ', '') || COALESCE(REGISTR.regNameCauntries, '')) as \"hintRegistrationData\", --Данные о регистрации
				...
				from
                    ...
                    left join rls.STRONGGROUPS sg  on sg.STRONGGROUPS_ID = am.STRONGGROUPID
					...
					LEFT JOIN LATERAL (
						...
						from
							...
							left join rls.v_PrepClass  on v_PrepClass.PrepClass_id = v_DrugNomen.PrepClass_id
	...
	function loadContragentList($data) {
		...
		if (!empty($data['Contragent_id']) && $data['Contragent_id'] > 0) {
			...
			if (!empty($data['query'])) {
				$filter .= " and (c.Contragent_Name iLIKE '%'||:query||'%' or  c.Contragent_Code::varchar iLIKE :query||'%')";
		...
		$query = "
			Select
				...
				c.ContragentType_id as \"ContragentType_id\",
				rtrim(ct.ContragentType_Name) as \"ContragentType_Name\",
				c.Lpu_id as \"Lpu_id\",
				c.Org_id as \"Org_id\",
				ot.OrgType_SysNick as \"OrgType_SysNick\",
				o.Server_id as \"OrgServer_id\",
				c.Org_pid as \"Org_pid\",
				c.OrgFarmacy_id as \"OrgFarmacy_id\",
				c.LpuSection_id as \"LpuSection_id\",
				c.Contragent_Code as \"Contragent_Code\",
				RTrim(c.Contragent_Name) as \"Contragent_Name\",
				to_char(ced.BegDate, 'DD.MM.YYYY') as \"BegDate\",

				to_char(ced.EndDate, 'DD.MM.YYYY') as \"EndDate\"
			...
			from
				...
				left join v_Org o  on o.Org_id = c.Org_id

				left join v_OrgType ot  on ot.OrgType_id = o.OrgType_id

				left join OrgFarmacy ofr  on ofr.OrgFarmacy_id = c.OrgFarmacy_id

				left join v_LpuSection ls  on ls.LpuSection_id = c.LpuSection_id

				LEFT JOIN LATERAL (

					select
						 COALESCE(ContragentExpDates.BegDate, c.Contragent_insDT) as BegDate,

						 ContragentExpDates.EndDate
					from
						GetContragentExpDates(c.Contragent_id, c.ContragentType_id) ContragentExpDates
				) ced ON true
			where
				{$filter}
			order by
	...
	function loadContragentKrymTList($data) {
        ...
        if (!empty($org_id)) { //фильтр по организации пользователя
            ...
            $query = "
                ...
                from
                    ...
                    left join v_Org o  on o.Org_id = c.Org_id  -- текущая организация

                    left join v_Org p_o  on p_o.Org_id = o.Org_pid -- родитель организации

                    left join v_Org pp_o  on pp_o.Org_id = p_o.Org_pid -- дед организации
                    ...
                    LEFT JOIN LATERAL (
                        ...
                        from
                            ...
                            left join v_ContragentType i_ct  on i_ct.ContragentType_id = i_c.ContragentType_id
                ...
                if (empty($org_data['P_Org_id'])) {
                    $org_filter = " and c.Org_id in (
                        ...
                        from
                            ...
                            left join ContragentType i_ct  on ct.ContragentType_id = i_c.ContragentType_id
		...
		if (!empty($data['Contragent_id']) && $data['Contragent_id'] > 0) {
			...
			if (!empty($data['query'])) {
				$filter .= " and (c.Contragent_Name iLIKE '%'||:query||'%' or  c.Contragent_Code iLIKE :query||'%')";
	...
	function loadDocumentUcStrOidCombo($filter) {
        ...
        if (!empty($filter['PrepSeries_IsDefect']) && $filter['PrepSeries_IsDefect'] > 0) {
            $ost_where[] = "COALESCE(ps.PrepSeries_IsDefect, 1) = :PrepSeries_IsDefect";

        }
        if (!empty($filter['DrugShipment_setDT_max'])) {
            $where[] = "cast(ds.DrugShipment_setDT as date) <= CAST(:DrugShipment_setDT_max as date)";
        }
                    ...
                    $join[] = "LEFT JOIN LATERAL (
                        ...
                        select
                            (case
                                ...
                                when COALESCE(isdef.YesNo_Code, 0) = 1 then 10
		...
		$q = "
		    select
				...
				at.AccountType_Name as \"AccountType_Name\",
				(
				    '№ '||ds.DrugShipment_Name||
				    COALESCE(', '||ps.PrepSeries_Ser, '')||

					COALESCE(', '||to_char(ps.PrepSeries_GodnDate, 'DD.MM.YYYY'), '')||


		            (
		                case
		                    when COALESCE(isdef.YesNo_Code, 0) = 1 then ', забракованная серия'
					...
					COALESCE(', '||cast(dn.DrugNds_Code as varchar(1)), '')||

					COALESCE(', '||cast(cast(ost.cnt as double precision) as varchar(20))||' '||gu_b.GoodsUnit_Nick, '')||

					(case
					    when :ShowEdOst = 1 then COALESCE(' / '||cast(cast(gu_ost.cnt as double precision) as varchar(20))||' '||gu.GoodsUnit_Nick, '')
				...
				dus.DrugLabResult_Name as \"DrugLabResult_Name\",
				dn.DrugNds_Code as \"DrugNds_Code\",
				cast(ost.cnt as double precision) as \"DocumentUcStr_OstCount\",
				...
				gu_b.GoodsUnit_id as \"GoodsUnit_bid\",
				COALESCE(gu_b.GoodsUnit_Nick, '') as \"GoodsUnit_bNick\",

				gu.GoodsUnit_id as \"GoodsUnit_id\",
				COALESCE(gu.GoodsUnit_Nick, '') as \"GoodsUnit_Nick\",

				cast(gu_ost.cnt as double precision) as \"GoodsUnit_OstCount\"
			from
				v_DocumentUcStr dus 
				...
				left join v_DocumentUcStr o_dus  on o_dus.DocumentUcStr_id = dus.DocumentUcStr_oid

				LEFT JOIN LATERAL (
					Select * from v_DocumentUc o_du  where o_du.DocumentUc_id = COALESCE(o_dus.DocumentUc_id, dus.DocumentUc_id) -- если ссылки на приходную партию нет, значит текущая партия и есть приходная
                    limit 1
				) o_du ON true
				left join v_AccountType at  on at.AccountType_id = ds.AccountType_id
				...
				left join v_GoodsUnit gu_b  on gu_b.GoodsUnit_id = COALESCE(dus.GoodsUnit_bid, :DefaultGoodsUnit_id)
				...
				LEFT JOIN LATERAL (
					...
					select (
						case
							when COALESCE(isnds.YesNo_Code, 0) = 1 then dus.DocumentUcStr_PriceR

							else dus.DocumentUcStr_PriceR*(1+(COALESCE(dn.DrugNds_Code, 0)/100.0))
					...
					where
						(
							...
							(
								...
								(ddt.DrugDocumentType_Code in (3, 6, 32, 35) and dor.Org_id = COALESCE(con_t.Org_id, lpu_t.Org_id)) or
				...
				LEFT JOIN LATERAL ( -- для ускорения рассчета, рассчет остатка в ед. списания сделан через пропорцию
				...
				LEFT JOIN LATERAL (
					select
						COALESCE(o_du.DrugFinance_id, ost.DrugFinance_id) as DrugFinance_id,
						COALESCE(o_du.WhsDocumentCostItemType_id, ost.WhsDocumentCostItemType_id) as WhsDocumentCostItemType_id
				) fin_val ON true
				left join v_DrugFinance df on df.DrugFinance_id = fin_val.DrugFinance_id
				left join v_WhsDocumentCostItemType wdcit on wdcit.WhsDocumentCostItemType_id = fin_val.WhsDocumentCostItemType_id
				{$join_sql}
			where
				{$where_sql}
			{$order_sql};
	...
	function generateDocumentUcNum($data) {
		...
		$query = "
			select
				COALESCE(:DrugDocumentType_Code, (select DrugDocumentType_Code from v_DrugDocumentType  where DrugDocumentType_id = :DrugDocumentType_id)) as \"DrugDocumentType_Code\",


				COALESCE((select Contragent_Code from v_Contragent  where Contragent_id = :Contragent_id), 0) as \"Contragent_Code\";
		...
		if (!isset($data['disable_limits'])) {
			...
			$where_array[] = "du.DocumentUc_setDate >= CAST(date_part('year', dbo.tzGetDate())::text || '-01-01' as date)";
			$where_array[] = "du.DocumentUc_setDate < CAST((date_part('year', dbo.tzGetDate()) + 1)::text || '-01-01' as date)";
		}
		...
		$query = "
			select
				COALESCE(max(cast(num.num as decimal(10))), 0) as \"max_num\"
			...
			from
				...
				LEFT JOIN LATERAL (
					...
					select
						substring(du.DocumentUc_Num, 1, position('/' in  du.DocumentUc_Num)-1) as num
				) num ON true
			where
				{$where}
				du.DocumentUc_Num iLIKE '%/%-%' and

				length(num.num) <= 10 and
				isnumeric(num.num) = 1;
		";
	...
	function setDrugShipmentSupply($data) {
		$q = "
			...
			from
				...
				left join v_WhsDocumentSupply wds  on wds.WhsDocumentUc_id = du.WhsDocumentUc_id
				...
				LEFT JOIN LATERAL (
					...
					from
						...
						left join v_DrugShipment dsh  on dsh.DrugShipment_id = dsl.DrugShipment_id
				...
				from
					...
					LEFT JOIN LATERAL (
						...
						where
                            ...
                            COALESCE(i_dor.GoodsUnit_id, :DefaultGoodsUnit_id::bigint) = COALESCE(dus.GoodsUnit_bid, :DefaultGoodsUnit_id) and
						...
						select 
							(
								case
									...
									then
										COALESCE(COALESCE(dus2.DocumentUcStr_PriceR, dus2.DocumentUcStr_Price), 0)

									else
										cast(COALESCE(COALESCE(dus2.DocumentUcStr_PriceR, dus2.DocumentUcStr_Price), 0)*(1+(COALESCE(dn.DrugNds_Code, 0)/100.0)) as decimal(12,2))

								end
							) as DocumentUcStr_Price
						from
							v_DocumentUcStr dus2 

							left join v_YesNo isnds  on isnds.YesNo_id = dus2.DocumentUcStr_IsNDS

							left join v_DrugNds dn  on dn.DrugNds_id = dus2.DrugNds_id
				...
				from
					...
					LEFT JOIN LATERAL (
						...
						from
							...
							left join v_DrugShipment dss  on dss.DrugShipment_id = dsl.DrugShipment_id
				...
				from
					...
					LEFT JOIN LATERAL (
						...
						from
							...
							left join v_Contragent c  on c.Contragent_id = i_dor.Contragent_id
							...
							LEFT JOIN LATERAL (
								...
								from
									...
									left join v_DocumentUc du2  on du2.DocumentUc_id = dus2.DocumentUc_id
						...
						where
							...
							i_dor.DrugShipment_id = ds2.DrugShipment_id and
							i_dor.PrepSeries_id = dus.PrepSeries_id and
							coalesce(i_dor.Org_id,c.Org_id,l.Org_id,0) = COALESCE(c_sid.Org_id,0) and
...
dus.DocumentUcStr_Count as \"DocumentUcStr_Count\",
						...
						LEFT JOIN LATERAL (
                            ...
                            where
                                ...
                                i_dor.DrugShipment_id = dsl.DrugShipment_id and
                                COALESCE(a.Lpu_id, 0) = COALESCE(farm.Lpu_id, 0) and
                    ...
                    if ( $drug['DrugOstatRegistry_Kolvo'] > 0 && $drug['DrugOstatRegistry_Kolvo'] >= $drug['DocumentUcStr_Count']) {
                        ...
                        $query = "
                            ...
                            from xp_DrugOstatRegistry_count(
                                ...
                                InnerTransaction_Disabled := 1::boolean,
	...
	public function updateDrugOstatRegistryForDocVozrUk($data) {
		...
		if (is_object($result)) {
			...
			foreach($drug_array as $drug) {
				...
				$query = "
					select Error_Code as \"Error_Code\", Error_Message as \"Error_Msg\"
					акщь xp_DrugOstatRegistry_count(
	...
	protected function _getDocumentUcData( $data ) {
		...
		$query = "
			...
			from
				...
				left join v_DocumentUc p_du  on p_du.DocumentUc_id = du.DocumentUc_pid
	...
	protected function _getDrugOstatRegistryData($data) {
		...
		$query = "
			...
			where
				...
				COALESCE(dor.GoodsUnit_id, :DefaultGoodsUnit_id::bigint) = COALESCE(:GoodsUnit_id::bigint, :DefaultGoodsUnit_id::bigint);
	...
	protected function _commitStorageDrugMove($data) {
		...
		$query = "
			...
			from dbo.p_StorageDrugMove_ins(
				...
				GoodsUnit_id := COALESCE(:GoodsUnit_id::bigint, :DefaultGoodsUnit_id::bigint),
				pmUser_id := :pmUser_id);
		";
	...
	*/
	protected function _updateDrugStorageZone($data) {
		...
		$query = "
			select
				DrugStorageZone_id as \"DrugStorageZone_id\",
				DrugStorageZone_Count as \"DrugStorageZone_Count\"
			from
				v_DrugStorageZone 
			...
			where
				...
				COALESCE(PrepSeries_id,0) = COALESCE(:PrepSeries_id,0::bigint) and

				COALESCE(DrugShipment_id,0) = COALESCE(:DrugShipment_id,0::bigint) and

                COALESCE(GoodsUnit_id, :DefaultGoodsUnit_id) = COALESCE(:GoodsUnit_id::bigint, :DefaultGoodsUnit_id::bigint)

				/*and COALESCE(DrugStorageZone_Price,0) = COALESCE(:DrugStorageZone_Price,0)*/
			limit 1
                    ...
                    if (count($ostat_array) == 0) {
                        ...
                        $query = "
                            ...
                            from
                                ...
                                inner join v_WhsDocumentSupplySpecDrug wdssd  on wdssd.WhsDocumentSupplySpec_id = wdss.WhsDocumentSupplySpec_id
	...
	function updateDrugOstatRegistryForDocOprih($data) {
		...
		$query = "
			...
			from
				...
				inner join v_DrugShipmentLink dsl  on dsl.DocumentUcStr_id = dus.DocumentUcStr_oid

				inner join v_DrugOstatRegistry dor  on dor.DrugShipment_id = dsl.DrugShipment_id
        ...
        if (count($drug_array) == 0) {
            $query = "
                select
                    c_t.Contragent_id as \"Contragent_id\",
                    c_t.Org_id as \"Org_id\",
                    dsl.DrugShipment_id as \"DrugShipment_id\",
                    dus.Drug_id as \"Drug_id\",
                    dus.PrepSeries_id as \"PrepSeries_id\",
                    (select Okei_id from v_Okei where Okei_Code = 778::text limit 1) as \"Okei_id\", --dor.Okei_id,
                    dus.DocumentUcStr_Count as \"DrugOstatRegistry_Kolvo\",
                ...
                from
                    ...
                    left join v_DrugOstatRegistry dor  on dor.DrugShipment_id = dsl.DrugShipment_id
                    ...
                    LEFT JOIN LATERAL (
                        ...
                        select (
                            case
                                ...
                                else cast(dus.DocumentUcStr_Price*(1+(COALESCE(dn.DrugNds_Code, 0)/100.0)) as decimal(16,4))

                            end
                        ) as price_nds
                    ) price ON true
                where
                    dus.DocumentUcStr_oid is null 
                    and
                    dus.DocumentUc_id = :DocumentUc_id;
            ";
	...
	function updateDrugOstatRegistryForDocNVP($data) {
		...
		$query = "
			select
				...
				ds.DrugShipment_id as \"DrugShipment_id\",
				ds.WhsDocumentSupply_id as \"WhsDocumentSupply_id\",
				ds.AccountType_id as \"AccountType_id\",
				ds.DrugShipment_pid as \"DrugShipment_pid\",
				d.Drug_Name as \"Drug_Name\",
				ps.PrepSeries_Ser as \"PrepSeries_Ser\",
				dus.StorageZone_id as \"StorageZone_id\",
				dus.DocumentUcStr_id as \"DocumentUcStr_id\",
		...
		$shl_query = "
			select DrugShipmentLink_id as \"DrugShipmentLink_id\", Error_Code as \"Error_Code\", Error_Message as \"Error_Msg\"
        ...
        if ($this->haveReserve(array('DocumentUc_id' => $data['DocumentUc_id']))) {
            ...
            foreach ($drug_arr as $drug) {
				...
				$query = "
					select
		                reserved_dor.Contragent_id as \"Contragent_id\",
						reserved_dor.Org_id as \"Org_id\",
						reserved_dor.Drug_id as \"Drug_id\",
						reserved_dor.PrepSeries_id as \"PrepSeries_id\",
						reserved_dor.Okei_id as \"Okei_id\",
						dorl.DrugOstatRegistryLink_Count as \"DrugOstatRegistry_Kolvo\",
						reserved_dor.DrugOstatRegistry_Sum as \"DrugOstatRegistry_Sum\",
						reserved_dor.DrugOstatRegistry_Cost as \"DrugOstatRegistry_Cost\",
						reserved_dor.GoodsUnit_id as \"GoodsUnit_id\",
						dus.DocumentUcStr_id as \"DocumentUcStr_id\"
		            from
		                v_DocumentUcStr dus 

		                left join v_DrugOstatRegistryLink dorl  on

		                    dorl.DrugOstatRegistryLink_TableName = 'DocumentUcStr' and
		                    dorl.DrugOstatRegistryLink_TableID = dus.DocumentUcStr_id
		                left join v_DrugOstatRegistry reserved_dor  on reserved_dor.DrugOstatRegistry_id = dorl.DrugOstatRegistry_id
			...
			foreach ($drug_arr as $drug) {
				...
				$query = "
					...
					where
						...
						dor.DrugOstatRegistry_Kolvo > 0 and
						(:DocumentUcStr_Price is null or dor.DrugOstatRegistry_Cost = :DocumentUcStr_Price) and
						COALESCE(dor.GoodsUnit_id, :DefaultGoodsUnit_id::bigint) = COALESCE(:GoodsUnit_id::bigint, :DefaultGoodsUnit_id::bigint) and
	...
	function updateDrugOstatRegistryForDocVozNakR($data) {
		...
		$query = "
			select
				...
				dus.StorageZone_id as \"StorageZone_id\",
				COALESCE(dus.DocumentUcStr_Price,0) as \"DocumentUcStr_Price_Original\",
	...
	function updateDrugOstatRegistryForDocVozNakP($data) {
		...
		$query = "
			select
				...
				Storage_sid as \"Storage_sid\"
			from
				DocumentUc 

			where
				DocumentUc_id = (select DocumentUc_pid from DocumentUc  where DocumentUc_id = (select DocumentUc_pid from DocumentUc  where DocumentUc_id = :DocumentUc_id));
		";
		...
		$query = "
			select
				...
				ds.WhsDocumentSupply_id as \"WhsDocumentSupply_id\",
				dus.StorageZone_id as \"StorageZone_id\",
				coalesce(dus.DocumentUcStr_Price_Original, 0) as \"DocumentUcStr_Price_Original\",
				dus.DocumentUcStr_id as \"DocumentUcStr_id\",
				dus.GoodsUnit_bid as \"GoodsUnit_bid\"
			from
				v_DocumentUcStr dus 
				...
				LEFT JOIN LATERAL (
					...
					where
						...
						COALESCE(i_p_dus.PrepSeries_id, 0) = COALESCE(dus.PrepSeries_id, 0)
                    limit 1
    ...
    function updateDrugOstatRegistryForDocVozOtd($data) {
        ...
        $sh_query = "
			...
			select @Res as DrugShipment_id, @Error_Code as Error_Code, @Error_Message as Error_Msg;
		";
        ...
        $shl_query = "
			...
			exec p_DrugShipmentLink_ins
				...
				@Error_Code = @ErrCode output,
				@Error_Message = @ErrMessage output;
			select @Res as DrugShipmentLink_id, @Error_Code as Error_Code, @Error_Message as Error_Msg;
		";*/
	...
	function createDokNakByDocumentUc($data) {
		...
		$query = "
			select
				...
				du.Contragent_tid as \"Contragent_tid\",
				tcon.Org_id as \"Org_tid\",
				dul.DocumentUcLink_id as \"DocumentUcLink_id\",
				du.DocumentUc_didDate as \"DocumentUc_didDate\"
			from
				v_DocumentUc du 
        ...
        if (count($error) == 0) {
            $query = "
                select
                    dsv.DocumentStrValues_id as \"DocumentStrValues_id\"
                from
                    v_DocumentStrValues dsv 
		...
		if (count($error) == 0) {
			foreach($drug_data as $drug) {
				...
				if (!empty($response['Error_Msg'])) {
                    ...
                    $query = "
                       select
                            pmd.pmMediaData_id as \"pmMediaData_id\"
                        from
                            v_pmMediaData pmd 

                        where
                            pmd.pmMediaData_ObjectName = 'DocumentUcStr' 
                            and pmd.pmMediaData_ObjectID = :pmMediaData_ObjectID;
                    ";
	...
	function createDocVozNakPByDocumentUc($data) {
		...
		//поиск склада
		$query = "
			select Storage_sid as \"Storage_id\" 
			from DocumentUc  where DocumentUc_id = (select DocumentUc_pid from DocumentUc  where DocumentUc_id = (select DocumentUc_pid from DocumentUc  where DocumentUc_id = :DocumentUc_id))
		";
	...
	function saveDrugListToWhsDocumentSupply($data) {
		...
		$query = "
			select
				COALESCE(max(wdss.WhsDocumentSupplySpec_PosCode), '0') as \"pos\"
			...
			where
				...
				wdss.WhsDocumentSupplySpec_PosCode not iLIKE '%.%' and

				wdss.WhsDocumentSupplySpec_PosCode not iLIKE '%,%' and

				wdss.WhsDocumentSupplySpec_PosCode not iLIKE '%e%' and

				isnumeric(wdss.WhsDocumentSupplySpec_PosCode) = 1 and
				length(wdss.WhsDocumentSupplySpec_PosCode) <= 18;
		...
		$query = "
			select
				...
				COALESCE(wdss.WhsDocumentSupply_id, wds.WhsDocumentSupply_id) as \"WhsDocumentSupply_id\",

				wdss.WhsDocumentSupplySpec_PosCode as \"WhsDocumentSupplySpec_PosCode\",
				COALESCE(wdss.Drug_id, dus.Drug_id) as \"Drug_id\",

				COALESCE(wdss.DrugComplexMnn_id, d.DrugComplexMnn_id) as \"DrugComplexMnn_id\",

				COALESCE(wdss.FIRMNAMES_id, f.NAMEID) as \"FIRMNAMES_id\",

				wdss.WhsDocumentSupplySpec_KolvoForm as \"WhsDocumentSupplySpec_KolvoForm\",
				wdss.DRUGPACK_id as \"DRUGPACK_id\",
				COALESCE(wdss.Okei_id, (select Okei_id from v_Okei  where Okei_Code::integer = 778)) as \"Okei_id\",

				cast((COALESCE(wdss.WhsDocumentSupplySpec_KolvoUnit, 0)+dus.DocumentUcStr_Count) as decimal(10,4)) as \"WhsDocumentSupplySpec_KolvoUnit\",

				wdss.WhsDocumentSupplySpec_Count as \"WhsDocumentSupplySpec_Count\",
				...
				COALESCE(wdss.WhsDocumentSupplySpec_NDS, dn.DrugNds_Code) as \"WhsDocumentSupplySpec_NDS\",

				case when wdss.WhsDocumentSupplySpec_PriceNDS is null
					then cast((COALESCE(wdss.WhsDocumentSupplySpec_KolvoUnit, 0)+dus.DocumentUcStr_Count)*price.price_nds as decimal(30,4))

					else cast((COALESCE(wdss.WhsDocumentSupplySpec_KolvoUnit, 0)+dus.DocumentUcStr_Count)*wdss.WhsDocumentSupplySpec_PriceNDS as decimal(12,4))
				...
				wdss.WhsDocumentSupplySpec_ShelfLifePersent as \"WhsDocumentSupplySpec_ShelfLifePersent\"
			from
				v_DocumentUc du 
				...
				LEFT JOIN LATERAL (
					...
					select (
						case
							when COALESCE(isnds.YesNo_Code, 0) = 0 then dus.DocumentUcStr_Price
				...
				LEFT JOIN LATERAL (
					...
					where
						...
						(
							(i_wdss.WhsDocumentSupplySpec_Price = dus.DocumentUcStr_Price and COALESCE(isnds.YesNo_Code, 0) = 0) or

							(i_wdss.WhsDocumentSupplySpec_PriceNDS = dus.DocumentUcStr_Price and COALESCE(isnds.YesNo_Code, 0) = 1)
		...
		foreach($spec_data as $spec) {
            ...
            $spec['WhsDocumentSupplySpec_PosCode'] = (string) $spec['WhsDocumentSupplySpec_PosCode'];
			...
			$query = "
				...
				from dbo.p_WhsDocumentSupplySpec_{$action}(
					WhsDocumentSupply_id := :WhsDocumentSupply_id,
		...
		if (count($spec_data) > 0) {
			...
			$query = "
				select
					sum(COALESCE(wdss.WhsDocumentSupplySpec_SumNDS, 0)) as \"supply_sum\"
							...
							if ($result[0]['new_code'] > 0) {
								...
								if ($object == 'Drug') {
									...
									if (is_object($r)) {
										...
										$p[$code_tbl . '_id'] = (isset($p[$code_tbl . '_id']) && !empty($p[$code_tbl . '_id'])) ? $p[$code_tbl . '_id'] : null;

										//добавляем запись в таблицу
										$q = "
											WITH cte AS (
												select PrepClass_id from rls.v_PrepClass  where PrepClass_Code = 2
											)
											select {$code_tbl}_id as \"code_id\", Error_Code as \"Error_Code\", Error_Message as \"Error_Msg\"
											from rls.p_{$code_tbl}_ins(
												...
												PrepClass_id :=  CASE WHEN (SELECT PrepClass_id FROM cte) IS NULL THEN :PrepClass_id ELSE (SELECT PrepClass_id FROM cte) END,
												Region_id := null,
												pmUser_id := :pmUser_id);
										";
										$r = $this->db->query($q, $p);
	...
	function checkInventExists($data) {
		...
		$query = "
			...
			where
				wdst.WhsDocumentStatusType_Code = 2::text and
    ...
    function checkDrugStorageZoneCount($data) {
        ...
        $query = "
            select
                p.StorageZone_id as \"StorageZone_id\",
                d.Drug_Name as \"Drug_Name\",
                (
                    case
                ...
                p.Doc_Kolvo as \"Doc_Kolvo\",
                p.AvailableDOR_Kolvo as \"AvailableDOR_Kolvo\",
                p.AvailableStorageZone_Kolvo as \"AvailableStorageZone_Kolvo\",
                (case
            ...
            from (
                    select
                        ...
                        doc.StorageZone_id,
                        doc.Doc_Kolvo,
                        (COALESCE(doc.Reserve_Kolvo, 0) + COALESCE(dor_kolvo.Kolvo, 0)) as AvailableDOR_Kolvo,

                        (
                            case
                                when doc.StorageZone_id is not null then COALESCE(sz_kolvo.Kolvo, 0) -- остаток на конкретном месте хранения

                                else COALESCE(doc.Reserve_Kolvo, 0) + COALESCE(dor_kolvo.Kolvo, 0) - COALESCE(sz_kolvo.Kolvo, 0) -- остаток на 'Без места хранения'
                    ...
                    from (
                            ...
                            from
                                ...
                                LEFT JOIN LATERAL ( -- резерв под документ учета
                        ...
                        LEFT JOIN LATERAL ( -- содержимое мест хранения (конкретного, если есть иденификатор места хранения, или всех с определенным набором характеристик)
                        ...
                        LEFT JOIN LATERAL ( -- свободные остатки в регистре
        ...
        $query = "
			select 
               ...
               wdu.WhsDocumentUc_id as \"WhsDocumentUc_id\",
                ...
                wdu.WhsDocumentType_id as \"WhsDocumentType_id\",
            ...
            from
                -- from
                dbo.v_WhsDocumentUc wdu 

                inner join dbo.v_WhsDocumentSpecificity wds  on wds.WhsDocumentUc_id = wdu.WhsDocumentUc_id
                ...
                left join dbo.v_WhsDocumentStatusType wdst  on wdst.WhsDocumentStatusType_id = COALESCE(wdu.WhsDocumentStatusType_id, 1)
    ...
    function loadWhsDocumentUcOrderList($data) {
        ...
        $query = "
            ...
            from
                ...
                inner join dbo.v_WhsDocumentUcOrder wduo  on wduo.WhsDocumentUc_id = wdu.WhsDocumentUc_id

                left join dbo.v_WhsDocumentType wdt  on wdt.WhsDocumentType_id = wdu.WhsDocumentType_id

                left join dbo.v_DrugFinance df  on df.DrugFinance_id = wduo.DrugFinance_id

                left join dbo.v_WhsDocumentCostItemType wdcit  on wdcit.WhsDocumentCostItemType_id = wduo.WhsDocumentCostItemType_id
    ...
    function loadDrugPackageBarCodeList($data) {
        ...
        $query = "
			select
                dpbc.DrugPackageBarCode_id as \"DrugPackageBarCode_id\",
                dpbc.DocumentUcStr_id as \"DocumentUcStr_id\",
                dpbc.DrugPackageBarCodeType_id as \"DrugPackageBarCodeType_id\",
                dpbc.DrugPackageBarCode_BarCode as \"DrugPackageBarCode_BarCode\",
                dpbc.DrugPackageBarCode_GTIN as \"DrugPackageBarCode_GTIN\",
                dpbc.DrugPackageBarCode_SeriesNum as \"DrugPackageBarCode_SeriesNum\",
    ...
    function moveToReserve($data) {
        ...
        $query = "
            ...
            from
                v_DrugOstatRegistry available_dor 
                ...
                LEFT JOIN LATERAL (
                    ...
                    where
                        ...
                        COALESCE(i_dor.Contragent_id, 0) = COALESCE(available_dor.Contragent_id, 0) and

                        COALESCE(i_dor.Org_id, 0) = COALESCE(available_dor.Org_id, 0) and

                        COALESCE(i_dor.DrugFinance_id, 0) = COALESCE(available_dor.DrugFinance_id, 0) and

                        COALESCE(i_dor.DrugShipment_id, 0) = COALESCE(available_dor.DrugShipment_id, 0) and

                        COALESCE(i_dor.Drug_id, 0) = COALESCE(available_dor.Drug_id, 0) and

                        COALESCE(i_dor.WhsDocumentCostItemType_id, 0) = COALESCE(available_dor.WhsDocumentCostItemType_id, 0) and

                        COALESCE(i_dor.PrepSeries_id, 0) = COALESCE(available_dor.PrepSeries_id, 0) and

                        COALESCE(i_dor.Okei_id, 0) = COALESCE(available_dor.Okei_id, 0) and

                        COALESCE(i_dor.Storage_id, 0) = COALESCE(available_dor.Storage_id, 0) and

                        COALESCE(i_dor.DrugOstatRegistry_Cost, 0) = COALESCE(available_dor.DrugOstatRegistry_Cost, 0) and

                        COALESCE(i_dor.Drug_did, 0) = COALESCE(available_dor.Drug_did, 0) and

                        COALESCE(i_dor.GoodsUnit_id, :DefaultGoodsUnit_id::bigint) = COALESCE(available_dor.GoodsUnit_id, :DefaultGoodsUnit_id::bigint)
                    limit 1

                ) reserved_dor ON true
            where
        ...
        if (count($error) == 0) {
            if (!empty($dor_data['Reserved_DrugOstatRegistry_id'])) { //редактируем найденную строку резерва
                ...
                if (!empty($response['Error_Msg'])) {
                    ...
                    $query = "
                        select
                            dorl.DrugOstatRegistryLink_id as \"DrugOstatRegistryLink_id\",
                            dorl.DrugOstatRegistryLink_Count as \"DrugOstatRegistryLink_Count\"
                        from
                            v_DrugOstatRegistryLink dorl 
        ...
        if (count($error) == 0) { //сохраняем информацию о резервировании остатков для строки документа учета
			...
			$query = "
				select
					dus.DocumentUcStr_Count as \"DocumentUcStr_Count\",
					COALESCE(dorl.cnt, 0) as \"DrugOstatRegistryLink_Count\"
		...
		elseif ($prescrVaccination) {  // (refs #182481)
			$result['Reserved_DrugOstatRegistry_id'] = !empty($dor_data['Reserved_DrugOstatRegistry_id']) ?  
																$dor_data['Reserved_DrugOstatRegistry_id'] 
																: $dor_id;
		}
        return $result;
    }
    ...
    function removeReserve($data) {
		...
		if ($delete_ostat) {
			$query = "
				select
					COALESCE(sum(dus.DocumentUcStr_Count), 0) as \"DocumentUcStr_Count\",

					COALESCE(sum(dorl.cnt), 0) as \"DrugOstatRegistryLink_Count\"
				...
				from
					...
					LEFT JOIN LATERAL (
						...
						where
							...
							COALESCE(i_dor.Contragent_id, 0) = COALESCE(reserved_dor.Contragent_id, 0) and

							COALESCE(i_dor.Org_id, 0) = COALESCE(reserved_dor.Org_id, 0) and

							COALESCE(i_dor.DrugFinance_id, 0) = COALESCE(reserved_dor.DrugFinance_id, 0) and

							COALESCE(i_dor.DrugShipment_id, 0) = COALESCE(reserved_dor.DrugShipment_id, 0) and

							COALESCE(i_dor.Drug_id, 0) = COALESCE(reserved_dor.Drug_id, 0) and

							COALESCE(i_dor.WhsDocumentCostItemType_id, 0) = COALESCE(reserved_dor.WhsDocumentCostItemType_id, 0) and

							COALESCE(i_dor.PrepSeries_id, 0) = COALESCE(reserved_dor.PrepSeries_id, 0) and

							COALESCE(i_dor.Okei_id, 0) = COALESCE(reserved_dor.Okei_id, 0) and

							COALESCE(i_dor.Storage_id, 0) = COALESCE(reserved_dor.Storage_id, 0) and

							COALESCE(i_dor.DrugOstatRegistry_Cost, 0) = COALESCE(reserved_dor.DrugOstatRegistry_Cost, 0) and

							COALESCE(i_dor.Drug_did, 0) = COALESCE(reserved_dor.Drug_did, 0) and

							COALESCE(i_dor.GoodsUnit_id, :DefaultGoodsUnit_id::bigint) = COALESCE(reserved_dor.GoodsUnit_id, :DefaultGoodsUnit_id::bigint)
                         limit 1

					) available_dor ON true
				where
					dorl.DrugOstatRegistryLink_id is not null and
					(
						(
            ...
            from
                ...
                LEFT JOIN LATERAL (
		            ...
		            from
		                ...
		                inner join v_DrugOstatRegistryLink dorl  on dorl.DrugOstatRegistry_id = dor.DrugOstatRegistry_id
    ...
    function getDocumentUcStrListByDrugOstatRegistry($data) {
        ...
        if (is_array($data['DrugOstatRegistryJSON'])) {
			$insert_sql = "
				WITH cte AS (
				SELECT * FROM (
				values (null::bigint, null::numeric(18,2))
			";
            foreach($data['DrugOstatRegistryJSON'] as $dor_data) {
                $insert_sql .= ",({$dor_data->DrugOstatRegistry_id}, {$dor_data->DrugOstatRegistry_Kolvo})
";
            }
			$insert_sql .= ") t (DrugOstatRegistry_id, DrugOstatRegistry_Kolvo)
				)";
        }
        ...
        $query = "
			...
			from
				...
				left join v_SubAccountType sat on sat.SubAccountType_id = dor.SubAccountType_id
				left join v_GoodsUnit gu on gu.GoodsUnit_id = COALESCE(dor.GoodsUnit_id, :DefaultGoodsUnit_id::bigint)
                                                ...
                                                if (!empty($header_data['Contragent_tid'])) {
                                                    $query = "
                                                        ...
                                                        from
                                                            ...
                                                            left join v_StorageStructLevel ssl  on ssl.Org_id = c.Org_id
                                ...
                                if ($doc_id > 0) {
                                    $query = "
                                        ...
                                        from
                                            ...
                                            left join v_Contragent c_s  on c_s.Contragent_id = du.Contragent_id
                                    ...
                                    if (!empty($data['WhsDocumentUc_id']) && (!empty($save_data['Drug_Ean']) || !empty($save_data['DrugPrepFasCode_Code']))) {
                                        $query = "
                                            select
                                                ...
                                                (
                                                    case
                                                        when dpfc.DrugPrepFasCode_Code = :DrugPrepFasCode_Code 
                                                        then 1
                                                        else 0
                                                    end
                                                ) as \"Code_Check\",
                                                (
                                                    case
                                                        when cast(p.PriceNds as numeric(16,4)) = cast(:PriceNds as numeric(16,4)) 
                                                        then 1
                                                        else 0
                                                    end
                                                ) as \"Price_Check\"
                                            from
                                                ...
                                                left join rls.v_DrugPrepFasCode dpfc  on dpfc.DrugPrepFas_id = d.DrugPrepFas_id

                                                left join v_DrugNds dn  on dn.DrugNds_Code = p.WhsDocumentSupplySpec_NDS
    ...
    function loadStorageZoneCombo($data) {
        ...
        if(!empty($data['StorageZone_id'])){
            ...
            if(!empty($data['Storage_id'])){
                ...
                $join[] = "left join v_StorageStructLevel ssl  on ssl.Storage_id = sz.Storage_id";

                $join[] = "left join v_Contragent c  on c.Org_id = ssl.Org_id";
        ...
        $query = "
            select 
                ...
                sut.StorageUnitType_Name as \"StorageUnitType_Name\",
                (
				    COALESCE(sz.StorageZone_Address, '')||

				    COALESCE(' / '||sut.StorageUnitType_Name, '')
    ...
    function loadStorageZoneByDrugIdCombo($data) {
    	...
    	if(!empty($data['Drug_id'])){
			...
			if(!empty($data['Storage_id'])){
	            ...
	            $fjoin .= "
	                left join v_StorageStructLevel ssl  on ssl.Storage_id = s.Storage_id

	                left join v_Contragent c  on c.Org_id = ssl.Org_id

	            ";
	            $filter .= " and c.Contragent_id = :Contragent_id ";
	            $fjoin2 .= "
	                left join v_StorageStructLevel ssl2  on ssl2.Storage_id = s2.Storage_id

	                left join v_Contragent c2  on c2.Org_id = ssl2.Org_id
			...
			$query = "
				select
					cast((COALESCE(drug_reg.DrugReg_Count,0) - COALESCE(drug_sz.Drug_Count,0)) as decimal) as \"DrugCountWsz\",

					gu_b.GoodsUnit_Name as \"GoodsUnit_Name\",
					gu_b.GoodsUnit_Nick as \"GoodsUnit_Nick\"
				from
					v_DrugOstatRegistry dor 

					inner join v_Storage s  on s.Storage_id = dor.Storage_id

					left join v_GoodsUnit gu_b  on gu_b.GoodsUnit_id = COALESCE(dor.GoodsUnit_id, :DefaultGoodsUnit_id::bigint)
					...
					LEFT JOIN LATERAL (
						...
						from
							...
							inner join v_Storage s2  on s2.Storage_id = dor2.Storage_id
        ...
        if(!empty($data['isPKU'])){
            $where .= " and COALESCE(sz.StorageZone_IsPKU, 1) = 2";
        ...
        if ($data['isEdOstEnabled'] == 'true') {
            ...
            $inner_join .= "
                LEFT JOIN LATERAL (
                    ...
                    from
                        v_DrugShipmentLink ii_dsl 

                        left join v_DocumentUcStr ii_dus  on ii_dus.DocumentUcStr_id = ii_dsl.DocumentUcStr_id

                        left join v_GoodsUnit ii_gu_b  on ii_gu_b.GoodsUnit_id = COALESCE(ii_dus.GoodsUnit_bid, :DefaultGoodsUnit_id)


                        left join v_GoodsUnit ii_gu  on ii_gu.GoodsUnit_id = ii_dus.GoodsUnit_id
        ...
        $query = "
            select
                sz.StorageZone_id as \"StorageZone_id\",
                COALESCE(dsz.DrugStorageZone_Count,0) as \"DrugStorageZone_Count\",

                (
                    COALESCE(sz.StorageZone_Address, '')||

                    COALESCE(case when COALESCE(sz.StorageZone_IsPKU, 1) = 2 then ' НС и ПВ' else null end, '')

                ) as \"StorageZone_Address\",
                COALESCE(dsz.StorageUnitType_Name, '') as \"StorageUnitType_Name\",
                ...
                (
                    ...
                    COALESCE(' / '||dsz.StorageUnitType_Name, '')||

                    ".($data['isCountEnabled'] == "true" ? "' / ' || COALESCE(cast(dsz.DrugStorageZone_Count as varchar(20)), '0')||" : "")."
            ...
            from
            	...
            	LEFT JOIN LATERAL (
                    ...
                    from
                        ...
                        left join v_StorageUnitType i_sut  on i_sut.StorageUnitType_id = sz.StorageUnitType_id
                ...
                left join v_GoodsUnit gu  on gu.GoodsUnit_id = COALESCE(dsz.GoodsUnit_id, :DefaultGoodsUnit_id)
            	{$join}
            where
                {$where}
        ";
    ...
    function loadLpuBuildingCombo($data) {
        ...
        if (!empty($data['LpuBuilding_id'])) {
            ...
            if (!empty($data['query'])) {
                $where[] = 'lb.LpuBuilding_Name iLIKE :query';
        ...
        /*if (!empty($data['Contragent_sid'])) {
            $query = "
                ...
                from
                    ...
                    left loin v_OrgType ot  on ot.OrgType_id = o.OrgType_id
    ...
    where Drug_id =:Drug_id
            ...
            from
                ...
                LEFT JOIN LATERAL (
                    ...
                    where
                        i_gpc.GoodsUnit_id = gu.GoodsUnit_id and
                        i_gpc.DrugComplexMnn_id = (SELECT DrugComplexMnn_id FROM cte) and
                        (
                            (SELECT Tradnames_id FROM cte) is null or
                            i_gpc.TRADENAMES_ID is null or
                            i_gpc.TRADENAMES_ID = (SELECT Tradnames_id FROM cte)
                        ) and
                        (
                            i_gpc.Org_id is null or
                            COALESCE(i_gpc.Org_id, 0) = COALESCE(CAST(:UserOrg_id as bigint), 0)
				...
				$with[] = "
					ls_list as (
						...
						from (
							...
							from
								storage_tree i_st
								left join v_StorageStructLevel i_ssl  on i_ssl.Storage_id = i_st.Storage_id

								left join v_LpuSection i_ls  on i_ls.LpuBuilding_id = i_ssl.LpuBuilding_id
							...
							from
								...
								left join v_LpuSection i_ls  on i_ls.LpuUnit_id = i_ssl.LpuUnit_id
					...
					if ($data['UserOrg_Type'] == 'lpu' && empty($data['Org_id']) && !empty($data['MedService_id'])) { //тип организации пользователя - МО
						...
						$with[] = "
							ssl_list_2 as (								
								...
								from
									storage_tree
									left join v_StorageStructLevel i_ssl  on i_ssl.Storage_id = storage_tree.Storage_id
							...
							$with[] = "
								ssl_list as (
									...
									from
										...
										left join v_StorageStructLevel i_ms_ssl  on i_ms_ssl.MedService_id = i_ssl.MedService_id
						...
						if (!empty($data['MedService_Storage_id'])) {
							$with[] = "ssl_ms_list as (
								...
								from
									...
									left join v_Storage i_s  on i_s.Storage_pid = i_ssl.Storage_id 
	...
	function loadMolMerchCombo($data) {
        ...
        if (!empty($data['Mol_id'])) {
            ...
            if (!empty($data['query'])) {
                $where[] = "mp.Person_Fio iLIKE :query";
    ...
    function saveDocumentUcLink($data) {
        ...
        $query = "
            select 
                DocumentUcLink_id as \"DocumentUcLink_id\"
            from
                v_DocumentUcLink 
    ...
    function getDrugOstatRegistryLink($data) {
    	$query = "
            select 
                ...
                DrugOstatRegistryLink_Count as \"DrugOstatRegistryLink_Count\"
            from
                v_DrugOstatRegistryLink 
    ...
    function getDrugOstatRegistryById($data) {
    	$query = "
            select 
				...
				DrugOstatRegistry_insDT as \"DrugOstatRegistry_insDT\",
				DrugOstatRegistry_updDT as \"DrugOstatRegistry_updDT\",
				Storage_id as \"Storage_id\",
				DrugOstatRegistry_Cost as \"DrugOstatRegistry_Cost\",
				Drug_did as \"Drug_did\",
				DrugOstatRegistry_OstatDate as \"DrugOstatRegistry_OstatDate\",
				DrugOstatRegistry_ImportDate as \"DrugOstatRegistry_ImportDate\",
		...
		if($docUcStr['DrugDocumentStatus_id'] == 2){
			...
			$query = "
	            select 
	                ...
	                dsz.DrugStorageZone_Price as \"DrugStorageZone_Price\",
		...
		if($docUc[0]['DrugDocumentStatus_id'] == 2){
			...
			foreach ($docUcStrs as $strData) {
				...
				$query = "
		            ...
		            where
		            	dus.Drug_id = :Drug_id
		            	and COALESCE(dus.GoodsUnit_bid, :DefaultGoodsUnit_id) = COALESCE(:GoodsUnit_bid, :DefaultGoodsUnit_id)
				...
				$query = "
		            ...
		            where
		            	...
		            	and dsz.DrugShipment_id = :DrugShipment_id
		                and dsz.Drug_id = :Drug_id
		                and COALESCE(dsz.GoodsUnit_id, :DefaultGoodsUnit_id) = COALESCE(:GoodsUnit_id::bigint, :DefaultGoodsUnit_id::bigint)
		            limit 1
    ...
    function getFirstUncompletedStorageWork($doc_id) {
        ...
        if ($doc_id > 0) {
            $query = "
                select 
                    dusw.DocumentUcStorageWork_id as \"DocumentUcStorageWork_id\",
                    dusw.WorkPlace_cid as \"WorkPlace_cid\",
                    dusw.WorkPlace_eid as \"WorkPlace_eid\",
                    dusw.Post_cid as \"Post_cid\",
                    dusw.Person_cid as \"Person_cid\",
                    dusw.Post_eid as \"Post_eid\",
                    dusw.Person_eid as \"Person_eid\",
                    dusw.DocumentUcTypeWork_id as \"DocumentUcTypeWork_id\",
                    dusw.DocumentUcStr_id as \"DocumentUcStr_id\"
                from
                    ...
                    inner join v_DocumentUcStorageWork dusw  on dusw.DocumentUcStr_id = dus.DocumentUcStr_id
        ...
        if (!empty($usw_data['DocumentUcStorageWork_id'])) {
            ...
            $query = "
                ...
                from
                    ...
                    left join v_DocumentUcStorageWork dusw  on dusw.DocumentUcStr_id = dus.DocumentUcStr_id and dusw.DocumentUcStorageWork_endDate is null

                where
                    dus.DocumentUc_id = :DocumentUc_id and
                    COALESCE(dds.DrugDocumentStatus_Code, 0) <> 4 and
	...
	*/
    ...
    function getDocRazPostId($spis_doc_id, $create_if_not_exists = false) {
        ...
        $query = "
            select
                ddt.DrugDocumentType_id as \"DrugDocumentType_id\"
            from
                v_DrugDocumentType ddt 
