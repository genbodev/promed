...
class EvnNotifyRegister_model extends EvnNotifyAbstract_model
{
	...
	function getPersonRegisterType_id()
	{
		if (empty($this->_PersonRegisterType_id)) {
			...
			select PersonRegisterType_id as "PersonRegisterType_id" from dbo.v_PersonRegisterType where PersonRegisterType_SysNick ilike :PersonRegisterType_SysNick
	...
	function loadDataCheckExists($Person_id, $evn_Diag_id = null)
	{
		...
		if (in_array($this->personRegisterTypeSysNick, array(
			...
			$query = "
				...
				from v_{$tableName} EN
					left join v_PersonRegister PR on PR.EvnNotifyBase_id = EN.{$tableName}_id
			...
			if (false == in_array($this->personRegisterTypeSysNick, swPersonRegister::listPersonRegisterTypeOneByPerson())) {
				...
				$params['Diag_id'] = $evn_Diag_id;
				$add_join_en = "
					left join v_Diag Diag on Diag.Diag_id = EN.Diag_id";
				$add_join_pr = "
					left join v_Diag Diag on Diag.Diag_id = PR.Diag_id";
				$add_where .= ' AND Diag.Diag_id in (select DD.Diag_id as Diag_id from v_Diag D left join v_Diag DD on D.Diag_pid = DD.Diag_pid where D.Diag_id = :Diag_id)';
			}
			$query = "
	...
	private function _loadDataForNolos01FR($params)
	{
		return $this->getFirstRowFromQuery("
			select
				case
					when exists(
						...
						where PP.Person_id = PS.Person_id
							and PP.PersonPrivilege_begDate <= CAST(dbo.tzGetDate() as date)
							...
							and not exists (
								...
								where PR.Person_id = PP.Person_id
									...
									and PR.PersonRefuse_Year = extract(year from CAST(dbo.tzGetDate() as date))
                                limit 1
							)
                        limit 1
					) then 2 else 1
				...
				(
					...
					where PP.Person_id = PS.Person_id
						...
						and (PP.PersonPrivilege_endDate is null or PP.PersonPrivilege_endDate >= CAST(dbo.tzGetDate() as date))
						and not exists (
							select PR.PersonRefuse_id as PersonRefuse_id from PersonRefuse PR
							where PR.Person_id = PP.Person_id
								and coalesce(PR.PersonRefuse_IsRefuse,1) = 2
								and PR.PersonRefuse_Year = extract(year from CAST(dbo.tzGetDate() as date))
                            limit 1
				...
				E.EvnNotifyRegister_Comment as \"EvnNotifyRegister_Comment\",
				...
				(
					select PSN.PersonSurName_SurName as PersonSurName_SurName
					from v_PersonSurName PSN
			...
			from v_EvnNotifyRegister E
				...
				left join PersonRegister PR on PR.PersonRegister_id = E.PersonRegister_id
				...
				left join Diag on Diag.Diag_id = coalesce(E.Diag_id,PR.Diag_id)
				...
				LEFT JOIN LATERAL (
					...
					from v_EvnVKExpert
						...
						inner join v_MedPersonal MP on MP.MedPersonal_id = MSMP.MedPersonal_id
							and MP.Lpu_id = MS.Lpu_id
					where v_EvnVKExpert.EvnVK_id = EvnVK.EvnVK_id
						and v_EvnVKExpert.ExpertMedStaffType_id = 1
					limit 1
				) predsed ON TRUE
				left join v_MedPersonal MP on MP.MedPersonal_id = E.MedPersonal_id
					and MP.Lpu_id = E.Lpu_id
				...
				left join v_Oktmo Oktmo on Oktmo.KLArea_id = coalesce(PAddress.KLTown_id, PAddress.KLCity_id, PAddress.KLSubRgn_id, PAddress.KLRgn_id)
				left join v_PersonPrivilege vPP on vPP.Person_id = PS.Person_id
	...
	private function _loadDataForNolos02FR($params)
	{
		return $this->getFirstRowFromQuery("
			select
				...
				to_char (Document.document_begdate, 'dd.mm.yyyy') as \"Document_begDate\",
				RTRIM(OD.Org_Name) as \"OrgDep_Name\",
				O.PersonRegisterOutCause_Name as \"PersonRegisterOutCause_Name\",
				MP.MedPersonal_Code as \"MedPersonal_Code\",
				MP.Person_Fio as \"Doctor_Fio\",
				MPPS.Person_Phone as \"Doctor_Phone\",
				predsed.Person_Fio as \"Predsedatel_Fio\",
			...
			left join Org OD on OD.Org_id = OrgDep.Org_id
			left join v_Lpu on v_Lpu.Lpu_id = E.Lpu_id
			left join PersonRegisterOutCause O on O.PersonRegisterOutCause_id = E.PersonRegisterOutCause_id
			left join Diag on Diag.Diag_id = coalesce(E.Diag_id,PR.Diag_id)
