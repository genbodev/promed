 ...
 * @version            20.12.2019
 */
class BSK_Register_User_model extends swPgModel
{
	...
	function getDataForEMK($data)
	{
		...
		$query = "  select 
                        ...
                        substring(to_char(R.BSKRegistry_insDT,'yyyy-mm-dd'),1,10) as \"BSKRegistry_insDT\",
	...
	function getInfoForPacientOnBSKRegistry($data)
	{
		...
		$query = "select 
                        ...
                        case when PR.MorbusType_id in (110,111,112) then to_char(PR.PersonRegister_setDate, 'dd.mm.yyyy') else to_char(BSKRegistry.BSKRegistry_setDate, 'dd.mm.yyyy') end as \"BSKRegistry_setDate\",
						...
						case 
							...
							when MT.MorbusType_id = 89 then 
								(select coalesce(elVal.BSKObservElementValues_data,RD.BSKRegistryData_data) BSKRegistryData_data
                ...
                left join lateral(
					...
					from dbo.BSKRegistry R
					where R.MorbusType_id = MT.MorbusType_id
	...
	function getMOforOKS()
	{
		...
		$query = "
            ...
            as 
            (
            	select AMO.withSTLpu_id as Lpu_id from dbo.AreaofResponsibilityMO AMO where coalesce(AMO.withSTLpu_id, 0) > 0
            	union all
            	select AMO.nwithoutSTLpu_id as Lpu_id from dbo.AreaofResponsibilityMO AMO where coalesce(AMO.nwithoutSTLpu_id, 0) > 0
            	union all
            	select AMO.CHKVLpu_id as Lpu_id from dbo.AreaofResponsibilityMO AMO where coalesce(AMO.CHKVLpu_id, 0) > 0
            )
            select 
		...
		if (!empty($BSKRegistry_id)) {
			...
			return $this->queryResult("  
				select
					...
					case when (BSKRegistryData_data ilike 'Да%') and (BSKObservElement_id between 277 and 299)
						then 1
						else 0
					end as \"checked\",
					case when right(BSKRegistryData_data, 1) != '.' and BSKObservElement_id between 277 and 299 
						then 1 
						else 0 
					end as \"isDoctor\",
					...
					coalesce(cast(rd.BSKRegistryData_AnswerInt as varchar),to_char(rd.BSKRegistryData_AnswerDT, 'yyyy-mm-dd hh24:mi'),rd.BSKRegistryData_AnswerText,rd.BSKRegistryData_data,'') as \"BSKRegistryData_data\",
					rd.BSKUnits_name as \"BSKUnits_name\",
					rd.BSKRegistryData_insDT as \"BSKRegistryData_insDT\",
					rd.BSKRegistryData_updDT as \"BSKRegistryData_updDT\",
					rd.pmUser_insID as \"pmUser_insID\",
					rd.pmUser_updID as \"pmUser_updID\",
					rd.BSKRegistryData_deleted as \"BSKRegistryData_deleted\"
	...
	function getResponsibilityMOZone($data)
	{
		if (is_numeric($data['EvnEKG_rezEKG'])) {
			...
			$query = "
    			...
    			FROM dbo.AreaofResponsibilityMO armo
    				...
    				AND ( armo.AreaofResponsibilityMO_WeekDay IS NULL OR armo.AreaofResponsibilityMO_WeekDay = (DATE_PART('dow' , dbo.tzGetDate()))
    		";
	...
	function getContraindicationsTLT($data)
	{
		...
		$query = "
             ...
             from dbo.BSKObservRecomendation  where BSKObservRecomendationType_id = :BSKObservRecomendationType_id
        ";
	...
	function getListEvents($data)
	{
		...
		switch ($data['MorbusType_id']) {
			case 84 :
				$filterDiag_id = " ltrim(d.Diag_Code) ilike 'i%'";
				$filtersUslugaComplex_Code = " (ltrim(uc.UslugaComplex_Code) = 'A06.10.006' or ltrim(uc.UslugaComplex_Code) ilike 'A16%')";
				$filterEventsTable = "ltrim(BSKE.BSKEvents_Code) = 'A06.10.006' or ltrim(BSKE.BSKEvents_Code) ilike 'A16%' or ltrim(BSKE.BSKEvents_Code) ilike 'i%'";
				break;
			case 19 :
				$filterDiag_id = " ltrim(d.Diag_Code) ilike 'i%'";
		...
		$query = "
            ...
            (		    --------
                        -- Т.к. диагнозы ставятся для каждого посещения (для ТФОМС) - нам нужны только 'точки' их появления
                        --------
            			select 
                                null as \"Events_id\",   
                        		Events_Code as \"Events_Code\",
                        		Events_Name as \"Events_Name\",
                                min(RTrim(coalesce(to_char(cast(Events_setDate as timestamp),'dd.mm.yyyy'),''))) as \"Events_setDate\",
            			...
            			from (
                				...
                				Select
                						...
                						0 as Events_Edit   
                				from EvnPL EPL
                				inner join Evn E on EPL.Evn_id = E.Evn_id  and coalesce(E.Evn_deleted,1) = 1
                				inner join dbo.Diag D on D.Diag_id = EPL.Diag_id		          
                				where   {$filterDiag_id} and E.Person_id = :Person_id     
                        ) as T
                        ...
                        SELECT
                            ...
                            Events_Code as \"Events_Code\",
                            Events_Name as \"Events_Name\",
                            RTrim(coalesce(to_char(cast(Events_setDate as timestamp),'dd.mm.yyyy'),'')) as \"Events_setDate\",
                            Events_Type as \"Events_Type\",
                            Events_Description as \"Events_Description\",
                            Events_Edit as \"Events_Edit\"
                        FROM
            			...
            			(
            				...
            				left join EvnUslugaCommon EUC on EUC.Evn_id = us.EvnUsluga_id
            				left join AttributeSignValue ASV on ASV.AttributeSignValue_TablePKey = EUC.Evn_id
	...
	function getDrugs($data)
	{
		...
		$query = "
            ...
            to_char(RD.BSKRegistryData_insDT,'dd.mm.yyyy') as \"BSKRegistryData_insDT\"
             from dbo.BSKRegistryData RD 
            left join dbo.BSKRegistry R on RD.BSKRegistry_id = R.BSKRegistry_id
            left join dbo.BSKObservElement E) on RD.BSKObservElement_id = E.BSKObservElement_id
            left join dbo.BSKObservElementGroup EG on E.BSKObservElementGroup_id = EG.BSKObservElementGroup_id
	...
	function getComboUnits($data)
	{
		...
		if (empty($BSKObservUnits_ids)) {
			$query = "select BSKObservUnits_id as \"BSKObservUnits_id\", BSKObservUnits_name as \"BSKObservUnits_name\" from dbo.BSKObservUnits";
		} else {
			$query = "select BSKObservUnits_id as \"BSKObservUnits_id\", BSKObservUnits_name as \"BSKObservUnits_name\" from dbo.BSKObservUnits where BSKObservUnits_id in (" . implode(',', $BSKObservUnits_ids) . ")";
		}
	...
	function getRecomendationByDate($data)
	{
		...
		$query = "
            select 
            	error_code as \"Error_Code\", 
            	error_message as \"Error_Msg\"
            from dbo.p_getRecomendationByDate (
                    ...
                    if ( strlen(trim($value)) != 0 || $registryData['MorbusType_id'] == 19) {
                        $result = $this->getFirstRowFromQuery("
							select
								...
								Error_Code as \"Error_Code\",
								Error_Message as \"Error_Msg\"
							from dbo.p_BSKRegistryData_insNEW
							(
	...
	function checkRegisterDate($data)
	{
		...
		$query = " 
			select 
				...
				BSKRegistry_setDate as \"BSKRegistry_setDate\",
				MorbusType_id as \"MorbusType_id\",
				Person_id as \"Person_id\",
				BSKRegistry_insDT as \"BSKRegistry_insDT\",
				BSKRegistry_updDT as \"BSKRegistry_updDT\",
				pmUser_insID as \"pmUser_insID\",
				pmUser_updID as \"pmUser_updID\",
				BSKRegistry_deleted as \"BSKRegistry_deleted\",
				BSKRegistry_isBrowsed as \"BSKRegistry_isBrowsed\",
				CmpCallCard_id as \"CmpCallCard_id\"
            from dbo.BSKRegistry where MorbusType_id = :MorbusType_id
            and Person_id = :Person_id and BSKRegistry_setDate BETWEEN CAST(:day_start as timestamp) AND CAST(:day_end as timestamp)
        ";
		$result = $this->db->query($query, $params);
		//echo getDebugSql($query, $params);
	...
	function preSaveRegistryData($data)
	{
		...
		$query = "select 
                ...
                from dbo.BSKObservElementValues V
                left join dbo.BSKObservElement E on E.BSKObservElement_id = V.BSKObservElement_id 
	...
	function getAnketsData($data)
	{
		...
		$query = "
            ...
            from dbo.BSKObservElementValues V
            inner join dbo.BSKObservElement E on E.BSKObservElement_id = V.BSKObservElement_id 
                       and E.BSKObservElement_deleted = 1
                       and E.BSKObservElement_stage = 1
            inner join dbo.BSKObservElementGroup G on G.BSKObservElementGroup_id = E.BSKObservElementGroup_id 
                       and 1=1
            inner join dbo.BSKObservElementLink L on L.BSKObservElement_id = E.BSKObservElement_id
	...
	function updateRegistry($data, $registryData)
	{
		...
		$q = 'SELECT BSKRegistryData_id as "BSKRegistryData_id", BSKObservElement_id as "BSKObservElement_id" FROM dbo.BSKRegistryData WHERE BSKRegistry_id = :BSKRegistry_id';
	...
	function getLastVizitRegistry($data)
	{
		...
		if ($data['MorbusType_id'] == 19) {
			if (empty($data['BSKRegistry_id'])) {
				$filter = "
                    r.Person_id = :Person_id
                and
                    r.MorbusType_id = :MorbusType_id
                ";
			} else {
				$filter = "
			...
			$query = "
					...
					MorbusType_id as \"MorbusType_id\"
					MorbusType_Code as \"MorbusType_Code\",
					MorbusType_Name as \"MorbusType_Name\",
					MorbusType_SysNick as \"MorbusType_SysNick\",
					BSKObject_id as \"BSKObject_id\",
					BSKObservElementGroup_id as \"BSKObservElementGroup_id\",
					BSKObservElementGroup_name as \"BSKObservElementGroup_name\",
					numA as \"numA\",
					...
					BSKObservElement_insDT as \"BSKObservElement_insDT\",
					BSKObservElement_updDT as \"BSKObservElement_updDT\",
					BSKObservElement_deleted as \"BSKObservElement_deleted\",
					BSKObservElement_Anketa  as \"BSKObservElement_Anketa\",
					pmUser_insID as \"pmUser_insID\",
					pmUser_updID as \"pmUser_updID\",
					BSKRegistry_id as \"BSKRegistry_id\",
					BSKRegistry_setDateFormat as \"BSKRegistry_setDateFormat\",
					BSKRegistry_setDate as \"BSKRegistry_setDate\",
					BSKRegistry_riskGroup as \"BSKRegistry_riskGroup\",
					Person_id as \"Person_id\",
					...
					from (
                        select 
                            ...
                            to_char(BSKRegistry.BSKRegistry_setDate, 'yyyy-mm-dd hh24:mi:ss.mmm') as BSKRegistry_setDate, 
                            BSKRegistry.BSKRegistry_riskGroup, 
                        ...
                        from dbo.BSKObject o
                        inner join dbo.MorbusType mt on mt.MorbusType_id = o.MorbusType_id and mt.MorbusType_id = 19
                        inner join dbo.BSKObservElementGroup oeg on oeg.BSKObject_id = o.BSKObject_id
                        inner join dbo.BSKObservElement oe on oe.BSKObservElementGroup_id = oeg.BSKObservElementGroup_id
                            and coalesce(oe.BSKObservElement_deleted,1) <> 2
                        left join lateral (
                            select
                                r.BSKRegistry_id,
                                r.BSKRegistry_setDate,
                            ...
                            from (
                                select
                                    ...
                                    r.BSKRegistry_riskGroup,
                                    r.Person_id,
                                    coalesce(r.pmUser_insID,1) as pmUser_insID,
                                    r.pmUser_updID,
                                    usr.Lpu_id,
                                    r.BSKRegistry_isBrowsed
                                from
                            ...
                            inner join dbo.BSKRegistryData rd on r.BSKRegistry_id = rd.BSKRegistry_id and rd.BSKObservElement_id = oe.BSKObservElement_id
                        ) BSKRegistry on true
					) as T
					order by num, coalesce (numA, BSKObservElement_id)
				";

		} else if ($data['BSKRegistry_id'] == null) {
			$query = "
                     select 
                        RD.BSKRegistryData_id as \"BSKRegistryData_id\",
                        RD.BSKRegistry_id as \"BSKRegistry_id\",
                        ...
                        R.BSKRegistry_setDate as \"BSKRegistry_setDate\",
                        R.MorbusType_id as \"MorbusType_id\",
                        R.Person_id as \"Person_id\",
                        R.BSKRegistry_insDT as \"BSKRegistry_insDT\",
                        R.BSKRegistry_updDT as \"BSKRegistry_updDT\",
                        R.pmUser_insID as \"pmUser_insID\",
                        R.pmUser_updID as \"pmUser_updID\",
                        R.BSKRegistry_deleted as \"BSKRegistry_deleted\",
                        R.BSKRegistry_isBrowsed as \"BSKRegistry_isBrowsed\",
                        R.CmpCallCard_id as \"CmpCallCard_id\",
                        (select Lpu_id from pmUserCache where pmUser_id = R.pmUser_insID limit 1) as \"Lpu_id\",
                        R.BSKRegistry_riskGroup as \"BSKRegistry_riskGroup\",
                        (select BSKObservElementFormat_id from dbo.BSKObservElement E  where E.BSKObservElement_id = RD.BSKObservElement_id) as \"BSKObservElementFormat_id\"
                     from dbo.BSKRegistryData RD 
                     left join dbo.BSKRegistry R  on R.BSKRegistry_id = RD.BSKRegistry_id
                     where R.Person_id = :Person_id and R.MorbusType_id = :MorbusType_id 
                     and R.BSKRegistry_id = (select BSKRegistry_id from dbo.BSKRegistry R where R.Person_id = :Person_id and R.MorbusType_id = :MorbusType_id  order by R.BSKRegistry_setDate DESC LIMIT 1)
                 ";
		} else {
			$query = "
				select 
					...
					R.BSKRegistry_isBrowsed as \"BSKRegistry_isBrowsed\",
					R.CmpCallCard_id as \"CmpCallCard_id\",
					(select Lpu_id from pmUser where pmUser_id = R.pmUser_insID limit 1) as \"Lpu_id\",
					(select BSKObservElementFormat_id from dbo.BSKObservElement E where E.BSKObservElement_id = RD.BSKObservElement_id limit 1) as \"BSKObservElementFormat_id\"
				from dbo.BSKRegistryData RD 
					left join dbo.BSKRegistry R  on R.BSKRegistry_id = RD.BSKRegistry_id
	...
	function addRegistryData($data)
	{
		...
		$query = "
                ...
                left join dbo.BSKObject BO  on BO.BSKObject_id = L.BSKObject_id
                -- конец финта
	...
	function getListObjectsCurrentUser($data)
	{
		...
		$query = "  select 
                        ...
                        MT.MorbusType_Name as \"MorbusType_Name\",
                        MT.MorbusType_id as \"MorbusType_id\",
                        to_char(R.PersonRegister_disDate, 'dd.mm.yyyy') as \"PersonRegister_disDate\",
                        to_char(ps.Person_deadDT , 'dd.mm.yyyy') as \"Person_deadDT\"
                    from v_PersonState PS
		    inner join dbo.PersonRegister R on R.Person_id = PS.Person_id
                    left join dbo.MorbusType MT on MT.MorbusType_id = R.MorbusType_id
                    left join dbo.BSKObject O on O.MorbusType_id = R.MorbusType_id
	...
	function loadPersonData($data)
	{
		...
		$query = "
			SELECT 
                ...
                from dbo.BSKRegistry  
                where MorbusType_id = 84 and Person_id =  ps.Person_id order by BSKRegistry_setDate DESC limit 1) as \"BSKRegistry_riskGroup\",
                
                (select BSKRegistryData_data from dbo.BSKRegistryData  where BSKRegistry_id = 
                (
				...
				coalesce(RTRIM(CAST(PS.PersonEvn_id as varchar)), '') as \"PersonEvn_id\",
				...
				coalesce(RTRIM(CAST(Sex.Sex_Code as varchar)), '') as \"Sex_Code\",
				coalesce(RTRIM(CAST(Sex.Sex_id as varchar)), '') as \"Sex_id\",
				...
				coalesce(RTRIM(DOr.Org_Name), '') as \"OrgDep_Name\",
				...
				coalesce(RTRIM(CAST(PJ.Org_id as varchar)), '') as \"JobOrg_id\",
				...
				coalesce(convert(varchar(10), ps.Person_deadDT, 104), '') as Person_deadDT,
				coalesce(convert(varchar(10), ps.Person_closeDT, 104), '') as Person_closeDT,
				ps.Person_IsDead,
			...
			FROM {$object} PS 
				...
				left join Org DOr  on DOr.Org_id = OrgDep.Org_id
				...
				and vper.PersonEvn_id = PS.PersonEvn_id limit 1
				) vper on true
				left join lateral
				(SELECT
					PP.Person_id
	...
	function getOksId($data)
	{
		...
		$query = "
            ...
            FROM
                v_Person P 
                left join lateral (select BSKRegistry_id from dbo.BSKRegistryData  where BSKObservElement_id = 398 and BSKRegistryData_data = :EvnPS_NumCard) BRD on true
            WHERE
	...
	*/
	function getDiagFromEvnPS($data){
		...
		$query = "
			...
			from v_EvnPS EvnPS
				inner join v_Diag Diag on Diag.Diag_id = EvnPS.Diag_id
			...
			from v_EvnPL epl
				...
				inner join lateral (
					...
					from v_EvnDiagPLSop DiagPS
						...
						and (dPS.Diag_Code ilike 'I%' or dPS.Diag_Code ilike 'G45%' or dPS.Diag_Code like 'G46%' or dPS.Diag_Code ilike 'J44%' or dPS.Diag_Code ilike 'N18%'
    ...
    function getListPersonCureHistoryDiagKardio($data)
    {
        ...
        $query = "
            ...
            select 
                ...
                Person_id as \"Person_id\",
                Evn_id as \"Evn_id\",
                EvnDiagPS_setDate as \"EvnDiagPS_setDate\",
                Diag_id as \"Diag_id\",
                Diag_FullName as \"Diag_FullName\",
                Lpu_id as \"Lpu_id\",
                Lpu_Nick as \"Lpu_Nick\",
                ...
                Person_Fio as \"Person_Fio\",
                NumCard as \"NumCard\",
                to_char(EvnDiagPS_setDate::timestamp, 'dd.mm.yyyy') as \"Diag_setDate\"
            from (
                select 
    ...
    function checkBSKforScreening($data)
    {
		...
		with ED as (
			...
			from v_EvnVizitPL vpl
			left join v_Diag d on d.Diag_id = vpl.Diag_id 
			where d.Diag_Code ILIKE 'I%'
			and vpl.Person_id = :Person_id
	
			union all
			--движение в КВС
			select 	vpl.Person_id
			from v_EvnSection vpl
			left join v_Diag d on d.Diag_id = vpl.Diag_id 
			where d.Diag_Code ILIKE 'I%'
			and vpl.Person_id = :Person_id
