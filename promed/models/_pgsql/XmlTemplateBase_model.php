 ...
 */
class XmlTemplateBase_model extends swPgModel
{
	/**
	 * @var bool
	 */
	...
	static function defAttributes()
	{
		...
		$arr['uslugaidlist'] = array(
			...
			'select' => "
				(
					SELECT string_agg(CAST(xtl.UslugaComplex_id as varchar), ',')
					FROM XmlTemplateLink xtl
		...
		$arr['xmlschema_data'] = array(
			...
			'select' => 'xs.xmlschema_data as "xmlschema_data"',
			'join' => 'left join XmlSchema xs on xs.XmlSchema_id = {ViewName}.XmlSchema_id',
		);
		return $arr;
	}
	...
	protected function _beforeQuerySavedData($fields, $viewName, $joins, $where, $params)
	{
		$sql = "
			...
			{$joins}
			where {$where}
				and coalesce({$viewName}.{$this->tableName()}_IsDeleted, 1) = 1
		";
	...
	public function loadGrid($data)
	{
		...
		if (!empty($this->_params['templName'])) {
			...
			if ($this->_params['templType']==1) {
				$filter_tpl .= " and xt.XmlTemplate_Caption ilike :templName";
				$filter_cat .= " and xtc.XmlTemplateCat_Name ilike :templName";
			}
			if ($this->_params['templType']==2) {
				//if ($this->_isAllowNewTables) {
				$html_join = 'inner join XmlTemplateHtml xth on xth.XmlTemplateHtml_id = xt.XmlTemplateHtml_id
						and xth.XmlTemplateHtml_HtmlTemplate ilike :templName';
				/*} else {
					$html_join = 'left join XmlTemplateHtml xth on xth.XmlTemplateHtml_id = xt.XmlTemplateHtml_id';
					$filter_tpl .= " and coalesce(xth.XmlTemplateHtml_HtmlTemplate, xt.XmlTemplate_HtmlTemplate) ilike :templName";
				}*/
				$filter_cat .= " and 1=2";
			}
			$isNeedItemPath = true;
			...
			$filter_tpl .= "
			 and (
			    ...
			    or xt.XmlTemplate_id in (
				    select t.XmlTemplate_id from v_UslugaComplex ucs
					inner join  v_UslugaComplex uc11 on ucs.UslugaComplex_2011id = uc11.UslugaComplex_2011id
					inner join  v_XmlTemplate t on t.UslugaComplex_id = uc11.UslugaComplex_id
		...
		if ($isNeedItemPath) {
			$add_joins .= 'left join dbo.v_XmlTemplateCat p0 on xtu.XmlTemplateCat_pid = p0.XmlTemplateCat_id
		...
		/*$settings_select = 'coalesce(xts.XmlTemplateSettings_Settings, xt.XmlTemplate_Settings) as XmlTemplate_Settings';
		if ($this->_isAllowNewTables) {*/
		...
		$query = "
			Select
			-- select
				xtu.Item_Key as \"Item_Key\",
				xtu.XmlTemplate_id as \"XmlTemplate_id\",
				xtu.XmlTemplateType_id as \"XmlTemplateType_id\",
				{$accessType} as \"accessType\",
				EvnClass.EvnClass_Name as \"EvnClass_Name\",
				xtu.Item_Name as \"Item_Name\",
				puc.pmUser_Name as \"pmUser_Name\",
				XTCV.XmlTemplateScope_Name as \"XmlTemplateScope_Name\",
				to_char(xtu.Item_updDate, 'dd.mm.yyyy') as \"Item_updDate\",
				xtu.XmlTemplate_Settings as \"XmlTemplate_Settings\",
				xtu.EvnClass_id as \"EvnClass_id\",
				xtu.XmlType_id as \"XmlType_id\",
				xtu.XmlTemplateScope_id as \"XmlTemplateScope_id\",
				xtu.XmlTemplateScope_eid as \"XmlTemplateScope_eid\",
				xtu.Lpu_id as \"Lpu_id\",
				xtu.LpuSection_id as \"LpuSection_id\",
				xtu.pmUser_insID as \"pmUser_insID\",
				xtu.XmlTemplateCat_id as \"XmlTemplateCat_id\",
			...
			-- from
				(
					Select
						...
						'XmlTemplate_'|| xt.XmlTemplate_id as Item_Key,
						xt.XmlTemplate_id,
						xt.Lpu_id,
						xt.LpuSection_id,
						xt.pmUser_insID,
						...
						{$settings_select},
						xt.EvnClass_id,
						coalesce(xt.XmlType_id,1) as XmlType_id,
					...
					from
						v_XmlTemplate xt
						left join XmlTemplateSettings xts on xts.XmlTemplateSettings_id = xt.XmlTemplateSettings_id
						{$html_join}
						left join lateral(
							select xtl.UslugaComplex_id
							from XmlTemplateLink xtl
					...
					where
						coalesce(xt.XmlTemplate_IsDeleted,1) = 1 {$filter_tpl}
				union all
					Select
						'XmlTemplateCat' as Item_Type,
						'XmlTemplateCat_'|| xtc.XmlTemplateCat_id as Item_Key,
				...
				left join EvnClass on EvnClass.EvnClass_id = xtu.EvnClass_id
				left join XmlTemplateScope XTCV on XTCV.XmlTemplateScope_id = xtu.XmlTemplateScope_id
				left join v_pmUserCache puc on puc.PMUser_id = xtu.pmUser_insID
				left join v_XmlTemplateDefault xtd on xtd.XmlTemplate_id = xtu.XmlTemplate_id and
					xtd.MedStaffFact_id = :MedStaffFact_id and
					coalesce(xtd.XmlType_id,1) = xtu.XmlType_id and
					xtd.pmUser_insID = :pmUser_id
				left join v_XmlTemplateCatDefault xtdcat on xtdcat.MedStaffFact_id = :MedStaffFact_id and
					xtdcat.EvnClass_id = xtu.EvnClass_id and
					coalesce(xtdcat.XmlType_id,1) = xtu.XmlType_id and
					xtdcat.pmUser_insID = :pmUser_id
				left join XmlTemplate t on t.XmlTemplate_id = xtu.XmlTemplate_id
				{$add_joins}
			-- end from
			where
			-- where
        ...
        $query = "
			...
			-- from
				(
					Select
						'XmlTemplate' as Item_Type,
						'XmlTemplate_' || xt.XmlTemplate_id::varchar as Item_Key,
						xt.XmlTemplate_id,
						xt.Lpu_id,
						xt.LpuSection_id,
						xt.pmUser_insID,
					...
					from
						...
						left join lateral (
							select 
							    xtl.UslugaComplex_id
							from
							    XmlTemplateLink xtl
							where
							    xtl.XmlTemplate_id = xt.XmlTemplate_id
							    {$filter_xtuc}
						    limit 1
						) xtuc on true
					where
				...
				union all
					Select
						'XmlTemplateCat' as Item_Type,
						'XmlTemplateCat_' || xtc.XmlTemplateCat_id::varchar as Item_Key,
						null as XmlTemplate_id,
						xtc.Lpu_id,
						xtc.LpuSection_id,
						xtc.pmUser_insID,
	...
	public function loadGridForPrint($data)
	{
		...
		$query = "
			...
			-- from
			v_XmlTemplate xt
			inner join v_XmlType xtype on xtype.XmlType_id = xt.XmlType_id
			left join v_XmlTemplateCat xtc on xtc.XmlTemplateCat_id = xt.XmlTemplateCat_id
			left join v_XmlTemplateEvnClass xtew on xtew.XmlTemplate_id = xt.XmlTemplate_id
			left join v_pmUserCache puc on puc.PMUser_id = xt.pmUser_insID
			-- end from
			where
			-- where
				(1=1) {$filter} 
				and {$visibleFilter}
				and coalesce(xt.XmlTemplate_IsDeleted,1) = 1
			-- end where
			order by
			-- order by
	...
	private function _addXmlTemplateLink($UslugaComplex_id)
	{
		return $this->db->query("
			INSERT INTO dbo.XmlTemplateLink
	...
	private function _clearXmlTemplateLink()
	{
		...
		return $this->db->query("
			DELETE FROM dbo.XmlTemplateLink
	...
	public function select($data)
	{
		...
		$res = $this->getFirstResultFromQuery("
			SELECT XmlTemplateFavorites_id as \"XmlTemplateFavorites_id\" 
			FROM dbo.v_XmlTemplateFavorites 
			WHERE XmlTemplate_id = :XmlTemplate_id 
		...
		if (empty($res)) {
			$sql = "
				select
					XmlTemplateFavorites_id as \"XmlTemplateFavorites_id\",
					Error_Code as \"Error_Code\",
					Error_Message as \"Error_Msg\"
				from dbo.p_XmlTemplateFavorites_ins(
					Server_id := :Server_id,
					XmlTemplateFavorites_id := 0,
					XmlTemplate_id := :XmlTemplate_id,
			...
			$params['XmlTemplateFavorites_id'] = $res;
			$sql = "
				select
					XmlTemplateFavorites_id as \"XmlTemplateFavorites_id\",
					Error_Code as \"Error_Code\",
					Error_Message as \"Error_Msg\"
				from dbo.p_XmlTemplateFavorites_upd(
					Server_id := :Server_id,
					XmlTemplateFavorites_id := :XmlTemplateFavorites_id,
					XmlTemplate_id := :XmlTemplate_id,
					XmlTemplateFavorites_CountLoad := (1 + COALESCE((
						SELECT sum(XmlTemplateFavorites_CountLoad)
						FROM dbo.v_XmlTemplateFavorites
						WHERE XmlTemplate_id = :XmlTemplate_id
						AND pmUser_insID = :pmUser_id
					),0))::int,
		...
		$result = $this->db->query($sql, array(
			...
			'XmlTemplate_id' => $this->id,
			'pmUser_id' => $this->promedUserId,
            'XmlTemplateFavorites_id' => $params['XmlTemplateFavorites_id'] ?? null
		));
	...
	public function loadCombo($data)
	{
		...
		$sql = "
			SELECT
				XmlTemplate_Caption as \"XmlTemplate_Caption\",
				XmlTemplate_id as \"XmlTemplate_id\"
			FROM
				v_XmlTemplate
			WHERE
				{$filter} and coalesce(XmlTemplate_IsDeleted, 1) = 1
			ORDER BY
	...
	public function loadXmlTemplateLinkList($data)
	{
		...
		$sql = "
			...
			FROM
				XmlTemplateLink xtl
				inner join v_UslugaComplex uc on uc.UslugaComplex_id = xtl.UslugaComplex_id
	...
	function checkExistsEvnPrescrPlan($data, $onlyWithPersonAgeGroup = true)
	{
		...
		if (false == $onlyWithPersonAgeGroup) {
			...
			(select tpl.XmlTemplate_id as \"XmlTemplate_id\" from XmlTemplate tpl
			where tpl.LpuSection_id = :LpuSection_id and exists (
				select DD.Diag_id from v_Diag DD where DD.Diag_id = :Diag_id and DD.Diag_pid = tpl.Diag_id limit 1
			) limit 1)';
		}
		return $this->getFirstResultFromQuery( "
			(select tpl.XmlTemplate_id as \"XmlTemplate_id\" from XmlTemplate tpl
			where tpl.LpuSection_id = :LpuSection_id and tpl.Diag_id = :Diag_id and tpl.PersonAgeGroup_id = :PersonAgeGroup_id limit 1)
			union
			(select tpl.XmlTemplate_id as \"XmlTemplate_id\" from XmlTemplate tpl
			where tpl.LpuSection_id = :LpuSection_id and exists (
				select DD.Diag_id from v_Diag DD where DD.Diag_id = :Diag_id and DD.Diag_pid = tpl.Diag_id limit 1
			) and tpl.PersonAgeGroup_id = :PersonAgeGroup_id limit 1)
			{$withoutPersonAgeGroup}
		", array(
			'LpuSection_id' => $this->_params['LpuSection_id'],
			'Diag_id' => $this->Diag_id,
	...
	function getEvnClassListForXmlType($XmlType_id) {
		$XmlType_Code = $this->getFirstResultFromQuery("
			select XmlType_Code as \"XmlType_Code\" from v_XmlType where XmlType_id = :XmlType_id limit 1
	...
	function loadXmlTamplateListForAPI($data) {
		...
		$filters[] = "coalesce(XT.Region_id, :Region_id) = :Region_id";
		...
		if (!empty($data['XmlTemplate_id'])) {
			...
			if (!empty($data['MedPersonal_id'])) {
				$filters[] = "exists(
					select * from v_XmlTemplateDefault XTD
