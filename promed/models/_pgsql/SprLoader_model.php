...
* @version			?
*/
class SprLoader_model extends SwPgModel
{
	/**
	 * Конструктор
	 */
		...
		for ($i = 0; $i < count($table_list); $i++)
		{
			if (in_array($table_list[$i], array('DrugDisp')))
			{
				$query = "
					select
						count(*) as \"cnt\",
						to_char(max(coalesce(SicknessDrug_updDT, SicknessDrug_insDT)), 'DD.MM.YYYY HH24:MI:SS') as \"{$table_list[$i]}_updDT\"
					from SicknessDrug
				";
			}
			else
			if (preg_match('/rls/', $table_list[$i]))
			{
				...
				$query = "
					select
						count(*) as \"cnt\", 
						to_char(max(coalesce({$table_list[$i]}_updDT, {$table_list[$i]}_insDT)), 'DD.MM.YYYY HH24:MI:SS') as \"{$table_list[$i]}_updDT\"
					from
						rls.v_{$table_list[$i]}
				";
			...
			if (in_array($table_list[$i], array('DrugFed', 'DrugReg')))
			{
				$query = "
					select
						count(*) as \"cnt\", 
						to_char(max(coalesce(Drug_updDT, Drug_insDT)), 'DD.MM.YYYY HH24:MI:SS') as \"{$table_list[$i]}_updDT\"
			...
			else
				{
					$check = $this->getFirstResultFromQuery("
						...
						where table_schema = 'dbo' and table_name ilike 'v_{$table_list[$i]}'
					");
					if (!$check) {
						$query = "
						...
						$query = "
							select
								count(*) as \"cnt\", 
								to_char(max(coalesce({$table_list[$i]}_updDT::timestamp, {$table_list[$i]}_insDT::timestamp)), 'DD.MM.YYYY HH24:MI:SS') as \"{$table_list[$i]}_updDT\"
							from v_{$table_list[$i]}
						";
					}
				}
			...
			if ( $table_list[$i] == 'OMSSprTerrAddit' )
				$query = "
					select
						count(*) as \"cnt\", 
						to_char(max(coalesce(OMSSprTerr_updDT, OMSSprTerr_insDT)), 'DD.MM.YYYY HH24:MI:SS') as \"OMSSprTerrAddit_updDT\"
					from v_OMSSprTerr
				";						
			...
			if ( $table_list[$i] == 'LpuSearch' )
				$query = "
					select
						count(*) as \"cnt\", 
						to_char(max(coalesce(Lpu_updDT, Lpu_insDT)), 'DD.MM.YYYY HH24:MI:SS') as \"LpuSearch_updDT\"
					from v_Lpu
				";
			...
			if ( $table_list[$i] == 'SFPrehospDirect' )
				$query = "
					select
						count(*) as \"cnt\",
						to_char(max(coalesce(PrehospDirect_updDT, PrehospDirect_insDT)), 'DD.MM.YYYY HH24:MI:SS') as \"SFPrehospDirect_updDT\"
	...
	function createNewVersionLocalDB($data)
	{
		...
		*/
		...
		//todo: stg.xp_LocalDBVersion_generate_old
		$query .= "
	...
	function getSyncTable($data) {
		...
		$query = "
		Select
			list.LocalDBList_id as \"LocalDBList_id\",
			...
			reg.RegionalLocalDbList_PgSql as \"SyncTable_sql\"
		from stg.LocalDBTables tb
			inner join stg.LocalDBList list on list.LocalDBList_name = LocalDBTables_Name
			left join lateral (
				select rldl.RegionalLocalDbList_PgSql 
				from stg.RegionalLocalDBList rldl 
				where rldl.LocalDBList_id = list.LocalDBList_id
				and rldl.RegionalLocalDbList_PgSql is not null
	...
	function getSyncTables($data) {
		...
		Select 
			distinct 
				...
				/*LocalDBList_Key as \"SyncTable_Key\"*/
		from stg.LocalDBTables tb
			inner join stg.LocalDBVersion ver on ver.LocalDBVersion_id = tb.LocalDBVersion_id 
			inner join stg.LocalDBList list on list.LocalDBList_name = LocalDBTables_Name
