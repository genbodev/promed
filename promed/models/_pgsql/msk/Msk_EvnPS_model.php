...
class Msk_EvnPS_model extends EvnPS_model {
	...
	protected function _printEvnPS($data = [], $response = []) {
			...
			foreach($response_temp as $j => $value) {
				...
				$query = "
					select 
                        ...
                        LS.LpuSection_Name || ' [Реанимация]' as \"LpuSection_Name\",
		...
		$print_data = [
			...
			,'EvnDirection_Num' => returnValidHTMLString(isset($response[0]['EvnDirection_Num'])?$response[0]['EvnDirection_Num']:'')
			,'EvnDirection_setDate' => returnValidHTMLString(isset($response[0]['EvnDirection_setDate'])?$response[0]['EvnDirection_setDate']:'')
		];
