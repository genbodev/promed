...
class Msk_Dlo_EvnRecept_model extends Dlo_EvnRecept_model {
	...
	public function loadReceptList($data) {
		...
		if( !empty($data['begDate']) ) {
			$filterList[] = "ER.EvnRecept_setDT >= CAST(:begDate as date)";
			$queryParams['begDate'] = $data['begDate'];
		}

		if( !empty($data['endDate']) ) {
			$filterList[] = "ER.EvnRecept_setDT <= CAST(:endDate as date)";
		...
		if( !empty($data['Search_BirthDay']) ) {
			$filterList[] = "PS.Person_BirthDay = CAST(:Person_BirthDay as date)";
		...
		$query = "
			select
				...
				,ER.EvnRecept_deleted as \"EvnRecept_deleted\"
			...
			from
				...
				left join dbo.v_MorbusType as mt on mt.MorbusType_id = COALESCE(wdcit.MorbusType_id, 1)
