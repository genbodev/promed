 ...
 */
class MorbusCommon_model extends SwPgModel
{
	...
	static function defAttributes()
	{
		...
		$arr['morbustype_sysnick'] = array(
			...
			'join' => 'inner join v_MorbusType on v_MorbusType.MorbusType_id = {ViewName}.MorbusType_id',
		);
		$arr['person_id'] = array(
			'properties' => array(
				self::PROPERTY_IS_SP_PARAM,
		...
		switch (true) {
			case (isset($data['Diag_id'])):
				...
				$query = "
					...
					FROM dbo.v_Diag
					inner join dbo.v_MorbusDiag md on md.Diag_id = v_Diag.Diag_id
					inner join dbo.v_MorbusType mt on mt.MorbusType_id = md.MorbusType_id
					WHERE v_Diag.Diag_id = :Diag_id
				";
				if (!empty($data['includeCommon'])) {
					$query .= "
					union all
					SELECT MorbusType_id as \"MorbusType_id\", MorbusType_SysNick as \"MorbusType_SysNick\", null as \"RegistryType\" FROM dbo.v_MorbusType
					WHERE MorbusType_SysNick ilike 'common'
					";
				}
				if (!empty($data['includePalliat'])) {
					$query .= "
					union all
					SELECT MorbusType_id as \"MorbusType_id\", MorbusType_SysNick as \"MorbusType_SysNick\", MorbusType_SysNick as \"RegistryType\" FROM dbo.v_MorbusType
					WHERE MorbusType_SysNick ilike 'palliat'
					";
				}
				$query .= "
			...
			case (isset($data['diag_list']) && is_array($data['diag_list'])):
				...
				if ( isset($data['Person_id'])) {
					...
					$before_select = "with TmpDiag as (
						select
						    ...
						    coalesce(DD.Diag_id, D.Diag_id) as Diag_gid
						from 
						    v_Diag D
					...
					$add_select_md = ", Diag.Diag_Code as \"Diag_Code\", ms.Morbus_id as \"Morbus_id\",
					case when exists(select TmpTable.Morbus_id as Morbus_id 
					from TmpTable
					...
					$add_join_md = "
					inner join dbo.v_Diag Diag on Diag.Diag_id = md.Diag_id
					left join lateral (
						select mb.MorbusBase_id as MorbusBase_id
						from dbo.v_MorbusBase mb
					...
					left join lateral (
						select ms.Morbus_id as Morbus_id
						from dbo.v_Morbus ms
						...
						and (
							...
							OR exists(select TmpDiag.Diag_gid as Diag_gid from TmpDiag where TmpDiag.Diag_sid = md.Diag_id and TmpDiag.Diag_sid = ms.Diag_id limit 1)
						)
						limit 1
					) ms on true
					";
					$add_select_mt = ", Diag.Diag_Code as \"Diag_Code\", null as \"Morbus_id\", 0 as \"disableAddEvnNotify\"";
				}
				$query = "
					{$before_select}
					SELECT mt.MorbusType_id as \"MorbusType_id\", mt.MorbusType_SysNick as \"MorbusType_SysNick\", md.Diag_id as \"Diag_id\" {$add_select_md}
					FROM dbo.v_MorbusDiag md
					inner join dbo.v_MorbusType mt on mt.MorbusType_id = md.MorbusType_id
					{$add_join_md}
				...
				$query .= "
					union all
					SELECT mt.MorbusType_id as \"MorbusType_id\", mt.MorbusType_SysNick as \"MorbusType_SysNick\", Diag.Diag_id as \"Diag_id\" {$add_select_mt}
					FROM v_MorbusPalliat MP
					left join dbo.v_Diag Diag on Diag.Diag_id = MP.Diag_id
					inner join dbo.v_MorbusType mt on mt.MorbusType_SysNick ilike 'palliat'
					...
					inner join dbo.v_Morbus M on M.Morbus_id = MG.Morbus_id
					inner join dbo.v_Diag Diag on Diag.Diag_id = MG.Diag_id
					inner join dbo.v_MorbusType mt on mt.MorbusType_SysNick = 'geriatrics'
					WHERE M.Person_id = :Person_id
				";
				if (!empty($data['includeCommon'])) {
					$query .= "
					union all
					SELECT MorbusType_id as \"MorbusType_id\", MorbusType_SysNick as \"MorbusType_SysNick\", Diag.Diag_id as \"Diag_id\"  {$add_select_mt}
					FROM dbo.v_Diag Diag
					inner join dbo.v_MorbusType on MorbusType_SysNick ilike 'common'
					WHERE Diag.Diag_id in ({$diag_list})
					and not exists (select md.Diag_id as Diag_id from dbo.v_MorbusDiag md where md.Diag_id = Diag.Diag_id)
					order by \"MorbusType_id\"
					";
				}
				break;
			default:
				$query = 'SELECT MorbusType_id as "MorbusType_id", MorbusType_SysNick as "MorbusType_SysNick" FROM dbo.v_MorbusType';
				break;
		}
	...
	public function loadMorbusList($data)
	{
		...
		AllMorbusPerson as (
			...
			ms.Diag_id as Diag_id,
			mb.MorbusType_id as MorbusType_id,
			mb.MorbusBase_id as MorbusBase_id,
			ms.Morbus_id as Morbus_id,
			ms.Morbus_disDT as Morbus_disDT
			from dbo.v_MorbusBase mb
			inner join dbo.v_Morbus ms on ms.MorbusBase_id = mb.MorbusBase_id
				{$onlyOpenMorbus}
		...
		$before_select .= ",
			OtherEvn as (
				select
				v_evn.Evn_id as Evn_id,
				AllMorbusPerson.Morbus_id as Morbus_id 
				from v_evn
				left join EvnSection on EvnSection.Evn_id = v_evn.Evn_id
				left join EvnVizitPl on EvnVizitPl.Evn_id = v_evn.Evn_id
				inner join v_MorbusDiag on v_MorbusDiag.Diag_id = COALESCE(EvnSection.Diag_id, EvnVizitPl.Diag_id)
				inner join v_MorbusType on v_MorbusType.MorbusType_id = v_MorbusDiag.MorbusType_id
				inner join AllMorbusPerson on AllMorbusPerson.MorbusType_id = v_MorbusDiag.MorbusType_id
				and (
					v_MorbusType.MorbusType_SysNick in ({$listMorbusTypeOneByPerson})
					OR exists(select TmpDiag.Diag_gid from TmpDiag where TmpDiag.Diag_sid = v_MorbusDiag.Diag_id and TmpDiag.Diag_gid = AllMorbusPerson.Diag_id limit 1)
				)
		...
		if ('onBeforeDeleteEvn' == $data['mode']) {
			...
			when mt.MorbusType_SysNick = 'onko' and exists(
				select EvnNotifyBase_id
				from v_EvnNotifyBase
				inner join v_EvnOnkoNotify EON on v_EvnNotifyBase.Morbus_id = EON.Morbus_id
				where v_EvnNotifyBase.Morbus_id = ms.Morbus_id
		...
		$query = "
			...
			mt.MorbusType_id as \"MorbusType_id\",
			mt.MorbusType_SysNick as \"MorbusType_SysNick\",
			ms.MorbusBase_id as \"MorbusBase_id\",
			ms.Morbus_id as \"Morbus_id\",
			case when exists(
				select AllMorbusPerson.Morbus_id as Morbus_id
				from AllMorbusPerson
			...
			{$add_select}
			FROM AllMorbusPerson ms
			inner join dbo.v_MorbusType mt on mt.MorbusType_id = ms.MorbusType_id
			inner join dbo.v_MorbusDiag md on md.MorbusType_id = ms.MorbusType_id and md.Diag_id in ({$diag_list})
			WHERE
			mt.MorbusType_SysNick in ({$listMorbusTypeOneByPerson})
			OR exists(select TmpDiag.Diag_gid from TmpDiag where TmpDiag.Diag_sid = md.Diag_id and TmpDiag.Diag_gid = ms.Diag_id limit 1)
		";
	...
	public function checkExistsExtended($MorbusType_SysNick, $Person_id, $Diag_id = null, $add_select = '', $add_join = '', $onlyOpen = false)
	{
		...
		if (false == in_array($MorbusType_SysNick, $this->listMorbusTypeOneByPerson)) {
			...
			if ($MorbusType_SysNick == 'onko') {
				...
				$morbus_add_where .= ' AND Morbus.Diag_id in (select DD.Diag_id as Diag_id from v_Diag D left join v_Diag DD on D.Diag_pid = DD.Diag_pid where D.Diag_id = :Diag_id)';
			}
		}
		$query = "
			select
				MorbusBase.MorbusBase_id as \"MorbusBase_id\",
				Morbus.Morbus_id as \"Morbus_id\"
				{$add_select}
			from
				v_MorbusBase MorbusBase
				inner join v_Morbus Morbus on MorbusBase.MorbusBase_id = Morbus.MorbusBase_id {$morbus_add_where}
				{$add_join}
			where
  ...
  public function loadLastEvnData($MorbusType_SysNick, $Evn_pid = null, $Person_id = null, $Diag_id = null, $sopid = null, $EvnDiagPLSop_id = null)
	{
		...
		if (false == in_array($MorbusType_SysNick, $this->listMorbusTypeOneByPerson)) {
			...
			if ($MorbusType_SysNick == 'onko') {
				$group_where = 'AND v_MorbusDiag.Diag_id = (select Diag_id from subQueryOne)';
			} else {
				$group_where = 'AND v_MorbusDiag.Diag_id in (
                            select DD.Diag_id as Diag_id
                            from v_Diag D
                            left join v_Diag DD on D.Diag_pid = DD.Diag_pid
                            where D.Diag_id = (select Diag_id from subQueryOne)
                                                            )';
			}
		}
		...
		$query = "
			with subQueryOne as (
			    select
			    	...
			    	coalesce(ED.Diag_id,EDPS.Diag_id,EPS.Diag_pid,PL.Diag_id,ST.Diag_id,DD.Diag_id) as Diag_id
			    from v_Evn Evn
				left join v_EvnVizitDispDop DD on Evn.Evn_id = DD.EvnVizitDispDop_id
				left join EvnVizitPL PL on Evn.Evn_id = PL.Evn_id
				left join EvnSection ST on Evn.Evn_id = ST.Evn_id
				left join EvnPS EPS on Evn.Evn_id = EPS.Evn_id
				left join v_EvnDiagPLStom EDPS on EDPS.EvnDiagPLStom_id = Evn.Evn_id
				left join v_EvnDiag ED on ED.EvnDiag_id = coalesce(:EvnDiagPLStomSop_id, :EvnDiagPLSop_id)::bigint
				where Evn.Evn_id = :Evn_pid
				and Evn.EvnClass_id in (11,13,14,30,32,19)
			),
			subQueryTwo as (
			    select
                    coalesce((select Person_id from subQueryOne), :Person_id) as Person_id,
                    coalesce(:Diag_id, (select Diag_id from subQueryOne)) as Diag_id
			)
			select
				Evn.Evn_id as \"Evn_id\",
				Evn.Person_id as \"Person_id\",
				(select Diag_id from subQueryTwo) as \"filter_Diag_id\",
			...
			left join EvnSection ST on Evn.Evn_id = ST.Evn_id
			left join EvnPS EPS on Evn.Evn_id = EPS.Evn_id
			left join v_EvnDiagPLStom EDPS on EDPS.EvnDiagPLStom_pid = Evn.Evn_id
			left join v_EvnDiag ED on ED.EvnDiag_id = coalesce(:EvnDiagPLStomSop_id, :EvnDiagPLSop_id)::bigint
			inner join v_MorbusType on v_MorbusType.MorbusType_SysNick = :MorbusType_SysNick
			inner join lateral (
				Select
				    morbusdiag_id as morbusdiag_id,
				    morbustype_id as morbustype_id,
				    diag_id as diag_id,
				    pmuser_insid as pmuser_insid,
				    pmuser_updid as pmuser_updid,
				    morbusdiag_insdt as morbusdiag_insdt,
				    morbusdiag_upddt as morbusdiag_upddt
				from  v_MorbusDiag where v_MorbusDiag.MorbusType_id = v_MorbusType.MorbusType_id
					AND v_MorbusDiag.Diag_id = coalesce(ED.Diag_id,EDPS.Diag_id,EPS.Diag_pid,PL.Diag_id,ST.Diag_id,DD.Diag_id)
					{$group_where}
                limit 1
			) v_MorbusDiag on true
			left join v_Diag D on D.Diag_id = v_MorbusDiag.Diag_id
			where
				Evn.Person_id = (select Person_id from subQueryOne)
				and Evn.EvnClass_id in (11,13,14,30,32,19)
				and exists(select md.Diag_id as Diag_id from v_MorbusDiag md where md.MorbusType_id = v_MorbusType.MorbusType_id and md.Diag_id = (select Diag_id from subQueryTwo) limit 1)
			order by
				Evn.Evn_setDT desc
		    limit 1
		";
        ...
        */
        ...
        $query = "
			with
                subQueryThree as
                (
                    select Evn.Person_id as Person_id
                    from v_Evn Evn
				    where Evn.Evn_id = :Evn_pid
				and Evn.EvnClass_id in (11,13,14,30,32,19)
                ...
                select
                        Evn.Person_id as Person_id,
                        coalesce(EPLDSO.Diag_spid,PL.Diag_spid,ST.Diag_spid,EPLDD.Diag_spid,ED.Diag_id,EDPS.Diag_id,EPS.Diag_pid,PL.Diag_id,ST.Diag_id,DD.Diag_id) as  Diag_id
                from v_Evn Evn
				    left join v_EvnVizitDispDop DD  on Evn.Evn_id = DD.EvnVizitDispDop_id
				    left join v_EvnPLDispDop13 EPLDD on EPLDD.EvnPLDispDop13_id = DD.EvnVizitDispDop_pid
					left join v_EvnPLDispScreenOnko EPLDSO on EPLDSO.EvnPLDispScreenOnko_pid = Evn.Evn_id
			        left join EvnVizitPL PL on Evn.Evn_id = PL.Evn_id
			        left join EvnSection ST on Evn.Evn_id = ST.Evn_id
			        left join EvnPS EPS on Evn.Evn_id = EPS.Evn_id
				    left join v_EvnDiagPLStom EDPS  on EDPS.EvnDiagPLStom_id = Evn.Evn_id
				    left join v_EvnDiag ED  on ED.EvnDiag_id = Cast(Coalesce(:EvnDiagPLStomSop_id,:EvnDiagPLSop_id) as bigint)
				where Evn.Evn_id = :Evn_pid
				and Evn.EvnClass_id in (11,13,14,30,32,19)
                ),
                ...
                (
                    select
                        CASE
                            WHEN (Coalesce(:Evn_pid::bigint, 0) > 0 and :Diag_id is null)
                                THEN (Select Person_id from subQueryTwo)
                            WHEN (Coalesce(:Evn_pid::bigint, 0)>0)
                                THEN (Select Person_id from subQueryThree)
                            ELSE :Person_id
                        END  as Person_id,

                        CASE WHEN (Coalesce(:Evn_pid::bigint, 0) > 0 and :Diag_id is null)
                            THEN (Select Diag_id from subQueryTwo)
                            ELSE :Diag_id
                        END  as Diag_id
                )
                select
				    Evn.Evn_id as \"Evn_id\",
				    Evn.Person_id as \"Person_id\",
				    (select Diag_id from subQueryOne)  as \"filter_Diag_id\",
				    D.Diag_Code as \"Diag_Code\",
				    v_MorbusDiag.Diag_id as \"Diag_id\"
			    from v_Evn Evn
			    left join v_EvnVizitDispDop DD  on Evn.Evn_id = DD.EvnVizitDispDop_id
			...
			where
				...
				and exists(
                        select md.Diag_id
                        from v_MorbusDiag md
                        where md.MorbusType_id = v_MorbusType.MorbusType_id
                            and md.Diag_id = (Select Diag_id from subQueryOne)
                        limit 1)
			order by
				Evn.Evn_setDT desc
            limit 1
	...
	public function loadEvnInfectNotifyList($evn_list)
	{
		...
		$query = "
			select EvnInfectNotify_id as \"EvnInfectNotify_id\", EvnInfectNotify_pid as \"EvnInfectNotify_pid\"
	...
	public function autoCreate($data, $isAllowTransaction = true)
	{
		...
		$data['MorbusBase_id'] = $this->getFirstResultFromQuery("
					SELECT MorbusBase_id as \"MorbusBase_id\"
					FROM v_MorbusBase
					WHERE Person_id = :Person_id
					AND MorbusType_id = :MorbusType_id
					AND MorbusBase_disDT is null
