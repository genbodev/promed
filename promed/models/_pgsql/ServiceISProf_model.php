...
class ServiceISProf_model extends swPgModel
{
	protected $host;
	protected $port;
	protected $user;
		...
		try {
			...
			$resp_child = $this->queryResult("
				with Person as (
					...
					where
						dbo.Age2(ps.Person_BirthDay, CAST(:begDate as date)) < 18
						and not exists (
							select 
								PreventionPersonList_id
							from
								r59.v_PreventionPersonList 
							where
								Person_id = evpl.Person_id
								and DATEDIFF('DAY', PreventionPersonList_insDT, CAST(:begDate as date)) < 315
					...
					from
						v_PersonState ps 
						inner join v_PersonHeight ph  on ph.Person_id = ps.Person_id
						inner join v_pmUserCache pu  on pu.pmUser_id = ph.pmUser_updID
					where
						dbo.Age2(ps.Person_BirthDay, CAST(:begDate as date)) < 18
					...
					from
						v_PersonState ps 
						inner join v_PersonWeight pw  on pw.Person_id = ps.Person_id
						inner join v_pmUserCache pu  on pu.pmUser_id = pw.pmUser_updID
					where
						dbo.Age2(ps.Person_BirthDay, CAST(:begDate as date)) < 18
						and PW.PersonWeight_updDT >= :begDate)
				
				select
					Person.MedPersonal_id as \"MedPersonal_id\",
					Person.KindId as \"KindId\",
					...
					case when d.DocumentType_id IN (3,13) then to_char(d.Document_begDate, 'YYYY-MM-DD') else null end as \"DocDate\",
				...
				from
					Person 
					inner join v_PersonState ps  on ps.Person_id = Person.Person_id
					INNER JOIN LATERAL (
						select 
							mp.Person_SurName,
							mp.Person_FirName,
					...
					left join v_KLArea kla  on kla.KLArea_id = coalesce(ua.KLTown_id, ua.KLCity_id, ua.KLSubRgn_id)
					left join v_KLStreet kls  on kls.KLStreet_id = ua.KLStreet_id
					left join v_PersonSprTerrDop pstd  on pstd.PersonSprTerrDop_id = ua.PersonSprTerrDop_id
					LEFT JOIN LATERAL (
						select 
			...
			if (!empty($messages)) {
				foreach ($messages as $Person_id => $message) {
					...
					$this->queryResult("
						select PreventionPersonList_id as \"PreventionPersonList_id\", Error_Code as \"Error_Code\", Error_Message as \"Error_Msg\"						
						from r59.p_PreventionPersonList_ins(
							PreventionPersonList_id := null,
