<?php
class EvnMediaFiles_model extends SwPgModel {
	/**
	 * Конструктор объекта
	 */
	...
	function getEvnMediaByEvn($data) {
		...
		$query = "
			select EvnMediaData_id as \"EvnMediaData_id\"
			from v_EvnMediaData
			where Evn_id = :Evn_id
		";
	...
	function loadList($data) {
		...
		if(isset($data['EvnXml_id']) && empty($data['Evn_id'])) {
			...
			$filters .= '
				and EMD.Evn_id = (select Evn.Evn_rid from v_EvnXml DOC inner join v_Evn Evn on Evn.Evn_id = DOC.Evn_id where DOC.EvnXml_id = :EvnXml_id)';
		}
		...
		switch ($data['filterType']) {
			case 'image':
				...
				$filters .= "
				and (
					lower(EMD.EvnMediaData_FilePath) like '%.jp%' 
					or lower(EMD.EvnMediaData_FilePath) like '%png'  
					or lower(EMD.EvnMediaData_FilePath) like '%bmp'  
					or lower(EMD.EvnMediaData_FilePath) like '%gif' 
					or lower(EMD.EvnMediaData_FilePath) like '%.tif%'
				)";
				break;
			case 'doc':
				//
	...
	function saveEvnMediaData($data) {
		...
		$query = "
			select
				EvnMediaData_id as \"EvnMediaData_id\",
	...
	function addToQueryEvn($data, $res) {
		...
		$qe = $this->getFirstRowFromQuery("
			select 
				...
				qeu.QueryEvnUser_id as \"QueryEvnUser_id\"
			from QueryEvn qe 
			inner join QueryEvnUser qeu on 
		...
		if (!$msg) {
			$sql = "
				select
					QueryEvnMessage_id as \"QueryEvnMessage_id\",
					Error_Code as \"Error_Code\",
					Error_Message as \"Error_Msg\"
		...
		$sql = "
			select
				QueryEvnMessageFile_id as \"QueryEvnMessageFile_id\",
				Error_Code as \"Error_Code\",
				Error_Message as \"Error_Msg\"
	...
	function linkFilesToEvn($data)
	{
		...
		foreach ($EvnMediaDataIds as $EvnMediaData_id)
		{
			$this->db->query($query, array(
				'EvnMediaData_id' => $EvnMediaData_id,
				'Evn_id' => $data['Evn_id'],
				'pmUser_id' => $session_params['pmuser_id']
			));
			$results[$EvnMediaData_id] = array(
				'EvnMediaData_id' => $EvnMediaData_id,
				'Error_Code' => null,
				'Error_Msg' => null,
			);
	...
	function deleteEvnMediaData($data) {
		...
		$tmp = $this->queryList("
			  select QueryEvnMessageFile_id as \"QueryEvnMessageFile_id\" 
			  from QueryEvnMessageFile where EvnMediaData_id = ?
		", array($data['EvnMediaData_id']));
		foreach($tmp as $qemf) {
			$this->db->query("
				select
					Error_Code as \"Error_Code\",
	...
	function getEvnMediaData($data) {
		$query = "
			select
				EvnMediaData_id as \"EvnMediaData_id\",
				Evn_id as \"EvnMediaData_id\",
				Evn_sid as \"EvnMediaData_id\",
				EvnMediaData_FileName as \"EvnMediaData_FileName\",
				EvnMediaData_FilePath as \"EvnMediaData_FilePath\",
				EvnMediaData_Comment as \"EvnMediaData_Comment\"
			from
				v_EvnMediaData
			where
				EvnMediaData_id = :EvnMediaData_id
	...
	public function getEvnMediaDataByNameAndId($data) {
		...
		$query = "
			...
			WHERE
				EMD.EvnMediaData_id = :EvnMediaData_id
				AND lower(EMD.EvnMediaData_FilePath) like lower(:fileName)
		";
	...
	function loadEvnMediaDataPanel($data) {
		$sql = "
			select
				...
				emd.EvnMediaData_Comment as \"EvnMediaData_Comment\",
				to_char(emd.EvnMediaData_insDT, 'DD.MM.YYYY') as \"EvnMediaData_insDT\"
				-- end select
			from
				-- from
				v_EvnMediaData emd,
