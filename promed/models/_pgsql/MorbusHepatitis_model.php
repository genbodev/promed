...
class MorbusHepatitis_model extends swPgModel
{
	/**
	...
	public function autoCreate($data, $isAllowTransaction = true)
	{
		...
		if (empty($qr[0])) {
            ...
            $queryParams["{$tableName}_id"] = $qr[0]["{$tableName}_id"];
            $query = "
				select :{$tableName}_id as \"{$tableName}_id\", '' as \"Error_Msg\", 0 as \"Error_Code\"
			";
		}
	...
	function getMorbusHepatitisQueueViewData($data)
	{
		...
		$query = '
			select 
				case when MH.Morbus_disDT is null and coalesce(EvnEdit.Evn_IsSigned,1) = 1 then \'edit\' else \'view\' end as "AccessType"
				,MHQ.MorbusHepatitisQueue_id as "MorbusHepatitisQueue_id"
				...
				,MHQ.MorbusHepatitis_id as "MorbusHepatitis_id"
				,:Evn_id as "MorbusHepatitis_pid"
			from
				v_MorbusHepatitisQueue MHQ 
				left join v_HepatitisQueueType HQT on MHQ.HepatitisQueueType_id = HQT.HepatitisQueueType_id
				left join v_YesNo IsCure on coalesce(MHQ.MorbusHepatitisQueue_IsCure,1) = IsCure.YesNo_id
				inner join v_MorbusHepatitis MH on MH.MorbusHepatitis_id = MHQ.MorbusHepatitis_id
				inner join v_MorbusBase MB on MH.MorbusBase_id = MB.MorbusBase_id
	...
	function getMorbusHepatitisPlanViewData($data)
	{
		...
		$query = '
			select 
				case when MH.Morbus_disDT is null and coalesce(EvnEdit.Evn_IsSigned,1) = 1
					then \'edit\'
					else \'view\'
				end as "AccessType"
				,case when exists(select MHP.MorbusHepatitisPlan_id from v_MorbusHepatitisPlan MHP where MHP.MorbusHepatitis_id = MH.MorbusHepatitis_id and coalesce(MHP.MorbusHepatitisPlan_Treatment, 1) = 1 limit 1) then 1 else 0 end as "hasOpenPlan"
				,MHP.MorbusHepatitisPlan_id as "MorbusHepatitisPlan_id"
				,MHP.MorbusHepatitisPlan_Year as "MorbusHepatitisPlan_Year"
				,MHP.MorbusHepatitisPlan_Month as "MorbusHepatitisPlan_Month"
				,MCT.MedicalCareType_Name as "MedicalCareType_Name"
				,L.Lpu_Nick as "Lpu_Nick"
				,Treatment.YesNo_Name as "MorbusHepatitisPlan_Treatment"
				,MHP.MorbusHepatitis_id as "MorbusHepatitis_id"
				,:Evn_id as "MorbusHepatitis_pid"
			from
				v_MorbusHepatitisPlan MHP 
				left join fed.v_MedicalCareType MCT on MHP.MedicalCareType_id = MCT.MedicalCareType_id
				left join v_YesNo Treatment on coalesce(MHP.MorbusHepatitisPlan_Treatment,1) = Treatment.YesNo_id
				left join v_Lpu L on L.Lpu_id = MHP.Lpu_id
	...
	function getMorbusHepatitisVaccinationViewData($data)
	{
		...
		$query = "
			select 
				case when ((MHV.Evn_id is null and MB.Person_id = :Evn_id) or (MHV.Evn_id = :Evn_id and coalesce(EvnEdit.Evn_IsSigned,1) = 1)) and MH.Morbus_disDT is null then 'edit' else 'view' end as \"AccessType\"
				,MHV.MorbusHepatitisVaccination_id as \"MorbusHepatitisVaccination_id\"
				,to_char(MHV.MorbusHepatitisVaccination_setDT, 'dd.mm.yyyy') as \"MorbusHepatitisVaccination_setDate\"
				,Drug.Drug_Name as \"Drug_Name\"
				,MHV.MorbusHepatitis_id as \"MorbusHepatitis_id\"
				,:Evn_id as \"MorbusHepatitis_pid\"
			from
				v_MorbusHepatitisVaccination MHV
				inner join v_MorbusHepatitis MH on MH.MorbusHepatitis_id = MHV.MorbusHepatitis_id
				inner join v_MorbusBase MB on MH.MorbusBase_id = MB.MorbusBase_id
				left join v_Evn EvnEdit on EvnEdit.Evn_id = :Evn_id and MB.Person_id != :Evn_id
				left join rls.v_Drug Drug on MHV.Drug_id = Drug.Drug_id
			where
	...
	function getMorbusHepatitisCureViewData($data)
	{
		...
		$query = "
			select 
				case when ((MHC.Evn_id is null and MB.Person_id = :Evn_id) or (MHC.Evn_id = :Evn_id and coalesce(EvnEdit.Evn_IsSigned,1) = 1)) and MH.Morbus_disDT is null then 'edit' else 'view' end as \"AccessType\"
				,MHC.MorbusHepatitisCure_id as \"MorbusHepatitisCure_id\"
				,to_char(MHC.MorbusHepatitisCure_begDT, 'dd.mm.yyyy') as \"MorbusHepatitisCure_begDate\"
				,to_char(MHC.MorbusHepatitisCure_endDT, 'dd.mm.yyyy') as \"MorbusHepatitisCure_endDate\"
				,Drug.Drug_Name as \"Drug_Name\"
				,HRC.HepatitisResultClass_Name as \"HepatitisResultClass_Name\"
				,HSET.HepatitisSideEffectType_Name as \"HepatitisSideEffectType_Name\"
				,MHC.MorbusHepatitisCureEffMonitoring_id as \"MorbusHepatitisCureEffMonitoring_id\"
				,MHC.MorbusHepatitis_id as \"MorbusHepatitis_id\"
				,:Evn_id as \"MorbusHepatitis_pid\"
			from
				v_MorbusHepatitisCure MHC 
				inner join v_MorbusHepatitis MH on MH.MorbusHepatitis_id = MHC.MorbusHepatitis_id
				inner join v_MorbusBase MB on MH.MorbusBase_id = MB.MorbusBase_id
				left join v_Evn EvnEdit on EvnEdit.Evn_id = :Evn_id and MB.Person_id != :Evn_id
				left join rls.v_Drug Drug on MHC.Drug_id = Drug.Drug_id
				left join v_HepatitisResultClass HRC on MHC.HepatitisResultClass_id = HRC.HepatitisResultClass_id
				left join v_HepatitisSideEffectType HSET on MHC.HepatitisSideEffectType_id = HSET.HepatitisSideEffectType_id
			where
	...
	function getMorbusHepatitisFuncConfirmViewData($data)
	{
		...
		$query = "
			select
				case when ((EvnUsluga.EvnUsluga_pid is null and EvnUsluga.Person_id = :Evn_id) or (EvnUsluga.EvnUsluga_pid = :Evn_id and coalesce(EvnEdit.Evn_IsSigned,1) = 1)) and MH.Morbus_disDT is null then 'edit' else 'view' end as \"AccessType\"
				,EvnUsluga.EvnUsluga_id as \"EvnUsluga_id\"
				,EvnUsluga.EvnClass_SysNick as \"EvnClass_SysNick\"
				,MHC.MorbusHepatitisFuncConfirm_id as \"MorbusHepatitisFuncConfirm_id\"
				,to_char(MHC.MorbusHepatitisFuncConfirm_setDT, 'dd.mm.yyyy') as \"MorbusHepatitisFuncConfirm_setDate\"
				,MHC.MorbusHepatitisFuncConfirm_Result as \"MorbusHepatitisFuncConfirm_Result\"
				,MHC.UslugaComplex_id as \"UslugaComplex_id\"
				,HCT.HepatitisFuncConfirmType_Name as \"HepatitisFuncConfirmType_Name\"
				,MHC.MorbusHepatitis_id as \"MorbusHepatitis_id\"
				,:Evn_id as \"MorbusHepatitis_pid\"
			from
				v_MorbusHepatitisFuncConfirm MHC
				inner join v_EvnUsluga EvnUsluga on MHC.Evn_id = EvnUsluga.EvnUsluga_id
				left join v_HepatitisFuncConfirmType HCT on HCT.HepatitisFuncConfirmType_id = MHC.HepatitisFuncConfirmType_id
				inner join v_MorbusHepatitis MH on MH.MorbusHepatitis_id = MHC.MorbusHepatitis_id
				left join v_Evn EvnEdit on EvnEdit.Evn_id = :Evn_id and EvnUsluga.Person_id != :Evn_id
			where
				MHC.MorbusHepatitis_id = :MorbusHepatitis_id
		";
	...
	function getMorbusHepatitisLabConfirmViewData($data)
	{
		...
		$query = "
			select
				...
				,EvnUsluga.EvnUsluga_id as \"EvnUsluga_id\"
				,EvnUsluga.EvnClass_SysNick as \"EvnClass_SysNick\"
				,MHC.MorbusHepatitisLabConfirm_id as \"MorbusHepatitisLabConfirm_id\"
				,to_char(MHC.MorbusHepatitisLabConfirm_setDT, 'dd.mm.yyyy') as \"MorbusHepatitisLabConfirm_setDate\"
				,MHC.MorbusHepatitisLabConfirm_Result as \"MorbusHepatitisLabConfirm_Result\"
				,MHC.UslugaComplex_id as \"UslugaComplex_id\"
				,HCT.HepatitisLabConfirmType_Name as \"HepatitisLabConfirmType_Name\"
				,MHC.MorbusHepatitis_id as \"MorbusHepatitis_id\"
				,:Evn_id as \"MorbusHepatitis_pid\"
			from
				v_MorbusHepatitisLabConfirm MHC
				inner join v_EvnUsluga EvnUsluga on MHC.Evn_id = EvnUsluga.EvnUsluga_id
				left join v_HepatitisLabConfirmType HCT on HCT.HepatitisLabConfirmType_id = MHC.HepatitisLabConfirmType_id
				inner join v_MorbusHepatitis MH on MH.MorbusHepatitis_id = MHC.MorbusHepatitis_id
				left join v_Evn EvnEdit on EvnEdit.Evn_id = :Evn_id and EvnUsluga.Person_id != :Evn_id
			where
				MHC.MorbusHepatitis_id = :MorbusHepatitis_id
	...
	function getMorbusHepatitisDiagViewData($data)
	{
		...
		$query = "
			select 
				case when ((MHD.Evn_id is null and MB.Person_id = :Evn_id) or (MHD.Evn_id = :Evn_id and coalesce(EvnEdit.Evn_IsSigned,1) = 1)) and MH.Morbus_disDT is null then 'edit' else 'view' end as \"AccessType\"
				,MHD.MorbusHepatitisDiag_id as \"MorbusHepatitisDiag_id\"
				,to_char(MHD.MorbusHepatitisDiag_setDT, 'dd.mm.yyyy') as \"MorbusHepatitisDiag_setDate\"
				,to_char(MHD.MorbusHepatitisDiag_ConfirmDT, 'dd.mm.yyyy') as \"MorbusHepatitisDiag_ConfirmDate\"
				,Lpu.Lpu_Nick as \"Lpu_Nick\"
				,LpuSection.LpuSectionProfile_Name as \"LpuSectionProfile_Name\"
				,MedPersonal.Person_Fin as \"MedPersonal_Name\"
				,HDAT.HepatitisDiagActiveType_Name as \"HepatitisDiagActiveType_Name\"
				,HDT.HepatitisDiagType_Name as \"HepatitisDiagType_Name\"
				,HFT.HepatitisFibrosisType_Name as \"HepatitisFibrosisType_Name\"
				,MHD.MorbusHepatitis_id as \"MorbusHepatitis_id\"
				,:Evn_id as \"MorbusHepatitis_pid\"
			from
				v_MorbusHepatitisDiag MHD 
				inner join v_MorbusHepatitis MH on MH.MorbusHepatitis_id = MHD.MorbusHepatitis_id
				inner join v_MorbusBase MB on MH.MorbusBase_id = MB.MorbusBase_id
				left join v_EvnSection ES on MHD.Evn_id = ES.EvnSection_id
				left join v_EvnVizitPl PL on MHD.Evn_id = PL.EvnVizitPl_id
				left join v_Lpu Lpu on coalesce(ES.Lpu_id,PL.Lpu_id) = Lpu.Lpu_id
				left join v_LpuSection LpuSection on coalesce(ES.LpuSection_id,PL.LpuSection_id) = LpuSection.LpuSection_id
				left join v_MedPersonal MedPersonal on MHD.MedPersonal_id = MedPersonal.MedPersonal_id and coalesce(ES.Lpu_id,PL.Lpu_id) = MedPersonal.Lpu_id
				left join v_HepatitisDiagActiveType HDAT on MHD.HepatitisDiagActiveType_id = HDAT.HepatitisDiagActiveType_id
				left join v_HepatitisDiagType HDT on MHD.HepatitisDiagType_id = HDT.HepatitisDiagType_id
				left join v_HepatitisFibrosisType HFT on MHD.HepatitisFibrosisType_id = HFT.HepatitisFibrosisType_id
				left join v_Evn EvnEdit on EvnEdit.Evn_id = :Evn_id and MB.Person_id != :Evn_id
			where
	...
	function getMorbusHepatitisEvnViewData($data)
	{
		...
		{
			$set_person_id = '(select Evn.Person_id from Evn where Evn.Evn_id = :Evn_id)';
		}
		else
		{
		...
		$query = "
			select
				...
				,to_char(Evn.EvnVizitPL_setDT, 'dd.mm.yyyy') as \"Evn_setDate\"
				,Lpu.Lpu_Nick as \"Lpu_Nick\"
				,LpuSection.LpuSectionProfile_Name as \"LpuSectionProfile_Name\"
			...
			from
				v_EvnVizitPL Evn
				inner join v_Diag d on Evn.Diag_id = d.Diag_id and (d.Diag_Code ilike 'B15.%' or d.Diag_Code ilike 'B16.%' or d.Diag_Code ilike 'B17.%' or d.Diag_Code ilike 'B18.%' or d.Diag_Code ilike 'B19.%')
				inner join v_Lpu Lpu on Evn.Lpu_id = Lpu.Lpu_id
				inner join v_LpuSection LpuSection on Evn.LpuSection_id = LpuSection.LpuSection_id
				left join v_MedPersonal MP on Evn.MedPersonal_id = MP.MedPersonal_id and Evn.Lpu_id = MP.Lpu_id
			where
				Evn.Person_id = {$set_person_id}
			union all
			select
				Evn.EvnSection_id as \"Evn_id\"
				,Evn.EvnSection_pid as \"Evn_pid\"
				,'EvnSection' as \"EvnClass_sysNick\"
				,to_char(Evn.EvnSection_setDT, 'dd.mm.yyyy') as \"Evn_setDate\"
				,Lpu.Lpu_Nick as \"Lpu_Nick\"
				,LpuSection.LpuSectionProfile_Name as \"LpuSectionProfile_Name\"
				,MP.Person_Fin as \"Person_Fin\"
				,:Evn_id as \"MorbusHepatitis_pid\"
	...
	function getMorbusHepatitisSopDiagViewData($data)
	{
		...
		$query = "
			...
			from
				v_EvnVizitPL Evn
				inner join v_Diag d on Evn.Diag_id = d.Diag_id and (d.Diag_Code ilike 'B15.%' or d.Diag_Code ilike 'B16.%' or d.Diag_Code ilike 'B17.%' or d.Diag_Code ilike 'B18.%' or d.Diag_Code ilike 'B19.%')
				inner join v_EvnDiagPLSop DSP on Evn.EvnVizitPL_id = DSP.EvnDiagPLSop_pid
				inner join v_Diag Diag on DSP.Diag_id = Diag.Diag_id
			where
				Evn.Person_id = {$set_person_id}
			union all
			select
			...
			from
				v_EvnSection Evn
				inner join v_Diag d on Evn.Diag_id = d.Diag_id and (d.Diag_Code ilike 'B15.%' or d.Diag_Code ilike 'B16.%' or d.Diag_Code ilike 'B17.%' or d.Diag_Code ilike 'B18.%' or d.Diag_Code ilike 'B19.%')
				inner join v_EvnDiagPS DSP on Evn.EvnSection_id = DSP.EvnDiagPS_pid
				inner join DiagSetClass DSC on DSC.DiagSetClass_id = DSP.DiagSetClass_id and DSC.DiagSetClass_Code = 3
				inner join v_Diag Diag on DSP.Diag_id = Diag.Diag_id
			where
				Evn.Person_id = {$set_person_id}
		";
	...
	function getMorbusHepatitisViewData($data)
	{
		...
		$query = '
			select
				' . swMorbus::getAccessTypeQueryPart('M', 'MB', 'MorbusHepatitis_pid', 'edit', 'view', 'AccessType', 'AND not exists(
									...
									where
										...
										and (
											exists (
												select v_MorbusHepatitisVaccination.Evn_id
												from v_MorbusHepatitisVaccination
												where v_MorbusHepatitisVaccination.Evn_id = Evn.Evn_id
												limit 1
											)
											OR exists (
												select v_MorbusHepatitisCure.Evn_id
												from v_MorbusHepatitisCure
												where v_MorbusHepatitisCure.Evn_id = Evn.Evn_id
												limit 1
											)
											OR exists (
												select v_MorbusHepatitisDiag.Evn_id
												from v_MorbusHepatitisDiag
												where v_MorbusHepatitisDiag.Evn_id = Evn.Evn_id
												limit 1
											)
											OR exists (
												select v_EvnUsluga.EvnUsluga_pid as Evn_id
												from v_EvnUsluga
												inner join v_MorbusHepatitisFuncConfirm MHC on MHC.Evn_id = v_EvnUsluga.EvnUsluga_id
											...
											OR exists (
												select v_EvnUsluga.EvnUsluga_pid as Evn_id
												from v_EvnUsluga
												inner join v_MorbusHepatitisLabConfirm MHC on MHC.Evn_id = v_EvnUsluga.EvnUsluga_id
												where v_EvnUsluga.EvnUsluga_pid = Evn.Evn_id
												limit 1
											)
										)
								) /* можно редактировать, если нет более актуального документа в рамках которого изменялась специфика */') . '
				,MH.HepatitisEpidemicMedHistoryType_id as "HepatitisEpidemicMedHistoryType_id"
				,HEMHT.HepatitisEpidemicMedHistoryType_Name as "HepatitisEpidemicMedHistoryType_Name"
				,MH.MorbusHepatitis_EpidNum as "MorbusHepatitis_EpidNum"
			...
			from 
				...
				inner join v_MorbusHepatitis MH on MH.Morbus_id = M.Morbus_id
				left join HepatitisEpidemicMedHistoryType HEMHT on HEMHT.HepatitisEpidemicMedHistoryType_id = MH.HepatitisEpidemicMedHistoryType_id
			where
