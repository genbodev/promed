...
class ApplicationCVI_model extends Scenario_model {
	...
	public function doLoadData($data = array())
	{
		...
		$query = "
			select
				CQA.CVIQuestionAnswer_FreeForm as \"CVIQuestionAnswer_FreeForm\",
				to_char(CQA.CVIQuestionAnswer_setDT, 'dd.mm.yyyy') as \"CVIQuestionAnswer_setDT\",
				CQA.CVIQuestionAnswer_refID as \"CVIQuestionAnswer_refID\",
				CQT.CVIQuestionType_id \"CVIQuestionType_id\"
			from v_CVIQuestion CQ
			left join v_CVIQuestionAnswer CQA on CQA.CVIQuestion_id = CQ.CVIQuestion_id
			left join v_CVIQuestionType CQT on CQT.CVIQuestionType_id = CQA.CVIQuestionType_id
			where 1=1 $where
			order by CQA.CVIQuestion_id, CQA.CVIQuestionType_id
			limit 10
		";
