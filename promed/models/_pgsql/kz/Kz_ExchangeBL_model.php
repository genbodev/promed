...
require_once(APPPATH . 'models/_pgsql/ExchangeBL_model.php');
...
class Kz_ExchangeBL_model extends ExchangeBL_model
{
        ...
        foreach ($response->return->ResultsDirection as $result) {
			...
			if (!empty($result->FinanceSourceID)) {
				$pt = $this->getFirstResultFromQuery(
					"select PayType_id from r101.PayTypeLink where PayTypeLink_SUR = ?",
					[$result->FinanceSourceID],
					true
				);

				$opt = $this->getFirstResultFromQuery(
					"select PayType_id from v_EvnDirection_all where EvnDirection_id = ?",
					[$evn_id],
					true
				);
	...
	function saveAISResponse($data) 
	{
		$check_record = $this->getFirstResultFromQuery("
			select AISResponse_id as \"AISResponse_id\" from r101.AISResponse where Evn_id = ?
		", [$data['Evn_id']], true);
		...
		return $this->queryResult("
			select AISResponse_id as \"AISResponse_id\", Error_Code as \"Error_Code\", Error_Message as \"Error_Msg\"                
			from r101.{$proc}(
    ...
    private function getPayTypeMoneyId()
    {
        ...
        if ($pay_type_money_id === null) {
            $sql = "SELECT PayType_id as \"PayType_id\" FROM v_PayType WHERE PayType_SysNick='money' LIMIT 1";
    ...
    private function getServiceList($sql_method, &$last_id)
    {
        ...
        foreach ($result as $v) {
			...
			$sql = ($sql_method == 'getEvnUslugaSql')?"
				select ucat.UslugaComplexAttributeType_SysNick as \"UslugaComplexAttributeType_SysNick\" from v_EvnUsluga EU
				...
				limit 100
			":"
				select ucat.UslugaComplexAttributeType_SysNick  as \"UslugaComplexAttributeType_SysNick\" from v_EvnUslugaPar EUP
			...
			if ($sql_method == 'getEvnUslugaDispDopSql') {//скрининг
				...
				$isInList = $this->getFirstRowFromQuery("
					select * from v_UslugaComplex UC
					inner join v_EvnUsluga EU on UC.UslugaComplex_id = EU.UslugaComplex_id
				...
				if ($isKPN || $isInList){
					...
					$isInFoms = $this->getFirstResultFromQuery("select Person_IsInFOMS from v_Person where Person_id=:Patient", ['Patient' => (string)$v['PatientID']]);
				...
				$sql = "
					select top 1 PTL.PayTypeLink_SUR from v_EvnUsluga EU
					inner join r101.v_PayTypeLink PTL on PTL.PayType_id = EU.PayType_id
			...
			if ($sql_method == 'getEvnUslugaSql') {
				$TreatmentReasonID = $this->getFirstResultFromQuery("select top 1 TreatmentClass_Code from v_TreatmentClass where TreatmentClass_id=:TC ", ['TC' => $TC],true);
			}
			...
			if ($TreatmentReasonID == 18 && empty($Visit_type)) {
				...
				$sql = "
					select EC.EvnClass_SysNick as nm, E.Evn_disDT as dt from v_Evn E
						inner join v_EvnClass EC on E.EvnClass_id = EC.EvnClass_id
						where 
							E.Person_id = :prsn
						...
						limit 100
					union
					select 'Cmp' as nm, CCC.CmpCallCard_HospitalizedTime as dt from v_CmpCallCard CCC
						where 
							CCC.Person_id = :prsn
			...
			/*if ($TreatmentReasonID == 17 && $v['age'] < 18) {
				$Group_id = 1;
			} else if ($TreatmentReasonID == 17 && $v['age'] >= 18) {
				switch ($v['MKB10']) {
					case 'Z13.1':
					case 'Z13.6': $Group_id = 2; break;
					case 'Z12.4': $Group_id = 3; break;
					case 'Z13.5': $Group_id = 4; break;
					case 'Z12.3': $Group_id = 6; break;
					case 'Z12.1': $Group_id = 7; break;
					default: $Group_id = 5;
				}
			}*/
			...
			if ($sql_method == 'getEvnUslugaDispDopSql') {
				$result = $this->db->query("
					...
					from 
						...
						inner join r101.ScreenCheckListResult sclr on sclr.EvnPLDisp_id = EUDD.EvnUslugaDispDop_pid
						inner join r101.ScreenCheckList scl on scl.ScreenCheckList_id = sclr.ScreenCheckList_id
						inner join r101.ScreenValue sv on sv.ScreenValue_id = sclr.ScreenValue_id
						inner join r101.EvnLinkAPP elapp on elapp.Evn_id = EUDD.EvnUslugaDispDop_pid
			...
			$tmp = [
				...
				'PatientBirthDate' => !empty($v['PatientBirthDate']) ? $v['PatientBirthDate']:null,
    ...
    private function getEvnUslugaSql($deleted = false)
    {
        ...
        return "
            SELECT 
                ...
                COALESCE(EU.EvnUsluga_Kolvo, 1) as \"Count\",
                EU.EvnUsluga_setDT as \"Date\",
                MO_Usluga.ID as \"Usluga_MO_ID\",
                'v_PersonState' as \"separator_3\",
                COALESCE(PRS.BDZ_id, PS.Person_id) as \"PatientID\",
				...
				ELAPP.VizitActiveType_id as \"VizitActiveType_id\",
				edlatemp.Screening_id as scrining_id,
                $deleted_select
                -- end select
            FROM
                -- from
                ...
                left join r101.EvnLinkAPP ELAPP on ELAPP.Evn_id = EVPL.EvnVizitPL_id
                LEFT JOIN v_EvnUsluga EUVizit  ON EUVizit.EvnUsluga_pid=EVPL.EvnVizitPL_id and EUVizit.EvnUsluga_IsVizitCode = 2
                INNER JOIN v_PersonState PS  ON PS.Person_id = EVPL.Person_id
                INNER JOIN v_Person PRS  ON PRS.Person_id = PS.Person_id
                INNER JOIN v_Sex Sex  ON Sex.Sex_id = PS.Sex_id
                INNER JOIN v_UslugaComplex UC  ON UC.UslugaComplex_id = EU.UslugaComplex_id
                LEFT JOIN v_Diag Diag  ON Diag.Diag_id = COALESCE(EU.Diag_id, EVPL.Diag_id)
                -- LEFT JOIN v_Mkb10Cause Mkb10  ON Mkb10.Mkb10Cause_id = Diag.Mkb10Cause_id
                LEFT JOIN v_MedStaffFact MSF  ON MSF.MedStaffFact_id = COALESCE(EU.MedStaffFact_id, EVPL.MedStaffFact_id)
                -- LEFT JOIN v_PayType PT  ON PT.PayType_id = COALESCE(EU.PayType_id, EVPL.PayType_id)
                ...
                LEFT JOIN v_EvnDirection_all ED  ON ED.EvnDirection_id = COALESCE(EVPL.EvnDirection_id, EU.EvnDirection_id)
                LEFT JOIN LATERAL (
                	select Lpu_id
                ...
                LEFT JOIN r101.GetMO MO_Customer ON MO_Customer.Lpu_id = COALESCE(ED.Lpu_sid, EVPL.Lpu_id) -- направившая МО, МО прикрепления пациента или МО, в которой оказан случай лечения
                LEFT JOIN r101.GetMO MO_Performer ON MO_Performer.Lpu_id = coalesce(EU.Lpu_uid,EU.Lpu_id) -- выполнившая МО
                LEFT JOIN r101.GetMO MO_Patient ON MO_Patient.Lpu_id = PS.Lpu_id -- прикрепления пациента
                LEFT JOIN r101.GetMO MO_Usluga ON MO_Usluga.Lpu_id = EU.Lpu_id -- ID МО, в которой внесен случай лечения/скрининг.
                INNER JOIN v_Lpu Lpu ON Lpu.Lpu_id=EU.Lpu_id -- оставляем только услуги выбранного региона
                LEFT JOIN r101.v_UslugaMedTypeLink UMTL ON UMTL.Evn_id=EU.EvnUsluga_id
                LEFT JOIN r101.v_UslugaMedTypeLink UMTL2 ON UMTL2.Evn_id=EVPL.EvnVizitPL_id
                ...
                left join r101.AISResponse ed_air1 on ed_air1.AISResponse_IsSuccess = 1 and ed_air1.Evn_id = EU.EvnDirection_id
				left join r101.AISResponse ed_air2 on ed_air2.AISResponse_IsSuccess = 1 and ed_air2.Evn_id = EVPL.EvnDirection_id
				left join TreatmentClass tc (nolock) on tc.TreatmentClass_id = EVPL.TreatmentClass_id
				--left join r101.EvnLinkAPPTemp edlatemp on edlatemp.Evn_id = EVPL.EvnVizitPL_id
				LEFT JOIN LATERAL (
                	select
						edlatemp.Screening_id 
					from 
						r101.EvnLinkAPPTemp edlatemp
					where 
					...
					from 
						r101.EvnLinkAPPTemp edlatemp
						inner join v_Evn e on E.evn_id = edlatemp.Evn_id
					where 
    ...
    private function getEvnUslugaDispDopSql($deleted = false)
    {
        ...
        return "
            SELECT 
                ...
                COALESCE(EUDD.EvnUslugaDispDop_Kolvo, 1) as \"Count\",
            ...
            FROM
                ...
                INNER JOIN 
                $v_EvnUsluga 
                EU  on EU.EvnUsluga_id = EUDD.EvnUslugaDispDop_id
                INNER JOIN v_EvnPLDisp EPLD  on EPLD.EvnPLDisp_id = EUDD.EvnUslugaDispDop_rid
                INNER JOIN v_EvnUslugaDispDop EUDDLast  on EUDDLast.EvnUslugaDispDop_rid = EPLD.EvnPLDisp_id
                ...
                INNER JOIN v_Person PRS  ON PRS.Person_id = PS.Person_id
                INNER JOIN v_Sex Sex  ON Sex.Sex_id = PS.Sex_id
                INNER JOIN v_UslugaComplex UC  ON UC.UslugaComplex_id = EUDD.UslugaComplex_id
                LEFT JOIN v_Diag Diag  ON Diag.Diag_id = EUDD.Diag_id
                -- LEFT JOIN v_Mkb10Cause Mkb10  ON Mkb10.Mkb10Cause_id = Diag.Mkb10Cause_id
                ...
                LEFT JOIN r101.GetMO MO_Performer ON MO_Performer.Lpu_id = coalesce(EUDD.Lpu_uid,EUDD.Lpu_id) -- выполнившая МО
                LEFT JOIN r101.GetMO MO_Patient ON MO_Patient.Lpu_id = PS.Lpu_id -- прикрепления пациента
                ...
                left join r101.AISResponse ed_air on ed_air.AISResponse_IsSuccess = 1 and ed_air.Evn_id = EU.EvnDirection_id
				left join r101.EvnPLDispScreenLink epldsl on epldsl.EvnPLDispScreen_id = eudd.EvnUslugaDispDop_pid
            ...
            WHERE
                ...
                AND COALESCE(EUDD.PayType_id, EPLD.PayType_id) <> :PayType_id
...
";
    ...
    private function getEvnFuncSql($deleted = false)
    {
        ...
        return "
            SELECT 
                ...
                COALESCE(EUP.EvnUslugaPar_Kolvo, 1) as \"Count\",
                ...
                COALESCE(UMT.UslugaMedType_Code, '1400') as \"ServiceCDSKind\",
				Lpu.Lpu_id as \"Lpu_id\",
				coalesce(tc.TreatmentClass_Code, '7') as \"TreatmentClass_Code\",
				ed_air.AISResponse_IDEPS as \"Refferal_id\",
                $deleted_select
                -- end select
            FROM
                ...
                INNER JOIN v_EvnDirection_all ED_all  ON ED_all.EvnDirection_id=EFR.EvnFuncRequest_pid
            	INNER JOIN 
                $v_EvnUslugaPar 
                EUP  ON EUP.EvnDirection_id = ED_all.EvnDirection_id
				...
				LEFT JOIN v_EvnPS EPS on EPS.EvnPS_id = EUP.EvnUslugaPar_pid
                LEFT JOIN v_EvnVizitPL EVPL  on EVPL.EvnVizitPL_id = EUP.EvnUslugaPar_pid
                LEFT JOIN v_EvnUsluga EUVizit  ON EUVizit.EvnUsluga_pid=EVPL.EvnVizitPL_id and EUVizit.EvnUsluga_IsVizitCode = 2
                INNER JOIN v_PersonState PS  ON PS.Person_id = EFR.Person_id
                INNER JOIN v_Person PRS  ON PRS.Person_id = PS.Person_id
                INNER JOIN v_Sex Sex  ON Sex.Sex_id = PS.Sex_id
                LEFT JOIN v_Diag Diag  ON Diag.Diag_id = COALESCE(EUP.Diag_id, EFR.Diag_id, ED_all.Diag_id)
                LEFT JOIN v_MedStaffFact MSF  ON MSF.MedStaffFact_id = COALESCE(EUP.MedStaffFact_id, EVPL.MedStaffFact_id)
                -- В направлении (EvnDirection) есть поля Lpu_sid - направившая МО, Lpu_did - мо, куда направиили.
                -- Customer - Идентификатор поставщика (sur_id) – Заполняется ID из СУР направившей МО.
                -- Performer - Идентификатор субподрядчика (sur_id) -  ID из СУР МО,  выполнившей услугу
                LEFT JOIN LATERAL (
                ...
                LEFT JOIN r101.GetMO MO_Customer ON MO_Customer.Lpu_id = EUP.Lpu_id -- направившая МО, МО прикрепления пациента или МО, в которой оказан случай лечения
                LEFT JOIN r101.GetMO MO_Performer ON MO_Performer.Lpu_id = coalesce(EUP.Lpu_uid,EUP.Lpu_id) -- выполнившая МО
                LEFT JOIN r101.GetMO MO_Patient ON MO_Patient.Lpu_id = PS.Lpu_id -- прикрепления пациента
                ...
                LEFT JOIN r101.v_UslugaMedTypeLink UMTL3 ON UMTL3.Evn_id=EUVizit.EvnUsluga_id
                LEFT JOIN r101.v_UslugaMedType UMT ON UMT.UslugaMedType_id=COALESCE(UMTL.UslugaMedType_id,UMTL2.UslugaMedType_id,UMTL3.UslugaMedType_id)
                left join r101.EvnLinkAPP edla on edla.Evn_id = ED_all.EvnDirection_id
				left join v_TreatmentClass tc on tc.TreatmentClass_id = edla.TreatmentClass_id
				left join r101.AISResponse ed_air on ed_air.AISResponse_IsSuccess = 1 and ed_air.Evn_id = ED_all.EvnDirection_id
                -- end from
            WHERE
                -- where
                EUP.EvnUslugaPar_id > :LAST_ID
                AND COALESCE(EUP.PayType_id, 0) <> :PayType_id
    ...
    private function getEvnLabSql($deleted = false)
    {
        ...
        return "
            ...
            FROM
                ...
                INNER JOIN $v_EvnUslugaPar 
                EUP  ON EUP.EvnLabSample_id=ELS.EvnLabSample_id
                LEFT JOIN v_EvnDirection ED  on ED.EvnDirection_id = ELR.EvnDirection_id
                INNER JOIN UslugaComplex UC ON UC.UslugaComplex_id=EUP.UslugaComplex_id
                LEFT JOIN v_EvnVizitPL EVPL  on EVPL.EvnVizitPL_id = EUP.EvnUslugaPar_pid
                LEFT JOIN v_EvnSection ES  on ES.EvnSection_id = EUP.EvnUslugaPar_pid
                LEFT JOIN v_EvnPS EPS on EPS.EvnPS_id = EUP.EvnUslugaPar_pid
                INNER JOIN v_PersonState PS  ON PS.Person_id = ELR.Person_id
                INNER JOIN v_Person PRS  ON PRS.Person_id = PS.Person_id
                INNER JOIN v_Sex Sex  ON Sex.Sex_id = PS.Sex_id
                LEFT JOIN v_Diag Diag  ON Diag.Diag_id = COALESCE(EUP.Diag_id, ELR.Diag_id)
                LEFT JOIN v_MedStaffFact MSF  ON MSF.MedStaffFact_id = COALESCE(EUP.MedStaffFact_id, EVPL.MedStaffFact_id)
                -- В направлении (EvnDirection) есть поля Lpu_sid - направившая МО, Lpu_did - мо, куда направиили.
                -- Customer - Идентификатор поставщика (sur_id) – Заполняется ID из СУР направившей МО.
                -- Performer - Идентификатор субподрядчика (sur_id) -  ID из СУР МО,  выполнившей услугу
    ...
    private function getEvnLabTestSql($deleted = false)
    {
        ...
        if (!$deleted) {
            $v_EvnUslugaPar = 'v_EvnUslugaPar';
            $app_status = ' AND COALESCE(UT.UslugaTest_IsAPP,0)<>' . static::APP_STATUS_SUCCESS;
            $deleted_condition = '';
            $deleted_select = 'null as "DeleteDate"';
        } else {
            $v_EvnUslugaPar = 'v_EvnUslugaPar_Del';
        ...
        return "
            SELECT 
                ...
                (
                    RIGHT(CAST(EUP.EvnUslugaPar_id as varchar), 3)
                    -- добиваем идентификатор лаб.теста до 15 символов
                    || RIGHT('000000000000000' || CAST(UslugaTest_id as varchar), 15)
                )
                as \"ID\",
                COALESCE(EUP.EvnUslugaPar_Kolvo, 1) as \"Count\",
                EUP.EvnUslugaPar_setDate as \"Date\",
            ...
            FROM
                -- from
                v_EvnLabRequest ELR
                inner join r101.PayTypeLink PTL on ELR.PayType_id = PTL.PayType_id 
                INNER JOIN v_EvnLabSample ELS  ON ELS.EvnLabRequest_id=ELR.EvnLabRequest_id
                INNER JOIN $v_EvnUslugaPar EUP  ON EUP.EvnLabSample_id=ELS.EvnLabSample_id
                LEFT JOIN v_EvnDirection ED  on ED.EvnDirection_id = ELR.EvnDirection_id
                INNER JOIN v_UslugaTest UT  ON UT.EvnLabSample_id=ELS.EvnLabSample_id AND UT.UslugaComplex_id<>EUP.UslugaComplex_id -- тесты лабораторных услуг
                INNER JOIN UslugaComplex UC ON UC.UslugaComplex_id=UT.UslugaComplex_id
                LEFT JOIN v_EvnSection ES on ES.EvnSection_id = EUP.EvnUslugaPar_pid
				LEFT JOIN v_EvnPS EPS on EPS.EvnPS_id = EUP.EvnUslugaPar_pid
                LEFT JOIN v_EvnVizitPL EVPL  on EVPL.EvnVizitPL_id = EUP.EvnUslugaPar_pid
                left join r101.EvnLinkAPP ELAPP on ELAPP.Evn_id = EVPL.EvnVizitPL_id
                left join v_EvnUsluga EUPVizit on EUPVizit.EvnUsluga_pid = EVPL.EvnVizitPL_id and EUPVizit.EvnUsluga_IsVizitCode = 2
                INNER JOIN v_PersonState PS  ON PS.Person_id = ELR.Person_id
                INNER JOIN v_Sex Sex  ON Sex.Sex_id = PS.Sex_id
                LEFT JOIN v_Diag Diag  ON Diag.Diag_id = COALESCE(EUP.Diag_id, ELR.Diag_id)
                ...
                LEFT JOIN LATERAL (
					select EDparent.* from v_EvnUsluga EUparent
					left JOIN v_EvnVizitPL EVPLparent on EVPLparent.EvnVizitPL_id = EUparent.EvnUsluga_pid
					LEFT JOIN v_EvnDirection_all EDparent ON EDparent.EvnDirection_id = isnull(EVPLparent.EvnDirection_id, EUparent.EvnDirection_id)
					where EUparent.EvnUsluga_id = UT.UslugaTest_pid
				) ParentTest ON true
                
				left join v_EvnUsluga EUparent on EUparent.EvnUsluga_id = UT.UslugaTest_pid
				left JOIN v_EvnVizitPL EVPLparent on EVPLparent.EvnVizitPL_id = EUparent.EvnUsluga_pid
				LEFT JOIN v_EvnDirection_all EDparent ON EDparent.EvnDirection_id = COALESCE(EVPLparent.EvnDirection_id, EUparent.EvnDirection_id)
				
                LEFT JOIN r101.GetMO MO_Customer ON MO_Customer.Lpu_id = COALESCE(EDparent.Lpu_sid, EUP.Lpu_id) -- направившая МО, МО прикрепления пациента или МО, в которой оказан случай лечения
                LEFT JOIN r101.GetMO MO_Performer ON MO_Performer.Lpu_id = coalesce(EUP.Lpu_uid,EUP.Lpu_id) -- выполнившая МО
                LEFT JOIN r101.GetMO MO_Patient ON MO_Patient.Lpu_id = PS.Lpu_id -- прикрепления пациента
                LEFT JOIN r101.GetMO MO_Usluga ON MO_Usluga.Lpu_id = ELR.Lpu_id -- ID МО, в которой внесен случай лечения/скрининг.
                INNER JOIN v_Lpu Lpu ON Lpu.Lpu_id=ELR.Lpu_id -- оставляем только услуги выбранного региона
                ...
                LEFT JOIN r101.v_UslugaMedTypeLink UMTL2 ON UMTL2.Evn_id=EUP.EvnUslugaPar_id
                LEFT JOIN r101.v_UslugaMedType UMT ON UMT.UslugaMedType_id=COALESCE(UMTL.UslugaMedType_id, UMTL2.UslugaMedType_id)
                left join r101.AISResponse ed_air on ed_air.AISResponse_IsSuccess = 1 and ed_air.Evn_id = coalesce (ParentTest.EvnDirection_id, ED.EvnDirection_id)
                -- end from
            WHERE
                -- where
    ...
    function SetReferral() {
        ...
        return "
			select 
				...
				to_char(ed.EvnDirection_setDate, 'YYYY-MM-DD') as \"direction_date\",
				mo.ID as \"sending_mo_id\",
				gph.PersonalID as \"personal_id\",
				gph.PostFuncID as \"position_id\",
				mol.ID as \"directed_mo_id\",
				coalesce(uc1.UslugaComplex_Code,'1') as \"service_code\",
				COALESCE(ptl.PayTypeLink_SUR, 50) as \"finance_source\",
				1 as \"icd_type\",
				...
				to_char(ed.EvnDirection_failDT, 'YYYY-MM-DD') as \"cancel_date\",
				edla.PayTypeKAZ_id as \"paymentType\",
				edla.ScreenType_id as group_id,--\"Group_id\",
				case 
					when ed.DirType_id = 15 then coalesce(ECPR.EvnCourseProc_Duration, 1) * coalesce(ECPR.EvnCourseProc_MaxCountDay, 1)
					else 1 
				end as \"Count\",
				coalesce (edlaSreen.Screening_id, edlatemp.Screening_id) as scrining_id--screening_id
			from v_EvnDirection_all ed 
				inner join v_Org org  on org.Org_id = ed.Org_oid
				left join v_Lpu l1 on l1.Org_id = org.Org_id
				inner join v_Lpu lpu  on lpu.Org_id = org.Org_id
				inner join v_Person p  on p.Person_id = ed.Person_id
				inner join r101.GetMO mo  on mo.Lpu_id = ed.Lpu_id
				inner join r101.GetMO mol  on mol.Lpu_id = lpu.Lpu_id
				inner join v_Diag d  on d.Diag_id = ed.Diag_id
				left join r101.AISResponse ar  on ar.Evn_id = ed.EvnDirection_id
				LEFT JOIN LATERAL (
					select pul.UslugaComplex_id
					from r101.ProfileUslugaLink pul
					where pul.LpuSectionProfile_id = ed.LpuSectionProfile_id
					limit 1
				) pul ON true
				left join v_UslugaComplex uc1 on uc1.UslugaComplex_id = pul.UslugaComplex_id
				LEFT JOIN LATERAL (
					select 
						gpw.PersonalID,
						gpw.PostFuncID
				...
				inner join v_TreatmentClass tc  on tc.TreatmentClass_id = edla.TreatmentClass_id
				left join r101.PayTypeLink ptl  on ptl.PayType_id = ed.PayType_id
				left join v_EvnPS evnps on evnps.EvnPS_id = ed.EvnDirection_rid
				LEFT JOIN LATERAL (
					select ECPR.*
					from v_EvnPrescrDirection EPD
						inner join v_EvnPrescrProc EPPR on EPPR.EvnPrescrProc_id = EPD.EvnPrescr_id
						inner join v_EvnCourseProc ECPR on ECPR.EvnCourseProc_id = EPPR.EvnCourse_id
					where EPD.EvnDirection_id = ED.EvnDirection_id
					limit 1
				) ECPR ON true
				LEFT JOIN LATERAL (
					select edlatemp.Screening_id
					from v_EvnPL EPL
						inner join v_EvnVizitPL EVPL on EPL.EvnPL_id = evpl.EvnVizitPL_pid
						inner join r101.EvnLinkAPPTemp edlatemp on edlatemp.Evn_id = evpl.EvnVizitPL_id
					where EPL.EvnPL_id = ED.EvnDirection_rid
					union
					select
						case when coalesce(tc.TreatmentClass_Code,'1') = '17' then edlatemp.Screening_id else null end as Screening_id
					from 
			...
			where
				...
				and COALESCE(ar.AISResponse_IsSuccess, 0) = 0
				and coalesce(uc1.UslugaComplex_Code,'1') != '1'
    ...
    function FinanceSource() {
        return "
			select 
				:Person_id as \"ID\",
				p.BDZ_id as \"person_id\",
				to_char(CAST(:EvnDirection_setDate as date), 'YYYY-MM-DD') as \"direction_date\",
				COALESCE(uc.UslugaComplex_Code, '1') as \"service_code\",
			...
			from v_Person p 
				left join r101.GetMO mo  on mo.Lpu_id = :Lpu_id
				left join v_Lpu lpu  on lpu.Org_id = :Org_oid
				left join r101.GetMO mol  on mol.Lpu_id = coalesce(lpu.Lpu_id,:Lpu_did)
				inner join v_Diag d  on d.Diag_id = :Diag_id
				left join r101.ProfileUslugaLink pul  on pul.LpuSectionProfile_id = :LpuSectionProfile_id
				left join v_UslugaComplex uc  on uc.UslugaComplex_id = COALESCE(CAST(:UslugaComplex_id as bigint), pul.UslugaComplex_id)
				inner join v_TreatmentClass tc  on tc.TreatmentClass_id = :TreatmentClass_id
			where 
    ...
    function getPayType($data)
    {
		...
		$result = $this->getFirstRowFromQuery(
			"select PT.PayType_id as \"PayType_id\", PT.PayType_Name  as \"PayType_Name\" from r101.PayTypeLink PTL 
    ...
    private function getPayTypeLocal($data, $result)
    {
		...
		$paytype_id = $this->getFirstResultFromQuery("
			...
			where 
				...
				(Prizn_Strahov = (select COALESCE(Person_IsInFOMS, 1) as Person_IsInFOMS from v_Person  where Person_id = :Person_id) or Prizn_Strahov is null) and 
				(Prizn_DispU = (
				select case when exists (
					...
					where 
						...
						Diag_id = :Diag_id and 
						COALESCE(PersonDisp_endDate, dbo.tzGetDate()) >= dbo.tzGetDate()
				) then 2 else 1 end as Prizn_DispU
			) or Prizn_DispU is null) and 
    ...
    private function getPayTypeStac($data)
    {
    	if (!empty($data['EvnPS_id'])) {
			$dir_data = $this->getFirstRowFromQuery("
				select 
					...
					ed.LpuUnitType_id as \"LpuUnitType_id\",
					edla.EvnLinkAPP_StageRecovery as \"EvnLinkAPP_StageRecovery\"
				from v_EvnPS eps  
					inner join v_EvnDirection  ed on eps.EvnDirection_id = ed.EvnDirection_id
					left join r101.EvnLinkAPP edla  on edla.Evn_id = ed.EvnDirection_id
			...
			if (empty($data['PurposeHospital_id'])) {
				$data['PurposeHospital_id'] = $this->getFirstResultFromQuery("select PurposeHospital_id  as \"PurposeHospital_id\" from r101.EvnLinkAPP  where Evn_id = :EvnPS_id", $data, true);
			}
		}
		
    	if (empty($data['Lpu_did'])) {
			$data['Lpu_did'] = $this->getFirstResultFromQuery("select Lpu_id  as \"Lpu_id\" from v_Lpu  where Org_id = :Org_oid", $data);
		}

		$data['isVillage'] = false === $this->getFirstResultFromQuery("
			select AttributeSignValue_id as \"AttributeSignValue_id\"
			from v_AttributeSignValue ASV 
				inner join v_AttributeSign as AS1  on AS1.AttributeSign_id = ASV.AttributeSign_id
			where 
				AS1.AttributeSign_Code = 17 and 
		...
		$pers_age = $this->getFirstResultFromQuery("select dbo.Age2(Person_Birthday, dbo.tzGetDate()) from v_PersonState  where Person_id = :Person_id", $data);
		...
		if (in_array($data['LpuUnitType_id'], [6, 7])) {
			...
			if ($data['PurposeHospital_id'] == 2) {
				$dus = $this->getFirstResultFromQuery("
					select DiagUslugaStac_id as \"DiagUslugaStac_id\"
					from r101.DiagUslugaStac  
					where 
						UslugaComplex_id = :UslugaComplex_id and
						(Diag_id = :Diag_id or Diag_id is null) and
						COALESCE(DiagUslugaStac_isVillage,1) = :isVillage and 
						LpuUnitType_id = 6
				", $data);
    ...
    private function _getPayTypeStac($data)
    {
		...
		$paytype_id = $this->getFirstResultFromQuery("
			select PayType_id as \"PayType_id\"
			from r101.FinanceStac 
			where 
				DirType_id = :DirType_id and
				(LpuUnitType_id = :LpuUnitType_id or LpuUnitType_id is null or DirType_id = 4) and 
				(COALESCE(FinanceStac_StageRecovery, 0) = COALESCE(CAST(:EvnLinkAPP_StageRecovery as bigint), 0)) and 
				(Diag_id = :Diag_id or Diag_id is null) and 
				(Diag_cid = :Diag_cid or Diag_cid is null) and 
				(FinanceStac_isInsured = (select COALESCE(Person_IsInFOMS, 1) as Person_IsInFOMS from v_Person  where Person_id = :Person_id) or FinanceStac_isInsured is null) and 
				(LpuSectionAge_id = (
				select case when dbo.Age2(Person_Birthday, dbo.tzGetDate()) >= 18 then 1 else 2 end as LpuSectionAge_id
				from v_PersonState  
				where Person_id = :Person_id
			) or LpuSectionAge_id is null) and 
				(FinanceStac_IsForeigner = (
				select case when (KLCountry_id = 398 or SocStatus_id = 10000092) then 1 else 2 end as FinanceStac_IsForeigner
				from v_PersonState  
				where Person_id = :Person_id
			) or FinanceStac_IsForeigner is null)
			order by (case PayType_id 
				when 151 then 1
				when 150 then 2
				when 153 then 3
	...
	{
		if (empty($data['Lpu_did'])) {
			$data['Lpu_did'] = $this->getFirstResultFromQuery("select Lpu_id from v_Lpu  where Org_id = :Org_oid", $data);
		...
		$chk = $this->getFirstResultFromQuery("
			select FinanceStac_id  as \"FinanceStac_id\"
			from r101.FinanceStac 
			where 
				DirType_id = 5 and 
		...
		$chk = $this->getFirstResultFromQuery("
			...
			where Person_id = :Person_id and (
					...
					COALESCE(ps.Person_IsInErz,1) = 1
				)
		", $data);
	...
	private function getFinSrcByCritery($data) {
		...
		$params = $this->getFirstRowFromQuery("
			select
				...
				to_char(ps.Person_BirthDay, 'YYYY-DD-MM') as \"Person_BirthDay\",
			...
			from v_Diag diag
				...
				left join r101.v_PurposeHospital ph on ph.PurposeHospital_id = :PurposeHospital_id
				left join v_Diag cdiag on cdiag.Diag_id = :Diag_cid
				left join r101.GetBed gb on gb.GetBed_id = :GetBed_id
		...
		if (is_array($pt) && $pt[1] > 0) {
			$pay_type = $this->getFirstResultFromQuery("select PayType_id as \"PayType_id\" from r101.PayTypeLink where PayTypeLink_PubCOD = ? limit 1", [$pt[1]], true);
		}
    ...
    function GetReferral()
    {
		...
		foreach($lpu_ids as $lpu_id) {
			...
			$moid = $this->getFirstResultFromQuery("select ID  as \"ID\" from r101.v_GetMO  where Lpu_id = ? ", [$lpu_id]);
    ...
    {		
		$Person_Inn = $this->getFirstResultFromQuery("select Person_Inn  as \"Person_Inn\" from v_PersonState  where Person_id = ? ", [$data['Person_id']]);
    ...
    private function processGetReferral($services, $lpu_id)
    {
		...
		foreach($services as $dir) {
			...
			//$Evn_id = $this->getFirstResultFromQuery("select Evn_id  as \"Evn_id\" from r101.AISResponse  where AISResponse_usid = ? ", [$dir->ID]);
			$Evn_id = $this->getFirstResultFromQuery("select Evn_id as \"Evn_id\" from r101.AISResponse where AISResponse_IDEPS = ?", [$ideps]);
			...
			if ($Evn_id != false) {
				...
				$direction = $this->getFirstRowFromQuery("
					select 
						...
						ed.EvnDirection_failDT as \"EvnDirection_failDT\",
				...
				$dir_list[] = [
					...
					'PrehospDirect_Name' => $this->getFirstResultFromQuery("
						select lpu.Lpu_Nick  as \"Lpu_Nick\"
						from v_Lpu lpu  
							inner join r101.v_GetMO mo  on mo.Lpu_id = lpu.Lpu_id
						where 
							mo.ID = ?
			...
			$PayType_id = $this->getFirstResultFromQuery("select PayType_id  as \"PayType_id\" from r101.PayTypeLink where PayTypeLink_SUR = ? ", [$dir->finance_source], true);

			$Diag_id = $this->getFirstResultFromQuery("select Diag_id  as \"Diag_id\" from v_Diag  where Diag_Code = ? ", [$dir->ocd_code], true);

			
			$Lpu = $this->getFirstRowFromQuery("
				select lpu.Lpu_id as \"Lpu_id\", lpu.Org_id as \"Org_id\", lpu.Lpu_Nick as \"Lpu_Nick\"
				from v_Lpu lpu  
					inner join r101.v_GetMO mo  on mo.Lpu_id = lpu.Lpu_id
				where 
					mo.ID = ?
			...
			$Lpu_did = $this->getFirstResultFromQuery("select Lpu_id from r101.v_GetMO  where ID = ? ", [$dir->directed_mo_id]);
			
			$TreatmentClass_id = $this->getFirstResultFromQuery("select TreatmentClass_id from v_TreatmentClass  where TreatmentClass_Code = ? ", [(string)$dir->direction_reason], true);
			
			$UslugaComplex = $this->getFirstRowFromQuery("
				select UslugaComplex_id as \"UslugaComplex_id\", UslugaComplex_pid  as \"UslugaComplex_pid\"
				from v_UslugaComplex  
				where UslugaComplex_Code = ? and dbo.tzGetDate() between UslugaComplex_begDT and COALESCE(UslugaComplex_endDT, dbo.tzGetDate())
			", [$dir->service_code]);
			
			if (false != $UslugaComplex) {
				$LpuSectionProfile_id = $this->getFirstResultFromQuery("select LpuSectionProfile_id  as \"LpuSectionProfile_id\" from r101.ProfileUslugaLink  where UslugaComplex_id = ? ", [$UslugaComplex['UslugaComplex_id']], true);
    ...
    function Prof_beginTemp() {
        ...
        return <<<SQL
			select
				...
				,to_char(evpl.EvnVizitPL_setDate, 'yyyy-mm-dd') as "screening_begin_date"
				,evpl.EvnVizitPL_id as "screening_mis_id"
				,mo.ID as "mo_id"
			from v_EvnVizitPL evpl
				inner join r101.EvnLinkAPP edla on edla.Evn_id = evpl.EvnVizitPL_id
				inner join v_Person p on p.Person_id = evpl.Person_id
                inner join v_Lpu Lpu on Lpu.Lpu_id = evpl.Lpu_id
				inner join r101.GetMO mo on mo.Lpu_id = evpl.Lpu_id
				left join r101.EvnLinkAPPTemp edlatemp on edlatemp.Evn_id = evpl.EvnVizitPL_id
			where 
...
SQL;
    ...
    function loadProfileUslugaLink() {
		return $this->queryResult("
			select  
				pul.ProfileUslugaLink_id as \"ProfileUslugaLink_id\",
				pul.LpuSectionProfile_id as \"LpuSectionProfile_id\",
				pul.UslugaComplex_id as \"UslugaComplex_id\",
				uca.UslugaComplexAttributeType_SysNick as \"UslugaComplex_AttributeList\"
			from r101.ProfileUslugaLink pul 
				LEFT JOIN LATERAL (
	...
	function sendEvnPLDispScreenAPP($data) {
		...
		$result = $this->getFirstRowFromQuery("
			select
				...
				p.bdz_id as \"PatientID\",
				to_char(screen.EvnPLDispScreen{$child}_setDT,'yyyy-mm-dd') as screening_begin_date,
			...
			from 
				v_EvnPLDispScreen{$child} screen
				inner join v_Person p on p.person_id = screen.Person_id
				inner join r101.v_getmo gmo on gmo.Lpu_id = screen.Lpu_id
				inner join r101.EvnPLDispScreen{$child}Link screenLink on screenLink.EvnPLDispScreen{$child}_id = screen.EvnPLDispScreen{$child}_id
				left join r101.EvnLinkAPP elapp on elapp.Evn_id = screen.EvnPLDispScreen{$child}_id
				left join lateral (
					select
						eudd.Diag_id as \"Diag_id\",
						d.Diag_Code,
						eudd.EvnUslugaDispDop_setDT as \"EvnUslugaDispDop_setDT\" 
		...
		$params = [
			...
			'ProfData' => [
				'Items' => [
					...
					'screening_begin_date' => $result['screening_begin_date'],
