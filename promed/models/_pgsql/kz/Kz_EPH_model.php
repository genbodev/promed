...
* @version      10.09.2009
*/
require_once(APPPATH.'models/_pgsql/EPH_model.php');
...
class Kz_EPH_model extends EPH_model
{
	...
	function GetCmpCloseCard($data)
	{
		...
		select 
			...
			RTrim(COALESCE(to_char(cast(CLC.AcceptTime as timestamp), 'DD.MM.YYYY'),'')) as \"AcceptTime\",
	...
	function GetCmpCallCardCostPrintNodeList($data) {
		...
		if ( isset($data['CmpCloseCard_id']) && $data['CmpCloseCard_id'] > 0 ) {
			$params['CmpCallCard_id'] = $this->getFirstResultFromQuery("select CmpCallCard_id  as \"CmpCallCard_id\" from r101.v_CmpCloseCard  where CmpCloseCard_id = :CmpCloseCard_id", $data);
