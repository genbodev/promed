	...
	function searchKLTown($data)
	{
		...
		$sql = "SELECT
				...
				WHERE
					...
					KLTown.KLArea_Name ILIKE :KLTown_Name " . $filter . " limit 200";
	...
	function searchKLCity($data)
	{
		...
		$sql = "SELECT
				...
				WHERE
					...
					KLCity.KLArea_Name LIKE :KLCity_Name " . $filter . " limit 200";
		$result = $this->db->query($sql, $queryParams);
        if (is_object($result))
        {
            return $result->result('array');
			...
			$sql = "
				...
				WHERE
					...
					and (
						...
						OR (
							(kla.KLAdr_Code ILIKE :subregion_id || '%' AND kla.KLAreaLevel_id in (3,5) )
							OR
	...
	function loadAddress($data,$addressType = null,$extraData = false)
	{
		...
		if($extraData){
			$extraSelect = " 
				...
				,KLAreaType_id as \"KLAreaType_id\"
				,KLAreaStat_id as \"KLAreaStat_id\"
				,Address_Nick as \"Address_Nick\"
				,PersonSprTerrDop_id as \"PersonSprTerrDop_id\"
				,AddressSpecObject_id as \"AddressSpecObject_id\"
				,Address_begDate as \"Address_begDate\"
			";
		}
		$query = "
			select
