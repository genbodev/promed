...
class Kz_Polka_PersonCard_model extends Polka_PersonCard_model {
	...
	function loadPersonCardAttachGrid($data) {
		...
		$query = "
			select
				...
				PS.Person_SurName||' '||PS.Person_FirName||rtrim(' '||COALESCE(PS.Person_SecName,'')) as \"Person_Fio\",
				GAC.GetAttachmentCase_Name as \"GetAttachmentCase_Name\",
			...
			from
				...
				left join r101.v_GetAttachmentCase GAC  on GAC.GetAttachmentCase_id = GA.GetAttachmentCase_id
				left join v_Lpu La  on La.Lpu_id = PCA.Lpu_aid
				LEFT JOIN LATERAL (
			...
			if (isset($data['ignorePersonCardExists']) && !$data['ignorePersonCardExists']) {
				$resp = $this->queryResult("
					select 
						...
						PC.Person_SurName||' '||PC.Person_FirName||COALESCE(' '||PC.Person_SecName,'') as \"Person_Fio\",
		...
		$query = "
			...
			from
				...
				INNER JOIN LATERAL(
					...
					order by Object_setDT desc
                    limit 1
				) OSL_LpuRegion ON true
				inner join v_LpuRegion LR  on LR.LpuRegion_id = OSL_LpuRegion.Object_id
				inner join v_LpuRegionType LRT  on LRT.Region_id = 101 and LRT.LpuRegionType_Code = CAST(GA.GetTerrServiceProfile_id as varchar)
				inner join Person P  on P.BDZ_id = GA.Person_id
			where
	...
	function createPersonCardByAttach($data) {
		...
		$count = $this->getFirstResultFromQuery("
			select count(PersonCard_id) as \"cnt\"
		...
		$query = "
			select 
				...
				PCA.Lpu_aid as \"Lpu_aid\",
				PCA.PersonCardAttach_id as \"PersonCardAttach_id\",
				COALESCE(PAC.PersonAmbulatCard_Num, 1) as \"PersonAmbulatCard_Num\",
				--to_char(PCA.PersonCardAttach_setDate, 'YYYY-MM-DD HH24:MI:SS') as \"PersonCardAttach_setDate\",
				to_char(GA.GetAttachment_begDate, 'YYYY-MM-DD') as \"PersonCardAttach_setDate\",
				LR.LpuRegion_id as \"LpuRegion_id\",
