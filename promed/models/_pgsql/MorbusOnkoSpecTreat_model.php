 ...
 */
class MorbusOnkoSpecTreat_model extends swPgModel
{
	/**
	 * Конструктор
	 */
	...
	protected function _validate()
	{
		...
		if (in_array($this->scenario, array(
			...
			if ( ConvertDateFormat($this->specSetDT, 'Y-m-d') > date('Y-m-d')) {
				throw new Exception('Дата начала специального лечения не должна быть больше текущей даты', 400);
			}
			if ( !empty($this->specDisDT) && ConvertDateFormat($this->specSetDT, 'Y-m-d') > ConvertDateFormat($this->specDisDT, 'Y-m-d')) {
			...
			if (!empty($Evn_id)) {
				...
				$check = $this->MorbusOnkoSpecifics_model->checkDatesBeforeSave(array(
					'Evn_id' => $Evn_id,
					'dateOnko' => ConvertDateFormat($this->specSetDT, 'Y-m-d')
				));
			...
			$params = array(
				...
				'MorbusOnkoLeave_id' => $this->MorbusOnkoLeave_id,
				'MorbusOnkoDiagPLStom_id' => $this->MorbusOnkoDiagPLStom_id,
				'specSetDT' => ConvertDateFormat($this->specSetDT, 'Y-m-d'),
			...
			$query = "
				with SpecTreat as (
					select
					MorbusOnko_id,
					to_char(MorbusOnkoSpecTreat_specSetDT, 'YYYY-MM-DD HH24:MI:SS') as MorbusOnkoSpecTreat_specSetDT,
					to_char(MorbusOnkoSpecTreat_specDisDT, 'YYYY-MM-DD HH24:MI:SS') as MorbusOnkoSpecTreat_specDisDT
					from v_MorbusOnkoSpecTreat 
					WHERE MorbusOnko_id = :MorbusOnko_id
					AND MorbusOnkoSpecTreat_id <> :MorbusOnkoSpecTreat_id
					{$filter}
				)
				
				SELECT MorbusOnkoSpecTreat_specSetDT as \"MorbusOnkoSpecTreat_specSetDT\", MorbusOnkoSpecTreat_specDisDT as \"MorbusOnkoSpecTreat_specDisDT\"
				FROM SpecTreat
			...
			if (!empty($this->specDisDT)) {
				$params['specDisDT'] = ConvertDateFormat($this->specDisDT, 'Y-m-d');
				$query .= "
				union all
				SELECT MorbusOnkoSpecTreat_specSetDT as \"MorbusOnkoSpecTreat_specSetDT\", MorbusOnkoSpecTreat_specDisDT as \"MorbusOnkoSpecTreat_specDisDT\"
				FROM SpecTreat
	...
	function getViewData($data)
	{
		...
		$query = "
			SELECT
				...
				,MOST.MorbusOnkoSpecTreat_id as \"MorbusOnkoSpecTreat_id\"
				,MO.MorbusOnko_id as \"MorbusOnko_id\"
				,to_char(MOST.MorbusOnkoSpecTreat_specSetDT, 'DD.MM.YYYY') as \"MorbusOnkoSpecTreat_specSetDT\"
				,to_char(MOST.MorbusOnkoSpecTreat_specDisDT, 'DD.MM.YYYY') as \"MorbusOnkoSpecTreat_specDisDT\"
				...
				,MOST.MorbusOnkoVizitPLDop_id as \"MorbusOnkoVizitPLDop_id\"
				,MOST.MorbusOnkoLeave_id as \"MorbusOnkoLeave_id\"
				,MOST.MorbusOnkoDiagPLStom_id as \"MorbusOnkoDiagPLStom_id\"
			FROM
				dbo.v_Morbus Morbus 
				INNER JOIN dbo.v_MorbusOnko MO  on Morbus.Morbus_id = MO.Morbus_id
				INNER JOIN dbo.v_MorbusOnkoSpecTreat MOST  on MO.MorbusOnko_id = MOST.MorbusOnko_id
				left join dbo.v_TumorPrimaryTreatType TPTT  ON MOST.TumorPrimaryTreatType_id = TPTT.TumorPrimaryTreatType_id
				left join dbo.v_OnkoCombiTreatType OCTT  ON MOST.OnkoCombiTreatType_id = OCTT.OnkoCombiTreatType_id
				left join dbo.v_OnkoSpecTreatType OSTT  ON MOST.OnkoSpecTreatType_id = OSTT.OnkoSpecTreatType_id
				left join dbo.v_TumorRadicalTreatIncomplType TRTIT  ON MOST.TumorRadicalTreatIncomplType_id = TRTIT.TumorRadicalTreatIncomplType_id
			where
				{$filter}
		";
		$params = array(
	...
	function doLoadEditForm($data)
	{
		...
		return array(array(
			...
			'MorbusOnkoVizitPLDop_id' => $this->MorbusOnkoVizitPLDop_id,
			'MorbusOnkoDiagPLStom_id' => $this->MorbusOnkoDiagPLStom_id,
			$this->tableName() . '_specSetDT' => ConvertDateFormat($this->specSetDT, 'd.m.Y'),
			$this->tableName() . '_specDisDT' => !empty($this->specDisDT) ? ConvertDateFormat($this->specDisDT, 'd.m.Y') : null,
	...
	function deleteMorbusOnkoSpecTreat($data)
	{
		...
		$query = "
			...
			from p_MorbusOnkoSpecTreat_del(
	...
	function loadDisabledDates($morbusonko_id, $id = null, $morbus_id = null, $morbusonkovizitpldop_id = null, $morbusonkoleave_id = null, $morbusonkodiagplstom_id = null)
	{
		...
		if ($morbusonkovizitpldop_id) {
			...
			$query = "
				SELECT MorbusOnkoSpecTreat_specSetDT as \"MorbusOnkoSpecTreat_specSetDT\", MorbusOnkoSpecTreat_specDisDT as \"MorbusOnkoSpecTreat_specDisDT\"
				from v_MorbusOnko 
				inner join v_MorbusOnkoSpecTreat  on v_MorbusOnkoSpecTreat.MorbusOnko_id = v_MorbusOnko.MorbusOnko_id
		...
		foreach($tmp as $row) {
			...
			if (!empty($row['MorbusOnkoSpecTreat_specSetDT'])) {
				$start = date_create($row['MorbusOnkoSpecTreat_specSetDT']);
				$response[] = $start->format('d.m.Y');
			}
			if (!empty($row['MorbusOnkoSpecTreat_specDisDT'])) {
				$end = date_create($row['MorbusOnkoSpecTreat_specDisDT']);
			}
			if($end !== null){
	...
	function loadOnkoLateComplTreatTypeList($data)
	{
		...
		$query = "
			SELECT
				MorbusOnkoSpecTreatLink_id as \"MorbusOnkoSpecTreatLink_id\"
				,OnkoLateComplTreatType_id as \"OnkoLateComplTreatType_id\"
			FROM
				dbo.v_MorbusOnkoSpecTreatLink 
			where
				MorbusOnkoSpecTreat_id = :MorbusOnkoSpecTreat_id
		";
		$params = array(
	...
	function deleteOnkoLateComplTreatTypeList($data)
	{
		...
		$query = "
			select 
				Error_Code as \"Error_Code\",
				Error_Message as \"Error_Msg\"
			from p_MorbusOnkoSpecTreatLink_del(
	...
	function saveOnkoLateComplTreatTypeList($data)
	{
		...
		$query = "
			select 
				...
				Error_Code as \"Error_Code\",
				Error_Message as \"Error_Msg\"
			from p_MorbusOnkoSpecTreatLink_ins(
				MorbusOnkoSpecTreat_id := :MorbusOnkoSpecTreat_id,
