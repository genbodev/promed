...
class CmpCallCardImport_model extends swPgModel
{
	...
	function getCmpResult($id)
	{
		...
		return $this->getFirstResultFromQuery("
			select CRL.CmpCloseCardCombo_id as \"CmpCloseCardCombo_id\"
			from v_CmpResultLink CRL
	...
	function getSocStatus($id)
	{
		if (empty($id)) return null;
		return $this->getFirstResultFromQuery("
			select CmpCloseCardCombo_id as \"CmpCloseCardCombo_id\"
			from v_CmpSocStatusLink
	...
	{
		return $this->getFirstResultFromQuery("
			select lvl1.KLArea_id as \"KLArea_id\"
			from dbo.KLArea lvl4
			inner join dbo.KLArea lvl2 on lvl2.KLArea_id = lvl4.KLArea_pid and lvl2.KLAreaLevel_id = 2
			inner join dbo.KLArea lvl1 on lvl1.KLArea_id = lvl2.KLArea_pid and lvl1.KLAreaLevel_id = 1
	...
	{
		return $this->getFirstResultFromQuery("
			select KLStreet_id as \"KLStreet_id\"
			from dbo.KLStreet
	...
	function saveCmpCloseCard($cccdata)
	{
		...
		$CmpCloseCard_id = $this->getFirstResultFromQuery("
			select CmpCloseCard_id as \"CmpCloseCard_id\" from v_CmpCloseCard where CmpCallCard_id = :CmpCallCard_id limit 1
		", array(
			'CmpCallCard_id' => $cccdata['CmpCallCard_id']
		));
	...
	function findPerson($CallCard)
	{
		...
		$result = $this->queryResult("
			...
			where 
				...
				{$secname_sql} and
				Sex_id = :Sex_id and
				extract(year from Person_Birthday::date) = extract(year from :Person_Birthday::date) and
				Polis_Num = :Polis_Num
		", array(
			'Person_SurName' => $CallCard->Pacient_Fam,
			'Person_FirName' => isset($CallCard->Pacient_Name) ? $CallCard->Pacient_Name : null,
		...
		$result = $this->queryResult("
			...
			where 
				...
				{$secname_sql} and
				Sex_id = :Sex_id and
				extract(year from Person_Birthday::date) = extract(year from :Person_Birthday::date)
		", array(
			'Person_SurName' => $CallCard->Pacient_Fam,
			'Person_FirName' => isset($CallCard->Pacient_Name) ? $CallCard->Pacient_Name : null,
			'Person_SecName' => isset($CallCard->Pacient_Patronomyc) ? $CallCard->Pacient_Patronomyc : null,
	...
	function findBrigadaHead($BrigadaHead, $Lpu_id)
	{
		...
		return $this->getFirstRowFromQuery("
			...
			from v_MedStaffFact 
			where {$where}
			and cast(WorkData_begDate as date) <= now() and (cast(WorkData_endDate as date) >= now() or WorkData_endDate is null)
			limit 1
		", array(
