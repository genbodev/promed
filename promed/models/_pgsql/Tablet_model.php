...
class Tablet_model extends Scenario_model
{
	...
	function doLoadCombo($data = array())
	{
		...
		$query = "
			select
				MITT.MethodsIFATabletType_id as \"MethodsIFATabletType_id\",
				TT.TabletType_id as \"TabletType_id\",
				TT.TabletType_Code as \"TabletType_Code\",
				TT.TabletType_VertSize as \"TabletType_VertSize\",
				TT.TabletType_HorizSize as \"TabletType_HorizSize\"
			from v_TabletType TT
				inner join v_MethodsIFATabletType MITT on TT.TabletType_id = MITT.TabletType_id
			where (1=1) {$where}
		";
	...
	function doLoadData($data = array()) {
		...
		if(!empty($data['AnalyzerTest_id'])) {
			...
			$join .= 'inner join v_MethodsIFAAnalyzerTest MIAT on MIAT.MethodsIFA_id = MI.MethodsIFA_id
				and MIAT.AnalyzerTest_id = :AnalyzerTest_id';
		}
		...
		$query = "
			select
				...
				T.Tablet_HoleCount as \"Tablet_HoleCount\",
				T.Tablet_VertSize as \"Tablet_VertSize\",
				T.Tablet_HorizSize as \"Tablet_HorizSize\",
				T.Tablet_Barcode as \"Tablet_Barcode\",
				T.Tablet_IsHorizFill as \"Tablet_IsHorizFill\",
				T.Tablet_defectDT as \"Tablet_defectDT\",
				T.Tablet_IsDoublesFill as \"Tablet_IsDoublesFill\",
				MI.MethodsIFA_Name as \"MethodsIFA_Name\",
				MI.MethodsIFA_id as \"MethodsIFA_id\",
				emptyHoles.emptyHolesCount as \"emptyHolesCount\",
				case
			...
			from v_Tablet T
				inner join v_MethodsIFATabletType MITT on MITT.MethodsIFATabletType_id = T.MethodsIFATabletType_id
				inner join v_MethodsIFA MI on MI.MethodsIFA_id = MITT.MethodsIFA_id
				{$join}
				left join lateral(
					select
						Hole_id
					from v_Hole H
					left join v_UslugaTest UT on UT.UslugaTest_id = H.UslugaTest_id
					where
				...
				left join lateral(
					...
					where
						...
						and T.Tablet_defectDT is null
						and coalesce(UslugaTest_ResultValue,'') <> ''
					limit 1
				) withResult on true
				left join lateral(
					select
				...
				left join lateral(
					select
						count(Hole_id) as emptyHolesCount
					from v_Hole
					where
	...
	public function doLoadEditForm($data = array()) {
		...
		$query = "
			select
				...
				T.Tablet_IsHorizFill as \"Tablet_IsHorizFill\",
				T.Tablet_IsDoublesFill as \"Tablet_IsDoublesFill\",
				T.Tablet_IsTestTablet as \"Tablet_IsTestTablet\",
				T.Tablet_Barcode as \"Tablet_Barcode\",
				MITT.MethodsIFATabletType_id as \"MethodsIFATabletType_id\",
				...
				T.Tablet_defectDT as \"Tablet_defectDT\",
				T.Tablet_Comment as \"Tablet_Comment\",
				MI.MethodsIFA_id as \"MethodsIFA_id\",
				case
					when coalesce(T.Tablet_IsTestTablet,1) = 2 then 3
					when coalesce(T.Tablet_IsDoublesFill,1) = 2 then 2
					else 1
			...
			from v_Tablet T
				left join v_MethodsIFATabletType MITT on MITT.MethodsIFATabletType_id = T.MethodsIFATabletType_id
				left join v_MethodsIFA MI on MI.MethodsIFA_id = MITT.MethodsIFA_id
				left join v_TabletType TT on TT.TabletType_id = MITT.TabletType_id
