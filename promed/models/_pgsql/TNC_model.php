...
class TNC_model extends swPgModel {
	/**
	...
	*/
	public function mergeEmergencyTeam($data) {
		...
		if (empty($queryParams['EmergencyTeamTNCRel_id'])) {
			$sql = "
				SELECT
					EmergencyTeamTNCRel_id as \"EmergencyTeamTNCRel_id\"
				FROM v_EmergencyTeamTNCRel
		...
		$query = "
			...
			from {$procedure}(
				EmergencyTeamTNCRel_id := :EmergencyTeamTNCRel_id,
		...
		if($filter){
			...
			$sql = "
				...
				FROM
					...
					LEFT JOIN v_ApiServiceType ast ON ms.ApiServiceType_id = ast.ApiServiceType_id
				WHERE
					".$filter."
				limit 1
			";
