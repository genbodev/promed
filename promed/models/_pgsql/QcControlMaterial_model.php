...
class QcControlMaterial_model extends Scenario_model
{
	...
	function doLoadData($data = array())
	{
		...
		if (!empty($data['QcControlMaterialValue_id'])) {
			...
			$join .= 'LEFT JOIN lis.QcControlMaterialValue QCMV ON QCMV.QcControlMaterial_id = QCM.QcControlMaterial_id';
		}

		if (!empty($data['QcControlMaterial_Name'])) {
			$params['QcControlMaterial_Name'] = $data['QcControlMaterial_Name'];
			$where .= " and QCM.QcControlMaterial_Name ilike '%'||:QcControlMaterial_Name||'%'";
		}
		...
		$query = "
			SELECT
				...
				QCM.QcControlMaterial_LotNum as \"QcControlMaterial_LotNum\",
				QCM.QcControlMaterial_CatalogNum as \"QcControlMaterial_CatalogNum\",
				case when QCM.QcControlMaterial_IsAttested = 2
					then 'true'
					else 'false'
				end as \"QcControlMaterial_IsAttested\",
				to_char(QcControlMaterial_ExpDate,'DD.MM.YYYY') as \"QcControlMaterial_ExpDate\",
				to_char(QCM.QcControlMaterial_begDT, 'DD.MM.YYYY') as \"QcControlMaterial_begDT\",
				to_char(QCM.QcControlMaterial_endDT,'DD.MM.YYYY') as \"QcControlMaterial_endDT\",
				QCM.QcControlMaterialType_id as \"QcControlMaterialType_id\",
				QCMT.QcControlMaterialType_Name as \"QcControlMaterialType_Name\"
			FROM lis.QcControlMaterial QCM
			LEFT JOIN lis.QcControlMaterialType QCMT ON QCMT.QcControlMaterialType_id = QCM.QcControlMaterialType_id
			{$join}
			WHERE QCM.Lpu_id = :Lpu_id {$where}
			ORDER BY QCM.QcControlMaterial_endDT desc, QCM.QcControlMaterial_insDT desc
		";
	...
	function _validateBeforeDisable()
	{
		...
		$query = "
			...
			WHERE QCM.QcControlMaterial_id = :QcControlMaterial_id
				...
				and (QMV.QcControlMaterialValue_endDT is null or (QCS.QcControlSeries_id is not null and QCS.QcControlSeries_endDT is null))
		";
