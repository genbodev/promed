 ...
 */
class EvnXmlBase_model extends swPgModel
{
	/**
	 * @var bool
	 */
	...
	static function defAttributes()
	{
		...
		$arr['xmldata'] = array(
			...
			'join' => 'left join XmlTemplateData xtd on xtd.XmlTemplateData_id = {ViewName}.XmlTemplateData_id',
		);
		...
		$arr['htmltemplate'] = array(
			...
			'join' => 'left join XmlTemplateHtml xth on xth.XmlTemplateHtml_id = {ViewName}.XmlTemplateHtml_id',
		);
		...
		$arr['printsettings'] = array(
			...
			'join' => 'left join XmlTemplateSettings xts on xts.XmlTemplateSettings_id = {ViewName}.XmlTemplateSettings_id',
		);
	...
	public function getEvnClassId()
	{
		if (empty($this->Evn_id)) {
			...
			select EvnClass_id  as "EvnClass_id" from Evn where Evn_id = :id
	...
	public function getFromEvnId()
	{
		if (empty($this->Evn_id)) {
			...
			$this->_From_Evn_id = $this->getFirstResultFromQuery("
				select Evn.Evn_pid as \"Evn_pid\"
				from EvnUsluga
				inner join Evn on Evn.Evn_id = EvnUsluga.EvnDirection_id
				where EvnUsluga.Evn_id = :id
			", array(
				'id' => $this->Evn_id
			), true);
	...
	protected function _beforeDelete($data = array())
	{
		...
		if (!empty($this->id)) {
			...
			$query = "
				update EvnDirectionOper set EvnXml_id = null where EvnXml_id = :EvnXml_id
			";
			...
			$query = "
				Select EvnXmlDirectionLink_id  as \"EvnXmlDirectionLink_id\" From v_EvnXmlDirectionLink where EvnXml_id = :EvnXml_id
			...
			if (!empty($result))
			{
				...
				{
					$this->db->query("
						SELECT dbo.p_EvnXmlDirectionLink_del(
	...
	function doLoadEvnXmlPanel($data)
	{
		...
		if ( !empty($data['EvnXml_id']) ) {
			...
			$from = "v_EvnXml EvnXml
					{$joinType} join v_Evn Evn on Evn.Evn_id = EvnXml.Evn_id";
		...
		$query = "
			select
				...
				EvnXml.XmlTemplate_id as \"XmlTemplate_id\",
				EvnXml.XmlTemplateType_id as \"XmlTemplateType_id\",
				EvnXml.EvnXml_IsSigned as \"EvnXml_IsSigned\",
	...
	function loadListViewData($data) {
		...
		$query = "
			select
				...
				EvnXml.EvnXml_Name as \"EvnXml_Name\",
				EvnXml.EvnXml_IsSigned as \"EvnXml_IsSigned\",
				to_char (EvnXml.EvnXml_insDT, 'dd.mm.yyyy') as \"EvnXml_Date\",
				EvnXml.pmUser_insID as \"pmUser_insID\",
				RTRIM(LTRIM(COALESCE(pmUserCache.pmUser_Name, ''))) as \"pmUser_Name\",
				{$frame} as \"frame\",
				to_char(EvnXml.EvnXml_setDT, 'DD.MM.YYYY HH24:MI') as \"EvnXml_setDT\",
				to_char(EvnXml.EvnXml_setDT, 'DD.MM.YYYY') as \"EvnXml_setDTDate\",
				to_char(EvnXml.EvnXml_setDT, 'HH24:MI') as \"EvnXml_setDTTime\"
			from
				v_EvnXml EvnXml 
	...
	function doLoadPrintData($data)
	{
		...
		if ( !empty($data['EvnXml_id']) ) {
			...
			$params = array('EvnXml_id' => $data['EvnXml_id']);
			$from = 'v_EvnXml EvnXml
					left join v_Evn Evn on Evn.Evn_id = EvnXml.Evn_id';
		} else if ( !empty($data['Evn_id']) ) {
			$filters = 'Evn.Evn_id = :Evn_id';
		...
		$query = "
			...
			from {$from}
				left join XmlTemplateData xtd  on xtd.XmlTemplateData_id = EvnXml.XmlTemplateData_id
				left join XmlTemplateHtml xth  on xth.XmlTemplateHtml_id = EvnXml.XmlTemplateHtml_id
			LEFT JOIN LATERAL ( -- временная заглушка для карелии, для нее печать документов идет с настройками ШАБЛОНА документа, а не настройками самого документа #117830
				SELECT 
					CASE :region
						WHEN 'kareliya' THEN (SELECT XmlTemplateSettings_id  FROM v_XmlTemplate WHERE XmlTemplate_id = EvnXml.XmlTemplate_id LIMIT 1)
						ELSE NULL
					END as XmlTemplateSettings_id
               	LIMIT 1
			) XT ON true
				left join XmlTemplateSettings xts  on xts.XmlTemplateSettings_id = COALESCE(XT.XmlTemplateSettings_id, EvnXml.XmlTemplateSettings_id)
			where {$filters}
            limit 1
		";
		// echo getDebugSQL($query, $params);
	...
	private function _loadParameterData($fieldName)
	{
		...
		$query = "
			...
			from
				v_ParameterValue P 
				left join v_ParameterValueListType PT  on P.ParameterValueListType_id = PT.ParameterValueListType_id
			where
		...
		UNION ALL
			...
			from
				v_ParameterValue P
				inner join v_ParameterValue Val on Val.ParameterValue_pid = P.ParameterValue_id
			where
				P.ParameterValue_SysNick = :fieldName
			order by
				P.ParameterValue_id
	...
	public function loadXmlDataSectionList($data)
	{
		$sql = "
			SELECT
				XmlDataSection_id as \"XmlDataSection_id\",
				XmlDataSection_Code as \"XmlDataSection_Code\",
				XmlDataSection_SysNick as \"XmlDataSection_SysNick\",
				XmlDataSection_Name as \"XmlDataSection_Name\"
			FROM
				dbo.v_XmlDataSection
		";
	...
	public function getLastEvnProtocolId($data)
	{
		$sql = "
			select
				evn.Evn_id as \"Evn_id\",
				evnx.EvnXml_id as \"EvnXml_id\"
			from
				v_Evn evn
				inner join v_EvnXml evnx on evn.Evn_id = evnx.Evn_id
			where
				evn.Evn_rid = ?
				and evn.EvnClass_id = ?
				and evnx.XmlType_id = ?
	...
	public function getXmlTemplateInfo($data)
	{
		...
		$sql = "
			...
			FROM
				dbo.v_EvnXml doc
				inner join dbo.v_XmlTemplate xt  on xt.XmlTemplate_id = doc.XmlTemplate_id
				left join dbo.v_XmlTemplateCat xtc  on xt.XmlTemplateCat_id = xtc.XmlTemplateCat_id
		...
		switch ($type) {
			...
			case 2: // Маркер одного документа
				...
				$query_templ = "
					WITH table1 AS (
						...
						FROM
							v_Evn Evn 
							inner join v_Evn EvnR  on EvnR.Evn_id = Evn.Evn_rid --родитель (если приемное в КВС)
							inner join v_Evn EvnAll  on EvnAll.Evn_rid = EvnR.Evn_id -- все события случая
							left join v_EvnSection ES  on ES.EvnSection_rid = EvnR.Evn_id and ES.EvnSection_IsPriem = 2 --приёмное (как отдельное движение)
							inner join lateral (
								select *
								from v_EvnXml EvnXml
								where
					...
					SELECT 
						table1.marker as \"marker\",
						table1.Evn_id as \"Evn_id\",
						table1.EvnClass_id as \"EvnClass_id\",
						table1.EvnXml_id as \"EvnXml_id\",
						table1.XmlType_id as \"XmlType_id\",
						EvnXml.EvnXml_Name as \"EvnXml_Name\",
						EvnXml.EvnXml_Data as \"EvnXml_Data\", 
						table1.XmlTemplateType_id as \"XmlTemplateType_id\",
						table1.EvnXml_Index as \"EvnXml_Index\",
						xts.XmlTemplateSettings_Settings as \"XmlTemplate_Settings\",
						xth.XmlTemplateHtml_HtmlTemplate as \"XmlTemplate_HtmlTemplate\",
						xtd.XmlTemplateData_Data as \"XmlTemplate_Data\"
					FROM table1 
					inner join v_EvnXml AS EvnXml ON table1.EvnXml_id = EvnXml.EvnXml_id
					left join XmlTemplateData xtd  on xtd.XmlTemplateData_id = table1.XmlTemplateData_id
					left join XmlTemplateHtml xth  on xth.XmlTemplateHtml_id = table1.XmlTemplateHtml_id
					left join XmlTemplateSettings xts  on xts.XmlTemplateSettings_id = table1.XmlTemplateSettings_id
					left join v_EvnXml CurrEvnXml  on CurrEvnXml.EvnXml_id = :EvnXml_id
					where 1=1
					--add_where
				";
				$query_list = array();
				foreach ($markers as $i => $marker) {
					...
					if ($type == 2) {
						...
						if ('last' == $marker['XmlDataSelectType_SysNick']) {
							$add_where = 'and (
								(coalesce(CurrEvnXml.XmlType_id, 0) <> EvnXml.XmlType_id and EvnXml.EvnXml_Index = (EvnXml.EvnXml_Count - 1)) or 
								(coalesce(CurrEvnXml.XmlType_id, 0) = EvnXml.XmlType_id and EvnXml.EvnXml_Index = (
			...
			case 3: // Маркер одного раздела документа
				...
				$query_templ = "
                    ...
                    from (
                    	select
							@index as marker,
							EvnXml.EvnXml_id,
							cast (unnest(xpath('data/@XmlDataSection_SysNick/text()', EvnXml.EvnXml_Data)) as text) as value
                    	from v_Evn Evn 
							inner join v_Evn EvnR  on EvnR.Evn_id = Evn.Evn_rid
							inner join v_Evn EvnAll  on EvnAll.Evn_rid = EvnR.Evn_id
							left join v_EvnSection ES  on ES.EvnSection_rid = EvnR.Evn_id and ES.EvnSection_IsPriem = 2
							inner join v_EvnXml EvnXml  on Evn.Evn_id = EvnXml.Evn_id --level_filter
							left join v_EvnXml CurrEvnXml  on CurrEvnXml.EvnXml_id =:EvnXml_id
                    	where Evn.Evn_id=:Evn_id
							and EvnXml.XmlType_id=@XmlType_id
							and (CurrEvnXml.EvnXml_id is null or EvnXml.EvnXml_insDT < CurrEvnXml.EvnXml_insDT)
							--add_where --add_orderby 
                    	limit 1
                    ) as t
				";
				$query_list = array();
				foreach ($markers as $i => $marker) {
					...
					if ('firstused' == $marker['XmlDataSelectType_SysNick']) {
						$add_where = "and COALESCE(cast(xpath('data/{$marker['XmlDataSection_SysNick']}', EvnXml.EvnXml_Data) as text), '') != ''";
						$add_orderby = ' order by EvnXml.EvnXml_Index asc';
					}
			...
			case 12: // Маркер списка протоколов услуг по коду ГОСТ-2011
				...
				foreach ($markers as $i => $marker) {
					...
					$EvnUslugaList_str = implode(" union\n", array_map(function($EvnUsluga) {
						return "select cast({$EvnUsluga['EvnUsluga_id']} as bigint) as EvnUsluga_id, cast({$EvnUsluga['EvnClass_id']} as bigint) as EvnClass_id";
					}, $EvnUslugaList));
			...
			case 13: // Маркер одного раздела протоколов услуг по коду ГОСТ-2011
				$query_templ = "
					...
					select
						cast(xpath(doc.EvnXml_Data,'data/@XmlDataSection_SysNick/text()') as text) as \"value\"
					from 
						usluga_list u
						inner join lateral (
							select *
	...
	function getEvnUslugaList($data) {
		...
		if (!empty($data['XmlDataLevel_SysNick']) && $data['XmlDataLevel_SysNick'] == 'priem') {
			$EvnSection_id = $this->getFirstResultFromQuery("
				select ES.EvnSection_id as \"EvnSection_id\"
				from v_Evn Evn
				inner join v_EvnSection ES on ES.EvnSection_rid = Evn.Evn_rid and ES.EvnSection_IsPriem = 2
	...
	function loadEvnXmlPanel($data)
	{
		...
		return $this->queryResult("
			select
				...
				to_char (ex.EvnXml_insDT, 'dd.mm.yyyy') as \"EvnXml_Date\",
				to_char (ex.EvnXml_insDT, 'HH24:MI') as \"EvnXml_Time\",
				COALESCE(pu.pmUser_Name, '') as \"pmUser_Name\",
				ex.XmlTemplate_id as \"XmlTemplate_id\",
				ex.EvnXml_IsSigned as \"EvnXml_IsSigned\"
			from
				v_EvnXml ex
				left join v_pmUser pu on pu.pmUser_id = ex.pmUser_insID
			where
				ex.Evn_id = :Evn_id
				{$filter}
		", $data);
	...
	function getEvnXmlForAPI($data) {
		...
		$query = "
			select
				...
				XTH.XmlTemplateHtml_HtmlTemplate as \"XmlTemplateHtml_HtmlTemplate\",
				EX.XmlTemplate_id as \"XmlTemplate_id\"
			from
				v_EvnXml EX 
				inner join v_Evn E  on E.Evn_id = EX.Evn_id
	...
	function createEvnXmlDirectionLink($data)
	{
		$query = "
			...
			from(
				...
				FROM
					v_EvnXmlDirectionLink
				WHERE
					EvnDirection_id = :EvnDirection_id
					AND EvnXml_id = :EvnXml_id
				LIMIT 1)
		...
		$query = "
			select 
            	EvnXmlDirectionLink_id as \"EvnXmlDirectionLink_id\",     
                Error_Code as \"Error_Code\", 
                Error_Message as \"Error_Msg\"
			from dbo.p_EvnXmlDirectionLink_ins( 
				EvnXml_id := :EvnXml_id,
				EvnDirection_id := :EvnDirection_id,
				pmUser_id := :pmUser_id
			);
	...
	function deleteEvnXmlDirectionLink($data)
	{
		$query = "
			select 
            	Error_Code as \"Error_Code\", 
                Error_Message as \"Error_Msg\"
			FROM dbo.p_EvnXmlDirectionLink_del(
				EvnXmlDirectionLink_id := :EvnXmlDirectionLink_id)
		";
	...
	function getEvnXmlForEvnDirectionList($EvnDirection_id)
	{
		...
		$query = "
			select
				...
				0 as \"frame\",
				1 as \"readOnly\"
			from v_EvnXmlDirectionLink EXDL 
