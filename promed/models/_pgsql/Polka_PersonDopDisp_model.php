...
class Polka_PersonDopDisp_model extends SwPgModel {
	/**
		...
		if ( isSuperadmin() ) {
			$sql = "
				select
					...
					rtrim(coalesce(ps.Person_SecName, '')) as \"SECNAME\",
					sx.Sex_Code as \"SEX\",
					rtrim(coalesce(to_char (cast(ps.Person_BirthDay as timestamp(3)),'dd.mm.yyyy'),'')) as \"BIRTHDAY\",
					rtrim(coalesce(addr.Address_Address, '')) as \"ADDR\",
					rtrim(coalesce(Polis.Polis_Ser, '')) as \"POL_SER\",
					rtrim(coalesce(Polis.Polis_Num, '')) as \"POL_NUM\",
					rtrim(coalesce(ps.Person_Snils, '')) as \"SNILS\",
					rtrim(coalesce(okvd.Okved_Code, '')) as \"O_OKVED\",
					rtrim(coalesce(og.Org_OGRN, '')) as \"ORG_OGRN\",
					OST.OMSSprTerr_Code as \"TERR_ID_A\",
					coalesce(OMSSprTerr3.OMSSprTerr_Code, OMSSprTerr3.OMSSprTerr_Code) as \"TERR_ID_B\",
				...
				from
					...
					LEFT JOIN LATERAL (
						Select PersonState.Polis_id as Polis_id
						from v_EvnVizitDispDop EVDD
						inner join v_Person_all PersonState on PersonState.PersonEvn_id = EVDD.PersonEvn_id and PersonState.Server_id = EVDD.Server_id
						where ps.Person_id = EVDD.Person_id and DopDispSpec_id = 1 and pdd.PersonDopDisp_Year = extract(year from EVDD.EvnVizitDispDop_setDate) and pdd.Lpu_id = EVDD.Lpu_id
						order by EVDD.EvnVizitDispDop_setDate desc
						limit 1
					) as EvnVizitTera ON TRUE
					left join v_Polis Polis on Polis.Polis_id = coalesce(EvnVizitTera.Polis_id, ps.Polis_id)
					left join Sex sx on sx.Sex_id = ps.Sex_id
					left join Address addr on ps.PAddress_id = addr.Address_id
					left join Address uaddr on ps.UAddress_id = uaddr.Address_id
					...
					left join OMSSprTerr OMSSprTerr3 on
					...
					left join Address org_addr on og.UAddress_id = org_addr.Address_id				
					left join OMSSprTerr OST on OST.OMSSprTerr_id = Polis.OMSSprTerr_id
					left join v_OrgSMO osmo on osmo.OrgSMO_id = Polis.OrgSMO_id
			...
			$sql = "
				select
					...
					rtrim(coalesce(ps.Person_SecName, '')) as \"SECNAME\",
					rtrim(coalesce(to_char (cast(ps.Person_BirthDay as timestamp(3)),'dd.mm.yyyy'),'')) as \"BIRTHDAY\",
					coalesce(sx.Sex_Code, 0) as \"SEX\",
					coalesce(os.OrgSMO_Name, '') as \"SMO_NAME\",
					rtrim(coalesce(Polis.Polis_Ser, '')) as \"POLIS_S\",
					rtrim(coalesce(Polis.Polis_Num, '')) as \"POLIS_N\",
					rtrim(coalesce(ps.Person_Snils, '')) as \"SNILS\",
					rtrim(coalesce(og.Org_Name, '')) as \"O_NAME\",
					rtrim(coalesce(og.Org_INN, '')) as \"O_INN\",
					rtrim(coalesce(og.Org_OGRN, '')) as \"O_OGRN\",
					rtrim(coalesce(addr.Address_Address, '')) as \"O_ADDR\",
					rtrim(coalesce(okvd.Okved_Code, '')) as \"O_OKVED\",
				...
				from
					...
					left join Job jb on jb.Job_id = ps.Job_id
					left join Org og on og.Org_id = jb.Org_id
					left join Address addr on addr.Address_id = og.UAddress_id
					left join Okved okvd on okvd.Okved_id = og.Okved_id
					...
					LEFT JOIN LATERAL (
						...
						where
							...
							and extract(year from EvnPLDispDop_setDate) = pdd.PersonDopDisp_Year
                        limit 1
					) as epldd ON TRUE
				where
					pdd.PersonDopDisp_Year = :PersonDopDisp_Year
	...
	*/
	function getPersonDopDispStreamInputList($data)
	{
		$sql = "
			SELECT DISTINCT
				v_PersonDopDisp.PersonDopDisp_id as \"PersonDopDisp_id\",
				...
				to_char(cast(v_PersonState.Person_BirthDay as timestamp(3)),'dd.mm.yyyy') as \"Person_BirthDay\"
	...
	*/
	function getPersonDopDispList($data)
	{
		...
		if (ArrayVal($data, 'PersonAge_From') != '' && ArrayVal($data, 'PersonAge_To') != '' )
			...
			+ case when date_part('month',v_PersonState.Person_BirthDay::date)>date_part('month',dbo.tzGetDate()::date)
			or (date_part('month',v_PersonState.Person_BirthDay::date)=date_part('month',dbo.tzGetDate::date)) and date_part('day',v_PersonState.Person_BirthDay::date)>date_part('day',dbo.tzGetDate()::date)
	...
	function getPersonDopDispYearsCombo($data)
	{
  		$sql = "
			SELECT
				...
				PersonDopDisp_Year as \"PersonDopDisp_Year\"
			FROM
				v_PersonDopDisp
			WHERE
				Lpu_id = {$data['Lpu_id']}
	...
	function getYearInOldRegistry($data)
	{
		...
		$sql = '
			...
			from
				v_PersonState pst
				inner join PersonDopDispRegOld pdd on pdd.PersonDopDisp_Year in (2009, 2010, 2011)
		...
		$sql = '
			...
			FROM
				v_PersonState pst
				inner join PersonDopDispRegOld pddro on pddro.PersonDopDisp_Year in (2006, 2007, 2008)
				...
				inner join v_PersonDisp pd on pd.Person_id = pst.Person_id and pd.PersonDisp_begDate between INTERVAL \'-31 "Day"\' + pddro.DKL and pddro.DKL
			WHERE
				pst.Person_id = ?
		';				
	...
	function deletePersonDopDisp($data)
	{
		...
		$sql = "
			...
			from
				...
				inner join v_PersonDopDisp pdd on pdd.PersonDopDisp_id = ? and pdd.Person_id = epldd.Person_id and extract(year from epldd.EvnPLDispDop_setDate) = pdd.PersonDopDisp_Year and epldd.Lpu_id = pdd.Lpu_id
		";
