...
class InnovaSysService_Model extends SwPgModel
{
    private $usCCode;
	...
	function makeRequest($data)
	{
		...
		foreach ($samplesData as $oneSampleData) {
			...
			foreach ($targetsData as $oneTargetData) {
				...
				$code = $this->common->GET('InnovaSysService/AISTargetCode', [
					'Code' => $oneTargetData['Code'],
					'Barcode' => $oneSampleData['Barcode']
				], 'single');
	...
	function getAISTargetCode($data, $barcode)
	{
		...
		from v_AttributeValue av
			inner join v_AttributeValue avpid on avpid.AttributeValue_rid = av.AttributeValue_id
				OR avpid.AttributeValue_id = av.AttributeValue_id
			left join v_Attribute a on avpid.Attribute_id = a.Attribute_id
		where (1 = 1)
			...
			and av.AttributeValue_id in (
				...
				from v_AttributeValue av
					inner join v_AttributeValue avpid on avpid.AttributeValue_rid = av.AttributeValue_id OR
				...
				where (1 = 1)
					...
					and (case
						...
						when a.AttributeValueType_id = 7
							then to_char(avpid.AttributeValue_ValueDate, 'dd.mm.yyyy')
		...
		$hosp = $this->queryResult("
			with mv as (			
				...
				from v_EvnLabSample els
					left join v_MedService on ms.MedService_id = els.MedService_id
	...
	function getPatient($data)
	{
		...
		$res = [];
		if (!empty($person_id)) {
			$res = $this->common->GET('Person/PersonForInnova', [
				'Person_id' => $person_id
			], 'single');
	...
	function getSamples($data)
	{
		...
		from v_EvnLabSample evl
			left join v_RefSample rs on rs.RefSample_id = evl.RefSample_id
			left join v_RefMaterial rm on rm.RefMaterial_id = rs.RefMaterial_id
			left join v_EvnLabRequest elr on elr.EvnLabRequest_id = evl.EvnLabRequest_id
			left join v_EvnDirection evn on evn.EvnDirection_id = elr.EvnDirection_id
		where
        ...
        $query = "
			...
			from
				...
				left join lateral (
					...
					from
						v_EvnUslugaPar eup
						left join v_LpuSection euls on eup.LpuSection_did = euls.LpuSection_id
					where 
	...
	function saveSampleResult($data)
    {
		...
		if (!empty($resp[0]['EvnLabSample_id'])) {
			...
			$tests = $this->common->GET('InnovaSysService/Tests', $data, 'list');
			if (!$this->isSuccessful($tests)) {
				return $tests;
			}
    ...
    function getCodeFromAISCode($data)
    {
        ...
        $query = "
            ...
            and cast(dbo.tzgetdate() as date) >= coalesce(av.AttributeValue_begDate, cast(dbo.tzgetdate() as date))
            and cast(dbo.tzgetdate() as date) <= coalesce(av.AttributeValue_endDate, cast(dbo.tzgetdate() as date))
