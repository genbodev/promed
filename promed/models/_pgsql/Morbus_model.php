...
class Morbus_model extends Abstract_model
{
		...
		if (null === $this->Evn_id) {
			...
			if ($this->Morbus_id) {
				...
				$cnt = $this->getFirstResultFromQuery(
					'SELECT count(*) FROM v_evn WHERE Morbus_id <> :Morbus_id AND Evn_id = :Evn_id',
					array(
				...
				if ($cnt === false) {
					...
					if ($cnt !== 0) {
						...
						$alreadyBindedTo->Morbus_id = $this->getFirstResultFromQuery(
							'SELECT Morbus_id FROM v_evn WHERE Evn_id = :Evn_id',
					...
					CASE evn_pid WHEN :Evn_id
						...
						CASE WHEN NOT EXISTS (
							...
							WHERE
								e.morbus_id = s.morbus_id
								AND evn_id <> :Evn_id
							) --и если это заболевание в других учетных документах не используется
							THEN 'true'
							ELSE 'false'
						END
						ELSE 'false'
	...
	public function detectMorbusType($Evn_id)
	{
		...
		$result = $this->getFirstResultFromQuery('SELECT MorbusType_id as "MorbusType_id" FROM dbo.v_MorbusDiag WHERE Diag_id = :Diag_id', array('Diag_id' => $Diag_id));
		if (!$result) {
	...
	public function detectDiagAndPersonByEvnId($Evn_id)
	{
		...
		if ($Evn_id) {
			...
			try {
				...
				$evn_view = $this->getFirstResultFromQuery("SELECT EvnClass_SysNick as \"EvnClass_SysNick\" FROM v_evn WHERE evn_id = :Evn_id", array('Evn_id' => $Evn_id));
				if ($evn_view) {
					$Diag_id = $this->getFirstResultFromQuery("select diag_id as \"diag_id\" from v_{$evn_view} where {$evn_view}_id = :Evn_id", array('Evn_id' => $Evn_id));
					if (!$Diag_id) {
		...
		try {
			if (null === $Person_id) {
				$Person_id = $this->getFirstResultFromQuery("select Person_id as \"Person_id\" from v_{$evn_view} where {$evn_view}_id = :Evn_id", array('Evn_id' => $Evn_id));
			}
		} catch (Exception $e) {
	...
	function getPersonId()
	{
		if (null === $this->personId) {
			if ($this->Evn_pid) {
				$this->personId = $this->getFirstResultFromQuery('SELECT Person_id as "Person_id" FROM v_evn WHERE evn_id = :Evn_id', array('Evn_id' => $this->Evn_pid));
			} else {
				if ($this->Evn_id) {
					$this->personId = $this->getFirstResultFromQuery('SELECT Person_id as "Person_id" FROM v_evn WHERE evn_id = :Evn_id', array('Evn_id' => $this->Evn_id));
				} else {
	...
	function __toString()
	{
		...
		if ($this->Diag_id) {
			$diag_name = $this->getFirstResultFromQuery('SELECT diag_fullName as "diag_fullName" FROM v_diag WHERE diag_id = :Diag_id',
	...
	public function getUsedByAnotherEvn()
	{
		if ($this->usedByAnotherEvn === null) {
			$cnt = $this->getFirstResultFromQuery('SELECT Count(*) FROM v_evn WHERE evn_id <> :Evn_id and Morbus_id = :Morbus_id',
	...
	function findPersonMorbusByDiag($Person_id = null)
	{
		...
		$where_clause = "MorbusBase_id IN ( SELECT
                                           ...
                                           WHERE
                                            person_id = :Person_id )
                        AND diag_id IN (
                            SELECT diag_id FROM v_diag
                              WHERE
                                diag_id IN (SELECT diag_id FROM dbo.v_MorbusDiag WHERE MorbusType_id = :MorbusType_id)
                                AND DiagLevel_id = 4
                                AND diag_pid = (select diag_pid FROM v_diag WHERE diag_id = :Diag_id)
                        )
	...
	public function closeMorbus($data) {
		$query = "
			...
			where 
			    Morbus_id = :Morbus_id
			Returning :Morbus_id as \"Morbus_id\", 0 as \"Error_code\", '' as \"Error_Msg\"
		";
		...
		try {
			...
			$result = $this->db->query($query, $p, true);
		...
		$query = '
			update PersonRegister
				SET Morbus_id = :Morbus_id,
					PersonRegister_updDT = dbo.tzGetDate(),
					pmUser_updID = :pmUser_id
