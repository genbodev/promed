...
class Ufa_EvnSection_model extends EvnSection_model
{
	...
	function loadKSGKPGKOEF($data)
	{
		...
		if (empty($data['LpuSectionProfile_Code'])) {
			$data['LpuSectionProfile_Code'] = $this->getFirstResultFromQuery("select LpuSectionProfile_Code as \"LpuSectionProfile_Code\" from v_LpuSectionProfile  where LpuSectionProfile_id = :LpuSectionProfile_id", $data);
		}
		if (empty($data['LpuSectionProfile_Code'])) {
		...
		$query = "
			SELECT
				...
				to_char(COALESCE(es.EvnSection_disDate, es.EvnSection_setDate), 'dd.mm.yyyy') as \"EvnSection_disDate\",
		...
		$Diag_pid = $this->getFirstResultFromQuery("
			select 
				d2.Diag_pid as \"Diag_pid\"
			from
				v_Diag d
				inner join v_Diag d1 on d1.Diag_id = d.Diag_pid -- группа диагнозов
				inner join v_Diag d2 on d2.Diag_id = d1.Diag_pid -- ещё группа диагнозов
			where
				d.Diag_id = :Diag_id
			limit 1
		", array(
		...
		if (!empty($Diag_pid) && empty($data['HTMedicalCareClass_id']) && !in_array($data['LpuSectionProfile_Code'], array('1040', '2040', '3040', '1087', '2087', '3087', '1039', '2039', '3039', '1074', '2074', '3074', '1084', '2084', '3084', '1036', '2036'))) {
			...
			$query = "
				...
				WHERE
					...
					and EXTRACT(YEAR FROM COALESCE(es.EvnSection_disDate, es.EvnSection_setDate)) = 2015
					and es.HTMedicalCareClass_id is null
					and (COALESCE(lsp.LpuSectionProfile_Code,'') not in ('1040', '2040', '3040', '1087', '2087', '3087', '1039', '2039', '3039', '1074', '2074', '3074', '1084', '2084', '3084', '1036', '2036'))
				order by
		...
		if (!empty($data['EvnSection_id'])) {
			$query = "
				...
				from v_EvnUsluga eu 
					...
					inner join v_MesOld mo on mo.Mes_id = mu.Mes_id and mo.MesType_id = 3 -- КСГ терапевтическая
					left join lateral(
						select 
							mo2.Mes_id as Mes_kid,
							mo2.Mes_Code as KPG
		...
		if (!empty($data['Diag_id'])) {
			$query = "
				...
				from v_MesOldDiag mod 
					inner join v_MesOld mo  on mo.Mes_id = mod.Mes_id and mo.MesType_id = 3 -- КСГ терапевтическая
				...
				where
					...
					and (COALESCE(mod.MesOldDiag_endDT,  :EvnSection_disDate) >= :EvnSection_disDate)
					and mo.Mes_begDT <= :EvnSection_disDate
					and (COALESCE(mo.Mes_endDT, :EvnSection_disDate)>= :EvnSection_disDate)
					and mt.MesTariff_begDT <= :EvnSection_disDate
					and (COALESCE(mt.MesTariff_endDT, :EvnSection_disDate)>= :EvnSection_disDate)
		...
		if (!empty($data['EvnSection_id'])) {
			$query = "
				...
				from v_EvnUsluga eu
					...
					inner join v_UslugaCategory ucat on ucat.UslugaCategory_id = uc.UslugaCategory_id and ucat.UslugaCategory_SysNick = 'gost2011'
					inner join v_MesOldUslugaComplex mu on mu.UslugaComplex_id = uc.UslugaComplex_id
					inner join v_MesOld mo on mo.Mes_id = mu.Mes_id and mo.MesType_id = 2 -- КСГ хирургическая
					left join lateral(
						select 
							mo2.Mes_id as Mes_kid,
							mo2.Mes_Code as KPG
		...
		if (!empty($data['Diag_id'])) {
			$query = "
				select
					...
					mokpg.Mes_kid as \"Mes_kid\"
				from v_MesOldDiag mod
					inner join v_MesOld mo on mo.Mes_id = mod.Mes_id and mo.MesType_id = 2 -- КСГ хирургическая
					left join v_Diag d on d.Diag_id = mod.Diag_id
					left join v_MesTariff mt on mt.Mes_id = mo.Mes_id -- Коэффициент КСГ
					left join lateral(
						select 
			...
			if (!empty($data['EvnSection_id'])) {
				...
				$data['LpuSectionProfile_id'] = $this->getFirstResultFromQuery("
					...
					FROM
						v_EvnSection es
						left join v_LpuSection ls on ls.LpuSection_id = es.LpuSection_id
						left join v_LpuSectionProfile lsp on lsp.LpuSectionProfile_id = COALESCE(:LpuSectionProfile_id, ls.LpuSectionProfile_id)
						left join lateral(
							select
		...
		if ($KSGOper && $KSGTerr) {
			if ($KSGOper['KSG'] == '12' && $KSGTerr['KSG'] == '11') {
				...
				$EvnUsluga_id = $this->getFirstResultFromQuery("select EvnUsluga_id as \"EvnUsluga_id\" from v_EvnUsluga eu inner join v_UslugaComplex uc on uc.UslugaComplex_id = eu.UslugaComplex_id where uc.UslugaComplex_Code in ('A16.20.079') and eu.EvnUsluga_pid = :EvnSection_id and eu.PayType_id = :PayTypeOms_id limit 1", array('EvnSection_id' => $data['EvnSection_id'], 'PayTypeOms_id' => $data['PayTypeOms_id']));
		...
		if (!empty($Diag_pid) && empty($data['HTMedicalCareClass_id']) && !in_array($data['LpuSectionProfile_Code'], array('1040', '2040', '3040', '1087', '2087', '3087', '1039', '2039', '3039', '1074', '2074', '3074', '1084', '2084', '3084', '1036', '2036'))) {
			...
			$query = "
				...
				WHERE
					es.EvnSection_pid = :EvnSection_pid and (EvnSection_id <> :EvnSection_id OR :EvnSection_id is NULL)
					and d2.Diag_pid = :Diag_pid
					and EXTRACT(YEAR FROM COALESCE(es.EvnSection_disDate, es.EvnSection_setDate)) = 2016
					and es.HTMedicalCareClass_id is null
					and (COALESCE(lsp.LpuSectionProfile_Code,'') not in ('1040', '2040', '3040', '1087', '2087', '3087', '1039', '2039', '3039', '1074', '2074', '3074', '1084', '2084', '3084', '1036', '2036'))
				order by
					es.EvnSection_setDate desc
		...
		if (!empty($Diag_pid) && empty($data['HTMedicalCareClass_id']) && !in_array($data['LpuSectionProfile_Code'], array('1040', '2040', '3040', '1087', '2087', '3087', '1039', '2039', '3039', '1074', '2074', '3074', '1084', '2084', '3084', '1036', '2036'))) {
			...
			$query = "
				...
				WHERE
					es.EvnSection_pid = :EvnSection_pid and (EvnSection_id <> :EvnSection_id OR :EvnSection_id is NULL)
					and d2.Diag_pid = :Diag_pid
					and EXTRACT(YEAR FROM COALESCE(es.EvnSection_disDate, es.EvnSection_setDate)) = 2017
					and es.HTMedicalCareClass_id is null
					and (COALESCE(lsp.LpuSectionProfile_Code,'') not in ('1040', '2040', '3040', '1087', '2087', '3087', '1039', '2039', '3039', '1074', '2074', '3074', '1084', '2084', '3084', '1036', '2036'))
				order by
					es.EvnSection_setDate desc
	...
	function loadKSGKPGKOEF2017($data)
	{
		...
		if (!empty($Diag_pid) && empty($data['HTMedicalCareClass_id']) && !in_array($data['LpuSectionProfile_Code'], array('1040', '2040', '3040', '1087', '2087', '3087', '1039', '2039', '3039', '1074', '2074', '3074', '1084', '2084', '3084', '1036', '2036'))) {
			...
			$query = "
				...
				WHERE
					es.EvnSection_pid = :EvnSection_pid and (EvnSection_id <> :EvnSection_id OR :EvnSection_id is NULL)
					and d2.Diag_pid = :Diag_pid
					and EXTRACT(YEAR FROM COALESCE(es.EvnSection_disDate, es.EvnSection_setDate)) = 2018
					and es.HTMedicalCareClass_id is null
					and (COALESCE(lsp.LpuSectionProfile_Code,'') not in ('1040', '2040', '3040', '1087', '2087', '3087', '1039', '2039', '3039', '1074', '2074', '3074', '1084', '2084', '3084', '1036', '2036'))
				order by
		...
		if ($needKSG && !empty($data['Diag_id'])) {
			$query = "
				...
				where
					mu.Diag_id = :Diag_id
					and (mu.UslugaComplex_id is null OR mu.UslugaComplex_id IN (select eu.UslugaComplex_id from v_EvnUsluga eu where eu.EvnUsluga_pid = :EvnSection_id and eu.EvnUsluga_setDT is not null and eu.EvnClass_id in (43,22,29,47) and eu.PayType_id = :PayTypeOms_id))
					and (mu.Diag_nid IS NULL OR mu.Diag_nid IN (select Diag_id from v_EvnDiagPS where DiagSetClass_id IN (2,3) and EvnDiagPS_pid = :EvnSection_id))
					and (mu.Sex_id = :Sex_id OR Sex_id IS NULL)
					and (
						(:Person_Age > 18 and mu.MesAgeGroup_id = 1)
		...
		if (!empty($Diag_pid) && empty($data['HTMedicalCareClass_id']) && !in_array($data['LpuSectionProfile_Code'], array('1040', '2040', '3040', '1087', '2087', '3087', '1039', '2039', '3039', '1074', '2074', '3074', '1084', '2084', '3084', '1036', '2036'))) {
			...
			$query = "
				...
				WHERE
					es.EvnSection_pid = :EvnSection_pid and (EvnSection_id <> :EvnSection_id OR :EvnSection_id is NULL)
					and d2.Diag_pid = :Diag_pid
					and EXTRACT(YEAR FROM COALESCE(es.EvnSection_disDate, es.EvnSection_setDate)) = 2019
					and es.HTMedicalCareClass_id is null
					and (COALESCE(lsp.LpuSectionProfile_Code,'') not in ('1040', '2040', '3040', '1087', '2087', '3087', '1039', '2039', '3039', '1074', '2074', '3074', '1084', '2084', '3084', '1036', '2036'))
				order by
					COALESCE(es.EvnSection_disDate, es.EvnSection_setDate) desc
	...
	function loadKSGKPGKOEF2019($data)
	{
		...
		if (in_array($data['LpuSectionProfile_Code'], ['1041', '2041', '3041'])) {
			...
			$Diag_Code = $this->getFirstResultFromQuery("select Diag_Code as \"Diag_Code\" from v_Diag where Diag_id = :Diag_id limit 1", ['Diag_id' => $data['Diag_id']]);

			$doNotGroupByDiag = (
				!empty($Diag_Code)
				&& (
					($Duration >= 6 && substr($Diag_Code, 0, 1) == 'O')
		...
		if ($doNotGroupByDiag === false) {
			...
			if (!empty($Diag_pid) && empty($data['HTMedicalCareClass_id']) && !in_array($data['LpuSectionProfile_Code'], array('1040', '2040', '3040', '1087', '2087', '3087', '1039', '2039', '3039', '1074', '2074', '3074', '1084', '2084', '3084', '1036', '2036'))) {
				...
				if (is_object($result)) {
					$resp = $result->result('array');
					if (!empty($resp[0]['EvnSection_disDate']) && $resp[0]['EvnSection_disDate'] > $data['EvnSection_disDate']) {
						$data['LastEvnSection_disDate'] = $resp[0]['EvnSection_disDate'];
					}
				}
			}
				...
				if (is_object($result)) {
					...
					foreach ($resp as $respone) {
						...
						if (
							...
							&& (
								($respone['EvnSection_Duration'] >= 6 && substr($respone['Diag_Code'], 0, 1) == 'O')
