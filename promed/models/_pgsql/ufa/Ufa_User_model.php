<?php
/**
 * Ufa_User_model - модель для работы с учетными записями пользователей (Уфа)
 *
...
class Ufa_User_model extends User_model
{
	/**
	 * Конструктор
	 */
		...
		if ($data['session']['orgtype'] != 'lpu') { // если это не ЛПУ
			...
			if (havingGroup('orgadmin')) {
				...
				$sql = "
					...
					where
						MS.Org_id = :Org_id
						and MS.MedService_begDT <= cast(dbo.tzGetDate() as date) and (MS.MedService_endDT >= cast(dbo.tzGetDate() as date) or MS.MedService_endDT is null)
						{$filter}
				";
			} else {
				$sql = "
			...
			$use_date = true;
			if ($use_date) {
				$filter_medstafffact = 'and cast(msf.WorkData_begDate as date) <= cast(dbo.tzGetDate() as date) and (cast(msf.WorkData_endDate as date) >= cast(dbo.tzGetDate() as date) or msf.WorkData_endDate is null)';
				$filter_medservicemedpersonal = 'and (cast(msmp.MedServiceMedPersonal_begDT as date) <= cast(dbo.tzGetDate() as date) and (cast(msmp.MedServiceMedPersonal_endDT as date) >= cast(dbo.tzGetDate() as date) or msmp.MedServiceMedPersonal_endDT is null))';
				$filter_medservice = 'and cast(MS.MedService_begDT as date) <= cast(dbo.tzGetDate() as date) and (cast(MS.MedService_endDT as date) >= cast(dbo.tzGetDate() as date) or MS.MedService_endDT is null)';
			}
			$filter = '';
			...
			$sql_medstafffact = "
				SELECT
					...
					null::bigint as \"MedService_IsLocalCMP\",
			...
			$sql_medservice = "
				SELECT
					case
							   ...
							   inner join dbo.v_LpuUnit LU on LU.LpuUnit_id = msf.LpuUnit_id
                                                  where msf.MedPersonal_id = msmp.MedPersonal_id
                                                    and msf.WorkData_endDate is null
					...
					case
                                            when mst.MedServiceType_SysNick = 'reanimation' then
						COALESCE(MS.LpuSection_id,0)
                                            else MS.LpuSection_id
