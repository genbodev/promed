...
class EvnSectionNarrowBed_model extends EvnAbstract_model
{
	...
	function loadEvnSectionNarrowBedGrid($data) {
		$query = "
			select
				ESU.EvnSectionNarrowBed_id as \"EvnSectionNarrowBed_id\",
				ESU.EvnSectionNarrowBed_pid as \"EvnSectionNarrowBed_pid\",
				ESU.PersonEvn_id as \"PersonEvn_id\",
				ESU.Server_id as \"Server_id\",
				LS.LpuSection_id as \"LpuSection_id\",
				LS.LpuSectionBedProfile_id as \"LpuSectionBedProfile_id\",
				to_char (ESU.EvnSectionNarrowBed_disDate, 'dd.mm.yyyy') as \"EvnSectionNarrowBed_disDate\",
				COALESCE(to_char(ESU.EvnSectionNarrowBed_disTime, 'HH24:mi') ,'') as \"EvnSectionNarrowBed_disTime\",
				to_char (ESU.EvnSectionNarrowBed_setDate, 'dd.mm.yyyy') as \"EvnSectionNarrowBed_setDate\",
				COALESCE(to_char(ESU.EvnSectionNarrowBed_setTime, 'HH24:mi') ,'') as \"EvnSectionNarrowBed_setTime\",
				RTRIM(LSP.LpuSectionProfile_Name) as \"LpuSectionProfile_Name\"
			from v_EvnSectionNarrowBed ESU 
				inner join LpuSection LS on LS.LpuSection_id = ESU.LpuSection_id
	...
	protected function _validate()
	{
		...
		if (self::SCENARIO_DO_SAVE == $this->scenario) {
			...
			$query = "
				WITH cte AS (
					SELECT 	cast(:setDT as timestamp) AS setDT,
							cast(:disDT as timestamp) AS disDT
                            )
				...
				where
					...
					AND (
						(ESU.EvnSectionNarrowBed_setDT < (SELECT setDT FROM cte)
						AND (ESU.EvnSectionNarrowBed_disDT is null OR ESU.EvnSectionNarrowBed_disDT > (SELECT setDT FROM cte)))
						OR (ESU.EvnSectionNarrowBed_setDT < (SELECT disDT FROM cte)
						AND (ESU.EvnSectionNarrowBed_disDT is null OR ESU.EvnSectionNarrowBed_disDT > (SELECT disDT FROM cte)))
						OR (ESU.EvnSectionNarrowBed_setDT > (SELECT setDT FROM cte)
						AND (ESU.EvnSectionNarrowBed_disDT is null OR ESU.EvnSectionNarrowBed_disDT < (SELECT disDT FROM cte)))
					)
			";
			if (isset($this->id)){
