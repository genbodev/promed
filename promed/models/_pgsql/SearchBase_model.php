 ...
 * @version			?
 */
class SearchBase_model extends SwPgModel {
	...
	function getPrivilegeFilters($data) {
		...
		if (!empty($data['PersonPrivilege_deleted'])) {
			$this->pt_filter .= " and COALESCE(PP.PersonPrivilege_deleted, 1) = :PersonPrivilege_deleted";
		...
		if (!empty($this->pt_filter)) {
			...
			if ($data['PrivilegeStateType_id'] == 1) {
				...
				$this->pt_filter .= " and (PP.PersonPrivilege_endDate is null or PP.PersonPrivilege_endDate > :getDT)";
			}

			$this->where .= " and exists (select personprivilege_id from v_PersonPrivilege PP ";
			$this->where .= " inner join v_PrivilegeType PT on PT.PrivilegeType_id = PP.PrivilegeType_id";
			//$this->where .= " left join v_PrivilegeAccessRights PAR on PAR.PrivilegeType_id = PP.PrivilegeType_id";
			...
			if ($this->getRegionNick() == 'ufa') {
				...
				$this->where .= " and exists (select PersonPrivilege_id from v_PersonPrivilege where PrivilegeType_id = {$alias}.PrivilegeType_id and Person_id = PS.Person_id {$lpuFilter} limit 1)";
			}
	...
	function getPersonCardFilters($data) {
		...
		if (strlen($data['PersonCard_Code']) > 0) {
			if (!empty($data['PartMatchSearch'])) {
				...
				$this->pcfilter .= " and PC.PersonCard_Code iLIKE '%'||:PersonCard_Code||'%'";
				...
				if (in_array($data['SearchFormType'], array('EvnPL', 'EvnVizitPL'))) {
					...
					$this->pcfilter .= " and exists(select * from v_PersonAmbulatCard PAC2 where PAC2.PersonAmbulatCard_Num = :PersonCard_Code and PAC2.Person_id = PS.Person_id and PAC2.Lpu_id = ".$data['session']['lpu_id'].' limit 1)';
				}
				else
		...
		if (isset($data['PersonCard_begDate_Range'][0])) {
			$this->pcfilter .= " and PC.PersonCard_begDate >= cast(:PersonCard_begDate_Range_0 as timestamp)";
			$this->queryParams['PersonCard_begDate_Range_0'] = $data['PersonCard_begDate_Range'][0];
		}

		// Прикрепление
		if (isset($data['PersonCard_begDate_Range'][1])) {
			$this->pcfilter .= " and PC.PersonCard_begDate <= cast(:PersonCard_begDate_Range_1 as timestamp)";
			$this->queryParams['PersonCard_begDate_Range_1'] = $data['PersonCard_begDate_Range'][1];
		}

		// Прикрепление
		if ($data['PersonCard_IsAttachCondit'] > 0) {
			$this->pcfilter .= " and COALESCE(PC.PersonCard_IsAttachCondit, 1) = :PersonCard_IsAttachCondit";
		...
		if ($data['PersonCard_IsDms'] > 0) {
			...
			$this->pcfilter .= " and " . $exists . " exists(
				...
				where
					...
					and PersonCard_endDate >= cast(replace(:getDT, '?', '') as timestamp)
		...
		if (!empty($this->pcfilter)) {
			if ($data['PersonCardStateType_id'] == 1) {
				if (isset($lrFlag) && $lrFlag) {
					$lrJoin = "
						left join lateral( select * from v_LpuRegion LR where LR.LpuRegion_id = PC.LpuRegion_id limit 1)lr on true
					";
				} else {
					$lrJoin = "";
				}
				$this->where .= " and exists(
					...
					from v_PersonCard PC
						left join v_personcardstate vPC on vPC.personcard_id = PC.personcard_id
						" . $lrJoin . "
					WHERE PC.Person_id = PS.Person_id
						and (vPC.PersonCardstate_endDate is null or cast(vPC.PersonCardstate_endDate as timestamp) > cast(replace(:getDT, '?', '') as timestamp))";
			} else {
				$this->where .= " and exists (select personcard_id from v_PersonCard_all PC ";
				$this->where .= " left join v_LpuRegion LR on LR.LpuRegion_id = PC.LpuRegion_id";
				$this->where .= " left join v_LpuRegion LR_Fap on LR_Fap.LpuRegion_id = PC.LpuRegion_fapid"; //https://redmine.swan.perm.ru/issues/78988
				$this->where .= " WHERE PC.Person_id = PS.Person_id";
			}
			$this->where .= $this->pcfilter . ' limit 1) ';
//			$this->where .= $this->pcfilter;
	...
	protected function getPersonPeriodicFilters($data) {
		...
		if (isset($data['Person_Birthday'])) {
			if ($data['SearchFormType'] == 'CmpCallCard' || $data['SearchFormType'] == 'CmpCloseCard') {
				$this->where .= " and COALESCE(" . $alias . ".Person_BirthDay, CCC.Person_BirthDay) = cast(:Person_Birthday as timestamp)";
			} else {
				$this->where .= " and " . $alias . ".Person_BirthDay = cast(:Person_Birthday as timestamp)";
			}
		...
		if (isset($data['Person_BirthdayYear'])) {
			$this->where .= " and date_part('year', " . $alias . ".Person_BirthDay) = :Person_BirthdayYear";
		...
		if (isset($data['EvnPLWOW_setDate_Range'][0])) {
			$this->where .= " and EPW.EvnPLWow_setDate >= cast (:EvnPLWOW_setDate_Range_0 as timestamp)";
			$this->queryParams['EvnPLWOW_setDate_Range_0'] = $data['EvnPLWOW_setDate_Range'][0];
		}
		if (isset($data['EvnPLWOW_setDate_Range'][1])) {
			$this->where .= " and EPW.EvnPLWow_setDate <= cast (:EvnPLWOW_setDate_Range_1 as timestamp)";
		...
		if (isset($data['Person_Birthday_Range'][0])) {
			if ($data['SearchFormType'] == 'CmpCallCard' || $data['SearchFormType'] == 'CmpCloseCard') {
				$this->where .= " and COALESCE(" . $alias . ".Person_BirthDay, CCC.Person_BirthDay) >= cast(:Person_Birthday_Range_0 as timestamp)";
			} else {
				$this->where .= " and " . $alias . ".Person_BirthDay >= cast(:Person_Birthday_Range_0 as timestamp)";
			}
		...
		if (isset($data['Person_Birthday_Range'][1])) {
			if ($data['SearchFormType'] == 'CmpCallCard' || $data['SearchFormType'] == 'CmpCloseCard') {
				$this->where .= " and COALESCE(" . $alias . ".Person_BirthDay, CCC.Person_BirthDay) <= cast(:Person_Birthday_Range_1 as timestamp)";
			} else {
				$this->where .= " and " . $alias . ".Person_BirthDay <= cast(:Person_Birthday_Range_1 as timestamp)";
			}
		...
		if (strlen($data['Person_Firname']) > 0) {
			...
			if ($data['SearchFormType'] == 'CmpCallCard') {
				$this->where .= " and lower(COALESCE(" . $alias . ".Person_FirName, CCC.Person_FirName)) like lower(:Person_Firname)";
			} else if ($data['SearchFormType'] == 'CmpCloseCard') {
				$this->where .= " and lower(COALESCE(CLC.Name, " . $alias . ".Person_FirName, CCC.Person_FirName)) like lower(:Person_Firname)";
			} else {
				$this->where .= " and lower(" . $alias . ".Person_FirName) like lower(:Person_Firname)";
			}
		...
		if (strlen($data['Person_Secname']) > 0) {
			...
			if ($data['SearchFormType'] == 'CmpCallCard') {
				$this->where .= " and lower(COALESCE(" . $alias . ".Person_SecName, CCC.Person_SecName)) like lower(:Person_Secname)";
			} else if ($data['SearchFormType'] == 'CmpCloseCard') {
				$this->where .= " and lower(COALESCE(CLC.Middle, " . $alias . ".Person_SecName, CCC.Person_SecName)) like lower(:Person_Secname)";
			} else {
				$this->where .= " and lower(" . $alias . ".Person_SecName) like lower(:Person_Secname)";
			}
		...
		if (strlen($data['Person_Surname']) > 0) {
			...
			switch($data['SearchFormType']) {
				case 'CmpCallCard':
					$this->where .= " and lower(COALESCE(" . $alias . ".Person_SurName, CCC.Person_SurName)) like lower(:Person_Surname)";
					break;

				case 'CmpCloseCard':
					$this->where .= " and lower(COALESCE(CLC.Fam, " . $alias . ".Person_Surname, CCC.Person_SurName)) like lower(:Person_Surname)";
					break;
				...
				case 'PersonCallCenter':
					if (allowPersonEncrypHIV($data['session']) && isSearchByPersonEncrypHIV($data['Person_Surname'])) {
						$this->queryParams['Person_Surname'] = rtrim($data['Person_Surname']);
						$this->where .= " and lower(PEH.PersonEncrypHIV_Encryp) like lower(:Person_Surname)";
					} else {
						$this->where .= " and lower(" . $alias . ".Person_SurName) like lower(:Person_Surname)";
					}
					break;
				default:
					$this->where .= " and lower(" . $alias . ".Person_SurName) like lower(:Person_Surname)";
		...
		if (isset($data['PersonAge'])) {
			if ($data['SearchFormType'] == 'CmpCallCard' || $data['SearchFormType'] == 'CmpCloseCard') {
				$this->where .= " and dbo.Age2(COALESCE(" . $alias . ".Person_BirthDay, CCC.Person_BirthDay), {$getdate}) = :PersonAge";
			} else {
		...
		if (isset($data['PersonAge_Max'])) {
			if ($data['SearchFormType'] == 'CmpCallCard' || $data['SearchFormType'] == 'CmpCloseCard') {
				$this->where .= " and dbo.Age2(COALESCE(" . $alias . ".Person_BirthDay, CCC.Person_BirthDay), {$getdate}) <= :PersonAge_Max";
			} else {
		...
		if (isset($data['PersonAge_Min'])) {
			if ($data['SearchFormType'] == 'CmpCallCard' || $data['SearchFormType'] == 'CmpCloseCard') {
				$this->where .= " and dbo.Age2(COALESCE(" . $alias . ".Person_BirthDay, CCC.Person_BirthDay), {$getdate}) >= :PersonAge_Min";
			} else {
		...
		if (isset($data['PersonBirthdayYear'])) {
			if ($data['SearchFormType'] == 'CmpCallCard' || $data['SearchFormType'] == 'CmpCloseCard') {
				$this->where .= " and date_part('year', COALESCE(" . $alias . ".Person_BirthDay, CCC.Person_BirthDay)) = :PersonBirthdayYear";
			} else {
				$this->where .= " and date_part('year', " . $alias . ".Person_BirthDay) = :PersonBirthdayYear";
			}
		...
		if (isset($data['PersonBirthdayYear_Max'])) {
			if ($data['SearchFormType'] == 'CmpCallCard' || $data['SearchFormType'] == 'CmpCloseCard') {
				$this->where .= " and date_part('year', COALESCE(" . $alias . ".Person_BirthDay, CCC.Person_BirthDay)) <= :PersonBirthdayYear_Max";
			} else {
				$this->where .= " and date_part('year', " . $alias . ".Person_BirthDay) <= :PersonBirthdayYear_Max";
			}
		...
		if (isset($data['PersonBirthdayYear_Min'])) {
			if ($data['SearchFormType'] == 'CmpCallCard' || $data['SearchFormType'] == 'CmpCloseCard') {
				$this->where .= " and date_part('year', COALESCE(" . $alias . ".Person_BirthDay, CCC.Person_BirthDay)) >= :PersonBirthdayYear_Min";
			} else {
				$this->where .= " and date_part('year', " . $alias . ".Person_BirthDay) >= :PersonBirthdayYear_Min";
			}
		...
		if (isset($data['PersonBirthdayMonth'])) {
			$this->where .= " and date_part('month', " . $alias . ".Person_BirthDay) = :PersonBirthdayMonth";
		...
		if ($data['Sex_id'] > 0) {
			if ($data['SearchFormType'] == 'CmpCallCard' || $data['SearchFormType'] == 'CmpCloseCard') {
				$this->where .= " and COALESCE(" . $alias . ".Sex_id, CCC.Sex_id) = :Sex_id";
			} else {
				$this->where .= " and COALESCE(" . $alias . ".Sex_id, 3) = :Sex_id";
			}
		...
		if (isset($data['Person_isIdentified'])) {
			if ($data['SearchFormType'] == 'CmpCallCard' || $data['SearchFormType'] == 'CmpCloseCard') {
				$this->where .= " and COALESCE(CCC.Person_id,0) " . ($data['Person_isIdentified'] == 2 ? "<>" : "=") . " 0";
			}
		}
		...
		if ((strlen($data['Document_Num']) > 0) || (strlen($data['Document_Ser']) > 0) || ($data['DocumentType_id'] > 0) || ($data['OrgDep_id'] > 0)) {
			$this->where .= " and exists (select Document_id from Document ";
		...
		if (($data['Org_id'] > 0) || ($data['Post_id'] > 0)) {
			$this->where .= " and exists (select Job_id from Job ";
		...
		if (strtolower($data['Person_NoAddress']) == 'on') {
			...
			if ($data['AddressStateType_id'] == 1) {
				$this->where .= " and exists (select AR.Address_id from Address AR ";

				if (!empty($data['Address_Street'])) {
					$this->where .= " inner join KLStreet KLS on KLS.KLStreet_id = AR.KLStreet_id ";
				}
				...
				// Адрес
				if (!empty($data['Address_Street'])) {
					$this->where .= " and KLS.KLStreet_Name ilike :Address_Street ";
				...
				$this->where .= " and exists (select AP.Address_id from Address AP ";

				if (!empty($data['Address_Street'])) {
					$this->where .= " inner join KLStreet KLS on KLS.KLStreet_id = AP.KLStreet_id ";
				}
				...
				$this->where .= " and (exists (select Address_id from Address AR  ";
		...
		if ($data['Person_NoPolis']) {
			...
			if ((strlen($data['Polis_Num']) > 0) || (strlen($data['Polis_Ser']) > 0) || ($data['PolisType_id'] > 0) || ($data['OrgSmo_id'] > 0) || ($data['OMSSprTerr_id'] > 0) || $data['Person_NoOrgSMO']) {
				$this->where .= " and exists (select Polis_id from Polis left join OmsSprTerr on OmsSprTerr.OmsSprTerr_id = Polis.OmsSprTerr_id";
	...
	function getSql($data) {
		...
		$this->queryParams = array(
			'getDT' => $this->getFirstResultFromQuery("select dbo.tzGetDate()")
		);
		...
		if (!empty($archive_database_enable)) {
			...
			$this->select .= "
				, case when COALESCE({$this->archiveAlias}.{$this->main_table}_IsArchive, 1) = 1 then 0 else 1 end as \"archiveRecord\"
			";

			if (empty($_REQUEST['useArchive'])) {
				// только актуальные
				$this->where .= " and COALESCE({$this->archiveAlias}.{$this->main_table}_IsArchive, 1) = 1";
				...
				$this->where .= " and COALESCE({$this->archiveAlias}.{$this->main_table}_IsArchive, 1) = 2";
			} else {
				// все из архивной
				$this->where .= "";
			}
