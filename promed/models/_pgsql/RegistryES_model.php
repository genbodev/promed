...
class RegistryES_model extends swPgModel {
		...
		if ($registry['RegistryESType_id'] == 3) {
			...
			foreach ($registry_data as $item) {
				...
				$this->db->query("
					...
					set RegistryESData_xmlExportPath = :RegistryESData_xmlExportPath
					where RegistryES_id = :RegistryES_id
						and EvnStick_Num = :EvnStick_Num
				", array(
						...
						if (!empty($status)) {
							if ($status->nodeValue == '1') {
								...
								$result_check = $this->dbMain->query("
									select 
										StickFSSDataGet_id as \"StickFSSDataGet_id\"
									from
										v_StickFSSDataGet 
								...
								if (!empty($resp_save[0]['StickFSSDataGet_id'])) {
									...
									$this->dbMain->query("
										update 
											EvnStickBase
										set
											StickFSSType_id = sft.StickFSSType_id
										from
											EvnStickBase esb
											inner join v_StickFSSDataGet sfdg  on sfdg.EvnStickBase_id = esb.Evn_id

											inner join v_StickFSSType sft  on sft.StickFSSType_Code = sfdg.StickFSSDataGet_StickStatus
								...
								if (!empty($StickFSSDataGet['StickFSSDataGet_StickReason'])) {
									$resp_spr = $this->queryResult("select StickCause_Name  as \"StickCause_Name\" from v_StickCause  where StickCause_Code = :StickFSSDataGet_StickReason limit 1", array(
								...
								if (!empty($StickFSSDataGet['StickFSSDataGet_StickResult'])) {
									$resp_spr = $this->queryResult("select StickLeaveType_Name  as \"StickLeaveType_Name\" from v_StickLeaveType  where StickLeaveType_Code = :StickFSSDataGet_StickResult limit 1", array(
								...
								if (!empty($StickFSSDataGet['StickFSSDataGet_StickStatus'])) {
									$resp_spr = $this->queryResult("select StickFSSType_Name  as \"StickFSSType_Name\" from v_StickFSSType  where StickFSSType_Code = :StickFSSDataGet_StickStatus limit 1", array(
								...
								if (!empty($StickFSSDataGet['StickFSSDataGet_Hash'])) {
									// получили хэш
									$this->db->query("update RegistryESData set RegistryESData_Hash = :RegistryESData_Hash where Evn_id = :Evn_id",
										array(
									...
									if ($OurData['RegistryESStatus_Code'] == 8) {
                                        ...
                                        $this->db->query("update RegistryESData set RegistryESDataStatus_id = :RegistryESDataStatus_id where RegistryES_id = :RegistryES_id and Evn_id = :Evn_id", array(
                                            'RegistryES_id' => $OurData['RegistryES_id'],
                                            'Evn_id' => $Evn_id,
									...
									if (!empty($StickFSSDataGet['StickFSSDataGet_FirstBegDate']) && !empty($OurData['FirstEvnStickWorkRelease_id'])) {
										$this->db->query("
											update EvnStickWorkRelease set EvnStickWorkRelease_IsInReg = 2, EvnStickWorkRelease_IsPaid = 2 where EvnStickWorkRelease_id = :EvnStickWorkRelease_id
										", array(
									...
									if (!empty($StickFSSDataGet['StickFSSDataGet_StickResult']) || !empty($StickFSSDataGet['StickFSSDataGet_changeDate']) || !empty($StickFSSDataGet['StickFSSDataGet_workDate'])) {
										$this->db->query("
											update EvnStickBase set EvnStickBase_IsInReg = 2, EvnStickBase_IsPaid = 2 where Evn_id = :EvnStickBase_id
										", array(
											'EvnStickBase_id' => $Evn_id
										));
									}
									...
									if ($OurData['RegistryESStatus_Code'] == 8) {
                                        ...
                                        $this->db->query("update RegistryESData set RegistryESDataStatus_id = 3 where RegistryES_id = :RegistryES_id and Evn_id = :Evn_id", array(
                                            'RegistryES_id' => $OurData['RegistryES_id'],
                                            'Evn_id' => $Evn_id
                                        ));
									}
	...
	function showFiles($data) {
		...
		$resp = $this->queryResult("
			select
				...
				to_char(RegistryESFiles_insDT, 'YYYY-MM-DD HH24:MI:SS') as \"date\"
	...
	function sendErrorMessage($RegistryES_id, $data) {
		...
		$query = "
			select
				REST.RegistryESType_Name as \"RegistryESType_Name\",
				RES.Lpu_id as \"Lpu_id\",
				RES.RegistryES_Num as \"RegistryES_Num\",
				to_char(RES.RegistryES_begDate, 'dd.mm.yyyy') as \"RegistryES_begDate\",
				LB.LpuBuilding_Name as \"LpuBuilding_Name\"
			from 
				v_RegistryES RES
				inner join v_RegistryESType REST on REST.RegistryESType_id = RES.RegistryESType_id
				left join v_LpuBuilding LB on LB.LpuBuilding_id = RES.LpuBuilding_id
			where
		...
		if (!empty($reg_resp)) {
			...
			$resp_users = $this->queryResult("
				...
				where
					...
					and exists(
						select
						pucgl.pmUserCacheGroupLink_id
						from v_pmUserCacheGroupLink pucgl
						where pucgl.pmUserCacheGroup_id = (
							select pmUserCacheGroup_id
							from dbo.pmUserCacheGroup
							where pmUserCacheGroup_SysNick = 'LpuAdmin'
							limit 1
						)
					...
					--and puc.pmUser_EvnClass ilike '%StickFSSData%' --подписка на уведомления
			", array(
	...
	function sendRegistryESToFSS($data) {
		...
		$resp = $this->queryResult("
			select
				RegistryES_id as \"RegistryES_id\",
				RegistryESType_id as \"RegistryESType_id\",
				RegistryES_xmlExportPath as \"RegistryES_xmlExportPath\"
			from
				v_RegistryES 
		...
		foreach($resp as $respone) {
			...
			if ($respone['RegistryESType_id'] == 3) {
				...
				$resp_resd = $this->queryResult("
					select
						Evn_id as \"Evn_id\",
						RegistryESData_xmlExportPath as \"RegistryESData_xmlExportPath\"
					from
						RegistryESData 
						...
						if (!empty($disableLnResponse)) {
							...
							if (!empty($status)) {
								...
								if ($regStatus == '0') {
									...
									$this->db->query("
										update EvnStickBase set EvnStickBase_IsInRegDel = 1 where Evn_id = :EvnStickBase_id
									", array(
				...
				$query = "
					update
						RegistryESData
					set
						RegistryESDataStatus_id = (case when REE.Evn_id IS NOT NULL then 3 else 2 end)
					from
						RegistryESData resd
						LEFT JOIN LATERAL(
							select 
					...
					where
						...
						and COALESCE(resd.RegistryESDataStatus_id,1) in (1,2)
				";
				...
				if (!empty($response)) {
					...
					$this->db->query("update RegistryES set RegistryES_xmlImportPath = :RegistryES_xmlImportPath where RegistryES_id = :RegistryES_id", array(
						'RegistryES_id' => $respone['RegistryES_id'],
		...
		$resp = $this->queryResult("
			select
				RegistryES_id as \"RegistryES_id\",
				RegistryES_xmlImportPath as \"RegistryES_xmlImportPath\"
			from
				v_RegistryES 
			...
			where
				...
				and RegistryES_xmlExpDT <= dbo.tzGetDate() - interval '1 hours'
			order by
				RegistryES_xmlExpDT asc
		", $queryParams);
	...
	function fixRegistryES($data) {
		...
		$resp = $this->queryResult("
			select 
				RegistryESFiles_PathFileResponse as \"RegistryESFiles_PathFileResponse\"
			from
				RegistryESFiles 
	...
	function parseXmlResponse($data, $RegistryES_id, $xml, $xmlReq = null) {
		...
		if (empty($this->StickFSSType)) {
			$resp_sft = $this->queryResult("
				select
					StickFSSType_id as \"StickFSSType_id\",
					StickFSSType_Code as \"StickFSSType_Code\"
				from
					StickFSSType SFT 				
		...
		if (!empty($prParseFilelnlpuResponse)) {
			...
			foreach ($row as $oneRow) {
				...
				$query = "
					select  
						StickFSSData_id as \"StickFSSData_id\",
						StickFSSData_StickNum as \"StickFSSData_StickNum\",
						StickFSSData_IsNeedMSE as \"StickFSSData_IsNeedMSE\"
					from v_StickFSSData
					where 
					...
					if ( !empty($lnState) && $lnState == '040' ) {
						if(empty($result)) {
							...
							$query = "
								...
								from
									v_RegistryES RES
									inner join Lpu L on L.Lpu_id = RES.Lpu_id
							...
							$query = "
								...
								from
									v_EvnStickBase ESB
									inner join v_Person_All P on P.PersonEvn_id = ESB.PersonEvn_id
								where
							...
							$resp_users = $this->queryResult("
                                ...
                                where puc.Lpu_id = :Lpu_id
                                      ...
                                      exists (
                                               ...
                                               where pucgl.pmUserCacheGroup_id =
                                                     ...
                                                     pucgl.pmUserCache_id = puc.PMUser_id
                                      ) -- АРМ администратора МО
                                      and
                                      COALESCE(puc.pmUser_deleted, 1) = 1 and
                                      puc.pmUser_EvnClass like '%StickFSSData%'
                                union
                                select pmUser_id as \"pmUser_id\"
                                from v_pmUserCache puc
                                ...
                                where puc.Lpu_id = :Lpu_id
                                      and
                                      mst.MedServiceType_SysNick = 'mstat' -- АРМ медицинского статистика
                                      and
                                      COALESCE(puc.pmUser_deleted, 1) = 1 and
                                      puc.pmUser_EvnClass like '%StickFSSData%'
							", array(
				...
				if (!empty($Evn_id)) {
					...
					$this->db->query("update RegistryESData set RegistryESData_Hash = :RegistryESData_Hash, RegistryESDataStatus_id = :RegistryESDataStatus_id where RegistryES_id = :RegistryES_id and Evn_id = :Evn_id",
						array(
					...
					if (!empty($lnState) && !empty($this->StickFSSType[$lnState])) {
						...
						$this->db->query("update RegistryESData set StickFSSType_id = :StickFSSType_id where RegistryES_id = :RegistryES_id and Evn_id = :Evn_id",
							array(
						...
						// Выполняем на основной БД
						$this->dbMain->query("update EvnStickBase set StickFSSType_id = :StickFSSType_id where Evn_id = :Evn_id",
							array(
								'StickFSSType_id' => $StickFSSType_id,
								'Evn_id' => $Evn_id
							)
			...
			$this->db->query("update RegistryESData set RegistryESDataStatus_id = 3 where RegistryES_id = :RegistryES_id and RegistryESDataStatus_id = 1", array('RegistryES_id' => $RegistryES_id));
			...
			if ($regStatus == '0') {
				...
				if (!$hasErrors) {
					$resp_evns = $this->queryResult("select Evn_id  as \"Evn_id\" from v_RegistryESData where RegistryES_id = :RegistryES_id and RegistryESDataStatus_id = 3", array(
	...
	function updateIsInRegIsPaid($data) {
		...
		$query = "
			update
				EvnStickBase
			set
				EvnStickBase_IsInReg = case when COALESCE(esb.EvnStickBase_IsPaid, 1) = 1 and resd.RegistryESDataStatus_id = 3 then 1 else esb.EvnStickBase_IsInReg end, -- снимаем IsInReg, если IsPaid <> 2 и есть ошибки  

				EvnStickBase_IsPaid = case when esb.EvnStickBase_IsInReg = 2 and resd.RegistryESDataStatus_id = 2 then 2 else esb.EvnStickBase_IsPaid end -- ставим IsPaid = 2, если IsInReg = 2 и нет ошибок
			from
				EvnStickBase esb
				inner join v_RegistryESData resd  on resd.Evn_id = esb.Evn_id
        ...
        $query = "
			update
				EvnStick
			set
				EvnStick_IsDateInReg = case when COALESCE(es.EvnStick_IsDateInFSS, 1) = 1 and resd.RegistryESDataStatus_id = 3 then 1 else es.EvnStick_IsDateInReg end, -- снимаем IsInReg, если IsPaid <> 2 и есть ошибки 
				EvnStick_IsDateInFSS = case when	es.EvnStick_IsDateInReg = 2	and resd.RegistryESDataStatus_id = 2 then 2 else es.EvnStick_IsDateInFSS end -- ставим IsPaid = 2, если IsInReg = 2 и нет ошибок	
			from
				EvnStick es
				inner join v_RegistryESData resd on resd.Evn_id = es.Evn_id
			where
				resd.RegistryES_id = :RegistryES_id
				{$filter}
		";
		...
		$query = "
			update
				EvnStickWorkRelease
			set
				EvnStickWorkRelease_IsInReg = case when COALESCE(eswr.EvnStickWorkRelease_IsPaid, 1) = 1 and resd.RegistryESDataStatus_id = 3 then 1 else eswr.EvnStickWorkRelease_IsInReg end, -- снимаем IsInReg, если IsPaid <> 2 и есть ошибки  
				EvnStickWorkRelease_IsPaid = case when eswr.EvnStickWorkRelease_IsInReg = 2 and resd.RegistryESDataStatus_id = 2 then 2 else eswr.EvnStickWorkRelease_IsPaid end -- ставим IsPaid = 2, если IsInReg = 2 и нет ошибок
			from
				EvnStickWorkRelease eswr
				inner join v_RegistryESData resd on resd.FirstEvnStickWorkRelease_id = eswr.EvnStickWorkRelease_id
			where
				resd.RegistryES_id = :RegistryES_id
				{$filter}
		";
		...
		$query = "
			...
			from
				EvnStickWorkRelease eswr
				inner join v_RegistryESData resd on resd.SecondEvnStickWorkRelease_id = eswr.EvnStickWorkRelease_id
			where
				resd.RegistryES_id = :RegistryES_id
				{$filter}
		";
		...
		$query = "
			...
			from
				EvnStickWorkRelease eswr
				inner join v_RegistryESData resd on resd.ThirdEvnStickWorkRelease_id = eswr.EvnStickWorkRelease_id
			where
				resd.RegistryES_id = :RegistryES_id
				{$filter}
		";
	...
	function doFLKControlForAll($data) {
		...
		$resp = $this->queryResult("
			...
			where
				...
				and RegistryES_updDT <= dbo.tzGetDate() - interval '1 hours'
			order by
	...
	function exportRegistryESToXml($data) {
		...
		if ($registry['RegistryESType_id'] == 3) {
			...
			$ogrn = $this->getFirstResultFromQuery("select Lpu_Ogrn from v_Lpu  where Lpu_id = :Lpu_id", array(
	...
	function exportRegistryESDataForCheckInFSS($data) {
		...
		$registry_data = $this->queryResult("
			select 
				RESD.Evn_id as \"Evn_id\",
				O.Org_OGRN as \"Lpu_OGRN\",
				RESD.EvnStick_Num as \"EvnStick_Num\",
				RESD.Person_Snils as \"Person_Snils\"
			from
				v_RegistryESData RESD 
	...
	function loadRegistryESGrid($data) {
		...
		if (!empty($data['EvnStick_Num'])) {
			$filters .= " and exists(select RESD.Evn_id from v_RegistryESData RESD  where RESD.EvnStick_Num = :EvnStick_Num and RESD.RegistryES_id = RES.RegistryES_id)";
		...
		$query = "
			select
				RES.RegistryES_id as \"RegistryES_id\",
				RegistryES_Num as \"RegistryES_Num\",
				REST.RegistryESType_Name as \"RegistryESType_Name\",
				to_char(RES.RegistryES_begDate, 'DD.MM.YYYY') as \"RegistryES_begDate\",

				to_char(RES.RegistryES_insDT, 'DD.MM.YYYY') as \"RegistryES_insDT\",

				to_char(RES.RegistryES_updDT, 'DD.MM.YYYY') as \"RegistryES_updDT\",

				RES.RegistryES_RecordCount as \"RegistryES_RecordCount\",			-- Всего ЛВН в реестре
				RESD.RegistryES_SuccessCount as \"RegistryES_SuccessCount\",		-- Принятые ЛВН в ФСС
				RES.RegistryES_ErrorCount as \"RegistryES_ErrorCount\",			-- Не принятые ЛВН в ФСС
				RES.Lpu_id as \"Lpu_id\",
				RES.Lpu_Nick as \"Lpu_Nick\",
				RES.Lpu_FSSRegNum as \"Lpu_FSSRegNum\",
				RES.Lpu_INN as \"Lpu_INN\",
				RES.Lpu_OGRN as \"Lpu_OGRN\",
				RES.RegistryES_UserFIO as \"RegistryES_UserFIO\",
				RES.RegistryES_UserPhone as \"RegistryES_UserPhone\",
				RES.RegistryES_UserEmail as \"RegistryES_UserEmail\",
				RES.RegistryES_Export as \"RegistryES_Export\",
				RES.RegistryESType_id as \"RegistryESType_id\",
				LB.LpuBuilding_Name as \"LpuBuilding_Name\",
				LS.LpuSection_Name as \"LpuSection_Name\",
				RESS.RegistryESStatus_id as \"RegistryESStatus_id\",
				RESS.RegistryESStatus_Code as \"RegistryESStatus_Code\",
				RESS.RegistryESStatus_Name as \"RegistryESStatus_Name\",
				case when (
					...
					OR (
						...
						and not exists(
							...
							where
								...
								and COALESCE(ret.RegistryESErrorType_Code::text, '0') <> '005' -- разрешить только при наличии ошибок ФЛК
			...
			from
				...
				left join v_RegistryESType REST  on REST.RegistryESType_id = RES.RegistryESType_id

				left join v_RegistryESStatus RESS  on RESS.RegistryESStatus_id = RES.RegistryESStatus_id

				left join v_LpuBuilding LB  on LB.LpuBuilding_id = RES.LpuBuilding_id

				left join v_LpuSection LS  on LS.LpuSection_id = RES.LpuSection_id

				LEFT JOIN LATERAL(

					select count(t.Evn_id) as RegistryES_SuccessCount
					from v_RegistryESData t 

					left join v_RegistryESDataStatus RESDS  on RESDS.RegistryESDataStatus_id = t.RegistryESDataStatus_id
			...
			select
				...
				to_char(RES.RegistryESQueue_insDT, 'DD.MM.YYYY') as \"RegistryES_insDT\",

				to_char(RES.RegistryESQueue_updDT, 'DD.MM.YYYY') as \"RegistryES_updDT\",
	...
	function loadRegistryESDataGrid($data) {
		...
		if (!empty($data['EvnStick_Num'])) {
			$filters .= " and RESD.EvnStick_Num iLIKE :EvnStick_Num";

			$params['EvnStick_Num'] = $data['EvnStick_Num']."%";
		}
		if (!empty($data['Person_Fio'])) {
			$filters .= " and rtrim(RESD.Person_SurName)||' '||rtrim(RESD.Person_FirName)||' '||rtrim(COALESCE(RESD.Person_SecName, '')) iLIKE :Person_Fio";
		...
		$query = "
			select
				RESD.Evn_id as \"Evn_id\",
				RESD.RegistryES_id as \"RegistryES_id\",
				case
					when RES.RegistryESStatus_id in (3,9) and RESD.RegistryESDataStatus_id = 1 and RES.RegistryESType_id in (1,3) then 1 
					else 0
				end as \"delAccess\",
				RESD.EvnStick_Num as \"EvnStick_Num\",
				RESD.Person_SurName||' '||RESD.Person_FirName||' '||COALESCE(RESD.Person_SecName, '') as \"Person_Fio\",

				to_char(RESD.Person_BirthDay, 'DD.MM.YYYY') as \"Person_BirthDay\",

				L.Lpu_Nick as \"Lpu_Nick\",
				O.Org_Nick as \"Org_Nick\",
				fMP.Person_Fio as \"MedPersonal_fFio\",
				dMP.Person_Fio as \"MedPersonal_dFio\",
				to_char(FirstEvnStickWorkRelease_begDT, 'DD.MM.YYYY') as \"EvnStickWorkRelease_begDate\",

				to_char (coalesce(
					ThirdEvnStickWorkRelease_endDT,
					SecondEvnStickWorkRelease_endDT,
					FirstEvnStickWorkRelease_endDT
				), 'dd.mm.yyyy') as \"EvnStickWorkRelease_endDate\",
				case
				...
				RESDS.RegistryESDataStatus_Name as \"RegistryESDataStatus_Name\",
				SFT.StickFSSType_Name as \"StickFSSType_Name\",
				case when ESBE.Evn_deleted = 2 then 'Да' else '' end as \"EvnStick_deleted\"
			from
				v_RegistryESData RESD 

				inner join v_RegistryESDataStatus RESDS  on RESDS.RegistryESDataStatus_id = RESD.RegistryESDataStatus_id

				inner join v_RegistryES RES  on RES.RegistryES_id = RESD.RegistryES_id

				inner join EvnStickBase ESB  on ESB.Evn_id = RESD.Evn_id

				inner join Evn ESBE  on ESBE.Evn_id = ESB.Evn_id

				left join EvnStick ESP  on ESP.Evn_id = ESBE.Evn_pid

				left join EvnStickBase ESBP  on ESBP.Evn_id = ESP.Evn_id

				left join v_Lpu L  on L.Lpu_id = ESBE.Lpu_id

				left join v_Org O  on O.Org_id = ESB.Org_id

				left join v_pmUserCache fpmUser  on fpmUser.pmUser_id = ESBE.pmUser_insID

				LEFT JOIN LATERAL (

					select t2.Person_FIO
					from v_EvnStickWorkRelease t1 

						inner join v_MedPersonal t2  on t2.MedPersonal_id = t1.MedPersonal_id

					where t1.EvnStickBase_id = ESB.Evn_id
					order by t1.EvnStickWorkRelease_begDT
                    limit 1
				) fMP ON true
				--left join v_pmUserCache dpmUser  on dpmUser.pmUser_id = ESBE.pmUser_updID

				left join v_MedStaffFact dMSF  on dMSF.MedStaffFact_id = COALESCE(ESBP.MedStaffFact_id,ESB.MedStaffFact_id)


				LEFT JOIN LATERAL (

					select Person_Fio from v_MedPersonal MP  where MP.MedPersonal_id = COALESCE(ESB.MedPersonal_id, dMSF.MedPersonal_id) 
                    limit 1


				) dMP ON true
				--inner join v_Evn rE  on rE.Evn_id = ESBE.Evn_rid

				inner join Evn rE  on rE.Evn_id = ESBE.Evn_rid

				inner join EvnClass rEС  on rEС.EvnClass_id = rE.EvnClass_id

				left join v_EvnPL EPL  on EPL.EvnPL_id = rE.Evn_id

				left join v_EvnPS EPS  on EPS.EvnPS_id = rE.Evn_id

				left join v_StickFSSType SFT  on SFT.StickFSSType_id = RESD.StickFSSType_id
	...
	function loadRegistryESErrorGrid($data) {
		...
		$query = "
			select
				...
				RESE.Evn_id as \"Evn_id\",
				RESD.EvnStick_Num as \"EvnStick_Num\",
				COALESCE(RESE.RegistryESError_Code, RESET.RegistryESErrorType_Code) as \"RegistryESError_Code\",
				RESET.RegistryESErrorType_Name,
				COALESCE(RESE.RegistryESError_Descr, RESET.RegistryESErrorType_Name) as \"RegistryESError_Descr\",
				RESD.Person_SurName||' '||RESD.Person_FirName||' '||COALESCE(RESD.Person_SecName, '') as \"Person_Fio\",
				to_char(RESD.Person_BirthDay, 'DD.MM.YYYY') as \"Person_BirthDay\",
				fMP.Person_Fio as \"MedPersonal_fFio\",
				dMP.Person_Fio as \"MedPersonal_dFio\",
				case
					when rEС.EvnClass_SysNick iLIKE 'EvnPL' then 'ТАП'
					when rEС.EvnClass_SysNick iLIKE 'EvnPS' then 'КВС'
				end as \"RegistryESType_Name\",
				case
					when rEС.EvnClass_SysNick iLIKE 'EvnPL' then EPL.EvnPL_NumCard
					when rEС.EvnClass_SysNick iLIKE 'EvnPS' then EPS.EvnPS_NumCard
				end as \"Evn_rNum\",
				RESDS.RegistryESDataStatus_Code as \"RegistryESDataStatus_Code\",
				RESDS.RegistryESDataStatus_Name as \"RegistryESDataStatus_Name\",
				SFT.StickFSSType_Name as \"StickFSSType_Name\",
				case when exists(select RegistryESFiles_id from v_RegistryESFiles  where RegistryES_id = RESD.RegistryES_id and Evn_id = RESD.Evn_id) then 1 else 0 end as \"existRegistryESFiles\",
				case when ESBE.Evn_deleted = 2 then 'Да' else '' end as \"EvnStick_deleted\"
			from
				v_RegistryESError RESE 

				left join v_RegistryESErrorType RESET  on RESET.RegistryESErrorType_id = RESE.RegistryESErrorType_id

				left join v_RegistryESErrorStageType RESEST  on RESEST.RegistryESErrorStageType_id = RESET.RegistryESErrorStageType_id

				inner join v_RegistryESData RESD  on RESD.Evn_id = RESE.Evn_id and RESD.RegistryES_id = RESE.RegistryES_id

				inner join EvnStickBase ESB  on ESB.Evn_id = RESE.Evn_id
				...
				left join lateral (
					select Person_Fio from v_MedPersonal MP  where MP.MedPersonal_id = ESB.MedPersonal_id
                    limit 1
				) dMP on true
				...
				left join v_RegistryESDataStatus RESDS  on RESDS.RegistryESDataStatus_id = RESD.RegistryESDataStatus_id
	...
	function saveRegistryES($data) {
		...
		$query = "
			select 
				L.Lpu_Nick as \"Lpu_Nick\",
				L.Lpu_FSSRegNum as \"Lpu_FSSRegNum\",
		...
		$query = "
			...
			from p_RegistryESQueue_ins(
				RegistryESQueue_id := null,
				RegistryES_id := :RegistryES_id,
				RegistryESStatus_id := :RegistryESStatus_id,
	...
	function checkExistRegistryESQueue($data){
		...
		$query = "
			select 
				RES.RegistryESQueue_id as \"RegistryESQueue_id\",
				RES.RegistryES_begDate as \"RegistryES_begDate\",
				RES.RegistryES_Num as \"RegistryES_Num\"
			from
				v_RegistryESQueue RES 
	...
	function checkAccessToReformRegistry($data) {
		$query = "
			select 
				RES.RegistryES_id as \"RegistryES_id\",
				RES.RegistryESStatus_id as \"RegistryESStatus_id\",
				RESS.RegistryESStatus_Name as \"RegistryESStatus_Name\"
			from
				v_RegistryES RES 
	...
	function loadRegistryESForm($data) {
		...
		$query = "
			select
				RES.RegistryES_id as \"RegistryES_id\",
				RES.RegistryES_Num as \"RegistryES_Num\",
				RES.LpuBuilding_id as \"LpuBuilding_id\",
				RES.LpuSection_id as \"LpuSection_id\",
				to_char(RES.RegistryES_insDT, 'DD.MM.YYYY') as \"RegistryES_insDT\",

				to_char(RES.RegistryES_begDate, 'DD.MM.YYYY') as \"RegistryES_begDate\",

				RES.RegistryES_RegRecCount as \"RegistryES_RegRecCount\",
				RES.RegistryESType_id as \"RegistryESType_id\"
			from v_RegistryES RES 
	...
	function getNewRegistryESNum($data) {
		...
		$query = "
			select
				COALESCE(max(RegistryES_Num), 0)+1 as \"RegistryES_Num\"
	...
	function _deleteRegistryESData($data) {
		...
		$this->db->query("
			update
				RegistryES 
			set
				RegistryES_RecordCount = (select count(Evn_id) from v_RegistryESData  where RegistryES_id =
                :RegistryES_id
                ),
				RegistryESStatus_id = case when (select count(Evn_id) from v_RegistryESData  where RegistryES_id =
                :RegistryES_id 
                and RegistryESDataStatus_id = 1) = 0 then 2 else RegistryESStatus_id end
			where
				RegistryES_id = :RegistryES_id
	...
	function getRegistryESDataCount($data) {
		...
		$query = "
			select RES.RegistryES_RecordCount as \"RegistryES_RecordCount\"
			from v_RegistryES RES 
	...
	function setRegistryESExport($data) {
		...
		$query = "
			update RegistryES
				...
				where RegistryES_id = :RegistryES_id
				RETURNING :RegistryES_id as \"RegistryES_id\", null as \"Error_Code\", null as \"Error_Msg\"
		";
	...
	function setAllRegistryESDataStatus($data) {
		...
		if ($params['RegistryESDataStatus_id'] != 1 && $params['RegistryESDataStatus_id'] !== null) {
			$filter = " and COALESCE(RegistryESDataStatus_id,1) = 1";
	...
	function setRegistryESStatus($data) {
		...
		if (!empty($resp[0]['RegistryES_id']) && empty($resp[0]['Error_Msg']) && $data['RegistryESStatus_id'] == 8) {
			...
			$items = $this->queryResult("select RESD.Evn_id  as \"Evn_id\" from v_RegistryESData RESD  where RESD.RegistryES_id = :RegistryES_id and RESD.RegistryESDataStatus_id = 1", array(
	...
	function setRegistryESDataStatus($data) {
		...
		$query = "
			update RegistryESData
				...
				RETURNING :Evn_id as \"Evn_id\", null as \"Error_Code\", null as \"Error_Msg\"
		";
	...
	function loadRegistryESIndividCertGrid($data) {
		return $this->queryResult("
			select
				emst.ExpertMedStaffType_id as \"ExpertMedStaffType_id\",
				emst.ExpertMedStaffType_Name as \"ExpertMedStaffType_Name\",
				reic.RegistryESIndividCert_CertSubjectName as \"RegistryESIndividCert_CertSubjectName\",
				reic.RegistryESIndividCert_CertThumbprint as \"RegistryESIndividCert_CertThumbprint\",
				reic.pmUser_Name as \"pmUser_Name\",
			...
			from
				...
				LEFT JOIN LATERAL (
					...
					from
						v_RegistryESIndividCert reic 

						left join v_pmUser pu  on pu.pmUser_id = reic.pmUser_insID
				...
				left join v_RegistryES re  on re.RegistryES_id = :RegistryES_id
	...
	function getRegistryESExport($data) {
		...
		$query = "
			select 
				...
				RES.Lpu_INN as \"Lpu_INN\",
				RES.Lpu_FSSRegNum as \"Lpu_FSSRegNum\",
				to_char(RES.RegistryES_begDate, 'YYYY-MM-DD') as \"RegistryES_Date\",
	...
	function doFLKControl($data)
	{
		...
		if ($registry['RegistryESType_id'] == 3) {
			...
			$items = $this->queryResult("select RESD.Evn_id  as \"Evn_id\" from v_RegistryESData RESD  where RESD.RegistryES_id = :RegistryES_id", array(
			...
			$resp_ds = $this->queryResult("
				...
				from
					...
					inner join DataStorage ds  on res.Lpu_id = ds.Lpu_id and DataStorage_Name = 'enable_sign_evnstick_auth_person' and DataStorage_Value = '1'
			...
			foreach($registry_data as $item) {
				...
				if (empty($item['LN_HASH'])) {
					...
					$resp_check = $this->queryResult("
						select
							case when esb.EvnStickBase_IsFSS = 2 or exists(select EvnStickWorkRelease_id from v_EvnStickWorkRelease where EvnStickBase_id = esb.EvnStickBase_id and EvnStickWorkRelease_IsPaid = 2) then 1 else 0 end as \"isInFSS\"
						from
							v_EvnStickBase esb 
			...
			$this->db->query("
				call rpt.p_Registry_EvnStick_InReg_upd(:RegistryES_id)
			", array(
				'RegistryES_id' => $data['RegistryES_id']
			));
	...
	function loadRegistryESDataForXmlManual($data, $mode = 'export') {
		...
		if ($mode == 'export') {
			$filter .= " 
				and (COALESCE(t.RegistryESDataStatus_id, 1) = 1
				...
				OR (
					...
					and not exists(
						...
						where
							...
							and COALESCE(ret.RegistryESErrorType_Code, '0') <> '005' -- разрешить только при наличии ошибок ФЛК

					)
				))
			";
		} else if ($mode == 'reconciliation') {
			$filter .= " and COALESCE(t.RegistryESDataStatus_id, 1) in (1,4)";

		}

		$query_xml = "
			select 
				xmlelement(name \"LpuLn\", xmlforest(
					COALESCE(RESD.Person_Snils,'') as \"SNILS\",
					rtrim(RESD.Person_SurName) as \"SURNAME\",
					rtrim(RESD.Person_FirName) as \"NAME\",
					rtrim(RESD.Person_SecName) as \"PATRONIMIC\",
					RESD.Org_StickNick as \"EMPLOYER\",
					RESD.RegistryESData_EmploymentType as \"EMPL_FLAG\",
					COALESCE(RESD.EvnStick_Num,'') as \"LN_CODE\",
					COALESCE(RESD.EvnStick_NumPrev,'') as \"PREV_LN_CODE\",
					RESD.EvnStick_IsPrimary as \"PRIMARY_FLAG\",
					RESD.EvnStick_IsDuplicate as \"DUPLICATE_FLAG\",
					to_char(RESD.EvnStick_setDate,'YYYY-MM-DD') as \"LN_DATE\",
					RESD.Lpu_StickNick as \"LPU_NAME\",
					RESD.Lpu_StickAddress as \"LPU_ADDRESS\",
					RESD.Lpu_OGRN as \"LPU_OGRN\",
					to_char(RESD.Person_BirthDay,'YYYY-MM-DD') as \"BIRTHDAY\",
					...
					COALESCE(RESD.EvnStick_NumPar, '') as \"PARENT_CODE\",
					to_char(RESD.EvnStick_StickDT,'YYYY-MM-DD') as \"DATE1\",
					to_char(RESD.EvnStick_sstEndDate,'YYYY-MM-DD') as \"DATE2\",
					...
					to_char(RESD.EvnStick_stacBegDate,'YYYY-MM-DD') as \"HOSPITAL_DT1\",
					to_char(RESD.EvnStick_stacEndDate,'YYYY-MM-DD') as \"HOSPITAL_DT2\",
					RESD.StickIrregularity_Code as \"HOSPITAL_BREACH_CODE\",
					to_char(RESD.EvnStick_irrDT,'YYYY-MM-DD') as \"HOSPITAL_BREACH_DT\",
					to_char(RESD.EvnStick_mseDT,'YYYY-MM-DD') as \"MSE_DT1\",
					to_char(RESD.EvnStick_mseRegDT,'YYYY-MM-DD') as \"MSE_DT2\",
					to_char(RESD.EvnStick_mseExamDT,'YYYY-MM-DD') as \"MSE_DT3\",
					...
					to_char(RESD.EvnStick_disDate,'YYYY-MM-DD') as \"OTHER_STATE_DT\",
					to_char(RESD.EvnStick_returnDate,'YYYY-MM-DD') as \"RETURN_DATE_LPU\",
					RESD.EvnStick_NumNext as \"NEXT_LN_CODE\",
					1 as \"LN_VERSION\"
				)) as LpuLn
			from 
				v_RegistryESData RESD 
				left join v_Sex Sex  on Sex.Sex_id = RESD.Sex_id
			where
	...
	function loadRegistryESDataForDisableXml($data) {
		$params = array('RegistryES_id' => $data['RegistryES_id']);
		$filter = '';
		$filter .= " and COALESCE(RESD.RegistryESDataStatus_id, 1) = 1";
		...
		$query = "
			select
				...
				COALESCE(SCD.StickCauseDel_Code, '010') as \"reasonCode\",

				'Отмена оформления' as \"reason\"
			from
				v_RegistryESData RESD 

				left join v_EvnStickBase ESB  on ESB.EvnStickBase_id = RESD.Evn_id

				left join v_StickCauseDel SCD  on SCD.StickCauseDel_id = ESB.StickCauseDel_id
	...
	function loadRegistryESDataForXml($data, $mode = 'export') {
		...
		if ($mode == 'export') {
			...
			$filter .= " and COALESCE(RESD.RegistryESDataStatus_id, 1) in (1,4)";
		...
		$query = "
			select
				RESD.Evn_id as \"Evn_id\",
				RESD.FirstEvnStickWorkRelease_id as \"FirstEvnStickWorkRelease_id\",
				RESD.SecondEvnStickWorkRelease_id as \"SecondEvnStickWorkRelease_id\",
				RESD.ThirdEvnStickWorkRelease_id as \"ThirdEvnStickWorkRelease_id\",
				RESD.EvnStick_Num as \"EvnStick_Num\",
				COALESCE(RESD.Person_Snils,'') as \"SNILS\",
				...
				COALESCE(cast(RESD.RegistryESData_EmploymentType as varchar(1)), '###') as \"LPU_EMPL_FLAG\",

				RESD.EvnStick_Num as \"LN_CODE\",
				RESD.EvnStick_NumPrev as \"PREV_LN_CODE\",
				COALESCE(RESD.EvnStick_IsPrimary,0) as \"PRIMARY_FLAG\",

				COALESCE(RESD.EvnStick_IsDuplicate,0) as \"DUPLICATE_FLAG\",

				to_char(RESD.EvnStick_setDate,'YYYY-MM-DD') as \"LN_DATE\",
				RESD.Lpu_StickNick as \"LPU_NAME\",
				COALESCE(RESD.Lpu_StickAddress,'-') as \"LPU_ADDRESS\",
				...
				COALESCE(to_char(RESD.EvnStick_StickDT,'YYYY-MM-DD'),'###') as \"DATE1\",

				COALESCE(to_char(RESD.EvnStick_sstEndDate,'YYYY-MM-DD'),'###') as \"DATE2\",

				RESD.EvnStick_sstNum as \"VOUCHER_NO\",
				RESD.Org_sstOGRN as \"VOUCHER_OGRN\",
				COALESCE(cast(RESD.FirstRelated_Age as varchar(1)),'###') as \"SERV1_AGE\",

				COALESCE(cast(RESD.FirstRelated_AgeMonth as varchar(1)),'###') as \"SERV1_MM\",

				RESD.FirstRelatedLinkType_Code as \"SERV1_RELATION_CODE\",
				RESD.FirstRelated_FIO as \"SERV1_FIO\",
				COALESCE(cast(RESD.SecondRelated_Age as varchar(1)),'###') as \"SERV2_AGE\",

				COALESCE(cast(RESD.SecondRelated_AgeMonth as varchar(1)),'###') as \"SERV2_MM\",

				RESD.SecondRelatedLinkType_Code as \"SERV2_RELATION_CODE\",
				RESD.SecondRelated_FIO as \"SERV2_FIO\",
				COALESCE(cast(RESD.EvnStick_IsRegPregnancy as varchar(1)), '###') as \"PREGN12W_FLAG\",

				COALESCE(to_char(RESD.EvnStick_stacBegDate,'YYYY-MM-DD'),'###') as \"HOSPITAL_DT1\",

				COALESCE(to_char(RESD.EvnStick_stacEndDate,'YYYY-MM-DD'),'###') as \"HOSPITAL_DT2\",

				RESD.StickIrregularity_Code as \"HOSPITAL_BREACH/HOSPITAL_BREACH_CODE\",
				to_char(RESD.EvnStick_irrDT,'YYYY-MM-DD') as \"HOSPITAL_BREACH/HOSPITAL_BREACH_DT\",
				COALESCE(to_char(RESD.EvnStick_mseDT,'YYYY-MM-DD'),'###') as \"MSE_DT1\",

				COALESCE(to_char(RESD.EvnStick_mseRegDT,'YYYY-MM-DD'),'###') as \"MSE_DT2\",

				COALESCE(to_char(RESD.EvnStick_mseExamDT,'YYYY-MM-DD'),'###')  as \"MSE_DT3\",
				...
				COALESCE(to_char(RESD.EvnStick_disDate,'YYYY-MM-DD'), '###') as \"LN_RESULT/OTHER_STATE_DT\",

				COALESCE(to_char(RESD.EvnStick_returnDate,'YYYY-MM-DD'), '###') as \"LN_RESULT/RETURN_DATE_LPU\",

				RESD.EvnStick_NumNext as \"LN_RESULT/NEXT_LN_CODE\",
				'0' as \"LN_STATE\",
				COALESCE(LASTRESD.RegistryESData_Hash, '') as \"LN_HASH\",

				-- первый период
				rtrim(RESD.FirstPostVK_Code) as \"TREAT1_CHAIRMAN_ROLE\",
				RESD.FirstMedPersonalVK_Fin as \"TREAT1_CHAIRMAN\",
				to_char(RESD.FirstEvnStickWorkRelease_begDT,'YYYY-MM-DD') as \"TREAT1_DT1\",
				to_char(RESD.FirstEvnStickWorkRelease_endDT,'YYYY-MM-DD') as \"TREAT1_DT2\",
				rtrim(RESD.FirstPost_Code) as \"TREAT1_DOCTOR_ROLE\",
				RESD.FirstMedPersonal_Fin as \"TREAT1_DOCTOR\",
				-- второй период
				rtrim(RESD.SecondPostVK_Code) as \"TREAT2_CHAIRMAN_ROLE\",
				RESD.SecondMedPersonalVK_Fin as \"TREAT2_CHAIRMAN\",
				to_char(RESD.SecondEvnStickWorkRelease_begDT,'YYYY-MM-DD') as \"TREAT2_DT1\",
				to_char(RESD.SecondEvnStickWorkRelease_endDT,'YYYY-MM-DD') as \"TREAT2_DT2\",
				rtrim(RESD.SecondPost_Code) as \"TREAT2_DOCTOR_ROLE\",
				RESD.SecondMedPersonal_Fin as \"TREAT2_DOCTOR\",
				-- третий период
				rtrim(RESD.ThirdPostVK_Code) as \"TREAT3_CHAIRMAN_ROLE\",
				RESD.ThirdMedPersonalVK_Fin as \"TREAT3_CHAIRMAN\",
				to_char(RESD.ThirdEvnStickWorkRelease_begDT,'YYYY-MM-DD') as \"TREAT3_DT1\",
				to_char(RESD.ThirdEvnStickWorkRelease_endDT,'YYYY-MM-DD') as \"TREAT3_DT2\",

				rtrim(RESD.ThirdPost_Code) as \"TREAT3_DOCTOR_ROLE\",
				RESD.ThirdMedPersonal_Fin as \"TREAT3_DOCTOR\",
				RESD.EvnStickLeave_Token as \"EvnStickLeave_Token\",
				RESD.EvnStickLeave_Hash as \"EvnStickLeave_Hash\",
				RESD.EvnStickLeave_SignedData as \"EvnStickLeave_SignedData\",
				RESD.StickIrregularity_Token as \"StickIrregularity_Token\",
				RESD.StickIrregularity_Hash as \"StickIrregularity_Hash\",
				RESD.StickIrregularity_SignedData as \"StickIrregularity_SignedData\",
				RESD.FirstVK_Token as \"FirstVK_Token\",
				RESD.FirstVK_Hash as \"FirstVK_Hash\",
				RESD.FirstVK_SignedData as \"FirstVK_SignedData\",
				RESD.SecondVK_Token as \"SecondVK_Token\",
				RESD.SecondVK_Hash as \"SecondVK_Hash\",
				RESD.SecondVK_SignedData as \"SecondVK_SignedData\",
				RESD.ThirdVK_Token as \"ThirdVK_Token\",
				RESD.ThirdVK_Hash as \"ThirdVK_Hash\",
				RESD.ThirdVK_SignedData as \"ThirdVK_SignedData\",
				RESD.FirstMedPersonal_Token as \"FirstMedPersonal_Token\",
				RESD.FirstMedPersonal_Hash as \"FirstMedPersonal_Hash\",
				RESD.FirstMedPersonal_SignedData as \"FirstMedPersonal_SignedData\",
				RESD.SecondMedPersonal_Token as \"SecondMedPersonal_Token\",
				RESD.SecondMedPersonal_Hash as \"SecondMedPersonal_Hash\",
				RESD.SecondMedPersonal_SignedData as \"SecondMedPersonal_SignedData\",
				RESD.ThirdMedPersonal_Token as \"ThirdMedPersonal_Token\",
				RESD.ThirdMedPersonal_Hash as \"ThirdMedPersonal_Hash\",
				RESD.ThirdMedPersonal_SignedData as \"ThirdMedPersonal_SignedData\",
				COALESCE(ESWR1.EvnStickWorkRelease_IsPaid, 1) as \"FirstEvnStickWorkRelease_IsPaid\",

				COALESCE(ESWR2.EvnStickWorkRelease_IsPaid, 1) as \"SecondEvnStickWorkRelease_IsPaid\",

				COALESCE(ESWR3.EvnStickWorkRelease_IsPaid, 1) as \"ThirdEvnStickWorkRelease_IsPaid\",

				COALESCE(ESB.EvnStickBase_IsFSS, 1) as \"EvnStickBase_IsFSS\"
		...
		foreach ($resp as &$respone) {
			if (empty($respone['LN_HASH']) && $respone['EvnStickBase_IsFSS'] == 2) {
				...
				$result_sfd = $this->dbMain->query("
					select 
						SFDG.StickFSSDataGet_Hash as \"StickFSSDataGet_Hash\"
					from
						v_EvnStickBase ESB 

						inner join v_StickFSSDataGet SFDG  on SFDG.StickFSSData_id = ESB.StickFSSData_id
	...
	function getRegistryESDataForCheckInFSS($data) {
		...
		$query = "
			select 
				...
				RESD.Evn_id as \"Evn_id\",
				RESDS.RegistryESDataStatus_Code as \"RegistryESDataStatus_Code\",
				RESD.Person_SurName as \"Person_SurName\",
				RESD.Person_FirName as \"Person_FirName\",
				RESD.Person_SecName as \"Person_SecName\",
				to_char(RESD.Person_BirthDay, 'YYYY-MM-DD') as \"Person_BirthDay\",

				RESD.Person_Snils as \"Person_Snils\",
				RESD.EvnStick_Num as \"EvnStick_Num\",
				to_char(RESD.EvnStick_setDate, 'YYYY-MM-DD') as \"EvnStick_setDate\",

				RESD.Lpu_StickNick as \"Lpu_StickNick\",
				RESD.StickCause_Code as \"StickCause_Code\",
				to_char(RESD.FirstEvnStickWorkRelease_begDT, 'YYYY-MM-DD') as \"FirstEvnStickWorkRelease_begDT\",

				to_char(RESD.FirstEvnStickWorkRelease_endDT, 'YYYY-MM-DD') as \"FirstEvnStickWorkRelease_endDT\",

				to_char(RESD.SecondEvnStickWorkRelease_begDT, 'YYYY-MM-DD') as \"SecondEvnStickWorkRelease_begDT\",

				to_char(RESD.SecondEvnStickWorkRelease_endDT, 'YYYY-MM-DD') as \"SecondEvnStickWorkRelease_endDT\",

				to_char(RESD.ThirdEvnStickWorkRelease_begDT, 'YYYY-MM-DD') as \"ThirdEvnStickWorkRelease_begDT\",

				to_char(RESD.ThirdEvnStickWorkRelease_endDT, 'YYYY-MM-DD') as \"ThirdEvnStickWorkRelease_endDT\",

				RESD.StickLeaveType_Code as \"StickLeaveType_Code\",
				RESD.EvnStick_disDate as \"EvnStick_disDate\",
				RESD.EvnStick_returnDate as \"EvnStick_returnDate\",
				RESD.FirstEvnStickWorkRelease_id as \"FirstEvnStickWorkRelease_id\",
				RESD.SecondEvnStickWorkRelease_id as \"SecondEvnStickWorkRelease_id\",
				RESD.ThirdEvnStickWorkRelease_id as \"ThirdEvnStickWorkRelease_id\",
				RESS.RegistryESStatus_Code as \"RegistryESStatus_Code\"
			from v_RegistryESData RESD 
				...
				left join v_RegistryES RES  on RES.RegistryES_id = RESD.RegistryES_id
	...
	function setRegistryESError($data) {
		...
		$query = "
			select 
				RegistryESErrorType_id as \"RegistryESErrorType_id\",
				RegistryESErrorType_Name as \"RegistryESErrorType_Name\"
			from
				v_RegistryESErrorType 
	...
	function updateEvnStickNotInReg($data) {
		$resp_r = $this->queryResult("select RegistryESType_id  as \"RegistryESType_id\" from v_RegistryES where RegistryES_id = :RegistryES_id", array(
			'RegistryES_id' => $data['RegistryES_id']
		));
		...
		if (empty($resp[0]['RegistryES_id'])) {
			...
			if ($RegistryESType_id == 3) {
				...
				$this->db->query("
					update EvnStickBase set EvnStickBase_IsInReg = 1 where Evn_id = :EvnStickBase_id and COALESCE(EvnStickBase_IsPaid, 1) = 1;
					update dbo.EvnStickWorkRelease set EvnStickWorkRelease_IsInReg = 1 where EvnStickBase_id  = :EvnStickBase_id and COALESCE(EvnStickWorkRelease_IsPaid, 1) = 1
				", array(
					'EvnStickBase_id' => $data['Evn_id']
				));
			}
	...
	function updateEvnStickIsInReg($data) {
		...
		$query = "
			update EvnStickBase set EvnStickBase_IsInReg = 1
				from v_RegistryESData RESD 

				where
					RESD.RegistryES_id = :RegistryES_id
					and RESD.Evn_id = EvnStickBase.Evn_id
					and (
						...
						OR (
							...
							and exists(
								...
								where
									...
									and COALESCE(ret.RegistryESErrorType_Code, '0') <> '005' -- только при наличии ошибок помимо ошибок подписи
	...
	function deleteRegistryESError($data, $type = null) {
		...
		if (!empty($type)) {
			if ($type == 'flk') {
				...
				$filter .= " and 1::varchar = (
					select RegistryESErrorStageType_Code
					from v_RegistryESErrorType RESET 

					inner join v_RegistryESErrorStageType RESEST  on RESEST.RegistryESErrorStageType_id = RESET.RegistryESErrorStageType_id

					where RESET.RegistryESErrorType_id = RegistryESError.RegistryESErrorType_id
				)";
			} else if ($type == 'fss') {
				$filter .= " and (RegistryESErrorType_id is null or 2::varchar = (
					select RegistryESErrorStageType_Code
					from v_RegistryESErrorType RESET 
		...
		if (is_array($data['RegistryESIndividCertGridData'])) {
			foreach($data['RegistryESIndividCertGridData'] as $one) {
				$resp_save = $this->queryResult("
					select Error_Code as \"Error_Code\", Error_Message as \"Error_Msg\"
					from p_RegistryESIndividCert_ins(
						RegistryES_id := :RegistryES_id, 
	...
	public function parseXmls($data) {
		...
		$resp = $this->queryResult("
			select
				...
				RESD.Evn_id as \"Evn_id\",
				RESD.RegistryES_id as \"RegistryES_id\",
				RES.RegistryES_xmlExportPath as \"RegistryES_xmlExportPath\"
			from
				v_RegistryES RES 
