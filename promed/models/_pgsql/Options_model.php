...
class Options_model extends SwPgModel {
	...
	function getOptionsGlobals($data,$option="")
	{
		...
		$sql = "
			select
				...
				Lpu_id as \"Lpu_id\",
				DataStorageGroup_id as \"DataStorageGroup_id\",
				DataStorage_Name as \"DataStorage_Name\",
				DataStorage_Value as \"DataStorage_Value\",
				DataStorage_begDT as \"DataStorage_begDT\",
				DataStorage_endDT as \"DataStorage_endDT\",
				pmUser_insID as \"pmUser_insID\",
				pmUser_updID as \"pmUser_updID\",
				DataStorage_insDT as \"DataStorage_insDT\",
				DataStorage_updDT as \"DataStorage_updDT\",
				PMUser_lid as \"PMUser_lid\",
				Org_id as \"Org_id\"
			from 
				DataStorage
	...
	function getPMGenValue($data, $obj = '') {
        ...
        $table_exist = false;
		$query = "select 1 as \"table_exist\" from information_schema.tables where table_name=lower('{$gentable}')";
        $result = $this->db->query($query);
        if (is_object($result)) {
            $resp_arr = $result->result('array');
            if ( isset($resp_arr[0]) ) {
                $table_exist = $resp_arr[0]['table_exist'];
            }
        }
        if (!$table_exist) {
            $query = "SELECT pmGen_Value as \"pmGen_Value\" FROM pmGen	WHERE pmGen_ObjectName = :pmGen_ObjectName and Lpu_id = :Lpu_id
				ORDER by
					pmGen_ObjectValue desc
					limit 1";
        } else {
            $query = "SELECT pmGen_Value as \"pmGen_Value\" FROM {$gentable} WHERE	pmGen_ObjectName = :pmGen_ObjectName and Lpu_id = :Lpu_id
				ORDER by pmGen_ObjectValue desc
					limit 1";
        }
	...
	function saveOptionsGlobals($data)
	{
		...
		foreach ($data as $key=>$value)
		{
			if ( !is_array($value) ) {
				$sql = "
				SELECT DataStorage_id as \"DataStorage_id\",
	...
	{
		$sql = "
			Select MAX(LocalDBVersion_Ver) as \"LocalDBVersion\" from stg.LocalDBVersion";
		$result = $this->db->query($sql);
		if (is_object($result))
		{
			$rs = $result->result('array');
	...
	function getDataStorageValues($data, $session)
	{
		...
		if ((isset($data['dates'])) && (count($data['dates'])>0))
		{
			...
			{
				$filter .= " and (DataStorage_begDT>=cast(:DataStorage_begDT as timestamp) or DataStorage_begDT is null)";
			...
			{
				$filter .= " and (DataStorage_endDT<=cast(:DataStorage_endDT as timestamp) or DataStorage_endDT is null)";
		...
		$sql = "
			with list as (
				SELECT *
				FROM v_DataStorage DataStorage
				WHERE {$filter}
			)
			select
				DataStorage.DataStorage_id as \"DataStorage_id\",
				DataStorageGroup.DataStorageGroup_SysNick as \"DataStorageGroup_SysNick\",
				DataStorage.DataStorage_Name as \"DataStorage_Name\",
				DataStorage.DataStorage_Value as \"DataStorage_Value\"
			from (
				...
				left join DataStorageGroup on DataStorageGroup.DataStorageGroup_id = DataStorage.DataStorageGroup_id
		";
		/*
		...
		if (count($data)>0)
		{
			for ($i=0; $i < count($data); $i++)
			{
				...
				if (isset($data[$i]['DataStorage_Name']))
				{
					...
					{
						$proc_params .= (!empty($proc_params)?',':'') . "{$k}=>:{$k}";
					}
					$query .= $proc_params . ")";
					/*
	...
	function loadCheckRegistryAccessGrid($data)
	{
		...
		$query = "
			...
			from v_DataStorage DS 
				inner join v_Lpu L  on L.Lpu_id = DS.Lpu_id
			where
				DS.DataStorage_Name ilike 'check_registry_access_lpu'
	...
	function saveCheckRegistryAccess($data) {
		...
		$query = "
			...
			from v_DataStorage
			where Lpu_id = :Lpu_id
			and DataStorage_Name ilike 'check_registry_access_lpu'
		";
		$result = $this->db->query($query, $params);
		if (!is_object($result)) {
			return false;
        ...
        FROM
        p_DataStorage_ins(                
				DataStorage_Name => CAST('check_registry_access_lpu' as varchar),
				DataStorage_Value => CAST(1 as varchar),
	...
	function loadPersonRegisterAutoIncludeGrid($data) {
		...
		$query = "
			select
				DS.DataStorage_id as \"DataStorage_id\",
				DS.DataStorage_Name as \"DataStorage_Name\",
				DS.DataStorage_Value as \"DataStorage_Value\"
			from v_DataStorage DS
			where DS.DataStorage_Name ilike 'register_%_auto_include'
		";
	...
	function getLpuUnitTypes($data) {
		...
		$query = "
			select distinct lut.LpuUnitType_SysNick as \"LpuUnitType_SysNick\"
			from
				v_MedStaffFact msf
	...
	function getMedServiceTypes($data) {
		...
		$query = "
			select distinct MST.MedServiceType_SysNick as \"MedServiceType_SysNick\" from MedServiceMedPersonal MSP 
	...
	/**
	 * Проверка опций записи по ЛО
	 */
	function getQueueOptions($data = NULL) {
				...
				array(
					...
					'items' => array(
						array(
							'xtype' => 'checkbox',
							'hidden' => false,
							'disabled' => !havingGroup('LpuAdmin') && !isSuperAdmin(),
				...
				array(
					...
					'hidden' => getRegionNick() != 'msk' || $this->getFirstResultFromQuery("
						...
						from v_MedStaffFact msf
							...
							inner join v_PostMed pm on pm.PostMed_id = msf.Post_id
						where pm.PostMed_Name ilike '%онколог%'
	...
	function fillNMPWorkTime() {
        ...
        $$ 
        declare 
        p_LpuHMPWorkTime_MoFrom time = :nmp_monday_beg_time;
        p_LpuHMPWorkTime_MoTo time = :nmp_monday_end_time;
        p_LpuHMPWorkTime_TuFrom time = :nmp_tuesday_beg_time;
        p_LpuHMPWorkTime_TuTo time = :nmp_tuesday_end_time;
        p_LpuHMPWorkTime_WeFrom time = :nmp_wednesday_beg_time;
        p_LpuHMPWorkTime_WeTo time = :nmp_wednesday_end_time;
        p_LpuHMPWorkTime_ThFrom time = :nmp_thursday_beg_time;
        p_LpuHMPWorkTime_ThTo time = :nmp_thursday_end_time;
        p_LpuHMPWorkTime_FrFrom time = :nmp_friday_beg_time;
        p_LpuHMPWorkTime_FrTo time = :nmp_friday_end_time;
        p_LpuHMPWorkTime_SaFrom time = :nmp_saturday_beg_time;
        p_LpuHMPWorkTime_SaTo time = :nmp_saturday_end_time;
        p_LpuHMPWorkTime_SuFrom time = :nmp_sunday_beg_time;
        p_LpuHMPWorkTime_SuTo time = :nmp_sunday_end_time;
        p_pmUser_id bigint = :pmUser_id;
        p_Error_Code varchar;
        p_Error_Message varchar(4000);
        p_Lpu_id bigint;
        p_MedService_id bigint;
        p_LpuHMPWorkTime_id bigint;
        med_service_cursor REFCURSOR;
        begin
        
        open med_service_cursor for 
        select	MS.Lpu_id,MS.MedService_id,	WT.LpuHMPWorkTime_id
        from	v_MedService MS
        inner join v_MedServiceType MST on	MST.MedServiceType_id = MS.MedServiceType_id
        left join v_LpuHMPWorkTime WT on	WT.MedService_id = MS.MedService_id
        where	MST.MedServiceType_SysNick = 'slneotl';
        loop 
        fetch med_service_cursor into p_Lpu_id,	p_MedService_id,	p_LpuHMPWorkTime_id ;
        EXIT WHEN NOT FOUND;
        if p_LpuHMPWorkTime_id is null
        then
        select fnc.LpuHMPWorkTime_id, fnc.Error_Code,fnc.Error_Message into p_LpuHMPWorkTime_id, p_Error_Code,p_Error_Message from p_LpuHMPWorkTime_ins(LpuHMPWorkTime_id := p_LpuHMPWorkTime_id,Lpu_id := p_Lpu_id,
        MedService_id := p_MedService_id, LpuHMPWorkTime_MoFrom := p_LpuHMPWorkTime_MoFrom,	LpuHMPWorkTime_MoTo := p_LpuHMPWorkTime_MoTo,LpuHMPWorkTime_TuFrom := p_LpuHMPWorkTime_TuFrom,
        LpuHMPWorkTime_TuTo := p_LpuHMPWorkTime_TuTo,LpuHMPWorkTime_WeFrom := p_LpuHMPWorkTime_WeFrom,LpuHMPWorkTime_WeTo := p_LpuHMPWorkTime_WeTo,LpuHMPWorkTime_ThFrom := p_LpuHMPWorkTime_ThFrom,
        LpuHMPWorkTime_ThTo := p_LpuHMPWorkTime_ThTo,LpuHMPWorkTime_FrFrom := p_LpuHMPWorkTime_FrFrom,LpuHMPWorkTime_FrTo := p_LpuHMPWorkTime_FrTo,LpuHMPWorkTime_SaFrom := p_LpuHMPWorkTime_SaFrom,
        LpuHMPWorkTime_SaTo := p_LpuHMPWorkTime_SaTo,LpuHMPWorkTime_SuFrom := p_LpuHMPWorkTime_SuFrom,LpuHMPWorkTime_SuTo := p_LpuHMPWorkTime_SuTo,	pmUser_id := p_pmUser_id) fnc;
        else
        select fnc.LpuHMPWorkTime_id, fnc.Error_Code,fnc.Error_Message into p_LpuHMPWorkTime_id, p_Error_Code,p_Error_Message from p_LpuHMPWorkTime_upd (LpuHMPWorkTime_id := p_LpuHMPWorkTime_id,Lpu_id := p_Lpu_id,
        MedService_id := p_MedService_id, LpuHMPWorkTime_MoFrom := p_LpuHMPWorkTime_MoFrom,	LpuHMPWorkTime_MoTo := p_LpuHMPWorkTime_MoTo,LpuHMPWorkTime_TuFrom := p_LpuHMPWorkTime_TuFrom,
        LpuHMPWorkTime_TuTo := p_LpuHMPWorkTime_TuTo,LpuHMPWorkTime_WeFrom := p_LpuHMPWorkTime_WeFrom,LpuHMPWorkTime_WeTo := p_LpuHMPWorkTime_WeTo,LpuHMPWorkTime_ThFrom := p_LpuHMPWorkTime_ThFrom,
        LpuHMPWorkTime_ThTo := p_LpuHMPWorkTime_ThTo,LpuHMPWorkTime_FrFrom := p_LpuHMPWorkTime_FrFrom,LpuHMPWorkTime_FrTo := p_LpuHMPWorkTime_FrTo,LpuHMPWorkTime_SaFrom := p_LpuHMPWorkTime_SaFrom,
        LpuHMPWorkTime_SaTo := p_LpuHMPWorkTime_SaTo,LpuHMPWorkTime_SuFrom := p_LpuHMPWorkTime_SuFrom,LpuHMPWorkTime_SuTo := p_LpuHMPWorkTime_SuTo,	pmUser_id := p_pmUser_id) fnc;
	...
	function checkAllowT9($session) {
		...
		$sql = "
			...
			from AccessRightsT9Limit t9
				...
				left join v_Lpu L on L.Org_id=t9.Org_id and L.Lpu_id = :Lpu_id
