 ...
 */
class AnalyzerWorksheet_model extends SwPgModel {
	...
	function loadList($filter) {
		...
		$q = "
			...
			FROM
				lis.v_AnalyzerWorksheet AnalyzerWorksheet
				LEFT JOIN lis.v_AnalyzerRack AnalyzerRack_id_ref ON AnalyzerRack_id_ref.AnalyzerRack_id = AnalyzerWorksheet.AnalyzerRack_id
				LEFT JOIN lis.v_AnalyzerWorksheetStatusType AnalyzerWorksheetStatusType_id_ref ON AnalyzerWorksheetStatusType_id_ref.AnalyzerWorksheetStatusType_id = AnalyzerWorksheet.AnalyzerWorksheetStatusType_id
				LEFT JOIN lis.v_AnalyzerWorksheetType AnalyzerWorksheetType_id_ref ON AnalyzerWorksheetType_id_ref.AnalyzerWorksheetType_id = AnalyzerWorksheet.AnalyzerWorksheetType_id
				LEFT JOIN lis.v_Analyzer Analyzer_id_ref ON Analyzer_id_ref.Analyzer_id = AnalyzerWorksheet.Analyzer_id
			$where_clause
		";
	...
	function work($data) {
		...
		$this->load->model('AnalyzerWorksheetEvnLabSample_model');
		$this->load->model('Lis_model');
	...
	function delete() {
		//проверяем статус
		$status = $this->getFirstResultFromQuery('SELECT AnalyzerWorksheetStatusType_id FROM lis.v_AnalyzerWorksheet WHERE AnalyzerWorksheet_id = :AnalyzerWorksheet_id', array('AnalyzerWorksheet_id' => $this->AnalyzerWorksheet_id));
		if ('1' !== $status) {
			throw new Exception('Удалять можено только те рабочие списки, которые имеют статус "Новые"');
		}
		//последовательно удаляем все пробы
		$samplesIndaShit = $this->db->query('SELECT AnalyzerWorksheetEvnLabSample_id FROM lis.v_AnalyzerWorksheetEvnLabSample WHERE AnalyzerWorksheet_id = :AnalyzerWorksheet_id', array('AnalyzerWorksheet_id' => $this->AnalyzerWorksheet_id));
