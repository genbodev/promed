...
class StickFSSData_model extends swPgModel {
	/**
	 *	Конструктор
	 */
	...
	public function loadStickFSSDataGrid($data) {
		...
		$query = "
			select
				SFD.StickFSSData_id as \"StickFSSData_id\",
				L.Lpu_id as \"Lpu_id\",
				L.Lpu_Nick as \"Lpu_Nick\",
				L.Lpu_FSSRegNum as \"Lpu_FSSRegNum\",
				O.Org_INN as \"Lpu_INN\",
				SFD.Lpu_OGRN as \"Lpu_OGRN\",
				PS.Person_Snils as \"Person_Snils\",
				SFD.StickFSSDataStatus_id as \"StickFSSDataStatus_id\",
				SFD.StickFSSData_Num as \"StickFSSData_Num\",
				SFT.StickFSSType_Code as \"StickFSSType_Code\",
				SFT.StickFSSType_Name as \"StickFSSType_Name\",
				SFD.StickFSSData_IsNeedMSE as \"StickFSSData_IsNeedMSE\",
				case 
				...
				to_char(SFD.StickFSSData_insDT, 'dd.mm.yyyy HH24:MI:SS') as \"StickFSSData_insDT\",
				...
				SFD.StickFSSData_StickNum as \"StickFSSData_StickNum\",
				to_char(SFD.StickFSSData_xmlExpDT, 'DD.MM.YYYY') || ' ' || to_char(SFD.StickFSSData_xmlExpDT, 'HH24:MI:SS') as \"StickFSSData_xmlExpDT\",


				SFDS.StickFSSDataStatus_Name as \"StickFSSDataStatus_Name\",
				case when exists (
					select 
					...
					from
						...
						inner join v_StickFSSDataGet SFDG  on SFE.StickFSSDataGet_id = SFDG.StickFSSDataGet_id
			...
			from
				...
				left join lateral(
					...
					from 
						v_EvnStickBase ESB
						inner join v_StickFSSType SFT on SFT.StickFSSType_id = ESB.StickFSSType_id
					where
						ESB.EvnStickBase_id = SFD.EvnStickBase_id
					limit 1
				) SFT on true
				
				left join v_pmUserCache puc  on puc.pmUser_id = SFD.pmUser_insID

				left join v_StickFSSDataStatus SFDS  on SFDS.StickFSSDataStatus_id = SFD.StickFSSDataStatus_id

				left join v_EvnStickBase ESB  on ESB.StickFSSData_id = SFD.StickFSSData_id

				left join v_PersonState PS  on PS.Person_id = SFD.Person_id

				left join v_EvnPL EvnPL  on ESB.EvnStickBase_pid = EvnPL.EvnPL_id

				left join v_EvnPLStom EvnPLStom  on ESB.EvnStickBase_pid = EvnPLStom.EvnPLStom_id

				left join v_EvnPS EvnPS  on ESB.EvnStickBase_pid = EvnPS.EvnPS_id

				left join v_Lpu L  on L.Lpu_id = SFD.Lpu_id
	...
	public function loadStickFSSDataGetGrid($data) {
		$query = "
			select
				COALESCE(SFDG.Person_SurName, '') || COALESCE(' ' || SFDG.Person_FirName, '') || COALESCE(' ' || SFDG.Person_SecName, '') as \"Person_Fio\",

				to_char(SFDG.Person_BirthDay, 'DD.MM.YYYY') as \"Person_BirthDay\",

				SFDG.Person_Snils as \"Person_Snils\",
				SFDG.StickFSSDataGet_StickNum as \"StickFSSDataGet_StickNum\",
				to_char(SFDG.StickFSSDataGet_StickSetDate, 'DD.MM.YYYY') as \"StickFSSDataGet_StickSetDate\",

				SFDG.Lpu_StickNick as \"Lpu_StickNick\",
				SFDG.StickFSSDataGet_StickReason as \"StickFSSDataGet_StickReason\",
				...
				SFDG.StickFSSDataGet_StickResult as \"StickFSSDataGet_StickResult\",
				to_char(SFDG.StickFSSDataGet_changeDate, 'DD.MM.YYYY') as \"StickFSSDataGet_changeDate\",
				to_char(SFDG.StickFSSDataGet_workDate, 'DD.MM.YYYY') as \"StickFSSDataGet_workDate\",
				to_char(SFDG.EvnStick_mseDT, 'DD.MM.YYYY') as \"EvnStick_mseDT\",
				to_char(SFDG.EvnStick_mseRegDT, 'DD.MM.YYYY') as \"EvnStick_mseRegDT\",
				to_char(SFDG.EvnStick_mseExamDT, 'DD.MM.YYYY') as \"EvnStick_mseExamDT\",
				SFDG.InvalidGroupType_id as \"InvalidGroupType_id\"
			from
				v_StickFSSDataGet SFDG 
	...
	public function loadStickFSSErrorGrid($data) {
		$query = "
			select
				SFE.StickFSSError_id as \"StickFSSError_id\",
				SFET.StickFSSErrorType_Code as \"StickFSSErrorType_Code\",
				SFET.StickFSSErrorType_Name as \"StickFSSErrorType_Name\",
				SFDG.StickFSSDataGet_StickNum as \"StickFSSDataGet_StickNum\"
			from
				v_StickFSSError SFE 

				left join v_StickFSSErrorType SFET  on SFET.StickFSSErrorType_id = SFE.StickFSSErrorType_id

				left join v_StickFSSDataGet SFDG  on SFDG.StickFSSDataGet_id = SFE.StickFSSDataGet_id
	...
	function getNewStickFSSDataNum($data) {
		...
		$query = "
			select
				COALESCE(max(StickFSSData_Num::integer), 0)+1 as \"StickFSSData_Num\",
			...
			from 
				...
				left join v_StickFSSData SFD  on SFD.Lpu_id = L.Lpu_id
		...
		if (empty($data['ignoreCheckExist'])) {
			...
			$resp = $this->queryResult("
				...
				from
					...
					left join v_PersonState PS  on PS.Person_id = ESB.Person_id
		...
		if (!empty($resp[0]['StickFSSData_id'])) {
			...
			$resp_users = $this->queryResult("
				...
				where
					puc.Lpu_id = :Lpu_id
                  and 
                    exists(select pucgl.pmUserCacheGroupLink_id
                               ...
                               where pucgl.pmUserCacheGroup_id =
                                     ...
                                     pucgl.pmUserCache_id = puc.PMUser_id) -- АРМ администратора МО

					and COALESCE(puc.pmUser_deleted, 1) = 1

					and puc.pmUser_EvnClass iLIKE '%StickFSSData%'
	...
	function getStickFssData($data) {
		...
		$query = "
			select
				SFD.EvnStickBase_id as \"EvnStickBase_id\",
				SFD.Lpu_id as \"Lpu_id\",
				SFD.Lpu_OGRN as \"Lpu_OGRN\",
				SFD.Person_id as \"Person_id\",
				SFD.Person_Snils as \"Person_Snils\",
				SFD.StickFSSData_createDate as \"StickFSSData_createDate\",
				SFD.StickFSSData_id as \"StickFSSData_id\",
				SFD.StickFSSData_IsNeedMSE as \"StickFSSData_IsNeedMSE\",
				SFD.StickFSSData_Num as \"StickFSSData_Num\",
				SFD.StickFSSData_StickNum as \"StickFSSData_StickNum\",
				SFD.StickFSSData_xmlExpDT as \"StickFSSData_xmlExpDT\",
				SFD.StickFSSData_xmlExportPath as \"StickFSSData_xmlExportPath\",
				SFD.StickFSSDataStatus_id as \"StickFSSDataStatus_id\",
	...
	function showFiles($data) {
		$resp = $this->queryResult("
			select
				...
				to_char(StickFSSData_xmlExpDT, 'YYYY-MM-DD HH24:MI:SS') as \"date\"
	...
	public function checkStickFSSDataGet($data) {
		$this->db->query("delete from StickFSSError where StickFSSDataGet_id = :StickFSSDataGet_id", array(
		...
		$resp = $this->queryResult("
			select 
				SFDG.StickFSSDataGet_id as \"StickFSSDataGet_id\",
				ESB.EvnStickBase_id as \"EvnStickBase_id\",
				SFDG.Org_StickNick as \"Org_StickNick\",
				SFDG.StickFSSDataGet_EmploymentType as \"StickFSSDataGet_EmploymentType\",
				SFDG.StickFSSDataGet_IsEmploymentServices as \"StickFSSDataGet_IsEmploymentServices\",
				SFDG.StickFSSDataGet_StickSetDate as \"StickFSSDataGet_StickSetDate\",
				...
				to_char(SFDG.Person_BirthDay, 'DD.MM.YYYY') as \"FSS_Person_BirthDay\",
				...
				PS.Sex_id as \"Sex_id\",
				SFDG.StickFSSDataGet_StickStatus as \"StickFSSDataGet_StickStatus\",
				to_char(SFDG.EvnStick_mseDT, 'YYYY-MM-DD') as \"EvnStick_mseDT\",
				to_char(SFDG.EvnStick_mseExamDT, 'YYYY-MM-DD') as \"EvnStick_mseExamDT\",
				to_char(SFDG.EvnStick_mseRegDT, 'YYYY-MM-DD') as \"EvnStick_mseRegDT\",
				SFDG.InvalidGroupType_id as \"InvalidGroupType_id\",
				to_char(ESWR.EvnStickWorkRelease_begDT, 'YYYY-MM-DD HH24:MI:SS') as \"EvnStickWorkRelease_begDT\",

				to_char(ESB.EvnStickBase_disDT, 'YYYY-MM-DD HH24:MI:SS') as \"EvnStickBase_disDT\",

				to_char (coalesce(SFDG.StickFSSDataGet_ThirdEndDate, SFDG.StickFSSDataGet_SecondEndDate, SFDG.StickFSSDataGet_FirstEndDate), 'YYYY-MM-DD HH24:MI:SS') as \"FSS_EvnStickWorkRelease_endDT\",
				ESB.EvnStickBase_id as \"EvnStickBase_id\",
				ES.StickLeaveType_id as \"StickLeaveType_id\"
			from
				v_StickFSSDataGet SFDG 

				left join v_StickFSSData SFD  on SFD.StickFSSData_id = SFDG.StickFSSData_id
	...
	public function saveStickFSSError($data) {
		return $this->queryResult("
			...
			from dbo.p_StickFSSError_ins(
            	StickFSSError_id := null,
		...
		if (!empty($FileOperationsLnUserGetLNDataOut)) {
			...
			if (!empty($status)) {
				if ($status->nodeValue == '1') {
					...
					$this->db->query("delete from StickFSSDataGet where StickFSSData_id = :StickFSSData_id", array(
						'StickFSSData_id' => $data['StickFSSData_id']
					));
					$resp_save = $this->queryResult("
							with mv as (
								select
									coalesce(ESB.EvnStickBase_id, SFD.EvnStickBase_id, ESB2.EvnStickBase_id) as EvnStickBase_id
								from
									v_StickFSSData SFD
									left join v_EvnStickBase ESB on ESB.StickFSSData_id = SFD.StickFSSData_id
									left join v_EvnStickBase ESB2 on ESB2.EvnStickBase_Num = SFD.StickFSSData_StickNum
								where
									SFD.StickFSSData_id = :StickFSSData_id
							)
							...
							from p_StickFSSDataGet_ins(
								EvnStickBase_id := (select EvnStickBase_id from mv),
								StickFSSData_id := :StickFSSData_id,
					...
					if (
						...
						$LvnOld_resp = $this->db->query("
							select
								...
								to_char(ESB.EvnStickBase_disDT, 'YYYY-MM-DD') as \"EvnStickBase_disDT\"
							from
								v_EvnStickBase ESB
								inner join v_StickLeaveType SLT on StickLeaveType_id = ESB.StickLeaveType_rid
							where
								ESB.EvnStickBase_Num = :StickFSSData_StickNum
							limit 1
						", $queryParams);
						...
						if (
							...
							$this->db->query("
							update EvnStick ES set
                            StickLeaveType_id = (select StickLeaveType_id from v_StickLeaveType where StickLeaveType_Code = :lnResult limit 1) 
                            where ES.EvnStickBase_Num = :StickFSSData_StickNum
							", $queryParams);
						}

						if (!empty($queryParams['lnResult_Date'])) {
							$this->db->query("
							update EvnStickBase
							set Evn_disDT = :lnResult_Date
							where EvnStickBase_Num = :StickFSSData_StickNum and (EvnStickBase.evn_deleted = 1 OR EvnStickBase.evn_deleted IS null)
							", $queryParams);
						}
					}
					...
					if (!empty($resp_save[0]['StickFSSDataGet_id'])) {
						...
						$this->db->query("
							...
							from
								EvnStickBase esb
								inner join v_StickFSSDataGet sfdg on sfdg.StickFSSDataGet_StickNum = esb.EvnStickBase_Num
					...
					$this->db->query("update StickFSSData set StickFSSDataStatus_id = 4 where StickFSSData_id = :StickFSSData_id", array(
						'StickFSSData_id' => $data['StickFSSData_id']
					));
					...
					$resp_users = $this->queryResult("
							...
							where
								...
								and exists(
									...
									from v_pmUserCacheGroupLink pucgl
									where pucgl.pmUserCacheGroup_id = (select pmUserCacheGroup_id from mv)
							...
							if (!empty($resp_err)) {
								...
								foreach ($resp_err as $one_err) {
									$resp_errname = $this->queryResult("
										select
											StickFSSErrorType_Name as \"StickFSSErrorType_Name\"
										from
											v_StickFSSErrorType
										where
											StickFSSErrorType_id = :StickFSSErrorType_id
					...
					$this->db->query("update StickFSSData set StickFSSDataStatus_id = 5 where StickFSSData_id = :StickFSSData_id", array(
						'StickFSSData_id' => $data['StickFSSData_id']
					));
	...
	public function queryStickFSSData($data) {
		...
		$this->db->query("update StickFSSData set StickFSSData_xmlExpDT = dbo.tzGetDate(), StickFSSData_xmlExportPath = :StickFSSData_xmlExportPath where StickFSSData_id = :StickFSSData_id", array(
			'StickFSSData_id' => $data['StickFSSData_id'],
		...
		if (!empty($response)) {
			...
			$this->db->query("update StickFSSData set StickFSSDataStatus_id = 2 where StickFSSData_id = :StickFSSData_id", array(
				'StickFSSData_id' => $data['StickFSSData_id']
			));
			...
			$zip->close();
			// записываем ответ
			$this->db->query("update StickFSSData set StickFSSData_xmlImportPath = :StickFSSData_xmlImportPath where StickFSSData_id = :StickFSSData_id", array(
				'StickFSSData_id' => $data['StickFSSData_id'],
