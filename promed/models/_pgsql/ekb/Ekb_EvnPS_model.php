	...
	function exportHospDataForTfomsToXml($data) {
		...
		$query1 = "
			...
			cast(extract(year from EvnDirection_setDT) as varchar)
				|| right(REPEATE('0', 4) || COALESCE(lp.Lpu_RegNomN2, ''), 4)
				|| right(REPEATE('0', 6) || cast(EvnDirection_Num as varchar), 6)
			as \"NUMBTREND\",	--T(16) Номер направления (MCOD+NP)
			to_char(EvnDirection_setDT, 'yyyy-mm-dd') as \"DATETREND\",
			case when dt.DirType_Code = 5 then 2 else 1 end as \"EXTR\", -- Int - Форма оказания медицинской помощи
			ls.LpuSection_Code as \"NPR_PODR\", -- T(8)-- numeric(4, 0) - Код подразделения медицинской организации, направившей на госпитализацию
			lp.Lpu_f003mcod as \"MO_TREND\", -- T(6)-- numeric(6, 0) - МО, направившая на госпитализацию	(реестровый номер, F003)
			lp1.Lpu_f003mcod as \"MO_IN\", -- T(6)-- numeric(6, 0) - МО, куда направлен пациент	(реестровый номер, F003)
			ls1.LpuSection_Code as \"PODR\", -- T(8) Код подразделения медицинской организации, куда выписано направление
			CEILING(fBP.LpuSectionBedProfile_Code) as \"PROFBERTH\", --numeric(4, 0) - Профиль койки
			pt.PolisType_CodeF008 as \"VPOLIS\",-- numeric(1, 0) , -- Тип документа, подтверждающего факт страхования по ОМС (F008)
			case when pt.PolisType_CodeF008 in (1) then po.Polis_Ser else Null end as \"SPOLIS\", -- Varchar(10) - Серия документа, подтверждающего факт страхования по ОМС
			case
				when pt.PolisType_CodeF008=1 then
					case when length(po.Polis_Num)<7 then right(repeate('0',7) || po.Polis_Num,7) else po.Polis_Num end
				when pt.PolisType_CodeF008=2 then
					case when length(po.Polis_Num)<9 then right(repeate('0',9) || po.Polis_Num,9) else po.Polis_Num end
				else
					case when length(pe.Person_EdNum)<16 then right(repeate('0',16) || pe.Person_EdNum,16) else pe.Person_EdNum end
			end as \"NPOLIS\", -- Numeric(20) - Номер документа, подтверждающего факт страхования по ОМС
			pe.Person_SurName as \"FAM\", --varchar(30) – Фамилия
			pe.Person_FirName as \"IM\", --varchar(30) – Имя
			pe.Person_SecName as \"OT\", --varchar(30) – Отчество
			case when pe.Sex_id=3 then 1 else pe.Sex_id end as \"W\", --numeric(1) – пол (1 - муж, 2 - жен)
			to_char(pe.Person_BirthDay, 'yyyy-mm-dd') as \"DR\", --date – дата рождения
			coalesce(pe.PersonPhone_Phone, 'не указан') as \"PHONE\", --Varchar(100) - Контактная информация
			coalesce(pa.Address_Address, ua.Address_Address, '') as \"ADRRESSG\", --Адрес проживания
			case when right(di.Diag_Code,1) = '.'
				then SUBSTRING(di.Diag_Code FROM 1 FOR LENGTH(di.Diag_Code)-1)
				else di.Diag_Code
			end as \"MKB\", --Char(4) - Код диагноза по МКБ
			dlo.CodeDLO as \"DOCTOR\", --Код врача, выписавшего больного на диспансеризацию
			case when eq.EvnQueue_id is not null
				then to_char(ed.EvnDirection_setDT + interval '29 days', 'yyyy-mm-dd')
				else to_char(tt.TimetableStac_setDate, 'yyyy-mm-dd')
			end as \"DATEPLAN\", -- date - Плановая дата госпитализации
			to_char(tt.TimetableStac_setDate, 'yyyy-mm-dd') as \"DATEAGREE\", --Согласованная дата госпитализации
			...
			left join persis.KodDLO dlo on mp.MedPersonal_id=dlo.MedWorker_id
			...
			--inner join dbo.v_LpuUnit lu1 on ls1.LpuUnit_id=lu1.LpuUnit_id and lu1.LpuUnitType_SysNick='stac'
			--inner join dbo.v_LpuBuilding lb1 on lb1.LpuBuilding_id = lu1.LpuBuilding_id
			--inner join dbo.LpuSectionProfile lspf on ls1.LpuSectionProfile_id=lspf.LpuSectionProfile_id
			...
			left join lateral (
				...
				from
					...
					inner join fed.v_LpuSectionBedProfileLink LSBPL on LSBPL.LpuSectionBedProfileLink_id = LSBS.LpuSectionBedProfileLink_fedid
					inner join fed.v_LpuSectionBedProfile fLSBP on fLSBP.LpuSectionBedProfile_id = LSBPL.LpuSectionBedProfile_fedid
					inner join dbo.v_LpuSectionBedProfile LSBP on LSBP.LpuSectionBedProfile_id = LSBS.LpuSectionBedProfile_id 
				where 
			...
			left join dbo.Address pa on pe.PAddress_id=pa.Address_id
			left join dbo.Address ua on pe.UAddress_id=ua.Address_id
			left join dbo.v_LpuUnit lu on lu.LpuUnit_id = ls1.LpuUnit_id
			left join lateral (
				select t.PersonCard_id, t.LpuRegion_id
			...
			left join r66.v_LpuSectionLink lsl on lsl.LpuSection_id = ls1.LpuSection_id
			left join fed.v_MedicalCareKind mck on mck.MedicalCareKind_id = lsl.MedicalCareKind_id
			left join dbo.DirFailType dft on dft.DirFailType_id = ed.DirFailType_id
			where (1=1)
			and ed.DirType_id in (1,5)
			and (:Lpu_id is null or ed.Lpu_id = :Lpu_id)
			--and ed.EvnDirection_setDT >= :Date + time ':startTime' - '2 days' and ed.EvnDirection_setDT <= :Date + time ':finalTime' - '1 day'
			and cast(ed.EvnDirection_setDT as date) = :Date - '1 day'
			and (ed.LpuSection_did is null
				or
				(lu.LpuUnitType_SysNick = 'stac'
				and ls1.LpuSection_Code <> 660267
				and mck.MedicalCareKind_Code in (3,31)
				and exists(
					select *
					from v_LpuSectionFinans lsf
					inner join v_PayType PayType on PayType.PayType_id = lsf.PayType_id
					where lsf.LpuSection_id = ls1.LpuSection_id
					and lsf.LpuSectionFinans_begDate <= ed.EvnDirection_setDT
					and (lsf.LpuSectionFinans_endDate is null or lsf.LpuSectionFinans_endDate > ed.EvnDirection_setDT)
					and PayType.PayType_SysNick = 'oms'
			...
			and coalesce(dft.DirFailType_Code, 0) != 14  -- Неверный ввод
		";
		...
		$query2 = "
			...
			case
				...
				then
					cast(extract(year from eps.EvnDirection_setDT) as varchar)
						|| right(REPEATE('0', 4) || COALESCE(lp.Lpu_RegNomN2, ''), 4)
						|| right(REPEATE('0', 6) || cast(eps.EvnDirection_Num as varchar), 6)
				else
					cast(extract(year from ed.EvnDirection_setDT) as varchar)
						|| right(REPEATE('0', 4) || COALESCE(lp.Lpu_RegNomN2, ''), 4)
						|| right(REPEATE('0', 6) || cast(ed.EvnDirection_Num as varchar), 6)
			end as \"NUMBTREND\",	--T(16) Номер направления (MCOD+NP)
			to_char(coalesce(ed.EvnDirection_setDT,eps.EvnDirection_setDT), 'yyyy-mm-dd') as \"DATETREND\", --дата направления
			prt.PrehospType_Code as \"EXTR\", -- Int - Форма оказания медицинской помощи
			lp.Lpu_f003mcod as \"MO_IN\", -- T(6)-- numeric(6, 0) - МО госпитализации	(реестровый номер, F003)
			ls.LpuSection_Code as \"PODR\", --Код подразделения МО госпитализации
			to_char(eps.EvnPS_setDt, 'yyyy-mm-ddThh24:mi:ss') as \"DFACT\", --Дата и время фактической госпитализации
			to_char(eps.EvnPS_setDt + '10 days', 'yyyy-mm-dd') as \"DLEAVE\",
			pt.PolisType_CodeF008 as \"VPOLIS\",-- numeric(1, 0) , -- Тип документа, подтверждающего факт страхования по ОМС (F008)
			case when pt.PolisType_CodeF008 in (1) then po.Polis_Ser else Null end as \"SPOLIS\", -- Varchar(10) - Серия документа, подтверждающего факт страхования по ОМС
			case
				when pt.PolisType_CodeF008=1 then
			...
			--left join dbo.v_LpuSection ls1 on ls1.LpuSection_id = ed.LpuSection_id
			...
			inner join dbo.v_LpuUnit lu on lu.LpuUnit_id = ls.LpuUnit_id
			left join r66.v_LpuSectionLink lsl on lsl.LpuSection_id = ls.LpuSection_id
			left join fed.v_MedicalCareKind mck on mck.MedicalCareKind_id = lsl.MedicalCareKind_id
			where (1=1)
			and (:Lpu_id is null or eps.Lpu_id=:Lpu_id)
			and eps.EvnPS_setDT >= :Date + time ':startTime' - '2 days' and eps.EvnPS_setDT <= :Date + time ':finalTime' - '1 day'
			and eps.PrehospWaifRefuseCause_id is null --and prt.PrehospType_Code=1
			and lu.LpuUnitType_SysNick = 'stac'
			and ls.LpuSection_Code <> 660267
			and mck.MedicalCareKind_Code in (3,31)
			...
			and
			(prt.PrehospType_SysNick = 'extreme' or ed.EvnDirection_id is not null
			or (SUBSTRING(eps.EvnDirection_Num FROM 1 FOR 4) = cast(extract(year from eps.EvnDirection_setDT) as varchar)
			and SUBSTRING(eps.EvnDirection_Num FROM 5 FOR 4) = right('0000' || coalesce(lp.Lpu_RegNomN2,'0'),4)
			and length(SUBSTRING(eps.EvnDirection_Num FROM 9 FOR LENGTH(eps.EvnDirection_Num))) > 0))
		";
		...
		$query3 = "
			...
			select
			0+ROW_NUMBER() over (order by ed.EvnDirection_id) as \"N_ZAP\", --Int -- bigint , Идентификатор направления
			cast(extract(year from ed.EvnDirection_setDT) as varchar) || case
			when LENGTH(COALESCE(cast(lp.Lpu_RegNomN2 as varchar),'0'))<4
			then REPEATE('0', 4-LENGTH(COALESCE(cast(lp.Lpu_RegNomN2 as varchar),'0'))) || COALESCE(cast(lp.Lpu_RegNomN2 as varchar),'0')
			else COALESCE(cast(lp.Lpu_RegNomN2 as varchar),'0') end
			|| right(REPEATE('0', 6) || cast(ed.EvnDirection_Num as varchar),6) as \"NUMBTREND\", --T(16) Номер направления (MCOD+NP)
			to_char(ed.EvnDirection_setDT, 'yyyy-mm-dd') as \"DATETREND\", --Дата направления
			lp.Lpu_f003mcod as \"MOTREND\", -- T(6)-- numeric(6, 0) - МО, направившая на госпитализацию	(реестровый номер, F003)
			case when dr.DirFailType_Code=5 then 1 when dr.DirFailType_Code=13 then 4 when dr.DirFailType_Code in (1,2,4,6,11,9,10) then 2 else 5 end as \"REASON\" -- int  Причина отмены направления
			from dbo.v_EvnDirection ed
			--left join r30.EvnNPR npr on ed.EvnDirection_id=npr.Evn_id
			...
			inner join v_Lpu lp on lp.Lpu_id = ed.Lpu_id
			left join v_LpuSection ls1 on ls1.LpuSection_id = ed.LpuSection_did
			left join dbo.v_LpuUnit lu on lu.LpuUnit_id = ls1.LpuUnit_id
			left join r66.v_LpuSectionLink lsl on lsl.LpuSection_id = ls1.LpuSection_id
			left join fed.v_MedicalCareKind mck on mck.MedicalCareKind_id = lsl.MedicalCareKind_id
			where(1=1)
			and ed.DirType_id in (1,5)
			and (:Lpu_id is null or ed.Lpu_id = :Lpu_id)
			--and ed.EvnDirection_failDT >= :Date + time ':startTime' - '2 days' and ed.EvnDirection_failDT <= :Date + time ':finalTime' - '1 day'
			and cast(ed.EvnDirection_failDT as date) = :Date - '1 day'
			and (ed.LpuSection_did is null
				or
				(lu.LpuUnitType_SysNick = 'stac'
				and ls1.LpuSection_Code <> 660267
		...
		foreach($lpu_arr as $lpu) {
			...
			$get_couch_info_arr = $this->getFirstRowFromQuery($query_couch_info, array(
				'Lpu_id' => $params['Lpu_id'],
				'Date' => Date('Y-m-d', strtotime($params['Date']))
			));
			if ($get_couch_info_arr == false) {
				return false;
			}

			$hosp_data_xml_arr[$lpu['fcode']]['m4'] = $get_couch_info_arr['get_couch_info_xml'];
