...
class AccessRightsTest_model extends AccessRights_model {
	...
	function loadAccessRightsGrid($data) {
		...
		$query = "
			...
			from
				v_AccessRightsTest ART
				left join v_AccessRightsName ARN on ARN.AccessRightsName_id = ART.AccessRightsName_id
			left join lateral (
                select
                    string_agg(UC.UslugaComplex_Code, ', ') as AccessRightsTest_Codes
                from
                     v_UslugaComplex UC
                left join v_AccessRightsTest ARTU on UC.UslugaComplex_id = ARTU.UslugaComplex_id
                where
	...
	function checkAccessRightsIntersection($data) {
		...
		$query = "
			select
					ARN.AccessRightsName_Name as \"AccessRightsName_Name\"
				from
					v_AccessRightsName ARN
					inner join v_AccessRightsTest ART on ARN.AccessRightsName_id = ART.AccessRightsName_id
				where
	...
	function saveAccessRightsTest($data) {
		...
		$query = "
		    select
		        AccessRightsTest_id as \"AccessRightsTest_id\",
	...
	function deleteAccessRights($data) {
		...
		$query = "
			select ART.AccessRightsTest_id as \"AccessRightsTest_id\"
	...
	function loadAccessRightsForm($data) {
		...
		$query = "
			select
				ART.AccessRightsTest_id as \"AccessRightsTest_id\",
