 ...
 */
class MorbusIBS_model extends swPGModel
{
	/**
	...
	*/
	...
	function setEvnNotifyIBS(EvnNotifyIBS_model $evn)
	{
		$morbus = $this->getFirstRowFromQuery('
			select 
				...
				MorbusIBS_id as "MorbusNephro_id"
			from v_MorbusIBS
	...
	public function autoCreate($data, $isAllowTransaction = true)
	{
		...
		$qr = $this->queryResult("
			select
				coalesce({$tableName}_id) as \"{$tableName}_id\"
			from v_{$tableName}
			where Morbus_id = :Morbus_id
            limit 1
		", $queryParams);

		if (isset($qr[0]["{$tableName}_id"]) && $qr[0]["{$tableName}_id"] == 0) {
			$query = "
				select
	...
	function getViewData($data)
	{
		...
		$query = "
			select
				...
				ndst.IBSType_id as \"IBSType_id\",
				ndst.IBSType_Name as \"IBSType_Name\",
				v_Diag.Diag_id as \"Diag_id\",
				v_Diag.Diag_FullName as \"Diag_Name\",
				MV.MorbusIBS_IsStenocardia as \"MorbusIBS_IsStenocardia\",
				IsStenocardia.YesNo_Name as \"IsStenocardia_Name\",
				MV.MorbusIBS_FuncClass as \"MorbusIBS_FuncClass\",
				MV.MorbusIBS_StressTest as \"IBSStressTest_id\",
				st.TubHistolResultType_Name as \"IBSStressTest_Name\",
				MV.MorbusIBS_IsEchocardiography as \"MorbusIBS_IsEchocardiography\",
				IsEchocardiography.YesNo_Name as \"IsEchocardiography_Name\",
				MV.MorbusIBS_IsHalterMonitor as \"MorbusIBS_IsHalterMonitor\",
				IsHalterMonitor.YesNo_Name as \"IsHalterMonitor_Name\",
				MV.MorbusIBS_IsMyocardInfarct as \"MorbusIBS_IsMyocardInfarct\",
				IsMyocardInfarct.YesNo_Name as \"IsMyocardInfarct_Name\",
				MV.MorbusIBS_IsMyocardIschemia as \"MorbusIBS_IsMyocardIschemia\",
				IsMyocardIschemia.YesNo_Name as \"IsMyocardIschemia_Name\",
				MV.MorbusIBS_IsFirstStenocardia as \"MorbusIBS_IsFirstStenocardia\",
				IsFirstStenocardia.YesNo_Name as \"IsFirstStenocardia_Name\",
				MV.MorbusIBS_IsNoStableStenocardia as \"MorbusIBS_IsNoStableStenocardia\",
				IsNoStableStenocardia.YesNo_Name as \"IsNoStableStenocardia_Name\",
				MV.MorbusIBS_IsRiseTS as \"MorbusIBS_IsRiseTS\",
				IsRiseTS.YesNo_Name as \"IsRiseTS_Name\",
				MV.MorbusIBS_IsSaveIschemia as \"MorbusIBS_IsSaveIschemia\",
				IsSaveIschemia.YesNo_Name as \"IsSaveIschemia_Name\",
				MV.MorbusIBS_IsBackStenocardia as \"MorbusIBS_IsBackStenocardia\",
				IsBackStenocardia.YesNo_Name as \"IsBackStenocardia_Name\",
				MV.MorbusIBS_IsShunting as \"MorbusIBS_IsShunting\",
				IsShunting.YesNo_Name as \"IsShunting_Name\",
				MV.MorbusIBS_IsStenting as \"MorbusIBS_IsStenting\",
				IsStenting.YesNo_Name as \"IsStenting_Name\",
				MV.MorbusIBS_IsKGIndication as \"MorbusIBS_IsKGIndication\",
				IsKGIndication.YesNo_Name as \"IsKGIndication_Name\",
				MV.UslugaComplex_id as \"UslugaComplex_id\",
				uc.UslugaComplex_Code as \"UslugaComplex_Code\",
				MV.MorbusIBS_IsKGConsent as \"MorbusIBS_IsKGConsent\",
				IsKGConsent.YesNo_Name as \"IsKGConsent_Name\",
				MV.MorbusIBS_IsKGFinished as \"MorbusIBS_IsKGFinished\",
				IsKGFinished.YesNo_Name as \"IsKGFinished_Name\",
				MV.MorbusIBS_id as \"MorbusIBS_id\",
			...
			from
				...
				inner join v_Diag on v_Diag.Diag_id = MV.Diag_nid
				left join v_IBSType ndst on ndst.IBSType_id = MV.IBSType_id
				left join v_UslugaComplex uc on uc.UslugaComplex_id = MV.UslugaComplex_id
				left join v_TubHistolResultType st on st.TubHistolResultType_id = MV.MorbusIBS_StressTest
				left join v_YesNo IsStenocardia on IsStenocardia.YesNo_id = MV.MorbusIBS_IsStenocardia
				left join v_YesNo IsBackStenocardia on IsBackStenocardia.YesNo_id = MV.MorbusIBS_IsBackStenocardia
				left join v_YesNo IsEchocardiography on IsEchocardiography.YesNo_id = MV.MorbusIBS_IsEchocardiography
				left join v_YesNo IsFirstStenocardia on IsFirstStenocardia.YesNo_id = MV.MorbusIBS_IsFirstStenocardia
				left join v_YesNo IsHalterMonitor on IsHalterMonitor.YesNo_id = MV.MorbusIBS_IsHalterMonitor
				left join v_YesNo IsKGConsent on IsKGConsent.YesNo_id = MV.MorbusIBS_IsKGConsent
				left join v_YesNo IsKGFinished on IsKGFinished.YesNo_id = MV.MorbusIBS_IsKGFinished
				left join v_YesNo IsKGIndication on IsKGIndication.YesNo_id = MV.MorbusIBS_IsKGIndication
				left join v_YesNo IsMyocardInfarct on IsMyocardInfarct.YesNo_id = MV.MorbusIBS_IsMyocardInfarct
				left join v_YesNo IsMyocardIschemia on IsMyocardIschemia.YesNo_id = MV.MorbusIBS_IsMyocardIschemia
				left join v_YesNo IsNoStableStenocardia on IsNoStableStenocardia.YesNo_id = MV.MorbusIBS_IsNoStableStenocardia
				left join v_YesNo IsRiseTS on IsRiseTS.YesNo_id = MV.MorbusIBS_IsRiseTS
				left join v_YesNo IsSaveIschemia on IsSaveIschemia.YesNo_id = MV.MorbusIBS_IsSaveIschemia
				left join v_YesNo IsShunting on IsShunting.YesNo_id = MV.MorbusIBS_IsShunting
				left join v_YesNo IsStenting on IsStenting.YesNo_id = MV.MorbusIBS_IsStenting
			where
	...
	protected function loadPrintData($id)
	{
		$query = "
			select
				MN.MorbusIBS_id as \"MorbusIBS_id\",
