...
class Krym_EvnSection_model extends EvnSection_model
{
	...
	function loadKSGKPGKOEF2018($data)
	{
		...
		$query = "
			SELECT
				...
				to_char(cast(COALESCE(es.EvnSection_disDate, es.EvnSection_setDate) to date), 'yyyy-mm-dd') as \"EvnSection_disDate\"
			FROM
				v_EvnSection es
			WHERE
			...
			$query = "
				...
				from
					...
					left join lateral(
						...
						from
							v_EvnDiagPS edps2
							inner join v_PolyTrauma pt2on pt2.Diag_id = edps2.Diag_id
						where
			...
			$query = "
				...
				from v_EvnUsluga eu
					...
					left join lateral (
						select  count(*) as cnt
						from v_MesLink ml
						inner join v_MesOld kpg on kpg.Mes_id = ml.Mes_sid
						inner join v_LpuSectionProfile lsp on lsp.LpuSectionProfile_id = kpg.LpuSectionProfile_id
	...
	protected function calcCoeffCTP2019($data)
	{
		...
		if ($data['LpuUnitType_id'] == '1' && isset($KSLPCodes[13])) {
			...
			if ( !empty($LuchTerUsluga_id) && !empty($EvnUslugaOper_id) ) {
				$needKSLP13 = true;
			}
			...
			if (!empty($data['EvnSectionIds'])) {
				$resp_es = $this->queryResult("
					...
					from
						...
						inner join fed.LpuSectionBedProfileLink LSBPLink on  LSBPLink.LpuSectionBedProfileLink_id = ES.LpuSectionBedProfileLink_fedid
						inner join v_LpuSectionBedProfile LSBP on LSBPLink.LpuSectionBedProfile_id = LSBP.LpuSectionBedProfile_id
						left join v_MesOldUslugaComplex mouc on mouc.Mes_id = (select Mes_id from v_MesOld where MesOld_Num = 'st38.001' limit 1) and mouc.Diag_id = es.Diag_id and mouc.MesOldUslugaComplex_begDT <= COALESCE(es.EvnSection_disDate, es.EvnSection_setDate) and (coalesce(mouc.MesOldUslugaComplex_endDT, es.EvnSection_disDate, es.EvnSection_setDate) >= COALESCE(es.EvnSection_disDate, es.EvnSection_setDate)) 
					where
						ds.Diag_Code = 'R54'
		...
		if ($data['LpuUnitType_id'] == '1' && isset($KSLPCodes[383])) {
			...
			$AttributeValue = $this->queryResult("
					...
					WHERE
						...
						and avis.AttributeVision_TablePKey = (select VolumeType_id from v_VolumeType where VolumeType_Code = '2018-СочетХирВмеш' limit 1)
						and avis.AttributeVision_IsKeyValue = 2
						and COALESCE(av.AttributeValue_begDate, :EvnSection_disDate) <= :EvnSection_disDate
						and COALESCE(av.AttributeValue_endDate, :EvnSection_disDate) >= :EvnSection_disDate
			...
			$EvnUsluga = $this->queryResult("
				with eus as (
					...
					FROM
						...
						inner join v_MesOldUslugaComplex mouc on mouc.UslugaComplex_id = eu.UslugaComplex_id and mouc.MesOldUslugaComplex_begDT <= eu.EvnUsluga_setDate and COALESCE(mouc.MesOldUslugaComplex_endDT, eu.EvnUsluga_setDate) >= eu.EvnUsluga_setDate
						left join v_LpuSection ls on ls.LpuSection_id = es.LpuSection_id
						left join v_LpuUnit lu on lu.LpuUnit_id = ls.LpuUnit_id
						inner join v_MesOld mo on mo.Mes_id = mouc.Mes_id and mo.MesType_id = (case when lu.LpuUnitType_id in (6,7,9) then 9 else 10 end)
					where
						es.EvnSection_id in (" . implode(',', $data['EvnSectionIds']) . ")
						and eu.EvnClass_id in (43,22,29)
						and eu.PayType_id = :PayTypeOms_id
	...
	protected function _recalcKSKP()
	{
		...
		// Движения должны группироваться по классу МКБ
		// Отмена группировки по классу МКБ
		// @task https://redmine.swan.perm.ru/issues/118452
	...
	protected function recalcIndexNumByGroup($data = array())
	{
		...
		if (empty($data['getGroupsOnly'])) {
			...
			from
				EvnSection es
				inner join Evn e on e.Evn_id = es.EvnSection_id
			where
				e.Evn_pid = :EvnSection_pid
			";
			*/
		...
		$resp_es = $this->queryResult("
			select
				...
				mtmes.Mes_Code as \"Mes_Code\",
				mtmes.MesOld_Num as \"MesOld_Num\",
