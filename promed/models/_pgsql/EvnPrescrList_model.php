 ...
 */
class EvnPrescrList_model extends swPgModel
{
	/**
	 * Конструктор
	 */
	...
	public function getPrescrPlanView($data) {
		...
		$query = "
			select
				...
				PrescriptionType_Code as \"PrescriptionType_Code\",
				PrescriptionType_Name as \"PrescriptionType_Name\"
			from v_PrescriptionType
		...
		$response = array(array(
			'days' => array(
				...
				'EvnPS_setDate'=> ConvertDateFormat($options['EvnPS_setDT'], 'd.m.Y'),
				'Evn_setDate'=> ConvertDateFormat($options['Evn_setDT'], 'd.m.Y'),
				'cur_date'=> ConvertDateFormat($cur_date, 'd.m.Y'),
				'days_diff'=> DateTime::createFromFormat('d.m.Y', $options['Evn_setDT'])->diff($cur_date)->days,
			),
		...
		foreach ($prescription as $prescr_group) {
			$response[] = array(
				...
				'PrescriptionType_Code'=>intval($prescr_group['code']),
    ...
    public function mGetPrescrPlanView($data)
    {
        ...
        $prescription_type = $result->result('array');
        foreach ($prescription_type as $row) {
            $prescription[$this->getIndexPrescriptionType($row['PrescriptionType_id'])] = [
                'id' => $row['PrescriptionType_id'],
                'code' => $row['PrescriptionType_Code'],
                'title' => $row['PrescriptionType_Name'],
                'items' => [],
		...
		$response['groups']['regime'] = ['PrescriptionType_id'=>1, 'PrescriptionType_name'=>"Режим"];
		$response['groups']['diet'] = ['PrescriptionType_id'=>2, 'PrescriptionType_name'=>"Диета"];
		$response['groups']['diag'] = ['PrescriptionType_id'=>3, 'PrescriptionType_name'=>"Диагностика"];
		$response['groups']['consul'] = ['PrescriptionType_id'=>4, 'PrescriptionType_name'=>"Консультация"];
		$response['groups']['drug'] = ['PrescriptionType_id'=>5, 'PrescriptionType_name'=>"Лекарственное лечение"];
		$response['groups']['proc'] = ['PrescriptionType_id'=>6, 'PrescriptionType_name'=>"Манипуляции и процедуры"];
		$response['groups']['surgical_treatment'] = ['PrescriptionType_id'=>7, 'PrescriptionType_name'=>"Оперативное лечение"];
		$response['groups']['mse'] = ['PrescriptionType_id'=>8, 'PrescriptionType_name'=>"МСЭ"];
		$response['groups']['vk'] = ['PrescriptionType_id'=>9, 'PrescriptionType_name'=>"ВК"];
		$response['groups']['watch'] = ['PrescriptionType_id'=>10, 'PrescriptionType_name'=>"Наблюдение"];
		$response['groups']['labdiag'] = ['PrescriptionType_id'=>11, 'PrescriptionType_name'=>"Лабораторная диагностика"];
		$response['groups']['funcdiag'] = ['PrescriptionType_id'=>12, 'PrescriptionType_name'=>"Инструментальная диагностика"];
		$response['groups']['consul_diag'] = ['PrescriptionType_id'=>13, 'PrescriptionType_name'=>"Консультационная услуга"];
	...
	function doloadEvnPrescrDoctorList($data) {
		...
		$query = "
			select
				...
				cast(coalesce(EP.EvnPrescr_setDT, evn.Evn_setDT) as date) as \"EvnPrescr_date\",
		...
		if ($isPolka) {
			...
			$addSelect .= "
				,date_part('day', coalesce(Regime.EvnPrescrRegime_setDT,Diet.EvnPrescrDiet_setDT,Obs.EvnPrescrObserv_setDT,EP.EvnPrescr_setDT) - cast(:EvnPrescr_begDate as timestamp)) + 1 as \"DayNum\"
				...
				,substring(coalesce(PMUP.PMUser_Name, '') from 1 for 1) as \"pmUserPrescr_Name\"
				,substring(coalesce(PMUE.PMUser_Name, '') from 1 for 1) as \"pmUserExec_Name\"
				,coalesce(PMUE.PMUser_surName, '') || ' '|| substring(coalesce(PMUE.PMUser_FirName, '') from 1 for 1) || '.' || substring(coalesce(PMUE.PMUser_SecName, '') from 1 for 1) || '.' as \"pmUserExec_FIO\"";
			$addJoin .= "
				left join v_pmUser PMUP on PMUP.pmUser_id = coalesce(Regime.pmUser_insID, Diet.pmUser_insID, Obs.pmUser_insID, EP.pmUser_insID)
		...
		$query = "
			select
				...
				LTRIM(to_char(ECTD.EvnCourseTreatDrug_KolvoEd, '9999999999D99')) as \"KolvoEd\",
				coalesce(df.NAME,Drug.DrugForm_Name) as \"DrugForm_Name\",
				case
					when ECTD.EvnCourseTreatDrug_Kolvo % 1 = 0
						then LTRIM(to_char(ECTD.EvnCourseTreatDrug_KolvoEd, '9999999999'))
					when ECTD.EvnCourseTreatDrug_Kolvo * 10 % 1 = 0
						then LTRIM(to_char(ECTD.EvnCourseTreatDrug_KolvoEd, '9999999999D9'))
					else
						LTRIM(to_char(ECTD.EvnCourseTreatDrug_KolvoEd, '9999999999D99'))
				end as \"Kolvo\",
				...
				--Параметры наблюдения as \"--Параметры наблюдения\",
		...
		убрал, пока не используется, но может позже понадобиться
				...
				left join YesNo IsCito on IsCito.YesNo_id = coalesce(Regime.EvnPrescrRegime_IsCito,Diet.EvnPrescrDiet_IsCito,Obs.EvnPrescrObserv_IsCito,EP.EvnPrescr_IsCito,1)
		 */
	...
	function doLoadPrescriptionGroupViewData($section, $evn_pid, $data = array())
	{
		...
		if (!empty($testFilter)){
			$join .= "inner join v_EvnPrescrLabDiag EPLD on EPLD.EvnPrescrLabDiag_id = EP.EvnPrescr_id
				...
				left join v_EvnDirection_all ED on epd.EvnDirection_id = ED.EvnDirection_id
				left join v_EvnLabRequest ELR on ELR.EvnDirection_id = epd.EvnDirection_id
			...
			$filter .= "
					and (
						...
						or exists (
							select Evn_id from v_Evn where Evn_id = :Evn_pid and EvnClass_sysNick = 'EvnSection' and Evn_setDT <= EP.EvnPrescr_setDT and (Evn_disDT is null or Evn_disDT >= EP.EvnPrescr_setDT) limit 1
			...
			select
				...
				case
					when 1 = pt.PrescriptionType_id
						then (select count(EP.PrescriptionType_id) from EP where 1 = EP.PrescriptionType_id)
					when 2 = pt.PrescriptionType_id
						then (select count(EP.PrescriptionType_id) from EP where 2 = EP.PrescriptionType_id)
					when 10 = pt.PrescriptionType_id
						then (select count(EP.PrescriptionType_id) from EP where 10 = EP.PrescriptionType_id)
					when 7 = pt.PrescriptionType_id
						then (select count(EP.PrescriptionType_id) from EP where 7 = EP.PrescriptionType_id)
					...
					when 12 = pt.PrescriptionType_id
						then (select count(EP.PrescriptionType_id) from EP where 12 = EP.PrescriptionType_id)
					when 13 = pt.PrescriptionType_id
						then (select count(EP.PrescriptionType_id) from EP where 13 = EP.PrescriptionType_id)
					when 5 = pt.PrescriptionType_id
						then (select count(EC.EvnClass_id) from EC where 114 = EC.EvnClass_id)
					when 6 = pt.PrescriptionType_id
						then (select count(EC.EvnClass_id) from EC where 116 = EC.EvnClass_id)
					else 0
				end as \"PrescriptionType_Cnt\"
			from v_PrescriptionType pt
	...
	private function _queryLoadViewData($data, &$options = array()) {
		...
		switch($data['parentEvnClass_SysNick']) {
			...
			case 'EvnVizitPL';
				...
				$edit_conditions = 'EvnVizitPL.Lpu_id = :Lpu_id AND coalesce(EvnVizitPL.EvnVizitPL_IsSigned,1) = 1';
				$add_join = 'left join v_EvnVizitPL EvnVizitPL on EvnVizitPL.EvnVizitPL_id = EP.EvnPrescr_pid';
				break;
			// назначения в рамках стоматологического посещения поликлиники
			case 'EvnVizitPLStom';
				$key_name = 'EvnPrescrStom_id';
				$edit_conditions = 'EvnVizitPLStom.Lpu_id = :Lpu_id AND coalesce(EvnVizitPLStom.EvnVizitPLStom_IsSigned,1) = 1';
				$add_join = 'left join v_EvnVizitPLStom EvnVizitPLStom on EvnVizitPLStom.EvnVizitPLStom_id = EP.EvnPrescr_pid';
				break;
			// назначения в рамках движения в стационаре
			case 'EvnSection';
				$key_name = 'EvnPrescrPlan_id';
				$edit_conditions = 'EvnSection.Lpu_id = :Lpu_id AND coalesce(EvnSection.EvnSection_IsSigned,1) = 1';
				$add_join = 'left join v_EvnSection EvnSection on EvnSection.EvnSection_id = EP.EvnPrescr_pid';
				break;
			// назначения в приемном отделении
			case 'EvnPS';
				$key_name = 'EvnPrescr_key';
				$edit_conditions = 'EvnPS.Lpu_id = :Lpu_id';
				$add_join = 'left join v_EvnPS EvnPS on EvnPS.EvnPS_id = EP.EvnPrescr_pid';
				break;
			...
			case 'EvnPSAll';
				$query = "
					select
						...
						to_char(Evn.Evn_setDT, 'dd.mm.yyyy') as \"Evn_setDT\"
					from v_Evn Evn
				...
				$key_name = 'EvnPrescr_key';
				$edit_conditions = 'Evn.Evn_id = :Evn_pid AND Evn.Lpu_id = :Lpu_id AND coalesce(Evn.Evn_IsSigned,1) = 1';
				$add_join = 'left join v_Evn Evn on Evn.Evn_id = EP.EvnPrescr_pid';
		...
		$query = "
			...
			and coalesce(PrescriptionStatusType_id,1) != 3
		";
		$result = $this->db->query($query, $queryParams);
		if ( !is_object($result) ) {
			return false;
		...
		$query = "
			select
				...
				EP.EvnPrescr_id
					||'-'|| coalesce(Regime.EvnPrescrRegime_id,Diet.EvnPrescrDiet_id,Obs.EvnPrescrObserv_id,EPOU.EvnPrescrOperUsluga_id,EPFDU.EvnPrescrFuncDiagUsluga_id,0)
				as \"{$key_name}\",
				...
				,case when coalesce(EU.EvnUsluga_id,EDr.EvnDrug_id) is null then 1 else 2 end as \"EvnPrescr_IsHasEvn\"
				...
				,case when (EP.PrescriptionType_id in (7,11,12,13) and 2 = EP.EvnPrescr_IsExec) then to_char(EP.EvnPrescr_updDT, 'dd.mm.yyyy')
					||' '||to_char(EP.EvnPrescr_updDT, 'HH24:MI:SS') 
					else null 
				...
				,PT.PrescriptionType_Code as \"PrescriptionType_Code\"
				,PT.PrescriptionType_Name as \"PrescriptionType_Name\"
				,(select count(EPin.PrescriptionType_id) from v_EvnPrescr EPin where EPin.EvnPrescr_pid = EP.EvnPrescr_pid and EPin.PrescriptionType_id = EP.PrescriptionType_id group by EPin.PrescriptionType_id) as \"cntInPrescriptionTypeGroup\"
				,coalesce(EP.EvnPrescr_IsCito,1) as \"EvnPrescr_IsCito\"
				,case
					when Regime.EvnPrescrRegime_id is not null
					then (select count(EvnPrescrRegime_id) from v_EvnPrescrRegime where EvnPrescrRegime_pid = EP.EvnPrescr_id and PrescriptionStatusType_id != 3)
					when Diet.EvnPrescrDiet_id is not null
					then (select count(EvnPrescrDiet_id) from v_EvnPrescrDiet where EvnPrescrDiet_pid = EP.EvnPrescr_id and PrescriptionStatusType_id != 3)
					when Obs.EvnPrescrObserv_id is not null
					then (select count(EvnPrescrObserv_id) from v_EvnPrescrObserv where EvnPrescrObserv_pid = EP.EvnPrescr_id and PrescriptionStatusType_id != 3)
				else
					null
				...
				coalesce(PDT.PrescriptionDietType_id, 0) as \"PrescriptionDietType_id\",--тип диеты
				coalesce(PDT.PrescriptionDietType_Code, '') as \"PrescriptionDietType_Code\",
				coalesce(PDT.PrescriptionDietType_Name, '') as \"PrescriptionDietType_Name\",
				--10
				...
				case
					when Obs.EvnPrescrObserv_id is not null
					then (select count(EvnPrescrObservPos_id) from v_EvnPrescrObservPos where EvnPrescr_id = EP.EvnPrescr_id)
				else
					null
				end as \"cntParam\"
				--5
				...
				,LTRIM(to_char(EPTD.EvnPrescrTreatDrug_KolvoEd, '9999999999D99')) as \"EvnPrescrTreatDrug_KolvoEd\"
				,coalesce(df.NAME,Drug.DrugForm_Name) as \"DrugForm_Name\"
				,LTRIM(to_char(EPTD.EvnPrescrTreatDrug_Kolvo, '9999999999D99')) as \"EvnPrescrTreatDrug_Kolvo\"
				,coalesce(ep_mu.SHORTNAME, ep_cu.SHORTNAME) as \"EdUnits_Nick\"
				...
				,LTRIM(to_char(ec_drug.EvnCourseTreatDrug_KolvoEd, '9999999999D99')) as \"EvnCourseTreatDrug_KolvoEd\"
				,coalesce(CourseDf.NAME,CourseDrug.DrugForm_Name) as \"CourseDrugForm_Name\"
				,LTRIM(to_char(ec_drug.EvnCourseTreatDrug_Kolvo, '9999999999D99')) as \"EvnCourseTreatDrug_Kolvo\"
				,coalesce(ec_mu.SHORTNAME, ec_cu.SHORTNAME) as \"CourseEdUnits_Nick\"
				...
				,(select count(EvnCourseTreatDrug_id) from v_EvnCourseTreatDrug where EP.PrescriptionType_id = 5 and EvnCourseTreat_id = EPT.EvnCourse_id)  as \"cntCourseDrug\"
				...
				--5,6
				,coalesce(EPT.EvnCourse_id, EPPR.EvnCourse_id) as \"EvnCourse_id\"
				,coalesce(to_char(coalesce(EPT.EvnPrescrTreat_setDT,EPPR.EvnPrescrProc_setDT), 'dd.mm.yyyy'), '') as \"graf_date\"
				,coalesce(ECT.EvnCourseTreat_MaxCountDay,ECPR.EvnCourseProc_MaxCountDay, 0) as \"MaxCountInDay\"
				,coalesce(ECT.EvnCourseTreat_MinCountDay,ECPR.EvnCourseProc_MinCountDay, 0) as \"MinCountInDay\"
				,coalesce(ECT.EvnCourseTreat_Duration,ECPR.EvnCourseProc_Duration, 0) as \"Duration\"
				,coalesce(ECT.EvnCourseTreat_ContReception,ECPR.EvnCourseProc_ContReception, 0) as \"ContReception\"
				,coalesce(ECT.EvnCourseTreat_Interval,ECPR.EvnCourseProc_Interval, 0) as \"Interval\"
				,case
					when EP.PrescriptionType_id = 5 then (select count(EvnPrescrTreat_id) from v_EvnPrescrTreat where ECT.EvnCourseTreat_id = EvnCourse_id)
					when EP.PrescriptionType_id = 6 then (select count(EvnPrescrProc_id) from v_EvnPrescrProc where ECPR.EvnCourseProc_id = EvnCourse_id)
					else 0
				end as \"PrescrCount\"
				...
				,UC.UslugaComplex_Name as \"UslugaComplex_Name\"
				--7,12
				,coalesce(EPOU.EvnPrescrOperUsluga_id, EPFDU.EvnPrescrFuncDiagUsluga_id) as \"TableUsluga_id\"
				,case
			...
			from v_EvnPrescr EP
				...
				left join YesNo IsCito on IsCito.YesNo_id = coalesce(EP.EvnPrescr_IsCito,1)
				...
				left join v_EvnPrescrTreat EPT on EP.PrescriptionType_id = 5 and EPT.EvnPrescrTreat_id = EP.EvnPrescr_id
				left join v_EvnCourseTreat ECT on EP.PrescriptionType_id = 5 and ECT.EvnCourseTreat_id = EPT.EvnCourse_id
				left join v_EvnPrescrTreatDrug EPTD on EP.PrescriptionType_id = 5 and EPTD.EvnPrescrTreat_id = EPT.EvnPrescrTreat_id
				left join v_EvnCourseTreatDrug ec_drug on EP.PrescriptionType_id = 5 and ec_drug.EvnCourseTreat_id = EPT.EvnCourse_id
				...
				left join rls.MASSUNITS ec_mu on EP.PrescriptionType_id = 5 and ec_drug.MASSUNITS_ID = ec_mu.MASSUNITS_ID
				left join rls.CUBICUNITS ec_cu on EP.PrescriptionType_id = 5 and ec_drug.CUBICUNITS_id = ec_cu.CUBICUNITS_id
				left join PerformanceType PFT on  EP.PrescriptionType_id = 5 and ECT.PerformanceType_id = PFT.PerformanceType_id
				left join PrescriptionIntroType PIT on EP.PrescriptionType_id = 5 and ECT.PrescriptionIntroType_id = PIT.PrescriptionIntroType_id
				...
				left join rls.CLSDRUGFORMS df on dcm.CLSDRUGFORMS_ID = df.CLSDRUGFORMS_ID
				left join rls.v_Drug CourseDrug on EP.PrescriptionType_id = 5 and CourseDrug.Drug_id = ec_drug.Drug_id
				left join rls.v_DrugComplexMnn CourseDcm on EP.PrescriptionType_id = 5 and CourseDcm.DrugComplexMnn_id = ec_drug.DrugComplexMnn_id
				left join rls.CLSDRUGFORMS CourseDf on CourseDcm.CLSDRUGFORMS_ID = CourseDf.CLSDRUGFORMS_ID
				left join v_EvnPrescrProc EPPR on EP.PrescriptionType_id = 6 and EPPR.EvnPrescrProc_id = EP.EvnPrescr_id
				left join v_EvnCourseProc ECPR on EP.PrescriptionType_id = 6 and ECPR.EvnCourseProc_id = EPPR.EvnCourse_id
				left join DurationType DTP on  EP.PrescriptionType_id in (5,6) and coalesce(ECPR.DurationType_id,ECT.DurationType_id) = DTP.DurationType_id
				left join DurationType DTN on  EP.PrescriptionType_id in (5,6) and coalesce(ECPR.DurationType_recid,ECT.DurationType_recid) = DTN.DurationType_id
				left join DurationType DTI on  EP.PrescriptionType_id in (5,6) and coalesce(ECPR.DurationType_intid,ECT.DurationType_intid) = DTI.DurationType_id
				left join v_EvnPrescrOper EPO on EP.PrescriptionType_id = 7 and EPO.EvnPrescrOper_id = EP.EvnPrescr_id
				--left join v_EvnPrescrOperUsluga EPOU on EP.PrescriptionType_id = 7 and EPOU.EvnPrescrOper_id = EP.EvnPrescr_id
				left join lateral (
					select * from v_EvnPrescrOperUsluga where EP.PrescriptionType_id = 7 and EvnPrescrOper_id = EP.EvnPrescr_id limit 100
				) EPOU on true
				...
				left join v_UslugaComplex UC on EP.PrescriptionType_id in (6,7,11,12,13) and UC.UslugaComplex_id = coalesce(EPPR.UslugaComplex_id, EPFDU.UslugaComplex_id, EPLD.UslugaComplex_id, EPOU.UslugaComplex_id, EPCU.UslugaComplex_id)
				left join v_UslugaComplex CUC on EP.PrescriptionType_id = 6 and CUC.UslugaComplex_id = ECPR.UslugaComplex_id
				left join v_EvnPrescrObserv Obs on EP.PrescriptionType_id = 10 and Obs.EvnPrescrObserv_pid = EP.EvnPrescr_id
				left join v_ObservTimeType OTT on EP.PrescriptionType_id = 10 and OTT.ObservTimeType_id = Obs.ObservTimeType_id
				left join v_EvnPrescrObservPos EPOP on EP.PrescriptionType_id = 10 and EPOP.EvnPrescr_id = EP.EvnPrescr_id
				left join ObservParamType OPT on EP.PrescriptionType_id = 10 and OPT.ObservParamType_id = EPOP.ObservParamType_id
				...
				left join lateral (
					...
					where EP.EvnPrescr_IsExec = 2 and UC.UslugaComplex_id is not null and EvnPrescr_id = EP.EvnPrescr_id limit 1
				) EU on true
				left join lateral (
					select EvnDrug_id, EvnDrug_setDT from v_EvnDrug
					where EP.EvnPrescr_IsExec = 2 and EP.PrescriptionType_id = 5 and EvnPrescr_id = EP.EvnPrescr_id limit 1
				) EDr on true
			...
			order by
				...
				coalesce(EPT.EvnCourse_id, EPPR.EvnCourse_id),
				EP.EvnPrescr_id,
	...
	function doLoadEvnPrescrCombo($data) {
		...
		if (!empty($data['withoutEvnDirection'])) {
			...
			$add_where_cause .= 'and not exists (select epd.EvnDirection_id from v_EvnPrescrDirection epd
					where EP.EvnPrescr_id = epd.EvnPrescr_id limit 1) ';
		}
		...
		if (in_array($data['PrescriptionType_Code'], array(6,7,11,12,13))) {
			$select_sp = ",UC.UslugaComplex_2011id as \"UslugaComplex_2011id\"
				,UC.UslugaComplex_Name as \"UslugaComplex_Name\"
				";
		} else {
		...
		switch ($data['PrescriptionType_Code']) {
			case 5:
				$join_sp = 'inner join v_EvnPrescrTreatDrug EPTD on EPTD.EvnPrescrTreat_id = EP.EvnPrescr_id
				...
				$select_sp .= ",EPTD.EvnPrescrTreatDrug_id as \"EvnPrescrTreatDrug_id\"
				,coalesce(Drug.DrugTorg_Name, dcm.DrugComplexMnn_RusName, '') as \"DrugTorg_Name\"
				,(select count(EvnPrescrTreatDrug_id) from v_EvnPrescrTreatDrug where EvnPrescrTreat_id = EP.EvnPrescr_id) as \"cntDrug\"
				,null as \"TableUsluga_id\"
				,0 as \"cntUsluga\"";
				break;
			case 6:
				$join_sp = 'inner join v_EvnPrescrProc TableUsluga on TableUsluga.EvnPrescrProc_id = EP.EvnPrescr_id
				' . $join_uc . 'v_UslugaComplex UC on UC.UslugaComplex_id = TableUsluga.UslugaComplex_id';
				...
				break;
			case 7:
				$join_sp = 'inner join v_EvnPrescrOperUsluga TableUsluga on TableUsluga.EvnPrescrOper_id = EP.EvnPrescr_id
				' . $join_uc . 'v_UslugaComplex UC on UC.UslugaComplex_id = TableUsluga.UslugaComplex_id';
				$select_sp .= ",null as \"EvnPrescrTreatDrug_id\"
				,'' as \"DrugTorg_Name\"
				,0 as \"cntDrug\"
				...
				break;
			case 11:
				$join_sp = 'inner join v_EvnPrescrLabDiag TableUsluga on TableUsluga.EvnPrescrLabDiag_id = EP.EvnPrescr_id
				' . $join_uc . 'v_UslugaComplex UC on UC.UslugaComplex_id = TableUsluga.UslugaComplex_id';
				$select_sp .= ",null as \"EvnPrescrTreatDrug_id\"
				,'' as \"DrugTorg_Name\"
				,0 as \"cntDrug\"
				...
				break;
			case 12:
				$join_sp = 'inner join v_EvnPrescrFuncDiagUsluga TableUsluga on TableUsluga.EvnPrescrFuncDiag_id = EP.EvnPrescr_id
				' . $join_uc . 'v_UslugaComplex UC on UC.UslugaComplex_id = TableUsluga.UslugaComplex_id';
				$select_sp .= ",null as \"EvnPrescrTreatDrug_id\"
				,'' as \"DrugTorg_Name\"
				,0 as \"cntDrug\"
				...
				break;
			case 13:
				$join_sp = 'inner join v_EvnPrescrConsUsluga TableUsluga on TableUsluga.EvnPrescrConsUsluga_id = EP.EvnPrescr_id
				' . $join_uc . 'v_UslugaComplex UC on UC.UslugaComplex_id = TableUsluga.UslugaComplex_id';
				$select_sp .= ",null as \"EvnPrescrTreatDrug_id\"
				,'' as \"DrugTorg_Name\"
				,0 as \"cntDrug\"
		...
		if (empty($data['PrescriptionType_Code'])) {
			$join_sp = 'left join v_EvnPrescrTreatDrug EPTD on EP.PrescriptionType_id = 5 and EPTD.EvnPrescrTreat_id = EP.EvnPrescr_id
				left join rls.v_Drug Drug on EP.PrescriptionType_id = 5 and Drug.Drug_id = EPTD.Drug_id
				left join rls.v_DrugComplexMnn dcm on EP.PrescriptionType_id = 5 and dcm.DrugComplexMnn_id = EPTD.DrugComplexMnn_id
				left join v_EvnPrescrProc EPPR on EP.PrescriptionType_id = 6 and EPPR.EvnPrescrProc_id = EP.EvnPrescr_id
				--left join v_EvnPrescrOperUsluga EPOU on EP.PrescriptionType_id = 7 and EPOU.EvnPrescrOper_id = EP.EvnPrescr_id
		...
		if (!empty($data['EvnPrescr_id'])) {
			...
			$add_where_cause .= 'and coalesce(EP.EvnPrescr_isExec,1) = 1 ';
		}
