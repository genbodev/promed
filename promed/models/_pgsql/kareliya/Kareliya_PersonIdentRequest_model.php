	...
	function savePersonKareliya($identData, $data) {
		...
		if (!empty($identResponse['PolisType_id'])) {
			$OMSSprTerr_id = $this->getFirstResultFromQuery("
				select OMSSprTerr_id as \"OMSSprTerr_id\" from v_OMSSprTerr  where OMSSprTerr_Code = :OMSSprTerr_Code limit 1
		...
		if ($updatePolis) {
			...
			$begDate = $this->getFirstResultFromQuery("
				select
				to_char(dateadd('second', 1, P.PersonEvn_insDT), 'yyyy-mm-dd hh24:mi:ss') as \"Polis_begDate\"
				from v_Person_all P
	...
	function PersonIdentPackage() {
		...
		$personQuery = "
			select
				...
				p.PersonPhone_Phone as \"PersonPhone_Phone\",
				p.Sex_id as \"PersonSex_id\",
				p.SocStatus_id as \"SocStatus_id\",
			...
			from
				v_Person_all p
				left join v_PersonPolis pol on pol.Polis_id = p.Polis_id
				left join v_PersonDocument doc on doc.Document_id = p.Document_id
