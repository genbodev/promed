 ...
 */
class EvnOnkoNotify_model extends swPgModel
{
	/**
	...
	function onkoSpecificFieldsValidate($onkoSpecific) {
		...
		if (empty($onkoSpecific['EvnNotifyBase_id']) || empty($onkoSpecific['EvnOnkoNotifyNeglected_id']))
		{
			...
			$tnm_check = (1 == $this->getFirstResultFromQuery("
				select case 
					when 
						exists (select OnkoTLink_id from v_OnkoTLink where Diag_id = :Diag_id and OnkoT_id is null) and
						exists (select OnkoNLink_id from v_OnkoNLink where Diag_id = :Diag_id and OnkoN_id is null) and
						exists (select OnkoMLink_id from v_OnkoMLink where Diag_id = :Diag_id and OnkoM_id is null)
					then 0
					else 1
				end as \"TNMcheck\"
	...
	function loadEvnDiagPLStomDataCheckExists($EvnDiagPLStom_id) {
		$query = "
			SELECT 
				MODS.OnkoT_id as \"OnkoT_id\",
				MODS.OnkoN_id as \"OnkoN_id\",
				MODS.OnkoM_id as \"OnkoM_id\",
				MODS.TumorStage_id as \"TumorStage_id\",
				MODS.MorbusOnkoDiagPLStom_setDiagDT as \"MorbusOnko_setDiagDT\",
				MOB.AutopsyPerformType_id as \"AutopsyPerformType_id\",
				MODS.Diag_id as \"Diag_id\",
				MOB.MorbusOnkoBase_deadDT as \"MorbusOnkoBase_deadDT\",
				MOB.Diag_did as \"Diag_did\",
				MODS.TumorAutopsyResultType_id as \"TumorAutopsyResultType_id\",
				MODS.OnkoDiagConfType_id as \"OnkoDiagConfType_id\",
				coalesce(
	...
	function loadDataCheckExists($Person_id, $evn_Diag_id = null)
	{
		...
		$response = swMorbus::getStaticMorbusCommon()->checkExistsExtended('onko', $Person_id, $evn_Diag_id,"
				...
				,PR.PersonRegister_id as \"PersonRegister_id\"
				,PR.PersonRegisterOutCause_id as \"PersonRegisterOutCause_id\"
				,MO.MorbusOnko_setDiagDT as \"MorbusOnko_setDiagDT\"
				...
				left join v_EvnOnkoNotifyNeglected EONN on EONN.EvnOnkoNotify_id = EN.EvnOnkoNotify_id
				left join v_MorbusOnkoBase MOB on MOB.MorbusBase_id = MorbusBase.MorbusBase_id
				inner join v_MorbusOnko MO on MO.Morbus_id = Morbus.Morbus_id"
		);
		if (empty($response)) {
			return $response;
		}
	...
	function load($data)
	{
		$query = "
			select distinct
				...
				EONN.Evn_id as \"EvnOnkoNotifyNeglected_id\",
				EON.Server_id as \"Server_id\",
				EON.PersonEvn_id as \"PersonEvn_id\",
				EON.Person_id as \"Person_id\",
				MOP.Ethnos_id as \"Ethnos_id\",
				MOP.KLAreaType_id as \"KLAreaType_id\",
				...
				Morbus.Diag_id as \"Diag_id\",-- диагноз заболевания и Топография (локализация) опухоли
			...
			from
				v_EvnOnkoNotify EON
			inner join v_MorbusOnko MO on MO.Morbus_id = EON.Morbus_id
			inner join v_MorbusOnkoPerson MOP on MOP.Person_id = EON.Person_id
			inner join v_Morbus Morbus on Morbus.Morbus_id = MO.Morbus_id
			inner join v_MorbusOnkoBase MOB on MOB.MorbusBase_id = Morbus.MorbusBase_id
			left join EvnOnkoNotifyNeglected EONN on EONN.EvnOnkoNotify_id = EON.EvnOnkoNotify_id
			where
	...
	function getEvnOnkoNotifyList($data) {
		$query = "
			...
			from 
				...
				inner join v_Diag Diag on Diag.Diag_id = M.Diag_id
			where 
	...
	function getDataForPrint($data)
	{
		$query = "
			select --distinct
				...
				coalesce(AreaType.KLAreaType_Name,'неизвестно') as \"KLAreaType_Name\",
				coalesce(rtrim(PersonAddress.Address_Address), '') as \"Person_Address\",
				Ethnos.Ethnos_Name as \"Ethnos_Name\",
				...
				to_char(coalesce(EON.EvnOnkoNotify_setFirstDT, MO.MorbusOnko_firstVizitDT), 'dd.mm.yyyy') as \"MorbusOnko_firstVizitDT\",
				to_char(coalesce(EON.EvnOnkoNotify_setDiagDT, MO.MorbusOnko_setDiagDT), 'dd.mm.yyyy') as \"MorbusOnko_setDiagDT\",
				...
				tcit.TumorCircumIdentType_Name as \"TumorCircumIdentType_Name\",
				odct.OnkoDiagConfType_Name as \"OnkoDiagConfType_Name\",
				coalesce(EON.EvnOnkoNotify_IsTumorDepoUnknown,MO.MorbusOnko_IsTumorDepoUnknown) as \"IsTumorDepoUnknown\",
				coalesce(EON.EvnOnkoNotify_IsTumorDepoBones,MO.MorbusOnko_IsTumorDepoBones) as \"IsTumorDepoBones\",
				coalesce(EON.EvnOnkoNotify_IsTumorDepoLiver,MO.MorbusOnko_IsTumorDepoLiver) as \"IsTumorDepoLiver\",
				coalesce(EON.EvnOnkoNotify_IsTumorDepoSkin,MO.MorbusOnko_IsTumorDepoSkin) as \"IsTumorDepoSkin\",
				coalesce(EON.EvnOnkoNotify_IsTumorDepoKidney,MO.MorbusOnko_IsTumorDepoKidney) as \"IsTumorDepoKidney\",
				coalesce(EON.EvnOnkoNotify_IsTumorDepoOvary,MO.MorbusOnko_IsTumorDepoOvary) as \"IsTumorDepoOvary\",
				coalesce(EON.EvnOnkoNotify_IsTumorDepoPerito,MO.MorbusOnko_IsTumorDepoPerito) as \"IsTumorDepoPerito\",
				coalesce(EON.EvnOnkoNotify_IsTumorDepoLympha,MO.MorbusOnko_IsTumorDepoLympha) as \"IsTumorDepoLympha\",
				coalesce(EON.EvnOnkoNotify_IsTumorDepoLungs,MO.MorbusOnko_IsTumorDepoLungs) as \"IsTumorDepoLungs\",
				coalesce(EON.EvnOnkoNotify_IsTumorDepoBrain,MO.MorbusOnko_IsTumorDepoBrain) as \"IsTumorDepoBrain\",
				coalesce(EON.EvnOnkoNotify_IsTumorDepoMarrow,MO.MorbusOnko_IsTumorDepoMarrow) as \"IsTumorDepoMarrow\",
				coalesce(EON.EvnOnkoNotify_IsTumorDepoOther,MO.MorbusOnko_IsTumorDepoOther) as \"IsTumorDepoOther\",
				coalesce(EON.EvnOnkoNotify_IsTumorDepoMulti,MO.MorbusOnko_IsTumorDepoMulti) as \"IsTumorDepoMulti\",
				lpu_s.Lpu_Nick as \"LpuS_Name\",
				lpu_m.Lpu_Nick as \"LpuM_Name\",
				mp_d.Person_Fin as \"MedPersonal_Fin\",
				lpu_d.Lpu_Name as \"Lpu_Name\",
				lpu_d.Lpu_Nick as \"LpuD_Name\",
				coalesce(rtrim(LpuAddress.Address_Address), '') as \"LpuD_Address\"
			from
				v_EvnOnkoNotify EON
				inner join v_MorbusOnkoPerson MOP on MOP.Person_id = EON.Person_id
				inner join v_Morbus M on M.Morbus_id = EON.Morbus_id
				inner join v_MorbusOnko MO on MO.Morbus_id = EON.Morbus_id
				left join v_PersonState PS on EON.Person_id = PS.Person_id
				left join v_Sex sex on PS.Sex_id = sex.Sex_id
				left join v_KLAreaType AreaType on MOP.KLAreaType_id = AreaType.KLAreaType_id
				left join v_Address PersonAddress on PS.UAddress_id = PersonAddress.Address_id
				left join v_Ethnos Ethnos on coalesce(EON.Ethnos_id,MOP.Ethnos_id) = Ethnos.Ethnos_id
				left join v_OnkoOccupationClass ooc on coalesce(EON.OnkoOccupationClass_id,MOP.OnkoOccupationClass_id) = ooc.OnkoOccupationClass_id
				left join v_TumorStage ts on coalesce(EON.TumorStage_id,MO.TumorStage_id) = ts.TumorStage_id
				left join v_OnkoM OnkoM on coalesce(EON.OnkoM_id,MO.OnkoM_id) = OnkoM.OnkoM_id
				left join v_OnkoN OnkoN on coalesce(EON.OnkoN_id,MO.OnkoN_id) = OnkoN.OnkoN_id
				left join v_OnkoT OnkoT on coalesce(EON.OnkoT_id,MO.OnkoT_id) = OnkoT.OnkoT_id
				left join v_TumorCircumIdentType tcit on coalesce(EON.TumorCircumIdentType_id,MO.TumorCircumIdentType_id) = tcit.TumorCircumIdentType_id
				left join v_Lpu lpu_d on EON.Lpu_id = lpu_d.Lpu_id
				left join v_MedPersonal mp_d on EON.MedPersonal_id = mp_d.MedPersonal_id and EON.Lpu_id = mp_d.Lpu_id
				left join v_Address LpuAddress on lpu_d.UAddress_id = LpuAddress.Address_id
				left join v_Lpu lpu_s on EON.Lpu_sid = lpu_s.Lpu_id
				left join v_Lpu lpu_m on EON.Lpu_mid = lpu_m.Lpu_id
	...
	function getDataForSpecific($data)
	{
		$query = "
			SELECT
				...
				to_char(EON.EvnOnkoNotify_setDate, 'dd.mm.yyyy') as \"EvnOnkoNotify_setDate\",
				case 
	...
	function loadAPI($data){
		...
		$q = "
			select distinct
				EON.EvnOnkoNotify_id as \"EvnOnkoNotify_id\",	-- Идентификатор извещения
				EON.Person_id as \"Person_id\",	-- ссылка на человека в Person
				EON.EvnOnkoNotify_pid as \"Evn_pid\",
				EON.Lpu_sid as \"Lpu_sid\",	-- Направить извещение
				EON.MedPersonal_id as \"MedPersonal_id\",	-- Врач, заполнивший  извещение 
				to_char(EON.EvnOnkoNotify_setDiagDT, 'dd.mm.yyyy') as \"EvnOnkoNotify_setDiagDT\",	-- дата заполнения извещения 
				to_char(EON.EvnOnkoNotify_setDT, 'dd.mm.yyyy') as \"EvnOnkoNotify_setDT\",
				MO.OnkoDiag_mid as \"OnkoDiag_mid\",	--  морфологический тип опухоли
				...
				MO.OnkoN_id as \"N\",
				MO.OnkoM_id as \"M\",
				to_char(MO.MorbusOnko_setDiagDT, 'dd.mm.yyyy') as \"MorbusOnko_setDiagDT\",	-- Дата установления диагноза
				MO.TumorStage_id as \"TumorStage_id\",	-- Стадия опухолевого процесса (справочник dbo.TumorStage);
				...
				EON.Lpu_niid as \"Lpu_niid\",	-- Лпу, не включившее в регистр
				EON.MedPersonal_niid as \"MedPersonal_niid\",	-- Врач, не включивший в регистр
				EON.PersonRegisterFailIncludeCause_id as \"PersonRegisterFailIncludeCause_id\",	-- Причина невключения в регистр  (справочник dbo.PersonRegisterFailIncludeCause)
				EON.EvnOnkoNotify_Comment as \"EvnOnkoNotify_Comment\"
			from
				v_EvnOnkoNotify EON
				inner join v_MorbusOnko MO on MO.Morbus_id = EON.Morbus_id
	...
	function saveEvnOnkoNotifyAPI($data){
		$query = 'SELECT PersonEvn_id as "PersonEvn_id", Person_id as "Person_id" FROM v_Evn WHERE Evn_id = :Evn_pid';
	...
	function updateEvnOnkoNotifyAPI($data){
		...
		SELECT
			...
			EvnOnkoNotify_id as "EvnOnkoNotify_id",
			EvnOnkoNotify_setDate as "EvnOnkoNotify_setDate",
			EvnOnkoNotify_setTime as "EvnOnkoNotify_setTime",
			EvnOnkoNotify_didDate as "EvnOnkoNotify_didDate",
			EvnOnkoNotify_didTime as "EvnOnkoNotify_didTime",
			EvnOnkoNotify_disDate as "EvnOnkoNotify_disDate",
			EvnOnkoNotify_disTime as "EvnOnkoNotify_disTime",
			EvnOnkoNotify_pid as "EvnOnkoNotify_pid",
			EvnOnkoNotify_rid as "EvnOnkoNotify_rid",
			Lpu_id as "Lpu_id",
			Server_id as "Server_id",
			PersonEvn_id as "PersonEvn_id",
			EvnOnkoNotify_setDT as "EvnOnkoNotify_setDT",
			EvnOnkoNotify_disDT as "EvnOnkoNotify_disDT",
			EvnOnkoNotify_didDT as "EvnOnkoNotify_didDT",
			EvnOnkoNotify_insDT as "EvnOnkoNotify_insDT",
			EvnOnkoNotify_updDT as "EvnOnkoNotify_updDT",
			EvnOnkoNotify_Index as "EvnOnkoNotify_Index",
			EvnOnkoNotify_Count as "EvnOnkoNotify_Count",
			pmUser_insID as "pmUser_insID",
			pmUser_updID as "pmUser_updID",
			Person_id as "Person_id",
			Morbus_id as "Morbus_id",
			EvnOnkoNotify_IsSigned as "EvnOnkoNotify_IsSigned",
			pmUser_signID as "pmUser_signID",
			EvnOnkoNotify_signDT as "EvnOnkoNotify_signDT",
			EvnOnkoNotify_IsArchive as "EvnOnkoNotify_IsArchive",
			EvnOnkoNotify_Guid as "EvnOnkoNotify_Guid",
			EvnOnkoNotify_IndexMinusOne as "EvnOnkoNotify_IndexMinusOne",
			EvnStatus_id as "EvnStatus_id",
			EvnOnkoNotify_statusDate as "EvnOnkoNotify_statusDate",
			EvnOnkoNotify_IsTransit as "EvnOnkoNotify_IsTransit",
			MorbusType_id as "MorbusType_id",
			MedPersonal_id as "MedPersonal_id",
			EvnOnkoNotify_niDate as "EvnOnkoNotify_niDate",
			...
			EvnOnkoNotify_IsAuto as "EvnOnkoNotify_IsAuto",
			MedStaffFact_id as "MedStaffFact_id",
			MedStaffFact_zid as "MedStaffFact_zid",
			Lpu_sid as "Lpu_sid",
			Ethnos_id as "Ethnos_id",
			OnkoOccupationClass_id as "OnkoOccupationClass_id",
			EvnOnkoNotify_setFirstDT as "EvnOnkoNotify_setFirstDT",
			EvnOnkoNotify_setDiagDT as "EvnOnkoNotify_setDiagDT",
			Diag_id as "Diag_id",
			EvnOnkoNotify_LocalTrumor as "EvnOnkoNotify_LocalTrumor",
			Diag_mid as "Diag_mid",
			OnkoT_id as "OnkoT_id",
			OnkoN_id as "OnkoN_id",
			OnkoM_id as "OnkoM_id",
			TumorStage_id as "TumorStage_id",
			EvnOnkoNotify_IsTumorDepoUnknown as "EvnOnkoNotify_IsTumorDepoUnknown",
			EvnOnkoNotify_IsTumorDepoLympha as "EvnOnkoNotify_IsTumorDepoLympha",
			EvnOnkoNotify_IsTumorDepoBones as "EvnOnkoNotify_IsTumorDepoBones",
			EvnOnkoNotify_IsTumorDepoLiver as "EvnOnkoNotify_IsTumorDepoLiver",
			EvnOnkoNotify_IsTumorDepoLungs as "EvnOnkoNotify_IsTumorDepoLungs",
			EvnOnkoNotify_IsTumorDepoBrain as "EvnOnkoNotify_IsTumorDepoBrain",
			EvnOnkoNotify_IsTumorDepoSkin as "EvnOnkoNotify_IsTumorDepoSkin",
			EvnOnkoNotify_IsTumorDepoKidney as "EvnOnkoNotify_IsTumorDepoKidney",
			EvnOnkoNotify_IsTumorDepoOvary as "EvnOnkoNotify_IsTumorDepoOvary",
			EvnOnkoNotify_IsTumorDepoPerito as "EvnOnkoNotify_IsTumorDepoPerito",
			EvnOnkoNotify_IsTumorDepoMarrow as "EvnOnkoNotify_IsTumorDepoMarrow",
			EvnOnkoNotify_IsTumorDepoOther as "EvnOnkoNotify_IsTumorDepoOther",
			EvnOnkoNotify_IsTumorDepoMulti as "EvnOnkoNotify_IsTumorDepoMulti",
			EvnOnkoNotify_IsDiagConfUnknown as "EvnOnkoNotify_IsDiagConfUnknown",
			EvnOnkoNotify_IsDiagConfMorfo as "EvnOnkoNotify_IsDiagConfMorfo",
			EvnOnkoNotify_IsDiagConfCito as "EvnOnkoNotify_IsDiagConfCito",
			EvnOnkoNotify_IsDiagConfExplo as "EvnOnkoNotify_IsDiagConfExplo",
			EvnOnkoNotify_IsDiagConfLab as "EvnOnkoNotify_IsDiagConfLab",
			EvnOnkoNotify_IsDiagConfClinic as "EvnOnkoNotify_IsDiagConfClinic",
			TumorCircumIdentType_id as "TumorCircumIdentType_id",
			Lpu_mid as "Lpu_mid",
			NotifyFillPlace_id as "NotifyFillPlace_id",
			NotifyDirectType_id as "NotifyDirectType_id" 
		FROM v_EvnOnkoNotify WHERE EvnOnkoNotify_id = :EvnOnkoNotify_id';
		$res = $this->getFirstRowFromQuery($query, $data);
