 ...
 * @version
 */
class AnalyzerWorksheetEvnLabSample_model extends swModel {
	...
	function load() {
		$q = "
			select
				AnalyzerWorksheetEvnLabSample_id, AnalyzerWorksheet_id, EvnLabSample_id, AnalyzerWorksheetEvnLabSample_X, AnalyzerWorksheetEvnLabSample_Y
			from
	...
	function getAnalyzerModelDimensions($data) {
		$query = "
			select
				AnalyzerRack.AnalyzerRack_DimensionX,
				AnalyzerRack.AnalyzerRack_DimensionY
			from lis.v_AnalyzerWorksheet AnalyzerWorksheet (nolock)
				left join lis.v_AnalyzerRack AnalyzerRack (nolock) on AnalyzerRack.AnalyzerRack_id =  AnalyzerWorksheet.AnalyzerRack_id
	...
	function loadMatrix($data) {
		...
		$query = "
			SELECT
				...
				awels.AnalyzerWorksheet_id,
				awels.EvnLabSample_id,
				awels.AnalyzerWorksheetEvnLabSample_X,
				awels.AnalyzerWorksheetEvnLabSample_Y,
				EvnLabSample.EvnLabSample_Num,
				RTRIM(LTRIM(ISNULL(ps.Person_Surname, '') + ' ' + SUBSTRING(ISNULL(ps.Person_Firname, ''),1,1) + ' ' + SUBSTRING(ISNULL(ps.Person_Secname, ''),1,1))) as Person_FIO
			FROM
				lis.v_AnalyzerWorksheetEvnLabSample awels WITH (NOLOCK)
				LEFT JOIN dbo.v_EvnLabSample EvnLabSample WITH (NOLOCK) ON EvnLabSample.EvnLabSample_id = awels.EvnLabSample_id
				LEFT JOIN dbo.v_EvnLabRequest EvnLabRequest WITH (NOLOCK) ON EvnLabSample.EvnLabRequest_id = EvnLabRequest.EvnLabRequest_id
				LEFT JOIN v_PersonState ps WITH (NOLOCK) ON ps.Person_id = EvnLabRequest.Person_id
			where {$filter}
	...
	function clearMatrix($data) {
		...
		$query = "
			...
			From lis.AnalyzerWorksheetEvnLabSample awels with (nolock)
			inner join lis.AnalyzerWorksheet av WITH (NOLOCK) ON av.AnalyzerWorksheet_id = awels.AnalyzerWorksheet_id and av.AnalyzerWorksheetStatusType_id = 1
	...
	function loadEvnLabSampleWithAnalyzerWorksheet($data) {
		...
		$query = "
			SELECT
				...
				EvnLabSample.EvnLabSample_Num,
				RTRIM(LTRIM(ISNULL(ps.Person_Surname, '') + ' ' + SUBSTRING(ISNULL(ps.Person_Firname, ''),1,1) + ' ' + SUBSTRING(ISNULL(ps.Person_Secname, ''),1,1))) as Person_FIO,
				link.lis_id -- если ранее заявка уже отправлялась, то ее возможно ее надо будет проапдейтить 
			FROM
				lis.v_AnalyzerWorksheetEvnLabSample awels WITH (NOLOCK)
				LEFT JOIN dbo.v_EvnLabSample EvnLabSample WITH (NOLOCK) ON EvnLabSample.EvnLabSample_id = awels.EvnLabSample_id
				LEFT JOIN dbo.v_EvnLabRequest EvnLabRequest WITH (NOLOCK) ON EvnLabSample.EvnLabRequest_id = EvnLabRequest.EvnLabRequest_id
				LEFT JOIN v_PersonState ps WITH (NOLOCK) ON ps.Person_id = EvnLabRequest.Person_id
				left join lis.v_Link link (nolock) on link.object_id = EvnLabSample.EvnLabSample_id and link.link_object = 'EvnLabSample'
			where {$filter}
			order by AnalyzerWorksheetEvnLabSample_Y, AnalyzerWorksheetEvnLabSample_X
		";
	...
	function loadList($filter) {
		...
		$q = "
			SELECT
				v_AnalyzerWorksheetEvnLabSample.AnalyzerWorksheetEvnLabSample_id, v_AnalyzerWorksheetEvnLabSample.AnalyzerWorksheet_id, v_AnalyzerWorksheetEvnLabSample.EvnLabSample_id, v_AnalyzerWorksheetEvnLabSample.AnalyzerWorksheetEvnLabSample_X, v_AnalyzerWorksheetEvnLabSample.AnalyzerWorksheetEvnLabSample_Y
				,AnalyzerWorksheet_id_ref.AnalyzerWorksheet_Name AnalyzerWorksheet_id_Name,
				EvnLabSample_id_ref.EvnLabSample_Num
			FROM
				lis.v_AnalyzerWorksheetEvnLabSample WITH (NOLOCK)
				LEFT JOIN lis.v_AnalyzerWorksheet AnalyzerWorksheet_id_ref WITH (NOLOCK) ON AnalyzerWorksheet_id_ref.AnalyzerWorksheet_id = v_AnalyzerWorksheetEvnLabSample.AnalyzerWorksheet_id
				LEFT JOIN dbo.v_EvnLabSample EvnLabSample_id_ref WITH (NOLOCK) ON EvnLabSample_id_ref.EvnLabSample_id = v_AnalyzerWorksheetEvnLabSample.EvnLabSample_id
			$where_clause
		";
	...
	function save() {
        ...
        $query = "
            ...
            from
                lis.AnalyzerWorksheetEvnLabSample with (nolock)
            where
            ...
            if (!empty($response[0])) {
                foreach ($response as $row) {
                    $query = "
                        ...
                        exec lis.p_AnalyzerWorksheetEvnLabSample_del
                            @AnalyzerWorksheetEvnLabSample_id = :AnalyzerWorksheetEvnLabSample_id,
		...
		$q = "
			declare
				...
				@ErrCode int,
				@ErrMessage varchar(4000);
			set @AnalyzerWorksheetEvnLabSample_id = :AnalyzerWorksheetEvnLabSample_id;
			exec lis." . $procedure . "
				...
				@Error_Code = @ErrCode output,
				@Error_Message = @ErrMessage output;
			select @AnalyzerWorksheetEvnLabSample_id as AnalyzerWorksheetEvnLabSample_id, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
		";
		$p = array(
...
DECLARE --получение списка свободных ячеек в штативе этого рабочего списка
	...
	(
	 SELECT 1
	 UNION ALL
	 SELECT ID + 1 FROM X WHERE ID<(SELECT AnalyzerRack_DimensionX FROM lis.AnalyzerRack with (nolock) WHERE AnalyzerRack_id  = (SELECT AnalyzerRack_id FROM lis.AnalyzerWorksheet with (nolock) WHERE AnalyzerWorksheet_id = @AnalyzerWorksheet_id))
	),
	Y(ID) AS
	(
	 SELECT 1
	 UNION ALL
	 SELECT ID + 1 FROM Y WHERE ID<(SELECT AnalyzerRack_DimensionY FROM lis.AnalyzerRack with (nolock) WHERE AnalyzerRack_id  = (SELECT AnalyzerRack_id FROM lis.AnalyzerWorksheet with (nolock) WHERE AnalyzerWorksheet_id = @AnalyzerWorksheet_id))
	)
	SELECT x.ID AS x, y.ID AS y FROM X,Y with(nolock)
	WHERE NOT EXISTS (SELECT * FROM lis.AnalyzerWorksheetEvnLabSample with (nolock) WHERE AnalyzerWorksheet_id = @AnalyzerWorksheet_id AND AnalyzerWorksheetEvnLabSample_X = x.ID AND AnalyzerWorksheetEvnLabSample_Y = y.ID)
	ORDER BY 1,2
BEGIN TRY
	BEGIN TRANSACTION
							...
							set
								els.Analyzer_id = (
									select top 1
										aw.Analyzer_id
									from
										lis.v_AnalyzerWorksheet aw (nolock)
										inner join lis.v_AnalyzerWorksheetEvnLabSample awels (nolock) on awels.AnalyzerWorksheet_id = aw.AnalyzerWorksheet_id
									where
