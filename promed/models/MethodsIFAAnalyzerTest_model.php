...
class MethodsIFAAnalyzerTest_model extends Scenario_model
{
	...
	function doLoadGrid($data = array())
	{
		...
		$query = "
			SELECT
				...
				MI.MethodsIFA_Code,
				MI.MethodsIFA_Name,
				MI.MethodsIFA_TestVolume,
				MI.MethodsIFA_Sens,
				MI.MethodsIFA_IncTemp,
				MI.MethodsIFA_IncTime,
				MI.MethodsIFA_MinValue,
				MI.MethodsIFA_MaxValue,
				MI.MethodsIFA_Wavelength,
				F.FULLNAME,
				U.Unit_Name
			FROM v_MethodsIFAAnalyzerTest MIAT with(nolock)
			INNER JOIN v_MethodsIFA MI with(nolock) on MI.MethodsIFA_id = MIAT.MethodsIFA_id
			LEFT JOIN v_Unit U on U.Unit_id = MI.Unit_id
	...
	function beforeSave()
	{
		...
		$query = "
			SELECT MethodsIFAAnalyzerTest_id
			FROM v_MethodsIFAAnalyzerTest with(nolock)
	...
	function _afterSave($result)
	{
		...
		$query = "
			SELECT
				MIRV.RefValues_id
			FROM
				v_MethodsIFARefValues MIRV with(nolock)
				LEFT JOIN lis.v_AnalyzerTestRefValues ATRV with(nolock) on ATRV.RefValues_id = MIRV.RefValues_id and ATRV.AnalyzerTest_id = :AnalyzerTest_id
			WHERE
	...
	protected function beforeDelete()
	{
		...
		$query = "
			...
			FROM
				lis.v_AnalyzerTestRefValues ATRV with(nolock)
				INNER JOIN v_MethodsIFAAnalyzerTest MIAT with(nolock) on MIAT.AnalyzerTest_id = ATRV.AnalyzerTest_id
				INNER JOIN v_MethodsIFARefValues MIRV with(nolock) on MIRV.MethodsIFA_id = MIAT.MethodsIFA_id and MIRV.RefValues_id = ATRV.RefValues_id
			WHERE
