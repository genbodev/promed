 ...
 */
class PersonRegisterBase_model extends swModel
{
	...
	function getPersonRegisterOutCauseCode()
	{
		...
		if (empty($this->_personRegisterOutCauseCode)) {
			...
			select PersonRegisterOutCause_Code from dbo.v_PersonRegisterOutCause (nolock) where PersonRegisterOutCause_id = :PersonRegisterOutCause_id
	...
	protected function _loadDataCheckExist()
	{
		...
		switch (true) {
			...
			case ('orphan' == $this->personRegisterTypeSysNick): // одинаково для 'nolos' и 'orphan'
				...
				break;
			/*
			case ('crazy' == $this->morbusTypeSysNick && 'narko' == $this->personRegisterTypeSysNick):
				$queryParams['MorbusType_id'] = $this->MorbusType_id;
				$add_where = 'AND PR.MorbusType_id = :MorbusType_id
				';
				$add_join = "inner join v_Diag diag with (nolock) on diag.Diag_id = PR.Diag_id AND diag.Diag_Code like 'F1%'
				";
				break;
			case ('crazy' == $this->morbusTypeSysNick && 'crazy' == $this->personRegisterTypeSysNick):
				$queryParams['MorbusType_id'] = $this->MorbusType_id;
				$add_where = 'AND PR.MorbusType_id = :MorbusType_id
				';
				$add_join = "inner join v_Diag diag with (nolock) on diag.Diag_id = PR.Diag_id AND diag.Diag_Code not like 'F1%'
				";
				break;
			case ('hepa' == $this->morbusTypeSysNick):
				$queryParams['MorbusType_id'] = $this->MorbusType_id;
				$add_where = 'AND PR.MorbusType_id = :MorbusType_id
				';
				break;
			case ('orphan' == $this->morbusTypeSysNick):
				$queryParams['MorbusType_id'] = $this->MorbusType_id;
				$add_where = 'AND PR.MorbusType_id = :MorbusType_id
				';
				break;
			case ('tub' == $this->morbusTypeSysNick):
				$queryParams['MorbusType_id'] = $this->MorbusType_id;
				$add_where = 'AND PR.MorbusType_id = :MorbusType_id
				';
				break;
			case ('nephro' == $this->morbusTypeSysNick):
				$queryParams['MorbusType_id'] = $this->MorbusType_id;
				$add_where = 'AND PR.MorbusType_id = :MorbusType_id
				';
				break;
			case ('prof' == $this->morbusTypeSysNick):
				$queryParams['MorbusType_id'] = $this->MorbusType_id;
				$add_where = 'AND PR.MorbusType_id = :MorbusType_id
				';
				...
				break;
			case ('ibs' == $this->morbusTypeSysNick):
				$queryParams['MorbusType_id'] = $this->MorbusType_id;
				$add_where = 'AND PR.MorbusType_id = :MorbusType_id
				';
				break;
			case ('hiv' == $this->morbusTypeSysNick):
				$queryParams['MorbusType_id'] = $this->MorbusType_id;
				$add_where = 'AND PR.MorbusType_id = :MorbusType_id
				';
				break;
			case ('diabetes' == $this->morbusTypeSysNick):
				$queryParams['MorbusType_id'] = $this->MorbusType_id;
				$add_where = 'AND PR.MorbusType_id = :MorbusType_id
				';
				break;
			case ('large family' == $this->morbusTypeSysNick):
				$queryParams['MorbusType_id'] = $this->MorbusType_id;
				$add_where = 'AND PR.MorbusType_id = :MorbusType_id
				';
				break;
	...
	function loadChangedPersonRegisterIdList($type_id)
	{
		...
		$lastExport = $this->getFirstResultFromQuery('
				...
				from PersonRegisterExport E with (nolock)
				inner join PersonRegister R with (nolock) on R.PersonRegister_id = E.PersonRegister_id
		...
		if (isset($lastExport) && $lastExport instanceof DateTime) {
			...
			$query = "
					declare @last_export datetime = cast ('{$lastExport}' as datetime)
					select E.PersonRegister_id
					from v_EvnNotifyRegister E with (nolock)
					where E.PersonRegisterType_id = :PersonRegisterType_id and E.PersonRegister_id is not null
					and E.EvnNotifyRegister_insDT >= @last_export
				";
		} else {
			$query = "
					select E.PersonRegister_id
	...
	protected function _checkPersonRegisterExist()
	{
		...
		if (count($response) > 0) {
			switch (true) {
				/*case ('crazy' == $this->morbusTypeSysNick && isset($response[0]['PersonRegisterOutCause_id'])):
					//echo"dsfsd";//Если тип заболевания Crazy(narko) и был исключен из регистра.
					$this->_saveResponse['PersonRegister_id'] = $response[0]['PersonRegister_id'];
					$this->_saveResponse['PersonRegisterOutCause_id'] = $response[0]['PersonRegisterOutCause_id'];
					$this->_saveResponse['Alert_Msg'] = 'Пациент был исключен из регистра. <br />Вернуть пациента в регистр?'; //(Новое/Предыдущее/Отмена)
	...
	{
		$result = $this->db->query('SELECT PersonRegisterType_id, PersonRegisterType_SysNick FROM dbo.v_PersonRegisterType (nolock)', array());
	...
	public function loadTypeListByDiagList($diag_list)
	{
		...
		$result = $this->db->query("
			select prt.PersonRegisterType_id, prt.PersonRegisterType_SysNick, mt.MorbusType_id, mt.MorbusType_SysNick, rtd.Diag_id from v_PersonRegisterDiag rtd (nolock)
			inner join v_PersonRegisterType prt (nolock) on prt.PersonRegisterType_id = rtd.PersonRegisterType_id
			left join v_MorbusType mt (nolock) on mt.MorbusType_id = rtd.MorbusType_id
	...
	public function loadTypeListByDiag($diag_id, $type_id = null)
	{
		...
		$result = $this->db->query("
			select prt.PersonRegisterType_id, prt.PersonRegisterType_SysNick, mt.MorbusType_id, mt.MorbusType_SysNick from v_PersonRegisterDiag rtd (nolock)
			inner join v_PersonRegisterType prt (nolock) on prt.PersonRegisterType_id = rtd.PersonRegisterType_id
			left join v_MorbusType mt (nolock) on mt.MorbusType_id = rtd.MorbusType_id
	...
	function getViewData($data)
	{
		...
		$query = "
			SELECT TOP 1
				...
				,PDG.PersonDecreedGroup_id
				,PDG.PersonDecreedGroup_Name
			FROM
				v_PersonRegister PR WITH (NOLOCK)
				inner join v_PersonRegisterType PRT WITH (NOLOCK) on PRT.PersonRegisterType_id = PR.PersonRegisterType_id AND PRT.PersonRegisterType_SysNick LIKE :PersonRegisterType_SysNick
	...
	function getPersonPrivilegeRegAllViewData($data)
	{
		...
		$query = '
			...
			FROM
				...
				inner join v_PrivilegeType PT WITH (NOLOCK) on PT.PrivilegeType_id = PP.PrivilegeType_id and PT.ReceptFinance_id = 2
				outer apply (
	...
	function getPersonPrivilegeFedAllViewData($data)
	{
		...
		$query = '
			...
			FROM
				v_PersonPrivilege PP WITH (NOLOCK)
				inner join v_PrivilegeType PT WITH (NOLOCK) on PT.PrivilegeType_id = PP.PrivilegeType_id and PT.ReceptFinance_id = 1
				outer apply (
					select top 1 PR.PersonRefuse_IsRefuse from PersonRefuse PR WITH (NOLOCK)
					where PR.Person_id = PP.Person_id and PR.PersonRefuse_Year = year(@curDate)
					order by PR.PersonRefuse_IsRefuse desc
		...
		try {
			...
			switch ($data['PersonRegisterType_SysNick']) {
				case 'nolos':
					...
					break;
				case 'orphan':
					$join = 'inner join v_Diag WITH (NOLOCK) on v_Diag.Diag_id = PR.Diag_id';
					$diag_filter = "(
						(v_Diag.Diag_Code not in ('E70.0','E70.1') AND PRD.Diag_id = PR.Diag_id)
						OR (v_Diag.Diag_Code in ('E70.0','E70.1') AND exists (
							select top 1 ND.Diag_id from v_Diag ND with (nolock)
			...
			$query = "
				...
				FROM
					...
					{$join}
					inner join v_PersonRegisterDiag PRD WITH (NOLOCK) on PRD.PersonRegisterType_id = PR.PersonRegisterType_id
						AND {$diag_filter}
				WHERE
					PR.PersonRegister_id = :PersonRegister_id
			...
			$query = "
				SELECT
					'view' as accessType
					,EvnRecept.Entity + '_' + convert(varchar,isnull(EvnRecept.EvnRecept_id,EvnRecept.ReceptOtov_id)) as PersonDrug_id
					...
					,rtrim(ISNULL(DrugMnn.DrugMnn_Name, dcm.DrugComplexMnn_RusName)) as DrugMnn_Name -- МНН
					,rtrim(ISNULL(RDR.Drug_Name, Drug.Drug_Name)) as DrugTorg_Name -- Торговое наименование
				...
				FROM
					...
					left join rls.v_Drug RDR with (NOLOCK) on RDR.Drug_id = EvnRecept.Drug_rlsid
					left join rls.v_DrugComplexMnn dcm with (NOLOCK) on dcm.DrugComplexMnn_id = isnull(EvnRecept.DrugComplexMnn_id,RDR.DrugComplexMnn_id)
