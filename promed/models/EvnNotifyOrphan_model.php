	...
	function load($data)
	{
		$query = '
			select
				...
				ENO.Lpu_oid,
				convert(varchar,ENO.EvnNotifyOrphan_setDT,104) as EvnNotifyOrphan_setDT,
				ENO.MedPersonal_id
			from
				v_EvnNotifyOrphan ENO with (nolock)
				left join v_EvnVizitPL PL with (nolock) on ENO.EvnNotifyOrphan_pid = PL.EvnVizitPL_id
				left join v_EvnSection ST with (nolock) on ENO.EvnNotifyOrphan_pid = ST.EvnSection_id
	...
	function del($data)
	{
		$query = '
			declare
				...
				@ErrCode bigint,
				@ErrMsg varchar(4000);
			set @Res = :EvnNotifyOrphan_id;
			exec p_EvnNotifyOrphan_del
				@EvnNotifyOrphan_id = @Res,
				@pmUser_id = :pmUser_id,
				@Error_Code = @ErrCode output,
				@Error_Message = @ErrMsg output;
			select @Res as EvnNotifyOrphan_id, @ErrCode as Error_Code, @ErrMsg as Error_Msg;
		';
	...
	*/
	function medpersonalBaseAttach($data)
	{
		...
		$query = '
			...
			left join v_MedStaffRegion MSR with (nolock) on MSR.LpuRegion_id = PC.LpuRegion_id and MSR.Lpu_id = PC.Lpu_id
			where
	...
	*/
	function loadMorbusOrphanData($data)
	{
		$query = '
			...
			inner join v_Morbus M with (nolock) on M.Person_id = PS.Person_id
		...
		*/
		...
		$query = '
			declare
				...
				@ErrMsg varchar(4000);
			set @Res = null;
			exec p_EvnDirectionOrphan_' . $procedure_action . '
				@EvnDirectionOrphan_id = @Res output,
	...
	function createEvnNotifyOrphanOut($data)
	{
		...
		$query = '
			declare
				...
				@ErrMsg varchar(4000);
			set @Res = null;
			exec p_EvnNotifyOrphanOut_' . $procedure_action . '
				@EvnNotifyOrphanOut_id = @Res output,
