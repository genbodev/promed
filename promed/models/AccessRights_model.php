...
class AccessRights_model extends swModel {
	/**
	 * Конструктор
	 */
	...
	function saveAccessRightsName($data) {
		...
		$query = "
			declare
				@AccessRightsName_id bigint = :AccessRightsName_id,
				@Error_Code bigint,
				@Error_Message varchar(4000);
			exec {$procedure}
				@AccessRightsName_id = @AccessRightsName_id output,
			...
			select @AccessRightsName_id as AccessRightsName_id, @Error_Code as Error_Code, @Error_Message as Error_Msg
		";
	...
	function loadAccessRightsLimitGrid($data) {
		...
		$query = "
			select
				...
				LBAR.LpuBuildingAccessRightsLink_id,
				ARL.AccessRightsLimit_id,
				case
			...
			from v_AccessRightsLimit ARL with(nolock)
				left join v_PostMed PM with(nolock) on PM.PostMed_id = ARL.Post_id
				left join v_Lpu L with(nolock) on L.Org_id = ARL.Org_id
				left join v_pmUserCache pmUser with(nolock) on pmUser.pmUser_id = ARL.AccessRightsType_User
				
				left join v_LpuBuildingAccessRightsLink LBAR with(nolock) on ARL.AccessRightsLimit_id = LBAR.AccessRightsLimit_id
				left join v_LpuBuilding LB with(nolock) on LB.LpuBuilding_id = LBAR.LpuBuilding_id
			where
	...
	function loadAccessRightsLimitUsersGrid($data) {
		...
		$query = "
			select
				...
				pmUser.pmUser_Login
			from v_pmUserCache pmUser with(nolock)
			left join v_AccessRightsLimit ARL with(nolock)
				on ARL.AccessRightsType_User = pmUser.PMUser_id and ARL.AccessRightsName_id = :AccessRightsName_id
			where pmUser.Lpu_id = :Lpu_id and ISNULL(pmUser.pmUser_deleted, 1) = 1
		";
	...
	function saveAccessRightsLimit($data) {
		...
		$query = "
			declare @Org_id bigint = (select top 1 Org_id from v_Lpu with (nolock) where Lpu_id = :Lpu_id);

			declare
				@AccessRightsLimit_id bigint = :AccessRightsLimit_id,
				@Error_Code bigint,
				@Error_Message varchar(4000);
			exec {$procedure}
				@AccessRightsLimit_id = @AccessRightsLimit_id output,
				@AccessRightsName_id = :AccessRightsName_id,
				@Post_id = :Post_id,
				@Org_id = @Org_id,
				...
				@Error_Code = @Error_Code output,
				@Error_Message = @Error_Message output
			select @AccessRightsLimit_id as AccessRightsLimit_id, @Error_Code as Error_Code, @Error_Message as Error_Msg
		";
	...
	function saveLpuBuildingAccessRightsLinkLimit($data){
		...
		$query = "
			declare
				@LpuBuildingAccessRightsLink_id bigint = :LpuBuildingAccessRightsLink_id,
				@Error_Code bigint,
				@Error_Message varchar(4000);
			exec {$procedure}
				@LpuBuildingAccessRightsLink_id = @LpuBuildingAccessRightsLink_id output,
				@AccessRightsLimit_id = :AccessRightsLimit_id,
				@LpuBuilding_id = :LpuBuilding_id,
				@pmUser_id = :pmUser_id,
				@Error_Code = @Error_Code output,
				@Error_Message = @Error_Message output
			select @LpuBuildingAccessRightsLink_id as LpuBuildingAccessRightsLink_id, @Error_Code as Error_Code, @Error_Message as Error_Msg
		";
	...
	function deleteLpuBuildingAccessRightsLink($data){
		...
		$query = "
			declare
				@Error_Code bigint,
				@Error_Message varchar(4000);
			exec p_LpuBuildingAccessRightsLink_del
	...
	function getAccessRightsLpuBuildingLimit($data){
		...
		$query  = "
			select
				LBAR.LpuBuildingAccessRightsLink_id,
				LBAR.AccessRightsLimit_id,
				LBAR.LpuBuilding_id
			from v_LpuBuildingAccessRightsLink LBAR with(nolock)
	...
	function getAccessRightsForUser($data) {
		...
		if (count($lpu_arr) > 0) {
			$limit_where[] = "(
				ARL.Org_id in (select Org_id from v_Lpu with (nolock) where Lpu_id in (".implode(',', $lpu_arr)."))
		...
		$query = "
			select
				...
				case when
					exists(
						...
						from v_AccessRightsLimit ARL with(nolock)
						left join v_LpuBuildingAccessRightsLink LBARL with(nolock) on LBARL.AccessRightsLimit_id = ARL.AccessRightsLimit_id
						where
			...
			from v_AccessRightsName ARN with(nolock)
				inner join v_AccessRightsType ART with(nolock) on ART.AccessRightsType_id = ARN.AccessRightsType_id
		";
		//echo getDebugSQL($query, $params);exit;
		$result = $this->db->query($query, $params);
		if (!is_object($result)) {
		...
		$query = "
			...
			from v_AccessRightsDiag ARD with(nolock)
				inner join v_Diag fD with(nolock) on fD.Diag_id = ARD.Diag_fid
		...
		$query = "
			select
				ARO.AccessRightsName_id,
				Lpu.Lpu_id
			from v_AccessRightsOrg ARO with(nolock)
				inner join v_Lpu Lpu with(nolock) on Lpu.Org_id = ARO.Org_id
				left join v_LpuBuildingAccessRightsLink LBARL with(nolock) on LBARL.AccessRightsOrg_id = ARO.AccessRightsOrg_id
			where 
	...
	function saveAccessRightsArmSmo($data) {
		...
		$query = "
			select
				ARO.Org_id
			from
				v_AccessRightsOrg ARO with (nolock)
				inner join v_AccessRightsName ARN with(nolock) on ARN.AccessRightsName_id = ARO.AccessRightsName_id
			where
		...
		$query = "
			select top 1 AccessRightsName_id
			from v_AccessRightsName with (nolock)
		...
		$query = "
			declare
				@AccessRightsOrg_id bigint = :AccessRightsOrg_id,
				@Error_Code bigint,
				@Error_Message varchar(4000);

			exec {$procedure}
				@AccessRightsOrg_id = @AccessRightsOrg_id output,
				@AccessRightsName_id = :AccessRightsName_id,
				@Org_id = :Org_id,
				@pmUser_id = :pmUser_id,
				@Error_Code = @Error_Code output,
				@Error_Message = @Error_Message output
			select @AccessRightsOrg_id as AccessRightsOrg_id, @Error_Code as Error_Code, @Error_Message as Error_Msg
		";
	...
	function loadAccessRightsArmSmoGrid($data) {
		...
		$query = "
			...
			from
				v_AccessRightsOrg ARO with (nolock)
				inner join v_AccessRightsName ARN with(nolock) on ARN.AccessRightsName_id = ARO.AccessRightsName_id
				left join v_OrgSMO SMO with (nolock) on SMO.Org_id = ARO.Org_id
			where
				1=1
				{$filterCodes}
		";
	...
	function checkAccessTfomsToFunctionalEMK($data) {
		...
		$query = "
			...
			from
				v_AccessRightsOrg ARO with (nolock)
				inner join v_AccessRightsName ARN with(nolock) on ARN.AccessRightsName_id = ARO.AccessRightsName_id
				left join OrgSMO OS with(nolock) on OS.Org_id=ARO.Org_id
			where
				ARO.Org_id = :Org_id
	...
	function loadAccessT9Grid($data) {
		$query = "
			select
				...
				ug.pmUserCacheGroup_SysNick as Group_Code,
		...
		if ( $result === true ) {
			...
			foreach($groups as $group) {
				...
				declare
					@Error_Code bigint,
					@Error_Message varchar(4000);
				exec p_AccessRightsT9Limit_ins
