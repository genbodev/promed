			...
			case 'registerMarksByRequisites': // регистрация выбытия кодов маркировки по реквизитам документа-основания
				...
				$query = "
					...
					from DocumentUc du with (nolock)
						outer apply (Select top 1 er.EvnRecept_Ser, er.EvnRecept_Num, dus.DocumentUc_id  
							from v_documentUcStr dus  with (nolock)
							join v_EvnRecept er  with (nolock) on er.EvnRecept_id = dus.EvnRecept_id
							 where {$where}
						) dus
				...
				$query = "
					...
					from DrugPackageBarCode bc with (nolock)
						inner join v_DocumentUcStr dus with (nolock) on dus.DocumentUcStr_id = bc.DocumentUcStr_id
				...
				if (is_object($result)) {
					...
					if (count($marks_array) > 0 ) {
						...
						$query = "
							...
							from dbo.DrugPackageBarCode bc with (nolock)
								inner join v_DocumentUcStr dus with (nolock) on dus.DocumentUcStr_id = bc.DocumentUcStr_id
								inner join v_DocumentUc du with (nolock) on du.DocumentUc_id = dus.DocumentUc_id
									and DrugDocumentType_id in(11)
									and du.DrugDocumentStatus_id in (2, 7)
							...
							update dbo.DrugPackageBarCode with (rowlock)
						...
						//$mdlpRequestId = $result->results->result->mdlpRequestId; // ИД отчета СУЗ
	...
	public function QueueUpRegisterMarksList($data) {
		...
		$query = "
				with er as (
					...
					Select top 100 NEWID() as rvRequestId, EvnRecept_id from er
