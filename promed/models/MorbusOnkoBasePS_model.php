 ...
 *
 */
class MorbusOnkoBasePS_model extends swModel
{
	/**
	 * Пользователь
	 * @var integer
	...
	public function read()
	{
		...
		$sql = "
				SELECT
					...
					MorbusOnkoBase_id,
					OnkoPurposeHospType_id,
					convert(varchar(10), MorbusOnkoBasePS_setDT, 104) as MorbusOnkoBasePS_setDT,
					convert(varchar(10), MorbusOnkoBasePS_disDT, 104) as MorbusOnkoBasePS_disDT,
				    OnkoHospType_id,
				    Diag_id,
				    Lpu_id,
					LpuSection_id,
					...
					Evn_id
				FROM
					dbo.v_MorbusOnkoBasePS with (nolock)
				WHERE
					MorbusOnkoBasePS_id = ?
			";
	...
	public function save($data = array())
	{
		...
		$sql = "
			declare
				...
				@ErrCode int,
				@ErrMsg varchar(4000);
			set @Res = :MorbusOnkoBasePS_id;
			exec {$procedure}
				@MorbusOnkoBasePS_id = @Res output,
			...
			select @Res as MorbusOnkoBasePS_id, @ErrCode as Error_Code, @ErrMsg as Error_Msg;
		";
		$params = array(
	...
	public function destroy($data = array())
	{
		...
		$sql = '
				...
				exec dbo.p_MorbusOnkoBasePS_del
	...
	function getViewData($data = array())
	{
		...
		$query = "
			SELECT
				'edit' as accessType,
				MOBPS.MorbusOnkoBasePS_id,
				MOBPS.MorbusOnkoBase_id,
				convert(varchar(10), MOBPS.MorbusOnkoBasePS_setDT, 104) as MorbusOnkoBasePS_setDT,
				convert(varchar(10), MOBPS.MorbusOnkoBasePS_disDT, 104) as MorbusOnkoBasePS_disDT,
				OPST.OnkoPurposeHospType_id,
			...
			FROM
				...
				INNER JOIN dbo.v_MorbusOnkoBasePS MOBPS WITH (NOLOCK) on MOB.MorbusOnkoBase_id = MOBPS.MorbusOnkoBase_id
				INNER JOIN dbo.v_OnkoPurposeHospType OPST WITH (NOLOCK) ON MOBPS.OnkoPurposeHospType_id = OPST.OnkoPurposeHospType_id
				left join v_Evn EvnEdit with (nolock) on EvnEdit.Evn_id = :Evn_id and Morbus.Person_id != :Evn_id
			where
	...
	function getMorbusOnkoSopDiagData($data = array())
	{
		...
		$query = "
			SELECT
				case when
					(
						...
						or (MOBPS.Evn_id = EvnEdit.Evn_id and isnull(EvnEdit.Evn_IsSigned,1) = 1)
			...
			FROM
				...
				left join v_Diag d with (nolock) d.Diag_id = mobdl.Diag_id
			where
				Morbus.Morbus_id = :Morbus_id
		";
		$params = array(
	...
	function getHosp($data) {
		$query = "
			...
			from v_MorbusBase MB with(nolock)
			join v_MorbusOnkoBase MOB with(nolock) on MOB.MorbusBase_id = MB.MorbusBase_id
			join v_MorbusOnkoBasePS MOBPS with(nolock) on MOBPS.MorbusOnkoBase_id = MOB.MorbusOnkoBase_id 
			...
			and (MOBPS.LpuSection_id is null or MOBPS.LpuSection_id = :LpuSection_id)
		";
	...
	function getMorbusBaseData($data) {
		$query = "
			...
			MO.OnkoTreatment_id as OnkoTreatment_id
			 from v_MorbusBase MB with(nolock)
			 join v_MorbusOnkoBase MOB with(nolock) on MOB.MorbusBase_id = MB.MorbusBase_id
			 join v_MorbusOnko MO with(nolock) on MO.MorbusBase_id = MOB.MorbusBase_id 
