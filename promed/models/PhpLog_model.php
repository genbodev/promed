 ...
 */
class PhpLog_model extends swModel {
	 ...
	 * @return array
	 */
	private function findOrCreateMethod($value, $ControllerID, &$new, $data = null, $controller) {
		$new = true;
		$Methods_IsUsePersonData = 0;
		$value = ($value == null) ? "" : $value;
		if ($data == null) {
		...
		$c_file = file_get_contents("promed/controllers/{$controller}.php");
		if (preg_match("#\/\*\*[^\/]+\@usePersonData([^\}]+)function(\s+){$value}#uis", $c_file)) {
			$Methods_IsUsePersonData = 1;
		}
		
		$query = "
			insert into Methods(Method_Name, Controller_ID, Method_Name_Ru, pmUser_insID, pmUser_updID, Methods_insDT, Methods_updDT, Methods_IsUsePersonData)
			values (:Method_Name, :Controller_ID, :Method_Name, :pmUser_id, :pmUser_id, getdate(), getdate(), :Methods_IsUsePersonData);
		";
		$queryParams = [
			"Method_Name" => $value,
			"Controller_ID" => $ControllerID,
			"Methods_IsUsePersonData" => $Methods_IsUsePersonData,
			"pmUser_id" => 1
		];
		$this->db->query($query, $queryParams);
		$query = "
	...
	private function updateControllersAndMethodsNames() {
		...
		$query = "
			...
			from {$this->default_db}.dbo.Methods T1
				,{$this->default_db}.dbo.Controllers T2
		...
		foreach ($dataMethods as $dataMethod) {
			$query = "
				update php_log.dbo.Methods
				set Method_Name_Ru = '{$dataMethod["Method_Name_Ru"]}'
				where Controller_ID in (select Controller_ID from php_log.dbo.Controllers where Controller_Name = '{$dataMethod["Controller_Name"]}')
	...
	function loadOldLogToNewFormat() {
		...
		for ($i = 0; $i < 1000; $i++) {
			...
			foreach ($result as $resultItem) {
				...
				$isNew = null;
				$methodRecord = $this->findOrCreateMethod($resultItem["Method"], $controllerRecord["Controller_ID"], $isNew, $methods, $resultItem["Controller"]);
				if ($isNew == true) {
					$methods[] = $methodRecord;
				}
				$dateTime = $resultItem["PHPLog_insDT"];
				...
				$sql .= getDebugSql("insert into php_log.dbo.PHPLog_{$tablePrefix}({$lineFields}) values ({$lineValues});", $sqlParams);
				$idxs[] = $resultItem["PHPLog_id"];
			}
			if (trim($sql) != "") {
				$this->db->query($sql);
				$this->db->query("delete from php_log.dbo.PHPLog2 where PHPLog_id in (" . implode(",", $idxs) . ");");
			}
		}
		return [];
	}
	...
	function transferDataFromMongoDB() {
		...
		if ($count > 0) {
			...
			try {
				$lastInsDT = $rows[$count_rows - 1]["PHPLog_insDT"]; // Удаляются все записи, переваленные из MongoDb->sysLog в MSSQL->PHPLog2.
	...
	function loadPhpLogGrid($data) {
		...
		if ($isNew == true) {
			...
			if (!empty($data["ARMType"])) {
				$queryRow = $this->getFirstRowFromQuery("select ARMType_id from {$this->default_db}.dbo.ARMType where ARMType_Name = '{$data["ARMType"]}'");
				$filters[] = "PL[N].ARMType_id = :ARMType";
				$queryParams["ARMType"] = $queryRow["ARMType_id"];
			}
			if (!empty($data["Controller"])) {
				$filters[] = "PL[N].Controller_ID in (select Controller_ID from Controllers where Controller_Name like :Controller + '%')";
				$queryParams["Controller"] = $data["Controller"];
			}
			if (!empty($data["Method"])) {
				$filters[] = "PL[N].Method_ID in (select Method_ID from Methods where Method_Name like :Method + '%')";
				$queryParams["Method"] = $data["Method"];
			}
			if (!empty($data["methodAdvanced"])) {
				$filters[] = "PL[N].Method_ID in (select Method_ID from Methods where Method_Name_Ru like :methodAdvanced + '%')";
			...
			if (!empty($data["ET_Query_from"])) {
				$filters[] = "ROUND(PL[N].ET_Query, 4) >= :ET_Query_from";
				$queryParams["ET_Query_from"] = $data["ET_Query_from"];
			}
			if (!empty($data["ET_Query_to"])) {
				$filters[] = "ROUND(PL[N].ET_Query, 4) <= :ET_Query_to";
				$queryParams["ET_Query_to"] = $data["ET_Query_to"];
			}
			if (!empty($data["PMUser_Login"])) {
				$filters[] = "PL[N].pmUser_insID in (select PMUser_id from {$this->default_db}.dbo.pmUserCache where PMUser_Login like '{$data["PMUser_Login"]}%')";
			}
			if (!empty($data["IP"])) {
				$filters[] = "PL[N].IP = :IP";
			...
			if (!empty($data["POST"])) {
				$filters[] = "PL[N].POST like '%' + :POST + '%'";
				$queryParams["POST"] = $data["POST"];
			}
			if (!empty($data['isUsePersonData']) && $data['isUsePersonData'] == 2) {
				$filters[] = "PL[N].Method_ID in (select Method_ID from Methods where Methods_IsUsePersonData = 1)";
			}
			...
			foreach ($tables as $table) {
				...
				$querys[] = "
					select
					    ...
					    ROUND(PL{$n}.ET_Query, 4) as ET_Query,
					    PL{$n}.IP as IP,
					...
					from php_log.dbo.{$table} PL{$n} with(nolock)
					{$subFilterString}
					{$dateFilter}
				";
				$nCount++;
			...
			$query = "
				...
				from
					-- from
					({$queryString}) Q
					left join {$this->default_db}.dbo.pmUserCache PUC on PUC.PMUser_id = Q.PMUser_id
					left join Controllers CTS on CTS.Controller_ID = Q.Controller_ID
					left join Methods MTS on MTS.Method_ID = Q.Method_ID
					left join {$this->default_db}.dbo.ARMType ARMT on ARMT.ARMType_id = Q.ARMType_id
					-- end from
				order by
					-- order by
					Q.PHPLog_insDT desc
			...
			if (!empty($data["Controller"])) {
				$filters[] = "(PL.Controller like :Controller + '%' or CTS.Controller_Name_Ru like :Controller + '%')";
				$queryParams["Controller"] = $data["Controller"];
			}
			if (!empty($data["Method"])) {
				$filters[] = "(PL.Method like :Method + '%' or MTS.Method_Name_Ru like :Method + '%')";
				$queryParams["Method"] = $data["Method"];
			}
			if (!empty($data["methodAdvanced"])) {
				$filters[] = "(PL.Method like :methodAdvanced + '%' or MTS.Method_Name_Ru like :methodAdvanced + '%')";
				$queryParams["methodAdvanced"] = $data["methodAdvanced"];
			}
			if (!empty($data["ET_from"])) {
			...
			if (!empty($data["ET_Query_from"])) {
				$filters[] = "ROUND(PL.ET_Query, 4) >= :ET_Query_from";
				$queryParams["ET_Query_from"] = $data["ET_Query_from"];
			}
			if (!empty($data["ET_Query_to"])) {
				$filters[] = "ROUND(PL.ET_Query, 4) <= :ET_Query_to";
				$queryParams["ET_Query_to"] = $data["ET_Query_to"];
			}
			if (!empty($data["PMUser_Login"])) {
				$filters[] = "PUC.PMUser_Login like :PMUser_Login + '%'";
			...
			if (!empty($data["POST"])) {
				$filters[] = "PL.POST like '%' + :POST + '%'";
				$queryParams["POST"] = $data["POST"];
			}
			if (!empty($data["ARMType"])) {
				$queryRow = $this->getFirstRowFromQuery("select ARMType_id from ARMType where ARMType_Name = '{$data["ARMType"]}'");
			...
			"
				: "";
			$join = ($this->isNew()) ? "left join {$this->default_db}.dbo.ARMType ARMT on ARMT.ARMType_id = PL.ARMType_id" : "";
			$fieldString = ($this->isNew()) ? ",ARMT.ARMType_Name as ARMType_Name" : "";
			$query = "
				select
					-- select
					PL.PHPLog_id,
				    convert(varchar(19), cast(PL.PHPLog_insDT as datetime), 120) as PHPLog_insDT,
				    PL.Controller + ' ('+ CTS.Controller_Name_Ru +')' as Controller,
				    PL.Method + IIF(MTS.Method_Name_Ru is not null, ' ('+ MTS.Method_Name_Ru +')', '') as Method,
				    MTS.Method_Name_Ru as Method_Name_Ru,
				    PL.ET,
				    PL.QueryString,
				    ROUND(PL.ET_Query, 4) as ET_Query,
				    PL.IP,
				    PL.Server_IP,
				    PL.AnswerError,
				    PL.POST,
				...
				from
					-- from
					{$this->logs_table} PL with(nolock)
					left join {$this->default_db}.dbo.pmUserCache PUC on PUC.PMUser_id = PL.pmUser_insID
					left join {$this->default_db}.dbo.Controllers CTS on CTS.Controller_Name = PL.Controller
					left join {$this->default_db}.dbo.Methods MTS on upper(MTS.Method_Name) = upper(PL.Method)
					{$join}
					-- end from
				{$filterString}
				order by
