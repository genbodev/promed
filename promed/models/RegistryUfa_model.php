...
class RegistryUfa_model extends Registry_model {
	...
	private $_registryTypeList = array(
		...
		19 => array('RegistryType_id' => 19, 'RegistryType_Name' => 'Взаиморасчеты по лечебно-диагностическим услугам', 'SP_Object' => 'EvnPLUslugaPar'),
	);
	...
	*/
	...
	{   
		...
		$query = "
			...
			exec {$this->scheme}.p_RegistryGroupDataUfa_del
				@Filter = :Filter,
				@Evn_ids = :Evn_ids,
				@Registry_id = :Registry_id,
	...
	*/ 
	...
	function loadEvnVizitErrorData($data)
	{
		...
		if ( $load_errors_only === true )
			$errors_join = " inner join {$this->scheme}.v_{$this->RegistryErrorObject} re with (nolock) on re.Registry_id = rd.Registry_id and re.Evn_id = rd.Evn_id and re.RegistryErrorType_id = 44 ";
	...
	function loadPersonInfoFromErrorRegistry($data)
	{
		...
		$query = "
			select distinct
				...
				convert(varchar(8), rd.Person_BirthDay , 112) as DATE_BORN,
				ISNULL(Sex.Sex_Code, 0) as SEX,
				rd.DocumentType_Code as DOC_TYPE,
				rtrim(isnull(rd.Document_Ser, '')) as DOC_SER,
				rtrim(isnull(rd.Document_Num, '')) as DOC_NUM,
				...
				rtrim(isnull(rd.Polis_Num, '')) as POL_NUM,
				case
			...
			from
				...
				left join Sex with (nolock) on Sex.Sex_id = rd.Sex_id
				left join v_Address_all addr with (nolock) on addr.Address_id = rd.Address_id
				left join OrgSmo osmo with (nolock) on osmo.OrgSmo_id = rd.OrgSmo_id
	...
	function loadRegistryDataForDbfUsing($type, $data)
	{
		...
		{
			case 1: //stac
				$query = "exec {$this->scheme}.p_Registry_EvnPS_Exp @Registry_id = :Registry_id";
				break;

			case 14: //htm
				$query = "exec {$this->scheme}.p_Registry_EvnHTM_Exp @Registry_id = :Registry_id";
				break;
		}
		$result = $this->db->query($query, $params);
		if (is_object($result))
	...
	function getMoreInfoRegistry($data){
        ...
        $query = "
            Select
                ...
                SUM(case when isnull(RD.RegistryData_IsBadVol, 1) = 2 then 0 else RD.RegistryData_Sum_R end) as sum_all,
                -- sum(case when RETF.Evn_id is not null then RD.RegistryData_Sum_R else null end) as sum_err,
                sum(case when isnull(rd.Paid_id,2) = 1 or isnull(RD.RegistryData_IsBadVol, 1) = 2 then RD.RegistryData_Sum_R else 0 end) as sum_err,
                --COUNT(distinct RD.Evn_rid) as count_numcard,
                --COUNT(distinct RD.Person_id) as count_person,
                ISNULL( N_C.count_numcard,0) as count_numcard,
                ISNULL(P_C.count_person,0) as count_person,
                ...
                sum(case when isnull(rd.Paid_id,2) = 2 and isnull(RD.RegistryData_IsBadVol, 1) = 1 then RD.RegistryData_KdFact else 0 end) as Registry_kd_good,
                sum(case when isnull(rd.Paid_id,2) = 1 or isnull(RD.RegistryData_IsBadVol, 1) = 2 then RD.RegistryData_KdFact else 0 end) as Registry_kd_err,
                r.RegistryType_id as RegistryType_id,
                -- end
				isnull(UET.uet, 0) as uet
            from
                r2.Registry r with (nolock)
                left join r2.v_{$this->RegistryDataObject} RD with (nolock) on  RD.Registry_id = r.Registry_id
                left join r2.v_RegistryErrorTFOMS RETF with(nolock) on (RETF.Registry_id = r.Registry_id and RETF.Evn_id = RD.Evn_id and isnull(RD.RegistryData_IsBadVol, 1) = 1)
                outer apply (
                    Select
                        top 1 Registry_State
                    from
                        r2.RegistryStateLog RS with (nolock)
                    where
                        RS.Registry_id = r.Registry_id
                ) rs
                outer apply (
                    select
						sum(RD.EvnVizit_UetOMS) as Uet
					from
						r2.v_RegistryData RD with (NOLOCK)
					where
						RD.Registry_id = :Registry_id
						and ((RD.LpuSectionProfile_Code in ('526','527','528','529','530','559','560','561','562','626','627','628','629','630','659','660','661','662','826','827','828','829','830','859','860','861','862') or RD.EvnClass_id in ('6','16'))
                        or RD.LpuSectionProfile_Code in ('577','677','877'))
                        and isnull(RD.RegistryData_IsBadVol, 1) = 1
                ) UET
                outer apply (
					select COUNT (distinct RD.Person_id) as count_person
					from r2.v_{$this->RegistryDataObject} RD with (NOLOCK)
					where
						RD.Registry_id = :Registry_id
					and isnull(RD.RegistryData_IsBadVol, 1) = 1
                ) P_C
            ...
            group by
                r.Registry_SumR, RS.Registry_State, r.RegistryType_id, UET.Uet, P_C.count_person, N_C.count_numcard
        ";
	...
	function loadRegistry($data) 
	{
		...
		{
			$filter .= ' and ISNULL(R.RegistrySubType_id, 1) = :RegistrySubType_id';
			$params['RegistrySubType_id'] = $data['RegistrySubType_id'];
		} else {
			$filter .= ' and ISNULL(R.RegistrySubType_id, 1) = 1';
		}
		if ( isset($data['Registry_IsNew']) && !isset($data['Registry_id']) ) {
			$filter .= ' and IsNull(R.Registry_IsNew, 1) = :Registry_IsNew';
		...
		if (empty($data['Registry_id']) && (empty($data['RegistryType_id']) || $data['RegistryType_id'] != 19)) {
		...
		if ($loadQueue)
		{
			...
			Select 
				...
				R.OrgSmo_id,
				R.Lpu_cid,
				lc.Lpu_Nick as LpuContragent_Name,
				R.Lpu_oid,
				R.LpuUnitSet_id,
				R.RegistryType_id,
				...
				--RTrim(IsNull(convert(varchar,cast(R.Registry_insDT as datetime),104),'')) as Registry_insDT,
			...
			left join v_OrgSmo OrgSmo with (NOLOCK) on OrgSmo.OrgSmo_id = R.OrgSmo_id
		 	left join v_Lpu l with (nolock) on l.Lpu_id = R.Lpu_oid 
			left join LpuUnitSet with (NOLOCK) on LpuUnitSet.LpuUnitSet_id = R.LpuUnitSet_id 
			left join v_DispClass DC with (NOLOCK) on DC.DispClass_id = R.DispClass_id 
			left join v_PayType pt with (nolock) on pt.PayType_id = ISNULL(R.PayType_id, @PayType_id)
			left join v_Lpu lc with (nolock) on lc.Lpu_id = R.Lpu_cid
			left join r2.RegistryStateLog RSL with (NOLOCK) on R.RegistryQueue_id = RSL.Registry_id
			where {$filter}";
		}
		else 
		{
			...
			Select 
				...
				isnull(R.Registry_CountIsBadVol, 0) as Registry_CountIsBadVol,
				...
				case when RegistryStatus_id = 4 then rhdcj.RegistryHealDepCheckJournal_AccRecSum else 0 end as Registry_SumPaid,
				...
				,RTrim(IsNull(convert(varchar,cast(R.Registry_orderDate as datetime),104),'')) as Registry_orderDate
			...
			from {$this->scheme}.{$source_table} R with (NOLOCK)
				left join v_Lpu l with (nolock) on l.Lpu_id = R.Lpu_oid
				left join r2.RegistryStateLog RSL with (nolock) on R.Registry_id = RSL.Registry_id
				left join v_DispClass DC with (NOLOCK) on DC.DispClass_id = R.DispClass_id
				left join v_PayType pt with (nolock) on pt.PayType_id = ISNULL(R.PayType_id, @PayType_id)
				...
				outer apply(select top 1 case when RE.Registry_id is not null then 1 else 0 end as RegistryDouble_IsData from {$this->scheme}.v_RegistryDouble RE with (NOLOCK) where RE.Registry_id = R.Registry_id) RegistryDouble
				outer apply(select top 1 case when RE.Registry_id is not null then 1 else 0 end as RegistryDataBadVol_IsData from {$this->scheme}.v_{$this->RegistryDataObject} RE with (NOLOCK) where RE.Registry_id = R.Registry_id and RE.RegistryData_IsBadVol = 2) RegistryDataBadVol
				left join v_Lpu lc with (nolock) on lc.Lpu_id = R.Lpu_cid
			where 
				{$filter}
			order by
	...
	function loadUnionRegistryErrorTFOMS($data)
	{
		...
		if (isset($data['Filter'])) {
			...
			if (is_array($filterData)) {
				foreach ($filterData as $column => $value) {
					...
					if ($r == "''") {
						$filter .= " and (rtrim(ltrim(" . $column . ")) is null or  rtrim(ltrim(" . $column . ")) ='')";
					} else {
		...
		$regData = $this->queryResult("select Registry_IsNotInsur, Registry_IsZNO, OrgSmo_id, convert(varchar(10), Registry_accDate, 120) as Registry_accDate from {$this->scheme}.v_Registry (nolock) where Registry_id = :Registry_id", array('Registry_id' => $data['Registry_id']));
		...
		if ($this->RegistryType_id == 19) {
			$fieldsList[] = "RD.MedPersonal_LabFIO";
			$fieldsList[] = "uc.UslugaComplex_Code";
			$fieldsList[] = "uc.UslugaComplex_Name";

			$joinList[] = "left join v_UslugaComplex as uc on uc.UslugaComplex_id = RD.UslugaComplex_id";
		}
		...
		$query = "
			select
				...
				ISNULL(cast(msc.MedSpecClass_Code as varchar) + '. ', '') + msc.MedSpecClass_Name as MedSpecOms_Name,
				RD.MedPersonal_Fio,
			...
			from
				...
				left join fed.v_MedSpecClass msc (nolock) on msc.MedSpecClass_id = rd.MedSpec_id
		...
		if (is_object($result)) {
			...
			if (!empty($resp[0]['Registry_id'])) {
				...
				if ( $resp[0]['Registry_IsZNO'] == 2 ) {
					...
					$filter_rd .= " and ISNULL(RD.RegistryData_IsZNO, 1) = 1 ";
				}
				...
				$query = "
					...
					from
						{$this->scheme}.v_RegistryGroupLink rgl (nolock)
						inner join {$this->scheme}.v_Registry rf (nolock) on rf.Registry_id = rgl.Registry_pid
						inner join {$this->scheme}.v_{$this->RegistryDataObject} rd (nolock) on rgl.Registry_id = rd.Registry_id {$filter_rd}
						left join v_OrgSmo os (nolock) on os.OrgSmo_id = rd.OrgSmo_id
					where
						rgl.Registry_pid = :Registry_id
						{$filter}
	...
	function loadUnionRegistry($data)
	{
		...
		else
		{
			...
			Select
				...
				,RTrim(IsNull(convert(varchar,cast(R.Registry_orderDate as datetime),104),'')) as Registry_orderDate
				,CASE WHEN R.Registry_IsNotInsur = 2 THEN 'true' ELSE 'false' END as Registry_IsNotInsurC
			from {$this->scheme}.v_RegistryUfa R with (NOLOCK)
				left join r2.RegistryStateLog RSL with (nolock) on R.Registry_id = RSL.Registry_id
				left join v_DispClass DC with (NOLOCK) on DC.DispClass_id = R.DispClass_id 
				outer apply(
					select top 1 RegistryQueue_id
	...
	*/
		...
		if (in_array($data['RegistryType_id'], $this->getAllowedRegistryTypes()))
		{
			if ($data['RegistrySubType_id'] == 3) {
				...
				$query = "
					...
					where
						RegistryType_id = :RegistryType_id
						and RegistrySubType_id = :RegistrySubType_id
						and Lpu_id = :Lpu_id	
						and YEAR(Registry_begDate) = YEAR(:Registry_begDate)
						and ISNULL(Lpu_oid,0) = ISNULL(:Lpu_oid,0)
			...
			$query = "
				...
				from
					{$this->scheme}.v_RegistryQueue (nolock)
				where
					ISNULL(Registry_id,0) = ISNULL(:Registry_id,0)
					and ISNULL(RegistryType_id,0) = ISNULL(:RegistryType_id,0)
					and ISNULL(RegistrySubType_id,0) = ISNULL(:RegistrySubType_id,0)
					...
					and ISNULL(Lpu_cid,0) = ISNULL(:Lpu_cid,0)
					and ISNULL(Registry_IsNotInsur,0) = ISNULL(:Registry_IsNotInsur,0)	
					and ISNULL(Registry_begDate,0) = ISNULL(:Registry_begDate,0)
					and ISNULL(Registry_endDate,0) = ISNULL(:Registry_endDate,0)
					and ISNULL(OrgSmo_id,0) = ISNULL(:OrgSmo_id,0)
					and ISNULL(Lpu_oid,0) = ISNULL(:Lpu_oid,0)
					and ISNULL(LpuUnitSet_id,0) = ISNULL(:LpuUnitSet_id,0)
					and ISNULL(Registry_Num,0) = ISNULL(:Registry_Num,0)
			";
	...
	function reformRegistry($data) 
	{
		$query = "
			...
			where
				r.Registry_id = ?
				and ISNULL(r.RegistrySubType_id, 1) <> 2
		";
	...
	function deleteRegistryError($data) 
	{
		...
		$query = "
			...
			exec {$this->scheme}.p_RegistryError_del
				@Registry_id = :Registry_id,
				@Evn_id = NULL,
	...
	function deleteRegistryErrorTFOMS($data) {
		...
		if ( !empty($data['RegistrySubType_id']) && $data['RegistrySubType_id'] == 2 ) {
			$regData = $this->queryResult("select Registry_IsNotInsur, OrgSmo_id, Registry_IsZNO, convert(varchar(10), Registry_accDate, 120) as Registry_accDate from {$this->scheme}.v_Registry (nolock) where Registry_id = :Registry_id", array('Registry_id' => $data['Registry_id']));
			...
			if ( $regData[0]['Registry_IsNotInsur'] == 2 ) {
				...
				$filter_rd = "and RD.Polis_id is not null";
				$filter = "and ISNULL(os.OrgSMO_RegNomC, '' ) = ''";
				$join = "left join v_OrgSmo os (nolock) on os.OrgSmo_id = rd.OrgSmo_id";
			}
			else {
				// @task https://redmine.swan.perm.ru//issues/109876
			...
			$mainQuery = "
				delete from {$this->scheme}.RegistryErrorTFOMS with (rowlock) where RegistryErrorTFOMS_id in (
					...
					from  
						...
						inner join {$this->scheme}.v_Registry R (nolock) on R.Registry_id = RGL.Registry_id
						inner join {$this->scheme}.v_RegistryErrorTFOMS RE with (nolock) on RE.Registry_id = R.Registry_id
						inner join {$this->scheme}.v_{$this->RegistryDataObject} RD (nolock) on RD.Registry_id = RE.Registry_id
	...
	function setErrorFromTFOMSImportRegistry($d, $data)
	{
		...
		if (!empty($params['IDCASE']) || !empty($params['SL_ID'])) {
			if ($data['Registry_id']>0)
			{
				...
				if (is_object($resp))
				{
					...
					if (is_array($ret) && (count($ret) > 0)) {
						...
						$query = "
							declare @Evn_id bigint, @Registry_id bigint, @ErrCode int,@ErrMessage varchar(4000);
								
							Select 
								@Evn_id = rd.Evn_id,
								@Registry_id = rd.Registry_id
							from
								{$from}
			...
			{
				$query = "
					declare @Evn_id bigint, @ErrCode int, @ErrMessage varchar(4000);
	...
	function loadRegistryErrorTFOMS($data)
	{
		...
		if ( in_array($this->RegistryType_id, array(7, 17)) ) {
		...
		if ( $this->RegistryType_id == 19 ) {
			$leftjoin .= " left join v_UslugaComplex as uc on uc.UslugaComplex_id = RD.UslugaComplex_id ";

			$addToSelect .= ",RD.MedPersonal_LabFIO";
			$addToSelect .= ",uc.UslugaComplex_Code";
			$addToSelect .= ",uc.UslugaComplex_Name";
		}
	...
	function setVizitNotInReg($data) 
	{
		...
		if ($data['Registry_id']>0)
		{
			...
			$query = "
				...
				exec {$this->scheme}.p_Registry_" . ($data['RegistrySubType_id'] == 2 ? "SMO_" : "") . "setPaid
					@Registry_id = :Registry_id,
					@pmUser_id = :pmUser_id,
					@Error_Code = @ErrCode output,
					@Error_Message = @ErrMessage output;
				...
				if (is_object($resp))
				{
					...
					if (is_array($ret) && (count($ret) > 0)) {
						...
						$query = "
							...
							exec {$this->scheme}.p_RegistryError_ins
	...
	function getRegistryFields($data) {
		...
		$query = "
			select 
				...
				ISNULL(convert(varchar(10), cast(Registry.Registry_insDT as datetime), 104), '') as Registry_insDT,
			...
			from {$this->scheme}.v_Registry Registry with (NOLOCK)
				...
				left join OrgRSchet ORS with(nolock) on Registry.OrgRSchet_mid = ORS.OrgRSchet_id
	...
	*/
	...
	function GetRegistryXmlExport($data) {
		...
		$query = "
			select
				...
				R.DispClass_id,
				pt.PayType_SysNick,
				ISNULL(R.RegistrySubType_id, 1) as RegistrySubType_id,
	...
	function getSmoListUfa(){
		...
		$query = "
			SELECT os.OrgSMO_id Smo_id,  
				RTRIM(LTRIM(REPLACE(REPLACE(o.Org_Nick, ' (РЕСПУБЛИКА БАШКОРТОСТАН)', ''), ' (РЕСПУБЛИКА БАШКОРТОСТАНА)', ''))) Smo_Nick, os.Orgsmo_f002smocod smo_code 
			FROM OrgSMO os with(nolock) 
			left join Org o with(nolock) on o.Org_id = os.Org_id
			WHERE os.OrgSmo_endDate is NULL
			--AND o.Org_OKATO LIKE '80%' 
			AND o.Org_Nick LIKE '%ашкорт%' 
			OR o.Org_Nick LIKE '%УФОМС%'
		...
		*/
		...
		$query = "
				 -- Поликлиника
					select 
					'polka' as name,
					(select top 1 [LIMIT]	 from r2.LIMIT_VOL_POLIK_PGG with(nolock) where CODE_MO = :Lpu_id and [YEAR] = :YEAR	) / 12 as limit,
					COUNT(RD.Evn_id) + 2100 as fact
					from r2.RegistryData RD with(nolock) 
					join r2.v_Registry R with(nolock) on RD.Registry_id = R.Registry_id 
					and not exists(select * from r2.RegistryError RE with(nolock) where RE.Evn_id = RD.Evn_id)
					join LpuUnitSet LP with(nolock) on LP.Lpu_id = R.Lpu_id and LP.LpuUnitSet_Code IN(1106925, 22151, 22159, 3006, 7019, 15016, 16149, 4113, 10023, 22158, 22117, 914, 134)
					where R.Registry_id IN (".implode(',', json_decode($data['Registry_ids'],1)).") group by(RD.NumCard)
				...
				--CMP    
					select 
					'smp' as name,
					(select top 1 [LIMIT]	 from r2.LIMIT_VOL_SMP_PGG with(nolock) where CODE_MO = :Lpu_id and [YEAR] = :YEAR) / 12 as limit,
					COUNT(RD.CmpCloseCard_id) + 5000 as fact
					from r2.RegistryDataCmp RD with(nolock) 
					join r2.v_Registry R with(nolock) on RD.Registry_id = R.Registry_id 
					and not exists(select * from r2.RegistryErrorCmp RE with(nolock) where RE.CmpCloseCard_id = RD.CmpCloseCard_id)
				...
				--Гемодиализ
					select
					'gemo' as name,
					 (select top 1 [LIMIT]	 from r2.LIMIT_VOL_GEMO_PGG with(nolock) where CODE_MO = :Lpu_id and [YEAR] = :YEAR) / 12 as limit,
					COUNT(RD.Evn_id)+12000 as fact
					from r2.RegistryData RD with(nolock) 
					join r2.Registry R with(nolock) on RD.Registry_id = R.Registry_id 
					and not exists(select * from r2.v_RegistryError RE with(nolock) where RE.Evn_id = RD.Evn_id)
					join LpuUnitSet LG with(nolock) on LG.Lpu_id = R.Lpu_id and LG.LpuUnitSet_Code IN(1106925, 22151, 22159, 3006, 7019, 15016, 16149, 4113, 10023, 22158, 22117, 914, 134)  
					where R.Registry_id IN (".implode(',', json_decode($data['Registry_ids'],1)).")
				...
				--Доп. исследования
					select
					'dopis' as name,
					(select top 1 [LIMIT]	 from r2.LIMIT_VOL_DOPIS_PGG with(nolock) where CODE_MO = :Lpu_id and [YEAR] = :YEAR) / 12 as limit,
					COUNT(RD.Evn_id) + 7000 as fact
					from r2.v_RegistryData RD with(nolock) 
					join r2.Registry R with(nolock) on RD.Registry_id = R.Registry_id 
					and not exists(select * from r2.v_RegistryError RE with(nolock) where RE.Evn_id = RD.Evn_id)
					join LpuUnitSet LD with(nolock) on LD.Lpu_id = R.Lpu_id and LD.LpuUnitSet_Code IN(120, 124, 112, 118, 116, 22137,  22139, 22141, 22145, 4009, 7007, 11009, 6005, 10033, 13006, 14062, 15015, 16039, 22133,  22135, 17003, 22170, 19035)  	
					where R.Registry_id IN (".implode(',', json_decode($data['Registry_ids'],1)).")
				...
				--Стационар
					select
					'stac' as name,
					(select top 1 [LIMIT]	 from r2.LIMIT_VOL_HOSP_PGG with(nolock) where CODE_MO = :Lpu_id and [YEAR] = :YEAR) / 12 as limit,
					COUNT(RD.Evn_id)+2400 as fact
					from r2.v_RegistryDataEvnPS RD with(nolock)
					join r2.Registry R with(nolock) on RD.Registry_id = R.Registry_id 
					and not exists(select * from r2.v_RegistryError RE with(nolock) where RE.Evn_id = RD.Evn_id)
					join dbo.LpuSection LS with(nolock) on RD.LpuSection_id = LS.LpuSection_id
					join dbo.LpuUnit LU with(nolock) on LU.LpuUnit_id = LS.LpuUnit_id
					where LU.LpuUnitType_id = 1 and R.Registry_id IN (".implode(',', json_decode($data['Registry_ids'],1)).")	
				...
				--Дневной стационар
					select
					'dstac' as name,
					(select top 1 [LIMIT]	 from r2.LIMIT_VOL_DHOSP_PGG with(nolock) where CODE_MO = :Lpu_id and [YEAR] = :YEAR) / 12 as limit,
					...
					join dbo.LpuSection LS with(nolock) on RD.LpuSection_id = LS.LpuSection_id
					join dbo.LpuUnit LU with(nolock) on LU.LpuUnit_id = LS.LpuUnit_id
					where LU.LpuUnitType_id IN(6,7,9) and R.Registry_id IN (".implode(',', json_decode($data['Registry_ids'],1)).")	  
	   ";
	...
	function cutVolumeRegisters($data){
		...
		$query = '
			select count(RD.Evn_id) as count from r2.v_RegistryData RD with(nolock) where cast(RD.Registry_id as varchar(10)) IN ('.implode(',',$Registry_ids).') 
			-------------------
			-- СНЯТЬ КОММЕНТ --
			-------------------
		...
		if ( is_object($res) ) {
				...
				if ( is_object($res) ) {
					...
					if(empty($temp)){
							...
							$query = "
								...
								Declare @Registry_ids varchar(max);
								
								Declare @Over_polka int;
								Declare @Over_smp int;
								Declare @Over_stac int;
								Declare @Over_dstac int;
								Declare @Over_dopis int;
								Declare @Over_gemo int;
								
								exec r2.p_cutVolumeRegistersUfa
			...
			if(!empty($Evn_ids)){
				$query = '
					select RD.Evn_id, RD.Registry_id, RD.RegistryType_id from r2.v_RegistryData RD with(nolock) where cast(RD.Evn_id as varchar(10)) in('.implode(',',$Evn_ids).');
				';
	...
	function setRegistryParamsByType($data = array(), $force = false) {
		...
		switch ( $this->RegistryType_id ) {
			...
			case 19:
				...
				$this->RegistryDataSLObject = null;
				$this->RegistryDataTempObject = 'RegistryDataParProf';
	...
	protected function _loadRegistrySCHETForXmlUsing($type, $data, $isFinal = false) {
		$queryModificator = ($isFinal === true && $type != 19 ? ", @OrgSmo_id = :OrgSmo_id, @Registry_IsNotInsur = :Registry_IsNotInsur" : "");
		$queryParams = array(
			'Registry_id' => $data['Registry_id'],
	...
	protected function _loadRegistryDataForXmlUsing($type, $data, $sluchDataFile, $personDataFile, $sluchDataTemplate, $personDataTemplate, $isFinal = false) {
		...
		// посещения
		$query = "
			exec {$p_vizit} @Registry_id = :Registry_id{$queryModificator}
		";
		$result = $this->db->query($query, $queryParams);
		...
		if ( !empty($p_usl) ) {
			$query = "
				exec {$p_usl} @Registry_id = :Registry_id{$queryModificator}
			";
			$result = $this->db->query($query, $queryParams);
		...
		if ( !empty($p_ds2) ) {
			$query = "
				exec {$p_ds2} @Registry_id = :Registry_id{$queryModificator}
			";
			$result = $this->db->query($query, $queryParams);
		...
		if ( !empty($p_ds3) ) {
			$query = "
				exec {$p_ds3} @Registry_id = :Registry_id{$queryModificator}
			";
			$result = $this->db->query($query, $queryParams);
		...
		if ( !empty($p_naz) ) {
			$query = "
				exec {$p_naz} @Registry_id = :Registry_id{$queryModificator}
			";
			$result = $this->db->query($query, $queryParams);
		...
		// люди
		$query = "
			exec {$p_pers} @Registry_id = :Registry_id{$queryModificator}
		";
		$result = $this->db->query($query, $queryParams);
	...
	protected function _loadRegistryDataForXmlUsingNew($type, $data, $sluchDataFile, $personDataFile, $sluchDataTemplate, $personDataTemplate, $isFinal = false) {
		...
		$query = "exec {$p_pers} @Registry_id = :Registry_id{$queryModificator}";
		...
		$query = "exec {$p_zsl} @Registry_id = :Registry_id{$queryModificator}";
		$this->textlog->add('Запуск ' . getDebugSQL($query, $queryParams));
		...
		$query = "exec {$p_sl} @Registry_id = :Registry_id{$queryModificator}";
		$this->textlog->add('Запуск ' . getDebugSQL($query, $queryParams));
		...
		$query = "exec {$p_usl} @Registry_id = :Registry_id{$queryModificator}";
		$this->textlog->add('Запуск ' . getDebugSQL($query, $queryParams));
		...
		// Диагнозы (DS2)
		if ( !empty($p_ds2) ) {
			$query = "exec {$p_ds2} @Registry_id = :Registry_id{$queryModificator}";
			$this->textlog->add('Запуск ' . getDebugSQL($query, $queryParams));
		...
		// Диагнозы (DS3)
		if ( !empty($p_ds3) ) {
			$query = "exec {$p_ds3} @Registry_id = :Registry_id{$queryModificator}";
			$this->textlog->add('Запуск ' . getDebugSQL($query, $queryParams));
		...
		// КСЛП (SL_KOEF)
		if ( !empty($p_kslp) ) {
			$query = "exec {$p_kslp} @Registry_id = :Registry_id{$queryModificator}";
			$this->textlog->add('Запуск ' . getDebugSQL($query, $queryParams));
		...
		// Назначения (NAZ)
		if ( !empty($p_naz) ) {
			$query = "exec {$p_naz} @Registry_id = :Registry_id{$queryModificator}";
			$this->textlog->add('Запуск ' . getDebugSQL($query, $queryParams));
	...
	protected function _loadRegistryDataForXmlByFunc($type, $data, $sluchDataFile, $personDataFile, $sluchDataTemplate, $personDataTemplate, $isFinal = false) {
		...
		if ( in_array($type, array(1, 2, 7, 9, 14, 17)) ) {
			if (in_array($data['PayType_SysNick'], array('bud', 'fbud')) && $type != 19) {
		...
		while ( $row = $resultUSL->_fetch_assoc() ) {
			...
			if (
				(
					($type == 2 && in_array($row['CODE_USL'], array('A08.30.006','A08.30.046.002', 'A08.30.046.003','A08.30.046.004','A08.30.046.005')))
					|| ($type == 14 && $row['CODE_USL'] == 'A16.12.004.009')
				)
				&& $row['KOL_USL'] > 1
			) {
				// Запоминаем количество услуг
		...
		$result = $this->db->query("
			set nocount on;
			select * into #zsl from {$fn_zsl} (:Registry_id{$execModificator});
			select * into #sl from {$fn_sl} (:Registry_id{$execModificator});
			select * into #pers from {$fn_pers} (:Registry_id{$execModificator});
			set nocount off;
	...
	public function setRegistryStatus($data) {
		...
		if ($RegistrySubType_id == 1 && ($PayType_SysNick != 'bud' || $RegistryType_id == 19) && $data['RegistryStatus_id']==2 && $RegistryStatus_id == 3) {
			...
			if ( in_array($RegistryType_id, array(1, 14)) ) {
				$query = "
					set nocount on;
					Declare @t Table (VolumeType_id bigint, KSG_code varchar(30), VolumeOverflowCount int);
					insert into @t exec {$this->scheme}.{$proc_volume}
						@Registry_id = :Registry_id,
						@debug = 0;
					set nocount off;
					...
					select
						vt.VolumeType_Code,
						t.VolumeOverflowCount,
						t.KSG_code
					from
						v_VolumeType vt (nolock)
						inner join @t t on t.VolumeType_id = vt.VolumeType_id
				";
			}
			else {
				$query = "
					set nocount on;
					Declare @t Table (VolumeType_id bigint, VolumeOverflowCount int);
					insert into @t exec {$this->scheme}.{$proc_volume}
						@Registry_id = :Registry_id,
						@debug = 0;
					set nocount off;
	...
	function saveUnionRegistry($data) {
		...
		if (count($resp) > 0 && !empty($resp[0]['Registry_id'])) {
			...
			$query = "
				declare @PayType_id bigint = :PayType_id;
				...
				where
					...
					and ISNULL(R.LpuUnitSet_id, 0) = ISNULL(:LpuUnitSet_id, 0)
					and ISNULL(R.Lpu_cid, 0) = ISNULL(:Lpu_cid, 0)
	...
	function loadUnionRegistryGrid($data)
	{
		$query = "
			Select
				...
				OS.OrgSmo_Name,
				R.Registry_Num,
				ISNULL(R.Registry_IsNotInsur, 1) as Registry_IsNotInsur,
				...
				isnull(R.Registry_RecordCount, 0) as Registry_Count,
				isnull(R.Registry_CountIsBadVol, 0) as Registry_CountIsBadVol,
				convert(varchar(10), R.Registry_updDT, 104) + ' ' + convert(varchar(8), R.Registry_updDT, 108) as Registry_updDT,
				R.Registry_pack as Registry_pack,
				DC.DispClass_id,
				DC.DispClass_Name,
				R.Lpu_cid,
				LC.Lpu_Nick as LpuContragent_Name,
				convert(varchar(10),R.Registry_orderDate,104) as Registry_orderDate
				-- end select
			from
				-- from
				{$this->scheme}.v_RegistryUfa R (nolock) -- объединённый реестр
			...
			where
				...
				and ISNULL(R.Registry_IsNew, 1) = ISNULL(:Registry_IsNew, 1)
				-- end where
			order by
				-- order by
				R.Registry_endDate DESC,
	...
	function setIsBadVol($data)
	{
		...
		$resp_reg = $this->queryResult("select Registry_id, RegistryStatus_id, RegistrySubType_id from {$this->scheme}.v_Registry (nolock) where Registry_id = :Registry_id", array('Registry_id' => $data['Registry_id']));
		...
		if ( in_array($this->RegistryType_id, array(2, 7, 9, 17)) && is_array($data['Evn_ids']) && count($data['Evn_ids']) > 0 ) {
			$Evn_ids = $this->queryResult("
				...
				where 
					...
					and rd.Evn_rid in (
						select Evn_rid
						from {$this->scheme}.v_{$this->RegistryDataObject} (nolock)
		...
		foreach($data['Evn_ids'] as $Evn_id) {
			...
			/*if ($this->RegistryType_id == 2) {
				$resp = $this->queryResult("
					...
					from
						{$this->scheme}.v_{$this->RegistryDataObject} rd (nolock)
						left join v_UslugaComplex uc (nolock) on uc.UslugaComplex_id = ISNULL(RD.Usluga_id, RD.UslugaComplex_id)
					where
						rd.Registry_id = :Registry_id
			...
			if ($this->RegistryType_id == 1) {
				...
				$resp = $this->queryResult("
					select
						rd.Evn_rid,
						rd.MaxEvn_id,
						rd.EvnSection_NumGroup
					from
						{$this->scheme}.{$this->RegistryDataObject} rd (nolock)
					where
						rd.Registry_id = :Registry_id
						and rd.Evn_id = :Evn_id
				", array(
		...
		if ( count($evnArray) > 0 ) {
			if ( $data['RegistryData_IsBadVol'] != 2 ) {
				if ( $this->RegistryType_id == 6 ) {
					...
					";
				} else if ( $this->RegistryType_id == 19 ) {
					$query = "
						select
							EvnUsluga_IsInReg as IsInReg
						from
							EvnUsluga (nolock)
						where
							Evn_id in (" . implode(',', $evnArray) . ")
			...
			if (!empty($this->RegistryDataSLObject)) {
				$slUpdate = "
					...
					from
						...
						inner join r2.{$this->RegistryDataObject} viz (nolock) on viz.Evn_rid = sl.Evn_rid and viz.Registry_id = sl.Registry_id
			...
			$query = "
				...
				begin try
					...
					update r2.{$this->RegistryDataObject} with (updlock)
					set {$this->RegistryDataObject}_IsBadVol = :RegistryData_IsBadVol,
					...
					if ( :RegistryType_id in (2,7,9,17) )
						begin
							update EvnVizit with (rowlock)
							set EvnVizit_IsInReg = :IsInReg
							where EvnVizit_id in (" . implode(',', $evnArray) . ")
						end

					if ( :RegistryType_id in (1,14) )
						begin
							update EvnSection with (rowlock)
							set EvnSection_IsInReg = :IsInReg
							where EvnSection_id in (" . implode(',', $evnArray) . ")
						end

					if ( :RegistryType_id in (6) )
						begin
							update dbo.CmpCloseCard with (rowlock)
							set CmpCloseCard_IsInReg = :IsInReg
							where CmpCloseCard_id in (" . implode(',', $evnArray) . ")
						end

					if ( :RegistryType_id in (19) )
						begin
							update dbo.EvnUsluga with (rowlock)
							set EvnUsluga_IsInReg = :IsInReg
							where EvnUsluga_id in (" . implode(',', $evnArray) . ")
						end
	...
	function deleteUnionRegistrys($data)
	{
		...
		$resp = $this->queryResult("
			select
				RGL.Registry_pid,
				RP.RegistryStatus_id,
				RGL.RegistryGroupLink_id
			from
				{$this->scheme}.v_Registry RP (nolock)
				inner join {$this->scheme}.v_RegistryGroupLink RGL (nolock) on RGL.Registry_pid = RP.Registry_id
			where
		...
		foreach($resp as $respone) {
			...
			$resp_rgl = $this->queryResult("
				select top 1 RegistryGroupLink_id from {$this->scheme}.v_RegistryGroupLink where Registry_pid = :Registry_pid
			", array(
			...
			if (is_numeric($OrgSmo_id)) {
				...
				if ($resp[0]['RegistryStatus_id'] == 2) {
					...
					if (!empty($data['Registry_IsZNO']) && $data['Registry_IsZNO'] == 2) {
						...
						$filter .= " and ISNULL(RF.Registry_IsZNO, 1) = 1";
					}
					...
					$resp_rf = $this->queryResult("
						select top 1
							...
							os.OrgSmo_Nick,
							rf.Registry_Num,
							convert(varchar(10), rf.Registry_accDate, 104) as Registry_accDate,
							rs.RegistryStatus_Name,
							rf.RegistryStatus_id
						from
							{$this->scheme}.v_RegistryGroupLink RGL (nolock)
							inner join {$this->scheme}.v_Registry RF (nolock) on RF.Registry_id = RGL.Registry_pid
							left join v_OrgSmo os (nolock) on os.OrgSmo_id = rf.OrgSmo_id
							left join v_RegistryStatus rs (nolock) on rs.RegistryStatus_id = rf.RegistryStatus_id
						where
							RGL.Registry_id = :Registry_id
							{$filter}
					", array(
	...
	function checkIncludeInUnioinRegistry($data)
	{
		...
		$resp_os = $this->queryResult("
			...
			from
				{$this->scheme}.v_Registry R (nolock)
				inner join {$this->scheme}.v_{$this->RegistryDataObject} RD (nolock) on RD.Registry_id = R.Registry_id and ISNULL(RD.RegistryData_IsBadVol, 1) = 1 -- только на случаи без превышения
				left join v_OrgSMO OS (nolock) on OS.OrgSmo_id = RD.OrgSmo_id
			where
				R.Registry_id = :Registry_id
				and not exists (
					...
					from
						{$this->scheme}.v_RegistryGroupLink RGL (nolock)
						inner join {$this->scheme}.v_Registry RP (nolock) on RP.Registry_id = RGL.Registry_pid
					where
						RGL.Registry_id = R.Registry_id
						and (
							RP.OrgSmo_id = RD.OrgSmo_id -- реестр по смо
							{$notStrah}
							OR (IsNull(os.OrgSMO_RegNomC,'')='' and RD.Polis_id is not null and RP.OrgSmo_id = 8) -- инотеры
						)
				)
			order by
				OrgSmo_Nick
	...
	function checkRegistryInGroupLink($data) {
		...
		$data['Registry_id'] = $this->getFirstResultFromQuery("SELECT Registry_pid FROM {$this->scheme}.v_RegistryGroupLink (nolock) WHERE Registry_id = :Registry_id", array('Registry_id'=>$data['Registry_id']));
	...
	function refreshRegistryVolumes($data)
	{
		$query = "
			...
			begin try
				exec {$this->scheme}.p_Registry_Volume_Refresh @Registry_id = :Registry_id;
			end try
	...
	public function exportRegistryToXml($data) {
		...
		try {
			...
			$ZGLV = array(
				array(
					'VERSION' => (in_array($data['PayType_SysNick'], array('bud', 'fbud')) && $type != 19) ? '1.0' : '3.2',
			...
			$SCHET[0]['VERSION'] = (in_array($data['PayType_SysNick'], array('bud', 'fbud')) && $type != 19) ? '1.0' : '3.1';
			...
			$this->setXSDSchema($type, $data['Registry_IsNew'], $data['Registry_IsZNO'], (in_array($data['PayType_SysNick'], array('bud', 'fbud')) && $type != 19));

			$H_registryValidate = true;
			$L_registryValidate = true;
			if ((!in_array($data['PayType_SysNick'], array('bud', 'fbud')) || $type == 19) && array_key_exists('check_implementFLK', $data['session']['setting']['server']) && $data['session']['setting']['server']['check_implementFLK'] && $res[0]) {
			...
			} else if (!in_array($data['PayType_SysNick'], array('bud', 'fbud')) && $type != 19) {
			...
			} else if ( in_array($type, array(2, 6, 7, 9, 17, 19)) ) {
    ...
    */
	...
	public function loadRegistryData($data) {
			...
			if (is_array($filterData)) {
				foreach ($filterData as $column => $value) {
					...
					elseif ($column == 'Mes_Code_KSG') {
						$column = 'Mes.Mes_Code + ISNULL(\' \' + Mes.MesOld_Num, \'\')';
					}
					elseif ($column == 'LpuSectionProfile_Name') {
					...
					// Костыль для фильтра Оплата = NULL
					if ($r == "''") {
						$filterList[] = "(rtrim(ltrim(" . $column . ")) is null or rtrim(ltrim(" . $column . ")) = '')";
					}
					else {
		...
		$joinList[] = "left join v_UslugaComplex U with (NOLOCK) on ISNULL(RD.Usluga_id, RD.UslugaComplex_id) = U.UslugaComplex_id -- связь стала с UslugaComplex_id (refs #13509)";
		$joinList[] = "left join v_Diag D with (NOLOCK) on RD.Diag_id =  D.Diag_id";
		$joinList[] = "left join v_EvnSection es (nolock) on ES.EvnSection_id = RD.Evn_id";
		$joinList[] = "left join v_MesOld m (nolock) on m.Mes_id = ES.Mes_id";
		$joinList[] = "left join v_LpuUnit LU (nolock) on LU.LpuUnit_id = LS.LpuUnit_id";
		$joinList[] = "left join v_LpuBuilding LB (nolock) on LB.LpuBuilding_id = LU.LpuBuilding_id";
		$joinList[] = "left join v_MesOld Mes with (nolock) on Mes.Mes_id = rd.MesItog_id";

		$fieldsList[] = "m.Mes_Code";
		$fieldsList[] = "U.UslugaComplex_Code as Usluga_Code";
		$fieldsList[] = "D.Diag_Code";
		$fieldsList[] = "case when RD.Paid_id = 1 then 'Нет' when RD.Paid_id = 2 then 'Да' else '' end as Paid";
		$fieldsList[] = "LB.LpuBuilding_Name";
		$fieldsList[] = "Mes.Mes_Code + ISNULL(' ' + Mes.MesOld_Num, '') as Mes_Code_KSG";
		$fieldsList[] = "RD.Mes_Code_KPG";

		if ( $this->RegistryDataObject == 'RegistryDataEvnPS' ) {
			$joinList[] = "left join v_HTMedicalCareClass htm (nolock) on htm.HTMedicalCareClass_id = rd.HTMedicalCareClass_id";
			$joinList[] = "left join v_VolumeType vt2 (nolock) on vt2.VolumeType_id = rd.VolumeType_sid";

			$fieldsList[] = "htm.HTMedicalCareClass_GroupCode";
			$fieldsList[] = "htm.HTMedicalCareClass_Name";
		...
		else if ( $this->RegistryDataObject == 'RegistryDataPar' ) {
			$joinList[] = "left join v_UslugaComplex as uc on uc.UslugaComplex_id = rd.UslugaComplex_id";
		...
		$joinList[] = "left join v_VolumeType vt (nolock) on vt.VolumeType_id = rd.VolumeType_id";
		$fieldsList[] = "vt.VolumeType_Code";

		if( !empty($data['RegistryData_IsBadVol']) && $data['RegistryData_IsBadVol'] == 2 ) {
			$filterList[] = "RD.RegistryData_IsBadVol = 2";
		}
		else {
			$filterList[] = "ISNULL(RD.RegistryData_IsBadVol, 1) = 1";
		}

		if ( $data['filterRecords'] == 2 ) {
			$filterList[] = "ISNULL(RD.Paid_id, 1) = 2";
		}
		else if ( $data['filterRecords'] == 3 ) {
			$filterList[] = "ISNULL(RD.Paid_id, 1) = 1";
		}
		...
		if ( !empty($data['OrgSmo_id']) ) {
			if ( $data['OrgSmo_id'] == 8 ) {
				$filterList[] = "IsNull(os.OrgSMO_RegNomC, '') = '' and RD.Polis_id IS NOT NULL";
			}
			else {
		...
		$joinList[] = "left join v_OrgSmo os with (nolock) on os.OrgSmo_id = RD.OrgSmo_id";
		$fieldsList[] = "
			case
				when OS.OrgSMO_RegNomC is not null then OS.OrgSmo_Nick
				when RD.Polis_id IS NOT NULL and IsNull(os.OrgSMO_RegNomC, '') = '' then 'Инотерриториальные'
		...
		";
		$fieldsList[] = "isnull(RD.RegistryData_Sum_R, 0) as RegistryData_Sum_R";
		$fieldsList[] = "ISNULL(RD.Paid_id,1) as RegistryData_IsPaid";
		...
		if (
			...
			if ( $Registry_IsNotInsur == 2 ) {
				...
				// инотеры
				$filter_rd = " and RD.Polis_id IS NOT NULL";
				$filterList[] = "IsNull(os.OrgSMO_RegNomC, '') = ''";
			}
			else {
				// @task https://redmine.swan.perm.ru//issues/109876
				if ( $Registry_accDate >= '2017-05-25' && $OrgSmo_id == 8000233 ) {
			...
			$from = "
				...
				inner join {$this->scheme}.v_{$this->RegistryDataObject} RD with (NOLOCK) on RD.Registry_id = RGL.Registry_id {$filter_rd}
			";
			...
			if ( $params['Registry_IsZNO'] == 2 ) {
				...
				$filterList[] = "ISNULL(RD.RegistryData_IsZNO, 1) = 1";
			}
		}
		...
		$query = "
			...
			where
				-- where
				R.Registry_id = :Registry_id
				and " . implode(PHP_EOL . 'and  ', $filterList) . "
				-- end where
			order by
				-- order by
				{$order}
	...
	public function loadRegistryEntiesSettings($data)
	{
		...
		$query = "
			...
			WHERE 
				...
				AND FLKSettings_EvnData LIKE '%ufa%'
		";
		$result = $this->db->query($query, $params);
		if (is_object($result))
		{
						...
						if ( ($disableEditPaid == 3 && empty($data['ignoreCheckRegistry'])) || $disableEditPaid == 2)  {
							...
							$query = "
								select top 1
									...
									convert(varchar(10), R.Registry_accDate, 104) as Registry_accDate
								from
									" . (!empty($scheme) ? $scheme . '.' : "") . "v_{$object} evn with (nolock)
									left join {$this->scheme}.v_{$this->RegistryDataObject} RD with (nolock) on RD.Evn_id = evn.{$object}_id
									left join {$this->scheme}.v_Registry R with (nolock) on R.Registry_id = RD.Registry_id
								where
	 ...
	 * Обнуление счетчика записей
	 */
	public function getLpuCidList($data = []) {
		return $this->queryResult("
			select
				LDC.Lpu_oid as Lpu_id,
				L.Lpu_Name,
				L.Lpu_Nick,
			...
			from v_LpuDispContract as LDC (nolock)
				inner join v_Lpu as L on L.Lpu_id = LDC.Lpu_oid
				inner join v_LpuSectionProfile as LSP on LSP.LpuSectionProfile_id = LDC.LpuSectionProfile_id
			where 
				LDC.Lpu_id = :Lpu_id
				and LSP.LpuSectionProfile_Code = '0070'
				and LDC.SideContractType_id = 2
 		", [
			'Lpu_id' => $data['Lpu_id']
		]);
	}
	 ...
	 * @return array|bool
	 */
	public function loadUnionRegistryErrorTFOMSFilter($data)
	{
		//Фильтр грида
		...
		$queryData = $this->loadUnionRegistryErrorTFOMS($data);
