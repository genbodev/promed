...
class FoodCook_model extends swModel {
	/**
	 * Конструктор
	 */
	...
	function loadFoodCookGrid($data) {
		...
		if ( !empty($data['FoodCook_Code']) ) {
			$filterList[] = "FC.FoodCook_Code like :FoodCook_Code";
			$params['FoodCook_Code'] = $data['FoodCook_Code'] . '%';
		}

		if ( !empty($data['FoodCook_Name']) ) {
			$filterList[] = "FC.FoodCook_Name like :FoodCook_Name";
		...
		$query = "
			select
				...
				FC.FoodCook_Protein,
				FC.FoodCook_Fat,
				FC.FoodCook_Carbohyd,
				FC.FoodCook_Time,
				FC.FoodCook_Mass
				-- end select
			from
				-- from
				v_FoodCook FC with (nolock)
				-- end from
			where
				-- where
	...
	function saveFoodCook($data) {
		...
		$query = "
			select top 1 FoodCook_id
			from v_FoodCook with (nolock)
		...
		$query = "
			...
			set @FoodCook_id = :FoodCook_id

			exec p_FoodCook_" . (!empty($data['FoodCook_id']) && $data['FoodCook_id'] > 0 ? "upd" : "ins") . "
				@FoodCook_id = @FoodCook_id output,
			...
			select @FoodCook_id as FoodCook_id, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
		";
	...
	function saveFoodCookSpec($data) {
		$query = "
			...
			set @Res = :FoodCookSpec_id;

			exec p_FoodCookSpec_" . (!empty($data['FoodCookSpec_id']) && $data['FoodCookSpec_id'] > 0 ? "upd" : "ins") . "
				@FoodCookSpec_id = @Res output,
	...
	function loadFoodCookSpecGrid($data) {
		$query = "
			select
				...
				fcs.FoodCookSpec_MassN,
				fcs.FoodCookSpec_MassB,
				fcs.Okei_nid,
				fcs.Okei_bid,
				fcs.FoodCookSpec_Time,
				fcs.FoodCookSpec_Descr
			from
				v_FoodCookSpec fcs with (nolock)
				left join v_FoodStuff fs with (nolock) on fs.FoodStuff_id = fcs.FoodStuff_id
			where
