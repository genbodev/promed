 ...
 * @version
 */
class RegisterListDetailLog_model extends swModel {
	...
	function load() {
		$q = "
			select
				RegisterListDetailLog_id, RegisterListDetailLog_setDT, RegisterListLogType_id, RegisterListDetailLog_Message, RegisterListLog_id
			from
				stg.v_RegisterListDetailLog with(nolock)
			where
	...
	function loadList($filter) {
		...
		SELECT
		-- select
				v_RegisterListDetailLog.RegisterListDetailLog_id, v_RegisterListDetailLog.RegisterListDetailLog_setDT, v_RegisterListDetailLog.RegisterListLogType_id, v_RegisterListDetailLog.RegisterListDetailLog_Message, v_RegisterListDetailLog.RegisterListLog_id
				,RegisterListLogType_id_ref.RegisterListLogType_Name RegisterListLogType_id_Name
		-- end select
		FROM
		-- from
				stg.v_RegisterListDetailLog WITH (NOLOCK)
				LEFT JOIN stg.v_RegisterListLogType RegisterListLogType_id_ref WITH (NOLOCK) ON RegisterListLogType_id_ref.RegisterListLogType_id = v_RegisterListDetailLog.RegisterListLogType_id
				LEFT JOIN stg.v_RegisterListLog RegisterListLog_id_ref WITH (NOLOCK) ON RegisterListLog_id_ref.RegisterListLog_id = v_RegisterListDetailLog.RegisterListLog_id
		-- end from
			$where_clause
		ORDER BY 
		-- order by
	...
	function save() {
		...
		$q = "
			declare
				@RegisterListDetailLog_id bigint,
				@ErrCode int,
				@ErrMessage varchar(4000);
			set @RegisterListDetailLog_id = :RegisterListDetailLog_id;
			exec stg." . $procedure . "
				@RegisterListDetailLog_id = @RegisterListDetailLog_id output,
			...
			select @RegisterListDetailLog_id as RegisterListDetailLog_id, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
		";
		$p = array(
	...
	function delete() {
		$q = "
			declare
				@ErrCode int,
				@ErrMessage varchar(4000);
			exec stg.p_RegisterListDetailLog_del
				@RegisterListDetailLog_id = :RegisterListDetailLog_id,
				@Error_Code = @ErrCode output,
				@Error_Message = @ErrMessage output;
