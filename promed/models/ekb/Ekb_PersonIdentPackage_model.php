...
class Ekb_PersonIdentPackage_model extends PersonIdentPackage_model {
	...
	function loadPersonIdentPackageGrid($data) {
		...
		if (!isSuperAdmin() && !empty($data['Lpu_id'])) {
			...
			$filters[] = "right(PIP.PersonIdentPackage_Name,15) like '/'+:mcode+'%'";
	...
	function loadPersonIdentPackagePosGrid($data) {
		...
		if (!empty($data['Person_FIO'])) {
			$filters[] = "PersonFIO.Value like :Person_FIO+'%'";
		...
		$query = "
			...
			with pos_list as (
				select *
				from v_PersonIdentPackagePos with(nolock)
			...
			select
				-- select
				PS.Person_id,
				convert(varchar(10), PIPP1.PersonIdentPackagePos_identDT, 104) as PersonIdentPackage_identDT,
				convert(varchar(10), PIPP2.PersonIdentPackagePos_identDT2, 104) as PersonIdentPackage_identDT2,
				PersonFIO.Value as Person_FIO,
				PIPPE.PersonIdentPackagePosErrorType_id,
				PIPPET.PersonIdentPackagePosErrorType_Code,
				PIPPET.PersonIdentPackagePosErrorType_Name,
				PIPPET.PersonIdentPackagePosErrorType_Decscription,
				PIS.PersonIdentState_id,
				PIS.PersonIdentState_Code,
				PIS.PersonIdentState_Name,
				PIPP2.Evn_id as EvnList,
				PIA.PersonIdentAlgorithm_Code,
				PIA.PersonIdentAlgorithm_Value,
				PIPP2.PersonIdentPackagePos_PolisNum,
				SMO.OrgSMO_Nick,
				PT.PolisType_Name,
				convert(varchar(10), PIPP2.PersonIdentPackagePos_recDate, 104) as PersonIdentPackagePos_recDate,
				convert(varchar(10), PIPP2.PersonIdentPackagePos_insurEndDate, 104) as PersonIdentPackagePos_insurEndDate,
				case 
					when len(PIPP2.PersonIdentPackagePos_Snils) = 11 
					then left(PIPP2.PersonIdentPackagePos_Snils,3)+'-'+substring(PIPP2.PersonIdentPackagePos_Snils,4,3)+'-'+substring(PIPP2.PersonIdentPackagePos_Snils,7,3)+' '+right(PIPP2.PersonIdentPackagePos_Snils,2)
					else PIPP2.PersonIdentPackagePos_Snils
				end as PersonIdentPackagePos_Snils,
				convert(varchar(10), PIPP2.PersonIdentPackagePos_BirthDay, 104) as PersonIdentPackagePos_BirthDay,
				SexP.Sex_Name
				-- end select
			from
				-- from
				...
				left join v_Evn E with(nolock) on E.Evn_id = PIPP2.Evn_id
				cross apply (
					select top 1
						PS.*
					from
				...
				left join v_PersonIdentAlgorithm PIA with(nolock) on PIA.PersonIdentAlgorithm_id = PIPP2.PersonIdentAlgorithm_id
				left join v_PersonIdentPackagePosError PIPPE with(nolock) on PIPPE.PersonIdentPackagePos_id = PIPP2.PersonIdentPackagePos_id
				left join v_PersonIdentPackagePosErrorType PIPPET with(nolock) on PIPPET.PersonIdentPackagePosErrorType_id = PIPPE.PersonIdentPackagePosErrorType_id
				left join v_PersonIdentState PIS with(nolock) on PIS.PersonIdentState_id = PIPP2.PersonIdentState_id
				left join v_PolisType PT with(nolock) on PT.PolisType_id = PIPP2.PolisType_id
				left join v_OrgSMO SMO with(nolock) on SMO.OrgSMO_id = PIPP2.PersonIdentPackagePos_Smo
				left join v_Sex SexP with(nolock) on SexP.Sex_id = PIPP2.PersonIdentPackagePos_Sex
				-- end from
			where
				-- where
		...
		if (count($result) > 0) {
			...
			from
				...
				inner join v_Evn E with(nolock) on E.Evn_id = PIPP.Evn_id
				left join v_EvnPLDisp EPLD with(nolock) on EPLD.EvnPLDisp_id = E.Evn_id
			where
	...
	function getDateTimeForPackageName($mcode, $date) {
		...
		$query = "
			if (not exists(
				...
				where PersonIdentPackage_Name like '%/'+:name+'.SCD'
			))
			begin
				select cast(:dt as datetime) as empty_dt
			end
			else
			begin
				with Package1 as (
					...
					from v_PersonIdentPackage with(nolock)
					where PersonIdentPackage_Name like '%/'+:mcode+'%.SCD'
				),
				Package as (
					select
					Date + cast(substring(Name, len(Name)-3, 2)+':'+substring(Name, len(Name)-1, 2) as datetime) as DT
					from Package1
				)
				select top 1 dateadd(minute, -1, p.DT) as empty_dt
				from Package p
				WHERE (SELECT 1 FROM Package t WHERE t.DT = dateadd(minute, -1, p.DT)) IS NULL
				order by p.DT desc
			end
		";
	...
	function addPersonIdentPackage($data) {
		...
		try {
			$mcode = $this->getFirstResultFromQuery("
				select top 1 Lpu_RegNomN2 from v_Lpu with(nolock) where Lpu_id = :Lpu_id
			...
			$getSelector = function($tmp_table = null) {
				$filter = "";
				if (!empty($tmp_table)) {
					$filter = "and not exists (select top 1 t.personId from {$tmp_table} t where t.personId = Person_id)";
				}
				return "
					select
						-- select
					...
					from
						-- from
						PersonIdentPackagePos with(nolock)
						-- end from
					where
						-- where
						isnull(PersonIdentPackage_id,0) = isnull(:tmpPersonIdentPackage_id,0)
			...
			while($queryCount($getSelector(), $countParams) > 0) {
				...
				$this->db->query("
					...
					IF OBJECT_ID(N'tempdb..{$tmp_person_table}', N'U') IS NOT NULL
					DROP TABLE {$tmp_person_table};
					create table {$tmp_person_table} (
						Person_id bigint primary key
					);
					
					IF OBJECT_ID(N'tempdb..{$tmp_pos_table}', N'U') IS NOT NULL
					DROP TABLE {$tmp_pos_table};
				...
				while (
					...
					$this->db->query("
						...
						declare @tmpPersonIdentPackage_id bigint = :tmpPersonIdentPackage_id;
						declare @currentSize int = :currentSize;
						delete {$tmp_pos_table};
						...
						where
							isnull(PIPP.PersonIdentPackage_id,0) = isnull(@tmpPersonIdentPackage_id,0)
					...
					$this->db->query("
						...
						from PersonIdentPackagePos PIPP with(rowlock)
					...
					$res = $this->db->query("
						...
						select
							...
							convert(varchar(10), PIPP1.PersonIdentPackagePos_identDT, 120) as date1,
							convert(varchar(10), PIPP2.PersonIdentPackagePos_identDT2, 120) as date2,
							...
							-- #111753
							case
								when DT.DocumentType_Code = 14 and len(D.Document_Ser) = 4 then LEFT(D.Document_Ser, 2) + ' ' + RIGHT(D.Document_Ser, 2)
								else D.Document_Ser
							end as docser,
							D.Document_Num as docnum,
							substring(PS.Person_Snils,1,3)+'-'+substring(PS.Person_Snils,4,3)+'-'+substring(PS.Person_Snils,7,3)+' '+right(PS.Person_Snils,2) as snils,
						...
						from
							...
							left join v_Evn E with(nolock) on E.Evn_id = PIPP2.Evn_id
							left join v_CmpCallCard CCC with(nolock) on CCC.CmpCallCard_id = PIPP2.CmpCallCard_id
							cross apply(
								select top 1 PS.*
					...
					$this->db->query("
						...
						set PersonIdentState_id = case when (
							(isnull(T.FAM,'') <> '' and isnull(T.IM,'') <> '' and isnull(T.OT,'') <> '' and T.DR is not null) 
					...
					$this->db->query("
						...
						from PersonIdentPackagePos PIPP with(rowlock)
							inner join {$tmp_pos_table} PIPP1 on PIPP1.PersonIdentPackagePos_id = PIPP.PersonIdentPackagePos_id
						where PIPP.PersonIdentState_id is null
					...
					$this->db->query("
						with stateList as (
							...
							from PersonIdentPackagePos PIPP with(nolock)
							inner join {$tmp_pos_table} PIPP1 on PIPP1.PersonIdentPackagePos_id = PIPP.PersonIdentPackagePos_id
						)
				...
				$query = " 
					...
					from
						-- from
						{$tmp_table} T
						inner join v_PersonIdentPackagePos PIPP with(nolock) on PIPP.PersonIdentPackagePos_id = T.id
						-- end from
					where 
						-- where
	...
	function importPersonIdentPackageResponse($data, $file, &$stat = array('RecAll' => 0, 'RecIdent' => 0, 'RecOk' => 0, 'RecErr' => 0, 'Errors' => array())) {
		...
		try {
			...
			$query = "
				...
				where PIP.PersonIdentPackage_Name like '%'+:package_name+'.SCD'
	...
	protected function _processResponseData($tmpTableName, $data, &$stat = array('RecAll' => 0, 'RecIdent' => 0, 'RecOk' => 0 , 'RecErr' => 0, 'Errors' => array())) {
		...
		$OMSSprTerr_id = $this->getFirstResultFromQuery("
			select top 1 OMSSprTerr_id from v_OMSSprTerr with(nolock) where KLRgn_id = :KLRgn_id
		", array('KLRgn_id' => 66));
		...
		$setIdentStateQuery = "
			declare
				@PersonIdentPackage_id bigint = :PersonIdentPackage_id,
			...
			set nocount on;
			begin try
				if @PersonIdentPackage_id is not null
				update PersonIdentPackagePos with(rowlock)
				set
		...
		$queryPersonData = "
			...
			-- from
				{$tmpTableName} pl
				left join Person P with(nolock) on P.Person_id = pl.Person_id
				left join v_Person_all PS with(nolock) on PS.PersonEvn_id = pl.PersonEvn_id and PS.Server_id = pl.Server_id
				left join v_Polis Polis with(nolock) on Polis.Polis_id = PS.Polis_id
			-- end from
			where
			-- where
		...
		$queryResponseData = "
			...
			-- select
				...
				PIPPE.PersonIdentPackagePosError_id,
				PIPPET.PersonIdentPackagePosErrorType_id,
				PIPPET.PersonIdentPackagePosErrorType_Name,
				pl.Person_id,
				pl.PersonEvn_id,
				pl.Server_id,
				...
				PIA.PersonIdentAlgorithm_id,
				PIA.PersonIdentAlgorithm_Code,
				{$OMSSprTerr_id} as OMSSprTerr_id,
				PT.PolisType_id,
				PFT.PolisFormType_id,
				SMO.OrgSMO_id,
				null as Polis_Ser,	--В файле нет серии
			...
			-- from
				{$tmpTableName} pl
				left join v_PolisType PT with(nolock) on PT.PolisType_CodeF008 = pl.VPOLIS
				left join v_PolisFormType PFT with(nolock) on PFT.PolisFormType_Code = pl.FPOLIS
				outer apply(
					select top 1 SMO.OrgSMO_id
					from v_OrgSMO SMO with(nolock)
					where SMO.Orgsmo_f002smocod like '66'+right('00'+cast(pl.SMO as varchar), 3)
				) SMO
				left join v_PersonIdentAlgorithm PIA with(nolock) on PIA.PersonIdentAlgorithm_Code = pl.ALG
				left join v_PersonIdentPackagePosErrorType PIPPET with(nolock) on PIPPET.PersonIdentPackagePosErrorType_Code = pl.ERRCODE
				left join v_PersonIdentPackagePosError PIPPE with(nolock) on PIPPE.PersonIdentPackagePos_id = pl.PersonIdentPackagePos_id
