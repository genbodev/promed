...
class Glossary_model extends swModel {
	var $scheme = "dbo";
		...
		foreach($synonym_list as $para){
			...
			$query = "
				Select
					...
					isnull(GlossaryTagType.GlossaryTagType_SysNick,'') as GlossaryTagType_SysNick,
					isnull(gl.Glossary_Descr,'') as Glossary_Descr,
					gl.Glossary_Word,
					gl.pmUser_did
				from
					v_Glossary gl with (NOLOCK)
					left join GlossaryTagType with (NOLOCK) on gl.GlossaryTagType_id = GlossaryTagType.GlossaryTagType_id
				where
	...
	*/
	function getGlossaryTagTypeBySysNick($data)
	{
		$query = "
			SELECT top 1 
				GlossaryTagType_id
				,GlossaryTagType_Code
				,GlossaryTagType_Name
				,GlossaryTagType_SysNick
			FROM
				GlossaryTagType with (NOLOCK)
			where
	...
	function checkDouble($data)
	{
		...
		$query = "
			Select
				...
				pmUser_did
			from
				v_Glossary with (NOLOCK)
			where
				Glossary_Word like :Glossary_Word
		...
		*/
		...
		$query = "
			...
			from
				-- from
				v_Glossary gl with (NOLOCK)
				left join v_GlossaryTagType gtt with(nolock) on gtt.GlossaryTagType_id = gl.GlossaryTagType_id
			-- end from
			where
				-- where
				(:Glossary_Word is null or gl.Glossary_Word like (:query)) and 
	...
	*/
	function saveRecord($data)
	{
		...
		else
		{
			...
			$syn_output = 'output';
			$syn_select = '@GlossarySynonym_id as GlossarySynonym_id,';
		}
		...
		$query = "
			declare
				...
				@GlossarySynonym_id bigint = :GlossarySynonym_id,
				@Glossary_id bigint = :Glossary_id;
				
			exec " .$proc. "
				@Glossary_id = @Glossary_id output, 
				@GlossarySynonym_id = @GlossarySynonym_id {$syn_output},
				...
				select @Glossary_id as Glossary_id, {$syn_select} @Error_Code as Error_Code, @Error_Message as Error_Msg;
		";
