<?php
class ModelGenerator_model extends swModel
{
	/**
	 * ModelGenerator_model constructor.
	 */
	...
	public function getTableInfo($db_tablename)
	{
		...
		$r = $this->db->query($q, array('table_name' => $db_tablename));
		$r = $r->result_array();
		$result = array();
		foreach ($r as $field) {
			$result[$field['column_name']] = $field;
		}
		$isEvn = ('0' !== $this->getFirstResultFromQuery('SELECT count(*) FROM EvnClass with(nolock) WHERE EvnClass_SysNick = :table_name', array('table_name' => $db_tablename)));
		if ($isEvn) {
			...
			foreach ($result as $field) {
				...
				if (empty($field['type_name'])) {
					...
					$result[$field['column_name']]['type_name'         ] = $inheritedFields[$field['column_name']]['type_name'         ];
					$result[$field['column_name']]['column_description'] = $inheritedFields[$field['column_name']]['column_description'];
					$result[$field['column_name']]['is_nullable'       ] = $inheritedFields[$field['column_name']]['is_nullable'       ];
					$result[$field['column_name']]['ref_table'         ] = $inheritedFields[$field['column_name']]['ref_table'         ];
					$result[$field['column_name']]['ref_col'           ] = $inheritedFields[$field['column_name']]['ref_col'           ];
					$result[$field['column_name']]['ref_name'          ] = $inheritedFields[$field['column_name']]['ref_name'          ];
					$result[$field['column_name']]['ref_schema'        ] = $inheritedFields[$field['column_name']]['ref_schema'        ];
				}
			}
		}
		$isMorbus = ('0' !== $this->getFirstResultFromQuery('SELECT count(*) FROM MorbusClass with(nolock) WHERE MorbusClass_SysNick = :table_name', array('table_name' => $db_tablename)));
		if ($isMorbus) {
