 ...
 * @version
 */
class CVIRegistry_model extends swModel {
	...
	public function processOpenRecords() {
		$query = "exec dbo.p_CVIRegistry_Refresh";
		try {
	...
	public function loadDiagList($params) {
		if (!empty($params['EvnPS_id'])) {
			$query = "
				select mdiag.Diag_Code
				from dbo.v_EvnPS (nolock) ps
				left join dbo.v_Diag (nolock) mdiag on mdiag.Diag_id = ps.Diag_id
				where ps.EvnPS_id = :EvnPS_id
				union all
				select sdiag.Diag_Code
				from dbo.v_EvnDiagPS (nolock) edps
				left join dbo.v_Diag (nolock) sdiag on sdiag.Diag_id = edps.Diag_id
	...
	public function loadContactedGrid($params) {
		$query = "select
				...
				pq.PersonQuarantine_approveDT,
				pc.PersonContactCVI_contactDate,
				lpu.Lpu_Nick
			from dbo.v_PersonContactCVI (nolock) pc
			inner join dbo.v_PersonState (nolock) ps on ps.Person_id = pc.Person_id
			inner join dbo.v_CVIRegistry (nolock) reg on reg.CVIRegistry_id = (select top 1 CVIRegistry_id
				from dbo.v_CVIRegistry (nolock)
				inner join dbo.v_PersonRegister (nolock) on v_PersonRegister.PersonRegister_id = v_CVIRegistry.PersonRegister_id
			...
			left join dbo.PersonQuarantine (nolock) pq on pq.PersonQuarantine_id = (select top 1 v_PersonQuarantine.PersonQuarantine_id
				from dbo.v_PersonQuarantine (nolock)
				where v_PersonQuarantine.Person_id = ps.Person_id and isnull(v_PersonQuarantine.PersonQuarantine_endDT, 1) = 1
				order by v_PersonQuarantine.PersonQuarantine_begDT desc
			)
			left join dbo.v_EvnPS (nolock) eps on eps.EvnPS_id = (select top 1 v_EvnPS.EvnPS_id
				from dbo.v_EvnPS (nolock)
				where v_EvnPS.Person_id = ps.Person_id and isnull(v_EvnPS.EvnPS_disDate, 1) = 1
				order by v_EvnPS.EvnPS_setDT desc
			)
			left join dbo.v_Lpu (nolock) lpu on lpu.Lpu_id = eps.Lpu_id
			left join dbo.v_Address (nolock) ua on ua.Address_id = ps.UAddress_id
			left join dbo.v_Address (nolock) pa on pa.Address_id = ps.PAddress_id
			left join dbo.v_PersonCardState (nolock) pcs on pcs.Person_id = pc.Person_id and pcs.LpuAttachType_id = 1
	...
	public function loadEvnPL($params) {
		...
		switch ($params['mode']) {
			case 'suspicion':
				$where .= " and isnull(reg.CVIRegistry_isConfirmed, 1) = 1";
				break;
			case 'confirmed':
				...
				$with = "
					,lastCVI as (
						select t.CVIRegistry_id from (
							...
							from dbo.v_CVIRegistry (nolock) cvireg
							inner join dbo.v_PersonRegister (nolock) register on register.PersonRegister_id = cvireg.PersonRegister_id
							where cvireg.CVIRegistry_isConfirmed = 2
						) as t
						where t.row = 1
					)
				";
				$where .= " and isnull(ps.Person_IsDead, 1) = 1";
				$where .= " and reg.CVIRegistry_isConfirmed = 2";
			...
			case 'notconfirmed':
				$where .= " and reg.CVIRegistry_disDT is not null and (
					(
						select count(CVIRegistry_id) from dbo.v_CVIRegistry (nolock)
						inner join dbo.v_PersonRegister (nolock) on v_PersonRegister.PersonRegister_id = v_CVIRegistry.PersonRegister_id
		...
		if (!empty($params['CVIRegistry_setDTRange'][0]) && !empty($params['CVIRegistry_setDTRange'][1])) {
			$where .= " and epl.EvnPL_setDate <= cast(:CVIRegistry_setDTEnd as datetime)";
			$where .= " and (epl.EvnPL_setDate is null or epl.EvnPL_setDate >= cast(:CVIRegistry_setDTBeg as datetime))";
		...
		if (!empty($params['CVIRegistry_disDTRange'][0]) && !empty($params['CVIRegistry_disDTRange'][1])) {
			$where .= " and (epl.EvnPL_disDate between cast(:CVIRegistry_disDTBeg as datetime) and cast(:CVIRegistry_disDTEnd as datetime))";
		...
		if (!empty($params['ControlCard_Type'])) {
			switch ($params['ControlCard_Type']) {
				case 1: $where .= " and exists (select PersonQuarantine_id from v_PersonQuarantine (nolock) where v_PersonQuarantine.Person_id = ps.Person_id and v_PersonQuarantine.PersonQuarantine_endDT is null)"; break;
				case 2: $where .= " and exists (select PersonQuarantine_id from v_PersonQuarantine (nolock) where v_PersonQuarantine.Person_id = ps.Person_id and v_PersonQuarantine.PersonQuarantine_endDT is not null)"; break;
				case 3: $where .= " and exists (select PersonQuarantine_id from v_PersonQuarantine (nolock) where v_PersonQuarantine.Person_id = ps.Person_id)"; break;
				case 4: $where .= " and not exists (select PersonQuarantine_id from v_PersonQuarantine (nolock) where v_PersonQuarantine.Person_id = ps.Person_id)"; break;
			}
		}
		if (!empty($params['ControlCard_OpenDateRange'][0]) && !empty($params['ControlCard_OpenDateRange'][1])) {
			$where .= " and exists (
				select PersonQuarantine_id from v_PersonQuarantine (nolock)
				where v_PersonQuarantine.Person_id = ps.Person_id
					and (
						v_PersonQuarantine.PersonQuarantine_endDT is null
						or v_PersonQuarantine.PersonQuarantine_endDT > cast(:ControlCard_OpenDateEnd as datetime)
					)
			)";
		...
		if (!empty($params['Diag_Code_From']) || !empty($params['Diag_Code_To'])) {
			...
			$where .= " and exists(
				select *
				from dbo.v_EvnDiagPLSop_all (nolock) edpls
				inner join v_Diag (nolock) d on d.Diag_id = edpls.Diag_id
		...
		$query = "with UslugaPar as (
				...
				from dbo.v_EvnUslugaPar (nolock)
				inner join dbo.v_UslugaComplex (nolock) uc on uc.UslugaComplex_id = v_EvnUslugaPar.UslugaComplex_id
			...
			select
				...
				(
					select substring(
						(
							select distinct ', ' + Diag.Diag_Code as 'data()'
							from dbo.v_EvnPL (nolock) pl
							inner join dbo.v_EvnVizitPL (nolock) vpl on vpl.EvnVizitPL_pid = pl.EvnPL_id
							inner join dbo.v_EvnDiagPLSop (nolock) DiagSop on DiagSop.EvnDiagPLSop_pid = vpl.EvnVizitPL_id and DiagSop.DiagSetClass_id <> 1
							inner join dbo.v_Diag (nolock) Diag on Diag.Diag_id = DiagSop.Diag_id
			...
			from (
				...
				from dbo.PersonRegister (nolock) pr
				inner join dbo.v_MorbusType (nolock) mt on mt.MorbusType_id = pr.MorbusType_id
				where mt.MorbusType_Code = 116
			) as pr
			inner join dbo.v_PersonState (nolock) ps on ps.Person_id = pr.Person_id
			inner join dbo.v_CVIRegistry (nolock) reg on reg.PersonRegister_id = pr.PersonRegister_id
			inner join dbo.v_EvnPL (nolock) epl on epl.EvnPL_id = reg.EvnPL_id
			left join dbo.v_Diag (nolock) epl_diag on epl_diag.Diag_id = epl.Diag_id
			left join dbo.v_ResultClass (nolock) epl_rc on epl_rc.ResultClass_id = epl.ResultClass_id
			left join v_PersonCardState (nolock) pc on pc.Person_id = ps.Person_id and pc.LpuAttachType_id = 1
			left join UslugaPar eup on eup.EvnUslugaPar_id = (select top 1 up.EvnUslugaPar_id from UslugaPar up where up.Person_id = ps.Person_id order by up.EvnUslugaPar_setDT desc)
			left join dbo.v_UslugaTest (nolock) ut on ut.UslugaTest_id = (select top 1 UslugaTest_id from dbo.v_UslugaTest (nolock) inner join dbo.v_EvnLabSample (nolock) els on els.EvnLabSample_id = v_UslugaTest.EvnLabSample_id where UslugaTest_ResultValue is not null and els.EvnLabSample_id = eup.EvnLabSample_id order by UslugaTest_setDT desc)
			where {$where}
		";
	...
	public function loadControlCard($params) {
		...
		if (!empty($params['CVIRegistry_setDTRange'][0]) && !empty($params['CVIRegistry_setDTRange'][1])) {
			$where .= " and
				(
					(
						ro.RepositoryObserv_arrivalDate <= cast(:CVIRegistry_setDTEnd as datetime)
						and (ro.RepositoryObserv_arrivalDate is null or ro.RepositoryObserv_arrivalDate >= cast(:CVIRegistry_setDTBeg as datetime))
					) or (
						ro.RepositoryObesrv_contactDate <= cast(:CVIRegistry_setDTEnd as datetime)
						and (ro.RepositoryObesrv_contactDate is null or ro.RepositoryObesrv_contactDate >= cast(:CVIRegistry_setDTBeg as datetime))
					)
				)";
			$params['CVIRegistry_setDTBeg'] = $params['CVIRegistry_setDTRange'][0];
			$params['CVIRegistry_setDTEnd'] = $params['CVIRegistry_setDTRange'][1];
		...
		if (!empty($params['CVIRegistry_disDTRange'][0]) && !empty($params['CVIRegistry_disDTRange'][1])) {
			$where .= " and (pq.PersonQuarantine_endDT between cast(:CVIRegistry_disDTBeg as datetime) and cast(:CVIRegistry_disDTEnd as datetime))";
			$params['CVIRegistry_disDTBeg'] = $params['CVIRegistry_disDTRange'][0];
			$params['CVIRegistry_disDTEnd'] = $params['CVIRegistry_disDTRange'][1];
		}
		if (!empty($params['ControlCard_Type'])) {
		...
		$query = "with FirstRepositoryObserv as (
				select * from (
					...
					from dbo.v_RepositoryObserv (nolock)
			...
			Quarantine as (
				select * from (
					...
					from dbo.v_PersonQuarantine (nolock) pq
					left join dbo.v_PersonQuarantineOpenReason (nolock) pqor on pqor.PersonQuarantineOpenReason_id = pq.PersonQuarantineOpenReason_id
					left join v_PersonCardState (nolock) pc on pc.Person_id = pq.Person_id and pc.LpuAttachType_id = 1
					where {$filter}
				) t where t.row = 1
			)
			...
			select
				...
				null as disDT
			from Quarantine pq
			inner join dbo.v_PersonState (nolock) ps on ps.Person_id = pq.Person_id
			left join dbo.MedStaffFactCache (nolock) msfc on msfc.MedStaffFact_id = pq.MedStaffFact_id
			...
			left join UslugaPar eup on eup.EvnUslugaPar_id = (select top 1 up.EvnUslugaPar_id from UslugaPar up where up.Person_id = ps.Person_id order by up.EvnUslugaPar_setDT desc)
			left join dbo.v_UslugaTest (nolock) ut on ut.UslugaTest_id = (select top 1 UslugaTest_id from dbo.v_UslugaTest (nolock) inner join dbo.v_EvnLabSample (nolock) els on els.EvnLabSample_id = v_UslugaTest.EvnLabSample_id where UslugaTest_ResultValue is not null and els.EvnLabSample_id = eup.EvnLabSample_id order by UslugaTest_setDT desc)
			left join dbo.v_PersonQuarantineCloseReason (nolock) pqcr on pqcr.PersonQuarantineCloseReason_id = pq.PersonQuarantineCloseReason_id
			where {$where}
		";
	...
	public function loadEvnPS($params) {
		...
		if (!empty($params['CVIRegistry_setDTRange'][0]) && !empty($params['CVIRegistry_setDTRange'][1])) {
			$where .= " and eps.EvnPS_setDate <= cast(:CVIRegistry_setDTEnd as datetime)";
			$where .= " and (eps.EvnPS_setDate is null or eps.EvnPS_setDate >= cast(:CVIRegistry_setDTBeg as datetime))";
			$params['CVIRegistry_setDTBeg'] = $params['CVIRegistry_setDTRange'][0];
			$params['CVIRegistry_setDTEnd'] = $params['CVIRegistry_setDTRange'][1];
		}
		if (!empty($params['CVIRegistry_disDTRange'][0]) && !empty($params['CVIRegistry_disDTRange'][1])) {
			$where .= " and (eps.EvnPS_disDate between cast(:CVIRegistry_disDTBeg as datetime) and cast(:CVIRegistry_disDTEnd as datetime))";
			$params['CVIRegistry_disDTBeg'] = $params['CVIRegistry_disDTRange'][0];
			$params['CVIRegistry_disDTEnd'] = $params['CVIRegistry_disDTRange'][1];
		}
		if (!empty($params['ControlCard_Type'])) {
		...
		if (!empty($params['Diag_Code_From']) || !empty($params['Diag_Code_To'])) {
			...
			$where .= " and exists(
				select *
				from dbo.v_EvnDiagPS (nolock) edps
				inner join dbo.v_Diag (nolock) edps_diag on edps_diag.Diag_id = edps.Diag_id
		...
		$query = "with Registry as (
			...
			UslugaPar as (
				...
				from dbo.v_EvnUslugaPar (nolock)
				inner join dbo.v_UslugaComplex (nolock) uc on uc.UslugaComplex_id = v_EvnUslugaPar.UslugaComplex_id
				where v_EvnUslugaPar.EvnUslugaPar_insDT > cast('2020-01-01' as datetime) and uc.UslugaComplex_Code in ('A26.05.066.002', 'A26.08.027.001', 'A26.08.046.002', 'A26.09.044.002', 'A26.09.060.002')
			){$with}
			...
			select
				...
				(
					select substring(
					   (
						   select ', ' + sd.Diag_Code as 'data()'
						   from dbo.v_EvnSection (nolock)
						   left join dbo.v_EvnDiagPS (nolock) on v_EvnDiagPS.EvnDiagPS_pid = v_EvnSection.EvnSection_id
						   left join dbo.v_Diag (nolock) sd on sd.Diag_id = v_EvnDiagPS.Diag_id
				...
				iif(eps_nutrition.EvnReanimatAction_id is not null, 'Зонд', 'Самостоятельно') as Nutrition,
				IIF(eps_ventilation.EvnReanimatAction_setDate is not null, str(datediff(hh, eps_ventilation.EvnReanimatAction_setDT, getdate())), 'Нет') as Ventilation,
				eup.EvnDirection_id as EvnDirection_id,
				ut.UslugaTest_ResultValue as UslugaTest_ResultValue,
			...
			inner join dbo.v_CVIRegistry (nolock) reg on reg.PersonRegister_id = pr.PersonRegister_id
			inner join dbo.v_PersonState (nolock) ps on ps.Person_id = pr.Person_id
			inner join dbo.v_EvnPS (nolock) eps on eps.EvnPS_id = reg.EvnPS_id
			left join v_PersonCardState (nolock) pc on pc.Person_id = ps.Person_id and pc.LpuAttachType_id = 1
			left join dbo.v_EvnSection (nolock) section on section.EvnSection_pid = eps.EvnPS_id and  section.EvnSection_Index = section.EvnSection_Count - 1
			left join dbo.v_LeaveType (nolock) eps_lt on eps_lt.LeaveType_id = section.LeaveType_id
			left join dbo.v_Diag (nolock) eps_diag on eps_diag.Diag_id = section.Diag_id
			left join dbo.v_Lpu_all (nolock) eps_lpu on eps_lpu.Lpu_id = section.Lpu_id
			left join dbo.v_EvnReanimatPeriod (nolock) eps_erp on eps_erp.EvnReanimatPeriod_pid = section.EvnSection_id and eps_erp.EvnReanimatPeriod_disDT is null
			...
			left join dbo.v_EvnReanimatAction (nolock) eps_nutrition on eps_nutrition.EvnReanimatAction_pid = eps_erp.EvnReanimatPeriod_id and eps_nutrition.ReanimatActionType_id = 3 and eps_nutrition.EvnReanimatAction_disDT is null and eps_nutrition.NutritiousType_id <> 4
			left join dbo.v_EvnReanimatAction (nolock) eps_ventilation on eps_ventilation.EvnReanimatAction_pid = eps_erp.EvnReanimatPeriod_id and eps_ventilation.ReanimatActionType_id = 1 and eps_ventilation.EvnReanimatAction_disDT is null
			left join UslugaPar eup on eup.EvnUslugaPar_id = (select top 1 up.EvnUslugaPar_id from UslugaPar up where up.Person_id = ps.Person_id order by up.EvnUslugaPar_setDT desc)
			left join dbo.v_UslugaTest (nolock) ut on ut.UslugaTest_id = (select top 1 UslugaTest_id from dbo.v_UslugaTest (nolock) inner join dbo.v_EvnLabSample (nolock) els on els.EvnLabSample_id = v_UslugaTest.EvnLabSample_id where UslugaTest_ResultValue is not null and els.EvnLabSample_id = eup.EvnLabSample_id order by UslugaTest_setDT desc)
			{$outerApply}
			where {$where}
	...
	public function loadResearch($params) {
		$query = "select
				...
				eup.EvnUslugaPar_id,
				eup_uc.UslugaComplex_id,
				eup_uc.UslugaComplex_Code,
				eup_uc.UslugaComplex_Name,
				ut.UslugaTest_ResultValue,
				ut.UslugaTest_ResultUnit
			from dbo.v_UslugaTest (nolock) ut
			inner join dbo.v_UslugaComplex (nolock) ut_uc on ut_uc.UslugaComplex_id = ut.UslugaComplex_id
			inner join dbo.v_EvnUslugaPar (nolock) eup on eup.EvnUslugaPar_id = ut.UslugaTest_pid
			inner join dbo.v_UslugaComplex (nolock) eup_uc on eup_uc.UslugaComplex_id = eup.UslugaComplex_id
	...
	public function loadDied($params) {
		...
		if (!empty($params['CVIRegistry_setDTRange'][0]) && !empty($params['CVIRegistry_setDTRange'][1])) {
			$where .= " and (
				(pr.PersonRegister_setDate is not null and pr.PersonRegister_setDate <= cast(:CVIRegistry_setDTEnd as datetime) and pr.PersonRegister_setDate >= cast(:CVIRegistry_setDTBeg as datetime) )
				or
				(ds.DeathSvid_DeathDate is not null and ds.DeathSvid_DeathDate <= cast(:CVIRegistry_setDTEnd as datetime) and ds.DeathSvid_DeathDate >= cast(:CVIRegistry_setDTBeg as datetime) )
			)";
			$params['CVIRegistry_setDTBeg'] = $params['CVIRegistry_setDTRange'][0];
			$params['CVIRegistry_setDTEnd'] = $params['CVIRegistry_setDTRange'][1];
		}
		if (!empty($params['CVIRegistry_disDTRange'][0]) && !empty($params['CVIRegistry_disDTRange'][1])) {
			$where .= " and (ds.DeathSvid_DeathDate between cast(:CVIRegistry_disDTBeg as datetime) and cast(:CVIRegistry_disDTEnd as datetime))";
			$params['CVIRegistry_disDTBeg'] = $params['CVIRegistry_disDTRange'][0];
			$params['CVIRegistry_disDTEnd'] = $params['CVIRegistry_disDTRange'][1];
		}
		if (!empty($params['LpuRegion_id']) && !empty($params['pmuser_id'])) {
		...
		$query = "with DeathSvid as (
				...
				from (
					...
					from dbo.v_DeathSvid (nolock) ds
					where DeathSvid_insDT > cast('2020-01-01' as datetime) and isnull(ds.DeathSvid_IsBad, 1) = 1 and ds.DeathSvid_IsActual = 2
				) as t
				left join dbo.v_Diag (nolock) idiag on idiag.Diag_id = t.Diag_iid
				left join dbo.v_Diag (nolock) tdiag on tdiag.Diag_id = t.Diag_tid
				left join dbo.v_Diag (nolock) mdiag on mdiag.Diag_id = t.Diag_mid
				left join dbo.v_Diag (nolock) ediag on ediag.Diag_id = t.Diag_eid
				left join dbo.v_Diag (nolock) odiag on odiag.Diag_id = t.Diag_oid
				where t.row = 1 and (
			...
			select
				...
				dbo.Age(ps.Person_BirthDay, getdate()) as Person_Age,
				concat(
					iif(ds.IDiag_Code is not null, ds.IDiag_Code + ' ', ''),
					iif(ds.TDiag_Code is not null, ds.TDiag_Code + ' ', ''),
					iif(ds.MDiag_Code is not null, ds.MDiag_Code + ' ', ''),
					iif(ds.EDiag_Code is not null, ds.EDiag_Code + ' ', ''),
					iif(ds.ODiag_Code is not null, ds.ODiag_Code + ' ', '')
				) as Diag_Code,
				'' as SopDiag,
				'' as TreatmentPlace,
				'' as Heft,
				...
				iif(cvireg.CVIRegistry_id is null, 2, 1) as IsRed
			from DeathSvid ds
			inner join dbo.v_PersonState (nolock) ps on ps.Person_id = ds.Person_id
			left join Reg pr on pr.Person_id = ps.Person_id
			left join CVIReg cvireg on cvireg.Person_id = ps.Person_id
			left join dbo.v_PersonCardState (nolock) pc on pc.Person_id = ps.Person_id and pc.LpuAttachType_id = 1
			left join UslugaPar eup on eup.EvnUslugaPar_id = (select top 1 up.EvnUslugaPar_id from UslugaPar up where up.Person_id = ps.Person_id order by up.EvnUslugaPar_setDT desc)
			left join dbo.v_UslugaTest (nolock) ut on ut.UslugaTest_id = (select top 1 UslugaTest_id from dbo.v_UslugaTest inner join dbo.v_EvnLabSample els on els.EvnLabSample_id = v_UslugaTest.EvnLabSample_id where UslugaTest_ResultValue is not null and els.EvnLabSample_id = eup.EvnLabSample_id order by UslugaTest_setDT desc)
			where {$where}
		";
	...
	public function findClosedCVI($params) {
		$query = "select top 1
			...
			reg.EvnPL_id,
			reg.EvnPS_id,
			reg.CVIRegistry_setDT,
			reg.CVIRegistry_disDT,
			pr.Person_id
			from dbo.v_CVIRegistry (nolock) reg
			inner join dbo.v_PersonRegister (nolock) pr on pr.PersonRegister_id = reg.PersonRegister_id
	...
	public function saveCVIEvent($params) {
		...
		if (!empty($sourceObjData) && $obj == 'EvnPL') {
			...
			if (in_array($sourceObjData['LeaveType_id'], ['22', '23'])) {
				$PersonRegisterOutCause_id = 3;
			} elseif ($sourceObjData['LeaveType_id'] == '24') {
