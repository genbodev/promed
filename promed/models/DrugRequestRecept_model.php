...
class DrugRequestRecept_model extends swModel {
	/**
	 * Конструктор
	 */
	...
	function load($data) {
		$q = "
			select
				drr.Server_id,
				drr.DrugRequestRecept_id,
				drr.DrugRequestPeriod_id,
				drr.DrugProtoMnn_id,
				drr.DrugRequestRecept_Kolvo,
				drr.DrugRequestRecept_KolvoRAS,
				drr.DrugRequestRecept_KolvoPurch,
				drr.DrugRequestRecept_KolvoDopPurch,
				dpm.DrugProtoMnn_Code,
				dpm.DrugProtoMnn_Name
			from
				dbo.v_DrugRequestRecept drr with (nolock)
				left join dbo.v_DrugProtoMnn dpm with (nolock) on dpm.DrugProtoMnn_id = drr.DrugProtoMnn_id
			where
	...
	function loadList($filter) {
		...
		$q = "
			...
			from
				dbo.v_DrugRequestRecept drr with (nolock)
				left join dbo.v_DrugRequestPeriod drp with (nolock) on drp.DrugRequestPeriod_id = drr.DrugRequestPeriod_id
				left join dbo.v_DrugProtoMnn dpm with (nolock) on dpm.DrugProtoMnn_id = drr.DrugProtoMnn_id
			$where_clause
		";
	...
	function loadDrugRequestReceptConsolidatedList($filter) {
		...
		$q = "
			...
			from
				...
				left join dbo.v_DrugRequestPeriod drp with (nolock) on drp.DrugRequestPeriod_id = drr.DrugRequestPeriod_id
				left join dbo.v_DrugProtoMnn dpm with (nolock) on dpm.DrugProtoMnn_id = drr.DrugProtoMnn_id
				left join dbo.v_ReceptFinance rf with (nolock) on rf.ReceptFinance_id = dpm.ReceptFinance_id
			where
				datepart(year, drp.DrugRequestPeriod_begDate) = :Year;
		";
		$result = $this->db->query($q, $filter);
	...
	function save($data) {
		...
		$q = "
			declare
				@DrugRequestRecept_id bigint,
				@ErrCode int,
				@ErrMessage varchar(4000);
			set @DrugRequestRecept_id = :DrugRequestRecept_id;
			exec dbo." . $procedure . "
				@Server_id = :Server_id,
				@DrugRequestRecept_id = @DrugRequestRecept_id output,
			...
			select @DrugRequestRecept_id as DrugRequestRecept_id, @ErrCode as Error_Code, @ErrMessage as Error_Msg;
		";
				...
				if ($data['DrugRequestRecept_id'] <= 0) { //при добавлении необходимо сгенерировать разнарядку
					$q = "
						declare
							@ErrCode int,
							@ErrMessage varchar(4000);
						exec dbo.p_DrugRequestReceptLpu_import
							@DrugRequestPeriod_id = :DrugRequestPeriod_id,
							@DrugProtoMnn_id = :DrugProtoMnn_id,
							@pmUser_id = :pmUser_id,
							@Error_Code = @ErrCode output,
					...
					$q = "
						declare
							@ErrCode int,
							@ErrMessage varchar(4000);
						exec dbo.p_DrugRequestReceptLpu_recount
							@DrugRequestPeriod_id = :DrugRequestPeriod_id,
							@DrugProtoMnn_id = :DrugProtoMnn_id,
							@pmUser_id = :pmUser_id,
							@Error_Code = @ErrCode output,
	...
	function delete() {
		$q = "
			declare
				@ErrCode int,
				@ErrMessage varchar(4000);
			exec dbo.p_DrugRequestRecept_del
				@DrugRequestRecept_id = :DrugRequestRecept_id,
				@Error_Code = @ErrCode output,
				@Error_Message = @ErrMessage output;
			select @ErrCode as Error_Code, @ErrMessage as Error_Msg;
		...
		if (count($id_array) == 2 && $id_array[0] > 0 && $id_array[1] > 0) {
			$q = "
				select
					drr.DrugRequestRecept_id
				from
					v_DrugRequestRecept drr with (nolock)
			...
			if ( is_object($r) ) {
				...
				foreach($request_arr as $request) {
					...
					$q = "
						select
							DrugRequestReceptLpu_id
						from
							v_DrugRequestReceptLpu with (nolock)
						where
							DrugRequestRecept_id = :DrugRequestRecept_id
					";
					$r = $this->db->query($q, array(
						...
						foreach($lpu_request_arr as $lpu_request) {
							...
							$q = "
								declare
									@ErrCode int,
									@ErrMessage varchar(4000);
								exec dbo.p_DrugRequestReceptLpu_del
	...
	function getDrugRequestReceptTotalKolvo($data) {
		...
		if ($data['Drug_id'] > 0 || $data['DrugMnn_id'] > 0) {
				...
				$where[] = "
					drr.DrugProtoMnn_id in (
						...
						from
							...
							left join DrugProto dpr with (nolock) on ds.DrugProto_id=dpr.DrugProto_id
							inner join drp with (nolock) on drp.DrugRequestPeriod_id=dpr.DrugRequestPeriod_id
							left join Drug d with (nolock) on d.Drug_id=ds.Drug_id
						where
							ds.Drug_id = :Drug_id
					)
				";
			...
			$q = "
				...
				select
					sum(case
						when drrl.DrugRequestReceptLpu_KolvoAcc > isnull(drrl.DrugRequestReceptLpu_KolvoRec,0) then drrl.DrugRequestReceptLpu_KolvoAcc - isnull(drrl.DrugRequestReceptLpu_KolvoRec,0)
						else 0
					end) as DrugRequestRecept_TotalKolvo
				from
					v_DrugRequestRecept drr with (nolock)
					inner join drp with(nolock) on drp.DrugRequestPeriod_id = drr.DrugRequestPeriod_id
					left join v_DrugProtoMnn dpm with (nolock) on dpm.DrugProtoMnn_id = drr.DrugProtoMnn_id
					left join v_DrugRequestReceptLpu drrl with (nolock) on drrl.DrugRequestRecept_id = drr.DrugRequestRecept_id
				{$where};
			";
