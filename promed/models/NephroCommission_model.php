...
class NephroCommission_model extends swModel
{
	/**
	 * Конструктор
	 */
	...
	function loadViewData($data)
	{
		...
		$query = "
				select $isOnlyLast
					...
					NC.NephroCommission_id,
					convert(varchar,NC.NephroCommission_date,104) as NephroCommission_date,
					NC.NephroCommission_protocolNumber,
					:MorbusNephro_id as MorbusNephro_id,
					:MorbusNephro_pid as MorbusNephro_pid
				from r2.v_NephroCommission NC with(nolock)
				left join v_MorbusNephro MN with(nolock) on MN.MorbusNephro_id = NC.MorbusNephro_id
	...
	function doLoadEditFormNephroCommission($data) {
		...
		$query = "
			select 
				NephroCommission_id,
				MorbusNephro_id,
				convert(varchar,NephroCommission_date,104) as NephroCommission_date,
				NephroCommission_protocolNumber
			from 
				r2.v_NephroCommission with(nolock)
			where
