...
class ElectronicDigitalSign_model extends swModel {
	/**
	 * Конструктор
	 */
	...
	function loadDocumentVersionList($data = array()) {
		...
		if (count($userids) > 0) {
			...
			$query = "
				SELECT pmUser_id, pmUser_Name FROM v_pmUserCache (nolock) WHERE pmUser_id IN ({$userids})
			";
		...
		$res[] = array(
			'Doc_id' => '1_1',
			'Doc_Version' => 1,
			'Doc_DateTime' => date('d.m.Y H:i:s', 1382873150)
		);
		$res[] = array(
			'Doc_id' => '1_2',
			'Doc_Version' => 2,
			'Doc_DateTime' => date('d.m.Y H:i:s')
		);
		*/
		return $res;
	}
