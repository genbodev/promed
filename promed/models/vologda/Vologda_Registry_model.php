...
class Vologda_Registry_model extends Registry_model {
	...
	public function loadRegistry($data) {
		...
		$IsFapField = "case when R.Registry_IsFAP = 2 then 'true' else 'false' end as Registry_IsFAP,";
		...
		if ( !empty($data['Registry_id']) ) {
			...
			$IsRepeatedField = "ISNULL(R.Registry_IsRepeated, 1) as Registry_IsRepeated,";
			$IsPersFinField = "ISNULL(R.Registry_isPersFin, 1) as Registry_isPersFin,";
			$IsFapField = "ISNULL(R.Registry_IsFAP, 1) as Registry_IsFAP,";
		}
	...
	public function loadUnionRegistryErrorTFOMS($data) {
		...
		$regData = $this->queryResult("select top 1
					r.Org_did,
					pt.PayType_SysNick,
					kn.KatNasel_SysNick
		...
		if ( $regData[0]['PayType_SysNick'] == 'oms' ) {
			...
			if ( $regData[0]['KatNasel_SysNick'] == 'oblast' ) {
				...
				$filter .= " and ISNULL(ost.KLRgn_id, 0) <> 35";
			}
		}
		...
		$query = "
			...
			from
				...
				inner join {$this->scheme}.v_{$this->RegistryDataObject} RD (nolock) on RD.Registry_id = RE.Registry_id and RD.Evn_id = RE.Evn_id
			...
			where
				...
				and ISNULL(RE.RegistryErrorTFOMSType_id, 0) != 2
	...
	public function deleteUnionRegistry($data = []) {
		...
		$this->db->query("
			...
			begin try
				delete from {$this->scheme}.RegistryGroupLink with (ROWLOCK) where Registry_pid = :Registry_id
			end try
	...
	public function deleteUnionRegistryWithData($data) {
		...
		$query = "
			...
			from
				{$this->scheme}.v_RegistryGroupLink rgl (nolock)
				inner join {$this->scheme}.v_Registry rf (nolock) on rf.Registry_id = rgl.Registry_pid
				inner join {$this->scheme}.{$this->RegistryDataObject} rd (nolock) on rgl.Registry_id = rd.Registry_id
				inner join v_OmsSprTerr ost with (nolock) on ost.OmsSprTerr_id = rd.OmsSprTerr_id
				left join v_OrgSmo os (nolock) on os.OrgSmo_id = rd.OrgSmo_id
			where
				rgl.Registry_pid = :Registry_id
				{$where}
		...
		if ((isset($data['RegistryStatus_id'])) && ($data['RegistryStatus_id']==5)) {
			$query = "
				Select
					...
					R.Lpu_id,
					R.Registry_IsNotInsur,
					ISNULL(R.Registry_isPersFin, 1) as Registry_isPersFin,
		...
		if ( in_array($data['RegistryType_id'], $this->getAllowedRegistryTypes()) ) {
			$query = "
				...
				where
					...
					and ISNULL(Registry_isPersFin,0) = ISNULL(:Registry_isPersFin,0)	
	...
	public function reformRegistry($data) {
		$row = $this->getFirstRowFromQuery("
			select
				...
				r.Registry_IsNotInsur,
				r.Registry_IsZNO,
				ISNULL(r.Registry_IsFAP, 1) as Registry_IsFAP,
	...
	public function deleteRegistryErrorTFOMS($data) {
		...
		if ( !empty($data['RegistrySubType_id']) && $data['RegistrySubType_id'] == 2 ) {
			...
			$mainQuery = "
				delete from {$this->scheme}.RegistryErrorTFOMS with (rowlock) where Registry_id = @Registry_id and RegistryErrorTFOMSType_id = :RegistryErrorTFOMSType_id;
			";
		}
	...
	public function setErrorFromSMOImportRegistry($d, $data) {
		...
		$RegistryErrorType_id = $this->getFirstResultFromQuery("SELECT TOP 1 RegistryErrorType_id FROM {$this->scheme}.RegistryErrorType with (nolock) WHERE RegistryErrorType_Code = :S_OSN", [
			'S_OSN' => $params['S_OSN']
		]);
	...
	function loadRegistryErrorTFOMS($data)
	{
		...
		$query = "
			Select
				...
				ps.Server_id,
				convert(varchar(10), ps.Person_BirthDay, 104) as Person_BirthDay,
				LTRIM(RTRIM(ISNULL(RD.Polis_Ser, '') + ' ' + ISNULL(RD.Polis_Num, ''))) as Person_Polis,
	...
	public function loadRegistryErrorBDZ($data) {
		...
		$query = "
			...
			from
				-- from
				{$this->scheme}.v_RegistryErrorTFOMS re with (nolock)
				left join {$this->scheme}.v_{$this->RegistryDataObject} rd with (nolock) on rd.Registry_id = re.Registry_id
					and rd.Evn_id = re.Evn_id
				left join v_Person_bdz ps with (nolock) on ps.PersonEvn_id = rd.PersonEvn_id and ps.Server_id = rd.Server_id
	...
	public function setErrorFromImportRegistry($d, $data) {
		...
		$RegistryErrorType_id = $this->getFirstResultFromQuery("SELECT TOP 1 RegistryErrorType_id FROM {$this->scheme}.RegistryErrorType with (nolock) WHERE RegistryErrorType_Code = :OSHIB", [
			'OSHIB' => $params['OSHIB']
		]);
	...
	protected function _loadRegistryDataForXmlByFunc($type, $data, $sluchDataFile, $personDataFile, $sluchDataTemplate, $personDataTemplate, $isFinal = false) {
		...
		if ( !empty($p_cons) ) {
			$query = "exec {$p_cons} @Registry_id = :Registry_id{$queryModificator}";
		...
		if ( !empty($p_lek_pr) ) {
			$query = "exec {$p_lek_pr} @Registry_id = :Registry_id{$queryModificator}";
			$this->textlog->add('Запуск ' . getDebugSQL($query, $queryParams));
			$queryResult = $this->db->query($query, $queryParams);
			$this->textlog->add('Выполнено');
			if ( !is_object($queryResult) ) {
		...
		if ( !empty($p_bdiag) ) {
			$query = "exec {$p_bdiag} @Registry_id = :Registry_id{$queryModificator}";
			$this->textlog->add('Запуск ' . getDebugSQL($query, $queryParams));
			$queryResult = $this->db->query($query, $queryParams);
			$this->textlog->add('Выполнено');
			if ( !is_object($queryResult) ) {
		...
		if ( !empty($p_bprot) ) {
			$query = "exec {$p_bprot} @Registry_id = :Registry_id{$queryModificator}";
			$this->textlog->add('Запуск ' . getDebugSQL($query, $queryParams));
			$queryResult = $this->db->query($query, $queryParams);
			$this->textlog->add('Выполнено');
			if ( !is_object($queryResult) ) {
		...
		// Направления (NAPR)
		if ( !empty($p_napr) ) {
			$query = "exec {$p_napr} @Registry_id = :Registry_id{$queryModificator}";
			$this->textlog->add('Запуск ' . getDebugSQL($query, $queryParams));
			$queryResult = $this->db->query($query, $queryParams);
			$this->textlog->add('Выполнено');
			if ( !is_object($queryResult) ) {
		...
		if ( !empty($p_onkousl) ) {
			$query = "exec {$p_onkousl} @Registry_id = :Registry_id{$queryModificator}";
			$this->textlog->add('Запуск ' . getDebugSQL($query, $queryParams));
			$queryResult = $this->db->query($query, $queryParams);
			$this->textlog->add('Выполнено');
			if ( !is_object($queryResult) ) {
		...
		$result = $this->db->query("
			set nocount on;
			select * into #zsl from {$fn_zsl} (:Registry_id{$fnQueryModificator});
			select * into #sl from {$fn_sl} (:Registry_id{$fnQueryModificator});
			select * into #pers from {$fn_pers} (:Registry_id{$fnQueryModificator});
			set nocount off;
	...
	public function saveUnionRegistry($data) {
		...
		if (!empty($data['KatNasel_id'])) {
			 ...
			 */
			$data['KatNasel_Code'] = $this->getFirstResultFromQuery("select top 1 KatNasel_Code from v_KatNasel (nolock) where KatNasel_id = :KatNasel_id", $data);
		...
		$query = "
			declare
				@KatNasel_Code bigint = :KatNasel_Code,
				@Error_Code bigint,
				@Error_Message varchar(4000),
				@Registry_id bigint = :Registry_id,
				@curdate datetime = dbo.tzGetDate();
		...
		if (count($resp) > 0 && !empty($resp[0]['Registry_id'])) {
			...
			if($resp[0]['KatNasel_Code'] != 2){
				$isPersFin = " and ISNULL(R.Registry_isPersFin, 1) = ISNULL(:Registry_isPersFin, 1)";
			}
			...
			$query = "
				...
				where
					...
					{$isPersFin}
					and ISNULL(R.PayType_id, 1) = ISNULL(:PayType_id, 1)
			...
			if ( $PayType_SysNick == 'oms' ) {
				...
				if ( $KatNasel_SysNick == 'oblast' ) {
					...
					$filterList[] = "ISNULL(ost.KLRgn_id, 0) <> 35";
				}
			}
			...
			$updResult = $this->getFirstRowFromQuery("
				...
				begin try
					-- Сперва обнуляем
					update {$this->RegistryEvnClass} with (rowlock)
					...
					from
						{$this->scheme}.v_Registry R with (NOLOCK)
						inner join {$this->scheme}.v_RegistryGroupLink RGL with (nolock) on RGL.Registry_pid = R.Registry_id
						inner join {$this->scheme}.v_{$this->RegistryDataObject} RD with (NOLOCK) on RD.Registry_id = RGL.Registry_id
						left join v_OmsSprTerr ost with (nolock) on ost.OmsSprTerr_id = rd.OmsSprTerr_id
					where
						R.Registry_id = :Registry_id
	...
	public function loadUnionRegistryGrid($data) {
		...
		$query = "
			...
			from
				...
				left join v_KatNasel KN with (nolock) on KN.KatNasel_id = R.KatNasel_id
				outer apply (
					select top 1 1 as RegistryErrorTFOMS_IsData
					from {$this->scheme}.v_RegistryGroupLink RGL with (nolock)
						inner join {$this->scheme}.v_RegistryErrorTFOMS RE with (nolock) on RE.Registry_id = RGL.Registry_id
	...
	function recountSumKolRegistry($data) {
		...
		if (!empty($resp[0]['Registry_id'])) {
			...
			$this->db->query("
				update {$this->scheme}.Registry with (rowlock) set Registry_SumR = :Registry_SumR, Registry_Sum = :Registry_Sum, Registry_RecordCount = :Registry_RecordCount where Registry_id = :Registry_id
			", array(
				'Registry_id' => $data['Registry_id'],
				'Registry_SumR' => !empty($resp_sum[0]['Sum'])?$resp_sum[0]['Sum']:0,
				'Registry_Sum' => !empty($resp_sum[0]['Sum'])?$resp_sum[0]['Sum']:0,
		...
		foreach ( $data['Registry_ids'] as $Registry_id ) {
			...
			if ( $data['RegistryStatus_id'] == 4 ) { // если переводим в оплаченные, то вызываем p_Registry_SMO_setPaid
				$result = $this->getFirstRowFromQuery("
					...
					exec {$this->scheme}.p_Registry_SMO_setPaid
						@Registry_id = :Registry_id,
						@pmUser_id = :pmUser_id,
						@Error_Code = @Error_Code output,
						@Error_Message = @Error_Message output;
			...
			", $data);
				...
				$result = $this->getFirstRowFromQuery("
					...
					exec {$this->scheme}.p_Registry_SMO_setUnPaid
						@Registry_id = :Registry_id,
						@pmUser_id = :pmUser_id,
						@Error_Code = @Error_Code output,
						@Error_Message = @Error_Message output;
	...
	public function loadRegistryData($data) {
		...
		$regData = $this->getFirstRowFromQuery("
			select
				...
				ISNULL(r.Registry_IsNew, 1) as \"Registry_IsNew\",
		...
		if ( !empty($data['Evn_disDate']) ) {
			$filterList[] = "convert(varchar(10), RD.Evn_disDate,104) = :Evn_disDate";
		...
		$joinList[] = "left join v_UslugaComplex U with (NOLOCK) on U.UslugaComplex_id = RD.UslugaComplex_id";
		...
		if (
			...
			$filterList[] = "ISNULL(RD.RegistryData_deleted, 1) = 1";
	...
	public function loadRegistryEntiesSettings($data = []) {
		...
		return $this->queryResult("
			...
			WHERE 
				...
				AND FLKSettings_EvnData LIKE '%vologda%'
				{$where}
		", $params);
	}
	...
	protected function _setXmlPackNum($data) {
		...
		if ( empty($data['Registry_IsNew']) ) {
			$filter='and ISNULL(RegistrySubType_id, 0) = ISNULL(:RegistrySubType_id, 0)';
		}
		$query = "
			declare
				 @packNum int
					...
					begin
						set @packNum = (
							...
							where Lpu_id = :Lpu_id
								...
								{$filter}
								and ISNULL(DispClass_id, 0) = ISNULL(:DispClass_id, 0)
								and ISNULL(Registry_IsNew, 0) = ISNULL(:Registry_IsNew, 0)
						);
	...
	public function getPersonDataByPersonEvnId($PersonEvn_id, $Registry_id, $RegistrySubType_id, $Registry_IsNew = false) {
		...
		if ( 2 == $RegistrySubType_id ) {
			$joinList[] = "inner join {$this->scheme}.v_RegistryGroupLink rgl with (nolock) on rgl.Registry_id = rd.Registry_id";
		...
		return $this->getFirstRowFromQuery("
			select top 1
				...
				convert(varchar(10), p.Person_Birthday, 120) as DR,
				convert(varchar(20), e.Evn_setDT, 120) as Evn_setDT
			from {$this->scheme}.v_{$this->RegistryDataObject} rd with (nolock)
				" . (count($joinList) > 0 ? implode(' ', $joinList) : "") . "
				inner join Evn e with (nolock) on e.Evn_id = rd.Evn_id
		...
		try {
			if ( !array_key_exists(intval($data['SMO']), $this->orgSmoList) ) {
				$this->orgSmoList[intval($data['SMO'])] = $this->getFirstResultFromQuery("
					select top 1 OrgSmo_id from v_OrgSMO (nolock) where Orgsmo_f002smocod = :SMO and KLRgn_id = 35
				", array(
					'SMO' => intval($data['SMO'])
				));
			}
			...
			if ( $hasPersonEvnChanges ) {
				$dbmain->query('exec [dbo].[xp_PersonTransferEvn] @Person_id = :Person_id', array('Person_id' => $Person_id));
			}
		...
		try {
			...
			$resultLast = $dbmain->query("
				select top 1
					...
					convert(varchar(20), PersonSurName_insDT, 120) as PersonSurName_insDT
		...
		try {
			...
			$resultLast = $dbmain->query("
				select top 1
					...
					convert(varchar(20), PersonFirName_insDT, 120) as PersonFirName_insDT
		...
		try {
			...
			$resultLast = $dbmain->query("
				select top 1
					...
					convert(varchar(20), PersonSecName_insDT, 120) as PersonSecName_insDT
		...
		try {
			...
			$resultLast = $dbmain->query("
				select top 1
					Server_id,
					PersonBirthday_id,
					convert(varchar(10), PersonBirthday_Birthday, 120) as PersonBirthday_Birthday,
					convert(varchar(20), PersonBirthday_insDT, 120) as PersonBirthday_insDT
	...
	public function checkRegistryDataIsInOtherRegistry($data) {
		...
		try {
			...
			if (
				...
				$join .= " left join v_OmsSprTerr ost with (nolock) on ost.OmsSprTerr_id = rd.OmsSprTerr_id";
				// для финального берём по другому
			...
			$checkResult = $this->getFirstRowFromQuery("
				select top 1 ro.Registry_Num, uro.Registry_Num as UnionRegistry_Num  
				from {$this->scheme}.v_{$this->RegistryDataObject} rd with (nolock)
					inner join {$this->scheme}.v_Registry r with (nolock) on r.Registry_id = rd.Registry_id
					inner join {$this->scheme}.v_RegistryGroupLink rgl with (nolock) on rgl.Registry_id = rd.Registry_id
					inner join {$EvnClass_SysNick} e with (nolock) on e.{$EvnClass_SysNick}_id = rd.Evn_id
					inner join {$this->scheme}.v_{$this->RegistryDataObject} rdo with (nolock) on rdo.Evn_id = e.{$EvnClass_SysNick}_id
						and rdo.Registry_id != rd.Registry_id
					inner join {$this->scheme}.v_Registry ro with (nolock) on ro.Registry_id = rdo.Registry_id
					left join {$this->scheme}.v_RegistryGroupLink rglo with (nolock) on rglo.Registry_id = rdo.Registry_id
					left join {$this->scheme}.v_Registry uro with (nolock) on uro.Registry_id = rglo.Registry_pid
						and uro.RegistryStatus_id != 4
					{$join}
				where rgl.Registry_pid = :Registry_id
					and ISNULL(e.{$EvnClass_SysNick}_IsInReg, 1) = 2
					and ISNULL(rd.Paid_id, 1) = 1
					and (
	...
	private function _loadUnionRegistryGridNew($data) {
		$query = "
			select 
				...
				case when R.Registry_IsRepeated = 2 then 'true' else 'false' end as \"Registry_IsRepeated\",
				case when R.Registry_isPersFin = 2 then 'true' else 'false' end as \"Registry_isPersFin\",
				case when R.Registry_IsFAP = 2 then 'true' else 'false' end as \"Registry_IsFAP\",
	...
	private function _saveUnionRegistryNew($data) {
		try {
			...
			switch ( $data['RegistryGroupType_id'] ) {
				case 1:
					$registrytypefilter = " and R.RegistryType_id in (1, 2) and ISNULL(R.Registry_IsZNO, 1) = ISNULL(:Registry_IsZNO, 1) and ISNULL(R.Registry_isPersFin, 1) = ISNULL(:Registry_isPersFin, 1) and ISNULL(R.Registry_IsFAP, 1) = ISNULL(:Registry_IsFAP, 1)";
					break;
				...
				case 15:
					$registrytypefilter = " and R.RegistryType_id = 15 and ISNULL(R.Registry_IsZNO, 1) = ISNULL(:Registry_IsZNO, 1)";
					break;

				case 21:
					$registrytypefilter = " and R.RegistryType_id = 6 and ISNULL(R.Registry_isPersFin, 1) = ISNULL(:Registry_isPersFin, 1)";
					break;
				...
				case 34:
					$registrytypefilter = " and R.RegistryType_id = 20 and ISNULL(R.Registry_IsZNO, 1) = ISNULL(:Registry_IsZNO, 1)";
					break;
			}
			...
			$resp_reg = $this->queryResult("
				...
				where
					...
					and ISNULL(R.Org_did, 0) = ISNULL(:Org_did, 0)
			...
			", [
				...
				'Registry_endDate' => $data['Registry_endDate'],
				'Registry_isPersFin' => $data['Registry_isPersFin'],
				'Registry_IsFAP' => $data['Registry_IsFAP'],
				'Registry_IsRepeated' => $data['Registry_IsRepeated'],
				'Registry_IsZNO' => $data['Registry_IsZNO'],
			]);
	...
	public function loadUnionRegistryEditForm($data) {
		return $this->queryResult("
			select
				...
				R.Registry_IsFAP as \"Registry_IsFAP\"
			from
				{$this->scheme}.v_Registry R (nolock)
			where
				R.Registry_id = :Registry_id
	...
	private function _saveRegistryQueueNew($data) {
		...
		$rq = $this->getFirstResultFromQuery("
			...
			where
				...
				and ISNULL(Registry_IsNotInsur, 0) = ISNULL(:Registry_IsNotInsur, 0)	
				and ISNULL(Registry_isPersFin, 0) = ISNULL(:Registry_isPersFin, 0)	
				and ISNULL(Registry_IsFAP, 0) = ISNULL(:Registry_IsFAP, 0)	
				and ISNULL(Registry_begDate, 0) = ISNULL(:Registry_begDate, 0)
				and ISNULL(Registry_endDate, 0) = ISNULL(:Registry_endDate, 0)
				and ISNULL(OrgSmo_id, 0) = ISNULL(:OrgSmo_id, 0)
				and ISNULL(Registry_Num,0) = ISNULL(:Registry_Num,0)
				and ISNULL(Registry_IsNew, 0) = ISNULL(:Registry_IsNew, 0)
				and ISNULL(Org_did, 0) = ISNULL(:Org_did, 0)
		", $params);
	...
	private function _exportRegistryToXmlNew($data = []) {
		...
		try {
			...
			switch ( true ) {
				...
				case ($registryIsUnion == true && $data['RegistryGroupType_id'] == 15):
					if ( $data['Registry_IsZNO'] == 2 ) {
						$pcode = 'LC';
						$scode = 'C';
					}
	...
	public function loadRegistryError($data = []) {
		...
		switch ( $this->RegistryType_id ) {
			...
			case 14:
				...
				$fieldsList[] = "m.Mes_Code";
				$fieldsList[] = "ISNULL(MP.Person_Fio, RD.MedPersonal_Fio) as MedPersonal_Fio";

				if ( !empty($data['MedPersonal_id']) ) {
					$filterList[] = "ISNULL(ES.MedPersonal_id, RD.MedPersonal_id) = :MedPersonal_id";
					$params['MedPersonal_id'] = $data['MedPersonal_id'];
				}
				break;
			...
			case 2:
				...
				if ( !empty($data['MedPersonal_id']) ) {
					$filterList[] = "ISNULL(evpl.MedPersonal_id, RD.MedPersonal_id) = :MedPersonal_id";
					$params['MedPersonal_id'] = $data['MedPersonal_id'];
				}
				break;
			...
			case 7:
				...
				if ( !empty($data['MedPersonal_id']) ) {
					$filterList[] = "ISNULL(eudd.MedPersonal_id, RD.MedPersonal_id) = :MedPersonal_id";
					$params['MedPersonal_id'] = $data['MedPersonal_id'];
				}
				break;
			...
			case 9:
				...
				if (!empty($data['MedPersonal_id'])) {
					$filterList[] = "ISNULL(evdo.MedPersonal_id, RD.MedPersonal_id) = :MedPersonal_id";
					$params['MedPersonal_id'] = $data['MedPersonal_id'];
				}
				break;

			default:
				$fieldsList[] = "RD.MedPersonal_Fio";
		...
		if ( in_array($this->RegistryType_id, array(7, 9, 12, 17)) ) {
			$joinList[] = "
				left join v_EvnPLDisp epd with (nolock) on epd.EvnPLDisp_id = COALESCE(RD.Evn_rid, RE.Evn_rid, RE.Evn_id)
			";
			$fieldsList[] = "epd.DispClass_id";
		}
	 ...
	 */
		...
		public function getLpuSidList($data){
			...
			return $this->queryResult("
			select distinct Lpu_sid from {$this->scheme}.v_RegistryDataPar with (nolock) where Registry_id = :Registry_id
		", $data);
		}
