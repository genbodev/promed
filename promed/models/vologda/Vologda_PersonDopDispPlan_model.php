...
class Vologda_PersonDopDispPlan_model extends PersonDopDispPlan_model {
	...
	function loadPersonDopDispPlanExportList($data) {
		...
		$query = "
			select
				...
				pddpe.PersonDopDispPlanExport_IsExportPeriod
				-- end select
			from
				-- from
				...
				inner join v_DispCheckPeriod dcp (nolock) on dcp.DispCheckPeriod_id = pddpe.DispCheckPeriod_id
				outer apply (
					select
						count(*) as cnt
					from
	...
	function loadExportErrorPlanDDList($data) {
		...
		$query = "
			...
			from
				...
				left join v_PersonDopDispPlan pddp (nolock) on pddp.PersonDopDispPlan_id = ppl.PersonDopDispPlan_id
				left join v_DispCheckPeriod dcp (nolock) on dcp.DispCheckPeriod_id = pddp.DispCheckPeriod_id
				left join v_DispClass dt (nolock) on dt.DispClass_id = pddp.DispClass_id
				-- end from
			where
				-- where
				{$filter}
	...
	function exportPersonDopDispPlan($data) {
		...
		if ($data['ignoreCheck'] != 2) {
			$resp_check = $this->queryList("
				declare @begDt date, @endDt date;
				
				select 
					@begDt = DispCheckPeriod_begDate,
					@endDt = {$endDtSelect}
				from DispCheckPeriod (nolock)
				...
				with t as (
					select
						...
						rtrim(replace(dcp.DispCheckPeriod_Name, year(dcp.DispCheckPeriod_begDate), '')) as DispCheckPeriod_Name
					from DispCheckPeriod dcp (nolock)
					inner join DispClass dc (nolock) on dc.DispClass_id in (1,5,28)
					where 
						dcp.PeriodCap_id = 4
						and dcp.DispCheckPeriod_begDate >= @begDt
						and dcp.DispCheckPeriod_begDate <= @endDt
						and not exists (
				...
				select distinct 
					' - ' + DispClass_Name + ': ' + stuff((
						select concat(', ', DispCheckPeriod_Name) 
						from t t1 with (nolock)
						where t.DispClass_id = t1.DispClass_id
						for XML path('')
					), 1, 1, '') DispCheckPeriod_Name,
					DispClass_id
				from t 
		...
		$data['PersonDopDispPlan_ids'] = $this->queryList("
			...
			select PersonDopDispPlan_id 
			from v_PersonDopDispPlan pddp (nolock) 
			inner join DispCheckPeriod dcp (nolock) on dcp.DispCheckPeriod_id = pddp.DispCheckPeriod_id
			where 
				pddp.Lpu_id = :Lpu_id and 
				pddp.DispClass_id in (1,5,28) and 
				dcp.DispCheckPeriod_begDate >= @begDt and 
				dcp.DispCheckPeriod_endDate <= @endDt
		", $data);
	...
	function _exportPersonDopDispPlan($data) {
		...
		$resp = $this->queryResult("
			...
			select
				...
				rtrim(ISNULL(ps.Person_Phone, [pi].PersonInfo_InternetPhone)) as PHONE,
				pddp.PersonDopDispPlan_Year as YEAR,
				convert(varchar(10), pd.PersonDisp_begDate, 120) as DAT_INC,
				pdvcount.D_PERIOD,
				case
				...
				convert(varchar(10), lapdv.PersonDispVizit_NextFactDate, 120) as DAT_PREV,
				MONTH(dcp.DispCheckPeriod_begDate) as MES,
				0 as MDP,
				null as DS,
				case
			...
			from
				...
				outer apply (
					...
					left join v_MedPersonal mp with (nolock) on pd.MedPersonal_id = mp.MedPersonal_id and pd.Lpu_id = mp.Lpu_id
	...
	function importPersonDopDispPlan($data) {
		...
		if (preg_match('/D\-F35\-M'.$Lpu_f003mcod.'\-([0-9]{4})\-([0-9]{1,2})/ui', $fname, $match)) {
			...
			if (!empty($resp_pddpe[0]['PersonDopDispPlanExport_id'])) {
				...
				foreach ( $xml->ZAP as $onezap ) {
					...
					if (empty($resp_ppl[0]['PlanPersonList_id']) || empty($PROV)) continue;
					
					$COMENTZ = $PROV->COMENTZ->__toString();
					$ENDR = $PROV->ENDR->__toString();
					if($ENDR == 0 && empty($COMENTZ)){
						...
						if(!empty($COMENTZ)){
							...
							$MO = $PROV->MO->__toString();
							$SMO = $PROV->SMO->__toString();
							$stat = $ENDR == 0 ? 'действующая' : 'полис закрыт';
	...
	function autoCreatePlan($data) {
		...
		$person_list = $this->queryList("
			select 
				pd.Person_id 
			from v_PersonDisp pd (nolock)
			inner join v_PersonState ps (nolock) on pd.Person_id = ps.Person_id
			inner join v_Lpu vl (nolock) on pd.Lpu_id = vl.Lpu_id
			where 
				pd.Lpu_id = :Lpu_id 
				and pd.PersonDisp_begDate <= :DispCheckPeriod_endDate  
				and isnull(pd.PersonDisp_endDate, :DispCheckPeriod_begDate) >= :DispCheckPeriod_begDate 
				and (
				...
				and exists (
					select top 1 pdv.PersonDispVizit_id
					from v_PersonDispVizit pdv (nolock)
					where 
						...
						and pdv.PersonDispVizit_NextDate <= isnull(pd.PersonDisp_endDate, :DispCheckPeriod_endDate)
						and pdv.PersonDispVizit_NextFactDate is null
				)
				and not exists (
					select top 1 pddp.PersonDopDispPlan_id
					from v_PersonDopDispPlan pddp (nolock) 
					inner join v_PlanPersonList ppl (nolock) on pddp.PersonDopDispPlan_id = ppl.PersonDopDispPlan_id
