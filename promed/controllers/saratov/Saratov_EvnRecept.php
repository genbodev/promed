...
/**
* Person - контроллер для работы с рецептами. Версия для Саратова
*
...
class Saratov_EvnRecept extends EvnRecept {
	...
	/**
	*  Получение номера рецепта для Саратова
	*  Входящие данные: нет
	...
	*/
	function getReceptNumber($returnValue = false, $convertFromUTF8 = true) {
		...
		// Для Саратова
		if ( $data['session']['region']['number'] != 64 ) {
	...
	/**
	*  Сохранение рецепта для Саратова
	*  Входящие данные: ...
	*  На выходе: JSON-строка
	*  Используется: форма редактирования рецепта
	*/
