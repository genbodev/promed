...
/**
 * Ufa_BSK_Register_User - контроллер для БСК (Башкирия)
...
class Ufa_BSK_Register_User extends swController
{
    var $model = "Ufa_BSK_Register_User_model";
                    ...
                    foreach ($v as $key => $val) {
                        ...
                        if ($value == $val['BSKObservElementValues_data']) {
                            ...
                            if (!in_array($k, array(
                                ...
                                if (sizeof($val['BSKObservElementValues_sign']) > 0) {
                                    $this->getResult($data);
                                }
                            } else {
    ...
    /**
     * Получение данных о типе МО
     */
    function getLpuType(){
        $data = $this->ProcessInputData('getLpuType', true);
        if ( $data === false ) { return false; }
        $response = $this->dbmodel->getLpuType($data);
        $this->ReturnData($response);
    }
