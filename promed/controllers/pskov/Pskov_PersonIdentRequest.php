...
class Pskov_PersonIdentRequest extends PersonIdentRequest {
	/**
	 * Конструктор
	 */
	...
	/**
	*  Выполнение запроса на идентификацию пациента для Пскова
	...
	*/
	function doPersonIdentRequest() {
		...
		/*
		if (!isSuperAdmin()) {
			$val['Error_Msg'] = 'Данный функционал в текущий момент в реализации';
			$this->ReturnData($val);
			return false;
		}
		*/
		...
		$this->load->library('swPersonIdentPskov');
		$identObject = new swPersonIdentPskov(
