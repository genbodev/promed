...
class Kareliya_EvnPS extends EvnPS {
	...
	function exportHospDataForTfomsToXml() {
		...
		$template = 'hosp_data_for_tfoms_kareliya';
		$from = $data['ARMType'] == 'superadmin' ? 'F' : 'M';
		...
		$out_dir = "re_xml_".time()."_"."hospDataForTfoms_kareliya";
		...
		foreach($hosp_data_xml_arr as $fcode => $lpu_hosp_data_xml) {
			...
			foreach($lpu_hosp_data_xml as $method => $data_xml) {
				...
				$file_name = $method."-".$from."-".$fcode."-".$data['Date'];
