...
class Penza_PersonCard extends PersonCard {
	/**
	 *	Конструктор
	 */
	function __construct() {
		...
		foreach ( $this->inputRules['loadAttachedList'] as $key => $array ) {
			switch ( $array['field'] ) {
				case 'AttachLpu_id':
				case 'Date_upload':
					$this->inputRules['loadAttachedList'][$key]['rules'] = 'required';
				break;
			}
		}
	}
	...
	/**
	 * Функция возвращает в CSV список прикрепленного населения к указанной СМО на указанную дату
	 */
	function loadAttachedList()
	{
		...
		$file_zip_sign = 'attached_list';
		$file_zip_name = EXPORTPATH_ATACHED_LIST.$out_dir."/".$file_zip_sign.".zip";

		$resp = $this->pcmodel->saveAttachedListToCSV($data, $file_zip_name, $out_dir);

		if ( $resp === false ) {
			$this->ReturnData(array('success' => false, 'Error_Msg' => toUTF('Ошибка при формировании списка для выгрузки')));
			return false;
		}
		else if ( !empty($resp['Error_Code']) && $resp['Error_Code'] == 1 ) {
				...
				if(isset($value[0]))
				{
					for($i=0; $i<count($value);$i++)
					{
						...
						if(isset($value[$i]['REFREASON']))
							$file_array[$i]['REFREASON'] = !is_array($value[$i]['REFREASON'])?$value[$i]['REFREASON']:'0';
						else
							$file_array[$i]['REFREASON'] = '0';
					}
				}
				else
				{
					...
					if(isset($value['REFREASON']))
						$file_array[$i]['REFREASON'] = !is_array($value['REFREASON'])?$value['REFREASON']:'0';
					else
						$file_array[$i]['REFREASON'] = '0';
				}
			}
			...
			else
			{
				...
				else
				{
					if($personCard_data['PersonCard_id'] != '0') //Нашли прикрепление (или открепление)
					{
						...
						$refreason = str_replace(" ", "", (mb_strtolower($file_array[$i]['REFREASON'])));
						//if(in_array($file_array[$i]['REFREASON'], array('0','3','5','Включен в Регистр'))){
						...
						{
							$log_arr[] = 'Результат: Отказано. Запись не принята по следующим причинам: '.$this->getErrorText($file_array[$i]['REFREASON']);
					...
					{
						$refreason = str_replace(" ", "", (mb_strtolower($file_array[$i]['REFREASON'])));
						//if(in_array($file_array[$i]['REFREASON'], array('0','3','5','Включен в Регистр'))){ //Если нет критических ошибок, ставим заявлению статус "Одобрено" и создаем прикрепление
						if(in_array($refreason, array('0','3','5','включенврегистр'))){
							$change_data = array(
