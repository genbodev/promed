 ...
 * )
 */
class LisUpdater extends SwREST_Controller {
	...
	function __construct() {
		...
		try {
			$lisConfig = $this->config->item('SwServiceCommon');
			$this->load->library('SwServiceApi', $lisConfig, 'common');

			$dbres = $this->common->GET('Options/DataStorageValues', [
				'DataStorageGroup_SysNick'=>'lis'
			]);
			...
			$response = array('Error_Msg' => 'Не удалось получить настройки для ЛИС');
	...
	/**
	 * @OA\Get(
	 *     	path="/api/LisUpdater/Directories",
	 *  	tags={"LisUpdater"},
	...
	function Directories_get() {
		...
		if (!isset($logindata['login'])) {
			//$response = array('success' => false, 'Error_Msg' => 'Не заполнены настройки подключения к ЛИС (логин, пароль, client id)<br/> для синхронизации справочников.<br/>Пожалуйста, обратитесь к системным администраторам.');
		...
		if ($this->Lis_model->isLogon()) {
			...
			$response = array('success' => false, 'Error_Msg' => 'Не удалось авторизироваться в ЛИМС-системе. <br/>Пожалуйста, обратитесь к разработчикам.');
		}
	...
	function getDirectoryFromXML(&$xml){
		...
		$data = $this->ProcessInputData('getDirectoryFromXML', null, true);
	...
	/**
	 * @OA\Get(
	 *     	path="/api/LisUpdater/DirectoryVersions",
	 *  	tags={"LisUpdater"},
