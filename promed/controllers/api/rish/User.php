...
class User extends SwREST_Controller {
	...
	function mlogin_post() {
		...
		if ($user && $result = $user->login($data['login'], $data['password'])) {
			...
			$PersonAtWork = $this->Messages_model->mCheckActiveWorkTime(array('pmUser_tid'=>$user->pmuser_id));
			...
			$resp = array(
				...
				'PersonAtWork' => $PersonAtWork['PersonAtWork']
			);
			...
			if (!empty($user->settings)) {
				...
				if (!empty($settings['defaultARM']['MedStaffFact_id'])) {
					...
					if (!empty($info[0]['MedStaffFact_id'])) {
						$resp['Server_id'] = $info[0]['Server_id'];
						...
						$_SESSION['CurLpuSection_id'] = $info[0]['LpuSection_id'];
					}
				} elseif(!empty($settings['defaultARM']['MedPersonal_id'])) {
					$this->load->model('MedPersonal_model');
					//	$infoMedPersonal = $this->MedPersonal_model->getMedPersonalInfoForAPI(array(
					//		'MedPersonal_id' => $settings['defaultARM']['MedPersonal_id']
					//	));

					if (!empty($infoMedPersonal[0]['MedPersonal_id'])) {
						$resp['MedPersonal_id'] = $infoMedPersonal[0]['MedPersonal_id'];
						$resp['Person_Fio'] = $infoMedPersonal[0]['Person_Fio'];
						$resp['Lpu_Nick'] = $infoMedPersonal[0]['Lpu_Nick'];
						$resp['Lpu_id'] = $infoMedPersonal[0]['Lpu_id'];
					}
				}

				$resp['ARMType'] = !empty($settings['defaultARM']['ARMType']) ? $settings['defaultARM']['ARMType'] : null;
				$resp['MedService_id'] = !empty($settings['defaultARM']['MedService_id']) ? $settings['defaultARM']['MedService_id'] : null;
			}
	...
	function mGetMedstaffFactList_get() {
		...
		$armTypes = explode(',', $data['ARMType']);
		$resp = array();

		foreach ($result as $workplace) {

			if (!in_array($workplace['ARMType'], $armTypes)) continue;

			$resp[] = array(
				'MedStaffFact_id' => $workplace['MedStaffFact_id'],
				'Person_Fio' => $workplace['MedPersonal_FIO'],
				'LpuSection_id' => $workplace['LpuSection_id'],
				'LpuSection_Name' => $workplace['LpuSection_Name'],
				'Lpu_Nick' => $workplace['Lpu_Nick'],
				'MedService_id' => !empty($workplace['MedService_id']) ? $workplace['MedService_id'] : null,
				'ARMType' => $workplace['ARMType']
			);
		}
	...
	function getArmData($data) {
		...
		if (!empty($data['MedStaffFact_id']) && empty($data['ARMType'])) {
			$data['ARMType'] = $this->User_model->getFirstLpuUnitTypeSysNickByMedStaffFact($data);
		}
	...
	function mSetCurrentARM_post() {
		...
		$data = $this->ProcessInputData('mSetCurrentARM', null, true, false);
		$data['lpu'] = null;
		...
		if (!empty($curArm)) {
			$_SESSION['CurARM'] = $curArm[0]; // загоняем в сессию все данные по текущему арму, чтобы использовать в других местах.
			// некоторые данные в сессию уже загонялись, пусть остаются отдельно
			$_SESSION['CurMedStaffFact_id'] = !empty($curArm[0]['MedStaffFact_id'])?$curArm[0]['MedStaffFact_id']:null;
			$_SESSION['CurLpuSection_id'] = !empty($curArm[0]['LpuSection_id'])?$curArm[0]['LpuSection_id']:null;
			$_SESSION['CurMedService_id'] = !empty($curArm[0]['MedService_id'])?$curArm[0]['MedService_id']:null;
			$_SESSION['CurLpuUnit_id'] = !empty($curArm[0]['LpuUnit_id'])?$curArm[0]['LpuUnit_id']:null;
			$_SESSION['CurArmType'] = !empty($curArm[0]['ARMType'])?$curArm[0]['ARMType']:null;
