...
class Person extends SwREST_Controller {
	protected  $inputRules = array(
		...
		'createPerson' => array(
			...
			array('field' => 'PersonPhone_Phone', 'label' => 'Телефонный номер', 'rules' => '', 'type' => 'string'),
			array('field' => 'PersonSnils_Snils', 'label' => 'СНИЛС', 'rules' => '', 'type' => 'snils'),
		...
		'createPolis' => array(
			...
			array('field' => 'Polis_Ser', 'label' => 'Серия полиса', 'rules' => '', 'type' => 'string'),
			array('field' => 'Polis_Num', 'label' => 'Номер полиса', 'rules' => 'required', 'type' => 'string'),
	...
	function index_post() {
		...
		if( isset($data['Person_isUnknown']) ){
			if(!in_array($data['Person_isUnknown'], array(0,1))){
				$this->response(array(
					'error_code' => 6,
					'error_msg' => 'неверное значение параметра Person_isUnknown'
				));
			}
	...
	function mloadPersonForm_get(){
		...
		$resp = $this->EPH_model->loadPersonForm($data);
	...
	protected $offline_map = array(
		...
		'person_overview' => array('model' => 'EPH_model', 'method' => 'loadPersonForm'),
