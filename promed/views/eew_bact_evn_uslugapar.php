...
$birt_suffix = $this->usePostgreLis ? '_pg' : '';
...
if ($isAllowStudyViews && !empty($StudyViews[0]['view'])) {
    $frameTpl = '<div class="frame clear">
        <div class="data-table"{visible}>
            <div class="caption">
            <h2><span >Прикрепленное изображение</span></h2>{externalLink}
            </div>
...
$mainTplData['{buttons}'] .= "<a onClick='printBirt({Report_FileName: \"BactEvnParCard_list{$birt_suffix}.rptdesign\",Report_Params: \"&paramEvnUslugaPar={EvnUslugaPar_id}\",Report_Format: \"pdf\"})' id='EvnUslugaPar_{EvnUslugaPar_id}_printEvnUslugaPar' class='button icon icon-print16' title='Печать'><span></span></a>";
