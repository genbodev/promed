<html>
 <head>
  <title>Личное заявление на прикрепление: {Person_FIO}</title>
  <style type="text/css">
 ...
 <body>
  ...
  <p class="ulText">Я, {Person_FIO}, {Person_Birthday}, {Sex_Name}
  <hr/>
  <p class="bottomText">(фамилия, имя, отчество, дата рождения, пол – М/Ж)</p>

  <p class="ulText">подтверждаю выбор {Lpu_Name}
  <hr/>
  <p class="bottomText">(полное наименование медицинской организации)</p>

  <p class="ulText">а также участкового врача {MedStaffRegion_Fio}
  <hr/>
  ...
  <p class="bottomText">(адрес проживания по постоянной регистрации, по временной регистрации,
   <?php if ($Person_PAddress && $Person_RAddress && $Person_PAddress != $Person_RAddress) echo('<span class="bottomText ulString">'); ?>по месту фактического проживания без регистрации<?php if ($Person_PAddress && $Person_RAddress && $Person_PAddress != $Person_RAddress) echo('</span>'); ?>
   - нужное подчеркнуть)
  </p>
  ...
  <p class="ulText">Паспорт (другой документ, удостоверяющий личность) {DocumentType_Name}: серия {Document_Ser} № {Document_Num}, выдан {Document_begDate}
  <hr/>

  <p class="ulText">{OrgDep_Name}&nbsp;
  <hr/>
  <p class="bottomText">(название органа, выдавшего документ)</p>

  <p class="ulText">Контактный телефон {Person_Phone}
  <hr/>
