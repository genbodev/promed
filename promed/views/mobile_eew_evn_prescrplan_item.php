		...
		<p><?php 
			...
			switch($PrescriptionType_id){
				...
				case 2://<период действия>  <тип диеты> <комментарий> 
					...
					break;
				case 4://<дата и время> <профиль> <комментарий> 
					$EvnPrescrItem_id = $EvnPrescrCons_id;
					echo '<b>{EvnPrescr_setDate}</b>'.((empty($EvnPrescr_setTime))?'':'&nbsp;<b>{EvnPrescr_setTime}</b>');
					echo '&nbsp;{LpuSectionProfile_Name}';
					if ( $IsCito_Code == 1 )
						echo  '&nbsp;<span style="color: red">Cito!</span>';
					echo (!empty($EvnPrescrCons_hint) ? '&nbsp;<img src="/img/icons/comment16.png" title="' . htmlspecialchars($EvnPrescrCons_hint) . '" />':'');
					break;
				case 10://<период действия> <Утро, День, Вечер> <параметры через запятую>, комментарий
					echo '<b>{EvnPrescr_setDate}</b>'.((empty($EvnPrescr_setTime))?'':'&nbsp;<b>{EvnPrescr_setTime}</b>');
					echo '&nbsp;Параметры наблюдения: {Params}'; 
					$EvnPrescrItem_id = $EvnPrescrObserv_id;
					echo (!empty($EvnPrescrObserv_hint) ? '&nbsp;<img src="/img/icons/comment16.png" title="' . htmlspecialchars($EvnPrescrObserv_hint) . '" />':'');
					break;
				case 5://
