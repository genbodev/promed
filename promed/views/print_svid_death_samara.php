...
КОРЕШОК МЕДИЦИНСКОГО СВИДЕТЕЛЬСТВА О СМЕРТИ К ФОРМЕ № 106/У-08<br/>
...
1. Фамилия, имя, отчество умершего: {Person_FIO}<br/>
	...
	<tr>
		<td style="border: 1px black dotted; vertical-align:top;">
			<table>
				...
				<tr><td colspan="2">Для врача, занимающегося частной практикой:</td></tr>
				<tr><td colspan="2">Номер лицензии на медицинскую деятельность</td></tr>
...
<div style="text-align: left; margin-bottom:1em;">
<table class="ct">
<tr><td class="small">1.</td><td>Фамилия, имя, отчество умершего: {Person_FIO}</td></tr>
...
<tr><td>6.</td><td>Местность: <span class="val_1">город</span> - 1, <span class="val_2">сельская</span> - 2 <div class="selector">{KlareaType_id}</div></td></tr>
<tr><td>7.</td><td>Место смерти: республика,край, область {DKLRGN_Name}</br>
	...
	<table style="width:100%;">
		<tr>
			...
			<td style="width:27%;">дом {DAddress_House}</td>		
			<td style="width:27%;">кв. {DAddress_Flat}</td>	
		</tr>	
	</table>
</td></tr>
<tr><td>8.</td><td>Местность: <span class="val_1">город</span> - 1, <span class="val_2">сельская</span> - 2 <div class="selector">{DKlareaType_id}</div></td></tr>
...
<tr><td>11.</td><td> Для детей, умерших в возрасте от 168 часов до 1 года: масса тела ребёнка при рождении <nobr>{DeathSvid_Mass} грамм - 1,</nobr> каким по счёту был ребёнок у матери <nobr>(считая умерших и несчитая мертворожденных) {DeathSvid_ChildCount} - 2,</nobr> дата рождения <nobr>матери {DeathSvid_MotherBirthday} - 3,</nobr> возраст <nobr>матери {DeathSvid_MotherAge} - 4,</nobr> фамилия <nobr>матери {DeathSvid_MotherFamaly} - 5,</nobr> <nobr>имя {DeathSvid_MotherName} - 6,</nobr> <nobr>отчество {DeathSvid_MotherSecName} - 7.</nobr></td></tr>
<tr><td>12.</td><td>*Семейное положение: <span class="val_1">состоял(а) в <nobr> зарегистрированном браке</span> - 1,</nobr> <span class="val_2">не состоял(а) в <nobr>зарегистрированном браке</span> - 2,</nobr>  <nobr><span class="val_3">неизвестно</span> - 3,</nobr><div class="selector">{DeathFamilyStatus_Code}</div></td></tr>
...
<tr><td>14.</td><td>*Занятность: был(а) занят(а) в экономике: <span class="val_1">руководители и специалисты высшего уровня <nobr>квалификации</span> - 1,</nobr> <span class="val_2">прочие <nobr>специалисты</span> - 2,</nobr> <span class="val_3">квалифицированные <nobr>рабочие</span> - 3,</nobr> <span class="val_4">неквалифицированные <nobr>рабочие</span> - 4,</nobr> <span class="val_5">занятые на военной <nobr>службе</span> - 5;</nobr> не был(а) занята(а) в экономике: <nobr><span class="val_6">пенсионеры</span> - 6,</nobr> <span class="val_7">студенты и <nobr>учащиеся</span> - 7,</nobr> <span class="val_8">работавшие в личном подсобном <nobr>хозяйстве</span> - 8,</nobr> <nobr><span class="val_9">безработные</span> - 9,</nobr> <nobr><span class="val_10">прочие</span> - 10.</nobr><div class="selector">{DeathZanat_id}</div></td></tr>
...
<tr style="page-break-before: always;"><td colspan="2">
	<table class="ct ctleft">
		<tr><td class="small">10.</td><td colspan="2">Причины смерти:</td><td colspan="3" class="tcent" style="border-left-style: dashed; border-right-style: dashed;">Приблизительный</br>период времени</br>между началом</br>патологического</br>процесса и смерти</td><td rowspan="9" class="small">&nbsp;</td><td colspan="5"><br><br>Код по <br>МКБ-10</td></tr>
		<tr><td rowspan="8">I.</td><td rowspan="2" class="small">а)</td><td class="dashed">{Diag1_Name}</td><td rowspan="8" class="small" style="border-left-style: dashed;">&nbsp;</td><td class="dashed tcent">&nbsp;</td><td rowspan="8" class="small" style="border-right-style: dashed;">&nbsp;</td><td class="diat">{D11}</td><td class="diat">{D12}</td><td class="diat">{D13}</td><td style="text-align: center">{D14}</td><td class="diat">{D15}</td></tr>
		<tr><td class="tcent">(болезнь или состояние непосредственно приведшее к смерти)</td><td>&nbsp;</td><td colspan="5">&nbsp;</td></tr>			
		<tr><td rowspan="2">б)</td><td class="dashed">{Diag2_Name}</td><td class="dashed tcent">&nbsp;</td><td class="diat">{D21}</td><td class="diat">{D22}</td><td class="diat">{D23}</td><td>{D24}</td><td class="diat">{D25}</td></tr>
		...
		<tr><td rowspan="2">в)</td><td class="dashed">{Diag3_Name}</td><td class="dashed tcent">{DeathSvid_PribPeriod}</td><td class="diat">{D31}</td><td class="diat">{D32}</td><td class="diat">{D33}</td><td>{D34}</td><td class="diat">{D35}</td></tr>
		<tr><td class="tcent">(первоначальная причина смерти указывается последней)</td><td>&nbsp;</td><td colspan="5">&nbsp;</td></tr>
		<tr><td rowspan="2">г)</td><td class="dashed">{Diag4_Name}</td><td class="dashed tcent">&nbsp;</td><td class="diat">{D41}</td><td class="diat">{D42}</td><td class="diat">{D43}</td><td>{D44}</td><td class="diat">{D45}</td></tr>
		...
		<tr><td>дата)</td><td colspan="4" class="dashed">{Diag5_Name}</td><td>&nbsp;</td><td class="diat">{D51}</td><td class="diat">{D52}</td><td class="diat">{D53}</td><td>{D54}</td><td class="diat">{D55}</td></tr>
...
<tr><td>16.</td><td> В случае смерти от несчастного случая, убийства, самоубийства, от военных и террористических действий, при неустановленном роде смерти - указать дату травмы (отравления): {DeathSvid_TraumaDate_Date}, время {DeathSvid_TraumaDate_Time}</br>
	а так же место и обстоятельства, при которых произошла травма (отравление):</br>																																																	
	{DeathSvid_TraumaObst}																																																		
</td></tr>																																																			
<tr><td>17.</td><td> Причина смерти установлена: <span class="val_1">врачом, только установившим смерть</span> - 1, <span class="val_2">лечащим врачом</span> - 2, <span class="val_3">фельдшером (акушеркой)</span> - 3, <span class="val_4">патологоанатомом</span> - 4, <span class="val_5">судебно-медицинским экспертом</span> - 5. <div class="selector">{DeathSetType_id}</div></td></tr>
<tr><td>18.</td><td> Я, врач (фельдшер, акушерка) {MedPersonal_FIO} </br> должность {MedPersonal_Post}</br>																																																		
	удостоверяю, что на основании: <span class="val_1">осмотра трупа</span> - 1, <span class="val_2">записей в медицинской документации</span> - 2, <span class="val_3">предшествующего наблюдения за больным</span> - 3, <span class="val_4">вскрытия</span> - 4 мною определена последовательность патологических процессов (состояний), приведших к смерти,
	и установлена следующая причина	<div class="selector">{DeathSetCause_id}</div>
</td></tr>
<tr><td colspan="2">
	<table class="ct ctleft">
		<tr><td class="small"></br>19.</td><td colspan="2"> смерти:</br>Причины смерти:</td><td colspan="3" class="tcent" style="border-left-style: dashed; border-right-style: dashed;">Приблизительный</br>период времени</br>между началом</br>патологического</br>процесса и смерти</td><td rowspan="9" class="small">&nbsp;</td><td colspan="5"><br><br>Код по <br> МКБ-10</td></tr>
		<tr><td rowspan="8">I.</td><td rowspan="2" class="small">а)</td><td class="dashed">{Diag1_Name}</td><td rowspan="8" class="small" style="border-left-style: dashed;">&nbsp;</td><td class="dashed tcent">&nbsp;</td><td rowspan="8" class="small" style="border-right-style: dashed;">&nbsp;</td><td class="diat">{D11}</td><td class="diat">{D12}</td><td class="diat">{D13}</td><td style="text-align: center">{D14}</td><td class="diat">{D15}</td></tr>
		<tr><td class="tcent">(болезнь или состояние непосредственно приведшее к смерти)</td><td>&nbsp;</td><td colspan="5">&nbsp;</td></tr>			
		<tr><td rowspan="2">б)</td><td class="dashed">{Diag2_Name}</td><td class="dashed tcent">&nbsp;</td><td class="diat">{D21}</td><td class="diat">{D22}</td><td class="diat">{D23}</td><td>{D24}</td><td class="diat">{D25}</td></tr>
		<tr><td class="tcent">(патологическое состояние, которое привело к возникновению вышеуказанной причины)</td><td>&nbsp;</td><td colspan="5">&nbsp;</td></tr>
		...
		<tr><td rowspan="2">г)</td><td class="dashed">{Diag4_Name}</td><td class="dashed tcent">&nbsp;</td><td class="diat">{D41}</td><td class="diat">{D42}</td><td class="diat">{D43}</td><td>{D44}</td><td class="diat">{D45}</td></tr>
		<tr><td class="tcent">(внешняя причина при травмах и отравлениях)</td><td>&nbsp;</td><td colspan="5">&nbsp;</td></tr>
		<tr><td rowspan="3">II.</td><td colspan="2">Прочие важные состояния, способствовавшие смерти, но не связанные с болезнью или патологическим состоянием,приведшим к ней, включая употребление алкоголя, наркотических средств, психотропных и других токсических веществ, содержание их в крови, а  так же операции (название, дата)</td><td colspan="3" class="tcent" style="border-left-style: dashed; border-right-style: dashed;"></td></tr>
		<tr><td></td><td colspan="4" class="dashed">{Diag5_Name}</td><td colspan="1" class="tcent" style="border-left-style: dashed; border-right-style: dashed;">&nbsp;</td><td class="diat">{D51}</td><td class="diat">{D52}</td><td class="diat">{D53}</td><td>{D54}</td><td class="diat">{D55}</td></tr>
		<tr><td colspan="11">{DeathSvid_Oper}</td></tr>
	</table>																																																	
</td></tr>																																																		
