...
<tr class="list-item" onmouseover="if (isMouseLeaveOrEnter(event, this)) document.getElementById('timetable_{timetable}_{timetable_id}_toolbar').style.display='block'" onmouseout="if (isMouseLeaveOrEnter(event, this)) document.getElementById('timetable_{timetable}_{timetable_id}_toolbar').style.display='none'">
	...
	<td class="content">
		...
		if (!empty($StatusFromLpu) && in_array($EvnStatus_id, array(12,13))) {
			?>/ <span style="color: red;">{StatusFromLpu}</span> <?php
		}
		if (!empty($StatusFromMP) && in_array($EvnStatus_id, array(12,13))) {
			?>/ <span style="color: red;">{StatusFromMP}</span> <?php
		}
		...
		<div><?php
            ...
            <div id="EvnDirectionStac_{EvnDirection_id}" style="margin: 0 0 0 0;">
                <span id="EvnDirectionStac_{EvnDirection_id}_addDoc" class="link"
            ...
            if (getRegionNick() == 'msk' && isCurrentARMType("remoteconsultcenter") && empty($RCP_FilePath)): ?>
				<span id="EvnDirectionStac_{EvnDirection_id}_uploadFile" class="link"
	...
	<td class="toolbar">
		<div id="timetable_{timetable}_{timetable_id}_toolbar" class="toolbar">
			...
			<a id="timetable_{timetable}_{timetable_id}_delete" class="button <?php if ( ! $allowCancel ): ?> disabled <?php endif; ?> icon icon-delete16" title="Отменить"><span></span></a>
			<?php
