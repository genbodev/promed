...
<?php
	if (!$data['readOnly']) {
		...
		foreach ($data['header'] as $day => $header) {
			...
			<a href=# onclick=\"Ext.getCmp('{$data['PanelID']}').openDayListTTMS('".date('d.m.Y', DayMinuteToTime($day, 0))."',{UslugaComplexMedService_id: {$data['msData']['UslugaComplexMedService_id']},UslugaComplex_id: {$data['msData']['UslugaComplex_id']},UslugaComplex_Name: '{$data['msData']['UslugaComplex_Name']}', MedService_id: {$data['msData']['MedService_id']}, MedService_Name: '{$MedService_Name}', Lpu_id: {$data['msData']['Lpu_id']}, Lpu_Nick: '{$Lpu_Nick}', LpuUnit_id: {$LpuUnit_id}, LpuUnit_Name: '{$LpuUnit_Name}'}); return false;\">Список записанных</a><hr>";
