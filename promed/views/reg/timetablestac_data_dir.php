<?php
// Вывод расписания стационара для выписки направлений. Основная часть с бирками
$i = 0;
while( !EmptyArrays( $data['data'] ) && $i <= 500 ) { // $i <= 500 - защита от зацикливания
	...
	for( $nCol = 0; $nCol < 21; $nCol++ ) {
		...
		if ( count( $data['data'][$nDay] ) > 0 ) {
			...
			$Time = new TTimetableStac($arFields, $data['reserved'], $data );
			
			// Ячейка для выписки направления
			$Time->PrintCellForDirection();
